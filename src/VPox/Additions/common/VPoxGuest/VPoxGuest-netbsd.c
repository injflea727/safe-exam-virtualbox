/* $Id: VPoxGuest-netbsd.c $ */
/** @file
 * VirtualPox Guest Additions Driver for NetBSD.
 */
...
*********************************************************************************************************************************/
/** The module name. */
#define DEVICE_NAME  "vpoxguest"


/*********************************************************************************************************************************
*   Structures and Typedefs                                                                                                      *
*********************************************************************************************************************************/
typedef struct VPoxGuestDeviceState
{
    device_t sc_dev;
    ...
    u_int              vpoxguest_state;

    device_t sc_wsmousedev;
    VMMDevReqMouseStatus *sc_vmmmousereq;
    PVPOXGUESTSESSION sc_session;
    struct tpcalib_softc sc_tpcalib;
} vpoxguest_softc;


struct vpoxguest_fdata
{
    vpoxguest_softc *sc;
    PVPOXGUESTSESSION session;
};

#define VPOXGUEST_STATE_INITOK 1 << 0
 ...
 */
static int VPoxGuestNetBSDMatch(device_t parent, cfdata_t match, void *aux);
static void VPoxGuestNetBSDAttach(device_t parent, device_t self, void *aux);
static void VPoxGuestNetBSDWsmAttach(vpoxguest_softc *sc);
static int VPoxGuestNetBSDDetach(device_t self, int flags);

/*
 * IRQ related functions.
 */
static int  VPoxGuestNetBSDAddIRQ(vpoxguest_softc *sc, struct pci_attach_args *pa);
static void VPoxGuestNetBSDRemoveIRQ(vpoxguest_softc *sc);
static int  VPoxGuestNetBSDISR(void *pvState);

/*
 * Character device file handlers.
 */
static int VPoxGuestNetBSDOpen(dev_t device, int flags, int fmt, struct lwp *process);
static int VPoxGuestNetBSDClose(struct file *fp);
static int VPoxGuestNetBSDIOCtl(struct file *fp, u_long cmd, void *addr);
static int VPoxGuestNetBSDIOCtlSlow(struct vpoxguest_fdata *fdata, u_long command, void *data);
static int VPoxGuestNetBSDPoll(struct file *fp, int events);

/*
 * wsmouse(4) accessops
 */
static int VPoxGuestNetBSDWsmEnable(void *cookie);
static void VPoxGuestNetBSDWsmDisable(void *cookie);
static int VPoxGuestNetBSDWsmIOCtl(void *cookie, u_long cmd, void *data, int flag, struct lwp *l);

static int VPoxGuestNetBSDSetMouseStatus(vpoxguest_softc *sc, uint32_t fStatus);


/*********************************************************************************************************************************
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
extern struct cfdriver vpoxguest_cd; /* CFDRIVER_DECL */
extern struct cfattach vpoxguest_ca; /* CFATTACH_DECL */

/*
 * The /dev/vpoxguest character device entry points.
 */
static struct cdevsw g_VPoxGuestNetBSDChrDevSW =
{
    VPoxGuestNetBSDOpen,
    noclose,
    noread,
...
static const struct fileops vpoxguest_fileops = {
    .fo_read = fbadop_read,
    .fo_write = fbadop_write,
    .fo_ioctl = VPoxGuestNetBSDIOCtl,
    .fo_fcntl = fnullop_fcntl,
    .fo_poll = VPoxGuestNetBSDPoll,
    .fo_stat = fbadop_stat,
    .fo_close = VPoxGuestNetBSDClose,
    .fo_kqfilter = fnullop_kqfilter,
    .fo_restart = fnullop_restart
};


const struct wsmouse_accessops vpoxguest_wsm_accessops = {
    VPoxGuestNetBSDWsmEnable,
    VPoxGuestNetBSDWsmIOCtl,
    VPoxGuestNetBSDWsmDisable,
};


static struct wsmouse_calibcoords vpoxguest_wsm_default_calib = {
...
CFATTACH_DECL_NEW(vpoxguest, sizeof(vpoxguest_softc),
    VPoxGuestNetBSDMatch, VPoxGuestNetBSDAttach, VPoxGuestNetBSDDetach, NULL);
...
static void VPoxGuestNetBSDAttach(device_t parent, device_t self, void *aux)
{
    ...
    aprint_normal(": VirtualPox Guest\n");
    ...
    if (ioh_valid)
    {
        ...
        if (memh_valid)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                 ...
                 * Add IRQ of VMMDev.
                 */
                rc = VPoxGuestNetBSDAddIRQ(sc, pa);
                if (RT_SUCCESS(rc))
                {
                    sc->vpoxguest_state |= VPOXGUEST_STATE_INITOK;
...
static int VPoxGuestNetBSDAddIRQ(vpoxguest_softc *sc, struct pci_attach_args *pa)
{
    ...
    sc->pfnIrqHandler = pci_intr_establish(sc->sc_pc, sc->ih, IPL_BIO, VPoxGuestNetBSDISR, sc);
...
static void VPoxGuestNetBSDWsmAttach(vpoxguest_softc *sc)
{
    struct wsmousedev_attach_args am = { &vpoxguest_wsm_accessops, sc };

    PVPOXGUESTSESSION session = NULL;
    ...
    tpcalib_ioctl(&sc->sc_tpcalib, WSMOUSEIO_SCALIBCOORDS,
                  &vpoxguest_wsm_default_calib, 0, 0);
    return;
...
static int VPoxGuestNetBSDDetach(device_t self, int flags)
{
    ...
    if ((sc->vpoxguest_state & VPOXGUEST_STATE_INITOK) == 0)
        return 0;

    /*
     * Reverse what we did in VPoxGuestNetBSDAttach.
     */
    if (sc->sc_vmmmousereq != NULL)
        VbglR0GRFree((VMMDevRequestHeader *)sc->sc_vmmmousereq);

    VPoxGuestNetBSDRemoveIRQ(sc);
...
static int
VPoxGuestNetBSDWsmEnable(void *cookie)
{
    vpoxguest_softc *sc = cookie;
    int rc;

    rc = VPoxGuestNetBSDSetMouseStatus(sc, VMMDEV_MOUSE_GUEST_CAN_ABSOLUTE
...
static void
VPoxGuestNetBSDWsmDisable(void *cookie)
{
    vpoxguest_softc *sc = cookie;
    VPoxGuestNetBSDSetMouseStatus(sc, 0);
}


static int
VPoxGuestNetBSDWsmIOCtl(void *cookie, u_long cmd, void *data, int flag, struct lwp *l)
{
    vpoxguest_softc *sc = cookie;
 ...
 *
 */
static int VPoxGuestNetBSDOpen(dev_t device, int flags, int fmt, struct lwp *pLwp)
{
    vpoxguest_softc *sc;
    struct vpoxguest_fdata *fdata;
    file_t *fp;
    int fd, error;

    LogFlow((DEVICE_NAME ": %s\n", __func__));

    if ((sc = device_lookup_private(&vpoxguest_cd, minor(device))) == NULL)
    {
    ...
    if (fdata != NULL)
    {
        ...
        if (error == 0)
        {
...
#if 0       /** @todo implement /dev/vpoxuser */
            if (!fUnrestricted)
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                return fd_clone(fp, fd, flags, &vpoxguest_fileops, fdata);
            }

            aprint_error_dev(sc->sc_dev, "VPox session creation failed\n");
            closef(fp); /* ??? */
...
static int VPoxGuestNetBSDClose(struct file *fp)
{
    struct vpoxguest_fdata *fdata = fp->f_data;
    vpoxguest_softc *sc = fdata->sc;
 ...
 *
 */
static int VPoxGuestNetBSDIOCtl(struct file *fp, u_long command, void *data)
{
    struct vpoxguest_fdata *fdata = fp->f_data;
    ...
    return VPoxGuestNetBSDIOCtlSlow(fdata, command, data);
}
...
MODULE(MODULE_CLASS_DRIVER, vpoxguest, "pci");

/*
 * XXX: See netbsd/vpoxguest.ioconf for the details.
*/
#if 0
#include "ioconf.c"
#else
...
/* device vpoxguest: wsmousedev */
static const struct cfiattrdata * const vpoxguest_attrs[] = { &wsmousedevcf_iattrdata, NULL };
CFDRIVER_DECL(vpoxguest, DV_DULL, vpoxguest_attrs);

static struct cfdriver * const cfdriver_ioconf_vpoxguest[] = {
    &vpoxguest_cd, NULL
};


static const struct cfparent vpoxguest_pspec = {
    "pci", "pci", DVUNIT_ANY
};
static int vpoxguest_loc[] = { -1, -1 };


static const struct cfparent wsmousedev_pspec = {
    "wsmousedev", "vpoxguest", DVUNIT_ANY
};
static int wsmousedev_loc[] = { 0 };


static struct cfdata cfdata_ioconf_vpoxguest[] = {
    /*  vpoxguest0 at pci? dev ? function ? */
    {
        .cf_name = "vpoxguest",
        .cf_atname = "vpoxguest",
        .cf_unit = 0,           /* Only unit 0 is ever used  */
        .cf_fstate = FSTATE_NOTFOUND,
        .cf_loc = vpoxguest_loc,
        .cf_flags = 0,
        .cf_pspec = &vpoxguest_pspec,
    },

    /* wsmouse* at vpoxguest? */
    { "wsmouse", "wsmouse", 0, FSTATE_STAR, wsmousedev_loc, 0, &wsmousedev_pspec },

    { NULL, NULL, 0, 0, NULL, 0, NULL }
};

static struct cfattach * const vpoxguest_cfattachinit[] = {
    &vpoxguest_ca, NULL
};

static const struct cfattachinit cfattach_ioconf_vpoxguest[] = {
    { "vpoxguest", vpoxguest_cfattachinit },
    { NULL, NULL }
};
#endif
...
static int
vpoxguest_modcmd(modcmd_t cmd, void *opaque)
{
    ...
    switch (cmd)
    {
        case MODULE_CMD_INIT:
            error = config_init_component(cfdriver_ioconf_vpoxguest,
                                          cfattach_ioconf_vpoxguest,
                                          cfdata_ioconf_vpoxguest);
            if (error)
                break;

            bmajor = cmajor = NODEVMAJOR;
            error = devsw_attach("vpoxguest",
                                 NULL, &bmajor,
                                 &g_VPoxGuestNetBSDChrDevSW, &cmajor);
            if (error)
            {
                if (error == EEXIST)
            ...
            error = do_sys_mknod(curlwp, "/dev/vpoxguest",
        ...
        case MODULE_CMD_FINI:
            error = config_fini_component(cfdriver_ioconf_vpoxguest,
                                          cfattach_ioconf_vpoxguest,
                                          cfdata_ioconf_vpoxguest);
            if (error)
                break;

            devsw_detach(NULL, &g_VPoxGuestNetBSDChrDevSW);
            break;
