...
#ifndef VPOX_INCLUDED_vmm_pdmdev_h
#define VPOX_INCLUDED_vmm_pdmdev_h
...
#include <VPox/vmm/pdmins.h>
#include <VPox/vmm/pdmcommon.h>
...
#include <VPox/err.h>  /* VINF_EM_DBG_STOP, also 120+ source files expecting this. */
 ...
 *
 * This structure is used when registering a device from VPoxInitDevices() in HC
...
typedef struct PDMPCIBUSREGR3
{
     ...
     *
     * @returns Strict VPox status code (mainly DBGFSTOP).
     * @param   pDevIns         Device instance of the PCI Bus.
     ...
     */
    DECLR3CALLBACKMEMBER(VPOXSTRICTRC, pfnConfigWrite,(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev,
     ...
     *          that the (root) bus will have done that already.
     */
    DECLR3CALLBACKMEMBER(VPOXSTRICTRC, pfnConfigRead,(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev,
...
typedef struct PDMIOAPICREG
{
     ...
     *
     * @returns Strict VPox status code - only the following informational status codes:
     ...
     *          Actually, as per 2018-07-21 this isn't true (bird).
     */
    DECLCALLBACKMEMBER(VPOXSTRICTRC, pfnSetEoi)(PPDMDEVINS pDevIns, uint8_t u8Vector);
...
#define PDM_DEVHLPR3_VERSION                    PDM_VERSION_MAKE_PP(0xffe7, 42, 1)
...
typedef struct PDMDEVHLPR3
{
    ...
    DECLR3CALLBACKMEMBER(VPOXSTRICTRC, pfnTimerLockClock,(PPDMDEVINS pDevIns, TMTIMERHANDLE hTimer, int rcBusy));
    /** Takes the clock lock then enters the specified critical section. */
    DECLR3CALLBACKMEMBER(VPOXSTRICTRC, pfnTimerLockClock2,(PPDMDEVINS pDevIns, TMTIMERHANDLE hTimer, PPDMCRITSECT pCritSect, int rcBusy));
     ...
     *
     * @returns VPox status code which must be passed up to the VMM.
     * @param   pDevIns             The device instance.
     ...
     *
     * This is used by the macros found in VPox/vmm/dbgftrace.h and is not
     ...
     * @param   u32Value            The value to write.
     */
    DECLR3CALLBACKMEMBER(VPOXSTRICTRC, pfnPCIConfigWrite,(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev,
                                                          uint32_t uAddress, unsigned cb, uint32_t u32Value));
     ...
     * @param   pu32Value           Where to return the value.
     */
    DECLR3CALLBACKMEMBER(VPOXSTRICTRC, pfnPCIConfigRead,(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev,
                                                         uint32_t uAddress, unsigned cb, uint32_t *pu32Value));
     ...
     * Register a PCI Bus.
     *
     * @returns VPox status code, but the positive values 0..31 are used to indicate
     ...
     * Resets the VM.
     *
     * @returns The appropriate VPox status code to pass around on reset.
     * @param   pDevIns             The device instance.
     ...
     * Suspends the VM.
     *
     * @returns The appropriate VPox status code to pass around on suspend.
     * @param   pDevIns             The device instance.
     * @thread  The emulation thread.
     */
     ...
     *
     * @returns The appropriate VPox status code to pass around.
     * @param   pDevIns             The device instance.
     ...
     * Power off the VM.
     *
     * @returns The appropriate VPox status code to pass around on power off.
     * @param   pDevIns             The device instance.
     * @thread  The emulation thread.
     */
     ...
     *
     * @note    It is strictly forbidden to call this internally in VPox!  This
...
typedef struct PDMDEVHLPR0
{
    ...
    DECLR0CALLBACKMEMBER(VPOXSTRICTRC, pfnTimerLockClock,(PPDMDEVINS pDevIns, TMTIMERHANDLE hTimer, int rcBusy));
    /** Takes the clock lock then enters the specified critical section. */
    DECLR0CALLBACKMEMBER(VPOXSTRICTRC, pfnTimerLockClock2,(PPDMDEVINS pDevIns, TMTIMERHANDLE hTimer, PPDMCRITSECT pCritSect, int rcBusy));
 ...
 */
DECLINLINE(VPOXSTRICTRC) PDMDevHlpTimerLockClock(PPDMDEVINS pDevIns, TMTIMERHANDLE hTimer, int rcBusy)
{
 ...
 */
DECLINLINE(VPOXSTRICTRC) PDMDevHlpTimerLockClock2(PPDMDEVINS pDevIns, TMTIMERHANDLE hTimer, PPDMCRITSECT pCritSect, int rcBusy)
{
...
/**
 * VPOX_STRICT wrapper for pHlp->pfnDBGFStopV.
 *
 * @returns VPox status code which must be passed up to the VMM.  This will be
 ...
 */
DECLINLINE(VPOXSTRICTRC) PDMDevHlpPCIConfigRead(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev, uint32_t uAddress,
 ...
 */
DECLINLINE(VPOXSTRICTRC) PDMDevHlpPCIConfigWrite(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev, uint32_t uAddress,
...
/** Pointer to callbacks provided to the VPoxDeviceRegister() call. */
typedef struct PDMDEVREGCB *PPDMDEVREGCB;

/**
 * Callbacks for VPoxDeviceRegister().
 */
...
/**
 * The VPoxDevicesRegister callback function.
 *
 ...
 */
typedef DECLCALLBACK(int) FNPDMVPOXDEVICESREGISTER(PPDMDEVREGCB pCallbacks, uint32_t u32Version);

/** @} */

RT_C_DECLS_END

#endif /* !VPOX_INCLUDED_vmm_pdmdev_h */
