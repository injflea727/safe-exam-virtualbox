...
"""
VirtualPox Validation Kit - Create VM with IMachine::applyDefaults() Test
"""
...
class SubTstDrvCreateVMWithDefaults1(base.SubTestDriverBase):
    ...
    def createVMWithDefaults(self, sGuestType):
        ...
        try:
            if self.oTstDrv.fpApiVer >= 4.2: # Introduces grouping (third parameter, empty for now).
                oVM = self.oTstDrv.oVPox.createMachine("", sName, [],
        ...
        try:
            if self.oTstDrv.fpApiVer >= 6.1:
                oVM.applyDefaults('')
                oVM.saveSettings();
            self.oTstDrv.oVPox.registerMachine(oVM)
        except:
            reporter.logXcpt()
            fRc = False
    ...
    def testCreateVMWithDefaults(self):
        ...
        if not self.oTstDrv.importVPoxApi():
            return reporter.error('importVPoxApi');

        # Get the guest OS types.
        try:
            aoGuestTypes = self.oTstDrv.oVPoxMgr.getArray(self.oTstDrv.oVPox, 'guestOSTypes')
