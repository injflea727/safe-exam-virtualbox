/* $Id: */
/** @file
 * VPoxGuestR3Lib - Ring-3 Support Library for VirtualPox guest additions, host version check.
 */
...
VBGLR3DECL(int) VbglR3HostVersionCheckForUpdate(HGCMCLIENTID idClient, bool *pfUpdate, char **ppszHostVersion, char **ppszGuestVersion)
{
    ...
    *pfUpdate = true;
    ...
    int rc = VbglR3GuestPropReadValueAlloc(idClient, "/VirtualPox/GuestAdd/CheckHostVersion", &pszCheckHostVersion);
    if (RT_FAILURE(rc))
    {
    ...
    if (RT_SUCCESS(rc) && *pfUpdate)
    {
        ...
        rc = VbglR3GuestPropReadValueAlloc(idClient, "/VirtualPox/HostInfo/VPoxVer", ppszHostVersion);
        if (RT_FAILURE(rc))
        {
            LogFlow(("Could not read VPox host version! rc = %Rrc\n", rc));
        }
        else
        {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_FAILURE(rc))
                LogFlow(("Could not read VPox guest version! rc = %Rrc\n", rc));
        }
    }
...
VBGLR3DECL(int) VbglR3HostVersionLastCheckedLoad(HGCMCLIENTID idClient, char **ppszVer)
{
    Assert(idClient > 0);
    AssertPtr(ppszVer);
    return VbglR3GuestPropReadValueAlloc(idClient, "/VirtualPox/GuestAdd/HostVerLastChecked", ppszVer);
}
...
VBGLR3DECL(int) VbglR3HostVersionLastCheckedStore(HGCMCLIENTID idClient, const char *pszVer)
{
    Assert(idClient > 0);
    AssertPtr(pszVer);
    return VbglR3GuestPropWriteValue(idClient, "/VirtualPox/GuestAdd/HostVerLastChecked", pszVer);
}
