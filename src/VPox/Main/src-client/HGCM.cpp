...
#define VPOX_HGCM_SVC_NAME_MAX_BYTES 1024
...
{
    private:
        VPOXHGCMSVCHELPERS m_svcHelpers;
        ...
        RTLDRMOD m_hLdrMod;
        PFNVPOXHGCMSVCLOAD m_pfnLoad;

        VPOXHGCMSVCFNTABLE m_fntable;
        ...
        static DECLCALLBACK(int)  svcHlpCallComplete(VPOXHGCMCALLHANDLE callHandle, int32_t rc);
        static DECLCALLBACK(int)  svcHlpDisconnectClient(void *pvInstance, uint32_t idClient);
        static DECLCALLBACK(bool) svcHlpIsCallRestored(VPOXHGCMCALLHANDLE callHandle);
        static DECLCALLBACK(bool) svcHlpIsCallCancelled(VPOXHGCMCALLHANDLE callHandle);
        ...
        static DECLCALLBACK(uint32_t) svcHlpGetRequestor(VPOXHGCMCALLHANDLE hCall);
    ...
    public:
        ...
        int HostCall(uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM *paParms);
        ...
        int GuestCall(PPDMIHGCMPORT pHGCMPort, PVPOXHGCMCMD pCmd, uint32_t u32ClientId, HGCMClient *pClient,
                      uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM aParms[], uint64_t tsArrival);
        void GuestCancelled(PPDMIHGCMPORT pHGCMPort, PVPOXHGCMCMD pCmd, uint32_t idClient);
};
...
int HGCMService::loadServiceDLL(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            m_fntable.u32Version = VPOX_HGCM_SVC_VERSION;
...
#define SVC_MSG_LOAD            (0)  /**< Load the service library and call VPOX_HGCM_SVCLOAD_NAME entry point. */
    ...
    while (!fQuit)
    {
        ...
        switch (u32MsgId)
        {
            ...
            case SVC_MSG_GUESTCALL:
            {
                ...
                if (pClient)
                {
                    pSvc->m_fntable.pfnCall(pSvc->m_fntable.pvService, (VPOXHGCMCALLHANDLE)pMsg, pMsg->u32ClientId,
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnCallComplete}
 */
/* static */ DECLCALLBACK(int) HGCMService::svcHlpCallComplete(VPOXHGCMCALLHANDLE callHandle, int32_t rc)
{
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnDisconnectClient}
 */
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnIsCallRestored}
 */
/* static */ DECLCALLBACK(bool) HGCMService::svcHlpIsCallRestored(VPOXHGCMCALLHANDLE callHandle)
{
    HGCMMsgHeader *pMsgHdr = (HGCMMsgHeader *)callHandle;
    AssertPtrReturn(pMsgHdr, false);

    PVPOXHGCMCMD pCmd = pMsgHdr->pCmd;
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnIsCallCancelled}
 */
/* static */ DECLCALLBACK(bool) HGCMService::svcHlpIsCallCancelled(VPOXHGCMCALLHANDLE callHandle)
{
    HGCMMsgHeader *pMsgHdr = (HGCMMsgHeader *)callHandle;
    AssertPtrReturn(pMsgHdr, false);
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnStamRegisterV}
 */
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnStamDeregisterV}
 */
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnInfoRegister}
 */
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnInfoDeregister}
 */
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnGetRequestor}
 */
/* static */ DECLCALLBACK(uint32_t) HGCMService::svcHlpGetRequestor(VPOXHGCMCALLHANDLE hCall)
{
...
/**
 * @interface_method_impl{VPOXHGCMSVCHELPERS,pfnGetVMMDevSessionId}
 */
...
int HGCMService::instanceCreate(const char *pszServiceLibrary, const char *pszServiceName, PUVM pUVM, PPDMIHGCMPORT pHgcmPort)
{
    ...
    if (!strncmp(pszServiceName, RT_STR_TUPLE("VPoxShared")))
        RTStrPrintf(szThreadName, sizeof(szThreadName), "Sh%s", pszServiceName + 10);
    else if (!strncmp(pszServiceName, RT_STR_TUPLE("VPox")))
...
/* static */ int HGCMService::LoadState(PSSMHANDLE pSSM, uint32_t uVersion)
{
    ...
    while (cServices--)
    {
        ...
        AssertRCReturn(rc, rc);
        AssertReturn(u32 <= VPOX_HGCM_SVC_NAME_MAX_BYTES, VERR_SSM_UNEXPECTED_DATA);

        /* Get the service name. */
        char szServiceName[VPOX_HGCM_SVC_NAME_MAX_BYTES];
 ...
 * @param pCmd           The VPox HGCM context.
 ...
 */
int HGCMService::GuestCall(PPDMIHGCMPORT pHGCMPort, PVPOXHGCMCMD pCmd, uint32_t u32ClientId, HGCMClient *pClient,
 ...
 * @return  VPox rc.
 */
void HGCMService::GuestCancelled(PPDMIHGCMPORT pHGCMPort, PVPOXHGCMCMD pCmd, uint32_t idClient)
{
 ...
 * @return VPox rc.
 */
int HGCMService::HostCall(uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM *paParms)
{
 ...
 */
void HGCMGuestCancelled(PPDMIHGCMPORT pHGCMPort, PVPOXHGCMCMD pCmd, uint32_t idClient)
{
