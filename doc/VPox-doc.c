/* $Id: VPox-doc.c $ */
/** @file
 * VirtualPox Top Level Documentation File.
 */
 ...
 *      - VPoxDev - Special PCI Device which serves as an interface between
 ...
 *          - DrvVDI - VirtualPox Drive Image Container Driver.
 ...
 *          - @ref pg_dev_vusb_old
 *  - Host Drivers.
 *      - SUPDRV - The Support driver (aka VPoxDrv).
 *          - @ref pg_sup
 *      - @ref pg_netflt
 *      - @ref pg_netadp
 *      - VPoxUSB - The USB support driver.
 *      - @ref pg_netflt
 *      - @ref pg_rawpci
 *  - Host Services.
 ...
 *      - @ref pg_vrdb_usb
 *  - Frontends:
 *      - VirtualPox - The default Qt-based GUI.
 *      - VPoxHeadless - The headless frontend.
 *      - VPoxManage - The CLI.
 *      - VPoxShell - An interactive shell written in python.
 *      - VPoxSDL - A very simple GUI.
 *      - VPoxBFE - A bare metal edition which does not use COM/XPCOM (barely
 *        maintained atm).
 *  - IPRT - Runtime Library for hiding host OS differences.
 *  - Validation Kit:
 *      - @ref pg_validationkit_guideline
 *      - @ref pg_bs3kit
 *  - @ref pg_vpox_guideline
 *
 ...
 *
 * VirtualPox defines a number of different execution context, this can be
 ...
 *        runs VirtualPox within it.  The absense of IN_RC and IN_GUEST
