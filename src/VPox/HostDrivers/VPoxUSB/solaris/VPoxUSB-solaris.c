/* $Id: VPoxUSB-solaris.c $ */
/** @file
 * VirtualPox USB Client Driver, Solaris Hosts.
 */
...
*********************************************************************************************************************************/
/** The module name. */
#define DEVICE_NAME                                     "vpoxusb"
/** The module description as seen in 'modinfo'. */
#define DEVICE_DESC_DRV                                 "VirtualPox USB"

/** -=-=-=-=-=-=- Standard Specifics -=-=-=-=-=-=- */
/** Max. supported endpoints. */
#define VPOXUSB_MAX_ENDPOINTS                           32
/** Size of USB Ctrl Xfer Header in bytes. */
#define VPOXUSB_CTRL_XFER_SIZE                          8
/**
 ...
 */
#define VPOXUSB_PKT_SIZE(pkt)                          (pkt & 0x07FF) * (1 + ((pkt >> 11) & 3))
/** Endpoint Xfer Type. */
#define VPOXUSB_XFER_TYPE(endp)                        ((endp)->EpDesc.bmAttributes & USB_EP_ATTR_MASK)
/** Endpoint Xfer Direction. */
#define VPOXUSB_XFER_DIR(endp)                         ((endp)->EpDesc.bEndpointAddress & USB_EP_DIR_IN)
/** Create an Endpoint index from an Endpoint address. */
#define VPOXUSB_GET_EP_INDEX(epaddr)                   (((epaddr) & USB_EP_NUM_MASK) + \
                                                       (((epaddr) & USB_EP_DIR_MASK) ? 16 : 0))


/** -=-=-=-=-=-=- Tunable Parameters -=-=-=-=-=-=- */
/** Time to wait while draining inflight UBRs on suspend, in seconds. */
#define VPOXUSB_DRAIN_TIME                              20
/** Ctrl Xfer timeout in seconds. */
#define VPOXUSB_CTRL_XFER_TIMEOUT                       15
/** Maximum URB queue length. */
#define VPOXUSB_URB_QUEUE_SIZE                         512
/** Maximum asynchronous requests per pipe. */
#define VPOXUSB_MAX_PIPE_ASYNC_REQS                     2
...
*   Kernel Entry Hooks                                                                                                           *
*********************************************************************************************************************************/
int VPoxUSBSolarisOpen(dev_t *pDev, int fFlag, int fType, cred_t *pCred);
int VPoxUSBSolarisClose(dev_t Dev, int fFlag, int fType, cred_t *pCred);
int VPoxUSBSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred);
int VPoxUSBSolarisWrite(dev_t Dev, struct uio *pUio, cred_t *pCred);
int VPoxUSBSolarisIOCtl(dev_t Dev, int Cmd, intptr_t pArg, int Mode, cred_t *pCred, int *pVal);
int VPoxUSBSolarisPoll(dev_t Dev, short fEvents, int fAnyYet, short *pReqEvents, struct pollhead **ppPollHead);
int VPoxUSBSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pArg, void **ppResult);
int VPoxUSBSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd);
int VPoxUSBSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd);
int VPoxUSBSolarisPower(dev_info_t *pDip, int Component, int Level);
 ...
 */
static struct cb_ops g_VPoxUSBSolarisCbOps =
{
    VPoxUSBSolarisOpen,
    VPoxUSBSolarisClose,
    nodev,                      /* b strategy */
 ...
 */
static struct dev_ops g_VPoxUSBSolarisDevOps =
{
    DEVO_REV,                   /* driver build revision */
    0,                          /* ref count */
    VPoxUSBSolarisGetInfo,
    nulldev,                    /* identify */
    nulldev,                    /* probe */
    VPoxUSBSolarisAttach,
    VPoxUSBSolarisDetach,
    nodev,                      /* reset */
    &g_VPoxUSBSolarisCbOps,
    NULL,                       /* bus ops */
    VPoxUSBSolarisPower,
 ...
 */
static struct modldrv g_VPoxUSBSolarisModule =
{
    &mod_driverops,             /* extern from kernel */
    DEVICE_DESC_DRV " " VPOX_VERSION_STRING "r" RT_XSTR(VPOX_SVN_REV),
    &g_VPoxUSBSolarisDevOps
};

/**
 * modlinkage: export install/remove/info to the kernel
 */
static struct modlinkage g_VPoxUSBSolarisModLinkage =
{
    MODREV_1,
    &g_VPoxUSBSolarisModule,
    NULL,
};

/**
 * vpoxusb_ep_t: Endpoint structure with info. for managing an endpoint.
 */
typedef struct vpoxusb_ep_t
{
...
/**
 * vpoxusb_isoc_req_t: Isoc IN. requests queued from device till they are reaped.
 */
typedef struct vpoxusb_isoc_req_t
{
...
/**
 * VPOXUSB_URB_STATE: Internal USB URB state.
 */
typedef enum VPOXUSB_URB_STATE
{
    VPOXUSB_URB_STATE_FREE     = 0x00,
    VPOXUSB_URB_STATE_INFLIGHT = 0x04,
    VPOXUSB_URB_STATE_LANDED   = 0x08
} VPOXUSB_URB_STATE;

/**
 * vpoxusb_urb_t: kernel URB representation.
 */
typedef struct vpoxusb_urb_t
{
    ...
    uint32_t                cIsocPkts;           /* Number of Isoc pkts */
    VUSBISOC_PKT_DESC       aIsocPkts[8];        /* Array of Isoc pkt descriptors */
    VPOXUSB_URB_STATE       enmState;            /* URB state (free/in-flight/landed). */
    struct vpoxusb_state_t *pState;              /* Pointer to the device instance */
    list_node_t             hListLink;           /* List node link handle */
} vpoxusb_urb_t;

/**
 * vpoxusb_power_t: Per Device Power Management info.
 */
typedef struct vpoxusb_power_t
{
...
/**
 * vpoxusb_state_t: Per Device instance state info.
 */
typedef struct vpoxusb_state_t
{
    ...
    vpoxusb_ep_t            aEps[VPOXUSB_MAX_ENDPOINTS]; /* Array of all endpoints structures */
    ...
    VPOXUSBREQ_CLIENT_INFO  ClientInfo;          /* Registration data */
    vpoxusb_power_t        *pPower;              /* Power Management */
    char                    szMfg[255];          /* Parsed manufacturer string */
    char                    szProduct[255];      /* Parsed product string */
} vpoxusb_state_t;
AssertCompileMemberSize(vpoxusb_state_t, szMfg,     USB_MAXSTRINGLEN);
AssertCompileMemberSize(vpoxusb_state_t, szProduct, USB_MAXSTRINGLEN);


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
LOCAL int    vpoxUsbSolarisInitEp(vpoxusb_state_t *pState, usb_ep_data_t *pEpData);
LOCAL int    vpoxUsbSolarisInitEpsForCfg(vpoxusb_state_t *pState);
LOCAL int    vpoxUsbSolarisInitEpsForIfAlt(vpoxusb_state_t *pState, uint8_t bIf, uint8_t bAlt);
LOCAL void   vpoxUsbSolarisDestroyAllEps(vpoxusb_state_t *pState);
LOCAL void   vpoxUsbSolarisDestroyEp(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp);
LOCAL void   vpoxUsbSolarisCloseAllPipes(vpoxusb_state_t *pState, bool fControlPipe);
LOCAL int    vpoxUsbSolarisOpenPipe(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp);
LOCAL void   vpoxUsbSolarisClosePipe(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp);
LOCAL int    vpoxUsbSolarisCtrlXfer(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp, vpoxusb_urb_t *pUrb);
LOCAL void   vpoxUsbSolarisCtrlXferCompleted(usb_pipe_handle_t pPipe, usb_ctrl_req_t *pReq);
LOCAL int    vpoxUsbSolarisBulkXfer(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp, vpoxusb_urb_t *purb);
LOCAL void   vpoxUsbSolarisBulkXferCompleted(usb_pipe_handle_t pPipe, usb_bulk_req_t *pReq);
LOCAL int    vpoxUsbSolarisIntrXfer(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp, vpoxusb_urb_t *pUrb);
LOCAL void   vpoxUsbSolarisIntrXferCompleted(usb_pipe_handle_t pPipe, usb_intr_req_t *pReq);
LOCAL int    vpoxUsbSolarisIsocXfer(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp, vpoxusb_urb_t *pUrb);
LOCAL void   vpoxUsbSolarisIsocInXferCompleted(usb_pipe_handle_t pPipe, usb_isoc_req_t *pReq);
LOCAL void   vpoxUsbSolarisIsocInXferError(usb_pipe_handle_t pPipe, usb_isoc_req_t *pReq);
LOCAL void   vpoxUsbSolarisIsocOutXferCompleted(usb_pipe_handle_t pPipe, usb_isoc_req_t *pReq);
LOCAL vpoxusb_urb_t  *vpoxUsbSolarisGetIsocInUrb(vpoxusb_state_t *pState, PVPOXUSBREQ_URB pUrbReq);
LOCAL vpoxusb_urb_t  *vpoxUsbSolarisQueueUrb(vpoxusb_state_t *pState, PVPOXUSBREQ_URB pUrbReq, mblk_t *pMsg);
LOCAL VUSBSTATUS      vpoxUsbSolarisGetUrbStatus(usb_cr_t Status);
LOCAL void   vpoxUsbSolarisConcatMsg(vpoxusb_urb_t *pUrb);
LOCAL void   vpoxUsbSolarisDeQueueUrb(vpoxusb_urb_t *pUrb, int URBStatus);
LOCAL void   vpoxUsbSolarisNotifyComplete(vpoxusb_state_t *pState);
LOCAL int    vpoxUsbSolarisProcessIOCtl(int iFunction, void *pvState, int Mode, PVPOXUSBREQ pUSBReq, void *pvBuf,
                                        size_t *pcbDataOut);
LOCAL bool   vpoxUsbSolarisIsUSBDevice(dev_info_t *pDip);

/** @name Device Operation Hooks
 * @{ */
LOCAL int    vpoxUsbSolarisSendUrb(vpoxusb_state_t *pState, PVPOXUSBREQ_URB pUrbReq, int Mode);
LOCAL int    vpoxUsbSolarisReapUrb(vpoxusb_state_t *pState, PVPOXUSBREQ_URB pUrbReq, int Mode);
LOCAL int    vpoxUsbSolarisClearEndPoint(vpoxusb_state_t *pState, uint8_t bEndpoint);
LOCAL int    vpoxUsbSolarisSetConfig(vpoxusb_state_t *pState, uint8_t bCfgValue);
LOCAL int    vpoxUsbSolarisGetConfig(vpoxusb_state_t *pState, uint8_t *pCfgValue);
LOCAL int    vpoxUsbSolarisSetInterface(vpoxusb_state_t *pState, uint8_t bIf, uint8_t bAlt);
LOCAL int    vpoxUsbSolarisCloseDevice(vpoxusb_state_t *pState, VPOXUSB_RESET_LEVEL enmReset);
LOCAL int    vpoxUsbSolarisAbortPipe(vpoxusb_state_t *pState, uint8_t bEndpoint);
LOCAL int    vpoxUsbSolarisGetConfigIndex(vpoxusb_state_t *pState, uint_t bCfgValue);
/** @} */

/** @name Hotplug & Power Management Hooks
 * @{ */
LOCAL void   vpoxUsbSolarisNotifyUnplug(vpoxusb_state_t *pState);
LOCAL int    vpoxUsbSolarisDeviceDisconnected(dev_info_t *pDip);
LOCAL int    vpoxUsbSolarisDeviceReconnected(dev_info_t *pDip);

LOCAL int    vpoxUsbSolarisInitPower(vpoxusb_state_t *pState);
LOCAL void   vpoxUsbSolarisDestroyPower(vpoxusb_state_t *pState);
LOCAL int    vpoxUsbSolarisDeviceSuspend(vpoxusb_state_t *pState);
LOCAL void   vpoxUsbSolarisDeviceResume(vpoxusb_state_t *pState);
LOCAL void   vpoxUsbSolarisDeviceRestore(vpoxusb_state_t *pState);
LOCAL void   vpoxUsbSolarisPowerBusy(vpoxusb_state_t *pState);
LOCAL void   vpoxUsbSolarisPowerIdle(vpoxusb_state_t *pState);
/** @} */
 ...
 * @{ */
LOCAL int    vpoxUsbSolarisSetConsumerCredentials(RTPROCESS Process, int Instance, void *pvReserved);
/** @} */
...
static void *g_pVPoxUSBSolarisState;

/** The default endpoint descriptor */
static usb_ep_descr_t g_VPoxUSBSolarisDefaultEpDesc = { 7, 5, 0, USB_EP_ATTR_CONTROL, 8, 0 };
 ...
 */
static const char *vpoxUsbSolarisEpType(vpoxusb_ep_t *pEp)
{
    uint8_t uType = VPOXUSB_XFER_TYPE(pEp);
    switch (uType)
    {
        case 0:  return "CTRL";
        case 1:  return "ISOC";
 ...
 */
static const char *vpoxUsbSolarisEpDir(vpoxusb_ep_t *pEp)
{
    return VPOXUSB_XFER_DIR(pEp) == USB_EP_DIR_IN ? "IN " : "OUT";
}
#endif
 ...
 */
static void vpoxUsbSolarisGetDeviceStrings(vpoxusb_state_t *pState)
{
 ...
 */
LOCAL int vpoxUsbSolarisQuerySymbols(void)
{
...
int _init(void)
{
     ...
     */
    modctl_t *pModCtl = mod_getctl(&g_VPoxUSBSolarisModLinkage);
    if (pModCtl)
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = ddi_soft_state_init(&g_pVPoxUSBSolarisState, sizeof(vpoxusb_state_t), 4 /* pre-alloc */);
        if (!rc)
        {
            rc = mod_install(&g_VPoxUSBSolarisModLinkage);
            if (!rc)
                return rc;

            LogRel((DEVICE_NAME ": _init: mod_install failed! rc=%d\n", rc));
            ddi_soft_state_fini(&g_pVPoxUSBSolarisState);
        }
        else
...
int _fini(void)
{
    ...
    rc = mod_remove(&g_VPoxUSBSolarisModLinkage);
    if (!rc)
    {
        ddi_soft_state_fini(&g_pVPoxUSBSolarisState);
        RTR0Term();
...
int _info(struct modinfo *pModInfo)
{
    ...
    return mod_info(&g_VPoxUSBSolarisModLinkage, pModInfo);
}
...
int VPoxUSBSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisAttach: pDip=%p enmCmd=%d\n", pDip, enmCmd));

    int rc;
    int instance = ddi_get_instance(pDip);
    vpoxusb_state_t *pState = NULL;

    switch (enmCmd)
    {
        case DDI_ATTACH:
        {
            rc = ddi_soft_state_zalloc(g_pVPoxUSBSolarisState, instance);
            if (rc == DDI_SUCCESS)
            {
                pState = ddi_get_soft_state(g_pVPoxUSBSolarisState, instance);
                if (RT_LIKELY(pState))
                {
                    pState->pDip             = pDip;
                    ...
                    list_create(&pState->hFreeUrbs, sizeof(vpoxusb_urb_t), offsetof(vpoxusb_urb_t, hListLink));
                    list_create(&pState->hInflightUrbs, sizeof(vpoxusb_urb_t), offsetof(vpoxusb_urb_t, hListLink));
                    list_create(&pState->hLandedUrbs, sizeof(vpoxusb_urb_t), offsetof(vpoxusb_urb_t, hListLink));
                     ...
                     */
                    if (vpoxUsbSolarisIsUSBDevice(pState->pDip))
                    {
                        /*
                        ...
                        if (rc == USB_SUCCESS)
                        {
                            ...
                            if (rc == USB_SUCCESS)
                            {
                                 ...
                                 */
                                vpoxUsbSolarisGetDeviceStrings(pState);
                                ...
                                if (rc == USB_SUCCESS)
                                {
                                    Log((DEVICE_NAME ": VPoxUSBSolarisAttach: cbMaxBulkXfer=%d\n", pState->cbMaxBulkXfer));

                                    /*
                                     * Initialize the default endpoint.
                                     */
                                    rc = vpoxUsbSolarisInitEp(pState, NULL /* pEp */);
                                    if (RT_SUCCESS(rc))
                                    {
                                        /*
                                         ...
                                         */
                                        rc = vpoxUsbSolarisInitPower(pState);
                                        if (RT_SUCCESS(rc))
                                        {
                                            /*
                                             * Initialize endpoints for the current config.
                                             */
                                            rc = vpoxUsbSolarisInitEpsForCfg(pState);
                                            AssertRC(rc);
                                            ...
                                            if (RT_LIKELY(rc == DDI_SUCCESS))
                                            {
                                                 ...
                                                 */
                                                rc = usb_register_hotplug_cbs(pState->pDip, &vpoxUsbSolarisDeviceDisconnected,
                                                                              &vpoxUsbSolarisDeviceReconnected);
                                                if (RT_LIKELY(rc == USB_SUCCESS))
                                                {
                                                    ...
                                                    pState->ClientInfo.pfnSetConsumerCredentials = &vpoxUsbSolarisSetConsumerCredentials;
                                                    rc = VPoxUSBMonSolarisRegisterClient(pState->pDip, &pState->ClientInfo);
                                                    if (RT_SUCCESS(rc))
                                                    {
#if 0
                                                    ...
                                                    LogRel((DEVICE_NAME ": VPoxUSBMonSolarisRegisterClient failed! rc=%d "
                                                    ...
                                                    LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: Failed to register hotplug callbacks! rc=%d\n", rc));

                                                ddi_remove_minor_node(pState->pDip, NULL);
                                            }
                                            else
                                                LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: ddi_create_minor_node failed! rc=%d\n", rc));

                                            mutex_enter(&pState->Mtx);
                                            vpoxUsbSolarisDestroyPower(pState);
                                            mutex_exit(&pState->Mtx);
                                        }
                                        else
                                            LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: Failed to init power management! rc=%d\n", rc));
                                    }
                                    else
                                        LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: vpoxUsbSolarisInitEp failed! rc=%d\n", rc));
                                }
                                else
                                    LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: usb_pipe_get_max_bulk_transfer_size failed! rc=%d\n", rc));
                                ...
                                LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: Failed to get device descriptor. rc=%d\n", rc));

                            usb_client_detach(pState->pDip, NULL);
                        }
                        else
                            LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: usb_client_attach failed! rc=%d\n", rc));
                    }
                    else
                    {
                        /* This would appear on every boot if it were LogRel() */
                        Log((DEVICE_NAME ": VPoxUSBSolarisAttach: Not a USB device\n"));
                    }
                }
                else
                    LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: Failed to get soft state\n", sizeof(*pState)));

                ddi_soft_state_free(g_pVPoxUSBSolarisState, instance);
            }
            else
                LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: Failed to alloc soft state. rc=%d\n", rc));
        ...
        case DDI_RESUME:
        {
            ...
            {
                LogRel((DEVICE_NAME ": VPoxUSBSolarisAttach: DDI_RESUME failed to get soft state on detach\n"));
                return DDI_FAILURE;
            }

            vpoxUsbSolarisDeviceResume(pState);
            return DDI_SUCCESS;
        }
...
int VPoxUSBSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisDetach: pDip=%p enmCmd=%d\n", pDip, enmCmd));

    int instance = ddi_get_instance(pDip);
    vpoxusb_state_t *pState = ddi_get_soft_state(g_pVPoxUSBSolarisState, instance);
    if (RT_UNLIKELY(!pState))
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisDetach: Failed to get soft state on detach\n"));
        return DDI_FAILURE;
    }
    ...
    switch (enmCmd)
    {
        case DDI_DETACH:
        {
             ...
             * already been closed by userland (via VPoxUSBSolarisClose() entry point).
            ...
            vpoxUsbSolarisNotifyUnplug(pState);
            mutex_exit(&pState->Mtx);
             ...
             * Close all pipes.
             */
            vpoxUsbSolarisCloseAllPipes(pState, true /* ControlPipe */);
             ...
             */
            vpoxUsbSolarisDestroyPower(pState);
            vpoxUsbSolarisDestroyAllEps(pState);

            /*
             * Free up all URB lists.
             */
            vpoxusb_urb_t *pUrb = NULL;
             ...
             */
            VPoxUSBMonSolarisUnregisterClient(pState->pDip);
        ...
        case DDI_SUSPEND:
        {
            int rc = vpoxUsbSolarisDeviceSuspend(pState);
            if (RT_SUCCESS(rc))
                return DDI_SUCCESS;
 ...
 * @returns Solaris error code.
 */
int VPoxUSBSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pvArg, void **ppvResult)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisGetInfo\n"));
        ...
        case DDI_INFO_DEVT2DEVINFO:
        {
            ...
            if (pState)
            {
                *ppvResult = (void *)pState->pDip;
                ...
                LogRel((DEVICE_NAME ": VPoxUSBSolarisGetInfo: Failed to get device state\n"));
            return DDI_FAILURE;
        }
...
/**
 * Callback invoked from the VirtualPox USB Monitor driver when a VM process
...
LOCAL int vpoxUsbSolarisSetConsumerCredentials(RTPROCESS Process, int Instance, void *pvReserved)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisSetConsumerCredentials: Process=%u Instance=%d\n", Process, Instance));
    vpoxusb_state_t *pState = ddi_get_soft_state(g_pVPoxUSBSolarisState, Instance);
    if (!pState)
    {
        LogRel((DEVICE_NAME ": vpoxUsbSolarisSetConsumerCredentials: Failed to get device state for instance %d\n", Instance));
    ...
    {
        LogRel((DEVICE_NAME ": vpoxUsbSolarisSetConsumerCredentials: Failed! Process %u already has client open\n",
                pState->Process));
...
int VPoxUSBSolarisOpen(dev_t *pDev, int fFlag, int fType, cred_t *pCred)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisOpen: pDev=%p fFlag=%d fType=%d pCred=%p\n", pDev, fFlag, fType, pCred));
    ...
    if (!pState)
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisOpen: Failed to get device state for instance %d\n", instance));
        return ENXIO;
    }
    ...
    {
        if (pState->Process == NIL_RTPROCESS)
            LogRel((DEVICE_NAME ": VPoxUSBSolarisOpen: No prior information about authorized process\n"));
        else
            LogRel((DEVICE_NAME ": VPoxUSBSolarisOpen: Process %u is already using this device instance\n", pState->Process));
...
int VPoxUSBSolarisClose(dev_t Dev, int fFlag, int fType, cred_t *pCred)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisClose: Dev=%d fFlag=%d fType=%d pCred=%p\n", Dev, fFlag, fType, pCred));

    int instance = getminor((dev_t)Dev);
    vpoxusb_state_t *pState = ddi_get_soft_state(g_pVPoxUSBSolarisState, instance);
    if (RT_UNLIKELY(!pState))
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisClose: Failed to get device state for instance %d\n", instance));
        return ENXIO;
    }
...
int VPoxUSBSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisRead\n"));
    return ENOTSUP;
}
...
int VPoxUSBSolarisWrite(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisWrite\n"));
    return ENOTSUP;
}
...
int VPoxUSBSolarisPoll(dev_t Dev, short fEvents, int fAnyYet, short *pReqEvents, struct pollhead **ppPollHead)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisPoll: Dev=%d fEvents=%d fAnyYet=%d pReqEvents=%p\n", Dev, fEvents, fAnyYet, pReqEvents));
    ...
    if (RT_UNLIKELY(!pState))
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisPoll: No state data for %d\n", instance));
        return ENXIO;
    }
...
int VPoxUSBSolarisPower(dev_info_t *pDip, int Component, int Level)
{
    LogFunc((DEVICE_NAME ": VPoxUSBSolarisPower: pDip=%p Component=%d Level=%d\n", pDip, Component, Level));

    int instance = ddi_get_instance(pDip);
    vpoxusb_state_t *pState = ddi_get_soft_state(g_pVPoxUSBSolarisState, instance);
    if (RT_UNLIKELY(!pState))
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisPower: Failed! State Gone\n"));
        return DDI_FAILURE;
    }
    ...
    if (pState->DevState == USB_DEV_ONLINE)
    {
        ...
        if (!USB_DEV_PWRSTATE_OK(pState->pPower->PowerStates, Level))
        {
            ...
            Log((DEVICE_NAME ": VPoxUSBSolarisPower: USB_DEV_PWRSTATE_OK failed\n"));
    }
    else
        rc = USB_SUCCESS;
...
int VPoxUSBSolarisIOCtl(dev_t Dev, int Cmd, intptr_t pArg, int Mode, cred_t *pCred, int *pVal)
{
    /* LogFunc((DEVICE_NAME ": VPoxUSBSolarisIOCtl: Dev=%d Cmd=%d pArg=%p Mode=%d\n", Dev, Cmd, pArg)); */
    ...
    if (RT_UNLIKELY(!pState))
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: No state data for %d\n", instance));
        return EINVAL;
    }
    ...
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: Bad request %#x size=%d expected=%d\n", Cmd, IOCPARM_LEN(Cmd),
                sizeof(ReqWrap)));
        return ENOTTY;
    }
    ...
    if (RT_UNLIKELY(rc))
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: ddi_copyin failed to read header pArg=%p Cmd=%d. rc=%d\n", pArg, Cmd, rc));
        return EINVAL;
    }

    if (ReqWrap.u32Magic != VPOXUSB_MAGIC)
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: Bad magic %#x; pArg=%p Cmd=%d\n", ReqWrap.u32Magic, pArg, Cmd));
        return EINVAL;
    }
    if (RT_UNLIKELY(   ReqWrap.cbData == 0
                    || ReqWrap.cbData > _1M*16))
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: Bad size %#x; pArg=%p Cmd=%d\n", ReqWrap.cbData, pArg, Cmd));
        return EINVAL;
    }
    ...
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: RTMemTmpAlloc failed to alloc %d bytes\n", ReqWrap.cbData));
        return ENOMEM;
    }
    ...
    {
        RTMemTmpFree(pvBuf);
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: ddi_copyin failed! pvBuf=%p pArg=%p Cmd=%d. rc=%d\n", pvBuf, pArg, Cmd, rc));
        return EFAULT;
    }
    if (RT_UNLIKELY(   ReqWrap.cbData == 0
                    || pvBuf == NULL))
    {
        RTMemTmpFree(pvBuf);
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: Invalid request! pvBuf=%p cbData=%d\n", pvBuf, ReqWrap.cbData));
        return EINVAL;
    }
     ...
     */
    size_t cbDataOut = 0;
    rc = vpoxUsbSolarisProcessIOCtl(Cmd, pState, Mode, &ReqWrap, pvBuf, &cbDataOut);
    ReqWrap.rc = rc;
    rc = 0;

    if (RT_UNLIKELY(cbDataOut > ReqWrap.cbData))
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: Too much output data %d expected %d Truncating!\n", cbDataOut,
                ReqWrap.cbData));
    ...
    /*
     * Copy VPOXUSBREQ back to userspace (which contains rc for USB operation).
     */
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        if (cbDataOut > 0)
        {
            ...
            if (RT_UNLIKELY(rc))
            {
                LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: ddi_copyout failed! pvBuf=%p pArg=%p Cmd=%d. rc=%d\n", pvBuf, pArg,
                        Cmd, rc));
                rc = EFAULT;
            }
        }
    ...
    {
        LogRel((DEVICE_NAME ": VPoxUSBSolarisIOCtl: ddi_copyout(1)failed! pReqWrap=%p pArg=%p Cmd=%d. rc=%d\n", &ReqWrap, pArg,
                Cmd, rc));
        rc = EFAULT;
    }
 ...
 * @param   pUSBReq             Pointer to the VPOXUSB request.
...
LOCAL int vpoxUsbSolarisProcessIOCtl(int iFunction, void *pvState, int Mode, PVPOXUSBREQ pUSBReq, void *pvBuf,
                                     size_t *pcbDataOut)
{
    /* LogFunc((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: iFunction=%d pvState=%p pUSBReq=%p\n", iFunction, pvState, pUSBReq)); */

    AssertPtrReturn(pvState, VERR_INVALID_PARAMETER);
    vpoxusb_state_t *pState = (vpoxusb_state_t *)pvState;
...
#define CHECKRET_MIN_SIZE(mnemonic, cbMin) \
    do { \
        if (cbData < (cbMin)) \
        { \
            LogRel((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: " mnemonic ": cbData=%#zx (%zu) min is %#zx (%zu)\n", \
                 cbData, cbData, (size_t)(cbMin), (size_t)(cbMin))); \
            return VERR_BUFFER_OVERFLOW; \
        } \
        if ((cbMin) != 0 && !VALID_PTR(pvBuf)) \
        { \
            LogRel((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: " mnemonic ": Invalid pointer %p\n", pvBuf)); \
    ...
    switch (iFunction)
    {
        case VPOXUSB_IOCTL_SEND_URB:
        {
            CHECKRET_MIN_SIZE("SEND_URB", sizeof(VPOXUSBREQ_URB));

            PVPOXUSBREQ_URB pUrbReq = (PVPOXUSBREQ_URB)pvBuf;
            rc = vpoxUsbSolarisSendUrb(pState, pUrbReq, Mode);
            *pcbDataOut = 0;
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: SEND_URB returned %d\n", rc));
            break;
        }

        case VPOXUSB_IOCTL_REAP_URB:
        {
            CHECKRET_MIN_SIZE("REAP_URB", sizeof(VPOXUSBREQ_URB));

            PVPOXUSBREQ_URB pUrbReq = (PVPOXUSBREQ_URB)pvBuf;
            rc = vpoxUsbSolarisReapUrb(pState, pUrbReq, Mode);
            *pcbDataOut = sizeof(VPOXUSBREQ_URB);
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: REAP_URB returned %d\n", rc));
            break;
        }

        case VPOXUSB_IOCTL_CLEAR_EP:
        {
            CHECKRET_MIN_SIZE("CLEAR_EP", sizeof(VPOXUSBREQ_CLEAR_EP));

            PVPOXUSBREQ_CLEAR_EP pClearEpReq = (PVPOXUSBREQ_CLEAR_EP)pvBuf;
            rc = vpoxUsbSolarisClearEndPoint(pState, pClearEpReq->bEndpoint);
            *pcbDataOut = 0;
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: CLEAR_EP returned %d\n", rc));
            break;
        }

        case VPOXUSB_IOCTL_SET_CONFIG:
        {
            CHECKRET_MIN_SIZE("SET_CONFIG", sizeof(VPOXUSBREQ_SET_CONFIG));

            PVPOXUSBREQ_SET_CONFIG pSetCfgReq = (PVPOXUSBREQ_SET_CONFIG)pvBuf;
            rc = vpoxUsbSolarisSetConfig(pState, pSetCfgReq->bConfigValue);
            *pcbDataOut = 0;
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: SET_CONFIG returned %d\n", rc));
            break;
        }

        case VPOXUSB_IOCTL_SET_INTERFACE:
        {
            CHECKRET_MIN_SIZE("SET_INTERFACE", sizeof(VPOXUSBREQ_SET_INTERFACE));

            PVPOXUSBREQ_SET_INTERFACE pSetInterfaceReq = (PVPOXUSBREQ_SET_INTERFACE)pvBuf;
            rc = vpoxUsbSolarisSetInterface(pState, pSetInterfaceReq->bInterface, pSetInterfaceReq->bAlternate);
            *pcbDataOut = 0;
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: SET_INTERFACE returned %d\n", rc));
            break;
        }

        case VPOXUSB_IOCTL_CLOSE_DEVICE:
        {
            CHECKRET_MIN_SIZE("CLOSE_DEVICE", sizeof(VPOXUSBREQ_CLOSE_DEVICE));

            PVPOXUSBREQ_CLOSE_DEVICE pCloseDeviceReq = (PVPOXUSBREQ_CLOSE_DEVICE)pvBuf;
            if (   pCloseDeviceReq->ResetLevel != VPOXUSB_RESET_LEVEL_REATTACH
                || (Mode & FKIOCTL))
            {
                rc = vpoxUsbSolarisCloseDevice(pState, pCloseDeviceReq->ResetLevel);
            }
            else
            {
            ...
            *pcbDataOut = 0;
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: CLOSE_DEVICE returned %d\n", rc));
            break;
        }

        case VPOXUSB_IOCTL_ABORT_PIPE:
        {
            CHECKRET_MIN_SIZE("ABORT_PIPE", sizeof(VPOXUSBREQ_ABORT_PIPE));

            PVPOXUSBREQ_ABORT_PIPE pAbortPipeReq = (PVPOXUSBREQ_ABORT_PIPE)pvBuf;
            rc = vpoxUsbSolarisAbortPipe(pState, pAbortPipeReq->bEndpoint);
            *pcbDataOut = 0;
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: ABORT_PIPE returned %d\n", rc));
            break;
        }

        case VPOXUSB_IOCTL_GET_CONFIG:
        {
            CHECKRET_MIN_SIZE("GET_CONFIG", sizeof(VPOXUSBREQ_GET_CONFIG));

            PVPOXUSBREQ_GET_CONFIG pGetCfgReq = (PVPOXUSBREQ_GET_CONFIG)pvBuf;
            rc = vpoxUsbSolarisGetConfig(pState, &pGetCfgReq->bConfigValue);
            *pcbDataOut = sizeof(VPOXUSBREQ_GET_CONFIG);
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: GET_CONFIG returned %d\n", rc));
            break;
        }

        case VPOXUSB_IOCTL_GET_VERSION:
        {
            CHECKRET_MIN_SIZE("GET_VERSION", sizeof(VPOXUSBREQ_GET_VERSION));

            PVPOXUSBREQ_GET_VERSION pGetVersionReq = (PVPOXUSBREQ_GET_VERSION)pvBuf;
            pGetVersionReq->u32Major = VPOXUSB_VERSION_MAJOR;
            pGetVersionReq->u32Minor = VPOXUSB_VERSION_MINOR;
            *pcbDataOut = sizeof(VPOXUSBREQ_GET_VERSION);
            rc = VINF_SUCCESS;
            Log((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: GET_VERSION returned %d\n", rc));
            break;
        }

        default:
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisProcessIOCtl: Unknown request %#x\n", iFunction));
            rc = VERR_NOT_SUPPORTED;
            *pcbDataOut = 0;
            break;
        }
...
LOCAL int vpoxUsbSolarisInitPower(vpoxusb_state_t *pState)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisInitPower: pState=%p\n", pState));

    int rc = usb_handle_remote_wakeup(pState->pDip, USB_REMOTE_WAKEUP_ENABLE);
    if (rc == USB_SUCCESS)
    {
        vpoxusb_power_t *pPower = RTMemAllocZ(sizeof(vpoxusb_power_t));
        if (RT_LIKELY(pPower))
        {
            mutex_enter(&pState->Mtx);
            ...
            if (rc == USB_SUCCESS)
            {
                ...
                if (rc != DDI_SUCCESS)
                {
                    LogRel((DEVICE_NAME ": vpoxUsbSolarisInitPower: Failed to raise power level usb(%#x,%#x)\n",
                            pState->pDevDesc->dev_descr->idVendor, pState->pDevDesc->dev_descr->idProduct));
                }
            }
            else
                Log((DEVICE_NAME ": vpoxUsbSolarisInitPower: Failed to create power components\n"));
    ...
    {
        Log((DEVICE_NAME ": vpoxUsbSolarisInitPower: Failed to enable remote wakeup, No PM!\n"));
        rc = VINF_SUCCESS;
    }
...
LOCAL void vpoxUsbSolarisDestroyPower(vpoxusb_state_t *pState)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDestroyPower: pState=%p\n", pState));

    if (pState->pPower)
    {
        mutex_exit(&pState->Mtx);
        vpoxUsbSolarisPowerBusy(pState);
        mutex_enter(&pState->Mtx);
            ...
            && pState->DevState != USB_DEV_DISCONNECTED)
        {
            ...
            rc = pm_raise_power(pState->pDip, 0 /* component */, USB_DEV_OS_FULL_PWR);
            if (rc != DDI_SUCCESS)
                Log((DEVICE_NAME ": vpoxUsbSolarisDestroyPower: Raising power failed! rc=%d\n", rc));

            rc = usb_handle_remote_wakeup(pState->pDip, USB_REMOTE_WAKEUP_DISABLE);
            if (rc != DDI_SUCCESS)
                Log((DEVICE_NAME ": vpoxUsbSolarisDestroyPower: Failed to disable remote wakeup\n"));
        }
        else
            mutex_exit(&pState->Mtx);
        ...
        if (rc != DDI_SUCCESS)
            Log((DEVICE_NAME ": vpoxUsbSolarisDestroyPower: Lowering power failed! rc=%d\n", rc));

        vpoxUsbSolarisPowerIdle(pState);
        mutex_enter(&pState->Mtx);
...
/**
 * Converts Solaris' USBA URB status to VPox's USB URB status.
 *
 * @param   Status          Solaris USBA USB URB status.
 *
 * @returns VPox USB URB status.
 */
LOCAL VUSBSTATUS vpoxUsbSolarisGetUrbStatus(usb_cr_t Status)
{
    switch (Status)
...
/**
 * Converts Solaris' USBA error code to VPox's error code.
 *
 * @param   UsbRc           Solaris USBA error code.
 *
 * @returns VPox error code.
 */
static int vpoxUsbSolarisToVPoxRC(int UsbRc)
{
    switch (UsbRc)
    {
...
/**
 * Converts Solaris' USBA device state to VPox's error code.
 *
 * @param   uDeviceState        The USB device state to convert.
 *
 * @returns VPox error code.
 */
static int vpoxUsbSolarisDeviceState(uint8_t uDeviceState)
{
    switch (uDeviceState)
    {
...
LOCAL bool vpoxUsbSolarisIsUSBDevice(dev_info_t *pDip)
{
    ...
    if (RT_LIKELY(rc == DDI_PROP_SUCCESS))
    {
        while (cCompatible--)
        {
            Log((DEVICE_NAME ": vpoxUsbSolarisIsUSBDevice: Compatible[%d]=%s\n", cCompatible, ppszCompatible[cCompatible]));
            if (!strncmp(ppszCompatible[cCompatible], "usb", 3))
            {
                Log((DEVICE_NAME ": vpoxUsbSolarisIsUSBDevice: Verified device as USB. pszCompatible=%s\n",
        ...
        Log((DEVICE_NAME ": vpoxUsbSolarisIsUSBDevice: USB property lookup failed, rc=%d\n", rc));
    ...
    if (pParentDip)
    {
        ...
        if (RT_LIKELY(rc == DDI_PROP_SUCCESS))
        {
            while (cCompatible--)
            {
                Log((DEVICE_NAME ": vpoxUsbSolarisIsUSBDevice: Parent compatible[%d]=%s\n", cCompatible,
                     ppszCompatible[cCompatible]));
                if (!strncmp(ppszCompatible[cCompatible], "usb", 3))
                {
                    Log((DEVICE_NAME ": vpoxUsbSolarisIsUSBDevice: Verified device as USB. parent pszCompatible=%s\n",
                            ppszCompatible[cCompatible]));
                    ddi_prop_free(ppszCompatible);
                    return true;
                }
            ...
            Log((DEVICE_NAME ": vpoxUsbSolarisIsUSBDevice: USB parent property lookup failed. rc=%d\n", rc));
    }
    else
        Log((DEVICE_NAME ": vpoxUsbSolarisIsUSBDevice: Failed to obtain parent device for property lookup\n"));
 ...
 *
 * @param   pState          The USB device instance.
 * @param   pUrbReq         Pointer to the VPox USB URB.
 * @param   Mode            The IOCtl mode.
 *
 * @returns VPox error code.
 */
LOCAL int vpoxUsbSolarisSendUrb(vpoxusb_state_t *pState, PVPOXUSBREQ_URB pUrbReq, int Mode)
{
    int iEpIndex = VPOXUSB_GET_EP_INDEX(pUrbReq->bEndpoint);
    Assert(iEpIndex >= 0 && iEpIndex < RT_ELEMENTS(pState->aEps));
    vpoxusb_ep_t *pEp = &pState->aEps[iEpIndex];
    AssertPtrReturn(pEp, VERR_INVALID_POINTER);
    Assert(pUrbReq);

#if 0
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisSendUrb: pState=%p pUrbReq=%p bEndpoint=%#x[%d] enmDir=%#x enmType=%#x "
    ...
    {
        LogRel((DEVICE_NAME ": vpoxUsbSolarisSendUrb: Invalid request - No data\n"));
        ...
        || pUrbReq->enmType == VUSBXFERTYPE_MSG)
    {
        ...
        if (RT_UNLIKELY(!pMsg))
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisSendUrb: Failed to allocate %u bytes\n", pUrbReq->cbData));
        ...
        if (RT_UNLIKELY(rc))
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisSendUrb: ddi_copyin failed! rc=%d\n", rc));
            freemsg(pMsg);
            return VERR_NO_MEMORY;
        }
    ...
    mutex_enter(&pState->Mtx);
    rc = vpoxUsbSolarisDeviceState(pState->DevState);
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        rc = vpoxUsbSolarisOpenPipe(pState, pEp);
        if (RT_UNLIKELY(RT_FAILURE(rc)))
        {
            mutex_exit(&pState->Mtx);
            freemsg(pMsg);
            LogRel((DEVICE_NAME ": vpoxUsbSolarisSendUrb: OpenPipe failed! pState=%p pUrbReq=%p bEndpoint=%#x enmDir=%#x "
        ...
        if (   pUrbReq->enmType == VUSBXFERTYPE_ISOC
            ...
            pUrb = vpoxUsbSolarisGetIsocInUrb(pState, pUrbReq);
        else
            pUrb = vpoxUsbSolarisQueueUrb(pState, pUrbReq, pMsg);
        ...
        if (RT_LIKELY(pUrb))
        {
            switch (pUrb->enmType)
            {
                ...
                {
                    rc = vpoxUsbSolarisCtrlXfer(pState, pEp, pUrb);
                    break;
                }

                case VUSBXFERTYPE_BULK:
                {
                    rc = vpoxUsbSolarisBulkXfer(pState, pEp, pUrb);
                    break;
                }

                case VUSBXFERTYPE_INTR:
                {
                    rc = vpoxUsbSolarisIntrXfer(pState, pEp, pUrb);
                    break;
                }

                case VUSBXFERTYPE_ISOC:
                {
                    rc = vpoxUsbSolarisIsocXfer(pState, pEp, pUrb);
                    break;
                }

                default:
                {
                    LogRelMax(5, (DEVICE_NAME ": vpoxUsbSolarisSendUrb: URB type unsupported %d\n", pUrb->enmType));
                    rc = VERR_NOT_SUPPORTED;
                    break;
                }
            }
            ...
            if (RT_FAILURE(rc))
            {
                ...
                else
                {
                    ...
                    pUrb->enmState = VPOXUSB_URB_STATE_FREE;
                    Assert(!pUrb->pMsg);
        ...
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisSendUrb: Failed to queue URB\n"));
            rc = VERR_NO_MEMORY;
            freemsg(pMsg);
        }
    }
...
LOCAL int vpoxUsbSolarisReapUrb(vpoxusb_state_t *pState, PVPOXUSBREQ_URB pUrbReq, int Mode)
{
    /* LogFunc((DEVICE_NAME ": vpoxUsbSolarisReapUrb: pState=%p pUrbReq=%p\n", pState, pUrbReq)); */
    ...
    if (RT_SUCCESS(rc))
    {
        vpoxusb_urb_t *pUrb = list_remove_head(&pState->hLandedUrbs);
        if (pUrb)
        {
            ...
            if (RT_LIKELY(pUrb->pMsg))
            {
                ...
                if (RT_LIKELY(pUrb->pvDataR3 != NIL_RTR3PTR))
                {
                    ...
                    if (RT_LIKELY(cbData))
                    {
                        ...
                        if (RT_UNLIKELY(rc))
                        {
                            LogRel((DEVICE_NAME ": vpoxUsbSolarisReapUrb: ddi_copyout failed! rc=%d\n", rc));
                            pUrbReq->enmStatus = VUSBSTATUS_INVALID;
                        }
                    }

                    Log((DEVICE_NAME ": vpoxUsbSolarisReapUrb: pvUrbR3=%p pvDataR3=%p cbData=%d\n", pUrbReq->pvUrbR3,
                ...
                else
                {
                    ...
                    LogRel((DEVICE_NAME ": vpoxUsbSolarisReapUrb: Missing pvDataR3!!\n"));
                }

                /*
                 * Free buffer allocated in vpoxUsbSolarisSendUrb or vpoxUsbSolaris[Ctrl|Bulk|Intr]Xfer().
                 */
                freemsg(pUrb->pMsg);
                pUrb->pMsg = NULL;
            }
...
LOCAL int vpoxUsbSolarisClearEndPoint(vpoxusb_state_t *pState, uint8_t bEndpoint)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisClearEndPoint: pState=%p bEndpoint=%#x\n", pState, bEndpoint));

    mutex_enter(&pState->Mtx);
    int rc = vpoxUsbSolarisDeviceState(pState->DevState);
    if (RT_SUCCESS(rc))
    {
        int iEpIndex = VPOXUSB_GET_EP_INDEX(bEndpoint);
        Assert(iEpIndex >= 0 && iEpIndex < RT_ELEMENTS(pState->aEps));
        vpoxusb_ep_t *pEp = &pState->aEps[iEpIndex];
        if (RT_LIKELY(pEp))
        {
            ...
            if (pEp->pPipe)
            {
                ...
                Log((DEVICE_NAME ": vpoxUsbSolarisClearEndPoint: bEndpoint=%#x[%d] returns %d\n", bEndpoint, iEpIndex, rc));

                rc = VINF_SUCCESS;
            }
            else
            {
                Log((DEVICE_NAME ": vpoxUsbSolarisClearEndPoint: Not opened to be cleared. Faking success. bEndpoint=%#x\n",
                     bEndpoint));
                rc = VINF_SUCCESS;
            }
        }
        ...
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisClearEndPoint: Endpoint missing! bEndpoint=%#x[%d]\n", bEndpoint, iEpIndex));
            rc = VERR_GENERAL_FAILURE;
        }
    }
    else
        Log((DEVICE_NAME ": vpoxUsbSolarisClearEndPoint: Device not online, state=%d\n", pState->DevState));
 ...
 * @returns VPox error code.
 */
LOCAL int vpoxUsbSolarisSetConfig(vpoxusb_state_t *pState, uint8_t bConfig)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisSetConfig: pState=%p bConfig=%u\n", pState, bConfig));

    mutex_enter(&pState->Mtx);
    int rc = vpoxUsbSolarisDeviceState(pState->DevState);
    if (RT_SUCCESS(rc))
    {
        vpoxUsbSolarisCloseAllPipes(pState, false /* ControlPipe */);
        int iCfgIndex = vpoxUsbSolarisGetConfigIndex(pState, bConfig);
            ...
            && iCfgIndex < pState->pDevDesc->dev_n_cfg)
        {
            ...
            if (rc == USB_SUCCESS)
            {
                int rc2 = vpoxUsbSolarisInitEpsForCfg(pState);
                AssertRC(rc2); NOREF(rc2);
                rc = VINF_SUCCESS;
            }
            else
            {
                LogRel((DEVICE_NAME ": vpoxUsbSolarisSetConfig: usb_set_cfg failed for iCfgIndex=%#x bConfig=%u rc=%d\n",
                            iCfgIndex, bConfig, rc));
                rc = vpoxUsbSolarisToVPoxRC(rc);
            }
        }
        else
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisSetConfig: Invalid iCfgIndex=%d bConfig=%u\n", iCfgIndex, bConfig));
 ...
 * @returns VPox error code.
 */
LOCAL int vpoxUsbSolarisGetConfig(vpoxusb_state_t *pState, uint8_t *pbConfig)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisGetConfig: pState=%p pbConfig=%p\n", pState, pbConfig));
    ...
    {
        LogRel((DEVICE_NAME ": vpoxUsbSolarisGetConfig: Failed, rc=%d\n", rc));
        rc = vpoxUsbSolarisToVPoxRC(rc);
    }

    Log((DEVICE_NAME ": vpoxUsbSolarisGetConfig: Returns %d bConfig=%u\n", rc, *pbConfig));
    return rc;
}
...
LOCAL int vpoxUsbSolarisSetInterface(vpoxusb_state_t *pState, uint8_t bIf, uint8_t bAlt)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisSetInterface: pState=%p bIf=%#x bAlt=%#x\n", pState, bIf, bAlt));
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (rc == USB_SUCCESS)
        {
            Log((DEVICE_NAME ": vpoxUsbSolarisSetInterface: Success, bIf=%#x bAlt=%#x\n", bIf, bAlt, rc));
            int rc2 = vpoxUsbSolarisInitEpsForIfAlt(pState, bIf, bAlt);
            AssertRC(rc2); NOREF(rc2);
            rc = VINF_SUCCESS;
        }
        else
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisSetInterface: usb_set_alt_if failed for bIf=%#x bAlt=%#x rc=%d\n", bIf, bAlt, rc));
            rc = vpoxUsbSolarisToVPoxRC(rc);
        }
    }
...
LOCAL int vpoxUsbSolarisCloseDevice(vpoxusb_state_t *pState, VPOXUSB_RESET_LEVEL enmReset)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisCloseDevice: pState=%p enmReset=%d\n", pState, enmReset));

    mutex_enter(&pState->Mtx);
    int rc = vpoxUsbSolarisDeviceState(pState->DevState);

    if (enmReset == VPOXUSB_RESET_LEVEL_CLOSE)
        vpoxUsbSolarisCloseAllPipes(pState, true /* ControlPipe */);
    else
        vpoxUsbSolarisCloseAllPipes(pState, false /* ControlPipe */);
    ...
    if (RT_SUCCESS(rc))
    {
        switch (enmReset)
        {
            case VPOXUSB_RESET_LEVEL_REATTACH:
                rc = usb_reset_device(pState->pDip, USB_RESET_LVL_REATTACH);
                break;

            case VPOXUSB_RESET_LEVEL_SOFT:
    ...
    Log((DEVICE_NAME ": vpoxUsbSolarisCloseDevice: Returns %d\n", rc));
    return rc;
}
...
LOCAL int vpoxUsbSolarisAbortPipe(vpoxusb_state_t *pState, uint8_t bEndpoint)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisAbortPipe: pState=%p bEndpoint=%#x\n", pState, bEndpoint));
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_LIKELY(pEp))
        {
            if (pEp->pPipe)
            {
                ...
                {
                    mutex_exit(&pState->Mtx);
                    LogRel((DEVICE_NAME ": vpoxUsbSolarisAbortPipe: Cannot reset default control pipe\n"));
                ...
                Log((DEVICE_NAME ": vpoxUsbSolarisAbortPipe: usb_pipe_drain_reqs returns %d\n", rc));
                rc = vpoxUsbSolarisToVPoxRC(rc);
            }
            else
            {
                LogRel((DEVICE_NAME ": vpoxUsbSolarisAbortPipe: pipe not open. bEndpoint=%#x\n", bEndpoint));
                rc = VERR_PIPE_IO_ERROR;
            }
        }
        else
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisAbortPipe: Invalid pipe bEndpoint=%#x[%d]\n", bEndpoint, iEpIndex));
    ...
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisAbortPipe: Returns %d\n", rc));
    return rc;
}
...
LOCAL int vpoxUsbSolarisInitEp(vpoxusb_state_t *pState, usb_ep_data_t *pEpData)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisInitEp: pState=%p pEpData=%p", pState, pEpData));

    /*
     * Is this the default endpoint?
     */
    usb_ep_descr_t *pEpDesc = NULL;
    vpoxusb_ep_t *pEp = NULL;
    int iEpIndex;
    if (!pEpData)
    {
        iEpIndex = 0;
        pEpDesc = &g_VPoxUSBSolarisDefaultEpDesc;
    }
    else
    {
        iEpIndex = VPOXUSB_GET_EP_INDEX(pEpData->ep_descr.bEndpointAddress);
    ...
    if (!pEp->fInitialized)
    {
        ...
        pEp->PipePolicy.pp_max_async_reqs = VPOXUSB_MAX_PIPE_ASYNC_REQS;
        pEp->fIsocPolling = false;
        list_create(&pEp->hIsocInUrbs, sizeof(vpoxusb_urb_t), offsetof(vpoxusb_urb_t, hListLink));
        pEp->cIsocInUrbs = 0;
        list_create(&pEp->hIsocInLandedReqs, sizeof(vpoxusb_isoc_req_t), offsetof(vpoxusb_isoc_req_t, hListLink));
        pEp->cbIsocInLandedReqs = 0;
        pEp->cbMaxIsocData = 0;
        pEp->fInitialized = true;
    }

    Log((DEVICE_NAME ": vpoxUsbSolarisInitEp: Success, %s[%2d] %s %s bEndpoint=%#x\n", !pEpData ? "Default " : "Endpoint",
         iEpIndex, vpoxUsbSolarisEpType(pEp), vpoxUsbSolarisEpDir(pEp), pEp->EpDesc.bEndpointAddress));
    return VINF_SUCCESS;
}
...
LOCAL int vpoxUsbSolarisInitEpsForCfg(vpoxusb_state_t *pState)
{
    ...
    {
        LogRel((DEVICE_NAME ": vpoxUsbSolarisInitEpsForCfg: Invalid current config index %u\n", uCfgIndex));
    ...
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisInitEpsForCfg: pState=%p bConfig=%u uCfgIndex=%u\n", pState, bConfig, uCfgIndex));
        ...
        for (uchar_t uAlt = 0; uAlt < cAlts; uAlt++)
        {
            ...
            if (pAlt->altif_descr.bAlternateSetting == 0)   /* Refer USB 2.0 spec 9.6.5 "Interface" */
            {
                ...
                for (uchar_t uEp = 0; uEp < cEps; uEp++)
                {
                    ...
                    int rc = vpoxUsbSolarisInitEp(pState, pEpData);
                    if (RT_FAILURE(rc))
                    {
                        LogRel((DEVICE_NAME ": vpoxUsbSolarisInitEpsForCfg: Failed to init endpoint! "
...
LOCAL int vpoxUsbSolarisInitEpsForIfAlt(vpoxusb_state_t *pState, uint8_t bIf, uint8_t bAlt)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisInitEpsForIfAlt: pState=%p bIf=%d uAlt=%d\n", pState, bIf, bAlt));

    /* Doesn't hurt to be paranoid */
    uint_t uCfgIndex = usb_get_current_cfgidx(pState->pDip);
    if (uCfgIndex >= pState->pDevDesc->dev_n_cfg)
    {
        LogRel((DEVICE_NAME ": vpoxUsbSolarisInitEpsForIfAlt: Invalid current config index %d\n", uCfgIndex));
        return VERR_OUT_OF_RANGE;
    }
                ...
                && pAlt->altif_descr.bAlternateSetting == bAlt)
            {
                ...
                for (uchar_t uEp = 0; uEp < cEps; uEp++)
                {
                    ...
                    if (RT_FAILURE(rc))
                    {
                        ...
                        LogRel((DEVICE_NAME ": vpoxUsbSolarisInitEpsForIfAlt: Failed to init endpoint! "
...
LOCAL void vpoxUsbSolarisDestroyAllEps(vpoxusb_state_t *pState)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDestroyAllEps: pState=%p\n", pState));

    Assert(mutex_owned(&pState->Mtx));
    for (unsigned i = 0; i < VPOXUSB_MAX_ENDPOINTS; i++)
    {
        vpoxusb_ep_t *pEp = &pState->aEps[i];
        if (pEp->fInitialized)
            vpoxUsbSolarisDestroyEp(pState, pEp);
    }
}
...
LOCAL void vpoxUsbSolarisDestroyEp(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDestroyEp: pState=%p pEp=%p\n", pState, pEp));

    Assert(pEp->fInitialized);
    Assert(mutex_owned(&pState->Mtx));
    vpoxusb_urb_t *pUrb = list_remove_head(&pEp->hIsocInUrbs);
    while (pUrb)
    {
        if (pUrb->pMsg)
            freemsg(pUrb->pMsg);
    ...
    vpoxusb_isoc_req_t *pIsocReq = list_remove_head(&pEp->hIsocInLandedReqs);
    while (pIsocReq)
    {
        kmem_free(pIsocReq, sizeof(vpoxusb_isoc_req_t));
 ...
 * @remarks Requires the device state mutex to be held.
 */
LOCAL void vpoxUsbSolarisCloseAllPipes(vpoxusb_state_t *pState, bool fDefault)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisCloseAllPipes: pState=%p\n", pState));

    for (int i = 1; i < VPOXUSB_MAX_ENDPOINTS; i++)
    {
        vpoxusb_ep_t *pEp = &pState->aEps[i];
        if (   pEp
            && pEp->pPipe)
        {
            Log((DEVICE_NAME ": vpoxUsbSolarisCloseAllPipes: Closing[%d]\n", i));
            vpoxUsbSolarisClosePipe(pState, pEp);
        }
    }

    if (fDefault)
    {
        vpoxusb_ep_t *pEp = &pState->aEps[0];
        if (   pEp
            && pEp->pPipe)
        {
            vpoxUsbSolarisClosePipe(pState, pEp);
            Log((DEVICE_NAME ": vpoxUsbSolarisCloseAllPipes: Closed default pipe\n"));
        }
    }
}
...
LOCAL int vpoxUsbSolarisOpenPipe(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp)
{
    ...
    if ((pEp->EpDesc.bEndpointAddress & USB_EP_NUM_MASK) == 0)
    {
        ...
        Log((DEVICE_NAME ": vpoxUsbSolarisOpenPipe: Default pipe opened\n"));
        return VINF_SUCCESS;
    }
    ...
    if (rc == USB_SUCCESS)
    {
        LogFunc((DEVICE_NAME ": vpoxUsbSolarisOpenPipe: Opened pipe, pState=%p pEp=%p\n", pState, pEp));
        usb_pipe_set_private(pEp->pPipe, (usb_opaque_t)pEp);

        /*
         * Determine input buffer size for Isoc. IN transfers.
         */
        if (   VPOXUSB_XFER_TYPE(pEp) == VUSBXFERTYPE_ISOC
            && VPOXUSB_XFER_DIR(pEp) == VUSB_DIR_TO_HOST)
        {
            /*
             ...
             */
            uint16_t cbMax = VPOXUSB_PKT_SIZE(pEp->EpDesc.wMaxPacketSize);
            if (cbMax <= 1024)
            {
            ...
            Log((DEVICE_NAME ": vpoxUsbSolarisOpenPipe: bEndpoint=%#x cbMaxIsocData=%u\n", pEp->EpDesc.bEndpointAddress,
                 pEp->cbMaxIsocData));
        }
    ...
    {
        LogRel((DEVICE_NAME ": vpoxUsbSolarisOpenPipe: Failed! rc=%d pState=%p pEp=%p\n", rc, pState, pEp));
        rc = VERR_BAD_PIPE;
    }
...
LOCAL void vpoxUsbSolarisClosePipe(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisClosePipe: pState=%p pEp=%p\n", pState, pEp));
    AssertPtr(pEp);
    ...
    if (pEp->pPipe)
    {
        ...
        if (pEp->pPipe == pState->pDevDesc->dev_default_ph)
        {
            ...
            mutex_enter(&pState->Mtx);
            Log((DEVICE_NAME ": vpoxUsbSolarisClosePipe: Closed default pipe\n"));
        ...
        else
        {
             ...
             */
            Log((DEVICE_NAME ": vpoxUsbSolarisClosePipe: Pipe bmAttributes=%#x bEndpoint=%#x\n", pEp->EpDesc.bmAttributes,
                 pEp->EpDesc.bEndpointAddress));
            mutex_exit(&pState->Mtx);
        ...
        Log((DEVICE_NAME ": vpoxUsbSolarisClosePipe: Success, bEndpoint=%#x\n", pEp->EpDesc.bEndpointAddress));
    }
 ...
 */
LOCAL int vpoxUsbSolarisGetConfigIndex(vpoxusb_state_t *pState, uint_t bConfig)
{
...
LOCAL vpoxusb_urb_t *vpoxUsbSolarisGetIsocInUrb(vpoxusb_state_t *pState, PVPOXUSBREQ_URB pUrbReq)
{
     ...
     */
    vpoxusb_urb_t *pUrb = RTMemAllocZ(sizeof(vpoxusb_urb_t));
    if (RT_LIKELY(pUrb))
    {
        pUrb->enmState = VPOXUSB_URB_STATE_INFLIGHT;
        pUrb->pState = pState;
        ...
        LogRel((DEVICE_NAME ": vpoxUsbSolarisGetIsocInUrb: Failed to alloc %d bytes\n", sizeof(vpoxusb_urb_t)));
    return pUrb;
}
...
{
    Assert(pUrbReq);
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisQueueUrb: pState=%p pUrbReq=%p\n", pState, pUrbReq));
     ...
     */
    vpoxusb_urb_t *pUrb = list_remove_head(&pState->hFreeUrbs);
    if (pUrb)
    {
        Assert(pUrb->enmState == VPOXUSB_URB_STATE_FREE);
        Assert(!pUrb->pMsg);
    ...
    else
    {
        ...
        if (cTotalUrbs >= VPOXUSB_URB_QUEUE_SIZE)
        {
            mutex_exit(&pState->Mtx);
            LogRelMax(5, (DEVICE_NAME ": vpoxUsbSolarisQueueUrb: Max queue size %u reached, refusing further transfers",
                          cTotalUrbs));
            return NULL;
        }
         ...
         */
        mutex_exit(&pState->Mtx);
        pUrb = RTMemAllocZ(sizeof(vpoxusb_urb_t));
        if (RT_UNLIKELY(!pUrb))
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisQueueUrb: Failed to alloc %d bytes\n", sizeof(vpoxusb_urb_t)));
            return NULL;
        }
        mutex_enter(&pState->Mtx);
    }
...
LOCAL void vpoxUsbSolarisDeQueueUrb(vpoxusb_urb_t *pUrb, int URBStatus)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDeQueue: pUrb=%p\n", pUrb));
    AssertPtrReturnVoid(pUrb);

    pUrb->enmStatus = vpoxUsbSolarisGetUrbStatus(URBStatus);
    if (pUrb->enmStatus != VUSBSTATUS_OK)
        Log((DEVICE_NAME ": vpoxUsbSolarisDeQueueUrb: URB failed! URBStatus=%d bEndpoint=%#x\n", URBStatus, pUrb->bEndpoint));

    vpoxusb_state_t *pState = pUrb->pState;
    if (RT_LIKELY(pState))
    {
        mutex_enter(&pState->Mtx);
        pUrb->enmState = VPOXUSB_URB_STATE_LANDED;
        ...
        vpoxUsbSolarisNotifyComplete(pUrb->pState);
        mutex_exit(&pState->Mtx);
        return;
    }
    ...
    LogRel((DEVICE_NAME ": vpoxUsbSolarisDeQueue: State Gone\n"));
}
...
LOCAL void vpoxUsbSolarisConcatMsg(vpoxusb_urb_t *pUrb)
{
        ...
        && pUrb->pMsg->b_cont)
    {
        ...
        if (RT_LIKELY(pFullMsg))
        {
            ...
            LogRel((DEVICE_NAME ": vpoxUsbSolarisConcatMsg: Failed. Expect glitches due to truncated data!\n"));
    }
}
 ...
 * @param   pUrb            The VPox USB URB.
 *
 * @returns VPox status code.
 */
LOCAL int vpoxUsbSolarisCtrlXfer(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp, vpoxusb_urb_t *pUrb)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisCtrlXfer: pState=%p pEp=%p pUrb=%p enmDir=%d cbData=%d\n", pState, pEp, pUrb,
             pUrb->enmDir, pUrb->cbDataR3));

    AssertPtrReturn(pUrb->pMsg, VERR_INVALID_PARAMETER);
    const size_t cbData = pUrb->cbDataR3 > VPOXUSB_CTRL_XFER_SIZE ? pUrb->cbDataR3 - VPOXUSB_CTRL_XFER_SIZE : 0;
    ...
    if (RT_LIKELY(pReq))
    {
        ...
        pReq->ctrl_wValue         = (pSetupData[3] << VPOXUSB_CTRL_XFER_SIZE) | pSetupData[2];
        pReq->ctrl_wIndex         = (pSetupData[5] << VPOXUSB_CTRL_XFER_SIZE) | pSetupData[4];
        pReq->ctrl_wLength        = (pSetupData[7] << VPOXUSB_CTRL_XFER_SIZE) | pSetupData[6];

        if (   pUrb->enmDir == VUSBDIRECTION_OUT
            && cbData)
        {
            bcopy(pSetupData + VPOXUSB_CTRL_XFER_SIZE, pReq->ctrl_data->b_wptr, cbData);
         ...
         */
        pReq->ctrl_cb             = vpoxUsbSolarisCtrlXferCompleted;
        pReq->ctrl_exc_cb         = vpoxUsbSolarisCtrlXferCompleted;
        pReq->ctrl_timeout        = VPOXUSB_CTRL_XFER_TIMEOUT;
        ...
        LogRel((DEVICE_NAME ": vpoxUsbSolarisCtrlXfer: Request failed! bEndpoint=%#x rc=%d\n", pUrb->bEndpoint, rc));

        usb_free_ctrl_req(pReq);
        return VERR_PIPE_IO_ERROR;
    }

    LogRel((DEVICE_NAME ": vpoxUsbSolarisCtrlXfer: Failed to alloc request for %u bytes\n", cbData));
    return VERR_NO_MEMORY;
}
...
LOCAL void vpoxUsbSolarisCtrlXferCompleted(usb_pipe_handle_t pPipe, usb_ctrl_req_t *pReq)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisCtrlXferCompleted: pPipe=%p pReq=%p\n", pPipe, pReq));
    Assert(pReq);
    Assert(!(pReq->ctrl_cb_flags & USB_CB_INTR_CONTEXT));

    vpoxusb_urb_t *pUrb   = (vpoxusb_urb_t *)pReq->ctrl_client_private;
    if (RT_LIKELY(pUrb))
    {
        /*
        ...
        if (RT_LIKELY(pSetupMsg))
        {
             ...
             *
             * The reason we choose to use vpoxUsbSolarisConcatMsg here is that we don't assume the
            ...
            pReq->ctrl_data = NULL;
            vpoxUsbSolarisConcatMsg(pUrb);
        }
        else
            LogRel((DEVICE_NAME ": vpoxUsbSolarisCtrlXferCompleted: Failed to alloc %u bytes for header\n", sizeof(VUSBSETUP)));

        /*
         * Update the URB and move to landed list for reaping.
         */
        vpoxUsbSolarisDeQueueUrb(pUrb, pReq->ctrl_completion_reason);
    }
    else
        LogRel((DEVICE_NAME ": vpoxUsbSolarisCtrlXferCompleted: Extreme error! missing private data\n"));
...
LOCAL int vpoxUsbSolarisBulkXfer(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp, vpoxusb_urb_t *pUrb)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisBulkXfer: pState=%p pEp=%p pUrb=%p enmDir=%d cbData=%d\n", pState, pEp, pUrb,
             pUrb->enmDir, pUrb->cbDataR3));
    ...
    if (RT_LIKELY(pReq))
    {
        ...
        pReq->bulk_cb             = vpoxUsbSolarisBulkXferCompleted;
        pReq->bulk_exc_cb         = vpoxUsbSolarisBulkXferCompleted;
        pReq->bulk_timeout        = 0;
        ...
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisBulkXfer: Requesting %d bytes when only %d bytes supported by device\n",
        ...
        LogRel((DEVICE_NAME ": vpoxUsbSolarisBulkXfer: Request failed! Ep=%#x rc=%d cbData=%u\n", pUrb->bEndpoint, rc,
                pReq->bulk_len));
    ...
    LogRel((DEVICE_NAME ": vpoxUsbSolarisBulkXfer: Failed to alloc bulk request\n"));
    return VERR_NO_MEMORY;
}
...
LOCAL void vpoxUsbSolarisBulkXferCompleted(usb_pipe_handle_t pPipe, usb_bulk_req_t *pReq)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisBulkXferCompleted: pPipe=%p pReq=%p\n", pPipe, pReq));

    Assert(pReq);
    Assert(!(pReq->bulk_cb_flags & USB_CB_INTR_CONTEXT));

    vpoxusb_ep_t *pEp = (vpoxusb_ep_t *)usb_pipe_get_private(pPipe);
    if (RT_LIKELY(pEp))
    {
        vpoxusb_urb_t *pUrb = (vpoxusb_urb_t *)pReq->bulk_client_private;
        if (RT_LIKELY(pUrb))
        {
            Assert(!pUrb->pMsg);
            if (   pUrb->enmDir == VUSBDIRECTION_IN
             ...
             */
            vpoxUsbSolarisDeQueueUrb(pUrb, pReq->bulk_completion_reason);
        }
        else
            LogRel((DEVICE_NAME ": vpoxUsbSolarisBulkXferCompleted: Extreme error! private request data missing!\n"));
    }
    else
        Log((DEVICE_NAME ": vpoxUsbSolarisBulkXferCompleted: Pipe Gone!\n"));
...
LOCAL int vpoxUsbSolarisIntrXfer(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp, vpoxusb_urb_t *pUrb)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisIntrXfer: pState=%p pEp=%p pUrb=%p enmDir=%d cbData=%d\n", pState, pEp, pUrb,
             pUrb->enmDir, pUrb->cbDataR3));
    ...
    if (RT_LIKELY(pReq))
    {
        ...
        pReq->intr_cb             = vpoxUsbSolarisIntrXferCompleted;
        pReq->intr_exc_cb         = vpoxUsbSolarisIntrXferCompleted;
        pReq->intr_timeout        = 0;
        ...
        LogRel((DEVICE_NAME ": vpoxUsbSolarisIntrXfer: usb_pipe_intr_xfer failed! rc=%d bEndpoint=%#x\n", rc, pUrb->bEndpoint));

        usb_free_intr_req(pReq);
        return VERR_PIPE_IO_ERROR;
    }

    LogRel((DEVICE_NAME ": vpoxUsbSolarisIntrXfer: Failed to alloc intr request\n"));
    return VERR_NO_MEMORY;
}
...
LOCAL void vpoxUsbSolarisIntrXferCompleted(usb_pipe_handle_t pPipe, usb_intr_req_t *pReq)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisIntrXferCompleted: pPipe=%p pReq=%p\n", pPipe, pReq));

    Assert(pReq);
    Assert(!(pReq->intr_cb_flags & USB_CB_INTR_CONTEXT));

    vpoxusb_urb_t *pUrb = (vpoxusb_urb_t *)pReq->intr_client_private;
    if (RT_LIKELY(pUrb))
    {
        if (   pUrb->enmDir == VUSBDIRECTION_IN
            && pReq->intr_data)
         ...
         * Update the URB and move to landed list for reaping.
         */
        vpoxUsbSolarisDeQueueUrb(pUrb, pReq->intr_completion_reason);
    }
    else
        LogRel((DEVICE_NAME ": vpoxUsbSolarisIntrXferCompleted: Extreme error! private request data missing\n"));
...
LOCAL int vpoxUsbSolarisIsocXfer(vpoxusb_state_t *pState, vpoxusb_ep_t *pEp, vpoxusb_urb_t *pUrb)
{
    /* LogFunc((DEVICE_NAME ": vpoxUsbSolarisIsocXfer: pState=%p pEp=%p pUrb=%p\n", pState, pEp, pUrb)); */
    ...
    if (pUrb->enmDir == VUSBDIRECTION_IN)
    {
        Log((DEVICE_NAME ": vpoxUsbSolarisIsocXfer: Isoc. IN - Queueing\n"));
        ...
        if (pEp->fIsocPolling)
        {
            ...
            {
                mutex_exit(&pState->Mtx);
                Log((DEVICE_NAME ": vpoxUsbSolarisIsocXfer: Max Isoc. data %d bytes queued\n", pEp->cbMaxIsocData));
    ...
    Log((DEVICE_NAME ": vpoxUsbSolarisIsocXfer: enmDir=%#x cIsocPkts=%d aIsocPkts[0]=%d cbDataR3=%d\n", pUrb->enmDir,
    ...
    if (RT_LIKELY(pReq))
    {
        ...
        if (pUrb->enmDir == VUSBDIRECTION_OUT)
        {
            ...
            pReq->isoc_cb             = vpoxUsbSolarisIsocOutXferCompleted;
            pReq->isoc_exc_cb         = vpoxUsbSolarisIsocOutXferCompleted;
        ...
        else
        {
            ...
            pReq->isoc_cb             = vpoxUsbSolarisIsocInXferCompleted;
            pReq->isoc_exc_cb         = vpoxUsbSolarisIsocInXferError;
        ...
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisIsocXfer: usb_pipe_isoc_xfer failed! rc=%d\n", rc));
            rc = VERR_PIPE_IO_ERROR;

            if (pUrb->enmDir == VUSBDIRECTION_IN)
            {
                mutex_enter(&pState->Mtx);
                vpoxusb_urb_t *pIsocFailedUrb = list_remove_tail(&pEp->hIsocInUrbs);
                if (pIsocFailedUrb)
                {
    ...
    {
        LogRel((DEVICE_NAME ": vpoxUsbSolarisIsocXfer: Failed to alloc isoc req for %d packets\n", pUrb->cIsocPkts));
        rc = VERR_NO_MEMORY;
    }
...
LOCAL void vpoxUsbSolarisIsocInXferCompleted(usb_pipe_handle_t pPipe, usb_isoc_req_t *pReq)
{
    /* LogFunc((DEVICE_NAME ": vpoxUsbSolarisIsocInXferCompleted: pPipe=%p pReq=%p\n", pPipe, pReq)); */

    vpoxusb_state_t *pState = (vpoxusb_state_t *)pReq->isoc_client_private;
    if (RT_LIKELY(pState))
    {
        vpoxusb_ep_t *pEp = (vpoxusb_ep_t *)usb_pipe_get_private(pPipe);
        if (   pEp
            && pEp->pPipe)
        {
            ...
            {
                Log((DEVICE_NAME ": vpoxUsbSolarisIsocInXferCompleted: Stopping polling! Too many errors\n"));
                mutex_exit(&pState->Mtx);
            ...
            {
                Log((DEVICE_NAME ": vpoxUsbSolarisIsocInXferCompleted: cIsocInUrbs=%d cbIsocInLandedReqs=%d\n", pEp->cIsocInUrbs,
                    ...
                    && pEp->cbIsocInLandedReqs == 0)
                {
                    ...
                    if (RT_LIKELY(pUrb))
                    {
                        ...
                        for (unsigned i = 0; i < pReq->isoc_pkts_count; i++)
                        {
                            ...
                            pUrb->aIsocPkts[i].enmStatus = vpoxUsbSolarisGetUrbStatus(pReq->isoc_pkt_descr[i].isoc_pkt_status);
                        }
                        ...
                        list_insert_tail(&pState->hLandedUrbs, pUrb);
                        ++pState->cLandedUrbs;
                        vpoxUsbSolarisNotifyComplete(pState);
                    }
                    else
                    {
                        /* Huh!? cIsocInUrbs is wrong then! Should never happen unless we decide to decrement cIsocInUrbs in
                           Reap time */
                        pEp->cIsocInUrbs = 0;
                        LogRel((DEVICE_NAME ": vpoxUsbSolarisIsocInXferCompleted: Extreme error! Isoc. counter borked!\n"));
                    }
                ...
                LogRel((DEVICE_NAME ": vpoxUsbSolarisIsocInXferCompleted: Data missing\n"));
        }
        else
            LogRel((DEVICE_NAME ": vpoxUsbSolarisIsocInXferCompleted: Pipe Gone\n"));
    }
    else
        Log((DEVICE_NAME ": vpoxUsbSolarisIsocInXferCompleted: State Gone\n"));
...
LOCAL void vpoxUsbSolarisIsocInXferError(usb_pipe_handle_t pPipe, usb_isoc_req_t *pReq)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisIsocInXferError: pPipe=%p pReq=%p\n", pPipe, pReq));

    vpoxusb_state_t *pState = (vpoxusb_state_t *)pReq->isoc_client_private;
    if (RT_UNLIKELY(!pState))
    {
        Log((DEVICE_NAME ": vpoxUsbSolarisIsocInXferError: State Gone\n"));
        usb_free_isoc_req(pReq);
        return;
    }
    ...
    if (RT_UNLIKELY(!pEp))
    {
        Log((DEVICE_NAME ": vpoxUsbSolarisIsocInXferError: Pipe Gone\n"));
        mutex_exit(&pState->Mtx);
        usb_free_isoc_req(pReq);
        return;
    }
    ...
    switch(pReq->isoc_completion_reason)
    {
        case USB_CR_NO_RESOURCES:
        {
            ...
            Log((DEVICE_NAME ": vpoxUsbSolarisIsocInXferError: Resubmitted Isoc. IN request due to unavailable resources\n"));
            return;
        }
        ...
        default:
        {
            Log((DEVICE_NAME ": vpoxUsbSolarisIsocInXferError: Stopping Isoc. IN polling due to rc=%d\n",
     ...
     */
    vpoxusb_urb_t *pUrb = list_remove_tail(&pEp->hIsocInUrbs);
    if (pUrb)
    {
        --pEp->cIsocInUrbs;
        Log((DEVICE_NAME ": vpoxUsbSolarisIsocInXferError: Deleting last queued URB as it failed\n"));
        freemsg(pUrb->pMsg);
        RTMemFree(pUrb);
        vpoxUsbSolarisNotifyComplete(pState);
    }
...
LOCAL void vpoxUsbSolarisIsocOutXferCompleted(usb_pipe_handle_t pPipe, usb_isoc_req_t *pReq)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisIsocOutXferCompleted: pPipe=%p pReq=%p\n", pPipe, pReq));

    vpoxusb_ep_t *pEp = (vpoxusb_ep_t *)usb_pipe_get_private(pPipe);
    if (RT_LIKELY(pEp))
    {
        vpoxusb_urb_t *pUrb = (vpoxusb_urb_t *)pReq->isoc_client_private;
        if (RT_LIKELY(pUrb))
        {
            size_t cbActPkt = 0;
            ...
            Log((DEVICE_NAME ": vpoxUsbSolarisIsocOutXferCompleted: cIsocPkts=%d cbData=%d cbActPkt=%d\n", pUrb->cIsocPkts,
             ...
             * Update the URB and move to landed list for reaping.
             */
            vpoxUsbSolarisDeQueueUrb(pUrb, pReq->isoc_completion_reason);
        }
        else
            Log((DEVICE_NAME ": vpoxUsbSolarisIsocOutXferCompleted: Missing private data!?! Dropping OUT pUrb\n"));
    }
    else
        Log((DEVICE_NAME ": vpoxUsbSolarisIsocOutXferCompleted: Pipe Gone\n"));
...
LOCAL int vpoxUsbSolarisDeviceDisconnected(dev_info_t *pDip)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDeviceDisconnected: pDip=%p\n", pDip));
    ...
    LogRel((DEVICE_NAME ": vpoxUsbSolarisDeviceDisconnected: Failed to get device state!\n"));
    return USB_FAILURE;
}
...
LOCAL int vpoxUsbSolarisDeviceReconnected(dev_info_t *pDip)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDeviceReconnected: pDip=%p\n", pDip));
    ...
    if (RT_LIKELY(pState))
    {
        vpoxUsbSolarisDeviceRestore(pState);
        return USB_SUCCESS;
    }

    LogRel((DEVICE_NAME ": vpoxUsbSolarisDeviceReconnected: Failed to get device state!\n"));
    return USB_FAILURE;
}
...
LOCAL void vpoxUsbSolarisDeviceRestore(vpoxusb_state_t *pState)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDeviceRestore: pState=%p\n", pState));
    ...
    if (rc != USB_SUCCESS)
    {
        ...
        vpoxUsbSolarisPowerIdle(pState);
        Log((DEVICE_NAME ": vpoxUsbSolarisDeviceRestore: Not the same device\n"));
        return;
    }
...
LOCAL int vpoxUsbSolarisDeviceSuspend(vpoxusb_state_t *pState)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDeviceSuspend: pState=%p\n", pState));
    ...
    switch (pState->DevState)
    {
        ...
        {
            LogRel((DEVICE_NAME ": vpoxUsbSolarisDeviceSuspend: Invalid device state %d\n", pState->DevState));
            break;
        }
        ...
        case USB_DEV_PWRED_DOWN:
        {
             ...
             * Drain pending URBs.
             */
            for (int i = 0; i < VPOXUSB_DRAIN_TIME; i++)
            {
            ...
            vpoxUsbSolarisCloseAllPipes(pState, true /* default pipe */);
            vpoxUsbSolarisNotifyUnplug(pState);
    ...
    mutex_exit(&pState->Mtx);
    Log((DEVICE_NAME ": vpoxUsbSolarisDeviceSuspend: Returns %d\n", rc));
    return rc;
}
...
LOCAL void vpoxUsbSolarisDeviceResume(vpoxusb_state_t *pState)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisDeviceResume: pState=%p\n", pState));
    return vpoxUsbSolarisDeviceRestore(pState);
}
...
LOCAL void vpoxUsbSolarisPowerBusy(vpoxusb_state_t *pState)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisPowerBusy: pState=%p\n", pState));
    AssertPtrReturnVoid(pState);
    ...
    if (pState->pPower)
    {
        ...
        if (rc != DDI_SUCCESS)
        {
            Log((DEVICE_NAME ": vpoxUsbSolarisPowerBusy: Busy component failed! rc=%d\n", rc));
            mutex_enter(&pState->Mtx);
            pState->pPower->PowerBusy--;
            mutex_exit(&pState->Mtx);
        }
...
LOCAL void vpoxUsbSolarisPowerIdle(vpoxusb_state_t *pState)
{
    LogFunc((DEVICE_NAME ": vpoxUsbSolarisPowerIdle: pState=%p\n", pState));
    AssertPtrReturnVoid(pState);
    ...
    if (pState->pPower)
    {
        ...
        if (rc == DDI_SUCCESS)
        {
            ...
            Log((DEVICE_NAME ": vpoxUsbSolarisPowerIdle: Idle component failed! rc=%d\n", rc));
    }
}
