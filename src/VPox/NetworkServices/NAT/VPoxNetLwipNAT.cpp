/* $Id: VPoxNetLwipNAT.cpp $ */
/** @file
 * VPoxNetNAT - NAT Service for connecting to IntNet.
 */
...
#if defined(VPOX_RAWSOCK_DEBUG_HELPER)          \
    && (defined(VPOX_WITH_HARDENING)            \
        || defined(RT_OS_WINDOWS)               \
        || defined(RT_OS_DARWIN))
# error Have you forgotten to turn off VPOX_RAWSOCK_DEBUG_HELPER?
#endif

#ifdef VPOX_RAWSOCK_DEBUG_HELPER
extern "C" int getrawsock(int type);
#endif

#include "../NetLib/VPoxPortForwardString.h"
...
static int vpoxNetNATLogInit(int argc, char **argv);


class VPoxNetLwipNAT: public VPoxNetBaseService, public NATNetworkEventAdapter
{
    friend class NATNetworkListener;
  public:
    VPoxNetLwipNAT(SOCKET icmpsock4, SOCKET icmpsock6);
    virtual ~VPoxNetLwipNAT();
    void usage(){                /** @todo should be implemented */ };
    int run();
    virtual int init(void);
    virtual int parseOpt(int rc, const RTGETOPTUNION& getOptVal);
    /* VPoxNetNAT always needs Main */
   ...
   private:
    ...
    ComNatListenerPtr m_VPoxListener;
    ComNatListenerPtr m_VPoxClientListener;
...
static VPoxNetLwipNAT *g_pLwipNat;
INTNETSEG VPoxNetLwipNAT::aXmitSeg[64];

/**
 * @note: this work on Event thread.
 */
HRESULT VPoxNetLwipNAT::HandleEvent(VPoxEventType_T aEventType, IEvent *pEvent)
{
    HRESULT hrc = S_OK;
    switch (aEventType)
    {
        case VPoxEventType_OnNATNetworkSetting:
        {
        ...
        case VPoxEventType_OnNATNetworkPortForward:
        {
        ...
        case VPoxEventType_OnNATNetworkStartStop:
        {
        ...
        {
            LogRel(("VPoxSVC became unavailable, exiting.\n"));
            shutdown();
            break;
        }
...
/*static*/ DECLCALLBACK(void) VPoxNetLwipNAT::onLwipTcpIpInit(void *arg)
{
    AssertPtrReturnVoid(arg);
    VPoxNetLwipNAT *pNat = static_cast<VPoxNetLwipNAT *>(arg);
    ...
    netif *pNetif = netif_add(&g_pLwipNat->m_LwipNetIf /* Lwip Interface */,
                              ...
                              g_pLwipNat /* state */,
                              VPoxNetLwipNAT::netifInit /* netif_init_fn */,
...
/*static*/ DECLCALLBACK(void) VPoxNetLwipNAT::onLwipTcpIpFini(void* arg)
{
    AssertPtrReturnVoid(arg);
 ...
 */
/*static*/ err_t VPoxNetLwipNAT::netifInit(netif *pNetif)
{
    err_t rcLwip = ERR_OK;

    AssertPtrReturn(pNetif, ERR_ARG);

    VPoxNetLwipNAT *pNat = static_cast<VPoxNetLwipNAT *>(pNetif->state);
...
/*static*/ err_t VPoxNetLwipNAT::netifLinkoutput(netif *pNetif, pbuf *pPBuf)
{
    AssertPtrReturn(pNetif, ERR_ARG);
    AssertPtrReturn(pPBuf, ERR_ARG);

    VPoxNetLwipNAT *self = static_cast<VPoxNetLwipNAT *>(pNetif->state);
    ...
    RT_ZERO(VPoxNetLwipNAT::aXmitSeg);

    size_t idx = 0;
    for (struct pbuf *q = pPBuf; q != NULL; q = q->next, ++idx)
    {
        AssertReturn(idx < RT_ELEMENTS(VPoxNetLwipNAT::aXmitSeg), ERR_MEM);

#if ETH_PAD_SIZE
        if (q == pPBuf)
        {
            VPoxNetLwipNAT::aXmitSeg[idx].pv = (uint8_t *)q->payload + ETH_PAD_SIZE;
            VPoxNetLwipNAT::aXmitSeg[idx].cb = q->len - ETH_PAD_SIZE;
        }
        else
#endif
        {
            VPoxNetLwipNAT::aXmitSeg[idx].pv = q->payload;
            VPoxNetLwipNAT::aXmitSeg[idx].cb = q->len;
        }
    }

    int rc = self->sendBufferOnWire(VPoxNetLwipNAT::aXmitSeg, idx,
...
VPoxNetLwipNAT::VPoxNetLwipNAT(SOCKET icmpsock4, SOCKET icmpsock6) : VPoxNetBaseService("VPoxNetNAT", "nat-network")
{
    LogFlowFuncEnter();
...
/*static*/ int VPoxNetLwipNAT::natServicePfRegister(NATSEVICEPORTFORWARDRULE& natPf)
{
    int lrc;
...
/*static*/ int VPoxNetLwipNAT::natServiceProcessRegisteredPf(VECNATSERVICEPF& vecRules)
{
    ITERATORNATSERVICEPF it;
...
int VPoxNetLwipNAT::init()
{
    ...
    /* virtualpox initialized in super class */
    rc = VPoxNetBaseService::init();
    if (RT_FAILURE(rc))
        return rc;

    const std::string &networkName = getNetworkName();
    hrc = virtualpox->FindNATNetworkByName(com::Bstr(networkName.c_str()).raw(),
                                           m_net.asOutParam());
    if (FAILED(hrc))
    {
        reportComError(virtualpox, "FindNATNetworkByName", hrc);
        return VERR_NOT_FOUND;
    }

    {
        ComEventTypeArray eventTypes;
        eventTypes.push_back(VPoxEventType_OnNATNetworkPortForward);
        eventTypes.push_back(VPoxEventType_OnNATNetworkSetting);
        rc = createNatListener(m_NatListener, virtualpox, this, eventTypes);
        AssertRCReturn(rc, rc);
    }


    // resolver changes are reported on vpox but are retrieved from
    // host so stash a pointer for future lookups
    hrc = virtualpox->COMGETTER(Host)(m_host.asOutParam());
    AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);

    {
        ComEventTypeArray eventTypes;
        eventTypes.push_back(VPoxEventType_OnHostNameResolutionConfigurationChange);
        eventTypes.push_back(VPoxEventType_OnNATNetworkStartStop);
        rc = createNatListener(m_VPoxListener, virtualpox, this, eventTypes);
        AssertRCReturn(rc, rc);
    }

    {
        ...
        rc = createClientListener(m_VPoxClientListener, virtualpoxClient, this, eventTypes);
        AssertRCReturn(rc, rc);
    }
    ...
    hrc = virtualpox->GetExtraData(bstrSourceIp4Key.raw(), bstrSourceIpX.asOutParam());
    ...
    hrc = virtualpox->GetExtraData(bstrSourceIp6Key.raw(), bstrSourceIpX.asOutParam());
    if (SUCCEEDED(hrc) && bstrSourceIpX.isNotEmpty())
    {
        RTNETADDRIPV6 addr;
        char *pszZone = NULL;
    ...
    com::Bstr bstr;
    hrc = virtualpox->COMGETTER(HomeFolder)(bstr.asOutParam());
    AssertComRCReturn(hrc, VERR_NOT_FOUND);
    if (!bstr.isEmpty())
    {
    ...
    vpoxLwipCoreInitialize(VPoxNetLwipNAT::onLwipTcpIpInit, this);
...
const char **VPoxNetLwipNAT::getHostNameservers()
{
    if (m_host.isNull())
        return NULL;
...
int VPoxNetLwipNAT::parseOpt(int rc, const RTGETOPTUNION& Val)
{
    switch (rc)
    {
        case 'p':
...
int VPoxNetLwipNAT::processFrame(void *pvFrame, size_t cbFrame)
{
...
int VPoxNetLwipNAT::processGSO(PCPDMNETWORKGSO pGso, size_t cbFrame)
{
...
int VPoxNetLwipNAT::run()
{
    /* Father starts receiving thread and enter event loop. */
    VPoxNetBaseService::run();

    vpoxLwipCoreFinalize(VPoxNetLwipNAT::onLwipTcpIpFini, this);

    m_vecPortForwardRule4.clear();
    m_vecPortForwardRule6.clear();

    destroyNatListener(m_NatListener, virtualpox);
    destroyNatListener(m_VPoxListener, virtualpox);
    destroyClientListener(m_VPoxClientListener, virtualpoxClient);
...
/* static */
HRESULT VPoxNetLwipNAT::reportComError(ComPtr<IUnknown> iface,
                                       const com::Utf8Str &strContext,
                                       HRESULT hrc)
{
...
/* static */
void VPoxNetLwipNAT::reportErrorInfoList(const com::ErrorInfo &info,
                                         const com::Utf8Str &strContext)
{
    if (strContext.isNotEmpty())
...
/* static */
void VPoxNetLwipNAT::reportErrorInfo(const com::ErrorInfo &info)
{
...
/* static */
void VPoxNetLwipNAT::reportError(const char *a_pcszFormat, ...)
{
    va_list ap;
...
extern "C" DECLEXPORT(int) TrustedMain(int argc, char **argv, char **envp)
{
    ...
    if (FAILED(hrc))
    {
        ...
        if (hrc == NS_ERROR_FILE_ACCESS_DENIED)
        {
            ...
            int vrc = com::GetVPoxUserHomeDirectory(szHome, sizeof(szHome), false);
            if (RT_SUCCESS(vrc))
            {
    ...
    rc = vpoxNetNATLogInit(argc, argv);
    // shall we bail if we failed to init logging?

    g_pLwipNat = new VPoxNetLwipNAT(icmpsock4, icmpsock6);
...
static int vpoxNetNATLogInit(int argc, char **argv)
{
    ...
    char szHome[RTPATH_MAX];
    rc = com::GetVPoxUserHomeDirectory(szHome, sizeof(szHome), false);
    if (RT_FAILURE(rc))
        return rc;

    const char *pcszNetwork = NULL;

    // XXX: This duplicates information from VPoxNetBaseService.cpp.
    // Perhaps option definitions should be exported as public static
    // member of VPoxNetBaseService?
    ...
    cch = RTStrPrintf(szEnvVarBase, sizeof(szEnvVarBase),
                      "VPOXNET_%s_RELEASE_LOG", szNetwork);
    if (cch >= sizeof(szEnvVarBase))
        return VERR_BUFFER_OVERFLOW;

    rc = com::VPoxLogRelCreate("NAT Network",
                               szLogFile,
...
static LPCWSTR g_WndClassName = L"VPoxNetNatLwipClass";
