/* $Id: VPoxControl.cpp $ */
/** @file
 * VPoxControl - Guest Additions Command Line Management Interface.
 */
...
# include "../VPoxGuest/lib/VPoxGuestR3LibInternal.h" /* HACK ALERT! Using vbglR3DoIOCtl directly!! */
#endif
...
{
    /* Allow for up to 15 characters command name length (VPoxControl.exe) with
...
static RTEXITCODE usage(enum VPoxControlUsage eWhich = USAGE_ALL)
{
    RTPrintf("Usage:\n\n");
...
#if defined(RT_OS_WINDOWS) && !defined(VPOX_CONTROL_TEST)
 ...
 */
static RTEXITCODE VPoxControlError(const char *pszFormat, ...)
{
 ...
 */
static RTEXITCODE VPoxCtrlGetOptError(int ch, PCRTGETOPTUNION pValueUnion)
{
    /** @todo prefix with current command. */
 ...
 * @param   ...         Format arguments.
 */
static RTEXITCODE VPoxControlSyntaxError(const char *pszFormat, ...)
{
    /** @todo prefix with current command. */
    va_list va;
    va_start(va, pszFormat);
...
static BOOL ResizeDisplayDevice(ULONG Id, DWORD Width, DWORD Height, DWORD BitsPerPixel)
{
    ...
    {
        Log(("VPoxDisplayThread : already at desired resolution.\n"));
        return FALSE;
    }
...
static DECLCALLBACK(RTEXITCODE) handleSetVideoMode(int argc, char *argv[])
{
    ...
    if (hmodUser)
    {
        ...
        Log(("VPoxService: g_pfnChangeDisplaySettingsExA=%p g_pfnChangeDisplaySettingsA=%p g_pfnEnumDisplaySettingsA=%p\n",
            ...
            && g_pfnEnumDisplaySettingsA)
        {
            ...
            VPoxControlError("Error retrieving API for display change!");
    }
    else
        VPoxControlError("Error retrieving handle to user32.dll!");

    return RTEXITCODE_SUCCESS;
}

static int checkVPoxVideoKey(HKEY hkeyVideo)
{
    ...
    if (status == ERROR_SUCCESS)
    {
        ...
        static char s_szDeviceDescription[] = "VirtualPox Graphics Adapter";
...
static HKEY getVideoKey(bool writable)
{
    ...
    {
        VPoxControlError("Error opening video device map registry key!\n");
        return 0;
    }
    ...
    /*
     * Scan all '\Device\VideoX' REG_SZ keys to find VPox video driver entry.
    ...
    for (iDevice = 0; iDevice < cDevices; iDevice++)
    {
            ...
            && _strnicmp(szVideoLocation, "\\REGISTRY\\Machine", 17) == 0)
        {
            ...
            if (status == ERROR_SUCCESS)
            {
                int rc = checkVPoxVideoKey(hkeyVideo);
                if (RT_SUCCESS(rc))
                {
    ...
    if (hkeyVideo == 0)
    {
        VPoxControlError("Error opening video registry key!\n");
    }
...
static DECLCALLBACK(RTEXITCODE) handleSetVideoAcceleration(int argc, char *argv[])
{
    ...
    if (hkeyVideo)
    {
        ...
        if (status != ERROR_SUCCESS)
        {
            VPoxControlError("Error %d writing video acceleration status!\n", status);
        }
        RegCloseKey(hkeyVideo);
    }
    return RTEXITCODE_SUCCESS;
...
static DECLCALLBACK(RTEXITCODE) videoFlagsGet(void)
{
    ...
    if (hkeyVideo)
    {
        ...
        DWORD dwKeyType;
        ULONG status = RegQueryValueExA(hkeyVideo, "VPoxVideoFlags", NULL, &dwKeyType, (LPBYTE)&dwFlags, &cbValue);
        if (status != ERROR_SUCCESS)
...
static DECLCALLBACK(RTEXITCODE) videoFlagsDelete(void)
{
    ...
    if (hkeyVideo)
    {
        ULONG status = RegDeleteValueA(hkeyVideo, "VPoxVideoFlags");
        if (status != ERROR_SUCCESS)
            VPoxControlError("Error %d deleting video flags.\n", status);
        RegCloseKey(hkeyVideo);
        return RTEXITCODE_SUCCESS;
    }
...
static DECLCALLBACK(RTEXITCODE) videoFlagsModify(bool fSet, int argc, char *argv[])
{
    if (argc != 1)
    {
        VPoxControlError("Mask required.\n");
        return RTEXITCODE_FAILURE;
    }
    ...
    if (RT_FAILURE(rc))
    {
        VPoxControlError("Invalid video flags mask.\n");
        return RTEXITCODE_FAILURE;
    }
    ...
    if (hkeyVideo)
    {
        ...
        status = RegSetValueExA(hkeyVideo, "VPoxVideoFlags", 0, REG_DWORD, (LPBYTE)&dwFlags, sizeof(dwFlags));
        if (status != ERROR_SUCCESS)
        {
            VPoxControlError("Error %d writing video flags.\n", status);
...
static DECLCALLBACK(RTEXITCODE) handleVideoFlags(int argc, char *argv[])
{
    ...
    if (argc != 1 && argc != 2)
    {
        VPoxControlError("Invalid number of arguments.\n");
        usage(VIDEO_FLAGS);
        return RTEXITCODE_FAILURE;
    }
    ...
    {
        VPoxControlError("Invalid command.\n");
        exitCode = RTEXITCODE_FAILURE;
    }
...
static DECLCALLBACK(RTEXITCODE) handleAddCustomMode(int argc, char *argv[])
{
    ...
    if (   (xres > (1 << 16))
        ...
        || (   (bpp != 16)
            ...
            && (bpp != 32)))
    {
        VPoxControlError("invalid mode specified!\n");
        return RTEXITCODE_FAILURE;
    }
 ...
 * This is accessed through the "VPoxGuestPropSvc" HGCM service.
 *
...
static RTEXITCODE getGuestProperty(int argc, char **argv)
{
    ...
    if (RT_FAILURE(rc))
        VPoxControlError("Failed to connect to the guest property service, error %Rrc\n", rc);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        for (unsigned i = 0; i < 10 && !fFinished; ++i)
        {
            ...
            {
                rc = VERR_NO_MEMORY;
                VPoxControlError("Out of memory\n");
            }
            else
            {
                pvBuf = pvTmpBuf;
        ...
        if (VERR_TOO_MUCH_DATA == rc)
            VPoxControlError("Temporarily unable to retrieve the property\n");
        else if (RT_FAILURE(rc) && rc != VERR_NOT_FOUND)
            VPoxControlError("Failed to retrieve the property value, error %Rrc\n", rc);
    }
...
static RTEXITCODE setGuestProperty(int argc, char *argv[])
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
            VPoxControlError("Failed to store the property value, error %Rrc\n", rc);
    }
...
static RTEXITCODE deleteGuestProperty(int argc, char *argv[])
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
            VPoxControlError("Failed to delete the property value, error %Rrc\n", rc);
    }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            while (RT_SUCCESS(rc) && pszName)
            {
                ...
                if (RT_FAILURE(rc))
                    VPoxControlError("Error while enumerating guest properties: %Rrc\n", rc);
            }
            ...
            VPoxControlError("Failed to enumerate the guest properties! Error: %Rrc\n", rc);
        VbglR3GuestPropDisconnect(u32ClientId);
    }
    else
        VPoxControlError("Failed to connect to the guest property service! Error: %Rrc\n", rc);
...
static RTEXITCODE waitGuestProperty(int argc, char **argv)
{
    ...
    for (unsigned i = 0; (RT_SUCCESS(rc) || rc == VERR_BUFFER_OVERFLOW) && !fFinished && i < 10; i++)
    {
        ...
        if (rc == VERR_TOO_MUCH_DATA)
            VPoxControlError("Temporarily unable to get a notification\n");
        else if (rc == VERR_INTERRUPTED)
            VPoxControlError("The request timed out or was interrupted\n");
        else if (RT_FAILURE(rc) && rc != VERR_NOT_FOUND)
            VPoxControlError("Failed to get a notification, error %Rrc\n", rc);
    }
...
/**
 * Access the guest property store through the "VPoxGuestPropSvc" HGCM
 * service.
 *
...
static RTEXITCODE sharedFolder_list(int argc, char **argv)
{
    ...
    if (RT_FAILURE(rc))
        VPoxControlError("Failed to connect to the shared folder service, error %Rrc\n", rc);
    else
    {
                    ...
                    for (char chDrive = 'A'; chDrive <= 'Z'; chDrive++)
                    {
                        ...
                        if (rcOs2 == NO_ERROR)
                        {
                            ...
                            if (   uBuf.FsQueryBuf.iType == FSAT_REMOTEDRV
                                && RTStrICmpAscii(pszFsdName, "VPOXSF") == 0)
                            {
...
# endif
                    ...
                    VPoxControlError("Error while getting the shared folder name for root node = %u, rc = %Rrc\n",
            ...
            VPoxControlError("Error while getting the shared folder mappings, rc = %Rrc\n", rc);
...
static RTEXITCODE sharedFolder_use(int argc, char **argv)
{
     ...
     */
    if (argc != 2)
        return VPoxControlSyntaxError("sharedfolder use: expected a drive letter and a shared folder name\n");

    const char *pszDrive  = argv[0];
    if (!RT_C_IS_ALPHA(pszDrive[0]) || pszDrive[1] != ':' || pszDrive[2] != '\0')
        return VPoxControlSyntaxError("sharedfolder use: not a drive letter: %s\n", pszDrive);

    static const char s_szTag[] = "VPoxControl";
    char        szzNameAndTag[256];
    const char *pszName   = argv[1];
    size_t cchName = strlen(pszName);
    if (cchName < 1)
        return VPoxControlSyntaxError("sharedfolder use: shared folder name cannot be empty!\n");
    if (cchName + 1 + sizeof(s_szTag) >= sizeof(szzNameAndTag))
        return VPoxControlSyntaxError("sharedfolder use: shared folder name is too long! (%s)\n", pszName);
    ...
    APIRET rcOs2 = DosFSAttach(pszDrive, "VPOXSF", szzNameAndTag, cchName + 1 + sizeof(s_szTag), FS_ATTACH);
    if (rcOs2 == NO_ERROR)
        return RTEXITCODE_SUCCESS;
    if (rcOs2 == ERROR_INVALID_FSD_NAME)
        return VPoxControlError("Shared folders IFS not installed?\n");
    return VPoxControlError("DosFSAttach/FS_ATTACH failed to attach '%s' to '%s': %u\n", pszName, pszDrive, rcOs2);
}
...
static RTEXITCODE sharedFolder_unuse(int argc, char **argv)
{
     ...
     */
    if (argc != 1)
        return VPoxControlSyntaxError("sharedfolder unuse: expected drive letter\n");
    const char *pszDrive = argv[0];
    if (!RT_C_IS_ALPHA(pszDrive[0]) || pszDrive[1] != ':' || pszDrive[2] != '\0')
        return VPoxControlSyntaxError("sharedfolder unuse: not a drive letter: %s\n", pszDrive);

    /*
     * Do the detaching.
     */
    APIRET rcOs2 = DosFSAttach(pszDrive, "VPOXSF", NULL, 0, FS_DETACH);
    if (rcOs2 == NO_ERROR)
        return RTEXITCODE_SUCCESS;
    return VPoxControlError("DosFSAttach/FS_DETACH failed on '%s': %u\n", pszDrive, rcOs2);
}
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: writecoredump}
 */
static DECLCALLBACK(RTEXITCODE) handleWriteCoreDump(int argc, char *argv[])
{
    ...
    {
        VPoxControlError("Error while taking guest core dump. rc=%Rrc\n", rc);
        return RTEXITCODE_FAILURE;
    }
}
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: help}
 */
static DECLCALLBACK(RTEXITCODE) handleDpc(int argc, char *argv[])
{
    RT_NOREF(argc,  argv);
    int rc = VERR_NOT_IMPLEMENTED;
# ifndef VPOX_CONTROL_TEST
    for (int i = 0; i < 30; i++)
    {
        VBGLREQHDR Req;
...
# endif
    if (RT_FAILURE(rc))
        return VPoxControlError("Error. rc=%Rrc\n", rc);
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: writelog}
 */
        ...
        while ((ch = RTGetOpt(&GetOptState, &ValueUnion)) != 0)
        {
            switch (ch)
            {
                case VINF_GETOPT_NOT_OPTION:
                {
                    ...
                    if (RT_FAILURE(rc))
                        return VPoxControlError("VbglR3WriteLog: %Rrc", rc);
                    break;
                }
                ...
                default:
                    return VPoxCtrlGetOptError(ch, &ValueUnion);
            }
        }
    }
    else
        return VPoxControlError("RTGetOptInit: %Rrc", rc);
    return RTEXITCODE_SUCCESS;
}


/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: takesnapshot}
 */
static DECLCALLBACK(RTEXITCODE) handleTakeSnapshot(int argc, char *argv[])
{
    ...
    return VPoxControlError("not implemented");
}

/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: savestate}
 */
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: suspend|pause}
 */
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: poweroff|powerdown}
 */
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: version}
 */
static DECLCALLBACK(RTEXITCODE) handleVersion(int argc, char *argv[])
{
    RT_NOREF1(argv);
    if (argc)
        return VPoxControlSyntaxError("getversion does not take any arguments");
    return printVersion();
}
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: ls}
 */
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: tar}
 */
...
/**
 * @callback_method_impl{FNVPOXCTRLCMDHANDLER, Command: unzip}
 */
...
typedef DECLCALLBACK(RTEXITCODE) FNVPOXCTRLCMDHANDLER(int argc, char *argv[]);
typedef FNVPOXCTRLCMDHANDLER *PFNVPOXCTRLCMDHANDLER;

/** The table of all registered command handlers. */
struct COMMANDHANDLER
{
    const char             *pszCommand;
    PFNVPOXCTRLCMDHANDLER   pfnHandler;
    bool                    fNeedDevice;
} g_aCommandHandlers[] =
{
#if defined(RT_OS_WINDOWS) && !defined(VPOX_CONTROL_TEST)
...
int main(int argc, char **argv)
{
    ...
    if (fShowLogo)
        RTPrintf(VPOX_PRODUCT " Guest Additions Command Line Management Interface Version "
                 VPOX_VERSION_STRING "\n"
            ...
            for (i = 0; i < RT_ELEMENTS(g_aCommandHandlers); i++)
                if (!strcmp(argv[iArg], g_aCommandHandlers[i].pszCommand))
                {
                    if (g_aCommandHandlers[i].fNeedDevice)
                    {
                        ...
                        if (RT_FAILURE(rrc))
                        {
                            VPoxControlError("Could not contact the host system.  Make sure that you are running this\n"
                                             "application inside a VirtualPox guest system, and that you have sufficient\n"
                                             "user permissions.\n");
