...
#ifndef VPOX_INCLUDED_dbg_h
#define VPOX_INCLUDED_dbg_h
...
/** @defgroup grp_dbg       The VirtualPox Debugger
 * @{
 */
...
typedef struct DBGCCMDHLP
{
    ...
    /**
     * Command helper for formatting and error message for a VPox status code.
     *
     * @returns VPox status code appropriate to return from a command.
     * @param   pCmdHlp     Pointer to the command callback structure.
     ...
     */
    DECLCALLBACKMEMBER(int, pfnVPoxError)(PDBGCCMDHLP pCmdHlp, int rc, const char *pszFormat, ...) RT_IPRT_FORMAT_ATTR(3, 4);
     ...
     */
    DECLCALLBACKMEMBER(int, pfnVPoxErrorV)(PDBGCCMDHLP pCmdHlp, int rc,
                                           const char *pszFormat, va_list args) RT_IPRT_FORMAT_ATTR(3, 0);
     ...
     *
     * @returns VPox status code to pass upwards.
     *
     * @param   pCmdHlp     Pointer to the command callback structure.
...
/**
 * @copydoc DBGCCMDHLP::pfnVPoxError
 */
DECLINLINE(int) RT_IPRT_FORMAT_ATTR(3, 4) DBGCCmdHlpVPoxError(PDBGCCMDHLP pCmdHlp, int rc, const char *pszFormat, ...)
{
    va_list va;

    va_start(va, pszFormat);
    rc = pCmdHlp->pfnVPoxErrorV(pCmdHlp, rc, pszFormat, va);
    va_end(va);
