/* $Id: VPoxGuest-solaris.c $ */
/** @file
 * VirtualPox Guest Additions Driver for Solaris.
 */
...
#define DEVICE_DESC              "VirtualPox GstDrv"
...
static int vgdrvSolarisIOCtlSlow(PVPOXGUESTSESSION pSession, int iCmd, int Mode, intptr_t iArgs);
...
/** Additional IPRT function we need to drag in for vpoxfs. */
PFNRT g_Deps[] =
{
...
int _init(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = ddi_soft_state_init(&g_pvgdrvSolarisState, sizeof(vpoxguest_state_t), 1);
        if (!rc)
        {
...
static int vgdrvSolarisOpen(dev_t *pDev, int fFlags, int fType, cred_t *pCred)
{
     ...
     *       to the vpox group detection like on linux.  Read config instead?
     */
    if (!(fFlags & FKLYR))
    {
        ...
        fRequestor |= VMMDEV_REQUESTOR_NO_USER_DEVICE; /** @todo implement vpoxuser device node. */
...
static int vgdrvSolarisClose(dev_t Dev, int flag, int fType, cred_t *pCred)
{
    ...
    PVPOXGUESTSESSION pSession = NULL;
    vpoxguest_state_t *pState = ddi_get_soft_state(g_pvgdrvSolarisState, getminor(Dev));
    if (!pState)
    {
...
static int vgdrvSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    ...
    PVPOXGUESTSESSION pSession = pState->pSession;
 ...
 */
static int vgdrvSolarisIOCtlSlow(PVPOXGUESTSESSION pSession, int iCmd, int Mode, intptr_t iArg)
{
    int         rc;
    uint32_t    cbBuf = 0;
    union
...
static int vgdrvSolarisPoll(dev_t Dev, short fEvents, int fAnyYet, short *pReqEvents, struct pollhead **ppPollHead)
{
    ...
    if (RT_LIKELY(pState))
    {
        PVPOXGUESTSESSION pSession  = (PVPOXGUESTSESSION)pState->pSession;
        uint32_t u32CurSeq = ASMAtomicUoReadU32(&g_DevExt.u32MousePosChangedSeq);
        if (pSession->u32MousePosChangedSeq != u32CurSeq)
        {
                ...
                if (g_pahIntrs)
                {
                        ...
                        && cIntrAllocated == 1)
                    {
                        ...
                        if (rc == DDI_SUCCESS)
                        {
                            ...
                            cmn_err(CE_CONT, "!vpoxguest: uIntrPriority=%d hilevel_pri=%d\n", uIntrPriority, ddi_intr_get_hilevel_pri());
