...
typedef IKDClient *(*PFNCreateVPoxKDClientEx)(unsigned version);
...
static DECLCALLBACK(VPOXSTRICTRC) vkdPortRead(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t *pu32, unsigned cb)
{
...
static DECLCALLBACK(VPOXSTRICTRC) vkdPortWrite(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t u32, unsigned cb)
{
    RT_NOREF(pvUser, cb);
    VIRTUALKD *pThis = PDMDEVINS_2_DATA(pDevIns, VIRTUALKD *);
...
static DECLCALLBACK(int) vkdConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    if (RT_SUCCESS(rc))
    {
        PFNCreateVPoxKDClientEx pfnInit;
        rc = RTLdrGetSymbol(pThis->hLib, "CreateVPoxKDClientEx", (void **)&pfnInit);
        if (RT_SUCCESS(rc))
        {
