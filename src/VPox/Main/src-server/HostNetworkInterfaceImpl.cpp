...
{
    AssertReturn(mVirtualPox != NULL, E_POINTER);
    return mVirtualPox->SetExtraData(BstrFmt("HostOnly/{%RTuuid}/%s", mGuid, szParamName).raw(), Bstr(strValue).raw());
}

HRESULT HostNetworkInterface::eraseAdapterConfigParameter(const char *szParamName)
{
    AssertReturn(mVirtualPox != NULL, E_POINTER);
    return mVirtualPox->SetExtraData(BstrFmt("HostOnly/{%RTuuid}/%s", mGuid, szParamName).raw(), NULL);
}
...
bool HostNetworkInterface::isInConfigFile(void)
{
    ...
    Assert(mVirtualPox != NULL);
    if (mVirtualPox == NULL)
        return false; /* Trigger config update, which will fail with proper return code */
    Bstr tmpName;
    mVirtualPox->GetExtraData(BstrFmt("HostOnly/{%RTuuid}/Name", mGuid).raw(), tmpName.asOutParam());
                                                   ...
                                                   const com::Utf8Str &aNetworkMask)
{
    ...
    if (aIPAddress.isEmpty())
    {
        if (m.IPAddress)
        {
            int rc = NetIfEnableStaticIpConfig(mVirtualPox, this, m.IPAddress, 0, 0);
            if (RT_SUCCESS(rc))
            {
                m.realIPAddress = 0;
#if defined(RT_OS_WINDOWS)
                ...
                if (FAILED(mVirtualPox->SetExtraData(BstrFmt("HostOnly/%s/IPAddress",
                                                             mInterfaceName.c_str()).raw(), NULL)))
                    return E_FAIL;
                if (FAILED(mVirtualPox->SetExtraData(BstrFmt("HostOnly/%s/IPNetMask",
                                                             mInterfaceName.c_str()).raw(), NULL)))
                    return E_FAIL;
#endif /* !defined(RT_OS_WINDOWS) */
                return S_OK;
    ...
    if (ip != INADDR_NONE)
    {
        ...
        if (mask != INADDR_NONE)
        {
            if (m.realIPAddress == ip && m.realNetworkMask == mask)
                return S_OK;
            int rc = NetIfEnableStaticIpConfig(mVirtualPox, this, m.IPAddress, ip, mask);
            if (RT_SUCCESS(rc))
            {
                m.realIPAddress   = ip;
                                                     ...
                                                     ULONG aIPV6NetworkMaskPrefixLength)
{
    ...
    if (aIPV6NetworkMaskPrefixLength > 128)
        return mVirtualPox->setErrorBoth(E_INVALIDARG, VERR_INVALID_PARAMETER,
    ...
    if (RT_FAILURE(rc))
    {
        return mVirtualPox->setErrorBoth(E_INVALIDARG, rc, "Invalid IPv6 address");
    }
        ...
        || m.realIPV6PrefixLength != aIPV6NetworkMaskPrefixLength)
    {
        ...
        rc = NetIfEnableStaticIpConfigV6(mVirtualPox, this, m.IPV6Address.c_str(),
                                         aIPV6Address.c_str(),
...
#else /* !defined(RT_OS_WINDOWS) */
            if (FAILED(mVirtualPox->SetExtraData(BstrFmt("HostOnly/%s/IPV6Address",
                                                         mInterfaceName.c_str()).raw(),
                                                 Bstr(aIPV6Address).raw())))
                return E_FAIL;
            if (FAILED(mVirtualPox->SetExtraData(BstrFmt("HostOnly/%s/IPV6NetMask",
                                                         mInterfaceName.c_str()).raw(),
...
HRESULT HostNetworkInterface::enableDynamicIPConfig()
{
    ...
    return E_NOTIMPL;
#else
    int rc = NetIfEnableDynamicIpConfig(mVirtualPox, this);
    if (RT_FAILURE(rc))
    {
...
HRESULT HostNetworkInterface::dHCPRediscover()
{
    ...
    return E_NOTIMPL;
#else
    int rc = NetIfDhcpRediscover(mVirtualPox, this);
    if (RT_FAILURE(rc))
    {
...
HRESULT HostNetworkInterface::i_setVirtualPox(VirtualPox *pVirtualPox)
{
    AutoCaller autoCaller(this);
    if (FAILED(autoCaller.rc())) return autoCaller.rc();

    AssertReturn(mVirtualPox != pVirtualPox, S_OK);

    unconst(mVirtualPox) = pVirtualPox;
    ...
    if (m.IPAddress == 0 && mIfType == HostNetworkInterfaceType_HostOnly)
    {
        ...
        HRESULT hrc = mVirtualPox->GetExtraData(BstrFmt("HostOnly/%s/IPAddress",
                                                        mInterfaceName.c_str()).raw(),
                                                tmpAddr.asOutParam());
        if (FAILED(hrc) || tmpAddr.isEmpty())
            tmpAddr = getDefaultIPv4Address(mInterfaceName);

        hrc = mVirtualPox->GetExtraData(BstrFmt("HostOnly/%s/IPNetMask",
                                                mInterfaceName.c_str()).raw(),
                                        tmpMask.asOutParam());
        if (FAILED(hrc) || tmpMask.isEmpty())
            tmpMask = Bstr(VPOXNET_IPV4MASK_DEFAULT);
    ...
    if (m.IPV6Address.isEmpty())
    {
        Bstr bstrIPV4Addr;
        Bstr tmpPrefixLen;
        HRESULT hrc = mVirtualPox->GetExtraData(BstrFmt("HostOnly/%s/IPV6Address",
                                                        mInterfaceName.c_str()).raw(),
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            if (!m.IPV6Address.isEmpty())
            {
                hrc = mVirtualPox->GetExtraData(BstrFmt("HostOnly/%s/IPV6PrefixLen",
                                                        mInterfaceName.c_str()).raw(),
