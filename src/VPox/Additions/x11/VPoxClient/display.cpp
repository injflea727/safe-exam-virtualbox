...
/** State information needed for the service.  The main VPoxClient code provides
...
static void doResize(struct DISPLAYSTATE *pState)
{
    ...
    if (!pState->fHaveRandR12)
    {
        ...
        pData = getRootProperty(pState, "VPOXVIDEO_PREFERRED_MODE", 1, XA_INTEGER);
        if (pData != NULL)
        {
...
static void runDisplay(struct DISPLAYSTATE *pState)
{
    ...
    /* Semantics: when VPOXCLIENT_STARTED is set, pre-1.3 X.Org Server driver
     ...
     */
    XChangeProperty(pState->pDisplay, DefaultRootWindow(pState->pDisplay), XInternAtom(pState->pDisplay, "VPOXCLIENT_STARTED", 0),
    ...
    while (true)
    {
        ...
        while (XPending(pDisplay))
        {
            ...
            if (   !pState->fHaveRandR12
                ...
                && event.xproperty.atom == XInternAtom(pDisplay, "VPOXVIDEO_NO_VT", False))
                doResize(pState);
            if (   !pState->fHaveRandR12
                && event.type == PropertyNotify
                && event.xproperty.state == PropertyNewValue
                && event.xproperty.window == DefaultRootWindow(pDisplay)
                && event.xproperty.atom == XInternAtom(pDisplay, "VPOXVIDEO_PREFERRED_MODE", False))
                doResize(pState);
            if (   pState->fHaveRandR12
