...
/** @file
 * VPox Qt GUI - UIUpdateManager class implementation.
 */
...
//#define VPOX_NEW_VERSION_TEST "5.1.12_0 http://unknown.unknown.org/0.0.0/VirtualPox-0.0.0-0-unknown.pkg"
...
/** Update step subclass to check for the new VirtualPox version. */
class UIUpdateStepVirtualPox : public UIUpdateStep
{
    Q_OBJECT;
...
public:
    ...
    /** Constructs update step passing @a pQueue and @a fForceCall to the base-class. */
    UIUpdateStepVirtualPox(UIUpdateQueue *pQueue, bool fForceCall)
        : UIUpdateStep(pQueue, fForceCall)
        , m_url("https://update.virtualpox.org/query.php")
    {}
...
/** Update step subclass to check for the new VirtualPox Extension Pack version. */
class UIUpdateStepVirtualPoxExtensionPack : public UIUpdateStep
{
    Q_OBJECT;
...
public:
    ...
    /** Constructs update step passing @a pQueue and @a fForceCall to the base-class. */
    UIUpdateStepVirtualPoxExtensionPack(UIUpdateQueue *pQueue, bool fForceCall)
        : UIUpdateStep(pQueue, fForceCall)
    {}
...
/*********************************************************************************************************************************
*   Class UIUpdateStepVirtualPox implementation.                                                                                 *
*********************************************************************************************************************************/

void UIUpdateStepVirtualPox::processNetworkReplyCanceled(UINetworkReply *pReply)
{
    Q_UNUSED(pReply);
...
void UIUpdateStepVirtualPox::processNetworkReplyFinished(UINetworkReply *pReply)
{
    /* Deserialize incoming data: */
    QString strResponseData(pReply->readAll());

#ifdef VPOX_NEW_VERSION_TEST
    strResponseData = VPOX_NEW_VERSION_TEST;
#endif
...
const QString UIUpdateStepVirtualPox::description() const
{
    return tr("Checking for a new VirtualPox version...");
}

void UIUpdateStepVirtualPox::sltStartStep()
{
    /* Compose query: */
    QUrlQuery url;
    url.addQueryItem("platform", uiCommon().virtualPox().GetPackageType());
    ...
    if (uiCommon().brandingIsActive())
    {
        ...
        url.addQueryItem("version", QString("%1_%2_%3").arg(uiCommon().virtualPox().GetVersion())
                                                       .arg(uiCommon().virtualPox().GetRevision())
                                                       .arg(uiCommon().brandingGetKey("VerSuffix")));
    }
    else
    {
        /* Use hard coded version set by VPOX_VERSION_STRING: */
        url.addQueryItem("version", QString("%1_%2").arg(uiCommon().virtualPox().GetVersion())
                                                    .arg(uiCommon().virtualPox().GetRevision()));
    }
    url.addQueryItem("count", QString::number(gEDataManager->applicationUpdateCheckCounter()));
    url.addQueryItem("branch", VPoxUpdateData(gEDataManager->applicationUpdateData()).branchName());
    QString strUserAgent(QString("VirtualPox %1 <%2>").arg(uiCommon().virtualPox().GetVersion()).arg(platformInfo()));
...
/* static */
QString UIUpdateStepVirtualPox::platformInfo()
{
    ...
    {
        /* Run script: */
        QByteArray result = QIProcess::singleShot(QString(szAppPrivPath) + "/VPoxSysInfo.sh");
        if (!result.isNull())
...
/*********************************************************************************************************************************
*   Class UIUpdateStepVirtualPoxExtensionPack implementation.                                                                    *
*********************************************************************************************************************************/

void UIUpdateStepVirtualPoxExtensionPack::sltStartStep()
{
    ...
    /* Get extension pack: */
    CExtPack extPack = uiCommon().virtualPox().GetExtensionPackManager().Find(GUI_ExtPackName);
    ...
    /* Get updated VirtualPox version: */
    const QString strVPoxVersion = vpoxVersion.toString();

    /* Skip the check if the extension pack is equal to or newer than VPox. */
    if (UIVersion(strExtPackVersion) >= vpoxVersion)
    {
        emit sigStepComplete();
        return;
    }
    ...
    if (strExtPackEdition.contains("ENTERPRISE"))
    {
        ...
        msgCenter().askUserToDownloadExtensionPack(GUI_ExtPackName, strExtPackVersion, strVPoxVersion);
    ...
    connect(pDl, &UIDownloaderExtensionPack::sigDownloadFinished,
            this, &UIUpdateStepVirtualPoxExtensionPack::sltHandleDownloadedExtensionPack);
    /* Also, destroyed downloader is a signal to finish the step: */
    connect(pDl, &UIDownloaderExtensionPack::destroyed,
            this, &UIUpdateStepVirtualPoxExtensionPack::sigStepComplete);
    /* Start downloading: */
    pDl->start();
}

void UIUpdateStepVirtualPoxExtensionPack::sltHandleDownloadedExtensionPack(const QString &strSource,
                                                                           const QString &strTarget,
                                                                           const QString &strDigest)
{
    ...
    if (msgCenter().proposeDeleteExtentionPack(QDir::toNativeSeparators(strTarget)))
    {
        ...
        /* Get the list of old extension pack files in VirtualPox homefolder: */
        const QStringList oldExtPackFiles = QDir(uiCommon().homeFolder()).entryList(QStringList("*.vpox-extpack"),
                                                                                      QDir::Files);
...
#ifdef VPOX_WITH_UPDATE_REQUEST
...
#endif /* VPOX_WITH_UPDATE_REQUEST */
}
...
void UIUpdateManager::sltCheckIfUpdateIsNecessary(bool fForceCall /* = false */)
{
    ...
    VPoxUpdateData currentData(gEDataManager->applicationUpdateData());
        ...
        fForceCall || currentData.isNeedToCheck())
    {
        ...
        new UIUpdateStepVirtualPox(m_pQueue, fForceCall);
        new UIUpdateStepVirtualPoxExtensionPack(m_pQueue, fForceCall);
        /* Start update queue: */
        m_pQueue->start();
    }
    else
...
void UIUpdateManager::sltHandleUpdateFinishing()
{
    ...
    VPoxUpdateData newData(currentData.periodIndex(), currentData.branchIndex());
