...
#define TST_VPOXTRAYSHOWBALLONMSG_NAME "VPoxTrayShowBallonMsg"
...
int main()
{
    ...
    int rc = RTPathAppend(szGuestInstallHelperDll, sizeof(szGuestInstallHelperDll),
                          "../../../repack/resources/VPoxGuestInstallHelper/VPoxGuestInstallHelper.dll");
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            /*
             * Tests VPoxTrayShowBallonMsg
             */
            PNSIS_PLUGIN_FUNC pfnVPoxTrayShowBallonMsg;
            rc = RTLdrGetSymbol(hLdrMod, TST_VPOXTRAYSHOWBALLONMSG_NAME, (void **)&pfnVPoxTrayShowBallonMsg);
            if (RT_SUCCESS(rc))
            {
                stack_t *pStack = NULL;
                ...
                pfnVPoxTrayShowBallonMsg(NULL /* hWnd */, NSIS_MAX_STRLEN, szVars, &pStack, NULL /* extra */);
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    RTTestIFailed("Sending message to VPoxTray failed (NSIS API changed?)\n");
                tstStackDestroy(pStack);
            }
            else
                RTTestIFailed("Loading pfnVPoxTrayShowBallonMsg failed: %Rrc", rc);
