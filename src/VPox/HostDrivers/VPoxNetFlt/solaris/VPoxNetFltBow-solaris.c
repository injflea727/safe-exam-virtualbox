...
*********************************************************************************************************************************/
/** The module name. */
#define DEVICE_NAME                     "vpoxbow"
/** The module descriptions as seen in 'modinfo'. */
#define DEVICE_DESC_DRV                 "VirtualPox NetBow"
/** The dynamically created VNIC name (hardcoded in NetIf-solaris.cpp).
 *  @todo move this define into a common header. */
#define VPOXBOW_VNIC_NAME               "vpoxvnic"
/** The VirtualPox VNIC template name (hardcoded in NetIf-solaris.cpp).
 *  @todo move this define into a common header. */
#define VPOXBOW_VNIC_TEMPLATE_NAME      "vpoxvnic_template"
/** Debugging switch for using symbols in kmdb */
# define LOCAL                          static
/** VPOXNETFLTVNIC::u32Magic */
# define VPOXNETFLTVNIC_MAGIC           0x0ddfaced
...
*   Kernel Entry Hooks                                                                                                           *
*********************************************************************************************************************************/
LOCAL int VPoxNetFltSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd);
LOCAL int VPoxNetFltSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd);
LOCAL int VPoxNetFltSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pArg, void **ppvResult);
 ...
 */
static struct modldrv g_VPoxNetFltSolarisModule =
{
...
static struct modlinkage g_VPoxNetFltSolarisModLinkage =
{
    MODREV_1,
    {
        &g_VPoxNetFltSolarisModule,
        NULL,
    }
};

/*
 * VPOXNETFLTVNICTEMPLATE: VNIC template information.
 */
typedef struct VPOXNETFLTVNICTEMPLATE
{
...
} VPOXNETFLTVNICTEMPLATE;
typedef struct VPOXNETFLTVNICTEMPLATE *PVPOXNETFLTVNICTEMPLATE;

/**
 * VPOXNETFLTVNIC: Per-VNIC instance data.
 */
typedef struct VPOXNETFLTVNIC
{
    /** Magic number (VPOXNETFLTVNIC_MAGIC). */
    uint32_t                    u32Magic;
    /** Whether we created the VNIC or not. */
    bool                        fCreated;
    /** Pointer to the VNIC template if any. */
    PVPOXNETFLTVNICTEMPLATE     pVNICTemplate;
    /** Pointer to the VirtualPox interface instance. */
    void                       *pvIf;
    /** The MAC handle. */
...
typedef struct VPOXNETFLTVNIC *PVPOXNETFLTVNIC;
...
static volatile uint64_t g_VPoxNetFltSolarisVNICId;


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
LOCAL mblk_t *vpoxNetFltSolarisMBlkFromSG(PVPOXNETFLTINS pThis, PINTNETSG pSG, uint32_t fDst);
LOCAL unsigned vpoxNetFltSolarisMBlkCalcSGSegs(PVPOXNETFLTINS pThis, mblk_t *pMsg);
LOCAL int vpoxNetFltSolarisMBlkToSG(PVPOXNETFLTINS pThis, mblk_t *pMsg, PINTNETSG pSG, unsigned cSegs, uint32_t fSrc);
LOCAL void vpoxNetFltSolarisRecv(void *pvData, mac_resource_handle_t hResource, mblk_t *pMsg, boolean_t fLoopback);
//LOCAL void vpoxNetFltSolarisAnalyzeMBlk(mblk_t *pMsg);
LOCAL int vpoxNetFltSolarisReportInfo(PVPOXNETFLTINS pThis, mac_handle_t hInterface, bool fIsVNIC);
LOCAL int vpoxNetFltSolarisInitVNIC(PVPOXNETFLTINS pThis, PVPOXNETFLTVNIC pVNIC);
LOCAL int vpoxNetFltSolarisInitVNICTemplate(PVPOXNETFLTINS pThis, PVPOXNETFLTVNICTEMPLATE pVNICTemplate);
LOCAL PVPOXNETFLTVNIC vpoxNetFltSolarisAllocVNIC(void);
LOCAL void vpoxNetFltSolarisFreeVNIC(PVPOXNETFLTVNIC pVNIC);
LOCAL void vpoxNetFltSolarisDestroyVNIC(PVPOXNETFLTVNIC pVNIC);
LOCAL int vpoxNetFltSolarisCreateVNIC(PVPOXNETFLTINS pThis, PVPOXNETFLTVNIC *ppVNIC);
DECLINLINE(int) vpoxNetFltSolarisGetLinkId(const char *pszMacName, datalink_id_t *pLinkId);
...
LOCAL int VPoxNetFltSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd)
{
    Log((DEVICE_NAME ":VPoxNetFltSolarisAttach pDip=%p enmCmd=%d\n", pDip, enmCmd));
...
LOCAL int VPoxNetFltSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd)
{
    Log((DEVICE_NAME ":VPoxNetFltSolarisDetach pDip=%p enmCmd=%d\n", pDip, enmCmd));
 ...
 * @returns corresponding solaris error code.
 */
LOCAL int VPoxNetFltSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pvArg, void **ppvResult)
{
    Log((DEVICE_NAME ":VPoxNetFltSolarisGetInfo pDip=%p enmCmd=%d pArg=%p instance=%d\n", pDip, enmCmd, getminor((dev_t)pvArg)));
 ...
 */
DECLINLINE(mblk_t *) vpoxNetFltSolarisMBlkFromSG(PVPOXNETFLTINS pThis, PINTNETSG pSG, uint32_t fDst)
{
    Log((DEVICE_NAME ":vpoxNetFltSolarisMBlkFromSG pThis=%p pSG=%p\n", pThis, pSG));
...
LOCAL int vpoxNetFltSolarisMBlkToSG(PVPOXNETFLTINS pThis, mblk_t *pMsg, PINTNETSG pSG, unsigned cSegs, uint32_t fSrc)
{
    Log((DEVICE_NAME ":vpoxNetFltSolarisMBlkToSG pThis=%p pMsg=%p pSG=%p cSegs=%d\n", pThis, pMsg, pSG, cSegs));
...
LOCAL void vpoxNetFltSolarisRecv(void *pvData, mac_resource_handle_t hResource, mblk_t *pMsg, boolean_t fLoopback)
{
    Log((DEVICE_NAME ":vpoxNetFltSolarisRecv pvData=%p pMsg=%p fLoopback=%d cbData=%d\n", pvData, pMsg, fLoopback,
         pMsg ? MBLKL(pMsg) : 0));
    ...
    if (   MBLKL(pMsg) >= sizeof(RTNETETHERHDR)
        && vpoxNetFltPortSolarisIsHostMac(pThis, &pEthHdr->SrcMac))
    ...
    {
        unsigned cSegs = vpoxNetFltSolarisMBlkCalcSGSegs(pThis, pCurMsg);
    ...
    if (RT_UNLIKELY(cFailed))
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisMBlkToSG failed for %u packets.\n", cFailed));
 ...
 *          here.
 */
LOCAL void vpoxNetFltSolarisLinkNotify(void *pvArg, mac_notify_type_t Type)
{
    LogRel((DEVICE_NAME ":vpoxNetFltSolarisLinkNotify pvArg=%p Type=%d\n", pvArg, Type));

    PVPOXNETFLTINS pThis = pvArg;
    ...
    switch (Type)
    {
        case MAC_NOTE_LINK:
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisLinkNotify link state change\n"));
            ...
            if (fDisconnectedFromHost != ASMAtomicUoReadBool(&pThis->fDisconnectedFromHost))
            {
                ...
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisLinkNotify link state change: new state=%s\n",
...
LOCAL int vpoxNetFltSolarisReportInfo(PVPOXNETFLTINS pThis, mac_handle_t hInterface, bool fIsVNIC)
{
    ...
    else
    {
        ...
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisReportInfo failed to get lower MAC handle for '%s'\n", pThis->szName));
     ...
     */
    pThis->u.s.hNotify = mac_notify_add(hLowerMac, vpoxNetFltSolarisLinkNotify, pThis);
    if (!pThis->u.s.hNotify)
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisReportInfo Warning! Failed to setup link notification hook.\n"));
#endif
    ...
    if (vpoxNetFltTryRetainBusyNotDisconnected(pThis))
    {
        ...
        Log((DEVICE_NAME ":vpoxNetFltSolarisReportInfo phys mac %.6Rhxs\n", &pThis->u.s.MacAddr));
        ...
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisReportInfo failed to retain interface. pThis=%p\n", pThis));
...
LOCAL int vpoxNetFltSolarisInitVNIC(PVPOXNETFLTINS pThis, PVPOXNETFLTVNIC pVNIC)
{
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        if (RT_LIKELY(!rc))
        {
            Log((DEVICE_NAME ":vpoxNetFltSolarisInitVNIC succesfully initialized VNIC.\n"));
            return VINF_SUCCESS;
        }
        else
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisInitVNIC mac_client_set_resources failed. rc=%d\n", rc));
        ...
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisInitVNIC failed to open mac client for '%s' rc=%d\n", pThis->szName, rc));
 ...
 */
LOCAL int vpoxNetFltSolarisGetLowerLinkName(mac_handle_t hVNICMacHandle, char *pszLowerLinkName)
{
    ...
    if (RT_LIKELY(hPhysLinkHandle))
    {
        ...
        int rc = vpoxNetFltSolarisGetLinkId(pszMacName, &PhysLinkId);
        if (RT_SUCCESS(rc))
        {
            ...
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisGetLowerLinkName failed to get link info. pszMacName=%s pszLowerLinkName=%s\n",
                    pszMacName, pszLowerLinkName));
            return VERR_INTNET_FLT_LOWER_LINK_INFO_NOT_FOUND;
        }

        LogRel((DEVICE_NAME ":vpoxNetFltSolarisGetLowerLinkName failed to get link id. pszMacName=%s pszLowerLinkName=%s\n",
                pszMacName, pszLowerLinkName));
        return VERR_INTNET_FLT_LOWER_LINK_ID_NOT_FOUND;
    }

    LogRel((DEVICE_NAME ":vpoxNetFltSolarisGetLowerLinkName failed to get lower-mac. pszLowerLinkName=%s\n", pszLowerLinkName));
...
LOCAL int vpoxNetFltSolarisInitVNICTemplate(PVPOXNETFLTINS pThis, PVPOXNETFLTVNICTEMPLATE pVNICTemplate)
{
    Log((DEVICE_NAME ":vpoxNetFltSolarisInitVNICTemplate pThis=%p pVNICTemplate=%p\n", pThis, pVNICTemplate));
    ...
    int rc = vpoxNetFltSolarisGetLinkId(pThis->szName, &VNICLinkId);
    if (RT_SUCCESS(rc))
    {
        /*
        ...
        if (!rc)
        {
            ...
            rc = vpoxNetFltSolarisGetLowerLinkName(hInterface, pVNICTemplate->szLinkName);
            if (RT_SUCCESS(rc))
            {
                /*
                ...
                if (RT_LIKELY(!rc))
                {
                    ...
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisInitVNICTemplate successfully init. VNIC template. szLinkName=%s "
                            "VLAN Id=%u\n", pVNICTemplate->szLinkName, pVNICTemplate->uVLANId));
                    return VINF_SUCCESS;
                }
                else
                {
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisInitVNICTemplate failed to open VNIC template. rc=%d\n", rc));
                    rc = VERR_INTNET_FLT_IF_FAILED;
                }
            }
            else
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisInitVNICTemplate failed to get lower linkname for VNIC template '%s'.\n",
                        pThis->szName));
        ...
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisInitVNICTemplate failed to open by link ID. rc=%d\n", rc));
            rc = VERR_INTNET_FLT_IF_FAILED;
        }
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisInitVNICTemplate failed to get VNIC template link Id. rc=%d\n", rc));
...
LOCAL PVPOXNETFLTVNIC vpoxNetFltSolarisAllocVNIC(void)
{
    PVPOXNETFLTVNIC pVNIC = RTMemAllocZ(sizeof(VPOXNETFLTVNIC));
    if (RT_UNLIKELY(!pVNIC))
        return NULL;

    pVNIC->u32Magic        = VPOXNETFLTVNIC_MAGIC;
 ...
 * @param   pVNIC           Pointer to the VNIC.
 */
DECLINLINE(void) vpoxNetFltSolarisFreeVNIC(PVPOXNETFLTVNIC pVNIC)
{
    RTMemFree(pVNIC);
}
...
LOCAL void vpoxNetFltSolarisDestroyVNIC(PVPOXNETFLTVNIC pVNIC)
{
    ...
    AssertMsgReturnVoid(pVNIC->u32Magic == VPOXNETFLTVNIC_MAGIC, ("pVNIC=%p u32Magic=%#x\n", pVNIC, pVNIC->u32Magic));
    if (pVNIC)
    {
        if (pVNIC->hClient)
        {
...
LOCAL int vpoxNetFltSolarisCreateVNIC(PVPOXNETFLTINS pThis, PVPOXNETFLTVNIC *ppVNIC)
{
    ...
    {
        pVNIC->pVNICTemplate = RTMemAllocZ(sizeof(VPOXNETFLTVNICTEMPLATE));
        if (RT_UNLIKELY(!pVNIC->pVNICTemplate))
        {
            vpoxNetFltSolarisFreeVNIC(pVNIC);
            return VERR_NO_MEMORY;
        }
         ...
         */
        rc = vpoxNetFltSolarisInitVNICTemplate(pThis, pVNIC->pVNICTemplate);
        if (RT_FAILURE(rc))
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisCreateVNIC failed to initialize VNIC from VNIC template. rc=%Rrc\n", rc));
            vpoxNetFltSolarisFreeVNIC(pVNIC);
            return rc;
        }
        ...
        if (uVLANId != VLAN_ID_NONE)
            fFlags |= MAC_VLAN;
#endif
        Log((DEVICE_NAME ":vpoxNetFltSolarisCreateVNIC pThis=%p VLAN Id=%u\n", pThis, uVLANId));
    }

    /*
     * Make sure the dynamic VNIC we're creating doesn't already exists, if so pick a new instance.
     * This is to avoid conflicts with users manually creating VNICs whose name starts with VPOXBOW_VNIC_NAME.
     */
    do
    {
        AssertCompile(sizeof(pVNIC->szName) > sizeof(VPOXBOW_VNIC_NAME "18446744073709551615" /* UINT64_MAX */));
        RTStrPrintf(pVNIC->szName, sizeof(pVNIC->szName), "%s%RU64", VPOXBOW_VNIC_NAME, g_VPoxNetFltSolarisVNICId);
        mac_handle_t hTmpMacHandle;
        rc = mac_open_by_linkname(pVNIC->szName, &hTmpMacHandle);
        if (rc)
            break;
        mac_close(hTmpMacHandle);
        ASMAtomicIncU64(&g_VPoxNetFltSolarisVNICId);
    } while (1);
    ...
    if (!rc)
    {
        ...
        if (!rc)
        {
             ...
             */
            rc = vpoxNetFltSolarisInitVNIC(pThis, pVNIC);
            if (RT_SUCCESS(rc))
            {
                Log((DEVICE_NAME ":vpoxNetFltSolarisCreateVNIC created VNIC '%s' over '%s' with random mac %.6Rhxs\n",
                     pVNIC->szName, pszLinkName, &GuestMac));
                *ppVNIC = pVNIC;
                return VINF_SUCCESS;
            }

            LogRel((DEVICE_NAME ":vpoxNetFltSolarisCreateVNIC vpoxNetFltSolarisInitVNIC failed. rc=%d\n", rc));
            mac_close(pVNIC->hInterface);
            pVNIC->hInterface = NULL;
        }
        else
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisCreateVNIC logrel failed to open VNIC '%s' over '%s'. rc=%d\n", pVNIC->szName,
                    pThis->szName, rc));
            rc = VERR_INTNET_FLT_VNIC_LINK_ID_NOT_FOUND;
        }

        vpoxNetFltSolarisDestroyVNIC(pVNIC);
    }
    else
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisCreateVNIC failed to create VNIC '%s' over '%s' rc=%d Diag=%d\n", pVNIC->szName,
                    pszLinkName, rc, Diag));
...
DECLINLINE(int) vpoxNetFltSolarisGetLinkId(const char *pszMacName, datalink_id_t *pLinkId)
{
    ...
    LogRel((DEVICE_NAME ":vpoxNetFltSolarisGetLinkId failed for '%s'. rc=%d\n", pszMacName, rc));
 ...
 * @returns VPox status code.
 */
DECLINLINE(int) vpoxNetFltSolarisSetPromisc(PVPOXNETFLTINS pThis, PVPOXNETFLTVNIC pVNIC)
{
    int rc = VINF_SUCCESS;
    if (!pVNIC->hPromisc)
    {
        rc = mac_promisc_add(pVNIC->hClient, MAC_CLIENT_PROMISC_FILTERED, vpoxNetFltSolarisRecv, pThis, &pVNIC->hPromisc,
                             MAC_PROMISC_FLAGS_NO_TX_LOOP | MAC_PROMISC_FLAGS_VLAN_TAG_STRIP | MAC_PROMISC_FLAGS_NO_PHYS);
        if (RT_UNLIKELY(rc))
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisSetPromisc failed. rc=%d\n", rc));
 ...
 * @param   pVNIC           Pointer to the VNIC.
 */
DECLINLINE(void) vpoxNetFltSolarisRemovePromisc(PVPOXNETFLTINS pThis, PVPOXNETFLTVNIC pVNIC)
{
    if (pVNIC->hPromisc)
    {
        mac_promisc_remove(pVNIC->hPromisc);
...
{
    Log((DEVICE_NAME ":vpoxNetFltPortOsSetActive pThis=%p fActive=%d\n", pThis, fActive));

    /*
     * Reactivate/quiesce the interface.
     */
    PVPOXNETFLTVNIC pVNIC = list_head(&pThis->u.s.hVNICs);
    if (fActive)
    {
        for (; pVNIC != NULL; pVNIC = list_next(&pThis->u.s.hVNICs, pVNIC))
            ...
            {
#if 0
                mac_rx_set(pVNIC->hClient, vpoxNetFltSolarisRecv, pThis);
#endif
                vpoxNetFltSolarisSetPromisc(pThis, pVNIC);
            }
    }
    else
    {
        for (; pVNIC != NULL; pVNIC = list_next(&pThis->u.s.hVNICs, pVNIC))
            if (pVNIC->hClient)
            {
                ...
                mac_rx_clear(pVNIC->hClient);
#endif
                vpoxNetFltSolarisRemovePromisc(pThis, pVNIC);
            }
    }
}
...
{
    Log((DEVICE_NAME ":vpoxNetFltOsDisconnectIt pThis=%p\n", pThis));
    return VINF_SUCCESS;
}
...
{
    Log((DEVICE_NAME ":vpoxNetFltOsConnectIt pThis=%p\n", pThis));
    return VINF_SUCCESS;
}
...
{
    Log((DEVICE_NAME ":vpoxNetFltOsDeleteInstance pThis=%p\n", pThis));
...
{
    Log((DEVICE_NAME ":vpoxNetFltOsInitInstance pThis=%p pvContext=%p\n", pThis, pvContext));

    /*
     * Figure out if the interface is a VNIC or a physical/etherstub/whatever NIC, then
     * do the actual VNIC creation if necessary in vpoxNetFltPortOsConnectInterface().
     */
    mac_handle_t hInterface;
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        if (!rc)
        {
            Log((DEVICE_NAME ":vpoxNetFltOsInitInstance pThis=%p physical interface '%s' detected.\n", pThis, pThis->szName));
            pThis->u.s.fIsVNIC = false;
        }
        else
        {
            pThis->u.s.fIsVNIC = true;
            if (RTStrNCmp(pThis->szName, VPOXBOW_VNIC_TEMPLATE_NAME, sizeof(VPOXBOW_VNIC_TEMPLATE_NAME) - 1) == 0)
            {
                Log((DEVICE_NAME ":vpoxNetFltOsInitInstance pThis=%p VNIC template '%s' detected.\n", pThis, pThis->szName));
        ...
        if (    pThis->u.s.fIsVNIC
            ...
            Log((DEVICE_NAME ":vpoxNetFltOsInitInstance pThis=%p VNIC '%s' detected.\n", pThis, pThis->szName));

        /*
         * Report info. (host MAC address, promiscuous, GSO capabilities etc.) to IntNet.
         */
        rc = vpoxNetFltSolarisReportInfo(pThis, hInterface, pThis->u.s.fIsVNIC);
        if (RT_FAILURE(rc))
            LogRel((DEVICE_NAME ":vpoxNetFltOsInitInstance failed to report info. rc=%d\n", rc));

        mac_close(hInterface);
    }
    else
    {
        LogRel((DEVICE_NAME ":vpoxNetFltOsInitInstance failed to open link '%s'! rc=%d\n", pThis->szName, rc));
        rc = VERR_INTNET_FLT_IF_FAILED;
    }
...
int vpoxNetFltOsPreInitInstance(PVPOXNETFLTINS pThis)
{
    ...
    list_create(&pThis->u.s.hVNICs, sizeof(VPOXNETFLTVNIC), offsetof(VPOXNETFLTVNIC, hNode));
...
int vpoxNetFltPortOsXmit(PVPOXNETFLTINS pThis, void *pvIfData, PINTNETSG pSG, uint32_t fDst)
{
     ...
     * Validate parameters.
     */
    PVPOXNETFLTVNIC pVNIC = pvIfData;
    AssertReturn(VALID_PTR(pVNIC), VERR_INVALID_POINTER);
    AssertMsgReturn(pVNIC->u32Magic == VPOXNETFLTVNIC_MAGIC,
    ...
    if (RT_LIKELY(pMsg))
    {
        Log((DEVICE_NAME ":vpoxNetFltPortOsXmit pThis=%p cbData=%d\n", pThis, MBLKL(pMsg)));
        ...
        LogRel((DEVICE_NAME ":vpoxNetFltPortOsXmit Xmit failed pVNIC=%p.\n", pVNIC));
    }
    else
    {
        LogRel((DEVICE_NAME ":vpoxNetFltPortOsXmit no memory for allocating Xmit packet.\n"));
        rc = VERR_NO_MEMORY;
    }
...
{
    Log((DEVICE_NAME ":vpoxNetFltPortOSNotifyMacAddress pszIf=%s pszVNIC=%s MAC=%.6Rhxs\n", pThis->szName,
             ((PVPOXNETFLTVNIC)pvIfData)->szName, pMac));

    /*
     * Validate parameters.
     */
    PVPOXNETFLTVNIC pVNIC = pvIfData;
    AssertMsgReturnVoid(VALID_PTR(pVNIC) && pVNIC->u32Magic == VPOXNETFLTVNIC_MAGIC,
                    ("Invalid pVNIC=%p magic=%#x (expected %#x)\n", pvIfData,
                     VALID_PTR(pVNIC) ? pVNIC->u32Magic : 0, VPOXNETFLTVNIC_MAGIC));
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        if (RT_LIKELY(!rc))
        {
            rc = vpoxNetFltSolarisSetPromisc(pThis, pVNIC);
#if 0
            if (RT_SUCCESS(rc))
            {
                /*
                 * Set the RX receive function.
                 * This shouldn't be necessary as vpoxNetFltPortOsSetActive() will be invoked after this, but in the future,
                 * if the guest NIC changes MAC address this may not be followed by a vpoxNetFltPortOsSetActive() call,
                 * so set it here anyway.
                 */
                mac_rx_set(pVNIC->hClient, vpoxNetFltSolarisRecv, pThis);
                Log((DEVICE_NAME ":vpoxNetFltPortOsNotifyMacAddress successfully added unicast address %.6Rhxs\n", pMac));
            }
            else
                LogRel((DEVICE_NAME ":vpoxNetFltPortOsNotifyMacAddress failed to set promiscuous mode. rc=%d\n", rc));
            mac_unicast_remove(pVNIC->hClient,  pVNIC->hUnicast);
            pVNIC->hUnicast = NULL;
#endif
        }
        ...
        {
            LogRel((DEVICE_NAME ":vpoxNetFltPortOsNotifyMacAddress failed to add primary unicast address. rc=%d Diag=%d\n", rc,
                    MacDiag));
        }
    }
    else
    {
        ...
        if (rc == ENOTSUP)
        {
            LogRel((DEVICE_NAME ":vpoxNetFltPortOsNotifyMacAddress: failed! a VNIC with mac %.6Rhxs probably already exists.",
                    pMac, rc));
            LogRel((DEVICE_NAME ":vpoxNetFltPortOsNotifyMacAddress: This NIC cannot establish connection. szName=%s szVNIC=%s\n",
                    pThis->szName, pVNIC->szName));
        }
        else
            LogRel((DEVICE_NAME ":vpoxNetFltPortOsNotifyMacAddress failed! mac %.6Rhxs rc=%d Diag=%d\n", pMac, rc, Diag));
    }
}
...
{
    Log((DEVICE_NAME ":vpoxNetFltPortOsConnectInterface pThis=%p pvIf=%p\n", pThis, pvIf));
    ...
    {
        PVPOXNETFLTVNIC pVNIC = NULL;
        rc = vpoxNetFltSolarisCreateVNIC(pThis, &pVNIC);
        if (RT_SUCCESS(rc))
        {
            /*
            ...
            *ppvIfData = pVNIC;
            ...
            LogRel((DEVICE_NAME ":vpoxNetFltPortOsConnectInterface failed to create VNIC rc=%d\n", rc));
    }
    else
    {
        /*
        ...
        if (RT_LIKELY(pVNIC))
        {
            ...
            if (!rc)
            {
                 ...
                 */
                rc = vpoxNetFltSolarisGetLinkId(pThis->szName, &pVNIC->hLinkId);
                if (RT_SUCCESS(rc))
                {
                    /*
                    ...
                    if (!rc)
                    {
                        ...
                        *ppvIfData = pVNIC;
                        ...
                        LogRel((DEVICE_NAME ":vpoxNetFltPortOsConnectInterface failed to initialize VNIC. rc=%d\n", rc));
                }
                else
                {
                    LogRel((DEVICE_NAME ":vpoxNetFltPortOsConnectInterface failed to get link id for '%s'. rc=%d\n",
                            pThis->szName, rc));
                }
            }
            else
            {
                LogRel((DEVICE_NAME ":vpoxNetFltPortOsConnectInterface failed to open VNIC '%s'. rc=%d\n", pThis->szName, rc));
                rc = VERR_OPEN_FAILED;
            }
        ...
        {
            LogRel((DEVICE_NAME ":vpoxNetFltOsInitInstance failed to allocate VNIC private data.\n"));
            rc = VERR_NO_MEMORY;
        }
    }
...
{
    Log((DEVICE_NAME ":vpoxNetFltPortOsDisconnectInterface pThis=%p\n", pThis));

    /*
     * It is possible we get called when vpoxNetFltPortOsConnectInterface() didn't succeed
    ...
    {
        AssertMsgReturn(pVNIC->u32Magic == VPOXNETFLTVNIC_MAGIC,
                        ("Invalid magic=%#x (expected %#x)\n", pVNIC->u32Magic, VPOXNETFLTVNIC_MAGIC), VERR_INVALID_POINTER);
            ...
            || pThis->u.s.fIsVNICTemplate)
        {
            ...
            Log((DEVICE_NAME ":vpoxNetFltPortOsDisconnectInterface destroying pVNIC=%p\n", pVNIC));
            vpoxNetFltSolarisDestroyVNIC(pVNIC);
            vpoxNetFltSolarisFreeVNIC(pVNIC);
        }
    }
