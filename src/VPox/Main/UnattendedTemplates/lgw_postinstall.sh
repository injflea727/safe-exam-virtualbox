...
MY_LOGFILE="${MY_TARGET}/var/log/vpoxpostinstall.log"
MY_CHROOT_CDROM="/cdrom"
MY_CDROM_NOCHROOT="/tmp/vpoxcdrom"
MY_EXITCODE=0
MY_DEBUG="" # "yes"
...
#
echo "******************************************************************************" >> "${MY_LOGFILE}"
echo "** VirtualPox Unattended Guest Installation - Late installation actions" >> "${MY_LOGFILE}"
echo "** Date:    `date -R`" >> "${MY_LOGFILE}"
...
if [ -f "${MY_TARGET}${MY_CHROOT_CDROM}/vpoxpostinstall.sh" ]; then
...
echo "proxy=@@VPOX_INSERT_PROXY@@" >> "${MY_TARGET}/etc/yum.conf"
@@VPOX_COND_END@@
...
echo "--------------------------------------------------" >> "${MY_LOGFILE}"
echo '** Installing VirtualPox Guest Additions...' | tee -a "${MY_LOGFILE}"
MY_IGNORE_EXITCODE=2  # returned if modules already loaded and reboot required.
log_command_in_target /bin/bash "${MY_CHROOT_CDROM}/vpoxadditions/VPoxLinuxAdditions.run" --nox11
...
MY_IGNORE_EXITCODE=
log_command_in_target usermod -a -G vpoxsf "@@VPOX_INSERT_USER_LOGIN@@"
@@VPOX_COND_END@@
...
log_command_in_target usermod -a -G wheel "@@VPOX_INSERT_USER_LOGIN@@"

echo "** Creating ${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/cloud-bridge.conf..." | tee -a "${MY_LOGFILE}"
cat >"${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/cloud-bridge.conf" <<'EOT'
# port 1194
# proto udp
port 443
proto tcp-server
...
echo "** Creating ${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/cloud-bridge.sh..." | tee -a "${MY_LOGFILE}"
cat >"${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/cloud-bridge.sh" <<'EOT'
...
EOT
log_command chmod +x "${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/cloud-bridge.sh"

echo "** Creating ${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/local-bridge.conf..." | tee -a "${MY_LOGFILE}"
cat >"${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/local-bridge.conf" <<'EOT'
dev tap0
# proto udp
# port 1194
proto tcp-client
...
echo "** Creating ${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/local-bridge.sh..." | tee -a "${MY_LOGFILE}"
cat >"${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/local-bridge.sh" <<'EOT'
...
sudo ip link set dev $br up
EOT
log_command chmod +x "${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/local-bridge.sh"

echo "** Creating ${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/.ssh/config..." | tee -a "${MY_LOGFILE}"
log_command mkdir "${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/.ssh"
cat >"${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/.ssh/config" <<'EOT'
Host *
    StrictHostKeyChecking no
EOT
log_command chmod 400 "${MY_TARGET}/home/@@VPOX_INSERT_USER_LOGIN@@/.ssh/config"

log_command_in_target chown -R @@VPOX_INSERT_USER_LOGIN@@:@@VPOX_INSERT_USER_LOGIN@@ "/home/@@VPOX_INSERT_USER_LOGIN@@"
...
After=vpoxadd.service

[Service]
ExecStart=/bin/sh -c 'su - vpox -c "cat /dev/zero | ssh-keygen -q -N \\\"\\\""'
ExecStartPost=/bin/sh -c 'VPoxControl guestproperty set "/VirtualPox/Gateway/PublicKey" "`cat ~vpox/.ssh/id_rsa.pub`" --flags TRANSIENT'
Type=oneshot
RemainAfterExit=yes
...
echo '** Creating /etc/sudoers.d/020_vpox_sudo...' | tee -a "${MY_LOGFILE}"
echo "@@VPOX_INSERT_USER_LOGIN@@ ALL=(ALL) NOPASSWD: ALL" > "${MY_TARGET}/etc/sudoers.d/020_vpox_sudo"
...
log_command_in_target test "${MY_CHROOT_CDROM}/vpoxvalidationkit/linux/@@VPOX_INSERT_OS_ARCH@@/TestExecService"
log_command mkdir -p "${MY_TARGET}/opt/validationkit" "${MY_TARGET}/media/cdrom"
log_command cp -R ${MY_CDROM_NOCHROOT}/vpoxvalidationkit/* "${MY_TARGET}/opt/validationkit/"
...
if [ -d "${MY_UNIT_PATH}" ]; then
    log_command cp "${MY_TARGET}/opt/validationkit/linux/vpoxtxs.service" "${MY_UNIT_PATH}/vpoxtxs.service"
    log_command chmod 644 "${MY_UNIT_PATH}/vpoxtxs.service"
    log_command_in_target systemctl -q enable vpoxtxs
...
# System V like:
    ...
    if [ -d "${MY_TARGET}/etc/rc.d/init.d/" ]; then
        ...
        log_command ln -s "../../../opt/validationkit/linux/vpoxtxs" "${MY_INIT_D_PARENT_PATH}/init.d/"
    else
    ...
    if chroot_which chkconfig; then     # Redhat based sysvinit systems
        log_command_in_target chkconfig --add vpoxtxs
    elif chroot_which insserv; then     # SUSE-based sysvinit systems
        log_command_in_target insserv vpoxtxs
    elif chroot_which update-rc.d; then # Debian/Ubuntu-based systems
        log_command_in_target update-rc.d vpoxtxs defaults
    elif chroot_which rc-update; then   # Gentoo Linux
        log_command_in_target rc-update add vpoxtxs default
    # Fall back on hardcoded symlinking.
    else
        log_command ln -s "../init.d/vpoxtxs" "${MY_INIT_D_PARENT_PATH}/rc0.d/K65vpoxtxs"
        log_command ln -s "../init.d/vpoxtxs" "${MY_INIT_D_PARENT_PATH}/rc1.d/K65vpoxtxs"
        log_command ln -s "../init.d/vpoxtxs" "${MY_INIT_D_PARENT_PATH}/rc6.d/K65vpoxtxs"
        log_command ln -s "../init.d/vpoxtxs" "${MY_INIT_D_PARENT_PATH}/rc2.d/S35vpoxtxs"
        log_command ln -s "../init.d/vpoxtxs" "${MY_INIT_D_PARENT_PATH}/rc3.d/S35vpoxtxs"
        log_command ln -s "../init.d/vpoxtxs" "${MY_INIT_D_PARENT_PATH}/rc4.d/S35vpoxtxs"
        log_command ln -s "../init.d/vpoxtxs" "${MY_INIT_D_PARENT_PATH}/rc5.d/S35vpoxtxs"
    fi
else
...
log_command @@VPOX_INSERT_POST_INSTALL_COMMAND@@
@@VPOX_COND_END@@
