...
/** @file
 * VPox Qt GUI - Implementation of Linux-specific keyboard functions.
 */
...
/* VPoxKeyboard uses the deprecated XKeycodeToKeysym(3) API, but uses it safely. */
...
static void dumpLayout(Display *pDisplay)
{
    LogRel(("Your keyboard layout does not appear to be fully supported by\n"
            "VirtualPox. If you are experiencing keyboard problems this.\n"
...
{
    LogRel(("Your keyboard type does not appear to be known to VirtualPox. If\n"
...
unsigned handleXKeyEvent(Display *pDisplay, unsigned int iDetail)
{
    ...
    LogRel3(("VPoxKeyboard: converting keycode %d to scancode %s0x%x\n",
