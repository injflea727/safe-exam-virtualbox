/* $Id: VPoxManageList.cpp $ */
/** @file
 * VPoxManage - The 'list' command.
 */
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listInternalNetworks(const ComPtr<IVirtualPox> pVirtualPox)
{
    HRESULT rc;
    com::SafeArray<BSTR> internalNetworks;
    CHECK_ERROR(pVirtualPox, COMGETTER(InternalNetworks)(ComSafeArrayAsOutParam(internalNetworks)));
 ...
 */
static HRESULT listNetworkInterfaces(const ComPtr<IVirtualPox> pVirtualPox,
                                     bool fIsBridged)
{
    HRESULT rc;
    ComPtr<IHost> host;
    CHECK_ERROR(pVirtualPox, COMGETTER(Host)(host.asOutParam()));
...
#ifndef VPOX_WITH_HOSTNETIF_API
        Bstr interfaceName;
 ...
 */
static HRESULT listCloudNetworks(const ComPtr<IVirtualPox> pVirtualPox)
{
    HRESULT rc;
    com::SafeIfaceArray<ICloudNetwork> cloudNetworks;
    CHECK_ERROR(pVirtualPox, COMGETTER(CloudNetworks)(ComSafeArrayAsOutParam(cloudNetworks)));
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listHostInfo(const ComPtr<IVirtualPox> pVirtualPox)
{
    static struct
    {
 ...
 */
static HRESULT listMedia(const ComPtr<IVirtualPox> pVirtualPox,
                         ...
                         bool fOptLong)
{
    ...
    for (size_t i = 0; i < aMedia.size(); ++i)
    {
        ...
        rc = showMediumInfo(pVirtualPox, pMedium, pszParentUUIDStr, fOptLong);
        ...
        if (children.size() > 0)
        {
            ...
            rc = listMedia(pVirtualPox, children, Utf8Str(uuid).c_str(), fOptLong);
        }
    }
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listHddBackends(const ComPtr<IVirtualPox> pVirtualPox)
{
    HRESULT rc;
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listUsbHost(const ComPtr<IVirtualPox> &pVirtualPox)
{
    HRESULT rc;
    ComPtr<IHost> Host;
    CHECK_ERROR_RET(pVirtualPox, COMGETTER(Host)(Host.asOutParam()), 1);
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listUsbFilters(const ComPtr<IVirtualPox> &pVirtualPox)
{
    HRESULT rc;
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listSystemProperties(const ComPtr<IVirtualPox> &pVirtualPox)
{
    ComPtr<ISystemProperties> systemProperties;
    CHECK_ERROR2I_RET(pVirtualPox, COMGETTER(SystemProperties)(systemProperties.asOutParam()), hrcCheck);
    ...
    pVirtualPox->COMGETTER(APIVersion)(str.asOutParam());
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listDhcpServers(const ComPtr<IVirtualPox> &pVirtualPox)
{
    HRESULT hrcRet = S_OK;
    com::SafeIfaceArray<IDHCPServer> DHCPServers;
    CHECK_ERROR2I_RET(pVirtualPox, COMGETTER(DHCPServers)(ComSafeArrayAsOutParam(DHCPServers)), hrcCheck);
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listExtensionPacks(const ComPtr<IVirtualPox> &pVirtualPox)
{
    ComObjPtr<IExtPackManager> ptrExtPackMgr;
    CHECK_ERROR2I_RET(pVirtualPox, COMGETTER(ExtensionPackManager)(ptrExtPackMgr.asOutParam()), hrcCheck);
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT listGroups(const ComPtr<IVirtualPox> &pVirtualPox)
{
    SafeArray<BSTR> groups;
    CHECK_ERROR2I_RET(pVirtualPox, COMGETTER(MachineGroups)(ComSafeArrayAsOutParam(groups)), hrcCheck);
 ...
 *
 * @returns See produceList.
 * @param   pVirtualPox         Reference to the IVirtualPox pointer.
 */
static HRESULT listVideoInputDevices(const ComPtr<IVirtualPox> pVirtualPox)
{
    HRESULT rc;
    ComPtr<IHost> host;
    CHECK_ERROR(pVirtualPox, COMGETTER(Host)(host.asOutParam()));
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox pointer.
 */
static HRESULT listScreenShotFormats(const ComPtr<IVirtualPox> pVirtualPox)
{
    HRESULT rc = S_OK;
    ComPtr<ISystemProperties> systemProperties;
    CHECK_ERROR(pVirtualPox, COMGETTER(SystemProperties)(systemProperties.asOutParam()));
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox pointer.
 */
static HRESULT listCloudProviders(const ComPtr<IVirtualPox> pVirtualPox)
{
    HRESULT rc = S_OK;
    ComPtr<ICloudProviderManager> pCloudProviderManager;
    CHECK_ERROR(pVirtualPox, COMGETTER(CloudProviderManager)(pCloudProviderManager.asOutParam()));
 ...
 */
static HRESULT listCloudProfiles(const ComPtr<IVirtualPox> pVirtualPox, bool fOptLong)
{
    HRESULT rc = S_OK;
    ComPtr<ICloudProviderManager> pCloudProviderManager;
    CHECK_ERROR(pVirtualPox, COMGETTER(CloudProviderManager)(pCloudProviderManager.asOutParam()));
...
#endif
#if defined(VPOX_WITH_CLOUD_NET)
    kListCloudNetworks,
#endif
    kListHostCpuIDs,
    kListHostInfo,
 ...
 * @param   pVirtualPox         Reference to the IVirtualPox smart pointer.
 */
static HRESULT produceList(enum enmListType enmCommand, bool fOptLong, bool fOptSorted, const ComPtr<IVirtualPox> &pVirtualPox)
{
    HRESULT rc = S_OK;
    switch (enmCommand)
    {
        ...
        case kListVMs:
        {
            ...
            rc = pVirtualPox->COMGETTER(Machines)(ComSafeArrayAsOutParam(machines));
            if (SUCCEEDED(rc))
            {
                /*
                 * Display it.
                ...
                if (!fOptSorted)
                {
                    for (size_t i = 0; i < machines.size(); ++i)
                        if (machines[i])
                            rc = showVMInfo(pVirtualPox, machines[i], NULL, fOptLong ? VMINFO_STANDARD : VMINFO_COMPACT);
                }
                else
                {
                    /*
                    ...
                    for (size_t i = 0; i < sortedMachines.size(); ++i)
                        rc = showVMInfo(pVirtualPox, sortedMachines[i].second, NULL, fOptLong ? VMINFO_STANDARD : VMINFO_COMPACT);
                }
            }
            break;
        }
        ...
        case kListRunningVMs:
        {
            ...
            if (SUCCEEDED(rc))
                rc = pVirtualPox->GetMachineStates(ComSafeArrayAsInParam(machines), ComSafeArrayAsOutParam(states));
            if (SUCCEEDED(rc))
            {
                /*
        ...
        case kListOsTypes:
        {
            ...
            rc = pVirtualPox->COMGETTER(GuestOSTypes)(ComSafeArrayAsOutParam(coll));
            if (SUCCEEDED(rc))
            {
                /*
                 * Iterate through the collection.
        ...
        case kListInternalNetworks:
            rc = listInternalNetworks(pVirtualPox);
            break;
...
#endif
            rc = listNetworkInterfaces(pVirtualPox, enmCommand == kListBridgedInterfaces);
            break;

#if defined(VPOX_WITH_CLOUD_NET)
        case kListCloudNetworks:
            rc = listCloudNetworks(pVirtualPox);
            break;
#endif
        case kListHostInfo:
            rc = listHostInfo(pVirtualPox);
            break;
        ...
        case kListHddBackends:
            rc = listHddBackends(pVirtualPox);
            break;

        case kListHdds:
        {
            com::SafeIfaceArray<IMedium> hdds;
            CHECK_ERROR(pVirtualPox, COMGETTER(HardDisks)(ComSafeArrayAsOutParam(hdds)));
            rc = listMedia(pVirtualPox, hdds, "base", fOptLong);
            break;
        }
        ...
        case kListDvds:
        {
            ...
            CHECK_ERROR(pVirtualPox, COMGETTER(DVDImages)(ComSafeArrayAsOutParam(dvds)));
            rc = listMedia(pVirtualPox, dvds, NULL, fOptLong);
            break;
        }
        ...
        case kListFloppies:
        {
            ...
            CHECK_ERROR(pVirtualPox, COMGETTER(FloppyImages)(ComSafeArrayAsOutParam(floppies)));
            rc = listMedia(pVirtualPox, floppies, NULL, fOptLong);
            break;
        }

        case kListUsbHost:
            rc = listUsbHost(pVirtualPox);
            break;

        case kListUsbFilters:
            rc = listUsbFilters(pVirtualPox);
            break;

        case kListSystemProperties:
            rc = listSystemProperties(pVirtualPox);
            break;

        case kListDhcpServers:
            rc = listDhcpServers(pVirtualPox);
            break;

        case kListExtPacks:
            rc = listExtensionPacks(pVirtualPox);
            break;

        case kListGroups:
            rc = listGroups(pVirtualPox);
            break;

        case kListNatNetworks:
        {
            com::SafeIfaceArray<INATNetwork> nets;
            CHECK_ERROR(pVirtualPox, COMGETTER(NATNetworks)(ComSafeArrayAsOutParam(nets)));
...
#undef PRINT_STRING_ARRAY
        ...
        case kListVideoInputDevices:
            rc = listVideoInputDevices(pVirtualPox);
            break;

        case kListScreenShotFormats:
            rc = listScreenShotFormats(pVirtualPox);
            break;

        case kListCloudProviders:
            rc = listCloudProviders(pVirtualPox);
            break;

        case kListCloudProfiles:
            rc = listCloudProfiles(pVirtualPox, fOptLong);
            break;
...
RTEXITCODE handleList(HandlerArg *a)
{
    ...
    while ((ch = RTGetOpt(&GetState, &ValueUnion)))
    {
        switch (ch)
        {
            ...
            case kListCloudProfiles:
                ...
                if (fOptMultiple)
                {
                    HRESULT hrc = produceList((enum enmListType)ch, fOptLong, fOptSorted, a->virtualPox);
                    if (FAILED(hrc))
    ...
    if (!fOptMultiple)
    {
        HRESULT hrc = produceList(enmOptCommand, fOptLong, fOptSorted, a->virtualPox);
        if (FAILED(hrc))
            return RTEXITCODE_FAILURE;
    }
