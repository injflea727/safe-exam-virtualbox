 ...
 * wrapper interface for the main code of the VPoxClient deamon.  The
...
int SeamlessMain::nextStateChangeEvent(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        switch (newMode)
        {
             ...
             * borderless and making the guest desktop transparent. */
                LogRelFlowFunc(("\"Visible region\" mode requested (VPoxClient)\n"));
                break;
            case VMMDev_Seamless_Disabled:
                LogRelFlowFunc(("\"Disabled\" mode requested (VPoxClient)\n"));
                break;
            case VMMDev_Seamless_Host_Window:
            /* One host window represents one guest window.  Not yet implemented. */
                LogRelFunc(("Unsupported \"host window\" mode requested (VPoxClient)\n"));
                return VERR_NOT_SUPPORTED;
            default:
                LogRelFunc(("Unsupported mode %d requested (VPoxClient)\n",
                            newMode));
                return VERR_NOT_SUPPORTED;
        }
    }
    ...
    {
        LogRelFunc(("VbglR3SeamlessWaitEvent returned %Rrc (VPoxClient)\n", rc));
    }
...
int SeamlessMain::startX11MonitorThread(void)
{
    ...
    rc = RTThreadCreate(&mX11MonitorThread, x11MonitorThread, this, 0,
                        ...
                        "X11 events");
    if (RT_FAILURE(rc))
        LogRelFunc(("Warning: failed to start X11 monitor thread (VPoxClient)\n"));
    return rc;
}
...
/** VPoxClient service class wrapping the logic for the seamless service while
...
{
    return ".vpoxclient-seamless.pid";
}
