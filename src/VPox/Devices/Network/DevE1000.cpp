...
/** Saved state version for VirtualPox 4.2 with VLAN tag fields.  */
# define E1K_SAVEDSTATE_VERSION_VPOX_42_VTAG  3
#else /* !E1K_WITH_TXD_CACHE */
/** The current Saved state version. */
# define E1K_SAVEDSTATE_VERSION         3
#endif /* !E1K_WITH_TXD_CACHE */
/** Saved state version for VirtualPox 4.1 and earlier.
 * These did not include VLAN tag fields.  */
#define E1K_SAVEDSTATE_VERSION_VPOX_41  2
/** Saved state version for VirtualPox 3.0 and earlier.
 * This did not include the configuration part nor the E1kEEPROM.  */
#define E1K_SAVEDSTATE_VERSION_VPOX_30  1
...
#if defined(VPOX_WITH_STATISTICS)
...
DECLINLINE(bool) e1kSetupGsoCtx(PPDMNETWORKGSO pGso, E1KTXCTX const *pCtx)
{
     ...
     */
    if (RT_UNLIKELY( pCtx->dw3.u8HDRLEN + pCtx->dw2.u20PAYLEN > VPOX_MAX_GSO_SIZE ))
    {
        E1kLog(("e1kSetupGsoCtx: HDRLEN(=%#x) + PAYLEN(=%#x) = %#x, max is %#x\n",
                pCtx->dw3.u8HDRLEN, pCtx->dw2.u20PAYLEN, pCtx->dw3.u8HDRLEN + pCtx->dw2.u20PAYLEN, VPOX_MAX_GSO_SIZE));
        return false;
    }
 ...
 * @thread  EMT
 */
static VPOXSTRICTRC e1kRegReadAlignedU32(PPDMDEVINS pDevIns, PE1KSTATE pThis, uint32_t offReg, uint32_t *pu32)
{
    Assert(!(offReg & 3));
 ...
 * @thread  EMT
 */
static VPOXSTRICTRC e1kRegWriteAlignedU32(PPDMDEVINS pDevIns, PE1KSTATE pThis, uint32_t offReg, uint32_t u32Value)
{
    VPOXSTRICTRC rc    = VINF_SUCCESS;
    int          index = e1kRegLookup(offReg);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) e1kMMIORead(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void *pv, uint32_t cb)
{
    RT_NOREF2(pvUser, cb);
    PE1KSTATE pThis  = PDMDEVINS_2_DATA(pDevIns, PE1KSTATE);
    ...
    VPOXSTRICTRC rcStrict = e1kRegReadAlignedU32(pDevIns, pThis, (uint32_t)off, (uint32_t *)pv);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) e1kMMIOWrite(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void const *pv, uint32_t cb)
{
    RT_NOREF2(pvUser, cb);
    PE1KSTATE pThis  = PDMDEVINS_2_DATA(pDevIns, PE1KSTATE);
    ...
    VPOXSTRICTRC rcStrict = e1kRegWriteAlignedU32(pDevIns, pThis, (uint32_t)off, *(uint32_t const *)pv);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) e1kIOPortIn(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t *pu32, unsigned cb)
{
    PE1KSTATE    pThis = PDMDEVINS_2_DATA(pDevIns, PE1KSTATE);
    VPOXSTRICTRC rc;
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) e1kIOPortOut(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t u32, unsigned cb)
{
    PE1KSTATE    pThis = PDMDEVINS_2_DATA(pDevIns, PE1KSTATE);
    VPOXSTRICTRC rc;
...
static int e1kCanReceive(PPDMDEVINS pDevIns, PE1KSTATE pThis)
{
    ...
    {
        E1KRXDESC desc;
        PDMDevHlpPhysRead(pDevIns, e1kDescAddr(RDBAH, RDBAL, rxdc.rdh), &desc, sizeof(desc));
        if (desc.status.fDD)
...
static DECLCALLBACK(int) e1kLoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
...
#ifdef E1K_WITH_TXD_CACHE
        &&  uVersion != E1K_SAVEDSTATE_VERSION_VPOX_42_VTAG
#endif /* E1K_WITH_TXD_CACHE */
        &&  uVersion != E1K_SAVEDSTATE_VERSION_VPOX_41
        &&  uVersion != E1K_SAVEDSTATE_VERSION_VPOX_30)
        return VERR_SSM_UNSUPPORTED_DATA_UNIT_VERSION;

    if (   uVersion > E1K_SAVEDSTATE_VERSION_VPOX_30
    ...
    if (uPass == SSM_PASS_FINAL)
    {
        ...
        AssertRCReturn(rc, rc);
        if (uVersion > E1K_SAVEDSTATE_VERSION_VPOX_41)
        {
...
#ifdef E1K_WITH_TXD_CACHE
        if (uVersion > E1K_SAVEDSTATE_VERSION_VPOX_42_VTAG)
        {
...
static void e1kR3ConfigurePciDev(PPDMPCIDEV pPciDev, E1KCHIP eChip)
{
    ...
    PDMPciDevSetWord( pPciDev, VPOX_PCI_SUBSYSTEM_VENDOR_ID, g_aChips[eChip].uPCISubsystemVendorId);
    PDMPciDevSetWord( pPciDev, VPOX_PCI_SUBSYSTEM_ID, g_aChips[eChip].uPCISubsystemId);

    PDMPciDevSetWord( pPciDev, VPOX_PCI_COMMAND,            0x0000);
    /* DEVSEL Timing (medium device), 66 MHz Capable, New capabilities */
    PDMPciDevSetWord( pPciDev, VPOX_PCI_STATUS,
                      VPOX_PCI_STATUS_DEVSEL_MEDIUM | VPOX_PCI_STATUS_CAP_LIST |  VPOX_PCI_STATUS_66MHZ);
    /* Stepping A2 */
    PDMPciDevSetByte( pPciDev, VPOX_PCI_REVISION_ID,          0x02);
    /* Ethernet adapter */
    PDMPciDevSetByte( pPciDev, VPOX_PCI_CLASS_PROG,           0x00);
    PDMPciDevSetWord( pPciDev, VPOX_PCI_CLASS_DEVICE,       0x0200);
    /* normal single function Ethernet controller */
    PDMPciDevSetByte( pPciDev, VPOX_PCI_HEADER_TYPE,          0x00);
    /* Memory Register Base Address */
    PDMPciDevSetDWord(pPciDev, VPOX_PCI_BASE_ADDRESS_0, 0x00000000);
    /* Memory Flash Base Address */
    PDMPciDevSetDWord(pPciDev, VPOX_PCI_BASE_ADDRESS_1, 0x00000000);
    /* IO Register Base Address */
    PDMPciDevSetDWord(pPciDev, VPOX_PCI_BASE_ADDRESS_2, 0x00000001);
    /* Expansion ROM Base Address */
    PDMPciDevSetDWord(pPciDev, VPOX_PCI_ROM_ADDRESS,    0x00000000);
    /* Capabilities Pointer */
    PDMPciDevSetByte( pPciDev, VPOX_PCI_CAPABILITY_LIST,      0xDC);
    /* Interrupt Pin: INTA# */
    PDMPciDevSetByte( pPciDev, VPOX_PCI_INTERRUPT_PIN,        0x01);
    /* Max_Lat/Min_Gnt: very high priority and time slice */
    PDMPciDevSetByte( pPciDev, VPOX_PCI_MIN_GNT,              0xFF);
    PDMPciDevSetByte( pPciDev, VPOX_PCI_MAX_LAT,              0x00);

    /* PCI Power Management Registers ****************************************/
    /* Capability ID: PCI Power Management Registers */
    PDMPciDevSetByte( pPciDev, 0xDC,            VPOX_PCI_CAP_ID_PM);
    ...
    PDMPciDevSetByte( pPciDev, 0xE4,          VPOX_PCI_CAP_ID_PCIX);
#ifdef E1K_WITH_MSI
    ...
    PDMPciDevSetWord( pPciDev, 0xE4 + 2,        VPOX_PCI_X_CMD_ERO);
