...
/** @file
 * VPox Qt GUI - UIExtraDataManager class implementation.
 */
...
/** Private QObject extension
  * providing UIExtraDataManager with the CVirtualPox event-source. */
...
void UIExtraDataEventHandler::prepareListener()
{
    ...
    eventTypes
        << KVPoxEventType_OnExtraDataCanChange
        << KVPoxEventType_OnExtraDataChanged;

    /* Register event listener for VirtualPox event source: */
    comEventSourceVPox.RegisterListener(m_comEventListener, eventTypes,
    ...
    if (gEDataManager->eventHandlingType() == EventHandlingType_Passive)
    {
        ...
        m_pQtListener->getWrapped()->registerSource(comEventSourceVPox, m_comEventListener);
    }
}
...
void UIExtraDataEventHandler::cleanupListener()
{
    ...
    /* Make sure VPoxSVC is available: */
    if (!uiCommon().isVPoxSVCAvailable())
        return;
    ...
    /* Unregister event listener for VirtualPox event source: */
    comEventSourceVPox.UnregisterListener(m_comEventListener);
}
...
void UIExtraDataManagerWindow::sltMachineRegistered(const QUuid &uID, bool fRegistered)
{
    ...
    if (fRegistered)
    {
        ...
        /* Get machine items: */
        const CMachineVector machines = uiCommon().virtualPox().GetMachines();
...
void UIExtraDataManagerWindow::sltSave()
{
    ...
    AssertReturnVoid(fOpened);
    {
        ...
        {
            stream.writeStartElement("VirtualPox");
            {
            ...
            stream.writeEndElement(); /* VirtualPox */
        }
...
{
    /* Prepare connections: */
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigMachineRegistered,
                         ...
                         pLayout == m_pPaneOfChooser->layout());
        {
            ...
            AssertPtrReturnVoid(m_pViewOfChooser);
            {
                ...
                AssertPtrReturnVoid(m_pModelSourceOfChooser);
                {
                    ...
                    CMachineVector machines = uiCommon().virtualPox().GetMachines();
                    foreach (const CMachine &machine, machines)
                                                  ...
                                                  const int iPosition /* = -1 */)
{
    ...
    const CMachine machine = vpox.FindMachine(uID.toString());
    /* Make sure VM is accessible: */
    if (vpox.isOk() && !machine.isNull() && machine.GetAccessible())
...
void UIExtraDataManager::hotloadMachineExtraDataMap(const QUuid &uID)
{
    ...
    CVirtualPox vpox = uiCommon().virtualPox();
    CMachine machine = vpox.FindMachine(uID.toString());
    AssertReturnVoid(vpox.isOk() && !machine.isNull());
...
void UIExtraDataManager::setExtraDataString(const QString &strKey, const QString &strValue, const QUuid &uID /* = GlobalID */)
{
    ...
    if (uID == GlobalID)
    {
        ...
        comVPox.SetExtraData(strKey, strValue);
        if (!comVPox.isOk())
            msgCenter().cannotSetExtraData(comVPox, strKey, strValue);
        /* Wipe out old keys: */
        foreach (const QString &strOldKey, g_mapOfObsoleteKeys.values(strKey))
        {
            comVPox.SetExtraData(strOldKey, QString());
            if (!comVPox.isOk())
            {
                msgCenter().cannotSetExtraData(comVPox, strOldKey, strValue);
                break;
            }
        }
    }
    ...
    else
    {
        /* Search for corresponding machine: */
        CVirtualPox comVPox = uiCommon().virtualPox();
        const CMachine comMachine = comVPox.FindMachine(uID.toString());
        AssertReturnVoid(comVPox.isOk() && !comMachine.isNull());
...
void UIExtraDataManager::setExtraDataStringList(const QString &strKey, const QStringList &value, const QUuid &uID /* = GlobalID */)
{
    ...
    if (uID == GlobalID)
    {
        ...
        CVirtualPox comVPox = uiCommon().virtualPox();
        /* Update global extra-data: */
        comVPox.SetExtraDataStringList(strKey, value);
        if (!comVPox.isOk())
            msgCenter().cannotSetExtraData(comVPox, strKey, value.join(","));
        /* Wipe out old keys: */
        foreach (const QString &strOldKey, g_mapOfObsoleteKeys.values(strKey))
        {
            comVPox.SetExtraData(strOldKey, QString());
            if (!comVPox.isOk())
            {
                msgCenter().cannotSetExtraData(comVPox, strOldKey, value.join(","));
                break;
            }
        }
    }
...
bool UIExtraDataManager::autoCaptureEnabled()
{
    ...
    {
#if defined(VPOX_WS_X11) && defined(DEBUG)
        fAutoCapture = false;
#else
        fAutoCapture = true;
#endif
...
QStringList UIExtraDataManager::vpoxManagerDetailsPaneElementOptions(DetailsElementType enmElementType)
{
...
void UIExtraDataManager::setVPoxManagerDetailsPaneElementOptions(DetailsElementType enmElementType, const QStringList &options)
{
    /* Compose full key from GUI_Details_Elements and enmElementType: */
    QString strElementType = gpConverter->toInternalString(enmElementType);
...
bool UIExtraDataManager::showMachineInVirtualPoxManagerChooser(const QUuid &uID)
{
    /* 'True' unless 'restriction' feature allowed: */
    return !isFeatureAllowed(GUI_HideFromManager, uID);
}

bool UIExtraDataManager::showMachineInVirtualPoxManagerDetails(const QUuid &uID)
{
    /* 'True' unless 'restriction' feature allowed: */
...
void UIExtraDataManager::prepareGlobalExtraDataMap()
{
    ...
    foreach (const QString &strKey, vpox.GetExtraDataKeys())
        m_data[GlobalID][strKey] = vpox.GetExtraData(strKey);
}
