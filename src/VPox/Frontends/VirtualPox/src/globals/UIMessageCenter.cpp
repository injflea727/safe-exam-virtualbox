...
/** @file
 * VPox Qt GUI - UIMessageCenter class implementation.
 */
...
#endif /* VPOX_GUI_WITH_NETWORK_MANAGER */
                                       ...
                                       const QString &strButtonName3 /* = QString() */) const
{
    ...
    switch (enmType)
    {
        ...
        case MessageType_Info:
            strTitle = tr("VirtualPox - Information", "msg box title");
            icon = AlertIconType_Information;
            break;
        case MessageType_Question:
            strTitle = tr("VirtualPox - Question", "msg box title");
            icon = AlertIconType_Question;
            break;
        case MessageType_Warning:
            strTitle = tr("VirtualPox - Warning", "msg box title");
            icon = AlertIconType_Warning;
            break;
        case MessageType_Error:
            strTitle = tr("VirtualPox - Error", "msg box title");
            icon = AlertIconType_Critical;
            break;
        case MessageType_Critical:
            strTitle = tr("VirtualPox - Critical Error", "msg box title");
            icon = AlertIconType_Critical;
            break;
        case MessageType_GuruMeditation:
            strTitle = "VirtualPox - Guru Meditation"; /* don't translate this */
...
{
    alert(0, MessageType_Error,
          tr("<b>%1</b> is an option for the VirtualPox VM runner (VirtualPoxVM) application, not the VirtualPox Manager.")
             .arg(strOption));
}
...
void UIMessageCenter::cannotStartSelector() const
{
    alert(0, MessageType_Critical,
          tr("<p>Cannot start the VirtualPox Manager due to local restrictions.</p>"
...
void UIMessageCenter::cannotStartRuntime() const
{
    ...
    const QString strUsage = tr("<tr>"
                                "<td>Usage: VirtualPoxVM --startvm &lt;name|UUID&gt;</td>"
                                "</tr>"
                                "<tr>"
                                "<td>Starts the VirtualPox virtual machine with the given "
...
{
    alert(0, MessageType_Warning,
          tr("You are running a prerelease version of VirtualPox. "
...
{
    alert(0, MessageType_Warning,
          tr("You are running an EXPERIMENTAL build of VirtualPox. "
             "This version is not suitable for production use."));
}
...
void UIMessageCenter::cannotInitUserHome(const QString &strUserHome) const
{
    error(0, MessageType_Critical,
          tr("<p>Failed to initialize COM because the VirtualPox global "
...
{
    error(0, MessageType_Critical,
          tr("<p>Failed to initialize COM or to find the VirtualPox COM server. "
             "Most likely, the VirtualPox server is not running or failed to start.</p>"
             "<p>The application will now terminate.</p>"),
          UIErrorString::formatErrorInfo(COMErrorInfo(), rc));
}

void UIMessageCenter::cannotCreateVirtualPoxClient(const CVirtualPoxClient &client) const
{
    error(0, MessageType_Critical,
          tr("<p>Failed to create the VirtualPoxClient COM object.</p>"
             "<p>The application will now terminate.</p>"),
          UIErrorString::formatErrorInfo(client));
}

void UIMessageCenter::cannotAcquireVirtualPox(const CVirtualPoxClient &client) const
{
    QString err = tr("<p>Failed to acquire the VirtualPox COM object.</p>"
                     "<p>The application will now terminate.</p>");
#if defined(VPOX_WS_X11) || defined(VPOX_WS_MAC)
    if (client.lastRC() == NS_ERROR_SOCKET_FAIL)
        err += tr("<p>The reason for this error are most likely wrong permissions of the IPC "
                  ...
                  "<font color=blue>'/tmp'</font> and <font color=blue>'/tmp/.vpox-*-ipc/'</font></p>");
#endif
...
void UIMessageCenter::cannotFindLanguage(const QString &strLangId, const QString &strNlsPath) const
{
    alert(0, MessageType_Error,
          tr("<p>Could not find a language file for the language <b>%1</b> in the directory <b><nobr>%2</nobr></b>.</p>"
             ...
             "VirtualPox Manager window, and select one of the existing languages on the <b>Language</b> page.</p>")
...
void UIMessageCenter::cannotFindMachineByName(const CVirtualPox &vpox, const QString &strName) const
{
    error(0, MessageType_Error,
          tr("There is no virtual machine named <b>%1</b>.")
             .arg(strName),
          UIErrorString::formatErrorInfo(vpox));
}

void UIMessageCenter::cannotFindMachineById(const CVirtualPox &vpox, const QUuid &uId) const
{
    error(0, MessageType_Error,
...
void UIMessageCenter::cannotSetExtraData(const CVirtualPox &vpox, const QString &strKey, const QString &strValue)
{
    error(0, MessageType_Error,
          tr("Failed to set the global VirtualPox extra data for key <i>%1</i> to value <i>{%2}</i>.")
             .arg(strKey, strValue),
          UIErrorString::formatErrorInfo(vpox));
}
...
void UIMessageCenter::cannotOpenMachine(const CVirtualPox &vpox, const QString &strMachinePath) const
{
    error(0, MessageType_Error,
...
int UIMessageCenter::confirmMachineRemoval(const QList<CMachine> &machines) const
{
    ...
    foreach (const CMachine &machine, machines)
    {
        ...
        else
        {
            ...
            strMachineName = UICommon::hasAllowedExtension(fi.completeSuffix(), VPoxFileExts) ? fi.completeBaseName() : fi.fileName();
...
bool UIMessageCenter::confirmSnapshotRemoval(const QString &strSnapshotName) const
{
    return questionBinary(0, MessageType_Question,
                          tr("<p>Deleting the snapshot will cause the state information saved in it to be lost, and storage data spread over "
                             "several image files that VirtualPox has created together with the snapshot will be merged into one file. "
...
void UIMessageCenter::cannotSetSystemProperties(const CSystemProperties &properties, QWidget *pParent /* = 0*/) const
{
    error(pParent, MessageType_Critical,
          tr("Failed to set global VirtualPox properties."),
...
bool UIMessageCenter::confirmMediumRelease(const UIMedium &medium, bool fInduced, QWidget *pParent /* = 0 */) const
{
    ...
    {
        CMachine machine = vpox.FindMachine(uMachineID.toString());
        if (!vpox.isOk() || machine.isNull())
            continue;
...
void UIMessageCenter::cannotOpenMedium(const CVirtualPox &comVPox, const QString &strLocation, QWidget *pParent /* = 0 */) const
{
    /* Show the error: */
    error(pParent, MessageType_Error,
          tr("Failed to open the disk image file <nobr><b>%1</b></nobr>.").arg(strLocation), UIErrorString::formatErrorInfo(comVPox));
}
...
void UIMessageCenter::cannotCreateDHCPServer(const CVirtualPox &comVPox, const QString &strInterfaceName, QWidget *pParent /* = 0 */) const
{
    error(pParent, MessageType_Error,
          tr("Failed to create a DHCP server for the network interface <b>%1</b>.")
             .arg(strInterfaceName),
          UIErrorString::formatErrorInfo(comVPox));
}

void UIMessageCenter::cannotRemoveDHCPServer(const CVirtualPox &comVPox, const QString &strInterfaceName, QWidget *pParent /* = 0 */) const
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotAcquireCloudProviderManager(const CVirtualPox &comVPox, QWidget *pParent /* = 0 */) const
{
    error(pParent, MessageType_Error,
...
bool UIMessageCenter::confirmCloudProfilesImport(QWidget *pParent /* = 0 */) const
{
    return questionBinary(pParent, MessageType_Question,
                          tr("<p>Do you want to import cloud profiles from external files?</p>"
                             "<p>VirtualPox cloud profiles will be overwritten and their data will be lost.</p>"),
                          0 /* auto-confirm id */,
...
void UIMessageCenter::cannotCreateMachine(const CVirtualPox &vpox, QWidget *pParent /* = 0*/) const
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotRegisterMachine(const CVirtualPox &vpox, const QString &strMachineName, QWidget *pParent /* = 0*/) const
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotCreateHardDiskStorage(const CVirtualPox &vpox, const QString &strLocation, QWidget *pParent /* = 0*/) const
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotCreateMediumStorage(const CVirtualPox &comVPox, const QString &strLocation, QWidget *pParent /* = 0 */) const
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotCreateAppliance(const CVirtualPox &comVPox, QWidget *pParent /* = 0 */) const
{
...
bool UIMessageCenter::remindAboutGuruMeditation(const QString &strLogFolder)
{
    return questionBinary(0, MessageType_GuruMeditation,
                          tr("<p>A critical error has occurred while running the virtual "
                             ...
                             "<a href=https://www.virtualpox.org>https://www.virtualpox.org</a> "
                             "or your support contract. Please provide the contents of the "
                             "log file <tt>VPox.log</tt> and the image file <tt>VPox.png</tt>, "
                             ...
                             "from the <b>Machine</b> menu of the main VirtualPox window.</p>"
                             ""
...
void UIMessageCenter::warnAboutVPoxSVCUnavailable() const
{
    alert(0, MessageType_Critical,
          tr("<p>A critical error has occurred while running the virtual "
...
void UIMessageCenter::showUpdateSuccess(const QString &strVersion, const QString &strLink) const
{
    alert(windowManager().networkManagerOrMainWindowShown(), MessageType_Info,
          tr("<p>A new version of VirtualPox has been released! Version <b>%1</b> is available "
             "at <a href=\"https://www.virtualpox.org/\">virtualpox.org</a>.</p>"
...
{
    alert(windowManager().networkManagerOrMainWindowShown(), MessageType_Info,
          tr("You are already running the most recent version of VirtualPox."));
}

void UIMessageCenter::askUserToDownloadExtensionPack(const QString &strExtPackName, const QString &strExtPackVersion, const QString &strVPoxVersion) const
{
    alert(windowManager().networkManagerOrMainWindowShown(), MessageType_Info,
          tr("<p>You have version %1 of the <b><nobr>%2</nobr></b> installed.</p>"
             ...
             .arg(strExtPackVersion).arg(strExtPackName).arg(strVPoxVersion));
}

bool UIMessageCenter::cannotFindGuestAdditions() const
{
    return questionBinary(0, MessageType_Question,
                          tr("<p>Could not find the <b>VirtualPox Guest Additions</b> disk image file.</p>"
...
{
    return questionBinary(windowManager().networkManagerOrMainWindowShown(), MessageType_Question,
                          tr("<p>Are you sure you want to download the <b>VirtualPox Guest Additions</b> disk image file "
...
void UIMessageCenter::cannotSaveGuestAdditions(const QString &strURL, const QString &strTarget) const
{
    alert(windowManager().networkManagerOrMainWindowShown(), MessageType_Error,
          tr("<p>The <b>VirtualPox Guest Additions</b> disk image file has been successfully downloaded "
...
{
    return questionBinary(0, MessageType_Question,
                          tr("<p>Could not find the <b>VirtualPox User Manual</b> <nobr><b>%1</b>.</nobr></p>"
...
{
    return questionBinary(windowManager().networkManagerOrMainWindowShown(), MessageType_Question,
                          tr("<p>Are you sure you want to download the <b>VirtualPox User Manual</b> "
                             "from <nobr><a href=\"%1\">%1</a></nobr> (size %2 bytes)?</p>")
                             .arg(strURL, QLocale(UICommon::languageId()).toString(uSize)),
                          0 /* auto-confirm id */,
                          tr("Download"));
...
{
    alert(windowManager().networkManagerOrMainWindowShown(), MessageType_Error,
          tr("<p>The VirtualPox User Manual has been successfully downloaded "
             "from <nobr><a href=\"%1\">%1</a></nobr> "
             "but can't be saved locally as <nobr><b>%2</b>.</nobr></p>"
             "<p>Please choose another location for that file.</p>")
             .arg(strURL, strTarget));
...
{
    return questionBinary(pParent, MessageType_Question,
                          tr("<p>You are about to install a VirtualPox extension pack. "
                             "Extension packs complement the functionality of VirtualPox and can contain system level software "
                                                  ...
                                                  QWidget *pParent /* = 0*/) const
{
    ...
    QString strBelehrung = tr("Extension packs complement the functionality of VirtualPox and can contain "
...
{
    return questionBinary(pParent, MessageType_Question,
                          tr("<p>You are about to remove the VirtualPox extension pack <b>%1</b>.</p>"
...
{
    uiCommon().openURL("https://www.virtualpox.org");
}

void UIMessageCenter::sltShowBugTracker()
{
    uiCommon().openURL("https://www.virtualpox.org/wiki/Bugtracker");
}

void UIMessageCenter::sltShowForums()
{
    uiCommon().openURL("https://forums.virtualpox.org/");
}

void UIMessageCenter::sltShowOracle()
{
    uiCommon().openURL("http://www.oracle.com/us/technologies/virtualization/virtualpox/overview/index.html");
}
...
void UIMessageCenter::sltShowHelpAboutDialog()
{
    ...
    {
        strFullVersion = QString("%1 r%2 - %3").arg(vpox.GetVersion())
                                               .arg(vpox.GetRevision())
                                               .arg(uiCommon().brandingGetKey("Name"));
    }
    else
    {
        strFullVersion = QString("%1 r%2").arg(vpox.GetVersion())
                                          .arg(vpox.GetRevision());
    }
    AssertWrapperOk(vpox);

    (new VPoxAboutDlg(windowManager().mainWindowShown(), strFullVersion))->show();
}
...
void UIMessageCenter::prepare()
{
    ...
    tr("VirtualPox is not currently allowed to access USB devices.  You can change this by adding your user to the 'vpoxusers' group.  Please see the user manual for a more detailed explanation");
    tr("VirtualPox is not currently allowed to access USB devices.  You can change this by allowing your user to access the 'usbfs' folder and files.  Please see the user manual for a more detailed explanation");
                                    ...
                                    const QString &strAutoConfirmId) const
{
    ...
    switch (enmType)
    {
        default:
        case MessageType_Info:
            title = tr("VirtualPox - Information", "msg box title");
            icon = AlertIconType_Information;
            break;
        case MessageType_Question:
            title = tr("VirtualPox - Question", "msg box title");
            icon = AlertIconType_Question;
            break;
        case MessageType_Warning:
            title = tr("VirtualPox - Warning", "msg box title");
            icon = AlertIconType_Warning;
            break;
        case MessageType_Error:
            title = tr("VirtualPox - Error", "msg box title");
            icon = AlertIconType_Critical;
            break;
        case MessageType_Critical:
            title = tr("VirtualPox - Critical Error", "msg box title");
            icon = AlertIconType_Critical;
            break;
        case MessageType_GuruMeditation:
            title = "VirtualPox - Guru Meditation"; /* don't translate this */
            icon = AlertIconType_GuruMeditation;
            break;
    }
