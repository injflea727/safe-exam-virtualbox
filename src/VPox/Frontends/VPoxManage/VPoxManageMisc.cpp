/* $Id: VPoxManageMisc.cpp $ */
/** @file
 * VPoxManage - VirtualPox's command-line interface.
 */
...
RTEXITCODE handleRegisterVM(HandlerArg *a)
{
     ...
     * has been redesigned. */
    rc = a->virtualPox->OpenMachine(Bstr(a->argv[0]).raw(),
                                    machine.asOutParam());
    if (rc == VPOX_E_FILE_ERROR)
    {
        ...
        CHECK_ERROR(a->virtualPox, OpenMachine(Bstr(szVMFileAbs).raw(),
                                               machine.asOutParam()));
    }
    else if (FAILED(rc))
        CHECK_ERROR(a->virtualPox, OpenMachine(Bstr(a->argv[0]).raw(),
                                               machine.asOutParam()));
    if (SUCCEEDED(rc))
    {
        ASSERT(machine);
        CHECK_ERROR(a->virtualPox, RegisterMachine(machine));
    }
...
RTEXITCODE handleUnregisterVM(HandlerArg *a)
{
    ...
    ComPtr<IMachine> machine;
    CHECK_ERROR_RET(a->virtualPox, FindMachine(Bstr(VMName).raw(),
                                               machine.asOutParam()),
                    RTEXITCODE_FAILURE);
...
RTEXITCODE handleCreateVM(HandlerArg *a)
{
    ...
    do
    {
        ...
        CHECK_ERROR_BREAK(a->virtualPox,
        ...
        if (fRegister)
        {
            CHECK_ERROR_BREAK(a->virtualPox, RegisterMachine(machine));
        }
...
RTEXITCODE handleMoveVM(HandlerArg *a)
{
    ...
    CHECK_ERROR_RET(a->virtualPox, FindMachine(Bstr(pszSrcName).raw(),
...
RTEXITCODE handleCloneVM(HandlerArg *a)
{
    ...
    CHECK_ERROR_RET(a->virtualPox, CreateMachine(bstrSettingsFile.raw(),
                                                 Bstr(pszTrgName).raw(),
                                                 ComSafeArrayAsInParam(groups),
                                                 NULL,
                                                 createFlags.raw(),
    ...
    if (fRegister)
        CHECK_ERROR_RET(a->virtualPox, RegisterMachine(trgMachine), RTEXITCODE_FAILURE);
...
RTEXITCODE handleStartVM(HandlerArg *a)
{
    ...
    /* make sure the VM process will by default start on the same display as VPoxManage */
    {
         ...
         ++it)
    {
        ...
        const char *pszVM = *it;
        ComPtr<IMachine> machine;
        CHECK_ERROR(a->virtualPox, FindMachine(Bstr(pszVM).raw(),
                                               machine.asOutParam()));
        if (machine)
        {
            ComPtr<IProgress> progress;
...
RTEXITCODE handleDiscardState(HandlerArg *a)
{
    ...
    ComPtr<IMachine> machine;
    CHECK_ERROR(a->virtualPox, FindMachine(Bstr(a->argv[0]).raw(),
                                           machine.asOutParam()));
    if (machine)
    {
        do
...
RTEXITCODE handleGetExtraData(HandlerArg *a)
{
    ...
    if (!strcmp(a->argv[0], "global"))
    {
        ...
        if (a->argc < 2 || !strcmp(a->argv[1], "enumerate"))
        {
            ...
            CHECK_ERROR(a->virtualPox, GetExtraDataKeys(ComSafeArrayAsOutParam(aKeys)));
                 ...
                 ++i)
            {
                ...
                Bstr bstrValue;
                CHECK_ERROR(a->virtualPox, GetExtraData(bstrKey.raw(),
        ...
        {
            Bstr value;
            CHECK_ERROR(a->virtualPox, GetExtraData(Bstr(a->argv[1]).raw(),
                                                    value.asOutParam()));
            if (!value.isEmpty())
                RTPrintf("Value: %ls\n", value.raw());
            else
...
RTEXITCODE handleSetExtraData(HandlerArg *a)
{
    ...
    if (!strcmp(a->argv[0], "global"))
    {
        ...
        if (a->argc < 3)
            CHECK_ERROR(a->virtualPox, SetExtraData(Bstr(a->argv[1]).raw(),
                                                    NULL));
        else if (a->argc == 3)
            CHECK_ERROR(a->virtualPox, SetExtraData(Bstr(a->argv[1]).raw(),
                                                    Bstr(a->argv[2]).raw()));
...
RTEXITCODE handleSetProperty(HandlerArg *a)
{
    ...
    a->virtualPox->COMGETTER(SystemProperties)(systemProperties.asOutParam());
...
static RTEXITCODE handleSharedFolderAdd(HandlerArg *a)
{
    ...
    CHECK_ERROR2I_RET(a->virtualPox, FindMachine(Bstr(pszMachineName).raw(), ptrMachine.asOutParam()), RTEXITCODE_FAILURE);
...
RTEXITCODE handleExtPack(HandlerArg *a)
{
    ...
    CHECK_ERROR2I_RET(a->virtualPox, COMGETTER(ExtensionPackManager)(ptrExtPackMgr.asOutParam()), RTEXITCODE_FAILURE);
            ...
            else
            {
                ...
                if (szDigest[0])
                    RTPrintf("License accepted. For batch installation add\n"
                             "--accept-license=%s\n"
                             "to the VPoxManage command line.\n\n", szDigest);
            }
        }
...
RTEXITCODE handleUnattendedDetect(HandlerArg *a)
{
    ...
    CHECK_ERROR2_RET(hrc, a->virtualPox, CreateUnattendedInstaller(ptrUnattended.asOutParam()), RTEXITCODE_FAILURE);
    ...
    while ((c = RTGetOpt(&GetState, &ValueUnion)) != 0)
    {
        switch (c)
        {
            case VINF_GETOPT_NOT_OPTION:
                ...
                CHECK_ERROR2_RET(hrc, a->virtualPox, FindMachine(Bstr(ValueUnion.psz).raw(), ptrMachine.asOutParam()), RTEXITCODE_FAILURE);
...
#undef SHOW_ATTR
    ...
    {
        if (!fDryRun)
            RTMsgInfo("VM '%ls' (%ls) is ready to be started (e.g. VPoxManage startvm).\n", bstrMachineName.raw(), bstrUuid.raw());
        hrc = S_OK;
    }
    else
    {
        com::SafeArray<IN_BSTR> aBstrEnv;
#if defined(RT_OS_LINUX) || defined(RT_OS_SOLARIS)
        /* make sure the VM process will start on the same display as VPoxManage */
        const char *pszDisplay = RTEnvGet("DISPLAY");
        if (pszDisplay)
            aBstrEnv.push_back(BstrFmt("DISPLAY=%s", pszDisplay).raw());
        const char *pszXAuth = RTEnvGet("XAUTHORITY");
...
static RTEXITCODE setCloudProfileProperties(HandlerArg *a, int iFirst, PCLOUDPROFILECOMMONOPT pCommonOpts)
{
    ...
    CHECK_ERROR2_RET(hrc, pVirtualPox,
