/* $Id: VPoxDnDDropTarget.cpp $ */
/** @file
 * VPoxDnDTarget.cpp - IDropTarget implementation.
 */
...
VPoxDnDDropTarget::VPoxDnDDropTarget(VPoxDnDWnd *pParent)
    : m_cRefs(1),
...
VPoxDnDDropTarget::~VPoxDnDDropTarget(void)
{
    reset();
...
STDMETHODIMP_(ULONG) VPoxDnDDropTarget::AddRef(void)
{
    return InterlockedIncrement(&m_cRefs);
}

STDMETHODIMP_(ULONG) VPoxDnDDropTarget::Release(void)
{
...
STDMETHODIMP VPoxDnDDropTarget::QueryInterface(REFIID iid, void **ppvObject)
{
 ...
 */
/* static */
void VPoxDnDDropTarget::DumpFormats(IDataObject *pDataObject)
{
    ...
    if (SUCCEEDED(hr2))
    {
                                  ...
                                  NULL /* pceltFetched */) == S_OK)
        {
            ...
            LogRel(("\tcfFormat=%RI16 (%s), tyMed=%RI32, dwAspect=%RI32, strCustomName=%ls, hr=%Rhrc\n",
                    ...
                    VPoxDnDDataObject::ClipboardFormatToString(curFormatEtc.cfFormat),
                    curFormatEtc.tymed,
...
STDMETHODIMP VPoxDnDDropTarget::DragEnter(IDataObject *pDataObject, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
    RT_NOREF(pt);
...
#ifdef DEBUG
    VPoxDnDDropTarget::DumpFormats(pDataObject);
#endif
    ...
    if (fmtEtc.cfFormat)
    {
        LogFlowFunc(("Found supported format %RI16 (%s)\n",
                     fmtEtc.cfFormat, VPoxDnDDataObject::ClipboardFormatToString(fmtEtc.cfFormat)));
         ...
         *       proxy window. */
        *pdwEffect = VPoxDnDDropTarget::GetDropEffect(grfKeyState, *pdwEffect);
    }
    else
    {
        ...
        *pdwEffect = DROPEFFECT_NONE;
        ...
        switch (hr)
        {
            ...
            {
                LogRel(("DnD: Drag and drop format is not supported by VPoxTray\n"));
                VPoxDnDDropTarget::DumpFormats(pDataObject);
                break;
            }
...
STDMETHODIMP VPoxDnDDropTarget::DragOver(DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
    RT_NOREF(pt);
    AssertPtrReturn(pdwEffect, E_INVALIDARG);
...
STDMETHODIMP VPoxDnDDropTarget::DragLeave(void)
{
#ifdef DEBUG_andy
...
STDMETHODIMP VPoxDnDDropTarget::Drop(IDataObject *pDataObject, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
    RT_NOREF(pt);
    AssertPtrReturn(pDataObject, E_INVALIDARG);
    AssertPtrReturn(pdwEffect,   E_INVALIDARG);
    ...
    LogFlowFunc(("mFormatEtc.cfFormat=%RI16 (%s), pDataObject=0x%p, grfKeyState=0x%x, x=%ld, y=%ld\n",
                 m_FormatEtc.cfFormat, VPoxDnDDataObject::ClipboardFormatToString(m_FormatEtc.cfFormat),
    ...
    if (m_FormatEtc.cfFormat) /* Did we get a supported format yet? */
    {
        ...
        AssertMsg(SUCCEEDED(hr),
                  ...
                  m_FormatEtc.cfFormat, VPoxDnDDataObject::ClipboardFormatToString(m_FormatEtc.cfFormat), hr));
    }
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                switch (m_FormatEtc.cfFormat)
                {
                    ...
                    case CF_HDROP:
                    {
                        ...
                        /** @todo Replace / merge the following code with VPoxShClWinDropFilesToStringList(). */
    ...
    LogFlowFunc(("Returning with hr=%Rhrc (%Rrc), mFormatEtc.cfFormat=%RI16 (%s), *pdwEffect=%RI32\n",
                 hr, rc, m_FormatEtc.cfFormat, VPoxDnDDataObject::ClipboardFormatToString(m_FormatEtc.cfFormat),
                 *pdwEffect));
 ...
 */
/* static */
DWORD VPoxDnDDropTarget::GetDropEffect(DWORD grfKeyState, DWORD dwAllowedEffects)
{
 ...
 */
void VPoxDnDDropTarget::reset(void)
{
    LogFlowFuncEnter();
 ...
 */
RTCString VPoxDnDDropTarget::Formats(void) const
{
    return m_strFormat;
}
 ...
 */
int VPoxDnDDropTarget::WaitForDrop(RTMSINTERVAL msTimeout)
{
