...
#include "VPoxDisp.h"
#include <VPox/RemoteDesktop/VRDEOrders.h>
...
static void vrdpReportDirtyPathBounds(PVPOXDISPDEV pDev, CLIPOBJ *pco, PATHOBJ *ppo)
{
    RECTFX rcfxBounds;
    RECTL rclBounds;
...
static void vrdpReportDirtyPath(PVPOXDISPDEV pDev, CLIPOBJ *pco, PATHOBJ *ppo)
{
    vrdpReportDirtyPathBounds(pDev, pco, ppo);
}

static void vrdpReportDirtyClip(PVPOXDISPDEV pDev, CLIPOBJ *pco, RECTL *prcl)
{
    if (prcl)
    {
...
static void vrdpReportDirtyRects(PVPOXDISPDEV pDev, VRDPCLIPRECTS *pClipRects)
{
    /* Ignore rects, report entire area. */
    vrdpReportDirtyRect (pDev, &pClipRects->rclDst);
}

__inline BOOL vrdpWriteHdr (PVPOXDISPDEV pDev, uint32_t u32Op)
{
    return VPoxVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, &u32Op, sizeof (u32Op));
}

static BOOL vrdpWriteBits (PVPOXDISPDEV pDev, uint8_t *pu8Bits, int lDelta, int32_t x, int32_t y, uint32_t cWidth, uint32_t cHeight, int bytesPerPixel)
{
    BOOL bRc = FALSE;
    ...
    bRc = VPoxVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, &bits, sizeof (bits));

    if (bRc)
    {
        while (cHeight--)
        {
            bRc = VPoxVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, pu8Bits, cWidth * bytesPerPixel);
 ...
 */
static BOOL vrdpReportOrder(PVPOXDISPDEV pDev, const void *pOrder, unsigned cbOrder, unsigned code)
{
    BOOL bRc = vrdpWriteHdr(pDev, VRDP_MAKE_OP(code));

    if (bRc)
    {
        VPoxVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, pOrder, cbOrder);
    }

    return bRc;
}

static BOOL vrdpReportBounds(PVPOXDISPDEV pDev, const RECTL *prcl)
{
...
static BOOL vrdpReportRepeat(PVPOXDISPDEV pDev, const CLIPRECTS *pRects)
{
    BOOL bRc = TRUE;
...
BOOL vrdpReportOrderGeneric (PVPOXDISPDEV pDev,
                             const VRDPCLIPRECTS *pClipRects,
                             const void *pvOrder,
                             unsigned cbOrder,
                             unsigned code)
...
static void vrdpReportOrderGenericBounds (PVPOXDISPDEV pDev,
                                          const VRDPCLIPRECTS *pClipRects,
                                          const VRDEORDERBOUNDS *pBounds,
                                          const void *pvOrder,
                                          unsigned cbOrder,
...
static void vrdpReportSolidRect (PVPOXDISPDEV pDev,
                                 const RECTL *prclTrg,
...
static void vrdpReportSolidBlt (PVPOXDISPDEV pDev,
                                const RECTL *prclTrg,
                                VRDPCLIPRECTS *pClipRects,
                                ULONG rgb,
                                uint8_t rop3)
...
static void vrdpReportPatBlt (PVPOXDISPDEV pDev,
                              const RECTL *prclTrg,
                              VRDPCLIPRECTS *pClipRects,
                              VRDPBRUSH *pBrush,
                              POINTL *pptlBrush,
...
static void vrdpReportDstBlt (PVPOXDISPDEV pDev,
                              const RECTL *prclTrg,
                              VRDPCLIPRECTS *pClipRects,
                              uint8_t rop3)
{
...
static void vrdpReportScreenBlt (PVPOXDISPDEV pDev,
                                 const RECTL *prclTrg,
                                 VRDPCLIPRECTS *pClipRects,
                                 POINTL *pptlSrc,
                                 uint8_t rop3)
...
static void vrdpReportMemBltRect (PVPOXDISPDEV pDev,
                                  RECTL *prcl,
                                  int xSrc,
                                  int ySrc,
                                  uint8_t rop3,
...
static void vrdpReportMemBlt (PVPOXDISPDEV pDev,
                              VRDPCLIPRECTS *pClipRects,
                              POINTL *pptlSrc,
                              const uint8_t rop3,
                              const VRDPBCHASH *phash)
...
static void vrdpReportCachedBitmap (PVPOXDISPDEV pDev,
                                    SURFOBJ *psoSrc,
                                    const VRDPBCHASH *phash)
{
    BOOL bRc;
...
static void vrdpReportDeletedBitmap (PVPOXDISPDEV pDev,
                                     const VRDPBCHASH *phash)
{
                       ...
                       BRUSHOBJ  *pbo, POINTL *pptlBrushOrg, LINEATTRS *plineattrs, MIX mix)
{
    ...
    {
        if (VPoxVBVAOrderSupported(&pDev->vbvaCtx, VRDE_ORDER_ELLIPSE))
        {
                    ...
                    BRUSHOBJ *pboFore, BRUSHOBJ *pboOpaque, POINTL *pptlOrg, MIX mix)
{
    ...
    else
    {
        ...
        LOG(("calling vpoxReportText fg %x bg %x", ulForeRGB, ulBackRGB));
                   ...
                   RECTL *prclTrg, POINTL *pptlSrc, POINTL *pptlMask, BRUSHOBJ *pbo, POINTL *pptlBrush, ROP4 rop4)
{
    ...
    PVPOXDISPDEV pDev = (PVPOXDISPDEV)psoTrg->dhpdev;
    ...
    clipResult = vrdpGetIntersectingClipRects(&clipRects, psoTrg, &rclTrg, pco,
                                              VPoxDispIsScreenSurface(psoSrc)? pptlSrc: NULL);
    ...
    else
    {
        ...
        if (ROP3_NEED_SRC(rop4))
        {
            ...
            /* MEMBLT or SCREENBLT. */
            if (VPoxDispIsScreenSurface(psoSrc))
            {
...
BOOL vrdpDrvRealizeBrush(BRUSHOBJ *pbo, SURFOBJ *psoTarget, SURFOBJ *psoPattern, SURFOBJ *psoMask, XLATEOBJ *pxlo, ULONG iHatch)
{
    ...
    VPoxDispDumpPSO(psoPattern, "psoPattern");
