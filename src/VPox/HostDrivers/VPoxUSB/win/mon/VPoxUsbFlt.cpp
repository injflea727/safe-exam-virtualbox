...
typedef enum
{
    VPOXUSBFLT_DEVSTATE_UNKNOWN = 0,
    VPOXUSBFLT_DEVSTATE_REMOVED,
    VPOXUSBFLT_DEVSTATE_REPLUGGING,
    VPOXUSBFLT_DEVSTATE_ADDED,
    VPOXUSBFLT_DEVSTATE_UNCAPTURED,
    VPOXUSBFLT_DEVSTATE_CAPTURING,
    VPOXUSBFLT_DEVSTATE_CAPTURED,
    VPOXUSBFLT_DEVSTATE_USED_BY_GUEST,
    VPOXUSBFLT_DEVSTATE_32BIT_HACK = 0x7fffffff
} VPOXUSBFLT_DEVSTATE;

typedef struct VPOXUSBFLT_DEVICE
{
    LIST_ENTRY      GlobalLe;
    ...
    BOOLEAN         fHighSpeed;
#endif
} VPOXUSBFLT_DEVICE, *PVPOXUSBFLT_DEVICE;

#define PVPOXUSBFLT_DEVICE_FROM_LE(_pLe) ( (PVPOXUSBFLT_DEVICE)( ((uint8_t*)(_pLe)) - RT_OFFSETOF(VPOXUSBFLT_DEVICE, GlobalLe) ) )
#define PVPOXUSBFLT_DEVICE_FROM_REPLUGGINGLE(_pLe)  ( (PVPOXUSBFLT_DEVICE)( ((uint8_t*)(_pLe)) - RT_OFFSETOF(VPOXUSBFLT_DEVICE, RepluggingLe) ) )
#define PVPOXUSBFLTCTX_FROM_LE(_pLe) ( (PVPOXUSBFLTCTX)( ((uint8_t*)(_pLe)) - RT_OFFSETOF(VPOXUSBFLTCTX, ListEntry) ) )

typedef struct VPOXUSBFLT_LOCK
{
    KSPIN_LOCK Lock;
    KIRQL OldIrql;
} VPOXUSBFLT_LOCK, *PVPOXUSBFLT_LOCK;

#define VPOXUSBFLT_LOCK_INIT() \
    KeInitializeSpinLock(&g_VPoxUsbFltGlobals.Lock.Lock)
#define VPOXUSBFLT_LOCK_TERM() do { } while (0)
#define VPOXUSBFLT_LOCK_ACQUIRE() \
    KeAcquireSpinLock(&g_VPoxUsbFltGlobals.Lock.Lock, &g_VPoxUsbFltGlobals.Lock.OldIrql);
#define VPOXUSBFLT_LOCK_RELEASE() \
    KeReleaseSpinLock(&g_VPoxUsbFltGlobals.Lock.Lock, g_VPoxUsbFltGlobals.Lock.OldIrql);


typedef struct VPOXUSBFLT_BLDEV
{
    LIST_ENTRY ListEntry;
    uint16_t   idVendor;
    uint16_t   idProduct;
    uint16_t   bcdDevice;
} VPOXUSBFLT_BLDEV, *PVPOXUSBFLT_BLDEV;

#define PVPOXUSBFLT_BLDEV_FROM_LE(_pLe) ( (PVPOXUSBFLT_BLDEV)( ((uint8_t*)(_pLe)) - RT_OFFSETOF(VPOXUSBFLT_BLDEV, ListEntry) ) )

typedef struct VPOXUSBFLTGLOBALS
{
    LIST_ENTRY DeviceList;
    LIST_ENTRY ContextList;
    /* devices known to misbehave */
    LIST_ENTRY BlackDeviceList;
    VPOXUSBFLT_LOCK Lock;
...
} VPOXUSBFLTGLOBALS, *PVPOXUSBFLTGLOBALS;
static VPOXUSBFLTGLOBALS g_VPoxUsbFltGlobals;

static bool vpoxUsbFltBlDevMatchLocked(uint16_t idVendor, uint16_t idProduct, uint16_t bcdDevice)
{
    for (PLIST_ENTRY pEntry = g_VPoxUsbFltGlobals.BlackDeviceList.Flink;
            pEntry != &g_VPoxUsbFltGlobals.BlackDeviceList;
            pEntry = pEntry->Flink)
    {
        PVPOXUSBFLT_BLDEV pDev = PVPOXUSBFLT_BLDEV_FROM_LE(pEntry);
...
static NTSTATUS vpoxUsbFltBlDevAddLocked(uint16_t idVendor, uint16_t idProduct, uint16_t bcdDevice)
{
    if (vpoxUsbFltBlDevMatchLocked(idVendor, idProduct, bcdDevice))
        return STATUS_SUCCESS;
    PVPOXUSBFLT_BLDEV pDev = (PVPOXUSBFLT_BLDEV)VPoxUsbMonMemAllocZ(sizeof (*pDev));
    if (!pDev)
    {
        AssertFailed();
    ...
    InsertHeadList(&g_VPoxUsbFltGlobals.BlackDeviceList, &pDev->ListEntry);
    return STATUS_SUCCESS;
}

static void vpoxUsbFltBlDevClearLocked()
{
    PLIST_ENTRY pNext;
    for (PLIST_ENTRY pEntry = g_VPoxUsbFltGlobals.BlackDeviceList.Flink;
            pEntry != &g_VPoxUsbFltGlobals.BlackDeviceList;
    ...
    {
        pNext = pEntry->Flink;
        VPoxUsbMonMemFree(pEntry);
    }
}

static void vpoxUsbFltBlDevPopulateWithKnownLocked()
{
    /* this one halts when trying to get string descriptors from it */
    vpoxUsbFltBlDevAddLocked(0x5ac, 0x921c, 0x115);
}


DECLINLINE(void) vpoxUsbFltDevRetain(PVPOXUSBFLT_DEVICE pDevice)
{
    Assert(pDevice->cRefs);
    ASMAtomicIncU32(&pDevice->cRefs);
}

static void vpoxUsbFltDevDestroy(PVPOXUSBFLT_DEVICE pDevice)
{
    Assert(!pDevice->cRefs);
    Assert(pDevice->enmState == VPOXUSBFLT_DEVSTATE_REMOVED);
    VPoxUsbMonMemFree(pDevice);
}

DECLINLINE(void) vpoxUsbFltDevRelease(PVPOXUSBFLT_DEVICE pDevice)
{
    uint32_t cRefs = ASMAtomicDecU32(&pDevice->cRefs);
    Assert(cRefs < UINT32_MAX/2);
    if (!cRefs)
    {
        vpoxUsbFltDevDestroy(pDevice);
    }
}

static void vpoxUsbFltDevOwnerSetLocked(PVPOXUSBFLT_DEVICE pDevice, PVPOXUSBFLTCTX pContext, uintptr_t uFltId, bool fIsOneShot)
{
...
static void vpoxUsbFltDevOwnerClearLocked(PVPOXUSBFLT_DEVICE pDevice)
{
...
static void vpoxUsbFltDevOwnerUpdateLocked(PVPOXUSBFLT_DEVICE pDevice, PVPOXUSBFLTCTX pContext, uintptr_t uFltId, bool fIsOneShot)
{
    if (pDevice->pOwner != pContext)
    {
        if (pDevice->pOwner)
            vpoxUsbFltDevOwnerClearLocked(pDevice);
        if (pContext)
            vpoxUsbFltDevOwnerSetLocked(pDevice, pContext, uFltId, fIsOneShot);
    }
    else if (pContext)
    {
        pDevice->uFltId = uFltId;
...
static PVPOXUSBFLT_DEVICE vpoxUsbFltDevGetLocked(PDEVICE_OBJECT pPdo)
{
#ifdef VPOX_USB_WITH_VERBOSE_LOGGING
    for (PLIST_ENTRY pEntry = g_VPoxUsbFltGlobals.DeviceList.Flink;
            pEntry != &g_VPoxUsbFltGlobals.DeviceList;
            pEntry = pEntry->Flink)
    {
        PVPOXUSBFLT_DEVICE pDevice = PVPOXUSBFLT_DEVICE_FROM_LE(pEntry);
        for (PLIST_ENTRY pEntry2 = pEntry->Flink;
                pEntry2 != &g_VPoxUsbFltGlobals.DeviceList;
                pEntry2 = pEntry2->Flink)
        {
            PVPOXUSBFLT_DEVICE pDevice2 = PVPOXUSBFLT_DEVICE_FROM_LE(pEntry2);
    ...
    {
        PVPOXUSBFLT_DEVICE pDevice = PVPOXUSBFLT_DEVICE_FROM_LE(pEntry);
        ASSERT_WARN(    pDevice->enmState == VPOXUSBFLT_DEVSTATE_REPLUGGING
                || pDevice->enmState == VPOXUSBFLT_DEVSTATE_UNCAPTURED
                || pDevice->enmState == VPOXUSBFLT_DEVSTATE_CAPTURING
                || pDevice->enmState == VPOXUSBFLT_DEVSTATE_CAPTURED
                || pDevice->enmState == VPOXUSBFLT_DEVSTATE_USED_BY_GUEST,
...
static NTSTATUS vpoxUsbFltPdoReplug(PDEVICE_OBJECT pDo)
{
    LOG(("Replugging PDO(0x%p)", pDo));
    NTSTATUS Status = VPoxUsbToolIoInternalCtlSendSync(pDo, IOCTL_INTERNAL_USB_CYCLE_PORT, NULL, NULL);
...
static bool vpoxUsbFltDevCanBeCaptured(PVPOXUSBFLT_DEVICE pDevice)
{
...
static PVPOXUSBFLTCTX vpoxUsbFltDevMatchLocked(PVPOXUSBFLT_DEVICE pDevice, uintptr_t *puId, bool fRemoveFltIfOneShot, bool *pfFilter, bool *pfIsOneShot)
{
    *puId = 0;
    *pfFilter = false;
    *pfIsOneShot = false;
    if (!vpoxUsbFltDevCanBeCaptured(pDevice))
    {
        LOG(("vpoxUsbFltDevCanBeCaptured returned false"));
        return NULL;
    }
    ...
    PVPOXUSBFLTCTX pOwner = VPoxUSBFilterMatchEx(&DevFlt, puId, fRemoveFltIfOneShot, pfFilter, pfIsOneShot);
    USBFilterDelete(&DevFlt);
    return pOwner;
}

static void vpoxUsbFltDevStateMarkReplugLocked(PVPOXUSBFLT_DEVICE pDevice)
{
    vpoxUsbFltDevOwnerUpdateLocked(pDevice, NULL, 0, false);
    pDevice->enmState = VPOXUSBFLT_DEVSTATE_REPLUGGING;
}

static bool vpoxUsbFltDevStateIsNotFiltered(PVPOXUSBFLT_DEVICE pDevice)
{
    return pDevice->enmState == VPOXUSBFLT_DEVSTATE_UNCAPTURED;
}

static bool vpoxUsbFltDevStateIsFiltered(PVPOXUSBFLT_DEVICE pDevice)
{
    return pDevice->enmState >= VPOXUSBFLT_DEVSTATE_CAPTURING;
}

static uint16_t vpoxUsbParseHexNumU16(WCHAR **ppStr)
{
    WCHAR       *pStr = *ppStr;
    WCHAR       wc;
    uint16_t    num = 0;
...
static uint8_t vpoxUsbParseHexNumU8(WCHAR **ppStr)
{
    WCHAR       *pStr = *ppStr;
    WCHAR       wc;
    uint16_t    num = 0;
...
static bool vpoxUsbParseHardwareID(WCHAR *pchIdStr, uint16_t *pVid, uint16_t *pPid, uint16_t *pRev)
{
...
#define REV_PREFIX  L"&REV_"
    ...
    *pVid = vpoxUsbParseHexNumU16(&pchIdStr);
    ...
    *pPid = vpoxUsbParseHexNumU16(&pchIdStr);
    ...
    *pRev = vpoxUsbParseHexNumU16(&pchIdStr);
...
static bool vpoxUsbParseCompatibleIDs(WCHAR *pchIdStr, uint8_t *pClass, uint8_t *pSubClass, uint8_t *pProt)
{
...
#define PRO_PREFIX  L"&Prot_"
    ...
    *pClass = vpoxUsbParseHexNumU8(&pchIdStr);
    ...
    *pSubClass = vpoxUsbParseHexNumU8(&pchIdStr);
    ...
    *pProt = vpoxUsbParseHexNumU8(&pchIdStr);
...
#define VPOXUSBMON_POPULATE_REQUEST_TIMEOUT_MS 10000

static NTSTATUS vpoxUsbFltDevPopulate(PVPOXUSBFLT_DEVICE pDevice, PDEVICE_OBJECT pDo /*, BOOLEAN bPopulateNonFilterProps*/)
{
    NTSTATUS                Status;
    ...
    pDevDr = (PUSB_DEVICE_DESCRIPTOR)VPoxUsbMonMemAllocZ(sizeof(*pDevDr));
    if (pDevDr == NULL)
    {
    ...
    do
    {
        ...
        Status = VPoxUsbToolGetDescriptor(pDo, pDevDr, sizeof(*pDevDr), USB_DEVICE_DESCRIPTOR_TYPE, 0, 0, VPOXUSBMON_POPULATE_REQUEST_TIMEOUT_MS);
        if (!NT_SUCCESS(Status))
        {
            ...
            rc = vpoxUsbParseHardwareID(wchPropBuf, &vid, &pid, &rev);
            if (!rc)
            {
            ...
            rc = vpoxUsbParseCompatibleIDs(wchPropBuf, &cls, &sub, &prt);
            if (!rc)
            {
                /* This *really* should not happen. */
                WARN(("Failed to parse Hardware ID"));
...
#if 0
        {
            ...
            NTSTATUS Status = VPoxUsbToolIoInternalCtlSendSync(pDo, IOCTL_INTERNAL_USB_GET_DEVICE_CONFIG_INFO, &HubInfo, NULL);
            ...
            if (Status == STATUS_SUCCESS)
            {
                ...
                rc = vpoxUsbParseHardwareID(HubInfo.HardwareIds.Buffer, &pid, &vid, &rev);
                if (!rc)
                {
                    /* This *really* should not happen. */
                    WARN(("Failed to parse Hardware ID"));
                 ...
                 */
                rc = vpoxUsbParseCompatibleIDs(HubInfo.CompatibleIds.Buffer, &cls, &sub, &prt);
                if (!rc)
                {
        ...
        Status = VPoxUsbToolIoInternalCtlSendSync(pDo, IOCTL_INTERNAL_USB_GET_TOPOLOGY_ADDRESS, &TopoAddr, NULL);
        ...
        if (vpoxUsbFltBlDevMatchLocked(pDevDr->idVendor, pDevDr->idProduct, pDevDr->bcdDevice))
        {
        ...
        if (pDevDr->iSerialNumber || pDevDr->iManufacturer || pDevDr->iProduct)
        {
            ...
            Status = VPoxUsbToolGetLangID(pDo, &langId, VPOXUSBMON_POPULATE_REQUEST_TIMEOUT_MS);
            if (!NT_SUCCESS(Status))
            {
                ...
                if (Status == STATUS_CANCELLED)
                {
                    ...
                    vpoxUsbFltBlDevAddLocked(pDevDr->idVendor, pDevDr->idProduct, pDevDr->bcdDevice);
                    Status = STATUS_UNSUCCESSFUL;
                }
                break;
            }
            ...
            {
                Status = VPoxUsbToolGetStringDescriptor(pDo, pDevice->szSerial, sizeof (pDevice->szSerial), pDevDr->iSerialNumber, langId, VPOXUSBMON_POPULATE_REQUEST_TIMEOUT_MS);
                if (!NT_SUCCESS(Status))
                {
            ...
            {
                Status = VPoxUsbToolGetStringDescriptor(pDo, pDevice->szMfgName, sizeof (pDevice->szMfgName), pDevDr->iManufacturer, langId, VPOXUSBMON_POPULATE_REQUEST_TIMEOUT_MS);
                if (!NT_SUCCESS(Status))
                {
            ...
            if (pDevDr->iProduct)
            {
                Status = VPoxUsbToolGetStringDescriptor(pDo, pDevice->szProduct, sizeof (pDevice->szProduct), pDevDr->iProduct, langId, VPOXUSBMON_POPULATE_REQUEST_TIMEOUT_MS);
                if (!NT_SUCCESS(Status))
                {
    ...
    VPoxUsbMonMemFree(pDevDr);
    LOG(("Populating Device(0x%p) for PDO(0x%p) Done, Status (0x%x)", pDevice, pDo, Status));
    return Status;
}

static bool vpoxUsbFltDevCheckReplugLocked(PVPOXUSBFLT_DEVICE pDevice, PVPOXUSBFLTCTX pContext)
{
    ...
    if (pDevice->enmState <= VPOXUSBFLT_DEVSTATE_ADDED)
    {
    ...
    PVPOXUSBFLTCTX pNewOwner = vpoxUsbFltDevMatchLocked(pDevice, &uId,
    ...
    if (!fFilter)
    {
        ...
        if (vpoxUsbFltDevStateIsNotFiltered(pDevice))
        {
            ...
            if (fIsOneShot)
            {
                ...
                int tmpRc = VPoxUSBFilterRemove(pNewOwner, uId);
                ...
                if (!pDevice->pOwner)
                {
                    ...
                    vpoxUsbFltDevOwnerUpdateLocked(pDevice, pNewOwner, uId, true);
                }
                else
                {
            ...
            else
            {
                ...
                if (pNewOwner)
                {
                    vpoxUsbFltDevOwnerUpdateLocked(pDevice, pNewOwner, uId, false);
                }
            }
        }
        else
    ...
    else
    {
        ...
        if (vpoxUsbFltDevStateIsFiltered(pDevice))
        {
            LOG(("Device (0x%p) IS filtered", pDevice));
            ...
            if (pNewOwner == pDevice->pOwner)
            {
                ...
                else
                {
                    ...
                    vpoxUsbFltDevOwnerUpdateLocked(pDevice, pDevice->pOwner, uId, false);
                }
            }
            else
            {
    ...
    if (bNeedReplug)
    {
        ...
        vpoxUsbFltDevStateMarkReplugLocked(pDevice);
    }
    else
    {
...
static void vpoxUsbFltReplugList(PLIST_ENTRY pList)
{
    PLIST_ENTRY pNext;
    ...
    {
        pNext = pEntry->Flink;
        PVPOXUSBFLT_DEVICE pDevice = PVPOXUSBFLT_DEVICE_FROM_REPLUGGINGLE(pEntry);
        LOG(("replugging matched PDO(0x%p), pDevice(0x%p)", pDevice->Pdo, pDevice));
        ASSERT_WARN(pDevice->enmState == VPOXUSBFLT_DEVSTATE_REPLUGGING
                || pDevice->enmState == VPOXUSBFLT_DEVSTATE_REMOVED,
                ("invalid state(0x%x) for device(0x%p)", pDevice->enmState, pDevice));

        vpoxUsbFltPdoReplug(pDevice->Pdo);
        ObDereferenceObject(pDevice->Pdo);
        vpoxUsbFltDevRelease(pDevice);
    }
}

typedef struct VPOXUSBFLTCHECKWALKER
{
    PVPOXUSBFLTCTX pContext;
} VPOXUSBFLTCHECKWALKER, *PVPOXUSBFLTCHECKWALKER;

static DECLCALLBACK(BOOLEAN) vpoxUsbFltFilterCheckWalker(PFILE_OBJECT pHubFile,
                                                         PDEVICE_OBJECT pHubDo, PVOID pvContext)
{
    PVPOXUSBFLTCHECKWALKER pData = (PVPOXUSBFLTCHECKWALKER)pvContext;
    PVPOXUSBFLTCTX pContext = pData->pContext;
    ...
    NTSTATUS Status = VPoxUsbMonQueryBusRelations(pHubDo, pHubFile, &pDevRelations);
    if (Status == STATUS_SUCCESS && pDevRelations)
    {
        ...
        for (ULONG k = 0; k < pDevRelations->Count; ++k)
        {
            ...
            VPOXUSBFLT_LOCK_ACQUIRE();
            PVPOXUSBFLT_DEVICE pDevice = vpoxUsbFltDevGetLocked(pDevObj);
            if (pDevice)
            {
                LOG(("Found existing device info (0x%p) for PDO 0x%p", pDevice, pDevObj));
                bool bReplug = vpoxUsbFltDevCheckReplugLocked(pDevice, pContext);
                if (bReplug)
                {
                    LOG(("Replug needed for device (0x%p)", pDevice));
                    InsertHeadList(&ReplugDevList, &pDevice->RepluggingLe);
                    vpoxUsbFltDevRetain(pDevice);
            ...
            VPOXUSBFLT_DEVICE Device;
            Status = vpoxUsbFltDevPopulate(&Device, pDevObj /*, FALSE /* only need filter properties */);
            if (NT_SUCCESS(Status))
            {
                uintptr_t uId = 0;
                bool fFilter = false;
                bool fIsOneShot = false;
                VPOXUSBFLT_LOCK_ACQUIRE();
                PVPOXUSBFLTCTX pCtx = vpoxUsbFltDevMatchLocked(&Device, &uId,
                                                               false, /* do not remove a one-shot filter */
                                                               &fFilter, &fIsOneShot);
                VPOXUSBFLT_LOCK_RELEASE();
                NOREF(pCtx);
            ...
            {
                WARN(("vpoxUsbFltDevPopulate for PDO 0x%p failed with Status 0x%x", pDevObj, Status));
                if (   Status == STATUS_CANCELLED
                    && g_VPoxUsbFltGlobals.dwForceReplugWhenDevPopulateFails)
                {
                    /*
        ...
        if (cReplugPdos)
        {
            for (ULONG k = 0; k < pDevRelations->Count; ++k)
            {
                ...
                Status = vpoxUsbFltPdoReplug(pDevRelations->Objects[k]);
                ASSERT_WARN(Status == STATUS_SUCCESS, ("vpoxUsbFltPdoReplug failed! Status(0x%x)", Status));
                ObDereferenceObject(pDevRelations->Objects[k]);
                if (!--cReplugPdos)
                    break;
            }
        ...
        vpoxUsbFltReplugList(&ReplugDevList);

        ExFreePool(pDevRelations);
    }
    else
    {
        WARN(("VPoxUsbMonQueryBusRelations failed for hub DO(0x%p), Status(0x%x), pDevRelations(0x%p)",
...
NTSTATUS VPoxUsbFltFilterCheck(PVPOXUSBFLTCTX pContext)
{
    ...
    VPOXUSBFLTCHECKWALKER Data;
    Data.pContext = pContext;
    vpoxUsbMonHubDevWalk(vpoxUsbFltFilterCheckWalker, &Data);
...
NTSTATUS VPoxUsbFltClose(PVPOXUSBFLTCTX pContext)
{
    ...
    /* 1. remove filters */
    VPoxUSBFilterRemoveOwner(pContext);
         ...
         pEntry = pEntry->Flink)
    {
        ...
        ASSERT_WARN(pDevice->enmState != VPOXUSBFLT_DEVSTATE_ADDED, ("VPOXUSBFLT_DEVSTATE_ADDED state for device(0x%p)", pDevice));
        ASSERT_WARN(pDevice->enmState != VPOXUSBFLT_DEVSTATE_REMOVED, ("VPOXUSBFLT_DEVSTATE_REMOVED state for device(0x%p)", pDevice));

        vpoxUsbFltDevOwnerClearLocked(pDevice);

        if (vpoxUsbFltDevCheckReplugLocked(pDevice, pContext))
        {
...
NTSTATUS VPoxUsbFltCreate(PVPOXUSBFLTCTX pContext)
{
    ...
    VPOXUSBFLT_LOCK_ACQUIRE();
    InsertHeadList(&g_VPoxUsbFltGlobals.ContextList, &pContext->ListEntry);
    VPOXUSBFLT_LOCK_RELEASE();
...
int VPoxUsbFltAdd(PVPOXUSBFLTCTX pContext, PUSBFILTER pFilter, uintptr_t *pId)
{
    ...
    *pId = 0;
...
#ifdef VPOX_USB_WITH_VERBOSE_LOGGING
    LOG(("VPoxUSBClient::addFilter: idVendor=%#x idProduct=%#x bcdDevice=%#x bDeviceClass=%#x bDeviceSubClass=%#x bDeviceProtocol=%#x bBus=%#x bPort=%#x Type%#x",
    ...
    VPOXUSBFLT_LOCK_ACQUIRE();
    /* Add the filter. */
    int rc = VPoxUSBFilterAdd(pFilter, pContext, &uId);
    VPOXUSBFLT_LOCK_RELEASE();
    if (RT_SUCCESS(rc))
    {
        ...
        VPoxUsbFltFilterCheck();
#endif
    }
    else
    {
        WARN(("VPoxUSBFilterAdd failed rc (%d)", rc));
...
int VPoxUsbFltRemove(PVPOXUSBFLTCTX pContext, uintptr_t uId)
{
    ...
    VPOXUSBFLT_LOCK_ACQUIRE();
    int rc = VPoxUSBFilterRemove(pContext, uId);
    if (!RT_SUCCESS(rc))
    {
        WARN(("VPoxUSBFilterRemove failed rc (%d)", rc));
        VPOXUSBFLT_LOCK_RELEASE();
        return rc;
    }
            ...
            pEntry = pEntry->Flink)
    {
        ...
        pDevice->uFltId = 0;
        /* clear the fIsFilterOneShot flag to ensure the device is replugged on the next VPoxUsbFltFilterCheck call */
...
static USBDEVICESTATE vpoxUsbDevGetUserState(PVPOXUSBFLTCTX pContext, PVPOXUSBFLT_DEVICE pDevice)
{
    if (vpoxUsbFltDevStateIsNotFiltered(pDevice))
        return USBDEVICESTATE_USED_BY_HOST_CAPTURABLE;

    /* the device is filtered, or replugging */
    if (pDevice->enmState == VPOXUSBFLT_DEVSTATE_REPLUGGING)
    {
...
//    Assert(pDevice->uFltId);
    ...
    {
        case VPOXUSBFLT_DEVSTATE_UNCAPTURED:
        case VPOXUSBFLT_DEVSTATE_CAPTURING:
            return USBDEVICESTATE_USED_BY_HOST_CAPTURABLE;
        case VPOXUSBFLT_DEVSTATE_CAPTURED:
            return USBDEVICESTATE_HELD_BY_PROXY;
        case VPOXUSBFLT_DEVSTATE_USED_BY_GUEST:
...
NTSTATUS VPoxUsbFltGetDevice(PVPOXUSBFLTCTX pContext, HVPOXUSBDEVUSR hDevice, PUSBSUP_GETDEV_MON pInfo)
{
            ...
            pEntry = pEntry->Flink)
    {
        PVPOXUSBFLT_DEVICE pDevice = PVPOXUSBFLT_DEVICE_FROM_LE(pEntry);
        Assert(pDevice->enmState != VPOXUSBFLT_DEVSTATE_REMOVED);
        Assert(pDevice->enmState != VPOXUSBFLT_DEVSTATE_ADDED);

        if (pDevice != hDevice)
            continue;

        USBDEVICESTATE enmUsrState = vpoxUsbDevGetUserState(pContext, pDevice);
...
NTSTATUS VPoxUsbFltPdoAdd(PDEVICE_OBJECT pPdo, BOOLEAN *pbFiltered)
{
    *pbFiltered = FALSE;
    ...
    VPOXUSBFLT_LOCK_ACQUIRE();
    pDevice = vpoxUsbFltDevGetLocked(pPdo);
    if (pDevice)
    {
        LOG(("found device (0x%p), state(%d) for PDO(0x%p)", pDevice, pDevice->enmState, pPdo));
        ASSERT_WARN(pDevice->enmState != VPOXUSBFLT_DEVSTATE_ADDED, ("VPOXUSBFLT_DEVSTATE_ADDED state for device(0x%p)", pDevice));
        ASSERT_WARN(pDevice->enmState != VPOXUSBFLT_DEVSTATE_REMOVED, ("VPOXUSBFLT_DEVSTATE_REMOVED state for device(0x%p)", pDevice));
        *pbFiltered = pDevice->enmState >= VPOXUSBFLT_DEVSTATE_CAPTURING;
        VPOXUSBFLT_LOCK_RELEASE();
        ObDereferenceObject(pPdo);
        return STATUS_SUCCESS;
    }
    VPOXUSBFLT_LOCK_RELEASE();
    pDevice = (PVPOXUSBFLT_DEVICE)VPoxUsbMonMemAllocZ(sizeof (*pDevice));
    if (!pDevice)
    {
    ...
    pDevice->enmState = VPOXUSBFLT_DEVSTATE_ADDED;
    pDevice->cRefs = 1;
    NTSTATUS Status = vpoxUsbFltDevPopulate(pDevice, pPdo /* , TRUE /* need all props */);
    if (!NT_SUCCESS(Status))
    {
        WARN(("vpoxUsbFltDevPopulate failed, Status 0x%x", Status));
        ObDereferenceObject(pPdo);
        VPoxUsbMonMemFree(pDevice);
        return Status;
    }
    ...
    bool fIsOneShot = false;
    PVPOXUSBFLTCTX pCtx;
    PVPOXUSBFLT_DEVICE pTmpDev;
    VPOXUSBFLT_LOCK_ACQUIRE();
    /* (paranoia) re-check the device is still not here */
    pTmpDev = vpoxUsbFltDevGetLocked(pPdo);
    ...
    if (pTmpDev)
    {
        ...
        ASSERT_WARN(pDevice->enmState != VPOXUSBFLT_DEVSTATE_ADDED, ("second try: VPOXUSBFLT_DEVSTATE_ADDED state for device(0x%p)", pDevice));
        ASSERT_WARN(pDevice->enmState != VPOXUSBFLT_DEVSTATE_REMOVED, ("second try: VPOXUSBFLT_DEVSTATE_REMOVED state for device(0x%p)", pDevice));
        *pbFiltered = pTmpDev->enmState >= VPOXUSBFLT_DEVSTATE_CAPTURING;
        VPOXUSBFLT_LOCK_RELEASE();
        VPoxUsbMonMemFree(pDevice);
        return STATUS_SUCCESS;
    }
    ...
    pCtx = vpoxUsbFltDevMatchLocked(pDevice, &uId,
    ...
    if (pCtx)
        vpoxUsbFltDevOwnerSetLocked(pDevice, pCtx, fIsOneShot ? 0 : uId, fIsOneShot);

    InsertHeadList(&g_VPoxUsbFltGlobals.DeviceList, &pDevice->GlobalLe);
...
{
    VPOXUSBFLT_DEVSTATE enmState = VPOXUSBFLT_DEVSTATE_REMOVED;
    ...
    PVPOXUSBFLT_DEVICE pDevice = vpoxUsbFltDevGetLocked(pPdo);
    if (pDevice)
    ...
    return enmState >= VPOXUSBFLT_DEVSTATE_CAPTURING;
}

NTSTATUS VPoxUsbFltPdoRemove(PDEVICE_OBJECT pPdo)
{
    PVPOXUSBFLT_DEVICE pDevice;
    VPOXUSBFLT_DEVSTATE enmOldState;
...
HVPOXUSBFLTDEV VPoxUsbFltProxyStarted(PDEVICE_OBJECT pPdo)
{
    ...
    /* NB: The USB proxy (VPoxUSB.sys) passes us the real PDO, not anything above that. */
    pDevice = vpoxUsbFltDevGetLocked(pPdo);
    /*
     * Prevent a host crash when vpoxUsbFltDevGetLocked fails to locate the matching PDO
     * in g_VPoxUsbFltGlobals.DeviceList (see @bugref{6509}).
     */
    if (pDevice == NULL)
    {
        WARN(("failed to get device for PDO(0x%p)", pPdo));
    }
    else if (pDevice->enmState == VPOXUSBFLT_DEVSTATE_CAPTURING)
    {
        pDevice->enmState = VPOXUSBFLT_DEVSTATE_CAPTURED;
...
{
    PVPOXUSBFLT_DEVICE pDevice = (PVPOXUSBFLT_DEVICE)hDev;
    /*
     * Prevent a host crash when VPoxUsbFltProxyStarted fails, returning NULL.
     * See @bugref{6509}.
     */
    if (pDevice == NULL)
    {
        WARN(("VPoxUsbFltProxyStopped called with NULL device pointer"));
        return;
    }
    VPOXUSBFLT_LOCK_ACQUIRE();
    if (pDevice->enmState == VPOXUSBFLT_DEVSTATE_CAPTURED
            || pDevice->enmState == VPOXUSBFLT_DEVSTATE_USED_BY_GUEST)
    {
...
static NTSTATUS vpoxUsbFltRegKeyQuery(PWSTR ValueName, ULONG ValueType, PVOID ValueData, ULONG ValueLength, PVOID Context, PVOID EntryContext)
{
...
{
    int rc = VPoxUSBFilterInit();
    if (RT_FAILURE(rc))
    {
        WARN(("VPoxUSBFilterInit failed, rc (%d)", rc));
        return STATUS_UNSUCCESSFUL;
    }

    memset(&g_VPoxUsbFltGlobals, 0, sizeof (g_VPoxUsbFltGlobals));
    InitializeListHead(&g_VPoxUsbFltGlobals.DeviceList);
    InitializeListHead(&g_VPoxUsbFltGlobals.ContextList);
    InitializeListHead(&g_VPoxUsbFltGlobals.BlackDeviceList);
    vpoxUsbFltBlDevPopulateWithKnownLocked();
    VPOXUSBFLT_LOCK_INIT();
    ...
    {
        {vpoxUsbFltRegKeyQuery, 0, L"ForceReplugWhenDevPopulateFails", &g_VPoxUsbFltGlobals.dwForceReplugWhenDevPopulateFails, REG_DWORD, &g_VPoxUsbFltGlobals.dwForceReplugWhenDevPopulateFails, sizeof(ULONG) },
        {                 NULL, 0,                               NULL,                                                   NULL,         0,                                                     0,             0 }
    };
    UNICODE_STRING UnicodePath = RTL_CONSTANT_STRING(L"\\VPoxUSB");

    NTSTATUS Status = RtlQueryRegistryValues(RTL_REGISTRY_CONTROL, UnicodePath.Buffer, &aParams[0], NULL, NULL);
    if (Status == STATUS_SUCCESS)
    {
        if (g_VPoxUsbFltGlobals.dwForceReplugWhenDevPopulateFails)
...
NTSTATUS VPoxUsbFltTerm()
{
    ...
    do
    {
        if (!IsListEmpty(&g_VPoxUsbFltGlobals.ContextList))
        {
            AssertFailed();
            bBusy = true;
            break;
    ...
    for (PLIST_ENTRY pEntry = g_VPoxUsbFltGlobals.DeviceList.Flink;
            pEntry != &g_VPoxUsbFltGlobals.DeviceList;
            pEntry = g_VPoxUsbFltGlobals.DeviceList.Flink)
    {
    ...
    vpoxUsbFltBlDevClearLocked();

    VPOXUSBFLT_LOCK_TERM();
