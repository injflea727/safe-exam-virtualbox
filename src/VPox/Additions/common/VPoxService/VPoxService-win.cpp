/* $Id: VPoxService-win.cpp $ */
/** @file
 * VPoxService - Guest Additions Service Skeleton, Windows Specific Parts.
 */
...
static RTEXITCODE vgsvcWinSetDesc(SC_HANDLE hService)
{
    ...
    if (g_pfnChangeServiceConfig2A)
    {
        ...
        desc.lpDescription = VPOXSERVICE_DESCRIPTION;
...
RTEXITCODE VGSvcWinInstall(void)
{
    ...
    SC_HANDLE  hService = CreateService(hSCManager,
                                        VPOXSERVICE_NAME, VPOXSERVICE_FRIENDLY_NAME,
                                        SERVICE_ALL_ACCESS,
    ...
    else
    {
        ...
        switch (dwErr)
        {
            case ERROR_SERVICE_EXISTS:
                ...
                hService = OpenService(hSCManager, VPOXSERVICE_NAME, SERVICE_ALL_ACCESS);
                if (hService)
                {
...
RTEXITCODE VGSvcWinUninstall(void)
{
    ...
    RTEXITCODE rcExit;
    SC_HANDLE  hService = OpenService(hSCManager, VPOXSERVICE_NAME, SERVICE_ALL_ACCESS );
    if (hService != NULL)
    {
        if (DeleteService(hService))
        {
            ...
            if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,
                ...
                == ERROR_SUCCESS)
            {
                RegDeleteKey(hKey, VPOXSERVICE_NAME);
                RegCloseKey(hKey);
            }
...
static int vgsvcWinStart(void)
{
    ...
    {
        rc = vgsvcWinAddAceToObjectsSecurityDescriptor(TEXT("\\\\.\\VPoxMiniRdrDN"), SE_FILE_OBJECT,
                                                       (LPTSTR)pBuiltinUsersSID, TRUSTEE_IS_SID,
                                                       FILE_GENERIC_READ | FILE_GENERIC_WRITE, SET_ACCESS, NO_INHERITANCE);
        /* If we don't find our "VPoxMiniRdrDN" (for Shared Folders) object above,
...
static DWORD vgsvcWinCtrlHandlerCommon(DWORD dwControl)
{
    ...
    switch (dwControl)
    {
        ...
        case SERVICE_CONTROL_SHUTDOWN:
        {
            ...
            {
                rc2 = VGSvcReportStatus(VPoxGuestFacilityStatus_Terminated);
                AssertRC(rc2);
            }
...
static void WINAPI vgsvcWinMain(DWORD argc, LPTSTR *argv)
{
    ...
    if (g_pfnRegisterServiceCtrlHandlerExA)
        g_hWinServiceStatus = g_pfnRegisterServiceCtrlHandlerExA(VPOXSERVICE_NAME, vgsvcWinCtrlHandlerNt5Plus, NULL);
    else
        g_hWinServiceStatus = RegisterServiceCtrlHandlerA(VPOXSERVICE_NAME, vgsvcWinCtrlHandlerNt4);
