...
static void showLogo(PRTSTREAM pStrm)
{
    ...
    if (!s_fShown)
    {
        RTStrmPrintf(pStrm, VPOX_PRODUCT " Disk Utility " VPOX_VERSION_STRING "\n"
...
static int handleConvert(HandlerArg *a)
{
    ...
    if (fStdOut && !pszDstFilename)
    {
        ...
        if (!RTStrICmp(pszDstFormat, "VMDK"))
            pszDstFilename = "VirtualPoxStream.vmdk";
        else
...
static DECLCALLBACK(int) vpoximgQueryBlockStatus(void *pvUser, uint64_t off,
...
static DECLCALLBACK(int) vpoximgQueryRangeUse(void *pvUser, uint64_t off, uint64_t cb,
                                              bool *pfUsed)
{
...
typedef struct VPOXIMGVFS
{
    /** Pointer to the next VFS handle. */
    struct VPOXIMGVFS *pNext;
    /** VFS handle. */
    RTVFS              hVfs;
} VPOXIMGVFS, *PVPOXIMGVFS;
...
static int handleCompact(HandlerArg *a)
{
    ...
    PVPOXIMGVFS pVPoxImgVfsHead = NULL;
            ...
            if (RT_SUCCESS(rc))
            {
                    ...
                    && RTDvmMapGetValidVolumes(hDvm) > 0)
                {
                    ...
                    while (RT_SUCCESS(rc))
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            PVPOXIMGVFS pVPoxImgVfs = (PVPOXIMGVFS)RTMemAllocZ(sizeof(VPOXIMGVFS));
                            if (!pVPoxImgVfs)
                            {
                                RTVfsRelease(hVfs);
                                rc = VERR_NO_MEMORY;
                            ...
                            pVPoxImgVfs->hVfs = hVfs;
                            pVPoxImgVfs->pNext = pVPoxImgVfsHead;
                            pVPoxImgVfsHead = pVPoxImgVfs;
                            RTDvmVolumeSetQueryBlockStatusCallback(hVol, vpoximgQueryBlockStatus, hVfs);
                        }
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        VDIfQueryRangeUse.pfnQueryRangeUse = vpoximgQueryRangeUse;
    ...
    while (pVPoxImgVfsHead)
    {
        PVPOXIMGVFS pVPoxImgVfsFree = pVPoxImgVfsHead;

        pVPoxImgVfsHead = pVPoxImgVfsHead->pNext;
        RTVfsRelease(pVPoxImgVfsFree->hVfs);
        RTMemFree(pVPoxImgVfsFree);
    }
