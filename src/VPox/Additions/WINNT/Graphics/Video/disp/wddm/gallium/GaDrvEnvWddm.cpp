/* $Id: GaDrvEnvWddm.cpp $ */
/** @file
 * VirtualPox Windows Guest Mesa3D - Gallium driver interface to the WDDM miniport driver.
 */
...
static HRESULT
vpoxDdiContextGetId(GaWddmCallbacks *pWddmCallbacks,
                    HANDLE hContext,
                    uint32_t *pu32Cid)
{
    HRESULT                   hr;
...
static void
vpoxDdiContextDestroy(GaWddmCallbacks *pWddmCallbacks,
...
static HRESULT
vpoxDdiContextCreate(GaWddmCallbacks *pWddmCallbacks,
                     ...
                     GAWDDMCONTEXTINFO *pContextInfo)
{
    ...
    if (hr == S_OK)
    {
        /* Query cid. */
        uint32_t u32Cid = 0;
        hr = vpoxDdiContextGetId(pWddmCallbacks, ddiCreateContext.hContext, &u32Cid);
        if (SUCCEEDED(hr))
        {
        ...
        {
            vpoxDdiContextDestroy(pWddmCallbacks, pContextInfo);
        }
    }
                                      ...
                                      uint32_t u32Cid)
{
    ...
    if (pContextInfo)
    {
        vpoxDdiContextDestroy(&pThis->mWddmCallbacks, pContextInfo);
                                     ...
                                     boolean vgpu10)
{
    ...
    hr = vpoxDdiContextCreate(&pThis->mWddmCallbacks,
    ...
    if (SUCCEEDED(hr))
    {
        ...
        vpoxDdiContextDestroy(&pThis->mWddmCallbacks,
                              pContextInfo);
    }
                                     ...
                                     uint32_t *pu32Sid)
{
    ...
    pData = (VPOXDISPIFESCAPE_GASURFACEDEFINE *)RTMemAllocZ(cbAlloc);
    if (!pData)
        return -1;
...
static HRESULT
vpoxDdiFenceCreate(GaWddmCallbacks *pWddmCallbacks,
                   GAWDDMCONTEXTINFO *pContextInfo,
...
static HRESULT
vpoxDdiFenceQuery(GaWddmCallbacks *pWddmCallbacks,
                                  ...
                                  GAFENCEQUERY *pFenceQuery)
{
    ...
    HRESULT hr = vpoxDdiFenceQuery(&pThis->mWddmCallbacks, u32FenceHandle, pFenceQuery);
    if (FAILED(hr))
        return -1;
...
static HRESULT
vpoxDdiFenceWait(GaWddmCallbacks *pWddmCallbacks,
                 uint32_t u32FenceHandle,
                                 ...
                                 uint32_t u32TimeoutUS)
{
    ...
    HRESULT hr = vpoxDdiFenceWait(&pThis->mWddmCallbacks, u32FenceHandle, u32TimeoutUS);
    return SUCCEEDED(hr) ? 0 : -1;
}

static HRESULT
vpoxDdiFenceUnref(GaWddmCallbacks *pWddmCallbacks,
                  uint32_t u32FenceHandle)
{
    HRESULT                       hr;
    D3DDDICB_ESCAPE               ddiEscape;
                                  ...
                                  uint32_t u32FenceHandle)
{
    ...
    vpoxDdiFenceUnref(&pThis->mWddmCallbacks, u32FenceHandle);
}
...
static HRESULT
vpoxDdiRender(GaWddmCallbacks *pWddmCallbacks,
              GAWDDMCONTEXTINFO *pContextInfo, uint32_t u32FenceHandle, void *pvCommands, uint32_t cbCommands)
{
    ...
    LogRel(("vpoxDdiRender: cbCommands = %d, u32FenceHandle = %d\n", cbCommands, u32FenceHandle));
    ...
    do
    {
        ...
        else
        {
            ...
            *(uint32_t *)pContextInfo->pCommandBuffer = 0;
            ...
            hr = vpoxCalcCommandLength(pu8Src, cbCommandChunk, cbAvail - sizeof(u32FenceHandle), &cbCommandChunk);
                              ...
                              GAFENCEQUERY *pFenceQuery)
{
    ...
    if (pFenceQuery)
    {
        hr = vpoxDdiFenceCreate(&pThis->mWddmCallbacks, pContextInfo, &u32FenceHandle);
    }

    if (SUCCEEDED(hr))
    {
        hr = vpoxDdiRender(&pThis->mWddmCallbacks, pContextInfo, u32FenceHandle, pvCommands, cbCommands);
        if (SUCCEEDED(hr))
        {
            if (pFenceQuery)
            {
                HRESULT hr2 = vpoxDdiFenceQuery(&pThis->mWddmCallbacks, u32FenceHandle, pFenceQuery);
                if (hr2 != S_OK)
                {
                    pFenceQuery->u32FenceStatus = GA_FENCE_STATUS_NULL;
                }
...
static HRESULT
vpoxDdiRegionCreate(GaWddmCallbacks *pWddmCallbacks,
                                    ...
                                    void **ppvMap)
{
    ...
    if (pThis->mWddmCallbacks.hDevice)
    {
        ...
        HRESULT hr = vpoxDdiRegionCreate(&pThis->mWddmCallbacks, u32RegionSize, pu32GmrId, ppvMap);
...
static HRESULT
vpoxDdiRegionDestroy(GaWddmCallbacks *pWddmCallbacks,
                     uint32_t u32GmrId)
{
    HRESULT                   hr;
    D3DDDICB_ESCAPE           ddiEscape;
                                     ...
                                     void *pvMap)
{
    ...
    if (pThis->mWddmCallbacks.hDevice)
    {
        vpoxDdiRegionDestroy(&pThis->mWddmCallbacks, u32GmrId);
    }
    else
    {
        RTMemFree(pvMap);
