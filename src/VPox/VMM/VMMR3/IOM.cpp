 ...
 */
DECL_NO_INLINE(static, VPOXSTRICTRC) iomR3MergeStatusSlow(VPOXSTRICTRC rcStrict, VPOXSTRICTRC rcStrictCommit,
                                                          int rcIom, PVMCPU pVCpu)
{
    ...
    AssertLogRelMsgFailed(("rcStrictCommit=%Rrc rcStrict=%Rrc IOPort={%#06x<-%#xx/%u} MMIO={%RGp<-%.*Rhxs} (rcIom=%Rrc)\n",
                           VPOXSTRICTRC_VAL(rcStrictCommit), VPOXSTRICTRC_VAL(rcStrict),
 ...
 *                          calling EMT.
 */
DECLINLINE(VPOXSTRICTRC) iomR3MergeStatus(VPOXSTRICTRC rcStrict, VPOXSTRICTRC rcStrictCommit, int rcIom, PVMCPU pVCpu)
{
    /* Simple. */
...
VMMR3_INT_DECL(VPOXSTRICTRC) IOMR3ProcessForceFlag(PVM pVM, PVMCPU pVCpu, VPOXSTRICTRC rcStrict)
{
    ...
    if (pVCpu->iom.s.PendingIOPortWrite.cbValue)
    {
        ...
        VPOXSTRICTRC rcStrictCommit = IOMIOPortWrite(pVM, pVCpu, pVCpu->iom.s.PendingIOPortWrite.IOPort,
                                                     pVCpu->iom.s.PendingIOPortWrite.u32Value,
    ...
    if (pVCpu->iom.s.PendingMmioWrite.cbValue)
    {
        ...
        if (idxMmioRegionHint < pVM->iom.s.cMmioRegs)
        {
            ...
            if (offRegion < pRegEntry->cbRegion && GCPhysMapping != NIL_RTGCPHYS)
            {
                ...
                VPOXSTRICTRC rcStrictCommit = iomR3MmioCommitWorker(pVM, pVCpu, pRegEntry, offRegion);
        ...
        VPOXSTRICTRC rcStrictCommit = PGMPhysWrite(pVM, pVCpu->iom.s.PendingMmioWrite.GCPhys,
