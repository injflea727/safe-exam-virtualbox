...
*********************************************************************************************************************************/
#define VPOX_PULSE_LIB "libpulse.so.0"

#define PROXY_STUB(function, rettype, signature, shortsig) \
    static rettype (*g_pfn_ ## function) signature; \
    \
    extern "C" rettype VPox_##function signature; \
    rettype VPox_##function signature \
    { \
...
#define PROXY_STUB_VOID(function, signature, shortsig) \
    ...
    \
    extern "C" void VPox_##function signature; \
    void VPox_##function signature \
    { \
...
static DECLCALLBACK(int32_t) drvHostAudioPulseLibInitOnce(void *pvUser)
{
    ...
    int rc = RTLdrLoadSystemEx(VPOX_PULSE_LIB, RTLDRLOAD_FLAGS_NO_UNLOAD, &hMod);
    if (RT_SUCCESS(rc))
    {
