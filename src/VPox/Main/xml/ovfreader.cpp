                                  ...
                                  const xml::ElementNode *pSectionElem)
{
    ...
    while ((pelmDisk = loopDisks.forAllNodes()))
    {
        ...
        else
        {
            ...
            // optional vpox:uuid attribute (if OVF was exported by VirtualPox != 3.2)
            pelmDisk->getAttributeValueN("uuid", d.uuidVPox, RT_XML_ATTR_TINY, "vpox");
...
void OVFReader::HandleVirtualSystemContent(const xml::ElementNode *pelmVirtualSystem)
{
    ...
    // peek under the <VirtualSystem> node whether we have a <vpox:Machine> node;
    // that case case, the caller can completely ignore the OVF but only load the VPox machine XML
    vsys.pelmVPoxMachine = pelmVirtualSystem->findChildElementNS("vpox", "Machine");
    ...
    while ((pelmThis = loop.forAllNodes()))
    {
                ...
                )
        {
            ...
            const xml::ElementNode *pelmVPoxOSType;
            if ((pelmVPoxOSType = pelmThis->findChildElementNS("vpox",            // namespace
                                                               "OSType")))        // element name
                vsys.strTypeVPox = pelmVPoxOSType->getValueN(RT_XML_CONTENT_SMALL);
        }
