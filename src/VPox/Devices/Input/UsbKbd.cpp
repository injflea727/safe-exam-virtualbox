 ...
 * @{ */
/** The highest USB usage code reported by the VPox emulated keyboard */
#define VPOX_USB_MAX_USAGE_CODE     0xE7
/** The size of an array needed to store all USB usage codes */
#define VPOX_USB_USAGE_ARRAY_SIZE   (VPOX_USB_MAX_USAGE_CODE + 1)
...
typedef struct USBHID
{
    ...
    uint8_t             abDepressedKeys[VPOX_USB_USAGE_ARRAY_SIZE];
...
{
    { USBHID_STR_ID_MANUFACTURER,   "VirtualPox"    },
...
static void usbHidBuildReport(PUSBHIDK_REPORT pReport, uint8_t *pabDepressedKeys)
{
    unsigned iBuf = 0;
    RT_ZERO(*pReport);
    for (unsigned iKey = 0; iKey < VPOX_USB_USAGE_ARRAY_SIZE; ++iKey)
    {
...
static DECLCALLBACK(int) usbHidKeyboardPutEvent(PPDMIKEYBOARDPORT pInterface, uint32_t idUsage)
{
    ...
    AssertReturn(u8HidCode <= VPOX_USB_MAX_USAGE_CODE, VERR_INTERNAL_ERROR);
