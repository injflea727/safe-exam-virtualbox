/* $Id: vpoxmrxnp.cpp $ */
/** @file
 * VirtualPox Windows Guest Shared Folders - Network provider dll
 */
...
#define MRX_VPOX_SERVER_NAME_U     L"VPOXSVR"
#define MRX_VPOX_SERVER_NAME_ALT_U L"VPOXSRV"

#define WNNC_DRIVER(major, minor) (major * 0x00010000 + minor)


static WCHAR vpoxToUpper(WCHAR wc)
{
                       ...
                       PULONG pOutputDataLen)
{
    ...
    HANDLE DeviceHandle = CreateFile(DD_MRX_VPOX_USERMODE_DEV_NAME_U,
    ...
    if (INVALID_HANDLE_VALUE != DeviceHandle)
    {
        ...
        if (!fSuccess)
        {
            ...
            Log(("VPOXNP: vbsfIOCTL: DeviceIoctl last error = %d\n", dwStatus));
        }
    ...
    else
    {
        ...
        if (!sLogged)
        {
            LogRel(("VPOXNP: vbsfIOCTL: Error opening device, last error = %d\n",
                    dwStatus));
            sLogged++;
        }
    }
...
DWORD APIENTRY NPGetCaps(DWORD nIndex)
{
    ...
    Log(("VPOXNP: GetNetCaps: Index = 0x%x\n", nIndex));
    ...
    switch (nIndex)
    {
        ...
        {
            vbsfIOCTL(IOCTL_MRX_VPOX_START, NULL, 0, NULL, NULL);
                                      ...
                                      DWORD dwChangeInfo)
{
    ...
    Log(("VPOXNP: NPPasswordChangeNotify\n"));
                                ...
                                DWORD dwFlags)
{
    ...
    Log(("VPOXNP: NPAddConnection3: dwFlags = 0x%x\n", dwFlags));
    Log(("VPOXNP: NPAddConnection3: Local Name:  %ls\n", pNetResource->lpLocalName ));
    Log(("VPOXNP: NPAddConnection3: Remote Name: %ls\n", pNetResource->lpRemoteName ));

    if (   pNetResource->dwType != RESOURCETYPE_DISK
        && pNetResource->dwType != RESOURCETYPE_ANY)
    {
        Log(("VPOXNP: NPAddConnection3: Incorrect net resource type %d\n", pNetResource->dwType));
        return WN_BAD_NETNAME;
    }

    /* Build connection name: \Device\VPoxMiniRdr\;%DriveLetter%:\vpoxsvr\share */

    lstrcpy(ConnectionName, DD_MRX_VPOX_FS_DEVICE_NAME_U);
    ...
    else
    {
        ...
        {
            LocalName[0] = vpoxToUpper(pNetResource->lpLocalName[0]);
            LocalName[1] = L':';
            LocalName[2] = L'\0';
    ...
    Log(("VPOXNP: NPAddConnection3: ConnectionName: [%ls], len %d, dwStatus 0x%08X\n",
    ...
    if (dwStatus == WN_SUCCESS)
    {
        ...
        {
            Log(("VPOXNP: NPAddConnection3: Connection [%ls] already connected.\n",
                 ConnectionName));
        ...
        else
        {
            ...
            {
                dwStatus = vbsfIOCTL(IOCTL_MRX_VPOX_ADDCONN,
                                     ConnectionName,
    ...
    Log(("VPOXNP: NPAddConnection3: Returned 0x%08X\n",
         dwStatus));
    return dwStatus;
}
                                  ...
                                  BOOL fForce)
{
    ...
    Log(("VPOXNP: NPCancelConnection: Name = %ls\n",
         pName));
    ...
    if (pName && pName[0] != 0)
    {
        ...
        if (pName[1] == L':')
        {
            ...
            LocalName[0] = vpoxToUpper(pName[0]);
            LocalName[1] = L':';
            LocalName[2] = L'\0';

            ULONG cbOut = sizeof(RemoteName) - sizeof(WCHAR); /* Trailing NULL. */

            dwStatus = vbsfIOCTL(IOCTL_MRX_VPOX_GETCONN,
                                 LocalName,
                                 sizeof(LocalName),
                                 (PVOID)RemoteName,
                                 &cbOut);
                ...
                && cbOut > 0)
            {
                ...
                if (lstrlen(DD_MRX_VPOX_FS_DEVICE_NAME_U) + 2 + lstrlen(LocalName) + lstrlen(RemoteName) + 1 > sizeof(ConnectionName) / sizeof(WCHAR))
                {
                    dwStatus = WN_BAD_NETNAME;
                }
                else
                {
                    ...
                    dwStatus = vbsfIOCTL(IOCTL_MRX_VPOX_DELCONN,
                                         ConnectionName,
                                         (lstrlen(ConnectionName) + 1) * sizeof(WCHAR),
                                         NULL,
                                         NULL);
        ...
        else
        {
            ...
            {
                /* Full remote path */
                if (lstrlen(DD_MRX_VPOX_FS_DEVICE_NAME_U) + 2 + lstrlen(pName) + 1 > sizeof(ConnectionName) / sizeof(WCHAR))
                {
                    dwStatus = WN_BAD_NETNAME;
                }
                else
    ...
    Log(("VPOXNP: NPCancelConnection: Returned 0x%08X\n",
         dwStatus));
    return dwStatus;
}
                               ...
                               LPDWORD pBufferSize)
{
    ...
    Log(("VPOXNP: NPGetConnection: pLocalName = %ls\n",
         pLocalName));
    ...
    if (pLocalName && pLocalName[0] != 0)
    {
        if (pLocalName[1] == L':')
        {
            ...
            LocalName[0] = vpoxToUpper(pLocalName[0]);
            LocalName[1] = L':';
            LocalName[2] = L'\0';
    ...
    if (dwStatus == WN_SUCCESS)
    {
        ...
        Log(("VPOXNP: NPGetConnection: RemoteName: %ls, cb %d\n",
        ...
        if (*pBufferSize >= len)
        {
            ...
            Log(("VPOXNP: NPGetConnection: returning pRemoteName: %ls\n",
                 pRemoteName));
        }
        else
        {
            if (*pBufferSize != 0)
            {
                ...
                Log(("VPOXNP: NPGetConnection: Buffer overflow: *pBufferSize = %d, len = %d\n",
                     *pBufferSize, len));
            }
    ...
    if ((dwStatus != WN_SUCCESS) &&
        (dwStatus != WN_MORE_DATA))
    {
        Log(("VPOXNP: NPGetConnection: Returned error 0x%08X\n",
             dwStatus));
    }
...
static const WCHAR *vpoxSkipServerPrefix(const WCHAR *pRemoteName, const WCHAR *pPrefix)
{
    while (*pPrefix)
    {
        if (vpoxToUpper(*pPrefix) != vpoxToUpper(*pRemoteName))
        {
            /* Not a prefix */
            return NULL;
        }
...
static const WCHAR *vpoxSkipServerName(const WCHAR *pRemoteName)
{
    ...
    {
        const WCHAR *pAfterPrefix = vpoxSkipServerPrefix(pRemoteName, MRX_VPOX_SERVER_NAME_U);

        if (!pAfterPrefix)
        {
            pAfterPrefix = vpoxSkipServerPrefix(pRemoteName, MRX_VPOX_SERVER_NAME_ALT_U);
        }
                          ...
                          LPHANDLE lphEnum)
{
    ...
    Log(("VPOXNP: NPOpenEnum: dwScope 0x%08X, dwType 0x%08X, dwUsage 0x%08X, pNetResource %p\n",
    ...
    *lphEnum = NULL;
    ...
    else
    {
        ...
        {
            Log(("VPOXNP: NPOpenEnum: pRemoteName %ls\n",
                 pNetResource->lpRemoteName));
        }
        ...
        switch (dwScope)
        {
                     ...
                     */
            {
                ...
                const WCHAR *pAfterName = vpoxSkipServerName(pNetResource->lpRemoteName);
                if (   pAfterName == NULL
            ...
            case RESOURCE_GLOBALNET: /* All resources on the network. */
            {
                ...
                {
                    /* Enumerate pNetResource->lpRemoteName container, which can be only the VPOXSVR container. */
                    const WCHAR *pAfterName = vpoxSkipServerName(pNetResource->lpRemoteName);
                    if (   pAfterName == NULL
                        || (*pAfterName != L'\\' && *pAfterName != 0))
                    {
            ...
            default:
                Log(("VPOXNP: NPOpenEnum: unsupported scope 0x%lx\n",
                     dwScope));
    ...
    if (dwStatus != WN_SUCCESS)
    {
        Log(("VPOXNP: NPOpenEnum: Returned error 0x%08X\n",
             dwStatus));
        if (pCtx)
        {
                              ...
                              LPDWORD pBufferSize)
{
    ...
    Log(("VPOXNP: NPEnumResource: hEnum %p, lpcCount %p, pBuffer %p, pBufferSize %p.\n",
         hEnum, lpcCount, pBuffer, pBufferSize));

    if (pCtx == NULL)
    {
        Log(("VPOXNP: NPEnumResource: WN_BAD_HANDLE\n"));
        return WN_BAD_HANDLE;
    }

    if (lpcCount == NULL || pBuffer == NULL)
    {
        Log(("VPOXNP: NPEnumResource: WN_BAD_VALUE\n"));
        return WN_BAD_VALUE;
    }

    Log(("VPOXNP: NPEnumResource: *lpcCount 0x%x, *pBufferSize 0x%x, pCtx->index %d\n",
    ...
    {
        Log(("VPOXNP: NPEnumResource: RESOURCE_CONNECTED\n"));

        memset(ConnectionList, 0, sizeof(ConnectionList));
        cbOut = sizeof(ConnectionList);

        dwStatus = vbsfIOCTL(IOCTL_MRX_VPOX_GETLIST,
                             NULL, 0,
                             ConnectionList,
                             &cbOut);
        ...
        if (dwStatus == WN_SUCCESS && cbOut > 0)
        {
            while (cEntriesCopied < *lpcCount && pCtx->index < RTL_NUMBER_OF(ConnectionList))
            {
                if (ConnectionList[pCtx->index])
                {
                    ...
                    cbEntry += sizeof(MRX_VPOX_PROVIDER_NAME_U);
                    ...
                    *pDst++ = L'\\';
                    ...
                    CopyMemory(pDst, MRX_VPOX_PROVIDER_NAME_U, sizeof(MRX_VPOX_PROVIDER_NAME_U));

                    Log(("VPOXNP: NPEnumResource: pRemoteName: %ls\n",
                         pNetResource->lpRemoteName));
    ...
    {
        Log(("VPOXNP: NPEnumResource: RESOURCE_GLOBALNET: root %d\n", pCtx->fRoot));
        ...
        if (pCtx->fRoot)
        {
            ...
            {
                /* Return VPOXSVR server.
                 * Determine the space needed for this entry.
                 */
                cbEntry = sizeof(NETRESOURCE);
                cbEntry += 2 * sizeof(WCHAR) + sizeof(MRX_VPOX_SERVER_NAME_U); /* \\ + the server name */
                cbEntry += sizeof(MRX_VPOX_PROVIDER_NAME_U);
                ...
                else
                {
                    ...
                    *pDst++ = L'\\';
                    *pDst++ = L'\\';
                    CopyMemory(pDst, MRX_VPOX_SERVER_NAME_U, sizeof(MRX_VPOX_SERVER_NAME_U));
                    pDst += sizeof(MRX_VPOX_SERVER_NAME_U) / sizeof(WCHAR);
        ...
        else
        {
            ...
            dwStatus = vbsfIOCTL(IOCTL_MRX_VPOX_GETGLOBALLIST,
                                 NULL,
                                 0,
                                 ConnectionList,
                                 &cbOut);
            ...
            if (dwStatus == WN_SUCCESS && cbOut > 0)
            {
                while (cEntriesCopied < *lpcCount && pCtx->index < RTL_NUMBER_OF(ConnectionList))
                {
                    if (ConnectionList[pCtx->index])
                    {
                        ...
                        dwStatus = vbsfIOCTL(IOCTL_MRX_VPOX_GETGLOBALCONN,
                        ...
                        cbRemoteName = (lstrlen(RemoteName) + 1) * sizeof(WCHAR);
                        cbEntry = sizeof(NETRESOURCE);
                        /* Remote name: \\ + vpoxsvr + \ + name. */
                        cbEntry += 2 * sizeof(WCHAR) + sizeof(MRX_VPOX_SERVER_NAME_U) + cbRemoteName;
                        cbEntry += sizeof(MRX_VPOX_PROVIDER_NAME_U);
                        ...
                        *pDst++ = L'\\';
                        *pDst++ = L'\\';
                        CopyMemory(pDst, MRX_VPOX_SERVER_NAME_U, sizeof(MRX_VPOX_SERVER_NAME_U) - sizeof(WCHAR));
                        pDst += sizeof(MRX_VPOX_SERVER_NAME_U) / sizeof(WCHAR) - 1;
                        *pDst++ = L'\\';
                        CopyMemory(pDst, RemoteName, cbRemoteName);
                        pDst += cbRemoteName / sizeof(WCHAR);
    ...
    {
        Log(("VPOXNP: NPEnumResource: dwScope 6\n"));
        dwStatus = WN_NO_MORE_ENTRIES;
    }
    else
    {
        Log(("VPOXNP: NPEnumResource: invalid dwScope 0x%x\n",
             pCtx->dwScope));
        return WN_BAD_HANDLE;
    }
    ...
    *lpcCount = cEntriesCopied;
    ...
    if (cEntriesCopied == 0 && dwStatus == WN_SUCCESS)
    {
        ...
        {
            Log(("VPOXNP: NPEnumResource: More Data Needed - %d\n",
                 cbEntry));
            *pBufferSize = cbEntry;
            dwStatus = WN_MORE_DATA;
        }
    ...
    Log(("VPOXNP: NPEnumResource: Entries returned %d, dwStatus 0x%08X\n",
...
DWORD APIENTRY NPCloseEnum(HANDLE hEnum)
{
    ...
    Log(("VPOXNP: NPCloseEnum: hEnum %p\n",
         hEnum));
    ...
    Log(("VPOXNP: NPCloseEnum: returns\n"));
    return WN_SUCCESS;
}
...
DWORD APIENTRY NPGetResourceParent(LPNETRESOURCE pNetResource,
                                   ...
                                   LPDWORD pBufferSize)
{
    Log(("VPOXNP: NPGetResourceParent: pNetResource %p, pBuffer %p, pBufferSize %p\n",
    ...
    if (   pAfterName == NULL
        || (*pAfterName != L'\\' && *pAfterName != 0))
    {
        Log(("VPOXNP: NPGetResourceParent: WN_BAD_NETNAME\n"));
        return WN_BAD_NETNAME;
    }
        ...
        || pLastSlash == pParent->lpRemoteName + 1)
    {
        ...
        cbEntry = sizeof(NETRESOURCE);
        cbEntry += sizeof(MRX_VPOX_PROVIDER_NAME_U); /* remote name */
        cbEntry += sizeof(MRX_VPOX_PROVIDER_NAME_U); /* provider name */

        if (cbEntry > *pBufferSize)
        {
            Log(("VPOXNP: NPGetResourceParent: WN_MORE_DATA 0x%x\n", cbEntry));
            *pBufferSize = cbEntry;
            dwStatus = WN_MORE_DATA;
        }
        else
        {
            ...
            CopyMemory (pStrings, MRX_VPOX_PROVIDER_NAME_U, sizeof(MRX_VPOX_PROVIDER_NAME_U));
            pStrings += sizeof(MRX_VPOX_PROVIDER_NAME_U) / sizeof(WCHAR);

            pNetResource->lpProvider = pStrings;
            CopyMemory (pStrings, MRX_VPOX_PROVIDER_NAME_U, sizeof(MRX_VPOX_PROVIDER_NAME_U));
            pStrings += sizeof(MRX_VPOX_PROVIDER_NAME_U) / sizeof(WCHAR);

            Log(("VPOXNP: NPGetResourceParent: no parent, strings %p/%p\n",
...
DWORD APIENTRY NPGetResourceInformation(LPNETRESOURCE pNetResource,
                                        ...
                                        LPWSTR *lplpSystem)
{
    Log(("VPOXNP: NPGetResourceInformation: pNetResource %p, pBuffer %p, pBufferSize %p, lplpSystem %p\n",
    ...
    if (   pNetResource == NULL
        ...
        || pBufferSize == NULL)
    {
        Log(("VPOXNP: NPGetResourceInformation: WN_BAD_VALUE\n"));
        return WN_BAD_VALUE;
    }

    Log(("VPOXNP: NPGetResourceInformation: pRemoteName %ls, *pBufferSize 0x%x\n",
    ...
    if (   pAfterName == NULL
        || (*pAfterName != L'\\' && *pAfterName != 0))
    {
        Log(("VPOXNP: NPGetResourceInformation: WN_BAD_NETNAME\n"));
        return WN_BAD_NETNAME;
    }
    ...
    if (pAfterName[0] == 0 || pAfterName[1] == 0)
    {
        /* "\\VPOXSVR" or "\\VPOXSVR\" */
        cbEntry = sizeof(NETRESOURCE);
        cbEntry += 2 * sizeof(WCHAR) + sizeof(MRX_VPOX_SERVER_NAME_U); /* \\ + server name */
        cbEntry += sizeof(MRX_VPOX_PROVIDER_NAME_U); /* provider name */

        if (cbEntry > *pBufferSize)
        {
            Log(("VPOXNP: NPGetResourceInformation: WN_MORE_DATA 0x%x\n", cbEntry));
            *pBufferSize = cbEntry;
            return WN_MORE_DATA;
        }
        ...
        *pStrings++ = L'\\';
        *pStrings++ = L'\\';
        CopyMemory (pStrings, MRX_VPOX_SERVER_NAME_U, sizeof(MRX_VPOX_SERVER_NAME_U));
        pStrings += sizeof(MRX_VPOX_SERVER_NAME_U) / sizeof(WCHAR);

        pNetResourceInfo->lpProvider = pStrings;
        CopyMemory (pStrings, MRX_VPOX_PROVIDER_NAME_U, sizeof(MRX_VPOX_PROVIDER_NAME_U));
        pStrings += sizeof(MRX_VPOX_PROVIDER_NAME_U) / sizeof(WCHAR);

        Log(("VPOXNP: NPGetResourceInformation: pRemoteName: %ls, strings %p/%p\n",
    ...
    if (*lp == 0)
    {
        /* It is a share only: \\vpoxsvr\share */
        cbEntry = sizeof(NETRESOURCE);
        cbEntry += 2 * sizeof(WCHAR) + sizeof(MRX_VPOX_SERVER_NAME_U); /* \\ + server name with trailing nul */
        ...
        *pStrings++ = L'\\';
        *pStrings++ = L'\\';
        CopyMemory(pStrings, MRX_VPOX_SERVER_NAME_U, sizeof(MRX_VPOX_SERVER_NAME_U) - sizeof (WCHAR));
        pStrings += sizeof(MRX_VPOX_SERVER_NAME_U) / sizeof(WCHAR) - 1;
    ...
    *pStrings++ = 0;
    ...
    Log(("VPOXNP: NPGetResourceInformation: pRemoteName: %ls, strings %p/%p\n",
         pNetResourceInfo->lpRemoteName, pStrings, (PBYTE)pBuffer + *pBufferSize));
    Log(("VPOXNP: NPGetResourceInformation: *lplpSystem: %ls\n", *lplpSystem));
                                  ...
                                  LPDWORD pBufferSize)
{
    ...
    Log(("VPOXNP: NPGetUniversalName: pLocalPath = %ls, InfoLevel = %d, *pBufferSize = %d\n",
    ...
    {
        Log(("VPOXNP: NPGetUniversalName: Bad dwInfoLevel value: %d\n",
             dwInfoLevel));
        return WN_BAD_LEVEL;
    }
    ...
    if (   pLocalPath == NULL
        ...
        || pLocalPath[1] != L':')
    {
        Log(("VPOXNP: NPGetUniversalName: Bad pLocalPath.\n"));
    ...
    if (dwInfoLevel == UNIVERSAL_NAME_INFO_LEVEL)
    {
            ...
            && dwStatus != WN_MORE_DATA)
        {
            ...
            {
                Log(("VPOXNP: NPGetUniversalName: NPGetConnection returned error 0x%lx\n",
                     dwStatus));
            }
            return dwStatus;
        }
        ...
        {
            Log(("VPOXNP: NPGetUniversalName: Remote name is empty.\n"));
        ...
        if (*pBufferSize < BufferRequired)
        {
            Log(("VPOXNP: NPGetUniversalName: WN_MORE_DATA BufferRequired: %d\n",
                 BufferRequired));
    ...
    Log(("VPOXNP: NPGetUniversalName: WN_SUCCESS\n"));
    return WN_SUCCESS;
}
                    ...
                    LPVOID pvReserved)
{
    ...
    switch (fdwReason)
    {
        case DLL_PROCESS_ATTACH:
            ...
            VbglR3Init();
            LogRel(("VPOXNP: DLL loaded.\n"));
            break;

        case DLL_PROCESS_DETACH:
            LogRel(("VPOXNP: DLL unloaded.\n"));
            VbglR3Term();
            /// @todo RTR3Term();
            break;
