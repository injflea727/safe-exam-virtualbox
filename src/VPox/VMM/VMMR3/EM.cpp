...
#if defined(LOG_ENABLED) || defined(VPOX_STRICT)
static const char *emR3GetStateName(EMSTATE enmState);
#endif
static VPOXSTRICTRC emR3Debug(PVM pVM, PVMCPU pVCpu, VPOXSTRICTRC rc);
#if defined(VPOX_WITH_REM) || defined(DEBUG)
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) emR3SetExecutionPolicy(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    /*
 ...
 * @param   pVCpu   The cross context virtual CPU structure.
 */
VPOXSTRICTRC emR3ExecutePendingIoPortWrite(PVM pVM, PVMCPU pVCpu)
{
    ...
    /* Do the work.*/
    VPOXSTRICTRC rcStrict = IOMIOPortWrite(pVM, pVCpu, uPort, uValue, cbValue);
    LogFlow(("EM/OUT: %#x, %#x LB %u -> %Rrc\n", uPort, uValue, cbValue, VPOXSTRICTRC_VAL(rcStrict) ));
 ...
 * @param   pVCpu   The cross context virtual CPU structure.
 */
VPOXSTRICTRC emR3ExecutePendingIoPortRead(PVM pVM, PVMCPU pVCpu)
{
    ...
    /* Do the work.*/
    uint32_t uValue = 0;
    VPOXSTRICTRC rcStrict = IOMIOPortRead(pVM, pVCpu, uPort, &uValue, cbValue);
    LogFlow(("EM/IN: %#x LB %u -> %Rrc, %#x\n", uPort, cbValue, VPOXSTRICTRC_VAL(rcStrict), uValue ));
    if (IOM_SUCCESS(rcStrict))
    {
        if (cbValue == 4)
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) emR3ExecuteSplitLockInstructionRendezvous(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 * @param   pVCpu   The cross context virtual CPU structure.
 */
VPOXSTRICTRC emR3ExecuteSplitLockInstruction(PVM pVM, PVMCPU pVCpu)
{
    LogFunc(("\n"));
 ...
 * Debug loop.
 *
 * @returns VPox status code for EM.
 * @param   pVM     The cross context VM structure.
 * @param   pVCpu   The cross context virtual CPU structure.
 * @param   rc      Current EM VPox status code.
 */
static VPOXSTRICTRC emR3Debug(PVM pVM, PVMCPU pVCpu, VPOXSTRICTRC rc)
{
    for (;;)
    {
        ...
        const VPOXSTRICTRC rcLast = rc;

        /*
         * Debug related RC.
         */
        switch (VPOXSTRICTRC_VAL(rc))
        {
            /*
            ...
            case VINF_EM_DBG_STEP:
                if (   pVCpu->em.s.enmState == EMSTATE_DEBUG_GUEST_RAW
                    ...
                    rc = VPOXSTRICTRC_TODO(emR3NemSingleInstruction(pVM, pVCpu, 0 /*fFlags*/));
#ifdef VPOX_WITH_REM /** @todo fix me? */
        ...
        switch (VPOXSTRICTRC_VAL(rc))
        {
            ...
            case VERR_DBGF_NOT_ATTACHED:
                switch (VPOXSTRICTRC_VAL(rcLast))
                {
                    case VINF_EM_DBG_HYPER_STEPPED:
                    case VINF_EM_DBG_HYPER_BREAKPOINT:
                    case VINF_EM_DBG_HYPER_ASSERTION:
 ...
 *
 * @returns VPox status code. The most important ones are: VINF_EM_STEP_EVENT,
...
static int emR3RemStep(PVM pVM, PVMCPU pVCpu)
{
    ...
    int rc = VPOXSTRICTRC_TODO(IEMExecOne(pVCpu)); NOREF(pVM);

    Log3(("emR3RemStep: returns %Rrc cs:eip=%04x:%08x\n", rc, CPUMGetGuestCS(pVCpu),  CPUMGetGuestEIP(pVCpu)));
    return rc;
}
#endif /* VPOX_WITH_REM || DEBUG */
 ...
 *
 * @returns VPox status code. The most important ones are: VINF_EM_RESCHEDULE,
...
#if defined(VPOX_STRICT) && defined(DEBUG_bird)
    ...
    *pfFFDone = false;
    ...
    for (;;)
    {
        ...
        if (RT_LIKELY(emR3IsExecutionAllowed(pVM, pVCpu)))
        {
            ...
            rc = VPOXSTRICTRC_TODO(IEMExecLots(pVCpu, 8192 /*cMaxInstructions*/, 4095 /*cPollRate*/, NULL /*pcInstructions*/));
        ...
        if (    VM_FF_IS_ANY_SET(pVM, VM_FF_HIGH_PRIORITY_POST_MASK)
            ...
            rc = VPOXSTRICTRC_TODO(emR3HighPriorityPostForcedActions(pVM, pVCpu, rc));
            ...
            ||  VMCPU_FF_IS_ANY_SET(pVCpu, VMCPU_FF_ALL_REM_MASK) )
        {
            ...
            VPOXVMM_EM_FF_ALL_RET(pVCpu, rc);
 ...
 *
 * @returns Strict VPox status code from IEMExecLots.
 * @param   pVM        The cross context VM structure.
 ...
 * @thread  EMT(pVCpu)
 */
static VPOXSTRICTRC emR3ExecuteIemThenRem(PVM pVM, PVMCPU pVCpu, bool *pfFFDone)
{
    ...
    *pfFFDone = false;
    ...
    while (pVCpu->em.s.cIemThenRemInstructions < 1024)
    {
        ...
        VPOXSTRICTRC rcStrict = IEMExecLots(pVCpu, 1024 - pVCpu->em.s.cIemThenRemInstructions /*cMaxInstructions*/,
        ...
        if (rcStrict != VINF_SUCCESS)
        {
            ...
            Log(("emR3ExecuteIemThenRem: returns %Rrc after %u instructions\n",
                 VPOXSTRICTRC_VAL(rcStrict), pVCpu->em.s.cIemThenRemInstructions));
            return rcStrict;
        }
...
# if !defined(VPOX_ALLOW_IF0) && !defined(VPOX_RUN_INTERRUPT_GATE_HANDLERS)
        if (!(EFlags.u32 & X86_EFL_IF))
        {
 ...
 *
 * @returns Strict VPox status code.  Typically @a rc, but may be upgraded to
 *          fatal error status code.
 *
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure.
 * @param   rc          The current strict VPox status code rc.
 */
VPOXSTRICTRC emR3HighPriorityPostForcedActions(PVM pVM, PVMCPU pVCpu, VPOXSTRICTRC rc)
{
    VPOXVMM_EM_FF_HIGH(pVCpu, pVM->fGlobalForcedActions, pVCpu->fLocalForcedActions, VPOXSTRICTRC_VAL(rc));
...
static int emR3SvmNstGstIntrIntercept(PVMCPU pVCpu)
{
    ...
    if (CPUMIsGuestSvmCtrlInterceptSet(pVCpu, &pVCpu->cpum.GstCtx, SVM_CTRL_INTERCEPT_INTR))
    {
        ...
        VPOXSTRICTRC rcStrict = IEMExecSvmVmexit(pVCpu, SVM_EXIT_INTR, 0, 0);
...
static int emR3SvmNstGstVirtIntrIntercept(PVMCPU pVCpu)
{
    ...
    {
        CPUM_ASSERT_NOT_EXTRN(pVCpu, IEM_CPUMCTX_EXTRN_SVM_VMEXIT_MASK);
        VPOXSTRICTRC rcStrict = IEMExecSvmVmexit(pVCpu, SVM_EXIT_VINTR, 0, 0);
        if (RT_SUCCESS(rcStrict))
        {
 ...
 * we exit everything.
 *
 * @returns VPox status code of equal or greater importance/severity than rc.
...
#define UPDATE_RC() \
    ...
    VPOXVMM_EM_FF_ALL(pVCpu, pVM->fGlobalForcedActions, pVCpu->fLocalForcedActions, rc);
        ...
        ||  (VMCPU_FF_NORMAL_PRIORITY_POST_MASK && VMCPU_FF_IS_ANY_SET(pVCpu, VMCPU_FF_NORMAL_PRIORITY_POST_MASK)) )
    {
        ...
        {
            CPUM_IMPORT_EXTRN_RCSTRICT(pVCpu, ~CPUMCTX_EXTRN_KEEPER_MASK, rc);
            rc2 = VPOXSTRICTRC_TODO(VMR3ResetFF(pVM));
            UPDATE_RC();
        }
        ...
        ||  VMCPU_FF_IS_ANY_SET(pVCpu, VMCPU_FF_HIGH_PRIORITY_PRE_MASK))
    {
        ...
        if (VMCPU_FF_IS_ANY_SET(pVCpu, VMCPU_FF_VMX_APIC_WRITE | VMCPU_FF_VMX_MTF | VMCPU_FF_VMX_PREEMPT_TIMER))
        {
            ...
            {
                rc2 = VPOXSTRICTRC_VAL(IEMExecVmxVmexitApicWrite(pVCpu));
            ...
            {
                rc2 = VPOXSTRICTRC_VAL(IEMExecVmxVmexit(pVCpu, VMX_EXIT_MTF, 0 /* uExitQual */));
            ...
            {
                rc2 = VPOXSTRICTRC_VAL(IEMExecVmxVmexitPreemptTimer(pVCpu));
                Assert(rc2 != VINF_VMX_INTERCEPT_NOT_ACTIVE);
                UPDATE_RC();
            }
        }
            ...
            && !TRPMHasTrap(pVCpu))                                  /* An event could already be scheduled for dispatching. */
        {
            ...
            if (fGif)
            {
                    ...
                    && !CPUMIsGuestVmxVirtNmiBlocking(&pVCpu->cpum.GstCtx))
                {
                    ...
                    rc2 = VPOXSTRICTRC_VAL(IEMExecVmxVmexit(pVCpu, VMX_EXIT_NMI_WINDOW, 0 /* uExitQual */));
                    ...
                    && !VMCPU_FF_IS_SET(pVCpu, VMCPU_FF_BLOCK_NMIS))
                {
                    ...
                    {
                        rc2 = VPOXSTRICTRC_VAL(IEMExecVmxVmexitXcptNmi(pVCpu));
                        Assert(rc2 != VINF_VMX_INTERCEPT_NOT_ACTIVE);
                        UPDATE_RC();
                    }
                    else
                    ...
                    {
                        rc2 = VPOXSTRICTRC_VAL(IEMExecSvmVmexit(pVCpu, SVM_EXIT_NMI, 0 /* uExitInfo1 */,  0 /* uExitInfo2 */));
                         ...
                         && CPUMIsGuestVmxVirtIntrEnabled(&pVCpu->cpum.GstCtx))
                {
                    ...
                    Assert(CPUMIsGuestVmxInterceptEvents(&pVCpu->cpum.GstCtx));
                    rc2 = VPOXSTRICTRC_VAL(IEMExecVmxVmexit(pVCpu, VMX_EXIT_INT_WINDOW, 0 /* uExitQual */));
                    AssertMsg(   rc2 != VINF_VMX_INTERCEPT_NOT_ACTIVE
                              && rc2 != VINF_PGM_CHANGE_MODE
                              && rc2 != VINF_VMX_VMEXIT
                              && rc2 != VINF_NO_CHANGE, ("%Rrc\n", rc2));
 ...
 *
 * @returns VPox status code, informational status codes may indicate failure.
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure.
 */
...
/** @todo r=bird: consider merging VINF_EM_RESCHEDULE_RAW with VINF_EM_RESCHEDULE_HM, they serve the same purpose here at least. */
                ...
                case VINF_EM_TRIPLE_FAULT:
                    if (!pVM->em.s.fGuruOnTripleFault)
                    {
                        ...
                        rc = VPOXSTRICTRC_TODO(VMR3ResetTripleFault(pVM));
            ...
            {
                VPOXVMM_EM_STATE_CHANGED(pVCpu, enmOldState, enmNewState, rc);
                ...
                VPOXVMM_EM_STATE_UNCHANGED(pVCpu, enmNewState, rc);
            ...
            switch (enmNewState)
            {
                 ...
                 */
                case EMSTATE_NEM:
                    rc = VPOXSTRICTRC_TODO(emR3NemExecute(pVM, pVCpu, &fFFDone));
                    break;
                ...
                case EMSTATE_IEM:
                {
                    ...
                    rc = VPOXSTRICTRC_TODO(EMR3HmSingleInstruction(pVM, pVCpu, EM_ONE_INS_FLAGS_RIP_CHANGE));
...
#endif
                        rc = VPOXSTRICTRC_TODO(IEMExecLots(pVCpu, 4096 /*cMaxInstructions*/, 2047 /*cPollRate*/, &cInstructions));
                    if (pVM->em.s.fIemExecutesAll)
                    {
                ...
                case EMSTATE_IEM_THEN_REM:
                {
                    ...
                    rc = VPOXSTRICTRC_TODO(emR3ExecuteIemThenRem(pVM, pVCpu, &fFFDone));
                ...
                case EMSTATE_DEBUG_GUEST_REM:
                    TMR3NotifySuspend(pVM, pVCpu);
                    rc = VPOXSTRICTRC_TODO(emR3Debug(pVM, pVCpu, rc));
                    TMR3NotifyResume(pVM, pVCpu);
