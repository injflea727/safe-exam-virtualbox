...
echo .
echo Usage: Single-1-Prepare.cmd [-o output-dir] [-e/--extpack puel.vpox-extpack]
...
rem
echo on
copy /y "%_MY_OPT_EXTPACK%" "%_MY_BINDIR%\Oracle_VM_VirtualPox_Extension_Pack.vpox-extpack" || goto end_failed
@echo off
...
call "%_MY_REPACK_DIR%\PackDriversForSubmission.cmd" -b "%_MY_BINDIR%" -a %_MY_OPT_ARCH% -e "%_MY_OPT_EXTPACK%" ^
    -o "%_MY_OPT_OUTDIR%\VPoxDrivers-%_MY_VER_REV%-%_MY_OPT_ARCH%.cab" || goto end_failed
echo .
cd /d "%_MY_SAVED_CD%"
...
echo *
echo * Created:
echo *     %_MY_OPT_OUTDIR%\VPoxDrivers-%_MY_VER_REV%-%_MY_OPT_ARCH%.cab
echo *
echo * Next steps:
