/* $Id: vpoxfs_vnode.c $ */
/** @file
 * VirtualPox File System for Solaris Guests, vnode implementation.
 ...
 * system on VirtualPox, though is meant to be usable with any hypervisor that
 ...
 * "vpoxfs_prov.h" and the module implementing them is shipped as part of the
 * VirtualPox Guest Additions for Solaris.
 *
 ...
 *
 * We suppport only read-only mmap (VPOXVFS_WITH_MMAP) i.e. MAP_SHARED,
...
#include <vm/pvn.h>
#if !defined(VPOX_VFS_SOLARIS_10U6)
...
/*
 * For now we'll use an I/O buffer that doesn't page fault for VirtualPox
	...
	)
{
	...
	{
        if (g_fVPoxVFS_SolOldAddrMap)
            g_VPoxVFS_SolAddrMap.MapAddr.pfnSol_map_addr_old(addrp, len, off, 1, flags);
        else
            g_VPoxVFS_SolAddrMap.MapAddr.pfnSol_map_addr(addrp, len, off, flags);
		if (*addrp == NULL)
			error = ENOMEM;
	}
	else
...
#else
    if (g_fVPoxVFS_SolOldAddrMap)
	    error = g_VPoxVFS_SolAddrMap.ChooseAddr.pfnSol_choose_addr_old(asp, addrp, len, off, 1, flags);
    else
	    error = g_VPoxVFS_SolAddrMap.ChooseAddr.pfnSol_choose_addr(asp, addrp, len, off, flags);
#endif
...
sffs_purge(struct sffs_data *sffs)
{
	...
	for (prev = NULL;;) {
		...
		if (node->sf_sffs == sffs) {
			if (node->sf_vnode != NULL)
				panic("vpoxfs: purge hit active vnode");
			sfnode_make_stale(node);
		} else {
			prev = node;
		}
