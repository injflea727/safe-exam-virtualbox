...
def _ControllerNameToBusAndType(sController):
    ...
    if sController == "IDE Controller":
        eBus  = vpoxcon.StorageBus_IDE;
        eType = vpoxcon.StorageControllerType_PIIX4;
    elif sController == "SATA Controller":
        eBus  = vpoxcon.StorageBus_SATA;
        eType = vpoxcon.StorageControllerType_IntelAhci;
    elif sController == "Floppy Controller":
        eType = vpoxcon.StorageControllerType_I82078;
        eBus  = vpoxcon.StorageBus_Floppy;
    elif sController == "SAS Controller":
        eBus  = vpoxcon.StorageBus_SAS;
        eType = vpoxcon.StorageControllerType_LsiLogicSas;
    elif sController == "SCSI Controller":
        eBus  = vpoxcon.StorageBus_SCSI;
        eType = vpoxcon.StorageControllerType_LsiLogic;
    elif sController == "BusLogic SCSI Controller":
        eBus  = vpoxcon.StorageBus_SCSI;
        eType = vpoxcon.StorageControllerType_BusLogic;
    elif sController == "NVMe Controller":
        eBus  = vpoxcon.StorageBus_PCIe;
        eType = vpoxcon.StorageControllerType_NVMe;
    elif sController == "VirtIO SCSI Controller":
        eBus  = vpoxcon.StorageBus_VirtioSCSI;
        eType = vpoxcon.StorageControllerType_VirtioSCSI;
    else:
        eBus  = vpoxcon.StorageBus_Null;
        eType = vpoxcon.StorageControllerType_Null;
    return (eBus, eType);


def _nameMachineState(eState):
    """ Gets the name (string) of a machine state."""
    if eState == vpoxcon.MachineState_PoweredOff: return 'PoweredOff';
    if eState == vpoxcon.MachineState_Saved: return 'Saved';
    if eState == vpoxcon.MachineState_Teleported: return 'Teleported';
    if eState == vpoxcon.MachineState_Aborted: return 'Aborted';
    if eState == vpoxcon.MachineState_Running: return 'Running';
    if eState == vpoxcon.MachineState_Paused: return 'Paused';
    if eState == vpoxcon.MachineState_Stuck: return 'GuruMeditation';
    if eState == vpoxcon.MachineState_Teleporting: return 'Teleporting';
    if eState == vpoxcon.MachineState_LiveSnapshotting: return 'LiveSnapshotting';
    if eState == vpoxcon.MachineState_Starting: return 'Starting';
    if eState == vpoxcon.MachineState_Stopping: return 'Stopping';
    if eState == vpoxcon.MachineState_Saving: return 'Saving';
    if eState == vpoxcon.MachineState_Restoring: return 'Restoring';
    if eState == vpoxcon.MachineState_TeleportingPausedVM: return 'TeleportingPausedVM';
    if eState == vpoxcon.MachineState_TeleportingIn: return 'TeleportingIn';
    if eState == vpoxcon.MachineState_DeletingSnapshotOnline: return 'DeletingSnapshotOnline';
    if eState == vpoxcon.MachineState_DeletingSnapshotPaused: return 'DeletingSnapshotPaused';
    if eState == vpoxcon.MachineState_RestoringSnapshot: return 'RestoringSnapshot';
    if eState == vpoxcon.MachineState_DeletingSnapshot: return 'DeletingSnapshot';
    if eState == vpoxcon.MachineState_SettingUp: return 'SettingUp';
    if hasattr(vpoxcon, 'MachineState_FaultTolerantSyncing'):
        if eState == vpoxcon.MachineState_FaultTolerantSyncing: return 'FaultTolerantSyncing';
    return 'Unknown-%s' % (eState,);


class VirtualPoxWrapper(object): # pylint: disable=too-few-public-methods
    """
    Wrapper around the IVirtualPox object that adds some (hopefully) useful
    utility methods
    ...
    def __init__(self, oVPox, oVPoxMgr, fpApiVer, oTstDrv):
        self.o          = oVPox;
        self.oVPoxMgr   = oVPoxMgr;
    ...
    def registerDerivedEventHandler(self, oSubClass, dArgs = None):
        """
        Create an instance of the given VirtualPoxEventHandlerBase sub-class
        and register it.

        The new instance is returned on success.  None is returned on error.
        """
        dArgsCopy = dArgs.copy() if dArgs is not None else dict();
        dArgsCopy['oVPox'] = self;
        return oSubClass.registerDerivedEventHandler(self.oVPoxMgr, self.fpApiVer, oSubClass, dArgsCopy,
                                                     self.o, 'IVirtualPox', 'IVirtualPoxCallback');
        ...
        try:
            ...
            try:
                if self.fpApiVer >= 4.1:
                    oIMedium = self.o.openMedium(sHdLocation, vpoxcon.DeviceType_HardDisk,
                                                 vpoxcon.AccessMode_ReadWrite, False);
                elif self.fpApiVer >= 4.0:
                    oIMedium = self.o.openMedium(sHdLocation, vpoxcon.DeviceType_HardDisk,
                                                 vpoxcon.AccessMode_ReadWrite);
                else:
                    oIMedium = self.o.openHardDisk(sHdLocation, vpoxcon.AccessMode_ReadOnly, False, "", False, "");
            except:
    ...
    def deleteHdByMedium(self, oIMedium):
        ...
        try:    oProgress = ProgressWrapper(oProgressCom, self.oVPoxMgr, self.oTstDrv, 'delete disk %s' % (oIMedium.location));
...
class ProgressWrapper(TdTaskBase):
    ...
    def __init__(self, oProgress, oVPoxMgr, oTstDrv, sName):
    ...
    def stringifyErrorInfo(self):
        ...
        try:
            ...
            sErr = vpox.stringifyErrorInfo(oErrInfo);
        return sErr;
    ...
    def stringifyResult(self):
        ...
        if self.isCompleted():
            if self.wasCanceled():
                sRet = 'Progress %s: Canceled, hrc=%s' % (self.sName, vpox.ComError.toString(self.getResult()));
            elif self.getResult() == 0:
                sRet = 'Progress %s: Success' % (self.sName,);
            elif self.getResult() > 0:
                sRet = 'Progress %s: Success (hrc=%s)' % (self.sName, vpox.ComError.toString(self.getResult()));
            else:
    ...
    def waitOnProgress(self, cMsInterval = 1000):
        """
        See vpox.TestDriver.waitOnProgress.
        """
        self.doQuickApiTest();
...
class SessionWrapper(TdTaskBase):
    ...
    def __init__(self, oSession, oVM, oVPox, oVPoxMgr, oTstDrv, fRemoteSession, sFallbackName = None, sLogFile = None):
        """
    ...
    def __pollTask(self):
        ...
        try:
            try:
                ...
                if vpox.ComError.notEqual(oXcpt, vpox.ComError.E_UNEXPECTED):
                    reporter.logXcpt();
                return True;
        finally:
            self.oTstDrv.processPendingEvents();
        ...
        # Switch
        if eState == vpoxcon.MachineState_Running:
            return False;
        if eState == vpoxcon.MachineState_Paused:
            return False;
        if eState == vpoxcon.MachineState_Teleporting:
            return False;
        if eState == vpoxcon.MachineState_LiveSnapshotting:
            return False;
        if eState == vpoxcon.MachineState_Starting:
            return False;
        if eState == vpoxcon.MachineState_Stopping:
            return False;
        if eState == vpoxcon.MachineState_Saving:
            return False;
        if eState == vpoxcon.MachineState_Restoring:
            return False;
        if eState == vpoxcon.MachineState_TeleportingPausedVM:
            return False;
        if eState == vpoxcon.MachineState_TeleportingIn:
            return False;

        # *Beeep* fudge!
        if self.fpApiVer < 3.2 \
          and eState == vpoxcon.MachineState_PoweredOff \
    ...
    def waitForTask(self, cMsTimeout = 0):
        ...
        while not fState:
            ...
            if cMsSleep > 10000:
                cMsSleep = 10000;
            try:    self.oVPoxMgr.waitForEvents(cMsSleep);
            except KeyboardInterrupt: raise;
            except: pass;
            if self.fnProcessEvents:
                self.fnProcessEvents();
    ...
    def registerEventHandlerForTask(self):
        ...
        if self.oConsoleEventHandler is not None:
            return True;
        self.oConsoleEventHandler = self.registerDerivedEventHandler(vpox.SessionConsoleEventHandler, {}, False);
    ...
    def needsPoweringOff(self):
        ...
        if eState == vpoxcon.MachineState_Paused:
            return True;
        if eState == vpoxcon.MachineState_Stuck:
            return True;
        if eState == vpoxcon.MachineState_Teleporting:
            return True;
        if eState == vpoxcon.MachineState_LiveSnapshotting:
        ...
        if eState == vpoxcon.MachineState_TeleportingIn:
            return True;
        if hasattr(vpoxcon, 'MachineState_FaultTolerantSyncing'):
            if eState == vpoxcon.MachineState_FaultTolerantSyncing:
                return True;
        return False;
    ...
    def assertPoweredOff(self):
        ...
        if eState == vpoxcon.MachineState_PoweredOff:
            return True;
    ...
    def getMachineStateWithName(self):
        ...
        try:
            ...
            return (None, '[error getting state: %s]' % (self.oVPoxMgr.xcptToString(),));
        finally:
            self.oTstDrv.processPendingEvents();
    ...
    def reportPrematureTermination(self, sPrefix = ''):
        ...
        if    enmState is not None \
          and enmState == vpoxcon.MachineState_Aborted \
    ...
    def close(self):
        ...
        vpox.TestDriver.aoRemoteSessions list.
        ...
        if self.o is not None:
            ...
            try:
                ...
                # Kludge to ignore VPoxSVC's closing of our session when the
                # direct session closes / VM process terminates.  Fun!
                try:    fIgnore = self.o.state == vpoxcon.SessionState_Unlocked;
    ...
    def enableVirtEx(self, fEnable):
        ...
        fRc = True;
        try:
            self.o.machine.setHWVirtExProperty(vpoxcon.HWVirtExPropertyType_Enabled, fEnable);
        except:
        ...
        # Force/unforce it.
        if fRc and hasattr(vpoxcon, 'HWVirtExPropertyType_Force'):
            try:
                self.o.machine.setHWVirtExProperty(vpoxcon.HWVirtExPropertyType_Force, fEnable);
            except:
            ...
            reporter.log('Warning! vpoxcon has no HWVirtExPropertyType_Force attribute.');
            ## @todo Modify CFGM to do the same for old VPox versions?
    ...
    def enableNestedPaging(self, fEnable):
        ...
        fRc = True;
        try:
            self.o.machine.setHWVirtExProperty(vpoxcon.HWVirtExPropertyType_NestedPaging, fEnable);
        except:
    ...
    def enableLongMode(self, fEnable):
        ...
        """
        # Supported.
        if self.fpApiVer < 4.2  or  not hasattr(vpoxcon, 'HWVirtExPropertyType_LongMode'):
            return True;

        # Enable/disable it.
        fRc = True;
        try:
            self.o.machine.setCPUProperty(vpoxcon.CPUPropertyType_LongMode, fEnable);
        except:
    ...
    def enableNestedHwVirt(self, fEnable):
        ...
        """
        # Supported.
        if self.fpApiVer < 5.3  or  not hasattr(vpoxcon, 'CPUPropertyType_HWVirt'):
            return True;

        # Enable/disable it.
        fRc = True;
        try:
            self.o.machine.setCPUProperty(vpoxcon.CPUPropertyType_HWVirt, fEnable);
        except:
    ...
    def enablePae(self, fEnable):
        ...
        try:
            if self.fpApiVer >= 3.2:    # great, ain't it?
                self.o.machine.setCPUProperty(vpoxcon.CPUPropertyType_PAE, fEnable);
            else:
                self.o.machine.setCpuProperty(vpoxcon.CpuPropertyType_PAE, fEnable);
        except:
        ...
        try:
            if fEnable:
                if self.fpApiVer >= 4.3:
                    cOhciCtls = self.o.machine.getUSBControllerCountByType(vpoxcon.USBControllerType_OHCI);
                    if cOhciCtls == 0:
                        self.o.machine.addUSBController('OHCI', vpoxcon.USBControllerType_OHCI);
                else:
                    self.o.machine.usbController.enabled = True;

                if self.fpApiVer >= 4.2:
                    self.o.machine.pointingHIDType = vpoxcon.PointingHIDType_ComboMouse;
                    self.o.machine.keyboardHIDType = vpoxcon.KeyboardHIDType_ComboKeyboard;
                else:
                    self.o.machine.pointingHidType = vpoxcon.PointingHidType_ComboMouse;
                    self.o.machine.keyboardHidType = vpoxcon.KeyboardHidType_ComboKeyboard;
            else:
                if self.fpApiVer >= 4.2:
                    self.o.machine.pointingHIDType = vpoxcon.PointingHIDType_PS2Mouse;
                    self.o.machine.keyboardHIDType = vpoxcon.KeyboardHIDType_PS2Keyboard;
                else:
                    self.o.machine.pointingHidType = vpoxcon.PointingHidType_PS2Mouse;
                    self.o.machine.keyboardHidType = vpoxcon.KeyboardHidType_PS2Keyboard;
        except:
        ...
        try:
            if fEnable:
                if self.fpApiVer >= 4.3:
                    ...
                    cEhciCtls = self.o.machine.getUSBControllerCountByType(vpoxcon.USBControllerType_EHCI);
                    if cEhciCtls == 0:
                        self.o.machine.addUSBController('EHCI', vpoxcon.USBControllerType_EHCI);
                else:
                    self.o.machine.usbController.enabled = True;
    ...
    def enableUsbXhci(self, fEnable):
        ...
        try:
            if fEnable:
                cXhciCtls = self.o.machine.getUSBControllerCountByType(vpoxcon.USBControllerType_XHCI);
                if cXhciCtls == 0:
                    self.o.machine.addUSBController('XHCI', vpoxcon.USBControllerType_XHCI);
            else:
                cXhciCtls = self.o.machine.getUSBControllerCountByType(vpoxcon.USBControllerType_XHCI);
                if cXhciCtls == 1:
    ...
    def setupBootLogo(self, fEnable, cMsLogoDisplay = 0):
        ...
        try:
            ...
            if fEnable:
                self.o.machine.BIOSSettings.bootMenuMode = vpoxcon.BIOSBootMenuMode_Disabled;
            else:
                self.o.machine.BIOSSettings.bootMenuMode = vpoxcon.BIOSBootMenuMode_MessageAndMenu;
        except:
    ...
    def getNicDriverNameFromType(self, eNicType):
        ...
        """
        if eNicType in (vpoxcon.NetworkAdapterType_Am79C970A, vpoxcon.NetworkAdapterType_Am79C973):
            sName = 'pcnet';
        elif eNicType in (vpoxcon.NetworkAdapterType_I82540EM,
                          vpoxcon.NetworkAdapterType_I82543GC,
                          vpoxcon.NetworkAdapterType_I82545EM):
            sName = 'e1000';
        elif eNicType == vpoxcon.NetworkAdapterType_Virtio:
            sName = 'virtio-net';
        else:
    ...
    def setupNatForwardingForTxs(self, iNic = 0, iHostPort = 5042):
        ...
        for sName in ('pcnet', 'e1000', 'virtio-net'):
            for sConfig in ('VPoxInternal/Devices/%s/%u/LUN#0/AttachedDriver/Config' % (sName, iNic), \
                            'VPoxInternal/Devices/%s/%u/LUN#0/Config' % (sName, iNic)):
                try:
        ...
        try:
            ...
            return False;
        if eAttType != vpoxcon.NetworkAttachmentType_NAT:
            return True;
        ...
        if self.fpApiVer >= 4.1:
            ...
            try:
                oNatEngine.addRedirect('txs', vpoxcon.NATProtocol_TCP, '127.0.0.1', '%s' % (iHostPort), '', '5042');
            except:
            ...
            if fTraceEnabled:
                sConfig = 'VPoxInternal/Devices/%s/%u/LUN#0/AttachedDriver/Config' % (sName, iNic)
            else:
                sConfig = 'VPoxInternal/Devices/%s/%u/LUN#0/Config' % (sName, iNic)
    ...
    def getDefaultNicName(self, eAttachmentType):
        ...
        """
        sRetName = '';
        if eAttachmentType == vpoxcon.NetworkAttachmentType_Bridged:
            if self.oTstDrv.sDefBridgedNic is not None:
                sRetName = self.oTstDrv.sDefBridgedNic;
            else:
                sRetName = 'eth0';
                try:
                    aoHostNics = self.oVPoxMgr.getArray(self.oVPox.host, 'networkInterfaces');
                    for oHostNic in aoHostNics:
                        if   oHostNic.interfaceType == vpoxcon.HostNetworkInterfaceType_Bridged \
                         and oHostNic.status == vpoxcon.HostNetworkInterfaceStatus_Up:
                            sRetName = oHostNic.name;
                            break;
                except:
                    reporter.errorXcpt();

        elif eAttachmentType == vpoxcon.NetworkAttachmentType_HostOnly:
            try:
                aoHostNics = self.oVPoxMgr.getArray(self.oVPox.host, 'networkInterfaces');
                for oHostNic in aoHostNics:
                    if oHostNic.interfaceType == vpoxcon.HostNetworkInterfaceType_HostOnly:
                        if oHostNic.status == vpoxcon.HostNetworkInterfaceStatus_Up:
                            sRetName = oHostNic.name;
                            break;
                        if sRetName == '':
                            sRetName = oHostNic.name;
            ...
            if sRetName == '':
                ...
                try:
                    (oIProgress, oIHostOnly) = self.oVPox.host.createHostOnlyNetworkInterface();
                    oProgress = ProgressWrapper(oIProgress, self.oVPoxMgr, self.oTstDrv, 'Create host only NIC');
                    oProgress.wait();
        ...
        elif eAttachmentType == vpoxcon.NetworkAttachmentType_Internal:
            sRetName = 'VPoxTest';

        elif eAttachmentType == vpoxcon.NetworkAttachmentType_NAT:
            sRetName = '';
            ...
            if eAttachmentType is not None:
                try:
                    if self.fpApiVer >= 4.1:
                        ...
                        if eAttachmentType == vpoxcon.NetworkAttachmentType_NAT:
                            oNic.attachToNAT();
                        elif eAttachmentType == vpoxcon.NetworkAttachmentType_Bridged:
    ...
    def setBootOrder(self, iPosition, eType):
        ...
        @param eType        device type (vpoxcon.DeviceType_HardDisk,
                            vpoxcon.DeviceType_DVD, vpoxcon.DeviceType_Floppy)
        """
        try:
    ...
    def attachDvd(self, sImage = None, sController = "IDE Controller", iPort = 1, iDevice = 0):
        ...
        if sImage is not None:
            ...
            try:
                oImage = self.oVPox.findDVDImage(sFullName);
            except:
                try:
                    if self.fpApiVer >= 4.1:
                        oImage = self.oVPox.openMedium(sFullName, vpoxcon.DeviceType_DVD, vpoxcon.AccessMode_ReadOnly, False);
                    elif self.fpApiVer >= 4.0:
                        oImage = self.oVPox.openMedium(sFullName, vpoxcon.DeviceType_DVD, vpoxcon.AccessMode_ReadOnly);
                    else:
                        oImage = self.oVPox.openDVDImage(sFullName, "");
                except vpox.ComException as oXcpt:
                    if oXcpt.errno != -1:
        ...
        try:
            if self.fpApiVer >= 4.0:
                self.o.machine.attachDevice(sController, iPort, iDevice, vpoxcon.DeviceType_DVD, oImage);
            else:
                self.o.machine.attachDevice(sController, iPort, iDevice, vpoxcon.DeviceType_DVD, sImageUuid);
        except:
    ...
    def attachHd(self, sHd, sController = "IDE Controller", iPort = 0, iDevice = 0, fImmutable = True, fForceResource = True):
        ...
        try:
            if self.fpApiVer >= 4.0:
                self.o.machine.attachDevice(sController, iPort, iDevice, vpoxcon.DeviceType_HardDisk, oHd);
            else:
                self.o.machine.attachDevice(sController, iPort, iDevice, vpoxcon.DeviceType_HardDisk, oHd.id);
        except:
            reporter.errorXcpt('attachDevice("%s",%s,%s,HardDisk,"%s") failed on "%s"' \
    ...
    def createBaseHd(self, sHd, sFmt = "VDI", cb = 10*1024*1024*1024, cMsTimeout = 60000, tMediumVariant = None):
        ...
        if tMediumVariant is None:
            tMediumVariant = (vpoxcon.MediumVariant_Standard, );

        try:
            if self.fpApiVer >= 5.0:
                oHd = self.oVPox.createMedium(sFmt, sHd, vpoxcon.AccessMode_ReadWrite, vpoxcon.DeviceType_HardDisk);
            else:
                oHd = self.oVPox.createHardDisk(sFmt, sHd);
            oProgressXpcom = oHd.createBaseStorage(cb, tMediumVariant);
            oProgress = ProgressWrapper(oProgressXpcom, self.oVPoxMgr, self.oTstDrv, 'create base disk %s' % (sHd));
    ...
    def createDiffHd(self, oParentHd, sHd, sFmt = "VDI"):
        ...
        if sFmt is None:
            try:
                ...
                lstCaps = self.oVPoxMgr.getArray(oHdFmt, 'capabilities');
                if vpoxcon.MediumFormatCapabilities_Differencing in lstCaps:
                    sFmt = oHdFmt.id;
                else:
                    sFmt = 'VDI';
            except:
        ...
        try:
            if self.fpApiVer >= 5.0:
                ...
                oHd = self.oVPox.createHardDisk(sFmt, sHd);
            oProgressXpcom = oParentHd.createDiffStorage(oHd, (vpoxcon.MediumVariant_Standard, ))
            oProgress = ProgressWrapper(oProgressXpcom, self.oVPoxMgr, self.oTstDrv, 'create diff disk %s' % (sHd));
            oProgress.wait();
            oProgress.logResult();
        except:
    ...
    def createAndAttachHd(self, sHd, sFmt = "VDI", sController = "IDE Controller", cb = 10*1024*1024*1024, # pylint: disable=too-many-arguments
        ...
        if fRc is False:
            try:
                ...
                oProgress = ProgressWrapper(oProgressCom, self.oVPoxMgr, self.oTstDrv, 'delete disk %s' % (sHd));
                oProgress.wait();
                oProgress.logResult();
    ...
    def detachHd(self, sController = "IDE Controller", iPort = 0, iDevice = 0):
        ...
        try:
            ...
            if    self.oVPoxMgr.xcptIsOurXcptKind() \
              and self.oVPoxMgr.xcptIsEqual(None, self.oVPoxMgr.constants.VPOX_E_OBJECT_NOT_FOUND):
    ...
    def attachFloppy(self, sFloppy, sController = "Floppy Controller", iPort = 0, iDevice = 0):
        ...
        try:
            oFloppy = self.oVPox.findFloppyImage(sFullName);
        except:
            try:
                if self.fpApiVer >= 4.1:
                    oFloppy = self.oVPox.openMedium(sFullName, vpoxcon.DeviceType_Floppy, vpoxcon.AccessMode_ReadOnly, False);
                elif self.fpApiVer >= 4.0:
                    oFloppy = self.oVPox.openMedium(sFullName, vpoxcon.DeviceType_Floppy, vpoxcon.AccessMode_ReadOnly);
                else:
                    oFloppy = self.oVPox.openFloppyImage(sFullName, "");
            except:
                reporter.errorXcpt('failed to open floppy "%s"' % (sFullName));
                return False;
        ## @todo the following works but causes trouble below (asserts in main).
        #try:
        #    oFloppy.type = vpoxcon.MediumType_Immutable;
        #except:
        ...
        try:
            if self.fpApiVer >= 4.0:
                self.o.machine.attachDevice(sController, iPort, iDevice, vpoxcon.DeviceType_Floppy, oFloppy);
            else:
                self.o.machine.attachDevice(sController, iPort, iDevice, vpoxcon.DeviceType_Floppy, oFloppy.id);
        except:
    ...
    def setupNic(self, sType, sXXX):
        ...
        Returns True on success and False on failure.  Error information is logged.
        """
        if sType == "PCNet":        enmType = vpoxcon.NetworkAdapterType_Am79C973;
        elif sType == "PCNetOld":   enmType = vpoxcon.NetworkAdapterType_Am79C970A;
        elif sType == "E1000":      enmType = vpoxcon.NetworkAdapterType_I82545EM; # MT Server
        elif sType == "E1000Desk":  enmType = vpoxcon.NetworkAdapterType_I82540EM; # MT Desktop
        elif sType == "E1000Srv2":  enmType = vpoxcon.NetworkAdapterType_I82543GC; # T Server
        elif sType == "Virtio":     enmType = vpoxcon.NetworkAdapterType_Virtio;
        else:
    ...
    def setupAudio(self, eAudioControllerType, fEnable = True, eAudioDriverType = None):
        ...
        :param eAudioControllerType:    The audio controller type (vpoxcon.AudioControllerType_XXX).
        :param fEnable:                 Whether to enable or disable the audio controller (default enable).
        :param eAudioDriverType:        The audio driver type (vpoxcon.AudioDriverType_XXX), picks something suitable
        ...
        if eAudioDriverType is None:
            ...
            if   sHost == 'darwin':    eAudioDriverType = vpoxcon.AudioDriverType_CoreAudio;
            elif sHost == 'win':       eAudioDriverType = vpoxcon.AudioDriverType_DirectSound;
            elif sHost == 'linux':     eAudioDriverType = vpoxcon.AudioDriverType_Pulse;
            elif sHost == 'solaris':   eAudioDriverType = vpoxcon.AudioDriverType_OSS;
            else:
                reporter.error('PORTME: Do not know which audio driver to pick for: %s!' % (sHost,));
                eAudioDriverType = vpoxcon.AudioDriverType_Null;
    ...
    def setupPreferredConfig(self):                                             # pylint: disable=too-many-locals
        ...
        try:
            oOsType = self.oVPox.getGuestOSType(sOsTypeId);
        except:
        ...
        try:
            ...
            if self.fpApiVer >= 3.2:
                ...
                fHpet           = False;
                eFirmwareType   = -1;
                eStorCtlType    = vpoxcon.StorageControllerType_PIIX4;
            if self.fpApiVer >= 4.0:
        ...
        if eStorCtlType in (vpoxcon.StorageControllerType_PIIX3,
                            vpoxcon.StorageControllerType_PIIX4,
                            vpoxcon.StorageControllerType_ICH6,):
    ...
    def teleport(self, sHostname = 'localhost', uPort = 6500, sPassword = 'password', cMsMaxDowntime = 250):
        ...
        try:
            ...
            return None;
        return ProgressWrapper(oProgress, self.oVPoxMgr, self.oTstDrv, 'teleport %s' % (self.sName,));
    ...
    def setupSerialToRawFile(self, iSerialPort, sRawFile):
        ...
        try:
            ...
            try:
                ...
                try:
                    oPort.hostMode = vpoxcon.PortMode_RawFile;
                except:
    ...
    def enableSerialPort(self, iSerialPort):
        ...
        try:
            ...
            try:
                oPort.hostMode = vpoxcon.PortMode_Disconnected;
            except:
    ...
    def powerOff(self, fFudgeOnFailure = True):
        ...
        if rc < 0:
            self.close();
            if fFudgeOnFailure:
                vpox.reportError(oProgress, 'powerDown for "%s" failed' % (self.sName));
                self.oTstDrv.waitOnDirectSessionClose(self.oVM, 5000); # fudge
            return None;
    ...
    def saveState(self, fPause = True):
        ...
        if     fPause is True \
           and self.oVM.state is vpoxcon.MachineState_Running:
            self.o.console.pause();
        if self.oVM.state is not vpoxcon.MachineState_Paused:
    ...
    def restoreSnapshot(self, oSnapshot, fFudgeOnFailure = True):
        ...
        if rc < 0:
            self.close();
            if fFudgeOnFailure:
                vpox.reportError(oProgress, 'restoreSnapshot for "%s" failed' % (self.sName));
            return None;
    ...
    def deleteSnapshot(self, oSnapshot, fFudgeOnFailure = True, cMsTimeout = 30 * 1000):
        ...
        try:
            ...
            oProgress = ProgressWrapper(oProgressCom, self.oVPoxMgr, self.oTstDrv, 'Delete Snapshot %s' % (oSnapshot));
            oProgress.wait(cMsTimeout);
            oProgress.logResult();
        except:
    ...
    def takeSnapshot(self, sName, sDescription = '', fPause = True, fFudgeOnFailure = True, cMsTimeout = 30 * 1000):
        ...
        try:
            ...
            oProgress = ProgressWrapper(oProgressCom, self.oVPoxMgr, self.oTstDrv, 'Take Snapshot %s' % (sName));
            oProgress.wait(cMsTimeout);
            oProgress.logResult();
        except:
        ...
        if     fPause is True \
           and self.oVM.state is vpoxcon.MachineState_Paused:
    ...
    def getPrimaryIp(self):
        ...
        """
        sIpAddr = self.getGuestPropertyValue('/VirtualPox/GuestInfo/Net/0/V4/IP');
        if vpox.isIpAddrValid(sIpAddr):
            return sIpAddr;
        return '';
    ...
    def getPid(self):
        ...
        if self.uPid is None and self.o is not None and self.fRemoteSession:
            try:
                ...
                if vpox.ComError.equal(oXcpt, vpox.ComError.E_UNEXPECTED):
                    try:
                        if self.fpApiVer >= 4.2:
    ...
    def addLogsToReport(self, cReleaseLogs = 1):
        ...
        for iLog in range(0, cReleaseLogs):
            try:
                if self.fpApiVer >= 3.2:
                    ...
                    sLogFile = '%s/VPox.log' % (self.oVM.logFolder,);
                else:
                    sLogFile = '%s/VPox.log.%u' % (self.oVM.logFolder, iLog);
            except:
        ...
        try:
            sLogFile = os.path.join(self.oVM.logFolder, 'VPoxHardening.log');
        except:
            reporter.logXcpt();
            fRc = False;
        else:
    ...
    def registerDerivedEventHandler(self, oSubClass, dArgs = None, fMustSucceed = True):
        ...
        try:
            ...
            if fMustSucceed or vpox.ComError.notEqual(oXcpt, vpox.ComError.E_UNEXPECTED):
    ...
    def enableVmmDevTestingPart(self, fEnabled, fEnableMMIO = False):
        ...
        fRc = True;
        try:
            self.o.machine.setExtraData('VPoxInternal/Devices/VMMDev/0/Config/TestingEnabled',
                                        '1' if fEnabled else '');
            self.o.machine.setExtraData('VPoxInternal/Devices/VMMDev/0/Config/TestingMMIO',
    ...
    def txsConnectViaTcp(self, cMsTimeout = 10*60000, sIpAddr = None, fNatForwardingForTxs = False):
        ...
        if sIpAddr is None:
            try:
                ...
                if enmAttachmentType == vpoxcon.NetworkAttachmentType_NAT:
                    fUseNatForTxs = True;
                elif enmAttachmentType == vpoxcon.NetworkAttachmentType_HostOnly and not sIpAddr:
                    # Get the MAC address and find the DHCP server.
                    sMacAddr      = oNic.MACAddress;
                    sHostOnlyNIC  = oNic.hostOnlyInterface;
                    oIHostOnlyIf  = self.oVPox.host.findHostNetworkInterfaceByName(sHostOnlyNIC);
                    sHostOnlyNet  = oIHostOnlyIf.networkName;
                    oIDhcpServer  = self.oVPox.findDHCPServerByNetworkName(sHostOnlyNet);
            except:
                reporter.errorXcpt();
                return None;
...
class TxsConnectTask(TdTaskBase):
    ...
    class TxsConnectTaskVPoxCallback(vpox.VirtualPoxEventHandlerBase):
        """ Class for looking for IPv4 address changes on interface 0."""
        def __init__(self, dArgs):
            vpox.VirtualPoxEventHandlerBase.__init__(self, dArgs);
        ...
        def onGuestPropertyChange(self, sMachineId, sName, sValue, sFlags):
            ...
            if    sMachineId == self.sMachineId \
              and sName  == '/VirtualPox/GuestInfo/Net/0/V4/IP':
    ...
    def __init__(self, oSession, cMsTimeout, sIpAddr, sMacAddr, oIDhcpServer, fReversedSetup, fnProcessEvents = None):
        ...
        if fReversedSetup is True:
            ...
            #try:
            #    oSession.delGuestPropertyValue('/VirtualPox/GuestInfo/Net/0/V4/IP');
            #except:
            #    reporter.logXcpt();

            # 1. Register the callback / event listener object.
            dArgs = {'oParentTask':self, 'sMachineId':oSession.o.machine.id};
            self.oVPoxEventHandler = oSession.oVPox.registerDerivedEventHandler(self.TxsConnectTaskVPoxCallback, dArgs);

            # 2. Query the guest properties.
            try:
                sIpAddr = oSession.getGuestPropertyValue('/VirtualPox/GuestInfo/Net/0/V4/IP');
            except:
                reporter.errorXcpt('IMachine::getGuestPropertyValue("/VirtualPox/GuestInfo/Net/0/V4/IP") failed');
    ...
    def toString(self):
        return '<%s cMsTimeout=%s, sIpAddr=%s, sNextIpAddr=%s, sMacAddr=%s, fReversedSetup=%s,' \
               ' oTxsSession=%s oVPoxEventHandler=%s>' \
             % (TdTaskBase.toString(self), self.cMsTimeout, self.sIpAddr, self.sNextIpAddr, self.sMacAddr, self.fReversedSetup,
                self.oTxsSession, self.oVPoxEventHandler);

    def _deregisterEventHandler(self):
        """Deregisters the event handler."""
        fRc = True;
        oVPoxEventHandler = self.oVPoxEventHandler;
        if oVPoxEventHandler is not None:
            self.oVPoxEventHandler = None;
            fRc = oVPoxEventHandler.unregister();
            oVPoxEventHandler.oParentTask = None; # Try avoid cylic deps.
        return fRc;
    ...
    def _setIp(self, sIpAddr, fInitCall = False):
        ...
        if   sIpAddr is not None \
         and sIpAddr != '':
            if vpox.isIpAddrValid(sIpAddr) or fInitCall:
                try:
...
class AdditionsStatusTask(TdTaskBase):
    ...
    class AdditionsStatusTaskCallback(vpox.EventHandlerBase):
        """ Class for looking for IPv4 address changes on interface 0."""
        def __init__(self, dArgs):
            self.oParentTask = dArgs['oParentTask'];
            vpox.EventHandlerBase.__init__(self, dArgs, self.oParentTask.oSession.fpApiVer,
        ...
        def handleEvent(self, oEvt):
            try:
                ...
                if enmType == vpoxcon.VPoxEventType_OnGuestAdditionsStatusChanged:
                    oParentTask = self.oParentTask;
                    if oParentTask:
    ...
    def __init__(self, oSession, oIGuest, cMsTimeout = 120000, aenmWaitForRunLevels = None, aenmWaitForActive = None,
        ...
        """
        TdTaskBase.__init__(self, utils.getCallerName());
        self.oSession               = oSession      # type: vpoxwrappers.SessionWrapper
        ...
        if not self.aenmWaitForRunLevels and not self.aenmWaitForActive and not self.aenmWaitForInactive:
            self.aenmWaitForRunLevels = [vpoxcon.AdditionsRunLevelType_Userland,];

        # Register the event handler on hosts which has it:
        if oSession.fpApiVer >= 6.1 or hasattr(vpoxcon, 'VPoxEventType_OnGuestAdditionsStatusChanged'):
            aenmEvents = (vpoxcon.VPoxEventType_OnGuestAdditionsStatusChanged,);
            dArgs = {
                'oParentTask': self,
            };
            self.oVPoxEventHandler = vpox.EventHandlerBase.registerDerivedEventHandler(oSession.oVPoxMgr,
                                                                                       oSession.fpApiVer,
    ...
    def toString(self):
        return '<%s cMsTimeout=%s, fSucceeded=%s, aenmWaitForRunLevels=%s, aenmWaitForActive=%s, aenmWaitForInactive=%s, ' \
             ...
             % (TdTaskBase.toString(self), self.cMsTimeout, self.fSucceeded, self.aenmWaitForRunLevels, self.aenmWaitForActive,
                self.aenmWaitForInactive, self.oVPoxEventHandler,);
    ...
    def _poll(self):
        ...
        for enmFacility in self.aenmWaitForActive:
            try:
                ...
                return True;
            if enmStatus != vpoxcon.AdditionsFacilityStatus_Active:
        ...
        for enmFacility in self.aenmWaitForInactive:
            try:
                ...
                reporter.errorXcpt('enmFacility=%s' % (enmFacility,));
                return True;
            if enmStatus not in (vpoxcon.AdditionsFacilityStatus_Inactive,
                                 vpoxcon.AdditionsFacilityStatus_Terminated):
