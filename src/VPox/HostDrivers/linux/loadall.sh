...
MY_DIR="${TARGET%/[!/]*}"
MY_GROUPNAME="vpoxusers"

#
# vpoxusers membership check.
#
if ! (id -Gn | grep -w ${MY_GROUPNAME}); then
    echo "loadall.sh: you're not a member of vpoxusers...";
    # Create the group.
...
#
if [ ${#} -eq 0 ]; then
    if [ -f "${MY_DIR}/src/vpoxdrv/vpoxdrv.ko" ]; then
        echo "Cleaning build folder."
        make -C "${MY_DIR}/src" clean > /dev/null
    fi
    sudo "${MY_DIR}/vpoxdrv.sh" setup

#
# Unload and clean up when '-u' is given.
#
elif [ ${#} -eq 1  -a "x${1}" = x-u ]; then
    sudo "${MY_DIR}/vpoxdrv.sh" cleanup
