 ...
 *
 * We strongly recomment users of the VPoxCAPI to check all return codes!
 */
...
# error "not supposed to involve any IPRT or VPox headers here."
#endif
 ...
 */
#ifdef WIN32
static BOOL VPOX_WINAPI ctrlCHandler(DWORD iInfo)
{
    (void)iInfo;
    g_fStop = 1;
    return TRUE;
...
static HRESULT EventListenerDemoProcessEvent(IEvent *event)
{
    ...
    switch (evType)
    {
        case VPoxEventType_OnMousePointerShapeChanged:
        ...
        case VPoxEventType_OnKeyboardLedsChanged:
            printf("OnMouseCapabilityChanged\n");
            break;
        ...
        case VPoxEventType_OnAdditionsStateChanged:
            printf("OnAdditionsStateChanged\n");
            break;

        case VPoxEventType_OnNetworkAdapterChanged:
            printf("OnNetworkAdapterChanged\n");
            break;

        case VPoxEventType_OnSerialPortChanged:
            printf("OnSerialPortChanged\n");
            break;

        case VPoxEventType_OnParallelPortChanged:
            printf("OnParallelPortChanged\n");
            break;

        case VPoxEventType_OnStorageControllerChanged:
            printf("OnStorageControllerChanged\n");
            break;

        case VPoxEventType_OnMediumChanged:
            printf("OnMediumChanged\n");
            break;

        case VPoxEventType_OnVRDEServerChanged:
            printf("OnVRDEServerChanged\n");
            break;

        case VPoxEventType_OnUSBControllerChanged:
            printf("OnUSBControllerChanged\n");
            break;

        case VPoxEventType_OnUSBDeviceStateChanged:
            printf("OnUSBDeviceStateChanged\n");
            break;

        case VPoxEventType_OnSharedFolderChanged:
            printf("OnSharedFolderChanged\n");
            break;

        case VPoxEventType_OnRuntimeError:
...
static HRESULT LoadTypeInfo(REFIID riid, ITypeInfo **pTInfo)
{
    HRESULT rc;
    ITypeLib *pTypeLib;
    rc = LoadRegTypeLib(&LIBID_VirtualPox, 1 /* major */, 0 /* minor */, 0 /* lcid */, &pTypeLib);
    if (FAILED(rc))
        return rc;
 ...
 *
 * @param   virtualPox ptr to IVirtualPox object
 * @param   session    ptr to ISession object
 */
static void registerActiveEventListener(IVirtualPox *virtualPox, ISession *session)
{
    ...
    if ((SUCCEEDED(rc)) && console)
    {
        ...
        if (SUCCEEDED(rc) && es)
        {
            ...
            {
                VPoxEventType_OnMousePointerShapeChanged,
                VPoxEventType_OnMouseCapabilityChanged,
                VPoxEventType_OnKeyboardLedsChanged,
                VPoxEventType_OnStateChanged,
                VPoxEventType_OnAdditionsStateChanged,
                VPoxEventType_OnNetworkAdapterChanged,
                VPoxEventType_OnSerialPortChanged,
                VPoxEventType_OnParallelPortChanged,
                VPoxEventType_OnStorageControllerChanged,
                VPoxEventType_OnMediumChanged,
                VPoxEventType_OnVRDEServerChanged,
                VPoxEventType_OnUSBControllerChanged,
                VPoxEventType_OnUSBDeviceStateChanged,
                VPoxEventType_OnSharedFolderChanged,
                VPoxEventType_OnRuntimeError,
                VPoxEventType_OnCanShowWindow,
                VPoxEventType_OnShowWindow
            };
            SAFEARRAY *interestingEventsSA = NULL;
            IEventListenerDemo *consoleListener = NULL;

            /* The VirtualPox API expects enum values as VT_I4, which in the
             * future can be hopefully relaxed. */
            interestingEventsSA = g_pVPoxFuncs->pfnSafeArrayCreateVector(VT_I4, 0,
                                                                           sizeof(s_auInterestingEvents)
                                                                         / sizeof(s_auInterestingEvents[0]));
            g_pVPoxFuncs->pfnSafeArrayCopyInParamHelper(interestingEventsSA, &s_auInterestingEvents,
                                                        sizeof(s_auInterestingEvents));
            ...
            if (consoleListener)
            {
                ...
                if (SUCCEEDED(rc))
                {
                    ...
                    while (!g_fStop)
                        g_pVPoxFuncs->pfnProcessEventQueue(250);
            ...
            g_pVPoxFuncs->pfnSafeArrayDestroy(interestingEventsSA);
...
static void PrintErrorInfo(const char *pszExecutable, const char *pszErrorMsg, HRESULT rc)
{
    ...
    rc2 = g_pVPoxFuncs->pfnGetException(&ex);
    if (SUCCEEDED(rc2) && ex)
    {
        IVirtualPoxErrorInfo *ei;
        rc2 = IErrorInfo_QueryInterface(ex, &IID_IVirtualPoxErrorInfo, (void **)&ei);
        if (SUCCEEDED(rc2) && ei != NULL)
        {
            ...
            do
            {
                ...
                BSTR textUtf16 = NULL;
                char *text = NULL;
                IVirtualPoxErrorInfo *ei_next = NULL;
                fprintf(stderr, "Extended error info (IVirtualPoxErrorInfo):\n");

                IVirtualPoxErrorInfo_get_ResultCode(ei, &resultCode);
                fprintf(stderr, "  resultCode=%#010x\n", (unsigned)resultCode);

                IVirtualPoxErrorInfo_get_Component(ei, &componentUtf16);
                g_pVPoxFuncs->pfnUtf16ToUtf8(componentUtf16, &component);
                g_pVPoxFuncs->pfnComUnallocString(componentUtf16);
                fprintf(stderr, "  component=%s\n", component);
                g_pVPoxFuncs->pfnUtf8Free(component);

                IVirtualPoxErrorInfo_get_Text(ei, &textUtf16);
                g_pVPoxFuncs->pfnUtf16ToUtf8(textUtf16, &text);
                g_pVPoxFuncs->pfnComUnallocString(textUtf16);
                fprintf(stderr, "  text=%s\n", text);
                g_pVPoxFuncs->pfnUtf8Free(text);

                rc2 = IVirtualPoxErrorInfo_get_Next(ei, &ei_next);
                if (FAILED(rc2))
                    ei_next = NULL;
                IVirtualPoxErrorInfo_Release(ei);
                ei = ei_next;
            } while (ei);
        }

        IErrorInfo_Release(ex);
        g_pVPoxFuncs->pfnClearException();
    }
}
 ...
 */
static void startVM(const char *argv0, IVirtualPox *virtualPox, ISession *session, BSTR id)
{
    HRESULT rc;
    ...
    SAFEARRAY *env        = NULL;
    BSTR sessionType;
    SAFEARRAY *groupsSA = g_pVPoxFuncs->pfnSafeArrayOutParamAlloc();

    rc = IVirtualPox_FindMachine(virtualPox, id, &machine);
    ...
    if (SUCCEEDED(rc))
    {
        ...
        ULONG cbGroups = 0;
        ULONG i, cGroups;
        g_pVPoxFuncs->pfnSafeArrayCopyOutParamHelper((void **)&groups, &cbGroups, VT_BSTR, groupsSA);
        g_pVPoxFuncs->pfnSafeArrayDestroy(groupsSA);
        ...
        for (i = 0; i < cGroups; ++i)
        {
            ...
            char *group;
            g_pVPoxFuncs->pfnUtf16ToUtf8(groups[i], &group);
            printf("Groups[%d]: %s\n", i, group);
            g_pVPoxFuncs->pfnUtf8Free(group);
        }
        for (i = 0; i < cGroups; ++i)
            g_pVPoxFuncs->pfnComUnallocString(groups[i]);
        g_pVPoxFuncs->pfnArrayOutFree(groups);
    }

    g_pVPoxFuncs->pfnUtf8ToUtf16("gui", &sessionType);
    rc = IMachine_LaunchVMProcess(machine, session, sessionType, ComSafeArrayAsInParam(env), &progress);
    g_pVPoxFuncs->pfnUtf16Free(sessionType);
    if (SUCCEEDED(rc))
    {
        BOOL completed;
        LONG resultCode;
        ...
        {
            IVirtualPoxErrorInfo *errorInfo;
            BSTR textUtf16;
            char *text;

            IProgress_get_ErrorInfo(progress, &errorInfo);
            IVirtualPoxErrorInfo_get_Text(errorInfo, &textUtf16);
            g_pVPoxFuncs->pfnUtf16ToUtf8(textUtf16, &text);
            printf("Error: %s\n", text);

            g_pVPoxFuncs->pfnComUnallocString(textUtf16);
            g_pVPoxFuncs->pfnUtf8Free(text);
            IVirtualPoxErrorInfo_Release(errorInfo);
        }
        else
        {
...
#ifdef USE_ACTIVE_EVENT_LISTENER
            registerActiveEventListener(virtualPox, session);
#else
 ...
 * @param   session     ptr to ISession object
 */
static void listVMs(const char *argv0, IVirtualPox *virtualPox, ISession *session)
{
    HRESULT rc;
    SAFEARRAY *machinesSA = g_pVPoxFuncs->pfnSafeArrayOutParamAlloc();
     ...
     */
    rc = IVirtualPox_get_Machines(virtualPox, ComSafeArrayAsOutIfaceParam(machinesSA, IMachine *));
    if (FAILED(rc))
    {
     ...
     */
    g_pVPoxFuncs->pfnSafeArrayCopyOutIfaceParamHelper((IUnknown ***)&machines, &machineCnt, machinesSA);
    g_pVPoxFuncs->pfnSafeArrayDestroy(machinesSA);

    if (!machineCnt)
    {
        g_pVPoxFuncs->pfnArrayOutFree(machines);
        printf("\tNo VMs\n");
        return;
    }
    ...
    for (i = 0; i < machineCnt; ++i)
    {
        ...
        if (isAccessible)
        {
            ...
            g_pVPoxFuncs->pfnUtf16ToUtf8(machineNameUtf16,&machineName);
            g_pVPoxFuncs->pfnComUnallocString(machineNameUtf16);
            printf("\tName:        %s\n", machineName);
            g_pVPoxFuncs->pfnUtf8Free(machineName);
        }
        else
...

        {
            ...
            g_pVPoxFuncs->pfnUtf16ToUtf8(uuidUtf16, &uuidUtf8);
            g_pVPoxFuncs->pfnComUnallocString(uuidUtf16);
            printf("\tUUID:        %s\n", uuidUtf8);
            g_pVPoxFuncs->pfnUtf8Free(uuidUtf8);
        }
        ...
        {
            {
                ...
                g_pVPoxFuncs->pfnUtf16ToUtf8(configFileUtf16, &configFileUtf8);
                g_pVPoxFuncs->pfnComUnallocString(configFileUtf16);
                printf("\tConfig file: %s\n", configFileUtf8);
                g_pVPoxFuncs->pfnUtf8Free(configFileUtf8);
            }
...

            {
                ...
                IVirtualPox_GetGuestOSType(virtualPox, typeId, &osType);
                g_pVPoxFuncs->pfnComUnallocString(typeId);
                IGuestOSType_get_Description(osType, &osNameUtf16);
                g_pVPoxFuncs->pfnUtf16ToUtf8(osNameUtf16,&osName);
                g_pVPoxFuncs->pfnComUnallocString(osNameUtf16);
                printf("\tGuest OS:    %s\n\n", osName);
                g_pVPoxFuncs->pfnUtf8Free(osName);
    ...
    if (scanf("%u", &start_id) == 1 && start_id < machineCnt)
    {
        ...
        if (machine)
        {
            ...
            IMachine_get_Id(machine, &uuidUtf16);
            startVM(argv0, virtualPox, session, uuidUtf16);
            g_pVPoxFuncs->pfnComUnallocString(uuidUtf16);
        }
    }
...
{
    IVirtualPoxClient *vpoxclient = NULL;
    IVirtualPox *vpox            = NULL;
    ...
    if (VPoxCGlueInit())
    {
        fprintf(stderr, "%s: FATAL: VPoxCGlueInit failed: %s\n",
                argv[0], g_szVPoxErrMsg);
        return EXIT_FAILURE;
    }

    {
        unsigned ver = g_pVPoxFuncs->pfnGetVersion();
        printf("VirtualPox version: %u.%u.%u\n", ver / 1000000, ver / 1000 % 1000, ver % 1000);
        ver = g_pVPoxFuncs->pfnGetAPIVersion();
        printf("VirtualPox API version: %u.%u\n", ver / 1000, ver % 1000);
    }

    g_pVPoxFuncs->pfnClientInitialize(NULL, &vpoxclient);
    if (!vpoxclient)
    {
        fprintf(stderr, "%s: FATAL: could not get VirtualPoxClient reference\n", argv[0]);
        return EXIT_FAILURE;
    }

    printf("----------------------------------------------------\n");

    rc = IVirtualPoxClient_get_VirtualPox(vpoxclient, &vpox);
    if (FAILED(rc) || !vpox)
    {
        PrintErrorInfo(argv[0], "FATAL: could not get VirtualPox reference", rc);
        return EXIT_FAILURE;
    }
    rc = IVirtualPoxClient_get_Session(vpoxclient, &session);
...
# endif /* WIN32 */
     ...
     * this vpox. Were not using fancy macros here so it
     * remains easy to see how we access C++'s vtable.
     */

    /* 1. Revision */
    rc = IVirtualPox_get_Revision(vpox, &revision);
    if (SUCCEEDED(rc))
        printf("\tRevision: %u\n", revision);
    else
    ...
    /* 2. Version */
    rc = IVirtualPox_get_Version(vpox, &versionUtf16);
    if (SUCCEEDED(rc))
    {
        char *version = NULL;
        g_pVPoxFuncs->pfnUtf16ToUtf8(versionUtf16, &version);
        printf("\tVersion: %s\n", version);
        g_pVPoxFuncs->pfnUtf8Free(version);
        g_pVPoxFuncs->pfnComUnallocString(versionUtf16);
    }
    else
        PrintErrorInfo(argv[0], "GetVersion() failed", rc);

    /* 3. Home Folder */
    rc = IVirtualPox_get_HomeFolder(vpox, &homefolderUtf16);
    if (SUCCEEDED(rc))
    {
        char *homefolder = NULL;
        g_pVPoxFuncs->pfnUtf16ToUtf8(homefolderUtf16, &homefolder);
        printf("\tHomeFolder: %s\n", homefolder);
        g_pVPoxFuncs->pfnUtf8Free(homefolder);
        g_pVPoxFuncs->pfnComUnallocString(homefolderUtf16);
    }
    else
        PrintErrorInfo(argv[0], "GetHomeFolder() failed", rc);

    listVMs(argv[0], vpox, session);
...
# endif /* WIN32 */
    ...
    if (vpox)
    {
        IVirtualPox_Release(vpox);
        vpox = NULL;
    }
    if (vpoxclient)
    {
        IVirtualPoxClient_Release(vpoxclient);
        vpoxclient = NULL;
    }

    g_pVPoxFuncs->pfnClientUninitialize();
    VPoxCGlueTerm();
