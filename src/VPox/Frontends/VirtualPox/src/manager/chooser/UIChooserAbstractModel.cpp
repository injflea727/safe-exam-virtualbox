...
/** @file
 * VPox Qt GUI - UIChooserAbstractModel class implementation.
 */
...
void UIChooserAbstractModel::sltMachineRegistered(const QUuid &uMachineId, const bool fRegistered)
{
    ...
    else
    {
        ...
        if (gEDataManager->showMachineInVirtualPoxManagerChooser(uMachineId))
        {
            /* Add new machine-item: */
            const CMachine comMachine = uiCommon().virtualPox().FindMachine(uMachineId.toString());
...
void UIChooserAbstractModel::prepareConnections()
{
    ...
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigMachineDataChange,
    ...
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigSnapshotTake,
            this, &UIChooserAbstractModel::sltSnapshotChanged);
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigSnapshotDelete,
            this, &UIChooserAbstractModel::sltSnapshotChanged);
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigSnapshotChange,
            this, &UIChooserAbstractModel::sltSnapshotChanged);
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigSnapshotRestore,
            this, &UIChooserAbstractModel::sltSnapshotChanged);
...
void UIChooserAbstractModel::loadTree()
{
    ...
    if (invisibleRoot())
    {
        ...
        foreach (const CMachine &comMachine, uiCommon().virtualPox().GetMachines())
        {
            const QUuid uMachineID = comMachine.GetId();
            if (!uMachineID.isNull() && gEDataManager->showMachineInVirtualPoxManagerChooser(uMachineID))
        ...
        CCloudProviderManager comCloudProviderManager = comVPox.GetCloudProviderManager();
        /* Show error message if necessary: */
        if (!comVPox.isOk())
            msgCenter().cannotAcquireCloudProviderManager(comVPox);
        else
        {
