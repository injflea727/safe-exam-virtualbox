...
EFI_DRIVER_BINDING_PROTOCOL gVPoxVgaDriverBinding = {
...
///
VPOX_VGA_VIDEO_MODES  VPoxVgaVideoModes[] =
{
...
const UINT32 VPoxVgaVideoModeCount = sizeof(VPoxVgaVideoModes) / sizeof(VPoxVgaVideoModes[0]);
  ...
  )
{
  ...
  //
  // See if this is a VirtualPox VGA or VMSVGA II PCI controller
  //
  if ( ((Pci.Hdr.VendorId == VPOX_VENDOR_ID) && (Pci.Hdr.DeviceId == VPOX_VGA_DEVICE_ID))
  ...
  )
{
  ...
  //
  Private = AllocateZeroPool (sizeof (VPOX_VGA_PRIVATE_DATA));
  if (Private == NULL) {
  ...
  //
  Private->Signature  = VPOX_VGA_PRIVATE_DATA_SIGNATURE;
  Private->Handle     = NULL;
  ...
  //
  Status = VPoxVgaVideoModeSetup (Private);
  if (EFI_ERROR (Status)) {
    goto Error;
  }
  ...
  if (FeaturePcdGet (PcdSupportUga)) {
    ...
    //
    Status = VPoxVgaUgaDrawConstructor (Private);
    ...
    if (Private->GopDevicePath == NULL) {
      ...
      //
      Status = VPoxVgaGraphicsOutputConstructor (Private);
      ASSERT_EFI_ERROR (Status);
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdSupportUga)) {
    ...
    //
    Private = VPOX_VGA_PRIVATE_DATA_FROM_UGA_DRAW_THIS (UgaDraw);
    VPoxVgaUgaDrawDestructor (Private);

    if (FeaturePcdGet (PcdSupportGop)) {
      VPoxVgaGraphicsOutputDestructor (Private);
      //
    ...
    // Get our private context information
    //
    Private = VPOX_VGA_PRIVATE_DATA_FROM_GRAPHICS_OUTPUT_THIS (GraphicsOutput);
                   ...
                   OUT UINT32                           *Depth)
{
    ...
    UINT32 W, H, BPP;
    VPOX_VGA_PRIVATE_DATA  *Private = This->Private;
    UINTN CurrentModeNumber = Private->CurrentMode;
    VPOX_VGA_MODE_DATA const *pCurrentMode = &Private->ModeData[CurrentModeNumber];
  ...
  )
{
  ...
  Status = EfiLibInstallDriverBindingComponentName2 (
             ImageHandle,
             SystemTable,
             &gVPoxVgaDriverBinding,
             ImageHandle,
             &gVPoxVgaComponentName,
             &gVPoxVgaComponentName2
             );
  ASSERT_EFI_ERROR (Status);
  ...
  //
  gVPoxVgaDriverSupportedEfiVersion.FirmwareVersion = PcdGet32 (PcdDriverSupportedEfiVersion);
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &ImageHandle,
                  &gEfiDriverSupportedEfiVersionProtocolGuid,
                  &gVPoxVgaDriverSupportedEfiVersion,
