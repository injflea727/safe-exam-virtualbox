...
# $Id: vpoxconfig.sh $
## @file
# VirtualPox Configuration Script, Solaris host.
#
...
DIR_VPOXBASE="$PKG_INSTALL_ROOT/opt/VirtualPox"
...
# "vpoxdrv" is also used in sed lines here (change those as well if it ever changes)
MOD_VPOXDRV=vpoxdrv
DESC_VPOXDRV="Host"

MOD_VPOXNET=vpoxnet
DESC_VPOXNET="NetAdapter"
MOD_VPOXNET_INST=8

MOD_VPOXFLT=vpoxflt
DESC_VPOXFLT="NetFilter (STREAMS)"

MOD_VPOXBOW=vpoxbow
DESC_VPOXBOW="NetFilter (Crossbow)"

MOD_VPOXUSBMON=vpoxusbmon
DESC_VPOXUSBMON="USBMonitor"
...
printusage()
{
    helpprint "VirtualPox Configuration Script"
...
check_isa()
{
    ...
    if test "x$currentisa" = "xi86xpv"; then
        errorprint "VirtualPox cannot run under xVM Dom0! Fatal Error, Aborting installation!"
        exit 1
    fi
}
...
check_module_arch()
{
    ...
    if test "x$cputype" != "xamd64" && test "x$cputype" != "xi386"; then
        errorprint "VirtualPox works only on i386/amd64 hosts, not $cputype"
        exit 1
    fi
}
...
module_added()
{
    ...
    # any substring matches: e.g "vpoxusb" & "vpoxusbmon"
...
unload_module()
{
    ...
    if test -n "$modid"; then
        ...
        if test "$?" -eq 0; then
            ...
            #
            # Hack for vpoxdrv. Delayed removing when VMM thread-context hooks are used.
...
load_vpoxflt()
{
    if test -f "$DIR_CONF/vpoxflt.conf"; then
        add_driver "$MOD_VPOXFLT" "$DESC_VPOXFLT" "$FATALOP"
        load_module "drv/$MOD_VPOXFLT" "$DESC_VPOXFLT" "$FATALOP"
    else
        # For custom pkgs that optionally ship this module, let's not fail but just warn
        warnprint "$DESC_VPOXFLT installation requested but not shipped in this package."
    fi
}

load_vpoxbow()
{
    if test -f "$DIR_CONF/vpoxbow.conf"; then
        add_driver "$MOD_VPOXBOW" "$DESC_VPOXBOW" "$FATALOP"
        load_module "drv/$MOD_VPOXBOW" "$DESC_VPOXBOW" "$FATALOP"
    else
        # For custom pkgs that optionally ship this module, let's not fail but just warn
        warnprint "$DESC_VPOXBOW installation requested but not shipped in this package."
    fi
}
...
install_drivers()
{
    if test -f "$DIR_CONF/vpoxdrv.conf"; then
        if test -n "_HARDENED_"; then
            add_driver "$MOD_VPOXDRV" "$DESC_VPOXDRV" "$FATALOP" "not-$NULLOP" "'* 0600 root sys','vpoxdrvu 0666 root sys'"
        else
            add_driver "$MOD_VPOXDRV" "$DESC_VPOXDRV" "$FATALOP" "not-$NULLOP" "'* 0666 root sys','vpoxdrvu 0666 root sys'"
        fi
        load_module "drv/$MOD_VPOXDRV" "$DESC_VPOXDRV" "$FATALOP"
    else
        errorprint "Extreme error! Missing $DIR_CONF/vpoxdrv.conf, aborting."
        return 1
    fi
    ...
    ## Add vpoxdrv to devlink.tab (KEEP TABS!)
    if test -f "$PKG_INSTALL_ROOT/etc/devlink.tab"; then
        sed -e '/name=vpoxdrv/d' -e '/name=vpoxdrvu/d' "$PKG_INSTALL_ROOT/etc/devlink.tab" > "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        echo "type=ddi_pseudo;name=vpoxdrv;minor=vpoxdrv	\D"  >> "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        echo "type=ddi_pseudo;name=vpoxdrv;minor=vpoxdrvu	\M0" >> "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        chmod 0644 "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        chown root:$group "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        mv -f "$PKG_INSTALL_ROOT/etc/devlink.vpox" "$PKG_INSTALL_ROOT/etc/devlink.tab"
    else
    ...
    if test "$REMOTEINST" -eq 0; then
        /usr/sbin/devfsadm -i "$MOD_VPOXDRV"
        if test "$?" -ne 0 || test ! -h "/dev/vpoxdrv" || test ! -h "/dev/vpoxdrvu" ; then
            errorprint "Failed to create device link for $MOD_VPOXDRV."
            exit 1
        fi
    fi

    # Load VPoxNetAdp
    if test -f "$DIR_CONF/vpoxnet.conf"; then
        add_driver "$MOD_VPOXNET" "$DESC_VPOXNET" "$FATALOP"
        load_module "drv/$MOD_VPOXNET" "$DESC_VPOXNET" "$FATALOP"
    fi

    # If both vpoxinst_vpoxbow and vpoxinst_vpoxflt exist, bail.
    if test -f "$PKG_INSTALL_ROOT/etc/vpoxinst_vpoxflt" && test -f "$PKG_INSTALL_ROOT/etc/vpoxinst_vpoxbow"; then
        errorprint "Force-install files '$PKG_INSTALL_ROOT/etc/vpoxinst_vpoxflt' and '$PKG_INSTALL_ROOT/etc/vpoxinst_vpoxbow' both exist."
        errorprint "Cannot load $DESC_VPOXFLT and $DESC_VPOXBOW drivers at the same time."
        return 1
    fi

    # If the force-install files exists, install blindly
    if test -f "$PKG_INSTALL_ROOT/etc/vpoxinst_vpoxflt"; then
        subprint "Detected: Force-load file $PKG_INSTALL_ROOT/etc/vpoxinst_vpoxflt."
        load_vpoxflt
    elif test -f "$PKG_INSTALL_ROOT/etc/vpoxinst_vpoxbow"; then
        subprint "Detected: Force-load file $PKG_INSTALL_ROOT/etc/vpoxinst_vpoxbow."
        load_vpoxbow
    else
        # If host is S10 or S11 (< snv_159) or vpoxbow isn't shipped, then load vpoxflt
        if     test "$HOST_OS_MAJORVERSION" -eq 10 \
           || (test "$HOST_OS_MAJORVERSION" -eq 11 && test "$HOST_OS_MINORVERSION" -lt 159) \
           ||  test ! -f "$DIR_CONF/vpoxbow.conf"; then
            load_vpoxflt
        else
            # For S11 snv_159+ load vpoxbow
            load_vpoxbow
        fi
    fi

    # Load VPoxUSBMon, VPoxUSB
    try_vpoxusb="no"
    if test -f "$DIR_CONF/vpoxusbmon.conf"; then
        if test -f "$PKG_INSTALL_ROOT/etc/vpoxinst_vpoxusb"; then
            subprint "Detected: Force-load file $PKG_INSTALL_ROOT/etc/vpoxinst_vpoxusb."
            try_vpoxusb="yes"
        else
            # For VirtualPox 3.1 the new USB code requires Nevada > 123 i.e. S12+ or S11 b124+
    ...
    if test "x$try_vpoxusb" = "xyes"; then
        # Add a group "vpoxuser" (8-character limit) for USB access.
        # All users which need host USB-passthrough support will have to be added to this group.
        groupadd vpoxuser >/dev/null 2>&1

        add_driver "$MOD_VPOXUSBMON" "$DESC_VPOXUSBMON" "$FATALOP" "not-$NULLOP" "'* 0666 root sys'"
        load_module "drv/$MOD_VPOXUSBMON" "$DESC_VPOXUSBMON" "$FATALOP"

        chown root:vpoxuser "/devices/pseudo/vpoxusbmon@0:vpoxusbmon"

        # Add vpoxusbmon to devlink.tab
        sed -e '/name=vpoxusbmon/d' "$PKG_INSTALL_ROOT/etc/devlink.tab" > "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        echo "type=ddi_pseudo;name=vpoxusbmon	\D" >> "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        chmod 0644 "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        chown root:$group "$PKG_INSTALL_ROOT/etc/devlink.vpox"
        mv -f "$PKG_INSTALL_ROOT/etc/devlink.vpox" "$PKG_INSTALL_ROOT/etc/devlink.tab"
        ...
        if test "$REMOTEINST" -eq 0; then
            /usr/sbin/devfsadm -i  "$MOD_VPOXUSBMON"
            if test "$?" -ne 0; then
                errorprint "Failed to create device link for $MOD_VPOXUSBMON."
                exit 1
            fi
        fi

        # Add vpoxusb if present
        ...
        if test -f "$DIR_CONF/vpoxusb.conf"; then
            add_driver "$MOD_VPOXUSB" "$DESC_VPOXUSB" "$FATALOP" "$NULLOP"
            load_module "drv/$MOD_VPOXUSB" "$DESC_VPOXUSB" "$FATALOP"
        fi
    fi
...
remove_drivers()
{
    ...
    # Remove vpoxdrv[u] from devlink.tab
    if test -f "$PKG_INSTALL_ROOT/etc/devlink.tab"; then
        devlinkfound=`cat "$PKG_INSTALL_ROOT/etc/devlink.tab" | grep vpoxdrv`
        ...
        # Remove vpoxusbmon from devlink.tab
        devlinkfound=`cat "$PKG_INSTALL_ROOT/etc/devlink.tab" | grep vpoxusbmon`
        if test -n "$devlinkfound"; then
            sed -e '/name=vpoxusbmon/d' "$PKG_INSTALL_ROOT/etc/devlink.tab" > "$PKG_INSTALL_ROOT/etc/devlink.vpox"
            chmod 0644 "$PKG_INSTALL_ROOT/etc/devlink.vpox"
            chown root:$group "$PKG_INSTALL_ROOT/etc/devlink.vpox"
    ...
    unload_module "$MOD_VPOXUSB" "$DESC_VPOXUSB" 0 "$fatal"
    rem_driver "$MOD_VPOXUSB" "$DESC_VPOXUSB" "$fatal"

    unload_module "$MOD_VPOXUSBMON" "$DESC_VPOXUSBMON" 0 "$fatal"
    rem_driver "$MOD_VPOXUSBMON" "$DESC_VPOXUSBMON" "$fatal"

    unload_module "$MOD_VPOXFLT" "$DESC_VPOXFLT" 0 "$fatal"
    rem_driver "$MOD_VPOXFLT" "$DESC_VPOXFLT" "$fatal"

    unload_module "$MOD_VPOXBOW" "$DESC_VPOXBOW" 0 "$fatal"
    rem_driver "$MOD_VPOXBOW" "$DESC_VPOXBOW" "$fatal"

    unload_module "$MOD_VPOXNET" "$DESC_VPOXNET" 0 "$fatal"
    rem_driver "$MOD_VPOXNET" "$DESC_VPOXNET" "$fatal"

    unload_module "$MOD_VPOXDRV" "$DESC_VPOXDRV" 1 "$fatal"
    rem_driver "$MOD_VPOXDRV" "$DESC_VPOXDRV" "$fatal"

    # remove devlinks
    if test -h "$PKG_INSTALL_ROOT/dev/vpoxdrv" || test -f "$PKG_INSTALL_ROOT/dev/vpoxdrv"; then
        rm -f "$PKG_INSTALL_ROOT/dev/vpoxdrv"
    fi
    if test -h "$PKG_INSTALL_ROOT/dev/vpoxdrvu" || test -f "$PKG_INSTALL_ROOT/dev/vpoxdrvu"; then
        rm -f "$PKG_INSTALL_ROOT/dev/vpoxdrvu"
    fi
    if test -h "$PKG_INSTALL_ROOT/dev/vpoxusbmon" || test -f "$PKG_INSTALL_ROOT/dev/vpoxusbmon"; then
        rm -f "$PKG_INSTALL_ROOT/dev/vpoxusbmon"
    fi

    # unpatch nwam/dhcpagent fix
    nwamfile="$PKG_INSTALL_ROOT/etc/nwam/llp"
    nwambackupfile=$nwamfile.vpox
    if test -f "$nwamfile"; then
        sed -e '/vpoxnet/d' $nwamfile > $nwambackupfile
    ...
    nmaskbackupfile=$nmaskfile.vpox
    if test -f "$nmaskfile"; then
        sed -e '/#VirtualPox_SectionStart/,/#VirtualPox_SectionEnd/d' $nmaskfile > $nmaskbackupfile
...
install_python_bindings()
{
    ...
    if test -x "$pythonbin"; then
        ...
        VPOX_INSTALL_PATH="$DIR_VPOXBASE"
        export VPOX_INSTALL_PATH
        cd $DIR_VPOXBASE/sdk/installer
        $pythonbin ./vpoxapisetup.py install > /dev/null
        if test "$?" -eq 0; then
...
# plumb vpoxnet0 instance
# failure: non fatal
plumb_net()
{
    # S11 175a renames vpoxnet0 as 'netX', undo this and rename it back (Solaris 12, Solaris 11.4 or newer)
    if test "$HOST_OS_MAJORVERSION" -ge 12 || (test "$HOST_OS_MAJORVERSION" -eq 11 && test "$HOST_OS_MINORVERSION" -ge 175); then
        vanityname=`dladm show-phys -po link,device | grep vpoxnet0 | cut -f1 -d':'`
        if test "$?" -eq 0 && test ! -z "$vanityname" && test "x$vanityname" != "xvpoxnet0"; then
            dladm rename-link "$vanityname" vpoxnet0
            if test "$?" -ne 0; then
                errorprint "Failed to rename vanity interface ($vanityname) to vpoxnet0"
            fi
        fi
    fi
    ...
    if test "$HOST_OS_MAJORVERSION" -ge 12 || (test "$HOST_OS_MAJORVERSION" -eq 11 && test "$HOST_OS_MINORVERSION" -ge 176); then
        $BIN_IPADM create-ip vpoxnet0
        if test "$?" -eq 0; then
            $BIN_IPADM create-addr -T static -a local="192.168.56.1/24" "vpoxnet0/v4addr"
            if test "$?" -eq 0; then
                subprint "Configured: NetAdapter 'vpoxnet0'"
            else
                warnprint "Failed to create local address for vpoxnet0!"
            fi
        else
            warnprint "Failed to create IP instance for vpoxnet0!"
        fi
    else
        $BIN_IFCONFIG vpoxnet0 plumb
        $BIN_IFCONFIG vpoxnet0 up
        if test "$?" -eq 0; then
            $BIN_IFCONFIG vpoxnet0 192.168.56.1 netmask 255.255.255.0 up
            ...
            if test -f $nmaskfile; then
                ...
                echo "#VirtualPox_SectionStart" >> $nmaskbackupfile
                inst=0
                networkn=56
                ...
                echo "#VirtualPox_SectionEnd" >> $nmaskbackupfile
                mv -f $nmaskbackupfile $nmaskfile
                ...
                if test "$recreatelink" -eq 1; then
                    ...
                    warnprint "VirtualPox installers incorrectly overwrote. Now the contents"
                    warnprint "of /etc/netmasks and /etc/inet/netmasks differ, therefore "
                    warnprint "VirtualPox will not attempt to overwrite /etc/netmasks as a"
            ...
            warnprint "Failed to bring up vpoxnet0!"
        fi
    fi
}


# unplumb all vpoxnet instances
# failure: fatal
unplumb_net()
{
    inst=0
    # use ipadm for Solaris 12, Solaris 11.5 or newer
    if test "$HOST_OS_MAJORVERSION" -ge 12 || (test "$HOST_OS_MAJORVERSION" -eq 11 && test "$HOST_OS_MINORVERSION" -ge 176); then
        while test "$inst" -ne $MOD_VPOXNET_INST; do
            vpoxnetup=`$BIN_IPADM show-addr -p -o addrobj vpoxnet$inst >/dev/null 2>&1`
            if test "$?" -eq 0; then
                $BIN_IPADM delete-addr vpoxnet$inst/v4addr
                $BIN_IPADM delete-ip vpoxnet$inst
                if test "$?" -ne 0; then
                    errorprint "VirtualPox NetAdapter 'vpoxnet$inst' couldn't be removed (probably in use)."
                    if test "x$fatal" = "x$FATALOP"; then
                        exit 1
                    fi
                fi
        ...
        inst=0
        while test "$inst" -ne $MOD_VPOXNET_INST; do
            vpoxnetup=`$BIN_IFCONFIG vpoxnet$inst >/dev/null 2>&1`
            if test "$?" -eq 0; then
                $BIN_IFCONFIG vpoxnet$inst unplumb
                if test "$?" -ne 0; then
                    errorprint "VirtualPox NetAdapter 'vpoxnet$inst' couldn't be unplumbed (probably in use)."
                    if test "x$fatal" = "x$FATALOP"; then
                        exit 1
                    fi
                fi
            ...
            # unplumb vpoxnet0 ipv6
            vpoxnetup=`$BIN_IFCONFIG vpoxnet$inst inet6 >/dev/null 2>&1`
            if test "$?" -eq 0; then
                $BIN_IFCONFIG vpoxnet$inst inet6 unplumb
                if test "$?" -ne 0; then
                    errorprint "VirtualPox NetAdapter 'vpoxnet$inst' IPv6 couldn't be unplumbed (probably in use)."
                    if test "x$fatal" = "x$FATALOP"; then
                        exit 1
                    fi
                fi
...
cleanup_install()
{
    ...
    # stop the services
    stop_service "Web service" "virtualpox/webservice" "svc:/application/virtualpox/webservice:default"
    stop_service "Balloon control service" "virtualpox/balloonctrl" "svc:/application/virtualpox/balloonctrl:default"
    stop_service "Autostart service" "virtualpox/autostart" "svc:/application/virtualpox/autostart:default"
    stop_service "Zone access service" "virtualpox/zoneaccess" "svc:/application/virtualpox/zoneaccess:default"

    # DEBUG x4600b: verify that the ZoneAccess process is really gone
    is_process_running "VPoxZoneAccess"
    if test "$?" -eq 1; then
        warnprint "VPoxZoneAccess is alive despite its service being dead. Killing..."
        stop_process "VPoxZoneAccess"
    fi

    # unplumb all vpoxnet instances for non-remote installs
    unplumb_net

    # Stop our other daemons, non-fatal
    stop_process "VPoxNetDHCP"
    stop_process "VPoxNetNAT"

    # Stop VPoxSVC quickly using SIGUSR1
    procname="VPoxSVC"
    procpid=`ps -eo pid,fname | grep $procname | grep -v grep | awk '{ print $1 }'`
    if test ! -z "$procpid" && test "$procpid" -ge 0; then
        kill -USR1 $procpid

        # Sleep a while and check if VPoxSVC is still running, if so fail uninstallation.
        sleep 2
        is_process_running "VPoxSVC"
        if test "$?" -eq 1; then
            errorprint "Cannot uninstall VirtualPox while VPoxSVC (pid $procpid) is still running."
            errorprint "Please shutdown all VMs and VirtualPox frontends before uninstalling VirtualPox."
            exit 1
        fi

        # Some VMs might still be alive after VPoxSVC as they poll less frequently before killing themselves
        # Just check for VPoxHeadless & VirtualPox frontends for now.
        is_process_running "VPoxHeadless"
        if test "$?" -eq 1; then
            errorprint "Cannot uninstall VirtualPox while VPoxHeadless is still running."
            errorprint "Please shutdown all VMs and VirtualPox frontends before uninstalling VirtualPox."
            exit 1
        fi

        is_process_running "VirtualPox"
        if test "$?" -eq 1; then
            errorprint "Cannot uninstall VirtualPox while any VM is still running."
            errorprint "Please shutdown all VMs and VirtualPox frontends before uninstalling VirtualPox."
            exit 1
        fi
    fi
...
postinstall()
{
    ...
    # We do this early so that when we invoke services or other VirtualPox processes, the dependent libraries are resolved.
    if test -d "/opt/VirtualPox/legacy/"; then
        if test "$HOST_OS_MAJORVERSION" -eq 10; then
            for lib in `ls -1 /opt/VirtualPox/legacy/`; do
            /usr/sbin/installf -c none $PKGINST /opt/VirtualPox/$lib=legacy/$lib s
            done
            for lib in `ls -1 /opt/VirtualPox/amd64/legacy/`; do
                /usr/sbin/installf -c none $PKGINST /opt/VirtualPox/amd64/$lib=legacy/$lib s
            done
        fi
    fi

    infoprint "Loading VirtualPox kernel modules..."
    install_drivers
    ...
    if test "$?" -eq 0; then
        if test -f "$DIR_CONF/vpoxnet.conf"; then
            ...
            if test -f "$nwamfile"; then
                ...
                # add all vpoxnet instances as static to nwam
                inst=0
                networkn=56
                while test "$inst" -ne 1; do
                    echo "vpoxnet$inst	static 192.168.$networkn.1" >> $nwambackupfile
                    inst=`expr $inst + 1`
                    networkn=`expr $networkn + 1`
                done
                mv -f $nwambackupfile $nwamfile
            ...
            # plumb and configure vpoxnet0 for non-remote installs
            if test "$REMOTEINST" -eq 0; then
                plumb_net
            fi
        fi

        if     test -f "$PKG_INSTALL_ROOT/var/svc/manifest/application/virtualpox/virtualpox-webservice.xml" \
            || test -f "$PKG_INSTALL_ROOT/var/svc/manifest/application/virtualpox/virtualpox-zoneaccess.xml" \
            || test -f "$PKG_INSTALL_ROOT/var/svc/manifest/application/virtualpox/virtualpox-balloonctrl.xml"\
            || test -f "$PKG_INSTALL_ROOT/var/svc/manifest/application/virtualpox/virtualpox-autostart.xml"; then
            ...
            if test "$REMOTEINST" -eq 1; then
                ...
                start_service "Zone access service" "virtualpox/zoneaccess" "svc:/application/virtualpox/zoneaccess:default" \
                            "/var/svc/log/application-virtualpox-zoneaccess:default.log"
            fi
        fi
        ...
        if test "$REMOTEINST" -eq 0; then
            if test -f "$DIR_VPOXBASE/sdk/installer/vpoxapisetup.py" || test -h "$DIR_VPOXBASE/sdk/installer/vpoxapisetup.py"; then
                ...
                if test -f "$PYTHONBIN" || test -h "$PYTHONBIN"; then
                    ...
                    rm -rf $DIR_VPOXBASE/sdk/installer/build
            ...
            warnprint "Skipped installing Python bindings. Run, as root, 'vpoxapisetup.py install' manually from the booted system."
        fi
...
--setupdrivers)
    remove_drivers "$fatal"
    infoprint "Installing VirtualPox drivers:"
    install_drivers
    ;;
*)
    printusage
