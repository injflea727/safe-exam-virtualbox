/* $Id: MediumIOImpl.cpp $ */
/** @file
 * VirtualPox COM class implementation: MediumIO
 */
...
{
    Data(Medium * const a_pMedium, VirtualPox * const a_pVirtualPox, bool a_fWritable, uint32_t a_cbSector = 512)
        : ptrMedium(a_pMedium)
        , ptrVirtualPox(a_pVirtualPox)
    ...
    /** Reference to the VirtualPox object the medium is part of. */
    ComPtr<VirtualPox>              ptrVirtualPox;
...
public:
          ...
          mVirtualPoxCaller(NULL)
    {
        ...
        VirtualPox *pVirtualPox = pMediumIO->m->ptrVirtualPox;
...
HRESULT MediumIO::StreamTask::executeTask()
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        vrc = VDCopy(mMediumIO->m->pHdd, VD_LAST_IMAGE, pDstDisk, m_strFormat.c_str(),
                     ...
                     pIfsImg, NULL);
        if (RT_FAILURE(vrc))
            hrc = mMediumIO->setErrorBoth(VPOX_E_FILE_ERROR, vrc,
 ...
 * @param   pVirtualPox     Pointer to the VirtualPox object the medium is part of.
 ...
 *
 */
HRESULT MediumIO::initForMedium(Medium *pMedium, VirtualPox *pVirtualPox, bool fWritable,
                                com::Utf8Str const &rStrKeyId, com::Utf8Str const &rStrPassword)
{
    LogFlowThisFunc(("pMedium=%p pVirtualPox=%p fWritable=%RTbool\n", pMedium, pVirtualPox, fWritable));
     ...
     */
    HRESULT hrc = S_OK;
    m = new(std::nothrow) Data(pMedium, pVirtualPox, fWritable);
    if (m)
    {
        /*
...
HRESULT MediumIO::read(LONG64 a_off, ULONG a_cbRead, std::vector<BYTE> &a_rData)
{
    ...
    {
        a_rData.resize(0);
        hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Error reading %u bytes at %RU64: %Rrc"), a_cbRead, a_off, vrc);
    }
    ...
    *a_pcbWritten = 0;
    ...
    if (RT_SUCCESS(vrc))
    {
        *a_pcbWritten = (ULONG)cbActual;
        ...
        hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Error writing %zu bytes at %RU64: %Rrc"), cbToWrite, a_off, vrc);
...
HRESULT MediumIO::formatFAT(BOOL a_fQuick)
{
    ...
    if (RT_SUCCESS(vrc))
        ...
        hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Error formatting (%Rrc): %s"), vrc, ErrInfo.Core.pszMsg);
    else
        hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Error formatting: %Rrc"), vrc);
...
HRESULT MediumIO::initializePartitionTable(PartitionTableType_T a_enmFormat, BOOL a_fWholeDiskInOneEntry)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("RTDvmMapInitialize failed: %Rrc"), vrc);
        RTDvmRelease(hVolMgr);
    }
    else
        hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("RTDvmCreate failed: %Rrc"), vrc);
                                  ...
                                  ComPtr<IProgress> &aProgress)
{
    ...
    try
    {
        ...
        rc = pProgress->init(m->ptrVirtualPox,
