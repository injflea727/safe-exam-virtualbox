/* $Id: VPoxGuestInst.cpp $ */
/** @file
 * Small tool to (un)install the VPoxGuest device driver.
 */
...
#include <VPox/VPoxGuest.h> /* for VPOXGUEST_SERVICE_NAME */
#include <iprt/err.h>
...
static int installDriver(bool fStartIt)
{
    ...
    const char    *pszSlashName = "\\VPoxGuest.sys";
    ...
    SC_HANDLE hService = CreateService(hSMgrCreate,
                                       VPOXGUEST_SERVICE_NAME,
                                       "VPoxGuest Support Driver",
...
static int uninstallDriver(void)
{
    ...
    SC_HANDLE hService = OpenService(hSMgr, VPOXGUEST_SERVICE_NAME, SERVICE_STOP | SERVICE_QUERY_STATUS | DELETE);
    if (hService)
    {
        /*
...
static HANDLE openDriver(void)
{
    ...
    hDevice = CreateFile(VPOXGUEST_DEVICE_NAME, // Win2k+: VPOXGUEST_DEVICE_NAME_GLOBAL
