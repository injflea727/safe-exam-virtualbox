/* $Id: VPoxSF-VNodeOps.cpp $ */
/** @file
 * VPoxSF - Darwin Shared Folders, VNode Operations.
 */
 ...
 */
static int vpoxSfDwnVnDefaultError(struct default_error_args_hack *pArgs)
{
    Log(("vpoxSfDwnVnDefaultError: %s\n", RT_VALID_PTR(pArgs) && RT_VALID_PTR(pArgs->a_desc) ? pArgs->a_desc->vdesc_name : "??"));
    RT_NOREF(pArgs);
    return ENOTSUP;
}


static int vpoxFsDwnVnGetAttr(struct vnop_getattr_args *pArgs)
{
#if 1
    RT_NOREF(pArgs);
    return ENOTSUP;
    ...
    vpoxvfs_mount_t   *pMount;
    struct vnode_attr *vnode_args;
    vpoxvfs_vnode_t   *pVnodeData;
    ...
    pMount      = (vpoxvfs_mount_t *)vfs_fsprivate(mp);     AssertReturn(pMount, EINVAL);
    pVnodeData           = (vpoxvfs_vnode_t *)vnode_fsnode(vnode);  AssertReturn(pVnodeData,      EINVAL);

    lck_rw_lock_shared(pVnodeData->pLock);

    rc = vpoxvfs_get_info_internal(mp, pVnodeData->pPath, &Info);
    if (rc == 0)
    {
        /* Set timestamps */
        ...
        VATTR_RETURN(vnode_args, va_mode,  vpoxvfs_h2g_mode_inernal(Info.Attr.fMode));
    ...
    {
        PDEBUG("getattr: unable to get VPoxVFS object info");
    }
...
#if 0
/**
 * Helper function for vpoxvfs_vnode_lookup(): create new vnode.
 */
static int
vpoxvfs_vnode_lookup_instantinate_vnode(vnode_t parent_vnode, char *entry_name, vnode_t *result_vnode)
{
    /* We need to construct full path to vnode in order to get
     * vpoxvfs_get_info_internal() to understand us! */
    ...
    if (pszCurDirPath)
    {
        ...
        if (rc == 0 && cbCurDirPath < MAXPATHLEN)
        {
            ...
            rc = vpoxvfs_guest_path_to_shflstring_path_internal(mp, pszCurDirPath, strlen(pszCurDirPath) + 1, &pSHFLPath);
            if (rc == 0)
            {
                rc = vpoxvfs_get_info_internal(mp, pSHFLPath, (PSHFLFSOBJINFO)&Info);
                if (rc == 0)
                {
                    enum vtype type;
                    ...
                    else
                    {
                        ...
                        vpoxvfs_put_path_internal((void **)&pSHFLPath);
                        return ENOENT;
                    }
                    /* Create new vnode */
                    rc = vpoxvfs_create_vnode_internal(mp, type, parent_vnode, FALSE, pSHFLPath, &vnode);
                    if (rc == 0)
                    {
...
/**
 * Helper function for vpoxvfs_vnode_lookup(): take care
 * about '.' and '..' directory entries.
 */
static int
vpoxvfs_vnode_lookup_dot_handler(struct vnop_lookup_args *pArgs, vnode_t *result_vnode)
{
    vnode_t vnode = NULL;
...
static int vpoxSfDwnVnLookup(struct vnop_lookup_args *pArgs)
{
#if 1
    RT_NOREF(pArgs);
    return ENOTSUP;
    ...
    pVnodeData = (vpoxvfs_vnode_t *)vnode_fsnode(pArgs->a_dvp);
    ...
    */
    ...
    if (vpoxvfs_vnode_lookup_dot_handler(pArgs, &vnode) == 0)
    {
        vnode_get(vnode);
        *pArgs->a_vpp = vnode;
    ...
    if (rc == -1) /* Record found */
    {
        ...
        if (vpoxvfs_exist_internal(vnode))
        {
    ...
    else
    {
        ...
        rc = vpoxvfs_vnode_lookup_instantinate_vnode(pArgs->a_dvp, pArgs->a_cnp->cn_nameptr, &vnode);
        if (rc == 0)
        {
...
static int vpoxSfDwnVnOpen(struct vnop_open_args *pArgs)
{
#if 1
    RT_NOREF(pArgs);
    return ENOTSUP;
    ...
    {
        PDEBUG("vnode '%s' (handle 0x%X) already has VPoxVFS object handle assigned, just return ok",
    ...
    /* At this point we must make sure that nobody is using VPoxVFS object handle */
    //if (pVnodeData->Handle != SHFL_HANDLE_NIL)
    //{
    //    PDEBUG("vnode has active VPoxVFS object handle set, aborting");
    //    lck_rw_unlock_exclusive(pVnodeData->pLock);
    //    return EINVAL;
    //}

    fHostFlags  = vpoxvfs_g2h_mode_inernal(pArgs->a_mode);
    fHostFlags |= (vnode_isdir(vnode) ? SHFL_CF_DIRECTORY : 0);

    SHFLHANDLE Handle;
    rc = vpoxvfs_open_internal(pMount, pVnodeData->pPath, fHostFlags, &Handle);
    if (rc == 0)
    {
...
static int vpoxSfDwnVnClose(struct vnop_close_args *pArgs)
{
#if 1
    RT_NOREF(pArgs);
    return ENOTSUP;
    ...
    /* At this point we must make sure that vnode has VPoxVFS object handle assigned */
    if (pVnodeData->pHandle == SHFL_HANDLE_NIL)
    {
        PDEBUG("vnode has no active VPoxVFS object handle set, aborting");
        lck_rw_unlock_exclusive(pVnodeData->pLock);
        return EINVAL;
    }

    rc = vpoxvfs_close_internal(pMount, pVnodeData->pHandle);
    if (rc == 0)
    {
        ...
        /* Forget about previously assigned VPoxVFS object handle */
 ...
 */
static int
vpoxvfs_vnode_readdir_copy_data(ino_t index, SHFLDIRINFO *Info, struct uio *uio, int *numdirent)
{
    struct dirent entry;
...
static int vpoxSfDwnVnReadDir(struct vnop_readdir_args *pArgs)
{
#if 1
    RT_NOREF(pArgs);
    return ENOTSUP;
    ...
    PDEBUG("Exploring VPoxVFS directory (%s), handle (0x%.8X), offset (0x%X), count (%d)", (char *)pVnodeData->pPath->String.utf8, (int)pVnodeData->pHandle, index, uio_iovcnt(uio));

    /* Currently, there is a problem when VbglR0SfDirInfo() is not able to
     * continue retrieve directory content if the same VPoxVFS handle is used.
    ...
    SHFLHANDLE Handle;
    rc = vpoxvfs_open_internal(pMount,
                               pVnodeData->pPath,
...
#if 0
    rc = VbglR0SfDirInfo(&g_vpoxSFClient, &pMount->pMap, Handle, 0, 0, index, &cbInfo, (PSHFLDIRINFO)Info, &cFiles);
#else
    SHFLSTRING *pMask = vpoxvfs_construct_shflstring("*", strlen("*"));
    if (pMask)
    {
        for (uint32_t cSkip = 0; (cSkip < index + 1) && (rc == VINF_SUCCESS); cSkip++)
        {
            //rc = VbglR0SfDirInfo(&g_vpoxSFClient, &pMount->pMap, Handle, 0 /* pMask */, 0 /* SHFL_LIST_RETURN_ONE */, 0, &cbInfo, (PSHFLDIRINFO)Info, &cFiles);

            uint32_t cbReturned = cbInfo;
            //rc = VbglR0SfDirInfo(&g_vpoxSFClient, &pMount->pMap, Handle, pMask, SHFL_LIST_RETURN_ONE, 0, &cbReturned, (PSHFLDIRINFO)Info, &cFiles);
...
#endif
    rc2 = vpoxvfs_close_internal(pMount, Handle);
    if (rc2 != 0)
    {
    ...
    switch (rc)
    {
        case VINF_SUCCESS:
        {
            rc = vpoxvfs_vnode_readdir_copy_data((ino_t)(index + 1), Info, uio, pArgs->a_numdirent);
            break;
        }
...
static int vpoxSfDwnVnPathConf(struct vnop_pathconf_args *pArgs)
{
    Log(("vpoxSfDwnVnPathConf:\n"));
    RT_NOREF(pArgs);
    return 0;
}
...
vnode_t vpoxSfDwnVnAlloc(mount_t pMount, enum vtype enmType, vnode_t pParent, uint64_t cbFile)
{
     ...
     */
    PVPOXSFDWNVNDATA pVnData = (PVPOXSFDWNVNDATA)RTMemAllocZ(sizeof(*pVnData));
    if (pVnData)
    {
        pVnData->hHandle = SHFL_HANDLE_NIL;
        ...
        VnParms.vnfs_str        = "vpoxsf";
        VnParms.vnfs_dvp        = pParent;
        VnParms.vnfs_fsnode     = pVnData;
        VnParms.vnfs_vops       = g_papfnVPoxSfDwnVnDirOpsVector;
    ...
    printf("vpoxSfDwnVnAlloc: out of memory!\n");
    return NULL;
}
 ...
 * Vnode operations.
 */
static struct vnodeopv_entry_desc g_VPoxSfDirOpsDescList[] =
{
#define VNODEOPFUNC int(*)(void *)
    { &vnop_default_desc,     (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_access_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, - probably not needed.
    //{ &vnop_advlock_desc,     (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, - later.
    //{ &vnop_allocate_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, - maybe, need shfl function
    { &vnop_blktooff_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_blockmap_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_bwrite_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_close_desc,       (VNODEOPFUNC)vpoxSfDwnVnClose },
    //{ &vnop_copyfile_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_create_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_exchange_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_fsync_desc,       (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_getattr_desc,     (VNODEOPFUNC)vpoxFsDwnVnGetAttr },
    //{ &vnop_getnamedstream_desc, (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_getxattr_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_inactive_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_ioctl_desc,       (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_link_desc,        (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_listxattr_desc,   (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_lookup_desc,      (VNODEOPFUNC)vpoxSfDwnVnLookup },
    { &vnop_mkdir_desc,       (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_mknod_desc,       (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_mmap_desc,        (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_mnomap_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_offtoblk_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_open_desc,        (VNODEOPFUNC)vpoxSfDwnVnOpen },
    { &vnop_pagein_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_pageout_desc,     (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_pathconf_desc,    (VNODEOPFUNC)vpoxSfDwnVnPathConf },
    /* { &vnop_print_desc,       (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, undefined in ML */
    { &vnop_read_desc,        (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_readdir_desc,     (VNODEOPFUNC)vpoxSfDwnVnReadDir },
    //{ &vnop_readdirattr_desc, (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, - hfs specific.
    { &vnop_readlink_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_reclaim_desc,     (VNODEOPFUNC)vpoxSfDwnVnReclaim },
    { &vnop_remove_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_removexattr_desc, (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_rename_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_revoke_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, - not needed
    { &vnop_rmdir_desc,       (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_searchfs_desc,    (VNODEOPFUNC)err_searchfs },
    //{ &vnop_select_desc,      (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, - not needed
    { &vnop_setattr_desc,     (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { &vnop_setxattr_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    //{ &vnop_strategy_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, - not needed
    { &vnop_symlink_desc,     (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    /* { &vnop_truncate_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, undefined in ML */
    //{ &vnop_whiteout_desc,    (VNODEOPFUNC)vpoxSfDwnVnDefaultError }, - not needed/supported
    { &vnop_write_desc,       (VNODEOPFUNC)vpoxSfDwnVnDefaultError },
    { NULL,                   (VNODEOPFUNC)NULL              },
#undef VNODEOPFUNC
};

/** ??? */
int (**g_papfnVPoxSfDwnVnDirOpsVector)(void *);

/**
 * VNode operation descriptors.
 */
struct vnodeopv_desc g_VPoxSfVnodeOpvDesc =
{
    &g_papfnVPoxSfDwnVnDirOpsVector,
    g_VPoxSfDirOpsDescList
};
