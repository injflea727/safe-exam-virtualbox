...
/** @file
 * VirtualPox Driver Interface to VMM device.
 */
...
#ifdef RT_OS_OS2
# define VPOXSHAREDFOLDERS_DLL "VPoxSFld"
#else
# define VPOXSHAREDFOLDERS_DLL "VPoxSharedFolders"
#endif
                                           ...
                                           uint32_t fFlags, PCRTTIMESPEC pTimeSpecTS)
{
    ...
    guest->i_setAdditionsStatus((VPoxGuestFacilityType)uFacility, (VPoxGuestFacilityStatus)uStatus, fFlags, pTimeSpecTS);
                                              ...
                                              const uint8_t *pabDetails, uint32_t cbDetails)
{
    ...
    pGuest->i_onUserStateChange(Bstr(pszUser), Bstr(pszDomain), (VPoxGuestUserState)uState,
 ...
 */
DECLCALLBACK(void) vmmdevUpdateGuestInfo(PPDMIVMMDEVCONNECTOR pInterface, const VPoxGuestInfo *guestInfo)
{
    ...
    else
    {
        ...
        guest->i_setAdditionsStatus(VPoxGuestFacilityType_All, VPoxGuestFacilityStatus_Inactive, 0 /*fFlags*/, &TimeSpecTS);
        pConsole->i_onAdditionsStateChange();
    }
}
 ...
 * @thread  The emulation thread.
 */
DECLCALLBACK(int) vmmdevReportStatistics(PPDMIVMMDEVCONNECTOR pInterface, VPoxGuestStatistics *pGuestStats)
{
    PDRVMAINVMMDEV pDrv = RT_FROM_MEMBER(pInterface, DRVMAINVMMDEV, Connector);
    Console *pConsole = pDrv->pVMMDev->getParent();
...
static DECLCALLBACK(int) iface_hgcmConnect(PPDMIHGCMCONNECTOR pInterface, PVPOXHGCMCMD pCmd,
...
static DECLCALLBACK(int) iface_hgcmDisconnect(PPDMIHGCMCONNECTOR pInterface, PVPOXHGCMCMD pCmd, uint32_t u32ClientID)
{
    Log9(("Enter\n"));
...
static DECLCALLBACK(int) iface_hgcmCall(PPDMIHGCMCONNECTOR pInterface, PVPOXHGCMCMD pCmd, uint32_t u32ClientID,
                                        uint32_t u32Function, uint32_t cParms, PVPOXHGCMSVCPARM paParms, uint64_t tsArrival)
{
    Log9(("Enter\n"));
...
static DECLCALLBACK(void) iface_hgcmCancelled(PPDMIHGCMCONNECTOR pInterface, PVPOXHGCMCMD pCmd, uint32_t idClient)
{
    Log9(("Enter\n"));
...
int VMMDev::hgcmLoadService(const char *pszServiceLibrary, const char *pszServiceName)
{
    ...
    Assert(   (mpDrv && mpDrv->pHGCMPort)
           || !strcmp(pszServiceLibrary, "VPoxHostChannel")
           || !strcmp(pszServiceLibrary, "VPoxSharedClipboard")
           || !strcmp(pszServiceLibrary, "VPoxDragAndDropSvc")
           || !strcmp(pszServiceLibrary, "VPoxGuestPropSvc")
           || !strcmp(pszServiceLibrary, "VPoxSharedCrOpenGL")
           );
...
int VMMDev::hgcmHostCall(const char *pszServiceName, uint32_t u32Function,
                         uint32_t cParms, PVPOXHGCMSVCPARM paParms)
{
    if (!hgcmIsActive())
        return VERR_INVALID_STATE;
...
void VMMDev::i_guestPropSet(const char *pszName, const char *pszValue, const char *pszFlags)
{
    ...
    hgcmHostCall("VPoxGuestPropSvc", GUEST_PROP_FN_HOST_SET_PROP, 3, &parms[0]);
}
...
int VMMDev::i_guestPropSetGlobalPropertyFlags(uint32_t fFlags)
{
    ...
    int rc = hgcmHostCall("VPoxGuestPropSvc", GUEST_PROP_FN_HOST_SET_GLOBAL_FLAGS, 1, &parm);
    if (RT_FAILURE(rc))
    {
...
int VMMDev::i_guestPropLoadAndConfigure()
{
     ...
     * Load the service
     */
    int rc = hgcmLoadService("VPoxGuestPropSvc", "VPoxGuestPropSvc");
    if (RT_FAILURE(rc))
    {
        LogRel(("VPoxGuestPropSvc is not available. rc = %Rrc\n", rc));
     ...
     */
    HGCMHostRegisterServiceExtension(&mpDrv->hHgcmSvcExtGstProps, "VPoxGuestPropSvc", Console::i_doGuestPropNotification, ptrConsole.m_p);
...
# endif
    ...
    Log(("Set VPoxGuestPropSvc property store\n"));
    return VINF_SUCCESS;
}
...
DECLCALLBACK(int) VMMDev::drvConstruct(PPDMDRVINS pDrvIns, PCFGMNODE pCfgHandle, uint32_t fFlags)
{
     ...
     */
    rc = pThis->pVMMDev->hgcmLoadService(VPOXSHAREDFOLDERS_DLL, "VPoxSharedFolders");
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc) && pLed)
        {
            ...
            parm.type = VPOX_HGCM_SVC_PARM_PTR;
            parm.u.pointer.addr = pLed;
            parm.u.pointer.size = sizeof(*pLed);

            rc = HGCMHostCall("VPoxSharedFolders", SHFL_FN_SET_STATUS_LED, 1, &parm);
        }
        else
     ...
     */
# ifdef VPOX_WITH_GUEST_CONTROL
    rc = pThis->pVMMDev->hgcmLoadService("VPoxGuestControlSvc", "VPoxGuestControlSvc");
    if (RT_SUCCESS(rc))
    {
        rc = HGCMHostRegisterServiceExtension(&pThis->hHgcmSvcExtGstCtrl, "VPoxGuestControlSvc",
                                              &Guest::i_notifyCtrlDispatcher,
                                              pThis->pVMMDev->mParent->i_getGuest());
        if (RT_SUCCESS(rc))
            LogRel(("Guest Control service loaded\n"));
        else
            LogRel(("Warning: Cannot register VPoxGuestControlSvc extension! rc=%Rrc\n", rc));
    }
    else
        LogRel(("Warning!: Failed to load the Guest Control Service! %Rrc\n", rc));
# endif /* VPOX_WITH_GUEST_CONTROL */
