...
/**
 * @interface_method_impl{DBGCCMDHLP,pfnVPoxErrorV}
 */
static DECLCALLBACK(int) dbgcHlpVPoxErrorV(PDBGCCMDHLP pCmdHlp, int rc, const char *pszFormat, va_list args)
{
    switch (rc)
    {
        case VINF_SUCCESS:
...
/**
 * @interface_method_impl{DBGCCMDHLP,pfnVPoxError}
 */
static DECLCALLBACK(int) dbgcHlpVPoxError(PDBGCCMDHLP pCmdHlp, int rc, const char *pszFormat, ...)
{
    va_list args;
    va_start(args, pszFormat);
    int rcRet = pCmdHlp->pfnVPoxErrorV(pCmdHlp, rc, pszFormat, args);
    va_end(args);
    return rcRet;
}
...
void dbgcInitCmdHlp(PDBGC pDbgc)
{
    ...
    pDbgc->CmdHlp.pfnVPoxErrorV         = dbgcHlpVPoxErrorV;
    pDbgc->CmdHlp.pfnVPoxError          = dbgcHlpVPoxError;
