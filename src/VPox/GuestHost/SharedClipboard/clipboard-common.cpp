...
int ShClEventSourceCreate(PSHCLEVENTSOURCE pSource, SHCLEVENTSOURCEID uID)
{
    ...
    pSource->idNextEvent  = RTRandU32Ex(1, VPOX_SHCL_MAX_EVENTS - 1);
...
SHCLEVENTID ShClEventIdGenerateAndRegister(PSHCLEVENTSOURCE pSource)
{
    ...
    for (uint32_t cTries = 0;; cTries++)
    {
        ...
        if (idEvent < VPOX_SHCL_MAX_EVENTS)
        { /* likely */ }
        else
                                ...
                                PRTUTF16 *ppwszDst, size_t *pcwDst)
{
    ...
    {
        if (pcszSrc[i] == VPOX_SHCL_LINEFEED)
            cwDst += 2; /* Space for VPOX_SHCL_CARRIAGERETURN + VPOX_SHCL_LINEFEED. */
        else
            ++cwDst;
    }
    ...
    if (RT_SUCCESS(rc))
    {
        for (unsigned i = 0, j = 0; i < cbSrc; ++i, ++j)
        {
            ...
            {
                pwszDst[j]     = VPOX_SHCL_CARRIAGERETURN;
                pwszDst[j + 1] = VPOX_SHCL_LINEFEED;
                ++j;
            }
        }
...
int ShClUtf16LFLenUtf8(PCRTUTF16 pcwszSrc, size_t cwSrc, size_t *pchLen)
{
    ...
    AssertMsgReturn(pcwszSrc[0] != VPOX_SHCL_UTF16BEMARKER,
                    ("Big endian UTF-16 not supported yet\n"), VERR_NOT_SUPPORTED);

    size_t cLen = 0;

    /* Don't copy the endian marker. */
    size_t i = pcwszSrc[0] == VPOX_SHCL_UTF16LEMARKER ? 1 : 0;
    ...
    for (; i < cwSrc; ++i, ++cLen)
    {
        ...
        if (pcwszSrc[i] == VPOX_SHCL_LINEFEED)
            ++cLen;
#ifdef RT_OS_DARWIN
        /* Check for a single carriage return (MacOS) */
        if (pcwszSrc[i] == VPOX_SHCL_CARRIAGERETURN)
            ++cLen;
#endif
        if (pcwszSrc[i] == 0)
        {
...
int ShClUtf16CRLFLenUtf8(PCRTUTF16 pcwszSrc, size_t cwSrc, size_t *pchLen)
{
    ...
    /* Calculate the size of the destination text string. */
    /* Is this Utf16 or Utf16-LE? */
    if (pcwszSrc[0] == VPOX_SHCL_UTF16LEMARKER)
        cLen = 0;
    else
        cLen = 1;
    ...
    {
        if (   (i + 1 < cwSrc)
            && (pcwszSrc[i]     == VPOX_SHCL_CARRIAGERETURN)
            && (pcwszSrc[i + 1] == VPOX_SHCL_LINEFEED))
        {
            ++i;
        }
        if (pcwszSrc[i] == 0)
...
int ShClConvUtf16LFToCRLF(PCRTUTF16 pcwszSrc, size_t cwcSrc, PRTUTF16 pu16Dst, size_t cwDst)
{
    ...
    for (; i < cwcSrc; ++i, ++j)
    {
        ...
        if (pcwszSrc[i] == VPOX_SHCL_LINEFEED)
        {
            pu16Dst[j] = VPOX_SHCL_CARRIAGERETURN;
            ++j;
...
#ifdef RT_OS_DARWIN
        /* Check for a single carriage return (MacOS) */
        else if (pcwszSrc[i] == VPOX_SHCL_CARRIAGERETURN)
        {
            /* Set CR.r */
            pu16Dst[j] = VPOX_SHCL_CARRIAGERETURN;
            ++j;
...
int ShClConvUtf16CRLFToLF(PCRTUTF16 pcwszSrc, size_t cwcSrc, PRTUTF16 pu16Dst, size_t cwDst)
{
    ...
    {
        pu16Dst[0] = VPOX_SHCL_UTF16LEMARKER;
        cwDstPos = 1;
    }
...
{
    if (uFormat & VPOX_SHCL_FMT_UNICODETEXT)
    {
        LogFunc(("VPOX_SHCL_FMT_UNICODETEXT:\n"));
        if (pv && cb)
            LogFunc(("%ls\n", pv));
        else
            LogFunc(("%p %zu\n", pv, cb));
    }
    else if (uFormat & VPOX_SHCL_FMT_BITMAP)
        LogFunc(("VPOX_SHCL_FMT_BITMAP\n"));
    else if (uFormat & VPOX_SHCL_FMT_HTML)
    {
        LogFunc(("VPOX_SHCL_FMT_HTML:\n"));
        if (pv && cb)
        {
            LogFunc(("%s\n", pv));
...
const char *ShClHostFunctionToStr(uint32_t uFn)
{
    switch (uFn)
    {
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_SET_MODE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_SET_TRANSFER_MODE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_SET_HEADLESS);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_CANCEL);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_ERROR);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_AREA_REGISTER);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_AREA_UNREGISTER);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_AREA_ATTACH);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_FN_AREA_DETACH);
    }
    return "Unknown";
}
...
const char *ShClHostMsgToStr(uint32_t uMsg)
{
    switch (uMsg)
    {
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_QUIT);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_READ_DATA);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_FORMATS_REPORT);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_CANCELED);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_READ_DATA_CID);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_STATUS);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_HDR_READ);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_HDR_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_ENTRY_READ);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_ENTRY_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_OPEN);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_CLOSE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_HDR_READ);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_HDR_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_ENTRY_READ);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_ENTRY_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_OPEN);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_CLOSE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_READ);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_CANCEL);
        RT_CASE_RET_STR(VPOX_SHCL_HOST_MSG_TRANSFER_ERROR);
    }
    return "Unknown";
}
...
const char *ShClGuestMsgToStr(uint32_t uMsg)
{
    switch (uMsg)
    {
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_REPORT_FORMATS);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_DATA_READ);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_DATA_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_CONNECT);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_REPORT_FEATURES);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_QUERY_FEATURES);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_MSG_PEEK_NOWAIT);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_MSG_PEEK_WAIT);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_MSG_GET);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_MSG_CANCEL);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_REPLY);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_ROOT_LIST_HDR_READ);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_ROOT_LIST_HDR_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_ROOT_LIST_ENTRY_READ);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_ROOT_LIST_ENTRY_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_LIST_OPEN);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_LIST_CLOSE);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_LIST_HDR_READ);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_LIST_HDR_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_LIST_ENTRY_READ);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_LIST_ENTRY_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_OBJ_OPEN);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_OBJ_CLOSE);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_OBJ_READ);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_OBJ_WRITE);
        RT_CASE_RET_STR(VPOX_SHCL_GUEST_FN_ERROR);
    }
    return "Unknown";
}
