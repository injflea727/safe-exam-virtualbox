/* $Id: VPoxSharedClipboardSvc-transfers.cpp $ */
/** @file
...
#include "VPoxSharedClipboardSvc-transfers.h"
...
extern PVPOXHGCMSVCHELPERS  g_pHelpers;
...
*   Prototypes                                                                                                                   *
*********************************************************************************************************************************/
static int shClSvcTransferSetListOpen(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
                                      uint64_t idCtx, PSHCLLISTOPENPARMS pOpenParms);
static int shClSvcTransferSetListClose(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
...
DECLCALLBACK(int) shClSvcTransferIfaceGetRoots(PSHCLPROVIDERCTX pCtx, PSHCLROOTLIST *ppRootList)
{
    ...
    PSHCLCLIENTMSG pMsgHdr = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_HDR_READ,
                                             VPOX_SHCL_CPARMS_ROOT_LIST_HDR_READ_REQ);
    if (pMsgHdr)
    {
        SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
        {
            HGCMSvcSetU64(&pMsgHdr->aParms[0], VPOX_SHCL_CONTEXTID_MAKE(pClient->State.uSessionID,
                    ...
                    if (pRootList)
                    {
                        if (pSrcRootListHdr->cRoots)
                        {
                            ...
                            if (pRootList->paEntries)
                            {
                                ...
                                {
                                    PSHCLCLIENTMSG pMsgEntry = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_ENTRY_READ,
                                                                               VPOX_SHCL_CPARMS_ROOT_LIST_ENTRY_READ_REQ);

                                    idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
                                    if (idEvent != NIL_SHCLEVENTID)
                                    {
                                        HGCMSvcSetU64(&pMsgEntry->aParms[0],
                                                      VPOX_SHCL_CONTEXTID_MAKE(pClient->State.uClientID,
                                                                               pCtx->pTransfer->State.uID, idEvent));
                                               ...
                                               PSHCLLISTOPENPARMS pOpenParms, PSHCLLISTHANDLE phList)
{
    ...
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_LIST_OPEN,
                                          VPOX_SHCL_CPARMS_LIST_OPEN);
    if (pMsg)
    {
        const SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
        {
            pMsg->idCtx = VPOX_SHCL_CONTEXTID_MAKE(pClient->State.uSessionID, pCtx->pTransfer->State.uID,
                                                   idEvent);
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        Assert(pReply->uType == VPOX_SHCL_REPLYMSGTYPE_LIST_OPEN);
...
DECLCALLBACK(int) shClSvcTransferIfaceListClose(PSHCLPROVIDERCTX pCtx, SHCLLISTHANDLE hList)
{
    ...
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_LIST_CLOSE,
                                          VPOX_SHCL_CPARMS_LIST_CLOSE);
    if (pMsg)
    {
        const SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
                                                  ...
                                                  SHCLLISTHANDLE hList, PSHCLLISTHDR pListHdr)
{
    ...
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_LIST_HDR_READ,
                                          VPOX_SHCL_CPARMS_LIST_HDR_READ_REQ);
    if (pMsg)
    {
        const SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
        {
            HGCMSvcSetU64(&pMsg->aParms[0], VPOX_SHCL_CONTEXTID_MAKE(pClient->State.uSessionID,
                                                                     pCtx->pTransfer->State.uID, idEvent));
                                                    ...
                                                    SHCLLISTHANDLE hList, PSHCLLISTENTRY pListEntry)
{
    ...
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_LIST_ENTRY_READ,
                                          VPOX_SHCL_CPARMS_LIST_ENTRY_READ);
    if (pMsg)
    {
        const SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
                                ...
                                PSHCLOBJHANDLE phObj)
{
    ...
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_OPEN,
                                          VPOX_SHCL_CPARMS_OBJ_OPEN);
    if (pMsg)
    {
        const SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    Assert(pReply->uType == VPOX_SHCL_REPLYMSGTYPE_OBJ_OPEN);
...
int shClSvcTransferIfaceObjClose(PSHCLPROVIDERCTX pCtx, SHCLOBJHANDLE hObj)
{
    ...
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_CLOSE,
                                          VPOX_SHCL_CPARMS_OBJ_CLOSE);
    if (pMsg)
    {
        const SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    Assert(pReply->uType == VPOX_SHCL_REPLYMSGTYPE_OBJ_CLOSE);
                                ...
                                void *pvData, uint32_t cbData, uint32_t fFlags, uint32_t *pcbRead)
{
    ...
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_READ,
                                          VPOX_SHCL_CPARMS_OBJ_READ_REQ);
    if (pMsg)
    {
        const SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
                                 ...
                                 void *pvData, uint32_t cbData, uint32_t fFlags, uint32_t *pcbWritten)
{
    ...
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_WRITE,
                                          VPOX_SHCL_CPARMS_OBJ_WRITE);
    if (pMsg)
    {
        const SHCLEVENTID idEvent = ShClEventIdGenerateAndRegister(&pCtx->pTransfer->Events);
        if (idEvent != NIL_SHCLEVENTID)
...
{
    const bool fHostToGuest =    uMode == VPOX_SHCL_MODE_HOST_TO_GUEST
                              || uMode == VPOX_SHCL_MODE_BIDIRECTIONAL;

    const bool fGuestToHost =    uMode == VPOX_SHCL_MODE_GUEST_TO_HOST
                              || uMode == VPOX_SHCL_MODE_BIDIRECTIONAL;
    ...
    switch (uMsg)
    {
        case VPOX_SHCL_GUEST_FN_ROOT_LIST_HDR_WRITE:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_ROOT_LIST_ENTRY_WRITE:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_LIST_HDR_WRITE:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_LIST_ENTRY_WRITE:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_OBJ_WRITE:
            fAllowed = fGuestToHost;
            break;

        case VPOX_SHCL_GUEST_FN_ROOT_LIST_HDR_READ:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_ROOT_LIST_ENTRY_READ:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_LIST_HDR_READ:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_LIST_ENTRY_READ:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_OBJ_READ:
            fAllowed = fHostToGuest;
            break;

        case VPOX_SHCL_GUEST_FN_CONNECT:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_NEGOTIATE_CHUNK_SIZE:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_MSG_PEEK_WAIT:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_MSG_PEEK_NOWAIT:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_REPORT_FEATURES:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_QUERY_FEATURES:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_MSG_GET:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_REPLY:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_MSG_CANCEL:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_ERROR:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_LIST_OPEN:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_LIST_CLOSE:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_OBJ_OPEN:
            RT_FALL_THROUGH();
        case VPOX_SHCL_GUEST_FN_OBJ_CLOSE:
 ...
 */
static int shClSvcTransferGetReply(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
                                   PSHCLREPLY pReply)
{
    int rc;

    if (cParms >= VPOX_SHCL_CPARMS_REPLY_MIN)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            switch (pReply->uType)
            {
                case VPOX_SHCL_REPLYMSGTYPE_TRANSFER_STATUS:
                {
                    if (cParms >= 6)
                ...
                case VPOX_SHCL_REPLYMSGTYPE_LIST_OPEN:
                {
                    if (cParms >= 6)
                ...
                case VPOX_SHCL_REPLYMSGTYPE_LIST_CLOSE:
                {
                    if (cParms >= 6)
                ...
                case VPOX_SHCL_REPLYMSGTYPE_OBJ_OPEN:
                {
                    if (cParms >= 6)
                ...
                case VPOX_SHCL_REPLYMSGTYPE_OBJ_CLOSE:
                {
                    if (cParms >= 6)
 ...
 */
static int shClSvcTransferGetRootListHdr(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
                                         PSHCLROOTLISTHDR pRootLstHdr)
{
    int rc;

    if (cParms == VPOX_SHCL_CPARMS_ROOT_LIST_HDR_WRITE)
    {
 ...
 */
static int shClSvcTransferGetRootListEntry(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
                                           PSHCLROOTLISTENTRY pListEntry)
{
    int rc;

    if (cParms == VPOX_SHCL_CPARMS_ROOT_LIST_ENTRY_WRITE)
    {
 ...
 */
static int shClSvcTransferGetListOpen(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
                                      PSHCLLISTOPENPARMS pOpenParms)
{
    int rc;

    if (cParms == VPOX_SHCL_CPARMS_LIST_OPEN)
    {
        uint32_t cbPath   = 0;
        uint32_t cbFilter = 0;
                                       ...
                                       uint64_t idCtx, SHCLLISTHANDLE hList)
{
    ...
    if (cParms == VPOX_SHCL_CPARMS_LIST_CLOSE)
    {
        HGCMSvcSetU64(&aParms[0], idCtx);
 ...
 */
static int shClSvcTransferGetListHdr(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
                                     PSHCLLISTHANDLE phList, PSHCLLISTHDR pListHdr)
{
    int rc;

    if (cParms == VPOX_SHCL_CPARMS_LIST_HDR)
    {
 ...
 */
static int shClSvcTransferSetListHdr(uint32_t cParms, VPOXHGCMSVCPARM aParms[], PSHCLLISTHDR pListHdr)
{
    int rc;
 ...
 */
static int shClSvcTransferGetListEntry(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
                                       PSHCLLISTHANDLE phList, PSHCLLISTENTRY pListEntry)
{
    int rc;

    if (cParms == VPOX_SHCL_CPARMS_LIST_ENTRY)
    {
        rc = HGCMSvcGetU64(&aParms[1], phList);
        if (RT_SUCCESS(rc))
 ...
 */
static int shClSvcTransferSetListEntry(uint32_t cParms, VPOXHGCMSVCPARM aParms[],
 ...
 */
static int shClSvcTransferGetObjDataChunk(uint32_t cParms, VPOXHGCMSVCPARM aParms[], PSHCLOBJDATACHUNK pDataChunk)
{
    ...
    if (cParms == VPOX_SHCL_CPARMS_OBJ_WRITE)
    {
...
/**
 * Handles a guest reply (VPOX_SHCL_GUEST_FN_REPLY) message.
 *
 * @returns VPox status code.
...
static int shClSvcTransferHandleReply(PSHCLCLIENT pClient, PSHCLTRANSFER pTransfer,
                                      uint32_t cParms, VPOXHGCMSVCPARM aParms[])
{
    RT_NOREF(pClient);
            ...
            if (pPayload)
            {
                ...
                switch (pReply->uType)
                {
                    ...
                    case VPOX_SHCL_REPLYMSGTYPE_OBJ_CLOSE:
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            const SHCLEVENTID idEvent = VPOX_SHCL_CONTEXTID_GET_EVENT(uCID);
 ...
 *
 * @returns VPox status code, or VINF_HGCM_ASYNC_EXECUTE if returning to the client will be deferred.
 * @param   pClient             Pointer to associated client.
                           ...
                           uint64_t tsArrival)
{
    ...
    /* A (valid) service extension is needed because VPoxSVC needs to keep track of the
    ...
    switch (u32Function)
    {
        default:
        {
            ...
            const SHCLTRANSFERID uTransferID = VPOX_SHCL_CONTEXTID_GET_TRANSFER(uCID);
    ...
    switch (u32Function)
    {
        ...
        case VPOX_SHCL_GUEST_FN_ROOT_LIST_HDR_READ:
        {
            if (cParms != VPOX_SHCL_CPARMS_ROOT_LIST_HDR_READ)
                break;
        ...
        case VPOX_SHCL_GUEST_FN_ROOT_LIST_ENTRY_READ:
        {
            if (cParms != VPOX_SHCL_CPARMS_ROOT_LIST_ENTRY_READ)
                break;
        ...
        case VPOX_SHCL_GUEST_FN_OBJ_OPEN:
        {
            ASSERT_GUEST_STMT_BREAK(cParms == VPOX_SHCL_CPARMS_OBJ_OPEN, VERR_WRONG_PARAMETER_COUNT);
        ...
        case VPOX_SHCL_GUEST_FN_OBJ_CLOSE:
        {
            if (cParms != VPOX_SHCL_CPARMS_OBJ_CLOSE)
                break;
        ...
        case VPOX_SHCL_GUEST_FN_OBJ_READ:
        {
            if (cParms != VPOX_SHCL_CPARMS_OBJ_READ)
                break;
                               ...
                               VPOXHGCMSVCPARM aParms[])
{
    ...
    switch (u32Function)
    {
        case VPOX_SHCL_HOST_FN_CANCEL: /** @todo Implement this. */
            break;

        case VPOX_SHCL_HOST_FN_ERROR: /** @todo Implement this. */
            break;
...
int shClSvcTransferAreaRegister(PSHCLCLIENTSTATE pClientState, PSHCLTRANSFER pTransfer)
{
    ...
    if (g_ExtState.pfnExtension)
    {
        ...
        rc = g_ExtState.pfnExtension(g_ExtState.pvExtension, VPOX_CLIPBOARD_EXT_FN_AREA_REGISTER, &parms, sizeof(parms));
        if (RT_SUCCESS(rc))
        {
            /* Note: Do *not* specify SHCLAREA_OPEN_FLAGS_MUST_NOT_EXIST as flags here, as VPoxSVC took care of the
...
int shClSvcTransferAreaUnregister(PSHCLCLIENTSTATE pClientState, PSHCLTRANSFER pTransfer)
{
    ...
    if (g_ExtState.pfnExtension)
    {
        ...
        rc = g_ExtState.pfnExtension(g_ExtState.pvExtension, VPOX_CLIPBOARD_EXT_FN_AREA_UNREGISTER, &parms, sizeof(parms));
        if (RT_SUCCESS(rc))
        {
                              ...
                              SHCLAREAID uID)
{
    ...
    if (g_ExtState.pfnExtension)
    {
        ...
        rc = g_ExtState.pfnExtension(g_ExtState.pvExtension, VPOX_CLIPBOARD_EXT_FN_AREA_ATTACH, &parms, sizeof(parms));
        if (RT_SUCCESS(rc))
            rc = pTransfer->pArea->OpenTemp(parms.uID /* Area ID */);
...
int shClSvcTransferAreaDetach(PSHCLCLIENTSTATE pClientState, PSHCLTRANSFER pTransfer)
{
    ...
    if (g_ExtState.pfnExtension)
    {
        ...
        rc = g_ExtState.pfnExtension(g_ExtState.pvExtension, VPOX_CLIPBOARD_EXT_FN_AREA_DETACH, &parms, sizeof(parms));
                              ...
                              int rcTransfer, PSHCLEVENTID pidEvent)
{
    ...
    PSHCLCLIENTMSG pMsgReadData = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_TRANSFER_STATUS,
                                                  VPOX_SHCL_CPARMS_TRANSFER_STATUS);
    if (!pMsgReadData)
        return VERR_NO_MEMORY;
    ...
    if (idEvent != NIL_SHCLEVENTID)
    {
        HGCMSvcSetU64(&pMsgReadData->aParms[0], VPOX_SHCL_CONTEXTID_MAKE(pClient->State.uSessionID,
                            ...
                            if (RT_SUCCESS(rc))
                            {
                                ...
                                if (RT_SUCCESS(rc))
                                {
                                    ...
                                    if (RT_SUCCESS(rc))
                                    {
                                        ...
                                        Assert(pReply->uType == VPOX_SHCL_REPLYMSGTYPE_TRANSFER_STATUS);
