...
//#ifdef DEBUG
//# define VPOX_WINE_DEBUG
//#endif

#ifdef VPOX_WINE_DEBUG
# ifndef VPOX_WINE_DEBUG_DEFINES
extern DWORD g_VPoxVDbgBreakOnD3DErr;
# else
#  ifdef DEBUG_misha
...
#  endif
DWORD g_VPoxVDbgBreakOnD3DErr = _ERR_BREAK_DEFAULT;
# endif

# define _ERR_BREAK() Assert(0)
# define _ERR_ASSERT(_e) Assert((_e))
# define _ERR_CHECK_BREAK(_t) do { \
        if (g_VPoxVDbgBreakOn##_t) { _ERR_BREAK(); } \
    } while (0)
# define _ERR_CHECK_ASSERT(_t, _e) do { \
        if (g_VPoxVDbgBreakOn##_t) { _ERR_ASSERT(_e); } \
    } while (0)
...
#define WINE_DECLARE_DEBUG_CHANNEL(ch) \
    static struct __wine_debug_channel __wine_dbch_##ch = { /* was: ~0, VPox:*/ UINT8_MAX, #ch }
...
# define VPOXWINEDBG_SHADERS
#endif

#if defined(VPOXWINEDBG_SHADERS) || defined(VPOX_WINE_WITH_PROFILE)
#include <stdio.h>
#include <stdarg.h>

void vpoxWDbgPrintF(char * szString, ...);
#endif

#ifdef VPOXWINEDBG_SHADERS
# define WDLOG(_m) do {\
        vpoxWDbgPrintF _m ; \
    } while (0)
#else
