/* $Id: VPoxManageGuestCtrl.cpp $ */
/** @file
 * VPoxManage - Implementation of guestcontrol command.
 */
 ...
 */
VPOX_LISTENER_DECLARE(GuestFileEventListenerImpl)
VPOX_LISTENER_DECLARE(GuestProcessEventListenerImpl)
VPOX_LISTENER_DECLARE(GuestSessionEventListenerImpl)
VPOX_LISTENER_DECLARE(GuestEventListenerImpl)
 ...
 *
 * @todo Make this handler available for all VPoxManage modules?
 */
...
static int gctlPrintError(com::ErrorInfo &errorInfo)
{
    ...
    {
        /* If we got a VPOX_E_IPRT error we handle the error in a more gentle way
         * because it contains more accurate info about what went wrong. */
        if (errorInfo.getResultCode() == VPOX_E_IPRT_ERROR)
...
static RTEXITCODE gctlCtxInitVmSession(PGCTLCMDCTX pCtx)
{
    ...
    CHECK_ERROR(pCtx->pArg->virtualPox, FindMachine(Bstr(pCtx->pszVmNameOrUuid).raw(), machine.asOutParam()));
    if (SUCCEEDED(rc))
    {
...
static RTEXITCODE gctlCtxInitGuestSession(PGCTLCMDCTX pCtx)
{
    ...
    if (RTStrAPrintf(&pszSessionName,
                     "[%RU32] VPoxManage Guest Control [%s] - %s",
 ...
 * process to the command line VPoxManage was started from.  Useful for e.g.
 * scripting.
 *
 ...
 *
 * We ASSUME that all VPoxManage status codes are in the range 0 thru 32.
 *
...
/**
 * Calculates the exit code of VPoxManage.
 *
...
static RTEXITCODE gctlHandleRunCommon(PGCTLCMDCTX pCtx, int argc, char **argv, bool fRunCmd, uint32_t fHelp)
{
    ...
    try
    {
        /* Wait for process start in any case. This is useful for scripting VPoxManage
...
static RTEXITCODE gctlHandleCopy(PGCTLCMDCTX pCtx, int argc, char **argv, bool fHostToGuest)
{
     ...
     * what and how to implement the file enumeration/recursive lookup, like VPoxManage
     * does in here.
     */
    enum GETOPTDEF_COPY
    {
...
static DECLCALLBACK(RTEXITCODE) gctlHandleUpdateAdditions(PGCTLCMDCTX pCtx, int argc, char **argv)
{
    ...
    while (strSource.isEmpty())
    {
        ...
        CHECK_ERROR_BREAK(pCtx->pArg->virtualPox, COMGETTER(SystemProperties)(pProperties.asOutParam()));
        Bstr strISO;
...
static DECLCALLBACK(RTEXITCODE) gctlHandleWatch(PGCTLCMDCTX pCtx, int argc, char **argv)
{
    ...
    try
    {
        ...
        do
        {
            ...
            eventTypes.push_back(VPoxEventType_OnGuestSessionRegistered);
            /** @todo Also register for VPoxEventType_OnGuestUserStateChanged on demand? */
...
RTEXITCODE handleGuestControl(HandlerArg *pArg)
{
    ...
    /*
     * VPoxManage guestcontrol [common-options] <VM> [common-options] <sub-command> ...
     *
