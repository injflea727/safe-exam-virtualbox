...
HRESULT VFSExplorer::init(VFSType_T aType, Utf8Str aFilePath, Utf8Str aHostname, Utf8Str aUsername,
                          Utf8Str aPassword, VirtualPox *aVirtualPox)
{
...
HRESULT VFSExplorer::i_updateFS(TaskVFSExplorer *aTask)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        rc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr ("Can't open directory '%s' (%Rrc)"), m->strPath.c_str(), vrc);
...
HRESULT VFSExplorer::i_deleteFS(TaskVFSExplorer *aTask)
{
    ...
    try
    {
             ...
             ++it, ++i)
        {
            ...
            if (RT_FAILURE(vrc))
                throw setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Can't delete file '%s' (%Rrc)"), szPath, vrc);
            if (aTask->m_ptrProgress)
                            ...
                            ComPtr<IProgress> &aProgress)
{
    ...
    try
    {
        ...
        rc = progress->init(mVirtualPox, static_cast<IVFSExplorer*>(this),
