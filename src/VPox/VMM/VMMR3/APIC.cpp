...
/** VirtualPox 5.1 beta2 - pre fActiveLintX. */
#define APIC_SAVED_STATE_VERSION_VPOX_51_BETA2    4
/** The saved state version used by VirtualPox 5.0 and
 *  earlier.  */
#define APIC_SAVED_STATE_VERSION_VPOX_50          3
/** The saved state version used by VirtualPox v3 and earlier.
 * This does not include the config.  */
#define APIC_SAVED_STATE_VERSION_VPOX_30          2
...
static void apicR3DumpState(PVMCPU pVCpu, const char *pszPrefix, uint32_t uVersion)
{
    ...
    switch (uVersion)
    {
        ...
        case APIC_SAVED_STATE_VERSION_VPOX_51_BETA2:
        {
            /* The auxiliary state. */
        ...
        case APIC_SAVED_STATE_VERSION_VPOX_50:
        case APIC_SAVED_STATE_VERSION_VPOX_30:
...
{
    AssertReturn(uVersion <= APIC_SAVED_STATE_VERSION_VPOX_50, VERR_NOT_SUPPORTED);
...
static DECLCALLBACK(int) apicR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (   uVersion != APIC_SAVED_STATE_VERSION
        && uVersion != APIC_SAVED_STATE_VERSION_VPOX_51_BETA2
        && uVersion != APIC_SAVED_STATE_VERSION_VPOX_50
        && uVersion != APIC_SAVED_STATE_VERSION_VPOX_30
    ...
    int rc = VINF_SUCCESS;
    if (uVersion > APIC_SAVED_STATE_VERSION_VPOX_30)
    {
    ...
    for (VMCPUID idCpu = 0; idCpu < pVM->cCpus; idCpu++)
    {
        ...
        if (uVersion > APIC_SAVED_STATE_VERSION_VPOX_50)
        {
            ...
            if (uVersion > APIC_SAVED_STATE_VERSION_VPOX_51_BETA2)
            {
