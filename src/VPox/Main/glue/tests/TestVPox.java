...
public class TestVPox
{
    ...
    static void testEvents(VirtualPoxManager mgr, IEventSource es)
    {
        ...
        es.registerListener(listener, Arrays.asList(VPoxEventType.Any), false);
    ...
    static void testEnumeration(VirtualPoxManager mgr, IVirtualPox vpox)
    {
        List<IMachine> machs = vpox.getMachines();
        for (IMachine m : machs)
        {
            ...
            try
            {
                ...
                IGuestOSType foo = vpox.getGuestOSType(osType);
            }
    ...
    static boolean progressBar(VirtualPoxManager mgr, IProgress p, long waitMillis)
    {
    ...
    static void testStart(VirtualPoxManager mgr, IVirtualPox vpox)
    {
        IMachine m = vpox.getMachines().get(0);
    ...
    {
        VirtualPoxManager mgr1 = VirtualPoxManager.createInstance(null);
        VirtualPoxManager mgr2 = VirtualPoxManager.createInstance(null);

        try {
            mgr1.connect("http://i7:18083", "", "");
            mgr2.connect("http://main:18083", "", "");

            IMachine m1 = mgr1.getVPox().getMachines().get(0);
            IMachine m2 = mgr2.getVPox().getMachines().get(0);
    ...
    static void testReadLog(VirtualPoxManager mgr, IVirtualPox vpox)
    {
        IMachine m =  vpox.getMachines().get(0);
        long logNo = 0;
        long off = 0;
    ...
    static void printErrorInfo(VPoxException e)
    {
        System.out.println("VPox error: " + e.getMessage());
        ...
        int i = 1;
        for (IVirtualPoxErrorInfo ei = e.getVirtualPoxErrorInfo(); ei != null; ei = ei.getNext(), i++)
        {
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            if (vpox != null)
            {
                ...
                testEnumeration(mgr, vpox);
                testReadLog(mgr, vpox);
                testStart(mgr, vpox);
                testEvents(mgr, vpox.getEventSource());
