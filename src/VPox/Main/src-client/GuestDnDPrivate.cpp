 ...
 *    VPoxGuestR3LibDragAndDrop.cpp) and the guest OS specific parts.
 *    Receive/send message from/to the HGCM service and does all guest specific
 *    operations. For Windows guests VPoxTray is in charge, whereas on UNIX-y guests
 *    VPoxClient will be used.
 *
 * Terminology:
 *
 ...
 * guest (e.g. /tmp/VirtualPox Dropped Files). We can't delete them after the
 ...
 *   - DnD service crash in the guest during a DnD op (e.g. crash of VPoxClient or X11)
...
GuestDnDState::GuestDnDState(const ComObjPtr<Guest>& pGuest)
    ...
    , m_fGuestFeatures0(VPOX_DND_GF_NONE)
                               ...
                               int rcOp /* = VINF_SUCCESS */, const Utf8Str &strMsg /* = "" */)
{
    ...
    switch (uStatus)
    {
        case DragAndDropSvc::DND_PROGRESS_ERROR:
        {
            ...
            if (   !m_pProgress.isNull()
                && !fCompleted)
                hr = m_pProgress->i_notifyComplete(VPOX_E_IPRT_ERROR,
                                                   COM_IIDOF(IGuest),
...
int GuestDnDState::onDispatch(uint32_t u32Function, void *pvParms, uint32_t cbParms)
{
    ...
    switch (u32Function)
    {
        ...
        {
            DragAndDropSvc::PVPOXDNDCBCONNECTDATA pCBData = reinterpret_cast<DragAndDropSvc::PVPOXDNDCBCONNECTDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::VPOXDNDCBCONNECTDATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PVPOXDNDCBREPORTFEATURESDATA pCBData = reinterpret_cast<DragAndDropSvc::PVPOXDNDCBREPORTFEATURESDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::VPOXDNDCBREPORTFEATURESDATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PVPOXDNDCBHGACKOPDATA pCBData = reinterpret_cast<DragAndDropSvc::PVPOXDNDCBHGACKOPDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::VPOXDNDCBHGACKOPDATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PVPOXDNDCBHGREQDATADATA pCBData = reinterpret_cast<DragAndDropSvc::PVPOXDNDCBHGREQDATADATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::VPOXDNDCBHGREQDATADATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PVPOXDNDCBHGEVTPROGRESSDATA pCBData =
               reinterpret_cast<DragAndDropSvc::PVPOXDNDCBHGEVTPROGRESSDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::VPOXDNDCBHGEVTPROGRESSDATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PVPOXDNDCBGHACKPENDINGDATA pCBData =
               reinterpret_cast<DragAndDropSvc::PVPOXDNDCBGHACKPENDINGDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::VPOXDNDCBGHACKPENDINGDATA) == cbParms, VERR_INVALID_PARAMETER);
    ...
    , m_cTransfersPending(0)
{
    ...
    const com::Utf8Str arrEntries[] = { VPOX_DND_FORMATS_DEFAULT };
 ...
 */
int GuestDnD::hostCall(uint32_t u32Function, uint32_t cParms, PVPOXHGCMSVCPARM paParms) const
{
    ...
    return pVMMDev->hgcmHostCall("VPoxDragAndDropSvc", u32Function, cParms, paParms);
}
 ...
 *
 * @returns Internal DnD action, or VPOX_DND_ACTION_IGNORE if not found / supported.
 * @param   enmAction               Main DnD action to convert.
 */
/* static */
VPOXDNDACTION GuestDnD::toHGCMAction(DnDAction_T enmAction)
{
    VPOXDNDACTION dndAction = VPOX_DND_ACTION_IGNORE;
    switch (enmAction)
    {
        case DnDAction_Copy:
            dndAction = VPOX_DND_ACTION_COPY;
            break;
        case DnDAction_Move:
            dndAction = VPOX_DND_ACTION_MOVE;
            break;
        case DnDAction_Link:
...
void GuestDnD::toHGCMActions(DnDAction_T                    enmDnDActionDefault,
                             VPOXDNDACTION                 *pDnDActionDefault,
                             const std::vector<DnDAction_T> vecDnDActionsAllowed,
                             VPOXDNDACTIONLIST             *pDnDLstActionsAllowed)
{
    VPOXDNDACTIONLIST dndLstActionsAllowed = VPOX_DND_ACTION_IGNORE;
    VPOXDNDACTION     dndActionDefault     = toHGCMAction(enmDnDActionDefault);
    ...
    if (!vecDnDActionsAllowed.empty())
    {
        ...
        if (isDnDIgnoreAction(dndActionDefault))
        {
            if (hasDnDCopyAction(dndLstActionsAllowed))
                ...
                dndActionDefault = VPOX_DND_ACTION_MOVE;
        }
    }
 ...
 */
/* static */
DnDAction_T GuestDnD::toMainAction(VPOXDNDACTION dndAction)
{
 ...
 */
/* static */
std::vector<DnDAction_T> GuestDnD::toMainActions(VPOXDNDACTIONLIST dndActionList)
{
 ...
 *
 * @returns VPox status code. Will return VERR_CANCELLED if the user has cancelled the progress object.
