...
class BaseTestVm(object):
    ...
    def _storageCtrlAndBusToName(self, oVPoxMgr, oVM, eCtrl, eBus):
        """
        ...
        """
        try:
            aoControllers = oVPoxMgr.getArray(oVM, 'storageControllers');
        except:
            reporter.errorXcpt();
            return None;
        asSummary = [];
    ...
    def createVm(self, oTestDrv, eNic0AttachType = None, sDvdImage = None):
        ...
        Returns same as vpox.TestDriver.createTestVM.
        """
        reporter.log2('');
    ...
    def getReconfiguredVm(self, oTestDrv, cCpus, sVirtMode, sParavirtMode = None):
        ...
        VPox VM object that is only present when rc is True.
        """
                ...
                if self._skipVmTest(oTestDrv, oVM):
                    ...
                    if oSession is not None:
                        ...
                        if sParavirtMode is not None and oSession.fpApiVer >= 5.0:
                            adParavirtProviders = {
                                g_ksParavirtProviderNone   : vpoxcon.ParavirtProvider_None,
                                g_ksParavirtProviderDefault: vpoxcon.ParavirtProvider_Default,
                                g_ksParavirtProviderLegacy : vpoxcon.ParavirtProvider_Legacy,
                                g_ksParavirtProviderMinimal: vpoxcon.ParavirtProvider_Minimal,
                                g_ksParavirtProviderHyperV : vpoxcon.ParavirtProvider_HyperV,
                                g_ksParavirtProviderKVM    : vpoxcon.ParavirtProvider_KVM,
                            };
    ...
    def isHostCpuAffectedByUbuntuNewAmdBug(self, oTestDrv):
        ...
        if not oTestDrv.isHostCpuAmd():
            return False;
        try:
            (uMaxExt, _, _, _) = oTestDrv.oVPox.host.getProcessorCPUIDLeaf(0, 0x80000000, 0);
            (uFamilyModel, _, _, _) = oTestDrv.oVPox.host.getProcessorCPUIDLeaf(0, 0x80000001, 0);
        except:
            reporter.logXcpt();
            return False;
    ...
    def getTestUserPassword(self, sUser = None):
        ...
        if sUser == 'test':
            return '';
        if sUser == 'vpoxuser': # Default unattended installation user and password.
            return 'changeme';
        return 'password';
    ...
    def getIGuestOSType(self, oVPoxWrapped):
        """
        ...
        """
        try:
            return oVPoxWrapped.o.getGuestOSType(self.sKind);
        except:
            reporter.errorXcpt('sVmName=%s sKind=%s' % (self.sVmName, self.sKind,));
        return None;

    def getRecommendedHddSize(self, oVPoxWrapped):
        """
        ...
        """
        if self.__cbHdd < 0:
            oGuestOSType = self.getIGuestOSType(oVPoxWrapped);
            if oGuestOSType:
                try:
    ...
    def getHddAddress(self, oVM, oVPoxWrapped):
        """
        ...
        try:
            aoAttachments = oVPoxWrapped.oVPoxMgr.getArray(oVM, 'mediumAttachments')
        except:
            reporter.errorXcpt();
        else:
            for oAtt in aoAttachments:
                try:
                    ...
                    reporter.errorXcpt();
                    return self.__tHddCtrlPortDev;
                if eType == vpoxcon.DeviceType_HardDisk:
            ...
            if oGuestOSType:
                try:
                    ...
                    self.__tHddCtrlPortDev = (self._storageCtrlAndBusToName(oVPoxWrapped.oVPoxMgr, oVM, eCtrl, eBus), 0, 0);
                    reporter.log2('getHddAddress: %s, %s, %s [IGuestOSType]' % self.__tHddCtrlPortDev);
        return self.__tHddCtrlPortDev;

    def getDvdAddress(self, oVM, oVPoxWrapped):
        """
        ...
        try:
            ...
            for oAtt in aoAttachments:
                try:
                    ...
                    reporter.errorXcpt();
                    return self.__tDvdCtrlPortDev;
                if eType == vpoxcon.DeviceType_DVD:
            ...
            if oGuestOSType:
                try:
                    ...
                    self.__tDvdCtrlPortDev = (self._storageCtrlAndBusToName(oVPoxWrapped.oVPoxMgr, oVM, eCtrl, eBus), 1, 0);
    ...
    def recreateRecommendedHdd(self, oVM, oTestDrv, sHddPath = None):
        ...
        cbHdd       = self.getRecommendedHddSize(oTestDrv.oVPox);
        tHddAddress = self.getHddAddress(oVM, oTestDrv.oVPox);
            ...
            if oSession is not None:
                ...
                if fRc:
                    ...
                    if fRc and oOldHd is not None:
                        fRc = fRc and oTestDrv.oVPox.deleteHdByMedium(oOldHd);
...
class TestVm(object):
    ...
    def createVm(self, oTestDrv, eNic0AttachType = None, sDvdImage = None):
        ...
        if eNic0AttachType is not None:
            ...
            eMyNic0AttachType = vpoxcon.NetworkAttachmentType_NAT;
        elif self.sNic0AttachType == 'bridged':
            eMyNic0AttachType = vpoxcon.NetworkAttachmentType_Bridged;
        else:
...
class TestVmSet(object):
    ...
    def getResourceSet(self):
        """
        Called vpox.TestDriver.getResourceSet and returns a list of paths of resources.
        """
        asResources = [];
        for oTestVm in self.aoTestVms:
