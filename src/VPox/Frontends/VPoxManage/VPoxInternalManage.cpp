/* $Id: VPoxInternalManage.cpp $ */
/** @file
 * VPoxManage - The 'internalcommands' command.
 *
 * VPoxInternalManage used to be a second CLI for doing special tricks,
 * not intended for general usage, only for assisting VPox developers.
 * It is now integrated into VPoxManage.
 */
...
void printUsageInternal(USAGECATEGORY enmCommand, PRTSTREAM pStrm)
{
    ...
    RTStrmPrintf(pStrm,
        "Usage: VPoxManage internalcommands <command> [command arguments]\n"
        "\n"
        "Commands:\n"
        "\n"
        ...
        ? "  createrawvmdk -filename <filename> -rawdisk <diskname>\n"
          ...
          "         VPoxManage internalcommands listpartitions\n"
          "\n"
        : "",
 ...
 */
static RTEXITCODE CmdLoadSyms(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aSession);
    HRESULT rc;
    ...
    CHECK_ERROR_RET(aVirtualPox, FindMachine(Bstr(argv[0]).raw(),
     ...
     */
    Utf8Str KeyStr;
    HRESULT hrc = NewUniqueKey(machine, "VPoxInternal/DBGF/loadsyms", KeyStr);
    if (SUCCEEDED(hrc))
        hrc = SetString(machine, "VPoxInternal/DBGF/loadsyms", KeyStr.c_str(), "Filename", pszFilename);
    if (SUCCEEDED(hrc) && argc >= 3)
        hrc = SetInt64(machine, "VPoxInternal/DBGF/loadsyms", KeyStr.c_str(), "Delta", offDelta);
    if (SUCCEEDED(hrc) && argc >= 4)
        hrc = SetString(machine, "VPoxInternal/DBGF/loadsyms", KeyStr.c_str(), "Module", pszModule);
    if (SUCCEEDED(hrc) && argc >= 5)
        hrc = SetUInt64(machine, "VPoxInternal/DBGF/loadsyms", KeyStr.c_str(), "ModuleAddress", ModuleAddress);
    if (SUCCEEDED(hrc) && argc >= 6)
        hrc = SetUInt64(machine, "VPoxInternal/DBGF/loadsyms", KeyStr.c_str(), "ModuleSize", ModuleSize);
 ...
 */
static RTEXITCODE CmdLoadMap(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aSession);
    HRESULT rc;
     ...
     */
    Utf8Str KeyStr;
    HRESULT hrc = NewUniqueKey(machine, "VPoxInternal/DBGF/loadmap", KeyStr);
    if (SUCCEEDED(hrc))
        hrc = SetString(machine, "VPoxInternal/DBGF/loadmap", KeyStr.c_str(), "Filename", pszFilename);
    if (SUCCEEDED(hrc))
        hrc = SetUInt64(machine, "VPoxInternal/DBGF/loadmap", KeyStr.c_str(), "Address", ModuleAddress);
    if (SUCCEEDED(hrc) && pszModule != NULL)
        hrc = SetString(machine, "VPoxInternal/DBGF/loadmap", KeyStr.c_str(), "Name", pszModule);
    if (SUCCEEDED(hrc) && offSubtrahend != 0)
        hrc = SetUInt64(machine, "VPoxInternal/DBGF/loadmap", KeyStr.c_str(), "Subtrahend", offSubtrahend);
    if (SUCCEEDED(hrc) && iSeg != UINT32_MAX)
        hrc = SetUInt64(machine, "VPoxInternal/DBGF/loadmap", KeyStr.c_str(), "Segment", iSeg);
...
static RTEXITCODE CmdSetHDUUID(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);
    Guid uuid;
    RTUUID rtuuid;
    enum eUuidType {
        HDUUID,
    ...
    if (!strcmp(argv[0], "sethduuid"))
    {
        uuidType = HDUUID;
        if (argc != 3 && argc != 2)
            return errorSyntax(USAGE_I_SETHDUUID, "Not enough parameters");
        /* if specified, take UUID, otherwise generate a new one */
        if (argc == 3)
        {
            if (RT_FAILURE(RTUuidFromStr(&rtuuid, argv[2])))
                return errorSyntax(USAGE_I_SETHDUUID, "Invalid UUID parameter");
            uuid = argv[2];
        } else
            uuid.create();
    }
    else if (!strcmp(argv[0], "sethdparentuuid"))
    {
        ...
        if (argc != 3)
            return errorSyntax(USAGE_I_SETHDPARENTUUID, "Not enough parameters");
        if (RT_FAILURE(RTUuidFromStr(&rtuuid, argv[2])))
            return errorSyntax(USAGE_I_SETHDPARENTUUID, "Invalid UUID parameter");
        uuid = argv[2];
    }
    else
        return errorSyntax(USAGE_I_SETHDUUID, "Invalid invocation");
    ...
    rc = VDInterfaceAdd(&vdInterfaceError.Core, "VPoxManage_IError", VDINTERFACETYPE_ERROR,
...
static RTEXITCODE CmdDumpHDInfo(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);
    ...
    if (argc != 1)
    {
        return errorSyntax(USAGE_I_DUMPHDINFO, "Not enough parameters");
    }
...
static RTEXITCODE CmdListPartitions(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);
    Utf8Str rawdisk;
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        {
            return errorSyntax(USAGE_I_LISTPARTITIONS, "Invalid parameter '%s'", argv[i]);
        }
    }

    if (rawdisk.isEmpty())
        return errorSyntax(USAGE_I_LISTPARTITIONS, "Mandatory parameter -rawdisk missing");
...
static RTEXITCODE CmdCreateRawVMDK(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);
    HRESULT rc = S_OK;
    Utf8Str filename;
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "-relative") == 0)
        {
            ...
            return errorSyntax(USAGE_I_CREATERAWVMDK, "Invalid parameter '%s'", argv[i]);
    }

    if (filename.isEmpty())
        return errorSyntax(USAGE_I_CREATERAWVMDK, "Mandatory parameter -filename missing");
    if (rawdisk.isEmpty())
        return errorSyntax(USAGE_I_CREATERAWVMDK, "Mandatory parameter -rawdisk missing");
    if (!pszPartitions && pszMBRFilename)
        return errorSyntax(USAGE_I_CREATERAWVMDK, "The parameter -mbr is only valid when the parameter -partitions is also present");
...
static RTEXITCODE CmdRenameVMDK(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);
    Utf8Str src;
    Utf8Str dst;
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        {
            return errorSyntax(USAGE_I_RENAMEVMDK, "Invalid parameter '%s'", argv[i]);
        }
    }

    if (src.isEmpty())
        return errorSyntax(USAGE_I_RENAMEVMDK, "Mandatory parameter -from missing");
    if (dst.isEmpty())
        return errorSyntax(USAGE_I_RENAMEVMDK, "Mandatory parameter -to missing");
    ...
    int vrc = VDInterfaceAdd(&vdInterfaceError.Core, "VPoxManage_IError", VDINTERFACETYPE_ERROR,
                             NULL, sizeof(VDINTERFACEERROR), &pVDIfs);
    AssertRC(vrc);
...
static RTEXITCODE CmdConvertToRaw(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);
    Utf8Str srcformat;
    Utf8Str src;
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        {
            return errorSyntax(USAGE_I_CONVERTTORAW, "Invalid parameter '%s'", argv[i]);
        }
    }

    if (src.isEmpty())
        return errorSyntax(USAGE_I_CONVERTTORAW, "Mandatory filename parameter missing");
    if (dst.isEmpty())
        return errorSyntax(USAGE_I_CONVERTTORAW, "Mandatory outputfile parameter missing");
...
static RTEXITCODE CmdConvertHardDisk(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);
    Utf8Str srcformat;
    Utf8Str dstformat;
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        {
            return errorSyntax(USAGE_I_CONVERTHD, "Invalid parameter '%s'", argv[i]);
        }
    }

    if (src.isEmpty())
        return errorSyntax(USAGE_I_CONVERTHD, "Mandatory input image parameter missing");
    if (dst.isEmpty())
        return errorSyntax(USAGE_I_CONVERTHD, "Mandatory output image parameter missing");
    ...
    vrc = VDInterfaceAdd(&vdInterfaceError.Core, "VPoxManage_IError", VDINTERFACETYPE_ERROR,
                         NULL, sizeof(VDINTERFACEERROR), &pVDIfs);
    AssertRC(vrc);
 ...
 */
static RTEXITCODE CmdRepairHardDisk(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);
    Utf8Str image;
    Utf8Str format;
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        {
            return errorSyntax(USAGE_I_REPAIRHD, "Invalid parameter '%s'", argv[i]);
        }
    }

    if (image.isEmpty())
        return errorSyntax(USAGE_I_REPAIRHD, "Mandatory input image parameter missing");
...
static RTEXITCODE CmdDebugLog(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    /*
     ...
     */
    if (argc < 1)
        return errorSyntax(USAGE_I_DEBUGLOG, "Missing VM name/UUID");
 ...
 */
static RTEXITCODE CmdGeneratePasswordHash(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    RT_NOREF(aVirtualPox, aSession);

    /* one parameter, the password to hash */
    if (argc != 1)
        return errorSyntax(USAGE_I_PASSWORDHASH, "password to hash required");
 ...
 */
static RTEXITCODE CmdGuestStats(int argc, char **argv, ComPtr<IVirtualPox> aVirtualPox, ComPtr<ISession> aSession)
{
    /* one parameter, guest name */
    if (argc < 1)
        return errorSyntax(USAGE_I_GUESTSTATS, "Missing VM name/UUID");
    ...
    if (argc > 1 && aUpdateInterval == 0)
        return errorSyntax(USAGE_I_GUESTSTATS, "Invalid update interval specified");
...
RTEXITCODE handleInternalCommands(HandlerArg *a)
{
    ...
    if (a->argc < 1)
        return errorSyntax(USAGE_S_ALL, "Command missing");
    ...
    if (!strcmp(pszCmd, "loadmap"))
        return CmdLoadMap(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "loadsyms"))
        return CmdLoadSyms(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    //if (!strcmp(pszCmd, "unloadsyms"))
    //    return CmdUnloadSyms(argc - 1, &a->argv[1]);
    if (!strcmp(pszCmd, "sethduuid") || !strcmp(pszCmd, "sethdparentuuid"))
        return CmdSetHDUUID(a->argc, &a->argv[0], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "dumphdinfo"))
        return CmdDumpHDInfo(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "listpartitions"))
        return CmdListPartitions(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "createrawvmdk"))
        return CmdCreateRawVMDK(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "renamevmdk"))
        return CmdRenameVMDK(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "converttoraw"))
        return CmdConvertToRaw(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "converthd"))
        return CmdConvertHardDisk(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    ...
    if (!strcmp(pszCmd, "debuglog"))
        return CmdDebugLog(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "passwordhash"))
        return CmdGeneratePasswordHash(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "gueststats"))
        return CmdGuestStats(a->argc - 1, &a->argv[1], a->virtualPox, a->session);
    if (!strcmp(pszCmd, "repairhd"))
        return CmdRepairHardDisk(a->argc - 1, &a->argv[1], a->virtualPox, a->session);

    /* default: */
    return errorSyntax(USAGE_S_ALL, "Invalid command '%s'", a->argv[0]);
}
