/* $Id: VPoxGuestR3LibGuestCtrl.cpp $ */
/** @file
 * VPoxGuestR3Lib - Ring-3 Support Library for VirtualPox guest additions, guest control.
 */
...
{
    return VbglR3HGCMConnect("VPoxGuestControlSvc", pidClient);
}
 ...
 * @returns VPox status code.
 * @param   idClient        The client ID returned by VbglR3GuestCtrlConnect().
 * @param   fGuestFeatures  Features to report, VPOX_GUESTCTRL_GF_XXX.
 * @param   pfHostFeatures  Where to store the features VPOX_GUESTCTRL_HF_XXX.
 */
VBGLR3DECL(int) VbglR3GuestCtrlReportFeatures(uint32_t idClient, uint64_t fGuestFeatures, uint64_t *pfHostFeatures)
{
    ...
    do
    {
        ...
        VbglHGCMParmUInt64Set(&Msg.f64Features1, VPOX_GUESTCTRL_GF_1_MUST_BE_ONE);
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (Msg.f64Features1.u.value64 & VPOX_GUESTCTRL_GF_1_MUST_BE_ONE)
 ...
 * @returns VPox status code.
 * @param   idClient        The client ID returned by VbglR3GuestCtrlConnect().
 * @param   pfHostFeatures  Where to store the host feature, VPOX_GUESTCTRL_HF_XXX.
 */
...
VBGLR3DECL(int) VbglR3GuestCtrlSessionGetOpen(PVBGLR3GUESTCTRLCMDCTX pCtx, PVBGLR3GUESTCTRLSESSIONSTARTUPINFO *ppStartupInfo)
{
    ...
    do
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            pStartupInfo->uSessionID = VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(pCtx->uContextID);
        }
...
VBGLR3DECL(int) VbglR3GuestCtrlSessionGetClose(PVBGLR3GUESTCTRLCMDCTX pCtx, uint32_t *pfFlags, uint32_t *pidSession)
{
    ...
    do
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (pidSession)
                *pidSession = VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(pCtx->uContextID);
        }
    } while (rc == VERR_INTERRUPTED && g_fVbglR3GuestCtrlHavePeekGetCancel);
    return rc;
}
