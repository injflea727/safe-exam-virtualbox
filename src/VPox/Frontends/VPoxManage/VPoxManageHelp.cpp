/* $Id: VPoxManageHelp.cpp $ */
/** @file
 * VPoxManage - help and other message output.
 */
...
static enum HELP_CMD_VPOXMANAGE    g_enmCurCommand = HELP_CMD_VPOXMANAGE_INVALID;
...
{
    Assert(g_enmCurCommand == HELP_CMD_VPOXMANAGE_INVALID);
 ...
 */
static uint32_t printBriefCommandOrSubcommandHelp(enum HELP_CMD_VPOXMANAGE enmCommand, uint64_t fSubcommandScope, PRTSTREAM pStrm)
{
 ...
 * @param   pStrm               The output stream.
 */
static void printFullCommandOrSubcommandHelp(enum HELP_CMD_VPOXMANAGE enmCommand, uint64_t fSubcommandScope, PRTSTREAM pStrm)
{
    uint32_t cPendingBlankLines = 0;
    uint32_t cFound = 0;
    for (uint32_t i = 0; i < g_cHelpEntries; i++)
    {
        PCRTMSGREFENTRY pHelp = g_apHelpEntries[i];
        if (   pHelp->idInternal == (int64_t)enmCommand
            || enmCommand == HELP_CMD_VPOXMANAGE_INVALID)
        {
            cFound++;
...
void showLogo(PRTSTREAM pStrm)
{
    ...
    if (!s_fShown)
    {
        RTStrmPrintf(pStrm, VPOX_PRODUCT " Command Line Management Interface Version "
                     VPOX_VERSION_STRING "\n"
...
void printUsage(USAGECATEGORY enmCommand, uint64_t fSubcommandScope, PRTSTREAM pStrm)
{
    ...
    if (enmCommand == USAGE_S_ALL)
        RTStrmPrintf(pStrm,
                     "  VPoxManage [<general option>] <command>\n"
                     " \n \n"
                     "General Options:\n \n"
...
#define SEP pcszSep1, pcszSep2
    ...
    if (enmCommand == USAGE_MODIFYVM || enmCommand == USAGE_S_ALL)
    {
        RTStrmPrintf(pStrm,
                     ...
                     "                            [--graphicscontroller none|vpoxvga|vmsvga|vpoxsvga]\n"
#else
                     "                            [--graphicscontroller none|vpoxvga]\n"
#endif
                     "                            [--monitorcount <number>]\n"
                     "                            [--accelerate3d on|off]\n"
...
#if defined(VPOX_WITH_NAT_SERVICE)
...
# if !defined(RT_OS_SOLARIS) || defined(VPOX_ONLY_DOCS)
                     " |\n"
                     "                            create |\n"
                     "                            remove <name>\n"
# else
...
#ifndef VPOX_ONLY_DOCS /* Converted to man page, not needed. */
    if (enmCommand == USAGE_S_ALL)
    {
        uint32_t cPendingBlankLines = 0;
        for (uint32_t i = 0; i < g_cHelpEntries; i++)
