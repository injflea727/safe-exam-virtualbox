...
# mouse autodetection, so on these systems we have to enable vpoxmouse in the
# X.Org configuration file as well as vpoxvideo.  When uninstalling, we enable
# the fbdev driver, which SUSE prefers over vesa, and we leave the references
# to vpoxmouse in place, as without the driver they are harmless.
...
foreach $cfg (@cfg_files) {
    ...
    if (open(CFG, $cfg)) {
        ...
        while (defined ($line = <CFG>)) {
            ...
            if ($line =~ /^\s*Section\s*"([a-zA-Z]+)"/i) {
                ...
                if ($line =~ /^\s*EndSection/i && $in_layout) {
                    ...
                    print TMP "  InputDevice  \"VPoxMouse\"\n"
                }
            ...
            if ($in_section) {
                ...
                # them with our one.
                if ($line =~ /^\s*driver\s+\"(fbdev|vga|vesa|vpoxvideo|ChangeMe)\"/i) {
                    $line =~ s/(fbdev|vga|vesa|vpoxvideo|ChangeMe)/$videodrv/i;
                }
                # Also keep track of whether this configuration file contains
                # an input device section for vpoxmouse.  If it does, we don't
                # need to add one later.
                if ($line =~ /^\s*driver\s+\"(?:vpoxmouse)\"/i)
                {
                    $have_mouse = 1
                }

                # We add vpoxmouse to the server layout section ourselves, so
                # remove any existing references to it.
                if (   $line =~ /^\s*inputdevice.*\"vpoxmouse\"/i)
                {
                    $line = "";
                }
            }
        ...
        # We always add a vpoxmouse section at the end for SUSE guests using
        # X.Org 1.5 if vpoxmouse is not referenced anywhere else in the file,
        ...
        if (!$have_mouse) {
            ...
            print TMP "        Identifier  \"VPoxMouse\"\n";
            print TMP "        Driver      \"$mousedrv\"\n";
            print TMP "        Option      \"Device\"     \"\/dev\/vpoxguest\"\n";
            print TMP "        Option      \"SendCoreEvents\"  \"on\"\n";
            print TMP "EndSection\n";
        }
        close(TMP);

        # We do not overwrite existing "$cfg.vpox" files in order to keep a
