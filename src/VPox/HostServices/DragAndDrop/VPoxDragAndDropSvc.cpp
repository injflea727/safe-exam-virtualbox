 ...
 *
 * @sa See src/VPox/Main/src-client/GuestDnDPrivate.cpp for more information.
 */
...
public:
    ...
    DragAndDropClient(uint32_t idClient)
        ...
        , fGuestFeatures0(VPOX_DND_GF_NONE)
        , fGuestFeatures1(VPOX_DND_GF_NONE)
    {
        RT_ZERO(m_SvcCtx);
    }
...
public:
    ...
    /** Guest feature flags, VPOX_DND_GF_0_XXX. */
    uint64_t                fGuestFeatures0;
    /** Guest feature flags, VPOX_DND_GF_1_XXX. */
...
{
public:
    explicit DragAndDropService(PVPOXHGCMSVCHELPERS pHelpers)
        : HGCM::AbstractService<DragAndDropService>(pHelpers)
        , m_pManager(NULL)
        , m_u32Mode(VPOX_DRAG_AND_DROP_MODE_OFF)
    {}

protected:
    int  init(VPOXHGCMSVCFNTABLE *pTable) RT_NOEXCEPT RT_OVERRIDE;
    ...
    int  clientQueryFeatures(uint32_t cParms, VPOXHGCMSVCPARM paParms[]) RT_NOEXCEPT;
    int  clientReportFeatures(DragAndDropClient *pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[]) RT_NOEXCEPT;
    void guestCall(VPOXHGCMCALLHANDLE callHandle, uint32_t idClient, void *pvClient, uint32_t u32Function,
                   uint32_t cParms, VPOXHGCMSVCPARM paParms[]) RT_NOEXCEPT RT_OVERRIDE;
    int  hostCall(uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[]) RT_NOEXCEPT RT_OVERRIDE;
...
private:
    ...
    /** Current drag and drop mode, VPOX_DRAG_AND_DROP_MODE_XXX. */
    uint32_t                           m_u32Mode;
    /** Host feature mask (VPOX_DND_HF_0_XXX) for DND_GUEST_REPORT_FEATURES
...
void DragAndDropClient::disconnect(void) RT_NOEXCEPT
{
     ...
     * Let the host know.
     */
    VPOXDNDCBDISCONNECTMSGDATA data;
    RT_ZERO(data);
    /** @todo Magic needed? */
    /** @todo Add context ID. */
...
int DragAndDropService::init(VPOXHGCMSVCFNTABLE *pTable) RT_NOEXCEPT
{
    ...
    modeSet(VPOX_DRAG_AND_DROP_MODE_OFF);

    /* Set host features. */
    m_fHostFeatures0 = VPOX_DND_HF_NONE;
 ...
 */
int DragAndDropService::clientReportFeatures(DragAndDropClient *pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[]) RT_NOEXCEPT
{
    RT_NOREF(pClient);
    ...
    ASSERT_GUEST_RETURN(fFeatures1 & VPOX_DND_GF_1_MUST_BE_ONE, VERR_INVALID_PARAMETER);
 ...
 * @param   paParms     Array of parameters.
 */
int DragAndDropService::clientQueryFeatures(uint32_t cParms, VPOXHGCMSVCPARM paParms[]) RT_NOEXCEPT
{
    /*
     * Validate the request.
     */
...
{
#ifndef VPOX_WITH_DRAG_AND_DROP_GH
    if (   u32Mode == VPOX_DRAG_AND_DROP_MODE_GUEST_TO_HOST
        || u32Mode == VPOX_DRAG_AND_DROP_MODE_BIDIRECTIONAL)
    {
        m_u32Mode = VPOX_DRAG_AND_DROP_MODE_OFF;
    ...
    switch (u32Mode)
    {
        case VPOX_DRAG_AND_DROP_MODE_OFF:
        case VPOX_DRAG_AND_DROP_MODE_HOST_TO_GUEST:
        case VPOX_DRAG_AND_DROP_MODE_GUEST_TO_HOST:
        case VPOX_DRAG_AND_DROP_MODE_BIDIRECTIONAL:
            m_u32Mode = u32Mode;
            break;
...
void DragAndDropService::guestCall(VPOXHGCMCALLHANDLE callHandle, uint32_t idClient,
                                   void *pvClient, uint32_t u32Function,
                                   uint32_t cParms, VPOXHGCMSVCPARM paParms[]) RT_NOEXCEPT
{
    RT_NOREF1(pvClient);
    ...
    switch (u32Function)
    {
        ...
        {
            if (modeGet() != VPOX_DRAG_AND_DROP_MODE_OFF)
                rc = VINF_SUCCESS;
            else
            {
        ...
        {
            if (   modeGet() == VPOX_DRAG_AND_DROP_MODE_BIDIRECTIONAL
                || modeGet() == VPOX_DRAG_AND_DROP_MODE_HOST_TO_GUEST)
                rc = VINF_SUCCESS;
            else
        ...
        case GUEST_DND_FN_GH_EVT_ERROR:
        {
            ...
            if (   modeGet() == VPOX_DRAG_AND_DROP_MODE_BIDIRECTIONAL
                || modeGet() == VPOX_DRAG_AND_DROP_MODE_GUEST_TO_HOST)
                rc = VINF_SUCCESS;
            else
#endif
...
#define GET_CONTEXT_ID_PARM0() \
    ...
    if (rc == VINF_SUCCESS) /* Note: rc might be VINF_HGCM_ASYNC_EXECUTE! */
    {
        ...
        {
            /*
             * Note: Older VPox versions with enabled DnD guest->host support (< 5.0)
            ...
            case GUEST_DND_FN_GET_NEXT_HOST_MSG:
            {
                ...
                if (cParms == 3)
                {
                    ...
                    if (RT_FAILURE(rc)) /* No queued messages available? */
                    {
                        ...
                        {
                            VPOXDNDCBHGGETNEXTHOSTMSG data;
                            RT_ZERO(data);
            ...
            case GUEST_DND_FN_REPORT_FEATURES:
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    VPOXDNDCBREPORTFEATURESDATA data;
                    RT_ZERO(data);
            ...
            case GUEST_DND_FN_HG_EVT_PROGRESS:
            {
                ...
                VPOXDNDCBHGEVTPROGRESSDATA data;
                RT_ZERO(data);
            ...
            case GUEST_DND_FN_GH_ACK_PENDING:
            {
                ...
                VPOXDNDCBGHACKPENDINGDATA data;
                RT_ZERO(data);
            ...
            /* New since protocol v2 (>= VPox 5.0). */
            case GUEST_DND_FN_GH_SND_FILE_HDR:
            {
            ...
            default:
            {
                ...
                if (rc == VERR_NO_DATA) /* Manager has no new messsages? Try asking the host. */
                {
                    if (m_SvcCtx.pfnHostCallback)
                    {
                        VPOXDNDCBHGGETNEXTHOSTMSGDATA data;
                        RT_ZERO(data);

                        data.hdr.uMagic = VPOX_DND_CB_MAGIC_MAKE(0 /* uFn */, 0 /* uVer */);
    ...
    do /* goto avoidance break-loop. */
    {
        if (fSendToGuest)
        {
            ...
            if (m_clientMap.empty()) /* At least one client on the guest connected? */
            {
                 ...
                 * not running VPoxTray/VPoxClient.
                 */
                rc = VERR_NOT_SUPPORTED;
                break;
            }
