/* $Id: VPoxManageCloud.cpp $ */
/** @file
 * VPoxManageCloud - The cloud related commands.
 */
...
static RTEXITCODE createCloudInstance(HandlerArg *a, int iFirst, PCLOUDCOMMONOPT pCommonOpts)
{
    ...
    CHECK_ERROR2_RET(hrc, a->virtualPox, CreateAppliance(pAppliance.asOutParam()), RTEXITCODE_FAILURE);
    ULONG vsdNum = 1;
...
static RTEXITCODE showCloudInstanceInfo(HandlerArg *a, int iFirst, PCLOUDCOMMONOPT pCommonOpts)
{
    ...
    for (size_t i=0; i<vsdHReadableArraySize ; ++i)
    {
        ...
        if (FAILED(hrc) || aVPoxValues.size() == 0)
            LogRel((vsdHReadableArray[i].strNotFound.c_str()));
        else
        {
            LogRel(("Size is %d", aVPoxValues.size()));
            for (size_t j = 0; j<aVPoxValues.size(); ++j)
            {
                RTPrintf(vsdHReadableArray[i].strFound.c_str(), aVPoxValues[j]);
            }
        }
...
static RTEXITCODE exportCloudImage(HandlerArg *a, int iFirst, PCLOUDCOMMONOPT pCommonOpts)
{
    ...
    Utf8Str strImageId;   /* XXX: this is vpox "image", i.e. medium */
    Utf8Str strBucketName;
    Utf8Str strObjectName;
    Utf8Str strDisplayName;
...
static HRESULT createLocalGatewayImage(ComPtr<IVirtualPox> virtualPox, const Bstr& aGatewayIso, const Bstr& aGuestAdditionsIso, const Bstr& aProxy)
{
    ...
    hrc = virtualPox->OpenMedium(strGatewayImage.raw(), DeviceType_HardDisk, AccessMode_ReadWrite, FALSE, hd.asOutParam());
    ...
    ComPtr<IMedium> iso;
    hrc = virtualPox->OpenMedium(aGatewayIso.raw(), DeviceType_DVD, AccessMode_ReadOnly, FALSE, iso.asOutParam());
    ...
    hrc = virtualPox->CreateMachine(NULL, strGatewayVM.raw(), ComSafeArrayAsInParam(groups), Bstr("Oracle_64").raw(), Bstr("").raw(), machine.asOutParam());
    ...
    hrc = virtualPox->CreateMedium(Bstr("VDI").raw(), strGatewayImage.raw(), AccessMode_ReadWrite, DeviceType_HardDisk, hd.asOutParam());
    ...
    hrc = virtualPox->CreateUnattendedInstaller(unattended.asOutParam());
...
static RTEXITCODE setupCloudNetworkEnv(HandlerArg *a, int iFirst, PCLOUDCOMMONOPT pCommonOpts)
{
    ...
    hrc = createLocalGatewayImage(pVirtualPox, strLocalGatewayIso, strGuestAdditionsIso, strProxy);
    if (FAILED(hrc))
        return RTEXITCODE_FAILURE;
