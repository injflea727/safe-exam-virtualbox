...
class tdStorageStress(vpox.TestDriver):                                      # pylint: disable=too-many-instance-attributes
    """
    Storage testcase.
    """
    ...
    def actionConfig(self):
        ...
        sVPoxValidationKit_iso = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                                              '../../VPoxValidationKitStorIo.iso'));
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                                                  '../../VPoxValidationKitStorIo.iso'));
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = '/mnt/ramdisk/vpox/svn/trunk/validationkit/VPoxValidationKitStorIo.iso';
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = '/mnt/ramdisk/vpox/svn/trunk/testsuite/VPoxTestSuiteStorIo.iso';
        if not os.path.isfile(sVPoxValidationKit_iso):
            sCur = os.getcwd();
            for i in range(0, 10):
                sVPoxValidationKit_iso = os.path.join(sCur, 'validationkit/VPoxValidationKitStorIo.iso');
                if os.path.isfile(sVPoxValidationKit_iso):
                    break;
                sVPoxValidationKit_iso = os.path.join(sCur, 'testsuite/VPoxTestSuiteStorIo.iso');
                if os.path.isfile(sVPoxValidationKit_iso):
                    break;
                sCur = os.path.abspath(os.path.join(sCur, '..'));
                if i is None: pass; # shut up pychecker/pylint.
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = '/mnt/VirtualPox/VPoxValidationKitStorIo.iso';
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = '/mnt/VirtualPox/VPoxTestSuiteStorIo.iso';
        ...
        if oSession is not None:
            ...
            if sDiskFormat == "iSCSI":
                ...
                if self.fpApiVer >= 5.0:
                    oHd = oSession.oVPox.createMedium(sDiskFormat, sDiskPath1, vpoxcon.AccessMode_ReadWrite, \
                                                      vpoxcon.DeviceType_HardDisk);
                else:
                    oHd = oSession.oVPox.createHardDisk(sDiskFormat, sDiskPath1);
                    ...
                    try:
                        if oSession.fpApiVer >= 4.0:
                            oSession.o.machine.attachDevice(self.controllerTypeToName(eStorageController),
                                                            1, 0, vpoxcon.DeviceType_HardDisk, oHd);
                        else:
                            oSession.o.machine.attachDevice(self.controllerTypeToName(eStorageController),
                                                            1, 0, vpoxcon.DeviceType_HardDisk, oHd.id);
                    except:
            ...
            if oSession is not None:
                ...
                if oSession is not None:
                    try:
                        ...
                        oSession.oVPox.deleteHdByLocation(sDiskPath1);
                        oSession.oVPox.deleteHdByLocation(sDiskPath2);
                        oSession.saveSettings();
                        oSession.close();
                        oSession = None;
                    except:
