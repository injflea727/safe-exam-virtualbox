...
/** @file
 * VirtualPox Linux host USB device enumeration.
 */
...
#ifdef VPOX_WITH_LINUX_COMPILER_H
 ...
 *
 * @returns VPox status code. pu8Port is set on success.
...
/** The value of the VPOX_USB environment variable. */
const char *s_pszTestEnvUsb;
/** The value of the VPOX_USB_ROOT environment variable. */
 ...
 * return test values. */
# define RTEnvGet(a) \
    (  !RTStrCmp(a, "VPOX_USB") ? s_pszTestEnvUsb \
     : !RTStrCmp(a, "VPOX_USB_ROOT") ? s_pszTestEnvUsbRoot \
     : NULL)
...
int USBProxyLinuxChooseMethod(bool *pfUsingUsbfsDevices, const char **ppszDevicesRoot)
{
     ...
     * The user may also specify a method and root using the VPOX_USB and
     * VPOX_USB_ROOT environment variables.  In this case we don't check
     * the root they provide for validity.
     */
    bool fUsbfsChosen = false;
    bool fSysfsChosen = false;
    const char *pszUsbFromEnv = RTEnvGet("VPOX_USB");
    const char *pszUsbRoot = NULL;
    if (pszUsbFromEnv)
    {
        bool fValidVPoxUSB = true;

        pszUsbRoot = RTEnvGet("VPOX_USB_ROOT");
        ...
        {
            LogRel(("Invalid VPOX_USB environment variable setting \"%s\"\n", pszUsbFromEnv));
            fValidVPoxUSB = false;
            pszUsbFromEnv = NULL;
        }
        if (!fValidVPoxUSB && pszUsbRoot)
            pszUsbRoot = NULL;
    }
    if (!pszUsbRoot)
    {
        if (   !fUsbfsChosen
            && USBProxyLinuxCheckDeviceRoot("/dev/vpoxusb", true))
        {
            fSysfsChosen = true;
            pszUsbRoot = "/dev/vpoxusb";
        }
    ...
    return pszUsbFromEnv ? VERR_NOT_FOUND
         : RTDirExists("/dev/vpoxusb") ? VERR_VUSB_USB_DEVICE_PERMISSION
