...
/** @file
 * VPox Qt GUI - UIMachineView class implementation.
 */
...
#endif /* VPOX_WS_WIN || VPOX_WS_X11 */
        display().NotifyScaleFactorChange(m_uScreenId,
                                          (uint32_t)(dScaleFactorFor3D * VPOX_OGL_SCALE_FACTOR_MULTIPLIER),
                                          (uint32_t)(dScaleFactorFor3D * VPOX_OGL_SCALE_FACTOR_MULTIPLIER));
...
void UIMachineView::sltHandleNotifyChange(int iWidth, int iHeight)
{
     ...
     * ourselves.  Windows guests should use VPoxVGA controllers, not VMSVGA. */
    if (   !isFullscreenOrSeamless()
        && uisession()->isGuestSupportsGraphics()
...
void UIMachineView::cleanupFrameBuffer()
{
     ...
     * Note: VPOX_WITH_CROGL additionally requires us to call DetachFramebuffer
...
#elif defined(VPOX_WS_WIN) || defined(VPOX_WS_X11)
