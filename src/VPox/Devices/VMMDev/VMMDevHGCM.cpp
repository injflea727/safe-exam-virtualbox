...
# include "dtrace/VPoxDD.h"
#else
# define VPOXDD_HGCMCALL_ENTER(a,b,c,d)             do { } while (0)
# define VPOXDD_HGCMCALL_COMPLETED_REQ(a,b)         do { } while (0)
# define VPOXDD_HGCMCALL_COMPLETED_EMT(a,b)         do { } while (0)
# define VPOXDD_HGCMCALL_COMPLETED_DONE(a,b,c,d)    do { } while (0)
#endif
...
*********************************************************************************************************************************/
typedef enum VPOXHGCMCMDTYPE
{
    VPOXHGCMCMDTYPE_LOADSTATE = 0,
    VPOXHGCMCMDTYPE_CONNECT,
    VPOXHGCMCMDTYPE_DISCONNECT,
    VPOXHGCMCMDTYPE_CALL,
    VPOXHGCMCMDTYPE_SizeHack = 0x7fffffff
} VPOXHGCMCMDTYPE;

/**
 * Information about a 32 or 64 bit parameter.
 */
typedef struct VPOXHGCMPARMVAL
{
 ...
 */
typedef struct VPOXHGCMPARMPTR
{
 ...
 *        into a bitfields like in VPOXHGCMPARMPAGES. */
    ...
    /** How the buffer should be copied VPOX_HGCM_F_PARM_*. */
 ...
 */
typedef struct VPOXHGCMGUESTPARM
{
    /** The parameter type. */
    ...
    union
    {
        VPOXHGCMPARMVAL       val;
        VPOXHGCMPARMPTR       ptr;
        VPOXHGCMPARMPAGES     Pages;
    } u;

} VPOXHGCMGUESTPARM;

typedef struct VPOXHGCMCMD
{
    /** Active commands, list is protected by critsectHGCMCmdList. */
    RTLISTNODE          node;

    /** The type of the command (VPOXHGCMCMDTYPE). */
    uint8_t             enmCmdType;
     ...
     * @note Only valid if VPOXGSTINFO2_F_REQUESTOR_INFO is set in
 ...
 */
typedef struct VPOXHGCMCMDCACHED
{
    VPOXHGCMCMD         Core;           /**< 120 */
    VPOXHGCMGUESTPARM   aGuestParms[6]; /**< 40 * 6 = 240 */
    VPOXHGCMSVCPARM     aHostParms[6];  /**< 24 * 6 = 144 */
} VPOXHGCMCMDCACHED;                    /**< 120+240+144 = 504 */
AssertCompile(sizeof(VPOXHGCMCMD) <= 120);
AssertCompile(sizeof(VPOXHGCMGUESTPARM) <= 40);
AssertCompile(sizeof(VPOXHGCMSVCPARM) <= 24);
AssertCompile(sizeof(VPOXHGCMCMDCACHED) <= 512);
AssertCompile(sizeof(VPOXHGCMCMDCACHED) > sizeof(VPOXHGCMCMD) + sizeof(HGCMServiceLocation));


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
DECLINLINE(void *) vmmdevR3HgcmCallMemAllocZ(PVMMDEVCC pThisCC, PVPOXHGCMCMD pCmd, size_t cbRequested);
...
/** Allocate and initialize VPOXHGCMCMD structure for HGCM request.
 *
 ...
 * @param   cParms          Number of HGCM parameters for VPOXHGCMCMDTYPE_CALL command.
 * @param   fRequestor      The VMMDevRequestHeader::fRequestor value.
 */
static PVPOXHGCMCMD vmmdevR3HgcmCmdAlloc(PVMMDEVCC pThisCC, VPOXHGCMCMDTYPE enmCmdType, RTGCPHYS GCPhys,
                                         uint32_t cbRequest, uint32_t cParms, uint32_t fRequestor)
{
     ...
     * Try use the cache.
     */
    VPOXHGCMCMDCACHED *pCmdCached;
    AssertCompile(sizeof(*pCmdCached) >= sizeof(VPOXHGCMCMD) + sizeof(HGCMServiceLocation));
    if (cParms <= RT_ELEMENTS(pCmdCached->aGuestParms))
    {
        if (sizeof(*pCmdCached) <= pThisCC->aHgcmAcc[idxHeapAcc].cbHeapBudget)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (enmCmdType == VPOXHGCMCMDTYPE_CALL)
                {
                ...
                else if (enmCmdType == VPOXHGCMCMDTYPE_CONNECT)
    ...
    const uint32_t cbCmd = sizeof(VPOXHGCMCMD) + cParms * (sizeof(VPOXHGCMGUESTPARM) + sizeof(VPOXHGCMSVCPARM))
                         + (enmCmdType == VPOXHGCMCMDTYPE_CONNECT ? sizeof(HGCMServiceLocation) : 0);
    if (cbCmd <= pThisCC->aHgcmAcc[idxHeapAcc].cbHeapBudget)
    {
        PVPOXHGCMCMD pCmd = (PVPOXHGCMCMD)RTMemAllocZ(cbCmd);
        if (pCmd)
        {
            ...
            if (enmCmdType == VPOXHGCMCMDTYPE_CALL)
            {
                ...
                if (cParms)
                {
                    pCmd->u.call.paGuestParms = (VPOXHGCMGUESTPARM *)((uint8_t *)pCmd
                                                                      + sizeof(struct VPOXHGCMCMD));
                    pCmd->u.call.paHostParms = (VPOXHGCMSVCPARM *)((uint8_t *)pCmd->u.call.paGuestParms
                                                                   + cParms * sizeof(VPOXHGCMGUESTPARM));
                }
            }
            else if (enmCmdType == VPOXHGCMCMDTYPE_CONNECT)
 ...
 */
static void vmmdevR3HgcmCmdFree(PPDMDEVINS pDevIns, PVMMDEV pThis, PVMMDEVCC pThisCC, PVPOXHGCMCMD pCmd)
{
    if (pCmd)
    {
        Assert(   pCmd->enmCmdType == VPOXHGCMCMDTYPE_CALL
               || pCmd->enmCmdType == VPOXHGCMCMDTYPE_CONNECT
               || pCmd->enmCmdType == VPOXHGCMCMDTYPE_DISCONNECT
               || pCmd->enmCmdType == VPOXHGCMCMDTYPE_LOADSTATE);
        if (pCmd->enmCmdType == VPOXHGCMCMDTYPE_CALL)
        {
            uint32_t i;
            for (i = 0; i < pCmd->u.call.cParms; ++i)
            {
                VPOXHGCMSVCPARM   * const pHostParm  = &pCmd->u.call.paHostParms[i];
                VPOXHGCMGUESTPARM * const pGuestParm = &pCmd->u.call.paGuestParms[i];
                ...
                {
                    Assert(pHostParm->type == VPOX_HGCM_SVC_PARM_PTR);
                ...
                {
                    Assert(pHostParm->type == VPOX_HGCM_SVC_PARM_PAGES);
                    ...
                    Assert(pHostParm->type != VPOX_HGCM_SVC_PARM_PTR && pHostParm->type != VPOX_HGCM_SVC_PARM_PAGES);
            }
        }
...
/** Add VPOXHGCMCMD to the list of pending commands.
 *
 ...
 */
static int vmmdevR3HgcmAddCommand(PPDMDEVINS pDevIns, PVMMDEV pThis, PVMMDEVCC pThisCC, PVPOXHGCMCMD pCmd)
{
    ...
    if (   pCmd->enmCmdType == VPOXHGCMCMDTYPE_CONNECT
        || pCmd->enmCmdType == VPOXHGCMCMDTYPE_DISCONNECT
        || pCmd->enmCmdType == VPOXHGCMCMDTYPE_CALL)
    {
...
/** Remove VPOXHGCMCMD from the list of pending commands.
 *
 * @returns VPox status code.
 * @param   pThisCC         The VMMDev ring-3 instance data.
 * @param   pCmd            Command to remove.
 */
static int vmmdevR3HgcmRemoveCommand(PVMMDEVCC pThisCC, PVPOXHGCMCMD pCmd)
{
    int rc = vmmdevR3HgcmCmdListLock(pThisCC);
    AssertRCReturn(rc, rc);
 ...
 */
DECLINLINE(PVPOXHGCMCMD) vmmdevR3HgcmFindCommandLocked(PVMMDEVCC pThisCC, RTGCPHYS GCPhys)
{
    PVPOXHGCMCMD pCmd;
    RTListForEach(&pThisCC->listHGCMCmd, pCmd, VPOXHGCMCMD, node)
    {
...
/** Copy VMMDevHGCMConnect request data from the guest to VPOXHGCMCMD command.
 *
 * @param   pHGCMConnect    The source guest request (cached in host memory).
 * @param   pCmd            Destination command.
 */
static void vmmdevR3HgcmConnectFetch(const VMMDevHGCMConnect *pHGCMConnect, PVPOXHGCMCMD pCmd)
{
...
{
    int rc;
    PVPOXHGCMCMD pCmd = vmmdevR3HgcmCmdAlloc(pThisCC, VPOXHGCMCMDTYPE_CONNECT, GCPhys, pHGCMConnect->header.header.size, 0,
...
/** Copy VMMDevHGCMDisconnect request data from the guest to VPOXHGCMCMD command.
 *
 * @param   pHGCMDisconnect The source guest request (cached in host memory).
 * @param   pCmd            Destination command.
 */
static void vmmdevR3HgcmDisconnectFetch(const VMMDevHGCMDisconnect *pHGCMDisconnect, PVPOXHGCMCMD pCmd)
{
...
{
    int rc;
    PVPOXHGCMCMD pCmd = vmmdevR3HgcmCmdAlloc(pThisCC, VPOXHGCMCMDTYPE_DISCONNECT, GCPhys, pHGCMDisconnect->header.header.size, 0,
 ...
 *
 * @returns VPOX_HGCM_F_PARM_DIRECTION_* flags.
...
{
    if (enmType == VMMDevHGCMParmType_LinAddr_In)  return VPOX_HGCM_F_PARM_DIRECTION_TO_HOST;
    if (enmType == VMMDevHGCMParmType_LinAddr_Out) return VPOX_HGCM_F_PARM_DIRECTION_FROM_HOST;
    return VPOX_HGCM_F_PARM_DIRECTION_BOTH;
}
 ...
 */
DECLINLINE(bool) vmmdevR3HgcmGuestBufferIsContiguous(const VPOXHGCMPARMPTR *pPtr)
{
    if (pPtr->cPages == 1)
        return true;
 ...
 */
static int vmmdevR3HgcmGuestBufferRead(PPDMDEVINSR3 pDevIns, void *pvDst, uint32_t cbDst, const VPOXHGCMPARMPTR *pPtr)
{
    /*
 ...
 */
static int vmmdevR3HgcmGuestBufferWrite(PPDMDEVINSR3 pDevIns, const VPOXHGCMPARMPTR *pPtr, const void *pvSrc,  uint32_t cbSrc)
{
    int rc = VINF_SUCCESS;
 ...
 *
 * @returns VPox status code that the guest should see.
 * @param   pDevIns         The device instance.
 * @param   pThisCC         The VMMDev ring-3 instance data.
 * @param   pCmd            Command structure where host parameters needs initialization.
 * @param   pbReq           The request buffer.
 */
static int vmmdevR3HgcmInitHostParameters(PPDMDEVINS pDevIns, PVMMDEVCC pThisCC, PVPOXHGCMCMD pCmd, uint8_t const *pbReq)
{
    AssertReturn(pCmd->enmCmdType == VPOXHGCMCMDTYPE_CALL, VERR_INTERNAL_ERROR);
    ...
    for (uint32_t i = 0; i < pCmd->u.call.cParms; ++i)
    {
        ...
        switch (pGuestParm->enmType)
        {
            ...
            {
                pHostParm->type = VPOX_HGCM_SVC_PARM_32BIT;
            ...
            {
                pHostParm->type = VPOX_HGCM_SVC_PARM_64BIT;
            ...
            case VMMDevHGCMParmType_ContiguousPageList:
            {
                ...
                pHostParm->type = VPOX_HGCM_SVC_PARM_PTR;
                ...
                if (cbData)
                {
                    ...
                    if (pGuestParm->u.ptr.fu32Direction & VPOX_HGCM_F_PARM_DIRECTION_TO_HOST)
                    {
            ...
            {
                pHostParm->type = VPOX_HGCM_SVC_PARM_PAGES;
...
/** Allocate and initialize VPOXHGCMCMD structure for a HGCMCall request.
 *
 * @returns VPox status code that the guest should see.
 * @param   pThisCC         The VMMDev ring-3 instance data.
...
static int vmmdevR3HgcmCallAlloc(PVMMDEVCC pThisCC, const VMMDevHGCMCall *pHGCMCall, uint32_t cbHGCMCall, RTGCPHYS GCPhys,
                                 VMMDevRequestType enmRequestType, PVPOXHGCMCMD *ppCmd, uint32_t *pcbHGCMParmStruct)
{
    ...
    PVPOXHGCMCMD pCmd = vmmdevR3HgcmCmdAlloc(pThisCC, VPOXHGCMCMDTYPE_CALL, GCPhys, cbHGCMCall, cParms,
 ...
 */
static void *vmmdevR3HgcmCallMemAllocEx(PVMMDEVCC pThisCC, PVPOXHGCMCMD pCmd, size_t cbRequested, bool fZero)
{
 ...
 */
DECLINLINE(void *) vmmdevR3HgcmCallMemAlloc(PVMMDEVCC pThisCC, PVPOXHGCMCMD pCmd, size_t cbRequested)
{
...
/** Copy VMMDevHGCMCall request data from the guest to VPOXHGCMCMD command.
 *
 * @returns VPox status code that the guest should see.
 * @param   pDevIns         The device instance.
 * @param   pThisCC         The VMMDev ring-3 instance data.
 ...
 */
static int vmmdevR3HgcmCallFetchGuestParms(PPDMDEVINS pDevIns, PVMMDEVCC pThisCC, PVPOXHGCMCMD pCmd,
                                           const VMMDevHGCMCall *pHGCMCall, uint32_t cbHGCMCall,
                                           VMMDevRequestType enmRequestType, uint32_t cbHGCMParmStruct)
{
    /*
     * Go over all guest parameters and initialize relevant VPOXHGCMCMD fields.
     * VPOXHGCMCMD must contain all information about the request,
...
/** @todo respect zero byte page lists...    */
                ...
                /* Flags. */
                ASSERT_GUEST_MSG_RETURN(VPOX_HGCM_F_PARM_ARE_VALID(pPageListInfo->flags),
                ...
                if (pGuestParm->enmType == VMMDevHGCMParmType_NoBouncePageList)
                {
                    ...
                    if (pPageListInfo->flags & VPOX_HGCM_F_PARM_DIRECTION_FROM_HOST)
            ...
            case VMMDevHGCMParmType_Embedded:
            {
                ...
                ASSERT_GUEST_MSG_RETURN(VPOX_HGCM_F_PARM_ARE_VALID(fFlags), ("%#x\n", fFlags), VERR_INVALID_FLAGS);
            ...
            if (RT_SUCCESS(rc))
            {
                    ...
                    &&  pCmd->u.call.cParms      == 5)
                {
                    ...
                    if (pCmd->pvReqLocked)
                    {
                        ...
                        pHeader->fu32Flags |= VPOX_HGCM_REQ_DONE;
                    }
                    else
                    {
...
int vmmdevR3HgcmCancel2(PVMMDEVCC pThisCC, RTGCPHYS GCPhys)
{
    ...
    PVPOXHGCMCMD pCmd = vmmdevR3HgcmFindCommandLocked(pThisCC, GCPhys);
    if (pCmd)
    {
        ...
        if (pThisCC->pHGCMDrv)
            pThisCC->pHGCMDrv->pfnCancelled(pThisCC->pHGCMDrv, pCmd,
                                            pCmd->enmCmdType == VPOXHGCMCMDTYPE_CALL ? pCmd->u.call.u32ClientID
                                            : pCmd->enmCmdType == VPOXHGCMCMDTYPE_CONNECT ? pCmd->u.connect.u32ClientID
                                            : pCmd->enmCmdType == VPOXHGCMCMDTYPE_DISCONNECT ? pCmd->u.disconnect.u32ClientID
                                            : 0);
    }
    else
        rc = VERR_NOT_FOUND;
 ...
 *                          embedded buffers.
 */
static int vmmdevR3HgcmCompleteCallRequest(PPDMDEVINS pDevIns, PVPOXHGCMCMD pCmd, VMMDevHGCMCall *pHGCMCall, uint8_t *pbReq)
{
    AssertReturn(pCmd->enmCmdType == VPOXHGCMCMDTYPE_CALL, VERR_INTERNAL_ERROR);
        ...
        )
    {
        ...
        switch (enmType)
        {
            ...
            case VMMDevHGCMParmType_64bit:
            {
                const VPOXHGCMPARMVAL * const pVal = &pGuestParm->u.val;
...
/** @todo Update the return buffer size? */
                const VPOXHGCMPARMPTR * const pPtr = &pGuestParm->u.ptr;
                if (   pPtr->cbData > 0
                    && (pPtr->fu32Direction & VPOX_HGCM_F_PARM_DIRECTION_FROM_HOST))
                {
 ...
 * @param   result          HGCM completion status code (VPox status code).
 * @param   pCmd            Completed command, which contains updated host parameters.
 *
 * @thread EMT
 */
static int hgcmCompletedWorker(PPDMIHGCMPORT pInterface, int32_t result, PVPOXHGCMCMD pCmd)
{
    ...
    if (result == VINF_HGCM_SAVE_STATE)
    {
         ...
         * VPOXHGCMCMD structures are not removed by vmmdevR3HgcmSaveState from the list,
    ...
    VPOXDD_HGCMCALL_COMPLETED_EMT(pCmd, result);
    ...
    if (RT_LIKELY(!pCmd->fCancelled))
    {
        ...
        else
        {
            ...
            /* Mark request as processed. */
            ASMAtomicOrU32(&pHeader->fu32Flags, VPOX_HGCM_REQ_DONE);
    ...
    VPOXDD_HGCMCALL_COMPLETED_DONE(pCmd, idFunction, idClient, result);
    vmmdevR3HgcmCmdFree(pDevIns, pThis, pThisCC, pCmd);
 ...
 * @param   pCmd            Completed command, which contains updated host parameters.
 */
DECLCALLBACK(int) hgcmR3Completed(PPDMIHGCMPORT pInterface, int32_t result, PVPOXHGCMCMD pCmd)
{
#if 0 /* This seems to be significantly slower.  Half of MsgTotal time seems to be spend here. */
    PVMMDEVCC pThisCC = RT_FROM_MEMBER(pInterface, VMMDEVCC, IHGCMPort);
    STAM_GET_TS(pCmd->tsComplete);

    VPOXDD_HGCMCALL_COMPLETED_REQ(pCmd, result);
 ...
 */
DECLCALLBACK(bool) hgcmR3IsCmdRestored(PPDMIHGCMPORT pInterface, PVPOXHGCMCMD pCmd)
{
    RT_NOREF(pInterface);
 ...
 */
DECLCALLBACK(bool) hgcmR3IsCmdCancelled(PPDMIHGCMPORT pInterface, PVPOXHGCMCMD pCmd)
{
    RT_NOREF(pInterface);
 ...
 */
DECLCALLBACK(uint32_t) hgcmR3GetRequestor(PPDMIHGCMPORT pInterface, PVPOXHGCMCMD pCmd)
{
    PVMMDEVCC pThisCC = RT_FROM_MEMBER(pInterface, VMMDEVCC, IHGCMPort);
    PVMMDEV   pThis   = PDMDEVINS_2_DATA(pThisCC->pDevIns, PVMMDEV);
    AssertPtrReturn(pCmd, VMMDEV_REQUESTOR_LOWEST);
    if (pThis->guestInfo2.fFeatures & VPOXGSTINFO2_F_REQUESTOR_INFO)
...
int vmmdevR3HgcmSaveState(PVMMDEVCC pThisCC, PSSMHANDLE pSSM)
{
    ...
    if (cCmds > 0)
    {
        RTListForEach(&pThisCC->listHGCMCmd, pCmd, VPOXHGCMCMD, node)
        {
            ...
            const uint32_t cParms = pCmd->enmCmdType == VPOXHGCMCMDTYPE_CALL ? pCmd->u.call.cParms : 0;
 ...
 *
 * Allocate VPOXHGCMCMD commands and add them to pThisCC->listHGCMCmd
...
int vmmdevR3HgcmLoadState(PPDMDEVINS pDevIns, PVMMDEV pThis, PVMMDEVCC pThisCC, PSSMHANDLE pSSM, uint32_t uVersion)
{
    ...
    if (uVersion >= VMMDEV_SAVED_STATE_VERSION_HGCM_PARAMS)
    {
        ...
        for (iCmd = 0; iCmd < cCmds; ++iCmd)
        {
            ...
            enmCmdType = (VPOXHGCMCMDTYPE)u32;
            ...
            PVPOXHGCMCMD pCmd = vmmdevR3HgcmCmdAlloc(pThisCC, enmCmdType, GCPhys, cbRequest, cParms, 0 /*fRequestor*/);
            ...
            if (enmCmdType == VPOXHGCMCMDTYPE_CALL)
            {
                ...
                for (i = 0; i < cParms; ++i)
                {
                    ...
                    if (   pGuestParm->enmType == VMMDevHGCMParmType_32bit
                        || pGuestParm->enmType == VMMDevHGCMParmType_64bit)
                    {
                        VPOXHGCMPARMVAL * const pVal = &pGuestParm->u.val;
                        ...
                        rc = SSMR3GetU32(pSSM, &pVal->cbValue);
                             ...
                             || pGuestParm->enmType == VMMDevHGCMParmType_ContiguousPageList)
                    {
                        VPOXHGCMPARMPTR * const pPtr = &pGuestParm->u.ptr;
            ...
            else if (enmCmdType == VPOXHGCMCMDTYPE_DISCONNECT)
            {
    ...
    else if (uVersion >= 9)
    {
        ...
        for (iCmd = 0; iCmd < cCmds; ++iCmd)
        {
            ...
            PVPOXHGCMCMD pCmd = vmmdevR3HgcmCmdAlloc(pThisCC, enmCmdType, GCPhys, cbRequest, cLinAddrs, 0 /*fRequestor*/);
            AssertReturn(pCmd, VERR_NO_MEMORY);
            ...
            if (cLinAddrs > 0)
            {
                ...
                {
                    VPOXHGCMPARMPTR * const pPtr = &pCmd->u.call.paGuestParms[i].u.ptr;
    ...
    else
    {
        ...
        for (iCmd = 0; iCmd < cCmds; ++iCmd)
        {
            ...
            PVPOXHGCMCMD pCmd = vmmdevR3HgcmCmdAlloc(pThisCC, VPOXHGCMCMDTYPE_LOADSTATE, GCPhys, cbRequest, 0, 0 /*fRequestor*/);
            AssertReturn(pCmd, VERR_NO_MEMORY);
 ...
 */
static int vmmdevR3HgcmRestoreConnect(PVMMDEVCC pThisCC, uint32_t uSavedStateVersion, const VPOXHGCMCMD *pLoadedCmd,
                                      VMMDevHGCMConnect *pReq, uint32_t cbReq, VMMDevRequestType enmRequestType,
                                      VPOXHGCMCMD **ppRestoredCmd)
{
    /* Verify the request.  */
    ASSERT_GUEST_RETURN(cbReq >= sizeof(*pReq), VERR_MISMATCH);
    if (uSavedStateVersion >= 9)
        ASSERT_GUEST_RETURN(pLoadedCmd->enmCmdType == VPOXHGCMCMDTYPE_CONNECT, VERR_MISMATCH);

    PVPOXHGCMCMD pCmd = vmmdevR3HgcmCmdAlloc(pThisCC, VPOXHGCMCMDTYPE_CONNECT, pLoadedCmd->GCPhys, cbReq, 0,
 ...
 * @param   ppRestoredCmd   Where to store pointer to newly allocated restored command.
 */
static int vmmdevR3HgcmRestoreDisconnect(PVMMDEVCC pThisCC, uint32_t uSavedStateVersion, const VPOXHGCMCMD *pLoadedCmd,
                                         ...
                                         VPOXHGCMCMD **ppRestoredCmd)
{
    ...
    ASSERT_GUEST_RETURN(cbReq >= sizeof(*pReq), VERR_MISMATCH);
    if (uSavedStateVersion >= 9)
        ASSERT_GUEST_RETURN(pLoadedCmd->enmCmdType == VPOXHGCMCMDTYPE_DISCONNECT, VERR_MISMATCH);

    PVPOXHGCMCMD pCmd = vmmdevR3HgcmCmdAlloc(pThisCC, VPOXHGCMCMDTYPE_DISCONNECT, pLoadedCmd->GCPhys, cbReq, 0,
                                             pReq->header.header.fRequestor);
    AssertReturn(pCmd, VERR_NO_MEMORY);
...
static int vmmdevR3HgcmRestoreCall(PPDMDEVINS pDevIns, PVMMDEV pThis, PVMMDEVCC pThisCC, uint32_t uSavedStateVersion,
                                   const VPOXHGCMCMD *pLoadedCmd, VMMDevHGCMCall *pReq, uint32_t cbReq,
                                   VMMDevRequestType enmRequestType, VPOXHGCMCMD **ppRestoredCmd)
{
    /* Verify the request.  */
    ASSERT_GUEST_RETURN(cbReq >= sizeof(*pReq), VERR_MISMATCH);
    if (uSavedStateVersion >= 9)
    {
        ASSERT_GUEST_RETURN(pLoadedCmd->enmCmdType == VPOXHGCMCMDTYPE_CALL, VERR_MISMATCH);
        Assert(pLoadedCmd->fCancelled == false);
    }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            VPOXHGCMGUESTPARM * const pLoadedParm = &pLoadedCmd->u.call.paGuestParms[iLinAddr];
            ...
            VPOXHGCMGUESTPARM * const pGuestParm = &pCmd->u.call.paGuestParms[iParm];
...
static int vmmdevR3HgcmRestoreCommand(PPDMDEVINS pDevIns, PVMMDEV pThis, PVMMDEVCC pThisCC, uint32_t uSavedStateVersion,
                                      const VPOXHGCMCMD *pLoadedCmd, const VMMDevHGCMRequestHeader *pReqHdr, uint32_t cbReq,
                                      VPOXHGCMCMD **ppRestoredCmd)
{
    int rc;
...
int vmmdevR3HgcmLoadStateDone(PPDMDEVINS pDevIns, PVMMDEV pThis, PVMMDEVCC pThisCC)
{
     ...
     *
     * Legacy commands have enmCmdType = VPOXHGCMCMDTYPE_LOADSTATE?
     */
    ...
    /* Resubmit commands. */
    PVPOXHGCMCMD pCmd, pNext;
    RTListForEachSafe(&listLoadedCommands, pCmd, pNext, VPOXHGCMCMD, node)
    {
        ...
        if (pThisCC->pHGCMDrv)
        {
            ...
            {
                PVPOXHGCMCMD pRestoredCmd = NULL;
            ...
            if (RT_SUCCESS(rcCmd))
            {
                switch (pCmd->enmCmdType)
                {
                    ...
                    case VPOXHGCMCMDTYPE_DISCONNECT:
                    {
                        vmmdevR3HgcmAddCommand(pDevIns, pThis, pThisCC, pCmd);
        ...
        else
        {
            ...
            pReqHdr->fu32Flags |= VPOX_HGCM_REQ_DONE;
...
void vmmdevR3HgcmDestroy(PPDMDEVINS pDevIns, PVMMDEV pThis, PVMMDEVCC pThisCC)
{
    ...
    {
        PVPOXHGCMCMD pCmd, pNext;
        RTListForEachSafe(&pThisCC->listHGCMCmd, pCmd, pNext, VPOXHGCMCMD, node)
        {
            vmmdevR3HgcmRemoveCommand(pThisCC, pCmd);
            vmmdevR3HgcmCmdFree(pDevIns, pThis, pThisCC, pCmd);
        }
 ...
 *
 * Keeps VPOXHGCMCMDCACHED and friends local.
 *
 * @returns VPox status code.
 * @param   pThisCC         The VMMDev ring-3 instance data.
 */
int vmmdevR3HgcmInit(PVMMDEVCC pThisCC)
{
    ...
    rc = RTMemCacheCreate(&pThisCC->hHgcmCmdCache, sizeof(VPOXHGCMCMDCACHED), 64, _1M, NULL, NULL, NULL, 0);
    AssertLogRelRCReturn(rc, rc);
