 ...
 *      generated code in out/.../src/VPox/Main/webservice/methodmaps.cpp
...
extern const unsigned char g_abVPoxWebWSDL[];
extern const unsigned g_cbVPoxWebWSDL;
...
static ComPtr<IVirtualPoxClient> g_pVirtualPoxClient = NULL;

// generated strings in methodmaps.cpp
extern const char       *g_pcszISession,
                        *g_pcszIVirtualPox,
                        *g_pcszIVirtualPoxErrorInfo;

// globals for vpoxweb command-line arguments
...
static const char       *g_pcszSID = "vpoxwebsrv";      // server ID for SSL session cache
#endif /* WITH_OPENSSL */
...
// this mutex protects the global VirtualPox reference below
static util::RWLockHandle       *g_pVirtualPoxLockHandle;

static ComPtr<IVirtualPox>      g_pVirtualPox = NULL;
...
public:
    ...
    static int fnHttpGet(struct soap *soap)
    {
        ...
        soap_send_raw(soap, (const char *)g_abVPoxWebWSDL, g_cbVPoxWebWSDL);
        soap_end_send(soap);
        return SOAP_OK;
    }
...
/****************************************************************************
 *
 * VirtualPoxClient event listener
 *
 ****************************************************************************/
    ...
    STDMETHOD(HandleEvent)(VPoxEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case VPoxEventType_OnVPoxSVCAvailabilityChanged:
            {
                ...
                if (!fAvailable)
                {
                    LogRel(("VPoxSVC became unavailable\n"));
                    {
                        util::AutoWriteLock vlock(g_pVirtualPoxLockHandle COMMA_LOCKVAL_SRC_POS);
                ...
                {
                    LogRel(("VPoxSVC became available\n"));
                    util::AutoWriteLock vlock(g_pVirtualPoxLockHandle COMMA_LOCKVAL_SRC_POS);
                    HRESULT hrc = g_pVirtualPoxClient->COMGETTER(VirtualPox)(g_pVirtualPox.asOutParam());
                    AssertComRC(hrc);
                }
                break;
            }
...
int main(int argc, char *argv[])
{
    ...
    RTStrmPrintf(g_pStdErr, VPOX_PRODUCT " web service Version " VPOX_VERSION_STRING "\n"
                            "(C) 2007-" VPOX_C_YEAR " " VPOX_VENDOR "\n"
                            "All rights reserved.\n");
    ...
    rc = com::VPoxLogRelCreate("web service", g_fDaemonize ? NULL : pszLogFile,
                               RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                               "all", "VPOXWEBSRV_RELEASE_LOG",
    ...
    if (g_fDaemonize)
    {
        ...
        {
            rc = com::GetVPoxUserHomeDirectory(szLogFile, sizeof(szLogFile));
            if (RT_FAILURE(rc))
                 return RTMsgErrorExit(RTEXITCODE_FAILURE, "could not get base directory for logging: %Rrc", rc);
            rc = RTPathAppend(szLogFile, sizeof(szLogFile), "vpoxwebsrv.log");
            if (RT_FAILURE(rc))
        ...
        rc = com::VPoxLogRelCreate("web service", pszLogFile,
                                   RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                                   "all", "VPOXWEBSRV_RELEASE_LOG",
    ...
    if (FAILED(hrc))
    {
        RTMsgError("failed to create the VirtualPoxClient object!");
        com::ErrorInfo info;
    ...
    hrc = g_pVirtualPoxClient->COMGETTER(VirtualPox)(g_pVirtualPox.asOutParam());
    if (FAILED(hrc))
    {
        RTMsgError("Failed to get VirtualPox object (rc=%Rhrc)!", hrc);
        return RTEXITCODE_FAILURE;
    }

    // set the authentication method if requested
    if (g_pVirtualPox && g_pcszAuthentication && g_pcszAuthentication[0])
    {
        ComPtr<ISystemProperties> pSystemProperties;
        g_pVirtualPox->COMGETTER(SystemProperties)(pSystemProperties.asOutParam());
    ...
    ComPtr<IEventListener> vpoxClientListener;
    {
        ...
        CHECK_ERROR(g_pVirtualPoxClient, COMGETTER(EventSource)(pES.asOutParam()));
        ComObjPtr<VirtualPoxClientEventListenerImpl> clientListener;
        clientListener.createObject();
        clientListener->init(new VirtualPoxClientEventListener());
        vpoxClientListener = clientListener;
    ...
    g_pVirtualPoxLockHandle = new util::RWLockHandle(util::LOCKCLASS_WEBSERVICE);
...
# endif
    ...
    /* VirtualPoxClient events unregistration. */
...
void RaiseSoapInvalidObjectFault(struct soap *soap,
                                 WSDLT_ID obj)
{
    _vpox__InvalidObjectFault *ex = soap_new__vpox__InvalidObjectFault(soap, 1);
    ex->badObjectID = obj;

    std::string str("VirtualPox error: ");
    str += "Invalid managed object reference \"" + obj + "\"";

    RaiseSoapFault(soap,
                   str.c_str(),
                   SOAP_TYPE__vpox__InvalidObjectFault,
                   ex);
}
                           ...
                           const com::Guid &iid)
{
    ...
    com::Utf8Str str = com::Utf8StrFmt("VirtualPox error: rc=%#lx", apirc);
    if (info.isFullAvailable() || info.isBasicAvailable())
    {
        const com::ErrorInfo *pInfo = &info;
        do
    ...
    _vpox__RuntimeFault *ex = soap_new__vpox__RuntimeFault(soap, 1);
    ComPtr<IVirtualPoxErrorInfo> pVirtualPoxErrorInfo;
    info.getVirtualPoxErrorInfo(pVirtualPoxErrorInfo);
    ex->resultCode = apirc;
    ex->returnval = createOrFindRefFromComPtr(idThis, g_pcszIVirtualPoxErrorInfo, pVirtualPoxErrorInfo);

    RaiseSoapFault(soap,
                   str.c_str(),
                   SOAP_TYPE__vpox__RuntimeFault,
                   ex);
}
                                    ...
                                    IVirtualPox **ppVirtualPox)
{
    ...
    {
        util::AutoReadLock vlock(g_pVirtualPoxLockHandle COMMA_LOCKVAL_SRC_POS);
        pVirtualPox = g_pVirtualPox;
    }
    if (pVirtualPox.isNull())
        return rc;
    pVirtualPox.queryInterfaceTo(ppVirtualPox);
    ...
    if (!fAuthLibLoaded)
    {
        ...
        pVirtualPox->COMGETTER(SystemProperties)(systemProperties.asOutParam());
 ...
 *  ComPtr<IVirtualPox>, for example. As we store the ComPtr<IUnknown> in
 ...
 *      findObjectFromRef() template function in vpoxweb.h
 ...
 *  createOrFindRefFromComPtr() template function in vpoxweb.h, which
 ...
 * @return
 */
int __vpox__IManagedObjectRef_USCOREgetInterfaceName(
    struct soap *soap,
    _vpox__IManagedObjectRef_USCOREgetInterfaceName *req,
    _vpox__IManagedObjectRef_USCOREgetInterfaceNameResponse *resp)
{
    RT_NOREF(soap);
    HRESULT rc = S_OK;
    WEBDEBUG(("-- entering %s\n", __FUNCTION__));
 ...
 * @return
 */
int __vpox__IManagedObjectRef_USCORErelease(
    struct soap *soap,
 ...
 *
 * This returns a managed object reference to the global IVirtualPox object; into this
 ...
 * @return
 */
int __vpox__IWebsessionManager_USCORElogon(
        struct soap *soap,
        ...
        _vpox__IWebsessionManager_USCORElogonResponse *resp)
{
    ...
    do
    {
                                        ...
                                        pVirtualPox.asOutParam())))
        {
            ...
            // global VirtualPox object; this encodes the websession ID in the MOR so
            // that it will be implicitly be included in all future requests of this
            // webservice client
            resp->returnval = createOrFindRefFromComPtr(id, g_pcszIVirtualPox, pVirtualPox);
            WEBDEBUG(("VirtualPox object ref is %s\n", resp->returnval.c_str()));
        }
        else
            rc = E_FAIL;
    } while (0);
 ...
 */
int __vpox__IWebsessionManager_USCOREgetSessionObject(
        struct soap*,
        ...
        _vpox__IWebsessionManager_USCOREgetSessionObjectResponse *resp)
{
    ...
    do
    {
        ...
        rc = g_pVirtualPoxClient->COMGETTER(Session)(pSession.asOutParam());
        if (FAILED(rc))
        {
        ...
        // return its MOR
        resp->returnval = createOrFindRefFromComPtr(req->refIVirtualPox, g_pcszISession, pSession);
 ...
 * @return
 */
int __vpox__IWebsessionManager_USCORElogoff(
        struct soap*,
        ...
        _vpox__IWebsessionManager_USCORElogoffResponse *resp)
{
...

    {
        ...
        WebServiceSession *pWebsession = WebServiceSession::findWebsessionFromRef(req->refIVirtualPox);
        if (pWebsession)
        {
