/* $Id: VPoxGuestR3LibClipboard.cpp $ */
/** @file
 * VPoxGuestR3Lib - Ring-3 Support Library for VirtualPox guest additions, Shared Clipboard.
 */
...
{
    int rc = VbglR3HGCMConnect("VPoxSharedClipboard", pidClient);
    if (RT_FAILURE(rc))
    {
...
VBGLR3DECL(int) VbglR3ClipboardConnectEx(PVBGLR3SHCLCMDCTX pCtx, uint64_t fGuestFeatures)
{
...
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
    pCtx->fGuestFeatures       |= VPOX_SHCL_GF_0_TRANSFERS;
    pCtx->cbChunkSize           = VPOX_SHCL_DEFAULT_CHUNK_SIZE; /** @todo Make this configurable. */
    pCtx->cbMaxChunkSize        = VPOX_SHCL_MAX_CHUNK_SIZE;     /** @todo Ditto. */
#endif
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (   (pCtx->fHostFeatures & VPOX_SHCL_HF_0_CONTEXT_ID)
                && (pCtx->fGuestFeatures & VPOX_SHCL_GF_0_CONTEXT_ID) )
            {
                pCtx->fUseLegacyProtocol = false;

#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
                if (   (pCtx->fHostFeatures & VPOX_SHCL_HF_0_TRANSFERS)
                    && (pCtx->fGuestFeatures & VPOX_SHCL_GF_0_TRANSFERS) )
                {
                    VPoxShClParmNegotiateChunkSize MsgChunkSize;
                    do
                    {
                        VBGL_HGCM_HDR_INIT(&MsgChunkSize.hdr, pCtx->idClient, VPOX_SHCL_GUEST_FN_NEGOTIATE_CHUNK_SIZE,
                                           VPOX_SHCL_CPARMS_NEGOTIATE_CHUNK_SIZE);
 ...
 * @param   fGuestFeatures  Features to report, VPOX_SHCL_GF_XXX.
 * @param   pfHostFeatures  Where to store the features VPOX_SHCL_HF_XXX.
 */
VBGLR3DECL(int) VbglR3ClipboardReportFeatures(uint32_t idClient, uint64_t fGuestFeatures, uint64_t *pfHostFeatures)
{
    ...
    do
    {
        ...
        VBGL_HGCM_HDR_INIT(&Msg.Hdr, idClient, VPOX_SHCL_GUEST_FN_REPORT_FEATURES, 2);
        VbglHGCMParmUInt64Set(&Msg.f64Features0, fGuestFeatures);
        VbglHGCMParmUInt64Set(&Msg.f64Features1, VPOX_SHCL_GF_1_MUST_BE_ONE);
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (Msg.f64Features1.u.value64 & VPOX_SHCL_GF_1_MUST_BE_ONE)
...
static int vbglR3ClipboardFormatsReportRecv(PVBGLR3SHCLCMDCTX pCtx, PSHCLFORMATS pfFormats)
{
    ...
    *pfFormats = 0;
    ...
    VBGL_HGCM_HDR_INIT(&Msg.Hdr, pCtx->idClient, VPOX_SHCL_GUEST_FN_MSG_GET, 2);
    Msg.id64Context.SetUInt32(VPOX_SHCL_HOST_MSG_FORMATS_REPORT);
...
/**
 * Fetches a VPOX_SHCL_HOST_MSG_READ_DATA_CID message.
 *
 * @returns VPox status code.
 * @param   pCtx                Shared Clipboard command context to use for the connection.
...
static int vbglR3ClipboardFetchReadDataCid(PVBGLR3SHCLCMDCTX pCtx, PSHCLFORMAT pfFormat)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.Hdr, pCtx->idClient, VPOX_SHCL_GUEST_FN_MSG_GET, 2);
    Msg.id64Context.SetUInt64(VPOX_SHCL_HOST_MSG_READ_DATA_CID);
...
/**
 * Fetches a VPOX_SHCL_HOST_MSG_READ_DATA message.
 *
 * @returns VPox status code.
 * @param   pCtx                Shared Clipboard command context to use for the connection.
 * @param   pfFormat            Where to return the requested format.
...
static int vbglR3ClipboardFetchReadData(PVBGLR3SHCLCMDCTX pCtx, PSHCLFORMAT pfFormat)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.Hdr, pCtx->idClient, VPOX_SHCL_GUEST_FN_MSG_GET, 2);
    Msg.id32Msg.SetUInt32(VPOX_SHCL_HOST_MSG_READ_DATA);
    Msg.f32Format.SetUInt32(0);
...
/**
 * Get a host message, legacy version (which does not have VPOX_SHCL_GUEST_FN_MSG_GET). Do not use anymore.
 *
 * Note: This is the old message which still is being used for the non-URI Shared Clipboard transfers,
 *       to not break compatibility with older additions / VPox versions.
 *
...
{
    VPoxShClGetHostMsgOld Msg;

    VBGL_HGCM_HDR_INIT(&Msg.hdr, idClient, VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT, VPOX_SHCL_CPARMS_GET_HOST_MSG_OLD);
                                        ...
                                        uint32_t *pcbRead)
{
    ...
    {
        VBGLIOCHGCMCALL      Hdr;
        VPoxShClParmDataRead Parms;
    } Msg;

    VBGL_HGCM_HDR_INIT(&Msg.Hdr, idClient, VPOX_SHCL_GUEST_FN_DATA_READ, VPOX_SHCL_CPARMS_DATA_READ);
 ...
 * @returns VPox status code.
 * @param   idClient        The client ID returned by VbglR3ClipboardConnect().
 * @param   pfHostFeatures  Where to store the host feature, VPOX_SHCL_HF_XXX.
 */
VBGLR3DECL(int) VbglR3ClipboardQueryFeatures(uint32_t idClient, uint64_t *pfHostFeatures)
{
    ...
    do
    {
        ...
        VBGL_HGCM_HDR_INIT(&Msg.Hdr, idClient, VPOX_SHCL_GUEST_FN_QUERY_FEATURES, 2);
                                           ...
                                           uint32_t *pcParameters, uint64_t *pidRestoreCheck)
{
    ...
    {
        VBGL_HGCM_HDR_INIT(&Msg.Hdr, pCtx->idClient, VPOX_SHCL_GUEST_FN_MSG_PEEK_WAIT, 2);
    ...
    {
        VBGL_HGCM_HDR_INIT(&Msg.Hdr, pCtx->idClient, VPOX_SHCL_GUEST_FN_MSG_CANCEL, 0);
...
static int vbglR3ClipboardRootListHdrRead(PVBGLR3SHCLCMDCTX pCtx, PSHCLROOTLISTHDR pRootListHdr)
{
    ...
    VPoxShClRootListHdrMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_ROOT_LIST_HDR_READ, VPOX_SHCL_CPARMS_ROOT_LIST_HDR_READ);
...
static int vbglR3ClipboardRootListEntryRead(PVBGLR3SHCLCMDCTX pCtx, uint32_t uIndex, PSHCLROOTLISTENTRY pRootListEntry)
{
    ...
    VPoxShClRootListEntryMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_ROOT_LIST_ENTRY_READ, VPOX_SHCL_CPARMS_ROOT_LIST_ENTRY_READ);
                                                 ...
                                                 PSHCLTRANSFERDIR pEnmDir, PSHCLTRANSFERREPORT pReport)
{
    ...
    VPoxShClTransferStatusMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_TRANSFER_STATUS);

    Msg.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_STATUS);
                                                   ...
                                                   SHCLTRANSFERSTATUS uStatus, int rcTransfer)
{
    ...
    VPoxShClReplyMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_REPLY, VPOX_SHCL_CPARMS_REPLY_MIN + 1);

    Msg.uContext.SetUInt64(pCtx->idContext);
    Msg.enmType.SetUInt32(VPOX_SHCL_REPLYMSGTYPE_TRANSFER_STATUS);
...
VBGLR3DECL(int) VbglR3ClipboardRootListHdrReadReq(PVBGLR3SHCLCMDCTX pCtx, uint32_t *pfRoots)
{
    ...
    VPoxShClRootListReadReqMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_ROOT_LIST_HDR_READ_REQ);

    Msg.ReqParms.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_HDR_READ);
    Msg.ReqParms.fRoots.SetUInt32(0);
...
VBGLR3DECL(int) VbglR3ClipboardRootListHdrReadReply(PVBGLR3SHCLCMDCTX pCtx, PSHCLROOTLISTHDR pRootListHdr)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_ROOT_LIST_HDR_WRITE, VPOX_SHCL_CPARMS_ROOT_LIST_HDR_WRITE);
...
VBGLR3DECL(int) VbglR3ClipboardRootListEntryReadReq(PVBGLR3SHCLCMDCTX pCtx, uint32_t *puIndex, uint32_t *pfInfo)
{
    ...
    VPoxShClRootListEntryReadReqMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_ROOT_LIST_ENTRY_READ_REQ);

    Msg.Parms.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_ENTRY_READ);
...
VBGLR3DECL(int) VbglR3ClipboardRootListEntryReadReply(PVBGLR3SHCLCMDCTX pCtx, uint32_t uIndex, PSHCLROOTLISTENTRY pEntry)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_ROOT_LIST_ENTRY_WRITE, VPOX_SHCL_CPARMS_ROOT_LIST_ENTRY_WRITE);
                                            ...
                                            PSHCLLISTHANDLE phList)
{
    ...
    VPoxShClListOpenMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_LIST_OPEN, VPOX_SHCL_CPARMS_LIST_OPEN);
...
VBGLR3DECL(int) VbglR3ClipboardListOpenRecv(PVBGLR3SHCLCMDCTX pCtx, PSHCLLISTOPENPARMS pOpenParms)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_LIST_OPEN);

    Msg.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_OPEN);
    Msg.fList.SetUInt32(0);
    Msg.cbPath.SetUInt32(pOpenParms->cbPath);
    Msg.pvPath.SetPtr(pOpenParms->pszPath, pOpenParms->cbPath);
    Msg.cbFilter.SetUInt32(pOpenParms->cbFilter);
...
VBGLR3DECL(int) VbglR3ClipboardListOpenReply(PVBGLR3SHCLCMDCTX pCtx, int rcReply, SHCLLISTHANDLE hList)
{
    ...
    Msg.uContext.SetUInt64(pCtx->idContext);
    Msg.enmType.SetUInt32(VPOX_SHCL_REPLYMSGTYPE_LIST_OPEN);
...
VBGLR3DECL(int) VbglR3ClipboardListCloseRecv(PVBGLR3SHCLCMDCTX pCtx, PSHCLLISTHANDLE phList)
{
    ...
    VPoxShClListCloseMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_LIST_CLOSE);

    Msg.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_CLOSE);
    Msg.uHandle.SetUInt64(0);
...
VBGLR3DECL(int) VbglR3ClipboardListCloseReply(PVBGLR3SHCLCMDCTX pCtx, int rcReply, SHCLLISTHANDLE hList)
{
    ...
    Msg.uContext.SetUInt64(pCtx->idContext);
    Msg.enmType.SetUInt32(VPOX_SHCL_REPLYMSGTYPE_LIST_CLOSE);
    Msg.rc.SetUInt32((uint32_t)rcReply); /** int vs. uint32_t */
    Msg.cbPayload.SetUInt32(0);
    Msg.pvPayload.SetPtr(NULL, 0);
...
VBGLR3DECL(int) VbglR3ClipboardListCloseSend(PVBGLR3SHCLCMDCTX pCtx, SHCLLISTHANDLE hList)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_LIST_CLOSE, VPOX_SHCL_CPARMS_LIST_CLOSE);
                                           ...
                                           PSHCLLISTHDR pListHdr)
{
    ...
    VPoxShClListHdrMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_LIST_HDR_READ, VPOX_SHCL_CPARMS_LIST_HDR);
...
VBGLR3DECL(int) VbglR3ClipboardListHdrReadRecvReq(PVBGLR3SHCLCMDCTX pCtx, PSHCLLISTHANDLE phList, uint32_t *pfFlags)
{
    ...
    VPoxShClListHdrReadReqMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_LIST_HDR_READ_REQ);

    Msg.ReqParms.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_HDR_READ);
                                            ...
                                            PSHCLLISTHDR pListHdr)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_LIST_HDR_WRITE, VPOX_SHCL_CPARMS_LIST_HDR);
                                             ...
                                             PSHCLLISTENTRY pListEntry)
{
    ...
    VPoxShClListEntryMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_LIST_ENTRY_READ, VPOX_SHCL_CPARMS_LIST_ENTRY);
...
VBGLR3DECL(int) VbglR3ClipboardListEntryReadRecvReq(PVBGLR3SHCLCMDCTX pCtx, PSHCLLISTHANDLE phList, uint32_t *pfInfo)
{
    ...
    VPoxShClListEntryReadReqMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_LIST_ENTRY_READ);

    Msg.ReqParms.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_LIST_ENTRY_READ);
    Msg.ReqParms.uHandle.SetUInt64(0);
    Msg.ReqParms.fInfo.SetUInt32(0);
                                              ...
                                              PSHCLLISTENTRY pListEntry)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_LIST_ENTRY_WRITE, VPOX_SHCL_CPARMS_LIST_ENTRY);
...
VBGLR3DECL(int) VbglR3ClipboardObjOpenRecv(PVBGLR3SHCLCMDCTX pCtx, PSHCLOBJOPENCREATEPARMS pCreateParms)
{
    ...
    VPoxShClObjOpenMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_OBJ_OPEN);

    Msg.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_OPEN);
    Msg.uHandle.SetUInt64(0);
...
VBGLR3DECL(int) VbglR3ClipboardObjOpenReply(PVBGLR3SHCLCMDCTX pCtx, int rcReply, SHCLOBJHANDLE hObj)
{
    ...
    Msg.uContext.SetUInt64(pCtx->idContext);
    Msg.enmType.SetUInt32(VPOX_SHCL_REPLYMSGTYPE_OBJ_OPEN);
    Msg.rc.SetUInt32((uint32_t)rcReply); /** int vs. uint32_t */
    Msg.cbPayload.SetUInt32(0);
    Msg.pvPayload.SetPtr(NULL, 0);
                                           ...
                                           PSHCLOBJHANDLE phObj)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_OBJ_OPEN, VPOX_SHCL_CPARMS_OBJ_OPEN);
...
VBGLR3DECL(int) VbglR3ClipboardObjCloseRecv(PVBGLR3SHCLCMDCTX pCtx, PSHCLOBJHANDLE phObj)
{
    ...
    VPoxShClObjCloseMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_OBJ_CLOSE);

    Msg.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_CLOSE);
    Msg.uHandle.SetUInt64(0);
...
VBGLR3DECL(int) VbglR3ClipboardObjCloseReply(PVBGLR3SHCLCMDCTX pCtx, int rcReply, SHCLOBJHANDLE hObj)
{
    ...
    Msg.uContext.SetUInt64(pCtx->idContext);
    Msg.enmType.SetUInt32(VPOX_SHCL_REPLYMSGTYPE_OBJ_CLOSE);
    Msg.rc.SetUInt32((uint32_t)rcReply); /** int vs. uint32_t */
    Msg.cbPayload.SetUInt32(0);
    Msg.pvPayload.SetPtr(NULL, 0);
...
VBGLR3DECL(int) VbglR3ClipboardObjCloseSend(PVBGLR3SHCLCMDCTX pCtx, SHCLOBJHANDLE hObj)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_OBJ_CLOSE, VPOX_SHCL_CPARMS_OBJ_CLOSE);
                                           ...
                                           uint32_t *pfFlags)
{
    ...
    VPoxShClObjReadReqMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_MSG_GET, VPOX_SHCL_CPARMS_OBJ_READ_REQ);

    Msg.ReqParms.uContext.SetUInt64(VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_READ);
    Msg.ReqParms.uHandle.SetUInt64(0);
                                           ...
                                           void *pvData, uint32_t cbData, uint32_t *pcbRead)
{
    ...
    VPoxShClObjReadWriteMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_OBJ_READ, VPOX_SHCL_CPARMS_OBJ_READ);
                                            ...
                                            void *pvData, uint32_t cbData, uint32_t *pcbWritten)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pCtx->idClient,
                       VPOX_SHCL_GUEST_FN_OBJ_WRITE, VPOX_SHCL_CPARMS_OBJ_WRITE);
 ...
 *
 * @returns VPox status code, or VERR_NOT_FOUND if transfer has not been found.
 * @param   pCmdCtx             Command context to use.
                                              ...
                                              PVBGLR3CLIPBOARDEVENT pEvent)
{
    ...
    if (!pCmdCtx->fUseLegacyProtocol)
    {
        ...
        switch (idMsg)
        {
            case VPOX_SHCL_HOST_MSG_TRANSFER_STATUS:
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    const SHCLTRANSFERID uTransferID = VPOX_SHCL_CONTEXTID_GET_TRANSFER(pCmdCtx->idContext);
                    ...
                    switch (transferReport.uStatus)
                    {
                        ...
                        case SHCLTRANSFERSTATUS_ERROR:
                        {
                            rc = vbglR3ClipboardTransferStop(pCmdCtx, pTransferCtx,
                                                             VPOX_SHCL_CONTEXTID_GET_TRANSFER(pCmdCtx->idContext));
                            break;
                        }
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        pEvent->u.TransferStatus.uID    = VPOX_SHCL_CONTEXTID_GET_TRANSFER(pCmdCtx->idContext);
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_HDR_READ:
            {
                uint32_t fRoots;
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_ROOT_LIST_ENTRY_READ:
            {
                uint32_t uIndex;
                uint32_t fInfo;
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_LIST_OPEN:
            {
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_LIST_CLOSE:
            {
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_LIST_HDR_READ:
            {
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_LIST_ENTRY_READ:
            {
                LogFlowFunc(("VPOX_SHCL_HOST_MSG_TRANSFER_LIST_ENTRY_READ\n"));
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_OPEN:
            {
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_CLOSE:
            {
                SHCLOBJHANDLE hObj;
            ...
            case VPOX_SHCL_HOST_MSG_TRANSFER_OBJ_READ:
            {
                SHCLOBJHANDLE hObj;
                uint32_t      cbBuf;
                uint32_t      fFlags;
...
VBGLR3DECL(int) VbglR3ClipboardEventGetNext(uint32_t idMsg, uint32_t cParms, PVBGLR3SHCLCMDCTX pCtx, PVBGLR3CLIPBOARDEVENT pEvent)
{
    ...
    if (!pCtx->fUseLegacyProtocol)
    {
        ...
        switch (idMsg)
        {
            ...
            case VPOX_SHCL_HOST_MSG_READ_DATA_CID:
            {
...
VBGLR3DECL(int) VbglR3ClipboardReportFormats(HGCMCLIENTID idClient, uint32_t fFormats)
{
    ...
    {
        VBGLIOCHGCMCALL             Hdr;
        VPoxShClParmReportFormats   Parms;
    } Msg;

    VBGL_HGCM_HDR_INIT(&Msg.Hdr, idClient, VPOX_SHCL_GUEST_FN_REPORT_FORMATS, VPOX_SHCL_CPARMS_REPORT_FORMATS);
 ...
 *
 * This is usually called in reply to a VPOX_SHCL_HOST_MSG_READ_DATA message
 * from the host.
 *
 * @returns VPox status code.
 * @param   idClient    The client id returned by VbglR3ClipboardConnect().
...
VBGLR3DECL(int) VbglR3ClipboardWriteData(HGCMCLIENTID idClient, uint32_t fFormat, void *pv, uint32_t cb)
{
    ...
    {
        VBGLIOCHGCMCALL             Hdr;
        VPoxShClParmDataWriteOld    Parms;
    } Msg;

    VBGL_HGCM_HDR_INIT(&Msg.Hdr, idClient, VPOX_SHCL_GUEST_FN_DATA_WRITE, VPOX_SHCL_CPARMS_DATA_WRITE_OLD);
    VbglHGCMParmUInt32Set(&Msg.Parms.f32Format, fFormat);
...
VBGLR3DECL(int) VbglR3ClipboardWriteDataEx(PVBGLR3SHCLCMDCTX pCtx, SHCLFORMAT fFormat, void *pvData, uint32_t cbData)
{
    ...
    else
    {
        ...
        {
            VBGLIOCHGCMCALL         Hdr;
            VPoxShClParmDataWrite   Parms;
        } Msg;

        VBGL_HGCM_HDR_INIT(&Msg.Hdr, pCtx->idClient, VPOX_SHCL_GUEST_FN_DATA_WRITE, VPOX_SHCL_CPARMS_DATA_WRITE);
...
VBGLR3DECL(int) VbglR3ClipboardWriteError(HGCMCLIENTID idClient, int rcErr)
{
    ...
    VPoxShClWriteErrorMsg Msg;
    RT_ZERO(Msg);

    VBGL_HGCM_HDR_INIT(&Msg.hdr, idClient, VPOX_SHCL_GUEST_FN_ERROR, VPOX_SHCL_CPARMS_ERROR);
