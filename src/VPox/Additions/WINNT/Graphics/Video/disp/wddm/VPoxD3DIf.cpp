...
{
    static const VPOXWDDMDISP_TSS_LOOKUP lookup[] =
    {
...
HRESULT VPoxD3DIfLockRect(PVPOXWDDMDISP_RESOURCE pRc, UINT iAlloc,
    ...
    switch (pRc->aAllocations[0].enmD3DIfType)
    {
        ...
        case VPOXDISP_D3DIFTYPE_CUBE_TEXTURE:
        {
            ...
            hr = pD3DIfCubeTex->LockRect(VPOXDISP_CUBEMAP_INDEX_TO_FACE(pRc, iAlloc),
                    VPOXDISP_CUBEMAP_INDEX_TO_LEVEL(pRc, iAlloc), pLockedRect, pRect, fLockFlags);
            Assert(hr == S_OK);
            break;
        }
...
HRESULT VPoxD3DIfUnlockRect(PVPOXWDDMDISP_RESOURCE pRc, UINT iAlloc)
{
    HRESULT hr = S_OK;
    Assert(pRc->cAllocations > iAlloc);
    switch (pRc->aAllocations[0].enmD3DIfType)
