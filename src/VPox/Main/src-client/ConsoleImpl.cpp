/* $Id: ConsoleImpl.cpp $ */
/** @file
 * VPox Console COM Class implementation
 */
...
#include <VPox/vmm/pdmasynccompletion.h>
#include <VPox/vmm/pdmnetifs.h>
#include <VPox/vmm/pdmstorageifs.h>
#ifdef VPOX_WITH_USB
# include <VPox/vmm/pdmusb.h>
#endif
#ifdef VPOX_WITH_NETSHAPER
# include <VPox/vmm/pdmnetshaper.h>
#endif /* VPOX_WITH_NETSHAPER */
...
public:
    ...
    STDMETHOD(HandleEvent)(VPoxEventType_T aType, IEvent *aEvent)
    {
        switch(aType)
        {
            case VPoxEventType_OnNATRedirect:
            {
                Bstr id;
            ...
            case VPoxEventType_OnHostNameResolutionConfigurationChange:
            {
                mConsole->i_onNATDnsChanged();
                break;
            }

            case VPoxEventType_OnHostPCIDevicePlug:
            {
                // handle if needed
                break;
            }

            case VPoxEventType_OnExtraDataChanged:
            {
...
VPOX_LISTENER_DECLARE(VmEventListenerImpl)
...
HRESULT Console::init(IMachine *aMachine, IInternalMachineControl *aControl, LockType_T aLockType)
{
    ...
    rc = mptrExtPackManager->initExtPackManager(NULL, VPOXEXTPACKCTX_VM_PROCESS);
        AssertComRCReturnRC(rc);
#endif
    ...
    if (aLockType == LockType_VM)
    {
        ...
        rc = aMachine->COMGETTER(Parent)(pVirtualPox.asOutParam());
        AssertComRC(rc);
        ComPtr<ISystemProperties> pSystemProperties;
        if (pVirtualPox)
            pVirtualPox->COMGETTER(SystemProperties)(pSystemProperties.asOutParam());
        ...
        /* VirtualPox events registration. */
        {
            ComPtr<IEventSource> pES;
            rc = pVirtualPox->COMGETTER(EventSource)(pES.asOutParam());
            AssertComRC(rc);
            ...
            eventTypes.push_back(VPoxEventType_OnNATRedirect);
            eventTypes.push_back(VPoxEventType_OnHostNameResolutionConfigurationChange);
            eventTypes.push_back(VPoxEventType_OnHostPCIDevicePlug);
            eventTypes.push_back(VPoxEventType_OnExtraDataChanged);
...
void Console::uninit()
{
    ...
    {
        ComPtr<IVirtualPox> pVirtualPox;
        HRESULT rc = mMachine->COMGETTER(Parent)(pVirtualPox.asOutParam());
        AssertComRC(rc);
        if (SUCCEEDED(rc) && !pVirtualPox.isNull())
            stopGateways(pVirtualPox, mGateways);
    }
 ...
 *       service to do this instead of detouring thru VPoxSVC.
 *       (IMachine::SetGuestProperty ends up in VPoxSVC, which in turns calls
...
{
    Bstr value;
    HRESULT hrc = mMachine->GetExtraData(Bstr("VPoxInternal2/EnableGuestPropertiesVRDP").raw(),
                                         value.asOutParam());
    if (   hrc   == S_OK
        && value == "1")
        return true;
...
void Console::i_guestPropertiesVRDPUpdateLogon(uint32_t u32ClientId, const char *pszUser, const char *pszDomain)
{
    ...
    RTStrPrintf(szPropNm, sizeof(szPropNm), "/VirtualPox/HostInfo/VRDP/Client/%u/Name", u32ClientId);
    Bstr clientName;
    ...
    RTStrPrintf(szPropNm, sizeof(szPropNm), "/VirtualPox/HostInfo/VRDP/Client/%u/User", u32ClientId);
    mMachine->SetGuestProperty(Bstr(szPropNm).raw(),
                               Bstr(pszUser).raw(),
                               bstrReadOnlyGuest.raw());

    RTStrPrintf(szPropNm, sizeof(szPropNm), "/VirtualPox/HostInfo/VRDP/Client/%u/Domain", u32ClientId);
    mMachine->SetGuestProperty(Bstr(szPropNm).raw(),
                               Bstr(pszDomain).raw(),
                               bstrReadOnlyGuest.raw());
    ...
    mMachine->SetGuestProperty(Bstr("/VirtualPox/HostInfo/VRDP/LastConnectedClient").raw(),
                               Bstr(szClientId).raw(),
                               bstrReadOnlyGuest.raw());
...
void Console::i_guestPropertiesVRDPUpdateActiveClient(uint32_t u32ClientId)
{
    ...
    mMachine->SetGuestProperty(Bstr("/VirtualPox/HostInfo/VRDP/ActiveClient").raw(),
                               Bstr(szClientId).raw(),
                               bstrFlags.raw());
...
void Console::i_guestPropertiesVRDPUpdateIPAddrChange(uint32_t u32ClientId, const char *pszIPAddr)
{
    ...
    RTStrPrintf(szPropNm, sizeof(szPropNm), "/VirtualPox/HostInfo/VRDP/Client/%u/IPAddr", u32ClientId);
...
void Console::i_guestPropertiesVRDPUpdateLocationChange(uint32_t u32ClientId, const char *pszLocation)
{
    ...
    RTStrPrintf(szPropNm, sizeof(szPropNm), "/VirtualPox/HostInfo/VRDP/Client/%u/Location", u32ClientId);
...
void Console::i_guestPropertiesVRDPUpdateOtherInfoChange(uint32_t u32ClientId, const char *pszOtherInfo)
{
    ...
    RTStrPrintf(szPropNm, sizeof(szPropNm), "/VirtualPox/HostInfo/VRDP/Client/%u/OtherInfo", u32ClientId);
...
void Console::i_guestPropertiesVRDPUpdateClientAttach(uint32_t u32ClientId, bool fAttached)
{
    ...
    char szPropNm[256];
    RTStrPrintf(szPropNm, sizeof(szPropNm), "/VirtualPox/HostInfo/VRDP/Client/%u/Attach", u32ClientId);
...
void Console::i_guestPropertiesVRDPUpdateDisconnect(uint32_t u32ClientId)
{
    ...
    mMachine->SetGuestProperty(Bstr("/VirtualPox/HostInfo/VRDP/LastDisconnectedClient").raw(),
                               Bstr(szClientId).raw(),
                               bstrReadOnlyGuest.raw());
...
int Console::i_VRDPClientLogon(uint32_t u32ClientId, const char *pszUser, const char *pszPassword, const char *pszDomain)
{
    ...
    if (SUCCEEDED(hrc) && value == "1")
    {
        ...
        hrc = i_getGuestProperty("/VirtualPox/GuestInfo/OS/NoLoggedInUsers",
...
HRESULT Console::i_loadDataFromSavedState()
{
    ...
    if (RT_FAILURE(vrc))
        rc = setErrorBoth(VPOX_E_FILE_ERROR, vrc,
...
int Console::i_loadStateFileExecInternal(PSSMHANDLE pSSM, uint32_t u32Version)
{
    ...
    for (uint32_t i = 0; i < size; ++i)
    {
        ...
        if (   u32Version >= CONSOLE_SAVED_STATE_VERSION_PRE_AUTO_MOUNT_POINT
#ifndef VPOX_OSE /* This broke saved state when introduced in r63916 (4.0). */
                                                     ...
                                                     uint32_t cbParms)
{
    ...
    else
    {
        ...
        rc = Global::vpoxStatusCodeFromCOM(hrc);
    }
    return rc;
}
                                              ...
                                              std::vector<Utf8Str> &aFlags)
{
    ...
    VPOXHGCMSVCPARM parm[3];
    parm[0].type = VPOX_HGCM_SVC_PARM_PTR;
    ...
    for (unsigned i = 0; i < 10 && (VERR_BUFFER_OVERFLOW == vrc); ++i)
    {
        ...
        parm[1].type = VPOX_HGCM_SVC_PARM_PTR;
        parm[1].u.pointer.addr = Utf8Buf.mutableRaw();
        parm[1].u.pointer.size = (uint32_t)cchBuf + 1024;

        parm[2].type = VPOX_HGCM_SVC_PARM_32BIT;
        parm[2].u.uint32 = 0;

        vrc = m_pVMMDev->hgcmHostCall("VPoxGuestPropSvc", GUEST_PROP_FN_HOST_ENUM_PROPS, 3, &parm[0]);
        Utf8Buf.jolt();
        if (parm[2].type != VPOX_HGCM_SVC_PARM_32BIT)
...
HRESULT Console::powerDown(ComPtr<IProgress> &aProgress)
{
    ...
    switch (mMachineState)
    {
        ...
        case MachineState_Saving:
            ...
            return setError(VPOX_E_INVALID_VM_STATE, tr("Cannot power down at this point during a save state"));
        ...
        case MachineState_TeleportingPausedVM:
            ...
            return setError(VPOX_E_INVALID_VM_STATE, tr("Cannot power down at this point in a teleportation"));
        ...
        case MachineState_OnlineSnapshotting:
            ...
            return setError(VPOX_E_INVALID_VM_STATE, tr("Cannot power down at this point in an online snapshot"));
        ...
        case MachineState_LiveSnapshotting:
            ...
            return setError(VPOX_E_INVALID_VM_STATE, tr("Cannot power down at this point in a live snapshot"));

        /* extra nice error message for a common case */
        case MachineState_Saved:
            return setError(VPOX_E_INVALID_VM_STATE, tr("Cannot power down a saved virtual machine"));
        case MachineState_Stopping:
            return setError(VPOX_E_INVALID_VM_STATE, tr("The virtual machine is being powered down"));
        default:
            return setError(VPOX_E_INVALID_VM_STATE,
    ...
    if (mfTurnResetIntoPowerOff)
    {
        ...
        mMachine->DeleteGuestProperty(Bstr("/VirtualPox/HostInfo/VMPowerOffReason").raw());
        mMachine->SetGuestProperty(Bstr("/VirtualPox/HostInfo/VMPowerOffReason").raw(),
...
HRESULT Console::reset()
{
    ...
    HRESULT rc = RT_SUCCESS(vrc) ? S_OK : setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not reset the machine (%Rrc)"), vrc);
...
HRESULT Console::i_doCPURemove(ULONG aCpu, PUVM pUVM)
{
    ...
    if (RT_SUCCESS(vrc) && !fLocked)
    {
         ...
         */
        PVMREQ pReq;
        vrc = VMR3ReqCallU(pUVM, 0, &pReq, 0 /* no wait! */, VMREQFLAGS_VPOX_STATUS,
                           (PFNRT)i_unplugCpu, 3,
        ...
        if (RT_SUCCESS(vrc))
        {
           ...
           rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Hot-Remove failed (rc=%Rrc)"), vrc);
    }
    else
        rc = setErrorBoth(VPOX_E_VM_ERROR, VERR_RESOURCE_BUSY,
...
HRESULT Console::i_doCPUAdd(ULONG aCpu, PUVM pUVM)
{
     ...
     */
    PVMREQ pReq;
    int vrc = VMR3ReqCallU(pUVM, 0, &pReq, 0 /* no wait! */, VMREQFLAGS_VPOX_STATUS,
                           (PFNRT)i_plugCpu, 3,
                           this, pUVM, aCpu);
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not add CPU to the machine (%Rrc)"), vrc);
...
HRESULT Console::powerButton()
{
    ...
    HRESULT rc = RT_SUCCESS(vrc) ? S_OK : setErrorBoth(VPOX_E_PDM_ERROR, vrc, tr("Controlled power off failed (%Rrc)"), vrc);
...
HRESULT Console::getPowerButtonHandled(BOOL *aHandled)
{
    ...
    *aHandled = FALSE;
    ...
    HRESULT rc = RT_SUCCESS(vrc) ? S_OK
               : setErrorBoth(VPOX_E_PDM_ERROR, vrc,
...
HRESULT Console::sleepButton()
{
    ...
    HRESULT rc = RT_SUCCESS(vrc) ? S_OK : setErrorBoth(VPOX_E_PDM_ERROR, vrc, tr("Sending sleep button event failed (%Rrc)"), vrc);
...
HRESULT Console::attachUSBDevice(const com::Guid &aId, const com::Utf8Str &aCaptureFilename)
{
    ...
    if (!mfVMHasUsbController)
        return setError(VPOX_E_PDM_ERROR, tr("The virtual machine does not have a USB controller"));
...
HRESULT Console::findUSBDeviceByAddress(const com::Utf8Str &aName, ComPtr<IUSBDevice> &aDevice)
{
    ...
    return setErrorNoLog(VPOX_E_OBJECT_NOT_FOUND, tr("Could not find a USB device with address '%s'"), aName.c_str());
...
HRESULT Console::findUSBDeviceById(const com::Guid &aId, ComPtr<IUSBDevice> &aDevice)
{
    ...
    return setErrorNoLog(VPOX_E_OBJECT_NOT_FOUND, tr("Could not find a USB device with uuid {%RTuuid}"), Guid(aId).raw());
        ...
        if (SUCCEEDED(hrc))
        {
                ...
                && mMachineState == MachineState_Paused)
            {
                ...
                hrc = RT_SUCCESS(vrc) ? S_OK
                    : setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not resume the machine execution (%Rrc)"), vrc);
            }
        }
    }
    else if (vrc == VERR_ALREADY_EXISTS)
        hrc = setErrorBoth(VPOX_E_OBJECT_IN_USE, vrc, tr("A password with the given ID already exists"));
...
HRESULT Console::removeDiskEncryptionPassword(const com::Utf8Str &aId)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        return setErrorBoth(VPOX_E_OBJECT_NOT_FOUND, vrc, tr("A password with the ID \"%s\" does not exist"), aId.c_str());
    else
...
HRESULT Console::clearAllDiskEncryptionPasswords()
{
    ...
    if (vrc == VERR_RESOURCE_IN_USE)
        return setErrorBoth(VPOX_E_OBJECT_IN_USE, vrc, tr("A password is still in use by the VM"));
    ...
    *pfResume = false;
    ...
    switch (enmVMState)
    {
        ...
        case VMSTATE_SOFT_RESETTING:
        {
            ...
            mVMStateChangeCallbackDisabled = false;
            if (RT_FAILURE(vrc))
                return setErrorInternal(VPOX_E_INVALID_VM_STATE,
                                        COM_IIDOF(IConsole),
                                        getStaticComponentName(),
...
HRESULT Console::i_doMediumChange(IMediumAttachment *aMediumAttachment, bool fForce, PUVM pUVM)
{
     ...
     */
    PVMREQ pReq;
    int vrc = VMR3ReqCallU(pUVM, VMCPUID_ANY, &pReq, 0 /* no wait! */, VMREQFLAGS_VPOX_STATUS,
 ...
 * so we have to go back to VPoxSVC and ask...  This is not ideal.
 */
HRESULT Console::i_onNATDnsChanged()
{
    ...
    ComPtr<IVirtualPox> pVirtualPox;
    hrc = mMachine->COMGETTER(Parent)(pVirtualPox.asOutParam());
    if (FAILED(hrc))
        return S_OK;

    ComPtr<IHost> pHost;
    hrc = pVirtualPox->COMGETTER(Host)(pHost.asOutParam());
    if (FAILED(hrc))
        return S_OK;
        ...
        if (strId.equals(Utf8Str(bstrKeyId)))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                if (pIBase)
                {
                    ...
                    {
                        hrc = setError(VPOX_E_PASSWORD_INCORRECT,
                                                     ...
                                                     INetworkAdapter *aNetworkAdapter)
{
    ...
    ComPtr<IVirtualPox> pVirtualPox;
    pThis->mMachine->COMGETTER(Parent)(pVirtualPox.asOutParam());
    ComPtr<ISystemProperties> pSystemProperties;
    if (pVirtualPox)
        pVirtualPox->COMGETTER(SystemProperties)(pSystemProperties.asOutParam());
    ChipsetType_T chipsetType = ChipsetType_PIIX3;
...
Utf8Str Console::VRDPServerErrorToMsg(int vrc)
{
    ...
    if (vrc == VERR_NET_ADDRESS_IN_USE)
    {
        ...
        errMsg = Utf8StrFmt(tr("VirtualPox Remote Desktop Extension server can't bind to the port(s): %s"),
                                Utf8Str(bstr).c_str());
    ...
    else if (vrc == VINF_NOT_SUPPORTED)
    {
         ...
         * Not fatal if we start the VM, fatal if the VM is already running. */
        LogRel(("VRDE: VirtualPox Remote Desktop Extension is not available.\n"));
        errMsg = Utf8Str("VirtualPox Remote Desktop Extension is not available");
    }
    else if (RT_FAILURE(vrc))
    {
        ...
        switch (vrc)
        {
            case VERR_FILE_NOT_FOUND:
                errMsg = Utf8StrFmt(tr("Could not find the VirtualPox Remote Desktop Extension library"));
                break;
            default:
...
HRESULT Console::i_sendACPIMonitorHotPlugEvent()
{
    ...
    HRESULT rc = RT_SUCCESS(vrc) ? S_OK
               : setErrorBoth(VPOX_E_PDM_ERROR, vrc, tr("Sending monitor hot-plug event failed (%Rrc)"), vrc);
 ...
 * @note Locks this object for writing.
 */
HRESULT Console::i_onUSBDeviceAttach(IUSBDevice *aDevice, IVirtualPoxErrorInfo *aError, ULONG aMaskedIfs,
                                     const Utf8Str &aCaptureFilename)
{
    ...
    if (FAILED(rc))
    {
        ...
        /* the error must be a VirtualPoxErrorInfo instance */
        ComPtr<IVirtualPoxErrorInfo> pError = eik.takeError();
...
HRESULT Console::i_onExtraDataChange(IN_BSTR aMachineId, IN_BSTR aKey, IN_BSTR aVal)
{
    ...
    if (ptrVM.isOk())
    {
        if (RTUtf16CmpAscii(aKey, "VPoxInternal2/TurnResetIntoPowerOff") == 0)
        {
...
{
#ifndef VPOX_WITH_GUEST_PROPS
    ReturnComNotImplemented();
#else  /* VPOX_WITH_GUEST_PROPS */
    ...
    try
    {
        ...
        parm[2].type = VPOX_HGCM_SVC_PARM_64BIT;
        parm[2].u.uint64 = 0;

        parm[3].type = VPOX_HGCM_SVC_PARM_32BIT;
        parm[3].u.uint32 = 0;

        int vrc = m_pVMMDev->hgcmHostCall("VPoxGuestPropSvc", GUEST_PROP_FN_HOST_GET_PROP,
                                          4, &parm[0]);
        /* The returned string should never be able to be greater than our buffer */
        AssertLogRel(vrc != VERR_BUFFER_OVERFLOW);
        AssertLogRel(RT_FAILURE(vrc) || parm[2].type == VPOX_HGCM_SVC_PARM_64BIT);
        if (RT_SUCCESS(vrc))
        {
            *aValue = szBuffer;
        ...
        else if (vrc == VERR_NOT_FOUND)
        {
            *aValue = "";
            ...
            rc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("The VPoxGuestPropSvc service call failed with the error %Rrc"), vrc);
    }
...
HRESULT Console::i_setGuestProperty(const Utf8Str &aName, const Utf8Str &aValue, const Utf8Str &aFlags)
{
    ...
    if (aFlags.isEmpty())
    {
        vrc = m_pVMMDev->hgcmHostCall("VPoxGuestPropSvc", GUEST_PROP_FN_HOST_SET_PROP_VALUE, 2, &parm[0]);
    }
    else
    {
        parm[2].type = VPOX_HGCM_SVC_PARM_PTR;
        parm[2].u.pointer.addr = (void*)aFlags.c_str();
        parm[2].u.pointer.size = (uint32_t)aFlags.length() + 1; /* The + 1 is the null terminator */

        vrc = m_pVMMDev->hgcmHostCall("VPoxGuestPropSvc", GUEST_PROP_FN_HOST_SET_PROP, 3, &parm[0]);
    }

    HRESULT hrc = S_OK;
    if (RT_FAILURE(vrc))
        hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("The VPoxGuestPropSvc service call failed with the error %Rrc"), vrc);
    return hrc;
#endif /* VPOX_WITH_GUEST_PROPS */
}
...
HRESULT Console::i_deleteGuestProperty(const Utf8Str &aName)
{
    ...
    VPOXHGCMSVCPARM parm[1];
    parm[0].type = VPOX_HGCM_SVC_PARM_PTR;
    parm[0].u.pointer.addr = (void*)aName.c_str();
    parm[0].u.pointer.size = (uint32_t)aName.length() + 1; /* The + 1 is the null terminator */

    int vrc = m_pVMMDev->hgcmHostCall("VPoxGuestPropSvc", GUEST_PROP_FN_HOST_DEL_PROP, 1, &parm[0]);
                                     ...
                                     IProgress *aProgress)
{
    ...
    rc = mMachine->GetExtraData(Bstr("VPoxInternal2/EnableDiskIntegrityDriver").raw(),
...
HRESULT Console::i_reconfigureMediumAttachments(const std::vector<ComPtr<IMediumAttachment> > &aAttachments)
{
    ...
    for (size_t i = 0; i < aAttachments.size(); ++i)
    {
         ...
         * IPC (to VPoxSVC) which takes a significant amount of time.
...
HRESULT Console::i_onVMProcessPriorityChange(VMProcPriority_T priority)
{
    ...
    if (RT_FAILURE(vrc))
        rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not set the priority of the process (%Rrc). Try to set it when VM is not started."), vrc);
 ...
 * in module VPoxC (and ConsoleImpl.cpp is part of it and thus can call it).
 * Extension modules must not link directly against VPoxC, (XP)COM is
 * handling this.
 */
...
HRESULT Console::i_pause(Reason_T aReason)
{
    ...
    switch (mMachineState)
    {
        ...
        case MachineState_OnlineSnapshotting:
            ...
            return setError(VPOX_E_INVALID_VM_STATE, tr("Already paused"));
    ...
    HRESULT hrc = S_OK;
    if (RT_FAILURE(vrc))
        hrc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not suspend the machine execution (%Rrc)"), vrc);
    ...
    else
    {
        ...
        else
        {
            ...
            if (   VMR3GetStateU(ptrVM.rawUVM()) == VMSTATE_SUSPENDED
                ...
                return setError(VPOX_E_INVALID_VM_STATE, tr("VM is paused due to host power management"));

            enmReason = aReason == Reason_Snapshot ? VMRESUMEREASON_STATE_SAVED : VMRESUMEREASON_USER;
        }

        // for snapshots: no state change callback, VPoxSVC does everything
                             ...
                             const Utf8Str &aStateFilePath, bool aPauseVM, bool &aLeftPaused)
{
    ...
    mMachine->GetExtraData(Bstr("VPoxInternal2/DisableSaveState").raw(), strDisableSaveState.asOutParam());
    if (strDisableSaveState == "1")
        return setError(VPOX_E_VM_ERROR,
    ...
    /* ensure the directory for the saved state file exists */
    {
        ...
        if (!RTDirExists(dir.c_str()))
        {
            ...
            if (RT_FAILURE(vrc))
                return setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Could not create a directory '%s' to save the state to (%Rrc)"),
                                    dir.c_str(), vrc);
        }
    }
    ...
    if (aPauseVM)
    {
        ...
        if (RT_FAILURE(vrc))
            return setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not suspend the machine execution (%Rrc)"), vrc);
        fPaused = true;
    }
...
#endif /* VPOX_WITH_AUDIO_RECORDING */
 ...
 *          Additions are available (via VPoxTray/VPoxClient).
 */
...
HRESULT Console::i_onShowWindow(BOOL aCheck, BOOL *aCanShow, LONG64 *aWinId)
{
    ...
    *aWinId = 0;
    ...
    if (aCheck)
    {
        evDesc.init(mEventSource, VPoxEventType_OnCanShowWindow);
    ...
    {
        evDesc.init(mEventSource, VPoxEventType_OnShowWindow, INT64_C(0));
        BOOL fDelivered = evDesc.fire(5000); /* Wait up to 5 secs for delivery */
        //Assert(fDelivered);
        if (fDelivered)
        {
...
HRESULT Console::i_consoleInitReleaseLog(const ComPtr<IMachine> aMachine)
{
    ...
    Utf8Str logFile = Utf8StrFmt("%s%cVPox.log",
                                 logDir.c_str(), RTPATH_DELIMITER);
    Utf8Str pngFile = Utf8StrFmt("%s%cVPox.png",
                                 logDir.c_str(), RTPATH_DELIMITER);
     ...
     */
    ComPtr<IVirtualPox> pVirtualPox;
    aMachine->COMGETTER(Parent)(pVirtualPox.asOutParam());
    ComPtr<ISystemProperties> pSystemProperties;
    pVirtualPox->COMGETTER(SystemProperties)(pSystemProperties.asOutParam());
    ...
    /* If we've made any directory changes, flush the directory to increase
       ...
       Tip: Try 'export VPOX_RELEASE_LOG_FLAGS=flush' if the last bits of the log
...
HRESULT Console::i_powerUp(IProgress **aProgress, bool aPaused)
{
    ...
    try
    {
        if (Global::IsOnlineOrTransient(mMachineState))
            throw setError(VPOX_E_INVALID_VM_STATE, tr("The virtual machine is already running or busy (machine state: %s)"),
                           Global::stringifyMachineState(mMachineState));
...
#if 0 /** @todo we should save it afterwards, but that isn't necessarily a good idea. Find a better place for this (VPoxSVC).  */
        ...
        if (mMachineState == MachineState_Saved)
        {
            ...
            if (RT_FAILURE(vrc))
            {
                ...
                throw setErrorBoth(VPOX_E_FILE_ERROR, vrc, errMsg.c_str());
            }
        }
        ...
        Bstr value;
        HRESULT hrc = mMachine->GetExtraData(Bstr("VPoxInternal2/CoreDumpEnabled").raw(), value.asOutParam());
        if (SUCCEEDED(hrc) && value == "1")
        {
            Bstr coreDumpDir, coreDumpReplaceSys, coreDumpLive;
            mMachine->GetExtraData(Bstr("VPoxInternal2/CoreDumpDir").raw(), coreDumpDir.asOutParam());
            mMachine->GetExtraData(Bstr("VPoxInternal2/CoreDumpReplaceSystemDump").raw(), coreDumpReplaceSys.asOutParam());
            mMachine->GetExtraData(Bstr("VPoxInternal2/CoreDumpLive").raw(), coreDumpLive.asOutParam());
        ...
        /* Tell VPoxSVC and Machine about the progress object so they can
             ...
             ++slot)
        {
            ...
            switch (netattach)
            {
                case NetworkAttachmentType_Bridged:
                {
                    ...
                    if (hostif.isEmpty())
                    {
                        throw setError(VPOX_E_HOST_ERROR,
                                       tr("VM cannot start because host interface networking requires a host interface name to be set"));
                    }
                    ComPtr<IVirtualPox> pVirtualPox;
                    mMachine->COMGETTER(Parent)(pVirtualPox.asOutParam());
                    ComPtr<IHost> pHost;
                    pVirtualPox->COMGETTER(Host)(pHost.asOutParam());
...
# endif
    ...
    if (RT_SUCCESS(vrc) || getObjectState().getState() == ObjectState::InUninit)
    {
        ...
        else
        {
            ...
            mpUVM = pUVM;
            pUVM = NULL;
            rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not destroy the machine. (Error: %Rrc)"), vrc);
        }
        ...
        rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not power off the machine. (Error: %Rrc)"), vrc);
                                   ...
                                   bool aUpdateServer /* = true */)
{
    ...
    if (mMachineState != aMachineState)
    {
        ...
        //      Event thread, like it is done in VirtualPox. This will make it
                                    ...
                                    bool aSetError /* = false */)
{
    ...
    if (aSetError)
        setError(VPOX_E_FILE_ERROR, tr("Could not find a shared folder named '%s'."), strName.c_str());
...
HRESULT Console::i_createSharedFolder(const Utf8Str &strName, const SharedFolderData &aData)
{
     ...
     */
    Bstr bstrValue;
    HRESULT hrc = mMachine->GetExtraData(BstrFmt("VPoxInternal2/SharedFoldersEnableSymlinksCreate/%s", strName.c_str()).raw(),
    ...
    if (pHostPath && pName && pAutoMountPoint)
    {
         ...
         */
        VPOXHGCMSVCPARM aParams[SHFL_CPARMS_ADD_MAPPING];
        ...
        vrc = m_pVMMDev->hgcmHostCall("VPoxSharedFolders", SHFL_FN_ADD_MAPPING, SHFL_CPARMS_ADD_MAPPING, aParams);
        if (RT_FAILURE(vrc))
...
HRESULT Console::i_removeSharedFolder(const Utf8Str &strName)
{
    ...
    parms.type = VPOX_HGCM_SVC_PARM_PTR;
    parms.u.pointer.addr = pMapName;
    parms.u.pointer.size = ShflStringSizeOfBuffer(pMapName);

    int vrc = m_pVMMDev->hgcmHostCall("VPoxSharedFolders",
    ...
    switch (enmState)
    {
        ...
        case VMSTATE_OFF:
        {
            ...
            if (that->mfTurnResetIntoPowerOff)
            {
                ...
                that->mMachine->DeleteGuestProperty(Bstr("/VirtualPox/HostInfo/VMPowerOffReason").raw());
                that->mMachine->SetGuestProperty(Bstr("/VirtualPox/HostInfo/VMPowerOffReason").raw(),
...
int Console::i_changeClipboardMode(ClipboardMode_T aClipboardMode)
{
    ...
    VPOXHGCMSVCPARM parm;
    parm.type = VPOX_HGCM_SVC_PARM_32BIT;
    ...
    switch (aClipboardMode)
    {
        ...
        case ClipboardMode_Disabled:
            ...
            parm.u.uint32 = VPOX_SHCL_MODE_OFF;
            break;
        case ClipboardMode_GuestToHost:
            LogRel(("Shared Clipboard: Mode: Guest to Host\n"));
            parm.u.uint32 = VPOX_SHCL_MODE_GUEST_TO_HOST;
            break;
        case ClipboardMode_HostToGuest:
            LogRel(("Shared Clipboard: Mode: Host to Guest\n"));
            parm.u.uint32 = VPOX_SHCL_MODE_HOST_TO_GUEST;
            break;
        case ClipboardMode_Bidirectional:
            LogRel(("Shared Clipboard: Mode: Bidirectional\n"));
            parm.u.uint32 = VPOX_SHCL_MODE_BIDIRECTIONAL;
            break;
    }

    int vrc = pVMMDev->hgcmHostCall("VPoxSharedClipboard", VPOX_SHCL_HOST_FN_SET_MODE, 1, &parm);
    if (RT_FAILURE(vrc))
...
int Console::i_changeClipboardFileTransferMode(bool aEnabled)
{
    ...
    parm.type     = VPOX_HGCM_SVC_PARM_32BIT;
    parm.u.uint32 = aEnabled ? VPOX_SHCL_TRANSFER_MODE_ENABLED : VPOX_SHCL_TRANSFER_MODE_DISABLED;

    int vrc = pVMMDev->hgcmHostCall("VPoxSharedClipboard", VPOX_SHCL_HOST_FN_SET_TRANSFER_MODE, 1 /* cParms */, &parm);
    if (RT_FAILURE(vrc))
...
int Console::i_changeDnDMode(DnDMode_T aDnDMode)
{
    ...
    switch (aDnDMode)
    {
        ...
        case DnDMode_Disabled:
            ...
            parm.u.uint32 = VPOX_DRAG_AND_DROP_MODE_OFF;
            break;
        case DnDMode_GuestToHost:
            LogRel(("Drag and drop mode: Guest to Host\n"));
            parm.u.uint32 = VPOX_DRAG_AND_DROP_MODE_GUEST_TO_HOST;
            break;
        case DnDMode_HostToGuest:
            LogRel(("Drag and drop mode: Host to Guest\n"));
            parm.u.uint32 = VPOX_DRAG_AND_DROP_MODE_HOST_TO_GUEST;
            break;
        case DnDMode_Bidirectional:
            LogRel(("Drag and drop mode: Bidirectional\n"));
            parm.u.uint32 = VPOX_DRAG_AND_DROP_MODE_BIDIRECTIONAL;
            break;
    }

    int rc = pVMMDev->hgcmHostCall("VPoxDragAndDropSvc",
...
#if ((defined(RT_OS_LINUX) && !defined(VPOX_WITH_NETFLT)) || defined(RT_OS_FREEBSD))
/**
...
# endif /* VPOX_STRICT */
    ...
    if (maTapFD[slot] != NIL_RTFILE)
    {
        ...
        if (FAILED(rc) || tapDeviceName.isEmpty())
        {
            ...
            isStatic = false;
            int rcVPox = RTFileClose(maTapFD[slot]);
            AssertRC(rcVPox);
...
#endif /* (RT_OS_LINUX || RT_OS_FREEBSD) && !VPOX_WITH_NETFLT */
...
HRESULT Console::i_powerDownHostInterfaces()
{
    ...
    for (ULONG slot = 0; slot < maxNetworkAdapters; slot++)
    {
        ...
        {
#if ((defined(RT_OS_LINUX) || defined(RT_OS_FREEBSD)) && !defined(VPOX_WITH_NETFLT))
...
HRESULT Console::i_captureUSBDevices(PUVM pUVM)
{
    ...
    if (mfVMHasUsbController)
    {
        /* release the lock before calling Host in VPoxSVC since Host may call
...
void Console::i_powerUpThreadTask(VMPowerUpTask *pTask)
{
    ...
    VirtualPoxBase::initializeComForThread();
    ...
    try
    {
        ...
        if (SUCCEEDED(rc) && RT_FAILURE(vrc))
        {
            ...
            if (!pTask->mErrorMsg.length())
            {
                 ...
                 * convert the VPox status code into a meaningful error message.
    ...
    /* Notify VPoxSVC and any waiting openRemoteSession progress object. */
...
Console::i_vmm2User_SaveState(PCVMM2USERMETHODS pThis, PUVM pUVM)
{
    ...
    return SUCCEEDED(hrc) ? VINF_SUCCESS : Global::vpoxStatusCodeFromCOM(hrc);
}
...
{
    NOREF(pThis); NOREF(pUVM); NOREF(pUVCpu);
    VirtualPoxBase::uninitializeComForThread();
}
...
Console::i_pdmIfSecKeyHlp_KeyMissingNotify(PPDMISECKEYHLP pInterface)
{
    ...
    pConsole->mMachine->DeleteGuestProperty(Bstr("/VirtualPox/HostInfo/DekMissing").raw());
    pConsole->mMachine->SetGuestProperty(Bstr("/VirtualPox/HostInfo/DekMissing").raw(),
