/** $Id: VPoxServiceClipboard-os2.cpp $ */
/** @file
 * VPoxService - Guest Additions Clipboard Service, OS/2.
 */
...
/** @page pg_vgsvc_clipboard VPoxService - Clipboard (OS/2)
 *
 * The Clipboard subservice provides clipboard sharing for OS/2 guests only.
 *
 * This was the second subservice that was added to VPoxService.  OS/2 is a
 * single user system and we don't provide any VPoxTray or VPoxClient like
 * processes. Because it's kind of simple system, it became natural to put the
 * clipboard sharing here in VPoxService for OS/2.
 *
...
static DECLCALLBACK(int) vgsvcClipboardOs2Init(void)
{
    ...
    if (g_hmqCtrl != NULLHANDLE)
    {
         ...
         */
        g_atomNothingChanged = WinAddAtom(WinQuerySystemAtomTable(), (PCSZ)"VirtualPox Clipboard Service");
        LONG lLastError = WinGetLastError(g_habCtrl);
        if (g_atomNothingChanged == 0)
            g_atomNothingChanged = WinFindAtom(WinQuerySystemAtomTable(), (PCSZ)"VirtualPox Clipboard Service");
    ...
    if (WinOpenClipbrd(g_habWorker))
    {
        ...
        if (WinEmptyClipbrd(g_habWorker))
        {
            ...
            if (WinSetClipbrdOwner(g_habWorker, g_hwndWorker))
            {
                 ...
                 */
                if (fFormats & (VPOX_SHCL_FMT_UNICODETEXT/* | VPOX_SHCL_FMT_HTML ?? */))
                {
                ...
                if (fFormats & VPOX_SHCL_FMT_BITMAP)
                {
                    /** @todo bitmaps */
                }
            }
...
static void *vgsvcClipboardOs2ConvertToPM(uint32_t fFormat, USHORT usFmt, void *pv, uint32_t cb)
{
    ...
    else if (usFmt == CF_TEXT)
    {
         ...
         * -lazybird
         */
        Assert(fFormat & VPOX_SHCL_FMT_UNICODETEXT);
        char *pszUtf8;
...
static void vgsvcClipboardOs2RenderFormat(USHORT usFmt)
{
    ...
    if (    usFmt == CF_TEXT
        ||  usFmt == g_atomOdin32UnicodeText)
        fFormat = VPOX_SHCL_FMT_UNICODETEXT;
    else /** @todo bitmaps */
        fFormat = 0;
    if (fFormat)
    {
...
static void vgsvcClipboardOs2ReportFormats(void)
{
    ...
    while ((ulFormat = WinEnumClipbrdFmts(g_habWorker, ulFormat)) != 0)
    {
        if (    ulFormat == CF_TEXT
            ...
            fFormats |= VPOX_SHCL_FMT_UNICODETEXT;
...
static MRESULT EXPENTRY vgsvcClipboardOs2WinProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    ...
    switch (msg)
    {
         ...
         */
        case WM_USER + VPOX_SHCL_HOST_MSG_FORMATS_REPORT:
         ...
         */
        case WM_USER + VPOX_SHCL_HOST_MSG_READ_DATA:
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    switch (Msg)
                    {
                        ...
                        case VPOX_SHCL_HOST_MSG_FORMATS_REPORT:
                            if (!WinPostMsg(g_hwndWorker, WM_USER + VPOX_SHCL_HOST_MSG_FORMATS_REPORT,
                        ...
                        case VPOX_SHCL_HOST_MSG_READ_DATA:
                            if (!WinPostMsg(g_hwndWorker, WM_USER + VPOX_SHCL_HOST_MSG_READ_DATA,
                                            MPFROMLONG(fFormats), 0))
...
static DECLCALLBACK(int) vgsvcClipboardOs2Worker(bool volatile *pfShutdown)
{
    ...
    if (g_habWorker != NULLHANDLE)
    {
        ...
        if (g_hmqWorker != NULLHANDLE)
        {
             ...
             */
            if (WinRegisterClass(g_habWorker, (PCSZ)"VPoxServiceClipboardClass", vgsvcClipboardOs2WinProc, 0, 0))
            {
                g_hwndWorker = WinCreateWindow(HWND_OBJECT,                             /* hwndParent */
                                               (PCSZ)"VPoxServiceClipboardClass",       /* pszClass */
                                               (PCSZ)"VirtualPox Clipboard Service",    /* pszName */
                                               0,                                       /* flStyle */
                                               0, 0, 0, 0,                              /* x, y, cx, cy */
