...
static void vpciConfigure(PPDMPCIDEV pPciDev, uint16_t uDeviceId, uint16_t uClass)
{
    ...
    PDMPciDevSetWord(pPciDev, VPOX_PCI_SUBSYSTEM_VENDOR_ID, DEVICE_PCI_SUBSYSTEM_VENDOR_ID);
    PDMPciDevSetWord(pPciDev, VPOX_PCI_SUBSYSTEM_ID,        DEVICE_PCI_SUBSYSTEM_BASE_ID + uDeviceId);

    /* ABI version, must be equal 0 as of 2.6.30 kernel. */
    PDMPciDevSetByte(pPciDev, VPOX_PCI_REVISION_ID,         0x00);
    /* Ethernet adapter */
    PDMPciDevSetByte(pPciDev, VPOX_PCI_CLASS_PROG,          0x00);
    PDMPciDevSetWord(pPciDev, VPOX_PCI_CLASS_DEVICE,        uClass);
    /* Interrupt Pin: INTA# */
