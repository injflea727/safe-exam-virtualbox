...
struct StorageController::Data
{
    Data(Machine * const aMachine)
        : pVirtualPox(NULL),
          ...
          pParent(aMachine)
    {
        unconst(pVirtualPox) = aMachine->i_getVirtualPox();
        unconst(pSystemProperties) = pVirtualPox->i_getSystemProperties();
    }
                                ...
                                ULONG aInstance, bool fBootable)
{
    ...
    if (FAILED(rc))
        return rc;
    rc = aParent->i_getVirtualPox()->i_getSystemProperties()->
