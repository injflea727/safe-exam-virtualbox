/* $Id: VPoxGuest-linux.c $ */
/** @file
 * VPoxGuest - Linux specifics.
 *
...
# define VPOXGUEST_WITH_INPUT_DRIVER
#endif
...
#ifdef VPOXGUEST_WITH_INPUT_DRIVER
...
#define DEVICE_NAME_USER        "vpoxuser"
...
#endif
static int  vgdrvLinuxIOCtlSlow(struct file *pFilp, unsigned int uCmd, unsigned long ulArg, PVPOXGUESTSESSION pSession);
...
static const struct pci_device_id g_VPoxGuestPciId[] =
{
    {
...
static PVPOXGUESTSESSION        g_pKernelSession = NULL;
#endif



/**
 * Converts a VPox status code to a linux error code.
 *
 ...
 */
static int vpoxguestOpenInputDevice(struct input_dev *pDev)
{
 ...
 */
static void vpoxguestCloseInputDevice(struct input_dev *pDev)
{
    NOREF(pDev);
...
static int __init vgdrvLinuxCreateInputDevice(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (g_pInputDevice)
        {
            ...
            g_pInputDevice->id.version = VPOX_SHORT_VERSION;
            g_pInputDevice->open       = vpoxguestOpenInputDevice;
            g_pInputDevice->close      = vpoxguestCloseInputDevice;
...
# endif
                ...
                g_pInputDevice->name = "VirtualPox mouse integration";
                return 0;
            }
...
#endif /* VPOXGUEST_WITH_INPUT_DRIVER */
...
static int __init vgdrvLinuxModInit(void)
{
    ...
    rc = RTLogCreate(&pRelLogger, 0 /* fFlags */, "all",
                     "VPOX_RELEASE_LOG", RT_ELEMENTS(s_apszGroups), s_apszGroups,
    ...
    if (rc >= 0 && g_pPciDev)
    {
        ...
        VPOXOSTYPE enmOSType = VPOXOSTYPE_Linux26;
#elif RTLNX_VER_MIN(2,6,0) && defined(RT_ARCH_AMD64)
        VPOXOSTYPE enmOSType = VPOXOSTYPE_Linux26_x64;
#elif RTLNX_VER_MIN(2,4,0) && defined(RT_ARCH_X86)
        VPOXOSTYPE enmOSType = VPOXOSTYPE_Linux24;
#elif RTLNX_VER_MIN(2,4,0) && defined(RT_ARCH_AMD64)
        VPOXOSTYPE enmOSType = VPOXOSTYPE_Linux24_x64;
#else
# warning "huh? which arch + version is this?"
        VPOXOSTYPE enmOsType = VPOXOSTYPE_Linux;
#endif
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (rc >= 0)
                    {
                        ...
                        if (rc >= 0)
                        {
                            ...
                            LogRel((DEVICE_NAME ": Successfully loaded version " VPOX_VERSION_STRING " r" __stringify(VPOX_SVN_REV) "\n"));
                            ...
                            printk(KERN_DEBUG DEVICE_NAME ": Successfully loaded version "
                                   VPOX_VERSION_STRING " r" __stringify(VPOX_SVN_REV) " (interface " RT_XSTR(VMMDEV_VERSION) ")\n");
                            return rc;
                        }
                    ...
                    {
                        LogRel((DEVICE_NAME ": vpoxguestCreateInputDevice failed with rc=%Rrc\n", rc));
...
static int vgdrvLinuxOpen(struct inode *pInode, struct file *pFilp)
{
     ...
     * ASSUMES that the gid of /dev/vpoxuser is what we should consider the special vpox group.
     */
    ...
    if (MINOR(pInode->i_rdev) == g_MiscDeviceUser.minor)
    {
        ...
        if (!vgdrvLinuxIsGroupZero(pInode->i_gid) && vgdrvLinuxIsInGroupEff(pInode->i_gid))
            fRequestor |= VMMDEV_REQUESTOR_GRP_VPOX;
    }
...
#endif
    VGDrvCommonCloseSession(&g_DevExt, (PVPOXGUESTSESSION)pFilp->private_data);
...
#endif
{
    PVPOXGUESTSESSION pSession = (PVPOXGUESTSESSION)pFilp->private_data;
    int rc;
...
static unsigned int vgdrvLinuxPoll(struct file *pFile, poll_table *pPt)
{
    PVPOXGUESTSESSION   pSession  = (PVPOXGUESTSESSION)pFile->private_data;
...
MODULE_DESCRIPTION(VPOX_PRODUCT " Guest Additions for Linux Module");
MODULE_LICENSE("GPL");
