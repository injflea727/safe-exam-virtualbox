/* $Id: HostImpl.cpp $ */
/** @file
 * VirtualPox COM class implementation: Host
 */
...
# ifdef VPOX_USE_LIBHAL
#  include "vpox-libhal.h"
 ...
 *
 * @param aParent   VirtualPox parent object.
 */
HRESULT Host::init(VirtualPox *aParent)
{
    HRESULT hrc;
...
#elif defined(RT_OS_LINUX) && defined(VPOX_WITH_DBUS)
    ...
    for (size_t i = 0; i < aGlobalExtraDataKeys.size(); ++i)
    {
        ...
        if (!strKey.startsWith("HostOnly/vpoxnet"))
            continue;

        size_t pos = strKey.find("/", sizeof("HostOnly/vpoxnet"));
        if (pos != Utf8Str::npos)
...
void Host::uninit()
{
     ...
     * cleaned up and VirtualPox shutting down.
     */
...
#if defined(RT_OS_WINDOWS) && defined(VPOX_WITH_NETFLT)
# define VPOX_APP_NAME L"VirtualPox"

static int vpoxNetWinAddComponent(std::list< ComObjPtr<HostNetworkInterface> > *pPist,
...
#endif /* defined(RT_OS_WINDOWS) && defined(VPOX_WITH_NETFLT) */
...
{
#if defined(RT_OS_WINDOWS) || defined(VPOX_WITH_NETFLT) /*|| defined(RT_OS_OS2)*/
# ifdef VPOX_WITH_HOSTNETIF_API
...
#   ifndef VPOX_WITH_NETFLT
    hr = E_NOTIMPL;
#   else /* #  if defined VPOX_WITH_NETFLT */
    INetCfg              *pNc;
    ...
    hr = VPoxNetCfgWinQueryINetCfg(FALSE,
                                   VPOX_APP_NAME,
                                   &pNc,
                                   &lpszApp);
    Assert(hr == S_OK);
    if (hr == S_OK)
    {
#    ifdef VPOX_NETFLT_ONDEMAND_BIND
...
#    else
        /* for the filter-based approach we get all miniports our filter (oracle_VPoxNetLwf)is bound to */
        hr = pNc->FindComponent(L"oracle_VPoxNetLwf", &pTcpIpNcc);
        if (hr != S_OK)
        {
            /* fall back to NDIS5 miniport lookup (sun_VPoxNetFlt) */
            hr = pNc->FindComponent(L"sun_VPoxNetFlt", &pTcpIpNcc);
        }
...
#    endif
        ...
        if (hr == S_OK)
        {
            hr = VPoxNetCfgWinGetBindingPathEnum(pTcpIpNcc, EBP_BELOW, &pEnumBp);
            Assert(hr == S_OK);
            if (hr == S_OK)
            {
                hr = VPoxNetCfgWinGetFirstBindingPath(pEnumBp, &pBp);
                ...
                while (hr == S_OK)
                {
                    ...
                    {
                        hr = VPoxNetCfgWinGetBindingInterfaceEnum(pBp, &pEnumBi);
                        Assert(hr == S_OK);
                        if (hr == S_OK)
                        {
                            hr = VPoxNetCfgWinGetFirstBindingInterface(pEnumBi, &pBi);
                            Assert(hr == S_OK);
                            while (hr == S_OK)
                            {
                                ...
                                if (hr == S_OK)
                                {
                                    ...
                                    if (hr == S_OK)
                                    {
                                        ...
                                        {
                                            vpoxNetWinAddComponent(&list, pMpNcc);
                                        }
                                    }
                                    VPoxNetCfgWinReleaseRef(pMpNcc);
                                }
                                VPoxNetCfgWinReleaseRef(pBi);

                                hr = VPoxNetCfgWinGetNextBindingInterface(pEnumBi, &pBi);
                            }
                            VPoxNetCfgWinReleaseRef(pEnumBi);
                        }
                    }
                    VPoxNetCfgWinReleaseRef(pBp);

                    hr = VPoxNetCfgWinGetNextBindingPath(pEnumBp, &pBp);
                }
                VPoxNetCfgWinReleaseRef(pEnumBp);
            }
            VPoxNetCfgWinReleaseRef(pTcpIpNcc);
        }
        else
        {
            LogRel(("failed to get the oracle_VPoxNetLwf(sun_VPoxNetFlt) component, error (0x%x)\n", hr));
        }

        VPoxNetCfgWinReleaseINetCfg(pNc, FALSE);
    }
#   endif /* #  if defined VPOX_WITH_NETFLT */
 ...
 * is loaded after VPoxSVC starts.
 */
...

{
     ...
     * called below grabs the VirtualPox lock. */
    ...
    if (RT_SUCCESS(r))
    {
         ...
         * so the interface gets re-created after vpoxnetadp.ko reload.
                                    ...
                                    const ComPtr<IHostUSBDeviceFilter> &aFilter)
{
    ...
    if (pFilter.isNull())
        return setError(VPOX_E_INVALID_OBJECT_STATE,
                        tr("The given USB device filter is not created within this VirtualPox instance"));
    ...
    alock.release();
    AutoWriteLock vpoxLock(m->pParent COMMA_LOCKVAL_SRC_POS);
                                     ...
                                     ComPtr<IHostUSBDevice> &aDevice)
{
    ...
    return setErrorNoLog(VPOX_E_OBJECT_NOT_FOUND,
 ...
 * to that drive; otherwise returns VPOX_E_OBJECT_NOT_FOUND.
 *
 ...
 * @return VPOX_E_OBJECT_NOT_FOUND if not found, or S_OK if found, or errors from getDrives().
 */
 ...
 * it sets pMedium to that drive; otherwise returns VPOX_E_OBJECT_NOT_FOUND.
 *
 * @param mediumType  Must be DeviceType_DVD or DeviceType_Floppy.
                                        ...
                                        BOOL aActiveChanged /* = FALSE */)
{
    ...
    if (aFilter->mInList)
    {
        ...
        // for that we should hold only the VirtualPox lock
        alock.release();
        AutoWriteLock vpoxLock(m->pParent COMMA_LOCKVAL_SRC_POS);
...
#if defined(RT_OS_SOLARIS) && defined(VPOX_USE_LIBHAL)
...
#endif  /* RT_OS_SOLARIS and VPOX_USE_HAL */
 ...
 *  Returns S_OK when the Proxy is present and OK, VPOX_E_HOST_ERROR (as a
...
# endif
    ...
    /*
     * Need to set the references to VirtualPox object in all interface objects
     * (see @bugref{6439}).
     */
    for (itNew = list.begin(); itNew != list.end(); ++itNew)
        (*itNew)->i_setVirtualPox(m->pParent);
...
void Host::i_generateMACAddress(Utf8Str &mac)
{
     ...
     * vendor ID (880027). The remaining 3 bytes will be taken from the
     * start of a GUID. This is a fairly safe algorithm.
     */
    Guid guid;
    guid.create();
    mac = Utf8StrFmt("880027%02X%02X%02X",
