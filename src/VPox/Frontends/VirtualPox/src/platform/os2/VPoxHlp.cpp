/* $Id: VPoxHlp.cpp $ */
/** @file
 * VPox Qt GUI - Implementation of OS/2-specific helpers that require to reside in a DLL
 */
...
/* Parameters for the keyboard hook (VPoxHlpInstallKbdHook()) */
 ...
 */
static
BOOL EXPENTRY vpoxInputHook (HAB aHab, PQMSG aMsg, ULONG aFS)
{
    if (aMsg->msg == WM_CHAR)
    {
        /* For foreign processes that didn't call VPoxHlpInstallKbdHook(),
         ...
         * called VPoxHlpInstallKbdHook(), we ignore the message. */
...
/**
 *  Uninstalls the keyboard hook installed by VPoxHlpInstallKbdHook().
 *  All arguments must match arguments passed to VPoxHlpInstallKbdHook(),
