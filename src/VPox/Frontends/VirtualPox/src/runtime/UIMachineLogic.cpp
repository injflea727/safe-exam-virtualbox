...
/** @file
 * VPox Qt GUI - UIMachineLogic class implementation.
 */
...
#define VPOX_WITH_REWORKED_SESSION_INFORMATION /**< Define for reworked session-information window.  @todo r-bird: What's this for? */
...
void UIMachineLogic::sltHandleVPoxSVCAvailabilityChange()
{
    /* Do nothing if VPoxSVC still availabile: */
    if (uiCommon().isVPoxSVCAvailable())
        return;

    /* Warn user about that: */
    msgCenter().warnAboutVPoxSVCUnavailable();

    /* Power VM off: */
    LogRel(("GUI: Request to power VM off due to VPoxSVC is unavailable.\n"));
    powerOff(false);
}
...
void UIMachineLogic::sltMachineStateChanged()
{
    ...
    switch (state)
    {
        case KMachineState_Stuck:
        {
            ...
            takeScreenshot(strLogFolder + "/VPox.png", "png");
...
void UIMachineLogic::sltUSBDeviceStateChange(const CUSBDevice &device, bool fIsAttached, const CVirtualPoxErrorInfo &error)
{
...
{
    /* We should watch for VPoxSVC availability changes: */
    connect(&uiCommon(), &UICommon::sigVPoxSVCAvailabilityChange,
            this, &UIMachineLogic::sltHandleVPoxSVCAvailabilityChange);
...
{
    /* We should stop watching for VPoxSVC availability changes: */
    disconnect(&uiCommon(), &UICommon::sigVPoxSVCAvailabilityChange,
               this, &UIMachineLogic::sltHandleVPoxSVCAvailabilityChange);
...
void UIMachineLogic::sltTakeScreenshot()
{
    ...
    const QString strFormatDefaultFileName = QString("VirtualPox").append("_").append(machine().GetName()).append("_").append(strCurrentTime);
...
void UIMachineLogic::sltInstallGuestAdditions()
{
    ...
    CSystemProperties systemProperties = uiCommon().virtualPox().GetSystemProperties();
    ...
    const QString &strName = QString("%1_%2.iso").arg(GUI_GuestAdditionsName, uiCommon().vpoxVersionStringNormalized());

    CMediumVector vec = vpox.GetDVDImages();
...
void UIMachineLogic::updateMenuDevicesNetwork(QMenu *pMenu)
{
    ...
    const ULONG uCount = qMin((ULONG)4, uiCommon().virtualPox().GetSystemProperties().GetMaxNetworkAdapters(chipsetType));
