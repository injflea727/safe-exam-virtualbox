/* $Id: tstVPoxAPIXPCOM.cpp $ */
/** @file
 *
 * tstVPoxAPIXPCOM - sample program to illustrate the VirtualPox
 *                   XPCOM API for machine management.
 *                   It only uses standard C/C++ and XPCOM semantics,
 *                   no additional VPox classes/macros/helpers.
 */
 ...
 * to use VirtualPox XPCOM API for learning puroses only. The program uses
 ...
 * ---------------------------
 *
 * This sample program needs to know where the VirtualPox core files reside
 * and where to search for VirtualPox shared libraries. Therefore, you need to
 * use the following (or similar) command to execute it:
 *
 *   $ env VPOX_XPCOM_HOME=../../.. LD_LIBRARY_PATH=../../.. ./tstVPoxAPIXPCOM
 *
 * The above command assumes that VPoxRT.so, VPoxXPCOM.so and others reside in
 * the directory ../../..
 */
...
/*
 * VirtualPox XPCOM interface. This header is generated
 ...
 *
 * @param virtualPox VirtualPox instance object.
 */
void listVMs(IVirtualPox *virtualPox)
{
    nsresult rc;
    ...
    rc = virtualPox->GetMachines(&machineCnt, &machines);
    if (NS_SUCCEEDED(rc))
    {
        /*
        ...
        for (PRUint32 i = 0; i < machineCnt; ++ i)
        {
            ...
            if (machine)
            {
                ...
                if (isAccessible)
                {
                    ...
                    virtualPox->GetGuestOSType(typeId.get(), &osType);
 ...
 * @param virtualPox VirtualPox instance object.
 */
void createVM(IVirtualPox *virtualPox)
{
    nsresult rc;
    /*
    ...
    rc = virtualPox->CreateMachine(NULL,        /* settings file */
    ...
    rc = virtualPox->GetGuestOSType(NS_LITERAL_STRING("Windows2000").get(),
     ...
     * register the VM.
     *
     * Also note that due to current VirtualPox limitations, the machine
     * must be registered *before* we can attach hard disks to it.
     */
    rc = virtualPox->RegisterMachine(machine);
    if (NS_FAILED(rc))
    {
    ...
    rc = virtualPox->CreateMedium(NS_LITERAL_STRING("VDI").get(),
    ...
    rc = virtualPox->OpenMedium(NS_LITERAL_STRING("/home/vpox/isos/winnt4ger.iso").get(),
                                DeviceType_DVD,
                                AccessMode_ReadOnly,
                                false /* fForceNewUuid */,
...
int main()
{
     ...
     * of our main interface, which is IVirtualPox.
     *
     ...
     */
    {
         ...
         * operations to ensure messages from the VirtualPox server and other
         ...
         * IVirtualPox is the root interface of VirtualPox and will be
        ...
        nsCOMPtr<IVirtualPox> virtualPox;
        rc = manager->CreateInstanceByContractID(NS_VIRTUALPOX_CONTRACTID,
                                                 nsnull,
                                                 NS_GET_IID(IVirtualPox),
                                                 getter_AddRefs(virtualPox));
        if (NS_FAILED(rc))
        {
            printf("Error, could not instantiate VirtualPox object! rc=%#x\n", rc);
            return -1;
        }
        printf("VirtualPox object created\n");
        ...
        /* this is enough to free the IVirtualPox instance -- smart pointers rule! */
        virtualPox = nsnull;
 ...
 * extended VirtualPox error info if it is available.
 */
void printErrorInfo()
{
    nsresult rc;
    ...
    if (NS_SUCCEEDED(rc))
    {
        ...
        if (NS_SUCCEEDED(rc))
        {
            ...
            {
                nsCOMPtr<IVirtualPoxErrorInfo> info;
                info = do_QueryInterface(ex, &rc);
                if (NS_SUCCEEDED(rc) && info)
                {
                    /* got extended error info */
                    printf("Extended error info (IVirtualPoxErrorInfo):\n");
