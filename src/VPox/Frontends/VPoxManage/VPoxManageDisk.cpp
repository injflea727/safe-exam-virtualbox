/* $Id: VPoxManageDisk.cpp $ */
/** @file
 * VPoxManage - The disk/medium related commands.
 */
                   ...
                   bool fSilent)
{
    ...
    if (!fSilent)
        CHECK_ERROR(a->virtualPox, OpenMedium(Bstr(pszFilenameOrUuid).raw(),
                                              enmDevType,
                                              enmAccessMode,
                                              fForceNewUuidOnOpen,
                                              pMedium.asOutParam()));
    else
        rc = a->virtualPox->OpenMedium(Bstr(pszFilenameOrUuid).raw(),
                                       enmDevType,
                                       enmAccessMode,
                                       fForceNewUuidOnOpen,
                                       pMedium.asOutParam());
                            ...
                            AccessMode_T enmAccessMode, ComPtr<IMedium> &pMedium)
{
    ...
    CHECK_ERROR(a->virtualPox, CreateMedium(Bstr(pszFormat).raw(),
...
RTEXITCODE handleModifyMedium(HandlerArg *a)
{
        ...
        && fModifyResizeMB)
    {
        ...
        // alternative to VPoxManage. If happens that one wants to resize the disk
    ...
    if (fModifyCompact)
    {
        ...
        if (FAILED(rc))
        {
            ...
            else if (rc == VPOX_E_NOT_SUPPORTED)
    ...
    do
    {
        ...
        if (fOptLong)
        {
            ...
            for (size_t i = 0; i < machineIds.size(); i++)
            {
                ...
                CHECK_ERROR(pVirtualPox, FindMachine(machineIds[i], pMachine.asOutParam()));
                if (pMachine)
                {
                    Bstr name;
...
RTEXITCODE handleShowMediumInfo(HandlerArg *a)
{
    ...
    rc = showMediumInfo(a->virtualPox, pMedium, strParentUUID.c_str(), true);
 ...
 *
 * @returns Exit code.
 * @param   pHandler        The handler state structure (for IVirtualPox).
