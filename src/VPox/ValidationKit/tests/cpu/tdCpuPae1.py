...
"""
VirtualPox Validation Kit - Catch PAE not enabled.
...
class tdCpuPae1ConsoleCallbacks(vpox.ConsoleEventHandlerBase):
    """
    For catching the PAE runtime error.
    """
    def __init__(self, dArgs):
        oTstDrv  = dArgs['oTstDrv'];
        oVPoxMgr = dArgs['oVPoxMgr']; _ = oVPoxMgr;

        vpox.ConsoleEventHandlerBase.__init__(self, dArgs, 'tdCpuPae1');
...
class tdCpuPae1(vpox.TestDriver):
    """
    PAE Test #1.
    """
    ...
    def actionConfig(self):
        ...
        oVM = self.createTestVM('tst-bs-pae', 2, sKind = 'Other', fVirtEx = False, fPae = False, \
                                sFloppy = os.path.join(self.sVPoxBootSectors, 'bootsector-pae.img') );
        if oVM is None:
            return False;
        return True;
