...
# define DHCP_EXECUTABLE_NAME "VPoxNetDHCP.exe"
#else
# define DHCP_EXECUTABLE_NAME "VPoxNetDHCP"
#endif
...
HRESULT DHCPServer::init(VirtualPox *aVirtualPox, const Utf8Str &aName)
{
    ...
    /* share VirtualPox weakly (parent remains NULL so far) */
    ...
    if (SUCCEEDED(hrc))
        hrc = m->globalConfig->initWithDefaults(aVirtualPox, this);
...
HRESULT DHCPServer::init(VirtualPox *aVirtualPox, const settings::DHCPServer &rData)
{
    ...
    HRESULT hrc = m->globalConfig.createObject();
    if (SUCCEEDED(hrc))
        hrc = m->globalConfig->initWithSettings(aVirtualPox, this, rData.globalConfig);
         ...
         it != rData.vecGroupConfigs.end() && SUCCEEDED(hrc); ++it)
    {
        ...
        if (SUCCEEDED(hrc))
            hrc = ptrGroupConfig->initWithSettings(aVirtualPox, this, *it);
        if (SUCCEEDED(hrc))
        {
            try
            {
         ...
         it != rData.mapIndividualConfigs.end() && SUCCEEDED(hrc); ++it)
    {
        ...
        if (it->second.strVMName.isEmpty())
        {
            ...
            if (SUCCEEDED(hrc))
                hrc = ptrIndiCfg->initWithSettingsAndMACAddress(aVirtualPox, this, it->second, &MACAddress);
        }
        else
        {
            ...
            if (SUCCEEDED(hrc))
            {
                ...
                hrc = ptrIndiCfg.createObject();
                if (SUCCEEDED(hrc))
                    hrc = ptrIndiCfg->initWithSettingsAndMachineIdAndSlot(aVirtualPox, this, it->second,
...
/**
 * Called by VirtualPox to save our settings.
 */
...
HRESULT DHCPServer::i_doSaveSettings()
{
    ...
    AutoWriteLock vpoxLock(m->pVirtualPox COMMA_LOCKVAL_SRC_POS);
    return m->pVirtualPox->i_saveSettings();
}
...
HRESULT DHCPServer::i_vmNameToIdAndValidateSlot(const com::Utf8Str &aVmName, LONG aSlot, com::Guid &idMachine)
{
    if ((ULONG)aSlot <= 32)
    {
        ...
        HRESULT hrc = m->pVirtualPox->i_findMachineByName(aVmName, true /*aSetError*/, &ptrMachine);
        if (SUCCEEDED(hrc))
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            if (a_fCreateIfNeeded)
            {
                ...
                if (SUCCEEDED(hrc))
                    hrc = a_rPtrConfig->initWithMachineIdAndSlot(m->pVirtualPox, this, idMachine, a_uSlot,
                                                                 m->uIndividualMACAddressVersion - UINT32_MAX / 4);
                if (SUCCEEDED(hrc))
                {
                    AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
...
HRESULT DHCPServer::start(const com::Utf8Str &aTrunkName, const com::Utf8Str &aTrunkType)
{
     ...
     */
    if (m->dhcp.isRunning())
        return setErrorBoth(VPOX_E_OBJECT_IN_USE, VERR_PROCESS_RUNNING,
    ...
    *aExpire = 0;
    ...
    for (uint32_t uReadAttempt = 0; ; uReadAttempt++)
    {
        ...
        catch (const xml::EIPRTFailure &e)
        {
            ...
            return setErrorBoth(VPOX_E_FILE_ERROR, vrc, "Reading '%s' failed: %Rrc - %s",
                                m->strLeasesFilename.c_str(), vrc, e.what());
        }
        catch (const RTCError &e)
        {
            if (e.what())
                return setError(VPOX_E_FILE_ERROR, "Reading '%s' failed: %s", m->strLeasesFilename.c_str(), e.what());
            return setError(VPOX_E_FILE_ERROR, "Reading '%s' failed: RTCError", m->strLeasesFilename.c_str());
        }
        catch (std::bad_alloc &)
        {
            return E_OUTOFMEMORY;
        ...
        {
            AssertFailed();
            return setError(VPOX_E_FILE_ERROR, tr("Reading '%s' failed: Unexpected exception"), m->strLeasesFilename.c_str());
        }
    ...
    return setError(VPOX_E_OBJECT_NOT_FOUND, tr("Could not find a lease for %RTmac"), &MacAddress);
}
                              ...
                              ComPtr<IDHCPConfig> &aConfig)
{
    ...
    switch (aScope)
    {
        ...
        case DHCPConfigScope_Group:
        {
            ...
            if (!aMayAdd)
                return setError(VPOX_E_OBJECT_NOT_FOUND, tr("Found no configuration for group %s"), aName.c_str());
            ComObjPtr<DHCPGroupConfig> ptrGroupConfig;
            HRESULT hrc = ptrGroupConfig.createObject();
            if (SUCCEEDED(hrc))
                hrc = ptrGroupConfig->initWithDefaults(m->pVirtualPox, this, aName);
            if (SUCCEEDED(hrc))
            {
                AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (RT_SUCCESS(vrc))
                {
                    ...
                    if (aMayAdd)
                    {
                        ...
                        if (SUCCEEDED(hrc))
                            hrc = ptrIndividualConfig->initWithMACAddress(m->pVirtualPox, this, &MACAddress);
                        if (SUCCEEDED(hrc))
                        {
                            AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
                        ...
                        return setError(VPOX_E_OBJECT_NOT_FOUND, tr("Found no configuration for MAC address %s"), strKey.c_str());
                }
                return E_OUTOFMEMORY;
            }
...
HRESULT DHCPServer::i_calcLeasesConfigAndLogFilenames(const com::Utf8Str &aNetwork) RT_NOEXCEPT
{
    ...
    int vrc = m->strLeasesFilename.assignNoThrow(m->pVirtualPox->i_homeDir());
    if (RT_SUCCESS(vrc))
