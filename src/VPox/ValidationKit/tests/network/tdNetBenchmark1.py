...
"""
VirtualPox Validation Kit - Networking benchmark #1.
"""
...
class tdNetBenchmark1(vpox.TestDriver):                                         # pylint: disable=too-many-instance-attributes
    """
    ...
    def actionConfig(self):
        ...
        if self.sGuestName is None and 'g2g' in self.asSetups:
            oVM = self.createTestVM('tst-guest2guest', 0, '3.0/tcp/rhel5.vdi', sKind = 'RedHat', fIoApic = True, \
                    eNic0Type = vpoxcon.NetworkAdapterType_I82545EM, \
                    eNic0AttachType = vpoxcon.NetworkAttachmentType_Bridged, \
                    fVirtEx = True, sDvdImage = sVPoxValidationKit_iso);
            if oVM is None:
                return False;
        ...
        if 'tst-rhel5' in self.asTestVMs:
            oVM = self.createTestVM('tst-rhel5', 1, '3.0/tcp/rhel5.vdi', sKind = 'RedHat', fIoApic = True, \
                eNic0AttachType = eNic0AttachType, sDvdImage = sVPoxValidationKit_iso);
            if oVM is None:
                return False;
    ...
    def test1OneVM(self, sVmName, asSkipNicTypes = (), asSupVirtModes = None, rSupCpus = range(1, 256)):
        ...
        for sNicType in self.asNicTypes:
            ...
            if sNicType == 'E1000':
                eNicType = vpoxcon.NetworkAdapterType_I82545EM;
            elif sNicType == 'PCNet':
                eNicType = vpoxcon.NetworkAdapterType_Am79C973;
            elif sNicType == 'Virtio':
                eNicType = vpoxcon.NetworkAdapterType_Virtio;
            else:
                eNicType = None;
