...
#include <Folders.h>
#include <Files.h>
# ifndef VPOX_WITH_NEWER_OSX_SDK
#  include <Memory.h>
# endif
#include <Processes.h>
#include <Gestalt.h>
...
# ifdef MOZ_DEFAULT_VPOX_XPCOM_HOME
    rv = localFile->InitWithNativePath(nsDependentCString(MOZ_DEFAULT_VPOX_XPCOM_HOME));
    if (NS_SUCCEEDED(rv))
        *aFile = localFile;
# else
    ...
    //	- if VPOX_XPCOM_HOME is defined, that is it
    ...
    if (!fPathSet)
    {
        ...
        /*
         * Env.var. VPOX_XPCOM_HOME first.
         */
        char *psz = PR_GetEnv("VPOX_XPCOM_HOME");
        if (psz)
        {
        ...
        if (buf2[0])
        {
            ...
            if (p)
            {
                p[p == buf2] = '\0';
            #ifdef DEBUG
                printf("debug: (1) VPOX_XPCOM_HOME=%s\n", buf2);
            #endif
                strcpy(szPath, buf2);
                fPathSet = true;
            }
    ...
    // The MOZ_DEFAULT_VPOX_XPCOM_HOME variable can be set at configure time with
    // a --with-default-mozilla-five-home=foo autoconf flag.
    //
    // The idea here is to allow for builds that have a default VPOX_XPCOM_HOME
...
#ifdef MOZ_DEFAULT_VPOX_XPCOM_HOME
    if (PR_GetEnv("VPOX_XPCOM_HOME") == nsnull)
        PR_SetEnv("VPOX_XPCOM_HOME=" MOZ_DEFAULT_VPOX_XPCOM_HOME);
#endif

    char *moz5 = PR_GetEnv("VPOX_XPCOM_HOME");
    ...
    if(!moz5 && firstWarning) {
        // Warn that VPOX_XPCOM_HOME not set, once.
        printf("Warning: VPOX_XPCOM_HOME not set.\n");
    ...
    *aFile = localFile;
    ...
    char *moz5 = getenv("VPOX_XPCOM_HOME");
    if (moz5)
    {
