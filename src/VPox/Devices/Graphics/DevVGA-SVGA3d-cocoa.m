...
- (void)vpoxSetPos:(NSPoint)pos;
- (void)vpoxSetSize:(NSSize)size;
- (void)vpoxScheduleCtxUpdate;
- (void)vpoxReshapePerform;
- (void)vpoxReshape;
- (void)vpoxBoundsDidChange:(NSNotification *)pNotification;
- (void)vpoxFrameDidChange:(NSNotification *)pNotification;
- (void)vpoxFrameDidChangeGlobal:(NSNotification *)pNotification;
- (BOOL)postsFrameChangedNotifications;
- (void)vpoxRemoveFromSuperviewAndHide;
- (void)vpoxUpdateCtxIfNecessary;
- (void)vpoxClearBackBufferIfNecessary;
    ...
    if (self)
    {
        ...
        if (pParentView)
        {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(vpoxBoundsDidChange:)
                                                         name:NSViewBoundsDidChangeNotification
                                                       object:self];
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(vpoxFrameDidChange:)
            ...
            //                                           object:self];
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(vpoxFrameDidChangeGlobal:)
...
- (void)vpoxSetPos:(NSPoint)pos
{
    Log(("OvlView(%p) vpoxSetPos: (%d,%d)\n", (void *)self, (int)pos.x, (int)pos.y));

    m_Pos = pos;
    [self vpoxReshape];

    LogFlow(("OvlView(%p) vpoxSetPos: returns\n", (void *)self));
}


- (void)vpoxSetSize:(NSSize)size
{
    Log(("OvlView(%p) vpoxSetSize: (%d,%d):\n", (void *)self, (int)size.width, (int)size.height));
    m_Size = size;
    [self vpoxReshape];
    LogFlow(("OvlView(%p) vpoxSetSize: returns\n", (void *)self));
}
...
- (void)vpoxUpdateCtxIfNecessary
{
    if (m_fUpdateCtx)
    {
        Log(("OvlView(%p) vpoxUpdateCtxIfNecessary: m_fUpdateCtx\n", (void *)self));
...
- (void)vpoxClearBackBufferIfNecessary
{
    ...
    if (m_cClears > 0)
    {
        ...
        Log(("OvlView(%p) vpoxClearBackBufferIfNecessary: m_cClears=%d\n", (void *)self, m_cClears));
        m_cClears--;
...
- (void)vpoxReshapePerform
{
    ...
    {
        LogFlow(("OvlView(%p) vpoxReshapePerform: moving (%d,%d) -> (%d,%d)\n",
    ...
    {
        LogFlow(("OvlView(%p) vpoxReshapePerform: resizing (%d,%d) -> (%d,%d)\n",
    ...
    m_fUpdateCtx = true;
    LogFlow(("OvlView(%p) vpoxReshapePerform: returns\n", self));
}


- (void)vpoxReshape
{
    LogFlow(("OvlView(%p) vpoxReshape:\n", (void *)self));

    /*
     * Resize the view.
     */
    if ([NSThread isMainThread])
        [self vpoxReshapePerform];
    else
    {
        [self performSelectorOnMainThread:@selector(vpoxReshapePerform) withObject:nil waitUntilDone:NO];
    ...
    LogFlow(("OvlView(%p) vpoxReshape: returns\n", (void *)self));
}
...
- (void)vpoxBoundsDidChange:(NSNotification *)pNotification
{
    ...
    LogFlow(("OvlView(%p) vpoxBoundsDidChange:\n", (void *)self));
...
{
    RT_NOREF(pNotification);
    LogFlow(("OvlView(%p) vpoxFrameDidChange:\n", (void *)self));
    self->m_fUpdateCtx = true;
}
...
{
    RT_NOREF(pNotification);
    LogFlow(("OvlView(%p) vpoxFrameDidChangeGlobal:\n", (void *)self));
    self->m_fUpdateCtx = true;
}

/** This enables the vpoxFrameDidChange notification. */
- (BOOL)postsFrameChangedNotifications
{
...
- (void)vpoxRemoveFromSuperviewAndHide
{
    LogFlow(("OvlView(%p) vpoxRemoveFromSuperviewAndHide:\n", (void *)self));
    if (m_pParentView)
    {
        /*
         ...
         *   #4  0x000000010cffb2bb in -[VMSVGA3DOverlayView vpoxRemoveFromSuperviewAndHide] (self=0x10a1da550, _cmd=0x10cffd734) at DevVGA-SVGA3d-cocoa.m:467
        ...
        if ([NSThread isMainThread])
        {
            LogFlow(("OvlView(%p) vpoxRemoveFromSuperviewAndHide: calling removeFromSuperview\n", (void *)self));
            [self removeFromSuperview];
            LogFlow(("OvlView(%p) vpoxRemoveFromSuperviewAndHide: calling setHidden\n", (void *)self));
            [self setHidden:YES];
            LogFlow(("OvlView(%p) vpoxRemoveFromSuperviewAndHide: calling setHidden\n", (void *)self));
        ...
        {
            LogFlow(("OvlView(%p) vpoxRemoveFromSuperviewAndHide: defering to main thread\n", (void *)self));
            vmsvga3dCocoaServiceRunLoop();
            [self performSelectorOnMainThread:@selector(vpoxRemoveFromSuperviewAndHide) withObject:nil waitUntilDone:YES];
            vmsvga3dCocoaServiceRunLoop();
            LogFlow(("OvlView(%p) vpoxRemoveFromSuperviewAndHide: main thread done\n", (void *)self));
        }
    }
}
...
VMSVGA3DCOCOA_DECL(void) vmsvga3dCocoaDestroyViewAndContext(NativeNSViewRef pView, NativeNSOpenGLContextRef pCtx)
{
    ...
    [pOvlView vpoxRemoveFromSuperviewAndHide];
...
VMSVGA3DCOCOA_DECL(void) vmsvga3dCocoaViewSetPosition(NativeNSViewRef pView, NativeNSViewRef pParentView, int x, int y)
{
    ...
    [(VMSVGA3DOverlayView *)pView vpoxSetPos:NSMakePoint(x, y)];
...
VMSVGA3DCOCOA_DECL(void) vmsvga3dCocoaViewUpdateViewport(NativeNSViewRef pView)
{
    ...
    [pOverlayView vpoxScheduleCtxUpdate];
...
VMSVGA3DCOCOA_DECL(void) vmsvga3dCocoaViewSetSize(NativeNSViewRef pView, int cx, int cy)
{
    ...
    [pOverlayView vpoxSetSize:NSMakeSize(cx, cy)];
...
void vmsvga3dCocoaViewMakeCurrentContext(NativeNSViewRef pView, NativeNSOpenGLContextRef pCtx)
{
    ...
    if (pOverlayView)
    {
        ...
        [pOverlayView vpoxUpdateCtxIfNecessary];
    }
    else
        [NSOpenGLContext clearCurrentContext];
...
void vmsvga3dCocoaSwapBuffers(NativeNSViewRef pView, NativeNSOpenGLContextRef pCtx)
{
    ...
    [pMyView vpoxUpdateCtxIfNecessary];
    [pMyView vpoxClearBackBufferIfNecessary];
