...
/** @file
 * VPox Qt GUI - UIFrameBuffer class implementation.
 */
...
#include "UICommon.h"
#ifdef VPOX_WITH_MASKED_SEAMLESS
# include "UIMachineWindow.h"
#endif /* VPOX_WITH_MASKED_SEAMLESS */
...
#endif /* VPOX_GUI_WITH_QTGLFRAMEBUFFER */
...
class ATL_NO_VTABLE UIFrameBufferPrivate : public QObject,
                                           ...
                                           VPOX_SCRIPTABLE_IMPL(IFramebuffer)
{
    Q_OBJECT;
...
public:
    ...
    /** EMT callback: Notifies frame-buffer about 3D backend event.
      * @param        uType Event type. VPOX3D_NOTIFY_TYPE_*.
...
# endif
    ...
    {
        QString s = uiCommon().virtualPox().GetExtraData("GUI/GLWidget");
...
class VPoxOverlayFrameBuffer : public UIFrameBufferPrivate
{
    Q_OBJECT;

public:

    VPoxOverlayFrameBuffer()
    {
    }
    ...
    virtual HRESULT init(UIMachineView *pView)
    {
        ...
        mOverlay.onResizeEventPostprocess (VPoxFBSizeInfo(this), QPoint(mpView->contentsX(), mpView->contentsY()));
        return S_OK;
    }
    ...
    STDMETHOD(ProcessVHWACommand)(BYTE *pCommand, LONG enmCmd, BOOL fGuestCmd)
    {
        ...
        int rc = mOverlay.onVHWACommand((struct VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *)pCommand, enmCmd, fGuestCmd != FALSE);
        UIFrameBufferPrivate::unlock();
    ...
    void performResize(int iWidth, int iHeight)
    {
        ...
        mOverlay.onResizeEventPostprocess(VPoxFBSizeInfo(this),
...
STDMETHODIMP UIFrameBufferPrivate::Notify3DEvent(ULONG uType, ComSafeArrayIn(BYTE, data))
{
    ...
    switch (uType)
    {
        case VPOX3D_NOTIFY_TYPE_3DDATA_VISIBLE:
        case VPOX3D_NOTIFY_TYPE_3DDATA_HIDDEN:
        {
            /* Notify machine-view with the async-signal
             * about 3D overlay visibility change: */
            BOOL fVisible = uType == VPOX3D_NOTIFY_TYPE_3DDATA_VISIBLE;
            LogRel2(("GUI: UIFrameBufferPrivate::Notify3DEvent: Sending to async-handler: "
                     "(VPOX3D_NOTIFY_TYPE_3DDATA_%s)\n",
        ...
        case VPOX3D_NOTIFY_TYPE_TEST_FUNCTIONAL:
        {
...
#if defined(VPOX_GUI_WITH_QTGLFRAMEBUFFER) && defined(RT_OS_LINUX)
        case VPOX3D_NOTIFY_TYPE_HW_SCREEN_CREATED:
        case VPOX3D_NOTIFY_TYPE_HW_SCREEN_DESTROYED:
        case VPOX3D_NOTIFY_TYPE_HW_SCREEN_UPDATE_BEGIN:
        case VPOX3D_NOTIFY_TYPE_HW_SCREEN_UPDATE_END:
        {
            HRESULT hr = S_OK;
            com::SafeArray<BYTE> notifyData(ComSafeArrayInArg(data));
            if (m_pGLWidget)
            {
                if (uType == VPOX3D_NOTIFY_TYPE_HW_SCREEN_CREATED)
                {
                    LogRel4(("GUI: Notify3DEvent VPOX3D_NOTIFY_TYPE_3D_SCREEN_CREATED\n"));
                ...
                else if (uType == VPOX3D_NOTIFY_TYPE_HW_SCREEN_DESTROYED)
                {
                    LogRel4(("GUI: Notify3DEvent VPOX3D_NOTIFY_TYPE_3D_SCREEN_DESTROYED\n"));

                    GLWidgetSource *p = new GLWidgetSourceImage(m_pGLWidget, &m_image);
                    m_pGLWidget->setSource(p, true);
                }
                else if (uType == VPOX3D_NOTIFY_TYPE_HW_SCREEN_UPDATE_BEGIN)
                {
                    /* Do nothing. */
                }
                else if (uType == VPOX3D_NOTIFY_TYPE_HW_SCREEN_UPDATE_END)
                {
                    struct NotifyData
                    {
                        uint64_t u64NativeHandle;
...
#endif /* defined(VPOX_GUI_WITH_QTGLFRAMEBUFFER) && defined(RT_OS_LINUX) */
...
UIFrameBuffer::UIFrameBuffer(bool m_fAccelerate2DVideo)
{
    ...
    {
        ComObjPtr<VPoxOverlayFrameBuffer> pFrameBuffer;
