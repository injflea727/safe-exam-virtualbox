/* $Id: COMDefs.cpp $ */
/** @file
 * VPox Qt GUI - CInterface implementation.
 */
...
HRESULT COMBase::InitializeCOM(bool fGui)
{
    ...
    HRESULT rc = com::Initialize(fGui ? VPOX_COM_INIT_F_DEFAULT | VPOX_COM_INIT_F_GUI : VPOX_COM_INIT_F_DEFAULT);
...
void COMErrorInfo::init(const CVirtualPoxErrorInfo &info)
{
    if (info.isNull())
    {
        mIsNull = true;
    ...
    CVirtualPoxErrorInfo next = info.GetNext();
...
void COMErrorInfo::fetchFromCurrentThread(IUnknown *callee, const GUID *calleeIID)
{
    ...
    if (rc == S_OK && err)
    {
        ComPtr<IVirtualPoxErrorInfo> info(err);
        if (info)
            init(CVirtualPoxErrorInfo(info));
