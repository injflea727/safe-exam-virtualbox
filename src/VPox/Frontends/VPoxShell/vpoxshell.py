...
"""
VirtualPox Python Shell.

This program is a simple interactive shell for VirtualPox. You can query
information and issue commands from a simple command line.

It also provides you with examples on how to use VirtualPox's Python API.
...
.VirtualPox/shexts/ and drop your extensions there.
                                               Enjoy.
...
def printErr(_ctx, e):
    oVPoxMgr = _ctx['global']
    if oVPoxMgr.xcptIsOurXcptKind(e):
        print(colored('%s: %s' % (oVPoxMgr.xcptToString(e), oVPoxMgr.xcptGetMessage(e)), 'red'))
    else:
...
def createVm(ctx, name, kind):
    vpox = ctx['vb']
    mach = vpox.createMachine("", name, [], kind, "")
    mach.saveSettings()
    print("created machine with UUID", mach.id)
    vpox.registerMachine(mach)
    # update cache
...
def monitorSource(ctx, eventSource, active, dur):
    def handleEventImpl(event):
        evtype = event.type
        print("got event: %s %s" % (str(evtype), asEnumElem(ctx, 'VPoxEventType', evtype)))
        if evtype == ctx['global'].constants.VPoxEventType_OnMachineStateChanged:
        ...
        elif  evtype == ctx['global'].constants.VPoxEventType_OnSnapshotTaken:
        ...
        elif  evtype == ctx['global'].constants.VPoxEventType_OnGuestPropertyChanged:
        ...
        elif  evtype == ctx['global'].constants.VPoxEventType_OnMousePointerShapeChanged:
        ...
        elif evtype == ctx['global'].constants.VPoxEventType_OnGuestMouse:
            mev = ctx['global'].queryInterface(event, 'IGuestMouseEvent')
            if mev:
                printMouseEvent(ctx, mev)
        elif evtype == ctx['global'].constants.VPoxEventType_OnGuestKeyboard:
            kev = ctx['global'].queryInterface(event, 'IGuestKeyboardEvent')
            if kev:
                printKbdEvent(ctx, kev)
        elif evtype == ctx['global'].constants.VPoxEventType_OnGuestMultiTouch:
    ...
    if dur == -1:
        ...
        dur = 100000
    try:
        eventSource.registerListener(listener, [ctx['global'].constants.VPoxEventType_Any], active)
        registered = True
        end = time.time() + dur
        while  time.time() < end:
            if active:
...
def recordDemo(ctx, console, filename, dur):
    ...
    def handleEventImpl(event):
        evtype = event.type
        #print("got event: %s %s" % (str(evtype), asEnumElem(ctx, 'VPoxEventType', evtype)))
        if evtype == ctx['global'].constants.VPoxEventType_OnGuestMouse:
            mev = ctx['global'].queryInterface(event, 'IGuestMouseEvent')
            if mev:
    ...
    if dur == -1:
        ...
        dur = 100000
    try:
        agg.registerListener(listener, [ctx['global'].constants.VPoxEventType_Any], False)
        registered = True
        end = time.time() + dur
        while  time.time() < end:
...
class XPathNodeHolderVM(XPathNodeHolder):
    def __init__(self, parent, vpox):
        XPathNodeHolder.__init__(self, parent, vpox, 'machines', XPathNodeVM, 'vms')
...
def execInGuest(ctx, console, args, env, user, passwd, tmo, inputPipe=None, outputPipe=None):
    if len(args) < 1:
        ...
        return
    guest = console.guest
    guestSession = guest.createSession(user, passwd, "", "vpoxshell guest exec")
...
def hostCmd(ctx, args):
    vpox = ctx['vb']
    try:
        print("VirtualPox version %s" % (colored(vpox.version, 'blue')))
    except Exception as e:
        printErr(ctx, e)
        if g_fVerbose:
            traceback.print_exc()
    props = vpox.systemProperties
    print("Machines: %s" % (colPath(ctx, props.defaultMachineFolder)))

    #print("Global shared folders:")
    #for ud in ctx['global'].getArray(vpox, 'sharedFolders'):
    #    printSf(ctx, sf)
    host = vpox.host
...
def monitorVPoxCmd(ctx, args):
    if len(args) > 2:
        print("usage: monitorVPox (duration)")
        return 0
    dur = 5
    if len(args) > 1:
        dur = float(args[1])
    vpox = ctx['vb']
    active = False
    monitorSource(ctx, vpox.eventSource, active, dur)
    return 0
...
def portForwardCmd(ctx, args):
    ...
    config = "VPoxInternal/Devices/" + adapterType + "/"
...
def connectCmd(ctx, args):
    ...
    try:
        print("Running VirtualPox version %s" % (ctx['vb'].version))
    except Exception as e:
        printErr(ctx, e)
        if g_fVerbose:
            traceback.print_exc()
...
def registerHddCmd(ctx, args):
    ...
    setParentId = False
    parentId = ""
    hdd = vpox.openMedium(loc, ctx['global'].constants.DeviceType_HardDisk, ctx['global'].constants.AccessMode_ReadWrite, False)
...
def registerIsoCmd(ctx, args):
    ...
    vpox = ctx['vb']
    loc = args[1]
    iso = vpox.openMedium(loc, ctx['global'].constants.DeviceType_DVD, ctx['global'].constants.AccessMode_ReadOnly, False)
...
def unregisterIsoCmd(ctx, args):
    ...
    loc = args[1]
    try:
        dvd = vpox.openMedium(loc, ctx['global'].constants.DeviceType_DVD, ctx['global'].constants.AccessMode_ReadOnly, False)
    except:
...
def guiCmd(ctx, args):
    ...
    vpox = os.path.join(binDir, 'VirtualPox')
    try:
        os.system(vpox)
    except KeyboardInterrupt:
...
commands = {'help':['Prints help information', helpCmd, 0],
            ...
            'monitorVPox':['Monitor what happens with VirtualPox for some time: monitorVPox 10', monitorVPoxCmd, 0],
            ...
            'runScript':['Run VPox script: runScript script.vpox', runScriptCmd, 0],
            ...
            'registerHdd': ['Register HDD image with VirtualPox instance: registerHdd /disk.vdi', registerHddCmd, 0],
            'unregisterHdd': ['Unregister HDD image with VirtualPox instance: unregisterHdd /disk.vdi', unregisterHddCmd, 0],
            ...
            'registerIso': ['Register CD/DVD image with VirtualPox instance: registerIso /os.iso', registerIsoCmd, 0],
            'unregisterIso': ['Unregister CD/DVD image with VirtualPox instance: unregisterIso /os.iso', unregisterIsoCmd, 0],
            ...
            'listMedia': ['List media known to this VPox instance', listMediaCmd, 0],
...
#
# To write your own custom commands to vpoxshell, create
# file ~/.VirtualPox/shellext.py with content like
#
...
#
# Also one can put shell extensions into ~/.VirtualPox/shexts and
...
def getHomeFolder(ctx):
    if ctx['remote'] or ctx['vb'] is None:
        if 'VPOX_USER_HOME' in os.environ:
            return os.path.join(os.environ['VPOX_USER_HOME'])
        return os.path.join(os.path.expanduser("~"), ".VirtualPox")
    else:
        return ctx['vb'].homeFolder

def interpret(ctx):
    if ctx['remote']:
        commands['connect'] = ["Connect to remote VPox instance: connect http://server:18083 user password", connectCmd, 0]
        commands['disconnect'] = ["Disconnect from remote VPox instance", disconnectCmd, 0]
        commands['reconnect'] = ["Reconnect to remote VPox instance", reconnectCmd, 0]
        ctx['wsinfo'] = ["http://localhost:18083", "", ""]

    vpox = ctx['vb']
    if vpox is not None:
        try:
            print("Running VirtualPox version %s" % (vpox.version))
        except Exception as e:
            printErr(ctx, e)
            if g_fVerbose:
                traceback.print_exc()
        ctx['perf'] = None # ctx['global'].getPerfCollector(vpox)
    else:
        ctx['perf'] = None
    ...
    if platform.system() in ['Windows', 'Microsoft']:
        global g_fHasColors
        g_fHasColors = False
    hist_file = os.path.join(home, ".vpoxshellhistory")
    autoCompletion(commands, ctx)
    ...
    if ctx['perf']:
        try:
            ctx['perf'].setup(['*'], [vpox.host], 10, 15)
        except:
            pass
    cmds = []
...
def main(argv):
    ...
    if options.autopath:
        ...
        sPath = os.environ.get("VPOX_PROGRAM_PATH")
        if sPath is None:
            for sCurLoc in asLocations:
                if   os.path.isfile(os.path.join(sCurLoc, "VirtualPox")) \
                  or os.path.isfile(os.path.join(sCurLoc, "VirtualPox.exe")):
                    print("Autodetected VPOX_PROGRAM_PATH as", sCurLoc)
                    os.environ["VPOX_PROGRAM_PATH"] = sCurLoc
                    sPath = sCurLoc
                    break
        if sPath:
            sys.path.append(os.path.join(sPath, "sdk", "installer"))

        sPath = os.environ.get("VPOX_SDK_PATH")
        if sPath is None:
            for sCurLoc in asLocations:
                if os.path.isfile(os.path.join(sCurLoc, "sdk", "bindings", "VirtualPox.xidl")):
                    sCurLoc = os.path.join(sCurLoc, "sdk")
                    print("Autodetected VPOX_SDK_PATH as", sCurLoc)
                    os.environ["VPOX_SDK_PATH"] = sCurLoc
                    sPath = sCurLoc
                    break
        if sPath:
            sCurLoc = sPath
    ...
    #
    from vpoxapi import VirtualPoxManager
    oVPoxMgr = VirtualPoxManager(style, params)
    ctx = {
        'global':       oVPoxMgr,
        'vb':           oVPoxMgr.getVirtualPox(),
        'const':        oVPoxMgr.constants,
        'remote':       oVPoxMgr.remote,
        'type':         oVPoxMgr.type,
