...
#define PR_FindLibrary VPoxNsprPR_FindLibrary
#endif /* VPOX_WITH_XPCOM_NAMESPACE_CLEANUP */

#include <string.h>

#ifdef VPOX_USE_MORE_IPRT_IN_NSPR
# include <iprt/ldr.h>
# include <iprt/path.h>
# include <iprt/err.h>
...
# ifdef XP_MAC
#  error "Misconfiguration: XP_MAC && VPOX_USE_MORE_IPRT_IN_NSPR are not intended to work together"
# endif

#else /* ! VPOX_USE_MORE_IPRT_IN_NSPR */
...
#endif /* !VPOX_USE_MORE_IPRT_IN_NSPR */
#ifdef VPOX_USE_IPRT_IN_NSPR
# include <iprt/mem.h>
 ...
 */
#ifndef VPOX_USE_MORE_IPRT_IN_NSPR /* RTLdr hides this. */
...
#if !defined(USE_DLFCN) && !defined(HAVE_STRERROR) && !defined(VPOX_USE_MORE_IPRT_IN_NSPR)
...
{
#if (!defined(XP_MAC) && !defined(XP_BEOS)) || defined(VPOX_USE_MORE_IPRT_IN_NSPR)
    PRLibrary *lm;
#endif
#ifdef XP_UNIX
    void *h;
...
#if defined(XP_PC) || defined(VPOX_USE_MORE_IPRT_IN_NSPR)
...
#else
# if defined(VPOX) && defined(XP_MACOSX)
...
#ifndef VPOX_USE_MORE_IPRT_IN_NSPR /* exclude big chunk */
...
PR_LoadStaticLibrary(const char *name, const PRStaticLinkTable *slt)
{
    ...
    lm->name = strdup(name);
    lm->refCount    = 1;
#if defined(XP_MAC) && !defined(VPOX_USE_MORE_IPRT_IN_NSPR)
