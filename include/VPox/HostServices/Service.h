...
#ifndef VPOX_INCLUDED_HostServices_Service_h
#define VPOX_INCLUDED_HostServices_Service_h
...
public:
    ...
    Message(uint32_t uMsg, uint32_t cParms, VPOXHGCMSVCPARM aParms[]);

    virtual ~Message(void);

    uint32_t GetParamCount(void) const;

    int GetData(uint32_t uMsg, uint32_t cParms, VPOXHGCMSVCPARM aParms[]) const;
...
public:
    ...
    static int CopyParms(PVPOXHGCMSVCPARM paParmsDst, uint32_t cParmsDst,
                         PVPOXHGCMSVCPARM paParmsSrc, uint32_t cParmsSrc,
                         bool fDeepCopy);

protected:

    int initData(uint32_t uMsg, uint32_t cParms, VPOXHGCMSVCPARM aParms[]);
...
public:
    ...
    int Complete(VPOXHGCMCALLHANDLE hHandle, int rcOp = VINF_SUCCESS);

    int CompleteDeferred(int rcOp = VINF_SUCCESS);

    uint32_t GetClientID(void) const;

    VPOXHGCMCALLHANDLE GetHandle(void) const;
    ...
    void SetDeferred(VPOXHGCMCALLHANDLE hHandle, uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);

    void SetSvcContext(const VPOXHGCMSVCTX &SvcCtx);
...
protected:
    ...
    int completeInternal(VPOXHGCMCALLHANDLE hHandle, int rcOp);
...
public:
     ...
     */
    static DECLCALLBACK(int) svcLoad(VPOXHGCMSVCFNTABLE *pTable)
    {
...
protected:
    explicit AbstractService(PVPOXHGCMSVCHELPERS pHelpers)
    {
        RT_ZERO(m_SvcCtx);
        m_SvcCtx.pHelpers = pHelpers;
    }
    virtual int  init(VPOXHGCMSVCFNTABLE *ptable) { RT_NOREF1(ptable); return VINF_SUCCESS; }
    ...
    virtual void guestCall(VPOXHGCMCALLHANDLE callHandle, uint32_t u32ClientID, void *pvClient, uint32_t eFunction, uint32_t cParms, VPOXHGCMSVCPARM paParms[]) = 0;
    virtual int  hostCall(uint32_t eFunction, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
    ...
    /**
     * @copydoc VPOXHGCMSVCFNTABLE::pfnUnload
    ...
    /**
     * @copydoc VPOXHGCMSVCFNTABLE::pfnConnect
    ...
    /**
     * @copydoc VPOXHGCMSVCFNTABLE::pfnCall
    ...
    /**
     * @copydoc VPOXHGCMSVCFNTABLE::pfnHostCall
    ...
    /**
     * @copydoc VPOXHGCMSVCFNTABLE::pfnRegisterExtension
...
#endif /* !VPOX_INCLUDED_HostServices_Service_h */
