/* $Id: VPoxDispDrawCmd.cpp $ */
/** @file
 * VPox XPDM Display driver drawing interface functions
 */
 ...
 *                  bitmap updates.
 *
 * These modes affect only VPoxDispDrv* functions in this file.
 *
...
#include "VPoxDisp.h"
#include "VPoxDispDrawCmd.h"

typedef struct _VPOXDISPCALLSTATS
{
    ULONG VPoxDispDrvLineTo;
    ULONG VPoxDispDrvStrokePath;
    ULONG VPoxDispDrvFillPath;
    ULONG VPoxDispDrvTextOut;
    ULONG VPoxDispDrvPaint;
    ULONG VPoxDispDrvSaveScreenBits;
    ULONG VPoxDispDrvBitBlt;
    ULONG VPoxDispDrvStretchBlt;
    ULONG VPoxDispDrvCopyBits;
} VPOXDISPCALLSTATS;

static VPOXDISPCALLSTATS gDispCallStats = {0,0,0,0,0,0,0,0,0};

#ifdef STAT_sunlover
# define STATDRVENTRY(a, b) do {if (VPoxDispIsScreenSurface(b)) gDispCallStats.VPoxDispDrv##a++; } while (0)
# define STATPRINT do {VPoxDispPrintStats();} while (0)
# define DUMPSURF(_s, _name) VPoxDispDumpPSO(_s, _name);
#else
...
#define VBVA_OPERATION(__psoDest, __fn, __a) do {                            \
    if (VPoxDispIsScreenSurface(__psoDest))                                  \
    {                                                                        \
        PVPOXDISPDEV pDev = (PVPOXDISPDEV)__psoDest->dhpdev;                 \
                                                                             \
        ...
        {                                                                    \
            ...
            if (  pDev->vbvaCtx.pVBVA->hostFlags.u32HostEvents               \
                & VPOX_VIDEO_INFO_HOST_EVENTS_F_VRDP_RESET)                  \
            {                                                                \
                vrdpReset(pDev);                                             \
                                                                             \
...
static void VPoxDispPrintStats(void)
{
    LOG(("LineTo = %u\n",
         "StrokePath = %u\n",
         "FillPath = %u\n",
         ...
         "StretchBlt = %u\n",
         "CopyBits = %u",
         gDispCallStats.VPoxDispDrvLineTo,
         gDispCallStats.VPoxDispDrvStrokePath,
         gDispCallStats.VPoxDispDrvFillPath,
         gDispCallStats.VPoxDispDrvTextOut,
         gDispCallStats.VPoxDispDrvPaint,
         gDispCallStats.VPoxDispDrvSaveScreenBits,
         gDispCallStats.VPoxDispDrvBitBlt,
         gDispCallStats.VPoxDispDrvStretchBlt,
         gDispCallStats.VPoxDispDrvCopyBits
       ));
}
...
static void ssbDiscardTopSlot(PVPOXDISPDEV pDev)
{
...
static void ssbDiscardUpperSlots(PVPOXDISPDEV pDev, ULONG_PTR ident)
{
...
BOOL APIENTRY
VPoxDispDrvLineTo(SURFOBJ *pso, CLIPOBJ *pco, BRUSHOBJ *pbo, LONG x1, LONG y1, LONG x2, LONG y2,
...
BOOL APIENTRY
VPoxDispDrvStrokePath(SURFOBJ *pso, PATHOBJ *ppo, CLIPOBJ *pco, XFORMOBJ *pxo,
...
BOOL APIENTRY
VPoxDispDrvFillPath(SURFOBJ *pso, PATHOBJ *ppo, CLIPOBJ *pco, BRUSHOBJ *pbo, POINTL *pptlBrushOrg,
...
BOOL APIENTRY
VPoxDispDrvTextOut(SURFOBJ *pso, STROBJ *pstro, FONTOBJ *pfo, CLIPOBJ *pco,
...
BOOL APIENTRY
VPoxDispDrvBitBlt(SURFOBJ *psoTrg, SURFOBJ *psoSrc, SURFOBJ *psoMask, CLIPOBJ *pco, XLATEOBJ *pxlo,
...
BOOL APIENTRY
VPoxDispDrvStretchBlt(SURFOBJ *psoDest, SURFOBJ *psoSrc, SURFOBJ *psoMask, CLIPOBJ *pco, XLATEOBJ *pxlo,
...
BOOL APIENTRY
VPoxDispDrvCopyBits(SURFOBJ *psoDest, SURFOBJ *psoSrc, CLIPOBJ *pco, XLATEOBJ *pxlo,
                    RECTL *prclDest, POINTL *pptlSrc)
{
    ...
    BOOL fDo = TRUE;
    if (   psoSrc
        && !VPoxDispIsScreenSurface(psoSrc)
        && VPoxDispIsScreenSurface(psoDest))
    {
        PVPOXDISPDEV pDev = ((PVPOXDISPDEV))psoDest->dhpdev;
