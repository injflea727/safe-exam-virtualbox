/* $Id: VPoxManageControlVM.cpp $ */
/** @file
 * VPoxManage - Implementation of the controlvm command.
 */
...
unsigned int getMaxNics(IVirtualPox* vpox, IMachine* mach)
{
    ...
    do {
        CHECK_ERROR_BREAK(vpox, COMGETTER(SystemProperties)(info.asOutParam()));
...
# endif /* VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS */
        ...
        else if (!strncmp(a->argv[1], "setlinkstate", 12))
        {
            ...
            ULONG NetworkAdapterCount = getMaxNics(a->virtualPox, sessionMachine);
...
#undef ITERATE_TO_NEXT_TERM
                 ...
                 || !strcmp(a->argv[1], "usbdetach"))
        {
            ...
            if (!guid.isValid())
            {
                ...
                {
                    ComPtr<IHost> host;
                    CHECK_ERROR_BREAK(a->virtualPox, COMGETTER(Host)(host.asOutParam()));
        ...
        else if (!strcmp(a->argv[1], "dvdattach"))
        {
            ...
            {
                ComPtr<IHost> host;
                CHECK_ERROR(a->virtualPox, COMGETTER(Host)(host.asOutParam()));
            ...
            else
            {
                ...
                uuid = a->argv[2];
                rc = a->virtualPox->GetDVDImage(uuid, dvdMedium.asOutParam());
                if (FAILED(rc) || !dvdMedium)
                {
                    /* must be a filename, check if it's in the collection */
                    rc = a->virtualPox->FindDVDImage(Bstr(a->argv[2]), dvdMedium.asOutParam());
                    /* not registered, do that on the fly */
                    if (!dvdMedium)
                    {
                        Bstr emptyUUID;
                        CHECK_ERROR(a->virtualPox, OpenDVDImage(Bstr(a->argv[2]), emptyUUID, dvdMedium.asOutParam()));
                    }
                }
                if (!dvdMedium)
                {
        ...
        else if (!strcmp(a->argv[1], "floppyattach"))
        {
            ...
            else
            {
                /* first assume it's a UUID */
                uuid = a->argv[2];
                rc = a->virtualPox->GetFloppyImage(uuid, floppyMedium.asOutParam());
                if (FAILED(rc) || !floppyMedium)
                {
                    /* must be a filename, check if it's in the collection */
                    rc = a->virtualPox->FindFloppyImage(Bstr(a->argv[2]), floppyMedium.asOutParam());
                    /* not registered, do that on the fly */
                    if (!floppyMedium)
                    {
                        Bstr emptyUUID;
                        CHECK_ERROR(a->virtualPox, OpenFloppyImage(Bstr(a->argv[2]), emptyUUID, floppyMedium.asOutParam()));
                    }
                }
                if (!floppyMedium)
                {
        ...
        else if (!strcmp(a->argv[1], "teleport"))
        {
            ...
            if (cMsTimeout)
            {
                ...
                if (FAILED(rc) && rc != VPOX_E_INVALID_OBJECT_STATE)
