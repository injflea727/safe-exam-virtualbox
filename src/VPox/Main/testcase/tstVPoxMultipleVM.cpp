/** @file
 * tstVPoxMultipleVM - load test for ClientWatcher.
 */
...
static int tstStartVM(IVirtualPox *pVPox, ISession *pSession, Bstr machineID, bool fSkipUnlock)
{
    HRESULT rc;
    ...
    rc = TST_COM_EXPR(pVPox->FindMachine(machineID.raw(), machine.asOutParam()));
    if(SUCCEEDED(rc))
...
static int tstStopVM(IVirtualPox* pVPox, ISession* pSession, Bstr machineID, bool fSkipUnlock)
{
    ComPtr<IMachine> machine;
    HRESULT rc = TST_COM_EXPR(pVPox->FindMachine(machineID.raw(), machine.asOutParam()));
    if (SUCCEEDED(rc))
    {
        Bstr machineName;
        rc = TST_COM_EXPR(machine->COMGETTER(Name)(machineName.asOutParam()));
 ...
 */
static int tstGetMachinesList(IVirtualPox *pVPox, uint32_t maxCount, TMachinesList &listToFill)
{
    com::SafeIfaceArray<IMachine> machines;
    HRESULT rc = TST_COM_EXPR(pVPox->COMGETTER(Machines)(ComSafeArrayAsOutParam(machines)));
    if (SUCCEEDED(rc))
    {
...
static int tstMachinesPack(IVirtualPox *pVPox, uint32_t maxPackSize, uint32_t percentage)
{
    HRESULT rc = S_OK;
    ...
    tstGetMachinesList(pVPox, maxPackSize, machinesList);
         ...
         ++it)
    {
        ...
        if (SUCCEEDED(rc))
        {
            rc = tstStartVM(pVPox, session, *it, !(alwaysUnlock || g_Counter++ % percN));
        }
         ...
         ++it)
    {
        ...
        if (SUCCEEDED(rc))
        {
            ...
            rc = tstStopVM(pVPox, session, *it, !(alwaysUnlock || g_Counter++ % percN));
        }
        RTSemEventSignal(g_PingEevent);
        RTThreadSleep(100);
    }
...
static int tstCreateMachines(IVirtualPox *pVPox)
{
    HRESULT rc = S_OK;
    ...
    for (uint32_t i = 0; i < g_Args.numberMachines; i++)
    {
        ...
        /* Default VM settings */
        CHECK_ERROR_L(pVPox, CreateMachine(NULL,                          /* Settings */
        ...
        if (SUCCEEDED(rc))
        {
            CHECK_ERROR_L(pVPox, RegisterMachine(ptrMachine));
...
static int tstClean(IVirtualPox *pVPox, IVirtualPoxClient *pClient)
{
    NOREF(pClient);
    HRESULT rc = S_OK;
    ...
    for (uint32_t i = 0; i < g_Args.numberMachines; i++)
    {
        ...
        CHECK_ERROR_L(pVPox, FindMachine(machineName.raw(), machine.asOutParam()));
        ...
        if (SUCCEEDED(rc))
        {
                    ...
                    || machineState == MachineState_Paused) )
            {
                rc = session.createInprocObject(CLSID_Session);
                if (SUCCEEDED(rc))
                    tstStopVM(pVPox, session, machineName, FALSE);
            }
        }
...
static DECLCALLBACK(int) tstThreadRun(RTTHREAD hThreadSelf, void *pvUser)
{
    ...
    if (SUCCEEDED(rc))
    {
        ...
        rc = TST_COM_EXPR(ptrVPoxClient.createInprocObject(CLSID_VirtualPoxClient));
        if (SUCCEEDED(rc))
            rc = TST_COM_EXPR(ptrVPoxClient->COMGETTER(VirtualPox)(ptrVPox.asOutParam()));
        if (SUCCEEDED(rc))
        {
            RTPrintf("Creating machines...\n");
            tstCreateMachines(ptrVPox);

            while (g_RunTest)
            {
                rc = tstMachinesPack(ptrVPox, maxPackSize, percentage);
            }

            RTPrintf("Deleting machines...\n");
            tstClean(ptrVPox, ptrVPoxClient);
        }
 ...
 *
 * Examples:
 *   - tstVPoxClientWatcherLoad --packsize 500 --lock 10 --time 14400 --machines 4000
 ...
 *
 *   - tstVPoxClientWatcherLoad --packsize 1 --lock 30 --time 3600 --machines 1000
...
{
    RT_NOREF(argc, argv);
    RTEXITCODE rcExit = RTTestInitAndCreate("tstVPoxMultipleVM", &g_hTest);
