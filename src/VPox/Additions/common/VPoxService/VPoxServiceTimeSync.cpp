/* $Id: VPoxServiceTimeSync.cpp $ */
/** @file
 * VPoxService - Guest Additions TimeSync Service.
 */
...
/** @page pg_vgsvc_timesync     VPoxService - The Time Sync Service
 *
 ...
 *      -# Inaccuracies in time source VirtualPox uses.
...
static DECLCALLBACK(int) vgsvcTimeSyncPreInit(void)
{
    ...
    {
        rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-interval",
                                 &g_TimeSyncInterval, 50, UINT32_MAX - 1);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-min-adjust",
                                     &g_cMsTimeSyncMinAdjust, 0, 3600000);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-latency-factor",
                                     &g_TimeSyncLatencyFactor, 1, 1024);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-max-latency",
                                     &g_cMsTimeSyncMaxLatency, 1, 3600000);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-set-threshold",
                                     &g_TimeSyncSetThreshold, 0, 7*24*60*60*1000 /* a week */);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
        {
            rc = VGSvcCheckPropExist(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-set-start");
            if (RT_SUCCESS(rc))
        ...
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
        {
            rc = VGSvcCheckPropExist(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-no-set-start");
            if (RT_SUCCESS(rc))
        ...
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
        {
            rc = VGSvcCheckPropExist(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-set-on-restore");
            if (RT_SUCCESS(rc))
        ...
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
        {
            rc = VGSvcCheckPropExist(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-no-set-on-restore");
            if (RT_SUCCESS(rc))
        ...
        {
            uint32_t uValue;
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--timesync-verbosity",
