...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
#if defined(IN_RING3) || defined(VPOX_HDA_WITH_ON_REG_ACCESS_DMA)
 ...
 *
 * @returns VPox status code. VINF_NO_CHANGE if the stream does not need
...
# ifdef VPOX_WITH_DTRACE
    VPOXDD_HDA_STREAM_SETUP((uint32_t)uSD, rc, pStreamShared->State.Cfg.Props.uHz,
...
# ifdef VPOX_WITH_DTRACE
    VPOXDD_HDA_STREAM_RESET((uint32_t)uSD);
# endif
    LogFunc(("[SD%RU8] Reset\n", uSD));
...
#endif /* IN_RING3 || VPOX_HDA_WITH_ON_REG_ACCESS_DMA */
#ifdef IN_RING3
...
#endif /* defined(IN_RING3) || defined(VPOX_HDA_WITH_ON_REG_ACCESS_DMA) */
#ifdef IN_RING3
...
# ifdef VPOX_WITH_DTRACE
                VPOXDD_HDA_STREAM_DMA_IN((uint32_t)uSD, (uint32_t)cbBufSrc, pStreamShared->State.offRead);
# endif
...
# ifdef VPOX_WITH_DTRACE
            VPOXDD_HDA_STREAM_DMA_OUT((uint32_t)uSD, (uint32_t)cbBufDst, pStreamShared->State.offWrite);
# endif
 ...
 */
VPOXSTRICTRC hdaStreamDoOnAccessDmaOutput(PPDMDEVINS pDevIns, PHDASTATE pThis, PHDASTREAM pStreamShared,
...
# ifdef VPOX_WITH_DTRACE
            VPOXDD_HDA_STREAM_DMA_OUT((uint32_t)pStreamShared->u8SD, cbChunk, pStreamShared->State.offWrite);
# endif
 ...
 *                          calculation.
 */
VPOXSTRICTRC hdaStreamMaybeDoOnAccessDmaOutput(PPDMDEVINS pDevIns, PHDASTATE pThis, PHDASTREAM pStreamShared, uint64_t tsNow)
{
...
# endif
            VPOXSTRICTRC   rcStrict = hdaStreamDoOnAccessDmaOutput(pDevIns, pThis, pStreamShared, tsNow, offNow - cbDmaTotal);
            LogFlowFunc(("[SD%RU8] LPIB=%#RX32 -> LPIB=%#RX32 offNow=%#x rcStrict=%Rrc\n", pStreamShared->u8SD,
                         uOldLpib, HDA_STREAM_REG(pThis, LPIB, pStreamShared->u8SD), offNow, VPOXSTRICTRC_VAL(rcStrict) ));
            return rcStrict;
        }
