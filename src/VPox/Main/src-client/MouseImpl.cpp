...
HRESULT Mouse::init (ConsoleMouseInterface *parent)
{
    ...
    mMouseEvent.init(mEventSource, VPoxEventType_OnGuestMouse,
                     0, 0, 0, 0, 0, 0);
                             ...
                             LONG fButtons)
{
    ...
    if (fButtons != 0)
    {
        ...
        evDesc.init(mEventSource, VPoxEventType_OnGuestMouse, mode, x, y,
                    dz, dw, fButtons);
        evDesc.fire(0);
    }
    else
    {
        mMouseEvent.reinit(VPoxEventType_OnGuestMouse, mode, x, y, dz, dw,
                           fButtons);
        mMouseEvent.fire(0);
    }
}
                                  ...
                                  uint32_t u32ScanTime)
{
    ...
    VPoxEventDesc evDesc;
    evDesc.init(mEventSource, VPoxEventType_OnGuestMultiTouch,
...
/**
 * Send an absolute mouse event to the VM. This requires either VirtualPox-
