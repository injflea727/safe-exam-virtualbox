/* $Id: VPoxGuestInstallHelper.cpp $ */
/** @file
 * VPoxGuestInstallHelper - Various helper routines for Windows guest installer.
 *                          Works with NSIS 3.x.
 */
...
/* Required structures/defines of VPoxTray. */
#include "../../VPoxTray/VPoxTrayMsg.h"


/*********************************************************************************************************************************
*   Defined Constants And Macros                                                                                                 *
*********************************************************************************************************************************/
#define VPOXINSTALLHELPER_EXPORT extern "C" void __declspec(dllexport)
 ...
 * overflows, use vpoxPopString() instead!
 *
 ...
 */
static int vpoxPopString(TCHAR *pszDest, size_t cchDest)
{
    int rc = VINF_SUCCESS;
...
static int vpoxPopULong(PULONG pulValue)
{
    int rc = VINF_SUCCESS;
...
static void vpoxPushHResultAsString(HRESULT hr)
{
...
static void vpoxPushRcAsString(int rc)
{
    TCHAR szErr[NSIS_MAX_STRLEN];
    if (RT_FAILURE(rc))
    {
...
/**
 * Connects to VPoxTray IPC under the behalf of the user running
 * in the current thread context.
 *
 * @return  IPRT status code.
 * @param   phSession               Where to store the IPC session.
 */
static int vpoxConnectToVPoxTray(RTLOCALIPCSESSION *phSession)
{
 ...
 */
VPOXINSTALLHELPER_EXPORT FileGetArchitecture(HWND hwndParent, int string_size, TCHAR *variables, stack_t **stacktop,
                                             extra_parameters *extra)
{
    ...
    int rc = vpoxPopString(szFile, sizeof(szFile) / sizeof(TCHAR));
    if (RT_SUCCESS(rc))
    {
#ifdef UNICODE
        char *pszFileUtf8;
 ...
 * @param   extra               Extra parameters. Currently unused.
 */
VPOXINSTALLHELPER_EXPORT FileGetVendor(HWND hwndParent, int string_size, TCHAR *variables, stack_t **stacktop,
                                       extra_parameters *extra)
{
    RT_NOREF(hwndParent, extra);
    ...
    if (RT_FAILURE(rc))
        vpoxPushRcAsString(rc);
}

/**
 * Shows a balloon message using VPoxTray's notification area in the
 * Windows task bar.
 *
 * @param   hwndParent          Window handle of parent.
 * @param   string_size         Size of variable string.
 ...
 * @param   extra               Extra parameters. Currently unused.
 */
VPOXINSTALLHELPER_EXPORT VPoxTrayShowBallonMsg(HWND hwndParent, int string_size, TCHAR *variables, stack_t **stacktop,
                                               extra_parameters *extra)
{
    RT_NOREF(hwndParent, extra);
    ...
    TCHAR szMsg[256];
    TCHAR szTitle[128];
    int rc = vpoxPopString(szMsg, sizeof(szMsg) / sizeof(TCHAR));
    if (RT_SUCCESS(rc))
        rc = vpoxPopString(szTitle, sizeof(szTitle) / sizeof(TCHAR));
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            uint32_t cbMsg = sizeof(VPOXTRAYIPCMSG_SHOWBALLOONMSG)
                           + (uint32_t)strlen(pszMsgUtf8)   + 1  /* Include terminating zero */
                           + (uint32_t)strlen(pszTitleUtf8) + 1; /* Ditto. */
            Assert(cbMsg);
            PVPOXTRAYIPCMSG_SHOWBALLOONMSG pIpcMsg = (PVPOXTRAYIPCMSG_SHOWBALLOONMSG)RTMemAlloc(cbMsg);
            if (pIpcMsg)
            {
                ...
                rc = vpoxPopULong((ULONG *)&pIpcMsg->uType);
                if (RT_SUCCESS(rc))
                    rc = vpoxPopULong((ULONG *)&pIpcMsg->uShowMS);

                if (RT_SUCCESS(rc))
                {
                    RTLOCALIPCSESSION hSession = 0;
                    rc = vpoxConnectToVPoxTray(&hSession);
                    if (RT_SUCCESS(rc))
                    {
                        VPOXTRAYIPCHEADER ipcHdr = { VPOXTRAY_IPC_HDR_MAGIC, 0 /* Header version */,
                                                     VPOXTRAYIPCMSGTYPE_SHOWBALLOONMSG, cbMsg };
