 ...
 */
EFI_STATUS VPoxBmQueryMediaFileNameForSFs(EFI_HANDLE hSFs, CHAR16 **ppwszFileName)
{
  ...
  *ppwszFileName = EFI_REMOVABLE_MEDIA_FILE_NAME;
  ...
  if (!EFI_ERROR(Status))
  {
    ...
    {
#if 0
# define VPOX_EFI_APPLE_MEDIA_FILE_NAME L"\\System\\Library\\CoreServices\\boot.efi"
       EFI_FILE_PROTOCOL *pFile = NULL;

       Status = pRoot->Open(pRoot, &pFile, VPOX_EFI_APPLE_MEDIA_FILE_NAME, EFI_FILE_MODE_READ,
                            EFI_FILE_READ_ONLY | EFI_FILE_HIDDEN | EFI_FILE_SYSTEM);
       if (!EFI_ERROR(Status))
       {
         *ppwszFileName = VPOX_EFI_APPLE_MEDIA_FILE_NAME;
         pFile->Close(pFile);
       }
#else /* Doesn't quite work yet. */
      VPOX_FS_BLESSED_FILE *Buffer = NULL;
      UINTN BufferSize = 0;

      Status = pRoot->GetInfo(pRoot, &gVPoxFsBlessedFileInfoGuid, &BufferSize, Buffer);
      if (Status == EFI_BUFFER_TOO_SMALL) {
        ...
        if (!EFI_ERROR(Status))
        {
          DEBUG ((EFI_D_INFO, "[Bds] VPoxBmQueryMediaFileNameForSFs: Got blessed file info %s\n", &Buffer->BlessedFile[0]));
...
#else
    Status = VPoxBmQueryMediaFileNameForSFs(Handle, &pwszFilename);
    if (!EFI_ERROR(Status))
...
#else
      Status = VPoxBmQueryMediaFileNameForSFs(SimpleFileSystemHandles[Index], &pwszFilename);
      if (!EFI_ERROR(Status))
