...
/** @file
 * VPox Qt GUI - UIMachineWindow class implementation.
 */
...
void UIMachineWindow::prepare()
{
    ...
    const QString strWindowClass = QString("VirtualPox Machine");
    ...
    if (gEDataManager->distinguishMachineWindowGroups(uiCommon().managedVMUuid()))
        strWindowName = QString("VirtualPox Machine UUID: %1").arg(uiCommon().managedVMUuid().toString());
...
void UIMachineWindow::closeEvent(QCloseEvent *pCloseEvent)
{
    ...
    if (closeAction == MachineCloseAction_Invalid)
    {
        ...
        {
            /* Else user misconfigured .vpox file, we will reject closing UI: */
            closeAction = MachineCloseAction_Invalid;
        }
...
void UIMachineWindow::prepareMachineView()
{
    ...
    bool bAccelerate2DVideo = machine().GetGraphicsAdapter().GetAccelerate2DVideoEnabled() && VPox2DHelpers::isAcceleration2DVideoAvailable();
