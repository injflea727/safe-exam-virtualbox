...
class VirtualTestSheriffCaseFile(object):
    ...
    def __init__(self, oSheriff, oTestSet, oTree, oBuild, oTestBox, oTestGroup, oTestCase):
        ...
        self.sSvcLog        = '';           # The VPoxSVC log file.  Empty string if not accessible.
    ...
    def isVPoxTest(self):
        """ Test classification: VirtualPox (using the build) """
        return self.oBuild.oCat.sProduct.lower() in [ 'virtualpox', 'vpox' ];

    def isVPoxUnitTest(self):
        """ Test case classification: The unit test doing all our testcase/*.cpp stuff. """
        return self.isVPoxTest() \
           and (self.oTestCase.sName.lower() == 'unit tests' or self.oTestCase.sName.lower() == 'misc: unit tests');

    def isVPoxInstallTest(self):
        """ Test case classification: VirtualPox Guest installation test. """
        return self.isVPoxTest() \
           and self.oTestCase.sName.lower().startswith('install:');

    def isVPoxUnattendedInstallTest(self):
        """ Test case classification: VirtualPox Guest installation test. """
        return self.isVPoxTest() \
           and self.oTestCase.sName.lower().startswith('uinstall:');

    def isVPoxUSBTest(self):
        """ Test case classification: VirtualPox USB test. """
        return self.isVPoxTest() \
           and self.oTestCase.sName.lower().startswith('usb:');

    def isVPoxStorageTest(self):
        """ Test case classification: VirtualPox Storage test. """
        return self.isVPoxTest() \
           and self.oTestCase.sName.lower().startswith('storage:');

    def isVPoxGAsTest(self):
        """ Test case classification: VirtualPox Guest Additions test. """
        return self.isVPoxTest() \
           and (   self.oTestCase.sName.lower().startswith('guest additions')
                or self.oTestCase.sName.lower().startswith('ga\'s tests'));

    def isVPoxAPITest(self):
        """ Test case classification: VirtualPox API test. """
        return self.isVPoxTest() \
           and self.oTestCase.sName.lower().startswith('api:');

    def isVPoxBenchmarkTest(self):
        """ Test case classification: VirtualPox Benchmark test. """
        return self.isVPoxTest() \
           and self.oTestCase.sName.lower().startswith('benchmark:');

    def isVPoxSmokeTest(self):
        """ Test case classification: Smoke test. """
        return self.isVPoxTest() \
           and self.oTestCase.sName.lower().startswith('smoketest');

    def isVPoxSerialTest(self):
        """ Test case classification: Smoke test. """
        return self.isVPoxTest() \
    ...
    def getSvcLog(self):
        """
        Tries to read the VPoxSVC log file as it typically not associated with a failing test result.
        Note! Returns the first VPoxSVC log file we find.
        """
        if not self.sSvcLog:
...
class VirtualTestSheriff(object): # pylint: disable=too-few-public-methods
    ...
    ktReason_XPCOM_VPoxSVC_Hang                        = ( 'API / (XP)COM',     'VPoxSVC hang' );
    ktReason_XPCOM_VPoxSVC_Hang_Plus_Heap_Corruption   = ( 'API / (XP)COM',     'VPoxSVC hang + heap corruption' );
    ...
    ktReason_GuestBug_CompizVPoxQt                     = ( 'Guest Bug',         'Compiz + VirtualPox Qt GUI crash' );
    ## @}
    ...
    kdatSimpleInstallUninstallMainLogReasonsPerOs = {
        'darwin': [
            ...
            ( True, ktReason_Host_DriverNotUnloading,
              'Can\'t remove kext org.virtualpox.kext.VPoxDrv; services failed to terminate - 0xe00002c7' ),
        ],
        'linux': [
            # ( Whether to stop on hit, reason tuple, needle text. )
            ...
            ( True, ktReason_Host_InstallationFailed,
              'vpoxdrv.sh: failed: Look at /var/log/vpox-install.log to find out what went wrong.' ),
            ( True, ktReason_Host_DriverNotUnloading,
              'Cannot unload module vpoxdrv'),
        ],
        'solaris': [
            # ( Whether to stop on hit, reason tuple, needle text. )
    ...
    def investigateVPoxUnitTest(self, oCaseFile):
        """
        Checks out a VPox unittest problem.
        """
        ...
        for oFailedResult in aoFailedResults:
            ...
            elif oFailedResult.sName == 'Installing VirtualPox':
            ...
            elif oFailedResult.sName == 'Uninstalling VirtualPox':
                sResultLog = TestSetData.extractLogSectionElapsed(sMainLog, oFailedResult.tsCreated, oFailedResult.tsElapsed);
    ...
    katSimpleMainAndVmLogReasonsDeprecated = [
        ...
        #           "VMSetError: VD: Backend 'VPoxIsoMaker' does not support async I/O"
    ...
    ## Things we search a VPoxHardening.log file for to figure out why something went bust.
    katSimpleVPoxHardeningLogReasons = [
        # ( Whether to stop on hit, reason tuple, needle text. )
        ( True,  ktReason_Host_DriverNotLoaded,                     'Error opening VPoxDrvStub:  STATUS_OBJECT_NAME_NOT_FOUND' ),
    ...
    katSimpleKernelLogReasons = [
        ...
        ( True,  ktReason_GuestBug_CompizVPoxQt,                    'error 4 in libQt5CoreVPox' ),
        ( True,  ktReason_GuestBug_CompizVPoxQt,                    'error 4 in libgtk-3' ),
    ];
    ...
    def investigateVMResult(self, oCaseFile, oFailedResult, sResultLog):
        ...
        def investigateLogSet():
            ...
            # Check VPoxHardening.log.
            if sNtHardLog is not None:
                fRet = self.scanLog([sNtHardLog,], self.katSimpleVPoxHardeningLogReasons, oCaseFile, oFailedResult.idTestResult);
                if fRet is True:
                    return fRet;
                fFoundSomething |= fRet is None;
        ...
        for oFile in oFailedResult.aoFiles:
            if oFile.sKind == TestResultFileData.ksKind_LogReleaseVm:
                if 'VPoxHardening.log' not in oFile.sFile:
                    if sVMLog is not None:
    ...
    ## Things we search a VPoxSVC log for to figure out why something went bust.
    ...
    def investigateSvcLogForVMRun(self, oCaseFile, sSvcLog):
        """
        Check the VPoxSVC log for a single VM run.
        """
        if sSvcLog:
    ...
    def investigateNtHardLogForVMRun(self, oCaseFile):
        ...
        for oLogFile in aoLogFiles:
            if oLogFile.sFile.find('VPoxHardening.log') >= 0:
    ...
    def investigateVPoxVMTest(self, oCaseFile, fSingleVM):
        """
        Checks out a VPox VM test.
        ...
        if len(aoFailedResults) == 1 and aoFailedResults[0] == oCaseFile.oTree:
            ...
            # be reported on the top result because vpoxinstall.py doesn't add an error for it.  It is easy to
            # ignore other failures in the test if we're not a little bit careful here.
            if sMainLog.find('vpoxinstaller: Exit code: -11 (') > 0:
                oCaseFile.noteReason(self.ktReason_XPCOM_Exit_Minus_11);
                return self.caseClosed(oCaseFile);

            # Hang after starting VPoxSVC (e.g. idTestSet=136307258)
            if self.isThisFollowedByTheseLines(sMainLog, 'oVPoxMgr=<vpoxapi.VirtualPoxManager object at',
                                               (' Timeout: ', ' Attempting to abort child...',) ):
                if sMainLog.find('*** glibc detected *** /') > 0:
                    oCaseFile.noteReason(self.ktReason_XPCOM_VPoxSVC_Hang_Plus_Heap_Corruption);
                else:
                    oCaseFile.noteReason(self.ktReason_XPCOM_VPoxSVC_Hang);
                return self.caseClosed(oCaseFile);
        ...
        for oFailedResult in aoFailedResults:
            ...
            sResultLog = TestSetData.extractLogSectionElapsed(sMainLog, oFailedResult.tsCreated, oFailedResult.tsElapsed);
            if oFailedResult.sName == 'Installing VirtualPox':
                self.investigateInstallUninstallFailure(oCaseFile, oFailedResult, sResultLog, fInstall = True)
        ...
        #
        # Check VPoxSVC.log and VPoxHardening.log for VM crashes if inconclusive on single VM runs.
        #
        if fSingleVM and len(oCaseFile.dReasonForResultId) < len(aoFailedResults):
            self.dprint(u'Got %u out of %u - checking VPoxSVC.log...'
    ...
    katSimpleApiMainLogReasons = [
        ...
        ( True,  ktReason_API_MoveVM_SharingViolation,              'rc=VPOX_E_IPRT_ERROR text="Could not copy the log file ' ),
        ( True,  ktReason_API_MoveVM_InvalidParameter,
          'rc=VPOX_E_IPRT_ERROR text="Could not copy the setting file ' ),
        ( True,  ktReason_API_Open_Session_Failed,                  'error: failed to open session for' ),
    ];

    def investigateVPoxApiTest(self, oCaseFile):
        """
        Checks out a VPox API test.
        """
    ...
    def reasoningFailures(self):
        ...
        for oTestSet in aoTestSets:
            ...
            elif oCaseFile.isVPoxUnitTest():
                self.dprint(u'investigateVPoxUnitTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxUnitTest(oCaseFile);

            elif oCaseFile.isVPoxInstallTest() or oCaseFile.isVPoxUnattendedInstallTest():
                self.dprint(u'investigateVPoxVMTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxVMTest(oCaseFile, fSingleVM = True);

            elif oCaseFile.isVPoxUSBTest():
                self.dprint(u'investigateVPoxVMTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxVMTest(oCaseFile, fSingleVM = True);

            elif oCaseFile.isVPoxStorageTest():
                self.dprint(u'investigateVPoxVMTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxVMTest(oCaseFile, fSingleVM = True);

            elif oCaseFile.isVPoxGAsTest():
                self.dprint(u'investigateVPoxVMTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxVMTest(oCaseFile, fSingleVM = True);

            elif oCaseFile.isVPoxAPITest():
                self.dprint(u'investigateVPoxApiTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxApiTest(oCaseFile);

            elif oCaseFile.isVPoxBenchmarkTest():
                self.dprint(u'investigateVPoxVMTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxVMTest(oCaseFile, fSingleVM = False);

            elif oCaseFile.isVPoxSmokeTest():
                self.dprint(u'investigateVPoxVMTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxVMTest(oCaseFile, fSingleVM = False);

            elif oCaseFile.isVPoxSerialTest():
                self.dprint(u'investigateVPoxVMTest is taking over %s.' % (oCaseFile.sLongName,));
                fRc = self.investigateVPoxVMTest(oCaseFile, fSingleVM = False);
