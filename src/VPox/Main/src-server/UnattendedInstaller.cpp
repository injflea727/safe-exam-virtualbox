...
/* static */ UnattendedInstaller *UnattendedInstaller::createInstance(VPOXOSTYPE enmOsType,
                                                                      ...
                                                                      Unattended *pParent)
{
    ...
    {
        if (enmOsType >= VPOXOSTYPE_WinVista)
    ...
    {
        if (enmOsType == VPOXOSTYPE_Debian || enmOsType == VPOXOSTYPE_Debian_x64)
            pUinstaller = new UnattendedDebianInstaller(pParent);
        else if (enmOsType >= VPOXOSTYPE_Ubuntu && enmOsType <= VPOXOSTYPE_Ubuntu_x64)
            pUinstaller = new UnattendedUbuntuInstaller(pParent);
        else if (enmOsType >= VPOXOSTYPE_RedHat && enmOsType <= VPOXOSTYPE_RedHat_x64)
        {
        ...
        else if (enmOsType >= VPOXOSTYPE_FedoraCore && enmOsType <= VPOXOSTYPE_FedoraCore_x64)
            pUinstaller = new UnattendedFedoraInstaller(pParent);
        else if (enmOsType >= VPOXOSTYPE_Oracle && enmOsType <= VPOXOSTYPE_Oracle_x64)
            pUinstaller = new UnattendedOracleLinuxInstaller(pParent);
#if 0 /* doesn't work, so convert later. */
        else if (enmOsType == VPOXOSTYPE_OpenSUSE || enmOsType == VPOXOSTYPE_OpenSUSE_x64)
    ...
    if (SUCCEEDED(hrc))
    {
        try
        {
            ...
            if (mpParent->i_getInstallTestExecService())
            {
                ...
                rVecArgs.append() = "/vpoxvalidationkit=/";
                rVecArgs.append() = "--pop";
            }
        }
        catch (std::bad_alloc &)
...
HRESULT UnattendedInstaller::finalizeAuxVisoFile(RTCList<RTCString> const &rVecArgs, const char *pszFilename, bool fOverwrite)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (RT_SUCCESS(vrc))
            ...
            hrc = mpParent->setErrorBoth(VPOX_E_FILE_ERROR, vrc, mpParent->tr("Error writing '%s' (%Rrc)"), pszFilename, vrc);
    }
    else
        hrc = mpParent->setErrorBoth(VPOX_E_FILE_ERROR, vrc, mpParent->tr("Failed to create '%s' (%Rrc)"), pszFilename, vrc);
...
HRESULT UnattendedInstaller::loadAndParseFileFromIso(RTVFS hVfsOrgIso, const char *pszFilename, AbstractScript *pEditor)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        hrc = mpParent->setErrorBoth(VPOX_E_FILE_ERROR, vrc, mpParent->tr("Failed to open '%s' on the ISO '%s' (%Rrc)"),
                                                               ...
                                                               RTVFS hVfsOrgIso, bool fOverwrite)
{
    ...
    try
    {
        ...
        /* Name the ISO. */
        rVecArgs.append() = "--volume-id=VPox Unattended Boot";
        ...
        rVecArgs.append() = "--boot-catalog=/isolinux/vpoxboot.cat";
#endif
    }
    catch (std::bad_alloc &)
    {
