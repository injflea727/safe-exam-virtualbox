...
/** @file
 * VirtualPox Main - Guest file handling.
 */
...
public:
    ...
    STDMETHOD(HandleEvent)(VPoxEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case VPoxEventType_OnGuestFileStateChanged:
            case VPoxEventType_OnGuestFileOffsetChanged:
            case VPoxEventType_OnGuestFileRead:
            case VPoxEventType_OnGuestFileWrite:
            {
...
VPOX_LISTENER_DECLARE(GuestFileListenerImpl)
    ...
    if (RT_SUCCESS(vrc))
    {
        try
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                eventTypes.push_back(VPoxEventType_OnGuestFileStateChanged);
                eventTypes.push_back(VPoxEventType_OnGuestFileOffsetChanged);
                eventTypes.push_back(VPoxEventType_OnGuestFileRead);
                eventTypes.push_back(VPoxEventType_OnGuestFileWrite);
...
int GuestFile::i_callbackDispatcher(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb)
{
...
int GuestFile::i_onFileNotify(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    ...
    AssertMsg(mObjectID == VPOX_GUESTCTRL_CONTEXTID_GET_OBJECT(pCbCtx->uContextID),
              ("File ID %RU32 does not match object ID %RU32\n", mObjectID,
               VPOX_GUESTCTRL_CONTEXTID_GET_OBJECT(pCbCtx->uContextID)));
    ...
    switch (dataCb.uType)
    {
        ...
        case GUEST_FILE_NOTIFYTYPE_READ_OFFSET:
        {
            ...
            ASSERT_GUEST_MSG_STMT_BREAK(pSvcCbData->mpaParms[idx].type == VPOX_HGCM_SVC_PARM_PTR,
                                        ("type=%u\n", pSvcCbData->mpaParms[idx].type),
                                        rc = VERR_WRONG_PARAMETER_TYPE);
            ASSERT_GUEST_MSG_STMT_BREAK(pSvcCbData->mpaParms[idx + 1].type == VPOX_HGCM_SVC_PARM_64BIT,
                                        ("type=%u\n", pSvcCbData->mpaParms[idx].type),
                                        rc = VERR_WRONG_PARAMETER_TYPE);
        ...
        case GUEST_FILE_NOTIFYTYPE_WRITE_OFFSET:
        {
            ASSERT_GUEST_MSG_STMT_BREAK(pSvcCbData->mParms == 5, ("mParms=%u\n", pSvcCbData->mParms),
                                        rc = VERR_WRONG_PARAMETER_COUNT);
            ASSERT_GUEST_MSG_STMT_BREAK(pSvcCbData->mpaParms[idx].type == VPOX_HGCM_SVC_PARM_32BIT,
                                        ("type=%u\n", pSvcCbData->mpaParms[idx].type),
                                        rc = VERR_WRONG_PARAMETER_TYPE);
            ASSERT_GUEST_MSG_STMT_BREAK(pSvcCbData->mpaParms[idx + 1].type == VPOX_HGCM_SVC_PARM_64BIT,
                                        ("type=%u\n", pSvcCbData->mpaParms[idx].type),
        ...
        case GUEST_FILE_NOTIFYTYPE_SET_SIZE:
            ASSERT_GUEST_MSG_STMT_BREAK(pSvcCbData->mParms == 4, ("mParms=%u\n", pSvcCbData->mParms),
                                        rc = VERR_WRONG_PARAMETER_COUNT);
            ASSERT_GUEST_MSG_STMT_BREAK(pSvcCbData->mpaParms[idx].type == VPOX_HGCM_SVC_PARM_64BIT,
                                        ("type=%u\n", pSvcCbData->mpaParms[idx].type),
                                        rc = VERR_WRONG_PARAMETER_TYPE);
...
int GuestFile::i_onGuestDisconnected(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestFile::i_setFileStatus(FileStatus_T fileStatus, int fileRc)
{
    ...
    if (mData.mStatus != fileStatus)
    {
        ...
        {
            hr = errorInfo->initEx(VPOX_E_IPRT_ERROR, fileRc,
                                     ...
                                     uint32_t uTimeoutMS, uint64_t *puOffset)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        if (evtType == VPoxEventType_OnGuestFileOffsetChanged)
        {
            if (puOffset)
            {
                             ...
                             void *pvData, size_t cbData, uint32_t *pcbRead)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        if (evtType == VPoxEventType_OnGuestFileRead)
        {
            vrc = VINF_SUCCESS;
                                     ...
                                     FileStatus_T *pFileStatus, int *prcGuest)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        Assert(evtType == VPoxEventType_OnGuestFileStateChanged);
                              ...
                              uint32_t uTimeoutMS, uint32_t *pcbWritten)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        if (evtType == VPoxEventType_OnGuestFileWrite)
        {
            if (pcbWritten)
            {
...
HRESULT GuestFile::close()
{
    ...
    if (RT_FAILURE(vrc))
    {
        ...
        return setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Closing guest file \"%s\" failed with %Rrc\n"),
...
HRESULT GuestFile::read(ULONG aToRead, ULONG aTimeoutMS, std::vector<BYTE> &aData)
{
    ...
    else
    {
        ...
        hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Reading from file \"%s\" failed: %Rrc"),
                          mData.mOpenInfo.mFilename.c_str(), vrc);
    }
...
HRESULT GuestFile::readAt(LONG64 aOffset, ULONG aToRead, ULONG aTimeoutMS, std::vector<BYTE> &aData)
{
    ...
    else
    {
        ...
        hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Reading from file \"%s\" (at offset %RU64) failed: %Rrc"),
...
HRESULT GuestFile::seek(LONG64 aOffset, FileSeekOrigin_T aWhence, LONG64 *aNewOffset)
{
    ...
    if (RT_SUCCESS(vrc))
        *aNewOffset = RT_MIN(uNewOffset, (uint64_t)INT64_MAX);
        ...
        hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Seeking file \"%s\" (to offset %RI64) failed: %Rrc"),
                          mData.mOpenInfo.mFilename.c_str(), aOffset, vrc);
...
HRESULT GuestFile::setSize(LONG64 aSize)
{
    ...
    try
    {
        lstEventTypes.push_back(VPoxEventType_OnGuestFileStateChanged);
        lstEventTypes.push_back(VPoxEventType_OnGuestFileSizeChanged);
    }
    catch (std::bad_alloc &)
    {
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (RT_SUCCESS(vrc))
        {
             ...
             * Wait for the event.
             */
            VPoxEventType_T enmEvtType;
            ComPtr<IEvent>  pIEvent;
            vrc = waitForEvent(pWaitEvent, RT_MS_1MIN / 2, &enmEvtType, pIEvent.asOutParam());
            if (RT_SUCCESS(vrc))
            {
                if (enmEvtType == VPoxEventType_OnGuestFileSizeChanged)
                    vrc = VINF_SUCCESS;
                else
                    vrc = VWRN_GSTCTL_OBJECTSTATE_CHANGED;
            }
    ...
    if (RT_SUCCESS(vrc))
        ...
        hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Setting the guest file size of \"%s\" to %RU64 (%#RX64) bytes failed: %Rrc"),
...
HRESULT GuestFile::write(const std::vector<BYTE> &aData, ULONG aTimeoutMS, ULONG *aWritten)
{
    ...
    if (RT_FAILURE(vrc))
        hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Writing %zu bytes to guest file \"%s\" failed: %Rrc"),
...
HRESULT GuestFile::writeAt(LONG64 aOffset, const std::vector<BYTE> &aData, ULONG aTimeoutMS, ULONG *aWritten)
{
    ...
    if (RT_FAILURE(vrc))
        hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Writing %zu bytes to file \"%s\" (at offset %RU64) failed: %Rrc"),
