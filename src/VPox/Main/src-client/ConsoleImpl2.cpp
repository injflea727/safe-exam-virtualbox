/* $Id: ConsoleImpl2.cpp $ */
/** @file
 * VPox Console COM Class implementation - VM Configuration Bits.
 *
...
// VPoxNetCfg-win.h needs winsock2.h and thus MUST be included before any other
// header file includes Windows.h.
#if defined(RT_OS_WINDOWS) && defined(VPOX_WITH_NETFLT)
...
#include <VPox/vmm/pdmapi.h> /* For PDMR3DriverAttach/PDMR3DriverDetach. */
#include <VPox/vmm/pdmusb.h> /* For PDMR3UsbCreateEmulatedDevice. */
#include <VPox/vmm/pdmdev.h> /* For PDMAPICMODE enum. */
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static Utf8Str *GetExtraDataBoth(IVirtualPox *pVirtualPox, IMachine *pMachine, const char *pszName, Utf8Str *pStrValue);
...
static int findEfiRom(IVirtualPox* vpox, FirmwareType_T aFirmwareType, Utf8Str *pEfiRomFile)
{
    Bstr aFilePath, empty;
    BOOL fPresent = FALSE;
    HRESULT hrc = vpox->CheckFirmwarePresent(aFirmwareType, empty.raw(),
                                             empty.asOutParam(), aFilePath.asOutParam(), &fPresent);
    AssertComRCReturn(hrc, Global::vpoxStatusCodeFromCOM(hrc));
 ...
 */
static int getSmcDeviceKey(IVirtualPox *pVirtualPox, IMachine *pMachine, Utf8Str *pStrKey, bool *pfGetKeyFromRealSMC)
{
    *pfGetKeyFromRealSMC = false;
     ...
     */
    GetExtraDataBoth(pVirtualPox, pMachine, "VPoxInternal2/SmcDeviceKey", pStrKey);
 ...
 * @param   pVirtualPox     Pointer to the IVirtualPox interface.
...
static Utf8Str *GetExtraDataBoth(IVirtualPox *pVirtualPox, IMachine *pMachine, const char *pszName, Utf8Str *pStrValue)
{
    ...
    {
        hrc = pVirtualPox->GetExtraData(bstrName.raw(), bstrValue.asOutParam());
        if (FAILED(hrc))
            throw hrc;
    }
...
# endif
     ...
     */
# ifdef VPOX_WITH_EXTPACK
    static const char *s_pszPCIRawExtPackName = "Oracle VM VirtualPox Extension Pack";
    if (!mptrExtPackManager->i_isExtPackUsable(s_pszPCIRawExtPackName))
        ...
        return VMR3SetError(pUVM, VERR_NOT_FOUND, RT_SRC_POS,
                N_("Implementation of the PCI passthrough framework not found!\n"
                   ...
                   "install the '%s' or disable PCI passthrough via VPoxManage"),
...
#define H()         AssertLogRelMsgReturn(!FAILED(hrc), ("hrc=%Rhrc\n", hrc), VERR_MAIN_CONFIG_CONSTRUCTOR_COM_ERROR)
    ...
    hrc = pMachine->COMGETTER(Parent)(virtualPox.asOutParam());                             H();

    ComPtr<IHost> host;
    hrc = virtualPox->COMGETTER(Host)(host.asOutParam());                                   H();

    ComPtr<ISystemProperties> systemProperties;
    hrc = virtualPox->COMGETTER(SystemProperties)(systemProperties.asOutParam());           H();
...
#if 0 /* enable to play with lots of memory. */
    if (RTEnvExist("VPOX_RAM_SIZE"))
        cRamMBs = RTStrToUInt64(RTEnvGet("VPOX_RAM_SIZE"));
#endif
    ...
    virtualPox->GetGuestOSType(osTypeId.raw(), pGuestOSType.asOutParam());
    ...
    try
    {
...
#if ARCH_BITS == 32 /* The recompiler must use VPoxREM64 (32-bit host only). */
            PCFGMNODE pREM;
            InsertConfigNode(pRoot, "REM", &pREM);
            InsertConfigInteger(pREM, "64bitEnabled", 1);
#endif

            /** @todo Not exactly pretty to check strings; VPOXOSTYPE would be better,
        ...
        /* Reset overwrite. */
        mfTurnResetIntoPowerOff = GetExtraDataBoth(virtualPox, pMachine,
                                                   "VPoxInternal2/TurnResetIntoPowerOff", &strTmp)->equals("1");
        ...
        InsertConfigNode(pNode,    "VPoxC", &pMod);
#ifdef VPOX_WITH_XPCOM
        // VPoxC is located in the components subdirectory
        char szPathVPoxC[RTPATH_MAX];
        rc = RTPathAppPrivateArch(szPathVPoxC, RTPATH_MAX - sizeof("/components/VPoxC"));   AssertRC(rc);
        strcat(szPathVPoxC, "/components/VPoxC");
        InsertConfigString(pMod,   "Path",  szPathVPoxC);
#else
        InsertConfigString(pMod,   "Path",  "VPoxC");
#endif
        ...
        if (fSmcEnabled)
        {
            ...
            Utf8Str strKey;
            rc = getSmcDeviceKey(virtualPox, pMachine, &strKey, &fGetKeyFromRealSMC);
            AssertRCReturn(rc, rc);
        ...
        switch (enmGraphicsController)
        {
            ...
            case GraphicsControllerType_VMSVGA:
                ...
                RT_FALL_THROUGH();
            case GraphicsControllerType_VPoxSVGA:
#endif
            case GraphicsControllerType_VPoxVGA:
        ...
        else
        {
            ...
            const char *pszEfiRomFile = eFwType == FirmwareType_EFIDUAL ? "VPoxEFIDual.fd"
                                      : eFwType == FirmwareType_EFI32   ? "VPoxEFI32.fd"
                                      :                                   "VPoxEFI64.fd";
#else
            Utf8Str efiRomFile;
            rc = findEfiRom(virtualPox, eFwType, &efiRomFile);
            AssertRCReturn(rc, rc);
            ...
            /* Get boot args */
            Utf8Str bootArgs;
            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiBootArgs", &bootArgs);

            /* Get device props */
            Utf8Str deviceProps;
            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiDeviceProps", &deviceProps);
            ...
            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiGraphicsMode", &strTmp);
            if (strTmp.isEmpty())
                GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiGopMode", &strTmp);
            if (!strTmp.isEmpty())
                u32GraphicsMode = strTmp.toUInt32();

            /* Get graphics resolution settings, with some sanity checking */
            Utf8Str strResolution;
            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiGraphicsResolution", &strResolution);
            ...
            else
            {
                uint32_t u32H = 0;
                uint32_t u32V = 0;
                GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiHorizontalResolution", &strTmp);
                if (strTmp.isEmpty())
                    GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiUgaHorizontalResolution", &strTmp);
                if (!strTmp.isEmpty())
                    u32H = strTmp.toUInt32();

                GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiVerticalResolution", &strTmp);
                if (strTmp.isEmpty())
                    GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/EfiUgaVerticalResolution", &strTmp);
                if (!strTmp.isEmpty())
...
# ifdef VPOX_WITH_EXTPACK
                    ...
                    {
                        /* Always fatal! Up to VPox 4.0.4 we allowed to start the VM anyway
...
# endif
...
#ifdef VPOX_WITH_USB
            {
                ...
                //      VPoxManage setextradata "myvm" "VPoxInternal/USB/USBProxy/GlobalConfig/Force11PacketSize" 1
...
# endif
        ...
        for (size_t i = 0; i < ctrls.size(); ++i)
        {
            ...
            switch (enmCtrlType)
            {
                ...
                case StorageControllerType_VirtioSCSI:
                {
                    ...
                    /* Attach the status driver */
                    AssertRelease(cPorts <= cLedVirtio);
            ...
            hrc = pMachine->GetExtraData(Bstr("VPoxInternal2/EnableDiskIntegrityDriver").raw(),
...
#endif /* VPOX_WITH_VIRTIO_NET_1_0 */
        ...
        for (ULONG uInstance = 0; uInstance < maxNetworkAdapters; ++uInstance)
        {
            ...
            InsertConfigNode(pInst, "Config", &pCfg);
#ifdef VPOX_WITH_2X_4GB_ADDR_SPACE   /* not safe here yet. */ /** @todo Make PCNet ring-0 safe on 32-bit mac kernels! */
            if (pDev == pDevPCNet)
        ...
        if (fAudioEnabled)
        {
            ...
            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/Audio/Device/TimerHz", &strTmp);
            const uint64_t uTimerHz = strTmp.toUInt64();

            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/Audio/Device/BufSizeInMs", &strTmp);
            const uint64_t uBufSizeInMs = strTmp.toUInt64();

            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/Audio/Device/BufSizeOutMs", &strTmp);
            const uint64_t uBufSizeOutMs = strTmp.toUInt64();

            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/Audio/Debug/Enabled", &strTmp);
            const bool fDebugEnabled = strTmp.equalsIgnoreCase("true") || strTmp.equalsIgnoreCase("1");

            Utf8Str strDebugPathOut;
            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/Audio/Debug/PathOut", &strDebugPathOut);

#ifdef VPOX_WITH_AUDIO_VALIDATIONKIT
            GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/Audio/VaKit/Enabled", &strTmp);
...
# endif
                case AudioDriverType_DirectSound:
                    /* Use the windows audio session (WAS) API rather than Direct Sound on windows
                       ...
                       Set extradata value "VPoxInternal2/Audio/WindowsDrv" "dsound" to no use WasAPI. */
                    pszAudioDriver = "DSoundAudio";
                    GetExtraDataBoth(virtualPox, pMachine, "VPoxInternal2/Audio/WindowsDrv", &strTmp); H();
            ...
            i_configAudioDriver(virtualPox, pMachine, pLunL0, pszAudioDriver, !!fAudioEnabledIn, !!fAudioEnabledOut);
            idxAudioLun++;
            ...
            if (fDebugEnabled)
            {
                ...
                InsertConfigNodeF(pInst, &pLunL0, "LUN#%u", idxAudioLun);
                i_configAudioDriver(virtualPox, pMachine, pLunL0, "DebugAudio", !!fAudioEnabledIn, !!fAudioEnabledOut);
                idxAudioLun++;
#endif /* VPOX_WITH_AUDIO_DEBUG */
            ...
            if (fVaKitEnabled)
            {
                 ...
                 */
                InsertConfigNodeF(pInst, &pLunL0, "LUN#%u", idxAudioLun);
                i_configAudioDriver(virtualPox, pMachine, pLunL0, "ValidationKitAudio", !!fAudioEnabledIn, !!fAudioEnabledOut);
                idxAudioLun++;

                LogRel(("Audio: ValidationKit driver active\n"));
            }
#endif /* VPOX_WITH_AUDIO_VALIDATIONKIT */
        }
...
# ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
            ...
            /* Load the service */
            rc = pVMMDev->hgcmLoadService("VPoxSharedClipboard", "VPoxSharedClipboard");
            if (RT_SUCCESS(rc))
            {
                ...
                rc = pVMMDev->hgcmHostCall("VPoxSharedClipboard", VPOX_SHCL_HOST_FN_SET_HEADLESS, 1, &parm);
...
# endif
...
#endif /* VPOX_WITH_SHARED_CLIPBOARD */
         ...
         */
        {
            ...
            if (   hrc   == S_OK
                && value == "1")
            {
                rc = pVMMDev->hgcmLoadService("VPoxHostChannel", "VPoxHostChannel");
                if (RT_FAILURE(rc))
                {
                    LogRel(("VPoxHostChannel is not available, rc=%Rrc\n", rc));
         ...
         */
        {
            ...
            /* Load the service */
            rc = pVMMDev->hgcmLoadService("VPoxDragAndDropSvc", "VPoxDragAndDropSvc");
            if (RT_FAILURE(rc))
            {
            ...
            {
                rc = HGCMHostRegisterServiceExtension(&m_hHgcmSvcExtDragAndDrop, "VPoxDragAndDropSvc",
                                                      &GuestDnD::notifyDnDDispatcher,
                                                      GuestDnDInst());
                if (RT_FAILURE(rc))
                    Log(("Cannot register VPoxDragAndDropSvc extension, rc=%Rrc\n", rc));
                else
                {
         ...
         */
        {
            ...
            hrc = virtualPox->COMGETTER(HomeFolder)(bstr.asOutParam());                     H();
            Utf8Str strVPoxHome = bstr;
            bstr.setNull();
            if (strVPoxHome.isNotEmpty())
                strVPoxHome.append("/");
            else
            {
                strVPoxHome = szHomeDir;
                strVPoxHome.append("/.vpox");
            }

            Utf8Str strFile(strVPoxHome);
     ...
     */
    if (RT_SUCCESS(rc))
        rc = i_configCfgmOverlay(pRoot, virtualPox, pMachine);

    /*
     * Dump all extradata API settings tweaks, both global and per VM.
     */
    if (RT_SUCCESS(rc))
        rc = i_configDumpAPISettingsTweaks(virtualPox, pMachine);
 ...
 *
 * @param   pVirtualPox         Pointer to IVirtualPox instance.
 ...
 */
void Console::i_configAudioDriver(IVirtualPox *pVirtualPox, IMachine *pMachine, PCFGMNODE pLUN, const char *pszDrvName,
...
#define H()     AssertLogRelMsgStmt(!FAILED(hrc), ("hrc=%Rhrc\n", hrc), \
    ...
    Utf8Str strTmp;
    GetExtraDataBoth(pVirtualPox, pMachine, "VPoxInternal2/Audio/Debug/Enabled", &strTmp);
    ...
    if (fDebugEnabled)
    {
        ...
        Utf8Str strDebugPathOut;
        GetExtraDataBoth(pVirtualPox, pMachine, "VPoxInternal2/Audio/Debug/PathOut", &strDebugPathOut);
        InsertConfigString(pCfg, "DebugPathOut",  strDebugPathOut.c_str());
    }

    /*
     * PCM input parameters (playback + recording).
     * We have host driver specific ones as: VPoxInternal2/Audio/<DrvName>/<Value>
     * And global ones for all host drivers: VPoxInternal2/Audio/<Value>
     */
    for (unsigned iDir = 0; iDir < 2; iDir++)
    {
        ...
        {
            char szExtra[128];
            RTStrPrintf(szExtra, sizeof(szExtra), "VPoxInternal2/Audio/%s/%s%s", pszDrvName, s_aToCopy[i].pszExtraName, pszDir);
            GetExtraDataBoth(pVirtualPox, pMachine, szExtra, &strTmp); /* throws hrc */
            if (strTmp.isEmpty())
            {
                RTStrPrintf(szExtra, sizeof(szExtra), "VPoxInternal2/Audio/%s%s", s_aToCopy[i].pszExtraName, pszDir);
                GetExtraDataBoth(pVirtualPox, pMachine, szExtra, &strTmp);
                if (strTmp.isEmpty())
                    continue;
            }
...
/**
 * Applies the CFGM overlay as specified by VPoxInternal/XXX extra data
 * values.
 *
 ...
 */
/* static */
int Console::i_configCfgmOverlay(PCFGMNODE pRoot, IVirtualPox *pVirtualPox, IMachine *pMachine)
{
    /*
    ...
    try
    {
        ...
        HRESULT hrc = pVirtualPox->GetExtraDataKeys(ComSafeArrayAsOutParam(aGlobalExtraDataKeys));
        AssertMsg(SUCCEEDED(hrc), ("VirtualPox::GetExtraDataKeys failed with %Rhrc\n", hrc));
            ...
            ++it, ++i2)
        {
            ...
            /*
             * We only care about keys starting with "VPoxInternal/" (skip "G:" or "M:")
             */
            if (!strKey.startsWith("VPoxInternal/"))
                continue;

            const char *pszExtraDataKey = strKey.c_str() + sizeof("VPoxInternal/") - 1;

            // get the value
            Bstr bstrExtraDataValue;
            if (i2 < cGlobalValues)
                // this is still one of the global values:
                hrc = pVirtualPox->GetExtraData(Bstr(strKey).raw(),
...
/**
 * Dumps the API settings tweaks as specified by VPoxInternal2/XXX extra data
 * values.
 *
 * @returns VPox status code.
 * @param   pVirtualPox     Pointer to the IVirtualPox interface.
 ...
 */
/* static */
int Console::i_configDumpAPISettingsTweaks(IVirtualPox *pVirtualPox, IMachine *pMachine)
{
    {
        SafeArray<BSTR> aGlobalExtraDataKeys;
        HRESULT hrc = pVirtualPox->GetExtraDataKeys(ComSafeArrayAsOutParam(aGlobalExtraDataKeys));
        ...
        for (size_t i = 0; i < aGlobalExtraDataKeys.size(); i++)
        {
            ...
            if (!strKey.startsWith("VPoxInternal2/"))
                continue;
...
#define H()         AssertLogRelMsgReturn(!FAILED(hrc), ("hrc=%Rhrc\n", hrc), VERR_MAIN_CONFIG_CONSTRUCTOR_COM_ERROR)
...
#ifdef VPOX_WITH_2X_4GB_ADDR_SPACE
        ...
        if (   enmGraphicsController == GraphicsControllerType_VMSVGA
            || enmGraphicsController == GraphicsControllerType_VPoxSVGA)
        {
...
#define H() \
    AssertMsgReturnStmt(SUCCEEDED(hrc), ("hrc=%Rhrc\n", hrc), if (phrc) *phrc = hrc, Global::vpoxStatusCodeFromCOM(hrc))
        ...
        else
        {
               ...
               )
            {
                ...
                // Additions were mounted and the user upgraded VirtualPox. Previously
...
#elif defined(VPOX_WITH_NETFLT)
                /*
                 * This is the new VPoxNetFlt+IntNet stuff.
                 */
                Bstr BridgedIfName;
...
# elif defined(RT_OS_SOLARIS)
                 ...
                 * VPoxManage and setting only the NIC name from there. If there is a space we
...
# elif defined(RT_OS_WINDOWS)
                ...
                hrc = VPoxNetCfgWinQueryINetCfg(&pNc, FALSE, L"VirtualPox", 10, &pszApp);
                Assert(hrc == S_OK);
                if (hrc != S_OK)
                {
                ...
                hrc = VPoxNetCfgWinGetComponentByGuid(pNc, &GUID_DEVCLASS_NET, (GUID*)hostIFGuid.raw(),
                                                      pAdaptorComponent.asOutParam());
                if (hrc != S_OK)
                {
                    VPoxNetCfgWinReleaseINetCfg(pNc, FALSE /*fHasWriteLock*/);
                    LogRel(("NetworkAttachmentType_Bridged: VPoxNetCfgWinGetComponentByGuid failed, hrc (0x%x)\n", hrc));
                    H();
                }
# define VPOX_WIN_BINDNAME_PREFIX "\\DEVICE\\"
                ...
                if (hrc == S_OK)
                {
                    ...
                    int cbFullBindNamePrefix = sizeof(VPOX_WIN_BINDNAME_PREFIX);
                    if (sizeof(szTrunkName) > cbFullBindNamePrefix + cwBindName)
                    {
                        strcpy(szTrunkName, VPOX_WIN_BINDNAME_PREFIX);
                ...
                {
                    VPoxNetCfgWinReleaseINetCfg(pNc, FALSE /*fHasWriteLock*/);
                    AssertLogRelMsgFailed(("NetworkAttachmentType_Bridged: VPoxNetCfgWinGetComponentByGuid failed, hrc (0x%x)",
                                           hrc));
                    H();
                }
...
#  if defined(RT_OS_FREEBSD)
                 ...
                 * interface via the current FreeBSD vpoxnetflt implementation.
                 */
                        ...
                        switch (hrc)
                        {
                            case E_ACCESSDENIED:
                                return VMSetError(VMR3GetVM(mpUVM), VERR_HOSTIF_INIT_FAILED, RT_SRC_POS,  N_(
                                                ...
                                                "change the group of that node to vpoxusers and make yourself "
...
# if defined(RT_OS_SOLARIS) || defined(RT_OS_DARWIN)
                 ...
                 * for the same interface name, eg: "nge0 - ethernet" (GUI) vs "nge0" (VPoxManage).
                 */
...
# else  /* defined VPOX_WITH_NETFLT*/
                ...
                {
                    VPoxNetCfgWinReleaseINetCfg(pNc, FALSE /*fHasWriteLock*/);
                    LogRel(("NetworkAttachmentType_HostOnly: VPoxNetCfgWinGetComponentByGuid failed, hrc=%Rhrc (0x%x)\n", hrc, hrc));
                    H();
                }
#  define VPOX_WIN_BINDNAME_PREFIX "\\DEVICE\\"
                char szTrunkName[INTNET_MAX_TRUNK_NAME];
                ...
                {
                    Log(("help-text=%ls\n", pwszHelpText));
                    if (!wcscmp(pwszHelpText, L"VirtualPox NDIS 6.0 Miniport Driver"))
                        fNdis6 = true;
                ...
                else
                {
                    ...
                    {
                        VPoxNetCfgWinReleaseINetCfg(pNc, FALSE /*fHasWriteLock*/);
                        AssertLogRelMsgFailed(("NetworkAttachmentType_HostOnly: VPoxNetCfgWinGetComponentByGuid failed, hrc=%Rhrc (0x%x)\n",
                                               hrc, hrc));
                        H();
                    }
                ...
                networkName = Bstr(szNetwork);
                trunkName   = Bstr(pszTrunk);
# endif /* defined VPOX_WITH_NETFLT*/
                ...
                hrc = virtualPox->GetExtraData(BstrFmt("HostOnly/%s/IPAddress",
                                                       pszHostOnlyName).raw(),
                                               tmpAddr.asOutParam());
                if (SUCCEEDED(hrc) && !tmpAddr.isEmpty())
                {
                    hrc = virtualPox->GetExtraData(BstrFmt("HostOnly/%s/IPNetMask",
                                                           pszHostOnlyName).raw(),
                                                   tmpMask.asOutParam());
                    if (SUCCEEDED(hrc) && !tmpMask.isEmpty())
                        ...
                        hrc = hostInterface->EnableStaticIPConfig(tmpAddr.raw(),
                                                                  Bstr(VPOXNET_IPV4MASK_DEFAULT).raw());
                }
                else
                {
                    /* Grab the IP number from the 'vpoxnetX' instance number (see netif.h) */
                ...
                hrc = virtualPox->GetExtraData(BstrFmt("HostOnly/%s/IPV6Address",
                                                       pszHostOnlyName).raw(),
                                               tmpAddr.asOutParam());
                if (SUCCEEDED(hrc))
                    hrc = virtualPox->GetExtraData(BstrFmt("HostOnly/%s/IPV6NetMask", pszHostOnlyName).raw(),
                                                   tmpMask.asOutParam());
            ...
            case NetworkAttachmentType_NATNetwork:
            {
                ...
                if (!bstr.isEmpty())
                {
                    /** @todo add intnet prefix to separate namespaces, and add trunk if dealing with vpoxnatX */
                    InsertConfigString(pLunL0, "Driver", "IntNet");
                    InsertConfigNode(pLunL0, "Config", &pCfg);
                    InsertConfigString(pCfg, "Network", bstr);
                    InsertConfigInteger(pCfg, "TrunkType", kIntNetTrunkType_WhateverNone);
            ...
            {
                static const char *s_pszCloudExtPackName = "Oracle VM VirtualPox Extension Pack";
                /*
...
# endif
                ...
                hrc = virtualPox->FindCloudNetworkByName(bstr.raw(), network.asOutParam());   H();
                hrc = startGateways(virtualPox, network, mGateways);                          H();
...
#endif /* VPOX_WITH_CLOUD_NET */
            {
                if (SUCCEEDED(hrc) && RT_SUCCESS(rc))
                {
                        ...
                        && eAttachmentType != NetworkAttachmentType_NATNetwork)
                    {
                        ...
                        hrc = virtualPox->FindDHCPServerByNetworkName(networkName.raw(),
