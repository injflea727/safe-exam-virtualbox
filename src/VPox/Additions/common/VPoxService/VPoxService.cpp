/* $Id: VPoxService.cpp $ */
/** @file
 * VPoxService - Guest Additions Service Skeleton.
 */
...
/** @page pg_vgsvc VPoxService
 *
 * VPoxService is a root daemon for implementing guest additions features.
 *
 ...
 * reason is that the VPoxService binary is typically statically linked, at
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnPreInit, Default Implementation}
 */
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnOption, Default Implementation}
 */
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnInit, Default Implementation}
 */
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnTerm, Default Implementation}
 */
    ...
    switch (enmPhase)
    {
        case RTLOGPHASE_BEGIN:
        {
            pfnLog(pLoggerRelease,
                   "VPoxService %s r%s (verbosity: %u) %s (%s %s) release log\n"
                   "Log opened %s\n",
                   RTBldCfgVersion(), RTBldCfgRevisionStr(), g_cVerbosity, VPOX_BUILD_TARGET,
...
int VGSvcLogCreate(const char *pszLogFile)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        /* Explicitly flush the log in case of VPOXSERVICE_RELEASE_LOG=buffered. */
...
/**
 * Reports the current VPoxService status to the host.
 *
 ...
 */
int VGSvcReportStatus(VPoxGuestFacilityStatus enmStatus)
{
    /*
     * VPoxGuestFacilityStatus_Failed is sticky.
     */
    static VPoxGuestFacilityStatus s_enmLastStatus = VPoxGuestFacilityStatus_Inactive;
    VGSvcVerbose(4, "Setting VPoxService status to %u\n", enmStatus);
    if (s_enmLastStatus != VPoxGuestFacilityStatus_Failed)
    {
        int rc = VbglR3ReportAdditionsStatus(VPoxGuestFacilityType_VPoxService, enmStatus, 0 /* Flags */);
        if (RT_FAILURE(rc))
        {
            VGSvcError("Could not report VPoxService status (%u), rc=%Rrc\n", enmStatus, rc);
            return rc;
        }
 ...
 * Starts the service.
 *
 * @returns VPox status code, errors are fully bitched.
 *
 * @remarks Also called from VPoxService-win.cpp, thus not static.
 */
int VGSvcStartServices(void)
{
    int rc;

    VGSvcReportStatus(VPoxGuestFacilityStatus_Init);
        ...
        if (g_aServices[j].fEnabled)
        {
            ...
            if (RT_FAILURE(rc))
            {
                if (rc != VERR_SERVICE_DISABLED)
                {
                    ...
                    VGSvcReportStatus(VPoxGuestFacilityStatus_Failed);
                    return rc;
                }
 ...
 *
 * This should be called even when VPoxServiceStartServices fails so it can
...
{
    VGSvcReportStatus(VPoxGuestFacilityStatus_Terminating);
     ...
     * sleeping in VPoxServiceMainWait).
     */
    ...
    VGSvcReportStatus(RT_SUCCESS(rc) ? VPoxGuestFacilityStatus_Paused : VPoxGuestFacilityStatus_Failed);
    return rc;
}
...
void VGSvcMainWait(void)
{
    ...
    VGSvcReportStatus(VPoxGuestFacilityStatus_Active);
...
#ifdef RT_OS_LINUX
/**
 * Check for a guest property and start VPoxDRMClient if it exists.
 *
 */
static void startDRMResize(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        rc = VGSvcCheckPropExist(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/DRMResize");
        if (RT_SUCCESS(rc))
        {
            ...
            RTPathAppend(szDRMClientPath, RTPATH_MAX, "VPoxDRMClient");
            const char *apszArgs[1] = { NULL };
            rc = RTProcCreate("VPoxDRMClient", apszArgs, RTENV_DEFAULT,
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1 && strcmp(argv[1], VPOXSERVICE_ARG1_UTF8_ARGV) == 0)
    {
        argv[1] = argv[0];
        argv++;
        argc--;
     ...
     * VPoxService and shouldn't be subject to /dev/vpoxguest, pid-files and
...
#ifdef VPOX_WITH_VPOXSERVICE_CONTROL
    /*
     * Check if we're the specially spawned VPoxService.exe process that
     * handles a guest control session.
     */
    if (   argc >= 2
        && !RTStrICmp(argv[1], VPOXSERVICECTRLSESSION_GETOPT_PREFIX))
        fUserSession = true;
#endif
...
#undef MATCHES
...
#ifdef RT_OS_WINDOWS
    /*
     * Make sure only one instance of VPoxService runs at a time.  Create a
    ...
    if (RTSystemGetNtVersion() >= RTSYSTEM_MAKE_NT_VERSION(5,0,0)) /* Windows 2000 */
        hMutexAppRunning = CreateMutexW(NULL, FALSE, L"Global\\" RT_CONCAT(L,VPOXSERVICE_NAME));
    else
        hMutexAppRunning = CreateMutexW(NULL, FALSE, RT_CONCAT(L,VPOXSERVICE_NAME));
...
# endif /* !RT_OS_NT4 */
    ...
    VGSvcReportStatus(VPoxGuestFacilityStatus_Terminated);
