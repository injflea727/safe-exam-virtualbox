/* $Id: VPoxServiceControlProcess.cpp $ */
/** @file
 * VPoxServiceControlThread - Guest process handling.
 */
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int                  vgsvcGstCtrlProcessAssignPID(PVPOXSERVICECTRLPROCESS pThread, uint32_t uPID);
static int                  vgsvcGstCtrlProcessLock(PVPOXSERVICECTRLPROCESS pProcess);
static int                  vgsvcGstCtrlProcessSetupPipe(const char *pszHowTo, int fd, PRTHANDLE ph, PRTHANDLE *pph,
                                                         PRTPIPE phPipe);
static int                  vgsvcGstCtrlProcessUnlock(PVPOXSERVICECTRLPROCESS pProcess);
/* Request handlers. */
static DECLCALLBACK(int)    vgsvcGstCtrlProcessOnInput(PVPOXSERVICECTRLPROCESS pThis, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                                       bool fPendingClose, void *pvBuf, uint32_t cbBuf);
static DECLCALLBACK(int)    vgsvcGstCtrlProcessOnOutput(PVPOXSERVICECTRLPROCESS pThis, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
 ...
 */
static int vgsvcGstCtrlProcessInit(PVPOXSERVICECTRLPROCESS pProcess,
                                   const PVPOXSERVICECTRLSESSION pSession,
 ...
 */
int VGSvcGstCtrlProcessFree(PVPOXSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlProcessStop(PVPOXSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
void VGSvcGstCtrlProcessRelease(PVPOXSERVICECTRLPROCESS pProcess)
{
 ...
 *                              Optional.
 */
int VGSvcGstCtrlProcessWait(const PVPOXSERVICECTRLPROCESS pProcess, RTMSINTERVAL msTimeout, int *prc)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcGstCtrlProcessPollsetCloseInput(PVPOXSERVICECTRLPROCESS pProcess, PRTPIPE phStdInW)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
    AssertPtrReturn(phStdInW, VERR_INVALID_POINTER);

    int rc = RTPollSetRemove(pProcess->hPollSet, VPOXSERVICECTRLPIPEID_STDIN);
...
static const char *vgsvcGstCtrlProcessPollHandleToString(uint32_t idPollHnd)
{
    switch (idPollHnd)
    {
        case VPOXSERVICECTRLPIPEID_UNKNOWN:
            return "unknown";
        case VPOXSERVICECTRLPIPEID_STDIN:
            return "stdin";
        case VPOXSERVICECTRLPIPEID_STDIN_WRITABLE:
            return "stdin_writable";
        case VPOXSERVICECTRLPIPEID_STDOUT:
            return "stdout";
        case VPOXSERVICECTRLPIPEID_STDERR:
            return "stderr";
        case VPOXSERVICECTRLPIPEID_IPC_NOTIFY:
            return "ipc_notify";
        default:
            return "unknown";
    }
 ...
 * @param   phStdInW            The standard input pipe handle.
 */
static int vgsvcGstCtrlProcessPollsetOnInput(PVPOXSERVICECTRLPROCESS pProcess, uint32_t fPollEvt, PRTPIPE phStdInW)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcGstCtrlProcessHandleOutputError(PVPOXSERVICECTRLPROCESS pProcess,
 ...
 *
 */
static int vgsvcGstCtrlProcessPollsetOnOutput(PVPOXSERVICECTRLPROCESS pProcess,
                                              uint32_t fPollEvt, PRTPIPE phPipeR, uint32_t idPollHnd)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcGstCtrlProcessProcLoop(PVPOXSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 *
 * This function also resolves internal VPoxService tools to its appropriate
 * executable path + name if VPOXSERVICE_NAME is specified as pszFilename.
 *
 * @return  IPRT status code.
...
static int vgsvcGstCtrlProcessResolveExecutable(const char *pszFilename, char *pszResolved, size_t cbResolved)
{
    ...
    if (   RTStrICmp(pszFilename, g_pszProgName) == 0
        || RTStrICmp(pszFilename, VPOXSERVICE_NAME) == 0)
 ...
 * @param  fExecutingSelf   Set if we're executing the VPoxService executable
                                           ...
                                           bool fExecutingSelf, char ***ppapszArgv)
{
...
#ifndef VPOXSERVICE_ARG1_UTF8_ARGV
    ...
    /* HACK ALERT! Older hosts (< VPox 6.1.x) did not allow the user to really specify the first
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        /* Insert --utf8-argv as the first argument if executing the VPoxService binary. */
        if (fExecutingSelf)
        {
            rc = RTStrDupEx(&papszNewArgv[iDst], VPOXSERVICE_ARG1_UTF8_ARGV);
            if (RT_SUCCESS(rc))
                iDst++;
        }
#endif
 ...
 */
static int vgsvcGstCtrlProcessAssignPID(PVPOXSERVICECTRLPROCESS pProcess, uint32_t uPID)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            fTryAgain = false;
            PVPOXSERVICECTRLPROCESS pProcessCur;
            RTListForEach(&pProcess->pSession->lstProcesses, pProcessCur, VPOXSERVICECTRLPROCESS, Node)
            {
                                            ...
                                            PRTPROCESS phProcess)
{
...
#ifdef RT_OS_WINDOWS
    /*
     * If sysprep should be executed do this in the context of VPoxService, which
    ...
    if (!RTStrICmp(pszExec, "sysprep"))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                 ...
                 * run under behalf of the account VPoxService was started under, most
                 * likely local system. */
    ...
    if (RTStrStr(pszExec, "vpox_") == pszExec) /** @todo WTF search the whole string for "vpox_" when all you want is to know if whether string starts with "vpox_" or not. geee^2 */
    {
        /* We want to use the internal toolbox (all internal
         * tools are starting with "vpox_" (e.g. "vpox_cat"). */
        fExecutingSelf = true;
        rc = vgsvcGstCtrlProcessResolveExecutable(VPOXSERVICE_NAME, szExecExp, sizeof(szExecExp));
    }
    else
    {
#endif
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         *
         * - If the host does not provide a dedicated argv[0] (< VPox 6.1.x), we use the
         ...
         *
         * - Newer hosts (>= VPox 6.1.x) provide a correct argv[0] independently of the actual
         *   executable name though, so actually use argv[0] *and* argv[1,2,n] as intended.
         */
        const bool fHasArgv0 = RT_BOOL(g_fControlHostFeatures0 & VPOX_GUESTCTRL_HF_0_PROCESS_ARGV0);
        ...
        else
        {
            ...
            if (fExecutingSelf)
                fProcCreateFlags |= VPOXSERVICE_PROC_F_UTF8_ARGV;
            if (fFlags)
            {
 ...
 */
static int vgsvcGstCtrlProcessProcessWorker(PVPOXSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
    ...
    const bool fHasArgv0    = RT_BOOL(g_fControlHostFeatures0 & VPOX_GUESTCTRL_HF_0_PROCESS_ARGV0); RT_NOREF(fHasArgv0);
                            ...
                            if (RT_SUCCESS(rc))
                            {
                                ...
                                if (RT_SUCCESS(rc))
                                    rc = RTPollSetAddPipe(pProcess->hPollSet,
                                                          pProcess->hPipeStdInW, RTPOLL_EVT_ERROR, VPOXSERVICECTRLPIPEID_STDIN);
                                /* Stdout. */
                                if (RT_SUCCESS(rc))
                                    rc = RTPollSetAddPipe(pProcess->hPollSet,
                                                          pProcess->hPipeStdOutR, uFlags, VPOXSERVICECTRLPIPEID_STDOUT);
                                /* Stderr. */
                                if (RT_SUCCESS(rc))
                                    rc = RTPollSetAddPipe(pProcess->hPollSet,
                                                          pProcess->hPipeStdErrR, uFlags, VPOXSERVICECTRLPIPEID_STDERR);
                                /* IPC notification pipe. */
                                if (RT_SUCCESS(rc))
                                    rc = RTPipeCreate(&pProcess->hNotificationPipeR, &pProcess->hNotificationPipeW, 0 /* Flags */);
                                if (RT_SUCCESS(rc))
                                    rc = RTPollSetAddPipe(pProcess->hPollSet,
                                                          pProcess->hNotificationPipeR, RTPOLL_EVT_READ, VPOXSERVICECTRLPIPEID_IPC_NOTIFY);
                                if (RT_SUCCESS(rc))
                                {
                                    AssertPtr(pProcess->pSession);
                                    bool fNeedsImpersonation = !(pProcess->pSession->fFlags & VPOXSERVICECTRLSESSION_FLAG_SPAWN);
                                    ...
                                    if (RT_SUCCESS(rc))
                                    {
                                        ...
                                        if (RT_FAILURE(RTPollSetQueryHandle(pProcess->hPollSet,
                                                                            VPOXSERVICECTRLPIPEID_IPC_NOTIFY, NULL)))
                                            pProcess->hNotificationPipeW = NIL_RTPIPE;
                                        if (RT_FAILURE(RTPollSetQueryHandle(pProcess->hPollSet,
                                                                            VPOXSERVICECTRLPIPEID_STDERR, NULL)))
                                            pProcess->hPipeStdErrR = NIL_RTPIPE;
                                        if (RT_FAILURE(RTPollSetQueryHandle(pProcess->hPollSet,
                                                                            VPOXSERVICECTRLPIPEID_STDOUT, NULL)))
                                            pProcess->hPipeStdOutR = NIL_RTPIPE;
                                        if (RT_FAILURE(RTPollSetQueryHandle(pProcess->hPollSet,
                                                                            VPOXSERVICECTRLPIPEID_STDIN, NULL)))
                                            pProcess->hPipeStdInW = NIL_RTPIPE;
                                    }
                                }
 ...
 * @param  pvUser           Pointer to a VPOXSERVICECTRLPROCESS structure.
 *
 */
static DECLCALLBACK(int) vgsvcGstCtrlProcessThread(RTTHREAD hThreadSelf, void *pvUser)
{
    RT_NOREF1(hThreadSelf);
    PVPOXSERVICECTRLPROCESS pProcess = (PVPOXSERVICECTRLPROCESS)pvUser;
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlProcessStart(const PVPOXSERVICECTRLSESSION pSession,
                             const PVBGLR3GUESTCTRLPROCSTARTUPINFO pStartupInfo, uint32_t uContextID)
{
     ...
     */
    PVPOXSERVICECTRLPROCESS pProcess = (PVPOXSERVICECTRLPROCESS)RTMemAlloc(sizeof(VPOXSERVICECTRLPROCESS));
    if (!pProcess)
        return VERR_NO_MEMORY;
...
static DECLCALLBACK(int) vgsvcGstCtrlProcessOnInput(PVPOXSERVICECTRLPROCESS pThis,
...
static DECLCALLBACK(int) vgsvcGstCtrlProcessOnOutput(PVPOXSERVICECTRLPROCESS pThis,
                                                     const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                                     uint32_t uHandle, uint32_t cbToRead, uint32_t fFlags)
{
    ...
    const PVPOXSERVICECTRLSESSION pSession = pThis->pSession;
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    ...
    if (pvBuf)
    {
        ...
        if (*phPipe != NIL_RTPIPE)
        {
            ...
            if (RT_FAILURE(rc))
            {
                RTPollSetRemove(pThis->hPollSet,   uHandle == OUTPUT_HANDLE_ID_STDERR
                                                 ? VPOXSERVICECTRLPIPEID_STDERR : VPOXSERVICECTRLPIPEID_STDOUT);
                RTPipeClose(*phPipe);
                *phPipe = NIL_RTPIPE;
        ...
        if (RT_SUCCESS(rc))
        {
            if (   pSession->fFlags & VPOXSERVICECTRLSESSION_FLAG_DUMPSTDOUT
                && (   uHandle == OUTPUT_HANDLE_ID_STDOUT
                    || uHandle == OUTPUT_HANDLE_ID_STDOUT_DEPRECATED)
               )
            {
                rc = vgsvcGstCtrlProcessDbgDumpToFileF(pvBuf, cbRead, "VPoxService_Session%RU32_PID%RU32_StdOut.txt",
                                                pSession->StartupInfo.uSessionID, pThis->uPID);
                AssertRC(rc);
            }
            else if (   pSession->fFlags & VPOXSERVICECTRLSESSION_FLAG_DUMPSTDERR
                     && uHandle == OUTPUT_HANDLE_ID_STDERR)
            {
                rc = vgsvcGstCtrlProcessDbgDumpToFileF(pvBuf, cbRead, "VPoxService_Session%RU32_PID%RU32_StdErr.txt",
                                                pSession->StartupInfo.uSessionID, pThis->uPID);
                AssertRC(rc);
            }
        }
...
static DECLCALLBACK(int) vgsvcGstCtrlProcessOnTerm(PVPOXSERVICECTRLPROCESS pThis)
{
    AssertPtrReturn(pThis, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlProcessRequestExV(PVPOXSERVICECTRLPROCESS pProcess, const PVBGLR3GUESTCTRLCMDCTX pHostCtx, bool fAsync,
...
static int vgsvcGstCtrlProcessRequestAsync(PVPOXSERVICECTRLPROCESS pProcess, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
...
#if 0 /* unused */
static int vgsvcGstCtrlProcessRequestWait(PVPOXSERVICECTRLPROCESS pProcess, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
...
int VGSvcGstCtrlProcessHandleInput(PVPOXSERVICECTRLPROCESS pProcess, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                   bool fPendingClose, void *pvBuf, uint32_t cbBuf)
{
...
int VGSvcGstCtrlProcessHandleOutput(PVPOXSERVICECTRLPROCESS pProcess, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                    uint32_t uHandle, uint32_t cbToRead, uint32_t fFlags)
{
    if (!ASMAtomicReadBool(&pProcess->fShutdown) && !ASMAtomicReadBool(&pProcess->fStopped))
...
int VGSvcGstCtrlProcessHandleTerm(PVPOXSERVICECTRLPROCESS pProcess)
{
    if (!ASMAtomicReadBool(&pProcess->fShutdown) && !ASMAtomicReadBool(&pProcess->fStopped))
