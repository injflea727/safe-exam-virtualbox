            ...
            while (cLeaves-- > 0)
            {
                    ...
                    && cpumR3IsEcxRelevantForCpuIdLeaf(uLeaf, &cSubLeaves, &fFinalEcxUnchanged))
                {
                    if (cSubLeaves > (uLeaf == 0xd ? 68U : 16U))
                    {
                        ...
                        LogRel(("\nPlease create a defect on virtualpox.org and attach this log file!\n\n"));
 ...
 * the hyper heap!
 *
 * @returns VPox status code (VMSetError called).
 * @param   pVM                 The cross context VM structure.
 ...
 *
 * @returns VPox status code (error message raised).
...
int cpumR3LoadCpuIdPre32(PVM pVM, PSSMHANDLE pSSM, uint32_t uVersion)
{
     ...
     * leaves because we've been allowed more is newer version of VPox.
     */
    uint32_t cElements;
