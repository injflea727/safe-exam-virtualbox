...
#endif /* VPOX_WITH_SHARED_CLIPBOARD_FUSE */
...
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
    if (Format == VPOX_SHCL_FMT_URI_LIST)
    {
 ...
 */
static int vpoxClipboardConnect(void)
{
    LogFlowFuncEnter();
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            rc = VbglR3ClipboardConnectEx(&g_Ctx.CmdCtx, VPOX_SHCL_GF_0_CONTEXT_ID);
            if (RT_FAILURE(rc))
...
#ifdef VPOX_WITH_SHARED_CLIPBOARD_FUSE
static DECLCALLBACK(int) vpoxClipoardFUSEThread(RTTHREAD hThreadSelf, void *pvUser)
{
    ...
    if (RT_SUCCESS(rc))
    {
        rc = RTPathAppend(Opts.szMountPoint, sizeof(Opts.szMountPoint), "VPoxSharedClipboard");
        if (RT_SUCCESS(rc))
        {
...
static int vpoxClipboardFUSEStart()
{
    LogFlowFuncEnter();

    PSHCLCONTEXT pCtx = &g_Ctx;

    int rc = RTThreadCreate(&pCtx->FUSE.Thread, vpoxClipoardFUSEThread, &pCtx->FUSE, 0,
...
static int vpoxClipboardFUSEStop()
{
    LogFlowFuncEnter();
...
{
    return ".vpoxclient-clipboard.pid";
}
...
static int run(struct VBCLSERVICE **ppInterface, bool fDaemonised)
{
    ...
    int rc = vpoxClipboardConnect();
    if (RT_SUCCESS(rc))
    {
#ifdef VPOX_WITH_SHARED_CLIPBOARD_FUSE
        rc = vpoxClipboardFUSEStart();
        if (RT_SUCCESS(rc))
        {
#endif
            rc = vpoxClipboardMain();

#ifdef VPOX_WITH_SHARED_CLIPBOARD_FUSE
            int rc2 = vpoxClipboardFUSEStop();
            if (RT_SUCCESS(rc))
                rc = rc2;
        }
#endif
