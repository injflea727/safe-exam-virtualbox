...
REM
REM Script to register the VirtualPox COM classes
...
REM correct VPoxSVC and register the right VPoxC.dll.
REM
...
set _VPOX_DIR=
cd "%_SCRIPT_SELF%\.."
for /f "tokens=*" %%d in ('cd') do set _VPOX_DIR=%%d\
cd "%_SCRIPT_CURDIR%"
...
:register_x86
@echo on
"%_VPOX_DIR%VPoxSVC.exe" /UnregServer
regsvr32 /s /u "%_VPOX_DIR%VPoxC.dll"
%windir%\system32\regsvr32 /s /u "%_VPOX_DIR%VPoxProxyStub.dll"
@if %fUninstallOnly% == 1 goto end
"%_VPOX_DIR%VPoxSVC.exe" /RegServer
"%_VPOX_DIR%VPoxSDS.exe" /RegService
regsvr32 /s    "%_VPOX_DIR%VPoxC.dll"
@if %fNoProxy% == 1 goto end
if exist "%_VPOX_DIR%VPoxProxyStub.dll"     %windir%\system32\regsvr32 /s "%_VPOX_DIR%VPoxProxyStub.dll"
@echo off
goto end
...
set s64BitProxyStub=VPoxProxyStubLegacy.dll
goto register_amd64_begin
:register_amd64_not_legacy
set s64BitProxyStub=VPoxProxyStub.dll
:register_amd64_begin
echo s64BitProxyStub=%s64BitProxyStub%
@echo on
"%_VPOX_DIR%VPoxSVC.exe" /UnregServer
"%_VPOX_DIR%VPoxSDS.exe" /UnregService
%windir%\system32\regsvr32 /s /u "%_VPOX_DIR%VPoxC.dll"
%windir%\syswow64\regsvr32 /s /u "%_VPOX_DIR%x86\VPoxClient-x86.dll"
%windir%\system32\regsvr32 /s /u "%_VPOX_DIR%%s64BitProxyStub%"
%windir%\syswow64\regsvr32 /s /u "%_VPOX_DIR%x86\VPoxProxyStub-x86.dll"
if %fUninstallOnly% == 1 goto end
"%_VPOX_DIR%VPoxSVC.exe" /RegServer
"%_VPOX_DIR%VPoxSDS.exe" /RegService
%windir%\system32\regsvr32 /s    "%_VPOX_DIR%VPoxC.dll"
%windir%\syswow64\regsvr32 /s    "%_VPOX_DIR%x86\VPoxClient-x86.dll"
if %fNoProxy% == 1 goto end
if exist "%_VPOX_DIR%%s64BitProxyStub%"         %windir%\system32\regsvr32 /s "%_VPOX_DIR%%s64BitProxyStub%"
if exist "%_VPOX_DIR%x86\VPoxProxyStub-x86.dll" %windir%\syswow64\regsvr32 /s "%_VPOX_DIR%x86\VPoxProxyStub-x86.dll"
@echo off
