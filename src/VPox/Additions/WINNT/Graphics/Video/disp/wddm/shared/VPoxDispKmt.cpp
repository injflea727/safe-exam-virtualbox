...
HRESULT vpoxDispKmtCallbacksInit(PVPOXDISPKMT_CALLBACKS pCallbacks)
{
    ...
    if (pCallbacks->hGdi32 != NULL)
    {
        ...
        {
            if (bSupportedWin8)
                pCallbacks->enmVersion = VPOXDISPKMT_CALLBACKS_VERSION_WIN8;
            else
                pCallbacks->enmVersion = VPOXDISPKMT_CALLBACKS_VERSION_VISTA_WIN7;
            return S_OK;
        }
        else
        {
...
HRESULT vpoxDispKmtAdpHdcCreate(HDC *phDc)
{
    HRESULT hr = E_FAIL;
    DISPLAY_DEVICE DDev;
...
static HRESULT vpoxDispKmtOpenAdapterViaHdc(const VPOXDISPKMT_CALLBACKS *pCallbacks, PVPOXDISPKMT_ADAPTER pAdapter)
{
    D3DKMT_OPENADAPTERFROMHDC OpenAdapterData = {0};
    HRESULT hr = vpoxDispKmtAdpHdcCreate(&OpenAdapterData.hDc);
    if (!SUCCEEDED(hr))
        return hr;
...
static HRESULT vpoxDispKmtOpenAdapterViaLuid(const VPOXDISPKMT_CALLBACKS *pCallbacks, PVPOXDISPKMT_ADAPTER pAdapter)
{
    if (pCallbacks->enmVersion < VPOXDISPKMT_CALLBACKS_VERSION_WIN8)
        return E_NOTIMPL;
...
{
    HRESULT hr = vpoxDispKmtOpenAdapterViaHdc(pCallbacks, pAdapter);
    if (SUCCEEDED(hr))
        return S_OK;

    hr = vpoxDispKmtOpenAdapterViaLuid(pCallbacks, pAdapter);
    if (SUCCEEDED(hr))
        return S_OK;
...
{
    VPOXWDDM_CREATECONTEXT_INFO Info = {0};
    Info.u32IfVersion = 9;
    Info.enmType = enmType;
    Info.u.vpox.crVersionMajor = 0; /* Not used */
    Info.u.vpox.crVersionMinor = 0; /* Not used */
    Info.u.vpox.hUmEvent = (uintptr_t)hEvent;
    Info.u.vpox.u64UmInfo = u64UmInfo;
    D3DKMT_CREATECONTEXT ContextData = {0};
    ContextData.hDevice = pDevice->hDevice;
    ContextData.NodeOrdinal = VPOXWDDM_NODE_ID_3D_KMT;
    ContextData.EngineAffinity = VPOXWDDM_ENGINE_ID_3D_KMT;
