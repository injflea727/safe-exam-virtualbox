...
/** @file
 *
 * VirtualPox API client session crash token handling
 */
...
#ifdef VPOX_WITH_SYS_V_IPC_SESSION_WATCHER
# include <errno.h>
# include <sys/types.h>
# include <sys/stat.h>
# include <sys/ipc.h>
...
# ifdef VPOX_WITH_NEW_SYS_V_KEYGEN
    mClientTokenId = "0";
# endif /* VPOX_WITH_NEW_SYS_V_KEYGEN */
    ...
    mMachine(pMachine)
{
    ...
    BstrFmt tokenId("Global\\VPoxSession-%s-VM-%RTuuid", strUserSid.c_str(), pMachine->mData->mUuid.raw());

    /* create security descriptor to allow SYNCHRONIZE access from any windows sessions and users.
     * otherwise VM can't open the mutex if VPoxSVC and VM are in different session (e.g. some VM
...
#elif defined(RT_OS_OS2)
    NOREF(pSessionMachine);
    Utf8Str ipcSem = Utf8StrFmt("\\SEM32\\VPOX\\VM\\{%RTuuid}",
...
# else /* !VPOX_WITH_NEW_SYS_V_KEYGEN */
...
# endif /* !VPOX_WITH_NEW_SYS_V_KEYGEN */
    ...
    /* ENOSPC can also be the result of VPoxSVC crashes without properly freeing
     * the token */
