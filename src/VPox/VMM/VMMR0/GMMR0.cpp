...
*********************************************************************************************************************************/
/** @def VPOX_USE_CRIT_SECT_FOR_GIANT
...
# define VPOX_USE_CRIT_SECT_FOR_GIANT
#endif
...
#ifdef VPOX_USE_CRIT_SECT_FOR_GIANT
 ...
 * @param   rc      The return value on failure. Use VERR_GMM_INSTANCE for VPox
 *                  status codes.
 */
 ...
 * @param   pGMM    The name of the pGMM variable.
 */
#if defined(VPOX_STRICT) && defined(GMMR0_WITH_SANITY_CHECK) && 0
...
#elif defined(VPOX_WITH_LINEAR_HOST_PHYS_MEM)
 ...
 *
 * @returns VPox status code (assuming success is ok).
 * @param   pMtxState   Pointer to the chunk mutex state.
 */
 ...
 *
 * @returns VPox status code.  On success, the giant GMM lock will be held, the
                                       ...
                                       uint32_t cPagesToAlloc, PGMMPAGEDESC paPages)
{
    ...
    if (GMM_CHECK_SANITY_UPON_ENTERING(pGMM))
    {
                      ...
                      &&  pGVM->gmm.s.Stats.Reserved.cShadowPages))
        {
            ...
            {
#if defined(VPOX_STRICT) && 0 /** @todo re-test this later. Appeared to be a PGM init bug. */
    ...
    for (iPage = 0; iPage < cPages; iPage++)
    {
        ...
        if (RT_LIKELY(pPage))
        {
            ...
            else if (RT_LIKELY(GMM_PAGE_IS_SHARED(pPage)))
            {
                Assert(pGVM->gmm.s.Stats.cSharedPages);
                Assert(pPage->Shared.cRefs);
#if defined(VPOX_WITH_PAGE_SHARING) && defined(VPOX_STRICT) && HC_ARCH_BITS == 64
...
#endif /* VPOX_WITH_PAGE_SHARING || (VPOX_STRICT && 64-BIT) */
 ...
 */
static PGMMSHAREDMODULE gmmR0ShModFindGlobal(PGMM pGMM, uint32_t uHash, uint32_t cbModule, VPOXOSFAMILY enmGuestOS,
 ...
 */
static int gmmR0ShModNewGlobal(PGMM pGMM, uint32_t uHash, uint32_t cbModule, VPOXOSFAMILY enmGuestOS,
                               uint32_t cRegions, const char *pszModuleName, const char *pszVersion,
 ...
 * @thread  EMT(idCpu)
 */
GMMR0DECL(int) GMMR0RegisterSharedModule(PGVM pGVM, VMCPUID idCpu, VPOXOSFAMILY enmGuestOS, char *pszModuleName,
    ...
    if (GMM_CHECK_SANITY_UPON_ENTERING(pGMM))
    {
        ...
        else
        {
            ...
            else
            {
                 ...
                 * by VPoxService crashing and being restarted... */
...
#if defined(VPOX_STRICT) && HC_ARCH_BITS == 64
...
#endif /* VPOX_STRICT && HC_ARCH_BITS == 64 */
