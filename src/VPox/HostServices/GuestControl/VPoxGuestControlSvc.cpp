 ...
 * - Host:   A host process (e.g. VPoxManage or another tool utilizing the Main API)
 *           which wants to control something on the guest.
 * - Client: A client (e.g. VPoxService) running inside the guest OS waiting for
 ...
 *
 * Starting at VPox 4.2 the context ID itself consists of a session ID, an object
...
#include <VPox/GuestHost/GuestControl.h> /** @todo r=bird: Why two headers??? */
...
{
    /** The call handle */
    VPOXHGCMCALLHANDLE mHandle;
    /** Number of parameters */
    uint32_t mNumParms;
    /** The call parameters */
    VPOXHGCMSVCPARM *mParms;
...
typedef struct HostMsg
{
    ...
    void Delete(void)
    {
        ...
        if (mpParms)
        {
            for (uint32_t i = 0; i < mParmCount; i++)
                if (mpParms[i].type == VPOX_HGCM_SVC_PARM_PTR)
                {
     ...
     */
    int Init(uint32_t idMsg, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
    {
         ...
         */
        if (paParms[0].type == VPOX_HGCM_SVC_PARM_64BIT)
        {
            m_idContextAndDst = paParms[0].u.uint64;
            AssertReturn(m_idContextAndDst & VPOX_GUESTCTRL_DST_BOTH, VERR_INTERNAL_ERROR_3);
        }
        else if (paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT)
        {
            AssertMsgFailed(("idMsg=%u %s - caller must set dst!\n", idMsg, GstCtrlHostMsgtoStr((eHostMsg)idMsg)));
            m_idContextAndDst = paParms[0].u.uint32 | VPOX_GUESTCTRL_DST_BOTH;
        }
        else
        ...
        mType   = idMsg;
        mParmCount = cParms;
        mpParms    = (VPOXHGCMSVCPARM *)RTMemAllocZ(sizeof(VPOXHGCMSVCPARM) * mParmCount);
         ...
         */
        mpParms[0].type     = VPOX_HGCM_SVC_PARM_32BIT;
     ...
     */
    inline void setPeekReturn(PVPOXHGCMSVCPARM paDstParms, uint32_t cDstParms)
    {
        Assert(cDstParms >= 2);
        if (paDstParms[0].type == VPOX_HGCM_SVC_PARM_32BIT)
        ...
        while (i-- > 2)
            ...
            {
                case VPOX_HGCM_SVC_PARM_32BIT: paDstParms[i].u.uint32 = ~(uint32_t)sizeof(uint32_t); break;
                case VPOX_HGCM_SVC_PARM_64BIT: paDstParms[i].u.uint32 = ~(uint32_t)sizeof(uint64_t); break;
                case VPOX_HGCM_SVC_PARM_PTR:   paDstParms[i].u.uint32 = mpParms[i - 2].u.pointer.size; break;
            }
    }
     ...
     */
    int CopyTo(VPOXHGCMSVCPARM paDstParms[], uint32_t cDstParms) const
    {
        LogFlowThisFunc(("[Msg %RU32] mParmCount=%RU32, m_idContext=%RU32 (Session %RU32)\n",
                         mType, mParmCount, m_idContext, VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(m_idContext)));
...
{
    PVPOXHGCMSVCHELPERS     m_pSvcHelpers;
    ...
    ClientState(PVPOXHGCMSVCHELPERS pSvcHelpers, uint32_t idClient)
...
private:
    ...
    /** Guest feature flags, VPOX_GUESTCTRL_GF_0_XXX. */
    uint64_t                m_fGuestFeatures0;
    /** Guest feature flags, VPOX_GUESTCTRL_GF_1_XXX. */
    uint64_t                m_fGuestFeatures1;

public:
    explicit GstCtrlService(PVPOXHGCMSVCHELPERS pHelpers)
        : mpHelpers(pHelpers)
    ...
    static DECLCALLBACK(void) svcCall(void *pvService, VPOXHGCMCALLHANDLE hCall, uint32_t idClient, void *pvClient,
                                      uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[], uint64_t tsArrival);
    static DECLCALLBACK(int)  svcHostCall(void *pvService, uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
...
private:
    int clientMakeMeMaster(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms);
    int clientReportFeatures(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientQueryFeatures(VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientMsgPeek(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[], bool fWait);
    int clientMsgGet(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientMsgCancel(ClientState *pClient, uint32_t cParms);
    int clientMsgSkip(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientSessionPrepare(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientSessionCancelPrepared(ClientState *pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientSessionAccept(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientSessionCloseOther(ClientState *pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientToMain(ClientState *pClient, uint32_t idMsg, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);

    int clientMsgOldGet(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientMsgOldFilterSet(ClientState *pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int clientMsgOldSkip(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms);

    int hostCallback(uint32_t idMsg, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int hostProcessMessage(uint32_t idMsg, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
...
static uint64_t const g_fGstCtrlHostFeatures0 = VPOX_GUESTCTRL_HF_0_NOTIFY_RDWR_OFFSET
                                              | VPOX_GUESTCTRL_HF_0_PROCESS_ARGV0;


/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnUnload,
...
/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnConnect,
...
GstCtrlService::svcConnect(void *pvService, uint32_t idClient, void *pvClient, uint32_t fRequestor, bool fRestoring)
{
     ...
     * point, so if the /dev/vpoxguest requirements checks out we pick the first
 ...
 * @param   paParms         Array of parameters.
 */
int GstCtrlService::clientMsgOldGet(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
 ...
 * @retval  VERR_ACCESS_DENIED if not using main VPoxGuest device not
 * @retval  VERR_RESOURCE_BUSY if there is already a master.
 * @retval  VERR_VERSION_MISMATCH if VPoxGuest didn't supply requestor info.
 ...
 * @param   cParms      Number of parameters.
 */
int GstCtrlService::clientMakeMeMaster(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms)
{
    /*
    ...
    /* The next assertion triggers upgrading GAs on some linux guests. Problem is that VPoxService is
    ...
    ASSERT_GUEST_LOGREL_MSG_RETURN(fRequestor != VMMDEV_REQUESTOR_LEGACY,
                                   ("Guest is using outdated VPoxGuest w/o requestor support.\n"
 ...
 * @param   paParms     Array of parameters.
 */
int GstCtrlService::clientReportFeatures(ClientState *pClient, VPOXHGCMCALLHANDLE hCall,
                                         uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate the request.
     */
    ASSERT_GUEST_RETURN(cParms == 2, VERR_WRONG_PARAMETER_COUNT);
    ASSERT_GUEST_RETURN(paParms[0].type == VPOX_HGCM_SVC_PARM_64BIT, VERR_WRONG_PARAMETER_TYPE);
    uint64_t const fFeatures0 = paParms[0].u.uint64;
    ASSERT_GUEST_RETURN(paParms[1].type == VPOX_HGCM_SVC_PARM_64BIT, VERR_WRONG_PARAMETER_TYPE);
    uint64_t const fFeatures1 = paParms[1].u.uint64;
    ASSERT_GUEST_RETURN(fFeatures1 & VPOX_GUESTCTRL_GF_1_MUST_BE_ONE, VERR_INVALID_PARAMETER);
     ...
     * Do the work.
     */
    VPOXHGCMSVCPARM aCopyForMain[2] = { paParms[0], paParms[1] };
 ...
 * @param   paParms     Array of parameters.
 */
int GstCtrlService::clientQueryFeatures(VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate the request.
     */
 ...
 *                      immediately.
 */
int GstCtrlService::clientMsgPeek(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[], bool fWait)
{
    /*
     * Validate the request.
     */
    ...
    uint32_t i              = 0;
    if (paParms[i].type == VPOX_HGCM_SVC_PARM_64BIT)
    {
    ...
    for (; i < cParms; i++)
    {
        ASSERT_GUEST_MSG_RETURN(paParms[i].type == VPOX_HGCM_SVC_PARM_32BIT, ("#%u type=%u\n", i, paParms[i].type),
                                VERR_WRONG_PARAMETER_TYPE);
 ...
 * @param   paParms     Array of parameters.
 */
int GstCtrlService::clientMsgGet(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate the request.
     *
     ...
     */
    uint32_t const idMsgExpected = cParms > 0 && paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT ? paParms[0].u.uint32
                                 : cParms > 0 && paParms[0].type == VPOX_HGCM_SVC_PARM_64BIT ? paParms[0].u.uint64
                                 : UINT32_MAX;
 ...
 */
int GstCtrlService::clientMsgSkip(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate the call.
     */
    ...
    if (cParms >= 1)
    {
        ASSERT_GUEST_RETURN(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE);
    ...
    if (cParms >= 2)
    {
        ASSERT_GUEST_RETURN(paParms[1].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE);
    ...
    if (pFirstMsg)
    {
            ...
            || idMsg == UINT32_MAX)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                 ...
                 */
                VPOXHGCMSVCPARM aReplyParams[5];
                ...
                switch (pFirstMsg->mType)
                {
                    ...
                    case HOST_MSG_FILE_OPEN:
                        ...
                        HGCMSvcSetU32(&aReplyParams[3], VPOX_GUESTCTRL_CONTEXTID_GET_OBJECT(pFirstMsg->m_idContext)); /* handle */
 ...
 * @param   paParms     The parameters.
 */
int GstCtrlService::clientSessionPrepare(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate parameters.
     */
    ...
    ASSERT_GUEST_RETURN(paParms[1].type == VPOX_HGCM_SVC_PARM_PTR, VERR_WRONG_PARAMETER_TYPE);
 ...
 * @param   paParms     The parameters.
 */
int GstCtrlService::clientSessionCancelPrepared(ClientState *pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate parameters.
     */
 ...
 * @param   paParms     The parameters.
 */
int GstCtrlService::clientSessionAccept(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate parameters.
     */
 ...
 * @param   paParms         Array of parameters.
 */
int GstCtrlService::clientSessionCloseOther(ClientState *pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate input.
     */
     ...
     */
    VPOXHGCMSVCPARM aParms[2];
    HGCMSvcSetU64(&aParms[0], idContext | VPOX_GUESTCTRL_DST_SESSION);
 ...
 * @param   paParms     Array of parameters.
 */
int GstCtrlService::clientMsgOldFilterSet(ClientState *pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
    ...
    ASSERT_GUEST_RETURN(paParms[2].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE);
    uint32_t fMaskRemove = paParms[2].u.uint32;
    ASSERT_GUEST_RETURN(paParms[3].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE); /* flags, unused */
    ...
    else
    {
        ...
        uint32_t idSession = VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(uValue);
 ...
 * @param   cParms      Number of parameters.
 */
int GstCtrlService::clientMsgOldSkip(ClientState *pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms)
{
    /*
     * Validate input and access.
     */
 ...
 * @param   paParms         Array of parameters.
 */
int GstCtrlService::clientToMain(ClientState *pClient, uint32_t idMsg, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     ...
     */
    ASSERT_GUEST_RETURN(cParms >= 1, VERR_WRONG_PARAMETER_COUNT);
    ASSERT_GUEST_RETURN(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_COUNT);
    uint32_t const idContext = paParms[0].u.uint32;
    uint32_t const idSession = VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(idContext);
...
/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnCall}
 *
...
GstCtrlService::svcCall(void *pvService, VPOXHGCMCALLHANDLE hCall, uint32_t idClient, void *pvClient,
                        uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[], uint64_t tsArrival)
{
 ...
 * @param   paParms         Array of parameters.
 */
int GstCtrlService::hostCallback(uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    ...
    if (mpfnHostCallback)
    {
        VPOXGUESTCTRLHOSTCALLBACK data = { cParms, paParms };
 ...
 * @param   paParms     Array of parameters.
 */
int GstCtrlService::hostProcessMessage(uint32_t idMsg, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     ...
     * waiting for a response from the guest side in case VPoxService on
    ...
    if (RT_SUCCESS(rc))
    {
        uint64_t const fDestinations = pHostMsg->m_idContextAndDst & VPOX_GUESTCTRL_DST_BOTH;
        HostMsg       *pHostMsg2     = NULL;
        if (fDestinations != VPOX_GUESTCTRL_DST_BOTH)
        { /* likely */ }
        else
        {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (fDestinations & VPOX_GUESTCTRL_DST_SESSION)
            {
                uint32_t const idSession = VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(pHostMsg->m_idContext);
                ...
                else
                {
                    ...
                    rc = !(fDestinations & VPOX_GUESTCTRL_DST_ROOT_SVC) ? VERR_NOT_FOUND : VWRN_NOT_FOUND;
                }
            }

            /* Does the message go to the root service? */
            if (   (fDestinations & VPOX_GUESTCTRL_DST_ROOT_SVC)
                && RT_SUCCESS(rc))
            {
                Assert(pHostMsg);
                if (m_pMasterClient)
...
/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnHostCall,
 *  Wraps to the hostProcessMessage() member function.}
 */
/*static*/ DECLCALLBACK(int)
GstCtrlService::svcHostCall(void *pvService, uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    AssertLogRelReturn(VALID_PTR(pvService), VERR_INVALID_PARAMETER);
    SELF *pThis = reinterpret_cast<SELF *>(pvService);
    AssertPtrReturn(pThis, VERR_INVALID_POINTER);
...
/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnSaveState}
 */
/*static*/ DECLCALLBACK(int)
...
/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnLoadState}
 */
/*static*/ DECLCALLBACK(int)
GstCtrlService::svcLoadState(void *pvService, uint32_t idClient, void *pvClient, PSSMHANDLE pSSM, uint32_t uVersion)
{
    ...
    else
    {
         ...
         * VPoxService session processes in the restored guest, and I don't
...
/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnRegisterExtension,
