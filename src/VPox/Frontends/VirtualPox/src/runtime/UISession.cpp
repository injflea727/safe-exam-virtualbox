/* $Id: UISession.cpp $ */
/** @file
 * VPox Qt GUI - UISession class implementation.
 */
...
#endif
#ifdef VPOX_GUI_WITH_KEYS_RESET_HANDLER
...
bool UISession::initialize()
{
    ...
    LogRel(("GUI: 2D video acceleration is %s\n",
           machine().GetGraphicsAdapter().GetAccelerate2DVideoEnabled() && VPox2DHelpers::isAcceleration2DVideoAvailable()
           ? "enabled" : "disabled"));
#endif /* VPOX_WITH_VIDEOHWACCEL */

/* Log whether HID LEDs sync is enabled: */
#if defined(VPOX_WS_MAC) || defined(VPOX_WS_WIN)
    LogRel(("GUI: HID LEDs sync is %s\n",
            uimachine()->machineLogic()->isHidLedsSyncEnabled()
            ? "enabled" : "disabled"));
#else /* !VPOX_WS_MAC && !VPOX_WS_WIN */
    LogRel(("GUI: HID LEDs sync is not supported on this platform\n"));
#endif /* !VPOX_WS_MAC && !VPOX_WS_WIN */
...
bool UISession::powerOff(bool fIncludingDiscard, bool &fServerCrashed)
{
    ...
    else
    {
        ...
        if (!console().isNull())
        {
           ...
           /* This can happen if VPoxSVC is not running: */
...
bool UISession::restoreCurrentSnapshot()
{
    ...
    do
    {
        ...
        const CMachine mach = vpox.FindMachine(uMachineID.toString());
        if (!vpox.isOk() || mach.isNull())
        {
            /* Unable to find VM: */
            msgCenter().cannotFindMachineById(vpox, uMachineID);
            break;
        }
...
void UISession::sltInstallGuestAdditionsFrom(const QString &strSource)
{
...
#ifndef VPOX_WITH_ADDITIONS_AUTOUPDATE_UI
    fDoMount = true;
#else /* VPOX_WITH_ADDITIONS_AUTOUPDATE_UI */
    ...
    if (guest().isOk() && comProgressInstall.isNotNull())
    {
        ...
        {
            /* If we got back a VPOX_E_NOT_SUPPORTED we don't complain (guest OS simply isn't
             * supported yet), so silently fall back to "old" .ISO mounting method. */
            if (   !SUCCEEDED_WARNING(rc)
                && rc != VPOX_E_NOT_SUPPORTED)
            {
...
#endif /* VPOX_WITH_ADDITIONS_AUTOUPDATE_UI */
...
#endif /* VPOX_GUI_WITH_KEYS_RESET_HANDLER */
...
# if defined(VPOX_WS_X11) && !defined(VPOX_GUI_WITH_CUSTOMIZATIONS1)
...
# else /* !VPOX_WS_X11 || VPOX_GUI_WITH_CUSTOMIZATIONS1 */
    connect(gpDesktop, &UIDesktopWidgetWatchdog::sigHostScreenWorkAreaResized,
            this, &UISession::sltHandleHostScreenAvailableAreaChange);
# endif /* !VPOX_WS_X11 || VPOX_GUI_WITH_CUSTOMIZATIONS1 */
#endif /* !VPOX_WS_MAC */
}
...
void UISession::loadSessionSettings()
{
    /* Load extra-data settings: */
    {
        ...
        /* Prepare machine-window icon: */
        {
            ...
            if (icon.isNull())
                icon = QIcon(":/VirtualPox_48px.png");
...
void UISession::cleanupSession()
{
    ...
    /* Close session: */
    if (!m_session.isNull() && uiCommon().isVPoxSVCAvailable())
    {
...
#elif defined(VPOX_WS_X11) || defined(VPOX_WS_MAC)
...
bool UISession::preprocessInitialization()
{
    ...
    ulong cCount = uiCommon().virtualPox().GetSystemProperties().GetMaxNetworkAdapters(machine().GetChipsetType());
...
bool UISession::mountAdHocImage(KDeviceType enmDeviceType, UIMediumDeviceType enmMediumType, const QString &strMediumName)
{
    ...
    {
        /* Open the medium: */
        const CMedium comMedium = comVPox.OpenMedium(strMediumName, enmDeviceType, KAccessMode_ReadWrite, false /* fForceNewUuid */);
        if (!comVPox.isOk() || comMedium.isNull())
        {
            popupCenter().cannotOpenMedium(activeMachineWindow(), comVPox, enmMediumType, strMediumName);
            return false;
        }
...
bool UISession::postprocessInitialization()
{
    ...
    const bool fIs64BitsGuest = uiCommon().virtualPox().GetGuestOSType(guest().GetOSTypeId()).GetIs64Bit();
    const bool fRecommendVirtEx = uiCommon().virtualPox().GetGuestOSType(guest().GetOSTypeId()).GetRecommendedVirtEx();
...
void UISession::updateActionRestrictions()
{
    ...
    /* Network stuff: */
    {
        ...
        ULONG uSlots = uiCommon().virtualPox().GetSystemProperties().GetMaxNetworkAdapters(chipsetType);
