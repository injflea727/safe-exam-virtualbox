...
#ifndef VPOX_INCLUDED_types_h
#define VPOX_INCLUDED_types_h
...
/** @defgroup grp_types     VPox Basic Types
 * @{
 */
...
/** @def VPOXSTRICTRC_STRICT_ENABLED
 * Indicates that VPOXSTRICTRC is in strict mode.
 */
...
# define VPOXSTRICTRC_STRICT_ENABLED 1
#endif

/** We need RTERR_STRICT_RC.  */
#if defined(VPOXSTRICTRC_STRICT_ENABLED) && !defined(RTERR_STRICT_RC)
# define RTERR_STRICT_RC 1
#endif

/**
 * Strict VirtualPox status code.
 *
 ...
 * handling.
 */
#ifdef VPOXSTRICTRC_STRICT_ENABLED
struct VPOXSTRICTRC
{
protected:
    /** The status code. */
...
public:
    ...
    VPOXSTRICTRC(int32_t const rc)
        : m_rc(rc)
    {
    }

    /** Getter that VPOXSTRICTRC_VAL can use. */
    ...
    bool operator==(const VPOXSTRICTRC &rRc) const  { return m_rc == rRc.m_rc; }
    bool operator!=(const VPOXSTRICTRC &rRc) const  { return m_rc != rRc.m_rc; }
    bool operator<=(const VPOXSTRICTRC &rRc) const  { return m_rc <= rRc.m_rc; }
    bool operator>=(const VPOXSTRICTRC &rRc) const  { return m_rc >= rRc.m_rc; }
    bool operator<(const VPOXSTRICTRC &rRc) const   { return m_rc <  rRc.m_rc; }
...
private:
     ...
     * @{ */
    VPOXSTRICTRC(uint8_t  rc) : m_rc(-999)          { NOREF(rc); }
    VPOXSTRICTRC(uint16_t rc) : m_rc(-999)          { NOREF(rc); }
    VPOXSTRICTRC(uint32_t rc) : m_rc(-999)          { NOREF(rc); }
    VPOXSTRICTRC(uint64_t rc) : m_rc(-999)          { NOREF(rc); }

    VPOXSTRICTRC(int8_t rc)   : m_rc(-999)          { NOREF(rc); }
    VPOXSTRICTRC(int16_t rc)  : m_rc(-999)          { NOREF(rc); }
    VPOXSTRICTRC(int64_t rc)  : m_rc(-999)          { NOREF(rc); }
    /** @} */
};
# ifdef _MSC_VER
...
# endif
#else
typedef int32_t VPOXSTRICTRC;
#endif

/** @def VPOXSTRICTRC_VAL
 * Explicit getter.
 * @param rcStrict  The strict VirtualPox status code.
 */
#ifdef VPOXSTRICTRC_STRICT_ENABLED
# define VPOXSTRICTRC_VAL(rcStrict) ( (rcStrict).getValue() )
#else
# define VPOXSTRICTRC_VAL(rcStrict) (rcStrict)
#endif

/** @def VPOXSTRICTRC_TODO
 * Returns that needs dealing with.
 * @param rcStrict  The strict VirtualPox status code.
 */
#define VPOXSTRICTRC_TODO(rcStrict) VPOXSTRICTRC_VAL(rcStrict)
...
/** @todo VPOXIDT -> VPOXDESCIDT, skip the complex variations. We'll never use them. */

/** IDT Entry, Task Gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct VPOXIDTE_TASKGATE
{
    /** Reserved. */
...
typedef VPOXIDTE_TASKGATE *PVPOXIDTE_TASKGATE;


/** IDT Entry, Intertupt gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct VPOXIDTE_INTERRUPTGATE
{
    /** Low offset word. */
...
} VPOXIDTE_INTERRUPTGATE;
#pragma pack()
/** Pointer to IDT Entry, Interrupt gate view. */
typedef  VPOXIDTE_INTERRUPTGATE *PVPOXIDTE_INTERRUPTGATE;

/** IDT Entry, Trap Gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct VPOXIDTE_TRAPGATE
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
typedef VPOXIDTE_TRAPGATE *PVPOXIDTE_TRAPGATE;

/** IDT Entry Generic view. */
#pragma pack(1)                         /* paranoia */
typedef struct VPOXIDTE_GENERIC
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
typedef VPOXIDTE_GENERIC *PVPOXIDTE_GENERIC;

/** IDT Type1 value. (Reserved for task gate!) */
#define VPOX_IDTE_TYPE1             0
/** IDT Type2 value - Task gate. */
#define VPOX_IDTE_TYPE2_TASK        0x5
/** IDT Type2 value - 16 bit interrupt gate. */
#define VPOX_IDTE_TYPE2_INT_16      0x6
/** IDT Type2 value - 32 bit interrupt gate. */
#define VPOX_IDTE_TYPE2_INT_32      0xe
/** IDT Type2 value - 16 bit trap gate. */
#define VPOX_IDTE_TYPE2_TRAP_16     0x7
/** IDT Type2 value - 32 bit trap gate. */
#define VPOX_IDTE_TYPE2_TRAP_32     0xf

/** IDT Entry. */
#pragma pack(1)                         /* paranoia */
typedef union VPOXIDTE
{
    /** Task gate view. */
    VPOXIDTE_TASKGATE       Task;
    /** Trap gate view. */
    VPOXIDTE_TRAPGATE       Trap;
    /** Interrupt gate view. */
    VPOXIDTE_INTERRUPTGATE  Int;
    /** Generic IDT view. */
    VPOXIDTE_GENERIC        Gen;
...
typedef VPOXIDTE *PVPOXIDTE;
/** Pointer to IDT Entry. */
typedef VPOXIDTE const *PCVPOXIDTE;

/** IDT Entry, 64-bit mode, Intertupt gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct VPOXIDTE64_INTERRUPTGATE
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
} VPOXIDTE64_INTERRUPTGATE;
#pragma pack()
/** Pointer to IDT Entry, 64-bit mode, Interrupt gate view. */
typedef  VPOXIDTE64_INTERRUPTGATE *PVPOXIDTE64_INTERRUPTGATE;

/** IDT Entry, 64-bit mode, Trap gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct VPOXIDTE64_TRAPGATE
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
typedef  VPOXIDTE64_TRAPGATE *PVPOXIDTE64_TRAPGATE;

/** IDT Entry, 64-bit mode, Generic view. */
#pragma pack(1)                         /* paranoia */
typedef struct VPOXIDTE64_GENERIC
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
typedef VPOXIDTE64_GENERIC *PVPOXIDTE64_GENERIC;

/** IDT Entry, 64-bit mode. */
#pragma pack(1)                         /* paranoia */
typedef union VPOXIDTE64
{
    /** Trap gate view. */
    VPOXIDTE64_TRAPGATE       Trap;
    /** Interrupt gate view. */
    VPOXIDTE64_INTERRUPTGATE  Int;
    /** Generic IDT view. */
    VPOXIDTE64_GENERIC        Gen;
...
#pragma pack()
/** Pointer to IDT Entry. */
typedef VPOXIDTE64 *PVPOXIDTE64;
/** Pointer to IDT Entry. */
typedef VPOXIDTE64 const *PCVPOXIDTE64;

#pragma pack(1)
/** IDTR */
typedef struct VPOXIDTR
{
    /** Size of the IDT. */
    uint16_t    cbIdt;
    /** Address of the IDT. */
 ...
 */
#define VPOXIDTE_OFFSET(desc) \
        (  ((uint32_t)((desc).Gen.u16OffsetHigh) << 16) \
         | (           (desc).Gen.u16OffsetLow        ) )

/** @def VPOXIDTE64_OFFSET
 * Return the offset of an IDT entry.
 */
#define VPOXIDTE64_OFFSET(desc) \
...
typedef VPOXTSS *PVPOXTSS;
/** Pointer to const task segment. */
typedef const VPOXTSS *PCVPOXTSS;
...
{
#if defined(IN_RC) || defined(VPOX_WITH_2X_4GB_ADDR_SPACE_IN_R0)
    /** The locked page. */
    void       *pvPage;
