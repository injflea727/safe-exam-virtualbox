...
"""
VirtualPox Validation Kit - Python Bindings Test #1
"""
...
class SubTstDrvPython1(base.SubTestDriverBase):
    ...
    def testEventQueueWaitingThreadProc(self):
        ...
        try:
            rc2 = self.oTstDrv.oVPoxMgr.waitForEvents(0);
        except:
            return True;
    ...
    def testEventQueueWaiting(self):
        ...
        for cMsTimeout in (0, 1, 2, 3, 256, 1000, 0):
            ...
            while True:
                try:
                    rc = self.oTstDrv.oVPoxMgr.waitForEvents(cMsTimeout);
                except:
                    reporter.errorXcpt();
                    break;
        ...
        sVPoxAssertSaved = os.environ.get('VPOX_ASSERT', 'breakpoint');
        os.environ['VPOX_ASSERT'] = 'ignore';
        oThread = threading.Thread(target=self.testEventQueueWaitingThreadProc);
        oThread.start();
        oThread.join();
        os.environ['VPOX_ASSERT'] = sVPoxAssertSaved;
    ...
    def interruptWaitEventsThreadProc(self):
        ...
        time.sleep(2);
        try:
            rc2 = self.oTstDrv.oVPoxMgr.interruptWaitEvents();
        except:
            reporter.errorXcpt();
        else:
            if rc2 is True:
    ...
    def testEventQueueInterrupt(self):
        ...
        for i in range(0, 10):
            try:
                rc = self.oTstDrv.oVPoxMgr.interruptWaitEvents();
            except:
                reporter.errorXcpt();
                break;
            if rc is not True:
        ...
        if reporter.testErrorCount() == 0:
            ...
            for i in range(0, 4):
                ...
                for _ in range(1, 100):
                    self.oTstDrv.oVPoxMgr.waitForEvents(0);
