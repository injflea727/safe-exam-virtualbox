...
#if defined(RT_OS_WINDOWS)
# define VPOX_SOCKET_EVENT (pData->phEvents[VPOX_SOCKET_EVENT_INDEX])
    HANDLE phEvents[VPOX_EVENT_COUNT];
#endif
#ifdef zone_mbuf
# undef zone_mbuf
#endif
...
# define QSOCKET_FOREACH(so, sonext, label)                              \
    for ((so)  = VPOX_X2(queue_ ## label ## _label).so_next;             \
         (so) != &(VPOX_X2(queue_ ## label ## _label));                  \
         (so) = (sonext))                                                \
    {                                                                    \
        (sonext) = (so)->so_next;                                        \
...
# define SOLOOKUP(so, label, src, sport, dst, dport)                                      \
    do {                                                                                  \
        (so) = solookup(&VPOX_X2(queue_ ## label ## _label), (src), (sport), (dst), (dport)); \
    } while (0)
