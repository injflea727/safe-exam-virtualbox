/* $Id: VPoxCredProvUtils.cpp $ */
/** @file
 * VPoxCredProvUtils - Misc. utility functions for VPoxCredProv.
 */
 ...
 */
void VPoxCredProvVerbose(DWORD dwLevel, const char *pszFormat, ...)
{
...
#ifdef LOG_ENABLED
        PRTSTREAM pStream;
        int rc2 = RTStrmOpen("C:\\VPoxCredProvLog.txt", "a", &pStream);
        if (RT_SUCCESS(rc2))
        {
 ...
 */
int VPoxCredProvReportStatus(VPoxGuestFacilityStatus enmStatus)
{
    VPoxCredProvVerbose(0, "VPoxCredProv: reporting status %d\n", enmStatus);

    int rc = VbglR3AutoLogonReportStatus(enmStatus);
    if (RT_FAILURE(rc))
        VPoxCredProvVerbose(0, "VPoxCredProv: failed to report status %d, rc=%Rrc\n", enmStatus, rc);
    return rc;
}
