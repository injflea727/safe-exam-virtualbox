...
#include <VPox/GuestHost/SharedClipboard-x11.h>
...
{
    { "INVALID",                            SHCLX11FMT_INVALID,     VPOX_SHCL_FMT_NONE },

    { "UTF8_STRING",                        SHCLX11FMT_UTF8,        VPOX_SHCL_FMT_UNICODETEXT },
    { "text/plain;charset=UTF-8",           SHCLX11FMT_UTF8,        VPOX_SHCL_FMT_UNICODETEXT },
    { "text/plain;charset=utf-8",           SHCLX11FMT_UTF8,        VPOX_SHCL_FMT_UNICODETEXT },
    { "STRING",                             SHCLX11FMT_TEXT,        VPOX_SHCL_FMT_UNICODETEXT },
    { "TEXT",                               SHCLX11FMT_TEXT,        VPOX_SHCL_FMT_UNICODETEXT },
    { "text/plain",                         SHCLX11FMT_TEXT,        VPOX_SHCL_FMT_UNICODETEXT },

    { "text/html",                          SHCLX11FMT_HTML,        VPOX_SHCL_FMT_HTML },
    { "text/html;charset=utf-8",            SHCLX11FMT_HTML,        VPOX_SHCL_FMT_HTML },
    { "application/x-moz-nativehtml",       SHCLX11FMT_HTML,        VPOX_SHCL_FMT_HTML },

    { "image/bmp",                          SHCLX11FMT_BMP,         VPOX_SHCL_FMT_BITMAP },
    { "image/x-bmp",                        SHCLX11FMT_BMP,         VPOX_SHCL_FMT_BITMAP },
    { "image/x-MS-bmp",                     SHCLX11FMT_BMP,         VPOX_SHCL_FMT_BITMAP },
    /** @todo Inkscape exports image/png but not bmp... */

#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
    { "text/uri-list",                      SHCLX11FMT_URI_LIST,    VPOX_SHCL_FMT_URI_LIST },
    { "x-special/gnome-copied-files",       SHCLX11FMT_URI_LIST,    VPOX_SHCL_FMT_URI_LIST },
    { "x-special/nautilus-clipboard",       SHCLX11FMT_URI_LIST,    VPOX_SHCL_FMT_URI_LIST },
    { "application/x-kde-cutselection",     SHCLX11FMT_URI_LIST,    VPOX_SHCL_FMT_URI_LIST },
...
{
    /** The format VPox would like the data in. */
    SHCLFORMAT       uFmtVPox;
...
/**
 * Returns the VPox format corresponding to a supported X11 format.
 *
 * @return  SHCLFORMAT for a specific format index.
 * @param   uFmtIdx             Format index to look up VPox format for.
 */
static SHCLFORMAT clipVPoxFormatForX11Format(SHCLX11FMTIDX uFmtIdx)
{
    AssertReturn(uFmtIdx < RT_ELEMENTS(g_aFormats), VPOX_SHCL_FMT_NONE);
    return g_aFormats[uFmtIdx].uFmtVPox;
}
...
static struct
{
    ...
    PSHCLX11CTX pCtx;
} g_aContexts[VPOX_SHARED_CLIPBOARD_X11_CONNECTIONS_MAX];
...
/**
 * Reports the formats currently supported by the X11 clipboard to VPox.
 *
 * @note    Runs in Xt event thread.
 *
 * @param   pCtx                The X11 clipboard context to use.
 */
static void clipReportFormatsToVPox(PSHCLX11CTX pCtx)
{
    SHCLFORMATS vpoxFmt  = clipVPoxFormatForX11Format(pCtx->idxFmtText);
                vpoxFmt |= clipVPoxFormatForX11Format(pCtx->idxFmtBmp);
                vpoxFmt |= clipVPoxFormatForX11Format(pCtx->idxFmtHTML);
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
                vpoxFmt |= clipVPoxFormatForX11Format(pCtx->idxFmtURI);
#endif
    ...
    LogRel2(("Shared Clipboard: X11 reported available VPox formats: %#x\n", vpoxFmt));

    ShClX11ReportFormatsCallback(pCtx->pFrontend, vpoxFmt);
}
...
/**
 * Tells VPox that X11 currently has nothing in its clipboard.
 *
 * @param  pCtx                 The X11 clipboard context to use.
 */
                                                          ...
                                                          size_t cTargets)
{
    ...
    for (unsigned i = 0; i < cTargets; ++i)
    {
        ...
        {
            if (   (clipVPoxFormatForX11Format(idxFmt) == VPOX_SHCL_FMT_UNICODETEXT)
                                                    ...
                                                    size_t cTargets)
{
    ...
    for (unsigned i = 0; i < cTargets; ++i)
    {
        ...
        if (idxFmt != NIL_CLIPX11FORMAT)
        {
            if (   (clipVPoxFormatForX11Format(idxFmt) == VPOX_SHCL_FMT_BITMAP)
                                                  ...
                                                  size_t cTargets)
{
    ...
    for (unsigned i = 0; i < cTargets; ++i)
    {
        ...
        if (idxFmt != NIL_CLIPX11FORMAT)
        {
            if (   (clipVPoxFormatForX11Format(idxFmt) == VPOX_SHCL_FMT_HTML)
                                                     ...
                                                     size_t cTargets)
{
    ...
    for (unsigned i = 0; i < cTargets; ++i)
    {
        ...
        if (idxFmt != NIL_CLIPX11FORMAT)
        {
            if (   (clipVPoxFormatForX11Format(idxFmt) == VPOX_SHCL_FMT_URI_LIST)
...
/**
 * Notifies the VPox clipboard about available data formats, based on the
...
static void clipThreadSignalStop(void *pvUserData, void *)
{
     ...
     * the VPox part *must* have returned before we do this. */
...
static int clipInitInternal(PSHCLX11CTX pCtx)
{
    ...
    int      cArgc  = 0;
    char    *pcArgv = 0;
    Display *pDisplay = XtOpenDisplay(pCtx->pAppContext, 0, 0, "VPoxShCl", 0, 0, &cArgc, &pcArgv);
    if (pDisplay == NULL)
    {
    ...
    if (RT_SUCCESS(rc))
    {
        pCtx->pWidget = XtVaAppCreateShell(0, "VPoxShCl",
...
int ShClX11Init(PSHCLX11CTX pCtx, PSHCLCONTEXT pParent, bool fHeadless)
{
    ...
    if (fHeadless)
    {
         ...
         * This is important for VPoxHeadless.
         */
 ...
 *
 * @note  Any requests from this object to get clipboard data from VPox
...
/**
 * Returns the targets supported by VPox.
 *
                                ...
                                int *piFormatReturn)
{
    ...
    do
    {
        idxFmt = clipEnumX11Formats(pCtx->vpoxFormats, idxFmt);
        if (idxFmt != NIL_CLIPX11FORMAT)
        {
 ...
 * data returned.
 *
 * @returns VPox status code. VERR_NO_DATA if no data available.
 * @param   pCtx                The X11 clipboard context to use.
 ...
 */
static int clipReadVPoxShCl(PSHCLX11CTX pCtx, SHCLFORMAT Format,
                            void **ppv, uint32_t *pcb)
{
    ...
    if (Format == VPOX_SHCL_FMT_UNICODETEXT)
    {
 ...
 *
 * @returns VPox status code. VERR_NO_DATA if no data was converted.
                                ...
                                int *piFormatReturn)
{
    ...
    LogFlowFunc(("vpoxFormats=0x%x, idxFmtX11=%u ('%s'), fmtX11=%u\n",
                 pCtx->vpoxFormats, idxFmtX11, g_aFormats[idxFmtX11].pcszAtom, fmtX11));

    LogRel2(("Shared Clipboard: Converting VPox formats %#x to '%s' for X11\n",
             pCtx->vpoxFormats, g_aFormats[idxFmtX11].pcszAtom));

    if (   ((fmtX11 == SHCLX11FMT_UTF8) || (fmtX11 == SHCLX11FMT_TEXT))
        && (pCtx->vpoxFormats & VPOX_SHCL_FMT_UNICODETEXT))
    {
        void    *pv = NULL;
        uint32_t cb = 0;
        rc = clipReadVPoxShCl(pCtx, VPOX_SHCL_FMT_UNICODETEXT, &pv, &cb);
        ...
        RTMemFree(pv);
             ...
             && (pCtx->vpoxFormats & VPOX_SHCL_FMT_BITMAP))
    {
        void    *pv = NULL;
        uint32_t cb = 0;
        rc = clipReadVPoxShCl(pCtx, VPOX_SHCL_FMT_BITMAP, &pv, &cb);
        if (RT_SUCCESS(rc) && (cb == 0))
            rc = VERR_NO_DATA;
        ...
        RTMemFree(pv);
            ...
            && (pCtx->vpoxFormats & VPOX_SHCL_FMT_HTML))
    {
        void    *pv = NULL;
        uint32_t cb = 0;
        rc = clipReadVPoxShCl(pCtx, VPOX_SHCL_FMT_HTML, &pv, &cb);
        if (RT_SUCCESS(rc) && (cb == 0))
            rc = VERR_NO_DATA;
        if (RT_SUCCESS(rc))
        {
            /**
             * The common VPox HTML encoding will be UTF-8.
...
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
    else if (pCtx->vpoxFormats & VPOX_SHCL_FMT_URI_LIST)
    {
        switch (fmtX11)
        {
    ...
    if (RT_FAILURE(rc))
    {
        ...
        LogRel(("Shared Clipboard: Converting VPox formats %#x to '%s' for X11 (idxFmtX11=%u, fmtX11=%u, atomTarget='%s') failed, rc=%Rrc\n",
                pCtx->vpoxFormats, g_aFormats[idxFmtX11].pcszAtom, idxFmtX11, fmtX11, pszAtomName ? pszAtomName : "unknown", rc));
        if (pszAtomName)
            XFree(pszAtomName);
    }
...
/**
 * Returns VPox's clipboard data for an X11 client.
 *
...
/**
 * Structure used to pass information about formats that VPox supports.
 */
typedef struct _CLIPNEWVPOXFORMATS
{
    /** Context information for the X11 clipboard. */
    PSHCLX11CTX pCtx;
    /** Formats supported by VPox. */
    SHCLFORMATS  Formats;
} CLIPNEWVPOXFORMATS, *PCLIPNEWVPOXFORMATS;



/**
 * Invalidates the local cache of the data in the VPox clipboard.
 *
 * @param   pCtx                The X11 clipboard context to use.
 */
 ...
 * event thread.
 *
 * @param pvUserData            Pointer to a CLIPNEWVPOXFORMATS structure containing
 *                              information about the VPox formats available and the
...
static void ShClX11ReportFormatsToX11Worker(void *pvUserData, void * /* interval */)
{
    ...
    CLIPNEWVPOXFORMATS *pFormats = (CLIPNEWVPOXFORMATS *)pvUserData;
...
int ShClX11ReportFormatsToX11(PSHCLX11CTX pCtx, SHCLFORMATS uFormats)
{
    ...
    PCLIPNEWVPOXFORMATS pFormats = (PCLIPNEWVPOXFORMATS)RTMemAlloc(sizeof(CLIPNEWVPOXFORMATS));
    if (pFormats)
    {
        pFormats->pCtx    = pCtx;
...
SHCL_X11_DECL(void) clipConvertDataFromX11Worker(void *pClient, void *pvSrc, unsigned cbSrc)
{
    ...
    LogFlowFunc(("pReq->uFmtVPox=%#x, pReq->idxFmtX11=%u, pReq->pCtx=%p\n", pReq->uFmtVPox, pReq->idxFmtX11, pReq->pCtx));

    LogRel2(("Shared Clipboard: Converting X11 format '%s' to VPox format %#x\n",
             g_aFormats[pReq->idxFmtX11].pcszAtom, pReq->uFmtVPox));

    AssertPtr(pReq->pCtx);
    Assert(pReq->uFmtVPox != VPOX_SHCL_FMT_NONE); /* Sanity. */
    ...
    else if (pReq->uFmtVPox == VPOX_SHCL_FMT_UNICODETEXT)
    {
    ...
    else if (pReq->uFmtVPox == VPOX_SHCL_FMT_BITMAP)
    {
        /* In which format is the clipboard data? */
        switch (clipRealFormatForX11Format(pReq->idxFmtX11))
        {
    ...
    else if (pReq->uFmtVPox == VPOX_SHCL_FMT_HTML)
    {
        /* In which format is the clipboard data? */
        switch (clipRealFormatForX11Format(pReq->idxFmtX11))
        {
            case SHCLX11FMT_HTML:
            {
                ...
                if (   cbSrc >= sizeof(RTUTF16)
                    && *(PRTUTF16)pvSrc == VPOX_SHCL_UTF16LEMARKER)
                {
...
# ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
    else if (pReq->uFmtVPox == VPOX_SHCL_FMT_URI_LIST)
    {
        /* In which format is the clipboard data? */
        switch (clipRealFormatForX11Format(pReq->idxFmtX11))
        {
...
# endif /* VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS */
    ...
    if (RT_FAILURE(rc))
        LogRel(("Shared Clipboard: Converting X11 format '%s' (idxFmtX11=%u) to VPox format %#x failed, rc=%Rrc\n",
                g_aFormats[pReq->idxFmtX11].pcszAtom, pReq->idxFmtX11, pReq->uFmtVPox, rc));
...
static void ShClX11ReadDataFromX11Worker(void *pvUserData, void * /* interval */)
{
    ...
    int rc = VERR_NO_DATA; /* VPox thinks we have data and we don't. */
...
#endif
    if (pReq->uFmtVPox & VPOX_SHCL_FMT_UNICODETEXT)
    {
...
/**
 * Called when VPox wants to read the X11 clipboard.
 *
 * @returns VPox status code.
 ...
 * @param   pCtx                Context data for the clipboard backend.
 * @param   Format              The format that the VPox would like to receive the data in.
