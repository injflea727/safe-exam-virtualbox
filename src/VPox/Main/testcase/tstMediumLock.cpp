...
int main(int argc, char *argv[])
{
    ...
    if (!RTTestSubErrorCount(hTest))
    {
        ...
        RTTestSub(hTest, "Getting VirtualPox reference");
        TEST_COM_SUCCESS(hTest, pVirtualPoxClient.createInprocObject(CLSID_VirtualPoxClient), "vpoxclient reference");
        TEST_COM_SUCCESS(hTest, pVirtualPoxClient->COMGETTER(VirtualPox)(pVirtualPox.asOutParam()), "vpox reference");
    }

    if (!RTTestSubErrorCount(hTest))
    {
        RTTestSub(hTest, "Creating temp hard disk medium");
        TEST_COM_SUCCESS(hTest, pVirtualPox->CreateMedium(Bstr("VDI").raw(), Bstr(szPathTemp).raw(), AccessMode_ReadWrite, DeviceType_HardDisk, pMedium.asOutParam()), "create medium");
        if (!pMedium.isNull())
        {
