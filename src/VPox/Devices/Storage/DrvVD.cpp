/* $Id: DrvVD.cpp $ */
/** @file
 * DrvVD - Generic VPox disk media driver.
 */
...
#include <VPox/vmm/pdmblkcache.h>
 ...
 * "VPoxInternal/Devices/piix3ide/0/LUN#<x>/Config/FlushInterval". @verbatim<x>@endverbatim
...
#define VPOX_PERIODIC_FLUSH

/** @def VPOX_IGNORE_FLUSH
 ...
 * "VPoxInternal/Devices/piix3ide/0/LUN#<x>/Config/IgnoreFlush". @verbatim<x>@endverbatim
 * must be replaced with the correct LUN number of the disk that should
 * ignore flush requests. The value of the key is a boolean. The default
 * is to ignore flushes, i.e. true. */
#define VPOX_IGNORE_FLUSH
...
/** Converts a pointer to VPOXDISK::IMedia to a PVPOXDISK. */
#define PDMIMEDIA_2_VPOXDISK(pInterface) \
    ( (PVPOXDISK)((uintptr_t)pInterface - RT_UOFFSETOF(VPOXDISK, IMedia)) )
...
typedef struct VPOXDISK *PVPOXDISK;

/**
 * VPox disk container, image information, private part.
 */

typedef struct VPOXIMAGE
{
    /** Pointer to next image. */
    struct VPOXIMAGE   *pNext;
...
/**
 * VPox disk container media main structure, private part.
 *
 ...
 */
typedef struct VPOXDISK
{
    /** The VPox disk container. */
    PVDISK                   pDisk;
    /** The media interface. */
    PDMIMEDIA                IMedia;
    /** Media port. */
...
DECLINLINE(void) drvvdMediaExIoReqBufFree(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq);
static int drvvdMediaExIoReqCompleteWorker(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq, int rcReq, bool fUpNotify);
static int drvvdMediaExIoReqReadWriteProcess(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq, bool fUpNotify);

/**
 * Internal: allocate new image descriptor and put it in the list
 */
static PVPOXIMAGE drvvdNewImage(PVPOXDISK pThis)
{
    AssertPtr(pThis);
    PVPOXIMAGE pImage = (PVPOXIMAGE)RTMemAllocZ(sizeof(VPOXIMAGE));
    if (pImage)
    {
        pImage->pVDIfsImage = NULL;
        PVPOXIMAGE *pp = &pThis->pImages;
        while (*pp != NULL)
            pp = &(*pp)->pNext;
        *pp = pImage;
        pImage->pNext = NULL;
 ...
 */
static void drvvdFreeImages(PVPOXDISK pThis)
{
    while (pThis->pImages != NULL)
    {
        PVPOXIMAGE p = pThis->pImages;
 ...
 */
static int drvvdSetReadonly(PVPOXDISK pThis)
{
    int rc = VINF_SUCCESS;
    if (   pThis->pDisk
 ...
 * @param   pThis               The driver instance data.
 */
static int drvvdSetWritable(PVPOXDISK pThis)
{
    int rc = VINF_SUCCESS;
                                             ...
                                             const char *pszFormat, va_list va)
{
    ...
    PVPOXDISK pThis = PDMINS_2_DATA(pDrvIns, PVPOXDISK);
...
static DECLCALLBACK(int) drvvdAsyncIOOpen(void *pvUser, const char *pszLocation,
                                          ...
                                          void **ppStorage)
{
    PVPOXDISK pThis = (PVPOXDISK)pvUser;
...
#endif /* VPOX_WITH_PDM_ASYNC_COMPLETION */
 ...
 */
static int drvvdKeyCheckPrereqs(PVPOXDISK pThis, bool fSetError)
{
    if (   pThis->pCfgCrypto
        && !pThis->pIfSecKey)
    {
                                   ...
                                   uint64_t off, void *pvBuf, size_t cbRead)
{
    ...
    PVPOXDISK pThis = PDMIMEDIA_2_VPOXDISK(pInterface);
...
{
    RT_NOREF(fEject);
    PVPOXDISK pThis = RT_FROM_MEMBER(pInterface, VPOXDISK, IMount);
...
{
    PVPOXDISK pThis = (PVPOXDISK)pvUser1;
 ...
 * @param   pThis     VPox disk container instance data.
 * @param   rc        Status code to check.
 */
bool drvvdMediaExIoReqIsRedoSetWarning(PVPOXDISK pThis, int rc)
{
 ...
 */
DECLINLINE(int) drvvdMediaExIoReqBufSync(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq, bool fToIoBuf)
{
    int rc = VINF_SUCCESS;
 ...
 */
static int drvvdMediaExIoReqInsert(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq)
{
    int rc = VINF_SUCCESS;
 ...
 * @param   pIoReq    I/O request to insert.
 */
static int drvvdMediaExIoReqRemove(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq)
{
    int rc = VINF_SUCCESS;
    unsigned idxBin = drvvdMediaExIoReqIdHash(pIoReq->uIoReqId);
 ...
 */
static void drvvdMediaExIoReqRetire(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq, int rcReq, bool fUpNotify)
{
 ...
 */
DECLINLINE(int) drvvdMediaExIoReqBufAlloc(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq, size_t cb)
{
 ...
 */
static int drvvdMediaExIoReqReadWrapper(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq, size_t cbReqIo, size_t *pcbReqIo)
{
    int rc = VINF_SUCCESS;
 ...
 * @param   pcbReqIo  Where to store the amount of transferred data.
 */
static int drvvdMediaExIoReqWriteWrapper(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq, size_t cbReqIo, size_t *pcbReqIo)
{
    int rc = VINF_SUCCESS;
 ...
 * @param   pIoReq    I/O request to process.
 */
static int drvvdMediaExIoReqFlushWrapper(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq)
{
    int rc = VINF_SUCCESS;
 ...
 * @param   pIoReq    I/O request to process.
 */
static int drvvdMediaExIoReqDiscardWrapper(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq)
{
    int rc = VINF_SUCCESS;
 ...
 * @param   pThis     VPox disk container instance data.
 */
static void drvvdMediaExIoReqProcessWaiting(PVPOXDISK pThis)
{
 ...
 */
static void drvvdMediaExIoReqLogRel(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq)
{
    uint64_t offStart = 0;
    size_t cbReq = 0;
    size_t cbLeft = 0;
 ...
 * @param   pThis     VPox disk container instance data.
 */
DECLINLINE(bool) drvvdMediaExIoReqIsVmRunning(PVPOXDISK pThis)
{
 ...
 */
static bool drvvdMediaExIoReqCancel(PVPOXDISK pThis, PPDMMEDIAEXIOREQINT pIoReq)
{
    bool fXchg = false;
...
{
    PVPOXDISK pThis = RT_FROM_MEMBER(pInterface, VPOXDISK, IMediaEx);
 ...
 */
static int drvvdSetupFilters(PVPOXDISK pThis, PCFGMNODE pCfg)
{
    int rc = VINF_SUCCESS;
 ...
 */
static int drvvdStatsRegister(PVPOXDISK pThis)
{
 ...
 * @param   pThis      The media driver instance.
 */
static void drvvdStatsDeregister(PVPOXDISK pThis)
{
    PPDMDRVINS pDrvIns = pThis->pDrvIns;
 ...
 *      Construct a VPox disk media driver instance.}
 */
static DECLCALLBACK(int) drvvdConstruct(PPDMDRVINS pDrvIns, PCFGMNODE pCfg, uint32_t fFlags)
{
...
#if defined(VPOX_PERIODIC_FLUSH) || defined(VPOX_IGNORE_FLUSH)
                                          "FlushInterval\0IgnoreFlush\0IgnoreFlushAsync\0"
#endif /* !(VPOX_PERIODIC_FLUSH || VPOX_IGNORE_FLUSH) */
                                           );
        }
        else
        {
        ...
        && RT_SUCCESS(rc))
    {
        ...
        while (pCurNode && RT_SUCCESS(rc))
        {
            ...
            PVPOXIMAGE pImage = drvvdNewImage(pThis);
            if (!pImage)
            {
                rc = VERR_NO_MEMORY;
                break;
...
#else /* !VPOX_WITH_PDM_ASYNC_COMPLETION */
                rc = PDMDrvHlpVMSetError(pDrvIns, VERR_PDM_DRVINS_UNKNOWN_CFG_VALUES,
...
/**
 * VPox disk container media driver registration record.
 */
const PDMDRVREG g_DrvVD =
{
    ...
    "",
    /* pszDescription */
    "Generic VPox disk media driver.",
    /* fFlags */
