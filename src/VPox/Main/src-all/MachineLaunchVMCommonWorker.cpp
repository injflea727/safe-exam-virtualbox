 ...
 * The function is a part of both VPoxSVC and VPoxSDS, so any calls to IVirtualPox
                                ...
                                RTPROCESS     &aPid)
{
    ...
    {
#ifdef IN_VPOXSVC
        /* VPoxSVC: clone the current environment */
        vrc = RTEnvClone(&hEnv, RTENV_DEFAULT);
#else
        /* VPoxSDS: Create a change record environment since RTProcCreateEx has to
                    build the final environment from the profile of the VPoxSDS caller. */
...
#  define OSX_APP_NAME           "VirtualPoxVM"
#  define OSX_APP_PATH_FMT       "/Resources/%s.app/Contents/MacOS/VirtualPoxVM"
#  define OSX_APP_PATH_WITH_NAME "/Resources/VirtualPoxVM.app/Contents/MacOS/VirtualPoxVM"
...
# else
        static const char s_szVirtualPox_exe[] = "VirtualPoxVM" HOSTSUFF_EXE;
        vrc = RTStrCopy(pszNamePart, cbBufLeft, s_szVirtualPox_exe);
        AssertRCReturn(vrc, vrc);
# endif
    ...
    {
        static const char s_szVPoxSDL_exe[] = "VPoxSDL" HOSTSUFF_EXE;
        vrc = RTStrCopy(pszNamePart, cbBufLeft, s_szVPoxSDL_exe);
        AssertRCReturn(vrc, vrc);
...
#else /* !VPOX_WITH_VPOXSDL */
    if (0)
        ;
#endif /* !VPOX_WITH_VPOXSDL */
    ...
    {
        /* On pre-4.0 the "headless" type was used for passing "--vrdp off" to VPoxHeadless to let it work in OSE,
         * which did not contain VRDP server. In VPox 4.0 the remote desktop server (VRDE) is optional,
         * and a VM works even if the server has not been installed.
         * So in 4.0 the "headless" behavior remains the same for default VPox installations.
         * Only if a VRDE has been installed and the VM enables it, the "headless" will work
         * differently in 4.0 and 3.x.
         */
        static const char s_szVPoxHeadless_exe[] = "VPoxHeadless" HOSTSUFF_EXE;
        vrc = RTStrCopy(pszNamePart, cbBufLeft, s_szVPoxHeadless_exe);
        AssertRCReturn(vrc, vrc);
...
#else /* !VPOX_WITH_HEADLESS */
    if (0)
        ;
#endif /* !VPOX_WITH_HEADLESS */
    else
