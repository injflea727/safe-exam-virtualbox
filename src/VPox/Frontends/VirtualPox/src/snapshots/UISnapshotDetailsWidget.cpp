...
/** @file
 * VPox Qt GUI - UISnapshotDetailsWidget class implementation.
 */
...
#ifdef VPOX_WITH_AUDIO_INOUT_INFO
                /* Output: */
                ++iRowCount;
...
#endif /* VPOX_WITH_AUDIO_INOUT_INFO */
            }
...
QString UISnapshotDetailsWidget::bootOrderReport(const CMachine &comMachine)
{
    ...
    for (ulong i = 1; i <= uiCommon().virtualPox().GetSystemProperties().GetMaxBootPosition(); ++i)
    {
...
QStringList UISnapshotDetailsWidget::networkReport(CMachine comMachine)
{
    ...
    const ulong iCount = uiCommon().virtualPox().GetSystemProperties().GetMaxNetworkAdapters(comMachine.GetChipsetType());
...
QStringList UISnapshotDetailsWidget::serialReport(CMachine comMachine)
{
    ...
    const ulong iCount = uiCommon().virtualPox().GetSystemProperties().GetSerialPortCount();
    for (ulong iSlot = 0; iSlot < iCount; ++iSlot)
    {
