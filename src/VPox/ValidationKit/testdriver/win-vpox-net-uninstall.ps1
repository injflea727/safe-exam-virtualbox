# $Id: win-vpox-net-uninstall.ps1 $
## @file
# VirtualPox Validation Kit - network cleanup script (powershell).
#
...
if ($ghostcon -eq $null) {
   ...
   $ghostcon.GetEnumerator() | ForEach-Object {
      ...
      } elseif (!($prop.PnPInstanceId.ToString() -match "SUN_VPOXNETFLTMP")) {
         Write-Host "WARNING! PnPInstanceId (" $prop.PnPInstanceId.ToString() ") does not match ROOT\SUN_VPOXNETFLTMP for" $_.PSChildName
      }
