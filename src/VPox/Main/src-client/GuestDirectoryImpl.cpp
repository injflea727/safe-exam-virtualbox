...
/** @file
 * VirtualPox Main - Guest directory handling.
 */
...
int GuestDirectory::init(Console *pConsole, GuestSession *pSession, ULONG aObjectID, const GuestDirectoryOpenInfo &openInfo)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        procInfo.mExecutable= Utf8Str(VPOXSERVICE_TOOL_LS);
...
int GuestDirectory::i_callbackDispatcher(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb)
{
 ...
 *
 * @return VPox status code. Will return VERR_NO_MORE_FILES if no more entries are available.
    ...
    else
    {
        switch (vrc)
        {
            ...
            case VERR_GSTCTL_PROCESS_EXIT_CODE:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Reading guest directory \"%s\" failed: %Rrc"),
                                  mData.mOpenInfo.mPath.c_str(), mData.mProcessTool.getRc());
                break;

            case VERR_PATH_NOT_FOUND:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Reading guest directory \"%s\" failed: Path not found"),
                                  mData.mOpenInfo.mPath.c_str());
                break;

            case VERR_NO_MORE_FILES:
                /* See SDK reference. */
                hr = setErrorBoth(VPOX_E_OBJECT_NOT_FOUND, vrc, tr("Reading guest directory \"%s\" failed: No more entries"),
                                  mData.mOpenInfo.mPath.c_str());
                break;

            default:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Reading guest directory \"%s\" returned error: %Rrc\n"),
