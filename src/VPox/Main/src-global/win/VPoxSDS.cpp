/* $Id: VPoxSDS.cpp $ */
/** @file
 * VPoxSDS - COM global service main entry (System Directory Service)
 */
...
/** @page pg_VPoxSDS    VPoxSDS - Per user CLSID_VirtualPox coordinater
 *
 * VPoxSDS is short for VirtualPox System Directory Service (SDS).  Its purpose
 * is to make sure there is only one CLSID_VirtualPox object running for each
 * user using VirtualPox on a Windows host system.
 *
 *
 * @section sec_vpoxsds_backgroud   Background
 *
 ...
 * level too.
 *
 * VPoxSVC is a service affected by this issue.  Using VirtualPox across logins
 * or between user elevation levels was impossible to do simultaneously.  This
 * was confusing and illogical to the user.
 *
 *
 * @section sec_vpoxsds_how         How it works
 *
 * VPoxSDS assists in working around this problem by tracking which VPoxSVC
 * server is currently providing CLSID_VirtualPox for a user.  Each VPoxSVC
 * instance will register itself with VPoxSDS when the CLSID_VirtualPox object
 * is requested via their class factory.  The first VPoxSVC registering for a
 * given user will be allowed to instantate CLSID_VirtualPox.  We will call this
 * the chosen one.  Subsequent VPoxSVC instance for the given user, regardless
 * of elevation, session, windows station, or whatever else, will be told to use
 * the instance from the first VPoxSVC.
 *
 * The registration call passes along an IVPoxSVCRegistration interface from
 * VPoxSVC.  VPoxSDS keeps this around for the chosen one only.  When other
 * VPoxSVC instances for the same user tries to register, VPoxSDS will ask the
 * choosen one for its CLSID_VirtualPox object and return it to the new
 * registrant.
 *
 * The chosen one will deregister with VPoxSDS before it terminates.  Should it
 * terminate abnormally, VPoxSDS will (probably) notice the next time it tries
 * to request CLSID_VirtualPox from it and replace it as the chosen one with the
 * new registrant.
 *
 *
 * @section sec_vpoxsds_locking     Locking
 *
 * VPoxSDS stores data in a map indexed by the stringified secure identifier
 ...
 * preventing a user from blocking all others from using VirtualPox by
 * suspending or debugging their chosen VPoxSVC process.
 *
 */
...
protected:
    ...
    virtual HRESULT preMessageLoop(int /*nShowCmd*/) throw()
    {
        ...
        {
            LogFunc(("VPoxSDS Service started/resumed without delay\n"));
...
#ifdef WITH_WATCHER
/**
 * Go-between for CComServiceModule and VirtualPoxSDS.
 */
...
/**
 * Main function for the VPoxSDS process.
 *
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    /*
     * Initialize the VPox runtime without loading the support driver.
     */
    ...
    while ((vrc = RTGetOpt(&GetOptState, &ValueUnion)))
    {
        switch (vrc)
        {
            ...
            case 'h':
            {
                ...
                MessageBoxW(NULL, s_wszHelpText, L"VPoxSDS - Usage", MB_OK);
                return 0;
            }
            ...
            case 'V':
            {
                ...
                MessageBoxW(NULL, pwszText, L"VPoxSDS - Version", MB_OK);
            ...
            default:
            {
                ...
                MessageBoxW(NULL, pwszText, L"VPoxSDS - Syntax error", MB_OK | MB_ICONERROR);
    ...
    /*
     * Default log location is %ProgramData%\VirtualPox\VPoxSDS.log, falling back
     * on %_CWD%\VPoxSDS.log (where _CWD typicaly is 'C:\Windows\System32').
     *
     * We change the current directory to %ProgramData%\VirtualPox\ if possible.
     *
     * We only create the log file when running VPoxSDS normally, but not
    ...
    if (fRun)
    {
        ...
        if (!pszLogFile || !*pszLogFile)
        {
            ...
            if (SHGetSpecialFolderPathW(NULL, wszAppData, CSIDL_COMMON_APPDATA, TRUE /*fCreate*/))
            {
                ...
                vrc = RTEnvGetUtf8("ProgramData", szLogFile, sizeof(szLogFile) - sizeof("VPoxSDS.log"), NULL);
            if (RT_SUCCESS(vrc))
            {
                vrc = RTPathAppend(szLogFile, sizeof(szLogFile), "VirtualPox\\");
                if (RT_SUCCESS(vrc))
                {
                    /* Make sure it exists. */
            ...
            if (RT_FAILURE(vrc))     /* ignore any failure above */
                szLogFile[0] = '\0';
            vrc = RTStrCat(szLogFile, sizeof(szLogFile), "VPoxSDS.log");
            if (RT_FAILURE(vrc))
        ...
        vrc = com::VPoxLogRelCreate("COM Service", pszLogFile,
                                    RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                                    VPOXSDS_LOG_DEFAULT, "VPOXSDS_RELEASE_LOG",
    ...
    if (SUCCEEDED(hrcExit))
    {
        HRESULT hrcSec = CoInitializeSecurity(NULL,
                                              ...
                                              NULL);
        LogRelFunc(("VPoxSDS: InitializeSecurity: %x\n", hrcSec));
        ...
        if (pServiceModule)
        {
            BEGIN_OBJECT_MAP(s_aObjectMap)
                OBJECT_ENTRY(CLSID_VirtualPoxSDS, VirtualPoxSDS)
            END_OBJECT_MAP()
            hrcExit = pServiceModule->init(s_aObjectMap, hInstance, &LIBID_VirtualPox,
                                           L"VPoxSDS",
                                           L"VirtualPox system service",
                                           L"Used as a COM server for VirtualPox API.");
            ...
            if (SUCCEEDED(hrcExit))
            {
                if (!fRun)
                {
                     ...
                     */
                    /// @todo The VPoxProxyStub should do all work for COM registration
                    if (fUnregister)
                ...
                else
                {
                    ...
                    LogRelFunc(("VPoxSDS: Calling _ServiceModule.RevokeClassObjects()...\n"));
                    CComServiceModule::s_pInstance = NULL;
                    pServiceModule->RevokeClassObjects();
                }

                LogRelFunc(("VPoxSDS: Calling _ServiceModule.Term()...\n"));
                pServiceModule->Term();
            }
            else
                LogRelFunc(("VPoxSDS: new CComServiceModule::Init failed: %Rhrc\n", hrcExit));

            LogRelFunc(("VPoxSDS: deleting pServiceModule (%p)\n", pServiceModule));
            delete pServiceModule;
            pServiceModule = NULL;
        }
        else
            LogRelFunc(("VPoxSDS: new CComServiceModule() failed\n"));

        LogRelFunc(("VPoxSDS: Calling com::Shutdown\n"));
        com::Shutdown();
    }
    else
        LogRelFunc(("VPoxSDS: COM initialization failed: %Rrc\n", hrcExit));

    LogRelFunc(("VPoxSDS: COM service process ends: hrcExit=%Rhrc (%#x)\n", hrcExit, hrcExit));
    return (int)hrcExit;
}
