...
/** The saved state version used by VirtualPox before the diagnostic
 * memory access was implemented. */
#define LSILOGIC_SAVED_STATE_VERSION_PRE_DIAG_MEM   4
/** The saved state version used by VirtualPox before the doorbell status flag
 * was changed from bool to a 32bit enum. */
#define LSILOGIC_SAVED_STATE_VERSION_BOOL_DOORBELL  3
/** The saved state version used by VirtualPox before SAS support was added. */
#define LSILOGIC_SAVED_STATE_VERSION_PRE_SAS        2
/** The saved state version used by VirtualPox 3.0 and earlier.  It does not
 * include the device config part. */
#define LSILOGIC_SAVED_STATE_VERSION_VPOX_30        1
 ...
 */
static VPOXSTRICTRC lsilogicRegisterWrite(PPDMDEVINS pDevIns, PLSILOGICSCSI pThis, uint32_t offReg, uint32_t u32)
{
 ...
 */
static VPOXSTRICTRC lsilogicRegisterRead(PPDMDEVINS pDevIns, PLSILOGICSCSI pThis, uint32_t offReg, uint32_t *pu32)
{
    int rc = VINF_SUCCESS;
    uint32_t u32 = 0;
    Assert(!(offReg & 3));
...
lsilogicIOPortRead(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t *pu32, unsigned cb)
{
    ...
    VPOXSTRICTRC rcStrict = lsilogicRegisterRead(pDevIns, pThis, offPort & ~(uint32_t)3, pu32);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) lsilogicMMIOWrite(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void const *pv, unsigned cb)
{
    PLSILOGICSCSI   pThis  = PDMDEVINS_2_DATA(pDevIns, PLSILOGICSCSI);
    uint32_t        u32;
    RT_NOREF_PV(pvUser);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) lsilogicMMIORead(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void *pv, unsigned cb)
{
    PLSILOGICSCSI   pThis  = PDMDEVINS_2_DATA(pDevIns, PLSILOGICSCSI);
 ...
 * @callback_method_impl{FNIOMMMIONEWREAD}
 */
static DECLCALLBACK(VPOXSTRICTRC) lsilogicDiagnosticRead(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void *pv, unsigned cb)
{
    RT_NOREF(pDevIns, pvUser, off, pv, cb);
...
static void lsilogicR3InitializeConfigurationPages(PPDMDEVINS pDevIns, PLSILOGICSCSI pThis, PLSILOGICSCSICC pThisCC)
{
    ...
    MPT_CONFIG_PAGE_HEADER_INIT_MANUFACTURING(&pPages->ManufacturingPage0,
                                              ...
                                              MPT_CONFIGURATION_PAGE_ATTRIBUTE_PERSISTENT_READONLY);
    strncpy((char *)pPages->ManufacturingPage0.u.fields.abChipName,          "VPox MPT Fusion", 16);
    strncpy((char *)pPages->ManufacturingPage0.u.fields.abChipRevision,      "1.0", 8);
    strncpy((char *)pPages->ManufacturingPage0.u.fields.abBoardName,         "VPox MPT Fusion", 16);
...
static int lsilogicR3PrepareBiosScsiRequest(PLSILOGICSCSI pThis, PLSILOGICSCSICC pThisCC)
{
    ...
    rc = vpoxscsiSetupRequest(&pThisCC->VPoxSCSI, &uLun, &pbCdb, &cbCdb, &cbBuf, &uTargetDevice);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) lsilogicR3IsaIOPortWriteStr(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort,
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) lsilogicR3IsaIOPortReadStr(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort,
                                                             uint8_t *pbDst, uint32_t *pcTransfers, unsigned cb)
{
    ...
    int rc = vpoxscsiReadString(pDevIns, &pThisCC->VPoxSCSI, offPort, pbDst, pcTransfers, cb);
    AssertMsg(rc == VINF_SUCCESS, ("Unexpected BIOS register read status: %Rrc\n", rc));
    return rc;
}
...
static DECLCALLBACK(int) lsilogicR3SaveExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM)
{
            ...
            if (cReqsRedo)
            {
                ...
                for (;;)
                {
                    ...
                    else
                    {
                        ...
                        vpoxscsiSetRequestRedo(&pThisCC->VPoxSCSI);
                    }
    ...
    vpoxscsiR3SaveExec(pHlp, &pThisCC->VPoxSCSI, pSSM);
...
static DECLCALLBACK(int) lsilogicR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (    uVersion != LSILOGIC_SAVED_STATE_VERSION
        ...
        &&  uVersion != LSILOGIC_SAVED_STATE_VERSION_VPOX_30)
    ...
    if (uVersion > LSILOGIC_SAVED_STATE_VERSION_VPOX_30)
    {
        for (unsigned i = 0; i < pThis->cDeviceStates; i++)
        {
            bool fPresent;
    ...
    rc = vpoxscsiR3LoadExec(pHlp, &pThisCC->VPoxSCSI, pSSM);
    if (RT_FAILURE(rc))
    {
