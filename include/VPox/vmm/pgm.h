...
#ifndef VPOX_INCLUDED_vmm_pgm_h
#define VPOX_INCLUDED_vmm_pgm_h
...
#include <VPox/vmm/gmm.h>               /* for PGMMREGISTERSHAREDMODULEREQ */
#include <iprt/x86.h>
 ...
 *
 * @returns Strict VPox status code (appropriate for ring-0 and raw-mode).
 * @param   pVM         The cross context VM structure.
 ...
 * @thread  EMT(pVCpu)
 */
typedef DECLCALLBACK(VPOXSTRICTRC) FNPGMRZPHYSPFHANDLER(PVMCC pVM, PVMCPUCC pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
 ...
 *
 * @returns Strict VPox status code in ring-0 and raw-mode context, in ring-3
 ...
 * @thread  EMT(pVCpu)
 */
typedef DECLCALLBACK(VPOXSTRICTRC) FNPGMPHYSHANDLER(PVMCC pVM, PVMCPUCC pVCpu, RTGCPHYS GCPhys, void *pvPhys, void *pvBuf, size_t cbBuf,
...
VMMDECL(VPOXSTRICTRC)   PGMInterpretInstruction(PVMCC pVM, PVMCPUCC pVCpu, PCPUMCTXCORE pRegFrame, RTGCPTR pvFault);
...
#if defined(IN_RC) || defined(VPOX_WITH_2X_4GB_ADDR_SPACE)
...
VMMR0DECL(VPOXSTRICTRC) PGMR0Trap0eHandlerNPMisconfig(PGVM pGVM, PGVMCPU pGVCpu, PGMMODE enmShwPagingMode,
...
# if defined(VPOX_WITH_RAW_MODE) || HC_ARCH_BITS == 32 /* (latter for 64-bit guests on 32-bit hosts) */
...
/** @name Page sharing
 * @{ */
VMMR3DECL(int)     PGMR3SharedModuleRegister(PVM pVM, VPOXOSFAMILY enmGuestOS, char *pszModuleName, char *pszVersion,
