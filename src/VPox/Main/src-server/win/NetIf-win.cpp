...
static int collectNetIfInfo(Bstr &strName, Guid &guid, PNETIFINFO pInfo, int iDefault)
{
    ...
    if (dwRc == NO_ERROR)
    {
        ...
        HRESULT hr = VPoxNetCfgWinGetAdapterSettings((const GUID *)guid.raw(), &Settings);
        if (hr == S_OK)
        {
            if (Settings.ip)
            {
            ...
            while (!endLoop)
            {
                ...
                switch (reply)
                {
                    case SVCHlpMsg::CreateHostOnlyNetworkInterface_OK:
                    {
                        ...
                        if (SUCCEEDED(rc))
                        {
                            rc = d->iface->i_setVirtualPox(d->ptrVPox);
                            if (SUCCEEDED(rc))
                            {
                                      ...
                                      SVCHlpMsg::Code aMsgCode)
{
    ...
    switch (aMsgCode)
    {
        case SVCHlpMsg::CreateHostOnlyNetworkInterface:
        {
            ...
            Bstr devId;
            hrc = VPoxNetCfgWinCreateHostOnlyNetworkInterface(NULL, false, Bstr(desiredName).raw(), guid.asOutParam(), devId.asOutParam(),
                                                              bstrErr.asOutParam());
#else /* !VPOXNETCFG_DELAYEDRENAME */
            hrc = VPoxNetCfgWinCreateHostOnlyNetworkInterface(NULL, false, Bstr(desiredName).raw(), guid.asOutParam(), name.asOutParam(),
                                                              bstrErr.asOutParam());
            ...
            {
                ULONG ip, mask;
                hrc = VPoxNetCfgWinGenHostOnlyNetworkNetworkIp(&ip, &mask);
                if (hrc == S_OK)
                {
                    ...
                    ip = ip | (1 << 24);
                    hrc = VPoxNetCfgWinEnableStaticIpConfig((const GUID*)guid.raw(), ip, mask);
                    if (hrc != S_OK)
                        LogRel(("VPoxNetCfgWinEnableStaticIpConfig failed (0x%x)\n", hrc));
                }
                else
                    LogRel(("VPoxNetCfgWinGenHostOnlyNetworkNetworkIp failed (0x%x)\n", hrc));
#ifdef VPOXNETCFG_DELAYEDRENAME
                hrc = VPoxNetCfgWinRenameHostOnlyConnection((const GUID*)guid.raw(), devId.raw(), name.asOutParam());
                if (hrc != S_OK)
                    LogRel(("VPoxNetCfgWinRenameHostOnlyConnection failed, error = 0x%x", hrc));
#endif /* VPOXNETCFG_DELAYEDRENAME */
        ...
        case SVCHlpMsg::RemoveHostOnlyNetworkInterface:
        {
            ...
            Utf8Str errMsg;
            hrc = VPoxNetCfgWinRemoveHostOnlyNetworkInterface((const GUID*)guid.raw(), bstrErr.asOutParam());
        ...
        case SVCHlpMsg::EnableDynamicIpConfig:
        {
            ...
            Utf8Str errMsg;
            hrc = VPoxNetCfgWinEnableDynamicIpConfig((const GUID *)guid.raw());
        ...
        case SVCHlpMsg::DhcpRediscover:
        {
            ...
            Utf8Str errMsg;
            hrc = VPoxNetCfgWinDhcpRediscover((const GUID *)guid.raw());
...
static int netIfListHostAdapters(INetCfg *pNc, std::list<ComObjPtr<HostNetworkInterface> > &list)
{
    ...
    /* VPoxNetAdp is available only when VPOX_WITH_NETFLT is enabled */
        ...
        while ((hr = pEnumComponent->Next(1, &pMpNcc, NULL)) == S_OK)
        {
            ...
            if (hr == S_OK)
            {
                if (uComponentStatus == 0)
                {
                    ...
                    {
                        LogFunc(("id = %ls\n", pId));
                        if (!_wcsnicmp(pId, L"sun_VPoxNetAdp", sizeof(L"sun_VPoxNetAdp")/2))
                        {
                            vpoxNetWinAddComponent(&list, pMpNcc, HostNetworkInterfaceType_HostOnly, -1);
                        }
                        CoTaskMemFree(pId);
                    }
                    else
...
int NetIfCreateHostOnlyNetworkInterface(VirtualPox *pVirtualPox,
                                        ...
                                        IN_BSTR aName)
{
    ...
    ComPtr<IHost> host;
    HRESULT rc = pVirtualPox->COMGETTER(Host)(host.asOutParam());
    if (SUCCEEDED(rc))
    {
        rc = progress->init(pVirtualPox, host,
        ...
        if (SUCCEEDED(rc))
        {
            ...
            d->name = aName;
            d->iface = iface;
            d->ptrVPox = pVirtualPox;

            rc = pVirtualPox->i_startSVCHelperClient(IsUACEnabled() == TRUE /* aPrivileged */,
...
int NetIfRemoveHostOnlyNetworkInterface(VirtualPox *pVirtualPox, const Guid &aId,
                                        IProgress **aProgress)
{
#ifndef VPOX_WITH_NETFLT
    return VERR_NOT_IMPLEMENTED;
...
int NetIfEnableStaticIpConfig(VirtualPox *pVPox, HostNetworkInterface * pIf, ULONG aOldIp, ULONG ip, ULONG mask)
{
    RT_NOREF(aOldIp);
#ifndef VPOX_WITH_NETFLT
    return VERR_NOT_IMPLEMENTED;
    ...
    if (SUCCEEDED(rc))
    {
//        ComPtr<VirtualPox> pVPox;
//        rc = pIf->getVirtualPox(pVPox.asOutParam());
//        if (SUCCEEDED(rc))
        {
            /* create a progress object */
            ComObjPtr<Progress> progress;
            progress.createObject();
//            ComPtr<IHost> host;
//            HRESULT rc = pVPox->COMGETTER(Host)(host.asOutParam());
//            if (SUCCEEDED(rc))
            {
                rc = progress->init(pVPox, (IHostNetworkInterface*)pIf,
...
//                    progress.queryInterfaceTo(aProgress);
                    ...
                    rc = pVPox->i_startSVCHelperClient(IsUACEnabled() == TRUE /* aPrivileged */,
                                                       netIfNetworkInterfaceHelperClient,
                                                       static_cast<void *>(d),
                                                       progress);
                    /* d is now owned by netIfNetworkInterfaceHelperClient(), no need to delete one here */
...
int NetIfEnableStaticIpConfigV6(VirtualPox *pVPox, HostNetworkInterface * pIf, const Utf8Str &aOldIPV6Address,
    ...
    else
    {
        while ((hr = pEnumComponent->Next(1, &pMiniport, NULL)) == S_OK)
        {
            ...
            if ((hr = pMiniport->GetDisplayName(&adapter.pName)) != S_OK)
                ...
                LogRelFunc(("failed to get device id (0x%x)\n", hr));
            else if (_wcsnicmp(adapter.pHwId, L"sun_VPoxNetAdp", sizeof(L"sun_VPoxNetAdp")/2))
...
static BOOL netIfIsWireless(INetCfgComponent *pAdapter)
{
    ...
    if (SUCCEEDED(hrc) && pwszBindName)
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            {
                DWORD rcWin = GetLastError();
#if 0 /* bird: Triggers on each VPoxSVC startup so, disabled.  Whoever want it, can enable using DEBUG_xxxx. */
...
static HRESULT netIfGetBoundAdapters(std::list<BoundAdapter> &boundAdapters)
{
    ...
    hr = VPoxNetCfgWinQueryINetCfg(&pNetCfg, FALSE,
                       VPOX_APP_NAME,
                       10000,
                       &lpszApp);
    Assert(hr == S_OK);
    ...
    if ((hr = pNetCfg->FindComponent(L"oracle_VPoxNetLwf", &pFilter)) != S_OK
        /* fall back to NDIS5 miniport lookup */
        && (hr = pNetCfg->FindComponent(L"sun_VPoxNetFlt", &pFilter)))
        LogRelFunc(("could not find either 'oracle_VPoxNetLwf' or 'sun_VPoxNetFlt' components (0x%x)\n", hr));
    else
    {
                                    ...
                                    else
                                    {
                                        ...
                                        if ((hr = pAdapter->GetId(&pwszHwId)) != S_OK)
                                            LogRelFunc(("%ls: failed to get hardware id (0x%x)\n",
                                                      pwszName, hr));
                                        else if (!_wcsnicmp(pwszHwId, L"sun_VPoxNetAdp", sizeof(L"sun_VPoxNetAdp")/2))
    ...
    VPoxNetCfgWinReleaseINetCfg(pNetCfg, FALSE);
        ...
        else
        {
            ...
            for (it = boundAdapters.begin(); it != boundAdapters.end(); ++it)
            {
                ...
                HostNetworkInterfaceType enmType =
                    _wcsnicmp((*it).pHwId, L"sun_VPoxNetAdp", sizeof(L"sun_VPoxNetAdp")/2) ?
