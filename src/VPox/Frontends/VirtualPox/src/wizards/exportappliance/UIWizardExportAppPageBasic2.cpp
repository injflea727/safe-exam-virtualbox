...
/** @file
 * VPox Qt GUI - UIWizardExportAppPageBasic2 class implementation.
 */
...
void UIWizardExportAppPage2::populateFormats()
{
    ...
    m_comCloudProviderManager = comVPox.GetCloudProviderManager();
...
void UIWizardExportAppPage2::populateFormProperties()
{
    ...
    if (m_comCloudProfile.isNotNull())
    {
        ...
        do
        {
            ...
            CVirtualPox comVPox = uiCommon().virtualPox();
            CMachine comMachine = comVPox.FindMachine(uMachineId.toString());
            if (!comVPox.isOk())
            {
                msgCenter().cannotFindMachineById(comVPox, uMachineId);
                break;
            }

            /* Create appliance: */
            CAppliance comAppliance = comVPox.CreateAppliance();
            if (!comVPox.isOk())
            {
                msgCenter().cannotCreateAppliance(comVPox);
                break;
            }
...
//                        m_pAccountPropertyTable->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Ignored);
    ...
    /* Setup connections: */
    if (gpManager)
        connect(gpManager, &UIVirtualPoxManager::sigCloudProfileManagerChange,
