...
/**
 * Initializes this object data with a stream block from VPOXSERVICE_TOOL_LS.
 *
...
/**
 * Parses stream block output data which came from the 'stat' (vpox_stat)
 * VPoxService toolbox command. The result will be stored in this object.
 *
...
/**
 * Parses stream block output data which came from the 'mktemp' (vpox_mktemp)
 * VPoxService toolbox command. The result will be stored in this object.
 *
 * @returns VPox status code.
 * @param   strmBlk             Stream block output data to parse.
 ...
 *
 * @return VPox status code. VERR_NOT_FOUND if key was not found.
 ...
 *
 * @return  VPox status code. Will return VERR_TOO_MUCH_DATA if the buffer's maximum (limit) has been reached.
...
GuestBase::GuestBase(void)
    : mConsole(NULL)
    , mNextContextID(RTRandU32() % VPOX_GUESTCTRL_MAX_CONTEXTS)
{
}
 ...
 *
 * @return VPox status code. VERR_NOT_FOUND if no handler has been found or VERR_NOT_SUPPORTED
 ...
 */
int GuestBase::dispatchGeneric(PVPOXGUESTCTRLHOSTCBCTX pCtxCb, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb)
{
...
int GuestBase::generateContextID(uint32_t uSessionID, uint32_t uObjectID, uint32_t *puContextID)
{
    ...
    if (   uSessionID >= VPOX_GUESTCTRL_MAX_SESSIONS
        || uObjectID  >= VPOX_GUESTCTRL_MAX_OBJECTS)
        return VERR_INVALID_PARAMETER;

    uint32_t uCount = ASMAtomicIncU32(&mNextContextID);
    uCount %= VPOX_GUESTCTRL_MAX_CONTEXTS;

    uint32_t uNewContextID = VPOX_GUESTCTRL_CONTEXTID_MAKE(uSessionID, uObjectID, uCount);
 ...
 */
int GuestBase::signalWaitEvent(VPoxEventType_T aType, IEvent *aEvent)
{
    int rc = RTCritSectEnter(&mWaitEventCritSect);
#ifdef DEBUG
    uint32_t cEvents = 0;
        ...
        if (itGroup != mWaitEventGroups.end())
        {
            ...
            while ((ItWaitEvt = itGroup->second.begin()) != itGroup->second.end())
            {
                LogFlowThisFunc(("Signalling event=%p, type=%ld (CID %#x: Session=%RU32, Object=%RU32, Count=%RU32) ...\n",
                                 ItWaitEvt->second, aType, ItWaitEvt->first, VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(ItWaitEvt->first),
                                 VPOX_GUESTCTRL_CONTEXTID_GET_OBJECT(ItWaitEvt->first), VPOX_GUESTCTRL_CONTEXTID_GET_COUNT(ItWaitEvt->first)));
 ...
 */
int GuestBase::signalWaitEventInternal(PVPOXGUESTCTRLHOSTCBCTX pCbCtx,
 ...
 * @param   pPayload            Additional wait event payload data set set on return. Optional.
 */
int GuestBase::signalWaitEventInternalEx(PVPOXGUESTCTRLHOSTCBCTX pCbCtx,
                                         int rc, int rcGuest,
 ...
 * GuestEventGroup to std::map<VPoxEventType_T, RTListAnchorClass>
 ...
 */
int GuestBase::waitForEvent(GuestWaitEvent *pWaitEvt, uint32_t msTimeout, VPoxEventType_T *pType, IEvent **ppEvent)
{
...
#define CASE_TOOL_ERROR(a_eType, a_strTool) \
    ...
    switch (guestErrorInfo.getType())
    {
        ...
        CASE_TOOL_ERROR(GuestErrorInfo::Type_ToolCat,    VPOXSERVICE_TOOL_CAT);
        CASE_TOOL_ERROR(GuestErrorInfo::Type_ToolLs,     VPOXSERVICE_TOOL_LS);
        CASE_TOOL_ERROR(GuestErrorInfo::Type_ToolMkDir,  VPOXSERVICE_TOOL_MKDIR);
        CASE_TOOL_ERROR(GuestErrorInfo::Type_ToolMkTemp, VPOXSERVICE_TOOL_MKTEMP);
        CASE_TOOL_ERROR(GuestErrorInfo::Type_ToolRm,     VPOXSERVICE_TOOL_RM);
        CASE_TOOL_ERROR(GuestErrorInfo::Type_ToolStat,   VPOXSERVICE_TOOL_STAT);
 ...
 */
int GuestObject::sendMessage(uint32_t uMessage, uint32_t cParms, PVPOXHGCMSVCPARM paParms)
{
#ifndef VPOX_GUESTCTRL_TEST_CASE
    ...
    if (pVMMDev)
    {
        ...
        paParms[0].u.uint64 = (uint64_t)paParms[0].u.uint32 | VPOX_GUESTCTRL_DST_SESSION;
