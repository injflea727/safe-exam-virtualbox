/* $Id: VPoxMpLogger.cpp $ */
/** @file
 * VPox WDDM Display logger implementation
 */
...
DECLCALLBACK(void) VPoxDispMpLoggerLog(const char *pszString)
{
    ...
    NTSTATUS Status = vpoxDispKmtOpenAdapter(&hAdapter);
    ...
    if (Status == 0)
    {
        ...
        PVPOXDISPIFESCAPE_DBGPRINT pCmd = (PVPOXDISPIFESCAPE_DBGPRINT)malloc(cbCmd);
        Assert(pCmd);
        if (pCmd)
        {
        ...
        Status = vpoxDispKmtCloseAdapter(hAdapter);
        Assert(Status == STATUS_SUCCESS);
    }
}
