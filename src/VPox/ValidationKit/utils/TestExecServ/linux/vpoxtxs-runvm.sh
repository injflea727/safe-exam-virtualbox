#!/bin/sh
## @file
# VirtualPox Test Execution Service Init Script.
#
...
SCRIPTNAME=vpoxtxs-runvm.sh

CDROM_PATH=/media/cdrom
SCRATCH_PATH=/tmp/vpoxtxs-scratch
SMOKEOUTPUT_PATH=/tmp/vpoxtxs-smoketestoutput
DEVKMSG_PATH=/dev/kmsg

PIDFILE="/var/run/vpoxtxs"

export TESTBOX_PATH_RESOURCES="/home/vpox/testrsrc"
...
start() {
    if ! test -f $PIDFILE; then
        ...
        fixAndTestBinary
        testRsrcPath
        $PYTHON_BINARY $SMOKETEST_SCRIPT -v -v -d --vpox-session-type gui --nic-attachment nat --quick all 1> "${SMOKEOUTPUT_PATH}" 2>&1
        RETVAL=$?
