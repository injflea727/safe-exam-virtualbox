...
/** @file
 * VPox Qt GUI - UIKeyboardHandler class implementation.
 */
        ...
        case kEventRawKeyUp:
        {
            ...
            if (uScanCode)
            {
                ...
                if (uEventKind != kEventRawKeyUp)
                    iFlags |= KeyPressed;
                if (uScanCode & VPOXKEY_EXTENDED)
                    iFlags |= KeyExtended;
                /** @todo KeyPause, KeyPrint. */
                uScanCode &= VPOXKEY_SCANCODE_MASK;
...
void UIKeyboardHandler::loadSettings()
{
    ...
    /* Fix for http://www.virtualpox.org/ticket/1296:
