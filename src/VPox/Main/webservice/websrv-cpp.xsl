...
<!--
    websrv-cpp.xsl:
        ...
        VirtualPox.xidl. This generated C++ code contains
 ...
 * Generator: src/VPox/Main/webservice/websrv-cpp.xsl
 */
 ...
 ****************************************************************************/
  ...
  <xsl:for-each select="//enum">
    ...
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat('vpox__', $enumname, ' ', $G_funcPrefixOutputEnumConverter, $enumname, '(', $enumname, '_T e)')" />
    <xsl:call-template name="emitNewline" />
    <xsl:text>{</xsl:text>
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat('    vpox__', $enumname, ' v;')" />
    <xsl:call-template name="emitNewline" />
    <xsl:call-template name="emitNewline" />
    ...
    <xsl:for-each select="const[not(@wsmap='suppress')]">
      ...
      <xsl:value-of select="concat('    v = vpox__', $enumname, '__')" />
    ...
    <xsl:call-template name="emitNewlineIndent8" />
    <xsl:value-of select="concat('    v = (vpox__', $enumname, ')0x7fffdead;')" />
    <xsl:call-template name="emitNewlineIndent8" />
    <xsl:text>break; </xsl:text>
    <xsl:call-template name="emitNewline" />
    <xsl:text>    }</xsl:text>
    ...
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat($enumname, '_T ', $G_funcPrefixInputEnumConverter, $enumname, '(vpox__', $enumname, ' v)')" />
    <xsl:call-template name="emitNewline" />
    <xsl:text>{</xsl:text>
    <xsl:call-template name="emitNewline" />
    ...
    <xsl:for-each select="const[not(@wsmap='suppress')]">
      <xsl:variable name="enumconst" select="@name" />
      <xsl:value-of select="concat('        case vpox__', $enumname, '__')" />
      <!-- escape all "_" in $enumconst -->
      <xsl:call-template name="escapeUnderscores">
        <xsl:with-param name="string" select="$enumconst" />
      </xsl:call-template>
 ...
 ****************************************************************************/
  ...
  <xsl:for-each select="//interface[@wsmap='struct']">
    ...
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat('vpox__', $structname, '* ', $G_funcPrefixOutputStructConverter, $structname, '(')" />
    <xsl:call-template name="emitNewline" />
    ...
    <xsl:value-of select="concat('    vpox__', $structname, ' *resp = NULL;')" />
    <xsl:call-template name="emitNewline" />

    <xsl:call-template name="emitPrologue"><xsl:with-param name="fSkipHRESULT" select="'1'"/></xsl:call-template>

    <xsl:value-of select="concat('        resp = soap_new_vpox__', $structname, '(soap, -1);')" />
    <xsl:call-template name="emitNewline" />
    <xsl:text>        if (!in)&#10;</xsl:text>
    <xsl:text>        {&#10;</xsl:text>
...
<xsl:template name="emitBeginOfFunctionHeader">
  ...
  <xsl:call-template name="emitNewline" />
  <xsl:value-of select="concat('int __vpox__', $ifname, '_USCORE', $method, '(')" />
  <xsl:call-template name="emitNewline" />
...
<xsl:template name="emitDocumentStyleArgStructs">
  ...
  <xsl:text>,</xsl:text>
  <xsl:call-template name="emitNewline" />
  <xsl:value-of select="concat('    _vpox__', $ifname, '_USCORE', $methodname, $G_requestMessageElementSuffix, ' *', $G_requestElementVarName)" />
  <xsl:if test="$fOutputs">
    <xsl:text>,</xsl:text>
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat('    _vpox__', $ifname, '_USCORE', $methodname, $G_responseMessageElementSuffix, ' *', $G_responseElementVarName)" />
...
<xsl:template name="emitObjForMethod">
  ...
  <xsl:param name="structprefix" />   <!-- with WSDL document style: req element prefix, like "vpox__IVirtualPox_USCOREcreateMachineRequestElement->" -->

  <xsl:choose>
    <xsl:when test="$wsmap='global'">
      <xsl:choose>
        <xsl:when test="$ifname='IVirtualPox'">
          <xsl:text>        // invoke method on global IVirtualPox instance</xsl:text>
          <xsl:call-template name="emitNewlineIndent8" />
          <xsl:text>ComPtr&lt;IVirtualPox&gt; pObj = G_pVirtualPox;</xsl:text>
          <xsl:call-template name="emitNewline" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="fatalError">
