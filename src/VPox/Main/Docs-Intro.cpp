 ...
 * attention to the "VirtualPox executables and components" chapter.  It lists
 * three processes, (1) VPoxSVC, (2) VirtualPox in manager mode and (3)
 * VirtualPox in VM mode.  This will be referred to as (1) server, (2) client
 * and (3) VM process, respectively.
 *
 *
 * @section sec_main_walk_thru_suspend  IConsole::Pause Walkthru
 *
 * The instigator can be a client (VirtualPox in manager mode, VPoxManage
