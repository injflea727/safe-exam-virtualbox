...
#include <VPox/hgcmsvc.h> /* For PVPOXHGCMSVCPARM. */
...
struct GuestDnDTransferRecvData : public GuestDnDTransferData
{
    ...
    /** The "VirtualPox Dropped Files" directory on the host we're going
...
public:
     ...
     */
    PVPOXHGCMSVCPARM getNextParam(void)
    {
        if (cParms >= cParmsAlloc)
        {
            if (!paParms)
                paParms = (PVPOXHGCMSVCPARM)RTMemAlloc(4 * sizeof(VPOXHGCMSVCPARM));
            else
                paParms = (PVPOXHGCMSVCPARM)RTMemRealloc(paParms, (cParmsAlloc + 4) * sizeof(VPOXHGCMSVCPARM));
            if (!paParms)
                throw VERR_NO_MEMORY;
            RT_BZERO(&paParms[cParmsAlloc], 4 * sizeof(VPOXHGCMSVCPARM));
            cParmsAlloc += 4;
        }
     ...
     */
    PVPOXHGCMSVCPARM getParms(void) const { return paParms; }
    ...
    void reset(void)
    {
        if (paParms)
        {
            ...
            {
                if (   paParms[i].type == VPOX_HGCM_SVC_PARM_PTR
    ...
    {
        PVPOXHGCMSVCPARM pParm = getNextParam();
        if (!pParm)
            return VERR_NO_MEMORY;
...
public:
    ...
    void setActionsAllowed(VPOXDNDACTIONLIST a) { m_dndLstActionsAllowed = a; }
    VPOXDNDACTIONLIST getActionsAllowed(void) const { return m_dndLstActionsAllowed; }

    void setActionDefault(VPOXDNDACTION a) { m_dndActionDefault = a; }
    VPOXDNDACTION getActionDefault(void) const { return m_dndActionDefault; }
...
public:
     ...
     *  successful drop. */
    VPOXDNDACTION         m_dndActionDefault;
    /** Actions supported by the guest in case of a successful drop. */
    VPOXDNDACTIONLIST     m_dndLstActionsAllowed;
...
public:
    ...
    int               hostCall(uint32_t u32Function, uint32_t cParms, PVPOXHGCMSVCPARM paParms) const;
...
public:
    ...
    static DnDAction_T              toMainAction(VPOXDNDACTION dndAction);
    static std::vector<DnDAction_T> toMainActions(VPOXDNDACTIONLIST dndActionList);
    static VPOXDNDACTION            toHGCMAction(DnDAction_T enmAction);
    static void                     toHGCMActions(DnDAction_T enmDefAction, VPOXDNDACTION *pDefAction, const std::vector<DnDAction_T> vecAllowedActions, VPOXDNDACTIONLIST *pLstAllowedActions);
    /** @}  */
