...
public:
    ...
    int                     i_dispatchToObject(PVPOXGUESTCTRLHOSTCBCTX pCtxCb, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb);
    int                     i_dispatchToThis(PVPOXGUESTCTRLHOSTCBCTX pCtxCb, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb);
    ...
    int                     i_onRemove(void);
    int                     i_onSessionStatusChange(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData);
    ...
    int                     i_sendMessage(uint32_t uFunction, uint32_t uParms, PVPOXHGCMSVCPARM paParms,
...
private:
    ...
    struct Data
    {
         ...
         *  Guest Additions < VPox 4.3 have version 1,
        ...
        uint64_t                    bmObjectIds[VPOX_GUESTCTRL_MAX_OBJECTS / sizeof(uint64_t) / 8];

        Data(void)
            : mpBaseEnvironment(NULL)
        {
            RT_ZERO(bmObjectIds);
            ASMBitSet(&bmObjectIds, VPOX_GUESTCTRL_MAX_OBJECTS - 1);    /* Reserved for the session itself? */
