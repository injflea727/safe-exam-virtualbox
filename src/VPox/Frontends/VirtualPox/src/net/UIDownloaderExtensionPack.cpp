...
/** @file
 * VPox Qt GUI - UIDownloaderExtensionPack class implementation.
 */
...
UIDownloaderExtensionPack::UIDownloaderExtensionPack()
{
    ...
    const QString strVersion = UIVersion(uiCommon().vpoxVersionStringNormalized()).effectiveReleasedVersion().toString();

    /* Prepare source/target: */
    const QString strUnderscoredName = QString(GUI_ExtPackName).replace(' ', '_');
    const QString strSourceName = QString("%1-%2.vpox-extpack").arg(strUnderscoredName, strVersion);
    const QString strSourcePath = QString("https://download.virtualpox.org/virtualpox/%1/").arg(strVersion);
    const QString strSource = strSourcePath + strSourceName;
    const QString strPathSHA256SumsFile = QString("https://www.virtualpox.org/download/hashes/%1/SHA256SUMS").arg(strVersion);
...
{
    return UIDownloader::description().arg(tr("VirtualPox Extension Pack"));
}
