                                ...
                                const POINT *pDstPoint)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            const uint32_t cbLine = vpoxWddmCalcRowSize(pSrcRect->left, pSrcRect->right, srcFormat);
            const uint32_t cRows = vpoxWddmCalcNumRows(pSrcRect->top, pSrcRect->bottom, srcFormat);
...
HRESULT APIENTRY GaDdiBlt(HANDLE hDevice, const D3DDDIARG_BLT *pData)
{
    ...
    vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p)\n", hDevice));
    vpoxVDbgPrintF(("src %p[%d] %d,%d %d,%d dst %p[%d] %d,%d %d,%d ColorKey 0x%08X Flags 0x%08X\n",
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    IDirect3DDevice9 *pDevice9If = VPOXDISP_D3DEV(pDevice);

    PVPOXWDDMDISP_RESOURCE pDstRc = (PVPOXWDDMDISP_RESOURCE)pData->hDstResource;
    PVPOXWDDMDISP_RESOURCE pSrcRc = (PVPOXWDDMDISP_RESOURCE)pData->hSrcResource;
    ...
    PVPOXWDDMDISP_ALLOCATION pSrcAlloc = &pSrcRc->aAllocations[pData->SrcSubResourceIndex];
//    PVPOXWDDMDISP_ALLOCATION pDstAlloc = &pDstRc->aAllocations[pData->DstSubResourceIndex];

    IDirect3DSurface9 *pSrcSurfIf = NULL;
    IDirect3DSurface9 *pDstSurfIf = NULL;

    HRESULT hr;
    hr = VPoxD3DIfSurfGet(pDstRc, pData->DstSubResourceIndex, &pDstSurfIf);
    Assert(hr == S_OK);
    if (hr == S_OK)
    {
        Assert(pDstSurfIf);

        hr = VPoxD3DIfSurfGet(pSrcRc, pData->SrcSubResourceIndex, &pSrcSurfIf);
        Assert(hr == S_OK);
        if (hr == S_OK)
        {
            Assert(pSrcSurfIf);
             ...
             * @todo Store the actual pool, where the resorce was created, in the VPOXWDDMDISP_RESOURCE structure.
             */
            const D3DPOOL poolSrc = vpoxDDI2D3DPool(pSrcRc->RcDesc.enmPool);
            const D3DPOOL poolDst = vpoxDDI2D3DPool(pDstRc->RcDesc.enmPool);
...
// seems to work                    AssertFailed(); /** @todo test this */
                ...
                else
                {
                     ...
                     * Can use StretchRect.
                     */
                    const D3DTEXTUREFILTERTYPE Filter = vpoxDDI2D3DBltFlags(pData->Flags);
                    ...
                    if (!pSrcRc->RcDesc.fFlags.RenderTarget || pDstRc->RcDesc.fFlags.RenderTarget)
                    {
                         ...
                         * At the moment NineDevice9_StretchRect includes VPox hack, see "Hack. Flip src Y."
                         */
                    ...
                    else
                    {
                        ...
                        const UINT Levels      = 1;
                        const DWORD Usage      = 0;
                        const D3DFORMAT Format = vpoxDDI2D3DFormat(pSrcRc->RcDesc.enmFormat);
    ...
    vpoxVDbgPrintF(("<== "__FUNCTION__", hDevice(0x%p), hr(0x%x)\n", hDevice, hr));
    return hr;
}
...
HRESULT APIENTRY GaDdiTexBlt(HANDLE hDevice, const D3DDDIARG_TEXBLT *pData)
{
    ...
    vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p)\n", hDevice));
    vpoxVDbgPrintF(("hDst %p, hSrc %p, face %d, dst %d,%d src %d,%d %d,%d\n",
                  pData->hDstResource,
                  pData->hSrcResource,
                  pData->CubeMapFace,
    ...
    Assert(    pDstRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_TEXTURE
            || pDstRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_CUBE_TEXTURE);
    Assert(    pSrcRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_TEXTURE
            || pSrcRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_CUBE_TEXTURE);
    ...
    HRESULT hr = S_OK;
/// @todo    VPOXVDBG_CHECK_SMSYNC(pDstRc);
/// @todo    VPOXVDBG_CHECK_SMSYNC(pSrcRc);
    ...
    {
        Assert(pDstRc->aAllocations[0].enmD3DIfType != VPOXDISP_D3DIFTYPE_VOLUME_TEXTURE);
        Assert(pSrcRc->aAllocations[0].enmD3DIfType != VPOXDISP_D3DIFTYPE_VOLUME_TEXTURE);
        ...
        IDirect3DSurface9 *pSrcSurfIf = NULL;
        IDirect3DSurface9 *pDstSurfIf = NULL;
        hr = VPoxD3DIfSurfGet(pDstRc, 0, &pDstSurfIf);
        Assert(hr == S_OK);
        if (hr == S_OK)
        {
            hr = VPoxD3DIfSurfGet(pSrcRc, 0, &pSrcSurfIf);
            Assert(hr == S_OK);
            if (hr == S_OK)
            {
                VPOXVDBG_CHECK_TEXBLT(hr = pDevice9If->UpdateSurface(pSrcSurfIf, &pData->SrcRect, pDstSurfIf, &pData->DstPoint);
                                      Assert(hr == S_OK),
...
HRESULT APIENTRY GaDdiVolBlt(HANDLE hDevice, const D3DDDIARG_VOLUMEBLT *pData)
{
    ...
    vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p)\n", hDevice));
    vpoxVDbgPrintF(("hDst %p, hSrc %p, dst %d,%d,%d src LT %d,%d RB %d,%d FB %d,%d\n",
                  pData->hDstResource,
                  pData->hSrcResource,
    ...
    for (Level = 0; Level < pSrcRc->cAllocations; ++Level)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                const UINT cbLine = vpoxWddmCalcRowSize(srcBox.Left, srcBox.Right, pSrcRc->RcDesc.enmFormat);
                ...
                for (INT d = 0; d < iDepth; ++d)
                {
                    ...
                    const UINT cRows = vpoxWddmCalcNumRows(0, iHeight, pSrcRc->RcDesc.enmFormat);
...
HRESULT APIENTRY GaDdiFlush(HANDLE hDevice)
{
    ...
    HRESULT hr = S_OK;
    if (   VPOXDISPMODE_IS_3D(pDevice->pAdapter)
...
HRESULT APIENTRY GaDdiPresent(HANDLE hDevice, const D3DDDIARG_PRESENT *pData)
{
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    IDirect3DDevice9 *pDevice9If = VPOXDISP_D3DEV(pDevice);
    PVPOXWDDMDISP_RESOURCE pSrcRc = NULL;
    PVPOXWDDMDISP_RESOURCE pDstRc = NULL;
    PVPOXWDDMDISP_ALLOCATION pSrcAlloc = NULL;
    PVPOXWDDMDISP_ALLOCATION pDstAlloc = NULL;

    if (pData->hSrcResource)
    {
        pSrcRc = (PVPOXWDDMDISP_RESOURCE)pData->hSrcResource;
    ...
    {
        pDstRc = (PVPOXWDDMDISP_RESOURCE)pData->hDstResource;
...
HRESULT APIENTRY GaDdiLock(HANDLE hDevice, D3DDDIARG_LOCK *pData)
{
    ...
    vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p) hResource 0x%p[%d] flags 0x%08X\n",
                    hDevice, pData->hResource, pData->SubResourceIndex, pData->Flags.Value));

    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)pData->hResource;
     ...
     *   Cube textures - set the shared handle to pAlloc->pvMem, because VPox version of Nine
    ...
    PVPOXWDDMDISP_ALLOCATION pAlloc = &pRc->aAllocations[pData->SubResourceIndex];
    const VPOXDISP_D3DIFTYPE enmD3DIfType = pAlloc->enmD3DIfType;
    const DWORD dwD3DLockFlags = vpoxDDI2D3DLockFlags(pData->Flags);
    ...
    if (VPOXDISPMODE_IS_3D(pDevice->pAdapter))
    {
        if (pData->Flags.NotifyOnly)
        {
            ...
            if (   enmD3DIfType == VPOXDISP_D3DIFTYPE_SURFACE
                || enmD3DIfType == VPOXDISP_D3DIFTYPE_VOLUME_TEXTURE)
            {
                /* Brute-force. */
                if (pAlloc->LockInfo.cLocks == 0)
                {
                    vpoxVDbgPrintF((__FUNCTION__", sync from backend\n"));
                    GaD3DResourceSynchMem(pRc, /* fToBackend */ false);
                }
            }

            //VPOXVDBG_CHECK_SMSYNC(pRc);
        }
        else
        {
            Assert(!pAlloc->pvMem);
        ...
        if (   enmD3DIfType == VPOXDISP_D3DIFTYPE_TEXTURE
            || enmD3DIfType == VPOXDISP_D3DIFTYPE_CUBE_TEXTURE
            || enmD3DIfType == VPOXDISP_D3DIFTYPE_SURFACE)
        {
                ...
                if (!fSameLock)
                {
                    switch (enmD3DIfType)
                    {
                        ...
                        case VPOXDISP_D3DIFTYPE_CUBE_TEXTURE:
                            hr = pD3DIfCubeTex->UnlockRect(VPOXDISP_CUBEMAP_INDEX_TO_FACE(pRc, pData->SubResourceIndex),
                                    VPOXDISP_CUBEMAP_INDEX_TO_LEVEL(pRc, pData->SubResourceIndex));
                            break;
            ...
            if (fNeedLock && SUCCEEDED(hr))
            {
                ...
                switch (enmD3DIfType)
                {
                    case VPOXDISP_D3DIFTYPE_TEXTURE:
                        ...
                        break;
                    case VPOXDISP_D3DIFTYPE_CUBE_TEXTURE:
                        hr = pD3DIfCubeTex->LockRect(VPOXDISP_CUBEMAP_INDEX_TO_FACE(pRc, pData->SubResourceIndex),
                                VPOXDISP_CUBEMAP_INDEX_TO_LEVEL(pRc, pData->SubResourceIndex),
                                &pAlloc->LockInfo.LockedRect,
                                pRect,
                                dwD3DLockFlags);
            ...
            if (SUCCEEDED(hr))
            {
                ...
                VPOXVDBG_DUMP_LOCK_ST(pData);

                hr = S_OK;
            }
        }
        else if (enmD3DIfType == VPOXDISP_D3DIFTYPE_VOLUME_TEXTURE)
        {
            Assert(pAlloc->pD3DIf);
        ...
        else if (enmD3DIfType == VPOXDISP_D3DIFTYPE_VERTEXBUFFER)
        {
            Assert(pAlloc->pD3DIf);
                ...
                else
                {
                    ...
                    if (bLocked && !pData->Flags.Discard)
                    {
                        ...
                        if (pRange)
                        {
                            ...
                            pr = NULL;
                        VPoxD3DIfLockUnlockMemSynch(pAlloc, &pAlloc->LockInfo.LockedRect, pr, false /*bool bToLockInfo*/);
                    }
                }

            }
        }
        else if (enmD3DIfType == VPOXDISP_D3DIFTYPE_INDEXBUFFER)
        {
            Assert(pAlloc->pD3DIf);
    ...
    else /* if !VPOXDISPMODE_IS_3D(pDevice->pAdapter) */
    {
        if (pAlloc->hAllocation)
        {
            if (pRc->RcDesc.enmPool != D3DDDIPOOL_SYSTEMMEM)
            {
                ...
                if (pData->Flags.AreaValid)
                {
                    offset = vpoxWddmCalcOffXYrd(pData->Area.left, pData->Area.top, pAlloc->SurfDesc.pitch,
                ...
                {
                    vpoxVDbgPrintF((__FUNCTION__": Implement Box area"));
                    Assert(0);
                    offset = 0;
                }
                else
            ...
            if (hr == S_OK)
            {
                ...
                {
                    if (pData->Flags.AreaValid)
                        vpoxWddmDirtyRegionAddRect(&pAlloc->DirtyRegion, &pData->Area);
                    else
                    {
                        Assert(!pData->Flags.RangeValid);
                        Assert(!pData->Flags.BoxValid);
                        vpoxWddmDirtyRegionAddRect(&pAlloc->DirtyRegion, NULL); /* <- NULL means the entire surface */
                    }
                }
...
HRESULT APIENTRY GaDdiUnlock(HANDLE hDevice, const D3DDDIARG_UNLOCK *pData)
{
    ...
    vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p) hResource 0x%p[%d]\n",
                    ...
                    || (!pAlloc->LockInfo.fFlags.Discard && !pAlloc->LockInfo.fFlags.NoOverwrite)))
            {
                ...
                if (pAlloc->pvMem && !pAlloc->LockInfo.fFlags.ReadOnly)
                {
                    ...
                    if (pAlloc->LockInfo.fFlags.RangeValid)
                    {
                        ...
                        pr = NULL;
                    VPoxD3DIfLockUnlockMemSynch(pAlloc, &pAlloc->LockInfo.LockedRect,
                            pr,
        ...
        if (hr == S_OK)
        {
            if (pData->Flags.NotifyOnly)
            {
                    ...
                    || enmD3DIfType == VPOXDISP_D3DIFTYPE_VOLUME_TEXTURE)
                {
                    ...
                    if (pAlloc->LockInfo.cLocks == 0)
                    {
                        vpoxVDbgPrintF((__FUNCTION__", sync to backend\n"));
            ...
            else
            {
                if (!pAlloc->LockInfo.cLocks)
                {
                    ...
                    if (hr == S_OK)
                    {
                        ...
                        Assert(pAlloc->DirtyRegion.fFlags & VPOXWDDM_DIRTYREGION_F_VALID);
                        VPoxD3DIfLockUnlockMemSynch(pAlloc, &LRect, &pAlloc->DirtyRegion.Rect, TRUE /* bool bToLockInfo*/);
                        vpoxWddmDirtyRegionClear(&pAlloc->DirtyRegion);
                        fDoUnlock = TRUE;
                    }
                    else
                    {
...
HRESULT APIENTRY GaDdiCreateVertexShaderFunc(HANDLE hDevice, D3DDDIARG_CREATEVERTEXSHADERFUNC *pData, const UINT *pCode)
{
    ...
    vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p) Size %d\n", hDevice, pData->Size));
...
#ifdef LOG_ENABLED
    vpoxVDbgPrintF(("Vertex shader code:\n"));
    ...
    {
        vpoxVDbgPrintF(("%08X", paTokens[iToken]));
    }
    vpoxVDbgPrintF(("\n"));
#endif
    ...
    if (pCode[0] == 0xFFFE0200)
    {
        ...
        vpoxVDbgPrintF(("Bumping version 2.0 to 2.1\n"));
...
static void vpoxWddmRequestAllocFree(D3DDDICB_ALLOCATE* pAlloc)
{
    RTMemFree(pAlloc);
}

static D3DDDICB_ALLOCATE* vpoxWddmRequestAllocAlloc(D3DDDIARG_CREATERESOURCE* pResource)
{
    /* allocate buffer for D3DDDICB_ALLOCATE + D3DDDI_ALLOCATIONINFO * numAllocs + PVPOXWDDM_RCINFO with aAllocInfos[numAllocs] */
    ...
    uint32_t cbRcInfo = sizeof (VPOXWDDM_RCINFO);
    cbBuf = offRcInfo + cbRcInfo;
    uint32_t offAllocInfos = (cbBuf + 7) & ~3;
    uint32_t cbAllocInfos = sizeof (VPOXWDDM_ALLOCINFO) * pResource->SurfCount;
...
HRESULT APIENTRY GaDdiCreateResource(HANDLE hDevice, D3DDDIARG_CREATERESOURCE *pResource)
{
    ...
    HRESULT hr = S_OK;
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    PVPOXWDDMDISP_ADAPTER pAdapter = pDevice->pAdapter;

    vpoxVDbgPrintF(("Format %d(0x%x), Shared %d, Pool %d, MsType %d, MsQuality %d, SurfCount %d, MipLevels %d, Fvf 0x%x, VidPnSourceId 0x%x, hResource 0x%x, Flags 0x%x, Rotation %d\n",
    ...
    {
        vpoxVDbgPrintF(("    [%d]: %dx%d @%d SysMem %p pitch %d, slice %d\n",
                        iSurf,
    ...
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)RTMemAllocZ(RT_UOFFSETOF_DYN(VPOXWDDMDISP_RESOURCE,
                                                                                      aAllocations[pResource->SurfCount]));
    if (!pRc)
    {
        WARN(("vpoxResourceAlloc failed"));
    ...
    for (UINT i = 0; i < pResource->SurfCount; ++i)
    {
        PVPOXWDDMDISP_ALLOCATION pAllocation = &pRc->aAllocations[i];
        const D3DDDI_SURFACEINFO *pSurf = &pResource->pSurfList[i];

        pAllocation->iAlloc      = i;
        pAllocation->pRc         = pRc;
        pAllocation->hAllocation = 0;
        pAllocation->enmType     = VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC;
        ...
        if (!vpoxWddmFormatToFourcc(pResource->Format))
            pAllocation->SurfDesc.bpp = vpoxWddmCalcBitsPerPixel(pResource->Format);
        else
        ...
        if (pSurf->SysMemPitch)
            ...
            pAllocation->SurfDesc.pitch = vpoxWddmCalcPitch(pSurf->Width, pResource->Format);

        pAllocation->SurfDesc.cbSize = vpoxWddmCalcSize(pAllocation->SurfDesc.pitch,
        ...
        if (pRc->RcDesc.enmPool == D3DDDIPOOL_SYSTEMMEM)
        {
            pAllocation->SurfDesc.d3dWidth = vpoxWddmCalcWidthForPitch(pAllocation->SurfDesc.pitch,
                                                                       pAllocation->SurfDesc.format);
    ...
    if (VPOXDISPMODE_IS_3D(pAdapter))
    {
    ...
    if (SUCCEEDED(hr) && bIssueCreateResource)
    {
        ...
        D3DDDICB_ALLOCATE *pDdiAllocate = vpoxWddmRequestAllocAlloc(pResource);
        if (pDdiAllocate)
        {
            IGaDirect3DDevice9Ex *pGaD3DDevice9Ex = NULL;
            if (bSetHostID)
            ...
            Assert(pDdiAllocate->PrivateDriverDataSize == sizeof(VPOXWDDM_RCINFO));

            PVPOXWDDM_RCINFO pRcInfo = (PVPOXWDDM_RCINFO)pDdiAllocate->pPrivateDriverData;
            ...
            for (UINT i = 0; i < pResource->SurfCount; ++i)
            {
                ...
                Assert(pDdiAllocInfo->PrivateDriverDataSize == sizeof(VPOXWDDM_ALLOCINFO));

                PVPOXWDDM_ALLOCINFO pWddmAllocInfo = (PVPOXWDDM_ALLOCINFO)pDdiAllocInfo->pPrivateDriverData;
                pWddmAllocInfo->enmType       = VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC;
            ...
            vpoxWddmRequestAllocFree(pDdiAllocate);
    ...
    vpoxVDbgPrintF(("<== "__FUNCTION__", hDevice(0x%p), pRc 0x%p, hr %x\n", hDevice, pRc, hr));
    return hr;
}
...
HRESULT APIENTRY GaDdiDestroyResource(HANDLE hDevice, HANDLE hResource)
{
    ...
    vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p) hResource %p\n", hDevice, hResource));

    HRESULT hr = S_OK;
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    PVPOXWDDMDISP_ADAPTER pAdapter = pDevice->pAdapter;
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)hResource;

    if (VPOXDISPMODE_IS_3D(pAdapter))
    {
        for (UINT i = 0; i < pRc->cAllocations; ++i)
        {
            PVPOXWDDMDISP_ALLOCATION pAlloc = &pRc->aAllocations[i];
            if (pAlloc->hSharedHandle)
            {
                ...
                else if (i == 0)
                {
                    ...
                    if (SUCCEEDED(hr2))
                    {
                        ...
                        /* Inform the miniport. */
                        VPOXDISPIFESCAPE_GASHAREDSID data;
                        RT_ZERO(data);
                        data.EscapeHdr.escapeCode = VPOXESC_GASHAREDSID;
...
HRESULT APIENTRY GaDdiOpenResource(HANDLE hDevice, D3DDDIARG_OPENRESOURCE *pResource)
{
    ...
    if (pRc)
    {
        ...
        for (UINT i = 0; i < pResource->NumAllocations; ++i)
        {
            ...
            if (pOAI->PrivateDriverDataSize != sizeof (VPOXWDDM_ALLOCINFO))
            {
                AssertFailed();
                hr = E_INVALIDARG;
                break;
            ...
            PVPOXWDDM_ALLOCINFO pWddmAllocInfo = (PVPOXWDDM_ALLOCINFO)pOAI->pPrivateDriverData;
            ...
            Assert(!pAllocation->hSharedHandle == (pAllocation->enmType == VPOXWDDM_ALLOC_TYPE_STD_SHAREDPRIMARYSURFACE));
        }
        ...
        if (!pResource->pPrivateDriverData || !pResource->PrivateDriverDataSize)
        {
            ...
            {
                PVPOXWDDMDISP_ALLOCATION pAlloc = &pRc->aAllocations[i];
                pAlloc->enmD3DIfType = VPOXDISP_D3DIFTYPE_SURFACE;
                pAlloc->pD3DIf = NULL;
            }

            D3DDDI_OPENALLOCATIONINFO *pOAI = &pResource->pOpenAllocationInfo[0];
            Assert(pOAI->pPrivateDriverData);
            Assert(pOAI->PrivateDriverDataSize >= sizeof(VPOXWDDM_ALLOCINFO));
            if (pOAI->pPrivateDriverData && pOAI->PrivateDriverDataSize >= sizeof(VPOXWDDM_ALLOCINFO))
            {
                PVPOXWDDM_ALLOCINFO pWddmAllocInfo = (PVPOXWDDM_ALLOCINFO)pOAI->pPrivateDriverData;
        ...
        else
        {
            ...
            Assert(pResource->PrivateDriverDataSize == sizeof(VPOXWDDM_RCINFO));
            if (pResource->PrivateDriverDataSize == sizeof(VPOXWDDM_RCINFO))
            {
                VPOXWDDM_RCINFO *pRcInfo = (VPOXWDDM_RCINFO *)pResource->pPrivateDriverData;
...
// ASMBreakpoint();
                ...
                if (SUCCEEDED(hr))
                {
                   ...
                   {
                       Assert(pGaD3DDevice9Ex);
                       PVPOXWDDMDISP_ALLOCATION pAllocation = &pRc->aAllocations[0]; /* First allocation is enough. */
                       uint32_t u32Sid;
        ...
        {
            pResource->hResource = pRc;
            vpoxVDbgPrintF(("<== "__FUNCTION__", pRc(0x%p)\n", pRc));
        }
        else
            RTMemFree(pRc);
    }
    ...
    {
        vpoxVDbgPrintR((__FUNCTION__": vpoxResourceAlloc failed for hDevice(0x%p), NumAllocations(%d)\n", hDevice, pResource->NumAllocations));
        hr = E_OUTOFMEMORY;
    }
...
HRESULT APIENTRY GaDdiDrawPrimitive(HANDLE hDevice, const D3DDDIARG_DRAWPRIMITIVE *pData, const UINT *pFlagBuffer)
{
    ...
    if (pDevice->cStreamSourcesUm)
    {
        ...
        if (pDevice->cStreamSourcesUm == 1)
        {
            for (UINT i = 0; i < RT_ELEMENTS(pDevice->aStreamSourceUm); ++i)
            {
                VPOXWDDMDISP_STREAMSOURCEUM *pStreamSourceUm = &pDevice->aStreamSourceUm[i];
...
HRESULT APIENTRY GaDdiDrawPrimitive2(HANDLE hDevice, const D3DDDIARG_DRAWPRIMITIVE2 *pData)
{
    ...
    {
        VPOXWDDMDISP_ALLOCATION *pStreamSource = pDevice->aStreamSource[0];
        VPOXWDDMDISP_STREAM_SOURCE_INFO *pStreamSourceInfo = &pDevice->StreamSourceInfo[0];

        Assert(pStreamSourceInfo->uiStride != 0);

        VPOXWDDMDISP_LOCKINFO *pLock = &pStreamSource->LockInfo;
        if (pLock->cLocks)
        {
...
static UINT vpoxWddmVertexCountFromPrimitive(D3DPRIMITIVETYPE PrimitiveType, UINT PrimitiveCount)
{
                                            ...
                                            UINT dwIndicesSize, const VOID *pIndexBuffer, const UINT *pFlagBuffer)
{
    ...
    {
        PVPOXWDDMDISP_ALLOCATION pAlloc = pDevice->aStreamSource[0];
        if (pAlloc->pvMem)
        {
...
HRESULT APIENTRY GaDdiSetRenderState(HANDLE hDevice, const D3DDDIARG_RENDERSTATE *pData)
{
    ...
    D3DRENDERSTATETYPE const enmD3DRenderStateType = vpoxDDI2D3DRenderStateType(pData->State);
...
HRESULT APIENTRY GaDdiSetTextureStageState(HANDLE hDevice, const D3DDDIARG_TEXTURESTAGESTATE *pData)
{
    ...
    VPOXWDDMDISP_TSS_LOOKUP lookup = vpoxDDI2D3DTestureStageStateType(pData->State);
...
HRESULT APIENTRY GaDdiSetTexture(HANDLE hDevice, UINT Stage, HANDLE hTexture)
{
    ...
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)hTexture;

    int const idx = VPOXWDDMDISP_SAMPLER_IDX(Stage);
    ...
    {
        // VPOXVDBG_CHECK_SMSYNC(pRc);
        if (pRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_TEXTURE)
        {
            pD3DIfTex = (IDirect3DTexture9*)pRc->aAllocations[0].pD3DIf;
        }
        else if (pRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_CUBE_TEXTURE)
        {
            pD3DIfTex = (IDirect3DCubeTexture9*)pRc->aAllocations[0].pD3DIf;
        }
        else if (pRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_VOLUME_TEXTURE)
        {
                                        ...
                                        const VOID *pUMBuffer)
{
    ...
    PVPOXWDDMDISP_STREAMSOURCEUM pStrSrcUm = &pDevice->aStreamSourceUm[pData->Stream];
...
HRESULT APIENTRY GaDdiSetIndices(HANDLE hDevice, const D3DDDIARG_SETINDICES *pData)
{
    ...
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)pData->hIndexBuffer;
    PVPOXWDDMDISP_ALLOCATION pAlloc = NULL;
...
HRESULT APIENTRY GaDdiSetDisplayMode(HANDLE hDevice, const D3DDDIARG_SETDISPLAYMODE *pData)
{
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    Assert(VPOXDISPMODE_IS_3D(pDevice->pAdapter));
...
HRESULT APIENTRY GaDdiSetStreamSource(HANDLE hDevice, const D3DDDIARG_SETSTREAMSOURCE *pData)
{
    ...
    AssertReturn(pData->Stream < VPOXWDDMDISP_MAX_VERTEX_STREAMS, E_INVALIDARG);
    ...
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)pData->hVertexBuffer;
    if (pRc)
    {
        // VPOXVDBG_CHECK_SMSYNC(pRc);
        Assert(pRc->cAllocations == 1);
...
HRESULT APIENTRY GaDdiColorFill(HANDLE hDevice, const D3DDDIARG_COLORFILL *pData)
{
    ...
    HRESULT hr = VPoxD3DIfSurfGet(pRc, pData->SubResourceIndex, &pSurfIf);
    Assert(hr == S_OK);
    if (hr == S_OK)
    {
        // VPOXVDBG_CHECK_SMSYNC(pRc);
...
HRESULT APIENTRY GaDdiCreateQuery(HANDLE hDevice, D3DDDIARG_CREATEQUERY *pData)
{
    ...
    PVPOXWDDMDISP_QUERY pQuery = (PVPOXWDDMDISP_QUERY)RTMemAllocZ(sizeof(VPOXWDDMDISP_QUERY));
    AssertReturn(pQuery, E_OUTOFMEMORY);

    D3DQUERYTYPE const d3dQueryType = vpoxDDI2D3DQueryType(pData->QueryType);
...
HRESULT APIENTRY GaDdiDestroyQuery(HANDLE hDevice, HANDLE hQuery)
{
    ...
    PVPOXWDDMDISP_QUERY pQuery = (PVPOXWDDMDISP_QUERY)hQuery;
    Assert(pQuery);
    if (pQuery)
    {
...
HRESULT APIENTRY GaDdiIssueQuery(HANDLE hDevice, const D3DDDIARG_ISSUEQUERY *pData)
{
    ...
    PVPOXWDDMDISP_QUERY pQuery = (PVPOXWDDMDISP_QUERY)pData->hQuery;
    AssertReturn(pQuery, E_INVALIDARG);

    pQuery->fQueryState.Value |= pData->Flags.Value;

    DWORD const d3dQueryFlags = vpoxDDI2D3DIssueQueryFlags(pData->Flags);
...
HRESULT APIENTRY GaDdiGetQueryData(HANDLE hDevice, const D3DDDIARG_GETQUERYDATA *pData)
{
    ...
    {
        case D3DDDIQUERYTYPE_EVENT:
            vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p) D3DDDIQUERYTYPE_EVENT %d\n", hDevice, *(BOOL *)pData->pData));
            break;
        case D3DDDIQUERYTYPE_OCCLUSION:
            vpoxVDbgPrintF(("==> "__FUNCTION__", hDevice(0x%p) D3DDDIQUERYTYPE_OCCLUSION %d\n", hDevice, *(UINT *)pData->pData));
            break;
        default:
            break;
    }
...
#if 0
static HRESULT vpoxWddmRenderTargetSet(PVPOXWDDMDISP_DEVICE pDevice, UINT iRt, PVPOXWDDMDISP_ALLOCATION pAlloc)
{
    IDirect3DDevice9 * pDevice9If = VPOXDISP_D3DEV(pDevice);
    HRESULT hr = S_OK;
    ...
    if (pAlloc)
    {
        hr = VPoxD3DIfSurfGet(pAlloc->pRc, pAlloc->iAlloc, &pD3D9Surf);
        if (FAILED(hr))
        {
...
HRESULT APIENTRY GaDdiSetRenderTarget(HANDLE hDevice, const D3DDDIARG_SETRENDERTARGET *pData)
{
    ...
    PVPOXWDDMDISP_ALLOCATION pAlloc = NULL;
    IDirect3DSurface9 *pD3D9Surf = NULL;
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)pData->hRenderTarget;
    if (pRc)
    {
        // VPOXVDBG_CHECK_SMSYNC(pRc);
...
HRESULT APIENTRY GaDdiSetDepthStencil(HANDLE hDevice, const D3DDDIARG_SETDEPTHSTENCIL *pData)
{
    ...
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)pData->hZBuffer;
    if (pRc)
    {
        // VPOXVDBG_CHECK_SMSYNC(pRc);
        Assert(pRc->cAllocations == 1);

        hr = VPoxD3DIfSurfGet(pRc, 0, &pD3D9Surf);
        if (FAILED(hr))
            WARN(("VPoxD3DIfSurfGet failed, hr (0x%x)",hr));
        else
            Assert(pD3D9Surf);
...
HRESULT APIENTRY GaDdiCreateVideoProcessDevice(HANDLE hDevice, D3DDDIARG_CREATEVIDEOPROCESSDEVICE *pData)
{
    ...
    HRESULT hr;
    if (pDevice)
        hr = VPoxDxvaCreateVideoProcessDevice(pDevice, pData);
    else
        hr = E_INVALIDARG;
...
HRESULT APIENTRY GaDdiDestroyVideoProcessDevice(HANDLE hDevice, HANDLE hVideoProcessor)
{
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    HRESULT hr = VPoxDxvaDestroyVideoProcessDevice(pDevice, hVideoProcessor);
...
HRESULT APIENTRY GaDdiVideoProcessBeginFrame(HANDLE hDevice, HANDLE hVideoProcessor)
{
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    HRESULT hr = VPoxDxvaVideoProcessBeginFrame(pDevice, hVideoProcessor);
...
HRESULT APIENTRY GaDdiVideoProcessEndFrame(HANDLE hDevice, D3DDDIARG_VIDEOPROCESSENDFRAME* pData)
{
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    HRESULT hr = VPoxDxvaVideoProcessEndFrame(pDevice, pData);
                                                  ...
                                                  const D3DDDIARG_SETVIDEOPROCESSRENDERTARGET *pData)
{
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    HRESULT hr = VPoxDxvaSetVideoProcessRenderTarget(pDevice, pData);
...
HRESULT APIENTRY GaDdiVideoProcessBlt(HANDLE hDevice, const D3DDDIARG_VIDEOPROCESSBLT *pData)
{
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    HRESULT hr = VPoxDxvaVideoProcessBlt(pDevice, pData);
...
HRESULT APIENTRY GaDdiCreateOverlay(HANDLE hDevice, D3DDDIARG_CREATEOVERLAY *pData)
{
    ...
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)pData->OverlayInfo.hResource;
    AssertReturn(pData->OverlayInfo.SubResourceIndex < pRc->cAllocations, E_INVALIDARG);

    HRESULT hr = S_OK;
    PVPOXWDDMDISP_OVERLAY pOverlay = (PVPOXWDDMDISP_OVERLAY)RTMemAllocZ(sizeof(VPOXWDDMDISP_OVERLAY));
    Assert(pOverlay);
    if (pOverlay)
    {
        PVPOXWDDMDISP_ALLOCATION pAlloc = &pRc->aAllocations[pData->OverlayInfo.SubResourceIndex];

        VPOXWDDM_OVERLAY_INFO OurInfo;
        ...
        vpoxWddmDirtyRegionClear(&OurInfo.DirtyRegion);

        Assert(!pAlloc->LockInfo.cLocks);
        vpoxWddmDirtyRegionUnite(&OurInfo.DirtyRegion, &pAlloc->DirtyRegion);
...
HRESULT APIENTRY GaDdiUpdateOverlay(HANDLE hDevice, const D3DDDIARG_UPDATEOVERLAY *pData)
{
    ...
    HRESULT hr = S_OK;
    PVPOXWDDMDISP_OVERLAY pOverlay = (PVPOXWDDMDISP_OVERLAY)pData->hOverlay;
...
HRESULT APIENTRY GaDdiFlipOverlay(HANDLE hDevice, const D3DDDIARG_FLIPOVERLAY *pData)
{
    ...
    PVPOXWDDMDISP_RESOURCE pRc = (PVPOXWDDMDISP_RESOURCE)pData->hSource;
    Assert(pRc);
    Assert(pRc->cAllocations > pData->SourceIndex);

    PVPOXWDDMDISP_ALLOCATION pAlloc = &pRc->aAllocations[pData->SourceIndex];

    HRESULT hr = S_OK;
    PVPOXWDDMDISP_OVERLAY pOverlay = (PVPOXWDDMDISP_OVERLAY)pData->hOverlay;

    VPOXWDDM_OVERLAYFLIP_INFO OurInfo;
    vpoxWddmDirtyRegionClear(&OurInfo.DirtyRegion);
    Assert(!pAlloc->LockInfo.cLocks);
    vpoxWddmDirtyRegionUnite(&OurInfo.DirtyRegion, &pAlloc->DirtyRegion);
...
HRESULT APIENTRY GaDdiDestroyDevice(HANDLE hDevice)
{
    ...
    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)hDevice;
    AssertReturn(pDevice->pAdapter->enmHwType == VPOXVIDEO_HWTYPE_VMSVGA, E_FAIL);
...
HRESULT APIENTRY GaDdiAdapterCreateDevice(HANDLE hAdapter, D3DDDIARG_CREATEDEVICE *pCreateData)
{
    ...
    vpoxVDbgPrint(("==> "__FUNCTION__", hAdapter(0x%p), Interface(%d), Version(%d)\n", hAdapter, pCreateData->Interface, pCreateData->Version));

    PVPOXWDDMDISP_ADAPTER pAdapter = (PVPOXWDDMDISP_ADAPTER)hAdapter;
    AssertReturn(pAdapter->enmHwType == VPOXVIDEO_HWTYPE_VMSVGA, E_INVALIDARG);

    PVPOXWDDMDISP_DEVICE pDevice = (PVPOXWDDMDISP_DEVICE)RTMemAllocZ(RT_UOFFSETOF_DYN(VPOXWDDMDISP_DEVICE,
    ...
    {
        vpoxVDbgPrintR((__FUNCTION__": RTMemAllocZ returned NULL\n"));
        hr = E_OUTOFMEMORY;
    }
     ...
     */
    extern void GaDrvTest(IGalliumStack *pGalliumStack, PVPOXWDDMDISP_DEVICE pDevice);
    if (SUCCEEDED(hr))
       GaDrvTest(pAdapter->D3D.pGalliumStack, pDevice);
#endif

    vpoxVDbgPrint(("<== "__FUNCTION__", hAdapter(0x%p)\n", hAdapter));
    return hr;
}
...
HRESULT APIENTRY GaDdiAdapterCloseAdapter(IN HANDLE hAdapter)
{
    ...
    if (VPOXDISPMODE_IS_3D(pAdapter))
    {
        VPoxDispD3DGlobalClose(&pAdapter->D3D, &pAdapter->Formats);
    }
#ifdef VPOX_WITH_VIDEOHWACCEL
    else
    {
        VPoxDispD3DGlobal2DFormatsTerm(pAdapter);
    }
#endif
...
static D3DDDIQUERYTYPE const gVPoxQueryTypes[] = {
        D3DDDIQUERYTYPE_EVENT,
        D3DDDIQUERYTYPE_OCCLUSION
};

#ifdef VPOX_WITH_VIDEOHWACCEL
static bool vpoxVhwaHasCKeying(PVPOXWDDMDISP_ADAPTER pAdapter)
{
    for (uint32_t i = 0; i < pAdapter->cHeads; ++i)
    {
        VPOXVHWA_INFO* pSettings = &pAdapter->aHeads[i].Vhwa.Settings;
        if (   (pSettings->fFlags & VPOXVHWA_F_ENABLED)
...
HRESULT APIENTRY GaDdiAdapterGetCaps(HANDLE hAdapter, const D3DDDIARG_GETCAPS *pData)
{
    ...
    vpoxVDbgPrint(("==> "__FUNCTION__", hAdapter(0x%p), caps type(%d)\n", hAdapter, pData->Type));
    ...
    switch (pData->Type)
    {
        ...
        {
            Assert(!VPOXDISPMODE_IS_3D(pAdapter));
            ...
            if (pData->DataSize >= sizeof(DDRAW_CAPS))
            {
                ...
                if (!VPOXDISPMODE_IS_3D(pAdapter))
                {
                    if (vpoxVhwaHasCKeying(pAdapter))
                    {
...
//                        pCaps->Caps2 |= DDRAW_CAPS2_FLIPNOVSYNC;
        ...
        case D3DDDICAPS_DDRAW_MODE_SPECIFIC:
        {
            ...
            if (pData->DataSize >= sizeof(DDRAW_MODE_SPECIFIC_CAPS))
            {
                ...
                if (!VPOXDISPMODE_IS_3D(pAdapter))
                {
                    VPOXVHWA_INFO *pSettings = &pAdapter->aHeads[pCaps->Head].Vhwa.Settings;
                    if (pSettings->fFlags & VPOXVHWA_F_ENABLED)
                    {
                        pCaps->Caps |= MODE_CAPS_OVERLAY | MODE_CAPS_OVERLAYSTRETCH;

                        if (pSettings->fFlags & VPOXVHWA_F_CKEY_DST)
                        {
                        ...
                        if (pSettings->fFlags & VPOXVHWA_F_CKEY_SRC)
                        {
        ...
        case D3DDDICAPS_GETD3DQUERYCOUNT:
            *((uint32_t*)pData->pData) = RT_ELEMENTS(gVPoxQueryTypes);
            break;

        case D3DDDICAPS_GETD3DQUERYDATA:
            Assert(pData->DataSize == RT_ELEMENTS(gVPoxQueryTypes) * sizeof(D3DDDIQUERYTYPE));
            memcpy(pData->pData, gVPoxQueryTypes, RT_ELEMENTS(gVPoxQueryTypes) * sizeof(D3DDDIQUERYTYPE));
            break;
...
//                        | D3DDEVCAPS_TEXTUREVIDEOMEMORY
        ...
        case D3DDDICAPS_GETVIDEOPROCESSORDEVICEGUIDCOUNT:
        {
            ...
            {
                if (pAdapter->AdapterInfo.u32AdapterCaps & VPOXWDDM_QAI_CAP_DXVA)
                    hr = VPoxDxvaGetDeviceGuidCount((UINT *)pData->pData);
                else
                    *(UINT *)pData->pData = 0;
            }
            else
        ...
        {
            hr = VPoxDxvaGetDeviceGuids((GUID *)pData->pData, pData->DataSize);
            break;
        }
        ...
        case D3DDDICAPS_GETVIDEOPROCESSORRTFORMATCOUNT:
        {
            if (pData->DataSize >= sizeof(UINT))
                if (pAdapter->AdapterInfo.u32AdapterCaps & VPOXWDDM_QAI_CAP_DXVA)
                    hr = VPoxDxvaGetOutputFormatCount((UINT *)pData->pData, (DXVADDI_VIDEOPROCESSORINPUT *)pData->pInfo,
        ...
        {
            hr = VPoxDxvaGetOutputFormats((D3DDDIFORMAT *)pData->pData, pData->DataSize,
        ...
        case D3DDDICAPS_GETVIDEOPROCESSORCAPS:
        {
            if (pData->DataSize >= sizeof(DXVADDI_VIDEOPROCESSORCAPS))
                hr = VPoxDxvaGetCaps((DXVADDI_VIDEOPROCESSORCAPS *)pData->pData,
                                     (DXVADDI_VIDEOPROCESSORINPUT *)pData->pInfo);
            else
                hr = E_INVALIDARG;
            break;
        ...
        case D3DDDICAPS_GETEXTENSIONCAPS:
            vpoxVDbgPrint((__FUNCTION__": unimplemented caps type(%d)\n", pData->Type));
            AssertFailed();
            if (pData->pData && pData->DataSize)
                memset(pData->pData, 0, pData->DataSize);
            break;
        ...
        default:
            vpoxVDbgPrint((__FUNCTION__": unknown caps type(%d)\n", pData->Type));
            AssertFailed();
    }
