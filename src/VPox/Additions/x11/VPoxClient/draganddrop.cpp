 ...
 */
#ifdef VPOX_WITH_DRAG_AND_DROP_PROMISES
# undef VPOX_WITH_DRAG_AND_DROP_PROMISES
#endif
...
/** List of Atoms. */
#define VPoxDnDAtomList RTCList<Atom>
...
public:
    ...
    inline RTCString xAtomListToString(const VPoxDnDAtomList &formatList)
    {
        RTCString format;
...
#ifdef DEBUG
# define VPOX_DND_FN_DECL_LOG(x) inline x /* For LogFlowXXX logging. */
#else
# define VPOX_DND_FN_DECL_LOG(x) x
#endif
...
public:
    ...
    RTMEM_IMPLEMENT_NEW_AND_DELETE();
#endif
    VPoxDnDProxyWnd(void);
    virtual ~VPoxDnDProxyWnd(void);
...
public:
    ...
    int sendFinished(Window hWndSource, VPOXDNDACTION dndAction);
...
public:
    ...
    int ghDropped(const RTCString &strFormat, VPOXDNDACTION dndActionRequested);
#endif

    /* Host -> Guest handling. */
    int hgEnter(const RTCList<RTCString> &formats, VPOXDNDACTIONLIST dndListActionsAllowed);
    int hgLeave(void);
    int hgMove(uint32_t uPosX, uint32_t uPosY, VPOXDNDACTION dndActionDefault);
    int hgDrop(uint32_t uPosX, uint32_t uPosY, VPOXDNDACTION dndActionDefault);
    ...
    int wndXDnDGetActionList(Window wndThis, VPoxDnDAtomList &lstActions) const;
    int wndXDnDGetFormatList(Window wndThis, VPoxDnDAtomList &lstTypes) const;
    int wndXDnDSetActionList(Window wndThis, const VPoxDnDAtomList &lstActions) const;
    int wndXDnDSetFormatList(Window wndThis, Atom atmProp, const VPoxDnDAtomList &lstFormats) const;

    /* Atom / HGCM formatting helpers. */
    int             appendFormatsToList(const RTCList<RTCString> &lstFormats, VPoxDnDAtomList &lstAtoms) const;
    int             appendDataToList(const void *pvData, uint32_t cbData, VPoxDnDAtomList &lstAtoms) const;
    static Atom     toAtomAction(VPOXDNDACTION dndAction);
    static int      toAtomActions(VPOXDNDACTIONLIST dndActionList, VPoxDnDAtomList &lstAtoms);
    static uint32_t toHGCMAction(Atom atom);
    static uint32_t toHGCMActions(const VPoxDnDAtomList &actionsList);
...
protected:
    ...
    VPoxDnDAtomList             m_lstAtomFormats;
    /** List of (Atom) actions the source window supports. */
    VPoxDnDAtomList             m_lstAtomActions;
...
int DragInstance::init(uint32_t uScreenID)
{
    ...
    do
    {
        ...
        XStoreName(m_pDisplay, m_wndProxy.hWnd, "VPoxClientWndDnD");

        /* Make the new window Xdnd aware. */
        Atom atmVer = VPOX_XDND_VERSION;
    ...
    switch (m_enmMode)
    {
        case HG:
        {
            ...
            if (e.xclient.message_type == xAtom(XA_XdndStatus))
            {
                ...
                const bool fAcceptDrop    = e.xclient.data.l[XdndStatusFlags] & VPOX_XDND_STATUS_FLAG_ACCEPT;
                /* Does the target want XdndPosition messages? */
                const bool fWantsPosition = e.xclient.data.l[XdndStatusFlags] & VPOX_XDND_STATUS_FLAG_WANTS_POS;
                ...
                {
                    VPOXDNDACTION dndAction = VPOX_DND_ACTION_IGNORE; /* Default is ignoring. */
            ...
            else if (e.xclient.message_type == xAtom(XA_XdndFinished))
            {
                ...
                const bool fSucceeded = e.xclient.data.l[XdndFinishedFlags] & VPOX_XDND_FINISHED_FLAG_SUCCEEDED;
        ...
        case GH:
        {
                     ...
                     && m_wndCur               == static_cast<Window>(e.xclient.data.l[XdndPositionWindow]))
            {
                ...
                m.data.l[XdndStatusAction]  = fAcceptDrop ? toAtomAction(VPOX_DND_ACTION_COPY) : None;
                     ...
                     && m_wndCur               == static_cast<Window>(e.xclient.data.l[XdndDropWindow]))
            {
                ...
                if (m_enmState != Dropped) /* Wrong mode? Bail out. */
                {
                    ...
                    /* Let the source know. */
                    rc = m_wndProxy.sendFinished(m_wndCur, VPOX_DND_ACTION_IGNORE);
...
# error "Implement me!"
                ...
                {
#endif /* VPOX_WITH_DRAG_AND_DROP_PROMISES */
                    ...
                    RT_NOREF(xRc);
#ifndef VPOX_WITH_DRAG_AND_DROP_PROMISES
                }
#endif
            }
            /* Anything else. */
 ...
 */
int DragInstance::hgMove(uint32_t uPosX, uint32_t uPosY, VPOXDNDACTION dndActionDefault)
{
    LogFlowThisFunc(("mode=%RU32, state=%RU32\n", m_enmMode, m_enmState));
        ...
        && newVer    != -1)
    {
        ...
        m.data.l[XdndEnterFlags]  = RT_MAKE_U32_FROM_U8(
                                    ...
                                    RT_MIN(VPOX_XDND_VERSION, newVer));
    ...
    if (newVer == -1)
    {
        ...
        rc = VbglR3DnDHGSendAckOp(&m_dndCtx, VPOX_DND_ACTION_IGNORE);
    }
    else
    {
        Assert(wndCursor != None);
 ...
 *                                  as soon as the operation successfully finishes.
 */
int DragInstance::hgDrop(uint32_t uPosX, uint32_t uPosY, VPOXDNDACTION dndActionDefault)
{
...
int DragInstance::ghIsDnDPending(void)
{
    ...
    VPOXDNDACTION     dndActionDefault = VPOX_DND_ACTION_IGNORE;
    VPOXDNDACTIONLIST dndActionList    = VPOX_DND_ACTION_IGNORE;
    ...
    if (RT_SUCCESS(rc2))
    {
        ...
        if (!strFormatsCur.isEmpty())
        {
            ...
            dndActionDefault = VPOX_DND_ACTION_COPY; /** @todo Handle default action! */
            dndActionList    = VPOX_DND_ACTION_COPY; /** @todo Ditto. */
 ...
 */
int DragInstance::ghDropped(const RTCString &strFormat, VPOXDNDACTION dndActionRequested)
{
        ...
        if (waitForX11Msg(evSelNotify, SelectionNotify, 5 * 1000 /* 5s timeout */))
        {
                ...
                && evSelNotify.xselection.target    == aFormat)
            {
                ...
                int xRc = XGetWindowProperty(m_pDisplay, m_wndProxy.hWnd,
                                             ...
                                             0                        /* Offset */,
                                             VPOX_MAX_XPROPERTIES     /* Length of 32-bit multiples */,
                ...
                if (fCancel)
                {
                    ...
                    rc = m_wndProxy.sendFinished(wndSource, VPOX_DND_ACTION_IGNORE);
                }
 ...
 * @param   lstActions              Reference to VPoxDnDAtomList to store the action into.
 */
int DragInstance::wndXDnDGetActionList(Window wndThis, VPoxDnDAtomList &lstActions) const
{
    Atom iActType = None;
    int iActFmt;
    ...
    if (cItems > 0)
    {
        ...
        for (unsigned i = 0; i < RT_MIN(VPOX_MAX_XPROPERTIES, cItems); i++)
        {
 ...
 * @param   lstTypes                Reference to VPoxDnDAtomList to store the formats into.
 */
int DragInstance::wndXDnDGetFormatList(Window wndThis, VPoxDnDAtomList &lstTypes) const
{
    Atom iActType = None;
    int iActFmt;
    unsigned long cItems, cbData;
 ...
 */
int DragInstance::wndXDnDSetActionList(Window wndThis, const VPoxDnDAtomList &lstActions) const
{
 ...
 */
int DragInstance::wndXDnDSetFormatList(Window wndThis, Atom atmProp, const VPoxDnDAtomList &lstFormats) const
{
...
/**
 * Appends a RTCString list to VPoxDnDAtomList list.
 *
 * @returns IPRT status code.
 * @param   lstFormats              Reference to RTCString list to convert.
 * @param   lstAtoms                Reference to VPoxDnDAtomList list to store results in.
 */
int DragInstance::appendFormatsToList(const RTCList<RTCString> &lstFormats, VPoxDnDAtomList &lstAtoms) const
{
    for (size_t i = 0; i < lstFormats.size(); ++i)
...
/**
 * Appends a raw-data string list to VPoxDnDAtomList list.
 *
 * @returns IPRT status code.
 ...
 * @param   lstAtoms                Reference to VPoxDnDAtomList list to store results in.
 */
int DragInstance::appendDataToList(const void *pvData, uint32_t cbData, VPoxDnDAtomList &lstAtoms) const
{
    RT_NOREF1(lstAtoms);
 ...
 */
/* static */
Atom DragInstance::toAtomAction(VPOXDNDACTION dndAction)
{
    /* Ignore is None. */
...
/**
 * Converts HGCM-based drag'n drop actions to a VPoxDnDAtomList list.
 *
 * @returns IPRT status code.
 * @param   dndActionList           HGCM drag'n drop actions to convert.
 * @param   lstAtoms                Reference to VPoxDnDAtomList to store actions in.
 */
/* static */
int DragInstance::toAtomActions(VPOXDNDACTIONLIST dndActionList, VPoxDnDAtomList &lstAtoms)
{
...
{
    uint32_t uAction = VPOX_DND_ACTION_IGNORE;

    if (atom == xAtom(XA_XdndActionCopy))
        uAction = VPOX_DND_ACTION_COPY;
    else if (atom == xAtom(XA_XdndActionMove))
        uAction = VPOX_DND_ACTION_MOVE;
    else if (atom == xAtom(XA_XdndActionLink))
        uAction = VPOX_DND_ACTION_LINK;

    return uAction;
}

/**
 * Converts an VPoxDnDAtomList list to an HGCM action list.
 *
 * @returns ORed HGCM action list.
 * @param   lstActions              List of Atom-based actions to convert.
 */
/* static */
uint32_t DragInstance::toHGCMActions(const VPoxDnDAtomList &lstActions)
{
    uint32_t uActions = VPOX_DND_ACTION_IGNORE;
...
/*********************************************************************************************************************************
 * VPoxDnDProxyWnd implementation.                                                                                               *
 ********************************************************************************************************************************/

VPoxDnDProxyWnd::VPoxDnDProxyWnd(void)
    : pDisp(NULL)
    , hWnd(0)
    , iX(0)
    , iY(0)
...
int VPoxDnDProxyWnd::init(Display *pDisplay)
{
    /** @todo What about multiple screens? Test this! */
...
void VPoxDnDProxyWnd::destroy(void)
{

}

int VPoxDnDProxyWnd::sendFinished(Window hWndSource, VPOXDNDACTION dndAction)
{
    /* Was the drop accepted by the host? That is, anything than ignoring. */
    bool fDropAccepted = dndAction > VPOX_DND_ACTION_IGNORE;
...
int DragAndDropService::run(bool fDaemonised /* = false */)
{
    ...
    do
    {
        ...
        /* Note: For multiple screen support in VPox it is not necessary to use
         * another screen number than zero. Maybe in the future it will become
         * necessary if VPox supports multiple X11 screens. */
...
/** VPoxClient service class wrapping the logic for the service while
 *  the main VPoxClient code provides the daemon logic needed by all services.
 */
...
{
    return ".vpoxclient-draganddrop.pid";
}
