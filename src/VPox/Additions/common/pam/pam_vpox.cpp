/* $Id: pam_vpox.cpp $ */
/** @file
 * pam_vpox - PAM module for auto logons.
 */
...
#define VPOX_MODULE_NAME                    "pam_vpox"

#define VPOX_PAM_FLAG_SILENT                "PAM_SILENT"
#define VPOX_PAM_FLAG_DISALLOW_NULL_AUTHTOK "PAM_DISALLOW_NULL_AUTHTOK"
#define VPOX_PAM_FLAG_ESTABLISH_CRED        "PAM_ESTABLISH_CRED"
#define VPOX_PAM_FLAG_DELETE_CRED           "PAM_DELETE_CRED"
#define VPOX_PAM_FLAG_REINITIALIZE_CRED     "PAM_REINITIALIZE_CRED"
#define VPOX_PAM_FLAG_REFRESH_CRED          "PAM_REFRESH_CRED"
 ...
 */
static void pam_vpox_writesyslog(char *pszBuf)
{
#ifdef RT_OS_LINUX
    openlog("pam_vpox", LOG_PID, LOG_AUTHPRIV);
    syslog(LOG_ERR, "%s", pszBuf);
    closelog();
#elif defined(RT_OS_SOLARIS)
    syslog(LOG_ERR, "pam_vpox: %s\n", pszBuf);
#endif
}
 ...
 */
static void pam_vpox_error(pam_handle_t *hPAM, const char *pszFormat, ...)
{
    RT_NOREF1(hPAM);
    va_list va;
    char *buf;
    ...
    {
        LogRel(("%s: Error: %s", VPOX_MODULE_NAME, buf));
        pam_vpox_writesyslog(buf);
        RTStrFree(buf);
    }
    va_end(va);
}
 ...
 * @param   ...                     Format arguments.
 */
static void pam_vpox_log(pam_handle_t *hPAM, const char *pszFormat, ...)
{
    RT_NOREF1(hPAM);
    if (g_verbosity)
    {
        ...
        if (RT_SUCCESS(RTStrAPrintfV(&buf, pszFormat, va)))
        {
             ...
             * sensitive data! */
            LogRel(("%s: %s", VPOX_MODULE_NAME, buf));
            /* Log to syslog */
            pam_vpox_writesyslog(buf);
            RTStrFree(buf);
        }
 ...
 */
static int vpox_set_msg(pam_handle_t *hPAM, int iStyle, const char *pszText)
{
        ...
        && conv)
    {
...
#endif
        pam_vpox_log(hPAM, "Showing message \"%s\" (type %d)", pszText, iStyle);
        ...
        if (resp != NULL) /* If we use PAM_TEXT_INFO we never will get something back! */
        {
            if (resp->resp)
            {
                pam_vpox_log(hPAM, "Response to message \"%s\" was \"%s\"",
                             pszText, resp->resp);
                /** @todo Save response!  */
                free(resp->resp);
            }
...
/**
 * Initializes pam_vpox.
 *
 * @return  IPRT status code.
 * @param   hPAM                    PAM handle.
 */
static int pam_vpox_init(pam_handle_t *hPAM)
{
#ifdef DEBUG
    ...
    pam_vpox_log(hPAM, "pam_vpox: %sr%s, running on %s\n",
                 RTBldCfgVersion(), RTBldCfgRevisionStr(), RTBldCfgTargetArch());

    int rc = RTR3InitDll(0);
    if (RT_FAILURE(rc))
    {
        pam_vpox_error(hPAM, "pam_vpox_init: could not init runtime! rc=%Rrc. Aborting\n", rc);
        return PAM_SUCCESS; /* Jump out as early as we can to not mess around. */
    }

    pam_vpox_log(hPAM, "pam_vpox_init: runtime initialized\n");
    if (RT_SUCCESS(rc))
    {
        rc = VbglR3InitUser();
        if (RT_FAILURE(rc))
        {
            switch(rc)
            {
                case VERR_ACCESS_DENIED:
                    pam_vpox_error(hPAM, "pam_vpox_init: access is denied to guest driver! Please make sure you run with sufficient rights. Aborting\n");
                    break;

                case VERR_FILE_NOT_FOUND:
                    pam_vpox_error(hPAM, "pam_vpox_init: guest driver not found! Guest Additions installed? Aborting\n");
                    break;

                default:
                    pam_vpox_error(hPAM, "pam_vpox_init: could not init VbglR3 library! rc=%Rrc. Aborting\n", rc);
                    break;
            }
        }
        pam_vpox_log(hPAM, "pam_vpox_init: guest lib initialized\n");
    }
...
#endif
        pam_vpox_log(hPAM, "pam_vpox_init: rhost=%s, tty=%s, prompt=%s\n",
 ...
 * @param   hPAM                    PAM handle.
 */
static void pam_vpox_shutdown(pam_handle_t *hPAM)
{
    RT_NOREF1(hPAM);
    VbglR3Term();
}
 ...
 * @param   hPAM                    PAM handle.
 */
static int pam_vpox_check_creds(pam_handle_t *hPAM)
{
    int rc = VbglR3CredentialsQueryAvailability();
    if (RT_FAILURE(rc))
    {
        if (rc != VERR_NOT_FOUND)
            pam_vpox_error(hPAM, "pam_vpox_check_creds: could not query for credentials! rc=%Rrc. Aborting\n", rc);
#ifdef DEBUG
        else
            pam_vpox_log(hPAM, "pam_vpox_check_creds: no credentials available\n");
#endif
    }
    else
    {
        ...
        if (RT_FAILURE(rc))
        {
            pam_vpox_error(hPAM, "pam_vpox_check_creds: could not retrieve credentials! rc=%Rrc. Aborting\n", rc);
        }
        else
        {
#ifdef DEBUG
            pam_vpox_log(hPAM, "pam_vpox_check_creds: credentials retrieved: user=%s, password=%s, domain=%s\n",
                         pszUsername, pszPassword, pszDomain);
#else
            /* Don't log passwords in release mode! */
            pam_vpox_log(hPAM, "pam_vpox_check_creds: credentials retrieved: user=%s, password=XXX, domain=%s\n",
            ...
            if (pamrc != PAM_SUCCESS)
            {
                pam_vpox_error(hPAM, "pam_vpox_check_creds: could not set user name! pamrc=%d, msg=%s. Aborting\n",
            ...
            else
            {
                ...
                if (pamrc != PAM_SUCCESS)
                    pam_vpox_error(hPAM, "pam_vpox_check_creds: could not set password! pamrc=%d, msg=%s. Aborting\n",
                                   pamrc, pam_strerror(hPAM, pamrc));
            ...
            VbglR3CredentialsDestroy(pszUsername, pszPassword, pszDomain,
                                     3 /* Three wipe passes */);
            pam_vpox_log(hPAM, "pam_vpox_check_creds: returned with pamrc=%d, msg=%s\n",
                         pamrc, pam_strerror(hPAM, pamrc));
        }
    }

#ifdef DEBUG
    pam_vpox_log(hPAM, "pam_vpox_check_creds: returned with rc=%Rrc\n", rc);
#endif
    return rc;
}
 ...
 */
static int pam_vpox_read_prop(pam_handle_t *hPAM, uint32_t uClientID,
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (pszFlags)
        {
                ...
                && !RTStrStr(pszFlags, "RDONLYGUEST"))
            {
                ...
                pam_vpox_error(hPAM, "pam_vpox_read_prop: key \"%s\" should be read-only on guest but it is not\n",
                               pszKey);
        ...
        {
            pam_vpox_error(hPAM, "pam_vpox_read_prop: key \"%s\" contains no/wrong flags (%s)\n",
                           pszKey, pszFlags);
            rc = VERR_ACCESS_DENIED;
        }
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            {
                pam_vpox_error(hPAM, "pam_vpox_read_prop: could not store value of key \"%s\"\n",
                               pszKey);
                rc = VERR_INVALID_PARAMETER;
            }

            if (RT_SUCCESS(rc))
                pam_vpox_log(hPAM, "pam_vpox_read_prop: read key \"%s\"=\"%s\"\n",
                             pszKey, pszValue);
        }
    }

    pam_vpox_log(hPAM, "pam_vpox_read_prop: read key \"%s\" with rc=%Rrc\n",
                 pszKey, rc);
    return rc;
}
 ...
 */
static int pam_vpox_wait_prop(pam_handle_t *hPAM, uint32_t uClientID,
 ...
 * @param   pvUser                  Pointer to a PAMVPOXTHREAD structure providing
 *                                  required data used / set by the thread.
 */
static DECLCALLBACK(int) pam_vpox_wait_thread(RTTHREAD hThreadSelf, void *pvUser)
{
    RT_NOREF1(hThreadSelf);
    PPAMVPOXTHREAD pUserData = (PPAMVPOXTHREAD)pvUser;
    AssertPtr(pUserData);
    ...
    if (RT_FAILURE(rc))
    {
        pam_vpox_error(pUserData->hPAM, "pam_vpox_wait_thread: Unable to connect to guest property service, rc=%Rrc\n", rc);
    }
    else
    {
        pam_vpox_log(pUserData->hPAM, "pam_vpox_wait_thread: clientID=%u\n", uClientID);
#endif
        for (;;)
        {
#ifdef VPOX_WITH_GUEST_PROPS
            if (uClientID)
            {
                rc = pam_vpox_wait_prop(pUserData->hPAM, uClientID,
                                        "/VirtualPox/GuestAdd/PAM/CredsWaitAbort",
                                        500 /* Wait 500ms, same as VPoxGINA/VPoxCredProv. */);
                switch (rc)
                {
                    case VINF_SUCCESS:
                    ...
                    case VERR_INTERRUPTED:
                        pam_vpox_error(pUserData->hPAM, "pam_vpox_wait_thread: The abort notification request timed out or was interrupted\n");
                        break;
                    ...
                    case VERR_TOO_MUCH_DATA:
                        pam_vpox_error(pUserData->hPAM, "pam_vpox_wait_thread: Temporarily unable to get abort notification\n");
                        break;

                    default:
                        pam_vpox_error(pUserData->hPAM, "pam_vpox_wait_thread: The abort notification request failed with rc=%Rrc\n", rc);
                        break;
                }

                if (RT_SUCCESS(rc)) /* Abort waiting. */
                {
                    pam_vpox_log(pUserData->hPAM, "pam_vpox_wait_thread: Got notification to abort waiting\n");
                    rc = VERR_CANCELLED;
                    break;
                }
            }
            ...
            if (   RT_SUCCESS(rc)
                || rc == VERR_TIMEOUT)
            {
                rc = pam_vpox_check_creds(pUserData->hPAM);
                if (RT_SUCCESS(rc))
                {
            ...
            {
                pam_vpox_log(pUserData->hPAM, "pam_vpox_wait_thread: Waiting thread has reached timeout (%dms), exiting ...\n",
    ...
    pam_vpox_log(pUserData->hPAM, "pam_vpox_wait_thread: Waiting thread returned with rc=%Rrc\n", rc);
    return rc;
}
 ...
 *                                  RT_INDEFINITE_WAIT to wait indefinitly.
 */
static int pam_vpox_wait_for_creds(pam_handle_t *hPAM, uint32_t uClientID, uint32_t uTimeoutMS)
{
    RT_NOREF1(uClientID);
    PAMVPOXTHREAD threadData;
    threadData.hPAM = hPAM;
    threadData.uTimeoutMS = uTimeoutMS;

    RTTHREAD threadWait;
    int rc = RTThreadCreate(&threadWait, pam_vpox_wait_thread,
                            (void *)&threadData, 0,
                            RTTHREADTYPE_DEFAULT, 0 /* Flags */, "pam_vpox");
    if (RT_SUCCESS(rc))
    {
        pam_vpox_log(hPAM, "pam_vpox_wait_for_creds: Waiting for credentials (%dms) ...\n", uTimeoutMS);
        ...
        pam_vpox_error(hPAM, "pam_vpox_wait_for_creds: Creating thread failed with rc=%Rrc\n", rc);

    pam_vpox_log(hPAM, "pam_vpox_wait_for_creds: Waiting for credentials returned with rc=%Rrc\n", rc);
    return rc;
}
...
DECLEXPORT(int) pam_sm_authenticate(pam_handle_t *hPAM, int iFlags, int argc, const char **argv)
{
    ...
    for (int i = 0; i < argc; i++)
    {
        if (!RTStrICmp(argv[i], "debug"))
            ...
            pam_vpox_error(hPAM, "pam_vpox_authenticate: unknown command line argument \"%s\"\n", argv[i]);
    }
    pam_vpox_log(hPAM, "pam_vpox_authenticate called\n");

    int rc = pam_vpox_init(hPAM);
    if (RT_FAILURE(rc))
        return PAM_SUCCESS; /* Jump out as early as we can to not mess around. */
    ...
    {
        char szVal[256];
        rc = pam_vpox_read_prop(hPAM, uClientId,
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            int rc2 = pam_vpox_read_prop(hPAM, uClientId,
            ...
            if (RT_SUCCESS(rc2))
            {
                ...
                if (!uTimeoutMS)
                {
                    pam_vpox_error(hPAM, "pam_vpox_authenticate: invalid waiting timeout value specified, defaulting to infinite timeout\n");
            ...
            rc2 = pam_vpox_read_prop(hPAM, uClientId,
            ...
            rc2 = vpox_set_msg(hPAM, 0 /* Info message */,
                               pszWaitMsg ? pszWaitMsg : "Waiting for credentials ...");
            if (RT_FAILURE(rc2)) /* Not critical. */
                pam_vpox_error(hPAM, "pam_vpox_authenticate: error setting waiting information message, rc=%Rrc\n", rc2);
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                rc = pam_vpox_check_creds(hPAM);
                if (rc == VERR_NOT_FOUND)
                {
                    rc = pam_vpox_wait_for_creds(hPAM, uClientId, uTimeoutMS);
                    if (rc == VERR_TIMEOUT)
                    {
                        pam_vpox_log(hPAM, "pam_vpox_authenticate: no credentials given within time\n");
                        ...
                        if (RT_SUCCESS(rc2))
                        {
                            rc2 = vpox_set_msg(hPAM, 0 /* Info message */, szVal);
                            AssertRC(rc2);
                        }
                    }
                    else if (rc == VERR_CANCELLED)
                    {
                        pam_vpox_log(hPAM, "pam_vpox_authenticate: waiting aborted\n");
    ...
    if (fFallback)
    {
        pam_vpox_log(hPAM, "pam_vpox_authenticate: falling back to old method\n");
        ...
        int rc2 = pam_vpox_check_creds(hPAM);
        if (RT_SUCCESS(rc))
            rc = rc2;
    }

    pam_vpox_shutdown(hPAM);

    pam_vpox_log(hPAM, "pam_vpox_authenticate: overall result rc=%Rrc\n", rc);
...
{
    pam_vpox_log(hPAM, "pam_vpox_setcred called, iFlags=0x%x\n", iFlags);
    for (int i = 0; i < argc; i++)
        pam_vpox_log(hPAM, "pam_vpox_setcred: argv[%d] = %s\n", i, argv[i]);
    return PAM_SUCCESS;
}
...
DECLEXPORT(int) pam_sm_acct_mgmt(pam_handle_t *hPAM, int iFlags, int argc, const char **argv)
{
    ...
    pam_vpox_log(hPAM, "pam_vpox_acct_mgmt called\n");
    return PAM_SUCCESS;
}
...
{
    RT_NOREF3(iFlags, argc, argv);
    pam_vpox_log(hPAM, "pam_vpox_open_session called\n");
    RTPrintf("This session was provided by VirtualPox Guest Additions. Have a lot of fun!\n");
    return PAM_SUCCESS;
}
...
{
    RT_NOREF3(iFlags, argc, argv);
    pam_vpox_log(hPAM, "pam_vpox_close_session called\n");
    return PAM_SUCCESS;
}
...
{
    RT_NOREF3(iFlags, argc, argv);
    pam_vpox_log(hPAM, "pam_vpox_sm_chauthtok called\n");
    return PAM_SUCCESS;
}
