...
#include "gallium/VPoxMPGaWddm.h"
#endif

#include <stdio.h>

#ifdef DEBUG
DWORD g_VPoxLogUm = VPOXWDDM_CFG_LOG_UM_BACKDOOR;
#else
DWORD g_VPoxLogUm = 0;
#endif
...
DWORD g_VPoxDisplayOnly = 0;

#define VPOXWDDM_MEMTAG 'MDBV'
PVOID vpoxWddmMemAlloc(IN SIZE_T cbSize)
{
    POOL_TYPE enmPoolType = (VPoxQueryWinVersion(NULL) >= WINVERSION_8) ? NonPagedPoolNx : NonPagedPool;
    return ExAllocatePoolWithTag(enmPoolType, cbSize, VPOXWDDM_MEMTAG);
}

PVOID vpoxWddmMemAllocZero(IN SIZE_T cbSize)
{
    PVOID pvMem = vpoxWddmMemAlloc(cbSize);
    if (pvMem)
...
DECLINLINE(void) VPoxWddmOaHostIDReleaseLocked(PVPOXWDDM_OPENALLOCATION pOa)
{
    Assert(pOa->cHostIDRefs);
    PVPOXWDDM_ALLOCATION pAllocation = pOa->pAllocation;
...
DECLINLINE(void) VPoxWddmOaHostIDCheckReleaseLocked(PVPOXWDDM_OPENALLOCATION pOa)
{
    if (pOa->cHostIDRefs)
        VPoxWddmOaHostIDReleaseLocked(pOa);
}

DECLINLINE(void) VPoxWddmOaRelease(PVPOXWDDM_OPENALLOCATION pOa)
{
    PVPOXWDDM_ALLOCATION pAllocation = pOa->pAllocation;
    KIRQL OldIrql;
    KeAcquireSpinLock(&pAllocation->OpenLock, &OldIrql);
    Assert(pAllocation->cOpens);
    VPoxWddmOaHostIDCheckReleaseLocked(pOa);
    --pAllocation->cOpens;
...
DECLINLINE(PVPOXWDDM_OPENALLOCATION) VPoxWddmOaSearchLocked(PVPOXWDDM_DEVICE pDevice, PVPOXWDDM_ALLOCATION pAllocation)
{
    for (PLIST_ENTRY pCur = pAllocation->OpenList.Flink; pCur != &pAllocation->OpenList; pCur = pCur->Flink)
    {
        PVPOXWDDM_OPENALLOCATION pCurOa = CONTAINING_RECORD(pCur, VPOXWDDM_OPENALLOCATION, ListEntry);
...
DECLINLINE(PVPOXWDDM_OPENALLOCATION) VPoxWddmOaSearch(PVPOXWDDM_DEVICE pDevice, PVPOXWDDM_ALLOCATION pAllocation)
{
    PVPOXWDDM_OPENALLOCATION pOa;
    KIRQL OldIrql;
    KeAcquireSpinLock(&pAllocation->OpenLock, &OldIrql);
    pOa = VPoxWddmOaSearchLocked(pDevice, pAllocation);
    KeReleaseSpinLock(&pAllocation->OpenLock, OldIrql);
    return pOa;
}

DECLINLINE(int) VPoxWddmOaSetHostID(PVPOXWDDM_DEVICE pDevice, PVPOXWDDM_ALLOCATION pAllocation, uint32_t hostID, uint32_t *pHostID)
{
    PVPOXWDDM_OPENALLOCATION pOa;
    KIRQL OldIrql;
    int rc = VINF_SUCCESS;
...
DECLINLINE(PVPOXWDDM_ALLOCATION) vpoxWddmGetAllocationFromHandle(PVPOXMP_DEVEXT pDevExt, D3DKMT_HANDLE hAllocation)
{
    DXGKARGCB_GETHANDLEDATA GhData;
    GhData.hObject = hAllocation;
    GhData.Type = DXGK_HANDLE_ALLOCATION;
    GhData.Flags.Value = 0;
    return (PVPOXWDDM_ALLOCATION)pDevExt->u.primary.DxgkInterface.DxgkCbGetHandleData(&GhData);
}

int vpoxWddmGhDisplayPostInfoScreen(PVPOXMP_DEVEXT pDevExt, const VPOXWDDM_ALLOC_DATA *pAllocData, const POINT * pVScreenPos, uint16_t fFlags)
{
    VBVAINFOSCREEN RT_UNTRUSTED_VOLATILE_HOST *pScreen =
        (VBVAINFOSCREEN RT_UNTRUSTED_VOLATILE_HOST *)VPoxHGSMIBufferAlloc(&VPoxCommonFromDeviceExt(pDevExt)->guestCtx,
                                                                          sizeof(VBVAINFOSCREEN),
                                                                          HGSMI_CH_VBVA,
                                                                          VBVA_INFO_SCREEN);
    if (!pScreen != NULL)
    {
        WARN(("VPoxHGSMIBufferAlloc failed"));
        return VERR_OUT_OF_RESOURCES;
    }

    int rc = vpoxWddmScreenInfoInit(pScreen, pAllocData, pVScreenPos, fFlags);
    if (RT_SUCCESS(rc))
    {
        pScreen->u32StartOffset = 0; //(uint32_t)offVram; /* we pretend the view is located at the start of each framebuffer */

        rc = VPoxHGSMIBufferSubmit(&VPoxCommonFromDeviceExt(pDevExt)->guestCtx, pScreen);
        if (RT_FAILURE(rc))
            WARN(("VPoxHGSMIBufferSubmit failed %d", rc));
    }
    else
        WARN(("VPoxHGSMIBufferSubmit failed %d", rc));

    VPoxHGSMIBufferFree(&VPoxCommonFromDeviceExt(pDevExt)->guestCtx, pScreen);

    return rc;
}

int vpoxWddmGhDisplayPostInfoView(PVPOXMP_DEVEXT pDevExt, const VPOXWDDM_ALLOC_DATA *pAllocData)
{
    ...
    VBVAINFOVIEW RT_UNTRUSTED_VOLATILE_HOST *pView =
        (VBVAINFOVIEW RT_UNTRUSTED_VOLATILE_HOST *)VPoxHGSMIBufferAlloc(&VPoxCommonFromDeviceExt(pDevExt)->guestCtx,
    ...
    pView->u32ViewSize      = vpoxWddmVramCpuVisibleSegmentSize(pDevExt)/VPoxCommonFromDeviceExt(pDevExt)->cDisplays;
    pView->u32MaxScreenSize = pView->u32ViewSize;

    int rc = VPoxHGSMIBufferSubmit (&VPoxCommonFromDeviceExt(pDevExt)->guestCtx, pView);
    if (RT_FAILURE(rc))
        WARN(("VPoxHGSMIBufferSubmit failed %d", rc));

    VPoxHGSMIBufferFree(&VPoxCommonFromDeviceExt(pDevExt)->guestCtx, pView);
    return rc;
}

NTSTATUS vpoxWddmGhDisplayPostResizeLegacy(PVPOXMP_DEVEXT pDevExt, const VPOXWDDM_ALLOC_DATA *pAllocData, const POINT * pVScreenPos, uint16_t fFlags)
{
    int rc;

    if (!(fFlags & (VBVA_SCREEN_F_DISABLED | VBVA_SCREEN_F_BLANK2)))
    {
        rc = vpoxWddmGhDisplayPostInfoView(pDevExt, pAllocData);
        if (RT_FAILURE(rc))
        {
            WARN(("vpoxWddmGhDisplayPostInfoView failed %d", rc));
            return STATUS_UNSUCCESSFUL;
        }
    }

    rc = vpoxWddmGhDisplayPostInfoScreen(pDevExt, pAllocData, pVScreenPos, fFlags);
    if (RT_FAILURE(rc))
    {
        WARN(("vpoxWddmGhDisplayPostInfoScreen failed %d", rc));
        return STATUS_UNSUCCESSFUL;
    }
...
NTSTATUS vpoxWddmGhDisplayPostResizeNew(PVPOXMP_DEVEXT pDevExt, const VPOXWDDM_ALLOC_DATA *pAllocData, const uint32_t *pTargetMap, const POINT * pVScreenPos, uint16_t fFlags)
{
...
NTSTATUS vpoxWddmGhDisplaySetMode(PVPOXMP_DEVEXT pDevExt, const VPOXWDDM_ALLOC_DATA *pAllocData)
{
    RT_NOREF(pDevExt);
    VPOXVIDEOOFFSET offVram = vpoxWddmAddrFramOffset(&pAllocData->Addr);;
    if (offVram == VPOXVIDEOOFFSET_VOID)
    ...
    ULONG cbLine  = VPOXWDDM_ROUNDBOUND(((width * bpp) + 7) / 8, 4);
    ...
    VPoxVideoSetModeRegisters(width, height, width, bpp, 0, (uint16_t)xOffset, (uint16_t)yOffset);
    /** @todo read back from port to check if mode switch was successful */

    return STATUS_SUCCESS;
}

static uint16_t vpoxWddmCalcScreenFlags(PVPOXMP_DEVEXT pDevExt, bool fValidAlloc, bool fPowerOff, bool fDisabled)
{
    uint16_t u16Flags;
    ...
    else
    {
        if (   !fDisabled
            && fPowerOff
            && RT_BOOL(VPoxCommonFromDeviceExt(pDevExt)->u16SupportedScreenFlags & VBVA_SCREEN_F_BLANK2))
        {
...
NTSTATUS vpoxWddmGhDisplaySetInfoLegacy(PVPOXMP_DEVEXT pDevExt, const VPOXWDDM_ALLOC_DATA *pAllocData, const POINT * pVScreenPos, uint8_t u8CurCyncState, bool fPowerOff, bool fDisabled)
{
    RT_NOREF(u8CurCyncState);
    NTSTATUS Status = STATUS_SUCCESS;
    bool fValidAlloc = pAllocData->SurfDesc.width > 0 && pAllocData->SurfDesc.height > 0;
    uint16_t fu16Flags = vpoxWddmCalcScreenFlags(pDevExt, fValidAlloc, fPowerOff, fDisabled);

    if (fValidAlloc)
    {
        if (pAllocData->SurfDesc.VidPnSourceId == 0)
            Status = vpoxWddmGhDisplaySetMode(pDevExt, pAllocData);
    }

    if (NT_SUCCESS(Status))
    {
        Status = vpoxWddmGhDisplayPostResizeLegacy(pDevExt, pAllocData, pVScreenPos,
                fu16Flags);
        if (NT_SUCCESS(Status))
        {
            return STATUS_SUCCESS;
            ...
            WARN(("vpoxWddmGhDisplayPostResize failed, Status 0x%x", Status));
    }
    else
        WARN(("vpoxWddmGhDisplaySetMode failed, Status 0x%x", Status));

    return Status;
}

NTSTATUS vpoxWddmGhDisplaySetInfoNew(PVPOXMP_DEVEXT pDevExt, const VPOXWDDM_ALLOC_DATA *pAllocData, const uint32_t *pTargetMap, const POINT * pVScreenPos, uint8_t u8CurCyncState, bool fPowerOff, bool fDisabled)
{
    RT_NOREF(u8CurCyncState);
    NTSTATUS Status = STATUS_SUCCESS;
    bool fValidAlloc = pAllocData->SurfDesc.width > 0 && pAllocData->SurfDesc.height > 0;
    ...
    if (NT_SUCCESS(Status))
    {
        Status = vpoxWddmGhDisplayPostResizeNew(pDevExt, pAllocData, pTargetMap, pVScreenPos, fu16Flags);
        if (NT_SUCCESS(Status))
        {
            return STATUS_SUCCESS;
        }
        else
            WARN(("vpoxWddmGhDisplayPostResizeNew failed, Status 0x%x", Status));
    }
    else
        WARN(("vpoxWddmGhDisplaySetMode failed, Status 0x%x", Status));
...
bool vpoxWddmGhDisplayCheckSetInfoFromSourceNew(PVPOXMP_DEVEXT pDevExt, PVPOXWDDM_SOURCE pSource, bool fReportTargets)
{
    if (pSource->u8SyncState == VPOXWDDM_HGSYNC_F_SYNCED_ALL)
    {
    ...
    NTSTATUS Status = vpoxWddmGhDisplaySetInfoNew(pDevExt, &pSource->AllocData, pTargetMap, &pSource->VScreenPos, pSource->u8SyncState, RT_BOOL(pSource->bBlankedByPowerOff), false);
    if (NT_SUCCESS(Status))
    {
        if (fReportTargets && (pSource->u8SyncState & VPOXWDDM_HGSYNC_F_CHANGED_LOCATION_ONLY) != VPOXWDDM_HGSYNC_F_CHANGED_LOCATION_ONLY)
        {
            VPOXWDDM_TARGET_ITER Iter;
            VPoxVidPnStTIterInit(pSource, pDevExt->aTargets, VPoxCommonFromDeviceExt(pDevExt)->cDisplays, &Iter);
    ...
    WARN(("vpoxWddmGhDisplaySetInfoNew failed, Status (0x%x)", Status));
    return false;
}

bool vpoxWddmGhDisplayCheckSetInfoFromSourceLegacy(PVPOXMP_DEVEXT pDevExt, PVPOXWDDM_SOURCE pSource, bool fReportTargets)
{
    if (!fReportTargets)
        return false;
    ...
    VPOXWDDM_TARGET_ITER Iter;
    VPoxVidPnStTIterInit(pSource, pDevExt->aTargets, VPoxCommonFromDeviceExt(pDevExt)->cDisplays, &Iter);
    uint8_t u8SyncState = VPOXWDDM_HGSYNC_F_SYNCED_ALL;
    VPOXWDDM_ALLOC_DATA AllocData = pSource->AllocData;
            ...
            pTarget = VPoxVidPnStTIterNext(&Iter))
    {
        ...
        NTSTATUS Status = vpoxWddmGhDisplaySetInfoLegacy(pDevExt, &AllocData, &pSource->VScreenPos, pSource->u8SyncState | pTarget->u8SyncState, pTarget->fBlankedByPowerOff, pTarget->fDisabled);
        if (NT_SUCCESS(Status))
            pTarget->u8SyncState = VPOXWDDM_HGSYNC_F_SYNCED_ALL;
        else
        {
            WARN(("vpoxWddmGhDisplaySetInfoLegacy failed, Status (0x%x)", Status));
            u8SyncState = 0;
        }
    }
...
bool vpoxWddmGhDisplayCheckSetInfoFromSourceEx(PVPOXMP_DEVEXT pDevExt, PVPOXWDDM_SOURCE pSource, bool fReportTargets)
{
    if (pDevExt->fCmdVbvaEnabled)
        return vpoxWddmGhDisplayCheckSetInfoFromSourceNew(pDevExt, pSource, fReportTargets);
    return vpoxWddmGhDisplayCheckSetInfoFromSourceLegacy(pDevExt, pSource, fReportTargets);
}
...
bool vpoxWddmGhDisplayCheckSetInfoFromSource(PVPOXMP_DEVEXT pDevExt, PVPOXWDDM_SOURCE pSource)
{
    ...
    return vpoxWddmGhDisplayCheckSetInfoFromSourceEx(pDevExt, pSource, fReportTargets);
}

bool vpoxWddmGhDisplayCheckSetInfoForDisabledTargetsNew(PVPOXMP_DEVEXT pDevExt)
{
    VPOXCMDVBVA_SCREENMAP_DECL(uint32_t, aTargetMap);
    ...
    {
        VPOXWDDM_TARGET *pTarget = &pDevExt->aTargets[i];
        ...
        if (pTarget->u8SyncState != VPOXWDDM_HGSYNC_F_SYNCED_ALL)
        {
    ...
    VPoxVidPnAllocDataInit(&AllocData, D3DDDI_ID_UNINITIALIZED);
    NTSTATUS Status = vpoxWddmGhDisplaySetInfoNew(pDevExt, &AllocData, aTargetMap, &VScreenPos, 0, false, true);
    if (!NT_SUCCESS(Status))
    {
        WARN(("vpoxWddmGhDisplaySetInfoNew failed %#x", Status));
        return false;
    }
...
bool vpoxWddmGhDisplayCheckSetInfoForDisabledTargetsLegacy(PVPOXMP_DEVEXT pDevExt)
{
    POINT VScreenPos = {0};
    bool fFound = false;
    VPOXWDDM_ALLOC_DATA AllocData;
    ...
    for (int i = 0; i < VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
    {
        ...
        NTSTATUS Status = vpoxWddmGhDisplaySetInfoLegacy(pDevExt, &AllocData, &VScreenPos, 0, pTarget->fBlankedByPowerOff, pTarget->fDisabled);
        if (NT_SUCCESS(Status))
            pTarget->u8SyncState = VPOXWDDM_HGSYNC_F_SYNCED_ALL;
        else
            WARN(("vpoxWddmGhDisplaySetInfoLegacy failed, Status (0x%x)", Status));
...
void vpoxWddmGhDisplayCheckSetInfoForDisabledTargets(PVPOXMP_DEVEXT pDevExt)
{
    if (pDevExt->fCmdVbvaEnabled)
        vpoxWddmGhDisplayCheckSetInfoForDisabledTargetsNew(pDevExt);
    else
        vpoxWddmGhDisplayCheckSetInfoForDisabledTargetsLegacy(pDevExt);
}

void vpoxWddmGhDisplayCheckSetInfoForDisabledTargetsCheck(PVPOXMP_DEVEXT pDevExt)
{
    bool fReportTargets = !pDevExt->fDisableTargetUpdate;

    if (fReportTargets)
        vpoxWddmGhDisplayCheckSetInfoForDisabledTargets(pDevExt);
}

void vpoxWddmGhDisplayCheckSetInfoEx(PVPOXMP_DEVEXT pDevExt, bool fReportTargets)
{
    for (int i = 0; i < VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
    {
        VPOXWDDM_SOURCE *pSource = &pDevExt->aSources[i];
        vpoxWddmGhDisplayCheckSetInfoFromSourceEx(pDevExt, pSource, fReportTargets);
    }
...
void vpoxWddmGhDisplayCheckSetInfo(PVPOXMP_DEVEXT pDevExt)
{
    bool fReportTargets = !pDevExt->fDisableTargetUpdate;
    vpoxWddmGhDisplayCheckSetInfoEx(pDevExt, fReportTargets);
}

PVPOXSHGSMI vpoxWddmHgsmiGetHeapFromCmdOffset(PVPOXMP_DEVEXT pDevExt, HGSMIOFFSET offCmd)
{
    if (HGSMIAreaContainsOffset(&VPoxCommonFromDeviceExt(pDevExt)->guestCtx.heapCtx.Heap.area, offCmd))
        return &VPoxCommonFromDeviceExt(pDevExt)->guestCtx.heapCtx;
    return NULL;
}

NTSTATUS vpoxWddmPickResources(PVPOXMP_DEVEXT pDevExt, PDXGK_DEVICE_INFO pDeviceInfo, PVPOXWDDM_HWRESOURCES pHwResources)
{
    RT_NOREF(pDevExt);
    NTSTATUS Status = STATUS_SUCCESS;
    USHORT DispiId;
...
static void vpoxWddmDevExtZeroinit(PVPOXMP_DEVEXT pDevExt, CONST PDEVICE_OBJECT pPDO)
{
    memset(pDevExt, 0, sizeof (VPOXMP_DEVEXT));
    pDevExt->pPDO = pPDO;
    PWCHAR pName = (PWCHAR)(((uint8_t*)pDevExt) + VPOXWDDM_ROUNDBOUND(sizeof(VPOXMP_DEVEXT), 8));
    RtlInitUnicodeString(&pDevExt->RegKeyName, pName);

    VPoxVidPnSourcesInit(pDevExt->aSources, RT_ELEMENTS(pDevExt->aSources), 0);

    VPoxVidPnTargetsInit(pDevExt->aTargets, RT_ELEMENTS(pDevExt->aTargets), 0);

    BOOLEAN f3DSupported = FALSE;
    uint32_t u32 = 0;
    if (VPoxVGACfgAvailable())
    {
        VPoxVGACfgQuery(VBE_DISPI_CFG_ID_3D, &u32, 0);
        f3DSupported = RT_BOOL(u32);

        VPoxVGACfgQuery(VBE_DISPI_CFG_ID_VMSVGA, &u32, 0);
    }

    pDevExt->enmHwType = u32 ? VPOXVIDEO_HWTYPE_VMSVGA : VPOXVIDEO_HWTYPE_VPOX;

    if (pDevExt->enmHwType == VPOXVIDEO_HWTYPE_VPOX)
    {
        pDevExt->f3DEnabled = FALSE;
    }
    else if (pDevExt->enmHwType == VPOXVIDEO_HWTYPE_VMSVGA)
    {
        pDevExt->f3DEnabled = f3DSupported;
    }
    else
    {
        /* No supported 3D hardware, Fallback to VPox 2D only. */
        pDevExt->enmHwType = VPOXVIDEO_HWTYPE_VPOX;
        pDevExt->f3DEnabled = FALSE;
    }
}

static void vpoxWddmSetupDisplaysLegacy(PVPOXMP_DEVEXT pDevExt)
{
    /* For WDDM, we simply store the number of monitors as we will deal with
     * VidPN stuff later */
    int rc = STATUS_SUCCESS;

    if (VPoxCommonFromDeviceExt(pDevExt)->bHGSMI)
    {
        ULONG ulAvailable = VPoxCommonFromDeviceExt(pDevExt)->cbVRAM
                            - VPoxCommonFromDeviceExt(pDevExt)->cbMiniportHeap
                            - VBVA_ADAPTER_INFORMATION_SIZE;

        ULONG ulSize;
        ULONG offset;
        offset = ulAvailable;
        rc = vpoxVdmaCreate (pDevExt, &pDevExt->u.primary.Vdma
                );
        AssertRC(rc);
        if (RT_SUCCESS(rc))
        {
            ...
            rc = vpoxVdmaEnable(pDevExt, &pDevExt->u.primary.Vdma);
            AssertRC(rc);
            if (RT_FAILURE(rc))
                vpoxVdmaDestroy(pDevExt, &pDevExt->u.primary.Vdma);
        }
        ...
        NTSTATUS Status = vpoxVideoAMgrCreate(pDevExt, &pDevExt->AllocMgr, offset, ulSize);
        ...
        if (RT_SUCCESS(rc))
        {
            ulAvailable = offset;
            ulSize = ulAvailable / 2;
            ulSize /= VPoxCommonFromDeviceExt(pDevExt)->cDisplays;
            ...
            Assert(ulSize * VPoxCommonFromDeviceExt(pDevExt)->cDisplays < ulAvailable);

            for (int i = VPoxCommonFromDeviceExt(pDevExt)->cDisplays-1; i >= 0; --i)
            {
                offset -= ulSize;
                rc = vpoxVbvaCreate(pDevExt, &pDevExt->aSources[i].Vbva, offset, ulSize, i);
                AssertRC(rc);
                if (RT_SUCCESS(rc))
                {
                    rc = vpoxVbvaEnable(pDevExt, &pDevExt->aSources[i].Vbva);
                    AssertRC(rc);
                    if (RT_FAILURE(rc))
                    {
                        /** @todo de-initialize */
        ...
        /* vpoxWddmVramCpuVisibleSize uses this value */
        pDevExt->cbVRAMCpuVisible = offset;

        rc = VPoxMPCmnMapAdapterMemory(VPoxCommonFromDeviceExt(pDevExt), (void**)&pDevExt->pvVisibleVram,
                                       0, vpoxWddmVramCpuVisibleSize(pDevExt));
        Assert(rc == VINF_SUCCESS);
        if (rc != VINF_SUCCESS)
            pDevExt->pvVisibleVram = NULL;

        if (RT_FAILURE(rc))
            VPoxCommonFromDeviceExt(pDevExt)->bHGSMI = FALSE;
    }
}

static NTSTATUS vpoxWddmSetupDisplaysNew(PVPOXMP_DEVEXT pDevExt)
{
    if (!VPoxCommonFromDeviceExt(pDevExt)->bHGSMI)
        return STATUS_UNSUCCESSFUL;

    ULONG cbAvailable = VPoxCommonFromDeviceExt(pDevExt)->cbVRAM
                            - VPoxCommonFromDeviceExt(pDevExt)->cbMiniportHeap
                            - VBVA_ADAPTER_INFORMATION_SIZE;
    ...
    int rc = VPoxMPCmnMapAdapterMemory(VPoxCommonFromDeviceExt(pDevExt), (void**)&pDevExt->pvVisibleVram,
                                   0, vpoxWddmVramCpuVisibleSize(pDevExt));
    if (RT_FAILURE(rc))
    {
...
static NTSTATUS vpoxWddmSetupDisplays(PVPOXMP_DEVEXT pDevExt)
{
    if (pDevExt->fCmdVbvaEnabled)
    {
        NTSTATUS Status = vpoxWddmSetupDisplaysNew(pDevExt);
        if (!NT_SUCCESS(Status))
            VPoxCommonFromDeviceExt(pDevExt)->bHGSMI = FALSE;
        return Status;
    }

    vpoxWddmSetupDisplaysLegacy(pDevExt);
    return VPoxCommonFromDeviceExt(pDevExt)->bHGSMI ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
}

static int vpoxWddmFreeDisplays(PVPOXMP_DEVEXT pDevExt)
{
    int rc = VINF_SUCCESS;

    Assert(pDevExt->pvVisibleVram);
    if (pDevExt->pvVisibleVram)
        VPoxMPCmnUnmapAdapterMemory(VPoxCommonFromDeviceExt(pDevExt), (void**)&pDevExt->pvVisibleVram);

    if (!pDevExt->fCmdVbvaEnabled)
    {
        for (int i = VPoxCommonFromDeviceExt(pDevExt)->cDisplays-1; i >= 0; --i)
        {
            rc = vpoxVbvaDisable(pDevExt, &pDevExt->aSources[i].Vbva);
            AssertRC(rc);
            if (RT_SUCCESS(rc))
            {
                rc = vpoxVbvaDestroy(pDevExt, &pDevExt->aSources[i].Vbva);
                AssertRC(rc);
                if (RT_FAILURE(rc))
                {
                    /** @todo */
        ...
        vpoxVideoAMgrDestroy(pDevExt, &pDevExt->AllocMgr);

        rc = vpoxVdmaDisable(pDevExt, &pDevExt->u.primary.Vdma);
        AssertRC(rc);
        if (RT_SUCCESS(rc))
        {
            rc = vpoxVdmaDestroy(pDevExt, &pDevExt->u.primary.Vdma);
            AssertRC(rc);
        }
    }
    ...
    )
{
    ...
    if (Status == STATUS_SUCCESS)
    {
        pDevExt = (PVPOXMP_DEVEXT)vpoxWddmMemAllocZero(VPOXWDDM_ROUNDBOUND(sizeof(VPOXMP_DEVEXT), 8) + cbRegKeyBuf);
        if (pDevExt)
        {
            PWCHAR pName = (PWCHAR)(((uint8_t*)pDevExt) + VPOXWDDM_ROUNDBOUND(sizeof(VPOXMP_DEVEXT), 8));
            memcpy(pName, RegKeyBuf, cbRegKeyBuf);
            vpoxWddmDevExtZeroinit(pDevExt, PhysicalDeviceObject);
    ...
    )
{
        ...
        )
    {
        ...
        if (Status == STATUS_SUCCESS)
        {
            Status = vpoxWddmPickResources(pDevExt, &DeviceInfo, &pDevExt->HwResources);
            if (Status == STATUS_SUCCESS)
            {
                 ...
                 */
                VPoxSetupDisplaysHGSMI(VPoxCommonFromDeviceExt(pDevExt),
                ...
                if (VPoxCommonFromDeviceExt(pDevExt)->bHGSMI)
                {
                    vpoxWddmSetupDisplays(pDevExt);
                    if (!VPoxCommonFromDeviceExt(pDevExt)->bHGSMI)
                        VPoxFreeDisplaysHGSMI(VPoxCommonFromDeviceExt(pDevExt));
                }
                if (VPoxCommonFromDeviceExt(pDevExt)->bHGSMI)
                {
                    LOGREL(("using HGSMI"));
                    *NumberOfVideoPresentSources = VPoxCommonFromDeviceExt(pDevExt)->cDisplays;
                    *NumberOfChildren = VPoxCommonFromDeviceExt(pDevExt)->cDisplays;
                    LOG(("sources(%d), children(%d)", *NumberOfVideoPresentSources, *NumberOfChildren));

                    vpoxVdmaDdiNodesInit(pDevExt);
                    vpoxVideoCmInit(&pDevExt->CmMgr);
                    vpoxVideoCmInit(&pDevExt->SeamlessCtxMgr);
                    ...
                    VPOXWDDM_CTXLOCK_INIT(pDevExt);
                    KeInitializeSpinLock(&pDevExt->SynchLock);

                    VPoxCommonFromDeviceExt(pDevExt)->fAnyX = VPoxVideoAnyWidthAllowed();
#if 0
                    vpoxShRcTreeInit(pDevExt);
#endif

#ifdef VPOX_WITH_VIDEOHWACCEL
                    vpoxVhwaInit(pDevExt);
#endif
                    VPoxWddmSlInit(pDevExt);
                    ...
                    DWORD dwVal = VPOXWDDM_CFG_DRV_DEFAULT;
                    HANDLE hKey = NULL;
                    WCHAR aNameBuf[100];
                    ...
                    if (hKey)
                    {
                        ...
                        if (!NT_SUCCESS(Status))
                        {
                            LOG(("vpoxWddmRegQueryValueDword failed, Status = 0x%x", Status));
                            dwVal = VPOXWDDM_CFG_DRV_DEFAULT;
                        }
                    }
                    ...
                    for (UINT i = 0; i < (UINT)VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
                    {
                        ...
                        if (i == 0 || (pDevExt->dwDrvCfgFlags & VPOXWDDM_CFG_DRV_SECONDARY_TARGETS_CONNECTED) || !hKey)
                        {
                        ...
                        {
                            swprintf(aNameBuf, L"%s%d", VPOXWDDM_REG_DRV_DISPFLAGS_PREFIX, i);
                            Status = vpoxWddmRegQueryValueDword(hKey, aNameBuf, &dwVal);
                            if (NT_SUCCESS(Status))
                            {
                                pTarget->fConnected = !!(dwVal & VPOXWDDM_CFG_DRVTARGET_CONNECTED);
                                pTarget->fConfigured = true;
                            }
                            else
                            {
                    ...
                    if (VPoxQueryWinVersion(NULL) >= WINVERSION_8)
                    {
                        ...
                        if (NT_SUCCESS(Status))
                        {
                            PVPOXWDDM_SOURCE pSource = &pDevExt->aSources[0];
                            ...
                            {
                                PhAddr = VPoxCommonFromDeviceExt(pDevExt)->phVRAM;
                                vpoxWddmDiInitDefault(&DisplayInfo, PhAddr, 0);
                            }
                            else
                            {
                            ...
                            vpoxWddmDiToAllocData(pDevExt, &DisplayInfo, &pSource->AllocData);

                            /* init the rest source infos with some default values */
                            for (UINT i = 1; i < (UINT)VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
                            {
                                PhAddr.QuadPart += pSource->AllocData.SurfDesc.cbSize;
                                PhAddr.QuadPart = ROUND_TO_PAGES(PhAddr.QuadPart);
                                vpoxWddmDiInitDefault(&DisplayInfo, PhAddr, i);
                    ...
                    VPoxWddmVModesInit(pDevExt);

#ifdef VPOX_WITH_MESA3D
                    if (pDevExt->enmHwType == VPOXVIDEO_HWTYPE_VMSVGA)
                    {
            ...
            {
                LOGREL(("vpoxWddmPickResources failed Status(0x%x), returning err", Status));
                Status = STATUS_UNSUCCESSFUL;
            }
        }
        else
    ...
    )
{
    ...
    VPoxWddmSlTerm(pDevExt);

    vpoxVideoCmTerm(&pDevExt->CmMgr);

    vpoxVideoCmTerm(&pDevExt->SeamlessCtxMgr);

    /* do everything we did on DxgkDdiStartDevice in the reverse order */
#ifdef VPOX_WITH_VIDEOHWACCEL
    vpoxVhwaFree(pDevExt);
#endif
#if 0
    vpoxShRcTreeTerm(pDevExt);
#endif

    int rc = vpoxWddmFreeDisplays(pDevExt);
    if (RT_SUCCESS(rc))
        VPoxFreeDisplaysHGSMI(VPoxCommonFromDeviceExt(pDevExt));
    AssertRC(rc);
    if (RT_SUCCESS(rc))
    {
        vpoxWddmVGuidFree(pDevExt);

        VPoxWddmVModesCleanup();
        /* revert back to the state we were right after the DxgkDdiAddDevice */
        vpoxWddmDevExtZeroinit(pDevExt, pDevExt->pPDO);
    }
    else
        Status = STATUS_UNSUCCESSFUL;
    ...
    )
{
    ...
    vpoxWddmMemFree(MiniportDeviceContext);
    ...
    )
{
    ...
    switch (VideoRequestPacket->IoControlCode)
    {
        ...
        case IOCTL_VIDEO_HANDLE_VIDEOPARAMETERS:
        {
            ...
            Result = VPoxVideoResetDevice((PVPOXMP_DEVEXT)HwDeviceExtension,
    ...
    )
{
    ...
    LOGF(("ENTER, context(0x%x)", MiniportDeviceContext));
    Assert(ChildRelationsSize == (VPoxCommonFromDeviceExt(pDevExt)->cDisplays + 1)*sizeof(DXGK_CHILD_DESCRIPTOR));
    for (int i = 0; i < VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
    {
    ...
    )
{
    ...
    if (ChildStatus->ChildUid >= (uint32_t)VPoxCommonFromDeviceExt(pDevExt)->cDisplays)
    {
    ...
    switch (ChildStatus->Type)
    {
        case StatusConnection:
        {
            ...
            VPOXWDDM_TARGET *pTarget = &pDevExt->aTargets[ChildStatus->ChildUid];
    ...
    switch (pQueryAdapterInfo->Type)
    {
        case DXGKQAITYPE_DRIVERCAPS:
        {
...
#endif
            pCaps->MaxPointerWidth  = VPOXWDDM_C_POINTER_MAX_WIDTH;
            pCaps->MaxPointerHeight = VPOXWDDM_C_POINTER_MAX_HEIGHT;
#ifdef VPOX_WITH_MESA3D
            if (pDevExt->enmHwType == VPOXVIDEO_HWTYPE_VPOX)
            {
                pCaps->MaxPointerWidth  = VPOXWDDM_C_POINTER_MAX_WIDTH_LEGACY;
                pCaps->MaxPointerHeight = VPOXWDDM_C_POINTER_MAX_HEIGHT_LEGACY;
            }
#endif
            ...
            if (!g_VPoxDisplayOnly)
            {
                ...
                for (int i = 0; i < VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
                {
                    if ( pDevExt->aSources[i].Vhwa.Settings.fFlags & VPOXVHWA_F_ENABLED)
                ...
                /** @todo this correlates with pCaps->SchedulingCaps.MultiEngineAware */
                pCaps->GpuEngineTopology.NbAsymetricProcessingNodes = VPOXWDDM_NUM_NODES;
#ifdef VPOX_WITH_MESA3D
                if (pDevExt->enmHwType == VPOXVIDEO_HWTYPE_VMSVGA)
                {
...
# define VPOXWDDM_SEGMENTS_COUNT 2
                if (!pQsOut->pSegmentDescriptor)
                {
                    /* we are requested to provide the number of segments we support */
                    pQsOut->NbSegment = VPOXWDDM_SEGMENTS_COUNT;
                }
                else if (pQsOut->NbSegment != VPOXWDDM_SEGMENTS_COUNT)
                {
                ...
                else
                {
                    ...
                    pDr->CpuTranslatedAddress = VPoxCommonFromDeviceExt(pDevExt)->phVRAM;
                    /* make sure the size is page aligned */
                    /** @todo need to setup VBVA buffers and adjust the mem size here */
                    pDr->Size = vpoxWddmVramCpuVisibleSegmentSize(pDevExt);
                    pDr->NbOfBanks = 0;
                    ...
                    /* make sure the size is page aligned */
                    /** @todo need to setup VBVA buffers and adjust the mem size here */
                    pDr->Size = vpoxWddmVramCpuInvisibleSegmentSize(pDevExt);
                    pDr->NbOfBanks = 0;
                    pDr->pBankRangeTable = 0;
                    pDr->CommitLimit = pDr->Size;
                    pDr->Flags.Value = 0;
        ...
        case DXGKQAITYPE_UMDRIVERPRIVATE:
            if (!g_VPoxDisplayOnly)
            {
                if (pQueryAdapterInfo->OutputDataSize >= sizeof(VPOXWDDM_QAI))
                {
                    VPOXWDDM_QAI *pQAI = (VPOXWDDM_QAI *)pQueryAdapterInfo->pOutputData;
                    memset(pQAI, 0, sizeof(VPOXWDDM_QAI));

                    pQAI->u32Version = VPOXVIDEOIF_VERSION;
                    pQAI->enmHwType = pDevExt->enmHwType;
                    pQAI->u32AdapterCaps = pDevExt->f3DEnabled ? VPOXWDDM_QAI_CAP_3D : 0;
                    pQAI->u32AdapterCaps |= VPOXWDDM_QAI_CAP_DXVA; /** @todo Fetch from registry. */
                    if (VPoxQueryWinVersion(NULL) >= WINVERSION_7)
                    {
                        // pQAI->u32AdapterCaps |= VPOXWDDM_QAI_CAP_WIN7;
                        // pQAI->u32AdapterCaps |= VPOXWDDM_QAI_CAP_DXVAHD; /** @todo Fetch from registry. */
                    }
...
#ifdef VPOX_WITH_VIDEOHWACCEL
                    pQAI->cInfos = VPoxCommonFromDeviceExt(pDevExt)->cDisplays;
                ...
                {
                    WARN(("incorrect buffer size %d, expected %d", pQueryAdapterInfo->OutputDataSize, sizeof(VPOXWDDM_QAI)));
    ...
    DXGKARG_CREATEDEVICE*  pCreateDevice)
{
    ...
    PVPOXWDDM_DEVICE pDevice = (PVPOXWDDM_DEVICE)vpoxWddmMemAllocZero(sizeof (VPOXWDDM_DEVICE));
    if (!pDevice)
    {
        WARN(("vpoxWddmMemAllocZero failed for WDDM device structure"));
    ...
    if (pCreateDevice->Flags.SystemDevice)
        pDevice->enmType = VPOXWDDM_DEVICE_TYPE_SYSTEM;
...
PVPOXWDDM_RESOURCE vpoxWddmResourceCreate(PVPOXMP_DEVEXT pDevExt, PVPOXWDDM_RCINFO pRcInfo)
{
    RT_NOREF(pDevExt);
    PVPOXWDDM_RESOURCE pResource = (PVPOXWDDM_RESOURCE)vpoxWddmMemAllocZero(RT_UOFFSETOF_DYN(VPOXWDDM_RESOURCE,
...
VOID vpoxWddmResourceRetain(PVPOXWDDM_RESOURCE pResource)
{
    ASMAtomicIncU32(&pResource->cRefs);
}

static VOID vpoxWddmResourceDestroy(PVPOXWDDM_RESOURCE pResource)
{
    vpoxWddmMemFree(pResource);
}

VOID vpoxWddmResourceWaitDereference(PVPOXWDDM_RESOURCE pResource)
{
    vpoxWddmCounterU32Wait(&pResource->cRefs, 1);
}

VOID vpoxWddmResourceRelease(PVPOXWDDM_RESOURCE pResource)
{
    uint32_t cRefs = ASMAtomicDecU32(&pResource->cRefs);
    Assert(cRefs < UINT32_MAX/2);
    if (!cRefs)
    {
        vpoxWddmResourceDestroy(pResource);
    }
}

void vpoxWddmAllocationDeleteFromResource(PVPOXWDDM_RESOURCE pResource, PVPOXWDDM_ALLOCATION pAllocation)
{
    Assert(pAllocation->pResource == pResource);
    if (pResource)
    {
        Assert(&pResource->aAllocations[pAllocation->iIndex] == pAllocation);
        vpoxWddmResourceRelease(pResource);
    }
    else
    {
        vpoxWddmMemFree(pAllocation);
    }
}

VOID vpoxWddmAllocationCleanupAssignment(PVPOXMP_DEVEXT pDevExt, PVPOXWDDM_ALLOCATION pAllocation)
{
    switch (pAllocation->enmType)
    {
        case VPOXWDDM_ALLOC_TYPE_STD_SHAREDPRIMARYSURFACE:
        case VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC:
        {
            if (pAllocation->bAssigned)
            {
                /** @todo do we need to notify host? */
                vpoxWddmAssignPrimary(&pDevExt->aSources[pAllocation->AllocData.SurfDesc.VidPnSourceId], NULL, pAllocation->AllocData.SurfDesc.VidPnSourceId);
            }
            break;
        }
        default:
...
VOID vpoxWddmAllocationCleanup(PVPOXMP_DEVEXT pDevExt, PVPOXWDDM_ALLOCATION pAllocation)
{
    RT_NOREF(pDevExt);
    switch (pAllocation->enmType)
    {
        ...
        {
#if 0
            if (pAllocation->enmType == VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC)
            {
                if (pAllocation->hSharedHandle)
                {
                    vpoxShRcTreeRemove(pDevExt, pAllocation);
                }
            }
#endif
            break;
        }
        case VPOXWDDM_ALLOC_TYPE_UMD_HGSMI_BUFFER:
        {
            break;
        }
        default:
...
VOID vpoxWddmAllocationDestroy(PVPOXWDDM_ALLOCATION pAllocation)
{
    ...
    vpoxWddmAllocationDeleteFromResource(pAllocation->pResource, pAllocation);
}

PVPOXWDDM_ALLOCATION vpoxWddmAllocationCreateFromResource(PVPOXWDDM_RESOURCE pResource, uint32_t iIndex)
{
    PVPOXWDDM_ALLOCATION pAllocation = NULL;
    if (pResource)
    {
        ...
        if (iIndex < pResource->cAllocations)
        {
            ...
            memset(pAllocation, 0, sizeof (VPOXWDDM_ALLOCATION));
        }
        vpoxWddmResourceRetain(pResource);
    }
    else
        pAllocation = (PVPOXWDDM_ALLOCATION)vpoxWddmMemAllocZero(sizeof (VPOXWDDM_ALLOCATION));
...
NTSTATUS vpoxWddmAllocationCreate(PVPOXMP_DEVEXT pDevExt, PVPOXWDDM_RESOURCE pResource, uint32_t iIndex, DXGK_ALLOCATIONINFO* pAllocationInfo)
{
    PAGED_CODE();

    NTSTATUS Status = STATUS_SUCCESS;

    Assert(pAllocationInfo->PrivateDriverDataSize == sizeof (VPOXWDDM_ALLOCINFO));
    if (pAllocationInfo->PrivateDriverDataSize >= sizeof (VPOXWDDM_ALLOCINFO))
    {
        PVPOXWDDM_ALLOCINFO pAllocInfo = (PVPOXWDDM_ALLOCINFO)pAllocationInfo->pPrivateDriverData;
        PVPOXWDDM_ALLOCATION pAllocation = vpoxWddmAllocationCreateFromResource(pResource, iIndex);
        Assert(pAllocation);
        if (pAllocation)
        {
            ...
            pAllocation->AllocData.Addr.offVram = VPOXVIDEOOFFSET_VOID;
            ...
            switch (pAllocInfo->enmType)
            {
                case VPOXWDDM_ALLOC_TYPE_STD_SHAREDPRIMARYSURFACE:
                case VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC:
                case VPOXWDDM_ALLOC_TYPE_STD_SHADOWSURFACE:
                case VPOXWDDM_ALLOC_TYPE_STD_STAGINGSURFACE:
                {
                    ...
                    switch (pAllocInfo->enmType)
                    {
                        ...
                        case VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC:
                            ...
                            if (pAllocInfo->fFlags.Overlay)
                            {
                                ...
                                int rc = vpoxVhwaHlpGetSurfInfo(pDevExt, pAllocation);
                                AssertRC(rc);
                                if (RT_SUCCESS(rc))
                                {
...
#endif
                            {
                                ...
                                if (pAllocInfo->fFlags.SharedResource)
                                {
                                    ...
                                    if (pAllocation->hSharedHandle)
                                    {
                                        vpoxShRcTreePut(pDevExt, pAllocation);
                                    }
#endif
                                }
...
//                    pAllocationInfo->Flags.SynchronousPaging = 1;
            ...
            if (Status != STATUS_SUCCESS)
                vpoxWddmAllocationDeleteFromResource(pResource, pAllocation);
        }
        else
        {
    ...
    {
        LOGREL(("ERROR: PrivateDriverDataSize(%d) less than header size(%d)", pAllocationInfo->PrivateDriverDataSize, sizeof (VPOXWDDM_ALLOCINFO)));
        Status = STATUS_INVALID_PARAMETER;
    }
    ...
    DXGKARG_CREATEALLOCATION*  pCreateAllocation)
{
    ...
    PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)hAdapter;
    NTSTATUS Status = STATUS_SUCCESS;
    PVPOXWDDM_RESOURCE pResource = NULL;

    if (pCreateAllocation->PrivateDriverDataSize)
    {
        Assert(pCreateAllocation->PrivateDriverDataSize == sizeof (VPOXWDDM_RCINFO));
        Assert(pCreateAllocation->pPrivateDriverData);
        if (pCreateAllocation->PrivateDriverDataSize < sizeof (VPOXWDDM_RCINFO))
        {
            WARN(("invalid private data size (%d)", pCreateAllocation->PrivateDriverDataSize));
            return STATUS_INVALID_PARAMETER;
        }

        PVPOXWDDM_RCINFO pRcInfo = (PVPOXWDDM_RCINFO)pCreateAllocation->pPrivateDriverData;
//            Assert(pRcInfo->RcDesc.VidPnSourceId < VPoxCommonFromDeviceExt(pDevExt)->cDisplays);
        ...
        if (VPOXWDDM_TRAILARRAY_MAXELEMENTSU32(VPOXWDDM_RESOURCE, aAllocations) < pRcInfo->cAllocInfos)
        {
            WARN(("number of allocations passed too big (%d), max is (%d)", pRcInfo->cAllocInfos, VPOXWDDM_TRAILARRAY_MAXELEMENTSU32(VPOXWDDM_RESOURCE, aAllocations)));
            return STATUS_INVALID_PARAMETER;
        }

        pResource = (PVPOXWDDM_RESOURCE)vpoxWddmMemAllocZero(RT_UOFFSETOF_DYN(VPOXWDDM_RESOURCE, aAllocations[pRcInfo->cAllocInfos]));
        if (!pResource)
        {
            WARN(("vpoxWddmMemAllocZero failed for (%d) allocations", pRcInfo->cAllocInfos));
            return STATUS_NO_MEMORY;
        }
    ...
    {
        Status = vpoxWddmAllocationCreate(pDevExt, pResource, i, &pCreateAllocation->pAllocationInfo[i]);
        if (Status != STATUS_SUCCESS)
        {
            WARN(("vpoxWddmAllocationCreate(%d) failed, Status(0x%x)", i, Status));
            /* note: i-th allocation is expected to be cleared in a fail handling code above */
            for (UINT j = 0; j < i; ++j)
            {
                PVPOXWDDM_ALLOCATION pAllocation = (PVPOXWDDM_ALLOCATION)pCreateAllocation->pAllocationInfo[j].hAllocation;
                vpoxWddmAllocationCleanup(pDevExt, pAllocation);
                vpoxWddmAllocationDestroy(pAllocation);
            }
            break;
        }
    }
    ...
    CONST DXGKARG_DESTROYALLOCATION*  pDestroyAllocation)
{
    ...
    PVPOXWDDM_RESOURCE pRc = (PVPOXWDDM_RESOURCE)pDestroyAllocation->hResource;
    PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)hAdapter;
    ...
    {
        PVPOXWDDM_ALLOCATION pAlloc = (PVPOXWDDM_ALLOCATION)pDestroyAllocation->pAllocationList[i];
        Assert(pAlloc->pResource == pRc);
        vpoxWddmAllocationCleanupAssignment(pDevExt, pAlloc);
        /* wait for all current allocation-related ops are completed */
        vpoxWddmAllocationCleanup(pDevExt, pAlloc);
        vpoxWddmAllocationDestroy(pAlloc);
    }

    if (pRc)
    {
        /* wait for all current resource-related ops are completed */
        vpoxWddmResourceWaitDereference(pRc);
        vpoxWddmResourceRelease(pRc);
    }
...
//    LOGF(("ENTER, hAdapter(0x%x)", hAdapter));
    ...
    PVPOXWDDM_ALLOCATION pAllocation = (PVPOXWDDM_ALLOCATION)pDescribeAllocation->hAllocation;
    ...
    DXGKARG_GETSTANDARDALLOCATIONDRIVERDATA*  pGetStandardAllocationDriverData)
{
    ...
    NTSTATUS Status = STATUS_SUCCESS;
    PVPOXWDDM_ALLOCINFO pAllocInfo = NULL;
    ...
    switch (pGetStandardAllocationDriverData->StandardAllocationType)
    {
        case D3DKMDT_STANDARDALLOCATION_SHAREDPRIMARYSURFACE:
        {
            ...
            {
                pAllocInfo = (PVPOXWDDM_ALLOCINFO)pGetStandardAllocationDriverData->pAllocationPrivateDriverData;
                memset (pAllocInfo, 0, sizeof (VPOXWDDM_ALLOCINFO));
                pAllocInfo->enmType = VPOXWDDM_ALLOC_TYPE_STD_SHAREDPRIMARYSURFACE;
                ...
                pAllocInfo->SurfDesc.bpp = vpoxWddmCalcBitsPerPixel(pAllocInfo->SurfDesc.format);
                pAllocInfo->SurfDesc.pitch = vpoxWddmCalcPitch(pGetStandardAllocationDriverData->pCreateSharedPrimarySurfaceData->Width, pAllocInfo->SurfDesc.format);
                pAllocInfo->SurfDesc.cbSize = vpoxWddmCalcSize(pAllocInfo->SurfDesc.pitch, pAllocInfo->SurfDesc.height, pAllocInfo->SurfDesc.format);
            ...
            pGetStandardAllocationDriverData->AllocationPrivateDriverDataSize = sizeof (VPOXWDDM_ALLOCINFO);
        ...
        case D3DKMDT_STANDARDALLOCATION_SHADOWSURFACE:
        {
            ...
            UINT bpp = vpoxWddmCalcBitsPerPixel(pGetStandardAllocationDriverData->pCreateShadowSurfaceData->Format);
            Assert(bpp);
            if (bpp != 0)
            {
                UINT Pitch = vpoxWddmCalcPitch(pGetStandardAllocationDriverData->pCreateShadowSurfaceData->Width, pGetStandardAllocationDriverData->pCreateShadowSurfaceData->Format);
                ...
                {
                    pAllocInfo = (PVPOXWDDM_ALLOCINFO)pGetStandardAllocationDriverData->pAllocationPrivateDriverData;
                    pAllocInfo->enmType = VPOXWDDM_ALLOC_TYPE_STD_SHADOWSURFACE;
                    ...
                    pAllocInfo->SurfDesc.bpp = vpoxWddmCalcBitsPerPixel(pAllocInfo->SurfDesc.format);
                    pAllocInfo->SurfDesc.pitch = vpoxWddmCalcPitch(pGetStandardAllocationDriverData->pCreateShadowSurfaceData->Width, pAllocInfo->SurfDesc.format);
                    pAllocInfo->SurfDesc.cbSize = vpoxWddmCalcSize(pAllocInfo->SurfDesc.pitch, pAllocInfo->SurfDesc.height, pAllocInfo->SurfDesc.format);
                    pAllocInfo->SurfDesc.depth = 0;
                    pAllocInfo->SurfDesc.slicePitch = 0;
        ...
        case D3DKMDT_STANDARDALLOCATION_STAGINGSURFACE:
        {
            ...
            {
                pAllocInfo = (PVPOXWDDM_ALLOCINFO)pGetStandardAllocationDriverData->pAllocationPrivateDriverData;
                pAllocInfo->enmType = VPOXWDDM_ALLOC_TYPE_STD_STAGINGSURFACE;
                ...
                pAllocInfo->SurfDesc.bpp = vpoxWddmCalcBitsPerPixel(pAllocInfo->SurfDesc.format);
                pAllocInfo->SurfDesc.pitch = vpoxWddmCalcPitch(pGetStandardAllocationDriverData->pCreateStagingSurfaceData->Width, pAllocInfo->SurfDesc.format);
                pAllocInfo->SurfDesc.cbSize = vpoxWddmCalcSize(pAllocInfo->SurfDesc.pitch, pAllocInfo->SurfDesc.height, pAllocInfo->SurfDesc.format);
                pAllocInfo->SurfDesc.depth = 0;
                pAllocInfo->SurfDesc.slicePitch = 0;
                pAllocInfo->SurfDesc.RefreshRate.Numerator = 0;
...
typedef struct VPOXWDDM_CALL_ISR
{
    PVPOXMP_DEVEXT pDevExt;
    ULONG MessageNumber;
} VPOXWDDM_CALL_ISR, *PVPOXWDDM_CALL_ISR;

static BOOLEAN vpoxWddmCallIsrCb(PVOID Context)
{
    PVPOXWDDM_CALL_ISR pdc = (PVPOXWDDM_CALL_ISR)Context;
 ...
 */
static void vpoxWddmPointerFindDimensionsColor(void const *pPixels, UINT Pitch, UINT Width, UINT Height,
                                               LONG *piMaxFilledPixel, LONG *piMaxFilledScanline)
{
    /* Windows always uses the maximum pointer size VPOXWDDM_C_POINTER_MAX_*
 ...
 */
static void vpoxWddmPointerFindDimensionsMono(void const *pPixels, UINT Pitch, UINT Width, UINT Height, BYTE Blank,
                                              LONG *piMaxFilledPixel, LONG *piMaxFilledScanline)
{
    /* Windows always uses the maximum pointer size VPOXWDDM_C_POINTER_MAX_*
 ...
 */
static void vpoxWddmPointerAdjustDimensions(LONG iMaxX, LONG iMaxY, UINT XHot, UINT YHot,
                                            ULONG *pWidth, ULONG *pHeight)
{
     ...
     */
    W = RT_MIN(W, VPOXWDDM_C_POINTER_MAX_WIDTH);
    H = RT_MIN(H, VPOXWDDM_C_POINTER_MAX_HEIGHT);

    *pWidth = W;
    *pHeight = H;
}

BOOL vpoxWddmPointerCopyColorData(CONST DXGKARG_SETPOINTERSHAPE* pSetPointerShape, PVIDEO_POINTER_ATTRIBUTES pPointerAttributes,
                                  bool fDwordAlignScanlines)
{
    ...
    vpoxWddmPointerFindDimensionsColor(pSetPointerShape->pPixels, pSetPointerShape->Pitch,
                                       pSetPointerShape->Width, pSetPointerShape->Height,
                                       &iMaxFilledPixel, &iMaxFilledScanline);

    vpoxWddmPointerAdjustDimensions(iMaxFilledPixel, iMaxFilledScanline,
    ...
    Assert(VPOXWDDM_POINTER_ATTRIBUTES_SIZE >= cbPointerAttributes);
    if (VPOXWDDM_POINTER_ATTRIBUTES_SIZE < cbPointerAttributes)
    {
        LOGREL(("VPOXWDDM_POINTER_ATTRIBUTES_SIZE(%d) < cbPointerAttributes(%d)", VPOXWDDM_POINTER_ATTRIBUTES_SIZE, cbPointerAttributes));
        return FALSE;
    }
...
BOOL vpoxWddmPointerCopyMonoData(CONST DXGKARG_SETPOINTERSHAPE* pSetPointerShape, PVIDEO_POINTER_ATTRIBUTES pPointerAttributes,
                                 bool fDwordAlignScanlines)
{
    ULONG srcMaskW, srcMaskH;
    ULONG dstBytesPerLine;
    ...
    vpoxWddmPointerFindDimensionsMono(pSetPointerShape->pPixels, pSetPointerShape->Pitch,
    ...
    vpoxWddmPointerFindDimensionsMono((BYTE *)pSetPointerShape->pPixels + pSetPointerShape->Height * pSetPointerShape->Pitch,
...
static BOOLEAN vpoxVddmPointerShapeToAttributes(CONST DXGKARG_SETPOINTERSHAPE* pSetPointerShape, PVPOXWDDM_POINTER_INFO pPointerInfo,
                                                bool fDwordAlignScanlines)
{
    ...
    pPointerAttributes->Enable &= VPOX_MOUSE_POINTER_VISIBLE;

    Assert(pSetPointerShape->Flags.Value == 1 || pSetPointerShape->Flags.Value == 2);
    if (pSetPointerShape->Flags.Color)
    {
        if (vpoxWddmPointerCopyColorData(pSetPointerShape, pPointerAttributes, fDwordAlignScanlines))
        {
            pPointerAttributes->Flags = VIDEO_MODE_COLOR_POINTER;
            pPointerAttributes->Enable |= VPOX_MOUSE_POINTER_ALPHA;
        }
        else
        {
            LOGREL(("vpoxWddmPointerCopyColorData failed"));
            AssertBreakpoint();
            return FALSE;
        }
    ...
    {
        if (vpoxWddmPointerCopyMonoData(pSetPointerShape, pPointerAttributes, fDwordAlignScanlines))
        {
            pPointerAttributes->Flags = VIDEO_MODE_MONO_POINTER;
        }
        else
        {
            LOGREL(("vpoxWddmPointerCopyMonoData failed"));
            AssertBreakpoint();
            return FALSE;
        }
    }
    ...
    pPointerAttributes->Enable |= VPOX_MOUSE_POINTER_SHAPE;
...
bool vpoxWddmUpdatePointerShape(PVPOXMP_DEVEXT pDevExt, PVIDEO_POINTER_ATTRIBUTES pAttrs, uint32_t cbLength)
{
#ifdef VPOX_WITH_MESA3D
    if (pDevExt->enmHwType != VPOXVIDEO_HWTYPE_VPOX)
    {
        ...
        if (fFlags & VPOX_MOUSE_POINTER_SHAPE)
        {
            ...
            if (fFlags & VPOX_MOUSE_POINTER_ALPHA)
            {
        ...
        RT_ZERO(attrs);
        attrs.Enable = pAttrs->Enable & VPOX_MOUSE_POINTER_VISIBLE;
        if (!VPoxMPCmnUpdatePointerShape(VPoxCommonFromDeviceExt(pDevExt), &attrs, sizeof(attrs)))
        {
            Status = STATUS_INVALID_PARAMETER;
        }
    ...
    return VPoxMPCmnUpdatePointerShape(VPoxCommonFromDeviceExt(pDevExt), pAttrs, cbLength);
}

static void vpoxWddmHostPointerEnable(PVPOXMP_DEVEXT pDevExt, BOOLEAN fEnable)
{
    VIDEO_POINTER_ATTRIBUTES PointerAttributes;
    RT_ZERO(PointerAttributes);
    if (fEnable)
    {
        PointerAttributes.Enable = VPOX_MOUSE_POINTER_VISIBLE;
    }
    vpoxWddmUpdatePointerShape(pDevExt, &PointerAttributes, sizeof(PointerAttributes));
}
...
//    LOGF(("ENTER, hAdapter(0x%x)", hAdapter));
    ...
    PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)hAdapter;
    PVPOXWDDM_POINTER_INFO pPointerInfo = &pDevExt->aSources[pSetPointerPosition->VidPnSourceId].PointerInfo;
    PVPOXWDDM_GLOBAL_POINTER_INFO pGlobalPointerInfo = &pDevExt->PointerInfo;
    PVIDEO_POINTER_ATTRIBUTES pPointerAttributes = &pPointerInfo->Attributes.data;
    BOOLEAN fScreenVisState = !!(pPointerAttributes->Enable & VPOX_MOUSE_POINTER_VISIBLE);
    ...
    if ((fVisStateChanged || fScreenChanged) && VPoxQueryHostWantsAbsolute())
    {
        if (fScreenChanged)
        {
            BOOLEAN bResult = vpoxWddmUpdatePointerShape(pDevExt, &pPointerInfo->Attributes.data, VPOXWDDM_POINTER_ATTRIBUTES_SIZE);
            if (!bResult)
            {
                vpoxWddmHostPointerEnable(pDevExt, FALSE);
            }
        }

        // Always update the visibility as requested. Tell the host to use the guest's pointer.
        vpoxWddmHostPointerEnable(pDevExt, pSetPointerPosition->Flags.Visible);
    }
...
//    LOGF(("ENTER, hAdapter(0x%x)", hAdapter));
    ...
    if (VPoxQueryHostWantsAbsolute())
    {
        /* mouse integration is ON */
        PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)hAdapter;
        PVPOXWDDM_POINTER_INFO pPointerInfo = &pDevExt->aSources[pSetPointerShape->VidPnSourceId].PointerInfo;
        bool const fDwordAlignScanlines = pDevExt->enmHwType != VPOXVIDEO_HWTYPE_VPOX;
        ...
        if (vpoxVddmPointerShapeToAttributes(pSetPointerShape, pPointerInfo, fDwordAlignScanlines))
        {
            pDevExt->PointerInfo.iLastReportedScreen = pSetPointerShape->VidPnSourceId;
            if (vpoxWddmUpdatePointerShape(pDevExt, &pPointerInfo->Attributes.data, VPOXWDDM_POINTER_ATTRIBUTES_SIZE))
                Status = STATUS_SUCCESS;
            else
            {
...
//    LOGF(("ENTER, hAdapter(0x%x)", hAdapter));
    ...
    NTSTATUS Status = STATUS_NOT_SUPPORTED;
    PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)hAdapter;
    Assert(pEscape->PrivateDriverDataSize >= sizeof (VPOXDISPIFESCAPE));
    if (pEscape->PrivateDriverDataSize >= sizeof (VPOXDISPIFESCAPE))
    {
        PVPOXDISPIFESCAPE pEscapeHdr = (PVPOXDISPIFESCAPE)pEscape->pPrivateDriverData;
        switch (pEscapeHdr->escapeCode)
        {
            case VPOXESC_SETVISIBLEREGION:
            {
#ifdef VPOX_DISPIF_WITH_OPCONTEXT
                PVPOXWDDM_CONTEXT pContext = (PVPOXWDDM_CONTEXT)pEscape->hContext;
                if (!pContext)
                {
                    WARN(("VPOXESC_SETVISIBLEREGION no context supplied!"));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }

                if (pContext->enmType != VPOXWDDM_CONTEXT_TYPE_CUSTOM_DISPIF_SEAMLESS)
                {
                    WARN(("VPOXESC_SETVISIBLEREGION invalid context supplied %d!", pContext->enmType));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
#endif
                /* visible regions for seamless */
                LPRGNDATA lpRgnData = VPOXDISPIFESCAPE_DATA(pEscapeHdr, RGNDATA);
                uint32_t cbData = VPOXDISPIFESCAPE_DATA_SIZE(pEscape->PrivateDriverDataSize);
                ...
                LOG(("IOCTL_VIDEO_VPOX_SETVISIBLEREGION cRects=%d", cRects));
                ...
                {
                    WARN(("VPOXESC_SETVISIBLEREGION: incorrect buffer size (%d), reported count (%d)", cbRects, lpRgnData->rdh.nCount));
                    Status = STATUS_INVALID_PARAMETER;
                }
                break;
            }
            case VPOXESC_ISVRDPACTIVE:
                ...
                Status = STATUS_SUCCESS;
                break;
            case VPOXESC_CONFIGURETARGETS:
            {
                LOG(("=> VPOXESC_CONFIGURETARGETS"));

                if (!pEscape->Flags.HardwareAccess)
                {
                    WARN(("VPOXESC_CONFIGURETARGETS called without HardwareAccess flag set, failing"));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
                ...
                if (!pContext)
                {
                    WARN(("VPOXESC_CONFIGURETARGETS no context supplied!"));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }

                if (pContext->enmType != VPOXWDDM_CONTEXT_TYPE_CUSTOM_DISPIF_RESIZE)
                {
                    WARN(("VPOXESC_CONFIGURETARGETS invalid context supplied %d!", pContext->enmType));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
#endif
                ...
                {
                    WARN(("VPOXESC_CONFIGURETARGETS invalid private driver size %d", pEscape->PrivateDriverDataSize));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
                ...
                {
                    WARN(("VPOXESC_CONFIGURETARGETS invalid command %d", pEscapeHdr->u32CmdSpecific));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
                ...
                for (int i = 0; i < VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
                {
                    ...
                    {
                        Status = VPoxWddmChildStatusConnect(pDevExt, (uint32_t)i, TRUE);
                        if (NT_SUCCESS(Status))
                            ++cAdjusted;
                        else
                            WARN(("VPOXESC_CONFIGURETARGETS vpoxWddmChildStatusConnectSecondaries failed Status 0x%x\n", Status));
                    }
                    ...
                    if (!hKey)
                    {
                        ...
                        if (!NT_SUCCESS(Status))
                        {
                            WARN(("VPOXESC_CONFIGURETARGETS IoOpenDeviceRegistryKey failed, Status = 0x%x", Status));
                            hKey = NULL;
                            continue;
                        }
                    }
                    ...
                    swprintf(aNameBuf, L"%s%d", VPOXWDDM_REG_DRV_DISPFLAGS_PREFIX, i);
                    Status = vpoxWddmRegSetValueDword(hKey, aNameBuf, VPOXWDDM_CFG_DRVTARGET_CONNECTED);
                    if (!NT_SUCCESS(Status))
                        WARN(("VPOXESC_CONFIGURETARGETS vpoxWddmRegSetValueDword (%d) failed Status 0x%x\n", aNameBuf, Status));
            ...
            case VPOXESC_SETALLOCHOSTID:
            {
                PVPOXWDDM_DEVICE pDevice = (PVPOXWDDM_DEVICE)pEscape->hDevice;
                if (!pDevice)
                {
                    WARN(("VPOXESC_SETALLOCHOSTID called without no device specified, failing"));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }

                if (pEscape->PrivateDriverDataSize != sizeof (VPOXDISPIFESCAPE_SETALLOCHOSTID))
                {
                    WARN(("invalid buffer size for VPOXDISPIFESCAPE_SETALLOCHOSTID, was(%d), but expected (%d)",
                            pEscape->PrivateDriverDataSize, sizeof (VPOXDISPIFESCAPE_SETALLOCHOSTID)));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
                ...
                if (!pEscape->Flags.HardwareAccess)
                {
                    WARN(("VPOXESC_SETALLOCHOSTID not HardwareAccess"));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }

                PVPOXDISPIFESCAPE_SETALLOCHOSTID pSetHostID = (PVPOXDISPIFESCAPE_SETALLOCHOSTID)pEscapeHdr;
                PVPOXWDDM_ALLOCATION pAlloc = vpoxWddmGetAllocationFromHandle(pDevExt, (D3DKMT_HANDLE)pSetHostID->hAlloc);
                if (!pAlloc)
                {
                ...
                pSetHostID->rc = VPoxWddmOaSetHostID(pDevice, pAlloc, pSetHostID->hostID, &pSetHostID->EscapeHdr.u32CmdSpecific);

                if (pAlloc->bAssigned)
                {
                    PVPOXMP_DEVEXT pDevExt = pDevice->pAdapter;
                    Assert(pAlloc->AllocData.SurfDesc.VidPnSourceId < (D3DDDI_VIDEO_PRESENT_SOURCE_ID)VPoxCommonFromDeviceExt(pDevExt)->cDisplays);
                    PVPOXWDDM_SOURCE pSource = &pDevExt->aSources[pAlloc->AllocData.SurfDesc.VidPnSourceId];
                    if (pSource->AllocData.hostID != pAlloc->AllocData.hostID)
                    {
                        ...
                        vpoxWddmGhDisplayCheckSetInfo(pDevExt);
                    }
                }
            ...
            case VPOXESC_ISANYX:
            {
                if (pEscape->PrivateDriverDataSize != sizeof (VPOXDISPIFESCAPE_ISANYX))
                {
                ...
                PVPOXDISPIFESCAPE_ISANYX pIsAnyX = (PVPOXDISPIFESCAPE_ISANYX)pEscapeHdr;
                pIsAnyX->u32IsAnyX = VPoxCommonFromDeviceExt(pDevExt)->fAnyX;
                Status = STATUS_SUCCESS;
                break;
            }
            case VPOXESC_UPDATEMODES:
            {
                LOG(("=> VPOXESC_UPDATEMODES"));

                if (!pEscape->Flags.HardwareAccess)
                {
                    WARN(("VPOXESC_UPDATEMODES called without HardwareAccess flag set, failing"));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
                ...
                if (!pContext)
                {
                    WARN(("VPOXESC_UPDATEMODES no context supplied!"));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
                ...
                if (pContext->enmType != VPOXWDDM_CONTEXT_TYPE_CUSTOM_DISPIF_RESIZE)
                {
                    WARN(("VPOXESC_UPDATEMODES invalid context supplied %d!", pContext->enmType));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }
#endif

                if (pEscape->PrivateDriverDataSize != sizeof (VPOXDISPIFESCAPE_UPDATEMODES))
                {
                    WARN(("VPOXESC_UPDATEMODES invalid private driver size %d", pEscape->PrivateDriverDataSize));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }

                VPOXDISPIFESCAPE_UPDATEMODES *pData = (VPOXDISPIFESCAPE_UPDATEMODES*)pEscapeHdr;
                Status = VPoxVidPnUpdateModes(pDevExt, pData->u32TargetId, &pData->Size);
                if (!NT_SUCCESS(Status))
                {
                    WARN(("VPoxVidPnUpdateModes failed Status(%#x)\n", Status));
                    return Status;
                }
            ...
            case VPOXESC_TARGET_CONNECTIVITY:
            {
                if (!pEscape->Flags.HardwareAccess)
                {
                    WARN(("VPOXESC_TARGET_CONNECTIVITY called without HardwareAccess flag set, failing"));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }

                if (pEscape->PrivateDriverDataSize != sizeof(VPOXDISPIFESCAPE_TARGETCONNECTIVITY))
                {
                    WARN(("VPOXESC_TARGET_CONNECTIVITY invalid private driver size %d", pEscape->PrivateDriverDataSize));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }

                VPOXDISPIFESCAPE_TARGETCONNECTIVITY *pData = (VPOXDISPIFESCAPE_TARGETCONNECTIVITY *)pEscapeHdr;
                LOG(("=> VPOXESC_TARGET_CONNECTIVITY[%d] 0x%08X", pData->u32TargetId, pData->fu32Connect));

                if (pData->u32TargetId >= (uint32_t)VPoxCommonFromDeviceExt(pDevExt)->cDisplays)
                {
                    WARN(("VPOXESC_TARGET_CONNECTIVITY invalid screen index 0x%x", pData->u32TargetId));
                    Status = STATUS_INVALID_PARAMETER;
                    break;
                }

                PVPOXWDDM_TARGET pTarget = &pDevExt->aTargets[pData->u32TargetId];
            ...
            case VPOXESC_DBGPRINT:
            {
                /* use RT_OFFSETOF instead of sizeof since sizeof will give an aligned size that might
                 * be bigger than the VPOXDISPIFESCAPE_DBGPRINT with a data containing just a few chars */
                Assert(pEscape->PrivateDriverDataSize >= RT_UOFFSETOF(VPOXDISPIFESCAPE_DBGPRINT, aStringBuf[1]));
                /* only do DbgPrint when pEscape->PrivateDriverDataSize > RT_OFFSETOF(VPOXDISPIFESCAPE_DBGPRINT, aStringBuf[1])
                 * since == RT_OFFSETOF(VPOXDISPIFESCAPE_DBGPRINT, aStringBuf[1]) means the buffer contains just \0,
                 * i.e. no need to print it */
                if (pEscape->PrivateDriverDataSize > RT_UOFFSETOF(VPOXDISPIFESCAPE_DBGPRINT, aStringBuf[1]))
                {
                    PVPOXDISPIFESCAPE_DBGPRINT pDbgPrint = (PVPOXDISPIFESCAPE_DBGPRINT)pEscapeHdr;
                    /* ensure the last char is \0*/
                    if (*((uint8_t*)pDbgPrint + pEscape->PrivateDriverDataSize - 1) == '\0')
                    {
                        if (g_VPoxLogUm & VPOXWDDM_CFG_LOG_UM_DBGPRINT)
                            DbgPrint("%s\n", pDbgPrint->aStringBuf);
                        if (g_VPoxLogUm & VPOXWDDM_CFG_LOG_UM_BACKDOOR)
                            LOGREL_EXACT(("%s\n", pDbgPrint->aStringBuf));
                    }
                }
                Status = STATUS_SUCCESS;
            ...
            case VPOXESC_DBGDUMPBUF:
            {
                Status = vpoxUmdDumpBuf((PVPOXDISPIFESCAPE_DBGDUMPBUF)pEscapeHdr, pEscape->PrivateDriverDataSize);
                break;
            }
            case VPOXESC_GUEST_DISPLAYCHANGED:
            {
                LOG(("=> VPOXESC_GUEST_DISPLAYCHANGED"));
                ...
                for (int i = 0; i < VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
                {
                    ...
                    vpoxWddmDisplaySettingsCheckPos(pDevExt, i);
                }
                Status = STATUS_SUCCESS;
                break;
            }
    ...
    {
        WARN(("pEscape->PrivateDriverDataSize(%d) < (%d)", pEscape->PrivateDriverDataSize, sizeof (VPOXDISPIFESCAPE)));
        Status = STATUS_BUFFER_TOO_SMALL;
    }
    ...
    )
{
    ...
    PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)hAdapter;
    NTSTATUS Status = VPoxVidPnIsSupported(pDevExt, pIsSupportedVidPnArg->hDesiredVidPn, &pIsSupportedVidPnArg->IsVidPnSupported);
    if (!NT_SUCCESS(Status))
    {
        WARN(("VPoxVidPnIsSupported failed Status(%#x)\n", Status));
        return Status;
    }
    ...
    )
{
    ...
    if (pRecommendFunctionalVidPnArg->PrivateDriverDataSize != sizeof (VPOXWDDM_RECOMMENDVIDPN))
    {
        WARN(("invalid size"));
        return STATUS_INVALID_PARAMETER;
    }

    VPOXWDDM_RECOMMENDVIDPN *pData = (VPOXWDDM_RECOMMENDVIDPN*)pRecommendFunctionalVidPnArg->pPrivateDriverData;
    Assert(pData);

    NTSTATUS Status = VPoxVidPnRecommendFunctional(pDevExt, pRecommendFunctionalVidPnArg->hRecommendedFunctionalVidPn, pData);
    if (!NT_SUCCESS(Status))
    {
        WARN(("VPoxVidPnRecommendFunctional failed %#x", Status));
        return Status;
    }
    ...
    )
{
    ...
    NTSTATUS Status = VPoxVidPnCofuncModality(pDevExt, pEnumCofuncModalityArg->hConstrainingVidPn, pEnumCofuncModalityArg->EnumPivotType, &pEnumCofuncModalityArg->EnumPivot);
    if (!NT_SUCCESS(Status))
    {
        WARN(("VPoxVidPnCofuncModality failed Status(%#x)\n", Status));
        return Status;
    }
    ...
    )
{
    ...
    PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)hAdapter;
    if ((UINT)VPoxCommonFromDeviceExt(pDevExt)->cDisplays <= pSetVidPnSourceAddress->VidPnSourceId)
    {
        WARN(("invalid VidPnSourceId (%d), for displays(%d)", pSetVidPnSourceAddress->VidPnSourceId, VPoxCommonFromDeviceExt(pDevExt)->cDisplays));
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (pDevExt->enmHwType != VPOXVIDEO_HWTYPE_VMSVGA)
#endif
    vpoxWddmDisplaySettingsCheckPos(pDevExt, pSetVidPnSourceAddress->VidPnSourceId);

    NTSTATUS Status = STATUS_SUCCESS;
    PVPOXWDDM_SOURCE pSource = &pDevExt->aSources[pSetVidPnSourceAddress->VidPnSourceId];
    ...
    {
        pAllocation = (PVPOXWDDM_ALLOCATION)pSetVidPnSourceAddress->hAllocation;
        vpoxWddmAssignPrimary(pSource, pAllocation, pSetVidPnSourceAddress->VidPnSourceId);
    }
    else
    ...
    if (pAllocation)
    {
        vpoxWddmAddrSetVram(&pAllocation->AllocData.Addr, pSetVidPnSourceAddress->PrimarySegment, (VPOXVIDEOOFFSET)pSetVidPnSourceAddress->PrimaryAddress.QuadPart);
    }

    if (g_VPoxDisplayOnly && !pAllocation)
    {
        /* the VRAM here is an absolute address, nto an offset!
         * convert to offset since all internal VPox functionality is offset-based */
        vpoxWddmAddrSetVram(&pSource->AllocData.Addr, pSetVidPnSourceAddress->PrimarySegment,
                vpoxWddmVramAddrToOffset(pDevExt, pSetVidPnSourceAddress->PrimaryAddress));
    }
    else
    {
        Assert(!g_VPoxDisplayOnly);
        vpoxWddmAddrSetVram(&pSource->AllocData.Addr, pSetVidPnSourceAddress->PrimarySegment,
                                                    pSetVidPnSourceAddress->PrimaryAddress.QuadPart);
    }
    ...
    if (pDevExt->enmHwType == VPOXVIDEO_HWTYPE_VMSVGA)
    {
        ...
        vpoxWddmDisplaySettingsQueryPos(pDevExt, pSetVidPnSourceAddress->VidPnSourceId, &pSource->VScreenPos);
    ...
    )
{
    ...
    if ((UINT)VPoxCommonFromDeviceExt(pDevExt)->cDisplays <= pSetVidPnSourceVisibility->VidPnSourceId)
    {
        WARN(("invalid VidPnSourceId (%d), for displays(%d)", pSetVidPnSourceVisibility->VidPnSourceId, VPoxCommonFromDeviceExt(pDevExt)->cDisplays));
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (pDevExt->enmHwType != VPOXVIDEO_HWTYPE_VMSVGA)
#endif
    vpoxWddmDisplaySettingsCheckPos(pDevExt, pSetVidPnSourceVisibility->VidPnSourceId);

    NTSTATUS Status = STATUS_SUCCESS;
    PVPOXWDDM_SOURCE pSource = &pDevExt->aSources[pSetVidPnSourceVisibility->VidPnSourceId];
    PVPOXWDDM_ALLOCATION pAllocation = pSource->pPrimaryAllocation;
    if (pAllocation)
    {
...
//        pSource->u8SyncState &= ~VPOXWDDM_HGSYNC_F_SYNCED_VISIBILITY;
    ...
    )
{
    ...
    VPOXWDDM_SOURCE *paSources = (VPOXWDDM_SOURCE*)RTMemAlloc(sizeof (VPOXWDDM_SOURCE) * VPoxCommonFromDeviceExt(pDevExt)->cDisplays);
    if (!paSources)
    {
    ...
    VPOXWDDM_TARGET *paTargets = (VPOXWDDM_TARGET*)RTMemAlloc(sizeof (VPOXWDDM_TARGET) * VPoxCommonFromDeviceExt(pDevExt)->cDisplays);
    if (!paTargets)
    {
        WARN(("RTMemAlloc failed"));
        RTMemFree(paSources);
    ...
    VPoxVidPnSourcesInit(paSources, VPoxCommonFromDeviceExt(pDevExt)->cDisplays, VPOXWDDM_HGSYNC_F_SYNCED_ALL);

    VPoxVidPnTargetsInit(paTargets, VPoxCommonFromDeviceExt(pDevExt)->cDisplays, VPOXWDDM_HGSYNC_F_SYNCED_ALL);

    VPoxVidPnSourcesCopy(paSources, pDevExt->aSources, VPoxCommonFromDeviceExt(pDevExt)->cDisplays);
    VPoxVidPnTargetsCopy(paTargets, pDevExt->aTargets, VPoxCommonFromDeviceExt(pDevExt)->cDisplays);
    ...
    do {
        ...
        {
            Status = VPoxVidPnCommitSourceModeForSrcId(
                    pDevExt,
                    pCommitVidPnArg->hFunctionalVidPn, pVidPnInterface,
                    (PVPOXWDDM_ALLOCATION)pCommitVidPnArg->hPrimaryAllocation,
                    pCommitVidPnArg->AffectedVidPnSourceId, paSources, paTargets, pCommitVidPnArg->Flags.PathPowerTransition);
            if (!NT_SUCCESS(Status))
            {
                WARN(("VPoxVidPnCommitSourceModeForSrcId for current VidPn failed Status 0x%x", Status));
                break;
            }
        }
        else
        {
            Status = VPoxVidPnCommitAll(pDevExt, pCommitVidPnArg->hFunctionalVidPn, pVidPnInterface,
                    (PVPOXWDDM_ALLOCATION)pCommitVidPnArg->hPrimaryAllocation,
                    paSources, paTargets);
            if (!NT_SUCCESS(Status))
            {
                WARN(("VPoxVidPnCommitAll for current VidPn failed Status 0x%x", Status));
                break;
            }
        }
        ...
        VPoxVidPnSourcesCopy(pDevExt->aSources, paSources, VPoxCommonFromDeviceExt(pDevExt)->cDisplays);
        VPoxVidPnTargetsCopy(pDevExt->aTargets, paTargets, VPoxCommonFromDeviceExt(pDevExt)->cDisplays);

        VPoxDumpSourceTargetArrays(paSources, paTargets, VPoxCommonFromDeviceExt(pDevExt)->cDisplays);
        ...
        if (pDevExt->enmHwType == VPOXVIDEO_HWTYPE_VMSVGA)
        {
            for (int i = 0; i < VPoxCommonFromDeviceExt(pDevExt)->cDisplays; ++i)
            {
                ...
                vpoxWddmDisplaySettingsQueryPos(pDevExt, i, &pSource->VScreenPos);
    ...
    )
{
    ...
    NTSTATUS Status = VPoxVidPnRecommendMonitorModes(pDevExt, pRecommendMonitorModesArg->VideoPresentTargetId,
            pRecommendMonitorModesArg->hMonitorSourceModeSet, pRecommendMonitorModesArg->pMonitorSourceModeSetInterface);
    if (!NT_SUCCESS(Status))
    {
        WARN(("VPoxVidPnRecommendMonitorModes failed %#x", Status));
        return Status;
    }
...
//    RT_BREAKPOINT();
    ...
    NTSTATUS Status = VPoxWddmSlGetScanLine(pDevExt, pGetScanLine);
    ...
    )
{
    ...
    switch (InterruptType)
    {
        ...
        {
            Status = VPoxWddmSlEnableVSyncNotification(pDevExt, Enable);
            if (NT_SUCCESS(Status))
...
#ifdef VPOX_WITH_VIDEOHWACCEL
    PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)hAdapter;
    PVPOXWDDM_OVERLAY pOverlay = (PVPOXWDDM_OVERLAY)vpoxWddmMemAllocZero(sizeof (VPOXWDDM_OVERLAY));
    Assert(pOverlay);
    if (pOverlay)
    {
        int rc = vpoxVhwaHlpOverlayCreate(pDevExt, pCreateOverlay->VidPnSourceId, &pCreateOverlay->OverlayInfo, pOverlay);
        AssertRC(rc);
        if (RT_SUCCESS(rc))
        {
...
#ifdef VPOX_WITH_MESA3D
    PVPOXWDDM_DEVICE pDevice = (PVPOXWDDM_DEVICE)hDevice;
    PVPOXMP_DEVEXT pDevExt = pDevice->pAdapter;
    if (pDevExt->enmHwType == VPOXVIDEO_HWTYPE_VMSVGA)
    {
    ...
    CONST DXGKARG_OPENALLOCATION  *pOpenAllocation)
{
    ...
    PVPOXWDDM_DEVICE pDevice = (PVPOXWDDM_DEVICE)hDevice;
    PVPOXMP_DEVEXT pDevExt = pDevice->pAdapter;
    PVPOXWDDM_RCINFO pRcInfo = NULL;
    if (pOpenAllocation->PrivateDriverSize)
    {
        Assert(pOpenAllocation->pPrivateDriverData);
        if (pOpenAllocation->PrivateDriverSize == sizeof (VPOXWDDM_RCINFO))
        {
            pRcInfo = (PVPOXWDDM_RCINFO)pOpenAllocation->pPrivateDriverData;
    ...
    if (Status == STATUS_SUCCESS)
    {
        ...
        for (; i < pOpenAllocation->NumAllocations; ++i)
        {
            ...
            Assert(pInfo->PrivateDriverDataSize == sizeof (VPOXWDDM_ALLOCINFO));
            Assert(pInfo->pPrivateDriverData);
            PVPOXWDDM_ALLOCATION pAllocation = vpoxWddmGetAllocationFromHandle(pDevExt, pInfo->hAllocation);
            if (!pAllocation)
            {
                WARN(("invalid handle"));
                Status = STATUS_INVALID_PARAMETER;
            ...
            if (pRcInfo)
            {
                Assert(pAllocation->enmType == VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC);

                if (pInfo->PrivateDriverDataSize != sizeof (VPOXWDDM_ALLOCINFO)
...
#ifdef VPOX_WITH_VIDEOHWACCEL
                PVPOXWDDM_ALLOCINFO pAllocInfo = (PVPOXWDDM_ALLOCINFO)pInfo->pPrivateDriverData;
            ...
            {
                KeReleaseSpinLock(&pAllocation->OpenLock, OldIrql);
                pOa = (PVPOXWDDM_OPENALLOCATION)vpoxWddmMemAllocZero(sizeof (VPOXWDDM_OPENALLOCATION));
                if (!pOa)
                {
                ...
                PVPOXWDDM_OPENALLOCATION pConcurrentOa;
                KeAcquireSpinLock(&pAllocation->OpenLock, &OldIrql);
                pConcurrentOa = VPoxWddmOaSearchLocked(pDevice, pAllocation);
                if (!pConcurrentOa)
        ...
        if (Status != STATUS_SUCCESS)
        {
            for (UINT j = 0; j < i; ++j)
            {
                ...
                PVPOXWDDM_OPENALLOCATION pOa2Free = (PVPOXWDDM_OPENALLOCATION)pInfo2Free->hDeviceSpecificAllocation;
                VPoxWddmOaRelease(pOa2Free);
            }
        }
    }
    ...
    CONST DXGKARG_CLOSEALLOCATION*  pCloseAllocation)
{
    ...
    {
        PVPOXWDDM_OPENALLOCATION pOa2Free = (PVPOXWDDM_OPENALLOCATION)pCloseAllocation->pOpenHandleList[i];
        PVPOXWDDM_ALLOCATION pAllocation = pOa2Free->pAllocation;
...
#define VPOXVDMACMD_DMA_PRESENT_BLT_MINSIZE() (VPOXVDMACMD_SIZE(VPOXVDMACMD_DMA_PRESENT_BLT))
#define VPOXVDMACMD_DMA_PRESENT_BLT_SIZE(_c) (VPOXVDMACMD_BODY_FIELD_OFFSET(UINT, VPOXVDMACMD_DMA_PRESENT_BLT, aDstSubRects[_c]))

DECLINLINE(BOOLEAN) vpoxWddmPixFormatConversionSupported(D3DDDIFORMAT From, D3DDDIFORMAT To)
{
...
#ifdef VPOX_WITH_VIDEOHWACCEL
    PVPOXWDDM_OVERLAY pOverlay = (PVPOXWDDM_OVERLAY)hOverlay;
    AssertPtr(pOverlay);
    int rc = vpoxVhwaHlpOverlayUpdate(pOverlay, &pUpdateOverlay->OverlayInfo);
    AssertRC(rc);
    if (RT_FAILURE(rc))
        Status = STATUS_UNSUCCESSFUL;
#else
    ...
    CONST DXGKARG_FLIPOVERLAY  *pFlipOverlay)
{
    ...
    PVPOXWDDM_OVERLAY pOverlay = (PVPOXWDDM_OVERLAY)hOverlay;
    AssertPtr(pOverlay);
    int rc = vpoxVhwaHlpOverlayFlip(pOverlay, pFlipOverlay);
    AssertRC(rc);
    if (RT_FAILURE(rc))
        Status = STATUS_UNSUCCESSFUL;
#else
    ...
    CONST HANDLE  hOverlay)
{
    ...
    PVPOXWDDM_OVERLAY pOverlay = (PVPOXWDDM_OVERLAY)hOverlay;
    AssertPtr(pOverlay);
    int rc = vpoxVhwaHlpOverlayDestroy(pOverlay);
    AssertRC(rc);
    if (RT_SUCCESS(rc))
        vpoxWddmMemFree(pOverlay);
    else
    ...
    DXGKARG_CREATECONTEXT  *pCreateContext)
{
    ...
    if (pCreateContext->NodeOrdinal >= VPOXWDDM_NUM_NODES)
    {
        WARN(("Invalid NodeOrdinal (%d), expected to be less that (%d)\n", pCreateContext->NodeOrdinal, VPOXWDDM_NUM_NODES));
        return STATUS_INVALID_PARAMETER;
    }
    ...
    PVPOXWDDM_DEVICE pDevice = (PVPOXWDDM_DEVICE)hDevice;
    PVPOXMP_DEVEXT pDevExt = pDevice->pAdapter;
    PVPOXWDDM_CONTEXT pContext = (PVPOXWDDM_CONTEXT)vpoxWddmMemAllocZero(sizeof (VPOXWDDM_CONTEXT));
    Assert(pContext);
    if (pContext)
    {
        ...
        vpoxVideoCmCtxInitEmpty(&pContext->CmContext);
        if (pCreateContext->Flags.SystemContext || pCreateContext->PrivateDriverDataSize == 0)
        {
            ...
            pContext->enmType = VPOXWDDM_CONTEXT_TYPE_SYSTEM;
        ...
        else
        {
            ...
            Assert(pCreateContext->PrivateDriverDataSize == sizeof (VPOXWDDM_CREATECONTEXT_INFO));
            Assert(pCreateContext->pPrivateDriverData);
            if (pCreateContext->PrivateDriverDataSize == sizeof (VPOXWDDM_CREATECONTEXT_INFO))
            {
                PVPOXWDDM_CREATECONTEXT_INFO pInfo = (PVPOXWDDM_CREATECONTEXT_INFO)pCreateContext->pPrivateDriverData;
                switch (pInfo->enmType)
                {
                    case VPOXWDDM_CONTEXT_TYPE_CUSTOM_DISPIF_RESIZE:
                    {
                        pContext->enmType = pInfo->enmType;
                        ASMAtomicIncU32(&pDevExt->cContextsDispIfResize);
                        break;
                    }
                    case VPOXWDDM_CONTEXT_TYPE_CUSTOM_DISPIF_SEAMLESS:
                    {
                        pContext->enmType = pInfo->enmType;
                        Status = vpoxVideoCmCtxAdd(&pDevice->pAdapter->SeamlessCtxMgr, &pContext->CmContext, (HANDLE)pInfo->u.vpox.hUmEvent, pInfo->u.vpox.u64UmInfo);
                        if (!NT_SUCCESS(Status))
                        {
                            WARN(("vpoxVideoCmCtxAdd failed, Status 0x%x", Status));
                        }
                        break;
                    }
#ifdef VPOX_WITH_MESA3D
                    case VPOXWDDM_CONTEXT_TYPE_GA_3D:
                    {
                        pContext->enmType = VPOXWDDM_CONTEXT_TYPE_GA_3D;
        ...
        if (Status == STATUS_SUCCESS)
        {
            ...
            pCreateContext->ContextInfo.DmaBufferSize = VPOXWDDM_C_DMA_BUFFER_SIZE;
            pCreateContext->ContextInfo.DmaBufferSegmentSet = 0;
            pCreateContext->ContextInfo.DmaBufferPrivateDataSize = VPOXWDDM_C_DMA_PRIVATEDATA_SIZE;
            pCreateContext->ContextInfo.AllocationListSize = VPOXWDDM_C_ALLOC_LIST_SIZE;
            pCreateContext->ContextInfo.PatchLocationListSize = VPOXWDDM_C_PATH_LOCATION_LIST_SIZE;
        //#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN7)
        //# error port to Win7 DDI
    ...
    CONST HANDLE  hContext)
{
    ...
    PVPOXMP_DEVEXT pDevExt = pContext->pDevice->pAdapter;
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    switch(pContext->enmType)
    {
        case VPOXWDDM_CONTEXT_TYPE_CUSTOM_DISPIF_RESIZE:
        {
            ...
            if (!cContexts)
            {
                if (pDevExt->fDisableTargetUpdate)
                {
                    ...
                    vpoxWddmGhDisplayCheckSetInfoEx(pDevExt, true);
                }
            }
            break;
        }
        case VPOXWDDM_CONTEXT_TYPE_CUSTOM_DISPIF_SEAMLESS:
        {
            Status = vpoxVideoCmCtxRemove(&pContext->pDevice->pAdapter->SeamlessCtxMgr, &pContext->CmContext);
            if (!NT_SUCCESS(Status))
                WARN(("vpoxVideoCmCtxRemove failed, Status 0x%x", Status));
    ...
    Status = vpoxVideoAMgrCtxDestroy(&pContext->AllocContext);
    if (NT_SUCCESS(Status))
    {
        Status = vpoxVideoCmCtxRemove(&pContext->pDevice->pAdapter->CmMgr, &pContext->CmContext);
        if (NT_SUCCESS(Status))
            vpoxWddmMemFree(pContext);
        else
            WARN(("vpoxVideoCmCtxRemove failed, Status 0x%x", Status));
        ...
        WARN(("vpoxVideoAMgrCtxDestroy failed, Status 0x%x", Status));
      ...
      )
{
...
#endif
    PVPOXWDDM_SOURCE pSource = &pDevExt->aSources[pPresentDisplayOnly->VidPnSourceId];
    Assert(pSource->AllocData.Addr.SegmentId == 1);
    VPOXWDDM_ALLOC_DATA SrcAllocData;
    ...
    for (UINT i = 0; i < pPresentDisplayOnly->NumMoves; ++i)
    {
        if (!bUpdateRectInited)
        {
            ...
            vpoxWddmRectUnite(&UpdateRect, &pPresentDisplayOnly->pMoves[i].DestRect);
        vpoxVdmaGgDmaBltPerform(pDevExt, &SrcAllocData, &pPresentDisplayOnly->pMoves[i].DestRect, &pSource->AllocData, &pPresentDisplayOnly->pMoves[i].DestRect);
    }
    ...
    for (UINT i = 0; i < pPresentDisplayOnly->NumDirtyRects; ++i)
    {
        ...
        vpoxVdmaGgDmaBltPerform(pDevExt, &SrcAllocData, pDirtyRect, &pSource->AllocData, pDirtyRect);
        ...
        if (!bUpdateRectInited)
        {
            ...
            vpoxWddmRectUnite(&UpdateRect, pDirtyRect);
    }

    if (bUpdateRectInited && pSource->bVisible)
    {
        VPOXVBVA_OP_WITHLOCK(ReportDirtyRect, pDevExt, pSource, &UpdateRect);
    }
...
static NTSTATUS vpoxWddmInitDisplayOnlyDriver(IN PDRIVER_OBJECT pDriverObject, IN PUNICODE_STRING pRegistryPath)
{
...
static NTSTATUS vpoxWddmInitFullGraphicsDriver(IN PDRIVER_OBJECT pDriverObject, IN PUNICODE_STRING pRegistryPath, VPOXVIDEO_HWTYPE enmHwType)
{
...
#ifdef VPOX_WITH_MESA3D
    if (enmHwType == VPOXVIDEO_HWTYPE_VMSVGA)
    {
    ...
    )
{
    ...
    if (RT_FAILURE(irc))
    {
        RTLogBackdoorPrintf("VPoxWddm: RTR0Init failed: %Rrc!\n", irc);
        return STATUS_UNSUCCESSFUL;
    }
...
#ifdef DEBUG
#define VPOXWDDM_BUILD_TYPE "dbg"
#else
#define VPOXWDDM_BUILD_TYPE "rel"
#endif

    LOGREL(("VPox WDDM Driver for Windows %s version %d.%d.%dr%d %s, %d bit; Built %s %s",
            VPoxQueryWinVersion(NULL) >= WINVERSION_8 ? "8+" : "Vista and 7",
    ...
    vpoxWddmDrvCfgInit(RegistryPath);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        VPOXVIDEO_HWTYPE enmHwType = VPOXVIDEO_HWTYPE_VPOX;
        ...
        if (VPoxVGACfgAvailable())
        {
            ...
            uint32_t u32;
            if (VPoxVGACfgQuery(VBE_DISPI_CFG_ID_VERSION, &u32, 0))
            {
            ...
            if (u32)
            {
                enmHwType = VPOXVIDEO_HWTYPE_VMSVGA;
            }
            LOGREL(("WDDM: VGA configuration: 3D %d, hardware type %d", f3DSupported, enmHwType));
        }

        if (enmHwType == VPOXVIDEO_HWTYPE_VPOX)
        {
            /* No 3D for legacy adapter. */
            f3DSupported = FALSE;
        }
        else if (enmHwType == VPOXVIDEO_HWTYPE_VMSVGA)
        {
            /* Nothing. */
        }
        else
        {
            /* No supported hardware, fallback to VPox 2D only. */
            enmHwType = VPOXVIDEO_HWTYPE_VPOX;
            f3DSupported = FALSE;
        }
        ...
        if (NT_SUCCESS(Status))
        {
            if (g_VPoxDisplayOnly)
            {
                Status = vpoxWddmInitDisplayOnlyDriver(DriverObject, RegistryPath);
            }
            else
            {
                Status = vpoxWddmInitFullGraphicsDriver(DriverObject, RegistryPath, enmHwType);
            }
