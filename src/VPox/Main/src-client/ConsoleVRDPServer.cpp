...
/** @file
 * VPox Console VRDP helper class.
 */
...
VPOX_LISTENER_DECLARE(VRDPConsoleListenerImpl)
...
typedef struct TSMFVRDPCTX
{
    ...
    VPOXHOSTCHANNELCALLBACKS *pCallbacks;
    void *pvCallbacks;
                                                                         ...
                                                                         uint32_t *pcbDataReturned)
{
    ...
    if (!pvChannel)
    {
        ...
        if (u32Code == VPOX_HOST_CHANNEL_CTRL_EXISTS)
        {
            *pcbDataReturned = 0;
            return VINF_SUCCESS;
        }
...
void ConsoleVRDPServer::setupTSMF(void)
{
    ...
    VPOXHOSTCHANNELINTERFACE hostChannelInterface =
    {
        this,
    ...
    int rc = pVMMDev->hgcmHostCall("VPoxHostChannel",
                                   VPOX_HOST_CHANNEL_HOST_FN_REGISTER,
                                   2,
                                   &parms.name);

    if (!RT_SUCCESS(rc))
    {
        Log(("VPOX_HOST_CHANNEL_HOST_FN_REGISTER failed with %Rrc\n", rc));
        return;
    }
...
#define VPOX_TSMF_HCH_CREATE_ACCEPTED (VPOX_HOST_CHANNEL_EVENT_USER + 0)
#define VPOX_TSMF_HCH_CREATE_DECLINED (VPOX_HOST_CHANNEL_EVENT_USER + 1)
#define VPOX_TSMF_HCH_DISCONNECTED    (VPOX_HOST_CHANNEL_EVENT_USER + 2)
                                           ...
                                           uint32_t u32ClientId)
{
...
#ifdef VPOX_WITH_VRDEAUTH_IN_VPOXSVC
    try
    {
    ...
    result = AuthLibAuthenticate(&mAuthLibCtx,
                                 ...
                                 u32ClientId);
#endif /* !VPOX_WITH_VRDEAUTH_IN_VPOXSVC */
                                                       ...
                                                       uint32_t cbData)
{
    ...
    switch (u32Function)
    {
        case VRDE_CLIPBOARD_FUNCTION_FORMAT_ANNOUNCE:
        {
            ...
            {
                rc = pServer->mpfnClipboardCallback(VPOX_CLIPBOARD_EXT_FN_FORMAT_ANNOUNCE,
                                                    u32Format,
                                                    (void *)pvData,
                                                    cbData);
            }
        ...
        case VRDE_CLIPBOARD_FUNCTION_DATA_READ:
        {
            if (pServer->mpfnClipboardCallback)
            {
                rc = pServer->mpfnClipboardCallback(VPOX_CLIPBOARD_EXT_FN_DATA_READ,
                                                    u32Format,
                                                    (void *)pvData,
                                                    cbData);
            }
...
ConsoleVRDPServer::ClipboardServiceExtension(void *pvExtension, uint32_t u32Function, void *pvParms, uint32_t cbParms)
{
    ...
    switch (u32Function)
    {
        case VPOX_CLIPBOARD_EXT_FN_SET_CALLBACK:
        {
            pServer->mpfnClipboardCallback = pParms->u.pfnCallback;
        } break;

        case VPOX_CLIPBOARD_EXT_FN_FORMAT_ANNOUNCE:
        {
        ...
        case VPOX_CLIPBOARD_EXT_FN_DATA_READ:
        {
        ...
        case VPOX_CLIPBOARD_EXT_FN_DATA_WRITE:
        {
            if (mpEntryPoints && pServer->mhServer)
            {
                mpEntryPoints->VRDEClipboard(pServer->mhServer,
...
void ConsoleVRDPServer::ClipboardCreate(uint32_t u32ClientId)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            rc = HGCMHostRegisterServiceExtension(&mhClipboard, "VPoxSharedClipboard", ClipboardServiceExtension, this);
            AssertRC(rc);
        }
