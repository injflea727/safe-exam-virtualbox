...
#define SSMFILEHDR_MAGIC_BASE                   "\177VirtualPox SavedState "
/** Saved state file magic indicating version 1.x. */
#define SSMFILEHDR_MAGIC_V1_X                   "\177VirtualPox SavedState V1."
/** Saved state file v1.1 magic. */
#define SSMFILEHDR_MAGIC_V1_1                   "\177VirtualPox SavedState V1.1\n"
/** Saved state file v1.2 magic. */
#define SSMFILEHDR_MAGIC_V1_2                   "\177VirtualPox SavedState V1.2\n\0\0\0"
/** Saved state file v2.0 magic. */
#define SSMFILEHDR_MAGIC_V2_0                   "\177VirtualPox SavedState V2.0\n\0\0\0"
 ...
 *
 * Added in r5xxxx on 2009-07-2?, VirtualPox v3.0.51.
 */
typedef struct SSMFILEHDR
{
    /** Magic string which identifies this file as a version of VPox saved state
 ...
 * Header of the saved state file.
 *
 * Added in r40980 on 2008-12-15, VirtualPox v2.0.51.
 *
 ...
 *
 * Added in r23677 on 2007-08-17, VirtualPox v1.4.1.
 */
...
{
    /*
     * Register a saved state unit which we use to put the VirtualPox version,
 ...
 *
 * @param   pStrm           The stream handle.
 * @param   rc              The VPox error status code.
 *
 * @thread  Any.
 */
 ...
 * Flush buffered data.
 *
 * @returns VPox status code. Returns VINF_EOF if we encounter a buffer with the
 ...
 *
 * @returns VPox status code. VERR_EOF gets translated into VINF_EOF.
 * @param   pStrm       The stream handle.
 *
 ...
 *
 * @returns VPox status code. Sets pSSM->rc on failure.
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns VPox status code. Will set pSSM->rc on error.
 * @param   pSSM            The saved state handle.
 */
static int ssmR3DataFlushBuffer(PSSMHANDLE pSSM)
{
...
/**
 * Saves a VPox unsigned integer item to the current data unit.
 *
 * @returns VPox status code.
 * @param   pSSM            The saved state handle.
...
/**
 * Saves a VPox signed integer item to the current data unit.
 *
 * @returns VPox status code.
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns VPox status code (pSSM->rc).
 * @param   pVM                 The cross context VM structure.
 * @param   pSSM                The saved state handle.
 */
 ...
 *
 * @returns VPox status code (no need to check pSSM->rc).
 * @param   pVM                 The cross context VM structure.
...
static int ssmR3WriteHeaderAndClearPerUnitData(PVM pVM, PSSMHANDLE pSSM)
{
    ...
    FileHdr.u16VerMajor  = VPOX_VERSION_MAJOR;
    FileHdr.u16VerMinor  = VPOX_VERSION_MINOR;
    FileHdr.u32VerBuild  = VPOX_VERSION_BUILD;
 ...
 *
 * @returns VPox status code. Set pSSM->rc on error.
 ...
 *
 * @returns VPox status code, pSSM->rc is set on error.
 *
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns VPox status code. Does NOT set pSSM->rc.
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns VPox status code. Sets pSSM->rc on error.
 * @param   pSSM            The saved state handle..
...
/**
 * Loads a VPox unsigned integer item from the current data unit.
 *
 * @returns VPox status code.
 * @param   pSSM            The saved state handle.
...
/**
 * Loads a VPox signed integer item from the current data unit.
 *
 * @returns VPox status code.
 * @param   pSSM            The saved state handle.
...
static int ssmR3ValidateHeaderInfo(PSSMHANDLE pSSM, bool fHaveHostBits, bool fHaveVersion)
{
    ...
    if (fHaveVersion)
    {
            ...
            ||  pSSM->u.Read.u32SvnRev > 10000000 /*100M*/)
        {
            ...
            return VERR_SSM_INTEGRITY_VPOX_VERSION;
        }
    }
    else
        AssertLogRelReturn(   pSSM->u.Read.u16VerMajor == 0
                           ...
                           && pSSM->u.Read.u32SvnRev   == 0,
                           VERR_SSM_INTEGRITY_VPOX_VERSION);
 ...
 *
 * @returns VPox status code.  May or may not set pSSM->rc, the returned
                         ...
                         SSMAFTER enmAfter, PFNVMPROGRESS pfnProgress, void *pvProgressUser)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (Handle.u.Read.u16VerMajor)
            LogRel(("SSM: File header: Format %u.%u, VirtualPox Version %u.%u.%u r%u, %u-bit host, cbGCPhys=%u, cbGCPtr=%u\n",
 ...
 * @returns VPox status code on other failures.
 *
...
/**
 * Query what the VPox status code of the operation is.
 *
 ...
 * to SSM.
 *
 * @returns VPox status code of the handle, or VERR_INVALID_PARAMETER.
 * @param   pSSM            The saved state handle.
...
/**
 * Get the VirtualPox SVN revision that created the saved state.
 *
...
/**
 * Gets the VirtualPox version that created the saved state.
 *
 * @returns VPOX_FULL_VERSION style version number.
...
VMMR3DECL(uint32_t)     SSMR3HandleVersion(PSSMHANDLE pSSM)
{
    if (pSSM->enmOp >= SSMSTATE_LOAD_PREP)
    {
        ...
        return VPOX_FULL_VERSION_MAKE(pSSM->u.Read.u16VerMajor, pSSM->u.Read.u16VerMinor, pSSM->u.Read.u32VerBuild);
    }
    return VPOX_FULL_VERSION;
}
