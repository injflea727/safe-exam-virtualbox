...
# include "VPoxDTraceLibCWrappers.h"
...
#ifdef VPOX /* Added volatile to signal handler variables. */
...
#ifndef VPOX /* stdout isn't a necessarily constant usable like this in C code. */
...
dof_prune(const char *fname)
{
	...
	if ((size_t/*vpox*/)read(fd, buf, sz) != sz)
	...
	for (mark = 0, i = 0; i < sz; i++) {
		 ...
		 */
		if (i != mark) {
			if ((size_t/*vpox*/)write(fd, &buf[mark], i - mark) != i - mark)
	...
	if (mark < sz) {
		if ((size_t/*vpox*/)write(fd, &buf[mark], sz - mark) != sz - mark)
			fatal("failed to write to %s", fname);
	}
...
etcsystem_prune(void)
{
	...
	if (write(fd, buf, strlen(buf)) < (ssize_t/*vpox*/)strlen(buf)) {
		(void) unlink(tmpname);
...
#ifdef VPOX
DECLEXPORT(int) RTCALL VPoxDTraceMain(int argc, char **argv)
#else
int
