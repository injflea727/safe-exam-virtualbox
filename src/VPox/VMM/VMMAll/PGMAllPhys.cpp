 ...
 *      Dummy for forcing ring-3 handling of the access.}
 */
VMMDECL(VPOXSTRICTRC) pgmPhysPfHandlerRedirectToHC(PVMCC pVM, PVMCPUCC pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
 ...
 */
DECLEXPORT(VPOXSTRICTRC) pgmPhysRomWritePfHandler(PVMCC pVM, PVMCPUCC pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                  RTGCPTR pvFault, RTGCPHYS GCPhysFault, void *pvUser)
{
    int             rc;
    ...
    else
    {
        ...
        switch (pRomPage->enmProt)
        {
            ...
            case PGMROMPROT_READ_RAM_WRITE_RAM: /* yes this will get here too, it's *way* simpler that way. */
            {
                ...
                void *pvDstPage;
#if defined(VPOX_WITH_PGM_NEM_MODE) && defined(IN_RING3)
 ...
 */
static VPOXSTRICTRC pgmPhysMmio2WriteHandlerCommon(PVMCC pVM, PVMCPUCC pVCpu, uintptr_t hMmio2, RTGCPHYS GCPhys, RTGCPTR GCPtr)
{
    /*
     * Get the MMIO2 range.
     */
 ...
 */
DECLEXPORT(VPOXSTRICTRC) pgmPhysMmio2WritePfHandler(PVMCC pVM, PVMCPUCC pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                    RTGCPTR pvFault, RTGCPHYS GCPhysFault, void *pvUser)
{
    RT_NOREF(pVCpu, uErrorCode, pRegFrame);
    VPOXSTRICTRC rcStrict = PGM_LOCK(pVM); /* We should already have it, but just make sure we do. */
...
#ifndef VPOX_WITH_RAM_IN_KERNEL
 ...
 *
 * @returns The following VPox status codes.
...
# endif
     ...
     * When VPOX_WITH_NEW_LAZY_PAGE_ALLOC isn't defined, there shouldn't be any.
     */
...
#elif defined(IN_RING0) && defined(VPOX_WITH_RAM_IN_KERNEL)
# ifdef VPOX_WITH_LINEAR_HOST_PHYS_MEM
...
static int pgmPhysPageMapCommon(PVMCC pVM, PPGMPAGE pPage, RTGCPHYS GCPhys, PPPGMPAGEMAP ppMap, void **ppv)
{
    ...
    *ppMap = NULL;
...
#else /* !VPOX_WITH_2X_4GB_ADDR_SPACE_IN_R0 */
        ...
        || PGM_PAGE_GET_TYPE(pPage) == PGMPAGETYPE_MMIO2_ALIAS_MMIO)
    {
        ...
        *ppMap = NULL;
# if   defined(IN_RING0) && defined(VPOX_WITH_RAM_IN_KERNEL) && defined(VPOX_WITH_LINEAR_HOST_PHYS_MEM)
...
# endif
    ...
    *ppMap = pMap;
    return VINF_SUCCESS;
# endif /* !IN_RING0 || !VPOX_WITH_RAM_IN_KERNEL */
#endif /* !VPOX_WITH_2X_4GB_ADDR_SPACE_IN_R0 */
}
...
#ifndef VPOX_WITH_2X_4GB_ADDR_SPACE_IN_R0
...
int pgmPhysPageLoadIntoTlbWithPage(PVMCC pVM, PPGMPAGE pPage, RTGCPHYS GCPhys)
{
        ...
        &&  !PGM_PAGE_IS_BALLOONED(pPage))
    {
        ...
        if (RT_FAILURE(rc))
            return rc;
# if !defined(IN_RING0) || !defined(VPOX_WITH_RAM_IN_KERNEL)
        pTlbe->pMap = pMap;
# endif
        pTlbe->pv = pv;
...
#if 0 /*def VPOX_WITH_2X_4GB_ADDR_SPACE_IN_R0 */
 ...
 *                      VPOX_WITH_2X_4GB_ADDR_SPACE_IN_R0 build option.
 *
 ...
 *
 * @returns Strict VPox status code in ring-0 and raw-mode, ignorable in ring-3.
 ...
 */
static VPOXSTRICTRC pgmPhysReadHandler(PVMCC pVM, PPGMPAGE pPage, RTGCPHYS GCPhys, void *pvBuf, size_t cb,
...
/** @todo Check how this can work for MMIO pages? */
    ...
    VPOXSTRICTRC rcStrict = VINF_PGM_HANDLER_DO_DEFAULT;
        ...
        || PGM_PAGE_IS_MMIO_OR_SPECIAL_ALIAS(pPage))
    {
        ...
        AssertLogRelMsg(PGM_HANDLER_PHYS_IS_VALID_STATUS(rcStrict, false),
                        ("rcStrict=%Rrc GCPhys=%RGp\n", VPOXSTRICTRC_VAL(rcStrict), GCPhys));
 ...
 *
 * @returns Strict VPox status code in raw-mode and ring-0, normal VPox status
 ...
 * @param   enmOrigin       The origin of this call.
 */
VMMDECL(VPOXSTRICTRC) PGMPhysRead(PVMCC pVM, RTGCPHYS GCPhys, void *pvBuf, size_t cbRead, PGMACCESSORIGIN enmOrigin)
{
    ...
    for (;;)
    {
        ...
        if (pRam && GCPhys >= pRam->GCPhys)
        {
            ...
            while (off < pRam->cb)
            {
                ...
                {
                    VPOXSTRICTRC rcStrict2 = pgmPhysReadHandler(pVM, pPage, pRam->GCPhys + off, pvBuf, cb, enmOrigin);
 ...
 * @param   enmOrigin   The origin of this call.
 */
static VPOXSTRICTRC pgmPhysWriteHandler(PVMCC pVM, PPGMPAGE pPage, RTGCPHYS GCPhys, void const *pvBuf, size_t cbWrite,
                                        PGMACCESSORIGIN enmOrigin)
{
    PGMPAGEMAPLOCK  PgMpLck;
    void           *pvDst = NULL;
            ...
            if (rcStrict == VINF_PGM_HANDLER_DO_DEFAULT)
            {
                ...
                AssertLogRelMsg(PGM_HANDLER_PHYS_IS_VALID_STATUS(rcStrict, true),
                                ("rcStrict=%Rrc GCPhys=%RGp pPage=%R[pgmpage] %s\n",
                                 VPOXSTRICTRC_VAL(rcStrict), GCPhys, pPage, pCur ? R3STRING(pCur->pszDesc) : ""));
        }
        else
            AssertLogRelMsgFailedReturn(("pgmPhysGCPhys2CCPtrInternal failed on %RGp / %R[pgmpage] -> %Rrc\n",
                                         GCPhys, pPage, VPOXSTRICTRC_VAL(rcStrict)), rcStrict);
    ...
    for (;;)
    {
         ...
         */
        VPOXSTRICTRC rcStrict2 = VINF_PGM_HANDLER_DO_DEFAULT;
        ...
        if (!offPhys)
        {
            ...
            AssertLogRelMsg(PGM_HANDLER_PHYS_IS_VALID_STATUS(rcStrict2, true),
                            ("rcStrict2=%Rrc (rcStrict=%Rrc) GCPhys=%RGp pPage=%R[pgmpage] %s\n", VPOXSTRICTRC_VAL(rcStrict2),
                             VPOXSTRICTRC_VAL(rcStrict), GCPhys, pPage,  pPhys ? R3STRING(pPhys->pszDesc) : ""));
        }
 ...
 */
VMMDECL(VPOXSTRICTRC) PGMPhysWrite(PVMCC pVM, RTGCPHYS GCPhys, const void *pvBuf, size_t cbWrite, PGMACCESSORIGIN enmOrigin)
{
    ...
    for (;;)
    {
        ...
        if (pRam && GCPhys >= pRam->GCPhys)
        {
            ...
            while (off < pRam->cb)
            {
                ...
                {
                    VPOXSTRICTRC rcStrict2 = pgmPhysWriteHandler(pVM, pPage, pRam->GCPhys + off, pvBuf, cb, enmOrigin);
                    if (PGM_PHYS_RW_IS_SUCCESS(rcStrict2))
                        PGM_PHYS_RW_DO_UPDATE_STRICT_RC(rcStrict, rcStrict2);
                    else
                    {
 ...
 *
 * @returns Strict VPox status, see PGMPhysRead for details.
 ...
 * @thread  EMT(pVCpu)
 */
VMMDECL(VPOXSTRICTRC) PGMPhysReadGCPtr(PVMCPUCC pVCpu, void *pvDst, RTGCPTR GCPtrSrc, size_t cb, PGMACCESSORIGIN enmOrigin)
{
    RTGCPHYS    GCPhys;
    uint64_t    fFlags;
    int         rc;
    ...
    for (;;)
    {
        ...
        if (cbRead < cb)
        {
            VPOXSTRICTRC rcStrict = PGMPhysRead(pVM, GCPhys, pvDst, cbRead, enmOrigin);
 ...
 *
 * @returns Strict VPox status, see PGMPhysWrite for details.
 * @retval  VERR_PAGE_TABLE_NOT_PRESENT if there is no page mapped at the
 *          specified virtual address.
 *
 * @param   pVCpu       The cross context virtual CPU structure of the calling EMT.
 ...
 * @param   enmOrigin       Who is calling.
 */
VMMDECL(VPOXSTRICTRC) PGMPhysWriteGCPtr(PVMCPUCC pVCpu, RTGCPTR GCPtrDst, const void *pvSrc, size_t cb, PGMACCESSORIGIN enmOrigin)
{
    RTGCPHYS    GCPhys;
    uint64_t    fFlags;
    int         rc;
    ...
    for (;;)
    {
        ...
        if (cbWrite < cb)
        {
            VPOXSTRICTRC rcStrict = PGMPhysWrite(pVM, GCPhys, pvSrc, cbWrite, enmOrigin);
            if (RT_LIKELY(rcStrict == VINF_SUCCESS))
            { /* likely */ }
            else
                return rcStrict;
 ...
 *
 *
 * @returns VPox status code suitable to scheduling.
 ...
 * additional checks.
 *
 * @returns VPox status code (no informational statuses).
 *
 * @param   pVM             The cross context VM structure.
 ...
 * or higher.
 *
 * @returns VPox status code from callback.
 * @param   pVM             The cross context VM structure.
