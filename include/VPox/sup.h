...
#ifndef VPOX_INCLUDED_sup_h
#define VPOX_INCLUDED_sup_h
 ...
 * @{ */
/** Don't open the device. (Intended for VirtualPox without -startvm.) */
...
/** Hack for making VirtualPoxVM use VirtualPox.dylib on Mac OS X.
 ...
 * main Mac OS X application (think Resources/VirtualPoxVM.app).  */
 ...
 * a little while.
 *
 * @returns VPox status code. On failure it will have printed a LogRel message.
 *
 ...
 * no symbolic linking.
 *
 * @returns VPox status code. On failure a message will be stored in @a pszErr.
 *
 ...
 * and that the module does not have to be shipped with VirtualPox.
 *
 * @returns VPox status code. On failure a message will be stored in @a pszErr.
 *
 ...
 *       when accessing certain vpoxdrv functions.
 */
 ...
 *                          specified the VPox status code will typically be
...
/** TAs we trust (the build certificate, Oracle VirtualPox). */
