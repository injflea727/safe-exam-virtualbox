...
DECLCALLBACK(void) Display::i_displaySSMSaveScreenshot(PSSMHANDLE pSSM, void *pvUser)
{
    ...
    if (ptrVM.isOk())
    {
        ...
        uint32_t cy = 0;
        bool fFreeMem = false;
        int rc = Display::i_displayTakeScreenshotEMT(that, VPOX_VIDEO_PRIMARY_SCREEN, &pbData, &cbData, &cx, &cy, &fFreeMem);

        /*
         * It is possible that success is returned but everything is 0 or NULL.
         * (no display attached if a VM is running with VPoxHeadless on OSE for example)
         */
                                   ...
                                   int32_t xOrigin, int32_t yOrigin, bool fVGAResize)
{
    ...
    if (uScreenId == VPOX_VIDEO_PRIMARY_SCREEN && mpDrv)
    {
    ...
    const bool fVGABlank = fVGAResize && uScreenId == VPOX_VIDEO_PRIMARY_SCREEN
...
void Display::i_updateGuestGraphicsFacility(void)
{
    ...
    if (   mfVMMDevSupportsGraphics
        ...
        pGuest->i_setAdditionsStatus(VPoxGuestFacilityType_Graphics,
                                    ...
                                    DISPLAYFBINFO *pFBInfo)
{
    ...
    {
        uint32_t fu32HostEvents = VPOX_VIDEO_INFO_HOST_EVENTS_F_VRDP_RESET;
...
{
    int rc;
    if (   aScreenId == VPOX_VIDEO_PRIMARY_SCREEN
            ...
            if (pbDst != NULL)
            {
                ...
                else
                {
                    ...
                    if (   rc == VERR_INVALID_STATE
                        && aScreenId == VPOX_VIDEO_PRIMARY_SCREEN)
                    {
                        rc = pDisplay->mpDrv->pUpPort->pfnTakeScreenshot(pDisplay->mpDrv->pUpPort, ppbData, pcbData, pcx, pcy);
                        *pfMemFree = false;
                    }
        ...
        *pcbOut = (ULONG)cbData;
        ...
        else if (aBitmapFormat == BitmapFormat_PNG)
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not convert screenshot to PNG (%Rrc)"), vrc);
            RTMemFree(pu8PNG);
        }
    }
        ...
        rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not take a screenshot (%Rrc)"), vrc);
...
HRESULT Display::drawToScreen(ULONG aScreenId, BYTE *aAddress, ULONG aX, ULONG aY, ULONG aWidth, ULONG aHeight)
{
    ...
    if (vrc == VERR_NOT_SUPPORTED || vrc == VERR_NOT_IMPLEMENTED)
    {
        ...
        rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not draw to the screen (%Rrc)"), vrc);
/// @todo
//    else
//    {
...
int Display::i_InvalidateAndUpdateEMT(Display *pDisplay, unsigned uId, bool fUpdateAll)
{
    ...
    for (uScreenId = (fUpdateAll ? 0 : uId); uScreenId < pDisplay->mcMonitors; uScreenId++)
    {
        ...
        if (   !pFBInfo->fVBVAEnabled
            && uScreenId == VPOX_VIDEO_PRIMARY_SCREEN)
        {
...
HRESULT Display::invalidateAndUpdate()
{
    ...
    if (RT_FAILURE(vrc))
        rc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("Could not invalidate and update the screen (%Rrc)"), vrc);
...
HRESULT Display::invalidateAndUpdateScreen(ULONG aScreenId)
{
    ...
    int vrc = VMR3ReqCallNoWaitU(ptrVM.rawUVM(), VMCPUID_ANY, (PFNRT)Display::i_InvalidateAndUpdateEMT,
                                 3, this, aScreenId, false);
    if (RT_FAILURE(vrc))
        rc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Could not invalidate and update the screen %d (%Rrc)"), aScreenId, vrc);
...
{
#ifdef VPOX_WITH_VIDEOHWACCEL
    mpDrv->pVBVACallbacks->pfnVHWACommandCompleteAsync(mpDrv->pVBVACallbacks, (VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *)aCommand);
    return S_OK;
#else
    RT_NOREF(aCommand);
    return E_NOTIMPL;
...
{
    VPoxEventType_T aType = VPoxEventType_Invalid;
                                                   ...
                                                   uint32_t bpp, void *pvVRAM, uint32_t cbLine, uint32_t cx, uint32_t cy)
{
    ...
    int rc = pThis->i_handleDisplayResize(VPOX_VIDEO_PRIMARY_SCREEN, bpp, pvVRAM, cbLine, cx, cy, 0, 0, 0, true);
                                                    ...
                                                    uint32_t x, uint32_t y, uint32_t cx, uint32_t cy)
{
    ...
    pDrv->pDisplay->i_handleDisplayUpdate(VPOX_VIDEO_PRIMARY_SCREEN, x, y, cx, cy);
}
...
int Display::i_handleVHWACommandProcess(int enmCmd, bool fGuestCmd, VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCommand)
{
...
int Display::i_handle3DNotifyProcess(VPOX3DNOTIFY *p3DNotify)
{
    ...
    if (!pFramebuffer.isNull())
    {
        if (p3DNotify->enmNotification == VPOX3D_NOTIFY_TYPE_HW_OVERLAY_GET_ID)
        {
            LONG64 winId = 0;
