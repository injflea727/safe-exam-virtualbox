...
public:
    ...
    void i_setAdditionsInfo(const com::Utf8Str &aInterfaceVersion, VPOXOSTYPE aOsType);
    void i_setAdditionsInfo2(uint32_t a_uFullVersion, const char *a_pszName, uint32_t a_uRevision, uint32_t a_fFeatures);
    bool i_facilityIsActive(VPoxGuestFacilityType enmFacility);
    bool i_facilityUpdate(VPoxGuestFacilityType a_enmFacility, VPoxGuestFacilityStatus a_enmStatus,
                          uint32_t a_fFlags, PCRTTIMESPEC a_pTimeSpecTS);
    ComObjPtr<Console> i_getConsole(void) { return mParent; }
    void i_setAdditionsStatus(VPoxGuestFacilityType a_enmFacility, VPoxGuestFacilityStatus a_enmStatus,
                              uint32_t a_fFlags, PCRTTIMESPEC a_pTimeSpecTS);
    void i_onUserStateChange(Bstr aUser, Bstr aDomain, VPoxGuestUserState enmState, const uint8_t *puDetails, uint32_t cbDetails);
    ...
    VPOXOSTYPE  i_getGuestOSType(void) const { return mData.mOSType; }
    /** Checks if the guest OS type is part of the windows NT family.  */
    bool        i_isGuestInWindowsNtFamily(void) const
    {
        return mData.mOSType < VPOXOSTYPE_OS2 && mData.mOSType >= VPOXOSTYPE_WinNT;
    }
#ifdef VPOX_WITH_GUEST_CONTROL
    int         i_dispatchToSession(PVPOXGUESTCTRLHOSTCBCTX pCtxCb, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb);
    ...
    /** Returns the VPOX_GUESTCTRL_GF_0_XXX mask reported by the guest. */
    uint64_t    i_getGuestControlFeatures0() const { return mData.mfGuestFeatures0; }
    /** Returns the VPOX_GUESTCTRL_GF_1_XXX mask reported by the guest. */
...
private:
    ...
    struct Data
    {
        Data() : mOSType(VPOXOSTYPE_Unknown),  mAdditionsRunLevel(AdditionsRunLevelType_None)
        ...
        VPOXOSTYPE                  mOSType;        /**@< For internal used. VPOXOSTYPE_Unknown if not reported. */
        Utf8Str                     mOSTypeId;
        ...
        /** Guest control features (reported by the guest), VPOX_GUESTCTRL_GF_0_XXX. */
        uint64_t                    mfGuestFeatures0;
        /** Guest control features (reported by the guest), VPOX_GUESTCTRL_GF_1_XXX. */
