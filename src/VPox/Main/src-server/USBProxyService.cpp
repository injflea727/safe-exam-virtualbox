...
/** @file
 * VirtualPox USB Proxy Service (base) class.
 */
                                            ...
                                            const std::vector<com::Utf8Str> &aPropertyNames, const std::vector<com::Utf8Str> &aPropertyValues)
{
    ...
    {
        alock.release();
        AutoWriteLock vpoxLock(mHost->i_parent() COMMA_LOCKVAL_SRC_POS);
...
HRESULT USBProxyService::setError(HRESULT aResultCode, const char *aText, ...)
{
    va_list va;
    va_start(va, aText);
    HRESULT rc = VirtualPoxBase::setErrorInternal(aResultCode,
                                                  COM_IIDOF(IHost),
                                                  "USBProxyService",
                                                  Utf8Str(aText, va),
                                                  false /* aWarning*/,
