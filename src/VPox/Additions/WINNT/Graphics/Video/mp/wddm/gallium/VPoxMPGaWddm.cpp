...
#include "VPoxMPGaWddm.h"
#include "../VPoxMPVidPn.h"
#include "VPoxMPGaExt.h"
...
{
    VPOXWDDM_EXT_GA *pGaDevExt = pDevExt->pGa;
...
NTSTATUS GaAdapterStart(PVPOXMP_DEVEXT pDevExt)
{
    ...
    {
        VPOXWDDM_EXT_GA *pGaDevExt = (VPOXWDDM_EXT_GA *)GaMemAllocZero(sizeof(*pGaDevExt));
        if (pGaDevExt)
        {
                     ...
                     VPOXGAHWINFO *pHWInfo)
{
    ...
    switch (enmHwType)
    {
        case VPOXVIDEO_HWTYPE_VMSVGA:
            pHWInfo->u32HwType = VPOX_GA_HW_TYPE_VMSVGA;
            break;
        default:
            Status = STATUS_NOT_SUPPORTED;
    }
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        if (pHWInfo->u32HwType == VPOX_GA_HW_TYPE_VMSVGA)
        {
                         ...
                         PVPOXWDDM_CONTEXT pContext)
{
    ...
    VPOXWDDM_EXT_VMSVGA *pSvga = pGaDevExt->hw.pSvga;
    uint32_t u32Cid;
...
static NTSTATUS gaSurfaceDefine(PVPOXWDDM_EXT_GA pGaDevExt,
...
static NTSTATUS gaSurfaceDestroy(PVPOXWDDM_EXT_GA pGaDevExt,
                                 uint32_t u32Sid)
{
    VPOXWDDM_EXT_VMSVGA *pSvga = pGaDevExt->hw.pSvga;
...
static NTSTATUS gaSharedSidInsert(PVPOXWDDM_EXT_GA pGaDevExt,
                                  uint32_t u32Sid,
...
static NTSTATUS gaSharedSidRemove(PVPOXWDDM_EXT_GA pGaDevExt,
                                  uint32_t u32Sid)
{
    VPOXWDDM_EXT_VMSVGA *pSvga = pGaDevExt->hw.pSvga;
...
static NTSTATUS gaPresent(PVPOXWDDM_EXT_GA pGaDevExt,
                          uint32_t u32Sid,
                          uint32_t u32Width,
                          uint32_t u32Height,
...
static void gaReportFence(PVPOXMP_DEVEXT pDevExt)
{
    ...
    PVPOXWDDM_EXT_VMSVGA pSvga = pGaDevExt->hw.pSvga;
...
static NTSTATUS gaPresentBlt(DXGKARG_PRESENT *pPresent,
                             ...
                             PVPOXWDDM_ALLOCATION pSrcAlloc,
                             DXGK_ALLOCATIONLIST *pDst,
                             PVPOXWDDM_ALLOCATION pDstAlloc,
                             uint8_t *pu8Target, uint32_t cbTarget, uint32_t *pu32TargetOut)
{
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    for (iSubRect = pPresent->MultipassOffset; iSubRect < pPresent->SubRectCnt; ++iSubRect)
    {
        ...
        if (iSubRect == 0)
        {
            if (   pSrcAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_SHADOWSURFACE
                || pSrcAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_STAGINGSURFACE)
            {
            ...
            else if (   pDstAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_SHADOWSURFACE
                     || pDstAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_STAGINGSURFACE)
            {
                /* Define GMRFB to point to the shadow/staging surface. */
                Status = gaGMRFBToVRAMSurface(pPresent, pSvga,
        ...
        if (pDstAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_SHAREDPRIMARYSURFACE)
        {
            /* To screen. */
            if (   pSrcAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_SHADOWSURFACE
                || pSrcAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_STAGINGSURFACE)
            {
                ...
                GALOGG(GALOG_GROUP_PRESENT, ("Blt: %s(%d) 0x%08X -> SHAREDPRIMARYSURFACE 0x%08X\n",
                    vpoxWddmAllocTypeString(pSrcAlloc),
            ...
            else if (pSrcAlloc->enmType == VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC)
            {
                /* From a surface. */
                ...
                || pDstAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_STAGINGSURFACE)
        {
            ...
            if (pSrcAlloc->enmType == VPOXWDDM_ALLOC_TYPE_STD_SHAREDPRIMARYSURFACE)
            {
                /* From screen. */
                GALOGG(GALOG_GROUP_PRESENT, ("Blt: SHAREDPRIMARYSURFACE 0x%08X -> %s(%d) 0x%08X\n",
                    pSrc->PhysicalAddress.LowPart,
                    vpoxWddmAllocTypeString(pDstAlloc),
...
NTSTATUS APIENTRY GaDxgkDdiRender(const HANDLE hContext, DXGKARG_RENDER *pRender)
{
    ...
    AssertReturn(pContext && pContext->enmType == VPOXWDDM_CONTEXT_TYPE_GA_3D, STATUS_INVALID_PARAMETER);
...
static NTSTATUS gaSoftwarePagingTransfer(PVPOXMP_DEVEXT pDevExt,
...
{
    PVPOXMP_DEVEXT pDevExt = (PVPOXMP_DEVEXT)Context;
    gaReportFence(pDevExt);
    return TRUE;
}
                                  ...
                                  const DXGKARG_ESCAPE *pEscape)
{
    ...
    const VPOXDISPIFESCAPE *pEscapeHdr = (VPOXDISPIFESCAPE *)pEscape->pPrivateDriverData;
    switch (pEscapeHdr->escapeCode)
    {
        case VPOXESC_GAGETCID:
        {
            if (!pContext)
            {
                Status = STATUS_INVALID_PARAMETER;
            ...
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GAGETCID))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GAGETCID *pGaGetCid = (VPOXDISPIFESCAPE_GAGETCID *)pEscapeHdr;
            pGaGetCid->u32Cid = pContext->u32Cid;
            Status = STATUS_SUCCESS;
            break;
        }
        case VPOXESC_GAREGION:
        {
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GAREGION))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GAREGION *pGaRegion = (VPOXDISPIFESCAPE_GAREGION *)pEscapeHdr;
        ...
        case VPOXESC_GAPRESENT:
        {
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GAPRESENT))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GAPRESENT *pGaPresent = (VPOXDISPIFESCAPE_GAPRESENT *)pEscapeHdr;
        ...
        case VPOXESC_GASURFACEDEFINE:
        {
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GASURFACEDEFINE))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GASURFACEDEFINE *pGaSurfaceDefine = (VPOXDISPIFESCAPE_GASURFACEDEFINE *)pEscapeHdr;
            if (pEscape->PrivateDriverDataSize - sizeof(VPOXDISPIFESCAPE_GASURFACEDEFINE) < pGaSurfaceDefine->cbReq)
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }
        ...
        case VPOXESC_GASURFACEDESTROY:
        {
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GASURFACEDESTROY))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GASURFACEDESTROY *pGaSurfaceDestroy = (VPOXDISPIFESCAPE_GASURFACEDESTROY *)pEscapeHdr;
            Status = gaSurfaceDestroy(pDevExt->pGa, pGaSurfaceDestroy->u32Sid);
            break;
        }
        case VPOXESC_GASHAREDSID:
        {
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GASHAREDSID))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GASHAREDSID *pGaSharedSid = (VPOXDISPIFESCAPE_GASHAREDSID *)pEscapeHdr;
        ...
        case VPOXESC_GAFENCECREATE:
        {
            ...
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GAFENCECREATE))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GAFENCECREATE *pFenceCreate = (VPOXDISPIFESCAPE_GAFENCECREATE *)pEscapeHdr;
            Status = GaFenceCreate(pDevExt->pGa, pDevice, &pFenceCreate->u32FenceHandle);
            break;
        }
        case VPOXESC_GAFENCEQUERY:
        {
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GAFENCEQUERY))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GAFENCEQUERY *pFenceQuery = (VPOXDISPIFESCAPE_GAFENCEQUERY *)pEscapeHdr;
        ...
        case VPOXESC_GAFENCEWAIT:
        {
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GAFENCEWAIT))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GAFENCEWAIT *pFenceWait = (VPOXDISPIFESCAPE_GAFENCEWAIT *)pEscapeHdr;
            Status = GaFenceWait(pDevExt->pGa, pFenceWait->u32FenceHandle, pFenceWait->u32TimeoutUS);
            break;
        }
        case VPOXESC_GAFENCEUNREF:
        {
            if (pEscape->PrivateDriverDataSize < sizeof(VPOXDISPIFESCAPE_GAFENCEUNREF))
            {
                Status = STATUS_INVALID_PARAMETER;
                break;
            }

            VPOXDISPIFESCAPE_GAFENCEUNREF *pFenceUnref = (VPOXDISPIFESCAPE_GAFENCEUNREF *)pEscapeHdr;
...
DECLINLINE(VPOXVIDEOOFFSET) vpoxWddmAddrVRAMOffset(VPOXWDDM_ADDR const *pAddr)
{
...
static void vpoxWddmRectCopy(void *pvDst, uint32_t cbDstBytesPerPixel, uint32_t cbDstPitch,
...
static NTSTATUS gaSourceBlitToScreen(PVPOXMP_DEVEXT pDevExt, VPOXWDDM_SOURCE *pSource, RECT const *pRect)
{
    NTSTATUS Status = STATUS_SUCCESS;
    PVPOXWDDM_EXT_VMSVGA pSvga = pDevExt->pGa->hw.pSvga;
                                              ...
                                              const DXGKARG_PRESENT_DISPLAYONLY *pPresentDisplayOnly)
{
    ...
    VPOXVIDEOOFFSET const offVRAM = vpoxWddmAddrVRAMOffset(&pSource->AllocData.Addr);
    AssertReturn(offVRAM != VPOXVIDEOOFFSET_VOID, STATUS_SUCCESS); /* Ignore such VidPn sources. */

    for (ULONG i = 0; i < pPresentDisplayOnly->NumMoves; ++i)
    {
        RECT *pRect = &pPresentDisplayOnly->pMoves[i].DestRect;
        vpoxWddmRectCopy(pDevExt->pvVisibleVram + offVRAM,    // dst pointer
...
NTSTATUS GaVidPnSourceReport(PVPOXMP_DEVEXT pDevExt, VPOXWDDM_SOURCE *pSource)
{
    ...
    VPOXVIDEOOFFSET offVRAM = vpoxWddmAddrVRAMOffset(&pSource->AllocData.Addr);
...
{
    POINT Pos = {0};
    NTSTATUS Status = vpoxWddmDisplaySettingsQueryPos(pDevExt, iSource, &Pos);
    if (NT_SUCCESS(Status))
    {
        PVPOXWDDM_SOURCE pSource = &pDevExt->aSources[iSource];
