...
USBLIB_DECL(int) USBLibInit(void)
{
    ...
    RTFILE File;
    int rc = RTFileOpen(&File, VPOXUSB_DEVICE_NAME, RTFILE_O_READWRITE | RTFILE_O_OPEN | RTFILE_O_DENY_NONE);
    if (RT_FAILURE(rc))
    {
        LogRel((USBLIBR3 ":failed to open the VPoxUSB monitor device node '%s' rc=%Rrc\n", VPOXUSB_DEVICE_NAME, rc));
        return rc;
    }
    g_hFile = File;
     ...
     */
    VPOXUSBREQ_GET_VERSION Req;
    bzero(&Req, sizeof(Req));
    rc = usblibDoIOCtl(VPOXUSBMON_IOCTL_GET_VERSION, &Req, sizeof(Req));
    if (RT_SUCCESS(rc))
    {
        if (   Req.u32Major != VPOXUSBMON_VERSION_MAJOR
            || Req.u32Minor < VPOXUSBMON_VERSION_MINOR)
        {
            rc = VERR_VERSION_MISMATCH;
            LogRel((USBLIBR3 ":USBMonitor version mismatch! driver v%d.%d, expecting ~v%d.%d\n",
                        Req.u32Major, Req.u32Minor, VPOXUSBMON_VERSION_MAJOR, VPOXUSBMON_VERSION_MINOR));
...
USBLIB_DECL(void *) USBLibAddFilter(PCUSBFILTER pFilter)
{
    ...
    VPOXUSBREQ_ADD_FILTER Req;
    Req.Filter = *pFilter;
    Req.uId = 0;

    int rc = usblibDoIOCtl(VPOXUSBMON_IOCTL_ADD_FILTER, &Req, sizeof(Req));
    if (RT_SUCCESS(rc))
        return (void *)Req.uId;

    AssertMsgFailed((USBLIBR3 ":VPOXUSBMON_IOCTL_ADD_FILTER  failed! rc=%Rrc\n", rc));
    return NULL;
}
...
USBLIB_DECL(void) USBLibRemoveFilter(void *pvId)
{
    ...
    VPOXUSBREQ_REMOVE_FILTER Req;
    Req.uId = (uintptr_t)pvId;

    int rc = usblibDoIOCtl(VPOXUSBMON_IOCTL_REMOVE_FILTER, &Req, sizeof(Req));
    if (RT_SUCCESS(rc))
        return;

    AssertMsgFailed((USBLIBR3 ":VPOXUSBMON_IOCTL_REMOVE_FILTER failed! rc=%Rrc\n", rc));
}
...
USBLIB_DECL(int) USBLibGetClientInfo(char *pszDeviceIdent, char **ppszClientPath, int *pInstance)
{
    ...
    VPOXUSBREQ_CLIENT_INFO Req;
    bzero(&Req, sizeof(Req));
    RTStrPrintf(Req.szDeviceIdent, sizeof(Req.szDeviceIdent), "%s", pszDeviceIdent);

    int rc = usblibDoIOCtl(VPOXUSBMON_IOCTL_CLIENT_INFO, &Req, sizeof(Req));
    if (RT_SUCCESS(rc))
    {
        *pInstance = Req.Instance;
        ...
        LogRel((USBLIBR3 ":USBLibGetClientInfo VPOXUSBMON_IOCTL_CLIENTPATH failed! rc=%Rrc\n", rc));
...
USBLIB_DECL(int) USBLibResetDevice(char *pszDevicePath, bool fReattach)
{
    ...
    size_t cbReq  = sizeof(VPOXUSBREQ_RESET_DEVICE) + cbPath;
    VPOXUSBREQ_RESET_DEVICE *pReq = (VPOXUSBREQ_RESET_DEVICE *)RTMemTmpAllocZ(cbReq);
    if (RT_UNLIKELY(!pReq))
    ...
    int rc = usblibDoIOCtl(VPOXUSBMON_IOCTL_RESET_DEVICE, pReq, cbReq);
    if (RT_FAILURE(rc))
        LogRel((USBLIBR3 ":VPOXUSBMON_IOCTL_RESET_DEVICE failed! rc=%Rrc\n", rc));
