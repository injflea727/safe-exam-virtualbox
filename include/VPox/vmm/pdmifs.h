...
#ifndef VPOX_INCLUDED_vmm_pdmifs_h
#define VPOX_INCLUDED_vmm_pdmifs_h
 ...
 * which normally means the same, i.e. the Main or VPoxBFE API.  Picture the
...
typedef struct PDMIMOUSEPORT
{
     ...
     *
     * @returns VPox status code.  Return VERR_TRY_AGAIN if you cannot process the
...
typedef struct VPOX3DNOTIFY VPOX3DNOTIFY;
/** Pointer to a 2D graphics acceleration command. */
typedef struct VPOXVHWACMD VPOXVHWACMD;
...
typedef struct PDMIVMMDEVCONNECTOR
{
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(void, pfnUpdateGuestInfo,(PPDMIVMMDEVCONNECTOR pInterface, const struct VPoxGuestInfo *pGuestInfo));
     ...
     *                              Use VPOX_FULL_VERSION_GET_MAJOR,
     *                              VPOX_FULL_VERSION_GET_MINOR and
     *                              VPOX_FULL_VERSION_GET_BUILD to access it.
     ...
     *                              by an underscore (VPOX_VERSION_STRING fashion).
     ...
     * @return  VPox rc. VINF_SUCCESS if VBVA was enabled.
     * @thread  The emulation thread.
     */
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnReportStatistics,(PPDMIVMMDEVCONNECTOR pInterface, struct VPoxGuestStatistics *pGuestStats));
 ...
 */
typedef struct VPOXHGCMCMD *PVPOXHGCMCMD;
...
typedef struct PDMIHGCMPORT
{
     ...
     * @param   pInterface          Pointer to this interface.
     * @param   rc                  The return code (VPox error code).
     * @param   pCmd                A pointer that identifies the completed command.
     */
    DECLR3CALLBACKMEMBER(int, pfnCompleted,(PPDMIHGCMPORT pInterface, int32_t rc, PVPOXHGCMCMD pCmd));
     ...
     */
    DECLR3CALLBACKMEMBER(bool, pfnIsCmdRestored,(PPDMIHGCMPORT pInterface, PVPOXHGCMCMD pCmd));
     ...
     * @param   pCmd                The command we're checking on.
     */
    DECLR3CALLBACKMEMBER(bool, pfnIsCmdCancelled,(PPDMIHGCMPORT pInterface, PVPOXHGCMCMD pCmd));
     ...
     */
    DECLR3CALLBACKMEMBER(uint32_t, pfnGetRequestor,(PPDMIHGCMPORT pInterface, PVPOXHGCMCMD pCmd));
...
typedef struct PDMIHGCMCONNECTOR
{
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnConnect,(PPDMIHGCMCONNECTOR pInterface, PVPOXHGCMCMD pCmd, PHGCMSERVICELOCATION pServiceLocation, uint32_t *pu32ClientID));
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnDisconnect,(PPDMIHGCMCONNECTOR pInterface, PVPOXHGCMCMD pCmd, uint32_t u32ClientID));
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnCall,(PPDMIHGCMCONNECTOR pInterface, PVPOXHGCMCMD pCmd, uint32_t u32ClientID, uint32_t u32Function,
                                       uint32_t cParms, struct VPOXHGCMSVCPARM *paParms, uint64_t tsArrival));
     ...
     */
    DECLR3CALLBACKMEMBER(void, pfnCancelled,(PPDMIHGCMCONNECTOR pInterface, PVPOXHGCMCMD pCmd, uint32_t idClient));
...
#endif /* !VPOX_INCLUDED_vmm_pdmifs_h */
