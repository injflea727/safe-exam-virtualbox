/* $Id: VPoxManageDHCPServer.cpp $ */
/** @file
 * VPoxManage - Implementation of dhcpserver command.
 */
...
static ComPtr<IDHCPServer> dhcpdFindServer(PDHCPDCMDCTX pCtx)
{
    ...
    if (pCtx->pszNetwork || pCtx->pszInterface)
    {
        ...
        if (!pCtx->pszNetwork)
        {
            ...
            CHECK_ERROR2_RET(hrc, pCtx->pArg->virtualPox, COMGETTER(Host)(ptrIHost.asOutParam()), ptrRet);
         ...
         */
        hrc = pCtx->pArg->virtualPox->FindDHCPServerByNetworkName(bstrNetName.raw(), ptrRet.asOutParam());
        if (SUCCEEDED(hrc))
            return ptrRet;
        if (pCtx->pszNetwork)
...
static DECLCALLBACK(RTEXITCODE) dhcpdHandleAddAndModify(PDHCPDCMDCTX pCtx, int argc, char **argv)
{
    ...
    for (size_t iPass = 0; iPass < 2; iPass++)
    {
        ...
        {
            ComPtr<IHost> host;
            CHECK_ERROR(pCtx->pArg->virtualPox, COMGETTER(Host)(host.asOutParam()));
        ...
        rc = pCtx->pArg->virtualPox->FindDHCPServerByNetworkName(NetName.mutableRaw(), ptrDHCPServer.asOutParam());
        if (pCtx->pCmdDef->fSubcommandScope == HELP_SCOPE_DHCPSERVER_ADD)
        {
            if (SUCCEEDED(rc))
                return errorArgument("DHCP server already exists");

            CHECK_ERROR(pCtx->pArg->virtualPox, CreateDHCPServer(NetName.mutableRaw(), ptrDHCPServer.asOutParam()));
            if (FAILED(rc))
...
static DECLCALLBACK(RTEXITCODE) dhcpdHandleRemove(PDHCPDCMDCTX pCtx, int argc, char **argv)
{
    ...
    {
        HRESULT hrc;
        CHECK_ERROR2(hrc, pCtx->pArg->virtualPox, RemoveDHCPServer(ptrDHCPServer));
        if (SUCCEEDED(hrc))
...
RTEXITCODE handleDHCPServer(HandlerArg *pArg)
{
    ...
    /*
     * VPoxManage dhcpserver [common-options] subcommand ...
     */
    DHCPDCMDCTX CmdCtx;
    CmdCtx.pArg         = pArg;
    CmdCtx.pCmdDef      = NULL;
