 ...
 *
 * Two classes, MainConfigFile and MachineConfigFile, represent the VirtualPox.xml and
 ...
 *
 * The code can read all VirtualPox settings files version 1.3 and higher. That version was
 * written by VirtualPox 2.0. It can write settings version 1.7 (used by VirtualPox 2.2 and
 * 3.0) and 1.9 (used by VirtualPox 3.1) and newer ones obviously.
 *
 * The settings versions enum is defined in src/VPox/Main/idl/VirtualPox.xidl. To introduce
 * a new settings version (should be necessary at most once per VirtualPox major release,
 ...
 *
 * Certainly ConfigFileBase::ConfigFileBase() will. Change VPOX_XML_VERSION below as well.
 * VPOX_XML_VERSION does not have to be changed if the settings for a default VM do not
 * touch newly introduced attributes or tags. It has the benefit that older VirtualPox
 ...
 * SettingsVersion enumeration in src/VPox/Main/idl/VirtualPox.xidl for details about which
 * version was used when.
 *
 * The settings versions checks are necessary because since version 3.1, VirtualPox no longer
 ...
 * settings file of an older version, then an old VirtualPox (before 3.1) will attempt to
 ...
 *      version (stored in m->sv) is high enough. That is, for VirtualPox 4.0, write it
 *      only if (m->sv >= SettingsVersion_v1_11).
 *
 *   5) You _must_ update xml/VirtualPox-settings.xsd to contain the new tags and attributes.
...
/** VirtualPox XML settings namespace */
#define VPOX_XML_NAMESPACE      "http://www.virtualpox.org/"

/** VirtualPox XML schema location (relative URI) */
#define VPOX_XML_SCHEMA "VirtualPox-settings.xsd"

/** VirtualPox XML settings version number substring ("x.y")  */
#define VPOX_XML_VERSION        "1.12"

/** VirtualPox OVF settings import default version number substring ("x.y").
 *
 * Think twice before changing this, as all VirtualPox versions before 5.1
 ...
 * software out there creates OVF files with the VirtualPox specific settings
 * but lacking the version attribute. This shouldn't happen any more, but
 * breaking existing OVF files isn't nice. */
#define VPOX_XML_IMPORT_VERSION "1.15"

/** VirtualPox XML settings version platform substring */
#if defined (RT_OS_DARWIN)
#   define VPOX_XML_PLATFORM     "macosx"
#elif defined (RT_OS_FREEBSD)
#   define VPOX_XML_PLATFORM     "freebsd"
#elif defined (RT_OS_LINUX)
#   define VPOX_XML_PLATFORM     "linux"
#elif defined (RT_OS_NETBSD)
#   define VPOX_XML_PLATFORM     "netbsd"
#elif defined (RT_OS_OPENBSD)
#   define VPOX_XML_PLATFORM     "openbsd"
#elif defined (RT_OS_OS2)
#   define VPOX_XML_PLATFORM     "os2"
#elif defined (RT_OS_SOLARIS)
#   define VPOX_XML_PLATFORM     "solaris"
#elif defined (RT_OS_WINDOWS)
#   define VPOX_XML_PLATFORM     "windows"
#else
#   error Unsupported platform!
#endif

/** VirtualPox XML settings full version string ("x.y-platform") */
#define VPOX_XML_VERSION_FULL   VPOX_XML_VERSION "-" VPOX_XML_PLATFORM

/** VirtualPox OVF import default settings full version string ("x.y-platform") */
#define VPOX_XML_IMPORT_VERSION_FULL   VPOX_XML_IMPORT_VERSION "-" VPOX_XML_PLATFORM
    ...
    : m(new Data)
{
    ...
    if (pstrFilename)
    {
        try
        {
            ...
            if (!m->pelmRoot || !m->pelmRoot->nameEquals("VirtualPox"))
                throw ConfigFileError(this, m->pelmRoot, N_("Root element in VirtualPox settings files must be \"VirtualPox\""));

            if (!(m->pelmRoot->getAttributeValue("version", m->strSettingsVersionFull)))
                throw ConfigFileError(this, m->pelmRoot, N_("Required VirtualPox/@version attribute is missing"));
    ...
    else
    {
        ...
        m->strSettingsVersionFull = VPOX_XML_VERSION_FULL;
...
/**
 * Reads a media registry entry from the main VirtualPox.xml file.
 *
 * Whereas the current media registry code is fairly straightforward, it was quite a mess
 * with settings format before 1.4 (VirtualPox 2.0 used settings format 1.3). The elements
...
/**
 * Reads a media registry entry from the main VirtualPox.xml file and recurses
 ...
 * VirtualPox 4.0, we can have media registries in both.
 *
 ...
 * VirtualPox settings version (e.g. "1.10-linux"). Used by
 * the XML format for the root element and by the OVF export
 * for the vpox:Machine element.
 * @param elm
 */
...
void ConfigFileBase::specialBackupIfFirstBump()
{
       ...
       )
    {
        ...
        strFilenameNew.append(strExt);                          // .xml for main config, .vpox for machine config
 ...
 * root "VirtualPox" element set up. This is used by both
...
void ConfigFileBase::createStubDocument()
{
    ...
    m->pelmRoot = m->pDoc->createRootElement("VirtualPox",
                                             "\n"
                                             "** DO NOT EDIT THIS FILE.\n"
                                             "** If you make changes to this file while any VirtualPox related application\n"
                                             "** is running, your changes will be overwritten later, without taking effect.\n"
                                             "** Use VPoxManage or the VirtualPox Manager GUI to make changes.\n"
);
    m->pelmRoot->setAttribute("xmlns", VPOX_XML_NAMESPACE);
...
#ifdef VPOX_WITH_SETTINGS_SCHEMA
    m->pelmRoot->setAttribute("xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance");
    m->pelmRoot->setAttribute("xsi:schemaLocation", VPOX_XML_NAMESPACE " " VPOX_XML_SCHEMA);
#endif
...
////////////////////////////////////////////////////////////////////////////////
//
// Structures shared between Machine XML and VirtualPox.xml
//
////////////////////////////////////////////////////////////////////////////////
...
/**
 * Reads one \<MachineEntry\> from the main VirtualPox.xml file.
    ...
    : ConfigFileBase(pstrFilename)
{
    ...
    if (    (!llDhcpServers.size())
         && (    (!pstrFilename)                    // empty VirtualPox.xml file
       ...
       )
    {
        DHCPServer srv;
#ifdef RT_OS_WINDOWS
        srv.strNetworkName = "HostInterfaceNetworking-VirtualPox Host-Only Ethernet Adapter";
#else
        srv.strNetworkName = "HostInterfaceNetworking-vpoxnet0";
#endif
...
void MainConfigFile::bumpSettingsVersionIfNeeded()
{
    ...
    {
        // VirtualPox 6.1 adds support for cloud networks.
    ...
    {
        // VirtualPox 5.1 add support for additional USB device sources.
    ...
    {
        // VirtualPox 4.3 adds NAT networks.
...
/**
 * Called from the IVirtualPox interface to write out VirtualPox.xml. This
...
GraphicsAdapter::GraphicsAdapter() :
    graphicsControllerType(GraphicsControllerType_VPoxVGA),
    ulVRAMSizeMB(8),
    cMonitors(1),
    fAccelerate3D(false),
...
{
    return graphicsControllerType == GraphicsControllerType_VPoxVGA
        && ulVRAMSizeMB == 8
        && cMonitors <= 1
        && !fAccelerate3D
        && !fAccelerate2DVideo;
 ...
 * i.e. files created by VirtualPox 4.0 and higher).
 * @return
 */
 ...
 *
 * This is used by the OVF code if a <vpox:Machine> element has been encountered
...
{
    // Ideally the version should be mandatory, but since VirtualPox didn't
    // care about it until 5.1 came with different defaults, there are OVF
    // files created by magicians (not using VirtualPox, which always wrote it)
    ...
    if (!(elmMachine.getAttributeValue("version", m->strSettingsVersionFull)))
        m->strSettingsVersionFull = VPOX_XML_IMPORT_VERSION_FULL;
                                            ...
                                            NetworkAdaptersList &ll)
{
    ...
    while ((pelmAdapter = nl1.forAllNodes()))
    {
        ...
        if (m->sv >= SettingsVersion_v1_16)
        {
            /* Starting with VirtualPox 5.1 the default is cable connected and
...
void MachineConfigFile::readAttachedNetworkMode(const xml::ElementNode &elmMode, bool fEnabled, NetworkAdapter &nic)
{
    ...
    else if (elmMode.nameEquals("VDE"))
    {
        ...
        // settings, so it's not mentioned in VirtualPox-settings.xsd)
        enmAttachmentType = NetworkAttachmentType_Generic;
                                         ...
                                         AudioAdapter &aa)
{
    ...
    if (elmAudioAdapter.getAttributeValue("driver", strTemp))
    {
        ...
        else if (strTemp == "DEFAULT") /* Kludge for setttings >= 1.19 (VPox 7.0). */
        {
                                     ...
                                     Hardware &hw)
{
    if (m->sv >= SettingsVersion_v1_16)
    {
        /* Starting with VirtualPox 5.1 the default is Default, before it was
    ...
    {
        /* Starting with VirtualPox 5.2 the default is disabled, before it was
    ...
    while ((pelmHwChild = nl1.forAllNodes()))
    {
        ...
        else if (pelmHwChild->nameEquals("Display"))
        {
            ...
            if (!pelmHwChild->getAttributeValue("controller", strGraphicsControllerType))
                hw.graphicsAdapter.graphicsControllerType = GraphicsControllerType_VPoxVGA;
            else
            {
                strGraphicsControllerType.toUpper();
                GraphicsControllerType_T type;
                if (strGraphicsControllerType == "VPOXVGA")
                    type = GraphicsControllerType_VPoxVGA;
                else if (strGraphicsControllerType == "VMSVGA")
                    type = GraphicsControllerType_VMSVGA;
                else if (strGraphicsControllerType == "VPOXSVGA")
                    type = GraphicsControllerType_VPoxSVGA;
                                       ...
                                       MachineUserData *pUserData)
{
    ...
    if (   pUserData->strTeleporterPassword.isNotEmpty()
        && !VPoxIsPasswordHashed(&pUserData->strTeleporterPassword))
        VPoxHashPassword(&pUserData->strTeleporterPassword);
}
                                         ...
                                         std::list<xml::ElementNode*> *pllElementsWithUuidAttributes)
{
    ...
    if (!hw.graphicsAdapter.areDefaultSettings())
    {
        ...
        if (hw.graphicsAdapter.graphicsControllerType != GraphicsControllerType_VPoxVGA)
        {
            const char *pcszGraphics;
            switch (hw.graphicsAdapter.graphicsControllerType)
            {
                case GraphicsControllerType_VPoxVGA:            pcszGraphics = "VPoxVGA"; break;
                case GraphicsControllerType_VMSVGA:             pcszGraphics = "VMSVGA"; break;
                case GraphicsControllerType_VPoxSVGA:           pcszGraphics = "VPoxSVGA"; break;
    ...
    if (!hw.vrdeSettings.areDefaultSettings(m->sv))
    {
        ...
        if (m->sv < SettingsVersion_v1_11)
        {
            /* In VPox 4.0 these attributes are replaced with "Properties". */
            Utf8Str strPort;
 ...
 *      settings version is at least v1.11 (VirtualPox 4.0).
 *
 ...
 *
 *  --  BuildMachineXML_WriteVPoxVersionAttribute: If set, add a settingsVersion
 *      attribute to the machine tag with the vpox settings version. This is for
...
void MachineConfigFile::buildMachineXML(xml::ElementNode &elmMachine,
                                        ...
                                        std::list<xml::ElementNode*> *pllElementsWithUuidAttributes)
{
    if (fl & BuildMachineXML_WriteVPoxVersionAttribute)
    {
...
void MachineConfigFile::bumpSettingsVersionIfNeeded()
{
    if (m->sv < SettingsVersion_v1_18)
    {
        ...
        // VirtualPox 6.1 adds a virtio-scsi storage controller.
    ...
    if (m->sv < SettingsVersion_v1_17)
    {
        ...
        // VirtualPox 6.0 adds nested hardware virtualization, using native API (NEM).
    ...
    if (m->sv < SettingsVersion_v1_16)
    {
        // VirtualPox 5.1 adds a NVMe storage controller, paravirt debug
    ...
    if (m->sv < SettingsVersion_v1_15)
    {
        // VirtualPox 5.0 adds paravirt providers, explicit AHCI port hotplug
    ...
    if (m->sv < SettingsVersion_v1_14)
    {
        // VirtualPox 4.3 adds default frontend setting, graphics controller
        // setting, explicit long mode setting, (video) capturing and NAT networking.
        if (   !hardwareMachine.strDefaultFrontend.isEmpty()
            || hardwareMachine.graphicsAdapter.graphicsControllerType != GraphicsControllerType_VPoxVGA
    ...
    if (m->sv < SettingsVersion_v1_13)
    {
        // VirtualPox 4.2 adds tracing, autostart, UUID in directory and groups.
    ...
    if (m->sv < SettingsVersion_v1_13)
    {
        // VirtualPox 4.2 changes the units for bandwidth group limits.
    ...
    if (m->sv < SettingsVersion_v1_12)
    {
        // VirtualPox 4.1 adds PCI passthrough and emulated USB Smart Card reader
    ...
    if (m->sv < SettingsVersion_v1_12)
    {
        // VirtualPox 4.1 adds a promiscuous mode policy to the network
    ...
    if (m->sv < SettingsVersion_v1_11)
    {
        // VirtualPox 4.0 adds HD audio, CPU priorities, ~fault tolerance~,
    ...
    if (m->sv < SettingsVersion_v1_10)
    {
         ...
         * then increase the version to at least VPox 3.2, which can have video channel properties.
         */
    ...
    if (m->sv < SettingsVersion_v1_11)
    {
         ...
         * "VideoChannel/Enabled" and "VideoChannel/Quality" then increase the version to VPox 4.0.
         */
        unsigned cOldProperties = 0;
    ...
    if (m->sv < SettingsVersion_v1_11)
    {
             ...
             ++it)
        {
                 ...
                 ++it2)
            {
                ...
                // Bandwidth limitations are new in VirtualPox 4.0 (1.11)
                if (m->sv < SettingsVersion_v1_11)
                {
        ...
        // VirtualPox before 3.1 had zero or one floppy and exactly one DVD,
    ...
    // VirtualPox 3.2: Check for non default I/O settings
    if (m->sv < SettingsVersion_v1_10)
    {
    ...
    // VirtualPox 3.2 adds NAT and boot priority to the NIC config in Main
    // VirtualPox 4.0 adds network bandwitdth
    if (m->sv < SettingsVersion_v1_11)
    {
        NetworkAdaptersList::const_iterator netit;
        for (netit = hardwareMachine.llNetworkAdapters.begin();
    ...
    try
    {
        ...
        buildMachineXML(*pelmMachine,
                        ...
                        | MachineConfigFile::BuildMachineXML_MediaRegistry,
                            // but not BuildMachineXML_WriteVPoxVersionAttribute
