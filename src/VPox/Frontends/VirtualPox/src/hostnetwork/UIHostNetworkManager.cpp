...
/** @file
 * VPox Qt GUI - UIHostNetworkManager class implementation.
 */
...
void UIHostNetworkManagerWidget::sltApplyHostNetworkDetailsChanges()
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                /* Get VPox for further activities: */
                CVirtualPox comVPox = uiCommon().virtualPox();

                /* Find corresponding DHCP server (create if necessary): */
                CDHCPServer comServer = comVPox.FindDHCPServerByNetworkName(strNetworkName);
                if (!comVPox.isOk() || comServer.isNull())
                    comServer = comVPox.CreateDHCPServer(strNetworkName);

                /* Show error message if necessary: */
                if (!comVPox.isOk() || comServer.isNull())
                    msgCenter().cannotCreateDHCPServer(comVPox, strNetworkName, this);
                else
                {
...
void UIHostNetworkManagerWidget::sltRemoveHostNetwork()
{
    ...
    else
    {
        ...
        else
        {
            ...
            const CDHCPServer &comServer = comVPox.FindDHCPServerByNetworkName(strNetworkName);
            if (comVPox.isOk() && comServer.isNotNull())
            {
                /* Remove server if any: */
                comVPox.RemoveDHCPServer(comServer);

                /* Show error message if necessary: */
                if (!comVPox.isOk())
                    msgCenter().cannotRemoveDHCPServer(comVPox, strInterfaceName, this);
            }
