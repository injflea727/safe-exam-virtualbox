...
/** @file
 * VPox Qt GUI - UIGlobalSettingsExtension class implementation.
 */
...
void UIGlobalSettingsExtension::loadToCacheFrom(QVariant &data)
{
    ...
    const CExtPackVector &packages = uiCommon().virtualPox().
...
void UIGlobalSettingsExtension::sltAddPackage()
{
    ...
    for (int i = 0; i < VPoxExtPackFileExts.size(); ++i)
        extensions << QString("*.%1").arg(VPoxExtPackFileExts[i]);
