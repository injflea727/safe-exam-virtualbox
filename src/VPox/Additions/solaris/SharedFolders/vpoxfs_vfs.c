/* $Id: vpoxfs_vfs.c $ */
/** @file
 * VirtualPox File System for Solaris Guests, VFS implementation.
 */
...
#define DEVICE_DESC                     "VirtualPox ShrdFS"
...
bool                     g_fVPoxVFS_SolOldAddrMap;
/** The map_addr()/choose_addr() hooks callout table structure. */
VPoxVFS_SolAddrMap       g_VPoxVFS_SolAddrMap;
...
static struct modlfs modlfs = {
	&mod_fsops,
	DEVICE_DESC " " VPOX_VERSION_STRING "r" VPOXSOLQUOTE(VPOX_SVN_REV),
	&sffs_vfsdef
};
...
_init()
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
#if defined(VPOX_VFS_SOLARIS_10U6)
            g_VPoxVFS_SolAddrMap.MapAddr.pfnSol_map_addr    = (void *)map_addr;
#else
            g_VPoxVFS_SolAddrMap.ChooseAddr.pfnSol_choose_addr = (void *)choose_addr;
#endif
        }
        else
        {
            g_fVPoxVFS_SolOldAddrMap = true;
#if defined(VPOX_VFS_SOLARIS_10U6)
            g_VPoxVFS_SolAddrMap.MapAddr.pfnSol_map_addr_old    = (void *)map_addr;
#else
            g_VPoxVFS_SolAddrMap.ChooseAddr.pfnSol_choose_addr_old = (void *)choose_addr;
#endif
        }
...
sffs_mount(vfs_t *vfsp, vnode_t *mvp, struct mounta *uap, cred_t *cr)
{
	    ...
	    (int)val == val)
	{
		...
		vfs_setmntopt(vfsp, "stat_ttl", VPOXSOLQUOTE(DEF_STAT_TTL_MS), 0);
