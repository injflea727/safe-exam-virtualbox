...
#ifndef VPOX_INCLUDED_com_ErrorInfo_h
#define VPOX_INCLUDED_com_ErrorInfo_h
...
COM_STRUCT_OR_CLASS(IVirtualPoxErrorInfo);
 ...
 *  member variables. This class can also handle non-VirtualPox errors (like
 *  standard COM errors).
 *
 * ProgressErrorInfo:
 *
 ...
 *
 *  The VirtualPox interface class for accessing error information from Main
 ...
 *
 * Errors itself are set in VirtualPoxBase::setErrorInternal. First a
 * IVirtualPoxErrorInfo object is created and the given error is saved within.
 * If MultiResult is active the current per thread error is fetched and
 * attached to the new created IVirtualPoxErrorInfo object. Next this object is
 ...
 *  or the extended IVirtualPoxErrorInfo interface when when it is available
 *  (i.e. a given IErrorInfo or nsIException instance implements it).
 *  Currently, IVirtualPoxErrorInfo is only available for VirtualPox components.
 *
 ...
 *  IVirtualPoxErrorInfo. If #isFullAvailable() returns true, it means that all
 ...
 *  and IErrorInfo/nsIException/IVirtualPoxErrorInfo attributes/methods:
 *
 *  ErrorInfo       IErrorInfo      nsIException    IVirtualPoxErrorInfo
 *  --------------------------------------------------------------------
...
public:
    ...
    /** Specialization for the IVirtualPoxErrorInfo smart pointer */
    ErrorInfo(const ComPtr<IVirtualPoxErrorInfo> &aPtr)
        : mIsBasicAvailable(false), mIsFullAvailable(false)
        , mResultCode(S_OK), mResultDetail(0)
        { init(aPtr); }

    /**
     *  Constructs a new ErrorInfo instance from the IVirtualPoxErrorInfo
     *  interface pointer. If this pointer is not NULL, both #isFullAvailable()
     *  and #isBasicAvailable() will return |true|.
     *
     *  @param aInfo    pointer to the IVirtualPoxErrorInfo interface that
     *                  holds error info to be fetched by this instance
     */
    ErrorInfo(IVirtualPoxErrorInfo *aInfo)
        : mIsBasicAvailable(false), mIsFullAvailable(false)
        , mResultCode(S_OK), mResultDetail(0)
        { init(aInfo); }
    ...
    HRESULT getVirtualPoxErrorInfo(ComPtr<IVirtualPoxErrorInfo> &pVirtualPoxErrorInfo);
...
protected:
    ...
    void init(IVirtualPoxErrorInfo *aInfo);
 ...
 *  IVirtualPoxErrorInfo instance to construct itself.
 */
...
public:
     ...
     *  and the errorInfo attribute returns a valid IVirtualPoxErrorInfo pointer,
...
#endif /* !VPOX_INCLUDED_com_ErrorInfo_h */
