...
#include <VPoxVideo.h> /* required by DevVGA.h */
...
*********************************************************************************************************************************/
#ifndef VPOX_VMSVGA3D_DEFAULT_OGL_PROFILE
# define VPOX_VMSVGA3D_DEFAULT_OGL_PROFILE 1.0
#endif
 ...
 */
#ifdef VPOX_VMSVGA3D_DUAL_OPENGL_PROFILE
...
/* Generated by VPoxDef2LazyLoad from the VPoxSVGA3D.def and VPoxSVGA3DObjC.def files. */
extern "C" int ExplicitlyLoadVPoxSVGA3D(bool fResolveAllImports, PRTERRINFO pErrInfo);
...
static bool vmsvga3dCheckGLExtension(PVMSVGA3DSTATE pState, float rsMinGLVersion, const char *pszWantedExtension)
{
    ...
    AssertMsg(   rsMinGLVersion == 0.0
              ...
              || VPOX_VMSVGA3D_DEFAULT_OGL_PROFILE == 2.1,
...
/**
 * @interface_method_impl{VPOXVMSVGASHADERIF,pfnSwitchInitProfile}
 */
static DECLCALLBACK(void) vmsvga3dShaderIfSwitchInitProfile(PVPOXVMSVGASHADERIF pThis, bool fOtherProfile)
{
#ifdef VPOX_VMSVGA3D_DUAL_OPENGL_PROFILE
...
/**
 * @interface_method_impl{VPOXVMSVGASHADERIF,pfnGetNextExtension}
 */
static DECLCALLBACK(bool) vmsvga3dShaderIfGetNextExtension(PVPOXVMSVGASHADERIF pThis, void **ppvEnumCtx,
...
int vmsvga3dInit(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC)
{
    ...
    rc = ExplicitlyLoadVPoxSVGA3D(true /*fResolveAllImports*/, RTErrInfoInitStatic(&ErrInfo));
    if (RT_FAILURE(rc))
    {
        LogRel(("VMSVGA3d: Error loading VPoxSVGA3D and resolving necessary functions: %Rrc - %s\n", rc, ErrInfo.Core.pszMsg));
        return rc;
    }
#ifdef RT_OS_DARWIN
    rc = ExplicitlyLoadVPoxSVGA3DObjC(true /*fResolveAllImports*/, RTErrInfoInitStatic(&ErrInfo));
    if (RT_FAILURE(rc))
    {
        LogRel(("VMSVGA3d: Error loading VPoxSVGA3DObjC and resolving necessary functions: %Rrc - %s\n", rc, ErrInfo.Core.pszMsg));
        return rc;
    }
#endif
...
int vmsvga3dPowerOn(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC)
{
    ...
    rc = vmsvga3dGatherExtensions(&pState->pszExtensions, VPOX_VMSVGA3D_DEFAULT_OGL_PROFILE);
    AssertRCReturn(rc, rc);
    ...
    rc = vmsvga3dGatherExtensions(&pState->pszOtherExtensions, VPOX_VMSVGA3D_OTHER_OGL_PROFILE);
    AssertRCReturn(rc, rc);
...
#ifdef VPOX_VMSVGA3D_DUAL_OPENGL_PROFILE /* The alternative profile has a higher number here (ati/darwin). */
    VMSVGA3D_SET_CURRENT_CONTEXT(pState, pOtherCtx);
   ...
   SVGA3D_DEVCAP_SURFACEFMT_BC5_UNORM              = 83,
    ...
    /* Cleanup */
    rc = vmsvga3dContextDestroy(pThisCC, 1);
    AssertRC(rc);
#ifdef VPOX_VMSVGA3D_DUAL_OPENGL_PROFILE
    rc = vmsvga3dContextDestroy(pThisCC, 2);
    AssertRC(rc);
#endif
                        ...
                        uint32_t cCopyBoxes, SVGA3dCopyBox *pBox)
{
    ...
    if (!VMSVGA3DSURFACE_HAS_HW_SURFACE(pSurfaceSrc))
    {
        ...
        if (!VMSVGA3DSURFACE_HAS_HW_SURFACE(pSurfaceDst))
        {
            ...
            rc = vmsvga3dBackCreateTexture(pState, pContext, pContext->id, pSurfaceDst);
            AssertRCReturn(rc, rc);
        }
                                  ...
                                  SVGA3dCopyBox const *pBox, PVMSVGA3DCONTEXT pContext, int rc, int iBox)
{
    ...
    case VMSVGA3D_OGLRESTYPE_BUFFER:
    {
        ...
        if (VMSVGA3D_GL_IS_SUCCESS(pContext))
        {
            ...
            {
#if defined(VPOX_STRICT) && defined(RT_OS_DARWIN)
...
#if !defined(VPOX_VMSVGA3D_DUAL_OPENGL_PROFILE) || !(defined(RT_OS_DARWIN))
...
#if 0 /* Unused */
static int vmsvga3dDrvNotifyHwScreen(PVGASTATECC pThisCC, VPOX3D_NOTIFY_TYPE enmNotification,
                                     uint32_t idScreen, Pixmap pixmap, void *pvData, size_t cbData)
{
    uint8_t au8Buffer[128];
    AssertLogRelMsgReturn(cbData <= sizeof(au8Buffer) - sizeof(VPOX3DNOTIFY),
                          ("cbData %zu", cbData),
                          VERR_INVALID_PARAMETER);

    VPOX3DNOTIFY *p = (VPOX3DNOTIFY *)&au8Buffer[0];
...
static void vmsvga3dDrvNotifyHwOverlay(PVGASTATECC pThisCC, VPOX3D_NOTIFY_TYPE enmNotification, uint32_t idScreen)
{
    uint8_t au8Buffer[128];
    VPOX3DNOTIFY *p = (VPOX3DNOTIFY *)&au8Buffer[0];
    p->enmNotification = enmNotification;
...
static int vmsvga3dDrvQueryWindow(PVGASTATECC pThisCC, uint32_t idScreen, Window *pWindow)
{
    uint8_t au8Buffer[128];
    VPOX3DNOTIFY *p = (VPOX3DNOTIFY *)&au8Buffer[0];
    p->enmNotification = VPOX3D_NOTIFY_TYPE_HW_OVERLAY_GET_ID;
    p->iDisplay = idScreen;
    p->u32Reserved = 0;
    p->cbData = sizeof(uint64_t);
    *(uint64_t *)&p->au8Data[0] = 0;
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (fSuccess)
            {
                ...
                vmsvga3dDrvNotifyHwOverlay(pThisCC, VPOX3D_NOTIFY_TYPE_HW_OVERLAY_CREATED, pScreen->idScreen);
            }
            else
            {
...
int vmsvga3dBackDestroyScreen(PVGASTATECC pThisCC, VMSVGASCREENOBJECT *pScreen)
{
    ...
    if (p)
    {
        ...
        vmsvga3dDrvNotifyHwOverlay(pThisCC, VPOX3D_NOTIFY_TYPE_HW_OVERLAY_DESTROYED, pScreen->idScreen);
                                    ...
                                    SVGASignedRect srcRect, uint32_t cRects, SVGASignedRect *paRects)
{
    ...
    if (!VMSVGA3DSURFACE_HAS_HW_SURFACE(pSurface))
    {
        ...
        rc = vmsvga3dBackCreateTexture(pState, &pState->SharedCtx, VMSVGA3D_SHARED_CTX_ID, pSurface);
        AssertRCReturn(rc, rc);
    }
...
int vmsvga3dSetRenderState(PVGASTATECC pThisCC, uint32_t cid, uint32_t cRenderStates, SVGA3dRenderState *pRenderState)
{
    ...
    for (unsigned i = 0; i < cRenderStates; i++)
    {
        ...
        case SVGA3D_RS_SEPARATEALPHABLENDENABLE: /* SVGA3dBool */
        {
            ...
            rc = vmsvga3dSetRenderState(pThisCC, cid, 2, renderstate);
            AssertRCReturn(rc, rc);
        ...
        case SVGA3D_RS_STENCILENABLE2SIDED:    /* SVGA3dBool */
        {
            ...
            rc = vmsvga3dSetRenderState(pThisCC, cid, RT_ELEMENTS(renderstate), renderstate);
            AssertRCReturn(rc, rc);
                    ...
                    for (uint32_t iStage = 0; iStage < RT_ELEMENTS(pContext->state.aTextureStates); iStage++)
                    {
                        for (uint32_t j = 0; j < RT_ELEMENTS(pContext->state.aTextureStates[0]); j++)
                        {
                            ...
                            if (    pTextureStateIter->name != SVGA3D_TS_INVALID
                                ...
                                vmsvga3dSetTextureState(pThisCC, pContext->id, 1, pTextureStateIter);
                        }
                    }
                }
    ...
    if (enabled)
    {
        if (index < SVGA3D_MAX_LIGHTS)
        {
           ...
           if (!pContext->state.aLightData[index].fValidData)
               vmsvga3dSetLightData(pThisCC, cid, index, (SVGA3dLightData *)&vmsvga3d_default_light);
        }
...
int vmsvga3dSetViewPort(PVGASTATECC pThisCC, uint32_t cid, SVGA3dRect *pRect)
{
    ...
    if (pContext->state.aTransformState[SVGA3D_TRANSFORM_PROJECTION].fValid == true)
        vmsvga3dSetTransform(pThisCC, cid, SVGA3D_TRANSFORM_PROJECTION,
    ...
    else
    {
        ...
        matrix[10] = 1.0;
        matrix[15] = 1.0;
        vmsvga3dSetTransform(pThisCC, cid, SVGA3D_TRANSFORM_PROJECTION, matrix);
    }
...
int vmsvga3dCommandClear(PVGASTATECC pThisCC, uint32_t cid, SVGA3dClearFlag clearFlag, uint32_t color, float depth, uint32_t stencil,
...
static int vmsvga3dResetTransformMatrices(PVGASTATECC pThisCC, PVMSVGA3DCONTEXT pContext)
{
    ...
    if (pContext->state.aTransformState[SVGA3D_TRANSFORM_VIEW].fValid == true)
        rc = vmsvga3dSetTransform(pThisCC, pContext->id, SVGA3D_TRANSFORM_VIEW,
    ...
    else
    {
        ...
        matrix[10] = 1.0;
        matrix[15] = 1.0;
        rc = vmsvga3dSetTransform(pThisCC, pContext->id, SVGA3D_TRANSFORM_VIEW, matrix);
    }

    /* Reset the projection matrix. */
    if (pContext->state.aTransformState[SVGA3D_TRANSFORM_PROJECTION].fValid == true)
    {
        rc = vmsvga3dSetTransform(pThisCC, pContext->id, SVGA3D_TRANSFORM_PROJECTION, pContext->state.aTransformState[SVGA3D_TRANSFORM_PROJECTION].matrix);
    }
    else
    {
        float matrix[16];
        ...
        matrix[10] = 1.0;
        matrix[15] = 1.0;
        rc = vmsvga3dSetTransform(pThisCC, pContext->id, SVGA3D_TRANSFORM_PROJECTION, matrix);
    }
    AssertRC(rc);
    return rc;
}
