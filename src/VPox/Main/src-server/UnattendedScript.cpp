...
{
    static const char s_szPrefix[]         = "@@VPOX_";
    static const char s_szPrefixInsert[]   = "@@VPOX_INSERT_";
    static const char s_szPrefixCond[]     = "@@VPOX_COND_";
    static const char s_szPrefixCondEnd[]  = "@@VPOX_COND_END@@";
    ...
    for (;;)
    {
        ...
        if (offPlaceholder != RTCString::npos)
        {
            ...
            /*
             * @@VPOX_COND_END@@: Pop one item of the conditional stack.
             */
                     ...
                     && strncmp(pszPlaceholder, s_szPrefixCondEnd, sizeof(s_szPrefixCondEnd) - 1U) == 0)
            {
                ...
                else
                {
                    hrc = mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR,
                                                   mpSetError->tr("%s without @@VPOX_COND_XXX@@ at offset %zu (%#zx)"),
                                                   s_szPrefixCondEnd, offPlaceholder, offPlaceholder);
                    break;
                }
            }
            /*
             * @@VPOX_COND_XXX@@: Push the previous outputting state and combine it with the
        ...
        if (offTemplate >= cchTemplate)
        {
            if (cConds == 0)
                return S_OK;
            if (cConds == 1)
                hrc = mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Missing @@VPOX_COND_END@@"));
            else
                hrc = mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Missing %u @@VPOX_COND_END@@"), cConds);
            break;
        }
    }
...
#define IS_PLACEHOLDER_MATCH(a_szMatch) \
        (   cchPlaceholder == sizeof("@@VPOX_INSERT_" a_szMatch) - 1U \
         && memcmp(pachPlaceholder, "@@VPOX_INSERT_" a_szMatch, sizeof("@@VPOX_INSERT_" a_szMatch) - 1U) == 0)
...
{
#define IS_PLACEHOLDER_MATCH(a_szMatch) \
        (   cchPlaceholder == sizeof("@@VPOX_COND_" a_szMatch "@@") - 1U \
         && memcmp(pachPlaceholder, "@@VPOX_COND_" a_szMatch "@@", sizeof("@@VPOX_COND_" a_szMatch "@@") - 1U) == 0)
#define IS_PLACEHOLDER_PARTIALLY_MATCH(a_szMatch) \
    ...
    {
        /* Check the passed string against format @@VPOX_COND_GUEST_VERSION[>8.04.0]@@. Allowed comparison operators are:
        ...
        {
            *pfOutputting = false;
            return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VPOX_COND_GUEST_VERSION[expr]@@: Missing ']' (%.*s)"),
                                            cchPlaceholder, pachPlaceholder);
        }
        size_t startPos = sizeof("@@VPOX_COND_GUEST_VERSION[") - 1;
        size_t  endPos = cchPlaceholder - endLength;
        if (startPos >= endPos)
        {
            *pfOutputting = false;
            return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VPOX_COND_GUEST_VERSION[expr]@@: Missing expr (%.*s)"),
                                            cchPlaceholder, pachPlaceholder);
        }
        ...
        {
            *pfOutputting = false;
            return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VPOX_COND_GUEST_VERSION[expr]@@: Only space, '>', '>=', '<', <=', and '=' are allowed at the start. (%.*s)"),
                                            cchPlaceholder, pachPlaceholder);
        }
        if (startPos >= endPos)
        {
            *pfOutputting = false;
            return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VPOX_COND_GUEST_VERSION[expr]@@: No version string found. (%.*s)"),
                                            cchPlaceholder, pachPlaceholder);
        }
        ...
        for (size_t i = startPos; i < endPos; ++i)
        {
            ...
            {
                *pfOutputting = false;
                return mpSetError->setErrorBoth(E_FAIL, VERR_PARSE_ERROR, mpSetError->tr("Malformed @@VPOX_COND_GUEST_VERSION[expr]@@: Version string must be consist of only digits and '.', and no spaces. (%.*s)"),
                                                cchPlaceholder, pachPlaceholder);
            }
        }
