/* $Id: VPoxServiceVMInfo-win.cpp $ */
/** @file
 * VPoxService - Virtual Machine Information for the Host, Windows specifics.
 */
...
#include "VPoxServiceVMInfo.h"
#include "../../WINNT/VPoxTray/VPoxTrayMsg.h" /* For IPC. */
...
typedef struct VPOXSERVICEVMINFOUSER
{
     ...
     *  ones. */
    ULONG ulLastSession;
} VPOXSERVICEVMINFOUSER, *PVPOXSERVICEVMINFOUSER;

/** Structure for the file information lookup. */
typedef struct VPOXSERVICEVMINFOFILE
{
    char *pszFilePath;
    char *pszFileName;
} VPOXSERVICEVMINFOFILE, *PVPOXSERVICEVMINFOFILE;

/** Structure for process information lookup. */
typedef struct VPOXSERVICEVMINFOPROC
{
    /** The PID. */
    DWORD id;
    /** The SID. */
    ...
    /** Interactive process. */
    bool fInteractive;
} VPOXSERVICEVMINFOPROC, *PVPOXSERVICEVMINFOPROC;


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static uint32_t vgsvcVMInfoWinSessionHasProcesses(PLUID pSession, PVPOXSERVICEVMINFOPROC const paProcs, DWORD cProcs);
static bool vgsvcVMInfoWinIsLoggedIn(PVPOXSERVICEVMINFOUSER a_pUserInfo, PLUID a_pSession);
static int  vgsvcVMInfoWinProcessesEnumerate(PVPOXSERVICEVMINFOPROC *ppProc, DWORD *pdwCount);
static void vgsvcVMInfoWinProcessesFree(DWORD cProcs, PVPOXSERVICEVMINFOPROC paProcs);
static int  vgsvcVMInfoWinWriteLastInput(PVPOXSERVICEVEPROPCACHE pCache, const char *pszUser, const char *pszDomain);
 ...
 */
static int vgsvcVMInfoWinProcessesGetModuleNameW(PVPOXSERVICEVMINFOPROC const pProc, PRTUTF16 *ppszName)
{
    *ppszName = NULL;
 ...
 */
static int vgsvcVMInfoWinProcessesGetTokenInfo(PVPOXSERVICEVMINFOPROC pProc, TOKEN_INFORMATION_CLASS tkClass)
{
    AssertPtrReturn(pProc, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcVMInfoWinProcessesEnumerate(PVPOXSERVICEVMINFOPROC *ppaProcs, PDWORD pcProcs)
{
    AssertPtr(ppaProcs);
    AssertPtr(pcProcs);
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        PVPOXSERVICEVMINFOPROC paProcs;
        paProcs = (PVPOXSERVICEVMINFOPROC)RTMemAllocZ(cProcesses * sizeof(VPOXSERVICEVMINFOPROC));
        if (paProcs)
        {
 ...
 */
static bool vgsvcVMInfoWinIsLoggedIn(PVPOXSERVICEVMINFOUSER pUserInfo, PLUID pSession)
{
...
static int vgsvcVMInfoWinWriteLastInput(PVPOXSERVICEVEPROPCACHE pCache, const char *pszUser, const char *pszDomain)
{
    ...
    char szPipeName[512 + sizeof(VPOXTRAY_IPC_PIPE_PREFIX)];
    memcpy(szPipeName, VPOXTRAY_IPC_PIPE_PREFIX, sizeof(VPOXTRAY_IPC_PIPE_PREFIX));
    int rc = RTStrCat(szPipeName, sizeof(szPipeName), pszUser);
    if (RT_SUCCESS(rc))
    {
        bool fReportToHost = false;
        VPoxGuestUserState userState = VPoxGuestUserState_Unknown;

        RTLOCALIPCSESSION hSession;
        rc = RTLocalIpcSessionConnect(&hSession, szPipeName, RTLOCALIPC_FLAGS_NATIVE_NAME);
        if (RT_SUCCESS(rc))
        {
            VPOXTRAYIPCHEADER ipcHdr =
            {
                /* .uMagic      = */ VPOXTRAY_IPC_HDR_MAGIC,
                /* .uHdrVersion = */ 0,
                /* .uMsgType    = */ VPOXTRAYIPCMSGTYPE_USERLASTINPUT,
                /* .cbMsgData   = */ 0 /* No msg */
            };
            ...
            if (RT_SUCCESS(rc))
            {
                VPOXTRAYIPCRES_USERLASTINPUT ipcRes;
                rc = RTLocalIpcSessionRead(hSession, &ipcRes, sizeof(ipcRes), NULL /* Exact read */);
                if (   RT_SUCCESS(rc)
                    /* If uLastInput is set to UINT32_MAX VPoxTray was not able to retrieve the
                    ...
                    && ipcRes.uLastInput != UINT32_MAX)
                {
                    userState = (ipcRes.uLastInput * 1000) < g_uVMInfoUserIdleThresholdMS
                              ? VPoxGuestUserState_InUse
                              : VPoxGuestUserState_Idle;

                    rc = VGSvcUserUpdateF(pCache, pszUser, pszDomain, "UsageState",
                                          userState == VPoxGuestUserState_InUse ? "InUse" : "Idle");

                    /*
                     * Note: vpoxServiceUserUpdateF can return VINF_NO_CHANGE in case there wasn't anything
                    ...
                    VGSvcVerbose(4, "User '%s' (domain '%s') is idle for %RU32, fReportToHost=%RTbool\n",
                        ...
                        if (userState == VPoxGuestUserState_Idle)
        ...
        else
        {
            switch (rc)
            {
                ...
                {
                    /* No VPoxTray (or too old version which does not support IPC) running
                       for the given user. Not much we can do then. */
                    VGSvcVerbose(4, "VPoxTray for user '%s' not running (anymore), no last input available\n", pszUser);
                    ...
                    fReportToHost = rc == VINF_SUCCESS;
                    if (fReportToHost)
                        userState = VPoxGuestUserState_Idle;
                    break;
                }
        ...
        if (fReportToHost)
        {
            Assert(userState != VPoxGuestUserState_Unknown);
 ...
 */
int VGSvcVMInfoWinWriteUsers(PVPOXSERVICEVEPROPCACHE pCache, char **ppszUserList, uint32_t *pcUsersInList)
{
    AssertPtrReturn(pCache, VERR_INVALID_POINTER);
    ...
    {
        PVPOXSERVICEVMINFOUSER pUserInfo;
        pUserInfo = (PVPOXSERVICEVMINFOUSER)RTMemAllocZ(cSessions * sizeof(VPOXSERVICEVMINFOUSER) + 1);
        if (!pUserInfo)
        ...
        else
        {
            ...
            for (ULONG i = 0; i < cSessions; i++)
            {
                ...
                VPOXSERVICEVMINFOUSER userSession;
                if (vgsvcVMInfoWinIsLoggedIn(&userSession, &paSessions[i]))
                {
                    ...
                    if (g_cVerbosity > 3)
                    {
                        ...
                        RTStrPrintf(szDebugSessionPath,  sizeof(szDebugSessionPath),
                                    "/VirtualPox/GuestInfo/Debug/LSA/Session/%RU32", userSession.ulLastSession);
                    ...
                    {
                        PVPOXSERVICEVMINFOUSER pCurUser = &pUserInfo[a];
                        AssertPtr(pCurUser);
                    ...
                    if (!fFoundUser)
                    {
                        ...
                        memcpy(&pUserInfo[cUniqueUsers], &userSession, sizeof(VPOXSERVICEVMINFOUSER));
            ...
            if (g_cVerbosity > 3)
                VGSvcWritePropF(s_uDebugGuestPropClientID, "/VirtualPox/GuestInfo/Debug/LSA",
            ...
            for (ULONG i = 0; i < cUniqueUsers; i++)
            {
                if (g_cVerbosity > 3)
                {
                    char szDebugUserPath[255]; RTStrPrintf(szDebugUserPath,  sizeof(szDebugUserPath), "/VirtualPox/GuestInfo/Debug/LSA/User/%RU32", i);
...
int VGSvcVMInfoWinGetComponentVersions(uint32_t uClientID)
{
    ...
    const VPOXSERVICEVMINFOFILE aVPoxFiles[] =
    {
        { szSysDir,     "VPoxControl.exe" },
        { szSysDir,     "VPoxHook.dll" },
        { szSysDir,     "VPoxDisp.dll" },
        { szSysDir,     "VPoxTray.exe" },
        { szSysDir,     "VPoxService.exe" },
        { szSysDir,     "VPoxMRXNP.dll" },
        { szSysDir,     "VPoxGINA.dll" },
        { szSysDir,     "VPoxCredProv.dll" },

 /* On 64-bit we don't yet have the OpenGL DLLs in native format.
    So just enumerate the 32-bit files in the SYSWOW directory. */
#ifdef RT_ARCH_AMD64
        { szSysWowDir,  "VPoxOGL-x86.dll" },
#else  /* !RT_ARCH_AMD64 */
        { szSysDir,     "VPoxOGL.dll" },
#endif /* !RT_ARCH_AMD64 */

        { szDriversDir, "VPoxGuest.sys" },
        { szDriversDir, "VPoxMouseNT.sys" },
        { szDriversDir, "VPoxMouse.sys" },
        { szDriversDir, "VPoxSF.sys"    },
        { szDriversDir, "VPoxVideo.sys" },
    };

    for (unsigned i = 0; i < RT_ELEMENTS(aVPoxFiles); i++)
    {
        char szVer[128];
        rc = VGSvcUtilWinGetFileVersionString(aVPoxFiles[i].pszFilePath, aVPoxFiles[i].pszFileName, szVer, sizeof(szVer));
        char szPropPath[256];
        RTStrPrintf(szPropPath, sizeof(szPropPath), "/VirtualPox/GuestAdd/Components/%s", aVPoxFiles[i].pszFileName);
