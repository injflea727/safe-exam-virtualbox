  ...
  )
{
  ...
  return gBS->InstallMultipleProtocolInterfaces (
                &Controller,
                &gEfiIdeControllerInitProtocolGuid, &gEfiIdeControllerInit.Core, /* VPox: We added .Core, probably for warnings. */
                NULL
                );
}
  ...
  )
{
  ...
  //
  if ((void *)IdeControllerInit != (void *)&gEfiIdeControllerInit) { /* VPox: Dunno why we're doing the (void *) thing, types should be the same... */
    return EFI_UNSUPPORTED;
  }
...
#ifdef VPOX
  VPoxLogFlowFuncEnter();
  VPoxLogFlowFuncMarkVar(Channel, "%d");
  *MaxDevices = 0;
  *Enabled = FALSE;
#endif
  if (Channel < ICH_IDE_MAX_CHANNEL) {
    ...
    for (;u8Device < ICH_IDE_MAX_CHANNEL; ++u8Device)
    {
        ...
        UINT64  u8DH = 0;
        ASSERT(pPciIo);
        VPoxLogFlowFuncMarkVar(u8Device, "%d");
        if (u8Device == 0)
            u8DH = 0xa0;
        else if (u8Device == 1)
            u8DH = 0xb0;
        ...
        VPoxLogFlowFuncMarkVar(u8DH, "%x");
        VPoxLogFlowFuncMarkVar(u64IoBase, "%llx");
        ...
        VPoxLogFlowFuncMarkVar(u8CS, "%x");
        VPoxLogFlowFuncMarkVar(u8SN, "%x");
...
#endif /* !VPOX */
    VPoxLogFlowFuncMarkVar(*MaxDevices, "%d");
    VPoxLogFlowFuncMarkVar(*Enabled, "%d");
    VPoxLogFlowFuncLeaveRC(EFI_SUCCESS);
    return EFI_SUCCESS;
  }
