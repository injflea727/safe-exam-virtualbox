...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
PVPOXHGCMSVCHELPERS g_pHelpers;
...
static DECLCALLBACK(int) svcUnload (void *)
{
    ...
    if (g_pHelpers)
        HGCMSvcHlpStamDeregister(g_pHelpers, "/HGCM/VPoxSharedFolders/*");
    return rc;
}
...
static DECLCALLBACK(int) svcDisconnect (void *, uint32_t u32ClientID, void *pvClient)
{
     ...
     *
     * Usually this will be done actively by VPoxService on the guest side when shutting down,
     * but the VM could be reset without having VPoxService the chance of cancelling those waits.
     *
...
static DECLCALLBACK(void) svcCall (void *, VPOXHGCMCALLHANDLE callHandle, uint32_t u32ClientID, void *pvClient,
                                   uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[], uint64_t tsArrival)
{
    RT_NOREF(u32ClientID, tsArrival);
#ifndef VPOX_WITHOUT_RELEASE_STATISTICS
    uint64_t tsStart;
    STAM_GET_TS(tsStart);
    ...
    switch (u32Function)
    {
        case SHFL_FN_QUERY_MAPPINGS:
        {
            ...
            else if (   paParms[0].type != VPOX_HGCM_SVC_PARM_32BIT   /* flags */
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_32BIT   /* numberOfMappings */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_PTR     /* mappings */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_QUERY_MAP_NAME:
        {
            ...
            else if (   paParms[0].type != VPOX_HGCM_SVC_PARM_32BIT /* Root. */
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR   /* Name. */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_CREATE:
        {
            ...
            if (cParms != SHFL_CPARMS_CREATE)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR   /* path */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_PTR   /* parms */
                    )
            {
        ...
        case SHFL_FN_CLOSE:
        {
            ...
            if (cParms != SHFL_CPARMS_CLOSE)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_64BIT   /* handle */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_READ:
        {
            ...
            ASSERT_GUEST_STMT_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* root */
            ASSERT_GUEST_STMT_BREAK(paParms[1].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* handle */
            ASSERT_GUEST_STMT_BREAK(paParms[2].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* offset */
            ASSERT_GUEST_STMT_BREAK(paParms[3].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* count */
            ASSERT_GUEST_STMT_BREAK(   paParms[4].type == VPOX_HGCM_SVC_PARM_PTR
                                    || paParms[4].type == VPOX_HGCM_SVC_PARM_PAGES, rc = VERR_WRONG_PARAMETER_TYPE); /* buffer */
            ...
            if (paParms[4].type == VPOX_HGCM_SVC_PARM_PTR)
        ...
        case SHFL_FN_LOCK:
            ...
            if (   paParms[0].type != VPOX_HGCM_SVC_PARM_32BIT   /* root */
                || paParms[1].type != VPOX_HGCM_SVC_PARM_64BIT   /* handle */
                || paParms[2].type != VPOX_HGCM_SVC_PARM_64BIT   /* offset */
                || paParms[3].type != VPOX_HGCM_SVC_PARM_64BIT   /* length */
                || paParms[4].type != VPOX_HGCM_SVC_PARM_32BIT   /* flags */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_LIST:
        {
            ...
            if (   paParms[0].type != VPOX_HGCM_SVC_PARM_32BIT   /* root */
                || paParms[1].type != VPOX_HGCM_SVC_PARM_64BIT   /* handle */
                || paParms[2].type != VPOX_HGCM_SVC_PARM_32BIT   /* flags */
                || paParms[3].type != VPOX_HGCM_SVC_PARM_32BIT   /* cb */
                || paParms[4].type != VPOX_HGCM_SVC_PARM_PTR     /* pPath */
                || paParms[5].type != VPOX_HGCM_SVC_PARM_PTR     /* buffer */
                || paParms[6].type != VPOX_HGCM_SVC_PARM_32BIT   /* resumePoint */
                || paParms[7].type != VPOX_HGCM_SVC_PARM_32BIT   /* cFiles (out) */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_READLINK:
        {
            ...
            if (   paParms[0].type != VPOX_HGCM_SVC_PARM_32BIT   /* root */
                || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR     /* path */
                || paParms[2].type != VPOX_HGCM_SVC_PARM_PTR     /* buffer */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_MAP_FOLDER_OLD:
        {
            ...
            else if (   paParms[0].type != VPOX_HGCM_SVC_PARM_PTR     /* path */
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_32BIT   /* root */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_32BIT   /* delimiter */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_MAP_FOLDER:
        {
            ...
            if (cParms != SHFL_CPARMS_MAP_FOLDER)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_32BIT   /* root */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_32BIT   /* delimiter */
                     || paParms[3].type != VPOX_HGCM_SVC_PARM_32BIT   /* fCaseSensitive */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_INFORMATION:
        {
            ...
            if (   paParms[0].type != VPOX_HGCM_SVC_PARM_32BIT   /* root */
                ...
                || paParms[2].type != VPOX_HGCM_SVC_PARM_32BIT   /* flags */
                || paParms[3].type != VPOX_HGCM_SVC_PARM_32BIT   /* cb */
                || paParms[4].type != VPOX_HGCM_SVC_PARM_PTR     /* buffer */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_REMOVE:
        {
            ...
            ASSERT_GUEST_STMT_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* root */
            ASSERT_GUEST_STMT_BREAK(paParms[1].type == VPOX_HGCM_SVC_PARM_PTR,   rc = VERR_WRONG_PARAMETER_TYPE); /* path */
            ...
            ASSERT_GUEST_STMT_BREAK(ShflStringIsValidIn(pStrPath, paParms[1].u.pointer.size,
                                    ...
                                    rc = VERR_INVALID_PARAMETER);
            ASSERT_GUEST_STMT_BREAK(paParms[2].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* flags */
        ...
        case SHFL_FN_RENAME:
        {
            ...
            if (cParms != SHFL_CPARMS_RENAME)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR   /* src */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_PTR   /* dest */
                     || paParms[3].type != VPOX_HGCM_SVC_PARM_32BIT /* flags */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_SYMLINK:
        {
            ...
            if (cParms != SHFL_CPARMS_SYMLINK)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR     /* newPath */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_PTR     /* oldPath */
                     || paParms[3].type != VPOX_HGCM_SVC_PARM_PTR     /* info */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_QUERY_MAP_INFO:
        {
            ...
            ASSERT_GUEST_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT); /* root */
            ASSERT_GUEST_BREAK(paParms[1].type == VPOX_HGCM_SVC_PARM_PTR);   /* name */
            ...
            ASSERT_GUEST_BREAK(paParms[2].type == VPOX_HGCM_SVC_PARM_PTR);   /* mountPoint */
            ...
            ASSERT_GUEST_BREAK(paParms[3].type == VPOX_HGCM_SVC_PARM_64BIT); /* flags */
            ASSERT_GUEST_BREAK(!(paParms[3].u.uint64 & ~(SHFL_MIQF_DRIVE_LETTER | SHFL_MIQF_PATH))); /* flags */
            ASSERT_GUEST_BREAK(paParms[4].type == VPOX_HGCM_SVC_PARM_32BIT); /* version */
        ...
        case SHFL_FN_WAIT_FOR_MAPPINGS_CHANGES:
        {
            ...
            ASSERT_GUEST_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT); /* uFolderMappingsVersion */
        ...
        case SHFL_FN_SET_FILE_SIZE:
        {
            ...
            ASSERT_GUEST_STMT_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* id32Root */
            ASSERT_GUEST_STMT_BREAK(paParms[1].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* u64Handle */
            ASSERT_GUEST_STMT_BREAK(paParms[2].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* cb64NewSize */
        ...
        case SHFL_FN_QUERY_FEATURES:
        {
            ...
            ASSERT_GUEST_STMT_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* f64Features */
            ASSERT_GUEST_STMT_BREAK(paParms[1].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* u32LastFunction */
        ...
        case SHFL_FN_COPY_FILE:
        {
            ...
            ASSERT_GUEST_STMT_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* i32RootSrc  */
            ASSERT_GUEST_STMT_BREAK(paParms[1].type == VPOX_HGCM_SVC_PARM_PTR, rc = VERR_WRONG_PARAMETER_TYPE);   /* pStrPathSrc */
            ...
            ASSERT_GUEST_STMT_BREAK(ShflStringIsValidIn(pStrPathSrc, paParms[1].u.pointer.size,
                                                        RT_BOOL(pClient->fu32Flags & SHFL_CF_UTF8)),
                                    rc = VERR_INVALID_PARAMETER);
            ASSERT_GUEST_STMT_BREAK(paParms[2].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* i32RootDst  */
            ASSERT_GUEST_STMT_BREAK(paParms[3].type == VPOX_HGCM_SVC_PARM_PTR, rc = VERR_WRONG_PARAMETER_TYPE);   /* pStrPathDst */
            ...
            ASSERT_GUEST_STMT_BREAK(ShflStringIsValidIn(pStrPathDst, paParms[3].u.pointer.size,
                                                        RT_BOOL(pClient->fu32Flags & SHFL_CF_UTF8)),
                                    rc = VERR_INVALID_PARAMETER);
            ASSERT_GUEST_STMT_BREAK(paParms[4].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* f32Flags */
        ...
        case SHFL_FN_COPY_FILE_PART:
        {
            ...
            ASSERT_GUEST_STMT_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* i32RootSrc  */
            ASSERT_GUEST_STMT_BREAK(paParms[1].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* u64HandleSrc */
            ASSERT_GUEST_STMT_BREAK(paParms[2].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* off64Src */
            ASSERT_GUEST_STMT_BREAK((int64_t)paParms[2].u.uint64 >= 0, rc = VERR_NEGATIVE_SEEK);
            ASSERT_GUEST_STMT_BREAK(paParms[3].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* i32RootDst  */
            ASSERT_GUEST_STMT_BREAK(paParms[4].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* u64HandleDst */
            ASSERT_GUEST_STMT_BREAK(paParms[5].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* off64Dst */
            ASSERT_GUEST_STMT_BREAK((int64_t)paParms[5].u.uint64 >= 0, rc = VERR_NEGATIVE_SEEK);
            ASSERT_GUEST_STMT_BREAK(paParms[6].type == VPOX_HGCM_SVC_PARM_64BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* cb64ToCopy */
            ASSERT_GUEST_STMT_BREAK(paParms[6].u.uint64 < _1E, rc = VERR_OUT_OF_RANGE);
            ASSERT_GUEST_STMT_BREAK(paParms[7].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* f32Flags */
        ...
        case SHFL_FN_SET_ERROR_STYLE:
        {
            ...
            ASSERT_GUEST_STMT_BREAK(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* enm32Style  */
            ...
            ASSERT_GUEST_STMT_BREAK(paParms[1].type == VPOX_HGCM_SVC_PARM_32BIT, rc = VERR_WRONG_PARAMETER_TYPE); /* u32Reserved */
 ...
 * security reasons.
 */
static DECLCALLBACK(int) svcHostCall (void *, uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    int rc = VINF_SUCCESS;
    ...
    case SHFL_FN_ADD_MAPPING:
    {
        ...
        else if (   paParms[0].type != VPOX_HGCM_SVC_PARM_PTR     /* host folder path */
                 || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR     /* map name */
                 || paParms[2].type != VPOX_HGCM_SVC_PARM_32BIT   /* fFlags */
                 || paParms[3].type != VPOX_HGCM_SVC_PARM_PTR     /* auto mount point */
                )
        {
            rc = VERR_INVALID_PARAMETER;
        }
    ...
    case SHFL_FN_REMOVE_MAPPING:
    {
        ...
        else if (   paParms[0].type != VPOX_HGCM_SVC_PARM_PTR     /* folder name */
                )
        {
            rc = VERR_INVALID_PARAMETER;
        }
...
extern "C" DECLCALLBACK(DECLEXPORT(int)) VPoxHGCMSvcLoad (VPOXHGCMSVCFNTABLE *ptable)
{
    ...
    Log(("SharedFolders host service: VPoxHGCMSvcLoad: ptable = %p\n", ptable));
    ...
    {
        Log(("SharedFolders host service: VPoxHGCMSvcLoad: ptable->cbSize = %u, ptable->u32Version = 0x%08X\n",
             ptable->cbSize, ptable->u32Version));

        if (    ptable->cbSize != sizeof (VPOXHGCMSVCFNTABLE)
            ||  ptable->u32Version != VPOX_HGCM_SVC_VERSION)
        {
            LogRelFunc(("SharedFolders host service: Version mismatch while loading: ptable->cbSize = %u (should be %u), ptable->u32Version = 0x%08X (should be 0x%08X)\n",
                        ptable->cbSize, sizeof (VPOXHGCMSVCFNTABLE), ptable->u32Version, VPOX_HGCM_SVC_VERSION));
        ...
        if (RT_SUCCESS(rc))
        {
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatQueryMappings,             STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_QUERY_MAPPINGS successes",          "/HGCM/VPoxSharedFolders/FnQueryMappings");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatQueryMappingsFail,         STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_QUERY_MAPPINGS failures",           "/HGCM/VPoxSharedFolders/FnQueryMappingsFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatQueryMapName,              STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_QUERY_MAP_NAME",                    "/HGCM/VPoxSharedFolders/FnQueryMapName");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCreate,                    STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CREATE/CREATE successes",           "/HGCM/VPoxSharedFolders/FnCreate");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCreateFail,                STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CREATE/CREATE failures",            "/HGCM/VPoxSharedFolders/FnCreateFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatLookup,                    STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CREATE/LOOKUP successes",           "/HGCM/VPoxSharedFolders/FnLookup");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatLookupFail,                STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CREATE/LOOKUP failures",            "/HGCM/VPoxSharedFolders/FnLookupFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatClose,                     STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CLOSE successes",                   "/HGCM/VPoxSharedFolders/FnClose");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCloseFail,                 STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CLOSE failures",                    "/HGCM/VPoxSharedFolders/FnCloseFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatRead,                      STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_READ successes",                    "/HGCM/VPoxSharedFolders/FnRead");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatReadFail,                  STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_READ failures",                     "/HGCM/VPoxSharedFolders/FnReadFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatWrite,                     STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_WRITE successes",                   "/HGCM/VPoxSharedFolders/FnWrite");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatWriteFail,                 STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_WRITE failures",                    "/HGCM/VPoxSharedFolders/FnWriteFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatLock,                      STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_LOCK successes",                    "/HGCM/VPoxSharedFolders/FnLock");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatLockFail,                  STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_LOCK failures",                     "/HGCM/VPoxSharedFolders/FnLockFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatList,                      STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_LIST successes",                    "/HGCM/VPoxSharedFolders/FnList");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatListFail,                  STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_LIST failures",                     "/HGCM/VPoxSharedFolders/FnListFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatReadLink,                  STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_READLINK successes",                "/HGCM/VPoxSharedFolders/FnReadLink");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatReadLinkFail,              STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_READLINK failures",                 "/HGCM/VPoxSharedFolders/FnReadLinkFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatMapFolderOld,              STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_MAP_FOLDER_OLD",                    "/HGCM/VPoxSharedFolders/FnMapFolderOld");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatMapFolder,                 STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_MAP_FOLDER successes",              "/HGCM/VPoxSharedFolders/FnMapFolder");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatMapFolderFail,             STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_MAP_FOLDER failures",               "/HGCM/VPoxSharedFolders/FnMapFolderFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatUnmapFolder,               STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_UNMAP_FOLDER successes",            "/HGCM/VPoxSharedFolders/FnUnmapFolder");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatUnmapFolderFail,           STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_UNMAP_FOLDER failures",             "/HGCM/VPoxSharedFolders/FnUnmapFolderFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationFail,           STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION early failures",        "/HGCM/VPoxSharedFolders/FnInformationFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationSetFile,        STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION/SET/FILE successes",    "/HGCM/VPoxSharedFolders/FnInformationSetFile");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationSetFileFail,    STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION/SET/FILE failures",     "/HGCM/VPoxSharedFolders/FnInformationSetFileFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationSetSize,        STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION/SET/SIZE successes",    "/HGCM/VPoxSharedFolders/FnInformationSetSize");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationSetSizeFail,    STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION/SET/SIZE failures",     "/HGCM/VPoxSharedFolders/FnInformationSetSizeFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationGetFile,        STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION/GET/FILE successes",    "/HGCM/VPoxSharedFolders/FnInformationGetFile");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationGetFileFail,    STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION/GET/FILE failures",     "/HGCM/VPoxSharedFolders/FnInformationGetFileFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationGetVolume,      STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION/GET/VOLUME successes",  "/HGCM/VPoxSharedFolders/FnInformationGetVolume");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatInformationGetVolumeFail,  STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_INFORMATION/GET/VOLUME failures",   "/HGCM/VPoxSharedFolders/FnInformationGetVolumeFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatRemove,                    STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_REMOVE successes",                  "/HGCM/VPoxSharedFolders/FnRemove");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatRemoveFail,                STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_REMOVE failures",                   "/HGCM/VPoxSharedFolders/FnRemoveFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCloseAndRemove,            STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CLOSE_AND_REMOVE successes",        "/HGCM/VPoxSharedFolders/FnCloseAndRemove");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCloseAndRemoveFail,        STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CLOSE_AND_REMOVE failures",         "/HGCM/VPoxSharedFolders/FnCloseAndRemoveFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatRename,                    STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_RENAME successes",                  "/HGCM/VPoxSharedFolders/FnRename");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatRenameFail,                STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_RENAME failures",                   "/HGCM/VPoxSharedFolders/FnRenameFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatFlush,                     STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_FLUSH successes",                   "/HGCM/VPoxSharedFolders/FnFlush");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatFlushFail,                 STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_FLUSH failures",                    "/HGCM/VPoxSharedFolders/FnFlushFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatSetErrorStyle,             STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_SET_ERROR_STYLE",                   "/HGCM/VPoxSharedFolders/FnSetErrorStyle");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatSetUtf8,                   STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_SET_UTF8",                          "/HGCM/VPoxSharedFolders/FnSetUtf8");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatSymlink,                   STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_SYMLINK successes",                 "/HGCM/VPoxSharedFolders/FnSymlink");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatSymlinkFail,               STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_SYMLINK failures",                  "/HGCM/VPoxSharedFolders/FnSymlinkFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatSetSymlinks,               STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_SET_SYMLINKS",                      "/HGCM/VPoxSharedFolders/FnSetSymlink");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatQueryMapInfo,              STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_QUERY_MAP_INFO",                    "/HGCM/VPoxSharedFolders/FnQueryMapInfo");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatQueryFeatures,             STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_QUERY_FEATURES",                    "/HGCM/VPoxSharedFolders/FnQueryFeatures");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCopyFile,                  STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_COPY_FILE successes",               "/HGCM/VPoxSharedFolders/FnCopyFile");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCopyFileFail,              STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_COPY_FILE failures",                "/HGCM/VPoxSharedFolders/FnCopyFileFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCopyFilePart,              STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_COPY_FILE_PART successes",          "/HGCM/VPoxSharedFolders/FnCopyFilePart");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCopyFilePartFail,          STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_COPY_FILE_PART failures",           "/HGCM/VPoxSharedFolders/FnCopyFilePartFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatWaitForMappingsChanges,    STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_WAIT_FOR_MAPPINGS_CHANGES successes", "/HGCM/VPoxSharedFolders/FnWaitForMappingsChanges");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatWaitForMappingsChangesFail,STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_WAIT_FOR_MAPPINGS_CHANGES failures","/HGCM/VPoxSharedFolders/FnWaitForMappingsChangesFail");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatCancelMappingsChangesWait, STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_CANCEL_MAPPINGS_CHANGES_WAITS",     "/HGCM/VPoxSharedFolders/FnCancelMappingsChangesWaits");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatUnknown,                   STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "SHFL_FN_???",                               "/HGCM/VPoxSharedFolders/FnUnknown");
             HGCMSvcHlpStamRegister(g_pHelpers, &g_StatMsgStage1,                 STAMTYPE_PROFILE, STAMVISIBILITY_ALWAYS, STAMUNIT_CALLS, "Time from VMMDev arrival to worker thread.","/HGCM/VPoxSharedFolders/MsgStage1");
        }
    }
