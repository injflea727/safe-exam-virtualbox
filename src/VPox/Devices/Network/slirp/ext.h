...
# define fprintf vpox_slirp_fprintf
# define printf vpox_slirp_printf

# ifndef vpox_slirp_printfV
DECLINLINE(void) vpox_slirp_printV(char *format, va_list args)
{
    char buffer[1024];
...
# ifndef vpox_slirp_printf
DECLINLINE(void) vpox_slirp_printf(char *format, ...)
{
    va_list args;
    va_start(args, format);
    vpox_slirp_printV(format, args);
    va_end(args);
}
# endif

# ifndef vpox_slirp_fprintf
DECLINLINE(void) vpox_slirp_fprintf(void *ignored, char *format, ...)
{
#  ifdef LOG_ENABLED
    va_list args;
    NOREF(ignored);
