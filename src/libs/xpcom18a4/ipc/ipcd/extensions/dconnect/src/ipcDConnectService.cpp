 ...
 *   Dmitry A. Kuminov <dmik@immotek.de>
 *
...
#define VPOX_XPCOM_SHUTDOWN_TIMEOUT_MS  (5000)
...
# else
  ...
  // during VPOX_XPCOM_SHUTDOWN_TIMEOUT_MS, join() those who
  // exited a working loop and abandon ones which have not
  // managed to do that when timeout occurred.
  LOG(("Worker threads: %d\n", mWorkers.Count()));
  uint64_t tsStart = RTTimeMilliTS();
  while ((tsStart + VPOX_XPCOM_SHUTDOWN_TIMEOUT_MS ) > RTTimeMilliTS() && mWorkers.Count() > 0)
  {
    ...
    if ((tsStart + VPOX_XPCOM_SHUTDOWN_TIMEOUT_MS ) < RTTimeMilliTS() || mWorkers.Count() == 0)
        break;
