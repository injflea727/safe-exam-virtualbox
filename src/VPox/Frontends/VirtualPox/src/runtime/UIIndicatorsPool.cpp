...
/** @file
 * VPox Qt GUI - UIIndicatorsPool class implementation.
 */
        ...
        foreach (const CStorageController &controller, machine.GetStorageControllers())
        {
            ...
            foreach (const CMediumAttachment &attachment, machine.GetMediumAttachmentsOfController(controller.GetName()))
            {
                ...
                if (attachment.GetType() != KDeviceType_DVD)
                    continue;
                /* Append attachment data: */
                UIMedium vpoxMedium(attachment.GetMedium(), UIMediumDeviceType_DVD);
                strAttData += s_strTableRow4
                    .arg(gpConverter->toString(StorageSlot(controller.GetBus(), attachment.GetPort(), attachment.GetDevice())))
                    .arg(vpoxMedium.isNull() || vpoxMedium.isHostDrive() ? vpoxMedium.name() : vpoxMedium.location());
                fAttachmentsPresent = true;
                if (!vpoxMedium.isNull())
        ...
        foreach (const CStorageController &controller, machine.GetStorageControllers())
        {
            ...
            foreach (const CMediumAttachment &attachment, machine.GetMediumAttachmentsOfController(controller.GetName()))
            {
                ...
                if (attachment.GetType() != KDeviceType_Floppy)
                    continue;
                /* Append attachment data: */
                UIMedium vpoxMedium(attachment.GetMedium(), UIMediumDeviceType_Floppy);
                strAttData += s_strTableRow4
                    .arg(gpConverter->toString(StorageSlot(controller.GetBus(), attachment.GetPort(), attachment.GetDevice())))
                    .arg(vpoxMedium.isNull() || vpoxMedium.isHostDrive() ? vpoxMedium.name() : vpoxMedium.location());
                fAttachmentsPresent = true;
...
public:
        ...
        , m_cMaxNetworkAdapters(0)
    {
        ...
        const CVirtualPox vpox = uiCommon().virtualPox();
        const CMachine machine = m_pSession->machine();
        m_cMaxNetworkAdapters = vpox.GetSystemProperties().GetMaxNetworkAdapters(machine.GetChipsetType());
...
private:
    ...
    void updateAppearance()
    {
        ...
        LONG64 iTimestamp;
        machine.GetGuestProperty("/VirtualPox/GuestInfo/Net/Count", strCount, iTimestamp, strFlags);
        ...
        if (fPropsValid)
        {
            ...
            {
                ipList << machine.GetGuestPropertyValue(QString("/VirtualPox/GuestInfo/Net/%1/V4/IP").arg(i));
                macList << machine.GetGuestPropertyValue(QString("/VirtualPox/GuestInfo/Net/%1/MAC").arg(i));
            }
        }
    ...
    void updateAppearance()
    {
        ...
        for (QMap<QString, QString>::const_iterator it = sfs.constBegin(); it != sfs.constEnd(); ++it)
        {
            ...
            if (UICommon::isDOSType(guest.GetOSTypeId()))
                strFullData += s_strTableRow2.arg(QString("<b>\\\\vpoxsvr\\%1</b>").arg(it.key()), it.value());
            else
