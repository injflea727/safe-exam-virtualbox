...
"""
VirtualPox Validation Kit - IAppliance Test #1
"""
...
class SubTstDrvAppliance1(base.SubTestDriverBase):
    ...
    def testImportOva(self, sOva):
        """ xxx """
        oVirtualPox = self.oTstDrv.oVPoxMgr.getVirtualPox();

        #
        # Import it as OVA.
        #
        try:
            oAppliance = oVirtualPox.createAppliance();
        except:
            return reporter.errorXcpt('IVirtualPox::createAppliance failed');

        try:
            oProgress = vpoxwrappers.ProgressWrapper(oAppliance.read(sOva), self.oTstDrv.oVPoxMgr, self.oTstDrv,
                                                     'read "%s"' % (sOva,));
        except:
        ...
        #
        try:
            oProgress = vpoxwrappers.ProgressWrapper(oAppliance.importMachines([]),
                                                     self.oTstDrv.oVPoxMgr, self.oTstDrv, 'importMachines "%s"' % (sOva,));
        except:
    ...
    def testImportOvaAsOvf(self, sOva):
        ...
        #
        try:
            oAppliance2 = oVirtualPox.createAppliance();
        except:
            return reporter.errorXcpt('IVirtualPox::createAppliance failed (#2)');

        try:
            oProgress = vpoxwrappers.ProgressWrapper(oAppliance2.read(sOvf), self.oTstDrv.oVPoxMgr, self.oTstDrv,
                                                     'read "%s"' % (sOvf,));
        except:
        ...
        try:
            oProgress = vpoxwrappers.ProgressWrapper(oAppliance2.importMachines([]),
                                                     self.oTstDrv.oVPoxMgr, self.oTstDrv, 'importMachines "%s"' % (sOvf,));
        except:
