...
"""
VirtualPox Validation Kit - Guest OS unattended installation tests.
"""
...
class UnattendedVm(vpoxtestvms.BaseTestVm):
    ...
    kasIdeIrqDelay     = [ 'VPoxInternal/Devices/piix3ide/0/Config/IRQDelay:1', ];

    def __init__(self, oSet, sVmName, sKind, sInstallIso, fFlags = 0):
        vpoxtestvms.BaseTestVm.__init__(self, sVmName, oSet = oSet, sKind = sKind,
    ...
    def _unattendedConfigure(self, oIUnattended, oTestDrv): # type: (Any, vpox.TestDriver) -> bool
        """
        ...
        try:    oIUnattended.validationKitIsoPath   = oTestDrv.sVPoxValidationKitIso;
        except: return reporter.errorXcpt();
    ...
    def _unattendedDoIt(self, oIUnattended, oVM, oTestDrv): # type: (Any, Any, vpox.TestDriver) -> bool
        """
        ...
        if eNic0AttachType is None:
            if self.isLinux() \
               and (   'ubuntu' in self.sKind.lower()
                ...
                eNic0AttachType = vpoxcon.NetworkAttachmentType_HostOnly;

            # Also use it for windows xp to prevent it from ever going online.
            if self.sKind in ('WindowsXP','WindowsXP_64',):
                eNic0AttachType = vpoxcon.NetworkAttachmentType_HostOnly;

        return vpoxtestvms.BaseTestVm._createVmDoIt(self, oTestDrv, eNic0AttachType, sDvdImage);
    ...
    def _createVmPost(self, oTestDrv, oVM, eNic0AttachType, sDvdImage):
        ...
        #fRc = fRc and oSession.setBootOrder(1, vpoxcon.DeviceType_HardDisk)
        #fRc = fRc and oSession.setBootOrder(2, vpoxcon.DeviceType_DVD)
    ...
    def _skipVmTest(self, oTestDrv, oVM):
        ...
        return vpoxtestvms.BaseTestVm._skipVmTest(self, oTestDrv, oVM);
    ...
    def getReconfiguredVm(self, oTestDrv, cCpus, sVirtMode, sParavirtMode = None):
        ...
        #
        (fRc, oVM) = vpoxtestvms.BaseTestVm.getReconfiguredVm(self, oTestDrv, cCpus, sVirtMode, sParavirtMode);
        if fRc is True:
            #
            ...
            if fRc is True:
                ...
                #
                try:
                    oIUnattended = oTestDrv.oVPox.createUnattendedInstaller();
                except:
    ...
    def unattendedDetectOs(self, oIUnattended, oTestDrv): # type: (Any, vpox.TestDriver) -> bool
        """
        ...
        try:
            ...
            if oTestDrv.oVPoxMgr.xcptIsNotEqual(None, oTestDrv.oVPoxMgr.statuses.E_NOTIMPL):
                return reporter.errorXcpt('sInstallIso=%s' % (sInstallIso,));
...
class tdGuestOsInstTest1(vpox.TestDriver):
    """
    ...
    def __init__(self):
        ...
        #
        oSet = vpoxtestvms.TestVmSet(self.oTestVmManager, fIgnoreSkippedVm = True);
    ...
    def testOneVmConfig(self, oVM, oTestVm): # type: (Any, UnattendedVm) -> bool
        ...
        if oSession is not None:
            ...
            #        Looks like VPoxService isn't there.
            ...
            if oTestVm.fOptInstallAdditions and fRc is True:
                ...
                aenmRunLevels = [vpoxcon.AdditionsRunLevelType_Userland,];
                if oTestVm.isLoggedOntoDesktop():
                    aenmRunLevels.append(vpoxcon.AdditionsRunLevelType_Desktop);
                fRc = self.waitForGAs(oSession, cMsTimeout = cMsTimeout / 2, aenmWaitForRunLevels = aenmRunLevels,
                                      aenmWaitForActive = (vpoxcon.AdditionsFacilityType_VPoxGuestDriver,
                                                           vpoxcon.AdditionsFacilityType_VPoxService,));
                reporter.testDone();
