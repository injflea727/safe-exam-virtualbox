...
/** @file
 * VPox Qt GUI - UIApplianceImportEditorWidget class implementation.
 */
...
void UIApplianceImportEditorWidget::prepareWidgets()
{
    ...
    if (m_pOptionsLayout)
    {
        ...
        if (m_pPathSelector)
        {
            ...
            m_pPathSelector->setDefaultPath(uiCommon().virtualPox().GetSystemProperties().GetDefaultMachineFolder());
            m_pPathSelector->setPath(uiCommon().virtualPox().GetSystemProperties().GetDefaultMachineFolder());
...
bool UIApplianceImportEditorWidget::setFile(const QString& strFile)
{
    ...
    if (!strFile.isEmpty())
    {
        ...
        m_pAppliance = new CAppliance(vpox.CreateAppliance());
