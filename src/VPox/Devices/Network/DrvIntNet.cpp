...
#define VPOX_WITH_DRVINTNET_IN_R0
 ...
 *
 * @returns VPox status code. Returning failure will naturally terminate the thread.
 ...
 * @returns VPox status code on other errors.
 ...
 *
 * @returns VERR_STATE_CHANGED if the state changed.
 * @returns Appropriate VPox status code (error) on fatal error.
...
#ifdef VPOX_WITH_DRVINTNET_IN_R0
 ...
 *
 * @returns VPox status code (error set on failure).
 * @param   pDrvIns             The driver instance.
...
static DECLCALLBACK(int) drvR3IntNetConstruct(PPDMDRVINS pDrvIns, PCFGMNODE pCfg, uint32_t fFlags)
{
    ...
    if (rc == VERR_CFGM_VALUE_NOT_FOUND)
        OpenReq.cbSend = RT_ALIGN_Z(VPOX_MAX_GSO_SIZE * 3, _1K);
    else if (RT_FAILURE(rc))
        return PDMDRV_SET_ERROR(pDrvIns, rc,
