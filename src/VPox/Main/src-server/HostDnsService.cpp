...
{
    Data(HostDnsServiceBase *aMonitor, VirtualPox *aParent)
        : pVirtualPox(aParent)
...
void HostDnsMonitorProxy::pollGlobalExtraData(void)
{
    ...
    if (RT_UNLIKELY(pVirtualPox == NULL))
        return;
    ...
    if (uNow - m->uLastExtraDataPoll >= RT_NS_30SEC || m->uLastExtraDataPoll == 0)
    {
         ...
         */
        const com::Bstr bstrHostDNSOrderIgnoreKey("VPoxInternal2/HostDNSOrderIgnore");
        com::Bstr bstrHostDNSOrderIgnore;
        pVirtualPox->GetExtraData(bstrHostDNSOrderIgnoreKey.raw(),
         ...
         */
        const com::Bstr bstrHostDNSSuffixesIgnoreKey("VPoxInternal2/HostDNSSuffixesIgnore");
        com::Bstr bstrHostDNSSuffixesIgnore;
        pVirtualPox->GetExtraData(bstrHostDNSSuffixesIgnoreKey.raw(),
...
HRESULT HostDnsMonitorProxy::init(VirtualPox* aParent)
{
...
void HostDnsMonitorProxy::notify(const HostDnsInformation &info)
{
    ...
    if (fNotify)
        m->pVirtualPox->i_onHostNameResolutionConfigurationChange();
}
