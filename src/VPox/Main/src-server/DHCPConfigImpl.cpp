...
/** @file
 * VirtualPox Main - IDHCPConfig, IDHCPConfigGlobal, IDHCPConfigGroup, IDHCPConfigIndividual implementation.
 */
...
HRESULT DHCPConfig::i_initWithDefaults(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent)
{
    unconst(m_pVirtualPox) = a_pVirtualPox;
    unconst(m_pParent)     = a_pParent;
    return S_OK;
}


HRESULT DHCPConfig::i_initWithSettings(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent, const settings::DHCPConfig &rConfig)
{
    unconst(m_pVirtualPox) = a_pVirtualPox;
    unconst(m_pParent)     = a_pParent;
...
{
    {
        ...
        if (it != m_OptionMap.end())
            ...
            return m_pHack->setError(VPOX_E_OBJECT_NOT_FOUND, m_pHack->tr("DHCP option %u was not found"), aOption);
    }
    return i_doWriteConfig();
}
...
/**
 * Causes the global VirtualPox configuration file to be written
 *
...
{
    AssertPtrReturn(m_pVirtualPox, E_FAIL);

    AutoWriteLock alock(m_pVirtualPox COMMA_LOCKVAL_SRC_POS);
    return m_pVirtualPox->i_saveSettings();
}
...
HRESULT DHCPGlobalConfig::initWithDefaults(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);

    HRESULT hrc = DHCPConfig::i_initWithDefaults(a_pVirtualPox, a_pParent);
    if (SUCCEEDED(hrc))
...
HRESULT DHCPGlobalConfig::initWithSettings(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent, const settings::DHCPConfig &rConfig)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);

    HRESULT hrc = DHCPConfig::i_initWithSettings(a_pVirtualPox, a_pParent, rConfig);
    if (SUCCEEDED(hrc))
        autoInitSpan.setSucceeded();
    else
...
HRESULT DHCPGlobalConfig::i_getNetworkMask(com::Utf8Str &a_rDst)
{
    ...
    if (it != m_OptionMap.end())
    {
        ...
        return setError(VPOX_E_OBJECT_NOT_FOUND, tr("DHCP option DHCPOption_SubnetMask is not in a legacy encoding"));
    }
    return setError(VPOX_E_OBJECT_NOT_FOUND, tr("DHCP option DHCPOption_SubnetMask was not found"));
}
...
HRESULT DHCPGroupConfig::initWithDefaults(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent, const com::Utf8Str &a_rName)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);
...
HRESULT DHCPGroupConfig::initWithSettings(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent, const settings::DHCPGroupConfig &a_rSrc)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);

    Assert(m_Conditions.size() == 0);
    HRESULT hrc = DHCPConfig::i_initWithSettings(a_pVirtualPox, a_pParent, a_rSrc);
    if (SUCCEEDED(hrc))
...
HRESULT DHCPIndividualConfig::initWithMachineIdAndSlot(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent,
...
HRESULT DHCPIndividualConfig::initWithMACAddress(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent, PCRTMAC a_pMACAddress)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);
...
HRESULT DHCPIndividualConfig::initWithSettingsAndMachineIdAndSlot(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent,
...
HRESULT DHCPIndividualConfig::initWithSettingsAndMACAddress(VirtualPox *a_pVirtualPox, DHCPServer *a_pParent,
...
HRESULT DHCPIndividualConfig::i_getMachineMAC(PRTMAC pMACAddress)
{
    ...
    HRESULT hrc = m_pVirtualPox->i_findMachine(m_idMachine, false /*fPermitInaccessible*/, true /*aSetError*/, &ptrMachine);
    if (SUCCEEDED(hrc))
    {
