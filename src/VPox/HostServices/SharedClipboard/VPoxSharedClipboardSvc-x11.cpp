/* $Id: VPoxSharedClipboardSvc-x11.cpp $ */
/** @file
...
struct SHCLCONTEXT
{
    ...
    /** X11 context data. */
    SHCLX11CTX           X11;
    /** Pointer to the VPox host client data structure. */
    PSHCLCLIENT          pClient;
...
int ShClSvcImplInit(VPOXHGCMSVCFNTABLE *pTable)
{
    ...
    for (uintptr_t i = 0; i < RT_ELEMENTS(pTable->acMaxClients); i++)
        pTable->acMaxClients[i] = RT_MIN(VPOX_SHARED_CLIPBOARD_X11_CONNECTIONS_MAX, pTable->acMaxClients[i]);
...
int ShClSvcImplConnect(PSHCLCLIENT pClient, bool fHeadless)
{
    ...
    if (ASMAtomicIncU32(&g_cShClConnections) > VPOX_SHARED_CLIPBOARD_X11_CONNECTIONS_MAX)
    {
...
int ShClSvcImplSync(PSHCLCLIENT pClient)
{
    ...
    if (ShClSvcIsBackendActive())
        return ShClSvcHostReportFormats(pClient, VPOX_SHCL_FMT_NONE);
    return VINF_SUCCESS;
}
...
/**
 * Reports formats available in the X11 clipboard to VPox.
 *
...
DECLCALLBACK(void) ShClX11ReportFormatsCallback(PSHCLCONTEXT pCtx, uint32_t fFormats)
{
    ...
    if (RT_SUCCESS(rc))
    {
        if (ShClSvcIsBackendActive())
        {
            ...
            if (fFormats != VPOX_SHCL_FMT_NONE) /* No formats to report? */
            {
...
DECLCALLBACK(int) ShClX11RequestDataForX11Callback(PSHCLCONTEXT pCtx, SHCLFORMAT fFormat, void **ppv, uint32_t *pcb)
{
    ...
    if (fFormat == VPOX_SHCL_FMT_URI_LIST)
        rc = VINF_SUCCESS;
    else
#endif
    {
...
int ShClSvcImplTransferGetRoots(PSHCLCLIENT pClient, PSHCLTRANSFER pTransfer)
{
    ...
    if (idEvent != NIL_SHCLEVENTID)
    {
        ...
        if (pReq)
        {
            ...
            rc = ShClX11ReadDataFromX11(&pClient->State.pCtx->X11, VPOX_SHCL_FMT_URI_LIST, pReq);
            if (RT_SUCCESS(rc))
            {
