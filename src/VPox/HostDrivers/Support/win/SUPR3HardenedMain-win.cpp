...
/** @file
 * VirtualPox Support Library - Hardened main(), windows bits.
 */
...
#ifndef VPOX_WITHOUT_HARDENDED_XCPT_LOGGING
/** Pointer to the bit of assembly code that will perform the original
 ...
 *
 *                              This is only needed to load VPoxRT.dll when
...
/**
 * Translates VPox status code (from supHardenedWinVerifyImageTrust) to an NT
 * status.
 *
 * @returns NT status.
                                         ...
                                         bool *pfCallRealApi, const char *pszCaller, bool fAvoidWinVerifyTrust, bool *pfQuiet)
{
    *pfCallRealApi = false;
     ...
     *      3. VirtualPox    - kernel code signing and integrity checks.
...
#else  /* VPOX_PERMIT_EVEN_MORE */
    /*
     * Require trusted installer + some kind of signature on everything, except
     * for the VPox bits where we require kernel code signing and special
     * integrity checks.
     */
    uint32_t fFlags = 0;
...
supR3HardenedMonitor_LdrLoadDll(PWSTR pwszSearchPath, PULONG pfFlags, PUNICODE_STRING pName, PHANDLE phMod)
{
    ...
    else
    {
        ...
        else
        {
             ...
             * a search on the API level, all VPox calls will have full paths.
...
#endif /* !VPOX_WITHOUT_HARDENDED_XCPT_LOGGING */
...
#ifdef RT_ARCH_X86
                          "(It is also possible you are running 32-bit VirtualPox under 64-bit windows.)\n"
#endif
                          ,
                          pszWhich,
 ...
 * the parent does.
 *
 * This is a necessary evil to make VPoxNetDhcp and VPoxNetNat termination from
 ...
 *
 * @param   hVPoxRT             The VPoxRT.dll handle.  We use RTThreadCreate to
 *                              spawn the thread to avoid duplicating thread
 *                              creation and thread naming code from IPRT.
 */
DECLHIDDEN(void) supR3HardenedWinCreateParentWatcherThread(HMODULE hVPoxRT)
{
    /*
     * Resolve runtime methods that we need.
     */
    PFNRTTHREADCREATE pfnRTThreadCreate = (PFNRTTHREADCREATE)GetProcAddress(hVPoxRT, "RTThreadCreate");
...
static DECL_NO_RETURN(void) supR3HardenedWinDoReSpawn(int iWhich)
{
    ...
    if (!CreateProcessW(g_wszSupLibHardenedExePath,
        ...
        supR3HardenedFatalMsg("supR3HardenedWinReSpawn", kSupInitOp_Misc, VERR_INVALID_NAME,
                              "Error relaunching VirtualPox VM process: %u\n"
                              "Command line: '%ls'",
    ...
    if (!NT_SUCCESS(rcNt))
        supR3HardenedFatalMsg("supR3HardenedWinReSpawn", kSupInitOp_Misc, VERR_INVALID_NAME,
                              "Error relaunching VirtualPox VM process: %#x\n"
                              "Command line: '%ls'",
                              rcNt, CmdLine.Buffer);
...
/**
 * Tries to open VPoxDrvErrorInfo and read extra error info from it.
 *
...
static void supR3HardenedWinOpenStubDevice(void)
{
    ...
    /*
     * Retry if we think driver might still be initializing (STATUS_NO_SUCH_DEVICE + \Drivers\VPoxDrv).
     */
    ...
    for (iTry = 0;; iTry++)
    {
        ...
        if (iTry > 0 && supR3HardenedWinGetMilliTS() - uMsTsStart > 5000)  /* 5 sec, at least two tries */
            break;
        if (!supR3HardenedWinDriverExists("VPoxDrv"))
        {
            /** @todo Consider starting the VPoxdrv.sys service. Requires 2nd process
    ...
    else
    {
         ...
         * extra information that goes into VPoxStartup.log so that we stand a
         * better chance resolving the issue.
         */
        char szErrorInfo[16384];
        int rc = VERR_OPEN_FAILED;
        if (SUP_NT_STATUS_IS_VPOX(rcNt)) /* See VPoxDrvNtErr2NtStatus. */
        {
            rc = SUP_NT_STATUS_TO_VPOX(rcNt);
            ...
            {
                SUP_DPRINTF(("Error opening VPoxDrvStub: VERR_SUPDRV_APIPORT_OPEN_ERROR\n"));
                ...
                supR3HardenedFatalMsg("supR3HardenedWinReSpawn", kSupInitOp_Misc, rc,
                                      ...
                                      "\n"
                                      "Error getting %s\\ApiPort in the driver from vpoxsup.\n"
                                      "\n"
                                      "Could be due to security software is redirecting access to it, so please include full "
                                      "details of such software in a bug report. VPoxStartup.log may contain details important "
...
/** @todo don't fail during early init, wait till later and try load the driver if missing or at least query the service manager for additional information. */
            if (   rcNt == STATUS_NO_SUCH_DEVICE
                || rcNt == STATUS_OBJECT_NAME_NOT_FOUND)
            {
                SUP_DPRINTF(("Error opening VPoxDrvStub: %s\n", pszDefine));
                if (supR3HardenedWinDriverExists("VPoxDrv"))
                    supR3HardenedFatalMsg("supR3HardenedWinReSpawn", kSupInitOp_Driver, VERR_OPEN_FAILED,
                                          "NtCreateFile(%ls) failed: %#x%s (%u retries)\n"
                                          "\n"
                                          "Driver is probably stuck stopping/starting. Try 'sc.exe query vpoxsup' to get more "
                    ...
                    supR3HardenedFatalMsg("supR3HardenedWinReSpawn", kSupInitOp_Driver, VERR_OPEN_FAILED,
                                          "NtCreateFile(%ls) failed: %#x%s (%u retries)\n"
                                          "\n"
                                          "Driver is does not appear to be loaded. Try 'sc.exe start vpoxsup', reinstall "
                                          "VirtualPox or reboot.%s"
                                          , s_wszName, rcNt, pszDefine, iTry,
                                          supR3HardenedWinReadErrorInfoDevice(szErrorInfo, sizeof(szErrorInfo),
                                                                              "\nVPoxDrvStub error: "));
            }
...
DECLHIDDEN(int) supR3HardenedWinReSpawn(int iWhich)
{
     ...
     * support driver via /Devices/VPoxDrvStub.  (We tried to do this
     * during the early init, but in case we had trouble accessing vpoxdrv
     * (renamed to vpoxsup in 7.0 and 6.1.34) we retry it here where we
...
DECLHIDDEN(void) supR3HardenedWinModifyDllSearchPath(uint32_t fFlags, const char *pszAppBinPath)
{
     ...
     * able to find the VPox DLLs.  This is done _after_ VPoxRT.dll is
 ...
 * software that is up to no good.  Once we've opened vpoxdrv (renamed to
 * vpoxsup in 7.0 and 6.1.34), the process should be locked down so tightly
...
DECLASM(uintptr_t) supR3HardenedEarlyProcessInit(void)
{
    ...
    {
        SUP_DPRINTF(("supR3HardenedVmProcessInit: Opening vpoxsup stub...\n"));
        supR3HardenedWinOpenStubDevice();
    ...
    {
        SUP_DPRINTF(("supR3HardenedVmProcessInit: Opening vpoxsup...\n"));
