/* $Id: VPoxSF-VfsOps.cpp $ */
/** @file
 * VPoxFS - Darwin Shared Folders, Virtual File System Operations.
 */
 ...
 */
static int vpoxSfDwnVfsGetAttr(mount_t pMount, struct vfs_attr *pFsAttr, vfs_context_t pContext)
{
    PVPOXSFMNTDATA pThis = (PVPOXSFMNTDATA)vfs_fsprivate(pMount);
    AssertReturn(pThis, EBADMSG);
    LogFlow(("vpoxSfDwnVfsGetAttr: %s\n", pThis->MntInfo.szFolder));
    RT_NOREF(pContext);
 ...
 * @param   pContext    Unused kAuth parameter.
 */
static int vpoxSfDwnVfsRoot(mount_t pMount, vnode_t *ppVnode, vfs_context_t pContext)
{
    PVPOXSFMNTDATA pThis = (PVPOXSFMNTDATA)vfs_fsprivate(pMount);
    AssertReturn(pThis, EBADMSG);
    LogFlow(("vpoxSfDwnVfsRoot: pThis=%p:{%s}\n", pThis, pThis->MntInfo.szFolder));
    RT_NOREF(pContext);
    ...
    if (pThis->pVnRoot)
    {
        ...
        if (rc == 0)
        {
            *ppVnode = pThis->pVnRoot;
            LogFlow(("vpoxSfDwnVfsRoot: return %p\n", *ppVnode));
            return 0;
        }
        Log(("vpoxSfDwnVfsRoot: vnode_get failed! %d\n", rc));
        return rc;
    }

    LogRel(("vpoxSfDwnVfsRoot: pVnRoot is NULL!\n"));
    return EILSEQ;
}
 ...
 *
 */
static int vpoxSfDwnVfsUnmount(mount_t pMount, int fFlags, vfs_context_t pContext)
{
    PVPOXSFMNTDATA pThis = (PVPOXSFMNTDATA)vfs_fsprivate(pMount);
    ...
    if (rc == 0)
    {
        ...
        {
            Log(("VPoxSF: umount failed: file system busy! (%s)\n", pThis->MntInfo.szFolder));
            rc = EBUSY;
        }
    }
    return rc;
 ...
 */
static int vpoxSfDwnVfsStart(mount_t pMount, int fFlags, vfs_context_t pContext)
{
 ...
 *                      We expect a VPOXSFDRWNMOUNTINFO structure.
 * @param   pContext    kAuth context needed in order to authentificate mount
 *                      operation.
 */
static int vpoxSfDwnVfsMount(mount_t pMount, vnode_t pDevVp, user_addr_t pUserData, vfs_context_t pContext)
{
    ...
    {
        LogRel(("VPoxSF: mount: MNT_UPDATE is not supported.\n"));
        return ENOTSUP;
    }
    if (pUserData == USER_ADDR_NULL)
    {
        LogRel(("VPoxSF: mount: pUserData is NULL.\n"));
        return EINVAL;
    }
     ...
     */
    PVPOXSFMNTDATA pThis = (PVPOXSFMNTDATA)RTMemAllocZ(sizeof(*pThis));
    if (!pThis)
        return ENOMEM;
    ...
    if (RT_FAILURE(rc))
    {
        LogRel(("VPoxSF: mount: Failed to copy in mount user data: %Rrc\n", rc));
        rc = EFAULT;
    }
    else if (pThis->MntInfo.u32Magic != VPOXSFDRWNMOUNTINFO_MAGIC)
    {
        LogRel(("VPoxSF: mount: Invalid user data magic (%#x)\n", pThis->MntInfo.u32Magic));
        rc = EINVAL;
    }
    ...
    {
        LogRel(("VPoxSF: mount: Invalid or empty share name!\n"));
        rc = EINVAL;
    }
    else
    {
        ...
        if (vpoxSfDwnConnect())
        {
            ...
            if (pName)
            {
                ...
                if (RT_SUCCESS(rc))
                {
                     ...
                     */
                    pThis->pVnRoot = vpoxSfDwnVnAlloc(pMount, VDIR, NULL /*pParent*/, 0);
                    if (pThis->pVnRoot)
                    {
                        /*
                        ...
                        LogRel(("VPoxSF: mount: Successfully mounted '%s' (uidMounter=%u).\n",
                                pThis->MntInfo.szFolder, pThis->uidMounter));
                        return 0;
                    }

                    LogRel(("VPoxSF: mount: Failed to allocate root node!\n"));
                    rc = ENOMEM;
                }
                else
                {
                    LogRel(("VPoxSF: mount: VbglR0SfMapFolder failed on '%s': %Rrc\n", pThis->MntInfo.szFolder, rc));
                    rc = ENOENT;
                }
            }
            else
        ...
        {
            LogRel(("VPoxSF: mount: Not connected to shared folders service!\n"));
            rc = ENOTCONN;
        }
    }
    RTMemFree(pThis);
