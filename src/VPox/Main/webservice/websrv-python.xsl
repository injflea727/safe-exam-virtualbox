...
<!--
    ...
    websrv-python.xsl:
        XSLT stylesheet that generates VirtualPox_services.py from
        VirtualPox.xidl. This Python file represents our
...
# Works only with ZSI 2.0 generated stubs (part of the VirtualPox SDK).
from VirtualPox_client import *
...
class IWebsessionManager2(IWebsessionManager, ObjectRefManager):
  ...
  def getPort(self):
      if self.port is None:
          try:
             self.port = vpoxServiceLocator().getvpoxPortType(self.url)
          except:
             self.port = vpoxServiceLocator().getvpoxServicePort(self.url)
      return self.port
