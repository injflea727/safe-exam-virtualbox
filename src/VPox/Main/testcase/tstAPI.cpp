...
static Bstr getObjectName(ComPtr<IVirtualPox> aVirtualPox, ComPtr<IUnknown> aObject);
static void queryMetrics(ComPtr<IVirtualPox> aVirtualPox,
                         ComPtr<IPerformanceCollector> collector,
                         ComSafeArrayIn(IUnknown *, objects));
static void listAffectedMetrics(ComPtr<IVirtualPox> aVirtualPox,
...
int main(int argc, char *argv[])
{
...

    {
        ...
        GetVPoxUserHomeDirectory(homeDir, sizeof(homeDir));
        RTPrintf("VirtualPox Home Directory = '%s'\n", homeDir);
    }
    ...
    RTPrintf("Creating VirtualPox object...\n");
    ...
    if (FAILED(rc))
        RTPrintf("ERROR: failed to create the VirtualPox object!\n");
    else
    {
...
#if 0
    // Testing VirtualPox::COMGETTER(ProgressOperations).
    // This is designed to be tested while running
    // "./VPoxManage clonehd src.vdi clone.vdi" in parallel.
    // It will then display the progress every 2 seconds.
    ////////////////////////////////////////////////////////////////////////////
    {
        RTPrintf("Testing VirtualPox::COMGETTER(ProgressOperations)...\n");

        for (;;) {
            com::SafeIfaceArray<IProgress> operations;

            CHECK_ERROR_BREAK(virtualPox,
    ...
    {
        {
            ComPtr<IVirtualPox> virtualPox2;

            RTPrintf("Creating one more VirtualPox object...\n");
            CHECK_RC(virtualPoxClient->COMGETTER(virtualPox2.asOutParam()));
            if (FAILED(rc))
            {
                CHECK_ERROR_NOCALL();
                break;
            ...
            RTPrintf("IVirtualPox(virtualPox)=%p IVirtualPox(virtualPox2)=%p\n",
                     (IVirtualPox *)virtualPox, (IVirtualPox *)virtualPox2);
            Assert((IVirtualPox *)virtualPox == (IVirtualPox *)virtualPox2);

            ComPtr<IUnknown> unk(virtualPox);
            ComPtr<IUnknown> unk2;
            unk2 = virtualPox2;

            RTPrintf("IUnknown(virtualPox)=%p IUnknown(virtualPox2)=%p\n",
                     (IUnknown *)unk, (IUnknown *)unk2);
            Assert((IUnknown *)unk == (IUnknown *)unk2);

            ComPtr<IVirtualPox> vb = unk;
            ComPtr<IVirtualPox> vb2 = unk;

            RTPrintf("IVirtualPox(IUnknown(virtualPox))=%p IVirtualPox(IUnknown(virtualPox2))=%p\n",
                    (IVirtualPox *)vb, (IVirtualPox *)vb2);
            Assert((IVirtualPox *)vb == (IVirtualPox *)vb2);
        }

        {
            ComPtr<IHost> host;
            CHECK_ERROR_BREAK(virtualPox, COMGETTER(Host)(host.asOutParam()));
...
//        getchar();
    ...
    {
        Bstr version;
        CHECK_ERROR_BREAK(virtualPox, COMGETTER(Version)(version.asOutParam()));
        RTPrintf("VirtualPox version = %ls\n", version.raw());
    }
#endif
    ...
    ////////////////////////////////////////////////////////////////////////////
    {
        RTPrintf("Calling IVirtualPox::Machines...\n");
    ...
    ////////////////////////////////////////////////////////////////////////////
    {
        ...
        rc = virtualPox->GetHardDisk(uuid, hardDisk.asOutParam());
        RTPrintf("virtualPox->GetHardDisk(null-uuid)=%08X\n", rc);

//        {
//            com::ErrorInfo info(virtualPox);
//            PRINT_ERROR_INFO(info);
//        }

        // call a method that will definitely succeed
        Bstr version;
        rc = virtualPox->COMGETTER(Version)(version.asOutParam());
        RTPrintf("virtualPox->COMGETTER(Version)=%08X\n", rc);
...
//        }
    ...
    do
    {
        ...
        CHECK_ERROR_BREAK(virtualPox, RegisterHardDisk(hd));
        RTPrintf("Enter to continue...\n");
        getchar();
    }
    while (FALSE);
    ...
    do
    {
        ...
        CHECK_ERROR_BREAK(virtualPox, FindVirtualDiskImage(src, vdi.asOutParam()));
        ComPtr<IHardDisk> hd = vdi;
        Guid id;
        CHECK_ERROR_BREAK(hd, COMGETTER(Id)(id.asOutParam()));
        CHECK_ERROR_BREAK(virtualPox, UnregisterHardDisk(id, hd.asOutParam()));
    }
    while (FALSE);
    RTPrintf("\n");
#endif
    ...
    do
    {
        ...
        for (size_t i = 0; i < RT_ELEMENTS(Names); ++ i)
        {
            ...
            rc = virtualPox->FindHardDisk(src, hd.asOutParam());
            if (SUCCEEDED(rc))
            {
                Guid id;
                Bstr location;
    ...
    do
    {
        ...
        CHECK_ERROR_BREAK(virtualPox, FindMachine(name, machine.asOutParam()));
        ...
        else
        {
            ...
            machine->COMSETTER(ExtraData)(L"VPoxSDL/SecureLabel", secureLabel);
        }
#endif
    }
    while (0);
    ...
    do
    {
        ComPtr<IMachine> machine;
#if defined(RT_OS_LINUX)
        Bstr baseDir = L"/tmp/vpox";
#else
        Bstr baseDir = L"C:\\vpox";
#endif
        Bstr name = L"machina";

        RTPrintf("Creating a new machine object(base dir '%ls', name '%ls')...\n",
                baseDir.raw(), name.raw());
        CHECK_ERROR_BREAK(virtualPox, CreateMachine(name, L"", baseDir, L"",
                                                    false,
                                                    machine.asOutParam()));
        ...
        CHECK_ERROR_BREAK(virtualPox, GetGuestOSType(type, guestOSType.asOutParam()));
    ...
    {
        ComPtr<IHost> host;
        CHECK_RC_BREAK(virtualPox->COMGETTER(Host)(host.asOutParam()));
    ...
    do
    {
        ...
        CHECK_RC_BREAK(virtualPox->OpenSession(session, guid));
#if 1
    ...
    do
    {
        ...
        Bstr name = L"dos";
        RTPrintf("Getting a machine object named '%ls'...\n", name.raw());
        CHECK_RC_BREAK(virtualPox->FindMachine(name, machine.asOutParam()));
        Guid guid;
        CHECK_RC_BREAK(machine->COMGETTER(Id)(guid.asOutParam()));
        RTPrintf("Opening a remote session for this machine...\n");
        ComPtr<IProgress> progress;
        CHECK_RC_BREAK(virtualPox->OpenRemoteSession(session, guid, Bstr("gui"),
    ...
    do
    {
        ...
        CHECK_RC_BREAK(virtualPox->OpenExistingSession(session, guid));
        ComPtr<IMachine> sessionMachine;
        RTPrintf("Getting machine session object...\n");
        CHECK_RC_BREAK(session->COMGETTER(Machine)(sessionMachine.asOutParam()));

#if 0
        Bstr extraDataKey = "VPoxSDL/SecureLabel";
...
#if 0 && defined(VPOX_WITH_RESOURCE_USAGE_API)
    do {
        // Get collector
        ComPtr<IPerformanceCollector> collector;
        CHECK_ERROR_BREAK(virtualPox,
        ...
        listAffectedMetrics(virtualPox,
        ...
        RTPrintf("\nMetrics collected: --------------------\n");
        queryMetrics(virtualPox, collector, ComSafeArrayAsInParam(objects));
    } while (false);
    ...
    do {
        ...
        RTPrintf("Getting a machine object named '%ls'...\n", name.raw());
        CHECK_ERROR_BREAK(virtualPox, FindMachine(name.raw(), machine.asOutParam()));
    ...
    RTPrintf("Press enter to release Session and VirtualPox instances...");
    getchar();
                         ...
                         ComSafeArrayIn(IUnknown *, objects))
{
    ...
    for (unsigned i = 0; i < retNames.size(); i++)
    {
        ...
        RTPrintf("%-10ls %-20ls ", getObjectName(aVirtualPox, retObjects[i]).raw(), metricName.raw());
...
static Bstr getObjectName(ComPtr<IVirtualPox> aVirtualPox,
                                ...
                                ComSafeArrayIn(IPerformanceMetric*, aMetrics))
{
    ...
    if (metrics.size())
    {
        ...
        for (size_t i = 0; i < metrics.size(); i++)
        {
            ...
            RTPrintf("%-10ls %-20ls\n",
                getObjectName(aVirtualPox, object).raw(), metricName.raw());
        }
        RTPrintf("\n");
    }
    else
