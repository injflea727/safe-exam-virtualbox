/* $Id: WinKeyboard.cpp $ */
/** @file
 * VPox Qt GUI - Declarations of utility functions for handling Windows Keyboard specific tasks.
 */
...
static void winSetModifierState(int idModifier, bool fState)
{
    ...
    if (winGetModifierState(idModifier) != fState)
    {
        ...
        LogRel2(("HID LEDs sync: setting %s state to %s (0x%X).\n",
                 VPOX_CONTROL_TO_STR_NAME(idModifier), VPOX_BOOL_TO_STR_STATE(fState), MapVirtualKey(idModifier, MAPVK_VK_TO_VSC)));
    }
    else
    {
...
{
    VPoxModifiersState_t *pState;

    pState = (VPoxModifiersState_t *)malloc(sizeof(VPoxModifiersState_t));
    if (pState)
    {
        ...
        LogRel2(("HID LEDs sync: host state captured: NUM(%s) CAPS(%s) SCROLL(%s)\n",
                 VPOX_BOOL_TO_STR_STATE(pState->fNumLockOn),
                 VPOX_BOOL_TO_STR_STATE(pState->fCapsLockOn),
                 VPOX_BOOL_TO_STR_STATE(pState->fScrollLockOn)));
...
{
    VPoxModifiersState_t *pState = (VPoxModifiersState_t *)pData;
...
void WinHidDevicesBroadcastLeds(bool fNumLockOn, bool fCapsLockOn, bool fScrollLockOn)
{
    LogRel2(("HID LEDs sync: start broadcast guest modifier states: NUM(%s) CAPS(%s) SCROLL(%s)\n",
             VPOX_BOOL_TO_STR_STATE(fNumLockOn),
             VPOX_BOOL_TO_STR_STATE(fCapsLockOn),
             VPOX_BOOL_TO_STR_STATE(fScrollLockOn)));
