...
/** @file
 * VPox Qt GUI - UIVirtualMachineItem class implementation.
 */
...
void UIVirtualMachineItemLocal::recache()
{
    ...
    if (m_fAccessible)
    {
        ...
        m_comAccessError = CVirtualPoxErrorInfo();
        ...
        m_fHasDetails = gEDataManager->showMachineInVirtualPoxManagerDetails(m_strId);
    }
    else
    {
        ...
        m_strName = UICommon::hasAllowedExtension(fi.completeSuffix(), VPoxFileExts)
                  ? fi.completeBaseName()
                  : fi.fileName();
