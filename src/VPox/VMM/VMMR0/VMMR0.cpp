 ...
 * executed. */
#if (defined(VPOX_STRICT) || 1) && !defined(VPOX_WITH_RAM_IN_KERNEL)
 ...
 * The runtime lives here (in VMMR0.r0) and VPoxDD*R0.r0 links against us. */
PFNRT g_VMMR0Deps[] =
{
    (PFNRT)RTCrc32,
    (PFNRT)RTOnce,
...
extern "C" { char _depends_on[] = "vpoxdrv"; }
#endif
...
#ifdef VPOX_WITH_64ON32_CMOS_DEBUG
    /*
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            if (RT_SUCCESS(rc))
                            {
                                ...
                                if (RT_SUCCESS(rc))
                                {
                                    ...
                                    if (RT_SUCCESS(rc))
                                    {
#ifdef VPOX_WITH_TRIPLE_FAULT_HACK
                                        VMM_CHECK_SMAP_CHECK(RT_NOTHING);
        ...
        case VMMR0_DO_HM_RUN:
        {
            for (;;) /* hlt loop */
            {
                              ...
                              && SUPIsTscDeltaAvailableForCpuSetIndex(iHostCpuSet)))
                {
                    ...
                    VPOXVMM_R0_VMM_RETURN_TO_RING3_HM(pGVCpu, CPUMQueryGuestCtxPtr(pGVCpu), rc);
#ifdef VPOX_WITH_STATISTICS
...
#  endif
             ...
             * Fire dtrace probe and collect statistics.
             */
            VPOXVMM_R0_VMM_RETURN_TO_RING3_NEM(pGVCpu, CPUMQueryGuestCtxPtr(pGVCpu), rc);
#  ifdef VPOX_WITH_STATISTICS
            vmmR0RecordRC(pGVM, pGVCpu, rc);
#  endif
            break;
...
#if 0 //def VPOX_WITH_PCI_PASSTHROUGH
        /*
...
/**
 * Override RTLogRelGetDefaultInstanceEx so we can do LogRel to VPox.log from EMTs in ring-0.
 */
