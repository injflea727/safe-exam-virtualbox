...
## @file
# VirtualPox X Server auto-start service unit test.
#
...
VPOX_FOLDER=$(cd "${OUR_FOLDER}/.." && pwd)
[ -d "${VPOX_FOLDER}" ] ||
  abort "Failed to change to directory ${VPOX_FOLDER}.\n"
cd "${VPOX_FOLDER}"
...
scripts/VPoxHeadlessXOrg.sh -c "${TEST_FOLDER}conf" &
PID=$!
...
touch "${XORG_FOLDER}/xorg.conf.2"
touch "${XORG_FOLDER}/xorg.conf.4"
if scripts/VPoxHeadlessXOrg.sh -c "${TEST_FOLDER}conf"; then
