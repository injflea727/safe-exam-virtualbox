...
/** @file
 * VPox storage devices - Virtio SCSI Driver
 *
 * Log-levels used:
 ...
 * VPox VirtIO core issues callback to this VirtIO device-specific implementation to handle
...
DECLINLINE(const char *) virtioGetReqRespText(uint32_t vpoxRc)
{
    switch (vpoxRc)
    {
                                                 ...
                                                 void *pvIoReqAlloc, int rcReq)
{
     ...
     * The APIs (virtio and VPox). Windows C-compiler complains otherwise */
 ...
 *
 * @returns VPox status code (logged by caller).
 */
 ...
 *
 * @returns VPox status code (ignored by the caller).
 * @param   pDevIns     The device instance.
 * @param   pThis       VirtIO SCSI shared instance data.
 * @param   pThisCC     VirtIO SCSI ring-3 instance data.
