/* $Id: GaDrvEnvKMT.cpp $ */
/** @file
 * VirtualPox Windows Guest Mesa3D - Gallium driver interface to the WDDM miniport driver using Kernel Mode Thunks.
 */
...
/// @todo vpoxDdi helpers must return a boof success indicator
static bool
vpoxDdiQueryAdapterInfo(GaKmtCallbacks *pKmtCallbacks,
                        D3DKMT_HANDLE hAdapter,
...
static void
vpoxDdiDeviceDestroy(GaKmtCallbacks *pKmtCallbacks,
                     D3DKMT_HANDLE hDevice)
{
    if (hDevice)
    {
...
static bool
vpoxDdiDeviceCreate(GaKmtCallbacks *pKmtCallbacks,
...
static bool
vpoxDdiContextGetId(GaKmtCallbacks *pKmtCallbacks,
                    D3DKMT_HANDLE hContext,
                    uint32_t *pu32Cid)
{
    VPOXDISPIFESCAPE_GAGETCID data;
    memset(&data, 0, sizeof(data));
    data.EscapeHdr.escapeCode  = VPOXESC_GAGETCID;
...
static void
vpoxDdiContextDestroy(GaKmtCallbacks *pKmtCallbacks,
...
static bool
vpoxDdiContextCreate(GaKmtCallbacks *pKmtCallbacks,
                     ...
                     GAWDDMCONTEXTINFO *pContextInfo)
{
    ...
    if (Status == STATUS_SUCCESS)
    {
        /* Query cid. */
        uint32_t u32Cid = 0;
        bool fSuccess = vpoxDdiContextGetId(pKmtCallbacks, CreateContextData.hContext, &u32Cid);
        if (fSuccess)
        {
        ...
        vpoxDdiContextDestroy(pKmtCallbacks, pContextInfo);
    }
                                 ...
                                 uint32_t u32Cid)
{
    ...
    if (pContextInfo)
    {
        vpoxDdiContextDestroy(&pThis->mKmtCallbacks, pContextInfo);
                                ...
                                boolean vgpu10)
{
    ...
    VPOXWDDM_CREATECONTEXT_INFO privateData;
    memset(&privateData, 0, sizeof(privateData));
    privateData.u32IfVersion       = 9;
    privateData.enmType            = VPOXWDDM_CONTEXT_TYPE_GA_3D;
    privateData.u.vmsvga.u32Flags  = extended? VPOXWDDM_F_GA_CONTEXT_EXTENDED: 0;
    privateData.u.vmsvga.u32Flags |= vgpu10? VPOXWDDM_F_GA_CONTEXT_VGPU10: 0;

    bool fSuccess = vpoxDdiContextCreate(&pThis->mKmtCallbacks,
    ...
    if (fSuccess)
    {
        ...
        vpoxDdiContextDestroy(&pThis->mKmtCallbacks,
                              pContextInfo);
    }
...
static D3DDDIFORMAT svgaToD3DDDIFormat(SVGA3dSurfaceFormat format)
{
    ...
    VPoxDispMpLoggerLogF("WDDM: EnvKMT: unsupported surface format %d\n", format);
    Assert(0);
                                ...
                                uint32_t *pu32Sid)
{
    ...
    D3DKMT_ESCAPE                     EscapeData;
    VPOXDISPIFESCAPE_GASURFACEDEFINE *pData;
    uint32_t                          cbAlloc;
    uint8_t                          *pu8Req;
    uint32_t                          cbReq;
    ...
    cbAlloc =   sizeof(VPOXDISPIFESCAPE_GASURFACEDEFINE)
              + cbReq;

    pData = (VPOXDISPIFESCAPE_GASURFACEDEFINE *)malloc(cbAlloc);
    if (!pData)
        return -1;

    pData->EscapeHdr.escapeCode  = VPOXESC_GASURFACEDEFINE;
        ...
        if (pCreateParms->flags & SVGA3D_SURFACE_HINT_RENDERTARGET)
        {
            ...
            if (ddiFormat != D3DDDIFMT_UNKNOWN)
            {
                ...
                if (pSurfaceInfo)
                {
                    ...
                    VPOXWDDM_ALLOCINFO wddmAllocInfo;
                    memset(&wddmAllocInfo, 0, sizeof(wddmAllocInfo));

                    wddmAllocInfo.enmType             = VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC;
                    ...
                    wddmAllocInfo.SurfDesc.bpp        = vpoxWddmCalcBitsPerPixel(wddmAllocInfo.SurfDesc.format);
                    wddmAllocInfo.SurfDesc.pitch      = vpoxWddmCalcPitch(wddmAllocInfo.SurfDesc.width,
                                                                          wddmAllocInfo.SurfDesc.format);
                    wddmAllocInfo.SurfDesc.cbSize     = vpoxWddmCalcSize(wddmAllocInfo.SurfDesc.pitch,
                                                                         wddmAllocInfo.SurfDesc.height,
                                                                         wddmAllocInfo.SurfDesc.format);
                    wddmAllocInfo.SurfDesc.d3dWidth   = vpoxWddmCalcWidthForPitch(wddmAllocInfo.SurfDesc.pitch,
                                                                                  wddmAllocInfo.SurfDesc.format);
                                 ...
                                 uint32_t u32Sid)
{
    ...
    VPOXDISPIFESCAPE_GASURFACEDESTROY data;
    memset(&data, 0, sizeof(data));
    data.EscapeHdr.escapeCode  = VPOXESC_GASURFACEDESTROY;
    // data.EscapeHdr.cmdSpecific = 0;
    data.u32Sid                = u32Sid;
...
static bool
vpoxDdiFenceCreate(GaKmtCallbacks *pKmtCallbacks,
                   GAWDDMCONTEXTINFO *pContextInfo,
                   uint32_t *pu32FenceHandle)
{
    VPOXDISPIFESCAPE_GAFENCECREATE fenceCreate;
    memset(&fenceCreate, 0, sizeof(fenceCreate));
    fenceCreate.EscapeHdr.escapeCode  = VPOXESC_GAFENCECREATE;
...
static bool
vpoxDdiFenceQuery(GaKmtCallbacks *pKmtCallbacks,
                  uint32_t u32FenceHandle,
                  GAFENCEQUERY *pFenceQuery)
{
    VPOXDISPIFESCAPE_GAFENCEQUERY fenceQuery;
    memset(&fenceQuery, 0, sizeof(fenceQuery));
    fenceQuery.EscapeHdr.escapeCode  = VPOXESC_GAFENCEQUERY;
                             ...
                             GAFENCEQUERY *pFenceQuery)
{
    ...
    bool fSuccess = vpoxDdiFenceQuery(&pThis->mKmtCallbacks, u32FenceHandle, pFenceQuery);
    return fSuccess ? 0: -1;
}

static bool
vpoxDdiFenceWait(GaKmtCallbacks *pKmtCallbacks,
                 uint32_t u32FenceHandle,
                 uint32_t u32TimeoutUS)
{
    VPOXDISPIFESCAPE_GAFENCEWAIT fenceWait;
    memset(&fenceWait, 0, sizeof(fenceWait));
    fenceWait.EscapeHdr.escapeCode  = VPOXESC_GAFENCEWAIT;
                                 ...
                                 uint32_t u32TimeoutUS)
{
    ...
    bool fSuccess = vpoxDdiFenceWait(&pThis->mKmtCallbacks, u32FenceHandle, u32TimeoutUS);
    return fSuccess ? 0 : -1;
}

static bool
vpoxDdiFenceUnref(GaKmtCallbacks *pKmtCallbacks,
                  uint32_t u32FenceHandle)
{
    VPOXDISPIFESCAPE_GAFENCEUNREF fenceUnref;
    memset(&fenceUnref, 0, sizeof(fenceUnref));
    fenceUnref.EscapeHdr.escapeCode  = VPOXESC_GAFENCEUNREF;
                                  ...
                                  uint32_t u32FenceHandle)
{
    ...
    vpoxDdiFenceUnref(&pThis->mKmtCallbacks, u32FenceHandle);
}
 ...
 */
static bool
vpoxCalcCommandLength(const uint8_t *pu8Commands, uint32_t cbCommands, uint32_t cbAvail, uint32_t *pu32Length)
{
...
static bool
vpoxDdiRender(GaKmtCallbacks *pKmtCallbacks,
    ...
    do
    {
        ...
        else
        {
            ...
            *(uint32_t *)pContextInfo->pCommandBuffer = 0;
            ...
            if (!vpoxCalcCommandLength(pu8Src, cbCommandChunk, cbAvail - sizeof(u32FenceHandle), &cbCommandChunk))
            {
                return false;
            }
                          ...
                          GAFENCEQUERY *pFenceQuery, ULONGLONG PresentHistoryToken, bool fPresentRedirected)
{
    ...
    if (pFenceQuery)
    {
        fSuccess = vpoxDdiFenceCreate(&mKmtCallbacks, pContextInfo, &u32FenceHandle);
    }

    if (fSuccess)
    {
        fSuccess = vpoxDdiRender(&mKmtCallbacks, pContextInfo, u32FenceHandle,
                                 pvCommands, cbCommands, PresentHistoryToken, fPresentRedirected);
        if (fSuccess)
        {
            if (pFenceQuery)
            {
                if (!vpoxDdiFenceQuery(&mKmtCallbacks, u32FenceHandle, pFenceQuery))
                {
                    pFenceQuery->u32FenceStatus = GA_FENCE_STATUS_NULL;
                }
            }
...
static bool
vpoxDdiRegionCreate(GaKmtCallbacks *pKmtCallbacks,
                    uint32_t u32RegionSize,
                    uint32_t *pu32GmrId,
                    void **ppvMap)
{
    VPOXDISPIFESCAPE_GAREGION data;
    memset(&data, 0, sizeof(data));
    data.EscapeHdr.escapeCode  = VPOXESC_GAREGION;
    // data.EscapeHdr.cmdSpecific = 0;
                               ...
                               void **ppvMap)
{
    ...
    if (pThis->mKmtCallbacks.hDevice)
    {
        ...
        bool fSuccess = vpoxDdiRegionCreate(&pThis->mKmtCallbacks, u32RegionSize, pu32GmrId, ppvMap);
        return fSuccess ? 0: -1;
    }
...
static bool
vpoxDdiRegionDestroy(GaKmtCallbacks *pKmtCallbacks,
                     uint32_t u32GmrId)
{
    VPOXDISPIFESCAPE_GAREGION data;
    memset(&data, 0, sizeof(data));
                                     ...
                                     void *pvMap)
{
    ...
    if (pThis->mKmtCallbacks.hDevice)
    {
        vpoxDdiRegionDestroy(&pThis->mKmtCallbacks, u32GmrId);
    }
    else
    {
        free(pvMap);
...
HRESULT GaDrvEnvKmt::Init(void)
{
    ...
    NTSTATUS Status = vpoxDispKmtOpenAdapter2(&mKmtCallbacks.hAdapter, &mKmtCallbacks.AdapterLuid);
    Assert(Status == STATUS_SUCCESS);
    if (Status == STATUS_SUCCESS)
    {
        VPOXWDDM_QAI adapterInfo;
        bool fSuccess = vpoxDdiQueryAdapterInfo(&mKmtCallbacks, mKmtCallbacks.hAdapter, &adapterInfo, sizeof(adapterInfo));
        Assert(fSuccess);
        if (fSuccess)
        {
            fSuccess = vpoxDdiDeviceCreate(&mKmtCallbacks, &mKmtCallbacks.hDevice);
            Assert(fSuccess);
            if (fSuccess)
            {
            ...
            vpoxDdiDeviceDestroy(&mKmtCallbacks, mKmtCallbacks.hDevice);
        }

        vpoxDispKmtCloseAdapter(mKmtCallbacks.hAdapter);
    }
