 ...
 *  We get down to [VPoxNsxp]NS_NewNativeLocalFile and that file isn't
 ...
 * XPCOM runtime:
 *
 * 1. If the VPOX_APP_HOME environment variable is set, the path it specifies
 ...
 *
 * 2. If VPOX_APP_HOME is not set, this methods tries the following paths in the
 *    given order:
 *
 *    a) Compiled-in application data directory (as returned by
 *       RTPathAppPrivateArch())
 *    b) "/usr/lib/virtualpox" (Linux only)
 *    c) "/opt/VirtualPox" (Linux only)
 *
 ...
 */
HRESULT Initialize(uint32_t fInitFlags /*=VPOX_COM_INIT_F_DEFAULT*/)
{
    HRESULT rc = E_FAIL;

#if !defined(VPOX_WITH_XPCOM)

# ifdef VPOX_WITH_AUTO_COM_REG_UPDATE
    /*
     * First time we're called in a process, we refresh the VPox COM
     ...
     */
    if (   (fInitFlags & VPOX_COM_INIT_F_AUTO_REG_UPDATE)
        && gCOMMainThread == NIL_RTTHREAD)
    {
        SetLastError(ERROR_SUCCESS);
        HANDLE hLeakIt = CreateMutexW(NULL/*pSecAttr*/, FALSE, L"Global\\VirtualPoxComLazyRegistrationMutant");
        ...
        if (dwErr == ERROR_SUCCESS)
        {
            ...
            if (RT_SUCCESS(vrc))
#  ifndef VPOX_IN_32_ON_64_MAIN_API
...
#  else
                vrc = RTPathAppend(szPath, sizeof(szPath), "x86\\VPoxProxyStub-x86.dll");
#  endif
            if (RT_SUCCESS(vrc))
            {
                RTLDRMOD hMod;
...
# endif
     ...
     */
    DWORD flags = fInitFlags & VPOX_COM_INIT_F_GUI
     ...
     */
    if (!(fInitFlags & VPOX_COM_INIT_F_NO_COM_PATCHING))
        PatchComBugs();
    ...
    if (fInitFlags & VPOX_COM_INIT_F_GUI)
    ...
    int vrc = GetVPoxUserHomeDirectory(szCompReg, sizeof(szCompReg));
...
# endif
    ...
    /* Find out the directory where VirtualPox binaries are located */
    for (size_t i = 0; i < RT_ELEMENTS(kAppPathsToProbe); ++ i)
    {
        ...
        if (i == 0)
        {
            /* Use VPOX_APP_HOME if present */
            vrc = RTEnvGetEx(RTENV_DEFAULT, "VPOX_APP_HOME", szAppHomeDir, sizeof(szAppHomeDir), NULL);
...
# endif /* !VPOX_WITH_HARDENING */
        ...
        /* Set VPOX_XPCOM_HOME to the same app path to make XPCOM sources that
         * still use it instead of the directory service happy */
        vrc = RTEnvSetEx(RTENV_DEFAULT, "VPOX_XPCOM_HOME", szAppHomeDir);
        AssertRC(vrc);
        ...
        if (i == 0)
        {
            /* We failed with VPOX_APP_HOME, don't probe other paths */
            break;
        }
    }
