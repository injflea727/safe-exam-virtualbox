...
/** Pointer to an VPox ISO make image instance. */
...
{
    //{ "vpox-iso-maker", VDTYPE_OPTICAL_DISC }, - clumsy.
...
static int visoOpenWorker(PVISOIMAGE pThis)
{
    ...
    if (RT_SUCCESS(rc))
    {
        if (cbFile <= VISO_MAX_FILE_SIZE)
        {
            static char const s_szCmdPrefix[] = "VPox-Iso-Maker ";
...
/**
 * VPox ISO maker backend.
 */
const VDIMAGEBACKEND g_VPoxIsoMakerBackend =
{
    /* u32Version */
