...
typedef DECLCALLBACK(void) FNVPOXWINELOGBACKDOOR(char* pcszStr);
typedef FNVPOXWINELOGBACKDOOR *PFNVPOXWINELOGBACKDOOR;
static PFNVPOXWINELOGBACKDOOR vpox_log_backdoor = NULL;
...
static DECLCALLBACK(void) vpox_log_backdoor_rt(char* pcszStr)
{
    RTLogPrintf("%s", pcszStr);
}
#else
static DECLCALLBACK(void) vpox_log_backdoor_dispmp(char* pcszStr)
{
    VPoxDispMpLoggerLog(pcszStr);
}
#endif
static void vpox_log_v(const char *pszFormat, va_list args)
{
    if (vpox_log_backdoor)
    {
...
static void debug_init(void)
{
    ...
    if ((wine_debug = getenv("WINEDEBUG")))
    {
        ...
        {
#ifdef VPOX_WITH_WDDM
            int rc = VPoxDispMpLoggerInit();
            if (RT_SUCCESS(rc))
                vpox_log_backdoor = vpox_log_backdoor_dispmp;
//            else
#else
                vpox_log_backdoor = vpox_log_backdoor_rt;
#endif
        }
...
#if !defined(VPOX_WITH_VMSVGA) || defined(RT_OS_WINDOWS)
