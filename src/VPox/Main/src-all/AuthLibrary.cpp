...
int AuthLibLoad(AUTHLIBRARYCONTEXT *pAuthLibCtx, const char *pszLibrary)
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
        {
             ...
             * Try to load new default 'VPoxAuth' instead.
             */
            if (RTStrICmp(pszLibrary, "VRDPAuth") == 0)
            {
                LogRel(("AUTH: Loading external authentication library 'VPoxAuth'\n"));
                rc = RTLdrLoadAppPriv("VPoxAuth", &pAuthLibCtx->hAuthLibrary);
            }
        }
    }
