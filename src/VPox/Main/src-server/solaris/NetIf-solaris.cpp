...
/** @todo Unify this define with VPoxNetFltBow-solaris.c */
...
static void vpoxSolarisAddHostIface(char *pszIface, int Instance, void *pvHostNetworkInterfaceList)
{
    ...
    if (SolarisNICMap.empty())
    {
        ...
        SolarisNICMap.insert(NICPair("vpoxnet", "VirtualPox Host Ethernet"));
        SolarisNICMap.insert(NICPair(VPOXBOW_VNIC_TEMPLATE_NAME, "VirtualPox VNIC Template"));
    ...
    if (Description != "VirtualPox Host Ethernet")
    {
        if (Description != "")
            RTStrPrintf(szNICDesc, sizeof(szNICDesc), "%s - %s", szNICInstance, Description.c_str());
        else if (!strncmp(szNICInstance, RT_STR_TUPLE(VPOXBOW_VNIC_TEMPLATE_NAME)))
        {
            /*
             * We want prefix matching only for "vpoxvnic_template" as it's possible to create "vpoxvnic_template_abcd123",
             * which our Solaris Crossbow NetFilter driver will interpret as a VNIC template.
             */
            Description = SolarisNICMap[VPOXBOW_VNIC_TEMPLATE_NAME];
            RTStrPrintf(szNICDesc, sizeof(szNICDesc), "%s - %s", szNICInstance, Description.c_str());
        }
        else
    ...
    if (strncmp(szNICInstance, RT_STR_TUPLE("vpoxnet")))
...
static boolean_t vpoxSolarisAddLinkHostIface(const char *pszIface, void *pvHostNetworkInterfaceList)
{
    /*
     ...
     * These names originate from VPoxNetFltBow-solaris.c, hardcoded here for now.
     *                                                                           .
     * ASSUMES template name is longer than 'vpoxvnic'.
     */
    if (    strncmp(pszIface, RT_STR_TUPLE(VPOXBOW_VNIC_TEMPLATE_NAME))
        && !strncmp(pszIface, RT_STR_TUPLE("vpoxvnic")))
        return _B_FALSE;
     ...
     * Add the interface.
     */
    vpoxSolarisAddHostIface(szIfaceName, Instance, pvHostNetworkInterfaceList);
...
static bool vpoxSolarisSortNICList(const ComObjPtr<HostNetworkInterface> Iface1, const ComObjPtr<HostNetworkInterface> Iface2)
{
    Bstr Iface1Str;
...
static bool vpoxSolarisSameNIC(const ComObjPtr<HostNetworkInterface> Iface1, const ComObjPtr<HostNetworkInterface> Iface2)
{
    Bstr Iface1Str;
    (*Iface1).COMGETTER(Name) (Iface1Str.asOutParam());
...
static int vpoxSolarisAddPhysHostIface(di_node_t Node, di_minor_t Minor, void *pvHostNetworkInterfaceList)
{
    NOREF(Minor);
    ...
    vpoxSolarisAddHostIface(pszDriverName, Instance, pvHostNetworkInterfaceList);
    return DI_WALK_CONTINUE;
}
...
int NetIfList(std::list <ComObjPtr<HostNetworkInterface> > &list)
{
    ...
    if (Root != DI_NODE_NIL)
    {
        di_walk_minor(Root, DDI_NT_NET, 0 /* flag */, &list, vpoxSolarisAddPhysHostIface);
        di_fini(Root);
    }
     ...
     */
    if (VPoxSolarisLibDlpiFound())
        g_pfnLibDlpiWalk(vpoxSolarisAddLinkHostIface, &list, 0);
                ...
                if (!rc)
                {
                    for (int i = 0; i < cIfaces; i++)
                    {
                        ...
                        if (rc >= 0)
                        {
                            ...
                            vpoxSolarisAddLinkHostIface(pszIface, &list);
                        }
#endif
     ...
     */
    list.sort(vpoxSolarisSortNICList);
    list.unique(vpoxSolarisSameNIC);
