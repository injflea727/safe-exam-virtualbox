...
/** @file
 * VPox Linux-specific Performance Classes implementation.
 */
...
#define VPOXVOLINFO_NAME "VPoxVolInfo"
...
void CollectorLinux::addVolumeDependencies(const char *pcszVolume, DiskList& listDisks)
{
    ...
    int rc = RTPathAppPrivateArch(szVolInfo,
                                  sizeof(szVolInfo) - sizeof("/" VPOXVOLINFO_NAME " ") - strlen(pcszVolume));
    if (RT_FAILURE(rc))
    {
        LogRel(("VolInfo: Failed to get program path, rc=%Rrc\n", rc));
        return;
    }
    strcat(szVolInfo, "/" VPOXVOLINFO_NAME " ");
