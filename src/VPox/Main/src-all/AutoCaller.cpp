/* $Id: AutoCaller.cpp $ */
/** @file
 * VirtualPox object state implementation
 */
...
ObjectState::ObjectState(VirtualPoxBase *aObj) :
 ...
 *
 * @param aObj      |this| pointer of the managed VirtualPoxBase object whose
 *                  init() method is being called.
 * @param aResult   Default initialization result.
 */
AutoInitSpan::AutoInitSpan(VirtualPoxBase *aObj,
...
/**
 * Places the managed VirtualPoxBase object to Ready/Limited state if the
 ...
 */
AutoReinitSpan::AutoReinitSpan(VirtualPoxBase *aObj)
    : mObj(aObj),
      mSucceeded(false),
      mOk(false)
{
...
/**
 * Places the managed VirtualPoxBase object to Ready state if the
 ...
 *       callers of the managed VirtualPoxBase object drops to zero!
 *
 * @param aObj  |this| pointer of the VirtualPoxBase object whose uninit()
 ...
 */
AutoUninitSpan::AutoUninitSpan(VirtualPoxBase *aObj, bool fTry /* = false */)
    : mObj(aObj),
      mInitFailed(false),
      mUninitDone(false),
      mUninitFailed(false)
...
/**
 *  Places the managed VirtualPoxBase object to the NotReady state.
 */
