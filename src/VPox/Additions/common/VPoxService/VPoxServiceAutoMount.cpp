/* $Id: VPoxServiceAutoMount.cpp $ */
/** @file
 * VPoxService - Auto-mounting for Shared Folders, only Linux & Solaris atm.
 */
...
/** @page pg_vgsvc_automount VPoxService - Shared Folder Automounter
 *
 ...
 * thinking is this is better of done from VPoxTray, some one argue that for
...
*********************************************************************************************************************************/
/** @def VPOXSERVICE_AUTOMOUNT_DEFAULT_DIR
 * Default mount directory (unix only).
 */
#ifndef VPOXSERVICE_AUTOMOUNT_DEFAULT_DIR
# define VPOXSERVICE_AUTOMOUNT_DEFAULT_DIR      "/media"
#endif

/** @def VPOXSERVICE_AUTOMOUNT_DEFAULT_PREFIX
 * Default mount prefix (unix only).
 */
#ifndef VPOXSERVICE_AUTOMOUNT_DEFAULT_PREFIX
# define VPOXSERVICE_AUTOMOUNT_DEFAULT_PREFIX   "sf_"
#endif
...
/** @def VPOXSERVICE_AUTOMOUNT_MIQF
 * The drive letter / path mount point flag.  */
#if defined(RT_OS_WINDOWS) || defined(RT_OS_OS2)
# define VPOXSERVICE_AUTOMOUNT_MIQF             SHFL_MIQF_DRIVE_LETTER
#else
# define VPOXSERVICE_AUTOMOUNT_MIQF             SHFL_MIQF_PATH
#endif
...
static char const       g_szTag[] = "VPoxAutomounter";
#elif defined(RT_OS_LINUX)
...
static int vbsvcAutoMountPrepareMountPointOld(const char *pszMountPoint, const char *pszShareName, RTGID gidGroup)
{
    ...
    RTFMODE fMode = RTFS_UNIX_IRWXU | RTFS_UNIX_IRWXG; /* Owner (=root) and the group (=vpoxsf) have full access. */
 ...
 *
 * @returns VPox status code reflecting unmount and mount point preparation
...
static int vbsvcAutoMountSharedFolderOld(const char *pszShareName, const char *pszMountPoint)
{
     ...
     */
    struct group *grp_vpoxsf = getgrnam("vpoxsf");
    if (!grp_vpoxsf)
    {
        VGSvcError("vbsvcAutoMountWorker: Group 'vpoxsf' does not exist\n");
        return VINF_SUCCESS;
    }

    int rc = vbsvcAutoMountPrepareMountPointOld(pszMountPoint, pszShareName, grp_vpoxsf->gr_gid);
    if (RT_SUCCESS(rc))
    {
# ifdef RT_OS_SOLARIS
        int const fFlags = MS_OPTIONSTR;
        char szOptBuf[MAX_MNTOPT_STR] = { '\0', };
        RTStrPrintf(szOptBuf, sizeof(szOptBuf), "uid=0,gid=%d,dmode=0770,fmode=0770,dmask=0000,fmask=0000", grp_vpoxsf->gr_gid);
                                            ...
                                            const char *pszMountDir, const char *pszSharePrefix, uint32_t uClientID)
{
     ...
     *
     * It also happily continues if the 'vpoxsf' group is missing, which is a waste
...
static int vbsvcAutoMountWorkerOld(bool volatile *pfShutdown)
{
        ...
        && cMappings)
    {
        ...
        if (rc == VERR_NOT_FOUND)
            rc = RTStrDupEx(&pszMountDir, VPOXSERVICE_AUTOMOUNT_DEFAULT_DIR);
        if (RT_SUCCESS(rc))
        {
...
static int vbsvcAutomounterQueryMountDirAndPrefix(char *pszDst, size_t cbDst)
{
     ...
     */
    /* Mount directory: */
    const char *pszDir = VPOXSERVICE_AUTOMOUNT_DEFAULT_DIR;
    char       *pszCfgDir;
    ...
    /* Prefix: */
    const char *pszPrefix = VPOXSERVICE_AUTOMOUNT_DEFAULT_PREFIX;
    char *pszCfgPrefix;
    ...
    if (RT_FAILURE(rc))
    {
        rc = RTStrCopy(pszDst, cbDst, VPOXSERVICE_AUTOMOUNT_DEFAULT_DIR "/" VPOXSERVICE_AUTOMOUNT_DEFAULT_PREFIX);
        AssertRC(rc);
    }
...
static int vbsvcAutomounterPopulateTable(PVBSVCAUTOMOUNTERTABLE pMountTable)
{
     ...
     */
    static const char s_szDevicePath[] = "\\Device\\VPoxMiniRdr\\;";
    for (char chDrive = 'Z'; chDrive >= 'A'; chDrive--)
    {
            ...
            && RTUtf16NICmpAscii(wszTargetPath, RT_STR_TUPLE(s_szDevicePath)) == 0)
        {
                ...
                && pwsz[2] == '\\')
            {
                ...
                if (RTUtf16NCmpAscii(&pwsz[3], RT_STR_TUPLE("VPoxSvr\\")) == 0)
                {
                    pwsz += 3 + 8;
                    if (*pwsz != '\\' && *pwsz)
                    {
    ...
    if (pFile)
    {
        ...
        while ((pEntry = getmntent(pFile)) != NULL)
            if (strcmp(pEntry->mnt_type, "vpoxsf") == 0)
...
#elif defined(RT_OS_SOLARIS)
    /*
     * Look thru the system mount table and inspect the vpoxsf mounts.
     */
    ...
    if (pFile)
    {
        ...
        while (getmntent(pFile, &Entry) == 0)
            if (strcmp(Entry.mnt_fstype, "vpoxfs") == 0)
            {
...
# error "PORTME!"
    ...
    if (RT_SUCCESS(rc))
    {
        for (uint32_t i = 0; i < cMappings && RT_SUCCESS(rc); i++)
        {
            ...
            char    *pszName    = NULL;
            char    *pszMntPt   = NULL;
            int rc2 = VbglR3SharedFolderQueryFolderInfo(g_idClientSharedFolders, idRootSrc,  VPOXSERVICE_AUTOMOUNT_MIQF,
...
static int vbsvcAutomounterQueryMountPoint(const char *pszMountPoint, const char *pszName)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            if (RTUtf16ICmpAscii(wszFileSystem, "VPoxSharedFolderFS") == 0)
            {
                char *pszLabel = NULL;
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (RTStrStartsWith(pszMountedName, "VPOX_"))
                        pszMountedName += sizeof("VPOX_") - 1;
                         ...
                         || (RTPATH_IS_SLASH(pszMountPoint[2]) && pszMountPoint[3] == '\0')))
            {
                    ...
                    && RTUtf16NICmpAscii(wszLabel, RT_STR_TUPLE(s_szDevicePath)) == 0)
                {
                    ...
                    if (   (pwsz[0] & ~(RTUTF16)0x20) == (wszFileSystem[0] & ~(RTUTF16)0x20)
                        ...
                        && pwsz[2] == '\\')
                    {
                        if (RTUtf16NICmpAscii(&pwsz[3], RT_STR_TUPLE("VPoxSvr\\")) == 0)
                        {
                            pwsz += 3 + 8;
                            char *pszMountedName = NULL;
                            rc = RTUtf16ToUtf8(pwsz, &pszMountedName);
...
static int vbsvcAutomounterMountIt(PVBSVCAUTOMOUNTERENTRY pEntry)
{
    ...
    int rc = RTUtf16CopyAscii(wszPrefixedName, RT_ELEMENTS(wszPrefixedName), "\\\\VPoxSvr\\");
    AssertRC(rc);
    ...
    NetRsrc.lpProvider      = L"VirtualPox Shared Folders"; /* Only try our provider. */
    ...
    if (cchName + 1 + sizeof(g_szTag) <= sizeof(szzNameAndTag))
    {
        ...
        APIRET rc = DosFSAttach(pEntry->pszActualMountPoint, "VPOXSF", szzNameAndTag, cchName + 1 + sizeof(g_szTag), FS_ATTACH);
        if (rc == NO_ERROR)
        {
            VGSvcVerbose(0, "vbsvcAutomounterMountIt: Successfully mounted '%s' on '%s'\n",
                         pEntry->pszName, pEntry->pszActualMountPoint);
    ...
    struct group *grp_vpoxsf = getgrnam("vpoxsf");
    if (grp_vpoxsf)
        gidMount = grp_vpoxsf->gr_gid;
    else
    {
        VGSvcError("vbsvcAutomounterMountIt: Group 'vpoxsf' does not exist\n");
        gidMount = 0;
    }
...
#  if defined(RT_OS_LINUX)
    ...
    /* Do the mounting. The fallback w/o tag is for the Linux vpoxsf fork
       which lagged a lot behind when it first appeared in 5.6. */
    errno = 0;
    rc = mount(pEntry->pszName, pEntry->pszActualMountPoint, "vpoxsf", fFlags, szOpts);
    if (rc != 0 && errno == EINVAL && RTStrVersionCompare(uts.release, "5.6.0") >= 0)
    {
        ...
        *strstr(szOpts, ",tag=") = '\0';
        ...
        rc = mount(pEntry->pszName, pEntry->pszActualMountPoint, "vpoxsf", fFlags, szOpts);
        if (rc == 0)
            VGSvcVerbose(0, "vbsvcAutomounterMountIt: Running outdated vpoxsf module without support for the 'tag' option?\n");
    }
    if (rc == 0)
    {
        VGSvcVerbose(0, "vbsvcAutomounterMountIt: Successfully mounted '%s' on '%s'\n",
...
#  elif defined(RT_OS_SOLARIS)
     ...
     *
     * The ',VPoxService=auto' option (g_szTag) is ignored by the kernel but helps
    ...
    rc = mount(pEntry->pszName, pEntry->pszActualMountPoint, MS_OPTIONSTR, "vpoxfs",
...
#elif defined(RT_OS_OS2)
        APIRET rcOs2 = DosFSAttach(pszMountPoint, "VPOXSF", NULL, 0, FS_DETACH);
        if (rcOs2 == NO_ERROR)
            return VINF_SUCCESS;
    ...
    for (uint32_t iSrc = 0; iSrc < cMappings; iSrc++)
    {
        ...
        if (iTable < pTable->cEntries)
        {
            ...
            if (pEntry->idRoot == idRootSrc)
            {
                ...
                char    *pszName    = NULL;
                char    *pszMntPt   = NULL;
                rc = VbglR3SharedFolderQueryFolderInfo(g_idClientSharedFolders, idRootSrc, VPOXSERVICE_AUTOMOUNT_MIQF,
                                                       &pszName, &pszMntPt, &fFlags, &uRootIdVer);
                if (RT_FAILURE(rc))
