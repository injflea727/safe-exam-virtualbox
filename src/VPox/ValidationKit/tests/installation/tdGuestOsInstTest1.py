...
"""
VirtualPox Validation Kit - Guest OS installation tests.
"""
...
class InstallTestVm(vpoxtestvms.TestVm):
    ...
    def __init__(self, oSet, sVmName, sKind, sInstallIso, sHdCtrlNm, cGbHdd, fFlags):
        vpoxtestvms.TestVm.__init__(self, sVmName, oSet = oSet, sKind = sKind, sHddControllerType = sHdCtrlNm,
        ...
        if oVM is not None:
            ...
            if oSession is not None:
                ...
                if fRc is True and oHd is not None:
                    ...
                    fRc = fRc and oTestDrv.oVPox.deleteHdByMedium(oHd);
    ...
    def getReconfiguredVm(self, oTestDrv, cCpus, sVirtMode, sParavirtMode = None):
        ...
        #
        (fRc, oVM) = vpoxtestvms.TestVm.getReconfiguredVm(self, oTestDrv, cCpus, sVirtMode, sParavirtMode);
        ...
        if fRc is True:
            ...
            if oSession is not None:
                if self.sHddControllerType == self.ksSataController:
                    fRc = fRc and oSession.setStorageControllerType(vpoxcon.StorageControllerType_IntelAhci,
                    ...
                    fRc = fRc and oSession.setStorageControllerType(vpoxcon.StorageControllerType_LsiLogic,
                                                                    self.sHddControllerType);
                try:
...
class tdGuestOsInstTest1(vpox.TestDriver):
    ...
    def actionConfig(self):
        if not self.importVPoxApi(): # So we can use the constant below.
            return False;
        return self.oTestVmSet.actionConfig(self, eNic0AttachType = vpoxcon.NetworkAttachmentType_NAT);
