...
static int pcbiosRegisterShutdown(PPDMDEVINS pDevIns, PDEVPCBIOS pThis, bool fNewShutdownPort)
{
    ...
    if (fNewShutdownPort)
        pThis->ShutdownPort = VPOX_BIOS_SHUTDOWN_PORT;
    else
        pThis->ShutdownPort = VPOX_BIOS_OLD_SHUTDOWN_PORT;
...
static DECLCALLBACK(int) pcbiosInitComplete(PPDMDEVINS pDevIns)
{
    ...
    /* Bochs/VPox BIOS specific way of specifying memory above 4GB in 64KB units.
...
static DECLCALLBACK(void) pcbiosMemSetup(PPDMDEVINS pDevIns, PDMDEVMEMSETUPCTX enmCtx)
{
    ...
    RTGCPHYS    GCPhys = VPOX_LANBOOT_SEG << 4;
    while (cPages > 0)
    {
...
static DECLCALLBACK(int)  pcbiosConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    rc = FwCommonPlantDMITable(pDevIns, pThis->au8DMIPage, VPOX_DMI_TABLE_SIZE,
    ...
    if (pThis->u8IOAPIC)
    {
        pThis->u32MPTableAddr = VPOX_DMI_TABLE_BASE + VPOX_DMI_TABLE_SIZE;
        FwCommonPlantMpsTable(pDevIns, pThis->au8DMIPage /* aka VPOX_DMI_TABLE_BASE */ + VPOX_DMI_TABLE_SIZE,
                              _4K - VPOX_DMI_TABLE_SIZE, pThis->cCpus);
        LogRel(("PcBios: MPS table at %08x\n", pThis->u32MPTableAddr));
    }

    rc = PDMDevHlpROMRegister(pDevIns, VPOX_DMI_TABLE_BASE, _4K, pThis->au8DMIPage, _4K,
        ...
        && enmMicroarch != kCpumMicroarch_Intel_80286)
    {
        ...
        if (pThis->pszLanBootFile)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    if (cbFileLanBoot <= _64K - (VPOX_LANBOOT_SEG << 4 & 0xffff))
                    {
        ...
        if (pu8LanBootBinary)
        {
            ...
            rc = PDMDevHlpROMRegister(pDevIns, VPOX_LANBOOT_SEG << 4,
                                      RT_MAX(cbLanBootBinary, _64K - (VPOX_LANBOOT_SEG << 4 & 0xffff)),
