 ...
 *          vpoxapi/PlatformMSCOM::interruptWaitEvents(). */
...
NativeEventQueue::NativeEventQueue()
{
    ...
    // situation is when NS_ShutdownXPCOM() is called while the VirtualPox component
    // is still alive (because it is still referenced): eventually, it results in
    // a VirtualPox::uninit() call from where it is already not possible to post
...
{
#ifndef VPOX_WITH_XPCOM
    /* Note! The event == NULL case is duplicated in vpoxapi/PlatformMSCOM::interruptWaitEvents(). */
