...
#ifdef VPOX
nsresult
InitXPCOMVPox_Impl(JNIEnv* env, jobject aVPoxBinDirectory)
{
...
{
#ifdef VPOX
  nsresult rv = InitXPCOMVPox_Impl(env, aMozBinDirectory);
  if (NS_SUCCEEDED(rv))
    return nsnull;
#else
  jobject servMan;
...
#if defined(XP_MACOSX) && !defined(VPOX)
  JAWT awt;
