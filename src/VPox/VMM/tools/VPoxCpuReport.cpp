/* $Id: VPoxCpuReport.cpp $ */
/** @file
 * VPoxCpuReport - Produces the basis for a CPU DB entry.
 */
 ...
 *
 * @returns VPox status code (failure if bad MSR behavior).
 * @param   uMsr                The MSR.
...
static int produceCpuReport(void)
{
    ...
    if (g_pReportOut)
    {
        ...
        vbCpuRepPrintf("/* $" "Id" "$ */\n"
                       ...
                       " * Generated at %s by VPoxCpuReport v%sr%s on %s.%s.\n"
                       " */\n"
                       "\n"
                       "/*\n"
                       " * Copyright (C) 2013-2020 Oracle Corporation\n"
                       ...
                       " */\n"
                       "\n"
                       "#ifndef VPOX_CPUDB_%s_h\n"
                       "#define VPOX_CPUDB_%s_h\n"
...
int main(int argc, char **argv)
{
    ...
    while ((iOpt = RTGetOpt(&State, &ValueUnion)) != 0)
    {
        switch (iOpt)
        {
            ...
            case 'h':
                RTPrintf("Usage: VPoxCpuReport [-m|--msrs-only] [-d|--msrs-dev] [-n|--no-msrs] [-h|--help] [-V|--version] [-o filename.h] [-l debug.log]\n");
