 ...
 * between the host and the guest.  The guest parts live in VPoxClient, VPoxTray
 * and VPoxService depending on the OS, with code shared between host and guest
 * under src/VPox/GuestHost/SharedClipboard/.
 *
 ...
 * VPOX_SHCL_GUEST_FN_MSG_GET / VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT, which waits
 ...
 *
 * The second guest message is VPOX_SHCL_GUEST_FN_REPORT_FORMATS, which tells
 * the host that the guest has new clipboard data available.  The third is
 * VPOX_SHCL_GUEST_FN_DATA_READ, which asks the host to send its clipboard data
 * and waits until it arrives.  The host supports at most one simultaneous
 * VPOX_SHCL_GUEST_FN_DATA_READ call from a guest - if a second call is made
 * before the first has returned, the first will be aborted.
 *
 * The last guest message is VPOX_SHCL_GUEST_FN_DATA_WRITE, which is used to
 ...
 *
 * Since VPox 6.1 a newer protocol (v1) has been established to also support
 * file transfers. This protocol uses a (per-client) message queue instead
 * (see VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT vs. VPOX_SHCL_GUEST_FN_GET_HOST_MSG).
 *
 * To distinguish the old (legacy) or new(er) protocol, the VPOX_SHCL_GUEST_FN_CONNECT
 ...
 * the host request.
 *
 * Also see the protocol changelog at VPoxShClSvc.h.
 *
 *
 * @section sec_uri_intro               Transferring files
 *
 * Since VPox x.x.x transferring files via Shared Clipboard is supported.
 * See the VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS define for supported / enabled
 ...
 * VPoxSVC (Main) is used for this task. A Shared Clipboard client can register,
 ...
 * sub folder "VirtualPox Shared Clipboards/clipboard-<ID>". VPoxSVC does not
 ...
 * (via VPoxTray / VPoxClient) or on the host (host service) to avoid code
 ...
 * attached with VPoxSVC. This is needed because multiple transfers from
...
#define VPOX_SHCL_SAVED_STATE_VER_CURRENT   VPOX_SHCL_SAVED_STATE_VER_6_1RC1
/** Adds the client's POD state and client state flags.
 * @since 6.1 RC1 */
#define VPOX_SHCL_SAVED_STATE_VER_6_1RC1    UINT32_C(0x80000004)
/** First attempt saving state during @bugref{9437} development.
 * @since 6.1 BETA 2   */
#define VPOX_SHCL_SAVED_STATE_VER_6_1B2     UINT32_C(0x80000003)
/** First structured version.
 * @since 3.1 / r53668 */
#define VPOX_SHCL_SAVED_STATE_VER_3_1       UINT32_C(0x80000002)
/** This was just a state memory dump, including pointers and everything.
 * @note This is not supported any more. Sorry.  */
#define VPOX_SHCL_SAVED_STATE_VER_NOT_SUPP  (ARCH_BITS == 64 ? UINT32_C(72) : UINT32_C(48))
/** @} */
...
static uint32_t g_uMode  = VPOX_SHCL_MODE_OFF;
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
/** Global Shared Clipboard (file) transfer mode. */
uint32_t g_fTransferMode = VPOX_SHCL_TRANSFER_MODE_DISABLED;
#endif
...
/** Host feature mask (VPOX_SHCL_HF_0_XXX) for VPOX_SHCL_GUEST_FN_REPORT_FEATURES
 * and VPOX_SHCL_GUEST_FN_QUERY_FEATURES. */
static uint64_t const g_fHostFeatures0 = VPOX_SHCL_HF_0_CONTEXT_ID
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
                                       | VPOX_SHCL_HF_0_TRANSFERS
#endif
                                       ;
...
static int shClSvcModeSet(uint32_t uMode)
{
    ...
    switch (uMode)
    {
        case VPOX_SHCL_MODE_OFF:
            RT_FALL_THROUGH();
        case VPOX_SHCL_MODE_HOST_TO_GUEST:
            RT_FALL_THROUGH();
        case VPOX_SHCL_MODE_GUEST_TO_HOST:
            RT_FALL_THROUGH();
        case VPOX_SHCL_MODE_BIDIRECTIONAL:
        {
            g_uMode = uMode;
 ...
 * @param   idMsg       The message ID (VPOX_SHCL_HOST_MSG_XXX) to use
...
/**
 * Sets the VPOX_SHCL_GUEST_FN_MSG_PEEK_WAIT and VPOX_SHCL_GUEST_FN_MSG_PEEK_NOWAIT
 * return parameters.
 *
 ...
 */
static void shClSvcMsgSetPeekReturn(PSHCLCLIENTMSG pMsg, PVPOXHGCMSVCPARM paDstParms, uint32_t cDstParms)
{
    Assert(cDstParms >= 2);
    ...
    while (i-- > 2)
        switch (pMsg->aParms[i - 2].type)
        {
            ...
            case VPOX_HGCM_SVC_PARM_PTR:   paDstParms[i].u.uint32 = pMsg->aParms[i - 2].u.pointer.size; break;
        }
}

/**
 * Sets the VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT return parameters.
 *
 ...
 */
static int shClSvcMsgSetOldWaitReturn(PSHCLCLIENTMSG pMsg, PVPOXHGCMSVCPARM paDstParms, uint32_t cDstParms)
{
    /*
     * Assert sanity.
     */
 ...
 *
 * @returns VPox status code, though the message is consumed regardless of what
 *          is returned.
...
void shClSvcClientDestroy(PSHCLCLIENT pClient)
{
    ...
    if (pClient->Pending.uType)
    {
        if (pClient->Pending.cParms > 1)
            HGCMSvcSetU32(&pClient->Pending.paParms[0], VPOX_SHCL_HOST_MSG_QUIT);
...
static int shClSvcClientNegogiateChunkSize(PSHCLCLIENT pClient, VPOXHGCMCALLHANDLE hCall,
                                           uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate the request.
     */
    ASSERT_GUEST_RETURN(cParms == VPOX_SHCL_CPARMS_NEGOTIATE_CHUNK_SIZE, VERR_WRONG_PARAMETER_COUNT);
    ...
    uint32_t const cbHostMaxChunkSize = VPOX_SHCL_MAX_CHUNK_SIZE; /** @todo Make this configurable. */
...
/**
 * Implements VPOX_SHCL_GUEST_FN_REPORT_FEATURES.
 *
 * @returns VPox status code.
 ...
 */
static int shClSvcClientReportFeatures(PSHCLCLIENT pClient, VPOXHGCMCALLHANDLE hCall,
                                       uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate the request.
    ...
    ASSERT_GUEST_RETURN(fFeatures1 & VPOX_SHCL_GF_1_MUST_BE_ONE, VERR_INVALID_PARAMETER);
...
/**
 * Implements VPOX_SHCL_GUEST_FN_QUERY_FEATURES.
 *
 * @returns VPox status code.
 * @retval  VINF_HGCM_ASYNC_EXECUTE on success (we complete the message here).
 * @retval  VERR_WRONG_PARAMETER_COUNT
 ...
 * @param   paParms     Array of parameters.
 */
static int shClSvcClientQueryFeatures(VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate the request.
     */
...
/**
 * Implements VPOX_SHCL_GUEST_FN_MSG_PEEK_WAIT and VPOX_SHCL_GUEST_FN_MSG_PEEK_NOWAIT.
 *
 * @returns VPox status code.
 ...
 */
static int shClSvcClientMsgPeek(PSHCLCLIENT pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[], bool fWait)
{
    /*
     * Validate the request.
     */
    ...
    if (idRestoreCheck != 0)
    {
        ...
        if (idRestoreCheck != idRestore)
        {
            ...
            LogFlowFunc(("[Client %RU32] VPOX_SHCL_GUEST_FN_MSG_PEEK_XXX -> VERR_VM_RESTORED (%#RX64 -> %#RX64)\n",
    ...
    if (pFirstMsg)
    {
        ...
        LogFlowFunc(("[Client %RU32] VPOX_SHCL_GUEST_FN_MSG_PEEK_XXX -> VINF_SUCCESS (idMsg=%s (%u), cParms=%u)\n",
    ...
    pClient->Pending.uType   = VPOX_SHCL_GUEST_FN_MSG_PEEK_WAIT;
    LogFlowFunc(("[Client %RU32] Is now in pending mode...\n", pClient->State.uClientID));
    return VINF_HGCM_ASYNC_EXECUTE;
}

/**
 * Implements VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT.
 *
 * @returns VPox status code.
 * @retval  VINF_SUCCESS if a message was pending and is being returned.
 * @retval  VINF_HGCM_ASYNC_EXECUTE if message wait is pending.
 ...
 * @note    Caller takes and leave the client's critical section.
 */
static int shClSvcClientMsgOldGet(PSHCLCLIENT pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate input.
     */
    ASSERT_GUEST_RETURN(cParms == VPOX_SHCL_CPARMS_GET_HOST_MSG_OLD, VERR_WRONG_PARAMETER_COUNT);
    ASSERT_GUEST_RETURN(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE); /* id32Msg */
    ASSERT_GUEST_RETURN(paParms[1].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE); /* f32Formats */
    ...
    else
    {
        ...
        pClient->Pending.cParms  = cParms;
        pClient->Pending.paParms = paParms;
        pClient->Pending.uType   = VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT;
...
/**
 * Implements VPOX_SHCL_GUEST_FN_MSG_GET.
 *
 * @returns VPox status code.
 ...
 */
static int shClSvcClientMsgGet(PSHCLCLIENT pClient, VPOXHGCMCALLHANDLE hCall, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Validate the request.
     */
...
static int shClSvcClientMsgCancel(PSHCLCLIENT pClient, uint32_t cParms)
{
    ...
    if (pClient->Pending.uType != 0)
    {
         ...
         */
        int rcComplete;
        if (pClient->Pending.uType == VPOX_SHCL_GUEST_FN_MSG_PEEK_WAIT)
        {
            Assert(pClient->Pending.cParms >= 2);
            if (pClient->Pending.paParms[0].type == VPOX_HGCM_SVC_PARM_64BIT)
                HGCMSvcSetU64(&pClient->Pending.paParms[0], VPOX_SHCL_HOST_MSG_CANCELED);
            else
                HGCMSvcSetU32(&pClient->Pending.paParms[0], VPOX_SHCL_HOST_MSG_CANCELED);
        ...
        {
            Assert(pClient->Pending.uType == VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT);
...
int shClSvcClientWakeup(PSHCLCLIENT pClient)
{
    ...
    if (pClient->Pending.uType != 0)
    {
        ...
        else if (pClient->Pending.uType == VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT) /* Legacy, Guest Additions < 6.1. */
 ...
 * @param   fFormats            The formats being requested, OR'ed together (VPOX_SHCL_FMT_XXX).
...
int ShClSvcDataReadRequest(PSHCLCLIENT pClient, SHCLFORMATS fFormats, PSHCLEVENTID pidEvent)
{
    ...
    while (fFormats)
    {
        ...
        SHCLFORMAT fFormat;
        if (fFormats & VPOX_SHCL_FMT_UNICODETEXT)
            fFormat = VPOX_SHCL_FMT_UNICODETEXT;
        else if (fFormats & VPOX_SHCL_FMT_BITMAP)
            fFormat = VPOX_SHCL_FMT_BITMAP;
        else if (fFormats & VPOX_SHCL_FMT_HTML)
            fFormat = VPOX_SHCL_FMT_HTML;
        else
        ...
        PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient,
                                              pClient->State.fGuestFeatures0 & VPOX_SHCL_GF_0_CONTEXT_ID
                                              ? VPOX_SHCL_HOST_MSG_READ_DATA_CID : VPOX_SHCL_HOST_MSG_READ_DATA,
                                              2);
        if (pMsg)
        {
            /*
            ...
            if (idEvent != NIL_SHCLEVENTID)
            {
                 ...
                 * Format the message.
                 */
                if (pMsg->idMsg == VPOX_SHCL_HOST_MSG_READ_DATA_CID)
                    HGCMSvcSetU64(&pMsg->aParms[0],
                                  VPOX_SHCL_CONTEXTID_MAKE(pClient->State.uSessionID, pClient->EventSrc.uID, idEvent));
                else
                    HGCMSvcSetU32(&pMsg->aParms[0], VPOX_SHCL_HOST_MSG_READ_DATA);
 ...
 *
 * @returns VPox status code. Returns VERR_NOT_FOUND when related event ID was not found.
...
int ShClSvcDataReadSignal(PSHCLCLIENT pClient, PSHCLCLIENTCMDCTX pCmdCtx, SHCLFORMAT uFormat, void *pvData, uint32_t cbData)
{
    ...
    SHCLEVENTID idEvent;
    if (!(pClient->State.fGuestFeatures0 & VPOX_SHCL_GF_0_CONTEXT_ID)) /* Legacy, Guest Additions < 6.1. */
    {
        /* Older Guest Additions (<= VPox 6.0) did not have any context ID handling, so we ASSUME that the last event registered
         * is the one we want to handle (as this all was a synchronous protocol anyway). */
        idEvent = ShClEventGetLast(&pClient->EventSrc);
    }
    else
        idEvent = VPOX_SHCL_CONTEXTID_GET_EVENT(pCmdCtx->uContextID);
...
/**
 * Reports available VPox clipboard formats to the guest.
 *
 ...
 * @returns VPox status code.
 * @param   pClient             Client to request to read data form.
 * @param   fFormats            The formats to report (VPOX_SHCL_FMT_XXX), zero
...
int ShClSvcHostReportFormats(PSHCLCLIENT pClient, SHCLFORMATS fFormats)
{
     ...
     */
    if (   uMode == VPOX_SHCL_MODE_BIDIRECTIONAL
        || uMode == VPOX_SHCL_MODE_HOST_TO_GUEST)
    { /* likely */ }
    else
        return VINF_SUCCESS;
     ...
     */
    if (!(g_fTransferMode & VPOX_SHCL_TRANSFER_MODE_ENABLED))
    {
        LogFlowFunc(("fFormats=%#x -> %#x\n", fFormats, fFormats & ~VPOX_SHCL_FMT_URI_LIST));
        fFormats &= ~VPOX_SHCL_FMT_URI_LIST;
    }
#endif
     ...
     */
    int rc;
    PSHCLCLIENTMSG pMsg = shClSvcMsgAlloc(pClient, VPOX_SHCL_HOST_MSG_FORMATS_REPORT, 2);
    if (pMsg)
    {
        HGCMSvcSetU32(&pMsg->aParms[0], VPOX_SHCL_HOST_MSG_FORMATS_REPORT);
...
/**
 * Handles the VPOX_SHCL_GUEST_FN_REPORT_FORMATS message from the guest.
 */
static int shClSvcClientReportFormats(PSHCLCLIENT pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    /*
     * Check if the service mode allows this operation and whether the guest is
    ...
    uint32_t uMode = ShClSvcGetMode();
    if (   uMode == VPOX_SHCL_MODE_BIDIRECTIONAL
        || uMode == VPOX_SHCL_MODE_GUEST_TO_HOST)
    { /* likely */ }
    else
     ...
     * Digest parameters.
     */
    ASSERT_GUEST_RETURN(   cParms == VPOX_SHCL_CPARMS_REPORT_FORMATS
                        || (   cParms == VPOX_SHCL_CPARMS_REPORT_FORMATS_61B
                            && (pClient->State.fGuestFeatures0 & VPOX_SHCL_GF_0_CONTEXT_ID)),
                        VERR_WRONG_PARAMETER_COUNT);

    uintptr_t iParm = 0;
    if (cParms == VPOX_SHCL_CPARMS_REPORT_FORMATS_61B)
    {
        ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_64BIT, VERR_WRONG_PARAMETER_TYPE);
        /* no defined value, so just ignore it */
        iParm++;
    }
    ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE);
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                if (g_ExtState.pfnExtension)
                {
                    ...
                    g_ExtState.pfnExtension(g_ExtState.pvExtension, VPOX_CLIPBOARD_EXT_FN_FORMAT_ANNOUNCE, &parms, sizeof(parms));
                }
                else
                {
#ifdef LOG_ENABLED
...
/**
 * Handles the VPOX_SHCL_GUEST_FN_DATA_READ message from the guest.
 */
static int shClSvcClientReadData(PSHCLCLIENT pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    LogFlowFuncEnter();
     ...
     */
    ASSERT_GUEST_RETURN(   cParms == VPOX_SHCL_CPARMS_DATA_READ
                        || (    cParms == VPOX_SHCL_CPARMS_DATA_READ_61B
                            &&  (pClient->State.fGuestFeatures0 & VPOX_SHCL_GF_0_CONTEXT_ID)),
                        VERR_WRONG_PARAMETER_COUNT);
    ...
    RT_ZERO(cmdCtx);
    if (cParms == VPOX_SHCL_CPARMS_DATA_READ_61B)
    {
        ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_64BIT, VERR_WRONG_PARAMETER_TYPE);
    ...
    SHCLFORMAT  uFormat = VPOX_SHCL_FMT_NONE;
    uint32_t    cbData  = 0;
    void       *pvData  = NULL;
    ...
    if (cParms != VPOX_SHCL_CPARMS_DATA_READ_61B)
    {
        ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_PTR, VERR_WRONG_PARAMETER_TYPE); /* Data buffer */
        pvData = paParms[iParm].u.pointer.addr;
        cbData = paParms[iParm].u.pointer.size;
        iParm++;
        ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE); /*cbDataReturned*/
        iParm++;
    }
    else
    {
    ...
    if (!(pClient->State.fGuestFeatures0 & VPOX_SHCL_GF_0_CONTEXT_ID))
    {
        if (pClient->State.POD.uFormat == VPOX_SHCL_FMT_NONE)
    ...
    if (g_ExtState.pfnExtension)
    {
        ...
        rc = g_ExtState.pfnExtension(g_ExtState.pvExtension, VPOX_CLIPBOARD_EXT_FN_DATA_READ, &parms, sizeof(parms));
...
int shClSvcClientWriteData(PSHCLCLIENT pClient, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    LogFlowFuncEnter();
     ...
     */
    ASSERT_GUEST_RETURN(pClient->State.fGuestFeatures0 & VPOX_SHCL_GF_0_CONTEXT_ID
                        ? cParms == VPOX_SHCL_CPARMS_DATA_WRITE || cParms == VPOX_SHCL_CPARMS_DATA_WRITE_61B
                        : cParms == VPOX_SHCL_CPARMS_DATA_WRITE_OLD,
                        VERR_WRONG_PARAMETER_COUNT);

    uintptr_t iParm = 0;
    SHCLCLIENTCMDCTX cmdCtx;
    RT_ZERO(cmdCtx);
    if (cParms > VPOX_SHCL_CPARMS_DATA_WRITE_OLD)
    {
        ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_64BIT, VERR_WRONG_PARAMETER_TYPE);
        cmdCtx.uContextID = paParms[iParm].u.uint64;
        uint64_t const idCtxExpected = VPOX_SHCL_CONTEXTID_MAKE(pClient->State.uSessionID, pClient->EventSrc.uID,
                                                                VPOX_SHCL_CONTEXTID_GET_EVENT(cmdCtx.uContextID));
    ...
    if (cParms == VPOX_SHCL_CPARMS_DATA_WRITE_61B)
    {
        ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE);
        ASSERT_GUEST_RETURN(paParms[iParm].u.uint32 == 0, VERR_INVALID_FLAGS);
        iParm++;
    ...
    ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE); /* Format bit. */
    uFormat = paParms[iParm].u.uint32;
    iParm++;
    if (cParms == VPOX_SHCL_CPARMS_DATA_WRITE_61B)
    {
        ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_32BIT, VERR_WRONG_PARAMETER_TYPE); /* "cbData" - duplicates buffer size. */
        iParm++;
    }
    ASSERT_GUEST_RETURN(paParms[iParm].type == VPOX_HGCM_SVC_PARM_PTR, VERR_WRONG_PARAMETER_TYPE); /* Data buffer */
    pvData = paParms[iParm].u.pointer.addr;
    ...
    if (g_ExtState.pfnExtension)
    {
        ...
        g_ExtState.pfnExtension(g_ExtState.pvExtension, VPOX_CLIPBOARD_EXT_FN_DATA_WRITE, &parms, sizeof(parms));
        rc = VINF_SUCCESS;
    }
    else
 ...
 */
static int shClSvcClientError(uint32_t cParms, VPOXHGCMSVCPARM paParms[], int *pRc)
{
    ...
    if (cParms == VPOX_SHCL_CPARMS_ERROR)
    {
...
static int svcInit(VPOXHGCMSVCFNTABLE *pTable)
{
    int rc = RTCritSectInit(&g_CritSect);

    if (RT_SUCCESS(rc))
    {
        shClSvcModeSet(VPOX_SHCL_MODE_OFF);
                                  ...
                                  uint64_t tsArrival)
{
    ...
    switch (u32Function)
    {
        case VPOX_SHCL_GUEST_FN_MSG_OLD_GET_WAIT:
            RTCritSectEnter(&pClient->CritSect);
        ...
        case VPOX_SHCL_GUEST_FN_REPORT_FORMATS:
            rc = shClSvcClientReportFormats(pClient, cParms, paParms);
            break;

        case VPOX_SHCL_GUEST_FN_DATA_READ:
            rc = shClSvcClientReadData(pClient, cParms, paParms);
            break;

        case VPOX_SHCL_GUEST_FN_DATA_WRITE:
        ...
        {
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
            if (   u32Function <= VPOX_SHCL_GUEST_FN_LAST
                && (pClient->State.fGuestFeatures0 &  VPOX_SHCL_GF_0_CONTEXT_ID) )
            {
                if (g_fTransferMode & VPOX_SHCL_TRANSFER_MODE_ENABLED)
...
void shclSvcClientStateReset(PSHCLCLIENTSTATE pClientState)
{
    ...
    pClientState->fGuestFeatures0 = VPOX_SHCL_GF_NONE;
    pClientState->fGuestFeatures1 = VPOX_SHCL_GF_NONE;

    pClientState->cbChunkSize     = VPOX_SHCL_DEFAULT_CHUNK_SIZE; /** @todo Make this configurable. */
    ...
    pClientState->POD.uFormat            = VPOX_SHCL_FMT_NONE;
                                     ...
                                     VPOXHGCMSVCPARM paParms[])
{
    ...
    switch (u32Function)
    {
        case VPOX_SHCL_HOST_FN_SET_MODE:
        {
            if (cParms != 1)
            {
                rc = VERR_INVALID_PARAMETER;
            ...
            {
                uint32_t u32Mode = VPOX_SHCL_MODE_OFF;
...
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
        case VPOX_SHCL_HOST_FN_SET_TRANSFER_MODE:
        {
            if (cParms != 1)
            {
                rc = VERR_INVALID_PARAMETER;
...
#endif
        case VPOX_SHCL_HOST_FN_SET_HEADLESS:
        {
            if (cParms != 1)
            {
                rc = VERR_INVALID_PARAMETER;
...
/**
 * VPox 6.1 Beta 1 version of s_aShClSSMClientState (no flags).
 */
...
/**
 * SSM descriptor table for what used to be the VPOXSHCLMSGCTX structure but is
...
static DECLCALLBACK(int) svcSaveState(void *, uint32_t u32ClientID, void *pvClient, PSSMHANDLE pSSM)
{
    ...
    SSMR3PutU32(pSSM, VPOX_SHCL_SAVED_STATE_VER_CURRENT);
...
static DECLCALLBACK(int) svcLoadState(void *, uint32_t u32ClientID, void *pvClient, PSSMHANDLE pSSM, uint32_t uVersion)
{
    ...
    if (lenOrVer == VPOX_SHCL_SAVED_STATE_VER_3_1)
        return svcLoadStateV0(u32ClientID, pvClient, pSSM, uVersion);

    if (   lenOrVer >= VPOX_SHCL_SAVED_STATE_VER_6_1B2
        && lenOrVer <= VPOX_SHCL_SAVED_STATE_VER_CURRENT)
    {
        if (lenOrVer >= VPOX_SHCL_SAVED_STATE_VER_6_1RC1)
        {
        ...
        for (uint64_t i = 0; i < cMsgs; i++)
        {
            ...
            {
                SHCLCLIENTMSG Msg;
                uint8_t abPadding[RT_UOFFSETOF(SHCLCLIENTMSG, aParms) + sizeof(VPOXHGCMSVCPARM) * 2];
            } u;
...
static DECLCALLBACK(int) extCallback(uint32_t u32Function, uint32_t u32Format, void *pvData, uint32_t cbData)
{
    ...
    if (itClient != g_mapClients.end())
    {
        ...
        switch (u32Function)
        {
            ...
            {
                LogFlowFunc(("VPOX_CLIPBOARD_EXT_FN_FORMAT_ANNOUNCE: g_ExtState.fReadingData=%RTbool\n", g_ExtState.fReadingData));
...
static DECLCALLBACK(int) svcRegisterExtension(void *, PFNHGCMSVCEXT pfnExtension, void *pvExtension)
{
    ...
    if (pfnExtension)
    {
        ...
        g_ExtState.pfnExtension(g_ExtState.pvExtension, VPOX_CLIPBOARD_EXT_FN_SET_CALLBACK, &parms, sizeof(parms));
