...
/** @file
 * VirtualPox Appliance private data definitions
 */
...
struct Appliance::ImportStack
{
    ...
    Utf8Str                         strOsTypeVPox;      // VirtualPox guest OS type as string
...
struct VirtualSystemDescription::Data
{
    ...
    ComPtr<Machine>         pMachine;           // VirtualPox machine this description was exported from (export only)

    settings::MachineConfigFile
                            *pConfig;           // machine config created from <vpox:Machine> element if found (import only)
};
