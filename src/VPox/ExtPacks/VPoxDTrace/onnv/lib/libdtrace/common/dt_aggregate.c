    ...
    int naggvars, dtrace_aggregate_walk_joined_f *func, void *arg)
{
	...
	for (i = 1, start = 0; (size_t/*vpox*/)i <= nentries; i++) {
		if ((size_t/*vpox*/)i < nentries &&
		 ...
		 */
		assert((size_t/*vpox*/)i - start <= (size_t/*vpox*/)naggvars);
	...
	for (i = 0; (size_t/*vpox*/)i < nbundles; i++) {
		for (j = 0; j < naggvars; j++)
			data[j + 1] = NULL;
