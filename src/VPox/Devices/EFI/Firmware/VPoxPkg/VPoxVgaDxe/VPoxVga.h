...
//
// VirtualPox VGA Controller Driver
//
...
#include "VPoxPkg.h"
#include "DevEFI.h"
#include "VPox/Graphics/VPoxVideoVBE.h"
#include "VPox/Graphics/VPoxVideoVBEPrivate.h"
...
//
// VirtualPox VGA Graphical Mode Data
//
typedef struct {
  UINT32  ModeNumber;
...
//
// VirtualPox VGA Private Data Structure
//
#define VPOX_VGA_PRIVATE_DATA_SIGNATURE  SIGNATURE_32 ('V', 'B', 'V', 'D')
...
typedef struct {
  ...
  UINTN                                 CurrentMode;
  UINTN                                 MaxMode;
  VPOX_VGA_MODE_DATA                    *ModeData;
  BOOLEAN                               HardwareNeedsStarting;
  UINT8                                 BarIndexFB;
  UINT16                                DeviceType;
  UINT16                                IOBase;
  UINT32                                VRAMSize;
} VPOX_VGA_PRIVATE_DATA;
...
typedef struct {
  ...
  UINT8   *SeqSettings;
  UINT8   MiscSetting;
} VPOX_VGA_VIDEO_MODES;

#define VPOX_VGA_PRIVATE_DATA_FROM_UGA_DRAW_THIS(a) \
  CR(a, VPOX_VGA_PRIVATE_DATA, UgaDraw, VPOX_VGA_PRIVATE_DATA_SIGNATURE)

#define VPOX_VGA_PRIVATE_DATA_FROM_GRAPHICS_OUTPUT_THIS(a) \
  CR(a, VPOX_VGA_PRIVATE_DATA, GraphicsOutput, VPOX_VGA_PRIVATE_DATA_SIGNATURE)
...
extern VPOX_VGA_VIDEO_MODES                       VPoxVgaVideoModes[];
extern const UINT32                               VPoxVgaVideoModeCount;
extern EFI_DRIVER_BINDING_PROTOCOL                gVPoxVgaDriverBinding;
extern EFI_COMPONENT_NAME_PROTOCOL                gVPoxVgaComponentName;
extern EFI_COMPONENT_NAME2_PROTOCOL               gVPoxVgaComponentName2;
extern EFI_DRIVER_SUPPORTED_EFI_VERSION_PROTOCOL  gVPoxVgaDriverSupportedEfiVersion;
...
//
EFI_STATUS
VPoxVgaUgaDrawConstructor (
  VPOX_VGA_PRIVATE_DATA  *Private
  );

EFI_STATUS
VPoxVgaUgaDrawDestructor (
  VPOX_VGA_PRIVATE_DATA  *Private
  );
...
//
EFI_STATUS
VPoxVgaGraphicsOutputConstructor (
  VPOX_VGA_PRIVATE_DATA  *Private
  );

EFI_STATUS
VPoxVgaGraphicsOutputDestructor (
  VPOX_VGA_PRIVATE_DATA  *Private
  );
...
EFI_STATUS
EFIAPI
VPoxVgaControllerDriverSupported (
...
EFI_STATUS
EFIAPI
VPoxVgaComponentNameGetDriverName (
...
EFI_STATUS
EFIAPI
VPoxVgaComponentNameGetControllerName (
  IN  EFI_COMPONENT_NAME_PROTOCOL                     *This,
...
EFI_STATUS
VPoxVgaVideoModeSetup (
  VPOX_VGA_PRIVATE_DATA  *Private
  );

UINT32 VPoxVgaGetVmVariable(UINT32 Variable, CHAR8* Buffer, UINT32 Size);
