/* $Id: ProgressImpl.cpp $ */
/** @file
 * VirtualPox Progress COM class implementation
 */
...
#if !defined(VPOX_COM_INPROC)
    /* assign (and therefore addref) initiator only if it is not VirtualPox
 ...
 *
 * @param aResultCode   Operation result (error) code, must not be S_OK.
 * @param vrc           VPox status code to associate with the error.
 * @param aIID          IID of the interface that defines the error.
...
HRESULT Progress::getInitiator(ComPtr<IUnknown> &aInitiator)
{
    ...
    {
        ComObjPtr<VirtualPox> pVirtualPox(mParent);
        pVirtualPox.queryInterfaceTo(aInitiator.asOutParam());
    }
#else
    mInitiator.queryInterfaceTo(aInitiator.asOutParam());
#endif
...
HRESULT Progress::getErrorInfo(ComPtr<IVirtualPoxErrorInfo> &aErrorInfo)
{
    AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
...
HRESULT Progress::setTimeout(ULONG aTimeout)
{
    ...
    if (!mCancelable)
        return setError(VPOX_E_INVALID_OBJECT_STATE, tr("Operation cannot be canceled"));
...
HRESULT Progress::waitForCompletion(LONG aTimeout)
{
    ...
    if (!mCompleted)
    {
        ...
        if (RT_FAILURE(vrc) && vrc != VERR_TIMEOUT)
            return setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Failed to wait for the task completion (%Rrc)"), vrc);
    }
 ...
 */
HRESULT Progress::notifyComplete(LONG aResultCode, const ComPtr<IVirtualPoxErrorInfo> &aErrorInfo)
{
