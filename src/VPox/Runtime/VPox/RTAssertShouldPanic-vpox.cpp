/* $Id: RTAssertShouldPanic-vpox.cpp $ */
/** @file
...
/** @def VPOX_RTASSERT_WITH_GDB
 * Enables the 'gdb' VPOX_ASSERT option.
 */
#if defined(DOXYGEN_RUNNING) \
 || (   !defined(VPOX_RTASSERT_WITH_GDB) \
     && !defined(IN_GUEST) \
     && !defined(IN_RT_STATIC) /* valkit too big, sorry */ \
     && !defined(RT_OS_OS2) \
     && !defined(RT_OS_WINDOWS))
# define VPOX_RTASSERT_WITH_GDB
#endif

/** @def VPOX_RTASSERT_WITH_WAIT
 * Enables the 'wait' VPOX_ASSERT option.
 */
#if defined(DOXYGEN_RUNNING) \
 || (   !defined(VPOX_RTASSERT_WITH_WAIT) \
     && defined(IN_RING3) \
     && !defined(RT_OS_OS2) \
     && !defined(RT_OS_WINDOWS))
# define VPOX_RTASSERT_WITH_WAIT
#endif

#ifdef VPOX_RTASSERT_WITH_GDB
...
{
    /*
     * Check for the VPOX_ASSERT variable.
     */
    const char *psz = RTEnvGet("VPOX_ASSERT");
    ...
    if (!strcmp(psz, "gdb"))
    {
        ...
        const char *pszTerm = RTEnvGet("VPOX_ASSERT_TERM");
        if (    !pszTerm
        ...
        /* And find gdb. */
        const char *pszGdb = RTEnvGet("VPOX_ASSERT_GDB");
        if (    !pszGdb
            ||  !RTPathExists(pszGdb))
        {
            pszGdb = "/usr/bin/gdb";
