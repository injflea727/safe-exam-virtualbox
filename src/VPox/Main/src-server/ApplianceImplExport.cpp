                          ...
                          ComPtr<IVirtualSystemDescription> &aDescription)
{
    ...
    try
    {
        ...
        // get guest OS
        Utf8Str strOsTypeVPox = mUserData->s.strOsType;
        // CPU count
        ...
        /* Guest OS type */
        ovf::CIMOSType_T cim = convertVPoxOSType2CIMOSType(strOsTypeVPox.c_str(), fLongMode);
        ...
        // the one VirtualPox IDE controller has two channels with two ports each, which is
...
//     <const name="HardDiskControllerIDE" value="6" />
        if (!pIDEController.isNull())
        {
            ...
            switch (ctlr)
            {
                case StorageControllerType_PIIX3: strVPox = "PIIX3"; break;
                case StorageControllerType_PIIX4: strVPox = "PIIX4"; break;
                case StorageControllerType_ICH6: strVPox = "ICH6"; break;
...
//     <const name="HardDiskControllerSCSI" value="8" />
        if (!pSCSIController.isNull())
        {
            ...
            if (SUCCEEDED(rc))
            {
                Utf8Str strVPox = "LsiLogic";       // the default in VPox
                switch (ctlr)
                {
                    case StorageControllerType_LsiLogic: strVPox = "LsiLogic"; break;
                    case StorageControllerType_BusLogic: strVPox = "BusLogic"; break;
                    default: break; /* Shut up MSC. */
                }
        ...
        {
            // VirtualPox considers the SAS controller a class of its own but in OVF
            // it should be a SCSI controller
            Utf8Str strVPox = "LsiLogicSas";
            lSCSIControllerIndex = (int32_t)pNewDesc->m->maDescriptions.size();
        ...
        if (!pVirtioSCSIController.isNull())
        {
            ...
            if (SUCCEEDED(rc))
            {
                Utf8Str strVPox = "VirtioSCSI";       // the default in VPox
                switch (ctlr)
                {
                    case StorageControllerType_VirtioSCSI: strVPox = "VirtioSCSI"; break;
                    default: break; /* Shut up MSC. */
                }
                ...
                case StorageBus_IDE:
                    ...
                    else if (lChannel == 0 && lDevice == 1) // primary slave
                    {
                        ...
                        lChannelVsys = 1;
                                                            ...
                                                            // as of VirtualPox 3.1 that can change
                    {
            ...
            switch (deviceType)
            {
                case DeviceType_HardDisk:
                    ...
                    pNewDesc->i_addEntry(VirtualSystemDescriptionType_HardDiskImage,
                                         ...
                                         strLocation, // vpox value: media path
                                        (uint32_t)(llSize / _1M),
                                         strExtra);
                    break;
...
HRESULT Appliance::i_writeCloudImpl(const LocationInfo &aLocInfo, ComObjPtr<Progress> &aProgress)
{
         ...
         ++it)
    {
        ...
        while (itSkipped != skipped.end())
        {
            if (strBootLocation.isEmpty())
                strBootLocation = (*itSkipped)->strVPoxCurrent;
            else
                (*itSkipped)->skipIt = true;
            ++itSkipped;
        }
        ...
        if (vsdescThis->i_findByType(VirtualSystemDescriptionType_HardDiskImage).empty())
            return setError(VPOX_E_OBJECT_NOT_FOUND, tr("There are no images to export to Cloud after preparation steps"));
        ...
        if (profileName.size() > 1)
            return setError(VPOX_E_OBJECT_NOT_FOUND, tr("Cloud: More than one profile name was found."));
        if (profileName.empty())
            return setError(VPOX_E_OBJECT_NOT_FOUND, tr("Cloud: Profile name wasn't specified."));

        if (profileName.front()->strVPoxCurrent.isEmpty())
            return setError(VPOX_E_OBJECT_NOT_FOUND, tr("Cloud: Cloud user profile name is empty"));

        LogRel(("profile name: %s\n", profileName.front()->strVPoxCurrent.c_str()));
    }
    ...
    if (SUCCEEDED(hrc))
    {
        if (aLocInfo.strProvider.equals("OCI"))
        {
            ...
            hrc = setErrorVrc(VPOX_E_NOT_SUPPORTED,
                           ...
                           ovf::OVFVersion_T enFormat)
{
    ...
    pelmRoot->setAttribute("xmlns:vpox", "http://www.virtualpox.org/ovf/machine");
    ...
    if (m->virtualSystemDescriptions.size() > 1)
    {
        ...
        pelmToAddVirtualSystemsTo->setAttribute("ovf:name", "ExportedVirtualPoxMachines");      // whatever
    }
    else
         ...
         ++itV)
    {
             ...
             ++itDisk)
        {
            ...
            // source path: where the VPox image is
            const Utf8Str &strSrcFilePath = pDiskEntry->strVPoxCurrent;
            ...
            {
                rc = mVirtualPox->OpenMedium(bstrSrcFilePath.raw(),
                                             DeviceType_HardDisk,
            ...
            // vpox: namespace since it's not part of the standard
            pelmDisk->setAttribute("vpox:uuid", Utf8StrFmt("%RTuuid", guidTarget.raw()).c_str());

            // now, we might have other XML elements from vpox:Machine pointing to this image,
                                              ...
                                              XMLStack &stack)
{
    ...
    if (llName.empty())
        throw setError(VPOX_E_NOT_SUPPORTED, tr("Missing VM name"));
    Utf8Str &strVMName = llName.back()->strVPoxCurrent;
    ...
    bool fProduct    = llProduct.size()    && !llProduct.back()->strVPoxCurrent.isEmpty();
    bool fProductUrl = llProductUrl.size() && !llProductUrl.back()->strVPoxCurrent.isEmpty();
    bool fVendor     = llVendor.size()     && !llVendor.back()->strVPoxCurrent.isEmpty();
    bool fVendorUrl  = llVendorUrl.size()  && !llVendorUrl.back()->strVPoxCurrent.isEmpty();
    bool fVersion    = llVersion.size()    && !llVersion.back()->strVPoxCurrent.isEmpty();
    if (fProduct || fProductUrl || fVendor || fVendorUrl || fVersion)
    {
        ...
        if (fProduct)
            pelmAnnotationSection->createChild("Product")->addContent(llProduct.back()->strVPoxCurrent);
        if (fVendor)
            pelmAnnotationSection->createChild("Vendor")->addContent(llVendor.back()->strVPoxCurrent);
        if (fVersion)
            pelmAnnotationSection->createChild("Version")->addContent(llVersion.back()->strVPoxCurrent);
        if (fProductUrl)
            pelmAnnotationSection->createChild("ProductUrl")->addContent(llProductUrl.back()->strVPoxCurrent);
        if (fVendorUrl)
            pelmAnnotationSection->createChild("VendorUrl")->addContent(llVendorUrl.back()->strVPoxCurrent);
    }
    ...
    if (llDescription.size() &&
        !llDescription.back()->strVPoxCurrent.isEmpty())
    {
        ...
        pelmAnnotationSection->createChild("Annotation")->addContent(llDescription.back()->strVPoxCurrent);
    }

    // license
    std::list<VirtualSystemDescriptionEntry*> llLicense = vsdescThis->i_findByType(VirtualSystemDescriptionType_License);
    if (llLicense.size() &&
        !llLicense.back()->strVPoxCurrent.isEmpty())
    {
        /* <EulaSection>
        ...
        pelmEulaSection->createChild("License")->addContent(llLicense.back()->strVPoxCurrent);
    }

    // operating system
    std::list<VirtualSystemDescriptionEntry*> llOS = vsdescThis->i_findByType(VirtualSystemDescriptionType_OS);
    if (llOS.empty())
        throw setError(VPOX_E_NOT_SUPPORTED, tr("Missing OS type"));
    ...
    Utf8Str strOSDesc;
    convertCIMOSType2VPoxOSType(strOSDesc, (ovf::CIMOSType_T)pvsdeOS->strOvf.toInt32(), "");
    pelmOperatingSystemSection->createChild("Description")->addContent(strOSDesc);
    // add the VirtualPox ostype in a custom tag in a different namespace
    xml::ElementNode *pelmVPoxOSType = pelmOperatingSystemSection->createChild("vpox:OSType");
    pelmVPoxOSType->setAttribute("ovf:required", "false");
    pelmVPoxOSType->addContent(pvsdeOS->strVPoxCurrent);
    ...
    // <vssd:VirtualSystemType>vmx-4</vssd:VirtualSystemType>
    const char *pcszHardware = "virtualpox-2.2";
    if (enFormat == ovf::OVFVersion_0_9)
    ...
    for (size_t uLoop = 1; uLoop <= 2; ++uLoop)
    {
             ...
             ++it, ++lIndexThis)
        {
            ...
            LogFlowFunc(("Loop %u: handling description entry ulIndex=%u, type=%s, strRef=%s, strOvf=%s, strVPox=%s, strExtraConfig=%s\n",
                         uLoop,
                         desc.ulIndex,
            ...
            switch (desc.type)
            {
                case VirtualSystemDescriptionType_CPU:
                    ...
                    if (uLoop == 1)
                    {
                        ...
                        desc.strVPoxCurrent.toInt(uTemp);
                ...
                case VirtualSystemDescriptionType_HardDiskControllerIDE:
                    ...
                    if (uLoop == 1)
                    {
                        ...
                        strResourceSubType = desc.strVPoxCurrent;
                ...
                case VirtualSystemDescriptionType_HardDiskControllerSATA:
                    ...
                    */
                    if (uLoop == 1)
                    {
                        ...
                        if (    desc.strVPoxCurrent.isEmpty()      // AHCI is the default in VirtualPox
                             || (!desc.strVPoxCurrent.compare("ahci", Utf8Str::CaseInsensitive))
                           )
                            strResourceSubType = "AHCI";
                        else
                            throw setError(VPOX_E_NOT_SUPPORTED,
                                            tr("Invalid config string \"%s\" in SATA controller"), desc.strVPoxCurrent.c_str());
                ...
                case VirtualSystemDescriptionType_HardDiskControllerSAS:
                    ...
                    */
                    if (uLoop == 1)
                    {
                        ...
                        if (    desc.strVPoxCurrent.isEmpty()      // LsiLogic is the default in VirtualPox
                             || (!desc.strVPoxCurrent.compare("lsilogic", Utf8Str::CaseInsensitive))
                            )
                            strResourceSubType = "lsilogic";
                        else if (!desc.strVPoxCurrent.compare("buslogic", Utf8Str::CaseInsensitive))
                            strResourceSubType = "buslogic";
                        else if (!desc.strVPoxCurrent.compare("lsilogicsas", Utf8Str::CaseInsensitive))
                ...
                case VirtualSystemDescriptionType_CDROM:
                    ...
                    if (uLoop == 2)
                    {
                        ...
                        //skip empty Medium. There are no information to add into section <References> or <DiskSection>
                        if (desc.strVPoxCurrent.isNotEmpty() &&
                            desc.skipIt == false)
                        {
                            // the following references the "<Disks>" XML block
                            strHostResource = Utf8StrFmt("/disk/%s", strDiskID.c_str());
                ...
                case VirtualSystemDescriptionType_NetworkAdapter:
                    ...
                    if (uLoop == 2)
                    {
                        ...
                        /* Set the hardware type to something useful.
                            ...
                            * E1000 cards. */
                        switch (desc.strVPoxCurrent.toInt32())
                        {
    ...
    // now that we're done with the official OVF <Item> tags under <VirtualSystem>, write out VirtualPox XML
    // under the vpox: namespace
    xml::ElementNode *pelmVPoxMachine = pelmVirtualSystem->createChild("vpox:Machine");
    // ovf:required="false" tells other OVF parsers that they can ignore this thing
    pelmVPoxMachine->setAttribute("ovf:required", "false");
    // ovf:Info element is required or VMware will bail out on the vpox:Machine element
    pelmVPoxMachine->createChild("ovf:Info")->addContent("Complete VirtualPox machine configuration in VirtualPox format");
    ...
    try
    {
        ...
        // write the machine config to the vpox:Machine element
        pConfig->buildMachineXML(*pelmVPoxMachine,
                                   settings::MachineConfigFile::BuildMachineXML_WriteVPoxVersionAttribute
...
HRESULT Appliance::i_writeFS(TaskOVF *pTask)
{
    ...
    AutoMultiWriteLock2 multiLock(&mVirtualPox->i_getMediaTreeLockHandle(), this->lockHandle() COMMA_LOCKVAL_SRC_POS);
...
HRESULT Appliance::i_writeFSOVA(TaskOVF *pTask, AutoWriteLockBase &writeLock)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            RTZipTarFsStreamSetOwner(hVfsFssTar, VPOX_VERSION_MAJOR,
                                       pTask->enFormat == ovf::OVFVersion_0_9 ? "vpoxovf09"
                                     : pTask->enFormat == ovf::OVFVersion_1_0 ? "vpoxovf10"
                                     : pTask->enFormat == ovf::OVFVersion_2_0 ? "vpoxovf20"
                                     :                                          "vpoxovf");
            RTZipTarFsStreamSetGroup(hVfsFssTar, VPOX_VERSION_MINOR,
                                     "vpox_v" RT_XSTR(VPOX_VERSION_MAJOR) "." RT_XSTR(VPOX_VERSION_MINOR) "."
                                     RT_XSTR(VPOX_VERSION_BUILD) "r" RT_XSTR(VPOX_SVN_REV));
...
HRESULT Appliance::i_writeFSOPC(TaskOPC *pTask)
{
         ...
         ++it)
    {
        ...
        const VirtualSystemDescriptionEntry *pDiskEntry = stack.mapDisks[strDiskID];
        const Utf8Str                       &strSrcFilePath = pDiskEntry->strVPoxCurrent;  // where the VPox image is
        ...
        if (pDiskEntry->type == VirtualSystemDescriptionType_HardDiskImage)
            hrc = mVirtualPox->i_findHardDiskByLocation(strSrcFilePath, true /*aSetError*/, &ptrSourceDisk);
        else
            hrc = mVirtualPox->i_findDVDOrFloppyImage(DeviceType_DVD, NULL /*aId*/, strSrcFilePath,
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            {
                RTZipTarFsStreamSetFileMode(hVfsFssTar, 0660, 0440);
                RTZipTarFsStreamSetOwner(hVfsFssTar, VPOX_VERSION_MAJOR, "vpoxopc10");
                RTZipTarFsStreamSetGroup(hVfsFssTar, VPOX_VERSION_MINOR,
                                         "vpox_v" RT_XSTR(VPOX_VERSION_MAJOR) "." RT_XSTR(VPOX_VERSION_MINOR) "."
                                         RT_XSTR(VPOX_VERSION_BUILD) "r" RT_XSTR(VPOX_SVN_REV));
    ...
    try
    {
        ...
        // Scope this to free the memory as soon as this is finished
        {
            ...
            if (RT_UNLIKELY(!pvBuf))
                throw setError(VPOX_E_FILE_ERROR, tr("Could not create OVF file '%s'"), strOvfFile.c_str());
             ...
             ++it)
        {
            ...
            if (pDiskEntry->type == VirtualSystemDescriptionType_HardDiskImage)
            {
                rc = mVirtualPox->i_findHardDiskByLocation(strSrcFilePath, true, &pSourceDisk);
                if (FAILED(rc)) throw rc;
            }
            else//may be CD or DVD
            {
                rc = mVirtualPox->i_findDVDOrFloppyImage(DeviceType_DVD,
                                                         NULL,
                                                         strSrcFilePath,
                                                         true,
                                                         &pSourceDisk);
