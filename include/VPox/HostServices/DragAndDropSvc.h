 ...
 * Protocol changelog:
 *
 *     Protocol v1 (VPox < 5.0, deprecated):
 ...
 *
 *     Protocol v2 (VPox 5.0 - VPox 5.0.8, deprecated):
 *         + Added support for guest to host transfers.
 *         + Added protocol version support through VPOXDNDCONNECTMSG. The host takes the installed
 *           Guest Additions version as indicator which protocol to use for communicating with the guest.
 *           The guest itself uses VPOXDNDCONNECTMSG to report its supported protocol version to the DnD service.
 *
 *     Protocol v3 (VPox 5.0.10 and up, deprecated):
 *         + Added VPOXDNDDISCONNECTMSG for being able to track client disconnects on host side (Main).
 *         + Added context IDs for every HGCM message. Not used yet and must be 0.
 *         + Added VPOXDNDSNDDATAHDR and VPOXDNDCBSNDDATAHDRDATA to support (simple) accounting of objects
 ...
 *         + Enhannced VPOXDNDGHSENDDATAMSG to support (rolling) checksums for the supplied data block.
 *         + VPOXDNDHGSENDDATAMSG and VPOXDNDGHSENDDATAMSG can now contain an optional checksum for the current data block.
 *         | VPOXDNDHGSENDFILEDATAMSG and VPOXDNDGHSENDFILEDATAMSG are now sharing the same HGCM mesasge.
 *         - Removed unused HOST_DND_FN_GH_RECV_DIR, HOST_DND_FN_GH_RECV_FILE_DATA and HOST_DND_FN_GH_RECV_FILE_HDR commands.
 *
 *     VPox 6.1.x and up, current:
 *         + Added GUEST_DND_FN_QUERY_FEATURES + GUEST_DND_FN_REPORT_FEATURES.
 *         - Protocol versioning support in VPOXDNDCONNECTMSG is now marked as being deprecated.
 *
 ** @todo:
 * - Split up messages which use VPOXDNDHGACTIONMSG into own functions and remove parameters which
 *   are not actually needed / used by a function. Why does HOST_DND_FN_HG_EVT_MOVE need all the format stuff, for example?
 */

#ifndef VPOX_INCLUDED_HostServices_DragAndDropSvc_h
#define VPOX_INCLUDED_HostServices_DragAndDropSvc_h
...
#include <VPox/GuestHost/DragAndDropDefs.h>
...
enum eHostFn
{
     ...
     *  Note: Only for protocol version 2 and up (>= VPox 5.0). */
...
enum eGuestFn
{
     ...
     * (VPOX_DND_GF_XXX), the host replies by replacing the parameter values with
     * the host ones (VPOX_DND_HF_XXX).
     *
...
typedef struct HGCMMsgHGSendFileData
{
    ...
    union
    {
        ...
        struct
        {
            /** Note: pvName is now part of the VPOXDNDHGSENDFILEHDRMSG message. */
            /** Note: cbName is now part of the VPOXDNDHGSENDFILEHDRMSG message. */
            /** Context ID. Unused at the moment. */
            HGCMFunctionParameter uContext;     /* OUT uint32_t */
            /** Current data chunk. */
            HGCMFunctionParameter pvData;       /* OUT ptr */
            /** Size (in bytes) of current data chunk. */
            HGCMFunctionParameter cbData;       /* OUT uint32_t */
            /** Note: fMode is now part of the VPOXDNDHGSENDFILEHDRMSG message. */
        } v2;
        struct
        {
            /** Context ID. Unused at the moment. */
...
typedef struct HGCMMsgConnect
{
    ...
    union
    {
        struct
        {
             ...
             *  Deprecated since VPox 6.1.x. Do not use / rely on it anymore. */
...
#define VPOX_DND_CB_MAGIC_MAKE(uFn, uVer) \
...
{
    CB_MAGIC_DND_CONNECT                   = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_CONNECT, 0),
    CB_MAGIC_DND_REPORT_FEATURES           = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_REPORT_FEATURES, 0),
    CB_MAGIC_DND_HG_GET_NEXT_HOST_MSG      = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_GET_NEXT_HOST_MSG, 0),
    CB_MAGIC_DND_HG_ACK_OP                 = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_HG_ACK_OP, 0),
    CB_MAGIC_DND_HG_REQ_DATA               = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_HG_REQ_DATA, 0),
    CB_MAGIC_DND_HG_EVT_PROGRESS           = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_HG_EVT_PROGRESS, 0),
    CB_MAGIC_DND_GH_ACK_PENDING            = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_GH_ACK_PENDING, 0),
    CB_MAGIC_DND_GH_SND_DATA               = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_GH_SND_DATA, 0),
    CB_MAGIC_DND_GH_SND_DATA_HDR           = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_GH_SND_DATA_HDR, 0),
    CB_MAGIC_DND_GH_SND_DIR                = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_GH_SND_DIR, 0),
    CB_MAGIC_DND_GH_SND_FILE_HDR           = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_GH_SND_FILE_HDR, 0),
    CB_MAGIC_DND_GH_SND_FILE_DATA          = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_GH_SND_FILE_DATA, 0),
    CB_MAGIC_DND_GH_EVT_ERROR              = VPOX_DND_CB_MAGIC_MAKE(GUEST_DND_FN_GH_EVT_ERROR, 0)
};

typedef struct VPOXDNDCBHEADERDATA
{
    /** Magic number to identify the structure. */
    uint32_t                    uMagic;
    /** Context ID to identify callback data. */
    uint32_t                    uContextID;
} VPOXDNDCBHEADERDATA, *PVPOXDNDCBHEADERDATA;

typedef struct VPOXDNDCBCONNECTDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    /** Protocol version to use.
     *  Deprecated since VPox 6.1.x. Do not use / rely on it anymore. */
    uint32_t                    uProtocolVersion;
    /** Connection flags; currently unused. */
    uint32_t                    fFlags;
} VPOXDNDCBCONNECTDATA, *PVPOXDNDCBCONNECTDATA;

typedef struct VPOXDNDCBREPORTFEATURESDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    uint32_t                    fGuestFeatures0;
} VPOXDNDCBREPORTFEATURESDATA, *PVPOXDNDCBREPORTFEATURESDATA;

typedef struct VPOXDNDCBDISCONNECTMSGDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
} VPOXDNDCBDISCONNECTMSGDATA, *PVPOXDNDCBDISCONNECTMSGDATA;

typedef struct VPOXDNDCBHGGETNEXTHOSTMSG
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    uint32_t                    uMsg;
    uint32_t                    cParms;
} VPOXDNDCBHGGETNEXTHOSTMSG, *PVPOXDNDCBHGGETNEXTHOSTMSG;

typedef struct VPOXDNDCBHGGETNEXTHOSTMSGDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    uint32_t                    uMsg;
    uint32_t                    cParms;
    PVPOXHGCMSVCPARM            paParms;
} VPOXDNDCBHGGETNEXTHOSTMSGDATA, *PVPOXDNDCBHGGETNEXTHOSTMSGDATA;

typedef struct VPOXDNDCBHGACKOPDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    uint32_t                    uAction;
} VPOXDNDCBHGACKOPDATA, *PVPOXDNDCBHGACKOPDATA;

typedef struct VPOXDNDCBHGREQDATADATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    char                       *pszFormat;
    uint32_t                    cbFormat;
} VPOXDNDCBHGREQDATADATA, *PVPOXDNDCBHGREQDATADATA;

typedef struct VPOXDNDCBHGEVTPROGRESSDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    ...
    uint32_t                    uStatus;
    uint32_t                    rc;
} VPOXDNDCBHGEVTPROGRESSDATA, *PVPOXDNDCBHGEVTPROGRESSDATA;

typedef struct VPOXDNDCBGHACKPENDINGDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    uint32_t                    uDefAction;
    ...
    char                       *pszFormat;
    uint32_t                    cbFormat;
} VPOXDNDCBGHACKPENDINGDATA, *PVPOXDNDCBGHACKPENDINGDATA;

/**
 * Data header.
 * New since protocol v3.
 */
typedef struct VPOXDNDDATAHDR
{
    /** Data transfer flags. Not yet used and must be 0. */
    uint32_t                    uFlags;
    /** Screen ID where the data originates from. */
    ...
    /** Size (in bytes) of checksum. */
    uint32_t                    cbChecksum;
} VPOXDNDDATAHDR, *PVPOXDNDSNDDATAHDR;

/* New since protocol v3. */
typedef struct VPOXDNDCBSNDDATAHDRDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    /** Actual header data. */
    VPOXDNDDATAHDR              data;
} VPOXDNDCBSNDDATAHDRDATA, *PVPOXDNDCBSNDDATAHDRDATA;

typedef struct VPOXDNDSNDDATA
{
    union
    {
        struct
...
} VPOXDNDSNDDATA, *PVPOXDNDSNDDATA;

typedef struct VPOXDNDCBSNDDATADATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    /** Actual data. */
    VPOXDNDSNDDATA              data;
} VPOXDNDCBSNDDATADATA, *PVPOXDNDCBSNDDATADATA;

typedef struct VPOXDNDCBSNDDIRDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    /** Directory path. */
    ...
    uint32_t                    fMode;
} VPOXDNDCBSNDDIRDATA, *PVPOXDNDCBSNDDIRDATA;

/* Note: Only for protocol version 2 and up (>= VPox 5.0). */
typedef struct VPOXDNDCBSNDFILEHDRDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    /** File path (name). */
    ...
    uint32_t                    fFlags;
} VPOXDNDCBSNDFILEHDRDATA, *PVPOXDNDCBSNDFILEHDRDATA;

typedef struct VPOXDNDCBSNDFILEDATADATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    ...
    union
    {
        ...
        /* Protocol v2 + v3: Have the file attributes (name, size, mode, ...)
                             in the VPOXDNDCBSNDFILEHDRDATA structure. */
        struct
        {
...
} VPOXDNDCBSNDFILEDATADATA, *PVPOXDNDCBSNDFILEDATADATA;

typedef struct VPOXDNDCBEVTERRORDATA
{
    /** Callback data header. */
    VPOXDNDCBHEADERDATA         hdr;
    int32_t                     rc;
} VPOXDNDCBEVTERRORDATA, *PVPOXDNDCBEVTERRORDATA;

} /* namespace DragAndDropSvc */

#endif /* !VPOX_INCLUDED_HostServices_DragAndDropSvc_h */
