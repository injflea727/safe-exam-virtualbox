/* $Id: VPoxDispDriverDDraw.cpp $ */
/** @file
 * VPox XPDM Display driver interface functions related to DirectDraw
 */
...
static void VPoxDispGetDDHalInfo(PVPOXDISPDEV pDev, DD_HALINFO *pHalInfo)
{
...
BOOL APIENTRY
VPoxDispDrvGetDirectDrawInfo(DHPDEV dhpdev, DD_HALINFO *pHalInfo, DWORD *pdwNumHeaps,
                             VIDEOMEMORY *pvmList, DWORD *pdwNumFourCCCodes, DWORD *pdwFourCC)
{
    PVPOXDISPDEV pDev = (PVPOXDISPDEV)dhpdev;
    LOGF_ENTER();

    VPoxDispGetDDHalInfo(pDev, pHalInfo);
    ...
    {
        rc = VPoxDispVHWAInitHostInfo1(pDev);
        VPOX_WARNRC_NOBP(rc);
    }

    if (pDev->vhwa.bEnabled)
    {
        rc = VPoxDispVHWAUpdateDDHalInfo(pDev, pHalInfo);
        VPOX_WARNRC(rc);
    ...
    *pdwNumFourCCCodes = 0;
    ...
    if (pDev->vhwa.bEnabled)
    {
        *pdwNumFourCCCodes = pDev->vhwa.numFourCC;
        ...
        {
            rc = VPoxDispVHWAInitHostInfo2(pDev, pdwFourCC);
            VPOX_WARNRC(rc);
        ...
        pHalInfo->GetDriverInfo = VPoxDispDDGetDriverInfo;
...
BOOL APIENTRY
VPoxDispDrvEnableDirectDraw(DHPDEV dhpdev, DD_CALLBACKS *pCallBacks, DD_SURFACECALLBACKS *pSurfaceCallBacks,
                            DD_PALETTECALLBACKS *pPaletteCallBacks)
{
    LOGF_ENTER();

    pCallBacks->dwSize                = sizeof(DD_CALLBACKS);
    pCallBacks->CreateSurface         = VPoxDispDDCreateSurface;
    pCallBacks->CanCreateSurface      = VPoxDispDDCanCreateSurface;
    pCallBacks->MapMemory             = VPoxDispDDMapMemory;
    ...
    pSurfaceCallBacks->Lock             = VPoxDispDDLock;
    pSurfaceCallBacks->Unlock           = VPoxDispDDUnlock;
    ...
    if (pDev->vhwa.bEnabled)
    {
        pSurfaceCallBacks->DestroySurface = VPoxDispDDDestroySurface;
        pSurfaceCallBacks->Flip = VPoxDispDDFlip;
        pSurfaceCallBacks->GetFlipStatus = VPoxDispDDGetFlipStatus;
        pSurfaceCallBacks->Blt = VPoxDispDDBlt;
        pSurfaceCallBacks->GetBltStatus = VPoxDispDDGetBltStatus;
        pSurfaceCallBacks->SetColorKey = VPoxDispDDSetColorKey;
        ...
        if(pDev->vhwa.caps & VPOXVHWA_CAPS_OVERLAY)
        {
            pSurfaceCallBacks->UpdateOverlay = VPoxDispDDUpdateOverlay;
            pSurfaceCallBacks->SetOverlayPosition = VPoxDispDDSetOverlayPosition;
...
{
    PVPOXDISPDEV pDev = (PVPOXDISPDEV)pDirectDraw->dhpdev;
    LOGF_ENTER();
