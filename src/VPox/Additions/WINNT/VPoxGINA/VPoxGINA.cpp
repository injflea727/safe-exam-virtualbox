/* $Id: VPoxGINA.cpp $ */
/** @file
 * VPoxGINA -- Windows Logon DLL for VirtualPox
 */
                    ...
                    LPVOID    pReserved)
{
    ...
    switch (dwReason)
    {
        case DLL_PROCESS_ATTACH:
        {
            ...
            VPoxGINALoadConfiguration();

            VPoxGINAVerbose(0, "VPoxGINA: v%s r%s (%s %s) loaded\n",
        ...
        {
            VPoxGINAVerbose(0, "VPoxGINA: Unloaded\n");
            VbglR3Term();
            /// @todo RTR3Term();
            break;
        }
...
BOOL WINAPI WlxNegotiate(DWORD dwWinlogonVersion,
                         DWORD *pdwDllVersion)
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: dwWinlogonVersion: %ld\n", dwWinlogonVersion);
    ...
    if (RT_FAILURE(rc))
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed loading MSGINA! rc=%Rrc\n", rc);
        return FALSE;
    }
    ...
    if (!GWlxNegotiate)
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxNegotiate\n");
        return FALSE;
    }
    GWlxInitialize = (PGWLXINITIALIZE)RTLdrGetFunction(hLdrMod, "WlxInitialize");
    if (!GWlxInitialize)
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxInitialize\n");
        return FALSE;
    }
    ...
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxDisplaySASNotice\n");
        return FALSE;
    }
    GWlxLoggedOutSAS =
    ...
    if (!GWlxLoggedOutSAS)
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxLoggedOutSAS\n");
        return FALSE;
    }
    ...
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxActivateUserShell\n");
        return FALSE;
    }
    GWlxLoggedOnSAS =
    ...
    if (!GWlxLoggedOnSAS)
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxLoggedOnSAS\n");
        return FALSE;
    }
    ...
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxDisplayLockedNotice\n");
        return FALSE;
    }
    GWlxIsLockOk = (PGWLXISLOCKOK)RTLdrGetFunction(hLdrMod, "WlxIsLockOk");
    if (!GWlxIsLockOk)
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxIsLockOk\n");
        return FALSE;
    }
    GWlxWkstaLockedSAS =
    ...
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxWkstaLockedSAS\n");
        return FALSE;
    }
    GWlxIsLogoffOk = (PGWLXISLOGOFFOK)RTLdrGetFunction(hLdrMod, "WlxIsLogoffOk");
    if (!GWlxIsLogoffOk)
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxIsLogoffOk\n");
        return FALSE;
    }
    GWlxLogoff = (PGWLXLOGOFF)RTLdrGetFunction(hLdrMod, "WlxLogoff");
    if (!GWlxLogoff)
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxLogoff\n");
        return FALSE;
    }
    GWlxShutdown = (PGWLXSHUTDOWN)RTLdrGetFunction(hLdrMod, "WlxShutdown");
    if (!GWlxShutdown)
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: failed resolving WlxShutdown\n");
        return FALSE;
    }
    /* GINA 1.1, optional */
    ...
    VPoxGINAVerbose(0, "VPoxGINA::WlxNegotiate: optional function pointers:\n"
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxInitialize\n");
    ...
    VPoxGINAReportStatus(VPoxGuestFacilityStatus_Init);
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxDisplaySASNotice\n");

    /* Check if there are credentials for us, if so simulate C-A-D */
    int rc = VbglR3CredentialsQueryAvailability();
    if (RT_SUCCESS(rc))
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxDisplaySASNotice: simulating C-A-D\n");
        /* Wutomatic C-A-D */
        pWlxFuncs->WlxSasNotify(hGinaWlx, WLX_SAS_TYPE_CTRL_ALT_DEL);
    }
    else
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxDisplaySASNotice: starting credentials poller\n");
        /* start the credentials poller thread */
        VPoxGINACredentialsPollerCreate();
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxLoggedOutSAS\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxActivateUserShell\n");
     ...
     */
    VPoxGINAReportStatus(VPoxGuestFacilityStatus_Terminated);
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxLoggedOnSAS: dwSasType=%ld\n", dwSasType);

    /*
     * We don't want to do anything special here since the OS should behave
     * as VPoxGINA wouldn't have been installed. So pass all calls down
     * to the original MSGINA.
     */

    /* Forward call to MSGINA. */
    VPoxGINAVerbose(0, "VPoxGINA::WlxLoggedOnSAS: Forwarding call to MSGINA ...\n");
    if (GWlxLoggedOnSAS)
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxDisplayLockedNotice\n");

    /* Check if there are credentials for us, if so simulate C-A-D */
    int rc = VbglR3CredentialsQueryAvailability();
    if (RT_SUCCESS(rc))
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxDisplayLockedNotice: simulating C-A-D\n");
        /* Automatic C-A-D */
        pWlxFuncs->WlxSasNotify(hGinaWlx, WLX_SAS_TYPE_CTRL_ALT_DEL);
    }
    else
    {
        VPoxGINAVerbose(0, "VPoxGINA::WlxDisplayLockedNotice: starting credentials poller\n");
        /* start the credentials poller thread */
        VPoxGINACredentialsPollerCreate();
        /* Forward call to MSGINA. */
        if (GWlxDisplayLockedNotice)
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxIsLockOk\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxWkstaLockedSAS, dwSasType=%ld\n", dwSasType);
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxIsLogoffOk\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxLogoff\n");

    /* No need to report the "active" status to the host here -- this will be done
     * when VPoxGINA gets the chance to hook the dialogs (again). */
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxShutdown\n");
     ...
     */
    VPoxGINAReportStatus(VPoxGuestFacilityStatus_Inactive);
...
BOOL WINAPI WlxScreenSaverNotify(PVOID pWlxContext, BOOL *pSecure)
{
    ...
    VPoxGINAVerbose(0, "VPoxGINA::WlxScreenSaverNotify, pSecure=%d\n",
                    pSecure ? *pSecure : 0);

    /* Report the status to "init" since the screensaver
     * (Winlogon) does not give VPoxGINA yet the chance to hook into dialogs
     ...
     *       VPoxGINA loaded and set the status to "terminated" again properly
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxStartApplication: pWlxCtx=%p, pszDesktopName=%ls, pEnvironment=%p, pszCmdLine=%ls\n",
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxNetworkProviderLoad\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxDisplayStatusMessage\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxGetStatusMessage\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxRemoveStatusMessage\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxGetConsoleSwitchCredentials\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxReconnectNotify\n");
...
{
    VPoxGINAVerbose(0, "VPoxGINA::WlxDisconnectNotify\n");
...
{
    RT_NOREF(pInfo);
    VPoxGINAVerbose(0, "VPoxGINA::WnpScreenSaverStop\n");
...
DWORD WINAPI VPoxGINADebug(void)
{
#ifdef DEBUG
    DWORD dwVersion;
