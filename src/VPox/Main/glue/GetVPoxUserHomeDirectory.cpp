/* $Id: GetVPoxUserHomeDirectory.cpp $ */
/** @file
 * MS COM / XPCOM Abstraction Layer - GetVPoxUserHomeDirectory.
 */
...
/**
 * Possible locations for the VirtualPox user configuration folder,
...
int GetVPoxUserHomeDirectory(char *aDir, size_t aDirLen, bool fCreateDir)
{
    ...
    *aDir = 0;
    ...
    int vrc = RTEnvGetEx(RTENV_DEFAULT, "VPOX_USER_HOME", szTmp, sizeof(szTmp), NULL);
    if (RT_SUCCESS(vrc) || vrc == VERR_ENV_VAR_NOT_FOUND)
    {
        ...
        else
        {
            ...
            if (RT_SUCCESS(vrc))
                vrc = RTPathAppend(g_szXdgConfigHome, sizeof(g_szXdgConfigHome), "VirtualPox");
            AssertMsg(vrc == VINF_SUCCESS || vrc == VERR_ENV_VAR_NOT_FOUND, ("%Rrc\n", vrc));
            if (RT_FAILURE_NP(vrc))
                vrc = RTStrCopy(g_szXdgConfigHome, sizeof(g_szXdgConfigHome), ".config/VirtualPox");
#endif
