/* $Id: VPoxMPVidModes.cpp $ */
/** @file
 * VPox Miniport video modes related functions
 */
...
# define VPOX_WITHOUT_24BPP_MODES
#endif

/* Custom video modes which are being read from registry at driver startup. */
static VIDEO_MODE_INFORMATION g_CustomVideoModes[VPOX_VIDEO_MAX_SCREENS] = { 0 };

static BOOLEAN
VPoxMPValidateVideoModeParamsGuest(PVPOXMP_DEVEXT pExt, uint32_t iDisplay, uint32_t xres, uint32_t yres, uint32_t bpp)
{
    ...
    switch (bpp)
    {
        case 32:
            break;
        case 24:
#ifdef VPOX_WITHOUT_24BPP_MODES
            return FALSE;
#else
            break;
#endif
...
static void
VPoxFillVidModeBPP(VIDEO_MODE_INFORMATION *pMode, ULONG bitsR, ULONG bitsG, ULONG bitsB,
...
static void
VPoxFillVidModeInfo(VIDEO_MODE_INFORMATION *pMode, ULONG xres, ULONG yres, ULONG bpp, ULONG index, ULONG yoffset)
{
    ...
    switch (bpp)
    {
#ifdef VPOX_WITH_8BPP_MODES
        case 8:
            VPoxFillVidModeBPP(pMode, 6, 6, 6, 0, 0, 0);

            pMode->AttributeFlags |= VIDEO_MODE_PALETTE_DRIVEN | VIDEO_MODE_MANAGED_PALETTE;
            break;
#endif
        case 16:
            VPoxFillVidModeBPP(pMode, 5, 6, 5, 0xF800, 0x7E0, 0x1F);
            break;
        case 24:
        case 32:
            VPoxFillVidModeBPP(pMode, 8, 8, 8, 0xFF0000, 0xFF00, 0xFF);
            break;
        default:
            Assert(0);
            break;
...
void VPoxMPCmnInitCustomVideoModes(PVPOXMP_DEVEXT pExt)
{
    ...
    VPoxFillVidModeInfo(&g_CustomVideoModes[0], 800, 600, 32, 0, 0);
    ...
    for (iMode=0; iMode<VPoxCommonFromDeviceExt(pExt)->cDisplays; ++iMode)
    {
        ...
        if (iMode==0)
        {
            ...
            rc = VPoxMPCmnRegQueryDword(Registry, L"CustomXRes", &CustomXRes);
            VPOXMP_WARN_VPS_NOBP(rc);
            rc = VPoxMPCmnRegQueryDword(Registry, L"CustomYRes", &CustomYRes);
            VPOXMP_WARN_VPS_NOBP(rc);
            rc = VPoxMPCmnRegQueryDword(Registry, L"CustomBPP", &CustomBPP);
            VPOXMP_WARN_VPS_NOBP(rc);
        }
        else
        {
            ...
            rc = VPoxMPCmnRegQueryDword(Registry, keyname, &CustomXRes);
            VPOXMP_WARN_VPS_NOBP(rc);
            swprintf(keyname, L"CustomYRes%d", iMode);
            rc = VPoxMPCmnRegQueryDword(Registry, keyname, &CustomYRes);
            VPOXMP_WARN_VPS_NOBP(rc);
            swprintf(keyname, L"CustomBPP%d", iMode);
            rc = VPoxMPCmnRegQueryDword(Registry, keyname, &CustomBPP);
            VPOXMP_WARN_VPS_NOBP(rc);
        }
        ...
        if (CustomXRes || CustomYRes || CustomBPP)
        {
            ...
            if (VPoxMPValidateVideoModeParamsGuest(pExt, iMode, CustomXRes, CustomYRes, CustomBPP))
            {
                VPoxFillVidModeInfo(&g_CustomVideoModes[iMode], CustomXRes, CustomYRes, CustomBPP, 0, 0);
            }
        }
    }
...
static bool VPoxMPVideoModesMatch(const PVIDEO_MODE_INFORMATION pMode1, const PVIDEO_MODE_INFORMATION pMode2)
{
...
static int
VPoxMPFindVideoMode(const PVIDEO_MODE_INFORMATION pModesTable, int cModes, const PVIDEO_MODE_INFORMATION pMode)
{
    for (int i = 0; i < cModes; ++i)
    {
        if (VPoxMPVideoModesMatch(pMode, &pModesTable[i]))
        {
            return i;
        }
    }
 ...
 */
static uint32_t
VPoxMPFillModesTable(PVPOXMP_DEVEXT pExt, int iDisplay, PVIDEO_MODE_INFORMATION pModesTable, size_t tableSize,
                     int32_t *pPrefModeIdx)
{
    ...
    size_t   maxModesPerColorDepth = VPOXMP_MAX_VIDEO_MODES / 2 / 4;
    ...
    for (; bytesPerPixel<=4; bytesPerPixel++)
    {
        ...
        if (!VPoxMPValidateVideoModeParamsGuest(pExt, iMode, 800, 600, bitsPerPixel))
            continue;

        VPoxFillVidModeInfo(&pModesTable[iMode], 800, 600, bitsPerPixel, iMode+1, 0);
    ...
    ULONG yOffset = VPoxGetHeightReduction();
        ...
        for (cAdded=0, resIndex=0; resIndex<RT_ELEMENTS(resolutionMatrix) && cAdded<maxModesPerColorDepth; resIndex++)
        {
            ...
            if (
                     ...
                     * So add 1024x768 resolution for win8 guest to allow user switch to it */
                       (   (VPoxQueryWinVersion(NULL) != WIN8 && VPoxQueryWinVersion(NULL) != WIN81)
                        || resolutionMatrix[resIndex].xRes != 1024
                        || resolutionMatrix[resIndex].yRes != 768)
                    &&
#endif
                       !VPoxLikesVideoMode(iDisplay, resolutionMatrix[resIndex].xRes,
            ...
            if (!VPoxMPValidateVideoModeParamsGuest(pExt, iDisplay, resolutionMatrix[resIndex].xRes, resolutionMatrix[resIndex].yRes, bitsPerPixel))
            {
            ...
            VPoxFillVidModeInfo(&pModesTable[iMode], resolutionMatrix[resIndex].xRes, resolutionMatrix[resIndex].yRes, bitsPerPixel, iMode+1, yOffset);
            ++iMode;
            ++cAdded;
        }
    }
    ...
    for (int curKey=0; curKey<128; curKey++)
    {
        ...
        rc = VPoxMPCmnRegQueryDword(Registry, keyname, &xres);
        VPOXMP_CHECK_VPS_BREAK(rc);

        swprintf(keyname, L"CustomMode%dHeight", curKey);
        rc = VPoxMPCmnRegQueryDword(Registry, keyname, &yres);
        VPOXMP_CHECK_VPS_BREAK(rc);

        swprintf(keyname, L"CustomMode%dBPP", curKey);
        rc = VPoxMPCmnRegQueryDword(Registry, keyname, &bpp);
        VPOXMP_CHECK_VPS_BREAK(rc);
        ...
        if (!VPoxLikesVideoMode(iDisplay, xres, yres, bpp))
        {
            /* host doesn't like this mode */
            break;
        }

        if (!VPoxMPValidateVideoModeParamsGuest(pExt, iDisplay, xres, yres, bpp))
        {
            /* guest does not like this mode */
            continue;
        }
        ...
        VPoxFillVidModeInfo(&pModesTable[iMode], xres, yres, bpp, iMode+1, yOffset);
        ...
        int foundIdx = VPoxMPFindVideoMode(pModesTable, iMode, &pModesTable[iMode]);
...
static BOOLEAN VPoxMPIsStartingUp(PVPOXMP_DEVEXT pExt, uint32_t iDisplay)
{
#ifdef VPOX_XPDM_MINIPORT
    RT_NOREF(iDisplay);
    return (pExt->CurrentMode == 0);
#else
    VPOXWDDM_SOURCE *pSource = &pExt->aSources[iDisplay];
    return !pSource->AllocData.SurfDesc.width || !pSource->AllocData.SurfDesc.height;
#endif
}

#ifdef VPOX_WDDM_MINIPORT
static const uint32_t g_aVPoxVidModesSupportedBpps[] = {
        32
#ifndef VPOX_WITHOUT_24BPP_MODES
        , 24
#endif
        , 16
#ifdef VPOX_WITH_8BPP_MODES
...
DECLINLINE(BOOLEAN) VPoxMPIsSupportedBpp(uint32_t bpp)
{
    for (int i = 0; i < RT_ELEMENTS(g_aVPoxVidModesSupportedBpps); ++i)
    {
        if (bpp == g_aVPoxVidModesSupportedBpps[i])
            return TRUE;
    }
    return FALSE;
}

DECLINLINE(uint32_t) VPoxMPAdjustBpp(uint32_t bpp)
{
    if (VPoxMPIsSupportedBpp(bpp))
        return bpp;
    Assert(g_aVPoxVidModesSupportedBpps[0] == 32);
    return g_aVPoxVidModesSupportedBpps[0];
}
#endif
 ...
 */
static BOOLEAN
VPoxMPValidateVideoModeParams(PVPOXMP_DEVEXT pExt, uint32_t iDisplay, uint32_t &xres, uint32_t &yres, uint32_t &bpp)
{
    /* Make sure all important video mode values are set */
    if (VPoxMPIsStartingUp(pExt, iDisplay))
    {
    ...
    else
    {
...
#else
        PVPOXWDDM_ALLOC_DATA pAllocData = pExt->aSources[iDisplay].pPrimaryAllocation ?
        ...
        /* VPox WDDM driver does not allow 24 modes since OS could choose the 24bit mode as default in that case,
        ...
        bpp  = bpp  ? bpp : VPoxMPAdjustBpp(pAllocData->SurfDesc.bpp);
#endif
    }
 ...
 */
static BOOLEAN
VPoxMPCheckPendingVideoMode(PVPOXMP_DEVEXT pExt, PVIDEO_MODE_INFORMATION pPendingMode)
{
    uint32_t xres=0, yres=0, bpp=0, display=0;

    /* Check if there's a pending display change request for this display */
    if (VPoxQueryDisplayRequest(&xres, &yres, &bpp, &display) && (xres || yres || bpp))
    {
        if (display >= RT_ELEMENTS(g_CustomVideoModes))
        {
            /*display = RT_ELEMENTS(g_CustomVideoModes) - 1;*/
            WARN(("VPoxQueryDisplayRequest returned invalid display number %d", display));
            return FALSE;
        }
    }
    else
    ...
    if (VPoxMPValidateVideoModeParams(pExt, display, xres, yres, bpp))
    {
        VPoxFillVidModeInfo(pPendingMode, xres, yres, bpp, display, 0);
        return TRUE;
    }
...
static void VPoxMPRegSaveModeInfo(PVPOXMP_DEVEXT pExt, uint32_t iDisplay, PVIDEO_MODE_INFORMATION pMode)
{
    VPOXMPCMNREGISTRY Registry;
    VP_STATUS rc;
    ...
    {
        /*First name without a suffix*/
        rc = VPoxMPCmnRegSetDword(Registry, L"CustomXRes", pMode->VisScreenWidth);
        VPOXMP_WARN_VPS(rc);
        rc = VPoxMPCmnRegSetDword(Registry, L"CustomYRes", pMode->VisScreenHeight);
        VPOXMP_WARN_VPS(rc);
        rc = VPoxMPCmnRegSetDword(Registry, L"CustomBPP", pMode->BitsPerPlane);
        VPOXMP_WARN_VPS(rc);
    }
    else
    {
        ...
        rc = VPoxMPCmnRegSetDword(Registry, keyname, pMode->VisScreenWidth);
        VPOXMP_WARN_VPS(rc);
        swprintf(keyname, L"CustomYRes%d", iDisplay);
        rc = VPoxMPCmnRegSetDword(Registry, keyname, pMode->VisScreenHeight);
        VPOXMP_WARN_VPS(rc);
        swprintf(keyname, L"CustomBPP%d", iDisplay);
        rc = VPoxMPCmnRegSetDword(Registry, keyname, pMode->BitsPerPlane);
        VPOXMP_WARN_VPS(rc);
    }
...
{
    return VPoxMPCmnGetVideoModeInfo(pExt, pExt->CurrentMode - 1);
}
...
void VPoxMPXpdmBuildVideoModesTable(PVPOXMP_DEVEXT pExt)
{
     ...
     * Up to VPOXMP_MAX_VIDEO_MODES elements can be used, the rest is reserved
     * for custom mode alternating indexes.
     */
    cStandartModes = VPoxMPFillModesTable(pExt, pExt->iDevice, pExt->aVideoModes, VPOXMP_MAX_VIDEO_MODES, NULL);
    ...
    bPending = VPoxMPCheckPendingVideoMode(pExt, &specialMode);
    ...
    if (!bHaveSpecial && VPoxMPIsStartingUp(pExt, pExt->iDevice))
    {
        ...
        if (VPoxMPValidateVideoModeParams(pExt, pExt->iDevice, xres, yres, bpp))
        {
            LOG(("Startup for screen %d validated %dx%d %d", pExt->iDevice, xres, yres, bpp));
            VPoxFillVidModeInfo(&specialMode, xres, yres, bpp, 0, 0);
            bHaveSpecial = TRUE;
        }
    }
    ...
    if (bHaveSpecial)
    {
        ...
        if (!VPoxMPIsStartingUp(pExt, pExt->iDevice))
        {
            if (bChanged)
            {
        ...
        VPoxMPRegSaveModeInfo(pExt, pExt->iDevice, &specialMode);
    }
