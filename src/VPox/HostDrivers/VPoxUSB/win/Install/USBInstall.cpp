/* $Id: USBInstall.cpp $ */
/** @file
 * VPox host drivers - USB drivers - Filter & driver installation, Installation code.
 */
...
#define SERVICE_NAME    "VPoxUSBMon"
/** Win32 Device name. */
#define DEVICE_NAME     "\\\\.\\VPoxUSBMon"
/** NT Device name. */
#define DEVICE_NAME_NT   L"\\Device\\VPoxUSBMon"
/** Win32 Symlink name. */
#define DEVICE_NAME_DOS  L"\\DosDevices\\VPoxUSBMon"
...
static DECLCALLBACK(void) vpoxUsbLog(VPOXDRVCFG_LOG_SEVERITY enmSeverity, char *pszMsg, void *pvContext)
{
    RT_NOREF1(pvContext);
    switch (enmSeverity)
    {
...
static DECLCALLBACK(void) vpoxUsbPanic(void *pvPanic)
{
    RT_NOREF1(pvPanic);
#ifndef DEBUG_bird
    AssertFailed();
...
int __cdecl main(int argc, char **argv)
{
    ...
    VPoxDrvCfgLoggerSet(vpoxUsbLog, NULL);
    VPoxDrvCfgPanicSet(vpoxUsbPanic, NULL);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = RTPathAppend(szInfFile, sizeof(szInfFile), "VPoxUSB.inf");
        }
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            HRESULT hr = VPoxDrvCfgInfInstall(pwszInfFile);
            if (hr == S_OK)
...
int usblibOsCreateService(void)
{
    ...
    if (hSMgrCreate)
    {
        ...
        int rc = RTPathExecDir(szDriver, sizeof(szDriver) - sizeof("\\VPoxUSBMon.sys"));
        if (RT_SUCCESS(rc))
        {
            strcat(szDriver, "\\VPoxUSBMon.sys");
            RTPrintf("Creating USB monitor driver service with path %s ...\n", szDriver);
            SC_HANDLE hService = CreateService(hSMgrCreate,
                                               SERVICE_NAME,
                                               "VPox USB Monitor Driver",
