 ...
 *
 * This assumes (1) you're calling a function that returns an VPox status code,
 * (2) that you want it's return code on success, and (3) that you wish to wait
 * for ever for it to return.
 *
 * @returns VPox status code.  In the unlikely event that VMR3ReqCallVU fails,
...
VMMR3_INT_DECL(int) VMR3ReqCallWait(PVM pVM, VMCPUID idDstCpu, PFNRT pfnFunction, unsigned cArgs, ...)
{
    ...
    va_list va;
    va_start(va, cArgs);
    int rc = VMR3ReqCallVU(pVM->pUVM, idDstCpu, &pReq, RT_INDEFINITE_WAIT, VMREQFLAGS_VPOX_STATUS,
...
VMMR3DECL(int) VMR3ReqCallWaitU(PUVM pUVM, VMCPUID idDstCpu, PFNRT pfnFunction, unsigned cArgs, ...)
{
    ...
    va_list va;
    va_start(va, cArgs);
    int rc = VMR3ReqCallVU(pUVM, idDstCpu, &pReq, RT_INDEFINITE_WAIT, VMREQFLAGS_VPOX_STATUS,
                           pfnFunction, cArgs, va);
    va_end(va);
    if (RT_SUCCESS(rc))
        rc = pReq->iStatus;
 ...
 *
 * @returns VPox status code returned by VMR3ReqCallVU.
 *
 * @param   pVM             The cross context VM structure.
 * @param   idDstCpu        The destination CPU(s). Either a specific CPU ID or
 *                          one of the following special values:
...
VMMR3DECL(int) VMR3ReqCallNoWait(PVM pVM, VMCPUID idDstCpu, PFNRT pfnFunction, unsigned cArgs, ...)
{
    va_list va;
    va_start(va, cArgs);
    int rc = VMR3ReqCallVU(pVM->pUVM, idDstCpu, NULL, 0, VMREQFLAGS_VPOX_STATUS | VMREQFLAGS_NO_WAIT,
                           pfnFunction, cArgs, va);
    va_end(va);
    return rc;
}
...
VMMR3DECL(int) VMR3ReqCallNoWaitU(PUVM pUVM, VMCPUID idDstCpu, PFNRT pfnFunction, unsigned cArgs, ...)
{
    va_list va;
    va_start(va, cArgs);
    int rc = VMR3ReqCallVU(pUVM, idDstCpu, NULL, 0, VMREQFLAGS_VPOX_STATUS | VMREQFLAGS_NO_WAIT,
                           pfnFunction, cArgs, va);
    va_end(va);
    return rc;
}
 ...
 *
 * @returns VPox status code of VMR3ReqCallVU.
 *
 * @param   pVM             The cross context VM structure.
 * @param   idDstCpu        The destination CPU(s). Either a specific CPU ID or
 *                          one of the following special values:
...
VMMR3DECL(int) VMR3ReqPriorityCallWait(PVM pVM, VMCPUID idDstCpu, PFNRT pfnFunction, unsigned cArgs, ...)
{
    ...
    va_list va;
    va_start(va, cArgs);
    int rc = VMR3ReqCallVU(pVM->pUVM, idDstCpu, &pReq, RT_INDEFINITE_WAIT, VMREQFLAGS_VPOX_STATUS | VMREQFLAGS_PRIORITY,
                           pfnFunction, cArgs, va);
    va_end(va);
    if (RT_SUCCESS(rc))
        rc = pReq->iStatus;
...
VMMR3DECL(int) VMR3ReqPriorityCallWaitU(PUVM pUVM, VMCPUID idDstCpu, PFNRT pfnFunction, unsigned cArgs, ...)
{
    ...
    va_list va;
    va_start(va, cArgs);
    int rc = VMR3ReqCallVU(pUVM, idDstCpu, &pReq, RT_INDEFINITE_WAIT, VMREQFLAGS_VPOX_STATUS | VMREQFLAGS_PRIORITY,
                           pfnFunction, cArgs, va);
    va_end(va);
    if (RT_SUCCESS(rc))
        rc = pReq->iStatus;
...
VMMR3DECL(int) VMR3ReqAlloc(PUVM pUVM, PVMREQ *ppReq, VMREQTYPE enmType, VMCPUID idDstCpu)
{
    ...
    while (--cTries >= 0)
    {
        ...
        if (pReq)
        {
            ...
            pReq->fFlags   = VMREQFLAGS_VPOX_STATUS;
