/* $Id: VPoxGuestR3LibAdditions.cpp $ */
/** @file
 * VPoxGuestR3Lib - Ring-3 Support Library for VirtualPox guest additions, Additions Info.
 */
...
/**
 * Opens the "VirtualPox Guest Additions" registry key.
 *
...
static int vbglR3WinOpenAdditionRegisterKey(PHKEY phKey)
{
    ...
    {
        L"SOFTWARE\\" RT_LSTR(VPOX_VENDOR_SHORT) L"\\VirtualPox Guest Additions",
#ifdef RT_ARCH_AMD64
        L"SOFTWARE\\Wow6432Node\\" RT_LSTR(VPOX_VENDOR_SHORT) L"\\VirtualPox Guest Additions",
#endif
        L"SOFTWARE\\Sun\\VirtualPox Guest Additions",
#ifdef RT_ARCH_AMD64
        L"SOFTWARE\\Wow6432Node\\Sun\\VirtualPox Guest Additions",
#endif
        L"SOFTWARE\\Sun\\xVM VirtualPox Guest Additions",
#ifdef RT_ARCH_AMD64
        L"SOFTWARE\\Wow6432Node\\Sun\\xVM VirtualPox Guest Additions",
#endif
    };
...
static int vbglR3GetAdditionsCompileTimeVersion(char **ppszVer, char **ppszVerExt, char **ppszRev)
{
    int rc = VINF_SUCCESS;
    if (ppszVer)
        rc = RTStrDupEx(ppszVer, VPOX_VERSION_STRING_RAW);
    if (RT_SUCCESS(rc))
    {
        if (ppszVerExt)
            rc = RTStrDupEx(ppszVerExt, VPOX_VERSION_STRING);
        if (RT_SUCCESS(rc))
        {
            if (ppszRev)
                rc = RTStrDupEx(ppszRev, RT_XSTR(VPOX_SVN_REV));
            if (RT_SUCCESS(rc))
                return VINF_SUCCESS;
 ...
 */
VBGLR3DECL(int) VbglR3ReportAdditionsStatus(VPoxGuestFacilityType enmFacility, VPoxGuestFacilityStatus enmStatus,
                                            uint32_t fReserved)
{
