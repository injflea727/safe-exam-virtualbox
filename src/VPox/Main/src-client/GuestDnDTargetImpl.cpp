...
/** @file
 * VPox Console COM Class implementation - Guest drag'n drop target.
 */
                              ...
                              DnDAction_T                     *aResultAction)
{
    ...
    VPOXDNDACTION     dndActionDefault     = 0;
    VPOXDNDACTIONLIST dndActionListAllowed = 0;
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            if (pState && RT_SUCCESS(pState->waitForGuestResponse()))
            {
                ...
                if (lstFormats.size() == 1) /* Exactly one format to use specified? */
                {
                    ...
                    hr = setError(VPOX_E_IPRT_ERROR, tr("Guest returned invalid drop formats (%zu formats)"), lstFormats.size());
            }
            else
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Waiting for response of dropped event failed (%Rrc)"), vrc);
        }
        else
            hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Sending dropped event to guest failed (%Rrc)"), vrc);
    }
    else
...
DECLCALLBACK(int) GuestDnDTarget::i_sendTransferDataCallback(uint32_t uMsg, void *pvParms, size_t cbParms, void *pvUser)
{
    ...
    switch (uMsg)
    {
        ...
        {
            PVPOXDNDCBHGGETNEXTHOSTMSG pCBData = reinterpret_cast<PVPOXDNDCBHGGETNEXTHOSTMSG>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(VPOXDNDCBHGGETNEXTHOSTMSG) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            PVPOXDNDCBHGGETNEXTHOSTMSGDATA pCBData
                = reinterpret_cast<PVPOXDNDCBHGGETNEXTHOSTMSGDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(VPOXDNDCBHGGETNEXTHOSTMSGDATA) == cbParms, VERR_INVALID_PARAMETER);
 ...
 *
 * @returns VPox status code. VINF_EOF if the transfer list is complete.
 * @param   pCtx                Send context to use.
...
HRESULT GuestDnDTarget::cancel(BOOL *aVeto)
{
    ...
    HRESULT hr = RT_SUCCESS(rc) ? S_OK : VPOX_E_IPRT_ERROR;
