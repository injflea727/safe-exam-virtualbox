...
#include <VPox/VMMDev.h>  /* For struct VMMDevDisplayDef - why is it in that file? */
...
typedef struct _DISPLAYFBINFO
{
    ...
    VPOXVIDEOINFOHOSTEVENTS *pHostEvents;
...
public:
    ...
    int  i_handleVHWACommandProcess(int enmCmd, bool fGuestCmd, VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCommand);
#endif
    int  i_handle3DNotifyProcess(VPOX3DNOTIFY *p3DNotify);
