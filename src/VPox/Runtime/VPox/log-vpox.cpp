/* $Id: log-vpox.cpp $ */
/** @file
 * VirtualPox Runtime - Logging configuration.
 */
 ...
 *
 * VPox uses the IPRT logging system which supports group level flags and multiple
 ...
 *
 * The default logger uses VPOX_LOG_DEST as destination specifier. File output is
 * enabled by default and goes to a file "./VPox-\<pid\>.log".
 *
 ...
 * VPOX_LOG environment variable and the .log debugger command can be used to
 ...
 * Example:
 *
 *      VPOX_LOG=+all+pgm.e.s.b.z.l-qemu
 *
...
/**
 * Creates the default logger instance for a VPox process.
 *
...
#define ASSERT_LOG_GROUP2(def, str) \
    ...
    ASSERT_LOG_GROUP(MAIN_VIRTUALPOX);
    ASSERT_LOG_GROUP(MAIN_VIRTUALPOXCLIENT);
    ASSERT_LOG_GROUP(MAIN_VIRTUALPOXSDS);
...
#undef ASSERT_LOG_GROUP2
    ...
    rc = RTLogCreate(&pLogger, 0, NULL, "VPOX_LOG", RT_ELEMENTS(g_apszGroups), &g_apszGroups[0],
# ifdef IN_GUEST
                     RTLOGDEST_USER /* backdoor */,
                     "./VPoxGAs-%04d-%02d-%02d-%02d-%02d-%02d.%03d-%s-%d.log",
# else
                     RTLOGDEST_FILE,
...
# ifndef IN_GUEST
    rc = RTLogCreate(&pLogger, 0, NULL, "VPOX_LOG", RT_ELEMENTS(g_apszGroups), &g_apszGroups[0], RTLOGDEST_FILE, "VPox-ring0.log");
# else  /* IN_GUEST */
    rc = RTLogCreate(&pLogger, 0, NULL, "VPOX_LOG", RT_ELEMENTS(g_apszGroups), &g_apszGroups[0], RTLOGDEST_USER, "VPox-ring0.log");
# endif /* IN_GUEST */
    if (RT_SUCCESS(rc))
    {
        /*
...
# if defined(DEBUG_michael) && defined(IN_GUEST)
        ...
        pLogger->fDestFlags |= RTLOGDEST_DEBUGGER | RTLOGDEST_STDOUT;
# endif
# if 0 /* vpoxdrv logging - ATTENTION: this is what we're referring to guys! Change to '# if 1'. */
        RTLogGroupSettings(pLogger, "all=~0 -default.l6.l5.l4.l3");
