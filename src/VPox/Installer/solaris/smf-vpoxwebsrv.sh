...
case $VW_OPT in
    start)
        if [ ! -f /opt/VirtualPox/vpoxwebsrv ]; then
            echo "ERROR: /opt/VirtualPox/vpoxwebsrv does not exist."
            return $SMF_EXIT_ERR_CONFIG
        fi

        if [ ! -x /opt/VirtualPox/vpoxwebsrv ]; then
            echo "ERROR: /opt/VirtualPox/vpoxwebsrv is not executable."
            return $SMF_EXIT_ERR_CONFIG
        fi
