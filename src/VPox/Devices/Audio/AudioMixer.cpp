 ...
 *
 * @returns VPox status code.  Generally always VINF_SUCCESS unless the input
 ...
 * to output stream in the VPox device context.
 *
 * @returns VPox status code.  Generally always VINF_SUCCESS unless the input
 *          is invalid.  Individual driver errors are suppressed and ignored.
 * @param   pSink       Mixer sink to control.
...
#ifdef VPOX_WITH_DTRACE
        VPOXDD_AUDIO_MIXER_SINK_AIO_OUT(idStream, cbWritten, offStream);
#endif
        offStream += cbWritten;
...
#ifdef VPOX_WITH_DTRACE
            VPOXDD_AUDIO_MIXER_SINK_AIO_IN(idStream, (uint32_t)cbDstBuf, offStream);
#endif
            offStream += cbDstBuf;
 ...
 *
 * @returns VPox status code (generally ignored).
