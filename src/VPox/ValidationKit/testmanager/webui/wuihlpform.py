...
class WuiHlpForm(object):
    ...
    def addListOfTestCaseArgs(self, sName, aoVariations, sLabel): # pylint: disable=too-many-statements
        ...
        if not fReadOnly:
            ...
            sHtml += u'}\n';
            if config.g_kfVPoxSpecific:
                sSecTimeoutDef = escapeAttr(unicode(oVarDefaults.cSecTimeout));
                sHtml += u'function vpox_%s_add_uni()\n' % (sName,);
                sHtml += u'{\n';
                ...
                sHtml += u'}\n';
                sHtml += u'function vpox_%s_add_uni_amd64()\n' % (sName,);
                sHtml += u'{\n';
                sHtml += u'    %s_extendListEx("1-hw", "1", "%s", "--cpu-counts 1 --virt-modes hwvirt", ' \
                         u' "fCpuHwVirt is True", "");\n' % (sName, sSecTimeoutDef);
                sHtml += u'    %s_extendListEx("1-np", "%s", "--cpu-counts 1 --virt-modes hwvirt-np", ' \
                         u' "fCpuNestedPaging is True", "");\n' % (sName, sSecTimeoutDef);
                sHtml += u'}\n';
                sHtml += u'function vpox_%s_add_smp()\n' % (sName,);
                sHtml += u'{\n';
        ...
        if not fReadOnly:
            ...
            if config.g_kfVPoxSpecific:
                sHtml += u'    [<a href="#" onClick="vpox_%s_add_uni()">Single CPU Variations</a>\n' % (sName,);
                sHtml += u'    <a href="#" onClick="vpox_%s_add_uni_amd64()">amd64</a>]\n' % (sName,);
                sHtml += u'    [<a href="#" onClick="vpox_%s_add_smp()">SMP Variations</a>]\n' % (sName,);
            sHtml += u'  </caption>\n';
