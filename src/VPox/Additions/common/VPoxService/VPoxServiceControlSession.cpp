/* $Id: VPoxServiceControlSession.cpp $ */
/** @file
 * VPoxServiceControlSession - Guest session handling. Also handles the spawned session processes.
 */
...
#include "VPoxServiceUtils.h"
#include "VPoxServiceControl.h"
...
enum
{
    VPOXSERVICESESSIONOPT_FIRST = 1000, /* For initialization. */
    VPOXSERVICESESSIONOPT_DOMAIN,
#ifdef DEBUG
    VPOXSERVICESESSIONOPT_DUMP_STDOUT,
    VPOXSERVICESESSIONOPT_DUMP_STDERR,
#endif
    VPOXSERVICESESSIONOPT_LOG_FILE,
    VPOXSERVICESESSIONOPT_USERNAME,
    VPOXSERVICESESSIONOPT_SESSION_ID,
    VPOXSERVICESESSIONOPT_SESSION_PROTO,
    VPOXSERVICESESSIONOPT_THREAD_ID
};


static int vgsvcGstCtrlSessionCleanupProcesses(const PVPOXSERVICECTRLSESSION pSession);
static int vgsvcGstCtrlSessionProcessRemoveInternal(PVPOXSERVICECTRLSESSION pSession, PVPOXSERVICECTRLPROCESS pProcess);
...
static int vgsvcGstCtrlSessionFileFree(PVPOXSERVICECTRLFILE pFile)
{
...
static PVPOXSERVICECTRLFILE vgsvcGstCtrlSessionFileGetLocked(const PVPOXSERVICECTRLSESSION pSession, uint32_t uHandle)
{
    AssertPtrReturn(pSession, NULL);

    /** @todo Use a map later! */
    PVPOXSERVICECTRLFILE pFileCur;
    RTListForEach(&pSession->lstFiles, pFileCur, VPOXSERVICECTRLFILE, Node)
    {
...
static int vgsvcGstCtrlSessionHandleDirRemove(PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
...
static int vgsvcGstCtrlSessionHandleFileOpen(PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
        PVPOXSERVICECTRLFILE pFile = (PVPOXSERVICECTRLFILE)RTMemAllocZ(sizeof(VPOXSERVICECTRLFILE));
        if (pFile)
        {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                                ...
                                ||           RTFS_IS_SYMLINK(objInfo.Attr.fMode))
                            {
                                ...
                                if (RT_SUCCESS(rc))
                                {
                                     ...
                                     * Succeeded!
                                     */
                                    uHandle = VPOX_GUESTCTRL_CONTEXTID_GET_OBJECT(pHostCtx->uContextID);
...
static int vgsvcGstCtrlSessionHandleFileClose(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
        PVPOXSERVICECTRLFILE pFile = vgsvcGstCtrlSessionFileGetLocked(pSession, uHandle);
        if (pFile)
        {
...
static int vgsvcGstCtrlSessionHandleFileRead(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                             void **ppvScratchBuf, uint32_t *pcbScratchBuf)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        int rc2;
        if (g_fControlHostFeatures0 & VPOX_GUESTCTRL_HF_0_NOTIFY_RDWR_OFFSET)
...
static int vgsvcGstCtrlSessionHandleFileReadAt(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                               void **ppvScratchBuf, uint32_t *pcbScratchBuf)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileWrite(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                              void **ppvScratchBuf, uint32_t *pcbScratchBuf)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileWriteAt(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                                void **ppvScratchBuf, uint32_t *pcbScratchBuf)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileSeek(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileTell(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileSetSize(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandlePathRename(PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcGstCtrlSessionHandlePathUserDocuments(PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
 ...
 * @param   pHostCtx        Host context.
 */
static int vgsvcGstCtrlSessionHandlePathUserHome(PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
 ...
 * @param   pHostCtx        Host context.
 */
static int vgsvcGstCtrlSessionHandleProcExec(PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
 ...
 *                              data.
 */
static int vgsvcGstCtrlSessionHandleProcInput(PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                              void **ppvScratchBuf, uint32_t *pcbScratchBuf)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        PVPOXSERVICECTRLPROCESS pProcess = VGSvcGstCtrlSessionRetainProcess(pSession, uPID);
        if (pProcess)
        {
 ...
 * @param   pHostCtx            The host context to use.
 */
static int vgsvcGstCtrlSessionHandleProcOutput(PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
 ...
 * @param   pHostCtx            The host context to use.
 */
static int vgsvcGstCtrlSessionHandleProcTerminate(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleProcWaitFor(const PVPOXSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
int VGSvcGstCtrlSessionHandler(PVPOXSERVICECTRLSESSION pSession, uint32_t uMsg, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                               void **ppvScratchBuf, uint32_t *pcbScratchBuf, volatile bool *pfShutdown)
{
     ...
     * operations.
     */
    bool const fImpersonated = RT_BOOL(pSession->fFlags & (  VPOXSERVICECTRLSESSION_FLAG_SPAWN
                                                           | VPOXSERVICECTRLSESSION_FLAG_ANONYMOUS));
 ...
 * @param   pvUser          Pointer to a VPOXSERVICECTRLSESSIONTHREAD structure.
 *
 */
static DECLCALLBACK(int) vgsvcGstCtrlSessionThread(RTTHREAD hThreadSelf, void *pvUser)
{
    PVPOXSERVICECTRLSESSIONTHREAD pThread = (PVPOXSERVICECTRLSESSIONTHREAD)pvUser;
    ...
    for (;;)
    {
        ...
        if (ASMAtomicReadBool(&pThread->fShutdown))
        {
            if (!msShutdownStart)
            {
                ...
                {
                    /* .idClient  = */  idClient,
                    /* .idContext = */  VPOX_GUESTCTRL_CONTEXTID_MAKE_SESSION(idSession),
...
/** @todo r=bird: This 100ms sleep is _extremely_ sucky! */
    ...
    VBGLR3GUESTCTRLCMDCTX ctx = { idClient, VPOX_GUESTCTRL_CONTEXTID_MAKE_SESSION(idSession),
...
/**
 * Reads the secret key the parent VPoxService instance passed us and pass it
...
static int vgsvcGstCtrlSessionReadKeyAndAccept(uint32_t idClient, uint32_t idSession)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            uint8_t abSecretKey[RT_SIZEOFMEMB(VPOXSERVICECTRLSESSIONTHREAD, abKey)];
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                else
                {
                    ...
                    rc = VbglR3GuestCtrlMsgFilterSet(idClient, VPOX_GUESTCTRL_CONTEXTID_MAKE_SESSION(idSession),
                                                     VPOX_GUESTCTRL_FILTER_BY_SESSION(idSession), 0);
                    if (RT_SUCCESS(rc))
 ...
 */
static void vgsvcGstCtrlSessionInvalidate(PVPOXSERVICECTRLSESSION pSession, uint32_t idClient)
{
    RT_NOREF(pSession);
 ...
 * @thread  main.
 */
static RTEXITCODE vgsvcGstCtrlSessionSpawnWorker(PVPOXSERVICECTRLSESSION pSession)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        VBGLR3GUESTCTRLCMDCTX ctx = { idClient, VPOX_GUESTCTRL_CONTEXTID_MAKE_SESSION(pSession->StartupInfo.uSessionID),
                                      0 /* uProtocol, unused */, 0 /* uNumParms, unused */ };
 ...
 */
PVPOXSERVICECTRLPROCESS VGSvcGstCtrlSessionRetainProcess(PVPOXSERVICECTRLSESSION pSession, uint32_t uPID)
{
    AssertPtrReturn(pSession, NULL);

    PVPOXSERVICECTRLPROCESS pProcess = NULL;
    int rc = RTCritSectEnter(&pSession->CritSect);
    if (RT_SUCCESS(rc))
    {
        PVPOXSERVICECTRLPROCESS pCurProcess;
        RTListForEach(&pSession->lstProcesses, pCurProcess, VPOXSERVICECTRLPROCESS, Node)
        {
...
int VGSvcGstCtrlSessionClose(PVPOXSERVICECTRLSESSION pSession)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        PVPOXSERVICECTRLPROCESS pProcess;
        RTListForEach(&pSession->lstProcesses, pProcess, VPOXSERVICECTRLPROCESS, Node)
        ...
        PVPOXSERVICECTRLPROCESS pProcessNext;
        RTListForEachSafe(&pSession->lstProcesses, pProcess, pProcessNext, VPOXSERVICECTRLPROCESS, Node)
        {
        ...
        PVPOXSERVICECTRLFILE pFile, pFileNext;
        RTListForEachSafe(&pSession->lstFiles, pFile, pFileNext, VPOXSERVICECTRLFILE, Node)
        {
...
int VGSvcGstCtrlSessionDestroy(PVPOXSERVICECTRLSESSION pSession)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
...
int VGSvcGstCtrlSessionInit(PVPOXSERVICECTRLSESSION pSession, uint32_t fFlags)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlSessionProcessAdd(PVPOXSERVICECTRLSESSION pSession, PVPOXSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
 ...
 * @param   pProcess                Guest process to remove.
 */
int VGSvcGstCtrlSessionProcessRemove(PVPOXSERVICECTRLSESSION pSession, PVPOXSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlSessionProcessStartAllowed(const PVPOXSERVICECTRLSESSION pSession, bool *pfAllowed)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionCleanupProcesses(const PVPOXSERVICECTRLSESSION pSession)
{
    ...
    PVPOXSERVICECTRLPROCESS pCurProcess, pNextProcess;
    RTListForEachSafe(&pSession->lstProcesses, pCurProcess, pNextProcess, VPOXSERVICECTRLPROCESS, Node)
    {
...
static int vgsvcVGSvcGstCtrlSessionThreadCreateProcess(const PVBGLR3GUESTCTRLSESSIONSTARTUPINFO pSessionStartupInfo,
                                                       PVPOXSERVICECTRLSESSIONTHREAD pSessionThread, uint32_t uCtrlSessionThread)
{
    RT_NOREF(uCtrlSessionThread);

    /*
     * Is this an anonymous session?  Anonymous sessions run with the same
     * privileges as the main VPoxService executable.
     */
    ...
    apszArgs[idxArg++] = VPOXSERVICE_ARG1_UTF8_ARGV; Assert(idxArg == 2);
#endif
...
#ifdef DEBUG
    if (g_Session.fFlags & VPOXSERVICECTRLSESSION_FLAG_DUMPSTDOUT)
        apszArgs[idxArg++] = "--dump-stdout";
    if (g_Session.fFlags & VPOXSERVICECTRLSESSION_FLAG_DUMPSTDERR)
 ...
 *
 * This will spawn a new VPoxService.exe instance under behalf of the given user
...
int VGSvcGstCtrlSessionThreadCreate(PRTLISTANCHOR pList, const PVBGLR3GUESTCTRLSESSIONSTARTUPINFO pSessionStartupInfo,
                                    PVPOXSERVICECTRLSESSIONTHREAD *ppSessionThread)
{
     ...
     * Main consistency. */
    PVPOXSERVICECTRLSESSIONTHREAD pSessionCur;
    RTListForEach(pList, pSessionCur, VPOXSERVICECTRLSESSIONTHREAD, Node)
    {
     ...
     */
    int rc;
    PVPOXSERVICECTRLSESSIONTHREAD pSessionThread = (PVPOXSERVICECTRLSESSIONTHREAD)RTMemAllocZ(sizeof(*pSessionThread));
    if (pSessionThread)
    {
 ...
 */
int VGSvcGstCtrlSessionThreadWait(PVPOXSERVICECTRLSESSIONTHREAD pThread, uint32_t uTimeoutMS, uint32_t fFlags)
{
    RT_NOREF(fFlags);
    AssertPtrReturn(pThread, VERR_INVALID_POINTER);
 ...
 * @param   fFlags                  Closing flags.
 */
int VGSvcGstCtrlSessionThreadDestroy(PVPOXSERVICECTRLSESSIONTHREAD pThread, uint32_t fFlags)
{
    AssertPtrReturn(pThread, VERR_INVALID_POINTER);
...
int VGSvcGstCtrlSessionThreadDestroyAll(PRTLISTANCHOR pList, uint32_t fFlags)
{
    ...
    PVPOXSERVICECTRLSESSIONTHREAD pSessIt;
    PVPOXSERVICECTRLSESSIONTHREAD pSessItNext;
    RTListForEachSafe(pList, pSessIt, pSessItNext, VPOXSERVICECTRLSESSIONTHREAD, Node)
    {
...
RTEXITCODE VGSvcGstCtrlSessionSpawnInit(int argc, char **argv)
{
    ...
    {
        { "--domain",          VPOXSERVICESESSIONOPT_DOMAIN,          RTGETOPT_REQ_STRING },
#ifdef DEBUG
        { "--dump-stdout",     VPOXSERVICESESSIONOPT_DUMP_STDOUT,     RTGETOPT_REQ_NOTHING },
        { "--dump-stderr",     VPOXSERVICESESSIONOPT_DUMP_STDERR,     RTGETOPT_REQ_NOTHING },
#endif
        { "--logfile",         VPOXSERVICESESSIONOPT_LOG_FILE,        RTGETOPT_REQ_STRING },
        { "--user",            VPOXSERVICESESSIONOPT_USERNAME,        RTGETOPT_REQ_STRING },
        { "--session-id",      VPOXSERVICESESSIONOPT_SESSION_ID,      RTGETOPT_REQ_UINT32 },
        { "--session-proto",   VPOXSERVICESESSIONOPT_SESSION_PROTO,   RTGETOPT_REQ_UINT32 },
#ifdef DEBUG
        { "--thread-id",       VPOXSERVICESESSIONOPT_THREAD_ID,       RTGETOPT_REQ_UINT32 },
#endif /* DEBUG */
    ...
    uint32_t fSession = VPOXSERVICECTRLSESSION_FLAG_SPAWN;
    ...
    while ((ch = RTGetOpt(&GetState, &ValueUnion)) != 0)
    {
        ...
        switch (ch)
        {
            case VPOXSERVICESESSIONOPT_DOMAIN:
                /* Information not needed right now, skip. */
                break;
#ifdef DEBUG
            case VPOXSERVICESESSIONOPT_DUMP_STDOUT:
                fSession |= VPOXSERVICECTRLSESSION_FLAG_DUMPSTDOUT;
                break;

            case VPOXSERVICESESSIONOPT_DUMP_STDERR:
                fSession |= VPOXSERVICECTRLSESSION_FLAG_DUMPSTDERR;
                break;
#endif
            case VPOXSERVICESESSIONOPT_SESSION_ID:
                g_Session.StartupInfo.uSessionID = ValueUnion.u32;
                break;

            case VPOXSERVICESESSIONOPT_SESSION_PROTO:
                g_Session.StartupInfo.uProtocol = ValueUnion.u32;
                break;
#ifdef DEBUG
            case VPOXSERVICESESSIONOPT_THREAD_ID:
                /* Not handled. Mainly for processs listing. */
                break;
#endif
            case VPOXSERVICESESSIONOPT_LOG_FILE:
            {
            ...
            case VPOXSERVICESESSIONOPT_USERNAME:
                /* Information not needed right now, skip. */
                break;
            ...
            {
                if (!RTStrICmp(ValueUnion.psz, VPOXSERVICECTRLSESSION_GETOPT_PREFIX))
                    break;
