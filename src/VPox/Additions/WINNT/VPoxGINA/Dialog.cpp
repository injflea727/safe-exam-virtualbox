/* $Id: Dialog.cpp $ */
/** @file
 * VPoxGINA - Windows Logon DLL for VirtualPox, Dialog Code.
 */
...
void hookDialogBoxes(PVOID pWinlogonFunctions, DWORD dwWlxVersion)
{
    ...
    VPoxGINAVerbose(0, "VPoxGINA::hookDialogBoxes\n");
    ...
    switch (dwWlxVersion)
    {
        ...
        default:
        {
            VPoxGINAVerbose(0, "VPoxGINA::hookDialogBoxes: unrecognized version '%d', nothing hooked!\n", dwWlxVersion);
                    ...
                    PCRTUTF16 pwszUser, PCRTUTF16 pwszPassword, PCRTUTF16 pwszDomain)
{
    ...
    if (hwndDomain)
    {
        ...
        VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: Trying to find domain entry in combo box ...\n");
        DWORD dwIndex = (DWORD) SendMessage(hwndDomain, CB_FINDSTRING,
                                            0, (LPARAM)pwszDomain);
        if (dwIndex != CB_ERR)
        {
            VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: Found domain at combo box pos %ld\n", dwIndex);
            SendMessage(hwndDomain, CB_SETCURSEL, (WPARAM) dwIndex, 0);
            EnableWindow(hwndDomain, FALSE);
        }
        else
        {
            VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: Domain not found in combo box ...\n");
            ...
            if (l > 255)
                VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: Warning! FQDN (domain) is too long (max 255 bytes), will be truncated!\n");
            ...
            if (wcslen(pwszUser) > 0) /* We need a user name that we can use in caes of a FQDN */
            {
                ...
                {
                    VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: Domain seems to be a FQDN (length)!\n");
                    bIsFQDN = TRUE;
                }
                else if (   l > 0
                         && wcsstr(pwszDomain, L".") != NULL) /* if we found a dot (.) in the domain name, this has to be a FQDN */
                {
                    VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: Domain seems to be a FQDN (dot)!\n");
                    bIsFQDN = TRUE;
                }
                ...
                if (bIsFQDN)
                {
                    ...
                    VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: FQDN user name is now: %s!\n", szUserFQDN);
                }
            }
        }
    }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_FAILURE(rc))
        {
            if (rc != VERR_NOT_FOUND)
                VPoxGINAVerbose(0, "VPoxGINA::credentialsHandle: error querying for credentials, rc=%Rrc\n", rc);
        }
    }

    if (RT_SUCCESS(rc))
    {
        VPoxGINAVerbose(0, "VPoxGINA::credentialsHandle: credentials available\n");
         ...
         */
        VPoxGINAReportStatus(VPoxGuestFacilityStatus_Terminating);
        ...
        {
#ifdef DEBUG
            VPoxGINAVerbose(0, "VPoxGINA::credentialsHandle: retrieved credentials: user=%ls, password=%ls, domain=%ls\n",
                            pwszUser, pwszPassword, pwszDomain);
#else
            VPoxGINAVerbose(0, "VPoxGINA::credentialsHandle: retrieved credentials: user=%ls, password=XXX, domain=%ls\n",
                            pwszUser, pwszDomain);
#endif
...
#ifdef DEBUG
    VPoxGINAVerbose(3, "VPoxGINA::credentialsHandle: returned with rc=%Rrc\n", rc);
#endif
    return rc;
}
                                          ...
                                          LPARAM lParam)   // second message parameter
{
    ...
    /*VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc\n");*/
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: got WM_INITDIALOG\n");
            ...
            VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: hwndUserId: %x, hwndPassword: %d, hwndDomain: %d\n",
                            s_hwndUserId, s_hwndPassword, s_hwndDomain);

            /* terminate the credentials poller thread, it's done is job */
            VPoxGINACredentialsPollerTerminate();
            ...
            if (RT_FAILURE(rc))
            {
                ...
                if (!uTimer)
                    VPoxGINAVerbose(0, "VPoxGINA::MyWlxLoggedOutSASDlgProc: failed creating timer! Last error: %ld\n",
                                    GetLastError());
            }
            break;
        }
                                       ...
                                       LPARAM lParam)   // second message parameter
{
    ...
    /*VPoxGINAVerbose(0, "VPoxGINA::MyWlxLockedSASDlgProc\n");*/
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            VPoxGINAVerbose(0, "VPoxGINA::MyWlxLockedSASDlgProc: WM_INITDIALOG\n");

            /* get the entry fields */
            s_hwndPassword = GetDlgItem(hwndDlg, IDC_WKSTALOCKED_PASSWORD);
            VPoxGINAVerbose(0, "VPoxGINA::MyWlxLockedSASDlgProc: hwndPassword: %d\n", s_hwndPassword);
            ...
            if (RT_FAILURE(rc))
            {
                ...
                if (!uTimer)
                    VPoxGINAVerbose(0, "VPoxGINA::MyWlxLockedSASDlgProc: failed creating timer! Last error: %ld\n",
                         GetLastError());
            }
            break;
        }
        ...
        case WM_DESTROY:
        {
            VPoxGINAVerbose(0, "VPoxGINA::MyWlxLockedSASDlgProc: WM_DESTROY\n");
...
int WINAPI MyWlxDialogBoxParam(HANDLE  hWlx,
                               ...
                               LPARAM  dwInitParam)
{
    VPoxGINAVerbose(0, "VPoxGINA::MyWlxDialogBoxParam: pszTemplate=%ls\n", pszTemplate);

    VPoxGINAReportStatus(VPoxGuestFacilityStatus_Active);
    ...
    if (((uintptr_t)pszTemplate >> 16) == 0)
    {
        ...
        switch ((DWORD)(uintptr_t)pszTemplate)
        {
            case IDD_WLXDIAPLAYSASNOTICE_DIALOG:
                VPoxGINAVerbose(0, "VPoxGINA::MyWlxDialogBoxParam: SAS notice dialog displayed; not handled\n");
                break;

            case IDD_WLXLOGGEDOUTSAS_DIALOG:     /* Windows NT 4.0. */
            case IDD_WLXLOGGEDOUTSAS_DIALOG2:    /* Windows 2000 and up. */
            {
                VPoxGINAVerbose(0, "VPoxGINA::MyWlxDialogBoxParam: returning hooked SAS logged out dialog\n");
            ...
            case IDD_SECURITY_DIALOG:
                VPoxGINAVerbose(0, "VPoxGINA::MyWlxDialogBoxParam: Security dialog displayed; not handled\n");
                break;

            case IDD_WLXWKSTALOCKEDSAS_DIALOG:   /* Windows NT 4.0. */
            case IDD_WLXWKSTALOCKEDSAS_DIALOG2:  /* Windows 2000 and up. */
            {
                VPoxGINAVerbose(0, "VPoxGINA::MyWlxDialogBoxParam: returning hooked SAS locked dialog\n");
            ...
            default:
                VPoxGINAVerbose(0, "VPoxGINA::MyWlxDialogBoxParam: dialog %p (%u) not handled\n",
