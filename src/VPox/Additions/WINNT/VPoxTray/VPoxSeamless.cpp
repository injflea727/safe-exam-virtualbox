...
typedef struct _VPOXSEAMLESSCONTEXT
{
    const VPOXSERVICEENV *pEnv;

    RTLDRMOD hModHook;

    BOOL    (* pfnVPoxHookInstallWindowTracker)(HMODULE hDll);
    BOOL    (* pfnVPoxHookRemoveWindowTracker)();

    PVPOXDISPIFESCAPE lpEscapeData;
} VPOXSEAMLESSCONTEXT, *PVPOXSEAMLESSCONTEXT;

typedef struct
{
    HDC     hdc;
    HRGN    hrgn;
} VPOX_ENUM_PARAM, *PVPOX_ENUM_PARAM;

static VPOXSEAMLESSCONTEXT g_Ctx = { 0 };

void VPoxLogString(HANDLE hDriver, char *pszStr);

DECLCALLBACK(int) VPoxSeamlessInit(const PVPOXSERVICEENV pEnv, void **ppInstance)
{
    LogFlowFuncEnter();

    PVPOXSEAMLESSCONTEXT pCtx = &g_Ctx; /* Only one instance at the moment. */
    AssertPtr(pCtx);
    ...
    /* We have to jump out here when using NT4, otherwise it complains about
       a missing API function "UnhookWinEvent" used by the dynamically loaded VPoxHook.dll below */
    ...
    else
    {
        ...
        rc = RTLdrLoadAppPriv(VPOXHOOK_DLL_NAME, &pCtx->hModHook);
        if (RT_SUCCESS(rc))
        {
            *(PFNRT *)&pCtx->pfnVPoxHookInstallWindowTracker = RTLdrGetFunction(pCtx->hModHook, "VPoxHookInstallWindowTracker");
            *(PFNRT *)&pCtx->pfnVPoxHookRemoveWindowTracker  = RTLdrGetFunction(pCtx->hModHook, "VPoxHookRemoveWindowTracker");

            if (   pCtx->pfnVPoxHookInstallWindowTracker
                && pCtx->pfnVPoxHookRemoveWindowTracker)
            {
                VPoxSeamlessSetSupported(TRUE);
...
void VPoxSeamlessDestroy(void *pInstance)
{
    LogFlowFuncEnter();
    ...
    PVPOXSEAMLESSCONTEXT pCtx = (PVPOXSEAMLESSCONTEXT)pInstance;
    AssertPtr(pCtx);

    VPoxSeamlessSetSupported(FALSE);

    /* Inform the host that we no longer support the seamless window mode. */
    if (pCtx->pfnVPoxHookRemoveWindowTracker)
        pCtx->pfnVPoxHookRemoveWindowTracker();
...
static void VPoxSeamlessInstallHook(void)
{
    PVPOXSEAMLESSCONTEXT pCtx = &g_Ctx; /** @todo r=andy Use instance data via service lookup (add void *pInstance). */
    AssertPtr(pCtx);
    ...
    if (pCtx->pfnVPoxHookInstallWindowTracker)
    {
        ...
        pCtx->pfnVPoxHookInstallWindowTracker(hMod);
    }
}

static void VPoxSeamlessRemoveHook(void)
{
    PVPOXSEAMLESSCONTEXT pCtx = &g_Ctx; /** @todo r=andy Use instance data via service lookup (add void *pInstance). */
    AssertPtr(pCtx);
...
static VPOXDISPIF_SEAMLESS gVPoxDispIfSeamless; /** @todo r=andy Move this into VPOXSEAMLESSCONTEXT? */


void VPoxSeamlessEnable(void)
{
    PVPOXSEAMLESSCONTEXT pCtx = &g_Ctx; /** @todo r=andy Use instance data via service lookup (add void *pInstance). */
    AssertPtr(pCtx);
    ...
    VPoxDispIfSeamlessCreate(&pCtx->pEnv->dispIf, &gVPoxDispIfSeamless, g_hSeamlessKmNotifyEvent);

    VPoxSeamlessInstallHook();
}

void VPoxSeamlessDisable(void)
{
    PVPOXSEAMLESSCONTEXT pCtx = &g_Ctx; /** @todo r=andy Use instance data via service lookup (add void *pInstance). */
    AssertPtr(pCtx);
    NOREF(pCtx);

    VPoxSeamlessRemoveHook();

    VPoxDispIfSeamlessTerm(&gVPoxDispIfSeamless);
}

BOOL CALLBACK VPoxEnumFunc(HWND hwnd, LPARAM lParam)
{
    PVPOX_ENUM_PARAM    lpParam = (PVPOX_ENUM_PARAM)lParam;
    ...
    LogFlow(("VPoxTray: VPoxEnumFunc %x\n", hwnd));
    ...
    {
        Log(("VPoxTray: Filter out shadow window style=%x exstyle=%x\n", dwStyle, dwExStyle));
        Log(("VPoxTray: Enum hwnd=%x rect (%d,%d) (%d,%d) (filtered)\n", hwnd, rectWindow.left, rectWindow.top, rectWindow.right, rectWindow.bottom));
        Log(("VPoxTray: title=%s style=%x exStyle=%x\n", szWindowText, dwStyle, dwExStyle));
        return TRUE;
    }
    ...
    {
        Log(("VPoxTray: Enum hwnd=%x rect (%d,%d)-(%d,%d) [%d x %d](applying)\n", hwnd,
        ...
        if (ret == ERROR)
        {
            Log(("VPoxTray: GetWindowRgn failed with rc=%d, adding antire rect\n", GetLastError()));
    ...
    {
        Log(("VPoxTray: Enum hwnd=%x rect (%d,%d)-(%d,%d) [%d x %d](ignored)\n", hwnd,
            rectWindow.left, rectWindow.top, rectWindow.right, rectWindow.bottom,
            rectWindow.left - rectWindow.right, rectWindow.bottom - rectWindow.top));
        Log(("VPoxTray: title=%s style=%x exStyle=%x\n", szWindowText, dwStyle, dwExStyle));
    }
...
void VPoxSeamlessCheckWindows(bool fForce)
{
    ...
    if (!VPoxDispIfSeamlesIsValid(&gVPoxDispIfSeamless))
        return;

    VPOX_ENUM_PARAM param;

    param.hdc       = GetDC(HWND_DESKTOP);
    param.hrgn      = 0;

    EnumWindows(VPoxEnumFunc, (LPARAM)&param);
    ...
    if (param.hrgn)
    {
        ...
        if (cbSize)
        {
            PVPOXDISPIFESCAPE lpEscapeData = (PVPOXDISPIFESCAPE)malloc(VPOXDISPIFESCAPE_SIZE(cbSize));
            if (lpEscapeData)
            {
                lpEscapeData->escapeCode = VPOXESC_SETVISIBLEREGION;
                LPRGNDATA lpRgnData = VPOXDISPIFESCAPE_DATA(lpEscapeData, RGNDATA);
                ...
                if (cbSize)
                {
                    ...
                    Log(("VPoxTray: New visible region: \n"));
                    for (DWORD i = 0; i < lpRgnData->rdh.nCount; i++)
                        Log(("VPoxTray: visible rect (%d,%d)(%d,%d)\n",
                             paRects[i].left, paRects[i].top, paRects[i].right, paRects[i].bottom));
#endif

                    LPRGNDATA lpCtxRgnData = VPOXDISPIFESCAPE_DATA(pCtx->lpEscapeData, RGNDATA);
                        ...
                        || memcmp(lpCtxRgnData, lpRgnData, cbSize))
                    {
                        ...
                        VPoxDispIfSeamlessSubmit(&gVPoxDispIfSeamless, lpEscapeData, cbSize);
                        ...
                        Log(("VPoxTray: Visible rectangles haven't changed; ignore\n"));
                }
 ...
 * requests
 */
static DECLCALLBACK(int) VPoxSeamlessWorker(void *pvInstance, bool volatile *pfShutdown)
{
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        switch (enmMode)
                        {
                            case VMMDev_Seamless_Disabled:
                                ...
                                PostMessage(g_hwndToolWindow, WM_VPOX_SEAMLESS_DISABLE, 0, 0);
                                break;
                            ...
                            case VMMDev_Seamless_Visible_Region:
                                ...
                                if (!fRet)
                                    LogRel(("Seamless: SystemParametersInfo SPI_SETSCREENSAVEACTIVE failed with %ld\n", GetLastError()));
                                PostMessage(g_hwndToolWindow, WM_VPOX_SEAMLESS_ENABLE, 0, 0);
                                break;
 ...
 */
VPOXSERVICEDESC g_SvcDescSeamless =
{
    /* pszName. */
    "seamless",
    /* pszDescription. */
