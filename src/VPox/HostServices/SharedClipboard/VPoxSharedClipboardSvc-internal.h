/* $Id: VPoxSharedClipboardSvc-internal.h $ */
/** @file
...
#ifndef VPOX_INCLUDED_SRC_SharedClipboard_VPoxSharedClipboardSvc_internal_h
#define VPOX_INCLUDED_SRC_SharedClipboard_VPoxSharedClipboardSvc_internal_h
...
typedef struct _SHCLCLIENTMSG
{
    ...
    /** Stored message ID (VPOX_SHCL_HOST_MSG_XXX). */
    uint32_t            idMsg;
    /** Context ID. */
    uint64_t            idCtx;
    ...
    VPOXHGCMSVCPARM     aParms[RT_FLEXIBLE_ARRAY];
} SHCLCLIENTMSG;
...
typedef struct SHCLCLIENTSTATE
{
    ...
    /** Guest feature flags, VPOX_SHCL_GF_0_XXX. */
    uint64_t                fGuestFeatures0;
    /** Guest feature flags, VPOX_SHCL_GF_1_XXX. */
...
int shClSvcTransferHandler(PSHCLCLIENT pClient, VPOXHGCMCALLHANDLE callHandle, uint32_t u32Function,
                           uint32_t cParms, VPOXHGCMSVCPARM paParms[], uint64_t tsArrival);
int shClSvcTransferHostHandler(uint32_t u32Function, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
/** @} */
 ...
 * @{
 */
DECLCALLBACK(void) VPoxSvcClipboardTransferPrepareCallback(PSHCLTRANSFERCALLBACKDATA pData);
DECLCALLBACK(void) VPoxSvcClipboardDataHeaderCompleteCallback(PSHCLTRANSFERCALLBACKDATA pData);
DECLCALLBACK(void) VPoxSvcClipboardDataCompleteCallback(PSHCLTRANSFERCALLBACKDATA pData);
DECLCALLBACK(void) VPoxSvcClipboardTransferCompleteCallback(PSHCLTRANSFERCALLBACKDATA pData, int rc);
DECLCALLBACK(void) VPoxSvcClipboardTransferCanceledCallback(PSHCLTRANSFERCALLBACKDATA pData);
DECLCALLBACK(void) VPoxSvcClipboardTransferErrorCallback(PSHCLTRANSFERCALLBACKDATA pData, int rc);
/** @} */
#endif /* VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS */
...
#endif /* !VPOX_INCLUDED_SRC_SharedClipboard_VPoxSharedClipboardSvc_internal_h */
