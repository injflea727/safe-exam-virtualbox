/* $Id: vbsf.cpp $ */
/** @file
 * Shared Folders - VPox Shared Folders.
 */
 ...
 * @param  pParms   Input:
 *                    - @a CreateFlags: Creation or open parameters, see include/VPox/shflsvc.h
 ...
 *                                      initial size.
 *                  Output:
 *                    - @a Result:      Shared folder status code, see include/VPox/shflsvc.h
        ...
        switch (rc)
        {
            case VERR_FILE_NOT_FOUND:
                ...
                /* This actually isn't an error, so correct the rc before return later,
                   because the driver (VPoxSF.sys) expects rc = VINF_SUCCESS and checks the result code. */
                fNoError = true;
                break;
 ...
 *
 *                      Output:
 *                        - @a Result Shared folder result code, see include/VPox/shflsvc.h
 ...
 * pages parameter.
 */
static int vbsfPagesToSgBuf(VPOXHGCMSVCPARMPAGES const *pPages, uint32_t cbLeft, PRTSGBUF pSgBuf)
{
...
int vbsfReadPages(SHFLCLIENTDATA *pClient, SHFLROOT idRoot, SHFLHANDLE hFile, uint64_t offFile,
                  uint32_t *pcbRead, PVPOXHGCMSVCPARMPAGES pPages)
{
...
int vbsfWritePages(SHFLCLIENTDATA *pClient, SHFLROOT idRoot, SHFLHANDLE hFile, uint64_t *poffFile,
                   uint32_t *pcbWrite, PVPOXHGCMSVCPARMPAGES pPages)
{
    uint64_t offFile = *poffFile;
