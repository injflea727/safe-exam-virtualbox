...
static BOOL VPoxDispVideoModeSupported(const PVIDEO_MODE_INFORMATION pMode)
{
 ...
 */
int VPoxDispMPGetVideoModes(HANDLE hDriver, PVIDEO_MODE_INFORMATION *ppModesTable, ULONG *pcModes)
{
    DWORD dwrc;
    ...
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    ...
    for (i=0; i<numModes.NumModes; ++i)
    {
        ...
        if (VPoxDispVideoModeSupported(pMode))
        {
            cSupportedModes++;
        }
        else
...
int VPoxDispMPGetPointerCaps(HANDLE hDriver, PVIDEO_POINTER_CAPABILITIES pCaps)
{
    ...
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    VPOX_WARN_IOCTLCB_RETRC("IOCTL_VIDEO_QUERY_POINTER_CAPABILITIES", cbReturned, sizeof(VIDEO_POINTER_CAPABILITIES), VERR_DEV_IO_ERROR);
...
int VPoxDispMPMapMemory(PVPOXDISPDEV pDev, PVIDEO_MEMORY_INFORMATION pMemInfo)
{
    ...
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    VPOX_WARN_IOCTLCB_RETRC("IOCTL_VIDEO_MAP_VIDEO_MEMORY", cbReturned, sizeof(vMemInfo), VERR_DEV_IO_ERROR);
...
int VPoxDispMPQueryHGSMIInfo(HANDLE hDriver, QUERYHGSMIRESULT *pInfo)
{
    ...
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    VPOX_WARN_IOCTLCB_RETRC("IOCTL_VIDEO_QUERY_HGSMI_INFO", cbReturned, sizeof(QUERYHGSMIRESULT), VERR_DEV_IO_ERROR);
...
int VPoxDispMPQueryHGSMICallbacks(HANDLE hDriver, HGSMIQUERYCALLBACKS *pCallbacks)
{
    ...
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    VPOX_WARN_IOCTLCB_RETRC("IOCTL_VIDEO_HGSMI_QUERY_CALLBACKS", cbReturned, sizeof(HGSMIQUERYCALLBACKS), VERR_DEV_IO_ERROR);
...
int VPoxDispMPHGSMIQueryPortProcs(HANDLE hDriver, HGSMIQUERYCPORTPROCS *pPortProcs)
{
    ...
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    VPOX_WARN_IOCTLCB_RETRC("IOCTL_VIDEO_HGSMI_QUERY_PORTPROCS", cbReturned, sizeof(HGSMIQUERYCPORTPROCS), VERR_DEV_IO_ERROR);
...
int VPoxDispMPVHWAQueryInfo(HANDLE hDriver, VHWAQUERYINFO *pInfo)
{
    ...
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    VPOX_WARN_IOCTLCB_RETRC("IOCTL_VIDEO_VHWA_QUERY_INFO", cbReturned, sizeof(VHWAQUERYINFO), VERR_DEV_IO_ERROR);
...
int VPoxDispMPSetVisibleRegion(HANDLE hDriver, PRTRECT pRects, DWORD cRects)
{
    ...
    dwrc = EngDeviceIoControl(hDriver, IOCTL_VIDEO_VPOX_SETVISIBLEREGION, pRects, cRects*sizeof(RTRECT),
                              NULL, 0, &cbReturned);
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
...
int VPoxDispMPShareVideoMemory(HANDLE hDriver, PVIDEO_SHARE_MEMORY pSMem, PVIDEO_SHARE_MEMORY_INFORMATION pSMemInfo)
{
    ...
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    VPOX_WARN_IOCTLCB_RETRC("IOCTL_VIDEO_SHARE_VIDEO_MEMORY", cbReturned,
...
int VPoxDispMPQueryRegistryFlags(HANDLE hDriver, ULONG *pulFlags)
{
    ...
    *pulFlags = 0;
    ulInfoLevel = VPOXVIDEO_INFO_LEVEL_REGISTRY_FLAGS;
    dwrc = EngDeviceIoControl(hDriver, IOCTL_VIDEO_QUERY_VPOXVIDEO_INFO, &ulInfoLevel, sizeof(DWORD),
                              pulFlags, sizeof(DWORD), &cbReturned);
    VPOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    VPOX_WARN_IOCTLCB_RETRC("IOCTL_VIDEO_QUERY_INFO", cbReturned, sizeof(DWORD), VERR_DEV_IO_ERROR);

    if (*pulFlags != 0)
        LogRel(("VPoxDisp: video flags 0x%08X\n", *pulFlags));
