...
"""
VirtualPox Validation Kit - Local teleportation testdriver.
"""
...
class tdTeleportLocal1(vpox.TestDriver):
    """
    ...
    def actionConfig(self):
        ...
        sVPoxValidationKit_iso = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../VPoxValidationKit.iso'));
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../VPoxTestSuite.iso'));
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = '/mnt/ramdisk/vpox/svn/trunk/validationkit/VPoxValidationKit.iso';
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = '/mnt/ramdisk/vpox/svn/trunk/testsuite/VPoxTestSuite.iso';
        if not os.path.isfile(sVPoxValidationKit_iso):
            sCur = os.getcwd();
            for i in range(0, 10):
                sVPoxValidationKit_iso = os.path.join(sCur, 'validationkit/VPoxValidationKit.iso');
                if os.path.isfile(sVPoxValidationKit_iso):
                    break;
                sVPoxValidationKit_iso = os.path.join(sCur, 'testsuite/VPoxTestSuite.iso');
                if os.path.isfile(sVPoxValidationKit_iso):
                    break;
                sCur = os.path.abspath(os.path.join(sCur, '..'));
                if i is not None: pass;     # shut up pychecker/pylint.
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = '/home/bird/validationkit/VPoxValidationKit.iso';
        if not os.path.isfile(sVPoxValidationKit_iso):
            sVPoxValidationKit_iso = '/home/bird/testsuite/VPoxTestSuite.iso';
        ...
        if 'tst-rhel5' in self.asTestVMs:
            oVM = self.createTestVMs('tst-rhel5', 1, '3.0/tcp/rhel5.vdi', sKind = 'RedHat', fIoApic = True, \
                                     sDvdImage = sVPoxValidationKit_iso);
            if oVM is None:
                return False;
    ...
    def createTestVMs(self, sName, iGroup, *tArgs, **dKeywordArgs):
        """
        Wrapper around vpox.createTestVM for creating two VMs, the source
                    ...
                    if oSessionSrc is not None:
                        ...
                        for sPassword in tsPasswords:
                            ...
                            if oProgressSrc:
                                ...
                                elif oProgressSrc.getErrInfoResultCode() != vpox.ComError.E_FAIL:
                                    reporter.testFailure('IConsole::teleport returns %s instead of E_FAIL' \
                                                         % (vpox.ComError.toString(oProgressSrc.getErrInfoResultCode()),));
                            ...
                            if oProgressSrc:
                                ...
                                if oProgressSrc.getResult() != oProgressDst.getResult():
                                    reporter.testFailure('Result differs - src=%s dst=%s' \
                                                         % (vpox.ComError.toString(oProgressSrc.getResult()),\
                                                            vpox.ComError.toString(oProgressDst.getResult())));
                                    ...
                                    reporter.testFailure('ErrorInfo::resultCode differs - src=%s dst=%s' \
                                                         % (vpox.ComError.toString(oProgressSrc.getErrInfoResultCode()),\
                                                            vpox.ComError.toString(oProgressDst.getErrInfoResultCode())));
    ...
    def test1Sub2SetPort(self, oSession, uPort, fInvalid = False):
        ...
        try:
            ...
            if not fInvalid or vpox.ComError.notEqual(oXcpt, vpox.ComError.E_INVALIDARG):
    ...
    def test1Sub1DoTeleport(self, oSession, sHostname, uPort, sPassword, cMsMaxDowntime, hrcExpected, sTestName):
        ...
        try:
            ...
            if vpox.ComError.equal(oXcpt, hrcExpected):
                fRc = True;
            else:
                reporter.testFailure('hresult %s, expected %s' \
                                     % (vpox.ComError.toString(oXcpt.hresult),
                                        vpox.ComError.toString(hrcExpected)));
        except Exception as oXcpt:
