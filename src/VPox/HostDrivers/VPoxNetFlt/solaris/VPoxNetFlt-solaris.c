/* $Id: VPoxNetFlt-solaris.c $ */
/** @file
 * VPoxNetFlt - Network Filter Driver (Host), Solaris Specific Code.
 */
...
#define VPOXNETFLT_SOLARIS_IPV6_POLLING
#ifdef VPOXNETFLT_SOLARIS_IPV6_POLLING
...
*********************************************************************************************************************************/
/** The module name. */
#define DEVICE_NAME                     "vpoxflt"
/** The module descriptions as seen in 'modinfo'. */
#define DEVICE_DESC_DRV                 "VirtualPox NetDrv"
#define DEVICE_DESC_MOD                 "VirtualPox NetMod"

#ifdef VPOXNETFLT_SOLARIS_IPV6_POLLING
/** Driver properties */
# define VPOXNETFLT_IP6POLLINTERVAL      "ipv6-pollinterval"
#endif

/** Maximum loopback packet queue size per interface */
#define VPOXNETFLT_LOOPBACK_SIZE        32
 ...
 * Stream Driver hooks.
 */
static int VPoxNetFltSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pArg, void **ppvResult);
static int VPoxNetFltSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd);
static int VPoxNetFltSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd);
static int VPoxNetFltSolarisQuiesceNotNeeded(dev_info_t *pDip);

/**
 * Stream Module hooks.
 */
static int VPoxNetFltSolarisModOpen(queue_t *pQueue, dev_t *pDev, int fFile, int fStream, cred_t *pCred);
static int VPoxNetFltSolarisModClose(queue_t *pQueue, int fFile, cred_t *pCred);
static int VPoxNetFltSolarisModReadPut(queue_t *pQueue, mblk_t *pMsg);
static int VPoxNetFltSolarisModWritePut(queue_t *pQueue, mblk_t *pMsg);
 ...
 * Streams: module info.
 */
static struct module_info g_VPoxNetFltSolarisModInfo =
{
    0xbad,                          /* module id */
    DEVICE_NAME,
    0,                              /* min. packet size */
 ...
 */
static struct qinit g_VPoxNetFltSolarisReadQ =
{
    VPoxNetFltSolarisModReadPut,
    NULL,                           /* service */
    VPoxNetFltSolarisModOpen,
    VPoxNetFltSolarisModClose,
    NULL,                           /* admin (reserved) */
    &g_VPoxNetFltSolarisModInfo,
    NULL                            /* module stats */
};
 ...
 */
static struct qinit g_VPoxNetFltSolarisWriteQ =
{
    VPoxNetFltSolarisModWritePut,
    NULL,                           /* service */
    NULL,                           /* open */
    NULL,                           /* close */
    NULL,                           /* admin (reserved) */
 ...
 */
static struct streamtab g_VPoxNetFltSolarisStreamTab =
{
    &g_VPoxNetFltSolarisReadQ,
    &g_VPoxNetFltSolarisWriteQ,
    NULL,                           /* muxread init */
    NULL                            /* muxwrite init */
};
 ...
 */
static struct cb_ops g_VPoxNetFltSolarisCbOps =
{
    nulldev,                        /* cb open */
    nulldev,                        /* cb close */
    nodev,                          /* b strategy */
 ...
 */
static struct dev_ops g_VPoxNetFltSolarisDevOps =
{
    DEVO_REV,                       /* driver build revision */
    0,                              /* ref count */
    VPoxNetFltSolarisGetInfo,
    nulldev,                        /* identify */
    nulldev,                        /* probe */
    VPoxNetFltSolarisAttach,
    VPoxNetFltSolarisDetach,
    nodev,                          /* reset */
    &g_VPoxNetFltSolarisCbOps,
    (struct bus_ops *)0,
    nodev,                          /* power */
    VPoxNetFltSolarisQuiesceNotNeeded
};

/**
 * modldrv: export driver specifics to kernel
 */
static struct modldrv g_VPoxNetFltSolarisDriver =
{
    &mod_driverops,                 /* extern from kernel */
    DEVICE_DESC_DRV " " VPOX_VERSION_STRING "r" RT_XSTR(VPOX_SVN_REV),
    &g_VPoxNetFltSolarisDevOps
};

/**
 * fmodsw: streams module ops
 */
static struct fmodsw g_VPoxNetFltSolarisModOps =
{
    DEVICE_NAME,
    &g_VPoxNetFltSolarisStreamTab,
 ...
 */
static struct modlstrmod g_VPoxNetFltSolarisModule =
{
    &mod_strmodops,                 /* extern from kernel */
    DEVICE_DESC_MOD " " VPOX_VERSION_STRING "r" RT_XSTR(VPOX_SVN_REV),
    &g_VPoxNetFltSolarisModOps
};

/**
 * modlinkage: export install/remove/info to the kernel
 */
static struct modlinkage g_VPoxNetFltSolarisModLinkage =
{
    MODREV_1,                        /* loadable module system revision */
    {
        &g_VPoxNetFltSolarisDriver,  /* streams driver framework */
        &g_VPoxNetFltSolarisModule,  /* streams module framework */
        NULL                         /* terminate array of linkage structures */
    }
};

struct vpoxnetflt_state_t;

/**
 * vpoxnetflt_dladdr_t: DL SAP address format
 */
typedef struct vpoxnetflt_dladdr_t
{
    ether_addr_t Mac;
    uint16_t SAP;
} vpoxnetflt_dladdr_t;

#define VPOXNETFLT_DLADDRL        sizeof(vpoxnetflt_dladdr_t)

/**
 * which stream is this?
 */
typedef enum VPOXNETFLTSTREAMTYPE
{
    kUndefined = 0,
    kIp4Stream = 0x1b,
    kIp6Stream = 0xcc,
    kArpStream = 0xab,
    kPromiscStream = 0xdf
} VPOXNETFLTSTREAMTYPE;

/**
 * loopback packet identifier
 */
typedef struct VPOXNETFLTPACKETID
{
    struct VPOXNETFLTPACKETID *pNext;
    uint16_t cbPacket;
    uint16_t Checksum;
    RTMAC SrcMac;
    RTMAC DstMac;
} VPOXNETFLTPACKETID;
typedef struct VPOXNETFLTPACKETID *PVPOXNETFLTPACKETID;

/**
 * vpoxnetflt_stream_t: per-stream data (multiple streams per interface)
 */
typedef struct vpoxnetflt_stream_t
{
    int DevMinor;                         /* minor device no. (for clone) */
    queue_t *pReadQueue;                  /* read side queue */
    struct vpoxnetflt_stream_t *pNext;    /* next stream in list */
    PVPOXNETFLTINS volatile pThis;        /* the backend instance */
    VPOXNETFLTSTREAMTYPE Type;            /* the type of the stream */
} vpoxnetflt_stream_t;

/**
 * vpoxnetflt_promisc_stream_t: per-interface dedicated stream data
 */
typedef struct vpoxnetflt_promisc_stream_t
{
    vpoxnetflt_stream_t Stream;           /* dedicated/promiscuous stream */
    ...
    PVPOXNETFLTPACKETID pHead;            /* loopback packet identifier head */
    PVPOXNETFLTPACKETID pTail;            /* loopback packet identifier tail */
} vpoxnetflt_promisc_stream_t;

typedef struct vpoxnetflt_promisc_params_t
{
    PVPOXNETFLTINS pThis;                 /* the backend instance */
    bool fPromiscOn;                      /* whether promiscuous req. on or off */
} vpoxnetflt_promisc_params_t;


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int vpoxNetFltSolarisSetRawMode(vpoxnetflt_promisc_stream_t *pPromiscStream);
/* static int vpoxNetFltSolarisSetFastMode(queue_t *pQueue); */

static int vpoxNetFltSolarisPhysAddrReq(queue_t *pQueue);
static void vpoxNetFltSolarisCachePhysAddr(PVPOXNETFLTINS pThis, mblk_t *pPhysAddrAckMsg);
static int vpoxNetFltSolarisBindReq(queue_t *pQueue, int SAP);
static int vpoxNetFltSolarisNotifyReq(queue_t *pQueue);

/*  static int vpoxNetFltSolarisUnitDataToRaw(PVPOXNETFLTINS pThis, mblk_t *pMsg, mblk_t **ppRawMsg); */
static int vpoxNetFltSolarisRawToUnitData(mblk_t *pMsg, mblk_t **ppDlpiMsg);

static inline void vpoxNetFltSolarisInitPacketId(PVPOXNETFLTPACKETID pTag, mblk_t *pMsg);
static int vpoxNetFltSolarisQueueLoopback(PVPOXNETFLTINS pThis, vpoxnetflt_promisc_stream_t *pPromiscStream, mblk_t *pMsg);
static bool vpoxNetFltSolarisIsOurMBlk(PVPOXNETFLTINS pThis, vpoxnetflt_promisc_stream_t *pPromiscStream, mblk_t *pMsg);

static mblk_t *vpoxNetFltSolarisMBlkFromSG(PVPOXNETFLTINS pThis, PINTNETSG pSG, uint32_t fDst);
static unsigned vpoxNetFltSolarisMBlkCalcSGSegs(PVPOXNETFLTINS pThis, mblk_t *pMsg);
static int vpoxNetFltSolarisMBlkToSG(PVPOXNETFLTINS pThis, mblk_t *pMsg, PINTNETSG pSG, unsigned cSegs, uint32_t fSrc);
static int vpoxNetFltSolarisRecv(PVPOXNETFLTINS pThis, vpoxnetflt_stream_t *pStream, queue_t *pQueue, mblk_t *pMsg);
/* static mblk_t *vpoxNetFltSolarisFixChecksums(mblk_t *pMsg); */
/* static void vpoxNetFltSolarisAnalyzeMBlk(mblk_t *pMsg); */
...
static dev_info_t *g_pVPoxNetFltSolarisDip = NULL;

/** The (common) global data. */
static VPOXNETFLTGLOBALS g_VPoxNetFltSolarisGlobals;

/** The list of all opened streams. */
vpoxnetflt_stream_t *g_VPoxNetFltSolarisStreams = NULL;

/** Global mutex protecting open/close. */
static RTSEMFASTMUTEX g_VPoxNetFltSolarisMtx = NIL_RTSEMFASTMUTEX;

/** Global credentials using during open/close. */
static cred_t *g_pVPoxNetFltSolarisCred = NULL;

/**
 * g_VPoxNetFltInstance is the current PVPOXNETFLTINS to be associated with the stream being created
 * in ModOpen. This is just shared global data between the dynamic attach and the ModOpen procedure.
 */
PVPOXNETFLTINS volatile g_VPoxNetFltSolarisInstance = NULL;

/** Goes along with the instance to determine type of stream being opened/created. */
VPOXNETFLTSTREAMTYPE volatile g_VPoxNetFltSolarisStreamType = kUndefined;

#ifdef VPOXNETFLT_SOLARIS_IPV6_POLLING
/** Global IPv6 polling interval */
static int g_VPoxNetFltSolarisPollInterval = -1;
#endif
...
static int
vpoxNetFltSolarisCtfGetMemberOffset(ctf_file_t *pCtfFile, const char *pszStruct, const char *pszMember, int *pOffset)
{
...
static int
vpoxNetFltSolarisProbeCtf(void)
{
    /*
    ...
    if (pModCtl)
    {
        ...
        if (pCtfFile)
        {
            rc = vpoxNetFltSolarisCtfGetMemberOffset(pCtfFile, "file_t", "f_vnode", &s_off_vnode);
            ctf_close(pCtfFile);
        }
        else
...
int _init(void)
{
     ...
     */
    modctl_t *pModCtl = mod_getctl(&g_VPoxNetFltSolarisModLinkage);
    if (pModCtl)
    ...
    if (RT_SUCCESS(rc))
    {
        rc = vpoxNetFltSolarisProbeCtf();
        if (RT_SUCCESS(rc))
        {
            /*
             * Initialize Solaris specific globals here.
             */
            g_VPoxNetFltSolarisStreams = NULL;
            g_VPoxNetFltSolarisInstance = NULL;
            g_pVPoxNetFltSolarisCred = crdup(kcred);
            if (RT_LIKELY(g_pVPoxNetFltSolarisCred))
            {
                rc = RTSemFastMutexCreate(&g_VPoxNetFltSolarisMtx);
                if (RT_SUCCESS(rc))
                {
                    /*
                     ...
                     */
                    memset(&g_VPoxNetFltSolarisGlobals, 0, sizeof(g_VPoxNetFltSolarisGlobals));
                    rc = vpoxNetFltInitGlobalsAndIdc(&g_VPoxNetFltSolarisGlobals);
                    if (RT_SUCCESS(rc))
                    {
                        rc = mod_install(&g_VPoxNetFltSolarisModLinkage);
                        if (!rc)
                            return rc;

                        LogRel((DEVICE_NAME ":mod_install failed. rc=%d\n", rc));
                        vpoxNetFltTryDeleteIdcAndGlobals(&g_VPoxNetFltSolarisGlobals);
                    }
                    else
                        LogRel((DEVICE_NAME ":failed to initialize globals.\n"));

                    RTSemFastMutexDestroy(g_VPoxNetFltSolarisMtx);
                    g_VPoxNetFltSolarisMtx = NIL_RTSEMFASTMUTEX;
                }
            }
            else
            {
            ...
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisProbeCtf failed. rc=%d\n", rc));
...
int _fini(void)
{
     ...
     */
    rc = vpoxNetFltTryDeleteIdcAndGlobals(&g_VPoxNetFltSolarisGlobals);
    if (RT_FAILURE(rc))
    {
    ...
    rc = mod_remove(&g_VPoxNetFltSolarisModLinkage);
    if (!rc)
    {
        if (g_pVPoxNetFltSolarisCred)
        {
            crfree(g_pVPoxNetFltSolarisCred);
            g_pVPoxNetFltSolarisCred = NULL;
        }

        if (g_VPoxNetFltSolarisMtx != NIL_RTSEMFASTMUTEX)
        {
            RTSemFastMutexDestroy(g_VPoxNetFltSolarisMtx);
            g_VPoxNetFltSolarisMtx = NIL_RTSEMFASTMUTEX;
        }
...
int _info(struct modinfo *pModInfo)
{
    ...
    int rc = mod_info(&g_VPoxNetFltSolarisModLinkage, pModInfo);
...
static int VPoxNetFltSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ":VPoxNetFltSolarisAttach pDip=%p enmCmd=%d\n", pDip, enmCmd));
    ...
    switch (enmCmd)
    {
        case DDI_ATTACH:
        {
            ...
            if (rc == DDI_SUCCESS)
            {
                g_pVPoxNetFltSolarisDip = pDip;
#ifdef VPOXNETFLT_SOLARIS_IPV6_POLLING
                /*
                 * Get the user prop. for polling interval.
                 */
                int Interval = ddi_getprop(DDI_DEV_T_ANY, pDip, DDI_PROP_DONTPASS, VPOXNETFLT_IP6POLLINTERVAL, -1 /* default */);
                if (Interval == -1)
                    Log((DEVICE_NAME ":vpoxNetFltSolarisSetupIp6Polling: no poll interval property specified. Skipping Ipv6 polling.\n"));
                else if (Interval < 1 || Interval > 120)
                {
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisSetupIp6Polling: Invalid polling interval %d. Expected between 1 and 120 secs.\n",
                                        Interval));
                    Interval = -1;
                }

                g_VPoxNetFltSolarisPollInterval = Interval;
#endif
                ddi_report_dev(pDip);
                return DDI_SUCCESS;
            }
            else
                LogRel((DEVICE_NAME ":VPoxNetFltSolarisAttach failed to create minor node. rc%d\n", rc));
            return DDI_FAILURE;
        }
...
static int VPoxNetFltSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ":VPoxNetFltSolarisDetach pDip=%p enmCmd=%d\n", pDip, enmCmd));
 ...
 * @returns  corresponding solaris error code.
 */
static int VPoxNetFltSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pvArg, void **ppvResult)
{
    LogFunc((DEVICE_NAME ":VPoxNetFltSolarisGetInfo pDip=%p enmCmd=%d pArg=%p instance=%d\n", pDip, enmCmd,
    ...
    switch (enmCmd)
    {
        ...
        {
            *ppvResult = g_pVPoxNetFltSolarisDip;
 ...
 * @returns corresponding solaris error code.
 */
static int VPoxNetFltSolarisModOpen(queue_t *pQueue, dev_t *pDev, int fOpenMode, int fStreamMode, cred_t *pCred)
{
    Assert(pQueue);

    LogFunc((DEVICE_NAME ":VPoxNetFltSolarisModOpen pQueue=%p pDev=%p fOpenMode=%d fStreamMode=%d\n", pQueue, pDev,
    ...
    if (pQueue->q_ptr)
    {
        LogRel((DEVICE_NAME ":VPoxNetFltSolarisModOpen invalid open.\n"));
        return ENOENT;
    }
     ...
     * g_VPoxNetFltSolarisMtx prevent races and that the instance gets
     * associated with the wrong streams.
     */
    if (pCred != g_pVPoxNetFltSolarisCred)
    {
        LogRel((DEVICE_NAME ":VPoxNetFltSolarisModOpen invalid credentials.\n"));
        return EACCES;
    }

    /*
     * Check for the VirtualPox instance.
     */
    PVPOXNETFLTINS pThis = g_VPoxNetFltSolarisInstance;
    if (!pThis)
    {
        LogRel((DEVICE_NAME ":VPoxNetFltSolarisModOpen failed to get VirtualPox instance.\n"));
        return ENOENT;
    }

    /*
     * Check VirtualPox stream type.
     */
    if (   g_VPoxNetFltSolarisStreamType != kPromiscStream
        && g_VPoxNetFltSolarisStreamType != kArpStream
        && g_VPoxNetFltSolarisStreamType != kIp6Stream
        && g_VPoxNetFltSolarisStreamType != kIp4Stream)
    {
        LogRel((DEVICE_NAME ":VPoxNetFltSolarisModOpen failed due to undefined VirtualPox open mode. Type=%d\n",
                g_VPoxNetFltSolarisStreamType));
        return ENOENT;
    }
     ...
     */
    minor_t DevMinor = 0;
    vpoxnetflt_stream_t *pStream = NULL;
    vpoxnetflt_stream_t **ppPrevStream = &g_VPoxNetFltSolarisStreams;
    ...
    if (g_VPoxNetFltSolarisStreamType == kPromiscStream)
    {
        vpoxnetflt_promisc_stream_t *pPromiscStream = RTMemAlloc(sizeof(vpoxnetflt_promisc_stream_t));
        if (RT_UNLIKELY(!pPromiscStream))
        {
            LogRel((DEVICE_NAME ":VPoxNetFltSolarisModOpen failed to allocate promiscuous stream data.\n"));
            return ENOMEM;
        }
...
#endif
        pStream = (vpoxnetflt_stream_t *)pPromiscStream;
    }
    else
    {
        /*
         ...
         */
        pStream = RTMemAlloc(sizeof(vpoxnetflt_stream_t));
        if (RT_UNLIKELY(!pStream))
        {
            LogRel((DEVICE_NAME ":VPoxNetFltSolarisModOpen failed to allocate stream data.\n"));
            return ENOMEM;
        }
    }
    ...
    /*
     * Pick up the current global VPOXNETFLTINS instance as
     * the one that we will associate this stream with.
     */
    ASMAtomicUoWritePtr(&pStream->pThis, pThis);
    pStream->Type = g_VPoxNetFltSolarisStreamType;
    switch (pStream->Type)
    {
        ...
        default:    /* Heh. */
        {
            LogRel((DEVICE_NAME ":VPoxNetFltSolarisModOpen huh!? Invalid stream type %d\n", pStream->Type));
            RTMemFree(pStream);
            return EINVAL;
        }
    }
    ...
    *ppPrevStream = pStream;
    ...
    {
        vpoxnetflt_promisc_stream_t *pPromiscStream = (vpoxnetflt_promisc_stream_t *)pStream;
         ...
         */
        int rc = vpoxNetFltSolarisBindReq(pStream->pReadQueue, 0 /* SAP */);
        if (RT_LIKELY(RT_SUCCESS(rc)))
        {
            /*
             * Request the physical address (we cache the acknowledgement).
             */
            rc = vpoxNetFltSolarisPhysAddrReq(pStream->pReadQueue);
            if (RT_LIKELY(RT_SUCCESS(rc)))
            {
                /*
                 * Ask for DLPI link notifications, don't bother check for errors here.
                 */
                vpoxNetFltSolarisNotifyReq(pStream->pReadQueue);

                /*
                 * Enable raw mode.
                 */
                rc = vpoxNetFltSolarisSetRawMode(pPromiscStream);
                if (RT_FAILURE(rc))
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisSetRawMode failed rc=%Rrc.\n", rc));
            }
            else
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisSetRawMode failed rc=%Rrc.\n", rc));
        }
        else
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisBindReq failed rc=%Rrc.\n", rc));
    }

    NOREF(fOpenMode);

    Log((DEVICE_NAME ":VPoxNetFltSolarisModOpen returns 0, DevMinor=%d pQueue=%p\n", DevMinor, pStream->pReadQueue));
 ...
 * @returns  corresponding solaris error code.
 */
static int VPoxNetFltSolarisModClose(queue_t *pQueue, int fOpenMode, cred_t *pCred)
{
    Assert(pQueue);

    LogFunc((DEVICE_NAME ":VPoxNetFltSolarisModClose pQueue=%p fOpenMode=%d\n", pQueue, fOpenMode));

    vpoxnetflt_stream_t *pStream = NULL;
    vpoxnetflt_stream_t **ppPrevStream = NULL;

    /*
     * Get instance data.
     */
    pStream = (vpoxnetflt_stream_t *)pQueue->q_ptr;
    if (RT_UNLIKELY(!pStream))
    {
        LogRel((DEVICE_NAME ":VPoxNetFltSolarisModClose failed to get stream.\n"));
        return ENXIO;
    }
    ...
    if (pStream->Type == kPromiscStream)
    {
         ...
         */
        PVPOXNETFLTPACKETID pCur = pPromiscStream->pHead;
        while (pCur)
        {
            PVPOXNETFLTPACKETID pNext = pCur->pNext;
            RTMemFree(pCur);
            pCur = pNext;
        }
        pPromiscStream->pHead = NULL;
     ...
     */
    for (ppPrevStream = &g_VPoxNetFltSolarisStreams; (pStream = *ppPrevStream) != NULL; ppPrevStream = &pStream->pNext)
        if (pStream == (vpoxnetflt_stream_t *)pQueue->q_ptr)
            break;
    *ppPrevStream = pStream->pNext;
     ...
     */
    vpoxNetFltRelease(pStream->pThis, false /* fBusy */);
...
static int VPoxNetFltSolarisModReadPut(queue_t *pQueue, mblk_t *pMsg)
{
    ...
    LogFunc((DEVICE_NAME ":VPoxNetFltSolarisModReadPut pQueue=%p pMsg=%p\n", pQueue, pMsg));

    bool fSendUpstream = true;
    vpoxnetflt_stream_t *pStream = pQueue->q_ptr;
    PVPOXNETFLTINS pThis = NULL;

    /*
     * In the unlikely case where VirtualPox crashed and this filter
    ...
    {
        fSendUpstream = false;
        pThis = ASMAtomicUoReadPtrT(&pStream->pThis, PVPOXNETFLTINS);
        if (RT_LIKELY(pThis))
        {
            /*
            ...
            vpoxNetFltRetain(pThis, true /* fBusy */);
            ...
            switch (DB_TYPE(pMsg))
            {
                case M_DATA:
                {
                    Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut M_DATA\n"));

                    if (   fActive
                        && pPromiscStream->fRawMode)
                    {
                        vpoxNetFltSolarisRecv(pThis, pStream, pQueue, pMsg);
                    }
                    break;
                }
                ...
                case M_PCPROTO:
                {
                    ...
                    Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: M_PCPROTO %d\n", Prim));
                    switch (Prim)
                    {
                        case DL_NOTIFY_IND:
                        {
                            ...
                            {
                                LogRel((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: Invalid notification size; expected>=%d"
                            ...
                            switch (pNotifyInd->dl_notification)
                            {
                                case DL_NOTE_PHYS_ADDR:
                                {
                                    ...
                                    if (!cOffset || !cbAddr)
                                    {
                                        LogRel((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: DL_NOTE_PHYS_ADDR."
                                                "Invalid offset/addr.\n"));
                                        fSendUpstream = false;
                                        break;
                                    }
                                    ...
                                    Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: DL_NOTE_PHYS_ADDR. New Mac=%.*Rhxs\n",
                                ...
                                case DL_NOTE_LINK_UP:
                                {
                                    if (ASMAtomicXchgBool(&pThis->fDisconnectedFromHost, false))
                                        Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: DL_NOTE_LINK_UP.\n"));
                                    break;
                                }
                                ...
                                case DL_NOTE_LINK_DOWN:
                                {
                                    if (!ASMAtomicXchgBool(&pThis->fDisconnectedFromHost, true))
                                        Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: DL_NOTE_LINK_DOWN.\n"));
                                    break;
                                }
                            }
                            break;
                        ...
                        case DL_BIND_ACK:
                        {
                             ...
                             */
                            Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: DL_BIND_ACK. Bound to requested SAP!\n"));
                            break;
                        }
                        ...
                        case DL_PHYS_ADDR_ACK:
                        {
                             ...
                             */
                            vpoxNetFltSolarisCachePhysAddr(pThis, pMsg);
                            break;
                        }
                        ...
                        case DL_OK_ACK:
                        {
                            ...
                            {
                                Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: M_PCPROTO: DL_OK_ACK: fPromisc is ON.\n"));
                                pPromiscStream->fPromisc = true;
                            }
                            else if (pOkAck->dl_correct_primitive == DL_PROMISCOFF_REQ)
                            {
                                Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: M_PCPROTO: DL_OK_ACK: fPromisc is OFF.\n"));
                                pPromiscStream->fPromisc = false;
                            }
                            break;
                        }
                ...
                case M_IOCACK:
                {
                    ...
                    if (pIOC->ioc_id == pPromiscStream->ModeReqId)
                    {
                        ...
                        Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: Mode acknowledgement. RawMode is %s\n",
                ...
                case M_IOCNAK:
                {
                    ...
                    {
                        pPromiscStream->fRawMode = false;
                        LogRel((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: WARNING! Mode not acknowledged. RawMode is %s\n",
                                pPromiscStream->fRawMode ? "ON" : "OFF"));
                    }
                    break;
                }
                ...
                case M_FLUSH:
                {
                     ...
                     */
                    Log((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: M_FLUSH\n"));
            ...
            LogRel((DEVICE_NAME ":VPoxNetFltSolarisModReadPut: Could not find VirtualPox instance!!\n"));
    }
...
static int VPoxNetFltSolarisModWritePut(queue_t *pQueue, mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":VPoxNetFltSolarisModWritePut pQueue=%p pMsg=%p\n", pQueue, pMsg));
...
static int vpoxNetFltSolarisSetRawMode(vpoxnetflt_promisc_stream_t *pPromiscStream)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisSetRawMode pPromiscStream=%p\n", pPromiscStream));
...
static int vpoxNetFltSolarisSetFastMode(queue_t *pQueue)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisSetFastMode pQueue=%p\n", pQueue));
    ...
    size_t cbReq = sizeof(dl_unitdata_req_t) + sizeof(vpoxnetflt_dladdr_t);
    ...
    pDataReq->dl_dest_addr_length = sizeof(vpoxnetflt_dladdr_t);
    pDataReq->dl_dest_addr_offset = sizeof(dl_unitdata_req_t);
    pDataReq->dl_priority.dl_min = 0;
    pDataReq->dl_priority.dl_max = 0;

    bzero(pDataReqMsg->b_rptr + sizeof(dl_unitdata_req_t), sizeof(vpoxnetflt_dladdr_t));
 ...
 * @returns VPox status code.
 */
static int vpoxNetFltSolarisPromiscReq(queue_t *pQueue, bool fPromisc)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisPromiscReq pQueue=%p fPromisc=%d\n", pQueue, fPromisc));
 ...
 *                          vpoxNetFltSolarisPromiscReqWrap().
 */
static void vpoxNetFltSolarisPromiscReqWrapExcl(queue_t *pQueue, mblk_t *pMsg)
{
    /*
     * Paranoia.
     */
    ...
    if (RT_UNLIKELY(!pMsg))
        LogRel((DEVICE_NAME ":VPoxNetFltSolarisPromiscReqWrapExcl pQueue=%p missing message!\n", pQueue));

    bool fPromisc = (MBLKL(pMsg) == 1);
    freemsg(pMsg);
    pMsg = NULL;
    int rc = vpoxNetFltSolarisPromiscReq(pQueue, fPromisc);
    if (RT_FAILURE(rc))
        LogRel((DEVICE_NAME ":VPoxNetFltSolarisPromiscReqWrapExcl vpoxNetFltSolarisPromiscReq failed. rc=%d\n", rc));
}
 ...
 *
 * @param pvData            Pointer to vpoxnetflt_promisc_params_t. See
 *                          vpoxNetFltPortOsSetActive().
 */
static void vpoxNetFltSolarisPromiscReqWrap(void *pvData)
{
    vpoxnetflt_promisc_params_t *pParams = pvData;
    if (RT_LIKELY(pParams))
    {
        PVPOXNETFLTINS pThis = pParams->pThis;
        vpoxnetflt_promisc_stream_t *pPromiscStream = ASMAtomicUoReadPtrT(&pThis->u.s.pPromiscStream,
                                                                          vpoxnetflt_promisc_stream_t *);
        if (   pPromiscStream
            && pPromiscStream->Stream.pReadQueue)
        {
             ...
             */
            qwriter(WR(pPromiscStream->Stream.pReadQueue), pMsg, vpoxNetFltSolarisPromiscReqWrapExcl, PERIM_OUTER);
            ASMAtomicWritePtr(&pPromiscStream->TimeoutId, NULL);
        }
        RTMemFree(pParams);
    }
...
static int vpoxNetFltSolarisPhysAddrReq(queue_t *pQueue)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisPhysAddrReq pQueue=%p\n", pQueue));
...
/**
 * Cache the MAC address into the VirtualPox instance given a physical
 * address acknowledgement message.
 *
 * @param   pThis       The instance.
 * @param   pMsg        Pointer to the physical address acknowledgement message.
 */
static void vpoxNetFltSolarisCachePhysAddr(PVPOXNETFLTINS pThis, mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisCachePhysAddr pThis=%p pMsg=%p\n", pThis, pMsg));
    ...
    if (pPhysAddrAck->dl_addr_length == sizeof(pThis->u.s.MacAddr))
    {
        ...
        Log((DEVICE_NAME ":vpoxNetFltSolarisCachePhysAddr: DL_PHYS_ADDR_ACK: Mac=%.*Rhxs\n",
                 sizeof(pThis->u.s.MacAddr), &pThis->u.s.MacAddr));

        if (vpoxNetFltTryRetainBusyNotDisconnected(pThis))
        {
    ...
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisCachePhysAddr: Invalid address size. expected=%d got=%d\n", ETHERADDRL,
...
static int vpoxNetFltSolarisBindReq(queue_t *pQueue, int SAP)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisBindReq SAP=%d\n", SAP));
...
static int vpoxNetFltSolarisNotifyReq(queue_t *pQueue)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisNotifyReq\n"));
 ...
 */
static int vpoxNetFltSolarisOpenDev(char *pszDev, vnode_t **ppVNode, vnode_t **ppVNodeHeld, TIUSER **ppUser)
{
    int rc;
        ...
        && pVNodeHeld)
    {
        ...
        if (!rc)
        {
            ...
            {
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenDev failed. pUser=%p fp=%p f_vnode=%p\n", pUser,
            ...
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenDev t_kopen failed. rc=%d\n", rc));

        VN_RELE(pVNodeHeld);
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenDev lookupname failed. rc=%d pVNodeHeld=%p\n", rc, pVNodeHeld));
...
/**
 * Close the device opened using vpoxNetFltSolarisOpenDev.
 *
 * @param   pVNodeHeld      Pointer to the held vnode of the device.
 * @param   pUser           Pointer to the file handle.
 */
static void vpoxNetFltSolarisCloseDev(vnode_t *pVNodeHeld, TIUSER *pUser)
{
    t_kclose(pUser, 0);
    VN_RELE(pVNodeHeld);
}
 ...
 */
static int vpoxNetFltSolarisAttachReq(ldi_handle_t hDevice, int PPA)
{
    int rc;
    ...
    if (!rc)
    {
        ...
        if (!rc)
        {
            ...
            if (cbMsg >= sizeof(t_uscalar_t))
            {
                ...
                {
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachReq ldi_getmsg succeeded, but unsupported op.\n"));
                    rc = VERR_NOT_SUPPORTED;
                }
                else                                            /* Garbled reply */
                {
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachReq ldi_getmsg succeeded, but invalid op."
                            " expected %d recvd %d\n", DL_OK_ACK, AckPrim));
            ...
            {
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachReq ldi_getmsg succeeded, but invalid size %d expected %d\n", cbMsg,
                            DL_OK_ACK_SIZE));
                rc = VERR_INVALID_FUNCTION;
            }
        }
        ...
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachReq ldi_getmsg failed. rc=%d\n", rc));
            rc = VERR_INVALID_FUNCTION;
        }
    }
    else
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachReq ldi_putmsg failed. rc=%d\n", rc));
 ...
 */
static int vpoxNetFltSolarisGetIfFlags(ldi_handle_t hDevice, struct lifreq *pInterface)
{
 ...
 * @param   pInterface  Pointer to the interface.
 */
static int vpoxNetFltSolarisSetMuxId(vnode_t *pVNode, struct lifreq *pInterface)
{
    struct strioctl IOCReq;
    int rc;
    int ret;
 ...
 */
static int vpoxNetFltSolarisMuxIdToFd(vnode_t *pVNode, int MuxId, int *pFd)
{
    int ret;
 ...
 */
static int vpoxNetFltSolarisRelinkIp4(vnode_t *pVNode, struct lifreq *pInterface, int IpMuxFd, int ArpMuxFd)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisRelinkIp4: pVNode=%p pInterface=%p IpMuxFd=%d ArpMuxFd=%d\n", pVNode,
        ...
        && !rc2)
    {
        ...
        rc = vpoxNetFltSolarisSetMuxId(pVNode, pInterface);
        if (RT_SUCCESS(rc))
            return VINF_SUCCESS;

        LogRel((DEVICE_NAME ":vpoxNetFltSolarisRelinkIp4: failed to set new Mux Id.\n"));
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisRelinkIp4: failed to link.\n"));
 ...
 */
static int vpoxNetFltSolarisRelinkIp6(vnode_t *pVNode, struct lifreq *pInterface, int Ip6MuxFd)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisRelinkIp6: pVNode=%p pInterface=%p Ip6MuxFd=%d\n", pVNode, pInterface, Ip6MuxFd));
    ...
    if (!rc)
    {
        ...
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisRelinkIp6: failed to set new Mux Id.\n"));
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisRelinkIp6: failed to link.\n"));
 ...
 */
static int vpoxNetFltSolarisDetermineModPos(bool fAttach, vnode_t *pVNode, int *pModPos)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisDetermineModPos: fAttach=%d pVNode=%p pModPos=%p\n", fAttach, pVNode, pModPos));
    ...
    if (!rc)
    {
        if (cMod < 1)
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisDetermineModPos: too few modules on host interface. cMod=%d\n"));
         ...
         * the host driver.
         */
        Log((DEVICE_NAME ":vpoxNetFltSolarisDetermineModPos: cMod=%d\n", cMod));
        if (fAttach)
        {
            *pModPos = cMod - 1;
            return VINF_SUCCESS;
        ...
        {
            Log((DEVICE_NAME ":vpoxNetFltSolarisDetermineModPos: failed to alloc memory for StrList.\n"));
            return VERR_NO_MEMORY;
        }
        ...
        if (!rc)
        {
            ...
            for (int i = 0; i < StrList.sl_nmods; i++)
            {
                ...
                {
                    Log((DEVICE_NAME ":vpoxNetFltSolarisDetermineModPos: Success! Found %s at %d.\n", DEVICE_NAME, i));
                    *pModPos = i;
            ...
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisDetermineModPos: failed to find %s in the host stack.\n", DEVICE_NAME));
        }
        else
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisDetermineModPos: failed to get module information. rc=%d\n"));

        RTMemFree(StrList.sl_modlist);
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisDetermineModPos: failed to get list of modules on host interface. rc=%d\n", rc));
    return VERR_GENERAL_FAILURE;
}
 ...
 */
static int vpoxNetFltSolarisOpenStyle2(PVPOXNETFLTINS pThis, ldi_ident_t *pDevId)
{
    /*
        ...
        && ddi_strtol(pszEnd, NULL, 10, &PPA) == 0)
    {
        *pszEnd = '\0';
        ...
        if (!rc)
        {
             ...
             */
            rc = vpoxNetFltSolarisAttachReq(pThis->u.s.hIface, (int)PPA);
            if (RT_SUCCESS(rc))
            {
                RTStrFree(pszDev);
                return rc;
            ...
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenStyle2 dl_attach failed. rc=%d szDev=%s PPA=%d rc=%d\n", rc, szDev, PPA));
        }
        else
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenStyle2 Failed to open. rc=%d szDev=%s PPA=%d\n", rc, szDev, PPA));
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenStyle2 Failed to construct PPA. pszDev=%s pszEnd=%s.\n", pszDev, pszEnd));
 ...
 * @param   pThis       The instance.
 */
static int vpoxNetFltSolarisOpenStream(PVPOXNETFLTINS pThis)
{
    ldi_ident_t DevId;
    ...
    if (rc)
    {
         ...
         * Try DLPI style 2.
         */
        rc = vpoxNetFltSolarisOpenStyle2(pThis, &DevId);
        if (RT_FAILURE(rc))
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenStream vpoxNetFltSolarisOpenStyle2 failed. rc=%d\n", rc));
        else
            rc = 0;
    }
    ...
    if (rc)
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenStream Failed to open '%s' rc=%d pszName='%s'\n", szDev, rc, pThis->szName));
        return VERR_INTNET_FLT_IF_FAILED;
    }
    ...
    if (!rc)
    {
        if (!ret)
        {
            if (RT_LIKELY(g_pVPoxNetFltSolarisCred))        /* Paranoia */
            {
                rc = RTSemFastMutexRequest(g_VPoxNetFltSolarisMtx);
                AssertRCReturn(rc, rc);

                g_VPoxNetFltSolarisInstance = pThis;
                g_VPoxNetFltSolarisStreamType = kPromiscStream;

                rc = ldi_ioctl(pThis->u.s.hIface, I_PUSH, (intptr_t)DEVICE_NAME, FKIOCTL, g_pVPoxNetFltSolarisCred, &ret);

                g_VPoxNetFltSolarisInstance = NULL;
                g_VPoxNetFltSolarisStreamType = kUndefined;

                RTSemFastMutexRelease(g_VPoxNetFltSolarisMtx);
            }
            else
            {
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenStream huh!? Missing credentials.\n"));
            ...
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenStream Failed to push filter onto host interface '%s'\n", pThis->szName));
        }
        else
            return VINF_SUCCESS;
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisOpenStream Failed to search for filter in interface '%s'.\n", pThis->szName));
 ...
 * @param   pThis       The instance.
 */
static void vpoxNetFltSolarisCloseStream(PVPOXNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisCloseStream pThis=%p\n"));
 ...
 * @param   fAttach     Is this an attach or detach.
 */
static int vpoxNetFltSolarisAttachIp4(PVPOXNETFLTINS pThis, bool fAttach)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4 pThis=%p fAttach=%d\n", pThis, fAttach));
    ...
    if (rc)
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to open the IP stream on '%s'.\n", pThis->szName));
    ...
    if (rc)
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to open the ARP stream on '%s'.\n", pThis->szName));
        ldi_ident_release(DeviceIdent);
     ...
     */
    rc = vpoxNetFltSolarisGetIfFlags(Ip4DevHandle, &Ip4Interface);
    if (RT_SUCCESS(rc))
    {
        /*
        ...
        rc = vpoxNetFltSolarisOpenDev(UDP_DEV_NAME, &pUdp4VNode, &pUdp4VNodeHeld, &pUdp4User);
        if (RT_SUCCESS(rc))
        {
            /*
            ...
            if (!rc)
            {
                ...
                int Ip4MuxFd;
                int ArpMuxFd;
                rc = vpoxNetFltSolarisMuxIdToFd(pUdp4VNode, Ip4Interface.lifr_ip_muxid, &Ip4MuxFd);
                rc2 = vpoxNetFltSolarisMuxIdToFd(pUdp4VNode, Ip4Interface.lifr_arp_muxid, &ArpMuxFd);
                if (   RT_SUCCESS(rc)
                    && RT_SUCCESS(rc2))
                {
                    /*
                        ...
                        && !rc2)
                    {
                            ...
                            && VNODE_FOR_FILE_T(pIpFile))
                        {
                             ...
                             */
                            rc = vpoxNetFltSolarisDetermineModPos(fAttach, pIp4VNode, &StrMod.pos);
                            rc2 = vpoxNetFltSolarisDetermineModPos(fAttach, pArpVNode, &ArpStrMod.pos);
                            if (   RT_SUCCESS(rc)
                                && RT_SUCCESS(rc2))
                            {
                                /*
                                ...
                                if (!rc)
                                {
                                    ...
                                    if (!rc)
                                    {
                                         ...
                                         */
                                        rc = vpoxNetFltSolarisRelinkIp4(pUdp4VNode, &Ip4Interface, Ip4MuxFd, ArpMuxFd);
                                        if (RT_SUCCESS(rc))
                                        {
                                            /*
                                             ...
                                             */
                                            vpoxNetFltSolarisCloseDev(pUdp4VNodeHeld, pUdp4User);
                                            ldi_close(ArpDevHandle, FREAD | FWRITE, kcred);
                                            ldi_close(Ip4DevHandle, FREAD | FWRITE, kcred);
                                            releasef(Ip4MuxFd);
                                            releasef(ArpMuxFd);

                                            Log((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: Success! %s %s@(IPv4:%d Arp:%d) "
                                        ...
                                        {
                                            LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: Relinking failed. Mode=%s rc=%d.\n",
                                    ...
                                    {
                                        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to %s the ARP stack. rc=%d\n",
                                    ...
                                    vpoxNetFltSolarisRelinkIp4(pUdp4VNode, &Ip4Interface, Ip4MuxFd, ArpMuxFd);
                                }
                                else
                                {
                                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to %s the IP stack. rc=%d\n",
                                            fAttach ? "inject into" : "eject from", rc));
                                }
                            }
                            else
                            {
                                LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to find position. rc=%d rc2=%d\n", rc,
                                        rc2));
                            }
                            ...
                            LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to get vnode from MuxFd.\n"));
                    }
                    else
                    {
                        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to unlink upper stream rc=%d rc2=%d.\n", rc,
                                rc2));
                    }
                }
                else
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to get MuxFd from MuxId. rc=%d rc2=%d\n", rc, rc2));
            }
            else
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to get Mux Ids. rc=%d\n", rc));
            vpoxNetFltSolarisCloseDev(pUdp4VNodeHeld, pUdp4User);
        }
        else
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: failed to open UDP. rc=%d\n", rc));
    ...
    else
    {
         ...
         */
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp4: Warning: seems '%s' is unplumbed.\n", pThis->szName));
        rc = VINF_SUCCESS;
    }
 ...
 * @param   fAttach     Is this an attach or detach.
 */
static int vpoxNetFltSolarisAttachIp6(PVPOXNETFLTINS pThis, bool fAttach)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6 pThis=%p fAttach=%d\n", pThis, fAttach));
    ...
    if (rc)
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to open the IPv6 stream on '%s'.\n", pThis->szName));
        return VERR_INTNET_FLT_IF_FAILED;
    }
     ...
     */
    rc = vpoxNetFltSolarisGetIfFlags(Ip6DevHandle, &Ip6Interface);
    if (RT_SUCCESS(rc))
    {
        /*
         * Open the UDP stream. We sort of cheat here and obtain the vnode so that we can perform
        ...
        rc = vpoxNetFltSolarisOpenDev(UDP6_DEV_NAME, &pUdp6VNode, &pUdp6VNodeHeld, &pUdp6User);
        if (RT_SUCCESS(rc))
        {
            /*
             * Get the multiplexor IDs.
            ...
            if (!rc)
            {
                 ...
                 */
                int Ip6MuxFd;
                rc = vpoxNetFltSolarisMuxIdToFd(pUdp6VNode, Ip6Interface.lifr_ip_muxid, &Ip6MuxFd);
                if (RT_SUCCESS(rc))
                {
                    /*
                     * We need to I_PUNLINK on these multiplexor IDs before we can start
                    ...
                    if (!rc)
                    {
                            ...
                            && VNODE_FOR_FILE_T(pIpFile))
                        {
                             ...
                             * Find the position on the host stack for attaching/detaching ourselves.
                             */
                            rc = vpoxNetFltSolarisDetermineModPos(fAttach, pIp6VNode, &StrMod.pos);
                            if (RT_SUCCESS(rc))
                            {
                                /*
                                 * Set global data which will be grabbed by ModOpen.
                                ...
                                if (!rc)
                                {
                                     ...
                                     * otherwise we've pretty much screwed up the host interface.
                                     */
                                    rc = vpoxNetFltSolarisRelinkIp6(pUdp6VNode, &Ip6Interface, Ip6MuxFd);
                                    if (RT_SUCCESS(rc))
                                    {
                                        /*
                                         * Close the devices ONLY during the return from function case; otherwise
                                         * we end up close twice which is an instant kernel panic.
                                         */
                                        vpoxNetFltSolarisCloseDev(pUdp6VNodeHeld, pUdp6User);
                                        ldi_close(Ip6DevHandle, FREAD | FWRITE, kcred);
                                        releasef(Ip6MuxFd);

                                        Log((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: Success! %s %s@(IPv6:%d) "
                                                "%s interface %s\n", fAttach ? "Injected" : "Ejected", StrMod.mod_name,
                                    ...
                                    {
                                        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: Relinking failed. Mode=%s rc=%d.\n",
                                                fAttach ? "inject" : "eject", rc));
                                    }
                                    ...
                                    vpoxNetFltSolarisRelinkIp6(pUdp6VNode, &Ip6Interface, Ip6MuxFd);
                                }
                                else
                                {
                                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to %s the IP stack. rc=%d\n",
                                            fAttach ? "inject into" : "eject from", rc));
                                }
                            }
                            else
                                LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to find position. rc=%d\n", rc));

                            releasef(Ip6MuxFd);
                        }
                        else
                             LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to get vnode from MuxFd.\n"));
                    }
                    else
                        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to unlink upper stream rc=%d.\n", rc));
                }
                else
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to get MuxFd from MuxId. rc=%d\n", rc));
            }
            else
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to get Mux Ids. rc=%d\n", rc));

            vpoxNetFltSolarisCloseDev(pUdp6VNodeHeld, pUdp6User);
        }
        else
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to open UDP. rc=%d\n", rc));

        rc = VERR_INTNET_FLT_IF_FAILED;
    }
    else
    {
        Log((DEVICE_NAME ":vpoxNetFltSolarisAttachIp6: failed to get IPv6 flags.\n", pThis->szName));
 ...
 */
static void vpoxNetFltSolarispIp6Timer(PRTTIMER pTimer, void *pvData, uint64_t iTick)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarispIp6Timer pTimer=%p pvData=%p\n", pTimer, pvData));

    PVPOXNETFLTINS pThis = (PVPOXNETFLTINS)pvData;
    if (   RT_LIKELY(pThis)
        && RT_LIKELY(pTimer))
    {
        vpoxnetflt_stream_t *pIp6Stream = ASMAtomicUoReadPtrT(&pThis->u.s.pIp6Stream, vpoxnetflt_stream_t *);
            ...
            && !fIp6Attaching)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                vpoxNetFltSolarisAttachIp6(pThis, true /* fAttach */);

                ASMAtomicUoWriteBool(&pThis->u.s.fAttaching, false);
                RTSemFastMutexRelease(pThis->u.s.hPollMtx);
            }
            else
                LogRel((DEVICE_NAME ":vpoxNetFltSolarispIp6Timer failed to obtain mutex. rc=%Rrc\n", rc));
        }
    }
 ...
 * @param   pThis           The instance.
 */
static int vpoxNetFltSolarisSetupIp6Polling(PVPOXNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisSetupIp6Polling pThis=%p\n", pThis));

    int rc = VERR_GENERAL_FAILURE;
    vpoxnetflt_promisc_stream_t *pPromiscStream = ASMAtomicUoReadPtrT(&pThis->u.s.pPromiscStream, vpoxnetflt_promisc_stream_t *);
    if (RT_LIKELY(pPromiscStream))
    {
        if (RT_LIKELY(pPromiscStream->pIp6Timer == NULL))
        {
             ...
             */
            int Interval = g_VPoxNetFltSolarisPollInterval;
            if (Interval < 1 || Interval > 120)
            {
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisSetupIp6Polling: Invalid polling interval %d. Expected between"
                        " 1 and 120 secs.\n", Interval));
            ...
            rc = RTTimerCreateEx(&pPromiscStream->pIp6Timer, Interval * (uint64_t)1000000000, RTTIMER_FLAGS_CPU_ANY,
                                vpoxNetFltSolarispIp6Timer, (void *)pThis);
            if (RT_SUCCESS(rc))
            {
                rc = RTTimerStart(pPromiscStream->pIp6Timer, 10 * (uint64_t)1000000000 /* 10 seconds to blastoff */);
                Log((DEVICE_NAME ":vpoxNetFltSolarisSetupIp6Polling: Ipv6 %d second timer begins firing in 10 seconds.\n",
                     Interval));
            }
            else
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisSetupIp6Polling: Failed to create timer. rc=%d\n", rc));
        }
        else
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisSetupIp6Polling: Polling already started.\n"));
            rc = VINF_SUCCESS;
        }
    }
    return rc;
 ...
 *          would panic the system (e.g. in vpoxNetFltSolarisFindInstance).
 */
static int vpoxNetFltSolarisDetachFromInterface(PVPOXNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisDetachFromInterface pThis=%p\n", pThis));

    ASMAtomicWriteBool(&pThis->fDisconnectedFromHost, true);
    vpoxNetFltSolarisCloseStream(pThis);
    int rc = VINF_SUCCESS;
    if (pThis->u.s.pIp4Stream)
        rc = vpoxNetFltSolarisAttachIp4(pThis, false /* fAttach */);
    if (pThis->u.s.pIp6Stream)
        rc = vpoxNetFltSolarisAttachIp6(pThis, false /* fAttach */);
 ...
 * @param   pThis           The instance.
 */
static int vpoxNetFltSolarisAttachToInterface(PVPOXNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisAttachToInterface pThis=%p\n", pThis));
     ...
     */
    ASMAtomicWriteBool(&pThis->fDisconnectedFromHost, true);
    int rc = vpoxNetFltSolarisOpenStream(pThis);
    if (RT_SUCCESS(rc))
    {
        rc = vpoxNetFltSolarisAttachIp4(pThis, true /* fAttach */);
        if (RT_SUCCESS(rc))
        {
            /*
             ...
             */
            int rc2 = vpoxNetFltSolarisAttachIp6(pThis, true /* fAttach */);
            ...
            if (   rc2 == VERR_INTNET_FLT_IF_NOT_FOUND
                && g_VPoxNetFltSolarisPollInterval != -1)
            {
                int rc3 = vpoxNetFltSolarisSetupIp6Polling(pThis);
                if (RT_FAILURE(rc3))
                {
                    /*
                     * If we failed to setup Ip6 polling, warn in the release log and continue.
                     */
                    LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachToInterface IPv6 polling inactive. rc=%Rrc\n", rc3));
                }
            }
#endif
        ...
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachToInterface vpoxNetFltSolarisOpenStream failed rc=%Rrc\n", rc));
...
static mblk_t *vpoxNetFltSolarisMBlkFromSG(PVPOXNETFLTINS pThis, PINTNETSG pSG, uint32_t fDst)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisMBlkFromSG pThis=%p pSG=%p\n", pThis, pSG));

    mblk_t *pMsg = allocb(pSG->cbTotal, BPRI_MED);
    if (RT_UNLIKELY(!pMsg))
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisMBlkFromSG failed to alloc %d bytes for mblk_t.\n", pSG->cbTotal));
        return NULL;
    }
...
static int vpoxNetFltSolarisMBlkToSG(PVPOXNETFLTINS pThis, mblk_t *pMsg, PINTNETSG pSG, unsigned cSegs, uint32_t fSrc)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisMBlkToSG pThis=%p pMsg=%p pSG=%p cSegs=%d\n", pThis, pMsg, pSG, cSegs));
    ...
    {
        Log((DEVICE_NAME ":vpoxNetFltSolarisMBlkToSG pulling up to length.\n"));
    ...
    Log((DEVICE_NAME ":vpoxNetFltSolarisMBlkToSG iSeg=%d pSG->cbTotal=%d msgdsize=%d\n", iSeg, pSG->cbTotal, msgdsize(pMsg)));
    return VINF_SUCCESS;
}
...
static int vpoxNetFltSolarisRawToUnitData(mblk_t *pMsg, mblk_t **ppDlpiMsg)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisRawToUnitData pMsg=%p\n", pMsg));

    if (DB_TYPE(pMsg) != M_DATA)
        return VERR_NO_MEMORY;

    size_t cbMsg = sizeof(dl_unitdata_ind_t) + 2 * sizeof(vpoxnetflt_dladdr_t);
    ...
    pDlpiData->dl_dest_addr_length = VPOXNETFLT_DLADDRL;
    pDlpiData->dl_dest_addr_offset = sizeof(dl_unitdata_ind_t);
    pDlpiData->dl_src_addr_length = VPOXNETFLT_DLADDRL;
    pDlpiData->dl_src_addr_offset = VPOXNETFLT_DLADDRL + sizeof(dl_unitdata_ind_t);

    PRTNETETHERHDR pEthHdr = (PRTNETETHERHDR)pMsg->b_rptr;

    vpoxnetflt_dladdr_t *pDlAddr = (vpoxnetflt_dladdr_t *)(pDlpiMsg->b_rptr + pDlpiData->dl_dest_addr_offset);
    pDlAddr->SAP = RT_BE2H_U16(pEthHdr->EtherType);
    bcopy(&pEthHdr->DstMac, &pDlAddr->Mac, sizeof(RTMAC));

    pDlAddr = (vpoxnetflt_dladdr_t *)(pDlpiMsg->b_rptr + pDlpiData->dl_src_addr_offset);
    pDlAddr->SAP = RT_BE2H_U16(pEthHdr->EtherType);
...
static int vpoxNetFltSolarisUnitDataToRaw(PVPOXNETFLTINS pThis, mblk_t *pMsg, mblk_t **ppRawMsg)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisUnitDataToRaw pMsg=%p\n", pMsg));

    if (   !pMsg->b_cont
        || DB_TYPE(pMsg) != M_PROTO)
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisUnitDataToRaw invalid input message.\n"));
    ...
    switch (pPrim->dl_primitive)
    {
        case DL_UNITDATA_IND:
        {
            ...
            vpoxnetflt_dladdr_t *pDLSapAddr = (vpoxnetflt_dladdr_t *)(pMsg->b_rptr + pDlpiMsg->dl_dest_addr_offset);
        ...
        default:
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisUnitDataToRaw Unknown M_PROTO. This shouldn't be happening!!"));
            return VERR_NET_PROTOCOL_ERROR;
        }
    }
     ...
     * The vpoxNetFltSolarisMBlkToSG function will handle chained mblk_t's.
     */
...
static int vpoxNetFltSolarisQueueLoopback(PVPOXNETFLTINS pThis, vpoxnetflt_promisc_stream_t *pPromiscStream, mblk_t *pMsg)
{
    ...
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisQueueLoopback pThis=%p pPromiscStream=%p pMsg=%p\n", pThis, pPromiscStream, pMsg));
    ...
    PVPOXNETFLTPACKETID pCur = NULL;
    if (pPromiscStream->cLoopback < VPOXNETFLT_LOOPBACK_SIZE
            ...
            && pPromiscStream->pHead->cbPacket == 0))
    {
        do
        {
            ...
            {
                pCur = RTMemAlloc(sizeof(VPOXNETFLTPACKETID));
                if (RT_UNLIKELY(!pCur))
                {
                    rc = VERR_NO_MEMORY;
                    break;
                ...
                vpoxNetFltSolarisInitPacketId(pCur, pMsg);
                ...
                Log((DEVICE_NAME ":vpoxNetFltSolarisQueueLoopback initialized head. checksum=%u.\n",
                     ...
                     && pPromiscStream->pHead->cbPacket == 0)
            {
                ...
                Log((DEVICE_NAME ":vpoxNetFltSolarisQueueLoopback re-used head checksum=%u cLoopback=%d.\n",
            ...
            else
            {
                ...
                Log((DEVICE_NAME ":vpoxNetFltSolarisQueueLoopback added head checksum=%u cLoopback=%d.\n", pCur->Checksum,
                        pPromiscStream->cLoopback));
                break;
            }
        } while (0);
    ...
    else
    {
         ...
         */
        PVPOXNETFLTPACKETID pPrev = NULL;
        pCur = pPromiscStream->pHead;
        ...
        vpoxNetFltSolarisInitPacketId(pCur, pMsg);
        Log((DEVICE_NAME ":vpoxNetFltSolarisQueueLoopback recycled tail!! checksum=%u cLoopback=%d\n", pCur->Checksum,
                pPromiscStream->cLoopback));
    }
...
static bool vpoxNetFltSolarisIsOurMBlk(PVPOXNETFLTINS pThis, vpoxnetflt_promisc_stream_t *pPromiscStream, mblk_t *pMsg)
{
    ...
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisIsOurMBlk pThis=%p pMsg=%p\n", pThis, pMsg));
    ...
    while (pCur)
    {
        ...
        Log((DEVICE_NAME ":vpoxNetFltSolarisIsOurMBlk found packet %p Checksum=%u cLoopback=%d\n", pMsg, Checksum,
                    pPromiscStream->cLoopback));
        break;
    }

    Log((DEVICE_NAME ":vpoxNetFltSolarisIsOurMBlk returns %d.\n", fIsOurPacket));
    mutex_exit(&pThis->u.s.hMtx);
    return fIsOurPacket;
}
 ...
 * Helper.
 */
DECLINLINE(bool) vpoxNetFltPortSolarisIsHostMac(PVPOXNETFLTINS pThis, PCRTMAC pMac)
{
    /*
...
static int vpoxNetFltSolarisRecv(PVPOXNETFLTINS pThis, vpoxnetflt_stream_t *pStream, queue_t *pQueue, mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisRecv pThis=%p pMsg=%p\n", pThis, pMsg));
    ...
    if (RT_UNLIKELY(MBLKL(pMsg) < sizeof(RTNETETHERHDR)))
    {
        ...
        if (cbMsg < sizeof(RTNETETHERHDR))
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisRecv %s: packet too small. Dropping packet.\n", pThis->szName));
            return VINF_SUCCESS;
        }
        ...
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisRecv msgpullup failed.\n"));
            return VERR_NO_MEMORY;
        }
    }
     ...
     */
    if (vpoxNetFltSolarisIsOurMBlk(pThis, pPromiscStream, pMsg))
    {
    ...
    PRTNETETHERHDR pEthHdr = (PRTNETETHERHDR)pMsg->b_rptr;
    if (vpoxNetFltPortSolarisIsHostMac(pThis, &pEthHdr->SrcMac))
    ...
    {
        mblk_t *pCorrectedMsg = vpoxNetFltSolarisFixChecksums(pMsg);
        if (pCorrectedMsg)
            pMsg = pCorrectedMsg;
    }
    vpoxNetFltSolarisAnalyzeMBlk(pMsg);
#endif
        ...
        if (pEthHdr->EtherType == RT_H2BE_U16(RTNET_ETHERTYPE_VLAN))
        {
            if (msgdsize(pMsg) > sizeof(RTNETETHERHDR) + sizeof(VLANHEADER))
            {
                    ...
                    && VLAN_ID(RT_BE2H_U16(pVlanHdr->Data)) == 0)
                {
                    ...
                    {
                        LogRel((DEVICE_NAME ":vpoxNetFltSolarisRecv insufficient memory for creating VLAN stripped packet"
                                " cbMsg=%u.\n", cbEthPrefix));
                        if (fCopied)
                            freemsg(pMsg);
                        return VERR_NO_MEMORY;
     ...
     */
    unsigned cSegs = vpoxNetFltSolarisMBlkCalcSGSegs(pThis, pMsg);
    PINTNETSG pSG = (PINTNETSG)alloca(RT_UOFFSETOF_DYN(INTNETSG, aSegs[cSegs]));
    int rc = vpoxNetFltSolarisMBlkToSG(pThis, pMsg, pSG, cSegs, fSrc);
    if (RT_SUCCESS(rc))
        pThis->pSwitchPort->pfnRecv(pThis->pSwitchPort, NULL /* pvIf */, pSG, fSrc);
    else
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisMBlkToSG failed. rc=%d\n", rc));
...
static mblk_t *vpoxNetFltSolarisFixChecksums(mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisFixChecksums pMsg=%p\n"));

    Assert(DB_TYPE(pMsg) == M_DATA);

    if (MBLKL(pMsg) < sizeof(RTNETETHERHDR))
    {
        LogRel((DEVICE_NAME ":vpoxNetFltSolarisFixChecksums Packet shorter than ethernet header size!\n"));
        return NULL;
    }
    ...
    if (pEthHdr->EtherType == RT_H2BE_U16(RTNET_ETHERTYPE_IPV4))
    {
        ...
        if (pMsg->b_cont)
        {
            ...
            {
                LogRel((DEVICE_NAME ":vpoxNetFltSolarisFixChecksums failed to alloc new message of %d bytes.\n", cbFullMsg));
                return NULL;
            }
...
static void vpoxNetFltSolarisAnalyzeMBlk(mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltSolarisAnalyzeMBlk pMsg=%p\n", pMsg));
...
{
    LogFunc((DEVICE_NAME ":vpoxNetFltPortOsSetActive pThis=%p fActive=%d\n", pThis, fActive));

    /*
     * Enable/disable promiscuous mode.
     */
    vpoxnetflt_promisc_params_t *pData = RTMemAllocZ(sizeof(vpoxnetflt_promisc_params_t));
    if (RT_LIKELY(pData))
    {
        /*
            ...
            && pPromiscStream->Stream.pReadQueue)
        {
            ...
            timeout_id_t TimeoutId = qtimeout(WR(pPromiscStream->Stream.pReadQueue), vpoxNetFltSolarisPromiscReqWrap,
                                              pData, 1 /* ticks */);
            ASMAtomicWritePtr(&pPromiscStream->TimeoutId, TimeoutId);
            return; /* pData will be freed by vpoxNetFltSolarisPromiscReqWrap() */
        }
        else
            LogRel((DEVICE_NAME ":vpoxNetFltPortOsSetActive pThis=%p fActive=%d missing stream!\n", pThis, fActive));
        RTMemFree(pData);
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetFltPortOsSetActive out of memory!\n"));
}


int vpoxNetFltOsDisconnectIt(PVPOXNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":vpoxNetFltOsDisconnectIt pThis=%p\n", pThis));

    vpoxNetFltSolarisDetachFromInterface(pThis);
...
{
    LogFunc((DEVICE_NAME ":vpoxNetFltOsDeleteInstance pThis=%p\n", pThis));
...
{
    LogFunc((DEVICE_NAME ":vpoxNetFltOsInitInstance pThis=%p\n"));
    ...
    {
#endif
        rc = vpoxNetFltSolarisAttachToInterface(pThis);
        if (RT_SUCCESS(rc))
            return rc;

        LogRel((DEVICE_NAME ":vpoxNetFltSolarisAttachToInterface failed. rc=%Rrc\n", rc));
        ...
        LogRel((DEVICE_NAME ":vpoxNetFltOsInitInstance failed to create poll mutex. rc=%Rrc\n", rc));
#endif
...
{
    NOREF(pvIfData);
    LogFunc((DEVICE_NAME ":vpoxNetFltPortOsXmit pThis=%p pSG=%p fDst=%d\n", pThis, pSG, fDst));
    ...
    if (fDst & INTNETTRUNKDIR_WIRE)
    {
        ...
        if (RT_LIKELY(pPromiscStream))
        {
            mblk_t *pMsg = vpoxNetFltSolarisMBlkFromSG(pThis, pSG, fDst);
            if (RT_LIKELY(pMsg))
            {
                Log((DEVICE_NAME ":vpoxNetFltPortOsXmit INTNETTRUNKDIR_WIRE\n"));

                vpoxNetFltSolarisQueueLoopback(pThis, pPromiscStream, pMsg);
                putnext(WR(pPromiscStream->Stream.pReadQueue), pMsg);
            }
            else
            {
                LogRel((DEVICE_NAME ":vpoxNetFltPortOsXmit vpoxNetFltSolarisMBlkFromSG failed.\n"));
                return VERR_NO_MEMORY;
            }
        }
    }
    ...
    if (fDst & INTNETTRUNKDIR_HOST)
    {
         ...
         */
        vpoxnetflt_stream_t *pIp4Stream = ASMAtomicUoReadPtrT(&pThis->u.s.pIp4Stream, vpoxnetflt_stream_t *);
        if (!pIp4Stream)
            return rc;
        ...
        if (RT_LIKELY(pMsg))
        {
            ...
            {
                Log((DEVICE_NAME ":vpoxNetFltPortOsXmit INTNETTRUNKDIR_HOST ARP\n"));

                vpoxnetflt_stream_t *pArpStream = ASMAtomicUoReadPtrT(&pThis->u.s.pArpStream, vpoxnetflt_stream_t *);
                if (pArpStream)
                {
                    /*
                     ...
                     */
                    mblk_t *pDlpiMsg;
                    rc = vpoxNetFltSolarisRawToUnitData(pMsg, &pDlpiMsg);
                    if (RT_SUCCESS(rc))
                    {
                        pMsg = pDlpiMsg;
                    ...
                    {
                        LogRel((DEVICE_NAME ":vpoxNetFltSolarisRawToUnitData failed!\n"));
                        freemsg(pMsg);
                        rc = VERR_NO_MEMORY;
                    }
                }
            ...
            else
            {
                     ...
                     && pIp6Stream)
                 {
                      ...
                      */
                     Log((DEVICE_NAME ":vpoxNetFltPortOsXmit INTNETTRUNKDIR_HOST IPv6\n"));
                 ...
                 else
                 {
                     ...
                     */
                    Log((DEVICE_NAME ":vpoxNetFltPortOsXmit INTNETTRUNKDIR_HOST IPv4\n"));
        ...
        {
            LogRel((DEVICE_NAME ":vpoxNetFltSolarisMBlkFromSG failed.\n"));
            rc = VERR_NO_MEMORY;
        }
    }
