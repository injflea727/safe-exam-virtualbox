...
# define dtrace_casptr(a_ppvDst, a_pvOld, a_pvNew) \
	VPoxDtCompareAndSwapPtr((void * volatile *)a_ppvDst, a_pvOld, a_pvNew)
DECLINLINE(void *) VPoxDtCompareAndSwapPtr(void * volatile *ppvDst, void *pvOld, void *pvNew)
{
	void *pvRet;
...
# define dtrace_cas32(a_pu32Dst, a_pu32Old, a_pu32New) \
	VPoxDtCompareAndSwapU32(a_pu32Dst, a_pu32Old, a_pu32New)
DECLINLINE(uint32_t) VPoxDtCompareAndSwapU32(uint32_t volatile *pu32Dst, uint32_t u32Old, uint32_t u32New)
{
	uint32_t u32Ret;
...
#ifndef VPOX /* We have our own assertion machinery. */
int
...
#ifndef VPOX /* no destructive stuff */
...
#endif
#ifndef VPOX /* Avoid idProbe = UINT32_MAX as it is used as invalid value by VTG. */
...
#ifdef VPOX
        if (   VPoxDtMutexInit(&dtrace_lock)
            || VPoxDtMutexInit(&dtrace_provider_lock)
            || VPoxDtMutexInit(&dtrace_meta_lock)
# ifdef DEBUG
            || VPoxDtMutexInit(&dtrace_errlock)
# endif
            )
            return (DDI_FAILURE);
#endif
...
#ifndef VPOX /* Reduce the area a bit just to be sure our vmem fake doesn't blow up. */
...
#endif
{
	...
	case DTRACEIOC_BUFSNAP: {
		...
		if (/*VPox value is is unsigned: desc.dtbd_cpu < 0 ||*/ desc.dtbd_cpu >= NCPU)
			return (EINVAL);
...
#endif
{
	...
	mutex_exit(&dtrace_provider_lock);
#ifdef VPOX
    VPoxDtMutexDelete(&dtrace_lock);
    VPoxDtMutexDelete(&dtrace_provider_lock);
    VPoxDtMutexDelete(&dtrace_meta_lock);
# ifdef DEBUG
    VPoxDtMutexDelete(&dtrace_errlock);
# endif
#endif
