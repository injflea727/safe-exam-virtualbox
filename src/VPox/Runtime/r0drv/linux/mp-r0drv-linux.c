...
# define VPOX_NR_CPUMASK_BITS   (nr_cpumask_bits)   /* same as nr_cpu_ids */
#else
# define VPOX_NR_CPUMASK_BITS   (NR_CPUS)
#endif
...
{
    return idCpu < RTCPUSET_MAX_CPUS && idCpu < VPOX_NR_CPUMASK_BITS ? (int)idCpu : -1;
}
...
{
    return (unsigned)iCpu < VPOX_NR_CPUMASK_BITS ? (RTCPUID)iCpu : NIL_RTCPUID;
}
...
# if RTLNX_VER_MIN(2,6,2) || defined(cpu_possible)
    return idCpu < VPOX_NR_CPUMASK_BITS && cpu_possible(idCpu);
# else /* < 2.5.29 */
...
# if RTLNX_VER_MIN(2,6,0) || defined(cpu_online)
    return idCpu < VPOX_NR_CPUMASK_BITS && cpu_online(idCpu);
# else /* 2.4: */
    return idCpu < VPOX_NR_CPUMASK_BITS && cpu_online_map & RT_BIT_64(idCpu);
# endif
#else
    return idCpu == RTMpCpuId();
#endif
