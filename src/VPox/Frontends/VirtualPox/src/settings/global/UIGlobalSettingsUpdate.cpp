...
/** @file
 * VPox Qt GUI - UIGlobalSettingsUpdate class implementation.
 */
...
struct UIDataSettingsGlobalUpdate
{
    ...
    UIDataSettingsGlobalUpdate()
        ...
        , m_periodIndex(VPoxUpdateData::PeriodUndefined)
        , m_branchIndex(VPoxUpdateData::BranchStable)
        , m_strDate(QString())
    {}
    ...
    VPoxUpdateData::PeriodType m_periodIndex;
    /** Holds the update branch type. */
    VPoxUpdateData::BranchType m_branchIndex;
...
void UIGlobalSettingsUpdate::loadToCacheFrom(QVariant &data)
{
    ...
    const VPoxUpdateData updateData(gEDataManager->applicationUpdateData());
...
void UIGlobalSettingsUpdate::getFromCache()
{
    ...
    if (m_pCheckBoxUpdate->isChecked())
    {
        ...
        if (oldUpdateData.m_branchIndex == VPoxUpdateData::BranchWithBetas)
            m_pRadioUpdateFilterBetas->setChecked(true);
        else if (oldUpdateData.m_branchIndex == VPoxUpdateData::BranchAllRelease)
...
void UIGlobalSettingsUpdate::retranslateUi()
{
    ...
    VPoxUpdateData::populate();
    m_pComboBoxUpdatePeriod->insertItems(0, VPoxUpdateData::list());
...
{
    const VPoxUpdateData data(periodType(), branchType());
...
VPoxUpdateData::PeriodType UIGlobalSettingsUpdate::periodType() const
{
    const VPoxUpdateData::PeriodType result = m_pCheckBoxUpdate->isChecked() ?
        (VPoxUpdateData::PeriodType)m_pComboBoxUpdatePeriod->currentIndex() : VPoxUpdateData::PeriodNever;
    return result == VPoxUpdateData::PeriodUndefined ? VPoxUpdateData::Period1Day : result;
}

VPoxUpdateData::BranchType UIGlobalSettingsUpdate::branchType() const
{
    if (m_pRadioUpdateFilterBetas->isChecked())
        return VPoxUpdateData::BranchWithBetas;
    else if (m_pRadioUpdateFilterEvery->isChecked())
        return VPoxUpdateData::BranchAllRelease;
    else
        return VPoxUpdateData::BranchStable;
}
...
bool UIGlobalSettingsUpdate::saveUpdateData()
{
    ...
    if (fSuccess && m_pCache->wasChanged())
    {
        ...
        const VPoxUpdateData newData(newUpdateData.m_periodIndex, newUpdateData.m_branchIndex);
