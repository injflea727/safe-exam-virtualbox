/* $Id: VPoxGuestR3Lib.cpp $ */
/** @file
 * VPoxGuestR3Lib - Ring-3 Support Library for VirtualPox guest additions, Core.
 */
...
# define VPOX_VBGLR3_XSERVER
#elif defined(VPOX_VBGLR3_XORG)
# include <sys/stat.h>
# include <fcntl.h>
# include <unistd.h>
# include <sys/ioctl.h>
...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
/** The VPoxGuest device handle. */
#ifdef VPOX_VBGLR3_XSERVER
static int g_File = -1;
...
static int vbglR3Init(const char *pszDeviceName)
{
...
#elif !defined (VPOX_VBGLR3_XSERVER)
   ...
   /* IOKit */
    ...
    CFDictionaryRef ClassToMatch = IOServiceMatching("org_virtualpox_VPoxGuest");
    if (!ClassToMatch)
    {
        LogRel(("IOServiceMatching(\"org_virtualpox_VPoxGuest\") failed.\n"));
        return VERR_GENERAL_FAILURE;
    }
    ...
    kr = IOServiceOpen(ServiceObject, mach_task_self(), VPOXGUEST_DARWIN_IOSERVICE_COOKIE, &uConnection);
    ...
    rc2 = RTLogCreate(&pReleaseLogger, 0, "all", "VPOX_RELEASE_LOG",
...
/**
 * Open the VPox R3 Guest Library.  This should be called by system daemons
 * and processes.
 */
VBGLR3DECL(int) VbglR3Init(void)
{
    return vbglR3Init(VPOXGUEST_DEVICE_NAME);
}


/**
 * Open the VPox R3 Guest Library.  Equivalent to VbglR3Init, but for user
 * session processes.
 */
VBGLR3DECL(int) VbglR3InitUser(void)
{
    return vbglR3Init(VPOXGUEST_USER_DEVICE_NAME);
}
...
VBGLR3DECL(void) VbglR3Term(void)
{
    ...
    if (cInits > 0)
        return;
#if !defined(VPOX_VBGLR3_XSERVER)
...
#else  /* VPOX_VBGLR3_XSERVER */
    int File = g_File;
    g_File = -1;
    if (File == -1)
        return;
    xf86close(File);
#endif /* VPOX_VBGLR3_XSERVER */
}
 ...
 *
 * @returns VPox status code as returned by VPoxGuestCommonIOCtl, or
