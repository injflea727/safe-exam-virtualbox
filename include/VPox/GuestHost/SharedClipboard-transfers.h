...
#ifndef VPOX_INCLUDED_GuestHost_SharedClipboard_transfers_h
#define VPOX_INCLUDED_GuestHost_SharedClipboard_transfers_h
...
{
    /** Message type of type VPOX_SHCL_REPLYMSGTYPE_XXX. */
    uint32_t uType;
...
{
    /** Listing flags (see VPOX_SHCL_LIST_FLAG_XXX). */
    uint32_t fList;
...
typedef struct SHCLTRANSFER
{
     ...
     *  On the host this area ID gets shared (maintained / locked) across all VMs via VPoxSVC. */
...
typedef struct _SHCLTRANSFERCTX
{
    ...
    uint64_t                    bmTransferIds[VPOX_SHCL_MAX_TRANSFERS / sizeof(uint64_t) / 8];
...
#endif /* !VPOX_INCLUDED_GuestHost_SharedClipboard_transfers_h */
