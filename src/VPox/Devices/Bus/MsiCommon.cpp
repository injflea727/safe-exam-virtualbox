...
{
    uint32_t idxMessageControl = pDev->Int.s.u8MsiCapOffset + VPOX_MSI_CAP_MESSAGE_CONTROL;
#ifdef IN_RING3
    if (pciDevIsPassthrough(pDev) && pDev->Int.s.pfnConfigRead)
    {
        uint32_t u32Value = 0;
        VPOXSTRICTRC rcStrict = pDev->Int.s.pfnConfigRead(pDev->Int.s.CTX_SUFF(pDevIns), pDev, idxMessageControl, 2, &u32Value);
...
{
    uint8_t iOff = msiIs64Bit(pDev) ? VPOX_MSI_CAP_MASK_BITS_64 : VPOX_MSI_CAP_MASK_BITS_32;
...
{
    uint8_t iOff = msiIs64Bit(pDev) ? VPOX_MSI_CAP_PENDING_BITS_64 : VPOX_MSI_CAP_PENDING_BITS_32;
    /* devices may have no masked/pending support */
    if (iOff >= pDev->Int.s.u8MsiCapSize)
        return NULL;
    iOff += pDev->Int.s.u8MsiCapOffset;
...
{
    return (msiGetMessageControl(pDev) & VPOX_PCI_MSI_FLAGS_ENABLE) != 0;
}

DECLINLINE(uint8_t) msiGetMme(PPDMPCIDEV pDev)
{
    return (msiGetMessageControl(pDev) & VPOX_PCI_MSI_FLAGS_QSIZE) >> 4;
}
...
DECLINLINE(RTGCPHYS) msiGetMsiAddress(PPDMPCIDEV pDev)
{
    if (msiIs64Bit(pDev))
    {
        uint32_t lo = PCIDevGetDWord(pDev, pDev->Int.s.u8MsiCapOffset + VPOX_MSI_CAP_MESSAGE_ADDRESS_LO);
        uint32_t hi = PCIDevGetDWord(pDev, pDev->Int.s.u8MsiCapOffset + VPOX_MSI_CAP_MESSAGE_ADDRESS_HI);
        return RT_MAKE_U64(lo, hi);
    }
    return PCIDevGetDWord(pDev, pDev->Int.s.u8MsiCapOffset + VPOX_MSI_CAP_MESSAGE_ADDRESS_32);
}

DECLINLINE(uint32_t) msiGetMsiData(PPDMPCIDEV pDev, int32_t iVector)
{
    int16_t  iOff = msiIs64Bit(pDev) ? VPOX_MSI_CAP_MESSAGE_DATA_64 : VPOX_MSI_CAP_MESSAGE_DATA_32;
        ...
        switch (reg)
        {
            ...
            default:
                if (pDev->abConfig[uAddr] != u8Val)
                {
                    ...
                    if (    !f64Bit
                         && (reg >= VPOX_MSI_CAP_MASK_BITS_32)
                         && (reg < VPOX_MSI_CAP_MASK_BITS_32 + 4)
                        )
                    {
                        maskUpdated = reg - VPOX_MSI_CAP_MASK_BITS_32;
                    }
                    if (    f64Bit
                         && (reg >= VPOX_MSI_CAP_MASK_BITS_64)
                         && (reg < VPOX_MSI_CAP_MASK_BITS_64 + 4)
                       )
                    {
                        maskUpdated = reg - VPOX_MSI_CAP_MASK_BITS_64;
                    }
...
int MsiR3Init(PPDMPCIDEV pDev, PPDMMSIREG pMsiReg)
{
    ...
    if (!fNoMasking)
    {
        ...
        if ((cVectors > VPOX_MSI_MAX_ENTRIES) || (1 << iMmc) < cVectors)
            return VERR_TOO_MUCH_DATA;

        /* We support per-vector masking */
        iFlags |= VPOX_PCI_MSI_FLAGS_MASKBIT;
        /* How many vectors we're capable of */
        iFlags |= iMmc;
    }

    if (f64bit)
        iFlags |= VPOX_PCI_MSI_FLAGS_64BIT;

    pDev->Int.s.u8MsiCapOffset = iCapOffset;
    pDev->Int.s.u8MsiCapSize   = f64bit ? VPOX_MSI_CAP_SIZE_64 : VPOX_MSI_CAP_SIZE_32;

    PCIDevSetByte(pDev,  iCapOffset + 0, VPOX_PCI_CAP_ID_MSI);
    PCIDevSetByte(pDev,  iCapOffset + 1, iNextOffset); /* next */
    PCIDevSetWord(pDev,  iCapOffset + VPOX_MSI_CAP_MESSAGE_CONTROL, iFlags);
