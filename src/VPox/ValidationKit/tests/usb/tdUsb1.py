...
"""
VirtualPox Validation Kit - USB testcase and benchmark.
"""
...
class tdUsbBenchmark(vpox.TestDriver):                                      # pylint: disable=too-many-instance-attributes
    """
    USB benchmark.
    """
    ...
    def attachUsbDeviceToVm(self, oSession, sVendorId, sProductId, iBusId,
        ...
        if sCaptureFile is None:
            ...
            # Try a few times to give VPoxSVC a chance to detect the new device.
            for _ in xrange(5):
                fFound = False;
                aoUsbDevs = self.oVPoxMgr.getArray(self.oVPox.host, 'USBDevices');
    ...
    def testUsbCompliance(self, oSession, oTxsSession, sUsbCtrl, sSpeed, sCaptureFile = None):
        """
        Test VirtualPoxs USB stack in a VM.
        """
        ...
        if fRc is True:
            ...
            self.oVPox.host.addUSBDeviceSource('USBIP', sGadgetHost, sGadgetHost + (':%s' % oUsbGadget.getUsbIpPort()), [], []);
            ...
            self.oVPox.host.removeUSBDeviceSource(sGadgetHost);
        else:
