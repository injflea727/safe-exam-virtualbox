/* $Id: VPoxVMMPreload.cpp $ */
/** @file
 * VPoxVMMPreload - Preload VPox the ring-0 modules.
 */
...
static RTEXITCODE ParseOptions(int argc, char **argv, bool *pfExit)
{
    ...
    while ((ch = RTGetOpt(&GetState, &ValueUnion)))
    {
        switch(ch)
        {
            ...
            case 'h':
                RTPrintf(VPOX_PRODUCT " VMM ring-0 Module Preloader Version " VPOX_VERSION_STRING
                         "(C) 2005-" VPOX_C_YEAR " " VPOX_VENDOR "\n"
                         "All rights reserved.\n"
                         "\n"
                         "Usage: VPoxVMMPreload [-hlqvV] [-o|--only <mod>]\n"
                         "\n");
                *pfExit = true;
