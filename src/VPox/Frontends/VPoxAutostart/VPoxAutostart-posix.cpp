/* $Id: VPoxAutostart-posix.cpp $ */
/** @file
 * VPoxAutostart - VirtualPox Autostart service.
 */
...
# define VPOXAUTOSTART_DAEMONIZE
#endif
...
{
    RTStrmPrintf(g_pStdErr, VPOX_PRODUCT " Autostart " VPOX_VERSION_STRING "\n"
...
static void displayHelp(const char *pszImage)
{
    ...
    RTStrmPrintf(g_pStdErr, "\nUse environment variable VPOXAUTOSTART_RELEASE_LOG for logging options.\n");
}
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    /* Don't start if the VirtualPox settings directory does not exist. */
    char szUserHomeDir[RTPATH_MAX];
    rc = com::GetVPoxUserHomeDirectory(szUserHomeDir, sizeof(szUserHomeDir), false /* fCreateDir */);
    if (RT_FAILURE(rc))
    ...
    rc = com::VPoxLogRelCreate("Autostart", g_fDaemonize ? NULL : pszLogFile,
                               RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                               "all", "VPOXAUTOSTART_RELEASE_LOG",
    ...
    if (g_fDaemonize)
    {
        ...
        if (!pszLogFile || !*pszLogFile)
        {
            ...
            rc = RTPathAppend(szLogFile, sizeof(szLogFile), "vpoxautostart.log");
            if (RT_FAILURE(rc))
        ...
        rc = com::VPoxLogRelCreate("Autostart", pszLogFile,
                                   RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                                   "all", "VPOXAUTOSTART_RELEASE_LOG",
