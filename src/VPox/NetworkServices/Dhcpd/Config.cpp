...
#include <VPox/com/utils.h>     /* For log initialization. */
...
/**
 * Initializes the m_strHome member with the path to ~/.VirtualPox or equivalent.
 *
 * @returns IPRT status code.
...
int Config::i_homeInit() RT_NOEXCEPT
{
    ...
    int rc = com::GetVPoxUserHomeDirectory(szHome, sizeof(szHome), false);
    if (RT_SUCCESS(rc))
        rc = m_strHome.assignNoThrow(szHome);
    else
        DHCP_LOG_MSG_ERROR(("unable to locate the VirtualPox home directory: %Rrc\n", rc));
    return rc;
}
...
/*static*/ int Config::i_logInitWithFilename(const char *pszFilename) RT_NOEXCEPT
{
    ...
    int rc = com::VPoxLogRelCreate("DHCP Server",
                                   pszFilename,
                                   RTLOGFLAGS_PREFIX_TIME_PROG,
                                   "all net_dhcpd.e.l.f.l3.l4.l5.l6",
                                   "VPOXDHCP_RELEASE_LOG",
                                   RTLOGDEST_FILE
#ifdef DEBUG
                                   | RTLOGDEST_STDERR
#endif
 ...
 *
 * VM Config entries are generated automatically from VirtualPox.xml
