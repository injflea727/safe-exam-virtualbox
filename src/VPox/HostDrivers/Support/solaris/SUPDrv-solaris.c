/* $Id: SUPDrv-solaris.c $ */
/** @file
 * VPoxDrv - The VirtualPox Support Driver - Solaris specifics.
 */
...
#define DEVICE_DESC              "VirtualPox HostDrv"
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int VPoxDrvSolarisOpen(dev_t *pDev, int fFlag, int fType, cred_t *pCred);
static int VPoxDrvSolarisClose(dev_t Dev, int fFlag, int fType, cred_t *pCred);
static int VPoxDrvSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred);
static int VPoxDrvSolarisWrite(dev_t Dev, struct uio *pUio, cred_t *pCred);
static int VPoxDrvSolarisIOCtl(dev_t Dev, int Cmd, intptr_t pArgs, int mode, cred_t *pCred, int *pVal);

static int VPoxDrvSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t Cmd);
static int VPoxDrvSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t Cmd);
static int VPoxDrvSolarisQuiesceNotNeeded(dev_info_t *pDip);

static int VPoxSupDrvErr2SolarisErr(int rc);
static int VPoxDrvSolarisIOCtlSlow(PSUPDRVSESSION pSession, int Cmd, int Mode, intptr_t pArgs);
 ...
 */
static struct cb_ops g_VPoxDrvSolarisCbOps =
{
    VPoxDrvSolarisOpen,
    VPoxDrvSolarisClose,
    nodev,                        /* b strategy */
 ...
 */
static struct dev_ops g_VPoxDrvSolarisDevOps =
{
    ...
    nulldev,                      /* identify */
    nulldev,                      /* probe */
    VPoxDrvSolarisAttach,
    VPoxDrvSolarisDetach,
    nodev,                        /* reset */
    &g_VPoxDrvSolarisCbOps,
    (struct bus_ops *)0,
    nodev,                        /* power */
    VPoxDrvSolarisQuiesceNotNeeded
};

/**
 * modldrv: export driver specifics to the kernel
 */
static struct modldrv g_VPoxDrvSolarisModule =
{
    &mod_driverops,               /* extern from kernel */
    DEVICE_DESC " " VPOX_VERSION_STRING "r" RT_XSTR(VPOX_SVN_REV),
    &g_VPoxDrvSolarisDevOps
};

/**
 * modlinkage: export install/remove/info to the kernel
 */
static struct modlinkage g_VPoxDrvSolarisModLinkage =
{
    MODREV_1,                     /* loadable module system revision */
    {
        &g_VPoxDrvSolarisModule,
...
static void *g_pVPoxDrvSolarisState;
...
int _init(void)
{
    ...
    LogFlowFunc(("vpoxdrv:_init\n"));
#endif

    /*
     * Prevent module autounloading.
     */
    modctl_t *pModCtl = mod_getctl(&g_VPoxDrvSolarisModLinkage);
    if (pModCtl)
        pModCtl->mod_loadflags |= MOD_NOAUTOUNLOAD;
    else
        cmn_err(CE_NOTE, "vpoxdrv: failed to disable autounloading!\n");
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = RTSpinlockCreate(&g_Spinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "VPoxDrvSol");
            if (RT_SUCCESS(rc))
            {
                rc = ddi_soft_state_init(&g_pVPoxDrvSolarisState, sizeof(vpox_devstate_t), 8);
                if (!rc)
                {
                    rc = mod_install(&g_VPoxDrvSolarisModLinkage);
                    if (!rc)
                        return rc; /* success */

                    ddi_soft_state_fini(&g_pVPoxDrvSolarisState);
                    LogRel(("vpoxdrv: mod_install failed! rc=%d\n", rc));
                }
                else
                    LogRel(("vpoxdrv: failed to initialize soft state.\n"));
            ...
            {
                LogRel(("VPoxDrvSolarisAttach: RTSpinlockCreate failed\n"));
        ...
        {
            LogRel(("VPoxDrvSolarisAttach: supdrvInitDevExt failed\n"));
            rc = EINVAL;
        }
        RTR0TermForced();
    }
    ...
    {
        LogRel(("VPoxDrvSolarisAttach: failed to init R0Drv\n"));
        rc = RTErrConvertToErrno(rc);
    }
...
int _fini(void)
{
    LogFlowFunc(("vpoxdrv:_fini\n"));

    /*
     * Undo the work we did at start (in the reverse order).
     */
    int rc = mod_remove(&g_VPoxDrvSolarisModLinkage);
    if (rc != 0)
        return rc;
...
int _info(struct modinfo *pModInfo)
{
    ...
    LogFlowFunc(("vpoxdrv:_init\n"));
#endif
    int e = mod_info(&g_VPoxDrvSolarisModLinkage, pModInfo);
    return e;
}
 ...
 */
static int VPoxDrvSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd)
{
    LogFlowFunc(("VPoxDrvSolarisAttach\n"));
    ...
    switch (enmCmd)
    {
        case DDI_ATTACH:
        {
            ...
            vpox_devstate_t *pState;

            if (ddi_soft_state_zalloc(g_pVPoxDrvSolarisState, instance) != DDI_SUCCESS)
            {
                LogRel(("VPoxDrvSolarisAttach: state alloc failed\n"));
                return DDI_FAILURE;
            }

            pState = ddi_get_soft_state(g_pVPoxDrvSolarisState, instance);
#endif
            ...
            if (rc != DDI_PROP_SUCCESS)
                LogRel(("vpoxdrv: Suspend/Resume notification registration failed.\n"));
        ...
        case DDI_RESUME:
        {
            ...
            LogFlow(("vpoxdrv: Awakened from suspend.\n"));
            return DDI_SUCCESS;
        }
 ...
 * @return  corresponding solaris error code.
 */
static int VPoxDrvSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd)
{
    LogFlowFunc(("VPoxDrvSolarisDetach\n"));
    switch (enmCmd)
    {
        case DDI_DETACH:
        {
...
#else
            int instance = ddi_get_instance(pDip);
            vpox_devstate_t *pState = ddi_get_soft_state(g_pVPoxDrvSolarisState, instance);
            ddi_remove_minor_node(pDip, NULL);
            ddi_soft_state_free(g_pVPoxDrvSolarisState, instance);
#endif
        ...
        case DDI_SUSPEND:
        {
            ...
            LogFlow(("vpoxdrv: Falling to suspend mode.\n"));
            return DDI_SUCCESS;
...
static int VPoxDrvSolarisOpen(dev_t *pDev, int fFlag, int fType, cred_t *pCred)
{
    ...
    LogFlowFunc(("VPoxDrvSolarisOpen: pDev=%p:%#x\n", pDev, *pDev));
     ...
     */
    vpox_devstate_t *pState = NULL;
    unsigned iOpenInstance;
    for (iOpenInstance = 0; iOpenInstance < 4096; iOpenInstance++)
    {
        if (    !ddi_get_soft_state(g_pVPoxDrvSolarisState, iOpenInstance) /* faster */
            &&  ddi_soft_state_zalloc(g_pVPoxDrvSolarisState, iOpenInstance) == DDI_SUCCESS)
        {
            pState = ddi_get_soft_state(g_pVPoxDrvSolarisState, iOpenInstance);
            break;
        }
    }
    if (!pState)
    {
        LogRel(("VPoxDrvSolarisOpen: too many open instances.\n"));
        return ENXIO;
    }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        *pDev = makedevice(getmajor(*pDev), iOpenInstance);
        LogFlow(("VPoxDrvSolarisOpen: Dev=%#x pSession=%p pid=%d r0proc=%p thread=%p\n",
                 *pDev, pSession, RTProcSelf(), RTR0ProcHandleSelf(), RTThreadNativeSelf() ));
        return 0;
    }

    /* failed - clean up */
    ddi_soft_state_free(g_pVPoxDrvSolarisState, iOpenInstance);

#else
    /*
     * Create a new session.
     * Sessions in Solaris driver are mostly useless. It's however needed
     * in VPoxDrvSolarisIOCtlSlow() while calling supdrvIOCtl()
     */
    rc = supdrvCreateSession(&g_DevExt, true /* fUser */, fUnrestricted, &pSession);
    if (RT_SUCCESS(rc))
    {
...
# error "Only one entry per process!"
        ...
        LogFlow(("VPoxDrvSolarisOpen success\n"));
    }
    ...
    {
        LogRel(("VPoxDrvSolarisOpen: All instances exhausted\n"));
        return ENXIO;
    }
    ...
    *pDev = makedevice(getmajor(*pDev), instance);
    ...
    return VPoxSupDrvErr2SolarisErr(rc);
}


static int VPoxDrvSolarisClose(dev_t Dev, int flag, int otyp, cred_t *cred)
{
    LogFlowFunc(("VPoxDrvSolarisClose: Dev=%#x\n", Dev));

#ifndef USE_SESSION_HASH
    /*
     * Get the session and free the soft state item.
     */
    vpox_devstate_t *pState = ddi_get_soft_state(g_pVPoxDrvSolarisState, getminor(Dev));
    if (!pState)
    {
        LogRel(("VPoxDrvSolarisClose: no state data for %#x (%d)\n", Dev, getminor(Dev)));
        return EFAULT;
    }

    PSUPDRVSESSION pSession = pState->pSession;
    pState->pSession = NULL;
    ddi_soft_state_free(g_pVPoxDrvSolarisState, getminor(Dev));

    if (!pSession)
    {
        LogRel(("VPoxDrvSolarisClose: no session in state data for %#x (%d)\n", Dev, getminor(Dev)));
        return EFAULT;
    }
    LogFlow(("VPoxDrvSolarisClose: Dev=%#x pSession=%p pid=%d r0proc=%p thread=%p\n",
    ...
    if (!pSession)
    {
        LogRel(("VPoxDrvSolarisClose: WHAT?!? pSession == NULL! This must be a mistake... pid=%d (close)\n", (int)Process));
        return EFAULT;
    }
#endif
...
static int VPoxDrvSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    LogFlowFunc(("VPoxDrvSolarisRead"));
    return 0;
}
...
static int VPoxDrvSolarisWrite(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    LogFlowFunc(("VPoxDrvSolarisWrite"));
    return 0;
}
 ...
 * @return  corresponding solaris error code.
 */
static int VPoxDrvSolarisIOCtl(dev_t Dev, int iCmd, intptr_t pArgs, int Mode, cred_t *pCred, int *pVal)
{
#ifndef USE_SESSION_HASH
    /*
    ...
    if (!pState)
    {
        LogRel(("VPoxDrvSolarisIOCtl: no state data for %#x (%d)\n", Dev, getminor(Dev)));
        return EINVAL;
    }
    ...
    if (!pSession)
    {
        LogRel(("VPoxDrvSolarisIOCtl: no session in state data for %#x (%d)\n", Dev, getminor(Dev)));
        return DDI_SUCCESS;
    }
#else
    const RTPROCESS     Process = RTProcSelf();
    ...
    if (!pSession)
    {
        LogRel(("VPoxSupDrvIOCtl: WHAT?!? pSession == NULL! This must be a mistake... pid=%d iCmd=%#x Dev=%#x\n",
    ...
    return VPoxDrvSolarisIOCtlSlow(pSession, iCmd, Mode, pArgs);
}
...
/**
 * Worker for VPoxSupDrvIOCtl that takes the slow IOCtl functions.
 *
 ...
 */
static int VPoxDrvSolarisIOCtlSlow(PSUPDRVSESSION pSession, int iCmd, int Mode, intptr_t iArg)
{
    int         rc;
    uint32_t    cbBuf = 0;
    union
    ...
    if (RT_UNLIKELY(    StackBuf.Hdr.cbIn < sizeof(StackBuf.Hdr)
                    ...
                    ||  cbBuf > _1M*16))
    {
        LogRel(("VPoxDrvSolarisIOCtlSlow: max(%#x,%#x); iCmd=%#x\n", StackBuf.Hdr.cbIn, StackBuf.Hdr.cbOut, iCmd));
        return EINVAL;
    }
    ...
    else
    {
        ...
        if (RT_UNLIKELY(!pHdr))
        {
            LogRel(("VPoxDrvSolarisIOCtlSlow: failed to allocate buffer of %d bytes for iCmd=%#x.\n", cbBuf, iCmd));
            return ENOMEM;
        }
    }
    ...
    if (RT_UNLIKELY(rc))
    {
        LogRel(("VPoxDrvSolarisIOCtlSlow: copy_from_user(,%#lx, %#x) failed; iCmd=%#x. rc=%d\n", iArg, cbBuf, iCmd, rc));
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        {
            LogRel(("VPoxDrvSolarisIOCtlSlow: too much output! %#x > %#x; iCmd=%#x!\n", cbOut, cbBuf, iCmd));
            cbOut = cbBuf;
        }
        ...
        {
            /* this is really bad */
            LogRel(("VPoxDrvSolarisIOCtlSlow: ddi_copyout(,%p,%d) failed. rc=%d\n", (void *)iArg, cbBuf, rc));
            rc = EFAULT;
        }
    }
    else
 ...
 */
int VPOXCALL SUPDrvSolarisIDC(uint32_t uReq, PSUPDRVIDCREQHDR pReq)
{
    PSUPDRVSESSION  pSession;
...
#if  defined(VPOX_WITH_NATIVE_SOLARIS_LOADING) \
 && !defined(VPOX_WITHOUT_NATIVE_R0_LOADER)
...
#else /* !VPOX_WITH_NATIVE_SOLARIS_LOADING */
...
#endif /* !VPOX_WITH_NATIVE_SOLARIS_LOADING */
