 ...
 */
VMMR3DECL(int) PDMR3DrvStaticRegistration(PVM pVM, FNPDMVPOXDRIVERSREGISTER pfnCallback)
{
    /*
    ...
    int rc = pfnCallback(&RegCB.Core, VPOX_VERSION);
    if (RT_FAILURE(rc))
        AssertMsgFailed(("VPoxDriversRegister failed with rc=%Rrc\n", rc));
...
int pdmR3DrvInit(PVM pVM)
{
    ...
    if (fLoadBuiltin)
    {
        ...
        rc = pdmR3DrvLoad(pVM, &RegCB, pszFilename, "VPoxDD");
...
static int pdmR3DrvLoad(PVM pVM, PPDMDRVREGCBINT pRegCB, const char *pszFilename, const char *pszName)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        FNPDMVPOXDRIVERSREGISTER *pfnVPoxDriversRegister;
        rc = PDMR3LdrGetSymbolR3(pVM, pszName, "VPoxDriversRegister", (void **)&pfnVPoxDriversRegister);
        if (RT_SUCCESS(rc))
        {
            Log(("PDM: Calling VPoxDriversRegister (%p) of %s (%s)\n", pfnVPoxDriversRegister, pszName, pszFilename));
            rc = pfnVPoxDriversRegister(&pRegCB->Core, VPOX_VERSION);
            if (RT_SUCCESS(rc))
        ...
        {
            AssertMsgFailed(("Failed to locate 'VPoxDriversRegister' in %s (%s) rc=%Rrc\n", pszName, pszFilename, rc));
 ...
 *
 * @returns VPox status code, including informational statuses.
 *
 * @param   pVM                 The cross context VM structure.
...
#ifdef VPOX_WITH_STATISTICS /** @todo rework this to always be compiled in */
    if (*pszName == '/')
