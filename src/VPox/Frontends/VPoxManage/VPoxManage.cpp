...
#define VBMG_CMD_TODO HELP_CMD_VPOXMANAGE_INVALID
...
typedef struct VBMGCMD
{
    ...
    enum HELP_CMD_VPOXMANAGE    enmCmdHelp;
    /** The handler. */
    RTEXITCODE                (*pfnHandler)(HandlerArg *pArg);
    /** VBMG_CMD_F_XXX,    */
    uint32_t                    fFlags;
} VBMGCMD;
/** Pointer to a const VPoxManage command descriptor. */
...
#if defined(RT_OS_WINDOWS) && !defined(VPOX_ONLY_DOCS)
...
# endif
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            if (SUCCEEDED(hrc))
            {
                /* Session secret. */
                if (pszSettingsPw)
                    CHECK_ERROR2I_STMT(virtualPox, SetSettingsSecret(Bstr(pszSettingsPw).raw()), rcExit = RTEXITCODE_FAILURE);
                else if (pszSettingsPwFile)
                    rcExit = settingsPasswordFile(virtualPox, pszSettingsPwFile);
                else
                ...
                if (rcExit == RTEXITCODE_SUCCESS)
                {
                     ...
                     * Call the handler.
                     */
                    HandlerArg handlerArg = { argc - iCmdArg, &argv[iCmdArg], virtualPox, session };
        ...
        {
            com::ErrorInfo info;
            RTMsgError("Failed to create the VirtualPox object!");
            if (!info.isFullAvailable() && !info.isBasicAvailable())
            {
                com::GluePrintRCMessage(hrc);
        ...
        /*
         * Terminate COM, make sure the virtualPox object has been released.
         */
        virtualPox.setNull();
        virtualPoxClient.setNull();
        NativeEventQueue::getMainEventQueue()->processEventQueue(0);
        com::Shutdown();
    }
    else
