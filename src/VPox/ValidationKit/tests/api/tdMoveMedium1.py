...
"""
VirtualPox Validation Kit - Medium Move Test #1
"""
    ...
    def moveTo(self, sLocation, aoMediumAttachments):
        for oAttachment in aoMediumAttachments:
            ...
            if self.oTstDrv.fpApiVer >= 5.3 and self.oTstDrv.uRevision > 124748:
                try:
                    oProgress = vpoxwrappers.ProgressWrapper(oMedium.moveTo(sLocation), self.oTstDrv.oVPoxMgr, self.oTstDrv,
                                                             'move "%s"' % (oMedium.name,));
                except:
                    return reporter.errorXcpt('Medium::moveTo("%s") for medium "%s" failed' % (sLocation, oMedium.name,));
            else:
                try:
                    oProgress = vpoxwrappers.ProgressWrapper(oMedium.setLocation(sLocation), self.oTstDrv.oVPoxMgr, self.oTstDrv,
                                                             'move "%s"' % (oMedium.name,));
                except:
