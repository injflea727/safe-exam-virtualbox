/* $Id: VPoxVideoIOCTL.h $ */
/** @file
 * VPox Miniport IOCTL related header
 */
...
#ifndef GA_INCLUDED_SRC_WINNT_Graphics_Video_common_xpdm_VPoxVideoIOCTL_h
#define GA_INCLUDED_SRC_WINNT_Graphics_Video_common_xpdm_VPoxVideoIOCTL_h
#ifndef RT_WITHOUT_PRAGMA_ONCE
# pragma once
#endif

#include <VPox/VMMDev.h> /* VBVAMEMORY */
#include <HGSMI.h>

/* ==================== VirtualPox specific VRP's ==================== */
...
#define IOCTL_VIDEO_QUERY_VPOXVIDEO_INFO \
    CTL_CODE(FILE_DEVICE_VIDEO, 0x436, METHOD_BUFFERED, FILE_ANY_ACCESS)

/* ==================== Data structures used by VirtualPox VRPS's ==================== */
typedef void* HVPOXVIDEOHGSMI;

/** Complete host commands addressed to the display */
typedef DECLCALLBACK(void) FNVPOXVIDEOHGSMICOMPLETION(HVPOXVIDEOHGSMI hHGSMI, struct VBVAHOSTCMD RT_UNTRUSTED_VOLATILE_HOST * pCmd);
typedef FNVPOXVIDEOHGSMICOMPLETION *PFNVPOXVIDEOHGSMICOMPLETION;

/** request the host commands addressed to the display */
typedef DECLCALLBACK(int) FNVPOXVIDEOHGSMICOMMANDS(HVPOXVIDEOHGSMI hHGSMI, uint8_t u8Channel, uint32_t iDevice,
                                                   struct VBVAHOSTCMD RT_UNTRUSTED_VOLATILE_HOST ** ppCmd);
typedef FNVPOXVIDEOHGSMICOMMANDS *PFNVPOXVIDEOHGSMICOMMANDS;

/** post guest command (offset) to the host */
typedef DECLCALLBACK(void) FNVPOXVIDEOHGSMIPOSTCOMMAND(HVPOXVIDEOHGSMI hHGSMI, HGSMIOFFSET offCmd);
typedef FNVPOXVIDEOHGSMIPOSTCOMMAND *PFNVPOXVIDEOHGSMIPOSTCOMMAND;
...
{
    HVPOXVIDEOHGSMI hContext;
    PFNVPOXVIDEOHGSMICOMPLETION pfnCompletionHandler;
    PFNVPOXVIDEOHGSMICOMMANDS   pfnRequestCommandsHandler;
} HGSMIQUERYCALLBACKS;
...
#define VPOXVIDEO_INFO_LEVEL_REGISTRY_FLAGS 1

#define VPOXVIDEO_REGISTRY_FLAGS_DISABLE_BITMAP_CACHE 0x00000001

#endif /* !GA_INCLUDED_SRC_WINNT_Graphics_Video_common_xpdm_VPoxVideoIOCTL_h */
