...
/* Saved context. */
typedef struct VPOXDXVAD3D9SAVEDSTATE
{
 ...
 */
typedef struct VPOXWDDMVIDEOPROCESSDEVICE
{
    /* Creation parameters. */
    ...
    PVPOXWDDMDISP_RESOURCE pRenderTarget;
    ...
    VPOXDXVAD3D9SAVEDSTATE SavedState;
} VPOXWDDMVIDEOPROCESSDEVICE;
...
static int vpoxDxvaFindDeviceGuid(GUID const *pGuid)
{
...
static int vpoxDxvaFindInputFormat(D3DDDIFORMAT enmFormat)
{
...
static HRESULT vpoxDxvaCopyToVertexBuffer(IDirect3DVertexBuffer9 *pVB, const void *pvSrc, int cbSrc)
{
    void *pvDst = 0;
...
static HRESULT vpoxDxvaDeviceStateSave(IDirect3DDevice9 *pDevice9, VPOXDXVAD3D9SAVEDSTATE *pState)
{
    HRESULT hr;
...
static void vpoxDxvaDeviceStateRestore(IDirect3DDevice9 *pDevice9, VPOXDXVAD3D9SAVEDSTATE const *pState)
{
    HRESULT hr;
...
static HRESULT vpoxDxvaUploadSample(VPOXWDDMVIDEOPROCESSDEVICE *pVideoProcessDevice,
...
static HRESULT vpoxDxvaInit(VPOXWDDMVIDEOPROCESSDEVICE *pVideoProcessDevice)
{
    HRESULT hr;
...
static HRESULT vpoxDxvaSetState(VPOXWDDMVIDEOPROCESSDEVICE *pVideoProcessDevice, DXVADDI_EXTENDEDFORMAT const *pSampleFormat)
{
    HRESULT hr;
...
static HRESULT vpoxDxvaUpdateVertexBuffer(VPOXWDDMVIDEOPROCESSDEVICE *pVideoProcessDevice,
                                          RECT const *pSrcRect,
                                          RECT const *pDstRect)
{
    HRESULT hr;
    ...
    hr = vpoxDxvaCopyToVertexBuffer(pVideoProcessDevice->pVB, aVertices, sizeof(aVertices));
    AssertReturn(hr == D3D_OK, hr);

    return S_OK;
}

static HRESULT vpoxDxvaProcessBlt(VPOXWDDMVIDEOPROCESSDEVICE *pVideoProcessDevice,
                                  ...
                                  IDirect3DTexture9 *paSrcTextures[])
{
    ...
    hr = vpoxDxvaDeviceStateSave(pDevice9, &pVideoProcessDevice->SavedState);
    if (hr == D3D_OK)
    {
        /* Set the required state for the blits, inclusding the render target. */
        hr = vpoxDxvaSetState(pVideoProcessDevice, &pData->pSrcSurfaces[0].SampleFormat);
        if (hr == D3D_OK)
        {
            ...
            /* Upload the source data to the staging texture. */
            hr = vpoxDxvaUploadSample(pVideoProcessDevice, pSrcTexture, pSrcSample->SrcSubResourceIndex);
            if (hr == D3D_OK)
            {
                /* Setup the blit dimensions. */
                hr = vpoxDxvaUpdateVertexBuffer(pVideoProcessDevice, &pSrcSample->SrcRect, &pSrcSample->DstRect);
                Assert(hr == D3D_OK);
                if (hr == D3D_OK)
                {
        ...
        vpoxDxvaDeviceStateRestore(pDevice9, &pVideoProcessDevice->SavedState);
    }
...
HRESULT VPoxDxvaGetOutputFormatCount(UINT *pcFormats, DXVADDI_VIDEOPROCESSORINPUT const *pVPI, bool fSubstream)
{
    ...
    if (pVPI)
    {
        if (vpoxDxvaFindDeviceGuid(pVPI->pVideoProcGuid) >= 0)
        {
            if (vpoxDxvaFindInputFormat(pVPI->VideoDesc.Format) >= 0)
            {
...
HRESULT VPoxDxvaGetCaps(DXVADDI_VIDEOPROCESSORCAPS *pVideoProcessorCaps,
...
HRESULT VPoxDxvaCreateVideoProcessDevice(PVPOXWDDMDISP_DEVICE pDevice, D3DDDIARG_CREATEVIDEOPROCESSDEVICE *pData)
{
     ...
     */
    VPOXWDDMVIDEOPROCESSDEVICE *pVideoProcessDevice =
        (VPOXWDDMVIDEOPROCESSDEVICE *)RTMemAllocZ(sizeof(VPOXWDDMVIDEOPROCESSDEVICE));
...
{
    VPOXWDDMVIDEOPROCESSDEVICE *pVideoProcessDevice = (VPOXWDDMVIDEOPROCESSDEVICE *)hVideoProcessor;
...
HRESULT VPoxDxvaVideoProcessBeginFrame(PVPOXWDDMDISP_DEVICE pDevice, HANDLE hVideoProcessor)
{
    ...
    {
        hr = vpoxDxvaInit(pVideoProcessDevice);
    }
...
{
    VPOXWDDMVIDEOPROCESSDEVICE *pVideoProcessDevice = (VPOXWDDMVIDEOPROCESSDEVICE *)pData->hVideoProcess;
    AssertReturn(pDevice == pVideoProcessDevice->pDevice, E_INVALIDARG);
    return S_OK;
}

HRESULT VPoxDxvaSetVideoProcessRenderTarget(PVPOXWDDMDISP_DEVICE pDevice,
                                            const D3DDDIARG_SETVIDEOPROCESSRENDERTARGET *pData)
{
    ...
    VPOXWDDMDISP_ALLOCATION *pAllocation = &pRc->aAllocations[pData->SubResourceIndex];
    AssertPtrReturn(pAllocation->pD3DIf, E_INVALIDARG);
    AssertReturn(pAllocation->enmD3DIfType == VPOXDISP_D3DIFTYPE_TEXTURE, E_INVALIDARG);
...
HRESULT VPoxDxvaVideoProcessBlt(PVPOXWDDMDISP_DEVICE pDevice, const D3DDDIARG_VIDEOPROCESSBLT *pData)
{
    ...
    PVPOXWDDMDISP_RESOURCE pSrcRc = (PVPOXWDDMDISP_RESOURCE)pData->pSrcSurfaces[0].SrcResource;
    AssertReturn(pSrcRc->cAllocations > pData->pSrcSurfaces[0].SrcSubResourceIndex, E_INVALIDARG);

    VPOXWDDMDISP_ALLOCATION *pAllocation = &pSrcRc->aAllocations[pData->pSrcSurfaces[0].SrcSubResourceIndex];
    AssertPtrReturn(pAllocation->pD3DIf, E_INVALIDARG);
    AssertReturn(pAllocation->enmD3DIfType == VPOXDISP_D3DIFTYPE_TEXTURE, E_INVALIDARG);
    ...
    HRESULT hr = vpoxDxvaProcessBlt(pVideoProcessDevice, pData, &pSrcTexture);
    return hr;
}
