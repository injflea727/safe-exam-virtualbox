...
/** No flags set. */
#define VPOX_EBMLWRITER_FLAG_NONE               0
/** The file handle was inherited. */
#define VPOX_EBMLWRITER_FLAG_HANDLE_INHERITED   RT_BIT(0)
...
int EBMLWriter::createEx(const char *a_pszFile, PRTFILE phFile)
{
    ...
    m_hFile   = *phFile;
    m_fFlags |= VPOX_EBMLWRITER_FLAG_HANDLE_INHERITED;
    m_strFile = a_pszFile;
...
void EBMLWriter::close(void)
{
    ...
    if (!(m_fFlags & VPOX_EBMLWRITER_FLAG_HANDLE_INHERITED))
    {
        RTFileClose(m_hFile);
        m_hFile = NIL_RTFILE;
    }

    m_fFlags  = VPOX_EBMLWRITER_FLAG_NONE;
    m_strFile = "";
}
