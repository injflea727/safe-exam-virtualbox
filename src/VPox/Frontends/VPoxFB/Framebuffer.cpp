/* $Id: Framebuffer.cpp $ */
/** @file
 * VPoxFB - implementation of VPoxDirectFB class.
 */
...
NS_IMPL_ISUPPORTS1_CI(VPoxDirectFB, IFramebuffer)
NS_DECL_CLASSINFO(VPoxDirectFB)

VPoxDirectFB::VPoxDirectFB(IDirectFB *aDFB, IDirectFBSurface *aSurface)
{
    dfb = aDFB;
    surface = aSurface;
...
NS_IMETHODIMP VPoxDirectFB::GetWidth(uint32 *width)
{
    if (!width)
...
NS_IMETHODIMP VPoxDirectFB::GetHeight(PRUint32 *height)
{
    if (!height)
        return NS_ERROR_INVALID_POINTER;
    *height = fbHeight;
...
NS_IMETHODIMP VPoxDirectFB::Lock()
{
...
NS_IMETHODIMP VPoxDirectFB::Unlock()
{
    // do we have an internal framebuffer?
    if (fbInternalSurface)
    {
...
NS_IMETHODIMP VPoxDirectFB::GetAddress(PRUint8 **address)
{
    if (!address)
        return NS_ERROR_INVALID_POINTER;
...
NS_IMETHODIMP VPoxDirectFB::GetBitsPerPixel(PRUint32 *bitsPerPixel)
{
    if (!bitsPerPixel)
        return NS_ERROR_INVALID_POINTER;
...
NS_IMETHODIMP VPoxDirectFB::GetBytesPerLine(PRUint32 *bytesPerLine)
{
    if (!bytesPerLine)
        return NS_ERROR_INVALID_POINTER;
...
NS_IMETHODIMP VPoxDirectFB::GetPixelFormat (PRUint32 *pixelFormat)
{
    if (!pixelFormat)
        return NS_ERROR_INVALID_POINTER;
...
NS_IMETHODIMP VPoxDirectFB::GetUsesGuestVRAM (PRBool *usesGuestVRAM)
{
    if (!usesGuestVRAM)
        return NS_ERROR_INVALID_POINTER;
    *usesGuestVRAM = false;
...
NS_IMETHODIMP VPoxDirectFB::GetHeightReduction(PRUint32 *heightReduction)
{
    if (!heightReduction)
        return NS_ERROR_INVALID_POINTER;
    *heightReduction = 0;
...
NS_IMETHODIMP VPoxDirectFB::GetOverlay(IFramebufferOverlay **overlay)
{
    if (!overlay)
        return NS_ERROR_INVALID_POINTER;
    /* Not yet implemented */
...
NS_IMETHODIMP VPoxDirectFB::GetWinId(PRint64 *winId)
{
    if (!winId)
        return NS_ERROR_INVALID_POINTER;
    *winId = 0;
...
NS_IMETHODIMP VPoxDirectFB::NotifyUpdate(PRUint32 x, PRUint32 y,
...
NS_IMETHODIMP VPoxDirectFB::RequestResize(PRUint32 aScreenId, PRUint32 pixelFormat, PRUint8 *vram,
...
NS_IMETHODIMP VPoxDirectFB::VideoModeSupported(PRUint32 w, PRUint32 h, PRUint32 bpp, PRBool *supported)
{
    if (!supported)
        return NS_ERROR_INVALID_POINTER;
    *supported = true;
...
NS_IMETHODIMP VPoxDirectFB::GetVisibleRegion(PRUint8 *rectangles, PRUint32 count, PRUint32 *countCopied)
{
...
NS_IMETHODIMP VPoxDirectFB::SetVisibleRegion(PRUint8 *rectangles, PRUint32 count)
{
    PRTRECT rects = (PRTRECT)rectangles;
...
NS_IMETHODIMP VPoxDirectFB::ProcessVHWACommand(PRUint8 *command, LONG enmCmd, BOOL fGuestCmd)
{
    RT_NOREF(command, enmCmd, fGuestCmd);
    return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP VPoxDirectFB::Notify3DEvent(PRUint32 type, PRUint8 *reserved)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}

int VPoxDirectFB::createSurface(uint32_t w, uint32_t h)
{
