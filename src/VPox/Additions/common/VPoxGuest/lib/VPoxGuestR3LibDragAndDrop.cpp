/* $Id: VPoxGuestR3LibDragAndDrop.cpp $ */
/** @file
 * VPoxGuestR3Lib - Ring-3 Support Library for VirtualPox guest additions, Drag & Drop.
 */
 ...
 */
static int vbglR3DnDHGRecvURIData(PVBGLR3GUESTDNDCMDCTX pCtx, PVPOXDNDSNDDATAHDR pDataHdr, PDNDDROPPEDFILES pDroppedFiles)
{
    AssertPtrReturn(pCtx,          VERR_INVALID_POINTER);
 ...
 */
static int vbglR3DnDHGRecvDataRaw(PVBGLR3GUESTDNDCMDCTX pCtx, PVPOXDNDSNDDATAHDR pDataHdr,
 ...
 */
static int vbglR3DnDHGRecvDataHdr(PVBGLR3GUESTDNDCMDCTX pCtx, PVPOXDNDSNDDATAHDR pDataHdr)
{
    AssertPtrReturn(pCtx,     VERR_INVALID_POINTER);
    AssertPtrReturn(pDataHdr, VERR_INVALID_POINTER);
 ...
 */
static int vbglR3DnDHGRecvDataLoop(PVBGLR3GUESTDNDCMDCTX pCtx, PVPOXDNDSNDDATAHDR pDataHdr,
                                   ...
                                   PVBGLR3GUESTDNDMETADATA pMeta)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         * the caller know.
         *
         * This keeps the actual (guest OS-)dependent client (like VPoxClient /
         * VPoxTray) small by not having too much redundant code.
         */
...
VBGLR3DECL(int) VbglR3DnDConnect(PVBGLR3GUESTDNDCMDCTX pCtx)
{
    ...
    /* Initialize header */
    int rc = VbglR3HGCMConnect("VPoxDragAndDropSvc", &pCtx->uClientID);
    if (RT_FAILURE(rc))
        return rc;
    Assert(pCtx->uClientID);

    /* Set the default protocol version we would like to use.
     * Deprecated since VPox 6.1.x, but let this set to 3 to (hopefully) not break things. */
    pCtx->uProtocolDeprecated = 3;

    pCtx->fHostFeatures  = VPOX_DND_HF_NONE;
    pCtx->fGuestFeatures = VPOX_DND_GF_NONE;

    /*
     * Get the VM's session ID.
     * This is not fatal in case we're running with an ancient VPox version.
     */
    pCtx->uSessionID = 0;
     ...
     * Note: This might fail when the Guest Additions run on an older VPox host (< VPox 5.0) which
    ...
    Msg.u.v3.uProtocol.SetUInt32(pCtx->uProtocolDeprecated); /* Deprecated since VPox 6.1.x. */
 ...
 * @param   fGuestFeatures  Features to report, VPOX_DND_GF_XXX.
 * @param   pfHostFeatures  Where to store the features VPOX_DND_HF_XXX.
 */
VBGLR3DECL(int) VbglR3DnDReportFeatures(uint32_t idClient, uint64_t fGuestFeatures, uint64_t *pfHostFeatures)
{
    ...
    do
    {
        ...
        VbglHGCMParmUInt64Set(&Msg.f64Features1, VPOX_DND_GF_1_MUST_BE_ONE);
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (Msg.f64Features1.u.value64 & VPOX_DND_GF_1_MUST_BE_ONE)
                rc = VERR_NOT_SUPPORTED;
...
VBGLR3DECL(int) VbglR3DnDGHSendAckPending(PVBGLR3GUESTDNDCMDCTX pCtx,
                                          VPOXDNDACTION dndActionDefault, VPOXDNDACTIONLIST dndLstActionsAllowed,
...
static int vbglR3DnDGHSendDataInternal(PVBGLR3GUESTDNDCMDCTX pCtx,
                                       void *pvData, uint64_t cbData, PVPOXDNDSNDDATAHDR pDataHdr)
{
    AssertPtrReturn(pCtx,     VERR_INVALID_POINTER);
    AssertPtrReturn(pvData,   VERR_INVALID_POINTER);
    AssertReturn(cbData,      VERR_INVALID_PARAMETER);
