 ...
 *  For determining the cached VPoxOglIsOfflineRenderingAppropriate result.}
 */
static DECLCALLBACK(int32_t) vpoxOglIsOfflineRenderingAppropriateOnce(void *pvUser)
{
...
bool RTCALL VPoxOglIsOfflineRenderingAppropriate(void)
{
    ...
    int rc = RTOnce(&s_Once, vpoxOglIsOfflineRenderingAppropriateOnce, &s_fCached);
    AssertRC(rc);
    return s_fCached;
}
...
{
    if (RTEnvExist("VPOX_3D_FORCE_SUPPORTED"))
    {
        LogRel(("VPOX_3D_FORCE_SUPPORTED is specified, skipping 3D test, and treating as supported\n"));
        return true;
    }
    ...
    CGLPixelFormatAttribute aAttribs[] =
    {
        ...
        kCGLPFAAccelerated,
        kCGLPFADoubleBuffer,
        VPoxOglIsOfflineRenderingAppropriate() ? kCGLPFAAllowOfflineRenderers : (CGLPixelFormatAttribute)NULL,
