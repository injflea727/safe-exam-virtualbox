...
#ifndef VPOX_INCLUDED_vd_h
#define VPOX_INCLUDED_vd_h
...
#include <VPox/vdmedia.h>
#include <VPox/vd-ifs.h>

RT_C_DECLS_BEGIN

#ifdef IN_RING0
# error "There are no VPox HDD Container APIs available in Ring-0 Host Context!"
#endif
...
/** @name VPox HDD container image flags
...
/** @name VPox raw disk or partition flags
 * @{
 */
...
/** @name VPox HDD container image open mode flags
 * @{
 */
...
/** @name VPox HDD container filter flags
 * @{
 */
...
/** @name VPox HDD container backend capability flags
 * @{
 */
/** Supports UUIDs as expected by VirtualPox code. */
 ...
 * @returns VPox status code.
 */
VPOXDDU_DECL(int) VDShutdown(void);
 ...
 */
VPOXDDU_DECL(int) VDPluginLoadFromFilename(const char *pszFilename);

/**
 * Load all plugins from a given path.
 *
 * @returns VPox statuse code.
 * @param   pszPath         The path to load plugins from.
 */
VPOXDDU_DECL(int) VDPluginLoadFromPath(const char *pszPath);
 ...
 */
VPOXDDU_DECL(int) VDPluginUnloadFromFilename(const char *pszFilename);
 ...
 */
VPOXDDU_DECL(int) VDPluginUnloadFromPath(const char *pszPath);
 ...
 */
VPOXDDU_DECL(int) VDBackendInfo(unsigned cEntriesAlloc, PVDBACKENDINFO pEntries,
 ...
 */
VPOXDDU_DECL(int) VDBackendInfoOne(const char *pszBackend, PVDBACKENDINFO pEntry);
 ...
 * @param   pcEntriesUsed   Number of entries returned.
 */
VPOXDDU_DECL(int) VDFilterInfo(unsigned cEntriesAlloc, PVDFILTERINFO pEntries,
                               unsigned *pcEntriesUsed);
 ...
 * @param   pEntry          Pointer to an entry.
 */
VPOXDDU_DECL(int) VDFilterInfoOne(const char *pszFilter, PVDFILTERINFO pEntry);
 ...
 */
VPOXDDU_DECL(int) VDCreate(PVDINTERFACE pVDIfsDisk, VDTYPE enmType, PVDISK *ppDisk);
 ...
 */
VPOXDDU_DECL(int) VDDestroy(PVDISK pDisk);
 ...
 */
VPOXDDU_DECL(int) VDGetFormat(PVDINTERFACE pVDIfsDisk, PVDINTERFACE pVDIfsImage,
 ...
 * @param   pVDIfsImage     Pointer to the per-image VD interface list.
 */
VPOXDDU_DECL(int) VDOpen(PVDISK pDisk, const char *pszBackend,
 ...
 */
VPOXDDU_DECL(int) VDCacheOpen(PVDISK pDisk, const char *pszBackend,
                              const char *pszFilename, unsigned uOpenFlags,
 ...
 */
VPOXDDU_DECL(int) VDFilterAdd(PVDISK pDisk, const char *pszFilter, uint32_t fFlags,
 ...
 */
VPOXDDU_DECL(int) VDCreateBase(PVDISK pDisk, const char *pszBackend,
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
VPOXDDU_DECL(int) VDCreateDiff(PVDISK pDisk, const char *pszBackend,
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
VPOXDDU_DECL(int) VDCreateCache(PVDISK pDisk, const char *pszBackend,
                                const char *pszFilename, uint64_t cbSize,
                                unsigned uImageFlags, const char *pszComment,
                                PCRTUUID pUuid, unsigned uOpenFlags,
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
VPOXDDU_DECL(int) VDMerge(PVDISK pDisk, unsigned nImageFrom,
 ...
 */
VPOXDDU_DECL(int) VDCopyEx(PVDISK pDiskFrom, unsigned nImage, PVDISK pDiskTo,
 ...
 *                          for the destination operation.
 */
VPOXDDU_DECL(int) VDCopy(PVDISK pDiskFrom, unsigned nImage, PVDISK pDiskTo,
                         const char *pszBackend, const char *pszFilename,
                         bool fMoveByRename, uint64_t cbSize,
                         unsigned uImageFlags, PCRTUUID pDstUuid,
                         unsigned uOpenFlags, PVDINTERFACE pVDIfsOperation,
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
VPOXDDU_DECL(int) VDCompact(PVDISK pDisk, unsigned nImage, PVDINTERFACE pVDIfsOperation);
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
VPOXDDU_DECL(int) VDResize(PVDISK pDisk, uint64_t cbSize,
                           PCVDGEOMETRY pPCHSGeometry,
                           PCVDGEOMETRY pLCHSGeometry,
                           PVDINTERFACE pVDIfsOperation);
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
VPOXDDU_DECL(int) VDPrepareWithFilters(PVDISK pDisk, PVDINTERFACE pVDIfsOperation);
 ...
 */
VPOXDDU_DECL(int) VDClose(PVDISK pDisk, bool fDelete);
 ...
 */
VPOXDDU_DECL(int) VDFilterRemove(PVDISK pDisk, uint32_t fFlags);
 ...
 * @param   fDelete         If true, delete the image from the host disk.
 */
VPOXDDU_DECL(int) VDCacheClose(PVDISK pDisk, bool fDelete);
 ...
 * @param   pDisk           Pointer to HDD container.
 */
VPOXDDU_DECL(int) VDCloseAll(PVDISK pDisk);
 ...
 * @param   pDisk           Pointer to HDD container.
 */
VPOXDDU_DECL(int) VDFilterRemoveAll(PVDISK pDisk);
 ...
 *                          Must be aligned to a sector boundary.
 */
VPOXDDU_DECL(int) VDRead(PVDISK pDisk, uint64_t uOffset, void *pvBuf, size_t cbRead);
 ...
 *                          Must be aligned to a sector boundary.
 */
VPOXDDU_DECL(int) VDWrite(PVDISK pDisk, uint64_t uOffset, const void *pvBuf, size_t cbWrite);
 ...
 * @param   pDisk           Pointer to HDD container.
 */
VPOXDDU_DECL(int) VDFlush(PVDISK pDisk);
 ...
 * @param   pDisk           Pointer to HDD container.
 */
VPOXDDU_DECL(unsigned) VDGetCount(PVDISK pDisk);
 ...
 * @param   pDisk           Pointer to HDD container.
 */
VPOXDDU_DECL(bool) VDIsReadOnly(PVDISK pDisk);
 ...
 * @param   nImage          Image number, counts from 0. 0 is always base image of container.
 */
VPOXDDU_DECL(uint32_t) VDGetSectorSize(PVDISK pDisk, unsigned nImage);
 ...
 * @param   nImage          Image number, counts from 0. 0 is always base image of container.
 */
VPOXDDU_DECL(uint64_t) VDGetSize(PVDISK pDisk, unsigned nImage);
 ...
 * @param   nImage          Image number, counts from 0. 0 is always base image of container.
 */
VPOXDDU_DECL(uint64_t) VDGetFileSize(PVDISK pDisk, unsigned nImage);
 ...
 */
VPOXDDU_DECL(int) VDGetPCHSGeometry(PVDISK pDisk, unsigned nImage, PVDGEOMETRY pPCHSGeometry);
 ...
 */
VPOXDDU_DECL(int) VDSetPCHSGeometry(PVDISK pDisk, unsigned nImage, PCVDGEOMETRY pPCHSGeometry);
 ...
 */
VPOXDDU_DECL(int) VDGetLCHSGeometry(PVDISK pDisk, unsigned nImage, PVDGEOMETRY pLCHSGeometry);
 ...
 */
VPOXDDU_DECL(int) VDSetLCHSGeometry(PVDISK pDisk, unsigned nImage, PCVDGEOMETRY pLCHSGeometry);
 ...
 */
VPOXDDU_DECL(int) VDQueryRegions(PVDISK pDisk, unsigned nImage, uint32_t fFlags,
 ...
 */
VPOXDDU_DECL(void) VDRegionListFree(PVDREGIONLIST pRegionList);
 ...
 */
VPOXDDU_DECL(int) VDGetVersion(PVDISK pDisk, unsigned nImage, unsigned *puVersion);
 ...
 */
VPOXDDU_DECL(int) VDBackendInfoSingle(PVDISK pDisk, unsigned nImage, PVDBACKENDINFO pBackendInfo);
 ...
 */
VPOXDDU_DECL(int) VDGetImageFlags(PVDISK pDisk, unsigned nImage, unsigned *puImageFlags);
 ...
 */
VPOXDDU_DECL(int) VDGetOpenFlags(PVDISK pDisk, unsigned nImage, unsigned *puOpenFlags);
 ...
 * @param   uOpenFlags      Image file open mode, see VD_OPEN_FLAGS_* constants.
 */
VPOXDDU_DECL(int) VDSetOpenFlags(PVDISK pDisk, unsigned nImage, unsigned uOpenFlags);
 ...
 */
VPOXDDU_DECL(int) VDGetFilename(PVDISK pDisk, unsigned nImage, char *pszFilename, unsigned cbFilename);
 ...
 */
VPOXDDU_DECL(int) VDGetComment(PVDISK pDisk, unsigned nImage, char *pszComment, unsigned cbComment);
 ...
 */
VPOXDDU_DECL(int) VDSetComment(PVDISK pDisk, unsigned nImage, const char *pszComment);
 ...
 */
VPOXDDU_DECL(int) VDGetUuid(PVDISK pDisk, unsigned nImage, PRTUUID pUuid);
 ...
 * @param   pUuid           New UUID of the image. If NULL, a new UUID is created.
 */
VPOXDDU_DECL(int) VDSetUuid(PVDISK pDisk, unsigned nImage, PCRTUUID pUuid);
 ...
 */
VPOXDDU_DECL(int) VDGetModificationUuid(PVDISK pDisk, unsigned nImage, PRTUUID pUuid);
 ...
 */
VPOXDDU_DECL(int) VDSetModificationUuid(PVDISK pDisk, unsigned nImage, PCRTUUID pUuid);
 ...
 */
VPOXDDU_DECL(int) VDGetParentUuid(PVDISK pDisk, unsigned nImage, PRTUUID pUuid);
 ...
 */
VPOXDDU_DECL(int) VDSetParentUuid(PVDISK pDisk, unsigned nImage, PCRTUUID pUuid);
 ...
 * @param   pDisk           Pointer to HDD container.
 */
VPOXDDU_DECL(void) VDDumpImages(PVDISK pDisk);
 ...
 */
VPOXDDU_DECL(int) VDDiscardRanges(PVDISK pDisk, PCRTRANGE paRanges, unsigned cRanges);
 ...
 */
VPOXDDU_DECL(int) VDAsyncRead(PVDISK pDisk, uint64_t off, size_t cbRead,
                              PCRTSGBUF pcSgBuf,
 ...
 * @param   pvUser2         User data which is passed on completion.
 */
VPOXDDU_DECL(int) VDAsyncWrite(PVDISK pDisk, uint64_t off, size_t cbWrite,
                               PCRTSGBUF pcSgBuf,
                               PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                               void *pvUser1, void *pvUser2);
 ...
 * @param   pvUser2         User data which is passed on completion.
 */
VPOXDDU_DECL(int) VDAsyncFlush(PVDISK pDisk,
                               PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                               void *pvUser1, void *pvUser2);
 ...
 * @param   pvUser2         User data which is passed on completion.
 */
VPOXDDU_DECL(int) VDAsyncDiscardRanges(PVDISK pDisk, PCRTRANGE paRanges, unsigned cRanges,
                                       PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                                       void *pvUser1, void *pvUser2);
 ...
 */
VPOXDDU_DECL(int) VDRepair(PVDINTERFACE pVDIfsDisk, PVDINTERFACE pVDIfsImage,
 ...
 *                          success.
 */
VPOXDDU_DECL(int) VDCreateVfsFileFromDisk(PVDISK pDisk, uint32_t fFlags,
 ...
 */
VPOXDDU_DECL(int) VDIfTcpNetInstDefaultCreate(PVDIFINST phTcpNetInst, PVDINTERFACE *ppVdIfs);
 ...
 */
VPOXDDU_DECL(void) VDIfTcpNetInstDefaultDestroy(VDIFINST hTcpNetInst);
/** @} */
 ...
 */
VPOXDDU_DECL(int) VDIoIterCreate(PVDISK pDisk, PVDIOITER phVdIoIter, PCVDIOITERSEG paIoIterSegs,
 ...
 */
VPOXDDU_DECL(uint32_t) VDIoIterRetain(VDIOITER hVdIoIter);
 ...
 * @param   hVdIoIter       The I/O iterator handle.
 */
VPOXDDU_DECL(uint32_t) VDIoIterRelease(VDIOITER hVdIoIter);
 ...
 * @param   hVdIoIter       The I/O iterator handle.
 */
VPOXDDU_DECL(uint32_t) VDIoIterGetSegmentCount(VDIOITER hVdIoIter);
 ...
 * @param   hVdIoIter       The I/O iterator handle.
 */
VPOXDDU_DECL(uint32_t) VDIoIterGetFlags(VDIOITER hVdIoIter);
 ...
 */
VPOXDDU_DECL(int) VDIoIterQuerySegment(VDIOITER hVdIoIter, uint32_t idx, PVDIOITERSEG pSegment);
 ...
 */
VPOXDDU_DECL(int) VDIoBufMgrCreate(PVDIOBUFMGR phIoBufMgr, size_t cbMax, uint32_t fFlags,
 ...
 */
VPOXDDU_DECL(int) VDIoBufMgrDestroy(VDIOBUFMGR hIoBufMgr);
 ...
 */
VPOXDDU_DECL(int) VDIoBufMgrAllocBuf(VDIOBUFMGR hIoBufMgr, PVDIOBUF phIoBuf, void **ppvIoBufAlloc,
 ...
 */
VPOXDDU_DECL(uint32_t) VDIoBufRetain(VDIOBUF hIoBuf);
 ...
 * @param   hIoBuf          The I/O buffer handle.
 */
VPOXDDU_DECL(uint32_t) VDIoBufRelease(VDIOBUF hIoBuf);
 ...
 */
VPOXDDU_DECL(int) VDIoQueueCreate(PVDIOQUEUE phVdIoQueue, PFNVDIOQUEUEREQCOMPLETE pfnIoReqComplete,
 ...
 */
VPOXDDU_DECL(int) VDIoQueueDestroy(VDIOQUEUE hVdIoQueue);
 ...
 */
VPOXDDU_DECL(int) VDIoQueueAttach(PVDISK pDisk, VDIOQUEUE hVdIoQueue);
 ...
 *                           before detaching.
 */
VPOXDDU_DECL(int) VDIoQueueDetach(VDIOQUEUE hVdIoQueue, bool fPurge);
 ...
 * @param   hVdIoQueue       The I/O queue.
 */
VPOXDDU_DECL(int) VDIoQueuePurge(VDIOQUEUE hVdIoQueue);
 ...
 */
VPOXDDU_DECL(int) VDIoQueueReqAlloc(VDIOQUEUE hVdIoQueue, PVDIOREQ phVdIoReq,
 ...
 */
VPOXDDU_DECL(int) VDIoQueueReqFree(VDIOREQ hVdIoReq);
 ...
 */
VPOXDDU_DECL(int) VDIoQueueReqCancelById(VDIOQUEUE hVdIoQueue, VDIOREQID uIoReqId);
 ...
 */
VPOXDDU_DECL(int) VDIoQueueReqCancelByHandle(VDIOREQ hVdIoReq);
 ...
 */
VPOXDDU_DECL(int) VDIoQueueReqSubmit(VDIOREQ hVdIoReq, VDIOREQTYPE enmType,
