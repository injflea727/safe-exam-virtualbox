 ...
 * @thread  EMT(pVCpu).
 */
VMM_INT_DECL(VPOXSTRICTRC) gimKvmHypercall(PVMCPUCC pVCpu, PCPUMCTX pCtx)
{
 ...
 *
 * @returns Strict VPox status code like CPUMQueryGuestMsr().
 ...
 */
VMM_INT_DECL(VPOXSTRICTRC) gimKvmReadMsr(PVMCPUCC pVCpu, uint32_t idMsr, PCCPUMMSRRANGE pRange, uint64_t *puValue)
{
    NOREF(pRange);
 ...
 *
 * @returns Strict VPox status code like CPUMSetGuestMsr().
 ...
 */
VMM_INT_DECL(VPOXSTRICTRC) gimKvmWriteMsr(PVMCPUCC pVCpu, uint32_t idMsr, PCCPUMMSRRANGE pRange, uint64_t uRawValue)
{
    NOREF(pRange);
    switch (idMsr)
    {
 ...
 * @thread  EMT(pVCpu).
 */
VMM_INT_DECL(VPOXSTRICTRC) gimKvmHypercallEx(PVMCPUCC pVCpu, PCPUMCTX pCtx, unsigned uDisOpcode, uint8_t cbInstr)
{
    Assert(pVCpu);
    Assert(pCtx);
    VMCPU_ASSERT_EMT(pVCpu);
        ...
        || uDisOpcode == OP_VMMCALL)
    {
         ...
         */
        VPOXSTRICTRC rcStrict = gimKvmHypercall(pVCpu, pCtx);
 ...
 * @thread  EMT(pVCpu).
 */
VMM_INT_DECL(VPOXSTRICTRC) gimKvmXcptUD(PVMCC pVM, PVMCPUCC pVCpu, PCPUMCTX pCtx, PDISCPUSTATE pDis, uint8_t *pcbInstr)
{
    VMCPU_ASSERT_EMT(pVCpu);
