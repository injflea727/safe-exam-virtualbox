...
{
    VPOXSTRICTRC rcStrict = PGMPhysRead(pVM, *pGCPhys, pvBuf, cbRead, enmOrigin);
                                                   ...
                                                   PGMACCESSORIGIN enmOrigin)
{
    ...
    VPOXSTRICTRC rcStrict = PGMPhysWrite(pVM, *pGCPhys, pvBuf, cbWrite, enmOrigin);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", VPOXSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
    return VINF_SUCCESS;
}
...
#else  /* !VPOX_WITH_PGM_NEM_MODE */
    RT_NOREF(pvMmio2);
#endif /* !VPOX_WITH_PGM_NEM_MODE */
...
#endif
            {
#ifndef VPOX_WITH_LINEAR_HOST_PHYS_MEM
 ...
 *
 * @returns VINF_SUCCESS (VPox strict status code).
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure of the calling EMT. Unused.
 * @param   pvUser      User parameter
 */
static DECLCALLBACK(VPOXSTRICTRC) pgmR3PhysChangeMemBalloonRendezvous(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) pgmR3PhysWriteProtectRAMRendezvous(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    int rc = VINF_SUCCESS;
 ...
 *
 */
static DECLCALLBACK(VPOXSTRICTRC) pgmR3PhysUnmapChunkRendezvous(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    int rc = VINF_SUCCESS;
    pgmLock(pVM);
