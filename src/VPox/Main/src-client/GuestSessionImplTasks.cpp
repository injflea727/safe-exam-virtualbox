...
/** @file
 * VirtualPox Main - Guest session tasks.
 */
...
int GuestSessionTask::RunAsync(const Utf8Str &strDesc, ComObjPtr<Progress> &pProgress)
{
    ...
    return Global::vpoxStatusCodeToCOM(hrc);
}
#endif
                                             ...
                                             bool fFollowSymlinks, bool fCanExist)
{
    ...
    if (RT_SUCCESS(rc))
    {
        if (!fCanExist)
        {
            setProgressErrorMsg(VPOX_E_IPRT_ERROR,
 ...
 *
 * @return VPox status code. VERR_ALREADY_EXISTS if directory on the guest already exists.
                                             ...
                                             FileCopyFlag_T fFileCopyFlags, uint64_t offCopy, uint64_t cbSize)
{
    ...
    if (   cbSize > 0
        && cbWrittenTotal == 0)
    {
        /* If nothing was transfered but the file size was > 0 then "vpox_cat" wasn't able to write
 ...
 *
 * @return VPox status code. VINF_NO_CHANGE if file was skipped.
...
int GuestSessionTask::fileCopyFromGuest(const Utf8Str &strSrc, const Utf8Str &strDst, FileCopyFlag_T fFileCopyFlags)
{
    ...
    if (RT_FAILURE(rc))
    {
        if (rc == VERR_GSTCTL_GUEST_ERROR)
            setProgressErrorMsg(VPOX_E_IPRT_ERROR, GuestSession::tr("Guest file lookup failed"),
    ...
    {
        if (rc == VERR_GSTCTL_GUEST_ERROR)
            setProgressErrorMsg(VPOX_E_IPRT_ERROR, GuestSession::tr("Guest file could not be opened"),
        ...
        || rc == VERR_FILE_NOT_FOUND)
    {
        if (!pszDstFile)
        {
            setProgressErrorMsg(VPOX_E_IPRT_ERROR, Utf8StrFmt(GuestSession::tr("No memory to allocate host file path")));
            rc = VERR_NO_MEMORY;
        }
        else
        {
...
HRESULT GuestSessionTaskCopyFrom::Init(const Utf8Str &strTaskDesc)
{
    ...
    if (RT_FAILURE(vrc))
    {
        ...
        setProgressErrorMsg(VPOX_E_IPRT_ERROR, strErrorInfo);
    }
    ...
    else
    {
        ...
        while (itSrc != mSources.end())
        {
            ...
            try
            {
                ...
                {
                    delete pFsList;
                    strErrorInfo = Utf8StrFmt(GuestSession::tr("Error adding host source '%s' to list: %Rrc"), strSrc.c_str(), rc);
                    break;
                }
    ...
    while (itList != mVecLists.end())
    {
        ...
        if (pList->mSourceSpec.enmType == FsObjType_Directory)
        {
            ...
            switch (dstObjData.mType)
            {
                ...
                default:
                    setProgressErrorMsg(VPOX_E_IPRT_ERROR,
                                        Utf8StrFmt(GuestSession::tr("Unknown object type (%#x) on guest for \"%s\""),
               ...
               && itEntry != pList->mVecEntries.end())
        {
            ...
            if (pList->mSourceSpec.enmType == FsObjType_Directory)
            {
                ...
                if (RT_FAILURE(rc))
                    setProgressErrorMsg(VPOX_E_IPRT_ERROR,
                                        Utf8StrFmt(GuestSession::tr("Building source host path for entry \"%s\" failed (%Rrc)"),
            ...
            if (dstObjData.mType == FsObjType_Directory)
            {
                ...
                if (RT_FAILURE(rc))
                    setProgressErrorMsg(VPOX_E_IPRT_ERROR,
                                        Utf8StrFmt(GuestSession::tr("Building destination guest path for entry \"%s\" failed (%Rrc)"),
                                                   pEntry->strPath.c_str(), rc));
            }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_FAILURE(rc))
            {
                ...
                {
                    case VERR_GSTCTL_GUEST_ERROR:
                        setProgressErrorMsg(VPOX_E_IPRT_ERROR, GuestFile::i_guestErrorToString(rcGuest, strFileDst.c_str()));
                        break;
...
int GuestSessionTaskUpdateAdditions::runFileOnGuest(GuestSession *pSession, GuestProcessStartupInfo &procInfo)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            case VERR_GSTCTL_GUEST_ERROR:
                setProgressErrorMsg(VPOX_E_IPRT_ERROR, GuestSession::tr("Running update file on guest failed"),
...
int GuestSessionTaskUpdateAdditions::Run(void)
{
    ...
    if (rc == VERR_TIMEOUT)
        hr = setProgressErrorMsg(VPOX_E_NOT_SUPPORTED,
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        Utf8Str strAddsVer;
        rc = getGuestProperty(pGuest, "/VirtualPox/GuestAdd/Version", strAddsVer);
        if (   RT_SUCCESS(rc)
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        Utf8Str strOSType;
        rc = getGuestProperty(pGuest, "/VirtualPox/GuestInfo/OS/Product", strOSType);
        if (RT_SUCCESS(rc))
        {
                ...
                || strOSType.contains("Windows", Utf8Str::CaseInsensitive))
            {
                 ...
                 */
                rc = getGuestProperty(pGuest, "/VirtualPox/GuestInfo/OS/Release", strOSVer);
                if (RT_FAILURE(rc))
                {
                    hr = setProgressErrorMsg(VPOX_E_NOT_SUPPORTED,
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_FAILURE(rc))
        {
            hr = setProgressErrorMsg(VPOX_E_IPRT_ERROR,
        ...
        else
        {
            ...
            else
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (fUseInstallDir)
                    {
                        rc = getGuestProperty(pGuest, "/VirtualPox/GuestAdd/InstallDir", strUpdateDir);
                        if (RT_SUCCESS(rc))
                        {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (RT_FAILURE(rc))
                    {
                        ...
                        {
                            case VERR_GSTCTL_GUEST_ERROR:
                                hr = setProgressErrorMsg(VPOX_E_IPRT_ERROR, GuestSession::tr("Creating installation directory on guest failed"),
                    ...
                    switch (osType)
                    {
                        case eOSType_Windows:
                        {
                            ...
                            if (fInstallCert)
                            {
                                ...
                                {
                                    { "vpox.cer",           "/CERT/VPOX.CER" },
                                    { "vpox-sha1.cer",      "/CERT/VPOX-SHA1.CER" },
                                    { "vpox-sha256.cer",    "/CERT/VPOX-SHA256.CER" },
                                    { "vpox-sha256-r3.cer", "/CERT/VPOX-SHA256-R3.CER" },
                                    { "oracle-vpox.cer",    "/CERT/ORACLE-VPOX.CER" },
                                };
                                ...
                                for (uint32_t i = 0; i < RT_ELEMENTS(s_aCertFiles); i++)
                                {
                                     ...
                                     *             existing VPox certificates. */
                                    GuestProcessStartupInfo siCertUtilRem;
                                    siCertUtilRem.mName = "VirtualPox Certificate Utility, removing old VirtualPox certificates";
                                    /* The argv[0] should contain full path to the executable module */
                                    siCertUtilRem.mArguments.push_back(strUpdateDir + "VPoxCertUtil.exe");
                                    ...
                                    siCertUtilRem.mArguments.push_back(strDstCert);
                                    mFiles.push_back(ISOFile("CERT/VPOXCERTUTIL.EXE",
                                                             strUpdateDir + "VPoxCertUtil.exe",
                                    ...
                                    siCertUtilAdd.mName = "VirtualPox Certificate Utility, installing VirtualPox certificates";
                                    /* The argv[0] should contain full path to the executable module */
                                    siCertUtilAdd.mArguments.push_back(strUpdateDir + "VPoxCertUtil.exe");
                             ...
                             * the guest's bitness. */
                            mFiles.push_back(ISOFile("VPOXWINDOWSADDITIONS-X86.EXE",
                                                     strUpdateDir + "VPoxWindowsAdditions-x86.exe",
                                                     ISOFILE_FLAG_COPY_FROM_ISO));
                            mFiles.push_back(ISOFile("VPOXWINDOWSADDITIONS-AMD64.EXE",
                                                     strUpdateDir + "VPoxWindowsAdditions-amd64.exe",
                                                     ISOFILE_FLAG_COPY_FROM_ISO));
                            /* The stub loader which decides which flavor to run. */
                            GuestProcessStartupInfo siInstaller;
                            siInstaller.mName = "VirtualPox Windows Guest Additions Installer";
                            ...
                            /* The argv[0] should contain full path to the executable module */
                            siInstaller.mArguments.push_back(strUpdateDir + "VPoxWindowsAdditions.exe");
                            ...
                            /* Don't quit VPoxService during upgrade because it still is used for this
                             * piece of code we're in right now (that is, here!) ... */
                            siInstaller.mArguments.push_back(Utf8Str("/no_vpoxservice_exit"));
                            /* Tell the installer to report its current installation status
                             * using a running VPoxTray instance via balloon messages in the
                             * Windows taskbar. */
                            ...
                            mFiles.push_back(ISOFile("VPOXWINDOWSADDITIONS.EXE",
                                                     strUpdateDir + "VPoxWindowsAdditions.exe",
