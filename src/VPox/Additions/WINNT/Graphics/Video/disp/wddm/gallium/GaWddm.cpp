...
HRESULT GaD3DResourceLockRect(PVPOXWDDMDISP_RESOURCE pRc, UINT iAlloc,
                              ...
                              DWORD dwLockFlags)
{
    ...
    VPOXWDDMDISP_ALLOCATION *pAllocation = &pRc->aAllocations[iAlloc];
    Assert(pAllocation->pD3DIf);

    const VPOXDISP_D3DIFTYPE enmD3DIfType = pAllocation->enmD3DIfType;
    switch (enmD3DIfType)
    {
        ...
        case VPOXDISP_D3DIFTYPE_CUBE_TEXTURE:
        {
            ...
            hr = pD3DIfCubeTex->LockRect(VPOXDISP_CUBEMAP_INDEX_TO_FACE(pRc, iAlloc),
                                         VPOXDISP_CUBEMAP_INDEX_TO_LEVEL(pRc, iAlloc),
                                         pLockedRect, pRect, dwLockFlags);
            Assert(hr == S_OK);
            break;
        }
        case VPOXDISP_D3DIFTYPE_VERTEXBUFFER:
        {
        ...
        case VPOXDISP_D3DIFTYPE_INDEXBUFFER:
        {
...
HRESULT GaD3DResourceUnlockRect(PVPOXWDDMDISP_RESOURCE pRc, UINT iAlloc)
{
    HRESULT hr;
    Assert(pRc->cAllocations > iAlloc);
    ...
    switch (enmD3DIfType)
    {
        ...
        {
            IDirect3DCubeTexture9 *pD3DIfCubeTex = (IDirect3DCubeTexture9 *)pAllocation->pD3DIf;
            hr = pD3DIfCubeTex->UnlockRect(VPOXDISP_CUBEMAP_INDEX_TO_FACE(pRc, iAlloc),
                                           VPOXDISP_CUBEMAP_INDEX_TO_LEVEL(pRc, iAlloc));
            Assert(hr == S_OK);
            break;
        }
...
HRESULT GaD3DResourceSynchMem(PVPOXWDDMDISP_RESOURCE pRc, bool fToBackend)
{
    ...
    if (   pRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_TEXTURE
        || pRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_CUBE_TEXTURE)
    {
    ...
    else if (pRc->aAllocations[0].enmD3DIfType == VPOXDISP_D3DIFTYPE_VOLUME_TEXTURE)
    {
        ...
        {
            PVPOXWDDMDISP_ALLOCATION pAlloc = &pRc->aAllocations[Level];
            Assert(pAlloc->pvMem);
            ...
            if (SUCCEEDED(hr))
            {
                ...
                const uint32_t cRows = vpoxWddmCalcNumRows(0, pAlloc->SurfDesc.height, pAlloc->SurfDesc.format);
    ...
    else
    {
        for (UINT i = 0; i < pRc->cAllocations; ++i)
        {
            ...
            VPoxD3DIfLockUnlockMemSynch(pAlloc, &Rect, NULL, fToBackend);
...
#if 0
void VPoxD3DIfLockUnlockMemSynch(PVPOXWDDMDISP_ALLOCATION pAlloc, D3DLOCKED_RECT *pLockInfo, RECT *pRect, bool bToLockInfo)
{
    ...
    if (!pRect)
    {
        ...
        else
        {
            ...
            uint32_t cRows = vpoxWddmCalcNumRows(0, pAlloc->SurfDesc.height, pAlloc->SurfDesc.format);
    ...
    else
    {
        ...
        uint32_t offAllocMemStart = vpoxWddmCalcOffXYrd(pRect->left, pRect->top, pAlloc->SurfDesc.pitch, pAlloc->SurfDesc.format);
        ...
        {
            uint32_t cbSize = vpoxWddmCalcSize(pAlloc->SurfDesc.pitch, pRect->bottom - pRect->top, pAlloc->SurfDesc.format);
            memcpy(pvDst, pvSrc, cbSize);
        }
        else
        {
            uint32_t pitch = RT_MIN(srcPitch, dstPitch);
            uint32_t cbCopyLine = vpoxWddmCalcRowSize(pRect->left, pRect->right, pAlloc->SurfDesc.format);
            Assert(pitch); NOREF(pitch);
            uint32_t cRows = vpoxWddmCalcNumRows(pRect->top, pRect->bottom, pAlloc->SurfDesc.format);
            for (UINT j = 0; j < cRows; ++j)
            {
...
HRESULT GaD3DIfCreateForRc(struct VPOXWDDMDISP_RESOURCE *pRc)
{
    ...
    for (UINT i = 0; i < pRc->cAllocations; ++i)
    {
        ...
        pAllocation->enmD3DIfType = VPOXDISP_D3DIFTYPE_UNDEFINED;
        pAllocation->pD3DIf = NULL;
    }

    PVPOXWDDMDISP_DEVICE pDevice = pRc->pDevice;
    IDirect3DDevice9 *pDevice9If = VPOXDISP_D3DEV(pDevice);
    HRESULT hr = E_FAIL;

    const DWORD d3dUsage                     = GaDDI2D3DUsage(pRc->RcDesc.fFlags);
    const D3DFORMAT d3dFormat                = vpoxDDI2D3DFormat(pRc->RcDesc.enmFormat);
    const D3DPOOL d3dPool                    = vpoxDDI2D3DPool(pRc->RcDesc.enmPool);
    const D3DMULTISAMPLE_TYPE d3dMultiSample = vpoxDDI2D3DMultiSampleType(pRc->RcDesc.enmMultisampleType);
    const DWORD d3dMultisampleQuality        = pRc->RcDesc.MultisampleQuality;
    const BOOL d3dLockable                   = !pRc->RcDesc.fFlags.NotLockable;

    if (   VPOXWDDMDISP_IS_TEXTURE(pRc->RcDesc.fFlags)
        || pRc->RcDesc.fFlags.VideoProcessRenderTarget
        || pRc->RcDesc.fFlags.DecodeRenderTarget)
    {
        PVPOXWDDMDISP_ALLOCATION pAllocation = &pRc->aAllocations[0];
        IDirect3DBaseTexture9 *pD3DIfTex = NULL;
        VPOXDISP_D3DIFTYPE enmD3DIfType = VPOXDISP_D3DIFTYPE_UNDEFINED;
        ...
        else
        {
            ...
            hr = pDevice9If->CreateTexture(pAllocation->SurfDesc.d3dWidth,
                                           ...
                                           pSharedHandle);
            Assert(hr == S_OK && pD3DIfTex);
            enmD3DIfType = VPOXDISP_D3DIFTYPE_TEXTURE;
        }

        if (SUCCEEDED(hr))
        {
            Assert(pD3DIfTex);
            Assert(enmD3DIfType != VPOXDISP_D3DIFTYPE_UNDEFINED);

            for (UINT i = 0; i < pRc->cAllocations; ++i)
            {
                PVPOXWDDMDISP_ALLOCATION p = &pRc->aAllocations[i];
    ...
    else if (pRc->RcDesc.fFlags.RenderTarget || pRc->RcDesc.fFlags.Primary)
    {
        ...
        for (UINT i = 0; i < pRc->cAllocations; ++i)
        {
            ...
            if (   pAllocation->enmType == VPOXWDDM_ALLOC_TYPE_UMD_RC_GENERIC
                || pAllocation->enmType == VPOXWDDM_ALLOC_TYPE_STD_SHAREDPRIMARYSURFACE)
            {
            ...
            pAllocation->enmD3DIfType = VPOXDISP_D3DIFTYPE_SURFACE;
    ...
    else if (pRc->RcDesc.fFlags.VertexBuffer)
    {
        for (UINT i = 0; i < pRc->cAllocations; ++i)
        {
            ...
            pAllocation->enmD3DIfType = VPOXDISP_D3DIFTYPE_VERTEXBUFFER;
    ...
    else if (pRc->RcDesc.fFlags.IndexBuffer)
    {
        for (UINT i = 0; i < pRc->cAllocations; ++i)
        {
            ...
            pAllocation->enmD3DIfType = VPOXDISP_D3DIFTYPE_INDEXBUFFER;
...
HRESULT GaD3DIfDeviceCreate(struct VPOXWDDMDISP_DEVICE *pDevice)
{
...
static int gaD3DIfSetHostId(IGaDirect3DDevice9Ex *pGaD3DDevice9Ex, PVPOXWDDMDISP_ALLOCATION pAlloc, uint32_t hostID, uint32_t *pHostID)
{
    VPOXDISPIFESCAPE_SETALLOCHOSTID SetHostID;
    RT_ZERO(SetHostID);
    SetHostID.EscapeHdr.escapeCode = VPOXESC_SETALLOCHOSTID;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        WARN(("pfnEscapeCb VPOXESC_SETALLOCHOSTID failed hr 0x%x", hr));

    return VERR_GENERAL_FAILURE;
}

IUnknown *GaD3DIfCreateSharedPrimary(struct VPOXWDDMDISP_ALLOCATION *pAlloc)
{
     ...
     */
    PVPOXWDDMDISP_RESOURCE pRc = pAlloc->pRc;

    AssertReturn(pAlloc->enmType = VPOXWDDM_ALLOC_TYPE_STD_SHAREDPRIMARYSURFACE, NULL);
    ...
    Assert(pAlloc->enmD3DIfType == VPOXDISP_D3DIFTYPE_SURFACE);

    IDirect3DSurface9 *pSurfIf = NULL;
    hr = VPoxD3DIfSurfGet(pRc, pAlloc->iAlloc, &pSurfIf);
    if (FAILED(hr))
    {
        WARN(("VPoxD3DIfSurfGet hr %#x", hr));
        return NULL;
    }
...
static HRESULT gaWddmGetD3D9Caps(VPOXWDDM_QAI const *pAdapterInfo, IDirect3D9Ex *pD3D9If, D3DCAPS9 *pCaps)
{
    ...
    if (RT_BOOL(pAdapterInfo->u32AdapterCaps & VPOXWDDM_QAI_CAP_DXVAHD))
...
static void gaWddmD3DBackendClose(PVPOXWDDMDISP_D3D pD3D)
{
...
HRESULT GaWddmD3DBackendOpen(PVPOXWDDMDISP_D3D pD3D, VPOXWDDM_QAI const *pAdapterInfo, PVPOXWDDMDISP_FORMATS pFormats)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            WARN(("vpoxWddmGetD3D9Caps failed hr = 0x%x", hr));
        }
        else
        {
    ...
    {
        WARN(("VPoxDispD3DOpen failed hr = 0x%x", hr));
    }
    return hr;
}
