...
#ifndef VPOX_INCLUDED_vmm_pdmpcidev_h
#define VPOX_INCLUDED_vmm_pdmpcidev_h
 ...
 */
typedef DECLCALLBACK(VPOXSTRICTRC) FNPCICONFIGREAD(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev,
 ...
 *          that is very likely be a lock order violation.
 */
typedef DECLCALLBACK(VPOXSTRICTRC) FNPCICONFIGWRITE(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev,
...
typedef struct PDMPCIDEV
{
     ...
     * @sa VPOX_PCI_DEVFN_MAKE, VPOX_PCI_DEVFN_FUN_MASK, VPOX_PCI_DEVFN_DEV_SHIFT */
    uint32_t                uDevFn;
...
{
    PDMPciDevSetWord(pPciDev, VPOX_PCI_VENDOR_ID, u16VendorId);
}
...
{
    return PDMPciDevGetWord(pPciDev, VPOX_PCI_VENDOR_ID);
}
...
{
    PDMPciDevSetWord(pPciDev, VPOX_PCI_DEVICE_ID, u16DeviceId);
}
...
{
    return PDMPciDevGetWord(pPciDev, VPOX_PCI_DEVICE_ID);
}
...
{
    PDMPciDevSetWord(pPciDev, VPOX_PCI_COMMAND, u16Command);
}
...
{
    return PDMPciDevGetWord(pPciDev, VPOX_PCI_COMMAND);
}
...
{
    return (PDMPciDevGetCommand(pPciDev) & VPOX_PCI_COMMAND_MASTER) != 0;
}
...
{
    return (PDMPciDevGetCommand(pPciDev) & VPOX_PCI_COMMAND_INTX_DISABLE) != 0;
}
...
{
    return PDMPciDevGetWord(pPciDev, VPOX_PCI_STATUS);
}
...
{
    PDMPciDevSetWord(pPciDev, VPOX_PCI_STATUS, u16Status);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_REVISION_ID, u8RevisionId);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_CLASS_PROG, u8ClassProg);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_CLASS_SUB, u8SubClass);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_CLASS_BASE, u8BaseClass);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_HEADER_TYPE, u8HdrType);
}
...
{
    return PDMPciDevGetByte(pPciDev, VPOX_PCI_HEADER_TYPE);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_BIST, u8Bist);
}
...
{
    return PDMPciDevGetByte(pPciDev, VPOX_PCI_BIST);
}
                                         ...
                                         uint32_t u32Addr)
{
    ...
    switch (iReg)
    {
        case 0: iReg = VPOX_PCI_BASE_ADDRESS_0; break;
        case 1: iReg = VPOX_PCI_BASE_ADDRESS_1; break;
        case 2: iReg = VPOX_PCI_BASE_ADDRESS_2; break;
        case 3: iReg = VPOX_PCI_BASE_ADDRESS_3; break;
        case 4: iReg = VPOX_PCI_BASE_ADDRESS_4; break;
        case 5: iReg = VPOX_PCI_BASE_ADDRESS_5; break;
...
{
    PDMPciDevSetWord(pPciDev, VPOX_PCI_SUBSYSTEM_VENDOR_ID, u16SubSysVendorId);
}
...
{
    return PDMPciDevGetWord(pPciDev, VPOX_PCI_SUBSYSTEM_VENDOR_ID);
}
...
{
    PDMPciDevSetWord(pPciDev, VPOX_PCI_SUBSYSTEM_ID, u16SubSystemId);
}
...
{
    return PDMPciDevGetWord(pPciDev, VPOX_PCI_SUBSYSTEM_ID);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_CAPABILITY_LIST, u8Offset);
}
...
{
    return PDMPciDevGetByte(pPciDev, VPOX_PCI_CAPABILITY_LIST);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_INTERRUPT_LINE, u8Line);
}
...
{
    return PDMPciDevGetByte(pPciDev, VPOX_PCI_INTERRUPT_LINE);
}
...
{
    PDMPciDevSetByte(pPciDev, VPOX_PCI_INTERRUPT_PIN, u8Pin);
}
...
{
    return PDMPciDevGetByte(pPciDev, VPOX_PCI_INTERRUPT_PIN);
}
...
#endif /* !VPOX_INCLUDED_vmm_pdmpcidev_h */
