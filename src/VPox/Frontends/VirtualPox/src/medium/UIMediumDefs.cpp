/* $Id: UIMediumDefs.cpp $ */
/** @file
 * VPox Qt GUI - UIMedium related implementations.
 */
...
QList<QPair<QString, QString> > UIMediumDefs::MediumBackends(const CVirtualPox &comVPox, KDeviceType enmType)
{
    /* Prepare a list of pairs with the form <tt>{"Backend Name", "*.suffix1 .suffix2 ..."}</tt>. */
    const CSystemProperties comSystemProperties = comVPox.GetSystemProperties();
...
QList<QPair<QString, QString> > UIMediumDefs::HDDBackends(const CVirtualPox &comVPox)
{
    return MediumBackends(comVPox, KDeviceType_HardDisk);
}

QList<QPair<QString, QString> > UIMediumDefs::DVDBackends(const CVirtualPox &comVPox)
{
    return MediumBackends(comVPox, KDeviceType_DVD);
}

QList<QPair<QString, QString> > UIMediumDefs::FloppyBackends(const CVirtualPox &comVPox)
{
    return MediumBackends(comVPox, KDeviceType_Floppy);
}

QString UIMediumDefs::getPreferredExtensionForMedium(KDeviceType enmDeviceType)
{
    CSystemProperties comSystemProperties = uiCommon().virtualPox().GetSystemProperties();
    QVector<CMediumFormat> mediumFormats = comSystemProperties.GetMediumFormats();
    for (int i = 0; i < mediumFormats.size(); ++i)
    {
        /* File extensions */
