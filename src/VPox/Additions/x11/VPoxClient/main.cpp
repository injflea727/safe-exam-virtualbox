/* $Id: main.cpp $ */
/** @file
 * VirtualPox Guest Additions - X11 Client.
 */
...
*   Defines                                                                                                                      *
*********************************************************************************************************************************/
#define VPOXCLIENT_OPT_NORESPAWN            950

#define VPOXCLIENT_OPT_SERVICES             980
#define VPOXCLIENT_OPT_CHECKHOSTVERSION     VPOXCLIENT_OPT_SERVICES
#define VPOXCLIENT_OPT_CLIPBOARD            VPOXCLIENT_OPT_SERVICES + 1
#define VPOXCLIENT_OPT_DRAGANDDROP          VPOXCLIENT_OPT_SERVICES + 2
#define VPOXCLIENT_OPT_SEAMLESS             VPOXCLIENT_OPT_SERVICES + 3
#define VPOXCLIENT_OPT_VMSVGA               VPOXCLIENT_OPT_SERVICES + 4
#define VPOXCLIENT_OPT_DISPLAY              VPOXCLIENT_OPT_SERVICES + 5
 ...
 */
static void vpoxClientSignalHandler(int cSignal)
{
 ...
 */
static int vpoxClientXLibErrorHandler(Display *pDisplay, XErrorEvent *pError)
{
    char errorText[1024];
 ...
 * when X exits.
 */
static int vpoxClientXLibIOErrorHandler(Display *pDisplay)
{
    RT_NOREF1(pDisplay);
 ...
 * cleans up and exits.
 */
static void vpoxClientSetSignalHandlers(void)
{
    struct sigaction sigAction;

    LogRelFlowFuncEnter();
    sigAction.sa_handler = vpoxClientSignalHandler;
 ...
 */
static void vpoxClientUsage(const char *pcszFileName)
{
    RTPrintf("Usage: %s "
...
# ifdef VPOX_WITH_GUEST_PROPS
             ...
             "--vmsvga"
             "[-d|--nodaemon]\n", pcszFileName);
    RTPrintf("Starts the VirtualPox DRM/X Window System guest services.\n\n");
    RTPrintf("Options:\n");
#ifdef VPOX_WITH_SHARED_CLIPBOARD
...
/**
 * The main loop for the VPoxClient daemon.
...
int main(int argc, char *argv[])
{
    ...
    static const RTGETOPTDEF s_aOptions[] =
    {
        ...
        { "--no-respawn",                   VPOXCLIENT_OPT_NORESPAWN,                 RTGETOPT_REQ_NOTHING },
        { "--version",                      'V',                                      RTGETOPT_REQ_NOTHING },
        { "--verbose",                      'v',                                      RTGETOPT_REQ_NOTHING },

        /* Services */
        { "--checkhostversion",             VPOXCLIENT_OPT_CHECKHOSTVERSION,          RTGETOPT_REQ_NOTHING },
#ifdef VPOX_WITH_SHARED_CLIPBOARD
        { "--clipboard",                    VPOXCLIENT_OPT_CLIPBOARD,                 RTGETOPT_REQ_NOTHING },
#endif
#ifdef VPOX_WITH_DRAG_AND_DROP
        { "--draganddrop",                  VPOXCLIENT_OPT_DRAGANDDROP,               RTGETOPT_REQ_NOTHING },
#endif
        { "--seamless",                     VPOXCLIENT_OPT_SEAMLESS,                  RTGETOPT_REQ_NOTHING },
        { "--vmsvga",                       VPOXCLIENT_OPT_VMSVGA,                    RTGETOPT_REQ_NOTHING },
        { "--display",                      VPOXCLIENT_OPT_DISPLAY,                   RTGETOPT_REQ_NOTHING },
    };
    ...
    while ((ch = RTGetOpt(&GetState, &ValueUnion)) != 0)
    {
        ...
        switch (ch)
        {
            ...
            case VPOXCLIENT_OPT_CHECKHOSTVERSION:
            {
                if (g_pService)
...
#endif
#ifdef VPOX_WITH_DRAG_AND_DROP
            case VPOXCLIENT_OPT_DRAGANDDROP:
            {
                if (g_pService)
                    return vbclSyntaxOnlyOneService();
    ...
    vpoxClientSetSignalHandlers();
#ifndef VPOXCLIENT_WITHOUT_X11
    /* Set an X11 error handler, so that we don't die when we get unavoidable
     * errors. */
    XSetErrorHandler(vpoxClientXLibErrorHandler);
    /* Set an X11 I/O error handler, so that we can shutdown properly on
     * fatal errors. */
    XSetIOErrorHandler(vpoxClientXLibIOErrorHandler);
#endif
