/* $Id: VPoxGuest-win.cpp $ */
/** @file
 * VPoxGuest - Windows specifics.
 */
...
#define VPOX_CM_PRE_VISTA_MASK (0x3f)
 ...
 */
typedef struct VPOXGUESTDEVEXTWIN
{
    /** The common device extension core. */
    VPOXGUESTDEVEXT         Core;
    ...
    /** Last system power action set (see VPoxGuestPower). */
...
typedef VPOXGUESTDEVEXTWIN *PVPOXGUESTDEVEXTWIN;
...
static NTSTATUS       vgdrvNtDeviceControlSlow(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession,
                                               PIRP pIrp, PIO_STACK_LOCATION pStack);
static NTSTATUS NTAPI vgdrvNtInternalIOCtl(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static void           vgdrvNtReadConfiguration(PVPOXGUESTDEVEXTWIN pDevExt);
...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDrvObj, PUNICODE_STRING pRegPath)
{
        ...
        && pNtHdrs->FileHeader.NumberOfSections < 64)
    {
        ...
        RTLogBackdoorPrintf("VPoxGuest: Bad pNtHdrs=%p: %#x\n", pNtHdrs, pNtHdrs->Signature);
#endif
    ...
    if (RT_FAILURE(rc))
    {
        RTLogBackdoorPrintf("VPoxGuest: RTR0Init failed: %Rrc!\n", rc);
    ...
    /* Use RTLogBackdoorPrintf to make sure that this goes to VPox.log on the host. */
    RTLogBackdoorPrintf("VPoxGuest: Windows version %u.%u, build %u\n", ulMajorVer, ulMinorVer, ulBuildNo);
    if (fCheckedBuild)
        RTLogBackdoorPrintf("VPoxGuest: Windows checked build\n");
    ...
    if (NT_SUCCESS(rcNt))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            {
                RTLogBackdoorPrintf("VPoxGuest: Missing HalAssignSlotResources!\n");
                rc = VERR_SYMBOL_NOT_FOUND;
            }
            ...
            {
                RTLogBackdoorPrintf("VPoxGuest: Missing HalGetBusDataByOffset!\n");
                rc = VERR_SYMBOL_NOT_FOUND;
            }
            ...
            {
                RTLogBackdoorPrintf("VPoxGuest: Missing HalSetBusDataByOffset!\n");
                rc = VERR_SYMBOL_NOT_FOUND;
            }
...
/**
 * Translates our internal NT version enum to VPox OS.
 *
 * @returns VPox OS type.
 * @param   enmNtVer            The NT version.
 */
static VPOXOSTYPE vgdrvNtVersionToOSType(VGDRVNTVER enmNtVer)
{
    VPOXOSTYPE enmOsType;
    switch (enmNtVer)
    {
        case VGDRVNTVER_WINNT310:   enmOsType = VPOXOSTYPE_WinNT3x; break;
        case VGDRVNTVER_WINNT350:   enmOsType = VPOXOSTYPE_WinNT3x; break;
        case VGDRVNTVER_WINNT351:   enmOsType = VPOXOSTYPE_WinNT3x; break;
        case VGDRVNTVER_WINNT4:     enmOsType = VPOXOSTYPE_WinNT4; break;
        case VGDRVNTVER_WIN2K:      enmOsType = VPOXOSTYPE_Win2k; break;
        case VGDRVNTVER_WINXP:      enmOsType = VPOXOSTYPE_WinXP; break;
        case VGDRVNTVER_WIN2K3:     enmOsType = VPOXOSTYPE_Win2k3; break;
        case VGDRVNTVER_WINVISTA:   enmOsType = VPOXOSTYPE_WinVista; break;
        case VGDRVNTVER_WIN7:       enmOsType = VPOXOSTYPE_Win7; break;
        case VGDRVNTVER_WIN8:       enmOsType = VPOXOSTYPE_Win8; break;
        case VGDRVNTVER_WIN81:      enmOsType = VPOXOSTYPE_Win81; break;
        case VGDRVNTVER_WIN10:      enmOsType = VPOXOSTYPE_Win10; break;
        case VGDRVNTVER_WIN11:      enmOsType = VPOXOSTYPE_Win11_x64; break;

        default:
            /* We don't know, therefore NT family. */
            enmOsType = VPOXOSTYPE_WinNT;
            break;
    }
#if ARCH_BITS == 64
    enmOsType = (VPOXOSTYPE)((int)enmOsType | VPOXOSTYPE_x64);
#endif
    return enmOsType;
}
 ...
 */
static NTSTATUS vgdrvNtInitDevExtFundament(PVPOXGUESTDEVEXTWIN pDevExt, PDEVICE_OBJECT pDevObj)
{
    RT_ZERO(*pDevExt);
 ...
 * @param   pDevExt             The device extension.
 */
static void vgdrvNtDeleteDevExtFundament(PVPOXGUESTDEVEXTWIN pDevExt)
{
 ...
 */
static NTSTATUS vgdrvNtScanPCIResourceList(PVPOXGUESTDEVEXTWIN pDevExt, PCM_RESOURCE_LIST pResList, bool fTranslated)
{
 ...
 */
static NTSTATUS vgdrvNt31ScanSlotResources(PVPOXGUESTDEVEXTWIN pDevExt, ULONG uBus, ULONG uSlot)
{
    /*
     ...
     */
    uint16_t fCmd = 0;
    g_pfnHalGetBusDataByOffset(PCIConfiguration, uBus, uSlot, &fCmd, VPOX_PCI_COMMAND, sizeof(fCmd));
    if (fCmd & VPOX_PCI_COMMAND_MEMORY)
    {
        uint16_t fCmdTmp = fCmd & ~VPOX_PCI_COMMAND_MEMORY;
        g_pfnHalSetBusDataByOffset(PCIConfiguration, uBus, uSlot, &fCmdTmp, VPOX_PCI_COMMAND, sizeof(fCmdTmp));
    }
    ...
    g_pfnHalGetBusDataByOffset(PCIConfiguration, uBus, uSlot, &aBars, VPOX_PCI_BASE_ADDRESS_0, sizeof(aBars));
    ...
    for (uint32_t i = 0; i < RT_ELEMENTS(aBars); i++)
    {
        ...
        else
        {
            ...
            if (!fGotMmio)
            {
                ...
                uint32_t const offPciBar = VPOX_PCI_BASE_ADDRESS_0 + i * 4;
    ...
    g_pfnHalGetBusDataByOffset(PCIConfiguration, uBus, uSlot, &Buf, VPOX_PCI_INTERRUPT_LINE, sizeof(Buf));
    ...
    if (fGotIoPorts && (!fGotMmio || Buf.bInterruptPin != 0))
    {
         ...
         */
        uint16_t fCmdNew = fCmd | VPOX_PCI_COMMAND_IO | VPOX_PCI_COMMAND_MEMORY | VPOX_PCI_COMMAND_MASTER;
        g_pfnHalSetBusDataByOffset(PCIConfiguration, uBus, uSlot, &fCmdNew, VPOX_PCI_COMMAND, sizeof(fCmdNew));
    ...
    if (!fGotIoPorts)
        LogRel(("VPoxGuest: Did not find I/O port range: %#x %#x %#x %#x %#x %#x\n",
                aBars[0], aBars[1], aBars[2], aBars[3], aBars[4], aBars[5]));
    if (!fGotMmio || Buf.bInterruptPin != 0)
        LogRel(("VPoxGuest: Got MMIO but no interrupts!\n"));

    g_pfnHalSetBusDataByOffset(PCIConfiguration, uBus, uSlot, &fCmd, VPOX_PCI_COMMAND, sizeof(fCmd));
 ...
 * @param   pDevExt     The device extension.
 */
static void vgdrvNtUnmapVMMDevMemory(PVPOXGUESTDEVEXTWIN pDevExt)
{
 ...
 */
static NTSTATUS vgdrvNtMapVMMDevMemory(PVPOXGUESTDEVEXTWIN pDevExt, PHYSICAL_ADDRESS PhysAddr, ULONG cbToMap,
 ...
 */
static NTSTATUS vgdrvNtSetupDevice(PVPOXGUESTDEVEXTWIN pDevExt, PDEVICE_OBJECT pDevObj,
                                   PIRP pIrp, PDRIVER_OBJECT pDrvObj, PUNICODE_STRING pRegPath)
{
    ...
    if (!pIrp)
    {
         ...
         */
        LogFlowFunc(("Starting to scan PCI resources of VPoxGuest ...\n"));

        /* Assign the PCI resources. */
        UNICODE_STRING      ClassName;
        RtlInitUnicodeString(&ClassName, L"VPoxGuestAdapter");
                                               ...
                                               void *pvData, ULONG offData, ULONG cbData)
{
     ...
     */
    ASMOutU32(PCI_CFG_ADDR,  idxAddrTop | (VPOX_PCI_HEADER_TYPE & ~3));
    uint8_t bHdrType = ASMInU8(PCI_CFG_DATA + (VPOX_PCI_HEADER_TYPE & 3));
    if (bHdrType == 0xff)
        return idxBus < 8 ? 2 : 0; /* No device here */
    if (   offData == VPOX_PCI_HEADER_TYPE
        && cbData == 1)
    {
    ...
    for (ULONG uBus = 0; uBus < PCI_MAX_BUSES; uBus++)
    {
        ...
        for (ULONG idxDevice = 0; idxDevice < PCI_MAX_DEVICES; idxDevice++)
        {
            ...
            ULONG cbRet = g_pfnHalGetBusDataByOffset(PCIConfiguration, uBus, Slot.u.AsULONG,
                                                     &bHeaderType, VPOX_PCI_HEADER_TYPE, sizeof(bHeaderType));
            if (cbRet == 0)
                break;
            ...
            for (ULONG idxFunction = 0; idxFunction < PCI_MAX_FUNCTION; idxFunction += cFunctionStep)
            {
                ...
                cbRet = g_pfnHalGetBusDataByOffset(PCIConfiguration, uBus, Slot.u.AsULONG, &Buf, VPOX_PCI_VENDOR_ID, sizeof(Buf));
                if (   cbRet == sizeof(Buf)
...
static NTSTATUS vgdrvNt4CreateDevice(PDRIVER_OBJECT pDrvObj, PUNICODE_STRING pRegPath)
{
    ...
    RtlInitUnicodeString(&DevName, VPOXGUEST_DEVICE_NAME_NT);
    PDEVICE_OBJECT pDeviceObject = NULL;
    rc = IoCreateDevice(pDrvObj, sizeof(VPOXGUESTDEVEXTWIN), &DevName, FILE_DEVICE_UNKNOWN, 0, FALSE, &pDeviceObject);
    if (NT_SUCCESS(rc))
    {
        Log(("vgdrvNt4CreateDevice: Device created\n"));

        UNICODE_STRING DosName;
        RtlInitUnicodeString(&DosName, VPOXGUEST_DEVICE_NAME_DOS);
        ...
        if (NT_SUCCESS(rc))
        {
            ...
            PVPOXGUESTDEVEXTWIN pDevExt = (PVPOXGUESTDEVEXTWIN)pDeviceObject->DeviceExtension;
 ...
 *
 * @remarks Parts of this is duplicated in VPoxGuest-win-legacy.cpp.
 */
static NTSTATUS NTAPI vgdrvNtNt5PlusAddDevice(PDRIVER_OBJECT pDrvObj, PDEVICE_OBJECT pDevObj)
{
    LogFlowFuncEnter();
    ...
    RtlInitUnicodeString(&DevName, VPOXGUEST_DEVICE_NAME_NT);
    PDEVICE_OBJECT pDeviceObject = NULL;
    NTSTATUS rcNt = IoCreateDevice(pDrvObj, sizeof(VPOXGUESTDEVEXTWIN), &DevName, FILE_DEVICE_UNKNOWN, 0, FALSE, &pDeviceObject);
    if (NT_SUCCESS(rcNt))
    {
        /*
 ...
 * @param   pDevExt         The device extension.
 */
static void vgdrvNtDeleteDeviceResources(PVPOXGUESTDEVEXTWIN pDevExt)
{
    ...
    if (pDevExt->Core.uInitState == VPOXGUESTDEVEXT_INIT_STATE_RESOURCES)
        VGDrvCommonDeleteDevExtResources(&pDevExt->Core);
    vgdrvNtUnmapVMMDevMemory(pDevExt);
}
 ...
 * @param   pDevExt         The device extension.
 */
static void vgdrvNtDeleteDeviceFundamentAndUnlink(PDEVICE_OBJECT pDevObj, PVPOXGUESTDEVEXTWIN pDevExt)
{
    /*
 ...
 */
static NTSTATUS vgdrvNtCheckIdle(PVPOXGUESTDEVEXTWIN pDevExt, const char *pszQueryNm)
{
...
static NTSTATUS NTAPI vgdrvNtNt5PlusPnP(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    PVPOXGUESTDEVEXTWIN pDevExt = (PVPOXGUESTDEVEXTWIN)pDevObj->DeviceExtension;
    PIO_STACK_LOCATION  pStack  = IoGetCurrentIrpStackLocation(pIrp);
    ...
    switch (bMinorFunction)
    {
...
#ifdef VPOX_REBOOT_ON_UNINSTALL
        ...
        /*
         * We do nothing here actually, esp. since this request is not expected for VPoxGuest.
        ...
        case IRP_MN_SURPRISE_REMOVAL:
        {
            ...
            LogRel(("VPoxGuest: unexpected device removal\n"));
 ...
 * @param pContext  Context value passed to IoSetCompletionRoutine in VPoxGuestPower.
 */
static NTSTATUS vgdrvNtNt5PlusPowerComplete(IN PDEVICE_OBJECT pDevObj, IN PIRP pIrp, IN PVOID pContext)
{
#ifdef VPOX_STRICT
    RT_NOREF1(pDevObj);
    PVPOXGUESTDEVEXTWIN pDevExt = (PVPOXGUESTDEVEXTWIN)pContext;
...
static void NTAPI vgdrvNtUnload(PDRIVER_OBJECT pDrvObj)
{
    ...
    if (pDevObj)
    {
        if (g_enmVGDrvNtVer <= VGDRVNTVER_WINNT4)
        {
            PVPOXGUESTDEVEXTWIN pDevExt = (PVPOXGUESTDEVEXTWIN)pDevObj->DeviceExtension;
            AssertPtr(pDevExt);
            AssertMsg(pDevExt->Core.uInitState == VPOXGUESTDEVEXT_INIT_STATE_RESOURCES,
...
static NTSTATUS NTAPI vgdrvNtClose(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    ...
    PVPOXGUESTSESSION pSession = (PVPOXGUESTSESSION)pFileObj->FsContext;
    if (pSession)
...
NTSTATUS NTAPI vgdrvNtDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    PVPOXGUESTDEVEXTWIN pDevExt  = (PVPOXGUESTDEVEXTWIN)pDevObj->DeviceExtension;
    PIO_STACK_LOCATION  pStack   = IoGetCurrentIrpStackLocation(pIrp);
    PVPOXGUESTSESSION   pSession = pStack->FileObject ? (PVPOXGUESTSESSION)pStack->FileObject->FsContext : NULL;
...
static VOID NTAPI vgdrvNtBugCheckCallback(PVOID pvBuffer, ULONG cbBuffer)
{
    ...
    {
        RTLogBackdoorPrintf("VPoxGuest: BugCheck! P0=%#zx P1=%#zx P2=%#zx P3=%#zx P4=%#zx\n", g_pauKiBugCheckData[0],
    ...
    {
        RTLogBackdoorPrintf("VPoxGuest: BugCheck!\n");
...
{
    PVPOXGUESTDEVEXTWIN pDevExtWin = (PVPOXGUESTDEVEXTWIN)pDevExt;
...
static BOOLEAN NTAPI vgdrvNtIsrHandler(PKINTERRUPT pInterrupt, PVOID pServiceContext)
{
    ...
    PVPOXGUESTDEVEXTWIN pDevExt = (PVPOXGUESTDEVEXTWIN)pServiceContext;
    if (pDevExt == NULL)
        return FALSE;
                                                  ...
                                                  PVOID pvValue, ULONG cbValue, PVOID pvUser, PVOID pvEntryCtx)
{
    ...
    if (cch < 64 && cch > 0)
    {
            ...
            switch (uValueType)
            {
                ...
                case REG_EXPAND_SZ:
                    ...
                    if (RT_SUCCESS(rc) && cch < _1K)
                    {
                        ...
                        if (RT_FAILURE(rc))
                        {
                            LogRel(("VPoxGuest: Failed to convert registry value '%ls' string data to UTF-8: %Rrc\n",
                                    pwszValueName, rc));
                            pszValue = NULL;
                        }
                    }
                    else if (RT_SUCCESS(rc))
                        LogRel(("VPoxGuest: Registry value '%ls' has a too long value: %#x (uvalueType=%#x)\n",
                                pwszValueName, cbValue, uValueType));
                    else
                        LogRel(("VPoxGuest: Registry value '%ls' has an invalid string value (cbValue=%#x, uvalueType=%#x)\n",
                                pwszValueName, cbValue, uValueType));
                    break;
                ...
                case REG_DWORD:
                    if (cbValue == sizeof(uint32_t))
                    {
                        ...
                        LogRel(("VPoxGuest: Registry value '%ls' has wrong length for REG_DWORD: %#x\n", pwszValueName, cbValue));
                    break;
                ...
                default:
                    LogRel(("VPoxGuest: Ignoring registry value '%ls': Unsupported type %#x\n", pwszValueName, uValueType));
                    break;
            }
            if (pszValue)
            {
        ...
        LogRel(("VPoxGuest: Ignoring registery value '%ls': name too long\n", pwszValueName));
    else
        LogRel(("VPoxGuest: Ignoring registery value with bad name\n", pwszValueName));
    NOREF(pvEntryCtx);
    return STATUS_SUCCESS;
}
...
static void vgdrvNtReadConfiguration(PVPOXGUESTDEVEXTWIN pDevExt)
{
    ...
    NTSTATUS rcNt = RtlQueryRegistryValues(RTL_REGISTRY_SERVICES, L"VPoxGuest", &aQuery[0], pDevExt, NULL /*pwszzEnv*/);
    if (!NT_SUCCESS(rcNt))
        LogRel(("VPoxGuest: RtlQueryRegistryValues failed: %#x\n", rcNt));
...
int VGDrvNtIOCtl_DpcLatencyChecker(void)
{
    ...
    if (!pData)
    {
        RTLogBackdoorPrintf("VPoxGuest: DPC: DPCDATA allocation failed.\n");
...
#endif /* VPOX_WITH_DPC_LATENCY_CHECKER */
