 ...
 *
 * @returns VPox status code. If success, then the search is completed.
...
int dbgfR3AsInit(PUVM pUVM)
{
     ...
     */
    rc = RTDbgCfgCreate(&pUVM->dbgf.s.hDbgCfg, "VPOXDBG_", true /*fNativePaths*/);
    AssertRCReturn(rc, rc);
    ...
    {
        { RTDBGCFGPROP_FLAGS,               "VPOXDBG_FLAGS",            "Flags"             },
        { RTDBGCFGPROP_PATH,                "VPOXDBG_PATH",             "Path"              },
        { RTDBGCFGPROP_SUFFIXES,            "VPOXDBG_SUFFIXES",         "Suffixes"          },
        { RTDBGCFGPROP_SRC_PATH,            "VPOXDBG_SRC_PATH",         "SrcPath"           },
    };
    ...
    /*
     * Prepend the NoArch and VPoxDbgSyms directories to the path.
     */
    char szPath[RTPATH_MAX];
    rc = RTPathAppPrivateNoArch(szPath, sizeof(szPath));
    AssertRCReturn(rc, rc);
#ifdef RT_OS_DARWIN
    rc = RTPathAppend(szPath, sizeof(szPath), "../Resources/VPoxDbgSyms/");
#else
    rc = RTDbgCfgChangeString(pUVM->dbgf.s.hDbgCfg, RTDBGCFGPROP_PATH, RTDBGCFGOP_PREPEND, szPath);
    AssertRCReturn(rc, rc);

    rc = RTPathAppend(szPath, sizeof(szPath), "VPoxDbgSyms/");
#endif
    AssertRCReturn(rc, rc);
    rc = RTDbgCfgChangeString(pUVM->dbgf.s.hDbgCfg, RTDBGCFGPROP_PATH, RTDBGCFGOP_PREPEND, szPath);
    AssertRCReturn(rc, rc);
 ...
 *
 * @returns VPox status code. See RTDbgAsSymbolByAddr.
 *
 * @param   pUVM            The user mode VM handle.
 * @param   hDbgAs          The address space handle.
