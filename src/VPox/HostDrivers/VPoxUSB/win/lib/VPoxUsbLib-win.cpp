/* $Id: VPoxUsbLib-win.cpp $ */
/** @file
 * VPox USB ring-3 Driver Interface library, Windows.
 */
...
typedef struct VPOXUSBGLOBALSTATE
{
    HANDLE hMonitor;
    HANDLE hNotifyEvent;
    ...
    HANDLE hTimerQueue;
    HANDLE hTimer;
} VPOXUSBGLOBALSTATE, *PVPOXUSBGLOBALSTATE;

typedef struct VPOXUSB_STRING_DR_ENTRY
{
    struct VPOXUSB_STRING_DR_ENTRY *pNext;
    UCHAR iDr;
    USHORT idLang;
    USB_STRING_DESCRIPTOR StrDr;
} VPOXUSB_STRING_DR_ENTRY, *PVPOXUSB_STRING_DR_ENTRY;

/**
 * This represents VPoxUsb device instance
 */
typedef struct VPOXUSB_DEV
{
    struct VPOXUSB_DEV *pNext;
    char                szName[512];
    char                szDriverRegName[512];
} VPOXUSB_DEV, *PVPOXUSB_DEV;


/*********************************************************************************************************************************
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static VPOXUSBGLOBALSTATE g_VPoxUsbGlobal;


static void usbLibVuFreeDevices(PVPOXUSB_DEV pDevInfos)
{
    while (pDevInfos)
    {
        PVPOXUSB_DEV pNext = pDevInfos->pNext;
        RTMemFree(pDevInfos);
        pDevInfos = pNext;
    }
}
...
static int usbLibVuDeviceValidate(PVPOXUSB_DEV pVuDev)
{
...
#ifndef VPOX_WITH_NEW_USB_ENUM
static int usbLibVuDevicePopulate(PVPOXUSB_DEV pVuDev, HDEVINFO hDevInfo, PSP_DEVICE_INTERFACE_DATA pIfData)
{
    ...
    do
    {
        ...
        LogRelFunc(("Found VPoxUSB on `%s' (rc=%d)\n", pVuDev->szName, rc));
        AssertRC(rc);
    } while (0);
...
static int usbLibVuGetDevices(PVPOXUSB_DEV *ppVuDevs, uint32_t *pcVuDevs)
{
    *ppVuDevs = NULL;
    *pcVuDevs = 0;

    HDEVINFO hDevInfo =  SetupDiGetClassDevs(&GUID_CLASS_VPOXUSB,
    ...
    for (int i = 0; ; ++i)
    {
        ...
        if (!SetupDiEnumDeviceInterfaces(hDevInfo,
                            ...
                            &GUID_CLASS_VPOXUSB, /* IN LPGUID InterfaceClassGuid */
                            i,
                            &IfData))
        {
        ...
        PVPOXUSB_DEV pVuDev = (PVPOXUSB_DEV)RTMemAllocZ(sizeof (*pVuDev));
        if (!pVuDev)
        {
...
static int usbLibDevPopulate(PUSBDEVICE pDev, PUSB_NODE_CONNECTION_INFORMATION_EX pConInfo, ULONG iPort, LPCSTR lpszDrvKeyName, LPCSTR lpszHubName, PVPOXUSB_STRING_DR_ENTRY pDrList)
{
...
static int usbLibVUsbDevicePopulate(PVPOXUSB_DEV pVuDev, HDEVINFO InfoSet, PSP_DEVICE_INTERFACE_DATA InterfaceData)
{
...
/* Enumerate proxied USB devices (with VPoxUSB.sys loaded). */
static int usbLibEnumVUsbDevices(PVPOXUSB_DEV *ppVuDevs, uint32_t *pcVuDevs)
{
    ...
    *pcVuDevs = 0;
    ...
    /* Enumerate all present devices which support the GUID_CLASS_VPOXUSB interface. */
    InfoSet = SetupDiGetClassDevs(&GUID_CLASS_VPOXUSB, NULL, NULL,
                                  (DIGCF_PRESENT | DIGCF_DEVICEINTERFACE));
    if (InfoSet == INVALID_HANDLE_VALUE)
    {
        DWORD dwErr = GetLastError();
        LogRelFunc(("SetupDiGetClassDevs for GUID_CLASS_VPOXUSB failed (dwErr=%u)\n", dwErr));
        AssertFailed();
        return VERR_GENERAL_FAILURE;
    }
    ...
    while (SetupDiEnumDeviceInterfaces(InfoSet, NULL, &GUID_CLASS_VPOXUSB, DeviceIndex, &InterfaceData))
    {
        /* we've now got the IfData */
        PVPOXUSB_DEV pVuDev = (PVPOXUSB_DEV)RTMemAllocZ(sizeof (*pVuDev));
        if (!pVuDev)
...
static int usbLibDevPopulate(PUSBDEVICE pDev, PUSB_NODE_CONNECTION_INFORMATION_EX pConInfo, ULONG iPort, LPCSTR lpszLocation, LPCSTR lpszDrvKeyName, LPCSTR lpszHubName, PVPOXUSB_STRING_DR_ENTRY pDrList)
{
    pDev->bcdUSB = pConInfo->DeviceDescriptor.bcdUSB;
    pDev->bDeviceClass = pConInfo->DeviceDescriptor.bDeviceClass;
    pDev->bDeviceSubClass = pConInfo->DeviceDescriptor.bDeviceSubClass;
...
static int usbLibDevStrDriverKeyGet(HANDLE hHub, ULONG iPort, LPSTR* plpszName)
{
    ...
    *plpszName = NULL;
    ...
    {
#ifdef VPOX_WITH_ANNOYING_USB_ASSERTIONS
...
static int usbLibDevStrDrEntryGet(HANDLE hHub, LPCSTR lpcszHubName, ULONG iPort, ULONG iDr, USHORT idLang, PVPOXUSB_STRING_DR_ENTRY *ppList)
{
    ...
    PVPOXUSB_STRING_DR_ENTRY pEntry =
        (PVPOXUSB_STRING_DR_ENTRY)RTMemAllocZ(sizeof(VPOXUSB_STRING_DR_ENTRY) + pDr->bLength + 2);
    AssertPtr(pEntry);
    if (!pEntry)
        return VERR_NO_MEMORY;
...
static void usbLibDevStrDrEntryFree(PVPOXUSB_STRING_DR_ENTRY pDr)
{
    RTMemFree(pDr);
}

static void usbLibDevStrDrEntryFreeList(PVPOXUSB_STRING_DR_ENTRY pDr)
{
    while (pDr)
    {
        PVPOXUSB_STRING_DR_ENTRY pNext = pDr->pNext;
        usbLibDevStrDrEntryFree(pDr);
        pDr = pNext;
    }
}

static int usbLibDevStrDrEntryGetForLangs(HANDLE hHub, LPCSTR lpcszHubName, ULONG iPort, ULONG iDr, ULONG cIdLang, const USHORT *pIdLang, PVPOXUSB_STRING_DR_ENTRY *ppList)
{
...
static int usbLibDevStrDrEntryGetAll(HANDLE hHub, LPCSTR lpcszHubName, ULONG iPort, PUSB_DEVICE_DESCRIPTOR pDevDr, PUSB_CONFIGURATION_DESCRIPTOR pCfgDr, PVPOXUSB_STRING_DR_ENTRY *ppList)
{
...
static int usbLibDevGetHubPortDevices(HANDLE hHub, LPCSTR lpcszHubName, ULONG iPort, PUSBDEVICE *ppDevs, uint32_t *pcDevs)
{
    ...
    PVPOXUSB_STRING_DR_ENTRY pList = NULL;
 ...
 * least prior to capturing (once VPoxUSB.sys is installed, a DriverKey must by definition be
...
static int usbLibMonDevicesCmp(PUSBDEVICE pDev, PVPOXUSB_DEV pDevInfo)
{
    int iDiff;
...
static int usbLibMonDevicesUpdate(PVPOXUSBGLOBALSTATE pGlobal, PUSBDEVICE pDevs, PVPOXUSB_DEV pDevInfos)
{
    ...
    for (; pDevInfos; pDevInfos = pDevInfos->pNext)
    {
        for (pDevs = pDevsHead; pDevs; pDevs = pDevs->pNext)
        {
            ...
            HVPOXUSBDEVUSR hDevice = Dev.hDevice;
...
static int usbLibGetDevices(PVPOXUSBGLOBALSTATE pGlobal, PUSBDEVICE *ppDevs, uint32_t *pcDevs)
{
    *ppDevs = NULL;
    *pcDevs = 0;
...
static int usbLibStateWaitChange(PVPOXUSBGLOBALSTATE pGlobal, RTMSINTERVAL cMillies)
{
...
{
    return usbLibStateWaitChange(&g_VPoxUsbGlobal, msWaitTimeout);
}

static int usbLibInterruptWaitChange(PVPOXUSBGLOBALSTATE pGlobal)
{
...
{
    return usbLibInterruptWaitChange(&g_VPoxUsbGlobal);
}
...
{
    Assert(g_VPoxUsbGlobal.hMonitor != INVALID_HANDLE_VALUE);
    return usbLibGetDevices(&g_VPoxUsbGlobal, ppDevices, pcbNumDevices);
}
...
USBLIB_DECL(void *) USBLibAddFilter(PCUSBFILTER pFilter)
{
    ...
    if (g_VPoxUsbGlobal.hMonitor == INVALID_HANDLE_VALUE)
    {
#ifdef VPOX_WITH_ANNOYING_USB_ASSERTIONS
        AssertFailed();
#endif
    ...
    if (!DeviceIoControl(g_VPoxUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_ADD_FILTER,
...
USBLIB_DECL(void) USBLibRemoveFilter(void *pvId)
{
    ...
    uId = (uintptr_t)pvId;
    if (!DeviceIoControl(g_VPoxUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_REMOVE_FILTER, &uId, sizeof(uId),  NULL, 0,&cbReturned, NULL))
    {
        DWORD dwErr = GetLastError();
        AssertFailed();
...
USBLIB_DECL(int) USBLibRunFilters(void)
{
    ...
    if (!DeviceIoControl(g_VPoxUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_RUN_FILTERS,
                NULL, 0,
                NULL, 0,
                &cbReturned, NULL))
    {
...
static VOID CALLBACK usbLibTimerCallback(__in PVOID lpParameter, __in BOOLEAN TimerOrWaitFired)
{
    ...
    SetEvent(g_VPoxUsbGlobal.hNotifyEvent);
}
...
static void usbLibOnDeviceChange(void)
{
     ...
     * */
    if (g_VPoxUsbGlobal.hTimer)
    {
        if (!DeleteTimerQueueTimer(g_VPoxUsbGlobal.hTimerQueue, g_VPoxUsbGlobal.hTimer, NULL))
        {
            DWORD dwErr = GetLastError(); NOREF(dwErr);
    ...
    if (!CreateTimerQueueTimer(&g_VPoxUsbGlobal.hTimer, g_VPoxUsbGlobal.hTimerQueue,
                               usbLibTimerCallback,
                               NULL,
                               500, /* ms*/
                               0,
...
{
    static LPCSTR   s_szVPoxUsbWndClassName = "VPoxUsbLibClass";
    const HINSTANCE hInstance               = (HINSTANCE)GetModuleHandle(NULL);
    RT_NOREF1(lpParameter);

    Assert(g_VPoxUsbGlobal.hWnd == NULL);
    g_VPoxUsbGlobal.hWnd = NULL;
    ...
    wc.lpszClassName = s_szVPoxUsbWndClassName;
    ATOM atomWindowClass = RegisterClass(&wc);
    if (atomWindowClass != 0)
        g_VPoxUsbGlobal.hWnd = CreateWindowEx(WS_EX_TOOLWINDOW | WS_EX_TRANSPARENT | WS_EX_TOPMOST,
                                              s_szVPoxUsbWndClassName, s_szVPoxUsbWndClassName,
                                              WS_POPUPWINDOW,
    ...
    if (g_VPoxUsbGlobal.hWnd)
    {
        /* Make sure it's really hidden. */
        SetWindowPos(g_VPoxUsbGlobal.hWnd, HWND_TOPMOST, -200, -200, 0, 0,
    ...
    if (atomWindowClass != NULL)
        UnregisterClass(s_szVPoxUsbWndClassName, hInstance);
...
USBLIB_DECL(int) USBLibInit(void)
{
    ...
    RT_ZERO(g_VPoxUsbGlobal);
    g_VPoxUsbGlobal.hMonitor = INVALID_HANDLE_VALUE;

    /*
     * Create the notification and interrupt event before opening the device.
     */
    g_VPoxUsbGlobal.hNotifyEvent = CreateEvent(NULL,  /* LPSECURITY_ATTRIBUTES lpEventAttributes */
                                               FALSE, /* BOOL bManualReset */
                                               FALSE, /* set to false since it will be initially used for notification thread startup sync */
                                               NULL   /* LPCTSTR lpName */);
    if (g_VPoxUsbGlobal.hNotifyEvent)
    {
        g_VPoxUsbGlobal.hInterruptEvent = CreateEvent(NULL,  /* LPSECURITY_ATTRIBUTES lpEventAttributes */
                                                      FALSE, /* BOOL bManualReset */
                                                      FALSE, /* BOOL bInitialState */
                                                      NULL   /* LPCTSTR lpName */);
        if (g_VPoxUsbGlobal.hInterruptEvent)
        {
            /*
             * Open the USB monitor device, starting if needed.
             */
            g_VPoxUsbGlobal.hMonitor = CreateFile(USBMON_DEVICE_NAME,
            ...
            if (g_VPoxUsbGlobal.hMonitor == INVALID_HANDLE_VALUE)
            {
                HRESULT hr = VPoxDrvCfgSvcStart(USBMON_SERVICE_NAME_W);
                if (hr == S_OK)
                {
                    g_VPoxUsbGlobal.hMonitor = CreateFile(USBMON_DEVICE_NAME,
                                                          GENERIC_READ | GENERIC_WRITE,
            ...
            if (g_VPoxUsbGlobal.hMonitor != INVALID_HANDLE_VALUE)
            {
                ...
                USBSUP_VERSION  Version = {0};
                DWORD           cbReturned = 0;
                if (DeviceIoControl(g_VPoxUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_GET_VERSION,
                                    NULL, 0,
                                    ...
                                    &cbReturned, NULL))
                {
                        ...
                        )
                    {
                         ...
                         */
                        g_VPoxUsbGlobal.hTimerQueue = CreateTimerQueue();
                        if (g_VPoxUsbGlobal.hTimerQueue)
                        {
                            g_VPoxUsbGlobal.hThread = CreateThread(
                            ...
                            if (g_VPoxUsbGlobal.hThread)
                            {
                                DWORD dwResult = WaitForSingleObject(g_VPoxUsbGlobal.hNotifyEvent, INFINITE);
                                ...
                                dwResult = WaitForSingleObject(g_VPoxUsbGlobal.hThread, INFINITE);
                                Assert(dwResult == WAIT_OBJECT_0);
                                BOOL fRc = CloseHandle(g_VPoxUsbGlobal.hThread); NOREF(fRc);
                                DWORD dwErr = GetLastError(); NOREF(dwErr);
                                AssertMsg(fRc, ("CloseHandle for hThread failed (dwErr=%u)\n", dwErr));
                                g_VPoxUsbGlobal.hThread = INVALID_HANDLE_VALUE;
                            }
                            else
                            {
                                DWORD dwErr = GetLastError(); NOREF(dwErr);
                            ...
                            DeleteTimerQueueEx(g_VPoxUsbGlobal.hTimerQueue, INVALID_HANDLE_VALUE /* see term */);
                            g_VPoxUsbGlobal.hTimerQueue = NULL;
                        }
                        else
                        {
                            DWORD dwErr = GetLastError(); NOREF(dwErr);
                ...
                CloseHandle(g_VPoxUsbGlobal.hMonitor);
                g_VPoxUsbGlobal.hMonitor = INVALID_HANDLE_VALUE;
            }
            else
            {
            ...
            CloseHandle(g_VPoxUsbGlobal.hInterruptEvent);
            g_VPoxUsbGlobal.hInterruptEvent = NULL;
        }
        else
        {
        ...
        CloseHandle(g_VPoxUsbGlobal.hNotifyEvent);
        g_VPoxUsbGlobal.hNotifyEvent = NULL;
    }
    else
    {
...
USBLIB_DECL(int) USBLibTerm(void)
{
    if (g_VPoxUsbGlobal.hMonitor == INVALID_HANDLE_VALUE)
    {
        Assert(g_VPoxUsbGlobal.hInterruptEvent == NULL);
        Assert(g_VPoxUsbGlobal.hNotifyEvent == NULL);
        return VINF_SUCCESS;
    }

    BOOL fRc;
    fRc = PostMessage(g_VPoxUsbGlobal.hWnd, WM_CLOSE, 0, 0);
    AssertMsg(fRc, ("PostMessage for hWnd failed (dwErr=%u)\n", GetLastError()));

    if (g_VPoxUsbGlobal.hThread != NULL)
    {
        DWORD dwResult = WaitForSingleObject(g_VPoxUsbGlobal.hThread, INFINITE);
        Assert(dwResult == WAIT_OBJECT_0); NOREF(dwResult);
        fRc = CloseHandle(g_VPoxUsbGlobal.hThread);
        AssertMsg(fRc, ("CloseHandle for hThread failed (dwErr=%u)\n", GetLastError()));
    }

    if (g_VPoxUsbGlobal.hTimer)
    {
        fRc = DeleteTimerQueueTimer(g_VPoxUsbGlobal.hTimerQueue, g_VPoxUsbGlobal.hTimer,
