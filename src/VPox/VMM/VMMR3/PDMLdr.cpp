...
{
#if !defined(PDMLDR_FAKE_MODE) && defined(VPOX_WITH_RAW_MODE_KEEP)
    /*
 ...
 *                          search in the private directory (/usr/lib/virtualpox on Unix).
 *                          Ignored if VPOX_PATH_SHARED_LIBS is not defined.
 */
 ...
 * @todo    Remove the fShared hack again once we don't need to link against VPoxDD anymore!
 */
static char *pdmR3File(const char *pszFile, const char *pszDefaultExt, const char *pszSearchPath, bool fShared)
{
 ...
 *
 * @returns VPox status code. Failure will stop the enumeration.
 * @param   hLdrMod         The loader module handle.
