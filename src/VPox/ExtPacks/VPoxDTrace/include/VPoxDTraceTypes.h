...
/* $Id: VPoxDTraceTypes.h $ */
/** @file
 * VPoxDTraceTypes.h - Fake a bunch of Solaris types.
 *
 * Contributed by: bird
 */
...
#ifndef VPOX_INCLUDED_SRC_VPoxDTrace_include_VPoxDTraceTypes_h
#define VPOX_INCLUDED_SRC_VPoxDTrace_include_VPoxDTraceTypes_h
...
/** Mark a cast added when porting the code to VPox.
 * Avoids lots of \#ifdef VPOX otherwise needed to mark up the changes. */
#define VBDTCAST(a_Type)        (a_Type)
/** Mark a type change made when porting the code to VPox.
 * This is usually signed -> unsigned type changes that avoids a whole lot of
 * comparsion warnings. */
#define VBDTTYPE(a_VPox, a_Org) a_VPox
...
#define KERNELBASE                  VPoxDtGetKernelBase()
uintptr_t VPoxDtGetKernelBase(void);


typedef struct VPoxDtCred
{
    int32_t                 cr_refs;
    RTUID                   cr_uid;
    RTUID                   cr_ruid;
...
#define crhold                                      VPoxDtCredHold
#define crfree                                      VPoxDtCredFree
...
#define curthread               (VPoxDtGetCurrentThread())


typedef struct VPoxDtProcess    proc_t;
# if 0 /* not needed ? */
struct VPoxDtProcess    proc_t;
{
/*    uint32_t                p_flag; - don't bother with this */
    RTPROCESS               p_pid;
    struct dtrace_helpers  *p_dtrace_helpers;
}
proc_t *VPoxDtGetCurrentProc(void);
# define curproc                 (VPoxDtGetCurrentProc())
/*# define SNOCD                  RT_BIT(0) - don't bother with this */
# endif

typedef struct VPoxDtTaskQueue  taskq_t;

typedef struct VPoxDtMutex
{
    RTSEMMUTEX              hMtx;
    RTNATIVETHREAD volatile hOwner;
} kmutex_t;
#define mutex_enter             VPoxDtMutexEnter
#define mutex_exit              VPoxDtMutexExit
#define MUTEX_HELD(a_pMtx)      VPoxDtMutexIsOwner(a_pMtx)
#define MUTEX_NOT_HELD(a_pMtx)  (!VPoxDtMutexIsOwner(a_pMtx))
...
extern struct VPoxDtMutex       g_DummyMtx;


typedef struct VPoxDtCpuCore
{
    RTCPUID             cpu_id;
...
extern cpucore_t                g_aVPoxDtCpuCores[RTCPUSET_MAX_CPUS];
#define cpu_core                (g_aVPoxDtCpuCores)

struct VPoxDtCred *VPoxDtGetCurrentCreds(void);
#define CRED()                  VPoxDtGetCurrentCreds()

proc_t *VPoxDtThreadToProc(kthread_t *);


#define ASSERT(a_Expr)          Assert(a_Expr)
#define panic                   VPoxDtPanic
void VPoxDtPanic(const char *pszFormat, ...);
#define cmn_err                 VPoxDtCmnErr
void VPoxDtCmnErr(int iLevel, const char *pszFormat, ...);
#define CE_WARN                 10
#define CE_NOTE                 11
#define uprintf                 VPoxDtUPrintf
#define vuprintf                VPoxDtUPrintfV
...
#define kmem_alloc              VPoxDtKMemAlloc
#define kmem_zalloc             VPoxDtKMemAllocZ
#define kmem_free               VPoxDtKMemFree
...
typedef struct VPoxDtMemCache   kmem_cache_t;
#define kmem_cache_create       VPoxDtKMemCacheCreate
#define kmem_cache_destroy      VPoxDtKMemCacheDestroy
#define kmem_cache_alloc        VPoxDtKMemCacheAlloc
#define kmem_cache_free         VPoxDtKMemCacheFree
...
void  VPoxDtKMemCacheDestroy(struct VPoxDtMemCache *pCache);
void *VPoxDtKMemCacheAlloc(struct VPoxDtMemCache *pCache, uint32_t fFlags);
void  VPoxDtKMemCacheFree(struct VPoxDtMemCache *pCache, void *pvMem);


typedef struct VPoxDtVMem       vmem_t;
#define VM_SLEEP                RT_BIT(0)
#define VM_BESTFIT              RT_BIT(1)
#define VMC_IDENTIFIER          RT_BIT(16)
#define vmem_create             VPoxDtVMemCreate
#define vmem_destroy            VPoxDtVMemDestroy
#define vmem_alloc              VPoxDtVMemAlloc
#define vmem_free               VPoxDtVMemFree
...
void  VPoxDtVMemDestroy(struct VPoxDtVMem *pVMemArena);
void *VPoxDtVMemAlloc(struct VPoxDtVMem *pVMemArena, size_t cbMem, uint32_t fFlags);
void  VPoxDtVMemFree(struct VPoxDtVMem *pVMemArena, void *pvMem, size_t cbMem);

/*
 * Copy In/Out
 */
#define copyin                      VPoxDtCopyIn
#define copyout                     VPoxDtCopyOut
...
# if 0 /* not needed */
# define ddi_soft_state_init        VPoxDtDdiSoftStateInit
# define ddi_soft_state_fini        VPoxDtDdiSoftStateTerm
# define ddi_soft_state_zalloc      VPoxDtDdiSoftStateAllocZ
# define ddi_get_soft_state         VPoxDtDdiSoftStateGet
# define ddi_soft_state_free        VPoxDtDdiSoftStateFree
int   VPoxDtDdiSoftStateInit(void **ppvSoftStates, size_t cbSoftState, uint32_t cMaxItems);
int   VPoxDtDdiSoftStateTerm(void **ppvSoftStates);
int   VPoxDtDdiSoftStateAllocZ(void *pvSoftStates, RTDEV uMinor);
int   VPoxDtDdiSoftStateFree(void *pvSoftStates, RTDEV uMinor);
void *VPoxDtDdiSoftStateGet(void *pvSoftStates, RTDEV uMinor);

typedef enum { DDI_ATT_CMD_DUMMY }  ddi_attach_cmd_t;
typedef enum { DDI_DETACH, DDI_SUSPEND }  ddi_detach_cmd_t;
typedef struct VPoxDtDevInfo        dev_info_t;
# define ddi_driver_major           VPoxDtDdiDriverMajor
# define ddi_report_dev             VPoxDtDdiReportDev
major_t VPoxDtDdiDriverMajor(struct VPoxDtDevInfo *pDevInfo);
void    VPoxDtDdiReportDev(struct VPoxDtDevInfo *pDevInfo);
# endif
...
struct dtrace_state;
extern int dtrace_open(struct dtrace_state **ppState, struct VPoxDtCred *cred_p);
...
RT_C_DECLS_END
#endif /* !VPOX_INCLUDED_SRC_VPoxDTrace_include_VPoxDTraceTypes_h */
