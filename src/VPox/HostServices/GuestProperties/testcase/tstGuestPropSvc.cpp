 ...
 */
void initTable(VPOXHGCMSVCFNTABLE *pTable, VPOXHGCMSVCHELPERS *pHelpers)
{
    pTable->cbSize              = sizeof (VPOXHGCMSVCFNTABLE);
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testSetPropsHost(VPOXHGCMSVCFNTABLE *ptable)
{
    RTTestISub("SET_PROPS_HOST");
    RTTESTI_CHECK_RETV(RT_VALID_PTR(ptable->pfnHostCall));

    VPOXHGCMSVCPARM aParms[4];
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testEnumPropsHost(VPOXHGCMSVCFNTABLE *ptable)
{
 ...
 */
int doSetProperty(VPOXHGCMSVCFNTABLE *pTable, const char *pcszName,
                  const char *pcszValue, const char *pcszFlags, bool isHost,
                  bool useSetProp)
{
    VPOXHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testSetProp(VPOXHGCMSVCFNTABLE *pTable)
{
    ...
    s_aSetProperties[] =
    {
        ...
        { "/VirtualPox/GuestAdd/SharedFolders/MountDir", "test", "", false, true, false },
        { "/VirtualPox/GuestAdd/SomethingElse", "test", "", false, true, true },
        { "/VirtualPox/HostInfo/VRDP/Client/1/Name", "test", "", false, false, false },
        { "/VirtualPox/GuestAdd/SharedFolders/MountDir", "test", "", true, true, true },
        { "/VirtualPox/HostInfo/VRDP/Client/1/Name", "test", "TRANSRESET", true, true, true },
    };
 ...
 *                    than the guest one
 */
static int doDelProp(VPOXHGCMSVCFNTABLE *pTable, const char *pcszName, bool isHost)
{
    VPOXHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
    int command = GUEST_PROP_FN_DEL_PROP;
    if (isHost)
        command = GUEST_PROP_FN_HOST_DEL_PROP;
    VPOXHGCMSVCPARM aParms[1];
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testDelProp(VPOXHGCMSVCFNTABLE *pTable)
{
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testGetProp(VPOXHGCMSVCFNTABLE *pTable)
{
...
g_aGetNotifications[] =
{
    ...
    { "/VirtualPox/GuestAdd/SomethingElse\0test\0",
      sizeof("/VirtualPox/GuestAdd/SomethingElse\0test\0") },
    { "/VirtualPox/GuestAdd/SharedFolders/MountDir\0test\0RDONLYGUEST",
      sizeof("/VirtualPox/GuestAdd/SharedFolders/MountDir\0test\0RDONLYGUEST") },
    { "/VirtualPox/HostInfo/VRDP/Client/1/Name\0test\0TRANSIENT, RDONLYGUEST, TRANSRESET",
      sizeof("/VirtualPox/HostInfo/VRDP/Client/1/Name\0test\0TRANSIENT, RDONLYGUEST, TRANSRESET") },
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testGetNotification(VPOXHGCMSVCFNTABLE *pTable)
{
...
struct asyncNotification_
{
    ...
    /** Return value */
    VPOXHGCMCALLHANDLE_TYPEDEF callHandle;
} g_AsyncNotification;

/**
 * Set up the test for the asynchronous GET_NOTIFICATION function.
 */
static void setupAsyncNotification(VPOXHGCMSVCFNTABLE *pTable)
{
 ...
 */
static void testAsyncNotification(VPOXHGCMSVCFNTABLE *pTable)
{
    RT_NOREF1(pTable);
    uint64_t u64Timestamp;
    uint32_t u32Size;
...
{
    VPOXHGCMSVCFNTABLE  svcTable;
    VPOXHGCMSVCHELPERS  svcHelpers;
    initTable(&svcTable, &svcHelpers);

    /* The function is inside the service, not HGCM. */
    RTTESTI_CHECK_RC_OK_RETV(VPoxHGCMSvcLoad(&svcTable));
 ...
 */
static int doSetGlobalFlags(VPOXHGCMSVCFNTABLE *pTable, uint32_t fFlags)
{
    VPOXHGCMSVCPARM paParm;
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testSetPropROGuest(VPOXHGCMSVCFNTABLE *pTable)
{
    ...
    RTTESTI_CHECK_RC_OK_RETV(VPoxHGCMSvcLoad(pTable));
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testDelPropROGuest(VPOXHGCMSVCFNTABLE *pTable)
{
