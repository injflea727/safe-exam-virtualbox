...
/** @file
 * VirtualPox Main - Generic NetIf implementation.
 */
...
#define VPOXNETADPCTL_NAME "VPoxNetAdpCtl"
...
static int NetIfAdpCtl(const char * pcszIfName, const char *pszAddr, const char *pszOption, const char *pszMask)
{
    ...
    int rc = RTPathExecDir(szAdpCtl, sizeof(szAdpCtl) - sizeof("/" VPOXNETADPCTL_NAME));
    if (RT_FAILURE(rc))
    {
        LogRel(("NetIfAdpCtl: failed to get program path, rc=%Rrc.\n", rc));
        return rc;
    }
    strcat(szAdpCtl, "/" VPOXNETADPCTL_NAME);
    args[0] = szAdpCtl;
    if (!RTPathExists(szAdpCtl))
    {
        LogRel(("NetIfAdpCtl: path %s does not exist. Failed to run " VPOXNETADPCTL_NAME " helper.\n",
                szAdpCtl));
...
{
    char szAdpCtl[RTPATH_MAX];
    int rc = RTPathExecDir(szAdpCtl, sizeof(szAdpCtl) - sizeof("/" VPOXNETADPCTL_NAME " ") - strlen(pcszCmd));
    if (RT_FAILURE(rc))
    {
    ...
    if (fp)
    {
        ...
        {
            if (!strncmp(VPOXNETADPCTL_NAME ":", pszBuffer, sizeof(VPOXNETADPCTL_NAME)))
            {
        ...
        {
            LogRel(("NetIfAdpCtlOut: No output from " VPOXNETADPCTL_NAME));
            rc = VERR_INTERNAL_ERROR;
        }
        pclose(fp);
    }
...
int NetIfEnableStaticIpConfig(VirtualPox * /* vPox */, HostNetworkInterface * pIf, ULONG aOldIp, ULONG aNewIp, ULONG aMask)
{
...
int NetIfEnableStaticIpConfigV6(VirtualPox * /* vPox */, HostNetworkInterface * pIf, const Utf8Str &aOldIPV6Address,
...
int NetIfEnableDynamicIpConfig(VirtualPox * /* vPox */, HostNetworkInterface * /* pIf */)
{
                                        ...
                                        const char *pcszName)
{
    ...
    ComPtr<IHost> host;
    hrc = pVirtualPox->COMGETTER(Host)(host.asOutParam());
    if (SUCCEEDED(hrc))
    {
        hrc = progress->init(pVirtualPox, host,
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            char szAdpCtl[RTPATH_MAX];
            vrc = RTPathExecDir(szAdpCtl, sizeof(szAdpCtl) - sizeof("/" VPOXNETADPCTL_NAME " add"));
            if (RT_FAILURE(vrc))
            {
                progress->i_notifyComplete(E_FAIL,
                                           COM_IIDOF(IHostNetworkInterface),
                                           HostNetworkInterface::getStaticComponentName(),
                                           "Failed to get program path, vrc=%Rrc\n", vrc);
                return vrc;
            }
            strcat(szAdpCtl, "/" VPOXNETADPCTL_NAME " ");
            ...
            if (fp)
            {
                ...
                if (fgets(szBuf, sizeof(szBuf), fp))
                {
                    ...
                    if (!strncmp(VPOXNETADPCTL_NAME ":", szBuf, sizeof(VPOXNETADPCTL_NAME)))
                    {
                        progress->i_notifyComplete(E_FAIL,
                                                   COM_IIDOF(IHostNetworkInterface),
                                                   HostNetworkInterface::getStaticComponentName(),
                                                   "%s", szBuf);
                        pclose(fp);
                        return Global::vpoxStatusCodeFromCOM(E_FAIL);
                    }
                    ...
                    else
                    {
                        ...
                        if (RT_FAILURE(vrc))
                        {
                            progress->i_notifyComplete(E_FAIL,
                                                       COM_IIDOF(IHostNetworkInterface),
                                                       HostNetworkInterface::getStaticComponentName(),
                                                       "Failed to get config info for %s (as reported by '" VPOXNETADPCTL_NAME " add')\n", szBuf);
                        }
                        else
                        {
                            ...
                            iface->i_setVirtualPox(pVirtualPox);
                    ...
                    if ((vrc = pclose(fp)) != 0)
                    {
                        progress->i_notifyComplete(E_FAIL,
                                                   COM_IIDOF(IHostNetworkInterface),
                                                   HostNetworkInterface::getStaticComponentName(),
                                                   "Failed to execute '%s' - exit status: %d", szAdpCtl, vrc);
                ...
                else
                {
                    ...
                    progress->i_notifyComplete(E_FAIL,
                                               COM_IIDOF(IHostNetworkInterface),
                                               HostNetworkInterface::getStaticComponentName(),
                                               "Failed to execute '%s' (errno %d). Check permissions!",
                                               szAdpCtl, errno);
                    pclose(fp);
            ...
            else
            {
                ...
                progress->i_notifyComplete(E_FAIL,
                                           COM_IIDOF(IHostNetworkInterface),
                                           HostNetworkInterface::getStaticComponentName(),
                                           "Failed to execute '%s' (errno %d / %Rrc). Check permissions!",
                                           szAdpCtl, errno, vrc);
            }
            if (RT_SUCCESS(vrc))
    ...
    return RT_FAILURE(vrc) ? vrc : SUCCEEDED(hrc) ? VINF_SUCCESS : Global::vpoxStatusCodeFromCOM(hrc);
                                        ...
                                        IProgress **aProgress)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        hrc = progress->init(pVirtualPox, host, "Removing host network interface", FALSE /* aCancelable */);
        if (SUCCEEDED(hrc))
        {
            progress.queryInterfaceTo(aProgress);
            ...
            if (RT_FAILURE(vrc))
                progress->i_notifyComplete(E_FAIL,
                                           COM_IIDOF(IHostNetworkInterface),
                                           HostNetworkInterface::getStaticComponentName(),
                                           "Failed to execute '" VPOXNETADPCTL_NAME " %s remove' (%Rrc)",
                                           strIfName.c_str(), vrc);
            else
                progress->i_notifyComplete(S_OK);
        }
        else
            vrc = Global::vpoxStatusCodeFromCOM(hrc);
    }
    else
        vrc = Global::vpoxStatusCodeFromCOM(hrc);
    return vrc;
