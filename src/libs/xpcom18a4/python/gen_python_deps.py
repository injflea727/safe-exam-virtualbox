...
def print_vars(vers, known, sep, bitness_magic):
    print("VPOX_PYTHON%s_INC=%s%s" %(vers, known[0], sep))
    if bitness_magic > 0:
        if known[2]:
            print("VPOX_PYTHON%s_LIB=%s%s" %(vers, known[2], sep))
        if known[1]:
            print("VPOX_PYTHON%s_LIB_X86=%s%s" %(vers, known[1], sep))
    else:
        print("VPOX_PYTHON%s_LIB=%s%s" %(vers, known[1], sep))
...
def main(argv):
    ...
    if target == 'darwin':
        ## @todo Pick up the locations from VPOX_PATH_MACOSX_SDK_10_*.
