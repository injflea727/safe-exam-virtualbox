/* $Id: VPoxLA.cpp $ */
/** @file
 * VPoxLA - VPox Location Awareness notifications.
 */
...
static VPOXLACONTEXT g_Ctx = { 0 };

static const char *g_pszPropActiveClient = "/VirtualPox/HostInfo/VRDP/ActiveClient";

static const char *g_pszPropAttachTemplate = "/VirtualPox/HostInfo/VRDP/Client/%u/Attach";
...
static const char *g_pszPropInfoTemplates[] = {
                                                "/VirtualPox/HostInfo/VRDP/Client/%u/Name",
                                                "/VirtualPox/HostInfo/VRDP/Client/%u/IPAddr",
                                                "/VirtualPox/HostInfo/VRDP/Client/%u/Location",
                                                "/VirtualPox/HostInfo/VRDP/Client/%u/OtherInfo"
                                              };
...
static BOOL GetVolatileEnvironmentKey(PVPOXLACONTEXT pCtx, WCHAR *pwszRegKey, DWORD cbRegKey)
{
    BOOL fFound = FALSE;
...
static BOOL laGetUtcInfoClientName(PVPOXLACONTEXT pCtx, WCHAR *pwszClientName, DWORD cbClientName)
{
    LONG lErr;
...
static BOOL laSetClientName(PVPOXLACONTEXT pCtx, const WCHAR *pwszClientName)
{
    LONG lErr;
...
static void laUpdateClientName(PVPOXLACONTEXT pCtx)
{
...
static void laOnClientLocationInfo(PVPOXLACONTEXT pCtx, char *pszClientInfo[][2])
{
    /*
...
static void laDoAttach(PVPOXLACONTEXT pCtx)
{
...
static void laDoDetach(PVPOXLACONTEXT pCtx)
{
...
static int laGetActiveClient(PVPOXLACONTEXT pCtx, uint64_t *pu64Timestamp, uint32_t *pu32Value)
{
...
static int laUpdateCurrentState(PVPOXLACONTEXT pCtx, uint32_t u32ActiveClientId, uint64_t u64ActiveClientTS)
{
...
static int laWait(PVPOXLACONTEXT pCtx, uint64_t *pu64Timestamp, uint32_t u32Timeout)
{
...
static void laProcessClientInfo(PVPOXLACONTEXT pCtx)
{
...
static void laProcessAttach(PVPOXLACONTEXT pCtx)
{
...
static void laDoActions(PVPOXLACONTEXT pCtx)
{
    /*
     * Check if the attach was changed.
     *
...
DECLCALLBACK(int) VPoxLAInit(const PVPOXSERVICEENV pEnv, void **ppInstance)
{
    ...
    PVPOXLACONTEXT pCtx = &g_Ctx; /* Only one instance at the moment. */
    AssertPtr(pCtx);

    pCtx->pEnv = pEnv;

    DWORD dwValue = 0;
    if (   laGetRegistryDWORD(L"SOFTWARE\\Oracle\\VirtualPox Guest Additions", L"VPoxTrayLog", &dwValue)
        && (dwValue & 0x10) != 0)
    {
    ...
    dwValue = 0x02;
    if (   laGetRegistryDWORD(L"SOFTWARE\\Oracle\\VirtualPox Guest Additions", L"VPoxTrayLA", &dwValue)
        && (dwValue & 0x02) == 0)
    {
...
DECLCALLBACK(void) VPoxLADestroy(void *pInstance)
{
    AssertPtrReturnVoid(pInstance);

    LogFlowFunc(("Destroying pInstance=%p\n", pInstance));

    PVPOXLACONTEXT pCtx = (PVPOXLACONTEXT)pInstance;
    AssertPtr(pCtx);
 ...
 */
DECLCALLBACK(int) VPoxLAWorker(void *pInstance, bool volatile *pfShutdown)
{
    AssertPtr(pInstance);
    ...
    /*
     * On name change event (/VirtualPox/HostInfo/VRDP/Client/%ID%/Name)
     *   Store the name in the registry (HKCU\Volatile Environment\UTCINFO_CLIENTNAME).
     * On a client attach event (/VirtualPox/HostInfo/VRDP/Client/%ID%/Attach -> 1):
     *   Execute ReconnectActions
     * On a client detach event (/VirtualPox/HostInfo/VRDP/Client/%ID%/Attach -> 0):
     *   Execute DisconnectActions
     *
     * The active connected client id is /VirtualPox/HostInfo/VRDP/ActiveClientClient.
     */
