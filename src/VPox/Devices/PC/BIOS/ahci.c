...
#define VPOX_AHCI_NO_DEVICE 0xffff
...
#define VPOXAHCI_PORT_WRITE_REG(iobase, port, reg, val)     \
    AHCI_WRITE_REG((iobase), AHCI_PORT_REG((port), (reg)), val)

/** Reads from the given port register. */
#define VPOXAHCI_PORT_READ_REG(iobase, port, reg, val)      \
...
static void ahci_port_cmd_sync(ahci_t __far *ahci, uint8_t val)
{
    ...
    if (port != 0xff)
    {
        ...
        /* Queue command. */
        VPOXAHCI_PORT_WRITE_REG(io_base, port, AHCI_REG_PORT_CI, 0x1);
...
static void ahci_port_deinit_current(ahci_t __far *ahci)
{
    ...
    if (port != 0xff)
    {
        ...
        VPOXAHCI_PORT_WRITE_REG(io_base, port, AHCI_REG_PORT_FB, 0);
        VPOXAHCI_PORT_WRITE_REG(io_base, port, AHCI_REG_PORT_FBU, 0);

        VPOXAHCI_PORT_WRITE_REG(io_base, port, AHCI_REG_PORT_CLB, 0);
        VPOXAHCI_PORT_WRITE_REG(io_base, port, AHCI_REG_PORT_CLBU, 0);

        /* Disable all interrupts. */
        VPOXAHCI_PORT_WRITE_REG(io_base, port, AHCI_REG_PORT_IE, 0);
...
static void ahci_port_init(ahci_t __far *ahci, uint8_t u8Port)
{
    ...
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_FB, ahci_addr_to_phys(&ahci->abFisRecv));
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_FBU, 0);

    DBG_AHCI("AHCI: CMD list area %lx\n", ahci_addr_to_phys(&ahci->aCmdHdr));
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_CLB, ahci_addr_to_phys(&ahci->aCmdHdr));
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_CLBU, 0);

    /* Disable all interrupts. */
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_IE, 0);
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_IS, 0xffffffff);
    /* Clear all errors. */
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_SERR, 0xffffffff);
...
int ahci_read_sectors(bio_dsk_t __far *bios_dsk)
{
    ...
    device_id = VPOX_GET_AHCI_DEVICE(bios_dsk->drqp.dev_id);
                         ...
                         uint16_t skip_b, uint32_t length, uint8_t inout, char __far *buffer)
{
    ...
    device_id = VPOX_GET_AHCI_DEVICE(device_id);
...
void ahci_port_detect_device(ahci_t __far *ahci, uint8_t u8Port)
{
    ...
    /* Reset connection. */
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_SCTL, 0x01);
    /*
     ...
     */
    VPOXAHCI_PORT_WRITE_REG(ahci->iobase, u8Port, AHCI_REG_PORT_SCTL, 0);
    ...
    while( *ticks < end_tick )
    {
        ...
        VPOXAHCI_PORT_READ_REG(ahci->iobase, u8Port, AHCI_REG_PORT_SSTS, val);
    ...
    if (ahci_ctrl_extract_bits(val, 0xfL, 0) == 0x3)
    {
        ...
        if (devcount_ahci < BX_MAX_AHCI_DEVICES)
        {
            ...
            VPOXAHCI_PORT_READ_REG(ahci->iobase, u8Port, AHCI_REG_PORT_SIG, val);
            if (val == 0x101)
            {
                uint64_t    sectors;
...
void BIOSCALL ahci_init(void)
{
    ...
    if (busdevfn != VPOX_AHCI_NO_DEVICE)
    {
