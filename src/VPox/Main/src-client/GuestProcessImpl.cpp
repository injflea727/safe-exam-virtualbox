...
/** @file
 * VirtualPox Main - Guest process handling.
 */
...
public:
    ...
    STDMETHOD(HandleEvent)(VPoxEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case VPoxEventType_OnGuestProcessStateChanged:
            case VPoxEventType_OnGuestProcessInputNotify:
            case VPoxEventType_OnGuestProcessOutput:
            {
...
VPOX_LISTENER_DECLARE(GuestProcessListenerImpl)
    ...
    if (RT_SUCCESS(vrc))
    {
        try
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                eventTypes.push_back(VPoxEventType_OnGuestProcessStateChanged);
                eventTypes.push_back(VPoxEventType_OnGuestProcessInputNotify);
                eventTypes.push_back(VPoxEventType_OnGuestProcessOutput);
...
HRESULT GuestProcess::getEnvironment(std::vector<com::Utf8Str> &aEnvironment)
{
    ...
    if (mData.mpSessionBaseEnv)
    {
        ...
        hrc = setError(VPOX_E_NOT_SUPPORTED, tr("The base environment feature is not supported by installed Guest Additions"));
    LogFlowThisFuncLeave();
    return hrc;
#endif
}
...
int GuestProcess::i_callbackDispatcher(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb)
{
...
int GuestProcess::i_onGuestDisconnected(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
...
int GuestProcess::i_onProcessInputStatus(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestProcess::i_onProcessNotifyIO(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestProcess::i_onProcessStatusChange(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestProcess::i_onProcessOutput(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    RT_NOREF(pCbCtx);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestProcess::i_setProcessStatus(ProcessStatus_T procStatus, int procRc)
{
    ...
    if (mData.mStatus != procStatus) /* Was there a process status change? */
    {
        ...
        {
            hr = errorInfo->initEx(VPOX_E_IPRT_ERROR, mData.mLastError,
...
int GuestProcess::i_startProcessInner(uint32_t cMsTimeout, AutoWriteLock &rLock, GuestWaitEvent *pEvent, int *prcGuest)
{
    ...
    if (cArgs)
    {
        ...
        if (!(fGuestControlFeatures0 & VPOX_GUESTCTRL_GF_0_PROCESS_ARGV0))
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (uProtocol < 2)
        {
            /* In protocol v1 (VPox < 4.3) the credentials were part of the execution
...
int GuestProcess::i_terminateProcess(uint32_t uTimeoutMS, int *prcGuest)
{
    ...
    {
        AssertPtr(mSession);
        /* Note: VPox < 4.3 (aka protocol version 1) does not
                                       ...
                                       ProcessInputStatus_T *pInputStatus, uint32_t *pcbProcessed)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        if (evtType == VPoxEventType_OnGuestProcessInputNotify)
        {
                                  ...
                                  void *pvData, size_t cbData, uint32_t *pcbRead)
{
    ...
    do
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            if (evtType == VPoxEventType_OnGuestProcessOutput)
            {
                                        ...
                                        ProcessStatus_T *pProcessStatus, int *prcGuest)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        Assert(evtType == VPoxEventType_OnGuestProcessStateChanged);
...
HRESULT GuestProcess::read(ULONG aHandle, ULONG aToRead, ULONG aTimeoutMS, std::vector<BYTE> &aData)
{
    ...
    else
    {
        ...
        switch (vrc)
        {
            ...
            default:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Reading from guest process \"%s\" (PID %RU32) failed: %Rrc"),
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
           case VERR_GSTCTL_GUEST_ERROR:
            ...
            default:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Terminating guest process \"%s\" (PID %RU32) failed: %Rrc"),
                                  mData.mProcess.mExecutable.c_str(), mData.mPID, vrc);
                break;
        }
    }
...
HRESULT GuestProcess::waitFor(ULONG aWaitFor, ULONG aTimeoutMS, ProcessWaitResult_T *aReason)
{
    ...
    else
    {
        switch (vrc)
        {
            ...
            default:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Waiting for guest process \"%s\" (PID %RU32) failed: %Rrc"),
                                  mData.mProcess.mExecutable.c_str(), mData.mPID, vrc);
                break;
        }
    }
                            ...
                            ULONG aTimeoutMS, ULONG *aWritten)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Writing to guest process \"%s\" (PID %RU32) failed: %Rrc"),
                                  mData.mProcess.mExecutable.c_str(), mData.mPID, vrc);
                break;
        }
    }
 ...
 * objects. Those objects are issued on the guest side as part of VPoxService's toolbox tools (think of a BusyBox-like approach)
...
int GuestProcessTool::exitCodeToRc(const char *pszTool, int32_t iExitCode)
{
    ...
    if (!RTStrICmp(pszTool, VPOXSERVICE_TOOL_CAT))
    {
        switch (iExitCode)
        {
            case VPOXSERVICETOOLBOX_CAT_EXITCODE_ACCESS_DENIED:     return VERR_ACCESS_DENIED;
            case VPOXSERVICETOOLBOX_CAT_EXITCODE_FILE_NOT_FOUND:    return VERR_FILE_NOT_FOUND;
            case VPOXSERVICETOOLBOX_CAT_EXITCODE_PATH_NOT_FOUND:    return VERR_PATH_NOT_FOUND;
            case VPOXSERVICETOOLBOX_CAT_EXITCODE_SHARING_VIOLATION: return VERR_SHARING_VIOLATION;
            case VPOXSERVICETOOLBOX_CAT_EXITCODE_IS_A_DIRECTORY:    return VERR_IS_A_DIRECTORY;
            default:                                                break;
        }
    }
    else if (!RTStrICmp(pszTool, VPOXSERVICE_TOOL_LS))
    {
        switch (iExitCode)
        {
    ...
    else if (!RTStrICmp(pszTool, VPOXSERVICE_TOOL_STAT))
    {
        switch (iExitCode)
        {
            case VPOXSERVICETOOLBOX_STAT_EXITCODE_ACCESS_DENIED:      return VERR_ACCESS_DENIED;
            case VPOXSERVICETOOLBOX_STAT_EXITCODE_FILE_NOT_FOUND:     return VERR_FILE_NOT_FOUND;
            case VPOXSERVICETOOLBOX_STAT_EXITCODE_PATH_NOT_FOUND:     return VERR_PATH_NOT_FOUND;
            case VPOXSERVICETOOLBOX_STAT_EXITCODE_NET_PATH_NOT_FOUND: return VERR_NET_PATH_NOT_FOUND;
            default:                                                  break;
        }
    }
    else if (!RTStrICmp(pszTool, VPOXSERVICE_TOOL_MKDIR))
    {
        switch (iExitCode)
        {
    ...
    else if (!RTStrICmp(pszTool, VPOXSERVICE_TOOL_MKTEMP))
    {
        switch (iExitCode)
        {
            case RTEXITCODE_FAILURE: return VERR_CANT_CREATE;
    ...
    else if (!RTStrICmp(pszTool, VPOXSERVICE_TOOL_RM))
    {
        switch (iExitCode)
        {
