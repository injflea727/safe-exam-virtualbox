...
static void shader_chache_put(PVPOXEXT_HASHCACHE pCache, IWineD3DBaseShaderImpl *pShader);
#endif
...
#define VPOX_SHADER_FROM_CACHE_ENTRY(_pEntry) RT_FROM_MEMBER(RT_FROM_MEMBER((_pEntry), IWineD3DBaseShaderClass, CacheEntry), IWineD3DBaseShaderImpl, baseShader)

static DECLCALLBACK(void) shader_cache_cleanup_entry(void *pvKey, struct VPOXEXT_HASHCACHE_ENTRY *pEntry)
{
    IWineD3DBaseShaderImpl *pShader = VPOX_SHADER_FROM_CACHE_ENTRY(pEntry);
...
static void shader_chache_init(PVPOXEXT_HASHCACHE pCache)
{
    VPoxExtCacheInit(pCache, 1024, shader_cache_hash, shader_cache_equal, shader_cache_cleanup_entry);
}

static void shader_chache_term(PVPOXEXT_HASHCACHE pCache)
{
    VPoxExtCacheTerm(pCache);
}
...
static void shader_chache_put(PVPOXEXT_HASHCACHE pCache, IWineD3DBaseShaderImpl *pShader)
{
    ...
    VPoxExtCachePut(pCache, pShader, &pShader->baseShader.CacheEntry);
}

static IWineD3DBaseShaderImpl* shader_chache_get(PVPOXEXT_HASHCACHE pCache, IWineD3DBaseShaderImpl *pShader)
{
    PVPOXEXT_HASHCACHE_ENTRY pEntry = VPoxExtCacheGet(pCache, pShader);
    IWineD3DBaseShaderImpl *pCachedShader;

    if (!pEntry)
        return pShader;

    pCachedShader = VPOX_SHADER_FROM_CACHE_ENTRY(pEntry);
