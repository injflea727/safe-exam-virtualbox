...
#define VPOX_ICH9PCI_SAVED_STATE_VERSION                VPOX_ICH9PCI_SAVED_STATE_VERSION_4KB_CFG_SPACE
/** 4KB config space */
#define VPOX_ICH9PCI_SAVED_STATE_VERSION_4KB_CFG_SPACE  4
/** Adds I/O region types and sizes for dealing changes in resource regions. */
#define VPOX_ICH9PCI_SAVED_STATE_VERSION_REGION_SIZES   3
/** This appears to be the first state we need to care about. */
#define VPOX_ICH9PCI_SAVED_STATE_VERSION_MSI            2
/** This is apparently not supported or has a grossly incomplete state, juding
 * from hints in the code. */
#define VPOX_ICH9PCI_SAVED_STATE_VERSION_NOMSI          1
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC)
 ...
 */
static VPOXSTRICTRC ich9pciConfigWrite(PPDMDEVINS pDevIns, PDEVPCIROOT pPciRoot, PciAddress const *pPciAddr,
                                       uint32_t u32Value, int cb, int rcReschedule)
{
    ...
    Log2Func(("%02x:%02x.%u reg %x(%u) %x %Rrc\n", pPciAddr->iBus, pPciAddr->iDeviceFunc >> 3, pPciAddr->iDeviceFunc & 0x7,
              pPciAddr->iRegister, cb, u32Value, VPOXSTRICTRC_VAL(rcStrict)));
    return rcStrict;
}
 ...
 */
static VPOXSTRICTRC ich9pciConfigRead(PDEVPCIROOT pPciRoot, PciAddress* pPciAddr, int cb, uint32_t *pu32Value, int rcReschedule)
{
    VPOXSTRICTRC rcStrict = VINF_SUCCESS;
#ifdef IN_RING3
    NOREF(rcReschedule);
    ...
    Log3Func(("%02x:%02x.%d reg %x(%d) gave %x %Rrc\n", pPciAddr->iBus, pPciAddr->iDeviceFunc >> 3, pPciAddr->iDeviceFunc & 0x7,
              pPciAddr->iRegister, cb, *pu32Value, VPOXSTRICTRC_VAL(rcStrict) ));
    return rcStrict;
}
...
ich9pciIOPortDataRead(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t *pu32, unsigned cb)
{
    ...
    if (!(offPort % cb))
    {
        ...
        *pu32 = 0xffffffff;
        ...
        LogFlowFunc(("offPort=%u cb=%#x (config=%#10x) -> %#x (%Rrc)\n", offPort, cb, *pu32, pThis->uConfigReg, VPOXSTRICTRC_VAL(rcStrict)));
        return rcStrict;
    }
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) ich9pciMcfgMMIOWrite(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void const *pv, unsigned cb)
{
    ...
    VPOXSTRICTRC rcStrict = ich9pciConfigWrite(pDevIns, pPciRoot, &aDest, u32, cb, VINF_IOM_R3_MMIO_WRITE);
    PCI_UNLOCK(pDevIns);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) ich9pciMcfgMMIORead(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void *pv, unsigned cb)
{
    PDEVPCIROOT pPciRoot = PDMINS_2_DATA(pDevIns, PDEVPCIROOT);
    ...
    VPOXSTRICTRC rcStrict = ich9pciConfigRead(pPciRoot, &aDest, cb, &u32Value, VINF_IOM_R3_MMIO_READ);
    PCI_UNLOCK(pDevIns);
...
DECLINLINE(PPDMPCIDEV) ich9pciFindBridge(PDEVPCIBUS pBus, uint8_t uBus)
{
    ...
    for (uint32_t iBridge = 0; iBridge < pBus->cBridges; iBridge++)
    {
        ...
        /* safe, only needs to go to the config space array */
        uint32_t uSecondary   = PDMPciDevGetByte(pBridge, VPOX_PCI_SECONDARY_BUS);
        /* safe, only needs to go to the config space array */
        uint32_t uSubordinate = PDMPciDevGetByte(pBridge, VPOX_PCI_SUBORDINATE_BUS);
...
uint32_t devpciR3GetCfg(PPDMPCIDEV pPciDev, int32_t iRegister, int cb)
{
    ...
    VPOXSTRICTRC rcStrict = VINF_PDM_PCI_DO_DEFAULT;
    if (pPciDev->Int.s.pfnConfigRead)
    ...
    AssertRCSuccess(VPOXSTRICTRC_VAL(rcStrict));
    return u32Value;
}

DECLINLINE(uint32_t) devpciGetRegionReg(int iRegion)
{
    return iRegion == VPOX_PCI_ROM_SLOT
         ?  VPOX_PCI_ROM_ADDRESS : (VPOX_PCI_BASE_ADDRESS_0 + iRegion * 4);
}
...
void devpciR3SetCfg(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev, int32_t iRegister, uint32_t u32Value, int cb)
{
    ...
    if (   rcStrict == VINF_PDM_PCI_DO_DEFAULT
        || (   rcStrict == VINF_SUCCESS
            && (   iRegister == VPOX_PCI_SECONDARY_BUS
                || iRegister == VPOX_PCI_SUBORDINATE_BUS)))
        rcStrict = devpciR3CommonConfigWriteWorker(pDevIns, PDMINS_2_DATA_CC(pDevIns, PDEVPCIBUSCC),
...
{
    for (uint8_t uPciDevNo = pBus->iDevSearch >> VPOX_PCI_DEVFN_DEV_SHIFT; uPciDevNo < VPOX_PCI_MAX_DEVICES; uPciDevNo++)
        if (   !pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, 0)]
            && !pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, 1)]
            && !pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, 2)]
            && !pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, 3)]
            && !pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, 4)]
            && !pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, 5)]
            && !pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, 6)]
            && !pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, 7)])
            return uPciDevNo;
    return UINT8_MAX;
}
                                             ...
                                             uint8_t uPciDevNo, uint8_t uPciFunNo, const char *pszName)
{
    ...
    AssertReturn(uPciDevNo < VPOX_PCI_MAX_DEVICES   || uPciDevNo == PDMPCIDEVREG_DEV_NO_FIRST_UNUSED, VERR_INVALID_PARAMETER);
    AssertReturn(uPciFunNo < VPOX_PCI_MAX_FUNCTIONS || uPciFunNo == PDMPCIDEVREG_FUN_NO_FIRST_UNUSED, VERR_INVALID_PARAMETER);
    ...
    {
        AssertLogRelMsgReturn(uPciDevNo < VPOX_PCI_MAX_DEVICES && uPciFunNo < VPOX_PCI_MAX_FUNCTIONS,
                              ("PDMPCIDEVREG_F_NOT_MANDATORY_NO not implemented for #Dev=%#x / #Fun=%#x\n", uPciDevNo, uPciFunNo),
                              VERR_NOT_IMPLEMENTED);
        if (pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, uPciFunNo)])
        {
    ...
    if (uPciDevNo == PDMPCIDEVREG_DEV_NO_FIRST_UNUSED)
    {
        ...
        AssertLogRelMsgReturn(uPciDevNo < VPOX_PCI_MAX_DEVICES, ("Couldn't find a free spot!\n"), VERR_PDM_TOO_PCI_MANY_DEVICES);
    ...
    else
    {
        ...
        if (uPciFunNo != PDMPCIDEVREG_FUN_NO_FIRST_UNUSED)
        {
            ...
            pClash = pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, uPciFunNo)];
            if (!pClash)
            { /* likely */ }
        ...
        else
        {
            ...
            for (uPciFunNo = 0, pClash = NULL; uPciFunNo < VPOX_PCI_MAX_FUNCTIONS; uPciFunNo++)
            {
                pClash = pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, uPciFunNo)];
                if (!pClash)
                    break;
        ...
        if (pClash)
        {
            ...
            for (uint8_t uMoveFun = 0; uMoveFun < VPOX_PCI_MAX_FUNCTIONS; uMoveFun++)
            {
                PPDMPCIDEV pMovePciDev = pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, uMoveFun)];
            ...
            AssertLogRelMsgReturn(uMoveToDevNo < VPOX_PCI_MAX_DEVICES,
            ...
            for (uint8_t uMoveFun = 0; uMoveFun < VPOX_PCI_MAX_FUNCTIONS; uMoveFun++)
            {
                ...
                if (pMovePciDev)
                {
                    ...
                    pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, uMoveFun)] = NULL;
                    pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uMoveToDevNo, uMoveFun)] = pMovePciDev;
                    pMovePciDev->uDevFn = VPOX_PCI_DEVFN_MAKE(uMoveToDevNo, uMoveFun);
                }
            }
        }
    }
     ...
     */
    Assert(!pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, uPciFunNo)]);
    pBus->apDevices[VPOX_PCI_DEVFN_MAKE(uPciDevNo, uPciFunNo)] = pPciDev;

    pPciDev->uDevFn                 = VPOX_PCI_DEVFN_MAKE(uPciDevNo, uPciFunNo);
                                                 ...
                                                 uint64_t hHandle, PFNPCIIOREGIONMAP pfnMapUnmap)
{
    ...
    AssertMsgReturn((unsigned)iRegion < VPOX_PCI_NUM_REGIONS,
                    ("Invalid iRegion=%d VPOX_PCI_NUM_REGIONS=%d\n", iRegion, VPOX_PCI_NUM_REGIONS),
                    VERR_INVALID_PARAMETER);
    ...
    {
        /* VPOX_PCI_BASE_ADDRESS_5 and VPOX_PCI_ROM_ADDRESS are excluded. */
        AssertMsgReturn(iRegion < VPOX_PCI_NUM_REGIONS - 2,
...
static int ich9pciR3CommonSaveExec(PDEVPCIBUS pBus, PSSMHANDLE pSSM)
{
    ...
    for (uint32_t uDevFn = 0; uDevFn < RT_ELEMENTS(pBus->apDevices); uDevFn++)
    {
        ...
        if (pDev)
        {
            ...
            SSMR3PutU32(pSSM, sizeof(pDev->abConfig));
            SSMR3PutMem(pSSM, pDev->abConfig, sizeof(pDev->abConfig));

            /* Device flags */
            SSMR3PutU32(pSSM, pDev->Int.s.fFlags);

            /* IRQ pin state */
            SSMR3PutS32(pSSM, pDev->Int.s.uIrqPinState);

            /* MSI info */
            SSMR3PutU8(pSSM, pDev->Int.s.u8MsiCapOffset);
            SSMR3PutU8(pSSM, pDev->Int.s.u8MsiCapSize);

            /* MSI-X info */
            SSMR3PutU8(pSSM, pDev->Int.s.u8MsixCapOffset);
            SSMR3PutU8(pSSM, pDev->Int.s.u8MsixCapSize);
            ...
            for (uint32_t iRegion = 0; iRegion < VPOX_PCI_NUM_REGIONS; iRegion++)
            {
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) ich9pcibridgeConfigWrite(PPDMDEVINSR3 pDevIns, uint8_t iBus, uint8_t iDevice,
                                                           uint32_t u32Address, unsigned cb, uint32_t u32Value)
{
    ...
    /* safe, only needs to go to the config space array */
    if (iBus != PDMPciDevGetByte(pDevIns->apPciDevs[0], VPOX_PCI_SECONDARY_BUS))
    {
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) ich9pcibridgeConfigRead(PPDMDEVINSR3 pDevIns, uint8_t iBus, uint8_t iDevice,
...
static int ich9pciR3CommonLoadExec(PPDMDEVINS pDevIns, PDEVPCIBUS pBus, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (   uVersion < VPOX_ICH9PCI_SAVED_STATE_VERSION_MSI
        || uVersion > VPOX_ICH9PCI_SAVED_STATE_VERSION)
    ...
    for (uint32_t uDevFn = 0; uDevFn < RT_ELEMENTS(pBus->apDevices); uDevFn++)
    {
        ...
        if (pDev)
        {
            ...
            devpciR3SetCfg(pDevIns, pDev, VPOX_PCI_COMMAND, 0 /*u32Value*/, 2 /*cb*/);
            /* safe, only needs to go to the config space array */
    ...
    for (uint32_t uDevFn = 0;; uDevFn++)
    {
        ...
        if (uVersion >= VPOX_ICH9PCI_SAVED_STATE_VERSION_4KB_CFG_SPACE)
        {
            rc = SSMR3GetU32(pSSM, &cbConfig);
        ...
        SSMR3GetMem(pSSM, u.DevTmp.abConfig, cbConfig);

        SSMR3GetU32(pSSM, &u.DevTmp.Int.s.fFlags);
        SSMR3GetS32(pSSM, &u.DevTmp.Int.s.uIrqPinState);
        SSMR3GetU8(pSSM, &u.DevTmp.Int.s.u8MsiCapOffset);
        SSMR3GetU8(pSSM, &u.DevTmp.Int.s.u8MsiCapSize);
        SSMR3GetU8(pSSM, &u.DevTmp.Int.s.u8MsixCapOffset);
        rc = SSMR3GetU8(pSSM, &u.DevTmp.Int.s.u8MsixCapSize);
        AssertRCReturn(rc, rc);
        ...
        if (uVersion >= VPOX_ICH9PCI_SAVED_STATE_VERSION_REGION_SIZES)
        {
            for (uint32_t iRegion = 0; iRegion < VPOX_PCI_NUM_REGIONS; iRegion++)
            {
        ...
        rc = devpciR3CommonRestoreRegions(pSSM, pDev, u.DevTmp.Int.s.aIORegions,
                                          uVersion >= VPOX_ICH9PCI_SAVED_STATE_VERSION_REGION_SIZES);
        if (RT_FAILURE(rc))
            break;
        Assert(!pciDevIsPassthrough(pDev));
...
static DECLCALLBACK(int) ich9pciR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (uVersion <= VPOX_ICH9PCI_SAVED_STATE_VERSION_NOMSI)
        return VERR_SSM_UNSUPPORTED_DATA_UNIT_VERSION;
    if (uVersion > VPOX_ICH9PCI_SAVED_STATE_VERSION)
        return VERR_SSM_UNSUPPORTED_DATA_UNIT_VERSION;
...
static void ich9pciBiosInitBridge(PPDMDEVINS pDevIns, PDEVPCIROOT pPciRoot, PDEVPCIBUS pBus)
{
    ...
    devpciR3SetByte(pDevIns, pBridge, VPOX_PCI_IO_BASE, (pPciRoot->uPciBiosIo >> 8) & 0xf0);
    ...
    devpciR3SetWord(pDevIns, pBridge, VPOX_PCI_MEMORY_BASE, (pPciRoot->uPciBiosMmio >> 16) & UINT32_C(0xffff0));
    ...
    if (u32IoAddressBase != pPciRoot->uPciBiosIo)
    {
        ...
        pPciRoot->uPciBiosIo = RT_ALIGN_32(pPciRoot->uPciBiosIo, _4K);
        devpciR3SetByte(pDevIns, pBridge, VPOX_PCI_IO_LIMIT, ((pPciRoot->uPciBiosIo - 1) >> 8) & 0xf0);
    }
    else
    {
        devpciR3SetByte(pDevIns, pBridge, VPOX_PCI_IO_BASE, 0xf0);
        devpciR3SetByte(pDevIns, pBridge, VPOX_PCI_IO_LIMIT, 0x00);
    }
    ...
    {
        pPciRoot->uPciBiosMmio = RT_ALIGN_32(pPciRoot->uPciBiosMmio, _1M);
        devpciR3SetWord(pDevIns, pBridge, VPOX_PCI_MEMORY_LIMIT, ((pPciRoot->uPciBiosMmio - 1) >> 16) & UINT32_C(0xfff0));
    }
    else
    {
        devpciR3SetWord(pDevIns, pBridge, VPOX_PCI_MEMORY_BASE, 0xfff0);
        devpciR3SetWord(pDevIns, pBridge, VPOX_PCI_MEMORY_LIMIT, 0x0000);
    }
     ...
     */
    devpciR3SetWord(pDevIns, pBridge, VPOX_PCI_PREF_MEMORY_BASE, 0xfff0);
    devpciR3SetWord(pDevIns, pBridge, VPOX_PCI_PREF_MEMORY_LIMIT, 0x0000);
    devpciR3SetDWord(pDevIns, pBridge, VPOX_PCI_PREF_BASE_UPPER32, 0x00000000);
    devpciR3SetDWord(pDevIns, pBridge, VPOX_PCI_PREF_LIMIT_UPPER32, 0x00000000);
}

static int ich9pciBiosInitDeviceGetRegions(PPDMPCIDEV pPciDev)
{
    uint8_t uHeaderType = devpciR3GetByte(pPciDev, VPOX_PCI_HEADER_TYPE) & 0x7f;
    if (uHeaderType == 0x00)
        /* Ignore ROM region here, which is included in VPOX_PCI_NUM_REGIONS. */
        return VPOX_PCI_NUM_REGIONS - 1;
...
static void ich9pciBiosInitDeviceBARs(PPDMDEVINS pDevIns, PDEVPCIROOT pPciRoot, PDEVPCIBUS pBus, PPDMPCIDEV pPciDev)
{
    ...
    for (int iRegion = 0; iRegion < cRegions; iRegion++)
    {
        ...
        if (cbRegSize64)
        {
                ...
                || uNew >= _4G)
            {
                ...
                {
                    uint16_t uVendor = devpciR3GetWord(pPciDev, VPOX_PCI_VENDOR_ID);
                    uint16_t uDevice = devpciR3GetWord(pPciDev, VPOX_PCI_DEVICE_ID);
    ...
    uint16_t uCmd = devpciR3GetWord(pPciDev, VPOX_PCI_COMMAND);
    if (fActiveMemRegion && !fSuppressMem)
        uCmd |= VPOX_PCI_COMMAND_MEMORY; /* Enable MMIO access. */
    if (fActiveIORegion)
        uCmd |= VPOX_PCI_COMMAND_IO; /* Enable I/O space access. */
    devpciR3SetWord(pDevIns, pPciDev, VPOX_PCI_COMMAND, uCmd);
}
...
static bool ich9pciBiosInitBridgePrefetchable(PPDMDEVINS pDevIns, PDEVPCIROOT pPciRoot, PDEVPCIBUS pBus, bool fUse64Bit, bool fDryrun)
{
    ...
    devpciR3SetDWord(pDevIns, pBridge, VPOX_PCI_PREF_BASE_UPPER32, uBase >> 32);
    devpciR3SetWord(pDevIns, pBridge, VPOX_PCI_PREF_MEMORY_BASE, (uint32_t)(uBase >> 16) & UINT32_C(0xfff0));
    devpciR3SetDWord(pDevIns, pBridge, VPOX_PCI_PREF_LIMIT_UPPER32, uLimit >> 32);
    devpciR3SetWord(pDevIns, pBridge, VPOX_PCI_PREF_MEMORY_LIMIT, (uint32_t)(uLimit >> 16) & UINT32_C(0xfff0));
...
static void ich9pciBiosInitAllDevicesOnBus(PPDMDEVINS pDevIns, PDEVPCIROOT pPciRoot, PDEVPCIBUS pBus)
{
    ...
    for (uint32_t uDevFn = 0; uDevFn < RT_ELEMENTS(pBus->apDevices); uDevFn++)
    {
        ...
        uint16_t uDevClass = devpciR3GetWord(pPciDev, VPOX_PCI_CLASS_DEVICE);
        switch (uDevClass)
        {
            case 0x0101:
                /* IDE controller */
            ...
            case 0x0300:
            {
                 ...
                 */
                uint16_t uCmd = devpciR3GetWord(pPciDev, VPOX_PCI_COMMAND);
                devpciR3SetWord(pDevIns, pPciDev, VPOX_PCI_COMMAND, uCmd | VPOX_PCI_COMMAND_IO);
                break;
            }
            default:
                break;
        ...
        /* map the interrupt */
        uint8_t iPin = devpciR3GetByte(pPciDev, VPOX_PCI_INTERRUPT_PIN);
        if (iPin != 0)
        {
            iPin--;
            ...
            devpciR3SetByte(pDevIns, pPciDev, VPOX_PCI_INTERRUPT_LINE, iIrq);
        }
    }
                                             ...
                                             uint32_t *pbmUsed, uint8_t uBusPrimary)
{
    ...
    *pbmUsed |= RT_BIT_32(pBus->iBus);
    ...
    if (pBus->iBus != 0)
    {
        devpciR3SetByte(pDevIns, pBridgeDev, VPOX_PCI_PRIMARY_BUS, uBusPrimary);
        devpciR3SetByte(pDevIns, pBridgeDev, VPOX_PCI_SECONDARY_BUS, pBus->iBus);
        ...
        devpciR3SetByte(pDevIns, pBridgeDev, VPOX_PCI_SUBORDINATE_BUS, 0xff);
    }
    ...
    if (pBus->iBus != 0)
        devpciR3SetByte(pDevIns, pBridgeDev, VPOX_PCI_SUBORDINATE_BUS, uMaxSubNum);
    ...
    if (pBus->iBus != 0)
        devpciR3SetWord(pDevIns, pBridgeDev, VPOX_PCI_COMMAND,
                          VPOX_PCI_COMMAND_IO
                        | VPOX_PCI_COMMAND_MEMORY
                        | VPOX_PCI_COMMAND_MASTER);

    /* safe, only needs to go to the config space array */
    Log2Func(("for bus %p: primary=%d secondary=%d subordinate=%d\n", pBus,
              PDMPciDevGetByte(pBridgeDev, VPOX_PCI_PRIMARY_BUS),
              PDMPciDevGetByte(pBridgeDev, VPOX_PCI_SECONDARY_BUS),
              PDMPciDevGetByte(pBridgeDev, VPOX_PCI_SUBORDINATE_BUS) ));
 ...
 */
DECLHIDDEN(VPOXSTRICTRC) devpciR3CommonConfigReadWorker(PPDMPCIDEV pPciDev, uint32_t uAddress, unsigned cb, uint32_t *pu32Value)
{
    uint32_t uValue;
 ...
 */
DECLCALLBACK(VPOXSTRICTRC) devpciR3CommonConfigRead(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev,
 ...
 */
static VPOXSTRICTRC devpciR3UpdateMappings(PPDMPCIDEV pPciDev, bool fP2PBridge)
{
    /* safe, only needs to go to the config space array */
    uint16_t const u16Cmd = PDMPciDevGetWord(pPciDev, VPOX_PCI_COMMAND);
    Log4(("devpciR3UpdateMappings: dev %u/%u (%s): u16Cmd=%#x\n",
          pPciDev->uDevFn >> VPOX_PCI_DEVFN_DEV_SHIFT, pPciDev->uDevFn & VPOX_PCI_DEVFN_FUN_MASK, pPciDev->pszNameR3, u16Cmd));
    for (unsigned iRegion = 0; iRegion < VPOX_PCI_NUM_REGIONS; iRegion++)
    {
        ...
        if (cbRegion != 0)
        {
                ...
                if (u16Cmd & VPOX_PCI_COMMAND_IO)
                {
                    ...
                    if (   uLast < _64K
                        ...
                        Log4(("devpciR3UpdateMappings: dev %u/%u (%s): region #%u: Disregarding invalid I/O port range: %#RX32..%#RX64\n",
                              pPciDev->uDevFn >> VPOX_PCI_DEVFN_DEV_SHIFT, pPciDev->uDevFn & VPOX_PCI_DEVFN_FUN_MASK,
            ...
            else if (u16Cmd & VPOX_PCI_COMMAND_MEMORY)
            {
                /* safe, only needs to go to the config space array */
                uint64_t uMemBase = PDMPciDevGetDWord(pPciDev, offCfgReg);
                if (f64Bit)
                {
                    Assert(iRegion < VPOX_PCI_ROM_SLOT);
                    /* safe, only needs to go to the config space array */
            ...
            Log4(("devpciR3UpdateMappings: dev %u/%u (%s): iRegion=%u addr=%#RX64 uNew=%#RX64\n",
                  pPciDev->uDevFn >> VPOX_PCI_DEVFN_DEV_SHIFT, pPciDev->uDevFn & VPOX_PCI_DEVFN_FUN_MASK, pPciDev->pszNameR3,
...
DECLINLINE(void) devpciR3WriteBarByte(PPDMPCIDEV pPciDev, uint32_t iRegion, uint32_t off, uint8_t bVal)
{
    ...
    {
        AssertLogRelReturnVoid(iRegion > 0 && iRegion < VPOX_PCI_ROM_SLOT);
        pRegion--;
        iRegion--;
        off += 4;
...
DECLINLINE(bool) devpciR3IsConfigByteWritable(uint32_t uAddress, uint8_t bHeaderType)
{
    switch (bHeaderType)
    {
        ...
        case 0x80: /* multi-function device */
            ...
            {
                /* Read-only registers. */
                case VPOX_PCI_VENDOR_ID:
                case VPOX_PCI_VENDOR_ID+1:
                case VPOX_PCI_DEVICE_ID:
                case VPOX_PCI_DEVICE_ID+1:
                case VPOX_PCI_REVISION_ID:
                case VPOX_PCI_CLASS_PROG:
                case VPOX_PCI_CLASS_SUB:
                case VPOX_PCI_CLASS_BASE:
                case VPOX_PCI_HEADER_TYPE:
                case VPOX_PCI_SUBSYSTEM_VENDOR_ID:
                case VPOX_PCI_SUBSYSTEM_VENDOR_ID+1:
                case VPOX_PCI_SUBSYSTEM_ID:
                case VPOX_PCI_SUBSYSTEM_ID+1:
                case VPOX_PCI_ROM_ADDRESS:
                case VPOX_PCI_ROM_ADDRESS+1:
                case VPOX_PCI_ROM_ADDRESS+2:
                case VPOX_PCI_ROM_ADDRESS+3:
                case VPOX_PCI_CAPABILITY_LIST:
                case VPOX_PCI_INTERRUPT_PIN:
                    return false;
        ...
        case 0x01: /* PCI-PCI bridge */
            switch (uAddress)
            {
                ...
                case VPOX_PCI_CLASS_BASE:
                case VPOX_PCI_HEADER_TYPE:
                case VPOX_PCI_ROM_ADDRESS_BR:
                case VPOX_PCI_ROM_ADDRESS_BR+1:
                case VPOX_PCI_ROM_ADDRESS_BR+2:
                case VPOX_PCI_ROM_ADDRESS_BR+3:
                case VPOX_PCI_INTERRUPT_PIN:
                    return false;
                /* Other registers can be written. */
                default:
 ...
 */
DECLHIDDEN(VPOXSTRICTRC) devpciR3CommonConfigWriteWorker(PPDMDEVINS pDevIns, PDEVPCIBUSCC pBusCC,
                                                         PPDMPCIDEV pPciDev, uint32_t uAddress, unsigned cb, uint32_t u32Value)
{
    ...
    if (uAddress + cb <= RT_MIN(pPciDev->cbConfig, sizeof(pPciDev->abConfig)))
    {
        ...
        else
        {
             ...
             *       try emulate that.
             */
            uint8_t const   bHeaderType     = devpciR3GetByte(pPciDev, VPOX_PCI_HEADER_TYPE);
            ...
            while (cb-- > 0)
            {
                ...
                switch (uAddress)
                {
                    case VPOX_PCI_COMMAND: /* Command register, bits 0-7. */
                        if (fWritable)
                        {
                            /* safe, only needs to go to the config space array */
                    ...
                    case VPOX_PCI_COMMAND+1: /* Command register, bits 8-15. */
                        if (fWritable)
                        {
                    ...
                    case VPOX_PCI_STATUS:  /* Status register, bits 0-7. */
                    ...
                    case VPOX_PCI_STATUS+1:  /* Status register, bits 8-15. */
                    ...
                    case VPOX_PCI_ROM_ADDRESS:    case VPOX_PCI_ROM_ADDRESS   +1: case VPOX_PCI_ROM_ADDRESS   +2: case VPOX_PCI_ROM_ADDRESS   +3:
                        fRom = true;
                        RT_FALL_THRU();
                    case VPOX_PCI_BASE_ADDRESS_0: case VPOX_PCI_BASE_ADDRESS_0+1: case VPOX_PCI_BASE_ADDRESS_0+2: case VPOX_PCI_BASE_ADDRESS_0+3:
                    case VPOX_PCI_BASE_ADDRESS_1: case VPOX_PCI_BASE_ADDRESS_1+1: case VPOX_PCI_BASE_ADDRESS_1+2: case VPOX_PCI_BASE_ADDRESS_1+3:
                    case VPOX_PCI_BASE_ADDRESS_2: case VPOX_PCI_BASE_ADDRESS_2+1: case VPOX_PCI_BASE_ADDRESS_2+2: case VPOX_PCI_BASE_ADDRESS_2+3:
                    case VPOX_PCI_BASE_ADDRESS_3: case VPOX_PCI_BASE_ADDRESS_3+1: case VPOX_PCI_BASE_ADDRESS_3+2: case VPOX_PCI_BASE_ADDRESS_3+3:
                    case VPOX_PCI_BASE_ADDRESS_4: case VPOX_PCI_BASE_ADDRESS_4+1: case VPOX_PCI_BASE_ADDRESS_4+2: case VPOX_PCI_BASE_ADDRESS_4+3:
                    case VPOX_PCI_BASE_ADDRESS_5: case VPOX_PCI_BASE_ADDRESS_5+1: case VPOX_PCI_BASE_ADDRESS_5+2: case VPOX_PCI_BASE_ADDRESS_5+3:
                        /* We check that, as same PCI register numbers as BARs may mean different registers for bridges */
                        if (!fP2PBridge)
                        {
                            uint32_t iRegion = fRom ? VPOX_PCI_ROM_SLOT : (uAddress - VPOX_PCI_BASE_ADDRESS_0) >> 2;
                            devpciR3WriteBarByte(pPciDev, iRegion, uAddress & 0x3, bVal);
                            fUpdateMappings = true;
                            break;
                        }
                        if (uAddress < VPOX_PCI_BASE_ADDRESS_2 || uAddress > VPOX_PCI_BASE_ADDRESS_5+3)
                        {
                        ...
                        if (   uAddress == VPOX_PCI_IO_BASE
                            || uAddress == VPOX_PCI_IO_LIMIT
                            || uAddress == VPOX_PCI_MEMORY_BASE
                            || uAddress == VPOX_PCI_MEMORY_LIMIT
                            || uAddress == VPOX_PCI_PREF_MEMORY_BASE
                            || uAddress == VPOX_PCI_PREF_MEMORY_LIMIT)
                        {
                            ...
                            bVal &= 0xf0;
                            if (   uAddress == VPOX_PCI_PREF_MEMORY_BASE
                                || uAddress == VPOX_PCI_PREF_MEMORY_LIMIT)
                                bVal |= 0x01;
                        }
 ...
 */
DECLCALLBACK(VPOXSTRICTRC) devpciR3CommonConfigWrite(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev,
...
static void devpciR3InfoPciBus(PDEVPCIBUS pBus, PCDBGFINFOHLP pHlp, unsigned iIndentLvl, bool fRegisters)
{
    ...
    for (uint32_t uDevFn = 0; uDevFn < RT_ELEMENTS(pBus->apDevices); uDevFn++)
    {
        ...
        if (pPciDev != NULL)
        {
            ...
            pHlp->pfnPrintf(pHlp, "%02x:%02x.%d %s%s: %04x-%04x %s%s%s",
                            ...
                            devpciR3GetWord(pPciDev, VPOX_PCI_VENDOR_ID), devpciR3GetWord(pPciDev, VPOX_PCI_DEVICE_ID),
                            pBus->fTypeIch9 ? "ICH9" : pBus->fTypePiix3 ? "PIIX3" : "?type?",
                            pciDevIsMsiCapable(pPciDev)  ? " MSI" : "",
                            pciDevIsMsixCapable(pPciDev) ? " MSI-X" : ""
                            );
            if (devpciR3GetByte(pPciDev, VPOX_PCI_INTERRUPT_PIN) != 0)
            {
                pHlp->pfnPrintf(pHlp, " IRQ%d", devpciR3GetByte(pPciDev, VPOX_PCI_INTERRUPT_LINE));
                pHlp->pfnPrintf(pHlp, " (INTA#->IRQ%d)", 0x10 + ich9pciSlot2ApicIrq(uDevFn >> 3, 0));
            }
            pHlp->pfnPrintf(pHlp, "\n");
            devpciR3InfoIndent(pHlp, iIndentLvl + 2);
            uint8_t uClassBase = devpciR3GetByte(pPciDev, VPOX_PCI_CLASS_BASE);
            uint8_t uClassSub  = devpciR3GetByte(pPciDev, VPOX_PCI_CLASS_SUB);
            ...
            devpciR3InfoIndent(pHlp, iIndentLvl + 2);
            uint16_t iCmd = devpciR3GetWord(pPciDev, VPOX_PCI_COMMAND);
            uint16_t iStatus = devpciR3GetWord(pPciDev, VPOX_PCI_STATUS);
            pHlp->pfnPrintf(pHlp, "Command: %04x, Status: %04x\n", iCmd, iStatus);
            devpciR3InfoIndent(pHlp, iIndentLvl + 2);
            pHlp->pfnPrintf(pHlp, "Bus master: %s\n", iCmd & VPOX_PCI_COMMAND_MASTER ? "Yes" : "No");
    ...
    if (pBus->cBridges > 0)
    {
        ...
        for (uint32_t iBridge = 0; iBridge < pBus->cBridges; iBridge++)
        {
            ...
            uint8_t uPrimary = devpciR3GetByte(pPciDevSub, VPOX_PCI_PRIMARY_BUS);
            uint8_t uSecondary = devpciR3GetByte(pPciDevSub, VPOX_PCI_SECONDARY_BUS);
            uint8_t uSubordinate = devpciR3GetByte(pPciDevSub, VPOX_PCI_SUBORDINATE_BUS);
            devpciR3InfoIndent(pHlp, iIndentLvl);
            ...
            if (   uPrimary != PDMPciDevGetByte(pPciDevSub, VPOX_PCI_PRIMARY_BUS)
                || uSecondary != PDMPciDevGetByte(pPciDevSub, VPOX_PCI_SECONDARY_BUS)
                || uSubordinate != PDMPciDevGetByte(pPciDevSub, VPOX_PCI_SUBORDINATE_BUS))
            {
                devpciR3InfoIndent(pHlp, iIndentLvl);
                pHlp->pfnPrintf(pHlp, "CACHE INCONSISTENCY: primary=%d secondary=%d subordinate=%d\n",
                                PDMPciDevGetByte(pPciDevSub, VPOX_PCI_PRIMARY_BUS),
                                PDMPciDevGetByte(pPciDevSub, VPOX_PCI_SECONDARY_BUS),
                                PDMPciDevGetByte(pPciDevSub, VPOX_PCI_SUBORDINATE_BUS));
            }
            devpciR3InfoIndent(pHlp, iIndentLvl);
            pHlp->pfnPrintf(pHlp, "behind bridge: ");
            uint8_t uIoBase  = devpciR3GetByte(pPciDevSub, VPOX_PCI_IO_BASE);
            uint8_t uIoLimit = devpciR3GetByte(pPciDevSub, VPOX_PCI_IO_LIMIT);
            ...
            devpciR3InfoIndent(pHlp, iIndentLvl);
            pHlp->pfnPrintf(pHlp, "behind bridge: ");
            uint32_t uMemoryBase  = devpciR3GetWord(pPciDevSub, VPOX_PCI_MEMORY_BASE);
            uint32_t uMemoryLimit = devpciR3GetWord(pPciDevSub, VPOX_PCI_MEMORY_LIMIT);
            ...
            devpciR3InfoIndent(pHlp, iIndentLvl);
            pHlp->pfnPrintf(pHlp, "behind bridge: ");
            uint32_t uPrefMemoryRegBase  = devpciR3GetWord(pPciDevSub, VPOX_PCI_PREF_MEMORY_BASE);
            uint32_t uPrefMemoryRegLimit = devpciR3GetWord(pPciDevSub, VPOX_PCI_PREF_MEMORY_LIMIT);
            ...
            {
                uPrefMemoryBase |= (uint64_t)devpciR3GetDWord(pPciDevSub, VPOX_PCI_PREF_BASE_UPPER32) << 32;
                uPrefMemoryLimit |= (uint64_t)devpciR3GetDWord(pPciDevSub, VPOX_PCI_PREF_LIMIT_UPPER32) << 32;
...
static DECLCALLBACK(int) ich9pciR3Construct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE  pCfg)
{
    ...
    PDMPciDevSetWord(&pBus->PciDev,  VPOX_PCI_SEC_STATUS, 0x0280);  /* secondary status */
     ...
     */
    rc = PDMDevHlpSSMRegisterEx(pDevIns, VPOX_ICH9PCI_SAVED_STATE_VERSION,
...
{
    /* Clear regions */
    for (int iRegion = 0; iRegion < VPOX_PCI_NUM_REGIONS; iRegion++)
    {
    ...
    {
        devpciR3SetWord(pDevIns, pDev, VPOX_PCI_COMMAND,
                          devpciR3GetWord(pDev, VPOX_PCI_COMMAND)
                        & ~(VPOX_PCI_COMMAND_IO | VPOX_PCI_COMMAND_MEMORY |
                            VPOX_PCI_COMMAND_MASTER | VPOX_PCI_COMMAND_SPECIAL |
                            VPOX_PCI_COMMAND_PARITY | VPOX_PCI_COMMAND_SERR |
                            VPOX_PCI_COMMAND_FAST_BACK | VPOX_PCI_COMMAND_INTX_DISABLE));

        /* Bridge device reset handlers processed later */
        if (!pciDevIsPci2PciBridge(pDev))
        {
            devpciR3SetByte(pDevIns, pDev, VPOX_PCI_CACHE_LINE_SIZE, 0x0);
            devpciR3SetByte(pDevIns, pDev, VPOX_PCI_INTERRUPT_LINE, 0x0);
        }

        /* Reset MSI message control. */
        if (pciDevIsMsiCapable(pDev))
            devpciR3SetWord(pDevIns, pDev, pDev->Int.s.u8MsiCapOffset + VPOX_MSI_CAP_MESSAGE_CONTROL,
                            devpciR3GetWord(pDev, pDev->Int.s.u8MsiCapOffset + VPOX_MSI_CAP_MESSAGE_CONTROL) & 0xff8e);

        /* Reset MSI-X message control. */
        if (pciDevIsMsixCapable(pDev))
            devpciR3SetWord(pDevIns, pDev, pDev->Int.s.u8MsixCapOffset + VPOX_MSIX_CAP_MESSAGE_CONTROL,
                            devpciR3GetWord(pDev, pDev->Int.s.u8MsixCapOffset + VPOX_MSIX_CAP_MESSAGE_CONTROL) & 0x3fff);
    }
}
...
static uint8_t ich9pcibridgeR3GetExpressPortTypeFromString(const char *pszExpressPortType)
{
    if (!RTStrCmp(pszExpressPortType, "EndPtDev"))
        return VPOX_PCI_EXP_TYPE_ENDPOINT;
    if (!RTStrCmp(pszExpressPortType, "LegEndPtDev"))
        return VPOX_PCI_EXP_TYPE_LEG_END;
    if (!RTStrCmp(pszExpressPortType, "RootCmplxRootPort"))
        return VPOX_PCI_EXP_TYPE_ROOT_PORT;
    if (!RTStrCmp(pszExpressPortType, "ExpressSwUpstream"))
        return VPOX_PCI_EXP_TYPE_UPSTREAM;
    if (!RTStrCmp(pszExpressPortType, "ExpressSwDownstream"))
        return VPOX_PCI_EXP_TYPE_DOWNSTREAM;
    if (!RTStrCmp(pszExpressPortType, "Express2PciBridge"))
        return VPOX_PCI_EXP_TYPE_PCI_BRIDGE;
    if (!RTStrCmp(pszExpressPortType, "Pci2ExpressBridge"))
        return VPOX_PCI_EXP_TYPE_PCIE_BRIDGE;
    if (!RTStrCmp(pszExpressPortType, "RootCmplxIntEp"))
        return VPOX_PCI_EXP_TYPE_ROOT_INT_EP;
    if (!RTStrCmp(pszExpressPortType, "RootCmplxEc"))
        return VPOX_PCI_EXP_TYPE_ROOT_EC;

    AssertLogRelMsgFailedReturn(("Unknown express port type specified"), VPOX_PCI_EXP_TYPE_ROOT_INT_EP);
}
...
static void ich9pciResetBridge(PPDMDEVINS pDevIns)
{
    ...
    if (pBus->iBus != 0)
    {
        ...
        devpciR3SetByte(pDevIns, pPciDev, VPOX_PCI_PRIMARY_BUS, 0);
        devpciR3SetByte(pDevIns, pPciDev, VPOX_PCI_SECONDARY_BUS, 0);
        devpciR3SetByte(pDevIns, pPciDev, VPOX_PCI_SUBORDINATE_BUS, 0);
...
static DECLCALLBACK(int) ich9pcibridgeR3Construct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    if (fExpress)
    {
        PDMPciDevSetCommand(pPciDev, VPOX_PCI_COMMAND_SERR);
        PDMPciDevSetStatus(pPciDev, VPOX_PCI_STATUS_CAP_LIST); /* Has capabilities. */
        PDMPciDevSetByte(pPciDev, VPOX_PCI_CACHE_LINE_SIZE, 8); /* 32 bytes */
        /* PCI Express */
        PDMPciDevSetByte(pPciDev, 0xa0 + 0, VPOX_PCI_CAP_ID_EXP); /* PCI_Express */
        PDMPciDevSetByte(pPciDev, 0xa0 + 1, 0); /* next */
        PDMPciDevSetWord(pPciDev, 0xa0 + 2,
                        /* version */ 0x2
                      | (uExpressPortType << 4));
        PDMPciDevSetDWord(pPciDev, 0xa0 + 4, VPOX_PCI_EXP_DEVCAP_RBE); /* Device capabilities. */
        ...
        PDMPciDevSetDWord(pPciDev, 0xa0 + 12,
                       ...
                       | VPOX_PCI_EXP_LNKCAP_LBNC
                       | /* Port Number */ ((2 + iInstance) << 24)); /* Link capabilities. */
        PDMPciDevSetWord(pPciDev, 0xa0 + 16, VPOX_PCI_EXP_LNKCTL_CLOCK); /* Link control. */
        PDMPciDevSetWord(pPciDev, 0xa0 + 18,
                        /* Current Link Speed */ 2
                      | /* Negotiated Link Width */ (16 << 4)
                      | VPOX_PCI_EXP_LNKSTA_SL_CLK); /* Link status. */
...
#if defined(IN_RING3)
    /* .pszRCMod = */               "VPoxDDRC.rc",
    /* .pszR0Mod = */               "VPoxDDR0.r0",
