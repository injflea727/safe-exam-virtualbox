...
/** @file
 * VPox Qt GUI - UIWizardCloneVM class implementation.
 */
...
bool UIWizardCloneVM::cloneVM()
{
    ...
    CMachine cloneMachine = vpox.CreateMachine(strSettingsFile, strName, QVector<QString>(), QString::null, QString::null);
    if (!vpox.isOk())
    {
    ...
    vpox.RegisterMachine(cloneMachine);
    if (!vpox.isOk())
    {
        msgCenter().cannotRegisterMachine(vpox, cloneMachine.GetName(), this);
        return false;
    }
...
{
    QString strDefaultMachineFolder = uiCommon().virtualPox().GetSystemProperties().GetDefaultMachineFolder();
