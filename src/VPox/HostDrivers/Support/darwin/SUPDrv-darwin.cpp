...
/** @file
 * VirtualPox Support Driver - Darwin Specific Code.
 */
...
*********************************************************************************************************************************/
RT_C_DECLS_BEGIN
static kern_return_t    VPoxDrvDarwinStart(struct kmod_info *pKModInfo, void *pvData);
static kern_return_t    VPoxDrvDarwinStop(struct kmod_info *pKModInfo, void *pvData);
...
static int              VPoxDrvDarwinOpen(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess);
static int              VPoxDrvDarwinClose(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess);
static int              VPoxDrvDarwinIOCtl(dev_t Dev, u_long iCmd, caddr_t pData, int fFlags, struct proc *pProcess);
#ifndef VPOX_WITHOUT_EFLAGS_AC_SET_IN_VPOXDRV
static int              VPoxDrvDarwinIOCtlSMAP(dev_t Dev, u_long iCmd, caddr_t pData, int fFlags, struct proc *pProcess);
#endif
static int              VPoxDrvDarwinIOCtlSlow(PSUPDRVSESSION pSession, u_long iCmd, caddr_t pData, struct proc *pProcess);

static int              VPoxDrvDarwinErr2DarwinErr(int rc);

static IOReturn         VPoxDrvDarwinSleepHandler(void *pvTarget, void *pvRefCon, UInt32 uMessageType, IOService *pProvider, void *pvMessageArgument, vm_size_t argSize);
RT_C_DECLS_END

static int              vpoxdrvDarwinResolveSymbols(void);
static bool             vpoxdrvDarwinCpuHasSMAP(void);
 ...
 */
class org_virtualpox_SupDrv : public IOService
{
    OSDeclareDefaultStructors(org_virtualpox_SupDrv);
...
OSDefineMetaClassAndStructors(org_virtualpox_SupDrv, IOService);
 ...
 * port right.
 */
class org_virtualpox_SupDrvClient : public IOUserClient
{
    OSDeclareDefaultStructors(org_virtualpox_SupDrvClient);
...
private:
    ...
    org_virtualpox_SupDrv  *m_pProvider;    /**< The service provider. */
...
OSDefineMetaClassAndStructors(org_virtualpox_SupDrvClient, IOUserClient);
...
DECLHIDDEN(kmod_start_func_t *) _realmain = VPoxDrvDarwinStart;
DECLHIDDEN(kmod_stop_func_t *)  _antimain = VPoxDrvDarwinStop;
...
static struct cdevsw    g_DevCW =
{
    ...
    /*.d_open  = */VPoxDrvDarwinOpen,
    /*.d_close = */VPoxDrvDarwinClose,
    /*.d_read  = */eno_rdwrt,
    /*.d_write = */eno_rdwrt,
    /*.d_ioctl = */VPoxDrvDarwinIOCtl,
    /*.d_stop  = */eno_stop,
    /*.d_reset = */eno_reset,
    /*.d_ttys  = */NULL,
    /*.d_select= */eno_select,
...
static kern_return_t    VPoxDrvDarwinStart(struct kmod_info *pKModInfo, void *pvData)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = RTSpinlockCreate(&g_Spinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "VPoxDrvDarwin");
            if (RT_SUCCESS(rc))
            {
                if (vpoxdrvDarwinCpuHasSMAP())
                {
                    g_fKernelFeatures |= SUPKERNELFEATURES_SMAP;
#ifndef VPOX_WITHOUT_EFLAGS_AC_SET_IN_VPOXDRV
                    LogRel(("disabling SMAP for VPoxDrvDarwinIOCtl\n"));
                    g_DevCW.d_ioctl = VPoxDrvDarwinIOCtlSMAP;
#endif
                }
                 ...
                 */
                rc = vpoxdrvDarwinResolveSymbols();
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (g_iMajorDeviceNo >= 0)
                    {
                        ...
                        if (g_hDevFsDeviceSys)
                        {
                            ...
                            {
                                LogRel(("VPoxDrv: version " VPOX_VERSION_STRING " r%d; IOCtl version %#x; IDC version %#x; dev major=%d\n",
                                        VPOX_SVN_REV, SUPDRV_IOC_VERSION, SUPDRV_IDC_VERSION, g_iMajorDeviceNo));

                                /* Register a sleep/wakeup notification callback */
                                g_pSleepNotifier = registerPrioritySleepWakeInterest(&VPoxDrvDarwinSleepHandler, &g_DevExt, NULL);
                                if (g_pSleepNotifier == NULL)
                                    LogRel(("VPoxDrv: register for sleep/wakeup events failed\n"));

                                return KMOD_RETURN_SUCCESS;
                            }

                            LogRel(("VPoxDrv: devfs_make_node(makedev(%d,1),,,,%s) failed\n", g_iMajorDeviceNo, DEVICE_NAME_USR));
                            devfs_remove(g_hDevFsDeviceSys);
                            g_hDevFsDeviceSys = NULL;
                        }
                        else
                            LogRel(("VPoxDrv: devfs_make_node(makedev(%d,0),,,,%s) failed\n", g_iMajorDeviceNo, DEVICE_NAME_SYS));

                        cdevsw_remove(g_iMajorDeviceNo, &g_DevCW);
                        g_iMajorDeviceNo = -1;
                    }
                    else
                        LogRel(("VPoxDrv: cdevsw_add failed (%d)\n", g_iMajorDeviceNo));
                }
#ifdef VPOX_WITH_DARWIN_R0_DARWIN_IMAGE_VERIFICATION
                ...
                LogRel(("VPoxDrv: RTSpinlockCreate failed (rc=%d)\n", rc));
            supdrvDeleteDevExt(&g_DevExt);
        }
        else
            printf("VPoxDrv: failed to initialize device extension (rc=%d)\n", rc);
        RTR0TermForced();
    }
    else
        printf("VPoxDrv: failed to initialize IPRT (rc=%d)\n", rc);
...
static int vpoxdrvDarwinResolveSymbols(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            LogRel(("VPoxDrv: vmx_resume=%p vmx_suspend=%p vmx_use_count=%p (%d) cr4=%#x\n",
                    g_pfnVmxResume, g_pfnVmxSuspend, g_pVmxUseCount, *g_pVmxUseCount, ASMGetCR4() ));
        }
        else
        {
            LogRel(("VPoxDrv: failed to resolve vmx stuff: vmx_resume=%Rrc vmx_suspend=%Rrc vmx_use_count=%Rrc", rc1, rc2, rc3));
            g_pfnVmxResume  = NULL;
...
# endif
            ...
            LogRel(("VPoxDrv: g_pfnRdMsrCarefully=%p g_pfnRdMsr64Carefully=%p g_pfnWrMsr64Carefully=%p\n",
        ...
        LogRel(("VPoxDrv: Failed to open kernel symbols, rc=%Rrc\n", rc));
    return rc;
}
...
static int supdrvDarwinInitCertStores(PSUPDRVDEVEXT pDevExt)
{
    ...
    if (RT_SUCCESS(rc))
    {
        for (uint32_t i = 0; i < g_cSUPTrustedTAs; i++)
        {
            ...
            {
                printf("VPoxDrv: Error loading g_aSUPTrustedTAs[%u]: %d\n", i, rc);
                rc = rc2;
            }
        }
        ...
        if (RT_FAILURE(rc2) && RT_SUCCESS(rc))
        {
            printf("VPoxDrv: Error loading g_cbSUPBuildCert: %d\n", rc);
            rc = rc2;
        }
    }
    return rc;
...
#endif /* VPOX_WITH_DARWIN_R0_DARWIN_IMAGE_VERIFICATION */
...
/**
 * Device open. Called on open /dev/vpoxdrv
 *
...
static int VPoxDrvDarwinOpen(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess)
{
    ...
    /*
     * Find the session created by org_virtualpox_SupDrvClient, fail
...
#ifdef DEBUG_DARWIN_GIP
    OSDBGPRINT(("VPoxDrvDarwinOpen: pid=%d '%s' pSession=%p rc=%d\n", proc_pid(pProcess), szName, pSession, rc));
#else
    Log(("VPoxDrvDarwinOpen: g_DevExt=%p pSession=%p rc=%d pid=%d\n", &g_DevExt, pSession, rc, proc_pid(pProcess)));
#endif
    return VPoxDrvDarwinErr2DarwinErr(rc);
}
...
static int VPoxDrvDarwinClose(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess)
{
    ...
    Log(("VPoxDrvDarwinClose: pid=%d\n", (int)RTProcSelf()));
    Assert(proc_pid(pProcess) == (int)RTProcSelf());

    /*
     * Hand the session closing to org_virtualpox_SupDrvClient.
     */
    org_virtualpox_SupDrvClient::sessionClose(RTProcSelf());
    return 0;
}
...
static int VPoxDrvDarwinIOCtl(dev_t Dev, u_long iCmd, caddr_t pData, int fFlags, struct proc *pProcess)
{
...
#ifdef VPOX_WITH_EFLAGS_AC_SET_IN_VPOXDRV
    /*
    ...
    {
        SUPR0Printf("VPoxDrvDarwinIOCtl: EFLAGS.AC=0 detected %u times, refusing all I/O controls!\n", g_DevExt.cBadContextCalls);
        return EDEVERR;
    }
#endif
    ...
    {
        OSDBGPRINT(("VPoxDrvDarwinIOCtl: WHAT?!? pSession == NULL! This must be a mistake... pid=%d iCmd=%#lx\n",
                    (int)Process, iCmd));
        return EINVAL;
    }
    ...
    if (   (uintptr_t)(iCmd - SUP_IOCTL_FAST_DO_FIRST) < (uintptr_t)32
        ...
        rc = VPoxDrvDarwinIOCtlSlow(pSession, iCmd, pData, pProcess);
...
{
    /*
     * Allow VPox R0 code to touch R3 memory. Setting the AC bit disables the
     * SMAP check.
     */
    RTCCUINTREG fSavedEfl = ASMAddFlags(X86_EFL_AC);

    int rc = VPoxDrvDarwinIOCtl(Dev, iCmd, pData, fFlags, pProcess);
...
#endif /* VPOX_WITHOUT_EFLAGS_AC_SET_IN_VPOXDRV */
...
{
    RT_NOREF(pProcess);
    LogFlow(("VPoxDrvDarwinIOCtlSlow: pSession=%p iCmd=%p pData=%p pProcess=%p\n", pSession, iCmd, pData, pProcess));
    ...
    if ((IOC_DIRMASK & iCmd) == IOC_INOUT)
    {
        ...
        {
            OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: cbReq=%#x < %#x; iCmd=%#lx\n", cbReq, (int)sizeof(*pHdr), iCmd));
            return EINVAL;
        }
        if (RT_UNLIKELY((pHdr->fFlags & SUPREQHDR_FLAGS_MAGIC_MASK) != SUPREQHDR_FLAGS_MAGIC))
        {
            OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: bad magic fFlags=%#x; iCmd=%#lx\n", pHdr->fFlags, iCmd));
            return EINVAL;
        }
        ...
        {
            OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: max(%#x,%#x) != %#x; iCmd=%#lx\n", pHdr->cbIn, pHdr->cbOut, cbReq, iCmd));
            return EINVAL;
        }
    }
    else if ((IOC_DIRMASK & iCmd) == IOC_VOID && !cbReq)
    {
        ...
        if (RT_UNLIKELY(rc))
        {
            OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: copyin(%llx,Hdr,) -> %#x; iCmd=%#lx\n", (unsigned long long)pUser, rc, iCmd));
        ...
        if (RT_UNLIKELY(    Hdr.cbIn < sizeof(Hdr)
                        ...
                        ||  cbReq > _1M*16))
        {
            OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: max(%#x,%#x); iCmd=%#lx\n", Hdr.cbIn, Hdr.cbOut, iCmd));
            IPRT_DARWIN_RESTORE_EFL_AC();
            return EINVAL;
        }
        ...
        if (RT_UNLIKELY(!pHdr))
        {
            OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: failed to allocate buffer of %d bytes; iCmd=%#lx\n", cbReq, iCmd));
            IPRT_DARWIN_RESTORE_EFL_AC();
            return ENOMEM;
        }
        ...
        if (RT_UNLIKELY(rc))
        {
            OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: copyin(%llx,%p,%#x) -> %#x; iCmd=%#lx\n",
    ...
    {
        Log(("VPoxDrvDarwinIOCtlSlow: huh? cbReq=%#x iCmd=%#lx\n", cbReq, iCmd));
        return EINVAL;
    }
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        if (pUser)
        {
            ...
            if (cbOut > cbReq)
            {
                OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: too much output! %#x > %#x; uCmd=%#lx!\n", cbOut, cbReq, iCmd));
                cbOut = cbReq;
            }
            rc = copyout(pHdr, pUser, cbOut);
            if (RT_UNLIKELY(rc))
                OSDBGPRINT(("VPoxDrvDarwinIOCtlSlow: copyout(%p,%llx,%#x) -> %d; uCmd=%#lx!\n",
    ...
    else
    {
        ...
        Log(("VPoxDrvDarwinIOCtlSlow: pid=%d iCmd=%lx pData=%p failed, rc=%d\n", proc_pid(pProcess), iCmd, (void *)pData, rc));
        rc = EINVAL;
    }

    Log2(("VPoxDrvDarwinIOCtlSlow: returns %d\n", rc));
    return rc;
}
 ...
 */
DECLEXPORT(int) VPOXCALL SUPDrvDarwinIDC(uint32_t uReq, PSUPDRVIDCREQHDR pReq)
{
    PSUPDRVSESSION  pSession;
 ...
 */
IOReturn VPoxDrvDarwinSleepHandler(void * /* pvTarget */, void *pvRefCon, UInt32 uMessageType,
                                   IOService *pProvider, void *pvMsgArg, vm_size_t cbMsgArg)
{
    RT_NOREF(pProvider, pvMsgArg, cbMsgArg);
    LogFlow(("VPoxDrv: Got sleep/wake notice. Message type was %x\n", uMessageType));
 ...
 */
static DECLCALLBACK(void) vpoxdrvDarwinVmxEnableFix(RTCPUID idCpu, void *pvUser1, void *pvUser2)
{
 ...
 */
int VPOXCALL supdrvOSEnableVTx(bool fEnable)
{
#ifdef VPOX_WITH_HOST_VMX
    int rc;
...
# endif
       )
    {
        ...
        if (fEnable)
        {
                ...
                if (version_major == 14 /* 14 = 10.10 = yosemite */)
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        if (fCaps & SUPVTCAPS_VT_X)
                            rc = RTMpOnAll(vpoxdrvDarwinVmxEnableFix, NULL, NULL);
                        else
                            rc = VERR_VMX_NO_VMX;
                    }
                    if (RT_FAILURE(rc))
            ...
            LogRel(("VPoxDrv: host_vmxon  -> vmx_use_count=%d rc=%Rrc\n", *g_pVmxUseCount, rc));
        }
        else
        {
            ...
            host_vmxoff();
            rc = VINF_SUCCESS;
            LogRel(("VPoxDrv: host_vmxoff -> vmx_use_count=%d\n", *g_pVmxUseCount));
        }
        IPRT_DARWIN_RESTORE_EFL_AC();
    }
    else
 ...
 */
bool VPOXCALL supdrvOSSuspendVTxOnCpu(void)
{
#ifdef VPOX_WITH_HOST_VMX
    /*
 ...
 */
void VPOXCALL   supdrvOSResumeVTxOnCpu(bool fSuspended)
{
#ifdef VPOX_WITH_HOST_VMX
    /*
                                                           ...
                                                           unsigned uSymbol, PRTLDRADDR pValue, void *pvUser)
{
     ...
     * Failed.
     */
    printf("VPoxDrv: Unable to resolve symbol '%s'.\n", pszSymbol);
...
int  VPOXCALL   supdrvOSLdrOpen(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage, const char *pszFilename)
{
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (cbImage == pImage->cbImageBits)
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        printf("VPoxDrv: Failed to allocate %u bytes for %s: %d\n", (unsigned)cbImage, pszFilename, rc);
                }
                else
                {
                    printf("VPoxDrv: Image size mismatch for %s: %#x, ring-3 says %#x\n",
                ...
                printf("VPoxDrv: RTLdrOpenInMemory(%s) failed: %d - %s\n", pszFilename, rc, pErrInfo->Core.pszMsg);
            else
                printf("VPoxDrv: RTLdrOpenInMemory(%s) failed: %d\n", pszFilename, rc);
            RTLdrClose(hLdrMod);
        }
        else if (pErrInfo && RTErrInfoIsSet(&pErrInfo->Core))
            printf("VPoxDrv: RTLdrOpenInMemory(%s) failed: %d - %s\n", pszFilename, rc, pErrInfo->Core.pszMsg);
    ...
    return rc;
#else  /* !VPOX_WITH_DARWIN_R0_DARWIN_IMAGE_VERIFICATION */
...
void VPOXCALL   supdrvOSLdrNotifyLoaded(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage)
{
    NOREF(pDevExt); NOREF(pImage);
}
...
void VPOXCALL   supdrvOSLdrNotifyOpened(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage, const char *pszFilename)
{
    ...
    {
        char szVar[80];
        RTStrPrintf(szVar, sizeof(szVar), "vpoximage"/*-%s*/, pImage->szName);
        char szValue[48];
 ...
 */
int VPOXCALL    supdrvDarwinResumeSuspendedKbds(void)
{
    IPRT_DARWIN_SAVE_EFL_AC();
 ...
 */
bool org_virtualpox_SupDrv::init(OSDictionary *pDictionary)
{
 ...
 * Free the object.
 */
void org_virtualpox_SupDrv::free(void)
{
 ...
 */
IOService *org_virtualpox_SupDrv::probe(IOService *pProvider, SInt32 *pi32Score)
{
 ...
 * Start this service.
 */
bool org_virtualpox_SupDrv::start(IOService *pProvider)
{
    LogFlow(("org_virtualpox_SupDrv::start([%p])\n", this));
 ...
 * Stop this service.
 */
void org_virtualpox_SupDrv::stop(IOService *pProvider)
{
    LogFlow(("org_virtualpox_SupDrv::stop([%p], %p)\n", this, pProvider));
 ...
 */
bool org_virtualpox_SupDrv::terminate(IOOptionBits fOptions)
{
    bool fRc;
    LogFlow(("org_virtualpox_SupDrv::terminate: reference_count=%d g_cSessions=%d (fOptions=%#x)\n",
    ...
    LogFlow(("org_virtualpox_SupDrv::terminate: returns %d\n", fRc));
    return fRc;
}
 ...
 */
bool org_virtualpox_SupDrvClient::initWithTask(task_t OwningTask, void *pvSecurityId, UInt32 u32Type)
{
    LogFlow(("org_virtualpox_SupDrvClient::initWithTask([%p], %#x, %p, %#x) (cur pid=%d proc=%p)\n",
    ...
    if (u32Type != SUP_DARWIN_IOSERVICE_COOKIE)
    {
        ...
        LogRelMax(10,("org_virtualpox_SupDrvClient::initWithTask: Bad cookie %#x (%s)\n", u32Type, szProcName));
        return false;
    }
 ...
 */
bool org_virtualpox_SupDrvClient::start(IOService *pProvider)
{
    LogFlow(("org_virtualpox_SupDrvClient::start([%p], %p) (cur pid=%d proc=%p)\n",
    ...
    {
        m_pProvider = OSDynamicCast(org_virtualpox_SupDrv, pProvider);
        if (m_pProvider)
        {
            Assert(!m_pSession);
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    Log(("org_virtualpox_SupDrvClient::start: created session %p for pid %d\n", m_pSession, (int)RTProcSelf()));
                    return true;
                }

                LogFlow(("org_virtualpox_SupDrvClient::start: already got a session for this process (%p)\n", pCur));
                supdrvSessionRelease(m_pSession);
            }

            m_pSession = NULL;
            LogFlow(("org_virtualpox_SupDrvClient::start: rc=%Rrc from supdrvCreateSession\n", rc));
        }
        else
            LogFlow(("org_virtualpox_SupDrvClient::start: %p isn't org_virtualpox_SupDrv\n", pProvider));
    }
    return false;
}
 ...
 */
/* static */ void org_virtualpox_SupDrvClient::sessionClose(RTPROCESS Process)
{
    /*
     * Find the session and remove it from the hash table.
     *
     * Note! Only one session per process. (Both start() and
     * VPoxDrvDarwinOpen makes sure this is so.)
     */
    const unsigned  iHash = SESSION_HASH(Process);
    RTSpinlockAcquire(g_Spinlock);
     ...
     */
    org_virtualpox_SupDrvClient *pThis = (org_virtualpox_SupDrvClient *)pSession->pvSupDrvClient;
 ...
 */
IOReturn org_virtualpox_SupDrvClient::clientClose(void)
{
    LogFlow(("org_virtualpox_SupDrvClient::clientClose([%p]) (cur pid=%d proc=%p)\n", this, RTProcSelf(), RTR0ProcHandleSelf()));
 ...
 */
IOReturn org_virtualpox_SupDrvClient::clientDied(void)
{
 ...
 */
bool org_virtualpox_SupDrvClient::terminate(IOOptionBits fOptions)
{
 ...
 */
bool org_virtualpox_SupDrvClient::finalize(IOOptionBits fOptions)
{
 ...
 */
void org_virtualpox_SupDrvClient::stop(IOService *pProvider)
{
