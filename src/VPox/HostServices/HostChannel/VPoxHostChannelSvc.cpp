...
static void VPoxHGCMParmUInt32Set(VPOXHGCMSVCPARM *pParm, uint32_t u32)
{
    pParm->type = VPOX_HGCM_SVC_PARM_32BIT;
    pParm->u.uint32 = u32;
}

static int VPoxHGCMParmUInt32Get(VPOXHGCMSVCPARM *pParm, uint32_t *pu32)
{
...
#if 0 /* unused */
static void VPoxHGCMParmPtrSet(VPOXHGCMSVCPARM *pParm, void *pv, uint32_t cb)
{
...
static int VPoxHGCMParmPtrGet(VPOXHGCMSVCPARM *pParm, void **ppv, uint32_t *pcb)
{
...
static PVPOXHGCMSVCHELPERS g_pHelpers = NULL;
                                  ...
                                  uint32_t cbEvent)
{
    if (cbEvent > 0)
    {
        ...
        VPoxHGCMParmPtrGet(&paParms[2], &pvParm, &cbParm);
    ...
    VPoxHGCMParmUInt32Set(&paParms[0], u32ChannelHandle);
    VPoxHGCMParmUInt32Set(&paParms[1], u32Id);
    VPoxHGCMParmUInt32Set(&paParms[3], cbEvent);
}
                                ...
                                uint32_t cbEvent)
{
    ...
    vpoxHostChannelEventParmsSet(pClient->async.paParms,
                                 u32ChannelHandle,
                                 u32Id,
                                 pvEvent,
                                 cbEvent);
...
static DECLCALLBACK(int) svcDisconnect(void *pvService, uint32_t u32ClientID, void *pvClient)
{
    ...
    VPOXHOSTCHCLIENT *pClient = (VPOXHOSTCHCLIENT *)pvClient;

    vpoxHostChannelClientDisconnect(pClient);

    memset(pClient, 0, sizeof(VPOXHOSTCHCLIENT));
...
static DECLCALLBACK(int) svcConnect(void *pvService, uint32_t u32ClientID, void *pvClient, uint32_t fRequestor, bool fRestoring)
{
    ...
    int rc = vpoxHostChannelClientConnect(pClient);
                                  ...
                                  uint64_t tsArrival)
{
    ...
    switch (u32Function)
    {
        case VPOX_HOST_CHANNEL_FN_ATTACH:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_FN_ATTACH\n"));

            if (cParms != 3)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else if (   paParms[0].type != VPOX_HGCM_SVC_PARM_PTR     /* name */
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_32BIT   /* flags */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_32BIT   /* handle */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                rc = VPoxHGCMParmPtrGet(&paParms[0], &pvName, &cbName);

                if (RT_SUCCESS(rc))
                {
                    rc = VPoxHGCMParmUInt32Get(&paParms[1], &u32Flags);
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        rc = vpoxHostChannelAttach(pClient, &u32Handle, (const char *)pvName, u32Flags);

                        if (RT_SUCCESS(rc))
                        {
                            VPoxHGCMParmUInt32Set(&paParms[2], u32Handle);
                        }
                    }
                }
            }
        ...
        case VPOX_HOST_CHANNEL_FN_DETACH:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_FN_DETACH\n"));

            if (cParms != 1)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else if (   paParms[0].type != VPOX_HGCM_SVC_PARM_32BIT   /* handle */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                rc = VPoxHGCMParmUInt32Get(&paParms[0], &u32Handle);

                if (RT_SUCCESS(rc))
                {
                    rc = vpoxHostChannelDetach(pClient, u32Handle);
                }
            }
        } break;

        case VPOX_HOST_CHANNEL_FN_SEND:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_FN_SEND\n"));
            ...
            if (cParms != 2)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR     /* data */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                if (RT_SUCCESS (rc))
                {
                    rc = VPoxHGCMParmPtrGet (&paParms[1], &pvData, &cbData);

                    if (RT_SUCCESS (rc))
                    {
                        rc = vpoxHostChannelSend(pClient, u32Handle, pvData, cbData);
                    }
                }
            }
        } break;

        case VPOX_HOST_CHANNEL_FN_RECV:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_FN_RECV\n"));
            ...
            if (cParms != 4)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR     /* data */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_32BIT   /* sizeReceived */
                     || paParms[3].type != VPOX_HGCM_SVC_PARM_32BIT   /* sizeRemaining */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                if (RT_SUCCESS (rc))
                {
                    ...
                    if (RT_SUCCESS (rc))
                    {
                        ...
                        rc = vpoxHostChannelRecv(pClient, u32Handle,
                                                 pvData, cbData,
                                                 &u32SizeReceived, &u32SizeRemaining);

                        if (RT_SUCCESS(rc))
                        {
                            VPoxHGCMParmUInt32Set(&paParms[2], u32SizeReceived);
                            VPoxHGCMParmUInt32Set(&paParms[3], u32SizeRemaining);
                        }
                    }
                }
            }
        ...
        case VPOX_HOST_CHANNEL_FN_CONTROL:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_FN_CONTROL\n"));
            ...
            if (cParms != 5)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_32BIT   /* code */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_PTR     /* parm */
                     || paParms[3].type != VPOX_HGCM_SVC_PARM_PTR     /* data */
                     || paParms[4].type != VPOX_HGCM_SVC_PARM_32BIT   /* sizeDataReturned */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                if (RT_SUCCESS (rc))
                {
                    rc = VPoxHGCMParmUInt32Get (&paParms[1], &u32Code);

                    if (RT_SUCCESS (rc))
                    {
                        rc = VPoxHGCMParmPtrGet (&paParms[2], &pvParm, &cbParm);

                        if (RT_SUCCESS (rc))
                        {
                            rc = VPoxHGCMParmPtrGet (&paParms[3], &pvData, &cbData);

                            if (RT_SUCCESS (rc))
                            {
                                uint32_t u32SizeDataReturned = 0;

                                rc = vpoxHostChannelControl(pClient, u32Handle, u32Code,
                                                            pvParm, cbParm,
                                                            pvData, cbData, &u32SizeDataReturned);
                                if (RT_SUCCESS(rc))
                                {
                                    VPoxHGCMParmUInt32Set(&paParms[4], u32SizeDataReturned);
                                }
                            }
                        }
                    }
        ...
        case VPOX_HOST_CHANNEL_FN_EVENT_WAIT:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_FN_EVENT_WAIT\n"));
            ...
            if (cParms != 4)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_32BIT   /* id */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_PTR     /* parm */
                     || paParms[3].type != VPOX_HGCM_SVC_PARM_32BIT   /* sizeReturned */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                rc = vpoxHostChannelEventWait(pClient, &fEvent, callHandle, paParms);
        ...
        case VPOX_HOST_CHANNEL_FN_EVENT_CANCEL:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_FN_EVENT_CANCEL\n"));
            ...
            {
                rc = vpoxHostChannelEventCancel(pClient);
            }
        } break;

        case VPOX_HOST_CHANNEL_FN_QUERY:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_FN_QUERY\n"));

            if (cParms != 5)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else if (   paParms[0].type != VPOX_HGCM_SVC_PARM_PTR     /* channel name */
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_32BIT   /* code */
                     || paParms[2].type != VPOX_HGCM_SVC_PARM_PTR     /* parm */
                     || paParms[3].type != VPOX_HGCM_SVC_PARM_PTR     /* data */
                     || paParms[4].type != VPOX_HGCM_SVC_PARM_32BIT   /* sizeDataReturned */
                    ...
                    if (RT_SUCCESS (rc))
                    {
                        ...
                        if (RT_SUCCESS (rc))
                        {
                            ...
                            if (RT_SUCCESS (rc))
                            {
                                ...
                                /** @todo make sure that pvName is a nul terminated */
                                rc = vpoxHostChannelQuery(pClient, (const char *)pvName, u32Code,
                                                          pvParm, cbParm,
                                                          pvData, cbData, &u32SizeDataReturned);
                                if (RT_SUCCESS(rc))
                                {
                                     ...
                                     VPOXHGCMSVCPARM paParms[])
{
    ...
    switch (u32Function)
    {
        case VPOX_HOST_CHANNEL_HOST_FN_REGISTER:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_HOST_FN_REGISTER\n"));
            ...
            if (cParms != 2)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != VPOX_HGCM_SVC_PARM_PTR /* iface */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    rc = VPoxHGCMParmPtrGet(&paParms[1], &pvInterface, &cbInterface);

                    if (RT_SUCCESS(rc))
                    {
                        rc = vpoxHostChannelRegister((const char *)pvName,
                                                     (VPOXHOSTCHANNELINTERFACE *)pvInterface, cbInterface);
                    }
                }
            }
        } break;

        case VPOX_HOST_CHANNEL_HOST_FN_UNREGISTER:
        {
            LogRel2(("svcCall: VPOX_HOST_CHANNEL_HOST_FN_UNREGISTER\n"));
            ...
            else
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    rc = vpoxHostChannelUnregister((const char *)pvName);
                }
            }
        } break;
...
void vpoxSvcClipboardCompleteReadData(VPOXHOSTCHCLIENT *pClient, int rc, uint32_t cbActual)
{
    VPOXHGCMCALLHANDLE callHandle = NULL;
    VPOXHGCMSVCPARM *paParms = NULL;
    bool fReadPending = false;
    if (vpoxSvcClipboardLock())  /* if not can we do anything useful? */
    {
        ...
        vpoxSvcClipboardUnlock();
    }
    if (fReadPending)
    {
        VPoxHGCMParmUInt32Set (&paParms[2], cbActual);
        g_pHelpers->pfnCallComplete (callHandle, rc);
    }
}

/**
 * SSM descriptor table for the VPOXHOSTCHCLIENT structure.
 */
static SSMFIELD const g_aClipboardClientDataFields[] =
{
    SSMFIELD_ENTRY(VPOXHOSTCHCLIENT, u32ClientID),  /* for validation purposes */
    SSMFIELD_ENTRY(VPOXHOSTCHCLIENT, fMsgQuit),
    SSMFIELD_ENTRY(VPOXHOSTCHCLIENT, fMsgReadData),
    SSMFIELD_ENTRY(VPOXHOSTCHCLIENT, fMsgFormats),
    SSMFIELD_ENTRY(VPOXHOSTCHCLIENT, u32RequestedFormat),
    SSMFIELD_ENTRY_TERM()
};
...
static DECLCALLBACK(int) svcSaveState(void *pvService, uint32_t u32ClientID, void *pvClient, PSSMHANDLE pSSM)
{
    ...
    vpoxSvcClipboardCompleteReadData (pClient, VINF_SUCCESS, 0);
 ...
 * VPOXHOSTCHCLIENT structure.  As the structure was saved as a whole
...
extern "C" DECLCALLBACK(DECLEXPORT(int)) VPoxHGCMSvcLoad(VPOXHGCMSVCFNTABLE *pTable)
{
    ...
    {
        LogRel2(("VPoxHGCMSvcLoad: pTable->cbSize = %d, pTable->u32Version = 0x%08X\n",
        ...
        else
        {
            ...
            pTable->cbClient = sizeof(VPOXHOSTCHCLIENT);
