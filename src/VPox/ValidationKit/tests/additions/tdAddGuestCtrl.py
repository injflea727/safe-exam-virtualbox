...
"""
VirtualPox Validation Kit - Guest Control Tests.
"""
...
from testdriver import vpoxtestfileset;
...
class tdTestGuestCtrlBase(object):
    ...
    def createSession(self, sName, fIsError = True):
        ...
        if self.oGuestSession is None:
            ...
            aeWaitFor = [ vpoxcon.GuestSessionWaitForFlag_Start, ];
            try:
                ...
                #
                if waitResult not in (vpoxcon.GuestSessionWaitResult_Start, vpoxcon.GuestSessionWaitResult_WaitFlagNotSupported):
                    # Just log, don't assume an error here (will be done in the main loop then).
...
class tdTestRemoveTree(tdTestRemoveBase):
    ...
    def execute(self, oSubTstDrv):
        ...
        oWrappedProgress = vpoxwrappers.ProgressWrapper(oProgress, oSubTstDrv.oTstDrv.oVPoxMgr, oSubTstDrv.oTstDrv,
        ...
        if vpoxcon.DirectoryRemoveRecFlag_ContentAndDir not in self.afFlags  and  not self.fNotExist:
...
class tdTestFileOpen(tdTestGuestCtrlBase):
    ...
    def __init__(self, sFile = "", eAccessMode = None, eAction = None, eSharing = None,
        ...
        tdTestGuestCtrlBase.__init__(self, oCreds = oCreds);
        self.sFile          = sFile;
        self.eAccessMode    = eAccessMode if eAccessMode is not None else vpoxcon.FileAccessMode_ReadOnly;
        self.eAction        = eAction if eAction is not None else vpoxcon.FileOpenAction_OpenExisting;
        self.eSharing       = eSharing if eSharing is not None else vpoxcon.FileSharingMode_All;
...
class tdTestFileOpenAndWrite(tdTestFileOpen):
    ...
    def __init__(self, sFile = "", eAccessMode = None, eAction = None, eSharing = None, # pylint: disable=too-many-arguments
        ...
        tdTestFileOpen.__init__(self, sFile, eAccessMode if eAccessMode is not None else vpoxcon.FileAccessMode_WriteOnly,
        ...
        self.fAppend   = (   eAccessMode in (vpoxcon.FileAccessMode_AppendOnly, vpoxcon.FileAccessMode_AppendRead)
                          or eAction == vpoxcon.FileOpenAction_AppendOrCreate);
    ...
    def doStepsOnOpenedFile(self, fExpectSuccess, oSubTst):
        ...
        for offFile, abBuf in self.atChunks:
            if self.fUseAtApi:
                ...
                if self.fAppend:
                    if self.abContent is not None: # Try avoid seek as it updates the cached offset in GuestFileImpl.
                        ...
                        try:
                            offSave        = self.oOpenedFile.seek(0, vpoxcon.FileSeekOrigin_Current);
                            offExpectAfter = self.oOpenedFile.seek(0, vpoxcon.FileSeekOrigin_End);
                            self.oOpenedFile.seek(offSave, vpoxcon.FileSeekOrigin_Begin);
                        except:
                            return reporter.errorXcpt();
                ...
                if offFile is not None:
                    try:
                        self.oOpenedFile.seek(offFile, vpoxcon.FileSeekOrigin_Begin);
                    except:
                        return reporter.errorXcpt('seek(%s,Begin)' % (offFile,));
                else:
                    try:
                        offFile = self.oOpenedFile.seek(0, vpoxcon.FileSeekOrigin_Current);
                    except:
                        return reporter.errorXcpt();
                if not self.fAppend:
            ...
            if self.abContent is not None:
                ...
                # (probably) windows, so VPoxService makes that behaviour generic across
                # all OSes.
                #
                if self.fAppend:
            ...
            try:
                ...
                offSeek = self.oOpenedFile.seek(0, vpoxcon.FileSeekOrigin_Current);
            except:
...
class tdTestSession(tdTestGuestCtrlBase):
    ...
    def getSessionCount(self, oVPoxMgr):
        """
        ...
        if self.oGuest is None:
            return 0;
        try:
            aoSession = oVPoxMgr.getArray(self.oGuest, 'sessions')
        except:
...
class tdStepSessionSetEnv(tdSessionStepBase):
    ...
    def execute(self, oTstDrv, oGstCtrlSession, sMsgPrefix):
        ...
        try:
            ...
            if vpox.ComError.equal(oXcpt, self.hrcExpected):
                return True;
            return reporter.errorXcpt('%s: Expected hrc=%#x (%s) got %#x (%s) instead (setenv %s=%s)'
                                      % (sMsgPrefix, self.hrcExpected, vpox.ComError.toString(self.hrcExpected),
                                         vpox.ComError.getXcptResult(oXcpt),
                                         vpox.ComError.toString(vpox.ComError.getXcptResult(oXcpt)),
...
class tdStepSessionBulkEnv(tdSessionStepBase):
    ...
    def execute(self, oTstDrv, oGstCtrlSession, sMsgPrefix):
        ...
        try:
            if oTstDrv.fpApiVer >= 5.0:
                oTstDrv.oVPoxMgr.setArray(oGstCtrlSession, 'environmentChanges', self.asEnv);
            else:
                oTstDrv.oVPoxMgr.setArray(oGstCtrlSession, 'environment', self.asEnv);
        except vpox.ComException as oXcpt:
            # Is this an expected failure?
            if vpox.ComError.equal(oXcpt, self.hrcExpected):
                return True;
...
class tdStepSessionCheckEnv(tdSessionStepBase):
    ...
    def execute(self, oTstDrv, oGstCtrlSession, sMsgPrefix):
        ...
        try:
            if oTstDrv.fpApiVer >= 5.0:
                asCurEnv = oTstDrv.oVPoxMgr.getArray(oGstCtrlSession, 'environmentChanges');
            else:
                asCurEnv = oTstDrv.oVPoxMgr.getArray(oGstCtrlSession, 'environment');
        except:
...
class tdStepStat(tdSessionStepBase):
    ...
    def __init__(self, sPath, hrcExpected = 0, fFound = True, fFollowLinks = True, enmType = None, oTestFsObj = None):
        ...
        self.enmType      = enmType if enmType is not None else vpoxcon.FsObjType_File;
    ...
    def execute(self, oTstDrv, oGstCtrlSession, sMsgPrefix):
        ...
        # Don't execute non-file tests on older VPox version.
        if oTstDrv.fpApiVer >= 5.0 or self.enmType == vpoxcon.FsObjType_File or not self.fFound:
            #
            # Call the API.
            #
            try:
                ...
                ##        VPOX_E_IPRT_ERROR and there seems to be no way to distinguish between
                ##        non-existing files/path and a lot of other errors.  Fix API and test!
                if not self.fFound:
                    return True;
                if vpox.ComError.equal(oXcpt, self.hrcExpected): # Is this an expected failure?
                    return True;
...
class tdStepStatDir(tdStepStat):
    ...
    def __init__(self, sDirPath, oTestDir = None):
        tdStepStat.__init__(self, sPath = sDirPath, enmType = vpoxcon.FsObjType_Directory, oTestFsObj = oTestDir);
...
class tdStepStatFile(tdStepStat):
    ...
    def __init__(self, sFilePath = None, oTestFile = None):
        tdStepStat.__init__(self, sPath = sFilePath, enmType = vpoxcon.FsObjType_File, oTestFsObj = oTestFile);
...
class tdStepStatFileSize(tdStepStat):
    ...
    def __init__(self, sFilePath, cbExactSize = 0):
        tdStepStat.__init__(self, sPath = sFilePath, enmType = vpoxcon.FsObjType_File);
...
class SubTstDrvAddGuestCtrl(base.SubTestDriverBase):
    """
    Sub-test driver for executing guest control (VPoxService, IGuest) tests.
    """
    ...
    def __init__(self, oTstDrv):
        ...
        self.oTestFiles     = None  # type: vpoxtestfileset.TestFileSet
    ...
    def prepareGuestForTesting(self, oSession, oTxsSession, oTestVm):
        ...
        self.oTestFiles = vpoxtestfileset.TestFileSet(oTestVm,
    ...
    def gctrlCopyFileFrom(self, oGuestSession, oTest, fExpected):
        ...
        oProgress = vpoxwrappers.ProgressWrapper(oCurProgress, self.oTstDrv.oVPoxMgr, self.oTstDrv, "gctrlFileCopyFrom");
        oProgress.wait();
    ...
    def gctrlCopyDirFrom(self, oGuestSession, oTest, fExpected):
        ...
        oProgress = vpoxwrappers.ProgressWrapper(oCurProgress, self.oTstDrv.oVPoxMgr, self.oTstDrv, "gctrlDirCopyFrom");
        oProgress.wait();
        if not oProgress.isSuccess():
            oProgress.logResult(fIgnoreErrors = not fExpected);
            return False;
    ...
    def gctrlCopyFileTo(self, oGuestSession, sSrc, sDst, afFlags, fIsError):
        ...
        if oCurProgress is None:
            return reporter.error('No progress object returned');
        oProgress = vpoxwrappers.ProgressWrapper(oCurProgress, self.oTstDrv.oVPoxMgr, self.oTstDrv, "gctrlCopyFileTo");
    ...
    def gctrlReadDirTree(self, oTest, oGuestSession, fIsError, sSubDir = None):
        ...
        ##       VPOX_E_OBJECT_NOT_FOUND or VPOX_E_IPRT_ERROR!
        ##
        ...
        while fRc is True:
            try:
                ...
                if vpox.ComError.notEqual(oXcpt, vpox.ComError.VPOX_E_OBJECT_NOT_FOUND):
                    ##
            ...
            if sName in ('.', '..', ):
                if eType != vpoxcon.FsObjType_Directory:
                    fRc = reporter.error('Wrong type for "%s": %d, expected %d (Directory)'
                                         % (sName, eType, vpoxcon.FsObjType_Directory));
            elif eType == vpoxcon.FsObjType_Directory:
            ...
            elif eType is vpoxcon.FsObjType_File:
                #reporter.log2('  File "%s"' % oFsObjInfo.name);
                cFiles += 1;
            elif eType is vpoxcon.FsObjType_Symlink:
                 #reporter.log2('  Symlink "%s" -- not tested yet' % oFsObjInfo.name);
                cOthers += 1;
            elif    oTestVm.isWindows() \
                 or oTestVm.isOS2() \
                 or eType not in (vpoxcon.FsObjType_Fifo, vpoxcon.FsObjType_DevChar, vpoxcon.FsObjType_DevBlock,
                                  vpoxcon.FsObjType_Socket, vpoxcon.FsObjType_WhiteOut):
                    ...
                    if isinstance(oFsObj, testfileset.TestDir):
                        if eType != vpoxcon.FsObjType_Directory:
                            fRc = reporter.error('%s: expected directory (%d), got eType=%d!'
                                                 % (oFsObj.sPath, vpoxcon.FsObjType_Directory, eType,));
                    elif isinstance(oFsObj, testfileset.TestFile):
                        if eType != vpoxcon.FsObjType_File:
                            fRc = reporter.error('%s: expected file (%d), got eType=%d!'
                                                 % (oFsObj.sPath, vpoxcon.FsObjType_File, eType,));
                    else:
    ...
    def gctrlExecute(self, oTest, oGuestSession, fIsError):
        ...
        iPid = -1;
        aeWaitFor = [ vpoxcon.ProcessWaitForFlag_Start, ];
        try:
            ...
            try:
                ...
                #
                if eStatus == vpoxcon.ProcessStatus_Started:

                    # What to wait for:
                    aeWaitFor = [ vpoxcon.ProcessWaitForFlag_Terminate, ];
                    if vpoxcon.ProcessCreateFlag_WaitForStdOut in oTest.afFlags:
                        aeWaitFor.append(vpoxcon.ProcessWaitForFlag_StdOut);
                    if vpoxcon.ProcessCreateFlag_WaitForStdErr in oTest.afFlags:
                        aeWaitFor.append(vpoxcon.ProcessWaitForFlag_StdErr);
                    ## @todo Add vpoxcon.ProcessWaitForFlag_StdIn.
                    ...
                    while True:
                        ...
                        # Process output:
                        for eFdResult, iFd, sFdNm in [ (vpoxcon.ProcessWaitResult_StdOut, 1, 'stdout'),
                                                       (vpoxcon.ProcessWaitResult_StdErr, 2, 'stderr'), ]:
                            if eWaitResult in (eFdResult, vpoxcon.ProcessWaitResult_WaitFlagNotSupported):
                        ...
                        ## Process input (todo):
                        #if eWaitResult in (vpoxcon.ProcessWaitResult_StdIn, vpoxcon.ProcessWaitResult_WaitFlagNotSupported):
                        #    reporter.log2('Process (PID %d) needs stdin data' % (iPid,));

                        # Termination or error?
                        if eWaitResult in (vpoxcon.ProcessWaitResult_Terminate,
                                           vpoxcon.ProcessWaitResult_Error,
                                           vpoxcon.ProcessWaitResult_Timeout,):
    ...
    def testGuestCtrlSessionEnvironment(self, oSession, oTxsSession, oTestVm): # pylint: disable=too-many-locals
        ...
        aoTests = [
            ...
            tdTestSessionEx([
                tdStepSessionSetEnv('', 'FOO', vpox.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                tdStepRequireMinimumApiVer(5.0), # 4.3 is too relaxed checking input!
                tdStepSessionBulkEnv(['', 'foo=bar'], vpox.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                tdStepSessionSetEnv('FOO=', 'BAR', vpox.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                ]),
        ...
        if (self.oTstDrv.fpApiVer < 6.1 and self.oTstDrv.fpApiVer >= 5.0) or not oTestVm.isWindows():
            aoTests.append(tdTestSessionEx([tdStepSessionSetEnv('=', '===', vpox.ComError.E_INVALIDARG),
                                            tdStepSessionCheckEnv(),
                                            tdStepSessionSetEnv('=FOO', 'BAR', vpox.ComError.E_INVALIDARG),
                                            tdStepSessionCheckEnv(),
                                            tdStepSessionBulkEnv(['=', 'foo=bar'], vpox.ComError.E_INVALIDARG),
                                            tdStepSessionCheckEnv(),
                                            tdStepSessionBulkEnv(['=FOO', 'foo=bar'], vpox.ComError.E_INVALIDARG),
                                            tdStepSessionCheckEnv(),
                                            tdStepSessionBulkEnv(['=D:=D:/tmp', 'foo=bar'], vpox.ComError.E_INVALIDARG),
                                            tdStepSessionCheckEnv(),
                                            tdStepSessionSetEnv('=D:', 'D:/temp', vpox.ComError.E_INVALIDARG),
                                            tdStepSessionCheckEnv(),
                                            ]));
    ...
    def testGuestCtrlSession(self, oSession, oTxsSession, oTestVm):
        ...
        for (i, tTest) in enumerate(atTests):
            ...
            if fRc2:
                if uProtocolVersion >= 2: # For Guest Additions < 4.3 getSessionCount() always will return 1.
                    cCurSessions = oCurTest.getSessionCount(self.oTstDrv.oVPoxMgr);
        ...
        for i in xrange(cMaxGuestSessions + 1):
            ...
            cCurSessions = aoMultiSessions[i].getSessionCount(self.oTstDrv.oVPoxMgr);
        ...
        for i in xrange(cMaxGuestSessions):
            ...
            try:
                aoGuestSessions = self.oTstDrv.oVPoxMgr.getArray(atTests[0][0].oGuest, 'sessions');
            except:
    ...
    def testGuestCtrlSessionFileRefs(self, oSession, oTxsSession, oTestVm):
        ...
        if eWaitResult not in (vpoxcon.GuestSessionWaitResult_Start, vpoxcon.GuestSessionWaitResult_WaitFlagNotSupported):
        ...
        for i in xrange(0, cStaleFiles):
            try:
                if self.oTstDrv.fpApiVer >= 5.0:
                    oGuestSession.fileOpen(sFile, vpoxcon.FileAccessMode_ReadOnly, vpoxcon.FileOpenAction_OpenExisting, 0);
                else:
        ...
        try:    cFiles = len(self.oTstDrv.oVPoxMgr.getArray(oGuestSession, 'files'));
        ...
        if fRc is True:
            ...
            for i in xrange(0, cStaleFiles):
                try:
                    if self.oTstDrv.fpApiVer >= 5.0:
                        oCurFile = oGuestSession.fileOpen(sFile, vpoxcon.FileAccessMode_ReadOnly,
                                                          vpoxcon.FileOpenAction_OpenExisting, 0);
                    else:
            ...
            for i, oFile in enumerate(aoFiles):
                try:
                    ...
                    if eFileStatus != vpoxcon.FileStatus_Closed:
                        fRc = reporter.error('Non-stale file #%d has status %d, expected %d'
                                             % (i, eFileStatus, vpoxcon.FileStatus_Closed));
    ...
    def testGuestCtrlSessionProcRefs(self, oSession, oTxsSession, oTestVm): # pylint: disable=too-many-locals
        ...
        for i in xrange(0, cStaleProcs):
            try:
                oGuestSession.processCreate(sCmd,
                                            ...
                                            [ vpoxcon.ProcessCreateFlag_WaitForStdOut ], 30 * 1000);
                # Note: Use a timeout in the call above for not letting the stale processes
                #       hanging around forever.  This can happen if the installed Guest Additions
                #       do not support terminating guest processes.
            except:
        ...
        try:    cProcesses = len(self.oTstDrv.oVPoxMgr.getArray(oGuestSession, 'processes'));
        except: fRc = reporter.errorXcpt();
        else:
        ...
        if fRc is True:
            ...
            for i, oProcess in enumerate(aoProcesses):
                try:
                    eWaitResult = oProcess.waitForArray([ vpoxcon.ProcessWaitForFlag_Terminate, ], 120 * 1000);
                    eProcessStatus = oProcess.status;
                except:
                    fRc = reporter.errorXcpt('Waiting for non-stale process #%d failed:' % (i,));
                else:
                    if eProcessStatus != vpoxcon.ProcessStatus_TerminatedNormally:
                        fRc = reporter.error('Waiting for non-stale processes #%d resulted in status %d, expected %d (wr=%d)'
                                             % (i, eProcessStatus, vpoxcon.ProcessStatus_TerminatedNormally, eWaitResult));
    ...
    def testGuestCtrlExec(self, oSession, oTxsSession, oTestVm):                # pylint: disable=too-many-locals,too-many-statements
        ...
        # Paths:
        sVPoxControl    = None; ## @todo Get path of installed Guest Additions. Later.
        ...
        if oTestVm.isWindows() or oTestVm.isOS2():
            ...
            if oTestVm.isWindows():
                sVPoxControl = "C:\\Program Files\\Oracle\\VirtualPox Guest Additions\\VPoxControl.exe";
        else:
            sImageOut = "/bin/ls";
            if oTestVm.isLinux(): ## @todo check solaris and darwin.
                sVPoxControl = "/usr/bin/VPoxControl"; # Symlink
        ...
        if oTestVm.isWindows() or oTestVm.isOS2():
            ...
            # atExec.extend([
                ...
                #              afFlags = [ vpoxcon.ProcessCreateFlag_WaitForStdOut, vpoxcon.ProcessCreateFlag_WaitForStdErr ]),
        ...
        if sVPoxControl:
            ...
            atExec.append([ tdTestExec(sCmd = sVPoxControl, asArgs = [ sVPoxControl, 'version' ]),
                           tdTestResultExec(fRc = True) ]);
        ...
        # No sessions left?
        try:
            aSessions = self.oTstDrv.oVPoxMgr.getArray(oSession.o.console.guest, 'sessions');
        except:
            fRc = reporter.errorXcpt();
        else:
        ...
        try:
            ...
            try:    cSessions = len(self.oTstDrv.oVPoxMgr.getArray(oSession.o.console.guest, 'sessions'));
            except: fRc = reporter.errorXcpt();
            else:
                if cSessions != 0:
                    fRc = reporter.error('Found %d stale session(s), expected 0' % (cSessions,));
    ...
    def threadForTestGuestCtrlSessionReboot(self, oGuestProcess):
        ...
        try:
            eWaitResult = oGuestProcess.waitForArray([ vpoxcon.ProcessWaitForFlag_Terminate ], 5 * 60 * 1000);
        except:
        ...
        if eWaitResult == vpoxcon.ProcessWaitResult_Terminate and eStatus == vpoxcon.ProcessStatus_Down:
        ...
        return reporter.error('Process wait across reboot failed: eWaitResult=%d, expected %d; eStatus=%d, expected %d'
                              % (eWaitResult, vpoxcon.ProcessWaitResult_Terminate, eStatus, vpoxcon.ProcessStatus_Down,));
    ...
    def testGuestCtrlSessionReboot(self, oSession, oTxsSession, oTestVm): # pylint: disable=too-many-locals
        ...
        try:
            ...
            try:
                eWaitResult = oGuestProcess.waitForArray([ vpoxcon.ProcessWaitForFlag_Start ], 30 * 1000);
            except:
                ...
                try:    eStatus = oGuestProcess.status;
                    ...
                    if eWaitResult != vpoxcon.ProcessWaitResult_Start:
                        fRc = reporter.error('wait for ProcessWaitForFlag_Start failed: %d, expected %d (Start)'
                                             % (eWaitResult, vpoxcon.ProcessWaitResult_Start,));
                    elif eStatus != vpoxcon.ProcessStatus_Started:
                        fRc = reporter.error('Unexpected process status after startup: %d, wanted %d (Started)'
                                             % (eStatus, vpoxcon.ProcessStatus_Started,));
                    else:
    ...
    def testGuestCtrlExecTimeout(self, oSession, oTxsSession, oTestVm):
        ...
        try:
            ...
            eWaitResult = oGuestSession.waitForArray([ vpoxcon.GuestSessionWaitForFlag_Start, ], 30 * 1000);
        except:
            return (reporter.errorXcpt(), oTxsSession);
        ...
        try:
            ...
            try:
                eWaitResult = oCurProcess.waitForArray([ vpoxcon.ProcessWaitForFlag_Start ], 30 * 1000);
            except:
                fRc = reporter.errorXcpt();
            else:
                if eWaitResult != vpoxcon.ProcessWaitResult_Start:
                    ...
                    for msWait in (1, 32, 2000,):
                        ...
                        try:
                            eWaitResult = oCurProcess.waitForArray([ vpoxcon.ProcessWaitForFlag_Terminate, ], msWait);
                        except:
                            fRc = reporter.errorXcpt();
                            break;
                        if eWaitResult != vpoxcon.ProcessWaitResult_Timeout:
            ...
            #
            # Create another process that doesn't terminate, but which will be killed by VPoxService
            ...
            try:
                ...
                try:
                    ...
                    if eWaitResult != vpoxcon.ProcessWaitResult_Start:
                        ...
                        try:
                            eWaitResult = oCurProcess.waitForArray([ vpoxcon.ProcessWaitForFlag_Terminate, ], 15 * 1000);
                        except:
                            fRc = reporter.errorXcpt();
                        else:
                            if eWaitResult != vpoxcon.ProcessWaitResult_Timeout:
    ...
    def testGuestCtrlDirCreate(self, oSession, oTxsSession, oTestVm):
        ...
        atTests.extend([
            ...
            [ tdTestDirCreate(sDirectory = oTestVm.pathJoin(sScratch, 'foo', 'bar', 'baz'),
                              afFlags = (vpoxcon.DirectoryCreateFlag_Parents,) ), tdTestResultSuccess() ],
            [ tdTestDirCreate(sDirectory = oTestVm.pathJoin(sScratch, 'foo', 'bar', 'baz'),
                              afFlags = (vpoxcon.DirectoryCreateFlag_Parents,) ), tdTestResultSuccess() ],
            # Long random names.
            ...
            try:
                ...
                if not sDirTemp:
                    ...
                    try:
                        ...
                        reporter.log2('%s: eType=%s (dir=%d)' % (sDirTemp, eType, vpoxcon.FsObjType_Directory,));
                        if eType != vpoxcon.FsObjType_Directory:
    ...
    def testGuestCtrlFileRemove(self, oSession, oTxsSession, oTestVm):
        ...
        aoTests = [
            ...
            tdTestRemoveTree(asTestDirs[0], afFlags = [vpoxcon.DirectoryRemoveRecFlag_None,], fRcExpect = False), # ditto
            # Empty paths:
            ...
            tdTestRemoveTree(asTestDirs[3], afFlags = [vpoxcon.DirectoryRemoveRecFlag_ContentOnly,], fRcExpect = True),
            tdTestRemoveDir(asTestDirs[3], fRcExpect = True),
            tdTestRemoveTree(asTestDirs[0], afFlags = [vpoxcon.DirectoryRemoveRecFlag_ContentAndDir,], fRcExpect = True),
            # No error if already delete (RTDirRemoveRecursive artifact).
            tdTestRemoveTree(asTestDirs[0], afFlags = [vpoxcon.DirectoryRemoveRecFlag_ContentAndDir,], fRcExpect = True),
            tdTestRemoveTree(asTestDirs[0], afFlags = [vpoxcon.DirectoryRemoveRecFlag_ContentOnly,],
                             fNotExist = True, fRcExpect = True),
            tdTestRemoveTree(asTestDirs[0], afFlags = [vpoxcon.DirectoryRemoveRecFlag_None,], fNotExist = True, fRcExpect = True),
        ];
        ...
        if fRc is True:
            ...
            if fRc is True:
                ...
                #
                if oTestVm.isWindows() or oTestVm.isOS2():
                    afFlags = [vpoxcon.DirectoryRemoveRecFlag_ContentAndDir,];
                else:
                    afFlags = [vpoxcon.DirectoryRemoveRecFlag_ContentOnly,];
                try:
                    ...
                    oWrappedProgress = vpoxwrappers.ProgressWrapper(oProgress, self.oTstDrv.oVPoxMgr, self.oTstDrv,
    ...
    def testGuestCtrlFileStat(self, oSession, oTxsSession, oTestVm): # pylint: disable=too-many-locals
        ...
        aoTests += [ tdTestSessionEx([ tdStepStat('', vpox.ComError.E_INVALIDARG), ]), ];
            ...
            if isinstance(oFsObj, testfileset.TestFile):
                if eType != vpoxcon.FsObjType_File:
                    fRc = reporter.error('sPath=%s type=file: eType=%s, expected %s!'
                                         % (oFsObj.sPath, eType, vpoxcon.FsObjType_File));
                ...
                if eType != vpoxcon.FsObjType_Directory:
                    fRc = reporter.error('sPath=%s type=dir: eType=%s, expected %s!'
                                         % (oFsObj.sPath, eType, vpoxcon.FsObjType_Directory));
                fFileExists = False;
                fDirExists  = True;
            else:
    ...
    def testGuestCtrlFileOpen(self, oSession, oTxsSession, oTestVm): # pylint: disable=too-many-locals
        ...
        atTests = [
            ...
            [ tdTestFileOpen(sFile = oTestVm.pathJoin(sTempDir, 'no-such-file-or-dir'),
                             eAction = vpoxcon.FileOpenAction_OpenExistingTruncated),                   tdTestResultFailure() ],
            [ tdTestFileOpen(sFile = oTestVm.pathJoin(sTempDir, 'no-such-file-or-dir'),
                             eAccessMode = vpoxcon.FileAccessMode_WriteOnly,
                             eAction = vpoxcon.FileOpenAction_OpenExistingTruncated),                   tdTestResultFailure() ],
            [ tdTestFileOpen(sFile = oTestVm.pathJoin(sTempDir, 'no-such-file-or-dir'),
                             eAccessMode = vpoxcon.FileAccessMode_ReadWrite,
                             eAction = vpoxcon.FileOpenAction_OpenExistingTruncated),                   tdTestResultFailure() ],
        ...
        atTests.extend([
            # O_EXCL and such:
            [ tdTestFileOpen(sFile = sFileForReading, eAction = vpoxcon.FileOpenAction_CreateNew,
                             eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                           tdTestResultFailure() ],
            [ tdTestFileOpen(sFile = sFileForReading, eAction = vpoxcon.FileOpenAction_CreateNew),      tdTestResultFailure() ],
            # Open a file.
            [ tdTestFileOpen(sFile = sFileForReading),                                                  tdTestResultSuccess() ],
            [ tdTestFileOpen(sFile = sFileForReading,
                             eAction = vpoxcon.FileOpenAction_OpenOrCreate),                            tdTestResultSuccess() ],
            # Create a new file.
            [ tdTestFileOpenCheckSize(sFile = asFiles[0], eAction = vpoxcon.FileOpenAction_CreateNew,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asFiles[0], eAction = vpoxcon.FileOpenAction_CreateNew,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultFailure() ],
            [ tdTestFileOpenCheckSize(sFile = asFiles[0], eAction = vpoxcon.FileOpenAction_OpenExisting,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asFiles[0], eAction = vpoxcon.FileOpenAction_CreateOrReplace,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asFiles[0], eAction = vpoxcon.FileOpenAction_OpenOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asFiles[0], eAction = vpoxcon.FileOpenAction_OpenExistingTruncated,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asFiles[0], eAction = vpoxcon.FileOpenAction_AppendOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            # Open or create a new file.
            [ tdTestFileOpenCheckSize(sFile = asFiles[1], eAction = vpoxcon.FileOpenAction_OpenOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            # Create or replace a new file.
            [ tdTestFileOpenCheckSize(sFile = asFiles[2], eAction = vpoxcon.FileOpenAction_CreateOrReplace,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            # Create and append to file (weird stuff).
            [ tdTestFileOpenCheckSize(sFile = asFiles[3], eAction = vpoxcon.FileOpenAction_AppendOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asFiles[4], eAction = vpoxcon.FileOpenAction_AppendOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_WriteOnly),                  tdTestResultSuccess() ],
            ...
            [ tdTestFileOpenCheckSize(sFile = asNonEmptyFiles[0], cbOpenExpected = len(sContent),
                                      eAction = vpoxcon.FileOpenAction_OpenOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asNonEmptyFiles[1], cbOpenExpected = len(sContent),
                                      eAction = vpoxcon.FileOpenAction_OpenOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_WriteOnly),                  tdTestResultSuccess() ],
            ...
            [ tdTestFileOpenCheckSize(sFile = asNonEmptyFiles[0], cbOpenExpected = len(sContent),
                                      eAction = vpoxcon.FileOpenAction_AppendOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_ReadWrite),                  tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asNonEmptyFiles[1], cbOpenExpected = len(sContent),
                                      eAction = vpoxcon.FileOpenAction_AppendOrCreate,
                                      eAccessMode = vpoxcon.FileAccessMode_WriteOnly),                  tdTestResultSuccess() ],
            ...
            [ tdTestFileOpenCheckSize(sFile = asNonEmptyFiles[0], eAccessMode = vpoxcon.FileAccessMode_WriteOnly,
                                      eAction = vpoxcon.FileOpenAction_OpenExistingTruncated),          tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asNonEmptyFiles[1], eAccessMode = vpoxcon.FileAccessMode_WriteOnly,
                                      eAction = vpoxcon.FileOpenAction_CreateOrReplace),                tdTestResultSuccess() ],
            [ tdTestFileOpenCheckSize(sFile = asNonEmptyFiles[2], eAction = vpoxcon.FileOpenAction_CreateOrReplace,
                                      eAccessMode = vpoxcon.FileAccessMode_WriteOnly),                  tdTestResultSuccess() ],
        ]);
    ...
    def testGuestCtrlFileRead(self, oSession, oTxsSession, oTestVm): # pylint: disable=too-many-branches,too-many-statements
        ...
        fRc = True;
        try:
            oFile = oGuestSession.fileOpenEx(sBigPath, vpoxcon.FileAccessMode_ReadWrite, vpoxcon.FileOpenAction_CreateOrReplace,
                                             vpoxcon.FileSharingMode_All, 0, []);
        except:
            ...
            while cbBigFile < (1024 + 32)*1024*1024:
                if not fUseFallback:
                    ...
                    try:
                        oFile.seek(cbBigFile, vpoxcon.FileSeekOrigin_Begin);
                        oFile.write(bytearray(1), 60*1000);
                    except:
                        reporter.logXcpt('cbBigFile=%s' % (sBigPath,));
                        break;
            try:
                cbBigFile = oFile.seek(0, vpoxcon.FileSeekOrigin_End);
            except:
                fRc = reporter.errorXcpt('sBigName=%s' % (sBigPath,));
            try:
                oFile.close();
        ...
        for oTestFile in aoExtraFiles + self.oTestFiles.aoFiles: # type: testfileset.TestFile
            ...
            #
            try:
                oFile = oGuestSession.fileOpenEx(oTestFile.sPath, vpoxcon.FileAccessMode_ReadOnly,
                                                 vpoxcon.FileOpenAction_OpenExisting, vpoxcon.FileSharingMode_All, 0, []);
            except:
            ...
            for cbChunk in acbChunks:
                ...
                try:
                    offFile = oFile.seek(0, vpoxcon.FileSeekOrigin_Begin);
                except:
            ...
            for _ in xrange(8):
                ...
                try:
                    offActual = oFile.seek(offFile, vpoxcon.FileSeekOrigin_Begin);
                except:
                ...
                try:
                    offActual = oFile.seek(0, vpoxcon.FileSeekOrigin_Current);
                except:
                    fRc = reporter.errorXcpt('%s: offFile=%s cbToRead=%s cbContent=%s (#1)'
                                             % (oTestFile.sPath, offFile, cbToRead, oTestFile.cbContent));
                else:
            ...
            try:
                ...
                if vpox.ComError.notEqual(oXcpt, vpox.ComError.E_INVALIDARG):
            ...
            ## @todo Document this behaviour in VirtualPox.xidl.
            try:
                oFile.seek(0, vpoxcon.FileSeekOrigin_Begin);
            except:
            ...
            #
            try:
                cbFile = oFile.seek(0, vpoxcon.FileSeekOrigin_End);
            except:
            ...
            if oTestFile.cbContent > 0:
                for _ in xrange(5):
                    ...
                    try:
                        offFile = oFile.seek(-offSeek, vpoxcon.FileSeekOrigin_End);
                    except:
    ...
    def testGuestCtrlFileWrite(self, oSession, oTxsSession, oTestVm): # pylint: disable=too-many-locals
        ...
        aoTests = [
            # Write at end:
            tdTestFileOpenAndWrite(sFile = sFile, eAction = vpoxcon.FileOpenAction_CreateNew, abContent = abContent,
            ...
            # Appending:
            tdTestFileOpenAndWrite(sFile = sFile, eAction = vpoxcon.FileOpenAction_AppendOrCreate, abContent = abContent,
            ...
            tdTestFileOpenAndWrite(sFile = sFile, eAction = vpoxcon.FileOpenAction_OpenExisting, abContent = abContent,
        ...
        if self.oTstDrv.fpApiVer > 5.2: # Copying directories via Main is supported only in versions > 5.2.
            atTests.extend([
                ...
                [ tdTestCopyToDir(sSrc = sScratchEmptyDirHst, sDst = sScratchDstDir1Gst,
                                 afFlags = [vpoxcon.DirectoryCopyFlag_CopyIntoExisting]),    tdTestResultSuccess() ],
                ...
                [ tdTestCopyToDir(sSrc = sScratchEmptyDirHst, sDst = sScratchDstDir2Gst + oTestVm.pathSep()),
                  tdTestResultFailure() ],
                [ tdTestCopyToDir(sSrc = sScratchEmptyDirHst, sDst = sScratchDstDir2Gst + oTestVm.pathSep(),
                                  afFlags = [vpoxcon.DirectoryCopyFlag_CopyIntoExisting]),
                  tdTestResultSuccess() ],
            ]);
            if not self.fSkipKnownBugs:
                atTests.extend([
    ...
    def testGuestCtrlUpdateAdditions(self, oSession, oTxsSession, oTestVm): # pylint: disable=too-many-locals
        ...
        sVPoxValidationKitIso = self.oTstDrv.sVPoxValidationKitIso;
        if not os.path.isfile(sVPoxValidationKitIso):
            return reporter.log('Validation Kit .ISO not found at "%s"' % (sVPoxValidationKitIso,));
        ...
        if oTestVm.isWindows():
            atTests.extend([
                ...
                # Wrong .ISO.
                [ tdTestUpdateAdditions(sSrc = sVPoxValidationKitIso), tdTestResultFailure() ],
                ...
                #                      sSrc = "V:\\Downloads\\VPoxGuestAdditions-r80354.iso"),
                #  tdTestResultSuccess() ]
            ]);
        else:
        ...
        for (i, tTest) in enumerate(atTests):
            ...
            try:
                ...
                if oCurProgress is not None:
                    oWrapperProgress = vpoxwrappers.ProgressWrapper(oCurProgress, self.oTstDrv.oVPoxMgr,
...
class tdAddGuestCtrl(vpox.TestDriver):                                         # pylint: disable=too-many-instance-attributes,too-many-public-methods
    """
    Guest control using VPoxService on the guest.
    """
    ...
    def actionConfig(self):
        if not self.importVPoxApi(): # So we can use the constant below.
            return False;

        eNic0AttachType = vpoxcon.NetworkAttachmentType_NAT;
    ...
    def testGuestCtrlManual(self, oSession, oTxsSession, oTestVm):                # pylint: disable=too-many-locals,too-many-statements,unused-argument,unused-variable
        ...
        aWaitFor = [ vpoxcon.GuestSessionWaitForFlag_Start ];
        ...
        for _ in xrange(100):
            ...
            aWaitFor = [ vpoxcon.ProcessWaitForFlag_Terminate ];
