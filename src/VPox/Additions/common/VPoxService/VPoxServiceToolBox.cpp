/* $Id: VPoxServiceToolBox.cpp $ */
/** @file
 * VPoxServiceToolbox - Internal (BusyBox-like) toolbox.
 */
...
enum
{
    VPOXSERVICETOOLBOXOPT_MACHINE_READABLE = 1000,
    VPOXSERVICETOOLBOXOPT_VERBOSE
};

/** Options indices for "vpox_cat". */
typedef enum VPOXSERVICETOOLBOXCATOPT
{
    VPOXSERVICETOOLBOXCATOPT_NO_CONTENT_INDEXED = 1000
} VPOXSERVICETOOLBOXCATOPT;

/** Flags for "vpox_ls". */
typedef enum VPOXSERVICETOOLBOXLSFLAG
{
    VPOXSERVICETOOLBOXLSFLAG_NONE,
    VPOXSERVICETOOLBOXLSFLAG_RECURSIVE,
    VPOXSERVICETOOLBOXLSFLAG_SYMLINKS
} VPOXSERVICETOOLBOXLSFLAG;

/** Flags for fs object output. */
typedef enum VPOXSERVICETOOLBOXOUTPUTFLAG
{
    VPOXSERVICETOOLBOXOUTPUTFLAG_NONE,
    VPOXSERVICETOOLBOXOUTPUTFLAG_LONG,
    VPOXSERVICETOOLBOXOUTPUTFLAG_PARSEABLE
} VPOXSERVICETOOLBOXOUTPUTFLAG;

/** The size of the directory entry buffer we're using. */
#define VPOXSERVICETOOLBOX_DIRENTRY_BUF_SIZE (sizeof(RTDIRENTRYEX) + RTPATH_MAX)
...
typedef struct VPOXSERVICETOOLBOXTOOL
{
     ...
     *
     * @todo r=bird: You better revert this, i.e. having pfnHandler return a VPox
...
} VPOXSERVICETOOLBOXTOOL;
/** Pointer to a const tool definition. */
typedef VPOXSERVICETOOLBOXTOOL const *PCVPOXSERVICETOOLBOXTOOL;

/**
 * An file/directory entry. Used to cache
 * file names/paths for later processing.
 */
typedef struct VPOXSERVICETOOLBOXPATHENTRY
{
    /** Our node. */
    RTLISTNODE  Node;
    /** Name of the entry. */
    char       *pszName;
} VPOXSERVICETOOLBOXPATHENTRY, *PVPOXSERVICETOOLBOXPATHENTRY;
...
*********************************************************************************************************************************/
/** Tool definitions. */
static VPOXSERVICETOOLBOXTOOL const g_aTools[] =
{
    { VPOXSERVICE_TOOL_CAT,    vgsvcToolboxCat   , NULL },
    { VPOXSERVICE_TOOL_LS,     vgsvcToolboxLs    , NULL },
    { VPOXSERVICE_TOOL_RM,     vgsvcToolboxRm    , NULL },
    { VPOXSERVICE_TOOL_MKTEMP, vgsvcToolboxMkTemp, NULL },
    { VPOXSERVICE_TOOL_MKDIR,  vgsvcToolboxMkDir , NULL },
    { VPOXSERVICE_TOOL_STAT,   vgsvcToolboxStat  , NULL }
};
...
{
    RTPrintf(VPOX_PRODUCT " Guest Toolbox Version "
             VPOX_VERSION_STRING "\n"
...
static void vgsvcToolboxShowUsage(void)
{
    ...
    RTPrintf("  VPoxService [--use-toolbox] vpox_<command> [<general options>] <parameters>\n\n"
             "General options:\n\n"
             "  --machinereadable          produce all output in machine-readable form\n"
             "  -V                         print version number and exit\n"
             "\n"
             "Commands:\n\n"
             "  vpox_cat    [<general options>] <file>...\n"
             "  vpox_ls     [<general options>] [--dereference|-L] [-l] [-R]\n"
             "      [--verbose|-v] [<file>...]\n"
             "  vpox_rm     [<general options>] [-r|-R] <file>...\n"
             "  vpox_mktemp [<general options>] [--directory|-d] [--mode|-m <mode>]\n"
             "      [--secure|-s] [--tmpdir|-t <path>] <template>\n"
             "  vpox_mkdir  [<general options>] [--mode|-m <mode>] [--parents|-p]\n"
             "      [--verbose|-v] <directory>...\n"
             "  vpox_stat   [<general options>] [--file-system|-f]\n"
...
static void vgsvcToolboxPathBufDestroy(PRTLISTNODE pList)
{
    ...
    PVPOXSERVICETOOLBOXPATHENTRY pEntry, pEntryNext;
    RTListForEachSafe(pList, pEntry, pEntryNext, VPOXSERVICETOOLBOXPATHENTRY, Node)
    {
...
static int vgsvcToolboxPathBufAddPathEntry(PRTLISTNODE pList, const char *pszName)
{
    ...
    int rc = VINF_SUCCESS;
    PVPOXSERVICETOOLBOXPATHENTRY pNode = (PVPOXSERVICETOOLBOXPATHENTRY)RTMemAlloc(sizeof(VPOXSERVICETOOLBOXPATHENTRY));
    if (pNode)
    {
...
/**
 * Performs the actual output operation of "vpox_cat".
 *
 * @return  IPRT status code.
...
static char g_paszCatHelp[] =
    "  VPoxService [--use-toolbox] vpox_cat [<general options>] <file>...\n\n"
    "Concatenate files, or standard input, to standard output.\n"
    "\n";


/**
 * Main function for tool "vpox_cat".
 *
 * @return  RTEXITCODE.
...
static RTEXITCODE vgsvcToolboxCat(int argc, char **argv)
{
    static const RTGETOPTDEF s_aOptions[] =
    {
        ...
        { "--no-content-indexed",  VPOXSERVICETOOLBOXCATOPT_NO_CONTENT_INDEXED,   RTGETOPT_REQ_NOTHING},
              ...
              && RT_SUCCESS(rc))
    {
        ...
        switch (ch)
        {
            ...
            case VPOXSERVICETOOLBOXCATOPT_NO_CONTENT_INDEXED:
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            PVPOXSERVICETOOLBOXPATHENTRY pNodeIt;
            RTListForEach(&inputList, pNodeIt, VPOXSERVICETOOLBOXPATHENTRY, Node)
            {
    ...
    if (RT_FAILURE(rc))
    {
        switch (rc)
        {
            case VERR_ACCESS_DENIED:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_CAT_EXITCODE_ACCESS_DENIED;

            case VERR_FILE_NOT_FOUND:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_CAT_EXITCODE_FILE_NOT_FOUND;

            case VERR_PATH_NOT_FOUND:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_CAT_EXITCODE_PATH_NOT_FOUND;

            case VERR_SHARING_VIOLATION:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_CAT_EXITCODE_SHARING_VIOLATION;

            case VERR_IS_A_DIRECTORY:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_CAT_EXITCODE_IS_A_DIRECTORY;
...
/** @todo r=bird: turns out the host doesn't use or need cname_len, so perhaps we could drop it? */
    if (!(fOutputFlags & VPOXSERVICETOOLBOXOUTPUTFLAG_LONG))
    {
        if (fOutputFlags & VPOXSERVICETOOLBOXOUTPUTFLAG_PARSEABLE)
        {
 ...
 * @param   fFlags          Flags of type VPOXSERVICETOOLBOXLSFLAG.
 * @param   fOutputFlags    Flags of type VPOXSERVICETOOLBOXOUTPUTFLAG.
 * @param   pIdCache        The ID cache.
 */
                                      ...
                                      uint32_t fFlags, uint32_t fOutputFlags, PVGSVCTOOLBOXIDCACHE pIdCache)
{
    ...
    else if (fFlags & VPOXSERVICETOOLBOXLSFLAG_RECURSIVE)
        RTPrintf("%s:\n", pszDir);
    ...
    for (;;)
    {
        ...
        size_t cbDirEntry = VPOXSERVICETOOLBOX_DIRENTRY_BUF_SIZE;
        ...
        switch (pDirEntry->Info.Attr.fMode & RTFS_TYPE_MASK)
        {
            ...
            {
                if (!(fFlags & VPOXSERVICETOOLBOXLSFLAG_SYMLINKS))
                    break;
                RT_FALL_THRU();
            }
            case RTFS_TYPE_DIRECTORY:
            {
                ...
                if (!(fFlags & VPOXSERVICETOOLBOXLSFLAG_RECURSIVE))
                    continue;
...
static int vgsvcToolboxLsHandleDir(const char *pszDir, uint32_t fFlags, uint32_t fOutputFlags, PVGSVCTOOLBOXIDCACHE pIdCache)
{
    ...
    union
    {
        uint8_t         abPadding[VPOXSERVICETOOLBOX_DIRENTRY_BUF_SIZE];
...
static char g_paszLsHelp[] =
    "  VPoxService [--use-toolbox] vpox_ls [<general options>] [option]...\n"
    "                                      [<file>...]\n\n"
...
/**
 * Main function for tool "vpox_ls".
 *
 * @return  RTEXITCODE.
 * @param   argc                    Number of arguments.
 * @param   argv                    Pointer to argument array.
...
static RTEXITCODE vgsvcToolboxLs(int argc, char **argv)
{
    static const RTGETOPTDEF s_aOptions[] =
    {
        { "--machinereadable", VPOXSERVICETOOLBOXOPT_MACHINE_READABLE,      RTGETOPT_REQ_NOTHING },
        { "--dereference",     'L',                                         RTGETOPT_REQ_NOTHING },
        { NULL,                'l',                                         RTGETOPT_REQ_NOTHING },
        { NULL,                'R',                                         RTGETOPT_REQ_NOTHING },
        { "--verbose",         VPOXSERVICETOOLBOXOPT_VERBOSE,               RTGETOPT_REQ_NOTHING}
    };
    ...
    bool     fVerbose     = false;
    uint32_t fFlags       = VPOXSERVICETOOLBOXLSFLAG_NONE;
    uint32_t fOutputFlags = VPOXSERVICETOOLBOXOUTPUTFLAG_NONE;
    ...
    while ((ch = RTGetOpt(&GetState, &ValueUnion)))
    {
        ...
        switch (ch)
        {
            ...
            case 'L': /* Dereference symlinks. */
                fFlags |= VPOXSERVICETOOLBOXLSFLAG_SYMLINKS;
                break;

            case 'l': /* Print long format. */
                fOutputFlags |= VPOXSERVICETOOLBOXOUTPUTFLAG_LONG;
                break;

            case VPOXSERVICETOOLBOXOPT_MACHINE_READABLE:
                fOutputFlags |= VPOXSERVICETOOLBOXOUTPUTFLAG_PARSEABLE;
                break;

            case 'R': /* Recursive processing. */
                fFlags |= VPOXSERVICETOOLBOXLSFLAG_RECURSIVE;
                break;

            case VPOXSERVICETOOLBOXOPT_VERBOSE:
                fVerbose = true;
                break;
    ...
    do
    {
        ...
        int rc2 = RTPathQueryInfoEx(pszPath, &objInfo,
                                    RTFSOBJATTRADD_UNIX,
                                    fFlags & VPOXSERVICETOOLBOXLSFLAG_SYMLINKS ? RTPATH_F_FOLLOW_LINK : RTPATH_F_ON_LINK);
        if (RT_SUCCESS(rc2))
        {
    ...
    if (fOutputFlags & VPOXSERVICETOOLBOXOUTPUTFLAG_PARSEABLE) /* Output termination. */
...
static char g_paszMkTempHelp[] =
    "  VPoxService [--use-toolbox] vpox_mktemp [<general options>] [<options>]\n"
    "                                          <template>\n\n"
...
/**
 * Report the result of a vpox_mktemp operation.
 *
...
/**
 * Main function for tool "vpox_mktemp".
 *
 * @return  RTEXITCODE.
 * @param   argc                    Number of arguments.
 * @param   argv                    Pointer to argument array.
...
static RTEXITCODE vgsvcToolboxMkTemp(int argc, char **argv)
{
    static const RTGETOPTDEF s_aOptions[] =
    {
        { "--machinereadable", VPOXSERVICETOOLBOXOPT_MACHINE_READABLE,
              ...
              && RT_SUCCESS(rc))
    {
        ...
        switch (ch)
        {
            ...
            case 'd':
                fFlags |= VPOXSERVICETOOLBOXMKTEMPFLAG_DIRECTORY;
                break;
            ...
            case 'm':
                ...
                break;
            case 's':
                fFlags |= VPOXSERVICETOOLBOXMKTEMPFLAG_SECURE;
                break;
    ...
    if (fFlags & VPOXSERVICETOOLBOXMKTEMPFLAG_SECURE && fModeSet)
    {
    ...
    if (fFlags & VPOXSERVICETOOLBOXMKTEMPFLAG_DIRECTORY)
    {
        rc = fFlags & VPOXSERVICETOOLBOXMKTEMPFLAG_SECURE
...
static char g_paszMkDirHelp[] =
    "  VPoxService [--use-toolbox] vpox_mkdir [<general options>] [<options>]\n"
    "                                         <directory>...\n\n"
    "Options:\n\n"
...
/**
 * Main function for tool "vpox_mkdir".
 *
 * @return  RTEXITCODE.
 * @param   argc                    Number of arguments.
 * @param   argv                    Pointer to argument array.
...
static char g_paszStatHelp[] =
    "  VPoxService [--use-toolbox] vpox_stat [<general options>] [<options>]\n"
    "                                        <file>...\n\n"
...
/**
 * Main function for tool "vpox_stat".
 *
 * @return  RTEXITCODE.
 * @param   argc                    Number of arguments.
 * @param   argv                    Pointer to argument array.
...
static RTEXITCODE vgsvcToolboxStat(int argc, char **argv)
{
    ...
    int rc = VINF_SUCCESS;
    uint32_t fOutputFlags = VPOXSERVICETOOLBOXOUTPUTFLAG_LONG; /* Use long mode by default. */
    ...
    if (RT_FAILURE(rc))
    {
        ...
        {
            case VERR_ACCESS_DENIED:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_STAT_EXITCODE_ACCESS_DENIED;

            case VERR_FILE_NOT_FOUND:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_STAT_EXITCODE_FILE_NOT_FOUND;

            case VERR_PATH_NOT_FOUND:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_STAT_EXITCODE_PATH_NOT_FOUND;

            case VERR_NET_PATH_NOT_FOUND:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_STAT_EXITCODE_NET_PATH_NOT_FOUND;

            case VERR_INVALID_NAME:
                return (RTEXITCODE)VPOXSERVICETOOLBOX_STAT_EXITCODE_INVALID_NAME;
 ...
 */
static PCVPOXSERVICETOOLBOXTOOL vgsvcToolboxLookUp(const char *pszTool)
{
...
int VGSvcToolboxExitCodeConvertToRc(const char *pszTool, RTEXITCODE rcExit)
{
    ...
    PCVPOXSERVICETOOLBOXTOOL pTool = vgsvcToolboxLookUp(pszTool);
    if (pTool)
...
bool VGSvcToolboxMain(int argc, char **argv, RTEXITCODE *prcExit)
{
    ...
    if (!pTool)
    {
         ...
         * when the first VPoxService argument is --use-toolbox.
         */
        if (argc < 2 || strcmp(argv[1], "--use-toolbox"))
        {
            /** @todo must check for 'vpox_' and fail with a complaint that the tool does
             *        not exist, because vgsvcGstCtrlProcessResolveExecutable will send
             *        us anything with 'vpox_' as a prefix and no absolute path.  So,
             *        handing non-existing vpox_xxx tools to the regular VPoxService main
     ...
     * Invoke the handler.
     */
    RTMsgSetProgName("VPoxService/%s", pszTool);
    AssertPtr(pTool);
