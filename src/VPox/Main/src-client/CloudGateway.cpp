...
{
public:
    CloudClient(ComPtr<IVirtualPox> virtualPox, const Bstr& strProvider, const Bstr& strProfile);
    ~CloudClient() {};
...
CloudClient::CloudClient(ComPtr<IVirtualPox> virtualPox, const Bstr& strProvider, const Bstr& strProfile)
{
    HRESULT hrc = virtualPox->COMGETTER(CloudProviderManager)(mManager.asOutParam());
...
static HRESULT startCloudGateway(ComPtr<IVirtualPox> virtualPox, ComPtr<ICloudNetwork> network, GatewayInfo& gateways)
{
    HRESULT hrc = S_OK;
    ...
    try {
        ...
        CloudClient client(virtualPox, gateways.mCloudProvider, gateways.mCloudProfile);
...
static HRESULT startLocalGateway(ComPtr<IVirtualPox> virtualPox, ComPtr<ISession> aSession, const com::Utf8Str &aCloudNetwork, GatewayInfo& gateways)
{
    /*
    ...
    HRESULT hrc = virtualPox->FindMachine(strGatewayVM.raw(), machine.asOutParam());
    if (SUCCEEDED(hrc))
    {
    ...
    else
    {
        ...
        hrc = virtualPox->CreateMachine(NULL, Bstr(strGatewayVM).raw(), ComSafeArrayAsInParam(groups), Bstr("Ubuntu_64").raw(), Bstr("").raw(), machine.asOutParam());
        if (FAILED(hrc))
        {
        ...
        hrc = virtualPox->RegisterMachine(machine);
        if (FAILED(hrc))
        {
        ...
        */
        ...
        hrc = virtualPox->COMGETTER(SystemProperties)(systemProperties.asOutParam());
        if (FAILED(hrc))
        {
        ...
        hrc = virtualPox->OpenMedium(BstrFmt("%ls\\gateways\\lgw.vdi", defaultMachineFolder.raw()).raw(), DeviceType_HardDisk, AccessMode_ReadWrite, FALSE, hd.asOutParam());
        if (FAILED(hrc))
        {
 ...
 #define LGW_FRONTEND "headless"
    ...
    hrc = virtualPox->COMGETTER(EventSource)(es.asOutParam());
    ComPtr<IEventListener> listener;
    hrc = es->CreateListener(listener.asOutParam());
    com::SafeArray <VPoxEventType_T> eventTypes(1);
    ...
    do
    {
        ...
        if (ev)
        {
            VPoxEventType_T aType;
            hrc = ev->COMGETTER(Type)(&aType);
            if (aType == VPoxEventType_OnGuestPropertyChanged)
            {
                ...
                if (aNextName == "/VirtualPox/Gateway/PublicKey")
                {
...
static bool getProxyForIpAddr(ComPtr<IVirtualPox> virtualPox, const com::Utf8Str &strIpAddr, Bstr &strProxyType, Bstr &strProxyHost, Bstr &strProxyPort)
{
#ifndef VPOX_WITH_PROXY_INFO
    RT_NOREF(virtualPox, strIpAddr, strProxyType, strProxyHost, strProxyPort);
    LogRel(("CLOUD-NET: Proxy support is disabled. Using direct connection.\n"));
    return false;
#else /* VPOX_WITH_PROXY_INFO */
    ComPtr<ISystemProperties> systemProperties;
    ProxyMode_T enmProxyMode;
    HRESULT hrc = virtualPox->COMGETTER(SystemProperties)(systemProperties.asOutParam());
    if (FAILED(hrc))
    {
        LogRel(("CLOUD-NET: Failed to obtain system properties. hrc=%x\n", hrc));
        return false;
    ...
    return true;
#endif /* VPOX_WITH_PROXY_INFO */
}


static HRESULT exchangeInfoBetweenGateways(ComPtr<IVirtualPox> virtualPox, ComPtr<ISession> aSession, GatewayInfo& gateways)
{
    RT_NOREF(virtualPox);
    HRESULT hrc = S_OK;
    do
    {
        ...
        for (int cTriesLeft = 6; cTriesLeft > 0; cTriesLeft--)
        {
            ...
            hrc = guest->CreateSession(Bstr("vpox").raw(), Bstr("vpox").raw(), NULL, Bstr("Cloud Gateway Impersonation").raw(), guestSession.asOutParam());
            if (FAILED(hrc))
            {
...
#define GUEST_CMD "/bin/sh"
        ...
        aArgs.push_back(Bstr("/home/vpox/local-bridge.sh").mutableRaw());
        ...
        if (getProxyForIpAddr(virtualPox, gateways.mCloudPublicIp, strPrimaryProxyType, strPrimaryProxyHost, strPrimaryProxyPort))
        {
            ...
            if (getProxyForIpAddr(virtualPox, gateways.mCloudSecondaryPublicIp, strSecondaryProxyType, strSecondaryProxyHost, strSecondaryProxyPort))
            {
...
HRESULT destroyLocalGateway(ComPtr<IVirtualPox> virtualPox, const GatewayInfo& gateways)
{
    ...
    HRESULT hrc = virtualPox->FindMachine(Bstr(gateways.mGatewayVM).raw(), localGateway.asOutParam());
    if (FAILED(hrc))
    {
...
static HRESULT terminateCloudGateway(ComPtr<IVirtualPox> virtualPox, const GatewayInfo& gateways)
{
...
HRESULT startGateways(ComPtr<IVirtualPox> virtualPox, ComPtr<ICloudNetwork> network, GatewayInfo& gateways)
{
    ...
    if (SUCCEEDED(hrc))
        hrc = startLocalGateway(virtualPox, session, strNetwork, gateways);
    if (SUCCEEDED(hrc))
        hrc = startCloudGateway(virtualPox, network, gateways);
    if (SUCCEEDED(hrc))
        hrc = exchangeInfoBetweenGateways(virtualPox, session, gateways);
...
HRESULT stopGateways(ComPtr<IVirtualPox> virtualPox, const GatewayInfo& gateways)
{
    HRESULT hrc = destroyLocalGateway(virtualPox, gateways);
    AssertComRC(hrc);
    hrc = terminateCloudGateway(virtualPox, gateways);
    AssertComRC(hrc);
    return hrc;
}
