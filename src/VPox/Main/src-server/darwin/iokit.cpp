...
/** The VPoxUSBDevice class name. */
#define VPOXUSBDEVICE_CLASS_NAME "org_virtualpox_VPoxUSBDevice"
...
#define VPOX_OSX_EL_CAPTIAN_VER 15
 ...
 *
 * The source mode for these events is defined as VPOX_IOKIT_MODE_STRING
...
void *DarwinSubscribeUSBNotifications(void)
{
    ...
    if (pNotify->NotifyPort)
    {
        ...
        if (pNotify->NotifyRLSrc)
        {
            ...
            CFRunLoopAddSource(RunLoopRef, pNotify->NotifyRLSrc, CFSTR(VPOX_IOKIT_MODE_STRING));
            ...
            CFRunLoopRemoveSource(RunLoopRef, pNotify->NotifyRLSrc, CFSTR(VPOX_IOKIT_MODE_STRING));
        }
...
void DarwinUnsubscribeUSBNotifications(void *pvOpaque)
{
    ...
    CFRunLoopRemoveSource(CFRunLoopGetCurrent(), pNotify->NotifyRLSrc, CFSTR(VPOX_IOKIT_MODE_STRING));
    IONotificationPortDestroy(pNotify->NotifyPort);
...
static bool darwinUSBDeviceIsGrabbedDetermineState(PUSBDEVICE pCur, io_object_t USBDevice)
{
    ...
    while ((Interface = IOIteratorNext(Interfaces)) != IO_OBJECT_NULL)
    {
        ...
        if (    krc == KERN_SUCCESS
            &&  !strcmp(szName, VPOXUSBDEVICE_CLASS_NAME))
        {
            CFMutableDictionaryRef PropsRef = 0;
            krc = IORegistryEntryCreateCFProperties(Interface, &PropsRef, kCFAllocatorDefault, kNilOptions);
            if (krc == KERN_SUCCESS)
            {
                fHaveOwner = darwinDictGetProcess(PropsRef, CFSTR(VPOXUSB_OWNER_KEY), &Owner);
                fHaveClient = darwinDictGetProcess(PropsRef, CFSTR(VPOXUSB_CLIENT_KEY), &Client);
                CFRelease(PropsRef);
            }
        }
...
/**
 * Worker for determining the USB device state for devices which are not captured by the VPoxUSB driver
...
static void darwinDeterminUSBDeviceState(PUSBDEVICE pCur, io_object_t USBDevice, CFMutableDictionaryRef /* PropsRef */)
{
    ...
    if (!darwinUSBDeviceIsGrabbedDetermineState(pCur, USBDevice))
    {
         ...
         * because VPoxUSB only exposes the legacy interfaces. Trying to use the new classes results in errors
         * because the I/O Kit USB library wants to use the new interfaces. The result is us losing the device
         * form the list when VPoxUSB has attached to the USB device.
         *
         ...
         *
         * The upside is that we can keep VPoxUSB untouched and still compatible with older OS X releases.
         */
        if (g_uMajorDarwin >= VPOX_OSX_EL_CAPTIAN_VER)
        {
...
PDARWINETHERNIC DarwinGetEthernetControllers(void)
{
    ...
    CFIndex cServices = 0;
    SCPreferencesRef PrefsRef = SCPreferencesCreate(kCFAllocatorDefault, CFSTR("org.virtualpox.VPoxSVC"), NULL);
    if (PrefsRef)
    {
 ...
 * To build:
 *      g++ -I ../../../../include -D IN_RING3 iokit.cpp   ../../../../out/darwin.x86/debug/lib/RuntimeR3.a  ../../../../out/darwin.x86/debug/lib/SUPR3.a  ../../../../out/darwin.x86/debug/lib/RuntimeR3.a ../../../../out/darwin.x86/debug/lib/VPox-kStuff.a  ../../../../out/darwin.x86/debug/lib/RuntimeR3.a -framework CoreFoundation -framework IOKit -framework SystemConfiguration -liconv -D STANDALONE_TESTCASE -o iokit -g && ./iokit
 */
