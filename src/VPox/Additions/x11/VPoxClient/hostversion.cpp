...
{
    return ".vpoxclient-hostversion.pid";
}
...
/** @todo Move this part in VbglR3 and just provide a callback for the platform-specific
          notification stuff, since this is very similar to the VPoxTray code. */
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            if (bUpdate)
            {
                ...
                RTStrPrintf(szTitle, sizeof(szTitle), "VirtualPox Guest Additions update available!");
#ifndef VPOX_OSE
...
* Guest Additions.  Distributors are encouraged to customise this. */
                RTStrPrintf(szMsg, sizeof(szMsg), "Your virtual machine is currently running the Guest Additions version %s. Since you are running a version of the Guest Additions provided by the operating system you installed in the virtual machine we recommend that you update it to at least version %s using that system's update features, or alternatively that you remove this version and then install the " VPOX_VENDOR_SHORT " Guest Additions package using the install option from the Devices menu. Please consult the documentation for the operating system you are running to find out how to update or remove the current Guest Additions package.", pszGuestVersion, pszHostVersion);
#endif
                rc = showNotify(szTitle, szMsg);
                VBClLogInfo("VirtualPox Guest Additions update available!\n");
                if (RT_FAILURE(rc))
