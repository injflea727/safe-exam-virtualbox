...
#ifndef VPOX_INCLUDED_HostServices_GuestControlSvc_h
#define VPOX_INCLUDED_HostServices_GuestControlSvc_h
...
#define VPOX_GUESTCTRL_SESSION_ID_BASE  1

/** Builds a context ID out of the session ID, object ID and an
 *  increasing count. */
#define VPOX_GUESTCTRL_CONTEXTID_MAKE(uSession, uObject, uCount) \
    (  (uint32_t)((uSession) &   0x1f) << 27 \
     | (uint32_t)((uObject)  &  0x7ff) << 16 \
     | (uint32_t)((uCount)   & 0xffff)       \
    )
/** Creates a context ID out of a session ID. */
#define VPOX_GUESTCTRL_CONTEXTID_MAKE_SESSION(uSession) \
    ((uint32_t)((uSession) & 0x1f) << 27)
/** Gets the session ID out of a context ID. */
#define VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(uContextID) \
    (((uContextID) >> 27) & 0x1f)
/** Gets the process ID out of a context ID. */
#define VPOX_GUESTCTRL_CONTEXTID_GET_OBJECT(uContextID) \
    (((uContextID) >> 16) & 0x7ff)
/** Gets the context count of a process out of a context ID. */
#define VPOX_GUESTCTRL_CONTEXTID_GET_COUNT(uContextID) \
    ((uContextID) & 0xffff)
/** Filter context IDs by session. Can be used in conjunction
 *  with VbglR3GuestCtrlMsgFilterSet(). */
#define VPOX_GUESTCTRL_FILTER_BY_SESSION(uSession) \
    (VPOX_GUESTCTRL_CONTEXTID_MAKE_SESSION(uSession) | 0xF8000000)

/**
 * Structure keeping the context of a host callback.
 */
typedef struct VPOXGUESTCTRLHOSTCBCTX
{
    /** HGCM message number. */
    uint32_t uMessage;
    /** The context ID. */
    ...
    uint32_t uProtocol;
} VPOXGUESTCTRLHOSTCBCTX, *PVPOXGUESTCTRLHOSTCBCTX;

/**
 * Structure for low level HGCM host callback from
 * the guest. No deep copy. */
typedef struct VPOXGUESTCTRLHOSTCALLBACK
{
    /** Number of HGCM parameters. */
    uint32_t         mParms;
    /** Actual HGCM parameters. */
    PVPOXHGCMSVCPARM mpaParms;
} VPOXGUESTCTRLHOSTCALLBACK, *PVPOXGUESTCTRLHOSTCALLBACK;
 ...
 * @{ */
#define VPOX_GUESTCTRL_DST_ROOT_SVC     RT_BIT_64(63)
#define VPOX_GUESTCTRL_DST_SESSION      RT_BIT_64(62)
#define VPOX_GUESTCTRL_DST_BOTH         ( VPOX_GUESTCTRL_DST_ROOT_SVC | VPOX_GUESTCTRL_DST_SESSION )
/** @} */
...
enum eGuestMsg
{
     ...
     * VPox status code to pass onto Main when skipping the message, defaults to
     ...
     *
     * Called by the root VPoxService to explicitly tell the host that's the master
     * service.  Required to use main VPoxGuest device node.  No parameters.
     *
     * @retval  VINF_SUCCESS on success.
     ...
     *
     * VPoxService makes this call before spawning a session process (must be
     ...
     *
     * VPoxService makes this call to clean up after spawning a session process
     ...
     *
     * VPoxService makes this call right after becoming master to indicate to the
     ...
     * guest with the guest features (VPOX_GUESTCTRL_GF_XXX), the host replies by
     * replacing the parameter values with the host ones (VPOX_GUESTCTRL_HF_XXX).
     *
     * @retval  VINF_SUCCESS on success.
...
enum GUEST_FILE_NOTIFYTYPE
{
    ...
    GUEST_FILE_NOTIFYTYPE_READ_OFFSET,  /**< @since 6.0.10, 5.2.32 - VPOX_GUESTCTRL_HF_0_NOTIFY_RDWR_OFFSET */
    GUEST_FILE_NOTIFYTYPE_WRITE = 40,
    GUEST_FILE_NOTIFYTYPE_WRITE_OFFSET, /**< @since 6.0.10, 5.2.32 - VPOX_GUESTCTRL_HF_0_NOTIFY_RDWR_OFFSET */
...
/** @name VPOX_GUESTCTRL_GF_XXX - Guest features.
 * @sa GUEST_MSG_REPORT_FEATURES
 * @{ */
/** Supports HOST_MSG_FILE_SET_SIZE. */
#define VPOX_GUESTCTRL_GF_0_SET_SIZE                RT_BIT_64(0)
...
#define VPOX_GUESTCTRL_GF_0_PROCESS_ARGV0           RT_BIT_64(1)
...
#define VPOX_GUESTCTRL_GF_0_PROCESS_DYNAMIC_SIZES   RT_BIT_64(2)
/** Bit that must be set in the 2nd parameter, will be cleared if the host reponds
 * correctly (old hosts might not). */
#define VPOX_GUESTCTRL_GF_1_MUST_BE_ONE             RT_BIT_64(63)
/** @} */
...
#define VPOX_GUESTCTRL_HF_0_NOTIFY_RDWR_OFFSET      RT_BIT_64(0)
/** Host supports sending (treating) argv[0] separately from the actual execution command.
 *  Needed when newer Guest Additions which support VPOX_GUESTCTRL_GF_0_PROCESS_ARGV0 run on an older
 *  host which doesn't in turn support VPOX_GUESTCTRL_HF_0_PROCESS_ARGV0. */
#define VPOX_GUESTCTRL_HF_0_PROCESS_ARGV0           RT_BIT_64(1)
/** @} */
...
#endif /* !VPOX_INCLUDED_HostServices_GuestControlSvc_h */
