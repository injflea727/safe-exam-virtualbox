/* $Id: VPoxGuest-freebsd.c $ */
/** @file
 * VirtualPox Guest Additions Driver for FreeBSD.
 */
...
static int        vgdrvFreeBSDIOCtlSlow(PVPOXGUESTSESSION pSession, u_long ulCmd, caddr_t pvData, struct thread *pTd);
...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static MALLOC_DEFINE(M_VPOXGUEST, "vpoxguest", "VirtualPox Guest Device Driver");
...
static void vgdrvFreeBSDClone(void *pvArg, struct ucred *pCred, char *pszName, int cchName, struct cdev **ppDev)
{
     ...
     * /dev/vpoxguest<N> where N = {0...255}.
     */
    if (!ppDev)
        return;
    if (strcmp(pszName, "vpoxguest") == 0)
        iUnit =  -1;
    else if (dev_stdclone(pszName, NULL, "vpoxguest", &iUnit) != 1)
        return;
    if (iUnit >= 256)
    {
...
#endif
{
    ...
    fRequestor |= VMMDEV_REQUESTOR_NO_USER_DEVICE; /** @todo implement /dev/vpoxuser
    if (!fUnrestricted)
...
{
    PVPOXGUESTSESSION pSession = (PVPOXGUESTSESSION)pDev->si_drv1;
...
{
    struct VPoxGuestDeviceState *pState = device_get_softc(pDevice);
...
static int vgdrvFreeBSDAddIRQ(device_t pDevice, void *pvState)
{
    int iResId = 0;
    int rc = 0;
    struct VPoxGuestDeviceState *pState = (struct VPoxGuestDeviceState *)pvState;
...
static int vgdrvFreeBSDAttach(device_t pDevice)
{
    int rc;
    int iResId;
    struct VPoxGuestDeviceState *pState;
...
static driver_t vgdrvFreeBSDDriver =
{
    DEVICE_NAME,
    vgdrvFreeBSDMethods,
    sizeof(struct VPoxGuestDeviceState),
};

static devclass_t vgdrvFreeBSDClass;

DRIVER_MODULE(vpoxguest, pci, vgdrvFreeBSDDriver, vgdrvFreeBSDClass, 0, 0);
MODULE_VERSION(vpoxguest, 1);
