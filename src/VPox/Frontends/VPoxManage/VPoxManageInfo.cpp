/* $Id: VPoxManageInfo.cpp $ */
/** @file
 * VPoxManage - The 'showvminfo' command and helper routines.
 */
...
#define SHOW_STRING_PROP_MAJ(a_pObj, a_Prop, a_pszMachine, a_pszHuman, a_pszUnless, a_uMajorVer) \
    ...
    { \
        ...
        Bstr bstr; \
        CHECK_ERROR2I_RET(a_pObj, COMGETTER(a_Prop)(bstr.asOutParam()), hrcCheck); \
        if ((a_uMajorVer) <= VPOX_VERSION_MAJOR || !bstr.equals(a_pszUnless)) \
        { \
            if (details == VMINFO_MACHINEREADABLE)\
                outputMachineReadableString(a_pszMachine, &bstr); \
            else \
...
#define SHOW_LONG64_PROP(a_pObj, a_Prop, a_pszMachine, a_pszHuman, a_pszUnit) \
     ...
     *    is all lowercase for "VPoxManage modifyvm" parameters. Any
    ...
    if (!fAccessible)
    {
        ...
        else
        {
            ...
            if (details != VMINFO_MACHINEREADABLE)
            {
                ...
                ComPtr<IVirtualPoxErrorInfo> accessError;
    ...
    pVirtualPox->GetGuestOSType(osTypeId.raw(), osType.asOutParam());
    if (!osType.isNull())
 ...
 *        checking where missing. */
    ...
    ULONG maxNICs = getMaxNics(pVirtualPox, machine);
...
/** @todo r=klaus dnsproxy etc needs to be dumped, too */
    ...
    pVirtualPox->COMGETTER(SystemProperties)(sysProps.asOutParam());
    ...
    /* globally shared folders first */
    {
        ...
        CHECK_ERROR_RET(pVirtualPox, COMGETTER(SharedFolders)(ComSafeArrayAsOutParam(sfColl)), rc);
...
RTEXITCODE handleShowVMInfo(HandlerArg *a)
{
    ...
    ComPtr<IMachine> machine;
    CHECK_ERROR(a->virtualPox, FindMachine(Bstr(VMNameOrUuid).raw(),
                                           machine.asOutParam()));
    if (FAILED(rc))
    ...
    else
    {
        ...
        rc = showVMInfo(a->virtualPox, machine, a->session, details);
