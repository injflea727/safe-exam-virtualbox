; $Id: VPoxGuestAdditionsCommon.nsh $
;; @file
; VPoxGuestAdditionsCommon.nsh - Common / shared utility functions.
;
...
!ifdef VPOX_WITH_LICENSE_INSTALL_RTF
  ; Copy license file (if any) into the installation directory
  FILE "/oname=${LICENSE_FILE_RTF}" "$%VPOX_BRAND_LICENSE_RTF%"
!endif

  FILE "$%VPOX_PATH_DIFX%\DIFxAPI.dll"
  FILE "$%PATH_OUT%\bin\additions\VPoxDrvInst.exe"

  FILE "$%PATH_OUT%\bin\additions\VPoxVideo.inf"
!ifdef VPOX_SIGN_ADDITIONS
  ${If} $g_strWinVersion == "10"
    FILE "$%PATH_OUT%\bin\additions\VPoxVideo.cat"
  ${Else}
    FILE "/oname=VPoxVideo.cat" "$%PATH_OUT%\bin\additions\VPoxVideo-PreW10.cat"
  ${EndIf}
!endif
...
Function ExtractFiles
  ...
  ; Root files
  SetOutPath "$0"
!if $%VPOX_WITH_LICENSE_INSTALL_RTF% == "1"
  FILE "/oname=${LICENSE_FILE_RTF}" "$%VPOX_BRAND_LICENSE_RTF%"
!endif

  ; Video driver
  SetOutPath "$0\VPoxVideo"
...
!if $%BUILD_TARGET_ARCH% == "x86"
  SetOutPath "$0\VPoxMouse\NT4"
  FILE "$%PATH_OUT%\bin\additions\VPoxMouseNT.sys"
!endif

  ; Guest driver
  SetOutPath "$0\VPoxGuest"
...
!endif
  ...
  ; Shared Folders
  SetOutPath "$0\VPoxSF"
  FILE "$%PATH_OUT%\bin\additions\VPoxSF.sys"
  FILE "$%PATH_OUT%\bin\additions\VPoxMRXNP.dll"
  !if $%BUILD_TARGET_ARCH% == "amd64"
    ; Only 64-bit installer: Also copy 32-bit DLLs on 64-bit target
    FILE "$%PATH_OUT%\bin\additions\VPoxMRXNP-x86.dll"
  !endif

  ; Auto-Logon
  SetOutPath "$0\AutoLogon"
  FILE "$%PATH_OUT%\bin\additions\VPoxGINA.dll"
  FILE "$%PATH_OUT%\bin\additions\VPoxCredProv.dll"
...
Function ${un}GetAdditionsVersion
  ...
  ReadRegStr $0 HKLM "SOFTWARE\$%VPOX_VENDOR_SHORT%\VirtualPox Guest Additions" "Version"

  ; Get revision
  ReadRegStr $g_strAddVerRev HKLM "SOFTWARE\$%VPOX_VENDOR_SHORT%\VirtualPox Guest Additions" "Revision"
...
!macro StopVPoxService un
Function ${un}StopVPoxService

  Push $0   ; Temp results
  Push $1
  Push $2   ; Image name of VPoxService
  Push $3   ; Safety counter

  StrCpy $3 "0" ; Init counter
  ${LogVerbose} "Stopping VPoxService ..."

svc_stop:

  ${LogVerbose} "Stopping VPoxService via SCM ..."
  ${If} $g_strWinVersion == "NT4"
    nsExec::Exec '"$SYSDIR\net.exe" stop VPoxService'
  ${Else}
    nsExec::Exec '"$SYSDIR\SC.exe" stop VPoxService'
  ${EndIf}
...
!ifdef _DEBUG
  ${LogVerbose} "Stopping VPoxService (as exe) ..."
!endif
...
exit:
  ...
  ${LogVerbose} "Stopping VPoxService done"
...
FunctionEnd
!macroend
!insertmacro StopVPoxService ""
!insertmacro StopVPoxService "un."

!macro StopVPoxTray un
Function ${un}StopVPoxTray

  Push $0   ; Temp results
  Push $1   ; Safety counter

  StrCpy $1 "0" ; Init counter
  ${LogVerbose} "Stopping VPoxTray ..."
...
exe_stop:
  ...
  ${nsProcess::FindProcess} "VPoxTray.exe" $0
  StrCmp $0 0 0 exit

  ${nsProcess::KillProcess} "VPoxTray.exe" $0
  Sleep "1000"           ; Wait a bit
  Goto exe_stop

exit:

  ${LogVerbose} "Stopping VPoxTray done"
...
FunctionEnd
!macroend
!insertmacro StopVPoxTray ""
!insertmacro StopVPoxTray "un."

!macro StopVPoxMMR un
Function ${un}StopVPoxMMR

  Push $0   ; Temp results
  Push $1   ; Safety counter

  StrCpy $1 "0" ; Init counter
  DetailPrint "Stopping VPoxMMR ..."
...
exe_stop:
  ...
  ${nsProcess::FindProcess} "VPoxMMR.exe" $0
  StrCmp $0 0 0 exit

  ${nsProcess::KillProcess} "VPoxMMR.exe" $0
  Sleep "1000"           ; Wait a bit
  Goto exe_stop

exit:

  DetailPrint "Stopping VPoxMMR done."
...
FunctionEnd
!macroend
!insertmacro StopVPoxMMR ""
!insertmacro StopVPoxMMR "un."
...
found:
  ...
  VPoxGuestInstallHelper::FileGetVendor "$0"
  ; Stack: <vendor> $1 $0
  Pop  $0 ; Get vendor
  Pop  $1 ; Restore $1
...
found:
  ...
  VPoxGuestInstallHelper::FileGetArchitecture "$0"
...
; a VPox XPDM driver installation before. This might be necessary for upgrading a
