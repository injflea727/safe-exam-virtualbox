...
#if defined(VPOX)
VOID EFIAPI VPoxLogWorker(const char *pszFormat, ...);

/** See RT_XSTR */
# define VPOX_XSTR(str)                         VPOX_STR(str)
/** See RT_STR */
# define VPOX_STR(str)                          #str
# if defined(EFI_LOG_ENABLED)
#  define VPoxLogFlowFuncEnter()                DEBUG((DEBUG_INFO, "%a:" VPOX_XSTR(__LINE__) ": ENTER\n", __FUNCTION__))
#  define VPoxLogFlowFuncLeave()                DEBUG((DEBUG_INFO, "%a:" VPOX_XSTR(__LINE__) ": LEAVE\n", __FUNCTION__))
#  define VPoxLogFlowFuncMark()                 DEBUG((DEBUG_INFO, "%a:" VPOX_XSTR(__LINE__) "\n", __FUNCTION__))
#  define VPoxLogFlowFuncLeaveRC(rc) \
    do { \
        EFI_STATUS rcLog = (rc); \
        VPoxLog(("%a:" VPOX_XSTR(__LINE__) ": LEAVE " #rc "=0x%x (%r)\n", __FUNCTION__, rcLog, rcLog)); \
    } while (0)
#  define VPoxLogFlowFuncMarkVar(var, varfmt) \
    DEBUG((DEBUG_INFO, "%a:" VPOX_XSTR(__LINE__) ": " #var "=" varfmt "\n", __FUNCTION__, (var)))
#  define VPoxLogFlowFuncMarkRC(rc) \
    do { \
        EFI_STATUS rcLog = (rc); \
        VPoxLog(("%a:" VPOX_XSTR(__LINE__) ": " #rc "=0x%x (%r)\n", __FUNCTION__, rcLog, rcLog)); \
    } while (0)
#  define VPoxLog(a)                            VPoxLogWorker a
# else  /* !EFI_LOG_ENABLED */
#  define VPoxLogFlowFuncEnter()                do {} while (0)
#  define VPoxLogFlowFuncLeave()                do {} while (0)
#  define VPoxLogFlowFuncLeaveRC(rc)            do {} while (0)
#  define VPoxLogFlowFuncMark()                 do {} while (0)
#  define VPoxLogFlowFuncMarkVar(var, varfmt)   do {} while (0)
#  define VPoxLogFlowFuncMarkRC(rc)             do {} while (0)
#  define VPoxLog(a)                            do {} while (0)
