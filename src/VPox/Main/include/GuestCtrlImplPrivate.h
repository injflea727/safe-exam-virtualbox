...
#include <VPox/err.h> /* VERR_GSTCTL_GUEST_ERROR */
...
protected:
        ...
        , m_fFlags(fFlags)
    {
        ...
        if (RT_FAILURE(rc))
            throw (Global::vpoxStatusCodeToCOM(rc));
    }
 ...
 * environment block (in VPoxService before launching a new process).
 */
...
public:
    ...
    enum Type
    {
        ...
        Type_Directory,
        /** Guest error is from a the built-in toolbox "vpox_cat" command. */
        Type_ToolCat,
        /** Guest error is from a the built-in toolbox "vpox_ls" command. */
        Type_ToolLs,
        /** Guest error is from a the built-in toolbox "vpox_rm" command. */
        Type_ToolRm,
        /** Guest error is from a the built-in toolbox "vpox_mkdir" command. */
        Type_ToolMkDir,
        /** Guest error is from a the built-in toolbox "vpox_mktemp" command. */
        Type_ToolMkTemp,
        /** Guest error is from a the built-in toolbox "vpox_stat" command. */
        Type_ToolStat,
...
struct GuestFsObjData
{
    ...
    /** @name Helper functions to extract the data from a certin VPoxService tool's guest stream block.
     * @{ */
...
/**
 * Class for parsing machine-readable guest process output by VPoxService'
 * toolbox commands ("vpox_ls", "vpox_stat" etc), aka "guest stream".
 */
...
public:
    ...
    /**
     * Initialization constructor. Will throw() VPox status code (rc).
     *
...
typedef std::list < VPoxEventType_T > GuestEventTypes;
...
typedef std::map < VPoxEventType_T, GuestWaitEvents > GuestEventGroup;
...
public:
    ...
    int signalWaitEvent(VPoxEventType_T aType, IEvent *aEvent);
    /** Signals a wait event using a guest rc. */
    int signalWaitEventInternal(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, int guestRc, const GuestWaitEventPayload *pPayload);
    /** Signals a wait event without letting public guest events know,
     *  extended director's cut version. */
    int signalWaitEventInternalEx(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, int rc, int guestRc, const GuestWaitEventPayload *pPayload);
...
public:
    ...
    int dispatchGeneric(PVPOXGUESTCTRLHOSTCBCTX pCtxCb, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb);
    ...
    int waitForEvent(GuestWaitEvent *pEvent, uint32_t uTimeoutMS, VPoxEventType_T *pType, IEvent **ppEvent);

#ifndef VPOX_GUESTCTRL_TEST_CASE
    HRESULT setErrorExternal(VirtualPoxBase *pInterface, const Utf8Str &strAction, const GuestErrorInfo &guestErrorInfo);
#endif
...
protected:
    ...
    virtual int i_callbackDispatcher(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb) = 0;
...
protected:
    ...
    int sendMessage(uint32_t uFunction, uint32_t cParms, PVPOXHGCMSVCPARM paParms);
...
protected:
     ...
     * and recalling seeing similar things in VirtualPox.xidl or some such place,
