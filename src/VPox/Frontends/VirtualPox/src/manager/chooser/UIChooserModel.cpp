...
/** @file
 * VPox Qt GUI - UIChooserModel class implementation.
 */
...
void UIChooserModel::setSelectedItem(const QString &strDefinition)
{
    ...
    else if (strItemType == "m")
    {
        ...
        CMachine comMachine = uiCommon().virtualPox().FindMachine(strItemDescriptor);
...
void UIChooserModel::sltMachineRegistered(const QUuid &uId, const bool fRegistered)
{
    ...
    else
    {
        ...
        if (gEDataManager->showMachineInVirtualPoxManagerChooser(uId))
        {
            ...
            CMachine comMachine = uiCommon().virtualPox().FindMachine(uId.toString());
