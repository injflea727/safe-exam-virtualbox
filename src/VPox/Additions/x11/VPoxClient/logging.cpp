...
int vbclLogNotify(const char *pszMessage)
{
    ...
    if (g_cRespawn == 0)
    {
        char *pszCommand = RTStrAPrintf2("notify-send \"VPoxClient: %s\"", pszMessage);
        if (pszCommand)
        {
            ...
            {
                pszCommand = RTStrAPrintf2("xmessage -buttons OK:0 -center \"VPoxClient: %s\"",
                                           pszMessage);
                if (pszCommand)
                {
                    ...
                    if (WEXITSTATUS(status) != 0)  /* Utility or extension not available. */
                    {
                        RTPrintf("VPoxClient: %s", pszMessage);
                    }
    ...
    switch (enmPhase)
    {
        case RTLOGPHASE_BEGIN:
        {
            pfnLog(pLoggerRelease,
                   "VPoxClient %s r%s (verbosity: %u) %s (%s %s) release log\n"
                   "Log opened %s\n",
