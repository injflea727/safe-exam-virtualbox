/* $Id: VPoxNetFltNobj.cpp $ */
/** @file
 * VPoxNetFltNobj.cpp - Notify Object for Bridged Networking Driver.
 ...
 */
#include "VPoxNetFltNobj.h"
#include <iprt/win/ntddndis.h>
#include <assert.h>
#include <stdio.h>

#include <VPoxNetFltNobjT_i.c>

#include <Olectl.h>

//# define VPOXNETFLTNOTIFY_DEBUG_BIND
...
void VPoxNetFltNobj::init(IN INetCfgComponent *pNetCfgComponent, IN INetCfg *pNetCfg, IN BOOL bInstalling)
{
    cleanup();
...
STDMETHODIMP VPoxNetFltNobj::Initialize(IN INetCfgComponent *pNetCfgComponent, IN INetCfg *pNetCfg, IN BOOL bInstalling)
{
    init(pNetCfgComponent, pNetCfg, bInstalling);
    return S_OK;
}

STDMETHODIMP VPoxNetFltNobj::ApplyRegistryChanges()
{
    return S_OK;
}

STDMETHODIMP VPoxNetFltNobj::ApplyPnpChanges(IN INetCfgPnpReconfigCallback *pCallback)
{
    RT_NOREF1(pCallback);
    return S_OK;
}

STDMETHODIMP VPoxNetFltNobj::CancelChanges()
{
    return S_OK;
}

static HRESULT vpoxNetFltWinQueryInstanceKey(IN INetCfgComponent *pComponent, OUT PHKEY phKey)
{
    LPWSTR pPnpId;
...
static HRESULT vpoxNetFltWinQueryDriverKey(IN HKEY InstanceKey, OUT PHKEY phKey)
{
    DWORD Type = REG_SZ;
...
static HRESULT vpoxNetFltWinQueryDriverKey(IN INetCfgComponent *pComponent, OUT PHKEY phKey)
{
    HKEY InstanceKey;
    HRESULT hr = vpoxNetFltWinQueryInstanceKey(pComponent, &InstanceKey);
    if (hr == S_OK)
    {
        hr = vpoxNetFltWinQueryDriverKey(InstanceKey, phKey);
        if (hr != S_OK)
        {
            NonStandardAssertBreakpoint();
        }
...
static HRESULT vpoxNetFltWinNotifyCheckNetAdp(IN INetCfgComponent *pComponent, OUT bool * pbShouldBind)
{
    HRESULT hr;
    LPWSTR pDevId;
    ...
    if (hr == S_OK)
    {
        if (!_wcsnicmp(pDevId, L"sun_VPoxNetAdp", sizeof(L"sun_VPoxNetAdp")/2))
        {
            *pbShouldBind = false;
        }
        else
...
static HRESULT vpoxNetFltWinNotifyCheckMsLoop(IN INetCfgComponent *pComponent, OUT bool * pbShouldBind)
{
    HRESULT hr;
    LPWSTR pDevId;
    hr = pComponent->GetId(&pDevId);
    if (hr == S_OK)
    {
        if (!_wcsnicmp(pDevId, L"*msloop", sizeof(L"*msloop")/2))
        {
            ...
            HKEY DriverKey;
            hr = vpoxNetFltWinQueryDriverKey(pComponent, &DriverKey);
            if (hr == S_OK)
            {
                DWORD Type = REG_SZ;
...
static HRESULT vpoxNetFltWinNotifyCheckLowerRange(IN INetCfgComponent *pComponent, OUT bool * pbShouldBind)
{
    HKEY DriverKey;
    HKEY InterfacesKey;
    HRESULT hr = vpoxNetFltWinQueryDriverKey(pComponent, &DriverKey);
    if (hr == S_OK)
    {
...
static HRESULT vpoxNetFltWinNotifyShouldBind(IN INetCfgComponent *pComponent, OUT bool *pbShouldBind)
{
    ...
    do
    {
        ...
        hr = vpoxNetFltWinNotifyCheckMsLoop(pComponent, pbShouldBind);
        if (hr == S_OK)
        {
        ...
        hr = vpoxNetFltWinNotifyCheckNetAdp(pComponent, pbShouldBind);
        if (hr == S_OK)
        {
            /* this is a VPoxNetAdp adapter,
             * the pbShouldBind already contains the result */
            break;
        }
        else if (hr != S_FALSE)
...
//        }
        ...
        hr = vpoxNetFltWinNotifyCheckLowerRange(pComponent, pbShouldBind);
        if (hr == S_OK)
        {
            /* the vpoxNetFltWinNotifyCheckLowerRange ccucceeded,
             * the pbShouldBind already contains the result */
            break;
        }
...
static HRESULT vpoxNetFltWinNotifyShouldBind(IN INetCfgBindingInterface *pIf, OUT bool *pbShouldBind)
{
    INetCfgComponent * pAdapterComponent;
    HRESULT hr = pIf->GetLowerComponent(&pAdapterComponent);
    if (hr == S_OK)
    {
        hr = vpoxNetFltWinNotifyShouldBind(pAdapterComponent, pbShouldBind);
...
static HRESULT vpoxNetFltWinNotifyShouldBind(IN INetCfgBindingPath *pPath, OUT bool *pbDoBind)
{
    ...
    if (hr == S_OK)
    {
        ...
        if (hr == S_OK)
        {
            ...
            do
            {
                ...
                if (hr == S_OK)
                {
                    hr = vpoxNetFltWinNotifyShouldBind(pBindingIf, pbDoBind);
...
static bool vpoxNetFltWinNotifyShouldBind(IN INetCfgBindingPath *pPath)
{
#ifdef VPOXNETFLTNOTIFY_DEBUG_BIND
    return VPOXNETFLTNOTIFY_DEBUG_BIND;
#else
    bool bShouldBind;
    HRESULT hr = vpoxNetFltWinNotifyShouldBind(pPath, &bShouldBind) ;
    if (hr != S_OK)
    {
        bShouldBind = VPOXNETFLTNOTIFY_ONFAIL_BINDDEFAULT;
    }
...
STDMETHODIMP VPoxNetFltNobj::NotifyBindingPath(IN DWORD dwChangeFlag, IN INetCfgBindingPath *pNetCfgBP)
{
    if (!(dwChangeFlag & NCN_ENABLE) || (dwChangeFlag & NCN_REMOVE) || vpoxNetFltWinNotifyShouldBind(pNetCfgBP))
        return S_OK;
    return NETCFG_S_DISABLE_QUERY;
}

STDMETHODIMP VPoxNetFltNobj::QueryBindingPath(IN DWORD dwChangeFlag, IN INetCfgBindingPath *pNetCfgBP)
{
    RT_NOREF1(dwChangeFlag);
    if (vpoxNetFltWinNotifyShouldBind(pNetCfgBP))
        return S_OK;
    return NETCFG_S_DISABLE_QUERY;
}
...
BEGIN_OBJECT_MAP(ObjectMap)
    OBJECT_ENTRY(CLSID_VPoxNetFltNobj, VPoxNetFltNobj)
END_OBJECT_MAP()
...
STDAPI DllRegisterServer()
{
    ...
    if (GetModuleFileNameW(GetModuleHandleW(L"VPoxNetFltNobj"), wszModule, MAX_PATH) == 0)
        return SELFREG_E_CLASS;

    try {
        AdHocRegKey keyCLSID(L"CLSID");
        AdHocRegKeyPtr pkeyNobjClass(keyCLSID.create(L"{f374d1a0-bf08-4bdc-9cb2-c15ddaeef955}",
                                                     L"VirtualPox Bridged Networking Driver Notify Object v1.1"));
    ...
    try {
        ...
        AdHocRegKeyPtr pkeyNobjLib(keyTypeLib.create(L"{2A0C94D1-40E1-439C-8FE8-24107CAB0840}\\1.1",
                                                     L"VirtualPox Bridged Networking Driver Notify Object v1.1 Type Library"));
