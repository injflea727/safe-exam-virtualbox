#!/bin/sh
# $Id: VPoxHeadlessXOrg.sh $
## @file
# VirtualPox X Server auto-start service.
#
...
CONFIGURATION_FILE=/etc/default/virtualpox
## The name of this script.
SCRIPT_NAME="$0"
## The service name.
SERVICE_NAME="vpoxheadlessxorg"
...
leave all other settings unchanged.
  ...
  HEADLESS_X_ORG_USERS
    ...
    will provide the configuration details via VirtualPox extra data.  This
  ...
  HEADLESS_X_ORG_SERVER_POST_COMMAND
    ...
    information to VirtualPox extra data for each of the users in the list
...
default_post_command()
{
  # screens=$1
  for i in ${HEADLESS_X_ORG_USERS}; do
    su ${i} -c "VPoxManage setextradata global ${EXTRA_DATA_KEY_DISPLAYS} \"${1}\""
    su ${i} -c "VPoxManage setextradata global ${EXTRA_DATA_KEY_AUTH} \"${HEADLESS_X_ORG_RUN_FOLDER}/xauth\""
  done
}
...
banner() {
  cat << EOF
${VPOX_PRODUCT} VPoxHeadless X Server start-up service Version ${VPOX_VERSION_STRING}
