...
*********************************************************************************************************************************/
typedef struct _VPOXDISPLAYCONTEXT
{
    ...
    VPOXDISPIF dispIf;
} VPOXDISPLAYCONTEXT, *PVPOXDISPLAYCONTEXT;

typedef enum
{
    VPOXDISPLAY_DRIVER_TYPE_UNKNOWN = 0,
    VPOXDISPLAY_DRIVER_TYPE_XPDM    = 1,
    VPOXDISPLAY_DRIVER_TYPE_WDDM    = 2
} VPOXDISPLAY_DRIVER_TYPE;


/*********************************************************************************************************************************
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static VPOXDISPLAYCONTEXT g_Ctx = { 0 };


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static VPOXDISPLAY_DRIVER_TYPE getVPoxDisplayDriverType(VPOXDISPLAYCONTEXT *pCtx);


static DECLCALLBACK(int) VPoxDisplayInit(const PVPOXSERVICEENV pEnv, void **ppInstance)
{
    LogFlowFuncEnter();

    PVPOXDISPLAYCONTEXT pCtx = &g_Ctx; /** @todo r=andy Use instance data via service lookup (add void *pInstance). */
    AssertPtr(pCtx);
    ...
    else if (uNtVersion >= RTSYSTEM_MAKE_NT_VERSION(5, 0, 0)) /* APIs available only on W2K and up. */
    {
        ...
        *(uintptr_t *)&pCtx->pfnEnumDisplayDevices = (uintptr_t)GetProcAddress(hUser, "EnumDisplayDevicesA");
        ...
        if (uNtVersion >= RTSYSTEM_MAKE_NT_VERSION(6, 0, 0))
        {
            ...
            VPOXDISPLAY_DRIVER_TYPE enmType = getVPoxDisplayDriverType(pCtx);
            if (enmType == VPOXDISPLAY_DRIVER_TYPE_WDDM)
            {
                LogFlowFunc(("WDDM driver is installed, switching display driver if to WDDM mode\n"));
                /* This is hacky, but the most easiest way. */
                VPOXDISPIF_MODE enmMode = uNtVersion < RTSYSTEM_MAKE_NT_VERSION(6, 1, 0)
                                        ? VPOXDISPIF_MODE_WDDM : VPOXDISPIF_MODE_WDDM_W7;
                DWORD dwErr = VPoxDispIfSwitchMode(const_cast<PVPOXDISPIF>(&pEnv->dispIf), enmMode, NULL /* old mode, we don't care about it */);
                if (dwErr == NO_ERROR)
                {
    ...
    if (RT_SUCCESS(rc))
    {
        VPOXDISPIFESCAPE_ISANYX IsAnyX = { 0 };
        IsAnyX.EscapeHdr.escapeCode = VPOXESC_ISANYX;
        DWORD err = VPoxDispIfEscapeInOut(&pEnv->dispIf, &IsAnyX.EscapeHdr, sizeof(uint32_t));
        if (err == NO_ERROR)
...
static DECLCALLBACK(void) VPoxDisplayDestroy(void *pInstance)
{
    RT_NOREF(pInstance);
    return;
}

static VPOXDISPLAY_DRIVER_TYPE getVPoxDisplayDriverType(PVPOXDISPLAYCONTEXT pCtx)
{
    VPOXDISPLAY_DRIVER_TYPE enmType = VPOXDISPLAY_DRIVER_TYPE_UNKNOWN;
    ...
    if (pCtx->pfnEnumDisplayDevices)
    {
        ...
        LogFlowFunc(("getVPoxDisplayDriverType: Checking for active VPox display driver (W2K+) ...\n"));
        ...
        while (EnumDisplayDevices(NULL,
                                  ...
                                  0))
        {
            LogFlowFunc(("getVPoxDisplayDriverType: DevNum:%d\nName:%s\nString:%s\nID:%s\nKey:%s\nFlags=%08X\n\n",
                          devNum,
            ...
            {
                LogFlowFunc(("getVPoxDisplayDriverType: Primary device\n"));

                /* WDDM driver can now have multiple incarnations,
                * if the driver name contains VirtualPox, and does NOT match the XPDM name,
                * assume it to be WDDM */
                if (strcmp(&dispDevice.DeviceString[0], "VirtualPox Graphics Adapter") == 0)
                    enmType = VPOXDISPLAY_DRIVER_TYPE_XPDM;
                else if (strstr(&dispDevice.DeviceString[0], "VirtualPox"))
                    enmType = VPOXDISPLAY_DRIVER_TYPE_WDDM;
    ...
    {
        LogFlowFunc(("getVPoxDisplayDriverType: Checking for active VPox display driver (NT or older) ...\n"));
        ...
        if (strcmp((char*)&tempDevMode.dmDeviceName[0], "VPoxDisp") == 0)
            enmType = VPOXDISPLAY_DRIVER_TYPE_XPDM;
    }
...
/** @todo r=andy The "display", "seamless" (and VPoxCaps facility in VPoxTray.cpp indirectly) is using this.
 *               Add a PVPOXDISPLAYCONTEXT here for properly getting the display (XPDM/WDDM) abstraction interfaces. */
                             ...
                             DWORD aBitsPerPixel, LONG aPosX, LONG aPosY, BOOL fEnabled, BOOL fExtDispSup)
{
    ...
    PVPOXDISPLAYCONTEXT pCtx = &g_Ctx; /* See todo above. */
...
DWORD VPoxDisplayGetCount(void)
{
    DISPLAY_DEVICE DisplayDevice;
...
DWORD VPoxDisplayGetConfig(const DWORD NumDevices, DWORD *pDevPrimaryNum, DWORD *pNumDevices,
...
static BOOL ResizeDisplayDevice(PVPOXDISPLAYCONTEXT pCtx,
                                ...
                                BOOL fExtDispSup)
{
    ...
    DWORD dmFields = 0;
    VPOXDISPLAY_DRIVER_TYPE enmDriverType = getVPoxDisplayDriverType(pCtx);
    ...
    VPoxDispIfCancelPendingResize(&pCtx->pEnv->dispIf);
    ...
    DWORD DevNum = 0;
    DWORD DevPrimaryNum = 0;
    DWORD dwStatus = VPoxDisplayGetConfig(NumDevices, &DevPrimaryNum, &DevNum, paDisplayDevices, paDeviceModes);
    if (dwStatus != NO_ERROR)
    {
        LogFlowFunc(("ResizeDisplayDevice: VPoxGetDisplayConfig failed, %d\n", dwStatus));
        return dwStatus;
    }
    ...
    for (i = 0; i < NumDevices; i++)
    {
        ...
        if (enmDriverType >= VPOXDISPLAY_DRIVER_TYPE_WDDM)
        {
    ...
    if (enmDriverType == VPOXDISPLAY_DRIVER_TYPE_WDDM)
    {
        DWORD err = VPoxDispIfResizeModes(&pCtx->pEnv->dispIf, Id, fEnabled, fExtDispSup, paDisplayDevices, paDeviceModes, DevNum);
...
static void doResize(PVPOXDISPLAYCONTEXT pCtx,
                     uint32_t iDisplay,
                     uint32_t cx,
                     uint32_t cy,
                     uint32_t cBits,
                     ...
                     bool     fChangeOrigin)
{
    ...
    {
        VPOXDISPLAY_DRIVER_TYPE enmDriverType = getVPoxDisplayDriverType(pCtx);
        if (enmDriverType == VPOXDISPLAY_DRIVER_TYPE_UNKNOWN)
        {
            LogFlowFunc(("vpoxDisplayDriver is not active\n"));
            break;
        }
...
static BOOL DisplayChangeRequestHandler(PVPOXDISPLAYCONTEXT pCtx)
{
    ...
    if (pCtx->pEnv->dispIf.enmMode >= VPOXDISPIF_MODE_WDDM_W7 &&
        RT_SUCCESS(rc = VbglR3GetDisplayChangeRequestMulti(cDisplays, &cDisplays, &aDisplays[0], true /* fAck */)))
    {
        ...
        return VPoxDispIfResizeDisplayWin7(&pCtx->pEnv->dispIf, cDisplays, &aDisplays[0]);
    }
    ...
    * this poll loop.
    *
    * Note! The interface we're using here was added in VPox 4.2.4.  As of 2017-08-16, this
 ...
 */
static DECLCALLBACK(int) VPoxDisplayWorker(void *pvInstance, bool volatile *pfShutdown)
{
    AssertPtr(pvInstance);
    PVPOXDISPLAYCONTEXT pCtx = (PVPOXDISPLAYCONTEXT)pvInstance;
    AssertPtr(pCtx->pEnv);
    ...
    PostMessage(g_hwndToolWindow, WM_VPOX_GRAPHICS_SUPPORTED, 0, 0);

    VPoxDispIfResizeStarted(&pCtx->pEnv->dispIf);
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            {
                // XPDM driver has VPoxDispDrvNotify to receive such a notifications
                if (pCtx->pEnv->dispIf.enmMode >= VPOXDISPIF_MODE_WDDM)
                {
                    VPOXDISPIFESCAPE EscapeHdr = { 0 };
                    EscapeHdr.escapeCode = VPOXESC_GUEST_DISPLAYCHANGED;

                    DWORD err = VPoxDispIfEscapeInOut(&pCtx->pEnv->dispIf, &EscapeHdr, 0);
                    LogFlowFunc(("VPoxDispIfEscapeInOut returned %d\n", err)); NOREF(err);
                }
            }
    ...
    PostMessage(g_hwndToolWindow, WM_VPOX_GRAPHICS_UNSUPPORTED, 0, 0);
 ...
 */
VPOXSERVICEDESC g_SvcDescDisplay =
{
    /* pszName. */
    "display",
    /* pszDescription. */
