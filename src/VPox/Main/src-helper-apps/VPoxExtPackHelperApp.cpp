/* $Id: VPoxExtPackHelperApp.cpp $ */
/** @file
 * VirtualPox Main - Extension Pack Helper Application, usually set-uid-to-root.
 */
...
static RTEXITCODE DoStandardOption(int ch)
{
    switch (ch)
    {
        case 'h':
        {
            RTMsgInfo(VPOX_PRODUCT " Extension Pack Helper App\n"
                      "(C) " VPOX_C_YEAR " " VPOX_VENDOR "\n"
                      "All rights reserved.\n"
                      "\n"
                      "This NOT intended for general use, please use VPoxManage instead\n"
...
static bool IsValidCertificateDir(const char *pszCertDir)
{
    ...
    if (RT_FAILURE(rc))
        return false;
    rc = RTPathAppend(szCorrect, sizeof(szCorrect), VPOX_EXTPACK_CERT_DIR);
    if (RT_FAILURE(rc))
        return false;
...
static bool IsValidBaseDir(const char *pszBaseDir)
{
    ...
    if (RT_FAILURE(rc))
        return false;
    rc = RTPathAppend(szCorrect, sizeof(szCorrect), VPOX_EXTPACK_INSTALL_DIR);
    if (RT_FAILURE(rc))
        return false;
...
static RTEXITCODE CommonUninstallWorker(const char *pszExtPackDir)
{
    ...
    if (RT_FAILURE(rc))
        return RTMsgErrorExit(RTEXITCODE_FAILURE,
                              ...
                              "If the problem persists, try running the command: VPoxManage extpack cleanup", rc);
...
/**
 * Wrapper around VPoxExtPackOpenTarFss.
 *
 * @returns success or failure, message displayed on failure.
...
{
    char szError[8192];
    int rc = VPoxExtPackOpenTarFss(hTarballFile, szError, sizeof(szError), phTarFss, NULL);
    if (RT_FAILURE(rc))
    {
        Assert(szError[0]);
...
/**
 * Wrapper around VPoxExtPackValidateMember.
 *
...
{
    char szError[8192];
    int rc = VPoxExtPackValidateMember(pszName, enmType, hVfsObj, szError, sizeof(szError));
    if (RT_FAILURE(rc))
    {
        Assert(szError[0]);
        return RTMsgErrorExit(RTEXITCODE_FAILURE, "%s", szError);
                                ...
                                const char *pszTarball)
{
    ...
    int rc = RTPathAbs(pszDirDst, szDstPath, sizeof(szDstPath) - VPOX_EXTPACK_MAX_MEMBER_NAME_LENGTH - 2);
    if (RT_FAILURE(rc))
...
/**
 * Wrapper around VPoxExtPackValidateTarball.
 *
 * @returns The program exit code.
                                         ...
                                         const char *pszTarballDigest, PRTMANIFEST phValidManifest)
{
    *phValidManifest = NIL_RTMANIFEST;
    ...
    char szError[8192];
    int rc = VPoxExtPackValidateTarball(hTarballFile, pszExtPackName, pszTarball, pszTarballDigest,
    ...
    for (;;)
    {
            ...
            && strcmp(Entry.szName, "..") != 0)
        {
            ...
            if (   pszMarker
                ...
                fCandidate = VPoxExtPackIsValidMangledName(Entry.szName, pszMarker - &Entry.szName[0]);
            if (fCandidate)
            {
                /*
 ...
 *                              may be missing when running under VPoxSVC.)
 */
                                         ...
                                         int iCmd, const char *pszDisplayInfoHack)
{
    ...
    if (RT_SUCCESS(rc))
        rc = RTPathAppend(szIconName, sizeof(szIconName), "../Resources/virtualpox.png");
    if (RT_FAILURE(rc))
    ...
    if (orc == errAuthorizationSuccess)
    {
        ...
        NOREF(iCmd);
        static char         s_szPrompt[]    = "VirtualPox needs further rights to make changes to your installation.\n\n";
...
# pragma GCC diagnostic pop
    ...
    if (fHaveDisplayVar && FindExecTool(szExecTool, sizeof(szExecTool), "kdesudo"))
    {
        ...
        papszArgs[cSuArgs - 2] = iCmd == CMD_INSTALL
                               ? "VirtualPox extension pack installer"
                               : iCmd == CMD_UNINSTALL
                               ? "VirtualPox extension pack uninstaller"
                               : "VirtualPox extension pack maintainer";
        papszArgs[cSuArgs - 1] = "--";
    }
    /*
             ...
             && FindExecTool(szXterm, sizeof(szXterm), "xterm"))
    {
        ...
        papszArgs[cSuArgs - 7] = iCmd == CMD_INSTALL
                               ? "VirtualPox extension pack installer - su"
                               : iCmd == CMD_UNINSTALL
                               ? "VirtualPox extension pack uninstaller - su"
                               : "VirtualPox extension pack maintainer - su";
        ...
        RTMsgError("Unable to locate 'pkexec', 'gksu' or 'su+xterm'. Try perform the operation using VPoxManage running as root");
    else
        RTMsgError("Unable to locate 'pkexec'. Try perform the operation using VPoxManage running as root");
    if (iSuArg != cSuArgs)
    {
...
static RTEXITCODE RelaunchElevated(int argc, char **argv, int iCmd, const char *pszDisplayInfoHack)
{
    ...
    rc = RTPathAppend(szTempDir, sizeof(szTempDir), "VPoxExtPackHelper-XXXXXX");
    if (RT_FAILURE(rc))
