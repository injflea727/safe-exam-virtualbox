...
int ShClTransferListEntryInit(PSHCLLISTENTRY pListEntry)
{
    ...
    if (pListEntry->pvInfo)
    {
        ...
        pListEntry->fInfo  = VPOX_SHCL_INFO_FLAG_FSOBJINFO;
 ...
 *
 * @returns VPox status code or VERR_NO_MORE_FILES if the end of the list has been reached.
 * @param   pTransfer           Clipboard transfer to handle.
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        if (pEntry->pvInfo)
                        {
                            ...
                            if (RT_SUCCESS(rc))
                            {
                                ...
                                pEntry->fInfo  = VPOX_SHCL_INFO_FLAG_FSOBJINFO;
                            }
                        }
                        else
                            rc = VERR_NO_MEMORY;
...
int ShClTransferCtxTransferRegister(PSHCLTRANSFERCTX pTransferCtx, PSHCLTRANSFER pTransfer, uint32_t *pidTransfer)
{
     ...
     * max-1 IDs.
     */
    uint32_t idTransfer = RTRandU32Ex(1, VPOX_SHCL_MAX_TRANSFERS - 2);

    if (!ASMBitTestAndSet(&pTransferCtx->bmTransferIds[0], idTransfer))
    { /* likely */ }
    else if (pTransferCtx->cTransfers < VPOX_SHCL_MAX_TRANSFERS - 2 /* First and last are not used */)
    {
        /* Forward search. */
        int iHit = ASMBitNextClear(&pTransferCtx->bmTransferIds[0], VPOX_SHCL_MAX_TRANSFERS, idTransfer);
        if (iHit < 0)
            iHit = ASMBitFirstClear(&pTransferCtx->bmTransferIds[0], VPOX_SHCL_MAX_TRANSFERS);
...
int ShClTransferCtxTransferRegisterByIndex(PSHCLTRANSFERCTX pTransferCtx, PSHCLTRANSFER pTransfer, uint32_t idTransfer)
{
    ...
    if (pTransferCtx->cTransfers < VPOX_SHCL_MAX_TRANSFERS - 2 /* First and last are not used */)
    {
        if (!ASMBitTestAndSet(&pTransferCtx->bmTransferIds[0], idTransfer))
        {
            RTListAppend(&pTransferCtx->List, &pTransfer->Node);
