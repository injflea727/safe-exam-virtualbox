...
/** @file
 * VPox Qt GUI - UISettingsDialogSpecific class implementation.
 */
...
#ifdef VPOX_WS_MAC
# define VPOX_GUI_WITH_TOOLBAR_SETTINGS
#endif
...
{
#ifdef VPOX_GUI_WITH_TOOLBAR_SETTINGS
...
{
    return tr("VirtualPox - %1").arg(titleExtension());
}
...
#if defined(VPOX_WITH_VIDEOHWACCEL) || defined(VPOX_WITH_3D_ACCELERATION)
...
#endif /* VPOX_WITH_VIDEOHWACCEL || VPOX_WITH_3D_ACCELERATION */
...
QString UISettingsDialogMachine::title() const
{
    ...
    const CMachine &machine = uiCommon().virtualPox().FindMachine(m_uMachineId.toString());
    if (!machine.isNull())
...
void UISettingsDialogMachine::prepare()
{
    ...
    m_machine = uiCommon().virtualPox().FindMachine(m_uMachineId.toString());
