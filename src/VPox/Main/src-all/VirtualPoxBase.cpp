/* $Id: VirtualPoxBase.cpp $ */
/** @file
 * VirtualPox COM base classes implementation
 */
...
/// @todo remove when VirtualPoxErrorInfo goes away from here
...
HRESULT VirtualPoxBase::BaseFinalConstruct()
{
    Assert(iFactoryStat == ~0U);
    if (g_pClassFactoryStatsLock)
    {
...
void VirtualPoxBase::BaseFinalRelease()
{
    if (g_pClassFactoryStatsLock)
    {
        AutoWriteLock alock(g_pClassFactoryStatsLock COMMA_LOCKVAL_SRC_POS);
 ...
 * subclasses such as VirtualPox, Host, Machine and others.
 *
 ...
 */
/* virtual */
RWLockHandle *VirtualPoxBase::lockHandle() const
{
    /* lazy initialization */
 ...
 * @code
        ...
        {
            rc = VirtualPox::handleUnexpectedExceptions(this, RT_SRC_POS);
        }
 * @endcode
 *
 ...
 *  */
/* static */
HRESULT VirtualPoxBase::handleUnexpectedExceptions(VirtualPoxBase *const aThis, RT_SRC_POS_DECL)
{
    try
    {
 ...
 */
/* static */
HRESULT VirtualPoxBase::setErrorInternal(HRESULT aResultCode,
                                         const GUID &aIID,
                                         ...
                                         LONG aResultDetail /* = 0*/)
{
    ...
    if (aText.isEmpty())
    {
        ...
        switch (aResultCode)
        {
            ...
            case VPOX_E_OBJECT_NOT_FOUND:      aText = "Object corresponding to the supplied arguments does not exist"; break;
            case VPOX_E_INVALID_VM_STATE:      aText = "Current virtual machine state prevents the operation"; break;
            case VPOX_E_VM_ERROR:              aText = "Virtual machine error occurred attempting the operation"; break;
            case VPOX_E_FILE_ERROR:            aText = "File not accessible or erroneous file contents"; break;
            case VPOX_E_IPRT_ERROR:            aText = "Runtime subsystem error"; break;
            case VPOX_E_PDM_ERROR:             aText = "Pluggable Device Manager error"; break;
            case VPOX_E_INVALID_OBJECT_STATE:  aText = "Current object state prohibits operation"; break;
            case VPOX_E_HOST_ERROR:            aText = "Host operating system related error"; break;
            case VPOX_E_NOT_SUPPORTED:         aText = "Requested operation is not supported"; break;
            case VPOX_E_XML_ERROR:             aText = "Invalid XML found"; break;
            case VPOX_E_INVALID_SESSION_STATE: aText = "Current session state prohibits operation"; break;
            case VPOX_E_OBJECT_IN_USE:         aText = "Object being in use prohibits operation"; break;
            case VPOX_E_PASSWORD_INCORRECT:    aText = "Incorrect password provided"; break;
    ...
    do
    {
        ComObjPtr<VirtualPoxErrorInfo> info;
        rc = info.createObject();
        if (FAILED(rc)) break;

#if !defined(VPOX_WITH_XPCOM)

        ComPtr<IVirtualPoxErrorInfo> curInfo;
        if (preserve)
        {
            ...
            if (FAILED(rc))
            {
                /* create a IVirtualPoxErrorInfo wrapper for the native
                 * IErrorInfo object */
                ComObjPtr<VirtualPoxErrorInfo> wrapper;
 ...
 * defined by the VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT macro.
 * @param   aResultCode
 * @return
 */
HRESULT VirtualPoxBase::setError(HRESULT aResultCode)
{
 ...
 * @return
 */
HRESULT VirtualPoxBase::setError(HRESULT aResultCode, const char *pcsz, ...)
{
    va_list args;
    va_start(args, pcsz);
 ...
 * @return
 */
HRESULT VirtualPoxBase::setError(const com::ErrorInfo &ei)
{
    /* whether multi-error mode is turned on */
    bool preserve = MultiResult::isMultiEnabled();
...
/**
 * Converts the VPox status code a COM one and sets the error info.
 *
 * The VPox status code is made available to the API user via
 * IVirtualPoxErrorInfo::resultDetail attribute.
 *
 * @param   vrc             The VPox status code.
 * @return  COM status code appropriate for @a vrc.
 *
 * @sa      VirtualPoxBase::setError(HRESULT)
 */
HRESULT VirtualPoxBase::setErrorVrc(int vrc)
{
    return setErrorInternal(Global::vpoxStatusCodeToCOM(vrc),
                            this->getClassIID(),
                            this->getComponentName(),
                            Utf8StrFmt("%Rrc", vrc),
                            false /* aWarning */,
 ...
 * @return  COM status code appropriate for @a vrc.
 *
 * @sa      VirtualPoxBase::setError(HRESULT, const char *, ...)
 */
HRESULT VirtualPoxBase::setErrorVrc(int vrc, const char *pcszMsgFmt, ...)
{
    va_list va;
    va_start(va, pcszMsgFmt);
    HRESULT hrc = setErrorInternal(Global::vpoxStatusCodeToCOM(vrc),
                                   this->getClassIID(),
                                   this->getComponentName(),
...
/**
 * Sets error info with both a COM status and an VPox status code.
 *
 * The VPox status code is made available to the API user via
 * IVirtualPoxErrorInfo::resultDetail attribute.
 *
 ...
 * @sa      VirtualPoxBase::setError(HRESULT)
 */
HRESULT VirtualPoxBase::setErrorBoth(HRESULT hrc, int vrc)
{
...
/**
 * Sets error info with a message and both a COM status and an VPox status code.
 *
 * The VPox status code is made available to the API user via
 * IVirtualPoxErrorInfo::resultDetail attribute.
 *
 ...
 * @sa      VirtualPoxBase::setError(HRESULT, const char *, ...)
 */
HRESULT VirtualPoxBase::setErrorBoth(HRESULT hrc, int vrc, const char *pcszMsgFmt, ...)
{
    va_list va;
    va_start(va, pcszMsgFmt);
 ...
 * @return
 */
HRESULT VirtualPoxBase::setWarning(HRESULT aResultCode, const char *pcsz, ...)
{
    va_list args;
    va_start(args, pcsz);
    HRESULT rc = setErrorInternal(aResultCode,
 ...
 * @return
 */
HRESULT VirtualPoxBase::setErrorNoLog(HRESULT aResultCode, const char *pcsz, ...)
{
    va_list args;
    va_start(args, pcsz);
    HRESULT rc = setErrorInternal(aResultCode,
 ...
 */
/*static*/
void VirtualPoxBase::clearError(void)
{
#if !defined(VPOX_WITH_XPCOM)
    ::SetErrorInfo(0, NULL);
#else
