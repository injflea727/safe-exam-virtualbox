/* $Id: VPoxNetLwfUninstall.cpp $ */
/** @file
 * NetLwfUninstall - VPoxNetLwf uninstaller command line tool
 */
...
#define VPOX_NETCFG_APP_NAME L"NetLwfUninstall"
...
static int VPoxNetLwfUninstall()
{
    INetCfg *pnc;
    ...
    if (hr == S_OK)
    {
        ...
        do
        {
            ...
            if (hr == S_OK)
            {
                hr = VPoxNetCfgWinNetLwfUninstall(pnc);
                if (hr != S_OK)
                {
                    wprintf(L"error uninstalling VPoxNetLwf (0x%x)\n", hr);
                    r = 1;
                }
                else
                {
