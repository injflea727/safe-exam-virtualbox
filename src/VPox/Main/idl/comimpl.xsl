...
<!--
    ...
    comimpl.xsl:
        ...
        interfaces described in VirtualPox.xidl.
 ...
 * Generator: src/VPox/Main/idl/comimpl.xsl
 */
  ...
  <xsl:for-each select="key('G_keyInterfacesByName', $name)/attribute[@name != 'midlDoesNotLikeEmptyInterfaces']">
    ...
    <xsl:choose>
      ...
      <xsl:when test="substring($aType, string-length($aType) - 1) = '_T'"> <!-- To avoid pedantic gcc warnings/errors. -->
        <xsl:value-of select="       '#ifdef VPOX_WITH_XPCOM_CPP_ENUM_HACK&#10;'"/>
...
<xsl:template name="genImplList">
  ...
  <xsl:choose>
    <xsl:when test="$name='IEvent'">
      <xsl:value-of select="       '#ifdef VPOX_WITH_XPCOM&#10;'" />
...
<xsl:template name="genEventImpl">
  ...
  <xsl:value-of select="concat('class ATL_NO_VTABLE ',$implName,
                        '&#10;    : public VirtualPoxBase,&#10;      VPOX_SCRIPTABLE_IMPL(',
                        @name, ')&#10;{&#10;')" />
  <xsl:value-of select="'public:&#10;'" />
  <xsl:value-of select="concat('    VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT(', $implName, ', ', @name, ')&#10;')" />
  ...
  <xsl:value-of select="concat('        VPOX_TWEAK_INTERFACE_ENTRY(', @name, ')&#10;')" />
  ...
  <xsl:text><![CDATA[
    ...
    STDMETHOD(COMGETTER(Type))(VPoxEventType_T *aType)
    {
...
<xsl:text><![CDATA[
    HRESULT init(IEventSource* aSource, VPoxEventType_T aType, BOOL aWaitable = TRUE)
    {
        NOREF(aWaitable);
...
private:
    ComObjPtr<VPoxVetoEvent>      mEvent;
]]></xsl:text>
    </xsl:when>
    <xsl:when test="$isReusable='yes'">
      <xsl:text>
<![CDATA[
    HRESULT init(IEventSource* aSource, VPoxEventType_T aType, BOOL aWaitable = FALSE)
    {
        mGeneration = 1;
...
<xsl:template name="genSwitchCase">
  ...
  <xsl:value-of select="concat('         case VPoxEventType_', @id, ':&#10;')"/>
  <xsl:value-of select="       '         {&#10;'"/>
  <xsl:choose>
...
<xsl:template name="genCommonEventCode">
  <xsl:call-template name="fileheader">
    <xsl:with-param name="name" select="'VPoxEvents.cpp'" />
  </xsl:call-template>
...
#include "EventImpl.h"
  ...
  <xsl:for-each select="//interface[@autogen=$G_kind]">
    ...
    <xsl:choose>
      <xsl:when test="$G_kind='VPoxEvent'">
  ...
  <xsl:text><![CDATA[
HRESULT VPoxEventDesc::init(IEventSource *aSource, VPoxEventType_T aType, ...)
{
    va_list args;
 ...
 <xsl:text><![CDATA[
HRESULT VPoxEventDesc::reinit(VPoxEventType_T aType, ...)
{
    va_list args;
...
<xsl:template name="genCommonEventHeader">
  <xsl:call-template name="fileheader">
    <xsl:with-param name="name" select="'VPoxEvents.h'" />
  </xsl:call-template>
...
#include "EventImpl.h"
  ...
  <!-- Interfaces -->
  <xsl:for-each select="//interface[@autogen='VPoxEvent']">
    ...
    <xsl:value-of select="       '    VPoxEventDesc evDesc;&#10;'"/>
    <xsl:value-of select="concat('    evDesc.init(aSource, VPoxEventType_',$evid)"/>
    <xsl:call-template name="genFactParams">
      <xsl:with-param name="name" select="$ifname" />
    </xsl:call-template>
    <xsl:value-of select="');&#10;'"/>
...
<xsl:template match="/">
  <!-- Global code -->
   <xsl:choose>
      ...
      <xsl:when test="$G_kind='VPoxEventHeader'">
