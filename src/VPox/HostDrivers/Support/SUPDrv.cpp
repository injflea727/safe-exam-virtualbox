/* $Id: SUPDrv.cpp $ */
/** @file
 * VPoxDrv - The VirtualPox Support Driver - Common code.
 */
...
#else
# define VPOXDRV_SESSION_CREATE(pvSession, fUser) do { } while (0)
# define VPOXDRV_SESSION_CLOSE(pvSession) do { } while (0)
# define VPOXDRV_IOCTL_ENTRY(pvSession, uIOCtl, pvReqHdr) do { } while (0)
# define VPOXDRV_IOCTL_RETURN(pvSession, uIOCtl, pvReqHdr, rcRet, rcReq) do { } while (0)
#endif
 ...
 * instantiation in log-vpox.c(pp).
 */
...
#if (defined(RT_OS_DARWIN) || defined(RT_OS_LINUX)) && !defined(VPOX_WITHOUT_EFLAGS_AC_SET_IN_VPOXDRV)
...
# endif
    ...
    SUPEXP_STK_BACK(    0,  RTTimeMilliTS),
    SUPEXP_STK_BACK(    0,  RTTimeNanoTS),
...
int VPOXCALL supdrvInitDevExt(PSUPDRVDEVEXT pDevExt, size_t cbSession)
{
    ...
    rc = RTLogCreate(&pRelLogger, 0 /* fFlags */, "all",
                     "VPOX_RELEASE_LOG", RT_ELEMENTS(s_apszGroups), s_apszGroups, RTLOGDEST_STDOUT | RTLOGDEST_DEBUGGER, NULL);
    if (RT_SUCCESS(rc))
    ...
    {
        SUPR0Printf("vpoxdrv: Requires SSE2 (cpuid(0).EDX=%#x)\n", ASMCpuId_EDX(1));
    ...
    if (pSession)
    {
        ...
        if (!rc)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                VPOXDRV_SESSION_CREATE(pSession, fUser);
...
{
    VPOXDRV_SESSION_CLOSE(pSession);
 ...
 */
int VPOXCALL supdrvSessionHashTabInsert(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPDRVSESSION *ppOsSessionPtr,
                                        void *pvUser)
{
    PSUPDRVSESSION  pCur;
    unsigned        iHash;
 ...
 *
 * @returns VPox status code that should be passed down to ring-3 unchanged.
...
{
    int rc;
    VPOXDRV_IOCTL_ENTRY(pSession, uIOCtl, pReqHdr);
    ...
    {
        OSDBGPRINT(("vpoxdrv: Bad ioctl request size; cbReq=%#lx\n", (long)cbReq));
        VPOXDRV_IOCTL_RETURN(pSession, uIOCtl, pReqHdr, VERR_INVALID_PARAMETER, VINF_SUCCESS);
        return VERR_INVALID_PARAMETER;
    }
    if (RT_UNLIKELY(   (pReqHdr->fFlags & SUPREQHDR_FLAGS_MAGIC_MASK) != SUPREQHDR_FLAGS_MAGIC
                    ...
                    || pReqHdr->cbOut > cbReq))
    {
        OSDBGPRINT(("vpoxdrv: Bad ioctl request header; cbIn=%#lx cbOut=%#lx fFlags=%#lx\n",
    ...
    {
        OSDBGPRINT(("vpoxdrv: Invalid pSession value %p (ioctl=%p)\n", pSession, (void *)uIOCtl));
        VPOXDRV_IOCTL_RETURN(pSession, uIOCtl, pReqHdr, VERR_INVALID_PARAMETER, VINF_SUCCESS);
        return VERR_INVALID_PARAMETER;
    }
    ...
    {
        OSDBGPRINT(("vpoxdrv: bad cookie %#lx / %#lx.\n", (long)pReqHdr->u32Cookie, (long)pReqHdr->u32SessionCookie));
        VPOXDRV_IOCTL_RETURN(pSession, uIOCtl, pReqHdr, VERR_INVALID_PARAMETER, VINF_SUCCESS);
        return VERR_INVALID_PARAMETER;
    }
    ...
    VPOXDRV_IOCTL_RETURN(pSession, uIOCtl, pReqHdr, pReqHdr->rc, rc);
    return rc;
}
 ...
 * @param   pReqHdr     The request header.
 */
int VPOXCALL supdrvIDC(uintptr_t uReq, PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPDRVIDCREQHDR pReqHdr)
{
    /*
...
SUPR0DECL(int) SUPR0PageAllocEx(PSUPDRVSESSION pSession, uint32_t cPages, uint32_t fFlags, PRTR3PTR ppvR3, PRTR0PTR ppvR0, PRTHCPHYS paPages)
{
    ...
    if (cPages < 1 || cPages > VPOX_MAX_ALLOC_PAGE_COUNT)
    {
        Log(("SUPR0PageAlloc: Illegal request cb=%u; must be greater than 0 and smaller than %uMB (VPOX_MAX_ALLOC_PAGE_COUNT pages).\n", cPages, VPOX_MAX_ALLOC_PAGE_COUNT * (_1M / _4K)));
        return VERR_PAGE_COUNT_OUT_OF_RANGE;
    }
...
void VPOXCALL supdrvBadContext(PSUPDRVDEVEXT pDevExt, const char *pszFile, uint32_t uLine, const char *pszExtra)
{
    ...
    if (RT_VALID_PTR(pszExtra) && *pszExtra)
        SUPR0Printf("vpoxdrv: Bad CPU context error at line %u in %s: %s\n", uLine, pszFile, pszExtra);
    else
        SUPR0Printf("vpoxdrv: Bad CPU context error at line %u in %s!\n", uLine, pszFile);
    ...
    *pfCaps = 0;
    ...
    if (ASMHasCpuId())
    {
        ...
        if (ASMIsValidStdRange(uMaxLeaf))
        {
            ...
            {
                /* Check VT-x support. In addition, VirtualPox requires MSR and FXSAVE/FXRSTOR to function. */
                ...
                || ASMIsHygonCpuEx(uVendorEbx, uVendorEcx, uVendorEdx))
            {
                ...
                /* Check AMD-V support. In addition, VirtualPox requires MSR and FXSAVE/FXRSTOR to function. */
 ...
 */
static int VPOXCALL supdrvQueryUcodeRev(uint32_t *puRevision)
{
    int  rc = VERR_UNSUPPORTED_CPU;
    RTTHREADPREEMPTSTATE PreemptState = RTTHREADPREEMPTSTATE_INITIALIZER;
...
SUPR0DECL(bool) SUPR0LdrIsLockOwnerByMod(void *hMod, bool fWantToHear)
{
     ...
     * introspection, so we wing it for now.  This API is VPOX_STRICT only.
     */
#ifdef SUPDRV_USE_MUTEX_FOR_LDR
