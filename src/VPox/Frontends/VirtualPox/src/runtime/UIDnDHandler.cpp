/* $Id: UIDnDHandler.cpp $ */
/** @file
 * VPox Qt GUI - UIDnDHandler class implementation.
 */
...
#  define DEBUG_DND_QT
#  ifdef RT_OS_WINDOWS
#   define DEBUG_DND_QT_LOGFILE "c:\\temp\\VPoxQt.log"
#  else
#   define DEBUG_DND_QT_LOGFILE "/var/tmp/vpoxqt.log"
#  endif
# endif /* DEBUG */
#endif
                                       ...
                                       const QMimeData *pMimeData)
{
    LogFlowFunc(("enmOpMode=%RU32, screenID=%RU32, x=%d, y=%d, action=%ld\n",
                 m_enmOpMode, screenID, x, y, toVPoxDnDAction(proposedAction)));
    ...
    KDnDAction result = m_dndTarget.Enter(screenID,
                                          x,
                                          y,
                                          toVPoxDnDAction(proposedAction),
                                          toVPoxDnDActions(possibleActions),
...
#    else
        /* Without having VPOX_WITH_DRAG_AND_DROP_PROMISES enabled drag and drop
         * will not work on OS X! It also requires some handcrafted patches within Qt
         * (which also needs VPOX_WITH_DRAG_AND_DROP_PROMISES set there). */
...
#  else /* !RT_OS_DARWIN */
    dropAction = pDrag->exec(actions, defAction);
#  endif /* RT_OS_DARWIN */
    LogRel3(("DnD: Ended with dropAction=%ld\n", UIDnDHandler::toVPoxDnDAction(dropAction)));
...
#else /* !VPOX_WITH_DRAG_AND_DROP_GH */
 ...
 */
/* static */
KDnDAction UIDnDHandler::toVPoxDnDAction(Qt::DropAction action)
{
 ...
 */
/* static */
QVector<KDnDAction> UIDnDHandler::toVPoxDnDActions(Qt::DropActions actions)
{
