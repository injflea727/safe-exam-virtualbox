/** $Id: VPoxClientClipboardHostToGuest.cpp $ */
/** @file
 * VPoxClient - Shared Clipboard Host -> Guest copying, Darwin.
 */
...
static int vbclClipboardGuestPasteData(PasteboardRef pPasteboard, UInt8 *pData, CFIndex cbDataSize, CFStringRef sFormat, bool fClear)
{
    ...
    if ((textData = CFDataCreate(kCFAllocatorDefault, pData, cbDataSize)))
    {
        ...
        if (rc != noErr)
        {
            VPoxClientVerbose(3, "unable to put data into guest's clipboard: %d\n", rc);
...
int vbclClipboardForwardToGuest(uint32_t u32ClientId, PasteboardRef pPasteboard, uint32_t fFormats)
{
    ...
    {
        if (fFormatsInternal & VPOX_SHCL_FMT_UNICODETEXT)
        {
            VPoxClientVerbose(3, "found VPOX_SHCL_FMT_UNICODETEXT: %d\n", fFormatsInternal);

            rc = vbclClipboardReadHostData(u32ClientId, VPOX_SHCL_FMT_UNICODETEXT, &pData, &cbDataSize, &cbMemSize);
            if (RT_SUCCESS(rc))
            {
            ...
            fFormatsInternal &= ~((uint32_t)VPOX_SHCL_FMT_UNICODETEXT);
        }

        else if (fFormatsInternal & VPOX_SHCL_FMT_BITMAP)
        {
            VPoxClientVerbose(3, "found VPOX_SHCL_FMT_BITMAP: %d\n", fFormatsInternal);

            rc = vbclClipboardReadHostData(u32ClientId, VPOX_SHCL_FMT_BITMAP, &pData, &cbDataSize, &cbMemSize);
            if (RT_SUCCESS(rc))
            {
                /* Store data in guest buffer */
            ...
            fFormatsInternal &= ~((uint32_t)VPOX_SHCL_FMT_BITMAP);
        }

        else if (fFormatsInternal & VPOX_SHCL_FMT_HTML)
        {
            VPoxClientVerbose(3, "found VPOX_SHCL_FMT_HTML: %d\n", fFormatsInternal);

            rc = vbclClipboardReadHostData(u32ClientId, VPOX_SHCL_FMT_HTML, &pData, &cbDataSize, &cbMemSize);
            if (RT_SUCCESS(rc))
            {
                /* Store data in guest buffer */
            ...
            fFormatsInternal &= ~((uint32_t)VPOX_SHCL_FMT_HTML);
        }

        else
        {
            VPoxClientVerbose(3, "received data in unsupported format: %d\n", fFormats);
            break;
        }
    }
