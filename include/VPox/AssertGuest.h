/** @file
 * VirtualPox - Guest input assertions.
 */
...
#ifndef VPOX_INCLUDED_AssertGuest_h
#define VPOX_INCLUDED_AssertGuest_h
...
/** @defgroup grp_vpox_assert_guest VPox Guest Input Assertion Macros
 * @{
 */
 ...
 *
 * These assertions will only trigger when VPOX_STRICT_GUEST is defined.  When
 ...
 * If VPOX_STRICT_GUEST is defined this macro will invoke RTAssertDoPanic if
 * RTAssertShouldPanic returns true. If VPOX_STRICT_GUEST isn't defined it won't
 * do any thing.
 */
#if defined(VPOX_STRICT_GUEST) && !defined(VPOX_STRICT_GUEST_DONT_PANIC)
 ...
 * In VPOX_STRICT_GUEST mode it will hit a breakpoint before returning.
 *
 * @param   a_Expr  Expression which should be true.
 ...
 * and return rc.
 *
 * In VPOX_STRICT_GUEST mode it will hit a breakpoint before executing the statement and
 * returning.
 *
 * @param   a_Expr  Expression which should be true.
 ...
 * In VPOX_STRICT_GUEST mode it will hit a breakpoint before breaking.
 *
 * @param   a_Expr  Expression which should be true.
 */
#ifdef VPOX_STRICT_GUEST
 ...
 * In VPOX_STRICT_GUEST mode it will hit a breakpoint before continuing.
 *
 * @param   a_Expr  Expression which should be true.
 */
#ifdef VPOX_STRICT_GUEST
 ...
 * Assert that an expression is true and breaks if it isn't.
 * In VPOX_STRICT_GUEST mode it will hit a breakpoint before doing break.
 *
 * @param   a_Expr  Expression which should be true.
 ...
 * An assertion failed, hit breakpoint (VPOX_STRICT_GUEST mode only) and return.
 *
 ...
 * An assertion failed, hit breakpoint (VPOX_STRICT_GUEST mode only), execute a
 ...
 * An assertion failed, hit breakpoint (VPOX_STRICT_GUEST mode only) and break.
 */
#ifdef VPOX_STRICT_GUEST
 ...
 * An assertion failed, hit breakpoint (VPOX_STRICT_GUEST mode only), execute
 ...
 * An assertion failed, hit breakpoint with message (VPOX_STRICT_GUEST mode only) and return.
 *
 * @param   a       printf argument list (in parenthesis).
 * @param   a_rc    What is to be presented to return.
 */
 ...
 * An assertion failed, hit breakpoint with message (VPOX_STRICT_GUEST mode only) and break.
 *
 * @param   a       printf argument list (in parenthesis).
 */
#ifdef VPOX_STRICT_GUEST
 ...
 *
 * These assertions will work like normal strict assertion when VPOX_STRICT_GUEST is
 * defined and LogRel statements when VPOX_STRICT_GUEST is undefined.  Typically
 * used for important guest input that it would be helpful to find in VPox.log
...
#endif /* !VPOX_INCLUDED_AssertGuest_h */
