/* $Id: vpoximg-mount.cpp $ */
/** @file
 * vpoximg-mount - Disk Image Flattening FUSE Program.
 */
...
#include "fuse.h"
#include "vpoximgCrypto.h"
#include "vpoximgMedia.h"
#include "SelfSizingTable.h"
#include "vpoximgOpts.h"
...
#define VPOX_EXTPACK                "Oracle VM VirtualPox Extension Pack"
#define VERBOSE                     g_vpoximgOpts.fVerbose

#define SAFENULL(strPtr)   (strPtr ? strPtr : "")
#define CSTR(arg)     Utf8Str(arg).c_str()          /* Converts XPCOM string type to C string type */

static struct fuse_operations g_vpoximgOps;         /** FUSE structure that defines allowed ops for this FS */

/**
 * Volume data.
 */
typedef struct VPOXIMGMOUNTVOL
{
    /** The volume handle. */
    RTDVMVOLUME                 hVol;
...
typedef VPOXIMGMOUNTVOL *PVPOXIMGMOUNTVOL;
...
static PVPOXIMGMOUNTVOL      g_paVolumes;
/** Number of detected volumes. */
static uint32_t              g_cVolumes;

VPOXIMGOPTS g_vpoximgOpts;

#define OPTION(fmt, pos, val) { fmt, offsetof(struct vpoximgOpts, pos), val }

static struct fuse_opt vpoximgOptDefs[] = {
...
static DECLCALLBACK(int) vpoximgThreadStartRead(void *pvUser)
{
    PRTCRITSECT vdioLock = (PRTCRITSECT)pvUser;
    return RTCritSectEnter(vdioLock);
}

static DECLCALLBACK(int) vpoximgThreadFinishRead(void *pvUser)
{
    PRTCRITSECT vdioLock = (PRTCRITSECT)pvUser;
    return RTCritSectLeave(vdioLock);
}

static DECLCALLBACK(int) vpoximgThreadStartWrite(void *pvUser)
{
    PRTCRITSECT vdioLock = (PRTCRITSECT)pvUser;
    return RTCritSectEnter(vdioLock);
}

static DECLCALLBACK(int) vpoximgThreadFinishWrite(void *pvUser)
{
    PRTCRITSECT vdioLock = (PRTCRITSECT)pvUser;
    return RTCritSectLeave(vdioLock);
}
...
briefUsage()
{
    RTPrintf("usage: vpoximg-mount [options] <mount point directory path>\n\n"
        "vpoximg-mount options:\n\n"
        "  [ { -i | --image } <specifier> ]   VirtualPox disk base image or snapshot,\n"
    ...
    RTPrintf("\n"
      "vpoximg-mount is a utility to make VirtualPox disk images available to the host\n"
...
static int
vpoximgOptHandler(void *data, const char *arg, int optKey, struct fuse_args *outargs)
{
    NOREF(data);
    NOREF(arg);
    NOREF(optKey);
 ...
 */
static int vpoxImgMntVfsObjQueryFromPath(const char *pszPath, PRTVFSOBJ phVfsObj)
{
...
static int vpoximgOp_open(const char *pszPath, struct fuse_file_info *pInfo)
{
    LogFlowFunc(("pszPath=%s\n", pszPath));
    int rc = 0;

    RTVFSOBJ hVfsObj;
    int rcIprt = vpoxImgMntVfsObjQueryFromPath(pszPath, &hVfsObj);
    if (RT_SUCCESS(rc))
    {
        uint32_t fNotSup = 0;
...
static int vpoximgOp_release(const char *pszPath, struct fuse_file_info *pInfo)
{
    NOREF(pszPath);
...
static int vpoximgOp_read(const char *pszPath, char *pbBuf, size_t cbBuf,
...
static int vpoximgOp_write(const char *pszPath, const char *pbBuf, size_t cbBuf,
                           off_t offset, struct fuse_file_info *pInfo)
{
    NOREF(pszPath);
    NOREF(pInfo);
    ...
    if (!g_vpoximgOpts.fRW)
    {
        LogFlowFunc(("WARNING: vpoximg-mount (FUSE FS) --rw option not specified\n"
...
static int
vpoximgOp_getattr(const char *pszPath, struct stat *stbuf)
{
    int rc = 0;
    ...
    else if (RTStrNCmp(pszPath + 1, g_pszImageName, strlen(g_pszImageName)) == 0)
    {
         ...
         * resolved path to VPox disk image, has appended to it formatted text
         * representing the offset range of the partition.
         *
         *  $ vpoximg-mount -i /stroll/along/the/path/simple_fixed_disk.vdi -p 1 /mnt/tmpdir
         *  $ ls /mnt/tmpdir
...
static int
vpoximgOp_readdir(const char *pszPath, void *pvBuf, fuse_fill_dir_t pfnFiller,
                              off_t offset, struct fuse_file_info *pInfo)

{
    ...
    if (!RTStrCmp(pszPath, "/"))
    {
         ...
         * a symbolic link back to the resolved path to the VPox virtual disk image,
         ...
         * to access the VirtualPox image as processed by the IRPT VD API.
         */
...
static int
vpoximgOp_readlink(const char *pszPath, char *buf, size_t size)
{
    NOREF(pszPath);
 ...
 * @returns nothing.
 */
static void vpoxImgMntVolumesDisplay(void)
{
    /*
    ...
    for (uint32_t i = 0; i < g_cVolumes; i++)
    {
        PVPOXIMGMOUNTVOL pVol = &g_paVolumes[i];
        ...
        tbl.setCell(row, colSize,       vpoximgScaledSize(cbVol));
 ...
 * @returns IPRT status code.
 */
static int vpoxImgMntVolumesSetup(void)
{
    g_cVolumes = 0;
    g_paVolumes = NULL;
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (   g_cVolumes != UINT32_MAX
                && g_cVolumes > 0)
            {
                g_paVolumes = (PVPOXIMGMOUNTVOL)RTMemAllocZ(g_cVolumes * sizeof(VPOXIMGMOUNTVOL));
...
main(int argc, char **argv)
{
    ...
    memset(&g_vpoximgOps, 0, sizeof(g_vpoximgOps));
    g_vpoximgOps.open        = vpoximgOp_open;
    g_vpoximgOps.read        = vpoximgOp_read;
    g_vpoximgOps.write       = vpoximgOp_write;
    g_vpoximgOps.getattr     = vpoximgOp_getattr;
    g_vpoximgOps.release     = vpoximgOp_release;
    g_vpoximgOps.readdir     = vpoximgOp_readdir;
    g_vpoximgOps.readlink    = vpoximgOp_readlink;

    struct fuse_args args = FUSE_ARGS_INIT(argc, argv);
    memset(&g_vpoximgOpts, 0, sizeof(g_vpoximgOpts));

    rc = fuse_opt_parse(&args, &g_vpoximgOpts, vpoximgOptDefs, vpoximgOptHandler);
    if (rc < 0 || argc < 2 || RTStrCmp(argv[1], "-?" ) == 0 || g_vpoximgOpts.fBriefUsage)
    {
        briefUsage();
        return 0;
    }

    if (g_vpoximgOpts.fAllowRoot)
     ...
     */
    if (g_vpoximgOpts.fWideList)
    {
        g_vpoximgOpts.fWide = true;
        g_vpoximgOpts.fList = true;
    }
    if (g_vpoximgOpts.fVerboseList)
    {
        g_vpoximgOpts.fVerbose = true;
        g_vpoximgOpts.fList    = true;
    }
    if (g_vpoximgOpts.fAllowRoot)
        fuse_opt_add_arg(&args, "-oallow_root");
...
# endif
    ...
    /*
     * Get the remote VirtualPox object and create a local session object.
     */
    ComPtr<IVirtualPoxClient> pVirtualPoxClient;
    ComPtr<IVirtualPox> pVirtualPox;

    hrc = pVirtualPoxClient.createInprocObject(CLSID_VirtualPoxClient);
    if (SUCCEEDED(hrc))
        hrc = pVirtualPoxClient->COMGETTER(VirtualPox)(pVirtualPox.asOutParam());

    if (FAILED(hrc))
        return RTMsgErrorExit(RTEXITCODE_FAILURE, "Failed to get IVirtualPox object! (hrc=%Rhrc)", hrc);

    if (g_vpoximgOpts.fList && g_vpoximgOpts.pszImageUuidOrPath == NULL)
    {
        vpoximgListVMs(pVirtualPox);
        return VINF_SUCCESS;
    }

    if (!g_vpoximgOpts.pszImageUuidOrPath)
    ...
    if (g_vpoximgOpts.pszImageUuidOrPath)
    {
        ...
        if (g_vpoximgOpts.fRW)
            CHECK_ERROR(pVirtualPox, OpenMedium(Bstr(g_vpoximgOpts.pszImageUuidOrPath).raw(), DeviceType_HardDisk,
        ...
        if (FAILED(rc))
            return RTMsgErrorExitFailure("\nCould't find specified VirtualPox base or snapshot disk image:\n%s",
                 g_vpoximgOpts.pszImageUuidOrPath);
    ...
    {
        RTPrintf("\nvpoximgMount: Encrypted disks not supported in this version\n\n");
        return -1;
    }
...
/* vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv */
       ...
       * What is here is an attempt to port the VPoxSVC specific code in i_openForIO to
       * a client's proximity. It is supplemented by code in vpoximgCrypto.cpp and
       * vpoximageCrypt.h that was lifed from SecretKeyStore.cpp along with the setup
       * task function.
       *
    ...
    if (SUCCEEDED(rc))
    {
        ...
        CHECK_ERROR(pVirtualPox, COMGETTER(ExtensionPackManager)(pExtPackManager.asOutParam()));
        BOOL fExtPackUsable;
        CHECK_ERROR(pExtPackManager, IsExtPackUsable((PRUnichar *)VPOX_EXTPACK, &fExtPackUsable));
        if (fExtPackUsable)
        {
            /* Load the PlugIn */

            CHECK_ERROR(pExtPackManager, Find((PRUnichar *)VPOX_EXTPACK, pExtPack.asOutParam()));
            if (RT_FAILURE(rc))
                return RTMsgErrorExitFailure(
            ...
            if (iPlugIn == pExtPackPlugIns.size())
                return RTMsgErrorExitFailure("Encryption is not supported because the extension pack '%s' "
                    "is missing the encryption PlugIn (old extension pack installed?)", VPOX_EXTPACK);
        ...
        vpoxImageCryptoSetup(pVDiskCryptoSettings, NULL,
...
/* **************** END IFDEF'D (STUBBED-OUT) CODE ***************** */
    ...
    if (RT_SUCCESS(rc))
    {
        g_VDIfThreadSync.pfnStartRead   = vpoximgThreadStartRead;
        g_VDIfThreadSync.pfnFinishRead  = vpoximgThreadFinishRead;
        g_VDIfThreadSync.pfnStartWrite  = vpoximgThreadStartWrite;
        g_VDIfThreadSync.pfnFinishWrite = vpoximgThreadFinishWrite;
        rc = VDInterfaceAdd(&g_VDIfThreadSync.Core, "vpoximg_ThreadSync", VDINTERFACETYPE_THREADSYNC,
   ...
   /*
    ...
    if (g_vpoximgOpts.fGstFs)
    {
        for (uint32_t i = 0; i < g_cVolumes; i++)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_FAILURE(rc))
                {
                    RTPrintf("\nvpoximg-mount: Failed to access filesystem on volume %u, ignoring\n", i);
     ...
     */
    if (VERBOSE)
        RTPrintf("\nvpoximg-mount: Going into background...\n");

    rc = fuse_main_real(args.argc, args.argv, &g_vpoximgOps, sizeof(g_vpoximgOps), NULL);

    int rc2 = RTVfsFileRelease(g_hVfsFileDisk);
    AssertRC(rc2);
    RTPrintf("vpoximg-mount: fuse_main -> %d\n", rc);
    return rc;
}
