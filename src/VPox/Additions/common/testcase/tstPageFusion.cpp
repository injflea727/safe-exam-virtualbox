...
/** @file
 * VPoxService - Guest page sharing testcase
 */
 ...
 */
void VPoxServicePageSharingCheckModule(MODULEENTRY32 *pModule)
{
 ...
 */
void VPoxServicePageSharingInspectModules(DWORD dwProcessId)
{
    HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, dwProcessId);
    if (hSnapshot == INVALID_HANDLE_VALUE)
    {
        printf("VPoxServicePageSharingInspectModules: CreateToolhelp32Snapshot failed with %d\n", GetLastError());
        return;
    }

    printf("VPoxServicePageSharingInspectModules\n");
    ...
    do
    {
        /** @todo when changing this make sure VPoxService.exe is excluded! */
        char *pszDot = strrchr(ModuleInfo.szModule, '.');
        if (    pszDot
            &&  (pszDot[1] == 'e' || pszDot[1] == 'E'))
            continue;   /* ignore executables for now. */

        VPoxServicePageSharingCheckModule(&ModuleInfo);
    }
 ...
 *
 */
void VPoxServicePageSharingInspectGuest()
{
    VPoxServicePageSharingInspectModules(GetCurrentProcessId());
...
/** @copydoc VPOXSERVICE::pfnInit */
static DECLCALLBACK(int) VPoxServicePageSharingInit(void)
{
    printf("VPoxServicePageSharingInit\n");
...
static DECLCALLBACK(void) VPoxServicePageSharingTerm(void)
{
    printf("VPoxServicePageSharingTerm\n");
...
int main(int argc, char **argv)
{
    ...
    VPoxServicePageSharingInit();

    VPoxServicePageSharingInspectGuest();

    VPoxServicePageSharingTerm();
    return 0;
}
