...
#ifndef VPOX_INCLUDED_SRC_Graphics_shaderlib_wined3d_private_h
#define VPOX_INCLUDED_SRC_Graphics_shaderlib_wined3d_private_h
...
#define COBJMACROS
#ifndef VPOX_WINE_WITHOUT_LIBWINE
#include "windef.h"
#include "winbase.h"
#include "winreg.h"
#include "wingdi.h"
...
# include "vpoxsharedrc.h"
#endif

#if defined(VPOX_WINE_WITH_SINGLE_CONTEXT) || defined(VPOX_WINE_WITH_SINGLE_SWAPCHAIN_CONTEXT)
# define VPoxTlsRefGetImpl(_tls) (TlsGetValue((DWORD)(_tls)))
# define VPoxTlsRefSetImpl(_tls, _val) (TlsSetValue((DWORD)(_tls), (_val)))
# define VPoxTlsRefAssertImpl Assert
# include <VPoxVideo3D.h>
#endif
...
struct VPOXVMSVGASHADERIF;
extern BOOL IWineD3DImpl_FillGLCaps(struct wined3d_adapter *adapter, struct VPOXVMSVGASHADERIF *pVPoxShaderIf);
...
#ifdef VPOX_WINE_WITH_PROFILE

#include <iprt/time.h>

#define VPOXWINEPROFILE_GET_TIME_NANO() RTTimeNanoTS()
#define VPOXWINEPROFILE_GET_TIME_MILLI() RTTimeMilliTS()

# define PRLOG(_m) do {\
        vpoxWDbgPrintF _m ; \
    } while (0)

typedef struct VPOXWINEPROFILE_ELEMENT
{
    uint64_t u64Time;
    uint32_t cu32Calls;
} VPOXWINEPROFILE_ELEMENT, *PVPOXWINEPROFILE_ELEMENT;

typedef struct VPOXWINEPROFILE_HASHMAP_ELEMENT
{
    VPOXEXT_HASHMAP_ENTRY MapEntry;
    VPOXWINEPROFILE_ELEMENT Data;
} VPOXWINEPROFILE_HASHMAP_ELEMENT, *PVPOXWINEPROFILE_HASHMAP_ELEMENT;

#define VPOXWINEPROFILE_HASHMAP_ELEMENT_FROMENTRY(_p) ((PVPOXWINEPROFILE_HASHMAP_ELEMENT)(((uint8_t*)(_p)) - RT_UOFFSETOF(VPOXWINEPROFILE_HASHMAP_ELEMENT, MapEntry)))

#define VPOXWINEPROFILE_ELEMENT_DUMP(_p, _pn) do { \
        PRLOG(("%s: t(%u);c(%u)\n", \
            (_pn), \
...
#define VPOXWINEPROFILE_ELEMENT_RESET(_p) do { \
        memset(_p, 0, sizeof (*(_p))); \
    } while (0)

#define VPOXWINEPROFILE_ELEMENT_STEP(_p, _t) do { \
        (_p)->u64Time += (_t); \
        ++(_p)->cu32Calls; \
    } while (0)

#define VPOXWINEPROFILE_HASHMAP_ELEMENT_CREATE()  ( (PVPOXWINEPROFILE_HASHMAP_ELEMENT)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof (VPOXWINEPROFILE_HASHMAP_ELEMENT)) )

#define VPOXWINEPROFILE_HASHMAP_ELEMENT_TERM(_pe) do { \
        HeapFree(GetProcessHeap(), 0, (_pe)); \
    } while (0)

DECLINLINE(PVPOXWINEPROFILE_HASHMAP_ELEMENT) vpoxWineProfileHashMapElementGet(PVPOXEXT_HASHMAP pMap, void *pvKey)
{
    PVPOXEXT_HASHMAP_ENTRY pEntry = VPoxExtHashGet(pMap, pvKey);
    if (pEntry)
    {
        return VPOXWINEPROFILE_HASHMAP_ELEMENT_FROMENTRY(pEntry);
    }
    else
    {
        PVPOXWINEPROFILE_HASHMAP_ELEMENT pElement = VPOXWINEPROFILE_HASHMAP_ELEMENT_CREATE();
        Assert(pElement);
        if (pElement)
            VPoxExtHashPut(pMap, pvKey, &pElement->MapEntry);
        return pElement;
    }
}

#define VPOXWINEPROFILE_HASHMAP_ELEMENT_STEP(_pm, _pk, _t) do { \
        PVPOXWINEPROFILE_HASHMAP_ELEMENT pElement = vpoxWineProfileHashMapElementGet(_pm, _pk); \
        VPOXWINEPROFILE_ELEMENT_STEP(&pElement->Data, _t); \
    } while (0)

static DECLCALLBACK(bool) vpoxWineProfileElementResetCb(struct VPOXEXT_HASHMAP *pMap, void *pvKey, struct VPOXEXT_HASHMAP_ENTRY *pValue, void *pvVisitor)
{
    PVPOXWINEPROFILE_HASHMAP_ELEMENT pElement = VPOXWINEPROFILE_HASHMAP_ELEMENT_FROMENTRY(pValue);
    VPOXWINEPROFILE_ELEMENT_RESET(&pElement->Data);
    return true;
}

static DECLCALLBACK(bool) vpoxWineProfileElementDumpCb(struct VPOXEXT_HASHMAP *pMap, void *pvKey, struct VPOXEXT_HASHMAP_ENTRY *pValue, void *pvVisitor)
{
    PVPOXWINEPROFILE_HASHMAP_ELEMENT pElement = VPOXWINEPROFILE_HASHMAP_ELEMENT_FROMENTRY(pValue);
    char *pName = (char*)pvVisitor;
    PRLOG(("%s[%d]:", pName, (uint32_t)pvKey));
    VPOXWINEPROFILE_ELEMENT_DUMP(&pElement->Data, "");
    return true;
}

#define VPOXWINEPROFILE_HASHMAP_RESET(_pm) do { \
        VPoxExtHashVisit((_pm), vpoxWineProfileElementResetCb, NULL); \
    } while (0)

#define VPOXWINEPROFILE_HASHMAP_DUMP(_pm, _pn) do { \
        VPoxExtHashVisit((_pm), vpoxWineProfileElementDumpCb, (_pn)); \
    } while (0)

static DECLCALLBACK(bool) vpoxWineProfileElementCleanupCb(struct VPOXEXT_HASHMAP *pMap, void *pvKey, struct VPOXEXT_HASHMAP_ENTRY *pValue, void *pvVisitor)
{
    PVPOXWINEPROFILE_HASHMAP_ELEMENT pElement = VPOXWINEPROFILE_HASHMAP_ELEMENT_FROMENTRY(pValue);
    VPOXWINEPROFILE_HASHMAP_ELEMENT_TERM(pElement);
    return true;
}

#define VPOXWINEPROFILE_HASHMAP_TERM(_pm) do { \
        VPoxExtHashCleanup((_pm), vpoxWineProfileElementCleanupCb, NULL); \
        VPoxExtHashVisit((_pm), vpoxWineProfileElementResetCb, NULL); \
    } while (0)

typedef struct VPOXWINEPROFILE_DRAWPRIM
{
    uint64_t u64LoadLocationTime;
    uint64_t u64CtxAcquireTime;
    uint64_t u64PostProcess;
    VPOXEXT_HASHMAP MapDrawPrimSlowVs;
    VPOXEXT_HASHMAP MapDrawPrimSlow;
    VPOXEXT_HASHMAP MapDrawPrimStrided;
    VPOXEXT_HASHMAP MapDrawPrimFast;
    uint32_t cu32Calls;
} VPOXWINEPROFILE_DRAWPRIM, *PVPOXWINEPROFILE_DRAWPRIM;

#define VPOXWINEPROFILE_DRAWPRIM_RESET_NEXT(_p) do { \
        (_p)->u64LoadLocationTime = 0; \
        (_p)->u64CtxAcquireTime = 0; \
        (_p)->u64PostProcess = 0; \
        VPOXWINEPROFILE_HASHMAP_RESET(&(_p)->MapDrawPrimSlowVs); \
        VPOXWINEPROFILE_HASHMAP_RESET(&(_p)->MapDrawPrimSlow); \
        VPOXWINEPROFILE_HASHMAP_RESET(&(_p)->MapDrawPrimStrided); \
        VPOXWINEPROFILE_HASHMAP_RESET(&(_p)->MapDrawPrimFast); \
    } while (0)

static DECLCALLBACK(uint32_t) vpoxWineProfileDrawPrimHashMapHash(void *pvKey)
{
    return (uint32_t)pvKey;
}

static DECLCALLBACK(bool) vpoxWineProfileDrawPrimHashMapEqual(void *pvKey1, void *pvKey2)
{
    return ((uint32_t)pvKey1) == ((uint32_t)pvKey2);
}

#define VPOXWINEPROFILE_DRAWPRIM_INIT(_p) do { \
        memset((_p), 0, sizeof (*(_p))); \
        VPoxExtHashInit(&(_p)->MapDrawPrimSlowVs, vpoxWineProfileDrawPrimHashMapHash, vpoxWineProfileDrawPrimHashMapEqual); \
        VPoxExtHashInit(&(_p)->MapDrawPrimSlow, vpoxWineProfileDrawPrimHashMapHash, vpoxWineProfileDrawPrimHashMapEqual); \
        VPoxExtHashInit(&(_p)->MapDrawPrimStrided, vpoxWineProfileDrawPrimHashMapHash, vpoxWineProfileDrawPrimHashMapEqual); \
        VPoxExtHashInit(&(_p)->MapDrawPrimFast, vpoxWineProfileDrawPrimHashMapHash, vpoxWineProfileDrawPrimHashMapEqual); \
    } while (0)

#define VPOXWINEPROFILE_DRAWPRIM_TERM(_p) do { \
        memset((_p), 0, sizeof (*(_p))); \
        VPOXWINEPROFILE_HASHMAP_TERM(&(_p)->MapDrawPrimSlowVs); \
        VPOXWINEPROFILE_HASHMAP_TERM(&(_p)->MapDrawPrimSlow); \
        VPOXWINEPROFILE_HASHMAP_TERM(&(_p)->MapDrawPrimStrided); \
        VPOXWINEPROFILE_HASHMAP_TERM(&(_p)->MapDrawPrimFast); \
    } while (0)
#else
...
#ifdef VPOX_WINE_WITH_SHADER_CACHE
    VPOXEXT_HASHCACHE vshaderCache;
    VPOXEXT_HASHCACHE pshaderCache;
#endif

#ifdef VPOX_WINE_WITH_PROFILE
    VPOXWINEPROFILE_DRAWPRIM DrawPrimProfile;
#endif
...
#ifdef VPOX_WITH_WDDM
HRESULT WINAPI IWineD3DResourceImpl_SetShRcState(IWineD3DResource *iface, VPOXWINEEX_SHRC_STATE enmState);
#endif
...
#define texture_gl_delete(_o, _t) do { \
        if (VPOXSHRC_IS_SHARED(_o)) GL_EXTCALL(glChromiumParameteriCR(GL_RCUSAGE_TEXTURE_CLEAR_CR, _t)); \
...
# define SFLAG_DONOTFREE_VPOXWDDM SFLAG_CLIENTMEM
#else
# define SFLAG_DONOTFREE_VPOXWDDM 0
#endif
...
#ifdef VPOX_WINE_WITH_SHADER_CACHE
    VPOXEXT_HASHCACHE_ENTRY CacheEntry;
 ...
 */
#ifdef VPOX_WITH_VMSVGA
# define VPOX_CHECK_GL_CALL(a_Expr) \
        do { \
            GLint rcCheckCall; \
            a_Expr; \
...
#else
# define VPOX_CHECK_GL_CALL(a_Expr) do { a_Expr; } while (0)
#endif

#endif /* !VPOX_INCLUDED_SRC_Graphics_shaderlib_wined3d_private_h */
