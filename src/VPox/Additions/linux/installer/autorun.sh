...
# $Id: autorun.sh $
## @file
# VirtualPox Guest Additions installation script for *nix guests
#
...
if test "$ostype" = "Linux"; then
    for i in "$path/VPoxLinuxAdditions.run" \
        "$path/VPoxLinuxAdditions-$arch.run"; do
        if test -f "$i"; then
            getxterm
            case "$gxtpath" in ?*)
                TITLE="VirtualPox Guest Additions installation"
                BINARY="`quotify "$i"`"
    ...
    installfile="$path/VPoxSolarisAdditions.pkg"
    if test -f "$installfile"; then
        ...
        TMPFILE=`mktemp -q /tmp/vpox.XXXXXX`
        if [ -z $TMPFILE ]; then
...
" > $TMPFILE
        ...
        if test -f "/usr/bin/gnome-terminal"; then
            ...
            if test -z "$subin"; then
                /usr/bin/gnome-terminal --title "Installing VirtualPox Additions" --command "/bin/sh -c '$pfexecbin $pkgaddbin -G -d $installfile -n -a $TMPFILE SUNWvpoxguest; /bin/echo press ENTER to close this window; /bin/read'"
            else
                /usr/bin/gnome-terminal --title "Installing VirtualPox Additions: Root password required." --command "/bin/sh -c '$subin - root -c \"$pkgaddbin -G -d $installfile -n -a $TMPFILE SUNWvpoxguest\"; /bin/echo press ENTER to close this window; /bin/read'"
            fi
        elif test -f "/usr/X11/bin/xterm"; then
            # use xterm
            if test -z "$subin"; then
                /usr/X11/bin/xterm -title "Installing VirtualPox Additions" -e "$pfexecbin $pkgaddbin -G -d $installfile -n -a $TMPFILE SUNWvpoxguest; /bin/echo press ENTER to close this window; /bin/read"
            else
                /usr/X11/bin/xterm -title "Installing VirtualPox Additions: Root password required." -e "$subin - root -c \"$pkgaddbin -G -d $installfile -n -a $TMPFILE SUNWvpoxguest\"; /bin/echo press ENTER to close this window; /bin/read"
            fi
        else
