/* $Id: VPoxDnDDataObject.cpp $ */
/** @file
 * VPoxDnDDataObject.cpp - IDataObject implementation.
 */
...
//# define VPOX_DND_DEBUG_FORMATS
#endif

/** @todo Implement IDataObjectAsyncCapability interface? */

VPoxDnDDataObject::VPoxDnDDataObject(LPFORMATETC pFormatEtc, LPSTGMEDIUM pStgMed, ULONG cFormats)
...
VPoxDnDDataObject::~VPoxDnDDataObject(void)
{
    if (m_paFormatEtc)
...
STDMETHODIMP_(ULONG) VPoxDnDDataObject::AddRef(void)
{
    return InterlockedIncrement(&m_cRefs);
}

STDMETHODIMP_(ULONG) VPoxDnDDataObject::Release(void)
{
...
STDMETHODIMP VPoxDnDDataObject::QueryInterface(REFIID iid, void **ppvObject)
{
...
STDMETHODIMP VPoxDnDDataObject::GetData(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium)
{
    ...
    if (m_enmStatus == Status_Dropped)
    {
        ...
        LogRel3(("DnD: cfFormat=%RI16, sFormat=%s, tyMed=%RU32, dwAspect=%RU32\n",
                 pThisFormat->cfFormat, VPoxDnDDataObject::ClipboardFormatToString(pFormatEtc->cfFormat),
...
STDMETHODIMP VPoxDnDDataObject::GetDataHere(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium)
{
...
STDMETHODIMP VPoxDnDDataObject::QueryGetData(LPFORMATETC pFormatEtc)
{
    LogFlowFunc(("\n"));
    return (LookupFormatEtc(pFormatEtc, NULL /* puIndex */)) ? S_OK : DV_E_FORMATETC;
}

STDMETHODIMP VPoxDnDDataObject::GetCanonicalFormatEtc(LPFORMATETC pFormatEtc, LPFORMATETC pFormatEtcOut)
{
    RT_NOREF(pFormatEtc);
    LogFlowFunc(("\n"));
...
STDMETHODIMP VPoxDnDDataObject::SetData(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium, BOOL fRelease)
{
    RT_NOREF(pFormatEtc, pMedium, fRelease);
    return E_NOTIMPL;
}

STDMETHODIMP VPoxDnDDataObject::EnumFormatEtc(DWORD dwDirection, IEnumFORMATETC **ppEnumFormatEtc)
{
    LogFlowFunc(("dwDirection=%RI32, mcFormats=%RI32, mpFormatEtc=%p\n", dwDirection, m_cFormats, m_paFormatEtc));

    HRESULT hr;
    if (dwDirection == DATADIR_GET)
        hr = VPoxDnDEnumFormatEtc::CreateEnumFormatEtc(m_cFormats, m_paFormatEtc, ppEnumFormatEtc);
    else
        hr = E_NOTIMPL;
...
STDMETHODIMP VPoxDnDDataObject::DAdvise(LPFORMATETC pFormatEtc, DWORD fAdvise, IAdviseSink *pAdvSink, DWORD *pdwConnection)
{
    RT_NOREF(pFormatEtc, fAdvise, pAdvSink, pdwConnection);
    return OLE_E_ADVISENOTSUPPORTED;
}

STDMETHODIMP VPoxDnDDataObject::DUnadvise(DWORD dwConnection)
{
    RT_NOREF(dwConnection);
    return OLE_E_ADVISENOTSUPPORTED;
}

STDMETHODIMP VPoxDnDDataObject::EnumDAdvise(IEnumSTATDATA **ppEnumAdvise)
{
 ...
 */
int VPoxDnDDataObject::Abort(void)
{
 ...
 */
/* static */
const char* VPoxDnDDataObject::ClipboardFormatToString(CLIPFORMAT fmt)
{
#if 0
    char szFormat[128];
 ...
 */
bool VPoxDnDDataObject::LookupFormatEtc(LPFORMATETC pFormatEtc, ULONG *puIndex)
{
    ...
    for (ULONG i = 0; i < m_cFormats; i++)
    {
            ...
            && pFormatEtc->dwAspect == m_paFormatEtc[i].dwAspect)
        {
            LogRel3(("DnD: Format found: tyMed=%RI32, cfFormat=%RI16, sFormats=%s, dwAspect=%RI32, ulIndex=%RU32\n",
                      pFormatEtc->tymed, pFormatEtc->cfFormat, VPoxDnDDataObject::ClipboardFormatToString(m_paFormatEtc[i].cfFormat),
    ...
    LogRel3(("DnD: Format NOT found: tyMed=%RI32, cfFormat=%RI16, sFormats=%s, dwAspect=%RI32\n",
             pFormatEtc->tymed, pFormatEtc->cfFormat, VPoxDnDDataObject::ClipboardFormatToString(pFormatEtc->cfFormat),
             pFormatEtc->dwAspect));
 ...
 */
void VPoxDnDDataObject::RegisterFormat(LPFORMATETC pFormatEtc, CLIPFORMAT clipFormat,
                                       ...
                                       DVTARGETDEVICE *pTargetDevice)
{
    ...
    LogFlowFunc(("Registered format=%ld, sFormat=%s\n",
                 pFormatEtc->cfFormat, VPoxDnDDataObject::ClipboardFormatToString(pFormatEtc->cfFormat)));
}
 ...
 */
void VPoxDnDDataObject::SetStatus(Status status)
{
 ...
 */
int VPoxDnDDataObject::Signal(const RTCString &strFormat,
