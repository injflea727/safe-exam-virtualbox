; $Id: VPoxGuestAdditionsVista.nsh $
;; @file
; VPoxGuestAdditionsVista.nsh - Guest Additions installation for Windows Vista/7.
;
...
Function Vista_CopyFiles
  ...
  ; VPoxNET drivers are not tested yet - commented out until officially supported and released
  ;FILE "$%PATH_OUT%\bin\additions\VPoxNET.inf"
  ;FILE "$%PATH_OUT%\bin\additions\VPoxNET.sys"
...
Function ${un}Vista_Uninstall
   ...
   Delete /REBOOTOK "$g_strSystemDir\VPoxCredProv.dll"

   Call ${un}VPoxMMR_Uninstall
...
!macro VPoxMMR_Uninstall un
Function ${un}VPoxMMR_Uninstall

  ; Remove VPoxMMR always

  DetailPrint "Uninstalling VPoxMMR."
  Call ${un}StopVPoxMMR

  DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Run" "VPoxMMR"

  Delete /REBOOTOK "$g_strSystemDir\VPoxMMR.exe"

  !if $%BUILD_TARGET_ARCH% == "amd64"
    Delete /REBOOTOK "$g_strSysWow64\VPoxMMRHook.dll"
    Delete /REBOOTOK "$INSTDIR\VPoxMMR-x86.exe"
    Delete /REBOOTOK "$INSTDIR\VPoxMMRHook-x86.dll"
  !else
    Delete /REBOOTOK "$g_strSystemDir\VPoxMMRHook.dll"
    Delete /REBOOTOK "$INSTDIR\VPoxMMR.exe"
    Delete /REBOOTOK "$INSTDIR\VPoxMMRHook.dll"
  !endif

FunctionEnd
!macroend
!insertmacro VPoxMMR_Uninstall ""
!insertmacro VPoxMMR_Uninstall "un."
