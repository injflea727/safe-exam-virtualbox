...
# $Id: vpoxadd-x11.sh $
## @file
# Linux Additions X11 setup init script ($Revision: 136499 $)
#
...
#
### BEGIN INIT INFO
# Provides:       vpoxadd-x11
# Required-Start:
# Required-Stop:
# Default-Start:
# Default-Stop:
# Description:    VirtualPox Linux Additions X11 setup
### END INIT INFO

PATH=$PATH:/bin:/sbin:/usr/sbin
CONFIG_DIR="/var/lib/VPoxGuestAdditions"
...
# Version of OL installed.  Needed for blacklisting vpoxvideo.
...
setup_opengl()
{
    ...
    rm -f /etc/ld.so.conf.d/00vpoxvideo.conf
    rm -Rf /var/lib/VPoxGuestAdditions/lib
    if /usr/bin/VPoxClient --check3d 2>/dev/null; then
        mkdir -p /var/lib/VPoxGuestAdditions/lib
        ln -sf "${INSTALL_DIR}/lib/VPoxOGL.so" /var/lib/VPoxGuestAdditions/lib/libGL.so.1
        ...
        if test -e /etc/selinux/config; then
            if command -v semanage > /dev/null; then
                semanage fcontext -a -t lib_t "/var/lib/VPoxGuestAdditions/lib/libGL.so.1"
            fi
            # This is needed on old Fedora/Redhat systems.  No one remembers which.
            chcon -h  -t lib_t "/var/lib/VPoxGuestAdditions/lib/libGL.so.1" 2>/dev/null
        fi
        echo "/var/lib/VPoxGuestAdditions/lib" > /etc/ld.so.conf.d/00vpoxvideo.conf
    fi
    ldconfig
}
...
setup()
{
    ...
    if test -n "${blacklist_vpoxvideo}"; then
        test -d /etc/modprobe.d &&
            echo "blacklist vpoxvideo" > /etc/modprobe.d/blacklist-vpoxvideo.conf
    else
        test -f /etc/modprobe.d/blacklist-vpoxvideo.conf &&
            rm -f /etc/modprobe.d/blacklist-vpoxvideo.conf
        ...
        ps -Af | grep -q '[X]org' || ${MODPROBE} -q vpoxvideo
    fi
    ...
    case $x_version in
        ...
        1.11.* )
            ...
            vpoxvideo_src=vpoxvideo_drv_111.so
            test "$system" = "redhat" && test -z "${vmsvga}" || setupxorgconf=""
            ;;
        1.10.* )
            xserver_version="X.Org Server 1.10"
            vpoxvideo_src=vpoxvideo_drv_110.so
            test "$system" = "redhat" && test -z "${vmsvga}" || setupxorgconf=""
            ;;
        1.9.* )
            xserver_version="X.Org Server 1.9"
            vpoxvideo_src=vpoxvideo_drv_19.so
            # Fedora 14 to 16 patched out vpoxvideo detection
            test "$system" = "redhat" && test -z "${vmsvga}" || setupxorgconf=""
            ;;
        1.8.* )
            xserver_version="X.Org Server 1.8"
            vpoxvideo_src=vpoxvideo_drv_18.so
            # Fedora 13 shipped without vpoxvideo detection
            test "$system" = "redhat" && test -z "${vmsvga}" || setupxorgconf=""
            ;;
        1.7.* )
            xserver_version="X.Org Server 1.7"
            vpoxvideo_src=vpoxvideo_drv_17.so
            setupxorgconf=""
            ;;
        1.6.* )
            xserver_version="X.Org Server 1.6"
            vpoxvideo_src=vpoxvideo_drv_16.so
            vpoxmouse_src=vpoxmouse_drv_16.so
        ...
        1.5.* )
            ...
            vpoxvideo_src=vpoxvideo_drv_15.so
            vpoxmouse_src=vpoxmouse_drv_15.so
        ...
        1.4.* )
            ...
            vpoxvideo_src=vpoxvideo_drv_14.so
            vpoxmouse_src=vpoxmouse_drv_14.so
            automouse=""
            ;;
        1.3.* )
            ...
            vpoxvideo_src=vpoxvideo_drv_13.so
            vpoxmouse_src=vpoxmouse_drv_13.so
            automouse=""
            ;;
        7.1.* | 7.2.* )
            xserver_version="X.Org 7.1"
            vpoxvideo_src=vpoxvideo_drv_71.so
            vpoxmouse_src=vpoxmouse_drv_71.so
            automouse=""
            ;;
        6.9.* | 7.0.* )
            xserver_version="X.Org 6.9/7.0"
            vpoxvideo_src=vpoxvideo_drv_70.so
            vpoxmouse_src=vpoxmouse_drv_70.so
            automouse=""
            ;;
        6.7* | 6.8.* | 4.2.* | 4.3.* )
            ...
            driver_ext=.o
            vpoxvideo_src=vpoxvideo_drv.o
            vpoxmouse_src=vpoxmouse_drv.o
            automouse=""
            autokeyboard=""
            case $x_version in
                6.8.* )
        ...
        1.12.* | 1.13.* | 1.14.* | 1.15.* | 1.16.* | 1.17.* | 1.18.* )
            ...
            vpoxvideo_src=vpoxvideo_drv_`echo ${x_version_short} | sed 's/\.//'`.so
            setupxorgconf=""
            test -f "${lib_dir}/${vpoxvideo_src}" ||
            {
    ...
    case "$vpoxvideo_src" in
        ?*)
        ln -s "${lib_dir}/$vpoxvideo_src" "$modules_dir/drivers/vpoxvideo_drv$driver_ext.new" &&
            mv "$modules_dir/drivers/vpoxvideo_drv$driver_ext.new" "$modules_dir/drivers/vpoxvideo_drv$driver_ext";;
        *)
        rm "$modules_dir/drivers/vpoxvideo_drv$driver_ext" 2>/dev/null
    esac
    case "$vpoxmouse_src" in
        ?*)
        ln -s "${lib_dir}/$vpoxmouse_src" "$modules_dir/input/vpoxmouse_drv$driver_ext.new" &&
            mv "$modules_dir/input/vpoxmouse_drv$driver_ext.new" "$modules_dir/input/vpoxmouse_drv$driver_ext";;
        *)
        rm "$modules_dir/input/vpoxmouse_drv$driver_ext" 2>/dev/null
    esac
    ...
    if test -n "$dox11config"; then
        ...
        # Those versions can autoload vpoxvideo though, so we don't need to
        # hack the configuration file for them.
        test "$system" = "debian" -a -d /usr/share/xserver-xorg/pci &&
        {
            rm -f "/usr/share/xserver-xorg/pci/vpoxvideo.ids"
            ln -s "${lib_dir}/vpoxvideo.ids" /usr/share/xserver-xorg/pci 2>/dev/null
        ...
        if test -n "$setupxorgconf"; then
            for i in $x11conf_files; do
                if test -r "$i"; then
                    if grep -q "VirtualPox generated" "$i"; then
                ...
                test -r "$i.vpox" && touch "$i.vpox"
            done
            # X.Org Server 1.5 and 1.6 can detect hardware they know, but they
            # need a configuration file for VPoxVideo.
            nobak_cfg="`expr "${main_cfg}" : '\([^.]*\)'`.vpox.nobak"
...
the VirtualPox Guest Additions and were not modified:
...
EOF
    ...
    case "$redhat_release" in
        ...
        Fedora\ release\ 7* | Fedora\ release\ 8* )
            semodule -i "${lib_dir}/vpox_x11.pp" > /dev/null 2>&1
            ;;
        # Similar for the accelerated graphics check on Fedora 15
        Fedora\ release\ 15* )
            semodule -i "${lib_dir}/vpox_accel.pp" > /dev/null 2>&1
            ;;
    esac
    ...
    chcon -t unconfined_execmem_exec_t '/usr/bin/VPoxClient' > /dev/null 2>&1
    semanage fcontext -a -t unconfined_execmem_exec_t '/usr/bin/VPoxClient' > /dev/null 2>&1

    # And set up VPoxClient to start when the X session does
    install_x11_startup_app "${lib_dir}/98vpoxadd-xclient" "${lib_dir}/vpoxclient.desktop" VPoxClient VPoxClient-all ||
        fail "Failed to set up VPoxClient to start automatically."
    ln -s "${lib_dir}/98vpoxadd-xclient" /usr/bin/VPoxClient-all 2>/dev/null
...
cleanup()
{
    ...
    if test -z "${nobak}"; then
        for i in $x11conf_files; do
            if test -r "$i.vpox"; then
                if test ! "$i" -nt "$i.vpox" -o -n "$legacy"; then
                    mv -f "$i.vpox" "$i"
                    grep -q -E 'vpoxvideo|vpoxmouse' "$i" &&
                        failed="$failed`printf "  $i\n"`"
                else
                    newer="$newer`printf "  $i ($i.vpox)\n"`"
                fi
            fi
        done
    fi
...
have been changed since they were generated by the VirtualPox Guest Additions.
...
EOF
    ...
    rm -f "$modules_dir/drivers/vpoxvideo_drv"* 2>/dev/null
    rm -f "$modules_dir/input/vpoxmouse_drv"* 2>/dev/null

    # Remove the link to vpoxvideo_dri.so
    for dir in /usr/lib/dri /usr/lib32/dri /usr/lib64/dri \
        ...
        if [ -d $dir ]; then
            rm -f "$dir/vpoxvideo_dri.so" 2>/dev/null
        fi
    done

    # Remove VPoxClient autostart files
    rm /etc/X11/Xsession.d/98vpoxadd-xclient 2>/dev/null
    rm /etc/X11/xinit.d/98vpoxadd-xclient 2>/dev/null
    rm /etc/X11/xinit/xinitrc.d/98vpoxadd-xclient.sh 2>/dev/null
    rm /etc/xdg/autostart/vpoxclient.desktop 2>/dev/null
    rm /usr/share/autostart/vpoxclient.desktop 2>/dev/null
    rm /usr/bin/VPoxClient-all 2>/dev/null

    # Remove other files
    rm /usr/share/xserver-xorg/pci/vpoxvideo.ids 2>/dev/null
    return 0
}
