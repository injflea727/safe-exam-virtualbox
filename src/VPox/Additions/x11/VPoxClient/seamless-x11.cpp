...
DECLCALLBACK(int) VPoxGuestWinFree(VPoxGuestWinInfo *pInfo, void *pvParam)
{
...
void SeamlessX11::freeWindowTree(void)
{
    ...
    LogRelFlowFuncEnter();
    mGuestWindows.detachAll(VPoxGuestWinFree, mDisplay);
    LogRelFlowFuncLeave();
}
...
void SeamlessX11::nextConfigurationEvent(void)
{
    ...
    case PropertyNotify:
        ...
        rebuildWindowTree();
        break;
    case VPoxShapeNotify:  /* This is defined wrong in my X11 header files! */
...
{
    VPoxGuestWinInfo *pInfo = mGuestWindows.find(hWin);
    if (pInfo)
    {
        XWindowAttributes winAttrib;
...
DECLCALLBACK(int) getRectsCallback(VPoxGuestWinInfo *pInfo,
...
int SeamlessX11::updateRects(void)
{
    ...
    mGuestWindows.doWithAll((PVPOXGUESTWINCALLBACK)getRectsCallback,
                            &rects);
    if (mpRects)
        RTMemFree(mpRects);
