; $Id: VPoxGuestAdditions.nsi $
; @file
; VPoxGuestAdditions.nsi - Main file for Windows Guest Additions installation.
;
...
!if $%VPOX_WITH_GUEST_INSTALLER_UNICODE% == "1"
...
!define WHQL_FAKE                   ; Enables faking of non WHQL signed / approved drivers
                                    ; Needs the VPoxWHQLFake.exe in the additions output directory!
!define WFP_FILE_EXCEPTION          ; Enables setting a temporary file exception for WFP proctected files

!define VENDOR_ROOT_KEY             "SOFTWARE\$%VPOX_VENDOR_SHORT%"

; Product defines
!define PRODUCT_NAME                "$%VPOX_PRODUCT% Guest Additions"
!define PRODUCT_DESC                "$%VPOX_PRODUCT% Guest Additions"
!define PRODUCT_VERSION             "$%VPOX_VERSION_MAJOR%.$%VPOX_VERSION_MINOR%.$%VPOX_VERSION_BUILD%.0"
!define PRODUCT_PUBLISHER           "$%VPOX_VENDOR%"
!define PRODUCT_COPYRIGHT           "(C) $%VPOX_C_YEAR% $%VPOX_VENDOR%"
!define PRODUCT_OUTPUT              "VPoxWindowsAdditions-$%BUILD_TARGET_ARCH%.exe"
!define PRODUCT_WEB_SITE            "http://www.virtualpox.org"
!define PRODUCT_INSTALL_KEY         "${VENDOR_ROOT_KEY}\VirtualPox Guest Additions"
...
VIAddVersionKey "FileVersion"       "$%VPOX_VERSION_STRING%"
...
!if $%VPOX_WITH_GUEST_INSTALL_HELPER% == "1"
  !addplugindir "$%PATH_TARGET_X86%\VPoxGuestInstallHelper"
!endif
...
!ifdef USE_MUI
  ...
  ; MUI Settings
  !define MUI_WELCOMEFINISHPAGE_BITMAP "$%VPOX_BRAND_WIN_ADD_INST_DLGBMP%"
  ...
  !if $%BUILD_TARGET_ARCH% == "x86"       ; 32-bit
    !define MUI_ICON "$%VPOX_NSIS_ICON_FILE%"
    !define MUI_UNICON "$%VPOX_NSIS_ICON_FILE%"
  !else   ; 64-bit
    !define MUI_ICON "$%VPOX_WINDOWS_ADDITIONS_ICON_FILE%"
    !define MUI_UNICON "$%VPOX_WINDOWS_ADDITIONS_ICON_FILE%"
  !endif

  ; Welcome page
  !insertmacro MUI_PAGE_WELCOME
!ifdef VPOX_WITH_LICENSE_DISPLAY
  ; License page
  !insertmacro MUI_PAGE_LICENSE "$(VPOX_LICENSE)"
...
!ifdef _DEBUG
  BrandingText "VirtualPox Windows Additions $%VPOX_VERSION_STRING% (r$%VPOX_SVN_REV%) - Debug Build"
!else
  BrandingText "VirtualPox Windows Additions $%VPOX_VERSION_STRING% r$%VPOX_SVN_REV%"
!endif

!ifdef VPOX_WITH_LICENSE_DISPLAY
  ; Set license language
  LicenseLangString VPOX_LICENSE ${LANG_ENGLISH} "$%VPOX_BRAND_LICENSE_RTF%"

  ; If license files not available (OSE / PUEL) build, then use the English one as default
  !ifdef VPOX_BRAND_fr_FR_LICENSE_RTF
    LicenseLangString VPOX_LICENSE ${LANG_FRENCH} "$%VPOX_BRAND_fr_FR_LICENSE_RTF%"
  !else
    LicenseLangString VPOX_LICENSE ${LANG_FRENCH} "$%VPOX_BRAND_LICENSE_RTF%"
  !endif
  !ifdef VPOX_BRAND_de_DE_LICENSE_RTF
    LicenseLangString VPOX_LICENSE ${LANG_GERMAN} "$%VPOX_BRAND_de_DE_LICENSE_RTF%"
  !else
    LicenseLangString VPOX_LICENSE ${LANG_GERMAN} "$%VPOX_BRAND_LICENSE_RTF%"
  !endif
!endif
...
Name "${PRODUCT_NAME} $%VPOX_VERSION_STRING%"
!ifdef UNINSTALLER_ONLY
  !echo "Uninstaller only!"
  OutFile "$%PATH_TARGET%\VPoxWindowsAdditions-$%BUILD_TARGET_ARCH%-uninst.exe"
!else
  OutFile "VPoxWindowsAdditions-$%BUILD_TARGET_ARCH%.exe"
!endif ; UNINSTALLER_ONLY

; Define default installation directory
!if $%BUILD_TARGET_ARCH% == "x86" ; 32-bit
  InstallDir  "$PROGRAMFILES32\$%VPOX_VENDOR_SHORT%\VirtualPox Guest Additions"
!else       ; 64-bit
  InstallDir  "$PROGRAMFILES64\$%VPOX_VENDOR_SHORT%\VirtualPox Guest Additions"
!endif
...
Var g_bNoVPoxServiceExit                ; Cmd line: Do not quit VPoxService before updating - install on next reboot
Var g_bNoVPoxTrayExit                   ; Cmd line: Do not quit VPoxTray before updating - install on next reboot
Var g_bNoVideoDrv                       ; Cmd line: Do not install the VPoxVideo driver
Var g_bNoGuestDrv                       ; Cmd line: Do not install the VPoxGuest driver
Var g_bNoMouseDrv                       ; Cmd line: Do not install the VPoxMouse driver
Var g_bNoStartMenuEntries               ; Cmd line: Do not create start menu entries
Var g_bWithAutoLogon                    ; Cmd line: Install VPoxGINA / VPoxCredProv for auto logon support
...
Var g_bPostInstallStatus                ; Cmd line: Post the overall installation status to some external program (VPoxTray)

; Platform parts of this installer
!include "VPoxGuestAdditionsLog.nsh"
!include "VPoxGuestAdditionsExternal.nsh"
!include "VPoxGuestAdditionsCommon.nsh"
!if $%BUILD_TARGET_ARCH% == "x86"       ; 32-bit only
  !include "VPoxGuestAdditionsNT4.nsh"
!endif
!include "VPoxGuestAdditionsW2KXP.nsh"
!include "VPoxGuestAdditionsVista.nsh"
!include "VPoxGuestAdditionsUninstall.nsh"    ; Product uninstallation
!include "VPoxGuestAdditionsUninstallOld.nsh" ; Uninstallation of deprecated versions which must be removed first
...
Function HandleCommandLine
  ...
  ${While} $2 <= $1                           ; Loop through all params
    ...
    ${Switch} $4
      ...
      ${Case} '/no_vpoxservice_exit' ; Not officially documented
        StrCpy $g_bNoVPoxServiceExit "true"
        ${Break}

      ${Case} '/no_vpoxtray_exit' ; Not officially documented
        StrCpy $g_bNoVPoxTrayExit "true"
        ${Break}
...
!if $%VPOX_WITH_GUEST_INSTALL_HELPER% == "1"
      ...
      ; - should not quit VPoxTray during the update, because ...
      ; - ... it should show the overall installation status
      ;   using VPoxTray's balloon message feature (since VPox 4.0)
...
usage:
  ...
  MessageBox MB_OK "${PRODUCT_NAME} Installer$\r$\n$\r$\n \
                    Usage: VPoxWindowsAdditions-$%BUILD_TARGET_ARCH% [OPTIONS] [/l] [/S] [/D=<PATH>]$\r$\n$\r$\n \
                    Options:$\r$\n \
...
sun_check:
  ...
  ; Check for old "Sun VirtualPox Guest Additions"
  ...
  MessageBox MB_YESNO $(VPOX_SUN_FOUND) /SD IDYES IDYES sun_uninstall
    Pop $2
    Pop $1
    Pop $0
    MessageBox MB_ICONSTOP $(VPOX_SUN_ABORTED) /SD IDOK
    Quit
...
Function CheckForInstalledComponents
  ...
  ; VPoxGINA already installed? So we need to update the installed version as well,
...
; Main Files
Section $(VPOX_COMPONENT_MAIN) SEC01

  SectionIn RO ; Section cannot be unselected (read-only)
  ${If} $g_bPostInstallStatus == "true"
    ${LogToVPoxTray} "0" "${PRODUCT_NAME} update started, please wait ..."
  ${EndIf}
  ...
  ${LogVerbose} "Version: $%VPOX_VERSION_STRING% (Rev $%VPOX_SVN_REV%)"
  ${If} $g_strAddVerMaj != ""
...
nt4: ; Windows NT4
  ...
  ${If} $R0 <> "6"
    MessageBox MB_YESNO $(VPOX_NT4_NO_SP6) /SD IDYES IDYES +2
    Quit
  ${EndIf}
...
notsupported:
  ...
  MessageBox MB_ICONSTOP $(VPOX_PLATFORM_UNSUPPORTED) /SD IDOK
  goto exit

success:

  ; Write a registry key with version and installation path for later lookup
  WriteRegStr HKLM "${PRODUCT_INSTALL_KEY}" "Version" "$%VPOX_VERSION_STRING_RAW%"
  WriteRegStr HKLM "${PRODUCT_INSTALL_KEY}" "VersionExt" "$%VPOX_VERSION_STRING%"
  WriteRegStr HKLM "${PRODUCT_INSTALL_KEY}" "Revision" "$%VPOX_SVN_REV%"
...
Section /o -$(VPOX_COMPONENT_AUTOLOGON) SEC02
  ...
  ; whether to replace it with the VirtualPox one or not
  ReadRegStr $0 HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon" "GinaDLL"
  ${If} $0 != ""
    ${If} $0 != "VPoxGINA.dll"
      ${LogVerbose} "Found another already installed GINA module: $0"
      MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON1 $(VPOX_COMPONENT_AUTOLOGON_WARN_3RDPARTY) /SD IDYES IDYES install
...
install:
  ...
  ; Do we need VPoxCredProv or VPoxGINA?
  ${If}   $R0 == 'Vista' ; Windows Vista.
  ${OrIf} $R0 == '7'     ; Windows 7.
  ${OrIf} $R0 == '8'     ; Windows 8.
  ${OrIf} $R0 == '8_1'   ; Windows 8.1 / Windows Server 2012 R2.
  ${OrIf} $R0 == '10'    ; Windows 10.
    ; Use VPoxCredProv on Vista and up.
    ${LogVerbose} "Installing VirtualPox credential provider ..."
    !insertmacro ReplaceDLL "$%PATH_OUT%\bin\additions\VPoxCredProv.dll" "$g_strSystemDir\VPoxCredProv.dll" "$INSTDIR"
    WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Authentication\Credential Providers\{275D3BCC-22BB-4948-A7F6-3A3054EBA92B}" "" "VPoxCredProv" ; adding to (default) key
    WriteRegStr HKCR "CLSID\{275D3BCC-22BB-4948-A7F6-3A3054EBA92B}" "" "VPoxCredProv"                       ; adding to (Default) key
    WriteRegStr HKCR "CLSID\{275D3BCC-22BB-4948-A7F6-3A3054EBA92B}\InprocServer32" "" "VPoxCredProv.dll"    ; adding to (Default) key
    WriteRegStr HKCR "CLSID\{275D3BCC-22BB-4948-A7F6-3A3054EBA92B}\InprocServer32" "ThreadingModel" "Apartment"
  ${Else} ; Use VPoxGINA on older Windows OSes (< Vista)
    ${LogVerbose} "Installing VirtualPox GINA ..."
    !insertmacro ReplaceDLL "$%PATH_OUT%\bin\additions\VPoxGINA.dll" "$g_strSystemDir\VPoxGINA.dll" "$INSTDIR"
    WriteRegStr HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon" "GinaDLL" "VPoxGINA.dll"
    ; Add Windows notification package callbacks for VPoxGINA
    WriteRegStr   HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon\Notify\VPoxGINA" "DLLName" "VPoxGINA.dll"
    WriteRegDWORD HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon\Notify\VPoxGINA" "Impersonate" 0
    WriteRegStr   HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon\Notify\VPoxGINA" "StopScreenSaver" "WnpScreenSaverStop"
  ${EndIf}
...
; Direct3D support
Section /o $(VPOX_COMPONENT_D3D) SEC03
...
Section /o $(VPOX_COMPONENT_STARTMENU) SEC04
...
!ifdef USE_MUI
  ...
  !insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT   ${SEC01} $(VPOX_COMPONENT_MAIN_DESC)
    !insertmacro MUI_DESCRIPTION_TEXT   ${SEC02} $(VPOX_COMPONENT_AUTOLOGON_DESC)
    !insertmacro MUI_DESCRIPTION_TEXT   ${SEC03} $(VPOX_COMPONENT_D3D_DESC)
    !insertmacro MUI_DESCRIPTION_TEXT   ${SEC04} $(VPOX_COMPONENT_STARTMENU_DESC)
...
!ifdef _DEBUG
  ...
  WriteRegDWORD HKLM "SYSTEM\CurrentControlSet\Services\VPoxGuest" "LoggingEnabled" 255
!endif
...
Function .onInstFailed
  ...
  ${LogVerbose} "$(VPOX_ERROR_INST_FAILED)"
  MessageBox MB_ICONSTOP $(VPOX_ERROR_INST_FAILED) /SD IDOK

  ${If} $g_bPostInstallStatus == "true"
    ${LogToVPoxTray} "2" "Error while installing ${PRODUCT_NAME}!"
  ${EndIf}
...
Function .onInstSuccess
  ...
  ${If} $g_bPostInstallStatus == "true"
    ${LogToVPoxTray} "0" "${PRODUCT_NAME} successfully updated!"
  ${EndIf}
...
Function .onInit
  ...
  StrCpy $g_iSfOrder "0"
  StrCpy $g_bNoVPoxServiceExit "false"
  StrCpy $g_bNoVPoxTrayExit "false"
...
!else
  ...
  System::Call 'kernel32::CreateMutexA(i 0, i 0, t "VPoxGuestInstaller") ?e'
  Pop $0
  ${If} $0 != 0
    Quit
  ${EndIf}
  ...
  ${If} $g_bOnlyExtract == "true"
    Call ExtractFiles
    MessageBox MB_OK|MB_ICONINFORMATION $(VPOX_EXTRACTION_COMPLETE) /SD IDOK
    Quit
  ${EndIf}
...
!if $%BUILD_TARGET_ARCH% == "amd64"
    MessageBox MB_ICONSTOP $(VPOX_NOTICE_ARCH_AMD64) /SD IDOK
!else
    MessageBox MB_ICONSTOP $(VPOX_NOTICE_ARCH_X86) /SD IDOK
!endif
    Abort "$(VPOX_NOTICE_ARCH_AMD64)"
  ${EndIf}
  ...
  Pop $0
  ${If} $0 != "Admin"
    MessageBox MB_ICONSTOP $(VPOX_NOADMIN) /SD IDOK
    Abort
  ${EndIf}
  ...
  ${If} $g_bUninstall == "true"
    Call Uninstall_Innotek
    Call Uninstall
    MessageBox MB_ICONINFORMATION|MB_OK $(VPOX_UNINST_SUCCESS) /SD IDOK
    Quit
  ${EndIf}
...
!ifdef USE_MUI
  ...
  !ifdef VPOX_INSTALLER_ADD_LANGUAGES
...
Function un.onInit
  ...
  MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 $(VPOX_UNINST_CONFIRM) /SD IDYES IDYES proceed
  Quit
...
!ifndef _DEBUG
  SetAutoClose true
  MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 $(VPOX_REBOOT_REQUIRED) /SD IDNO IDYES restart
