...
static int hmR3InitFinalizeR0(PVM pVM)
{
    ...
    if (   !pVM->hm.s.vmx.fSupported
        ...
        &&  RTEnvExist("VPOX_HWVIRTEX_IGNORE_SVM_IN_USE"))
    {
        LogRel(("HM: VPOX_HWVIRTEX_IGNORE_SVM_IN_USE active!\n"));
        pVM->hm.s.svm.fSupported        = true;
...
static int hmR3InitFinalizeR0Intel(PVM pVM)
{
    ...
    AssertLogRelReturn(pVM->hm.s.vmx.u64HostFeatCtrl != 0, VERR_HM_IPE_4);
    ...
    hmR3VmxReportFeatCtlMsr(pVM->hm.s.vmx.u64HostFeatCtrl);
 ...
 * @param   pvUser  Unused.
 */
static DECLCALLBACK(VPOXSTRICTRC) hmR3RemovePatches(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 *
 */
static DECLCALLBACK(VPOXSTRICTRC) hmR3ReplaceTprInstr(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    /*
 ...
 *
 */
static DECLCALLBACK(VPOXSTRICTRC) hmR3PatchTprInstr(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    /*
     * Only execute the handler on the VCPU the original patch request was
     * issued. (The other CPU(s) might not yet have switched to protected
 ...
 *
 * @param   pVM         The cross context VM structure.
 * @param   iStatusCode VPox status code.
 */
