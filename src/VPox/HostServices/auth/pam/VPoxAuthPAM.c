/** @file
 *
 * VirtualPox External Authentication Library:
 ...
 * environment variable VPOX_AUTH_PAM_SERVICE. If the variable
 * is not specified, then the 'login' PAM service is used.
 */
#define VPOX_AUTH_PAM_SERVICE_NAME_ENV_OLD "VRDP_AUTH_PAM_SERVICE"
#define VPOX_AUTH_PAM_SERVICE_NAME_ENV "VPOX_AUTH_PAM_SERVICE"
#define VPOX_AUTH_PAM_DEFAULT_SERVICE_NAME "login"


/* The debug log file name.
 *
 * If defined, debug messages will be written to the file specified in the
 * VPOX_AUTH_DEBUG_FILENAME (or deprecated VRDP_AUTH_DEBUG_FILENAME) environment
 * variable:
 *
 * export VPOX_AUTH_DEBUG_FILENAME=pam.log
 *
 * The above will cause writing to the pam.log.
 */
#define VPOX_AUTH_DEBUG_FILENAME_ENV_OLD "VRDP_AUTH_DEBUG_FILENAME"
#define VPOX_AUTH_DEBUG_FILENAME_ENV "VPOX_AUTH_DEBUG_FILENAME"
 ...
 * and does not harm.
 */
#define VPOX_AUTH_USE_PAM_DLLOAD


#ifdef VPOX_AUTH_USE_PAM_DLLOAD
...
# endif
#endif /* VPOX_AUTH_USE_PAM_DLLOAD */
...
{
#if defined(VPOX_AUTH_DEBUG_FILENAME_ENV) || defined(VPOX_AUTH_DEBUG_FILENAME_ENV_OLD)
    va_list va;
...
#if defined(VPOX_AUTH_DEBUG_FILENAME_ENV)
    filename = getenv (VPOX_AUTH_DEBUG_FILENAME_ENV);
#endif /* VPOX_AUTH_DEBUG_FILENAME_ENV */

#if defined(VPOX_AUTH_DEBUG_FILENAME_ENV_OLD)
    if (filename == NULL)
    {
        filename = getenv (VPOX_AUTH_DEBUG_FILENAME_ENV_OLD);
    }
#endif /* VPOX_AUTH_DEBUG_FILENAME_ENV_OLD */
    ...
    va_end (va);
#endif /* VPOX_AUTH_DEBUG_FILENAME_ENV || VPOX_AUTH_DEBUG_FILENAME_ENV_OLD */
}
...
{
    const char *service = getenv (VPOX_AUTH_PAM_SERVICE_NAME_ENV);

    if (service == NULL)
    {
        service = getenv (VPOX_AUTH_PAM_SERVICE_NAME_ENV_OLD);

        if (service == NULL)
        {
            service = VPOX_AUTH_PAM_DEFAULT_SERVICE_NAME;
        }
    }
        ...
        if (rc == PAM_SUCCESS)
        {
            ...
            if (rc == PAM_SUCCESS)
            {
                ...
                if (rc == PAM_AUTHINFO_UNAVAIL
                    &&
                    getenv("VPOX_PAM_ALLOW_INACTIVE") != NULL)
                {
