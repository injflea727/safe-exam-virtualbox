...
HRESULT ErrorInfo::getVirtualPoxErrorInfo(ComPtr<IVirtualPoxErrorInfo> &pVirtualPoxErrorInfo)
{
    HRESULT rc = S_OK;
    if (mErrorInfo)
        rc = mErrorInfo.queryInterfaceTo(pVirtualPoxErrorInfo.asOutParam());
    else
        pVirtualPoxErrorInfo.setNull();
    return rc;
}
...
void ErrorInfo::init(IVirtualPoxErrorInfo *info)
{
    ...
    ComPtr<IVirtualPoxErrorInfo> next;
