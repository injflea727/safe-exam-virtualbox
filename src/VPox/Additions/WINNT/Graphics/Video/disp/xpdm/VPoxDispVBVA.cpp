...
int VPoxDispVBVAInit(PVPOXDISPDEV pDev)
{
    ...
    int rc = VPoxDispMPQueryHGSMIInfo(pDev->hDriver, &info);
    if (RT_SUCCESS(rc))
    {
        HGSMIQUERYCALLBACKS callbacks;
        rc = VPoxDispMPQueryHGSMICallbacks(pDev->hDriver, &callbacks);
        if (RT_SUCCESS(rc))
        {
            HGSMIQUERYCPORTPROCS portProcs;
            rc = VPoxDispMPHGSMIQueryPortProcs(pDev->hDriver, &portProcs);
            if (RT_SUCCESS(rc))
            {
    ...
    if (pDev->hgsmi.bSupported)
    {
        ...
        DWORD dwrc = EngDeviceIoControl(pDev->hDriver, IOCTL_VIDEO_HGSMI_HANDLER_ENABLE, &HandlerReg, sizeof(HandlerReg),
                                        0, NULL, &cbReturned);
        VPOX_WARN_WINERR(dwrc);

#ifdef VPOX_WITH_VIDEOHWACCEL
        if (NO_ERROR == dwrc)
        {
            VPoxDispVHWAInit(pDev);
        }
#endif
    }
    ...
    {
        ULONG cbAvailable;
        VPOXDISPVRAMLAYOUT *vram = &pDev->layout;
    ...
    VPoxVBVASetupBufferContext(&pDev->vbvaCtx, pDev->layout.offVBVABuffer, pDev->layout.cbVBVABuffer);
...
void vbvaReportDirtyRect(PVPOXDISPDEV pDev, RECTL *pRectOrig)
{
    if (pDev && pRectOrig)
    {
        ...
        VPoxVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, &hdr, sizeof(hdr));
    }
}

static void vbvaReportDirtyPath(PVPOXDISPDEV pDev, PATHOBJ *ppo)
{
    RECTFX rcfxBounds;
    RECTL rclBounds;
...
static void vbvaReportDirtyClip(PVPOXDISPDEV pDev, CLIPOBJ *pco, RECTL *prcl)
{
    if (prcl)
    {
