...
int main (int argc, char* argv[])
{
    ...
    char szSetupStub[_MAX_PATH] = {"VPoxStub.exe"};
    char szOutput[_MAX_PATH] = {"VirtualPox-MultiArch.exe"};
    HANDLE hUpdate = NULL;

    do
    {
        printf(VPOX_PRODUCT " Stub Builder v%d.%d.%d.%d\n",
        ...
        VPOXSTUBBUILDPKG stbBuildPkg[VPOXSTUB_MAX_PACKAGES] = {0};
        VPOXSTUBPKG stbPkg[VPOXSTUB_MAX_PACKAGES] = {0};
        VPOXSTUBPKGHEADER stbHeader =
        {
            "vpox$tub",    /* File magic. */
            1,             /* Version. */
            0              /* No files yet. */
        ...
        for (int i=1; i<argc; i++)
        {
            ...
            else if (0 == stricmp(argv[i], "-target-all") && argc > i+1)
            {
                ...
                stbBuildPkg[stbHeader.byCntPkgs].byArch = VPOXSTUBPKGARCH_ALL;
                stbHeader.byCntPkgs++;
                i++;
            }
            ...
            {
                hr = ::StringCchCopy(stbBuildPkg[stbHeader.byCntPkgs].szSourcePath, _MAX_PATH, argv[i+1]);
                stbBuildPkg[stbHeader.byCntPkgs].byArch = VPOXSTUBPKGARCH_X86;
                stbHeader.byCntPkgs++;
                i++;
            }
            ...
            {
                hr = ::StringCchCopy(stbBuildPkg[stbHeader.byCntPkgs].szSourcePath, _MAX_PATH, argv[i+1]);
                stbBuildPkg[stbHeader.byCntPkgs].byArch = VPOXSTUBPKGARCH_AMD64;
                stbHeader.byCntPkgs++;
                i++;
            }
            else
        ...
        for (BYTE i=0; i<stbHeader.byCntPkgs; i++)
        {
            ...
            hr = UpdateResource(hUpdate, RT_RCDATA, szHeaderName, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), pPackage, sizeof(VPOXSTUBPKG));
        ...
        if (!::UpdateResource(hUpdate, RT_RCDATA, "MANIFEST", MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), &stbHeader, sizeof(VPOXSTUBPKGHEADER)))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
            break;
        }
