/* $Id: VPoxWatchdog.cpp $ */
/** @file
 * VPoxWatchdog.cpp - VirtualPox Watchdog.
 */
...
ComPtr<IVirtualPox>                 g_pVirtualPox = NULL;
 ...
 */
typedef struct VPOXWATCHDOGMOD
{
    ...
    bool            fEnabled;
} VPOXWATCHDOGMOD, *PVPOXWATCHDOGMOD;

static VPOXWATCHDOGMOD g_aModules[] =
{
...
static ComPtr<IEventListener>    g_pVPoxEventListener = NULL;
 ...
 */
class VirtualPoxEventListener
{
    public:
        VirtualPoxEventListener()
        {
        }

        virtual ~VirtualPoxEventListener()
        {
        }
        ...
        STDMETHOD(HandleEvent)(VPoxEventType_T aType, IEvent *aEvent)
        {
            switch (aType)
            {
                ...
                case VPoxEventType_OnVPoxSVCAvailabilityChanged:
                {
                    ...
                    if (!fAvailable)
                    {
                        serviceLog("VPoxSVC became unavailable\n");
                        watchdogShutdown();
                    }
                    else
                    {
                        serviceLog("VPoxSVC became available\n");
...
typedef ListenerImpl<VirtualPoxEventListener> VirtualPoxEventListenerImpl;
VPOX_LISTENER_DECLARE(VirtualPoxEventListenerImpl)
...
static int machineAdd(const Bstr &strUuid)
{
    ...
    do
    {
        ...
        CHECK_ERROR_BREAK(g_pVirtualPox, FindMachine(strUuid.raw(), machine.asOutParam()));
...
static int vmListBuild()
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        HRESULT hrc = g_pVirtualPox->COMGETTER(Machines)(ComSafeArrayAsOutParam(machines));
        if (SUCCEEDED(hrc))
        {
            /*
 ...
 *
 * @returns VPox status code, error message displayed.
 */
...
static int watchdogStartModules()
{
    ...
    for (unsigned j = 0; j < RT_ELEMENTS(g_aModules); j++)
    {
        const PVPOXWATCHDOGMOD pMod = &g_aModules[j];
        if (pMod->fEnabled)
        {
...
static RTEXITCODE watchdogMain(/*HandlerArg *a */)
{
    ...
    do
    {
         ...
         * - g_pEventSourceClient for VPoxClient events (like VPoxSVC handling)
         */
        CHECK_ERROR_BREAK(g_pVirtualPox, COMGETTER(EventSource)(g_pEventSource.asOutParam()));
        CHECK_ERROR_BREAK(g_pVirtualPoxClient, COMGETTER(EventSource)(g_pEventSourceClient.asOutParam()));

        ComObjPtr<VirtualPoxEventListenerImpl> vpoxListenerImpl;
        vpoxListenerImpl.createObject();
        vpoxListenerImpl->init(new VirtualPoxEventListener());

        com::SafeArray<VPoxEventType_T> eventTypes;
        eventTypes.push_back(VPoxEventType_OnMachineRegistered);
        eventTypes.push_back(VPoxEventType_OnMachineStateChanged);
        eventTypes.push_back(VPoxEventType_OnVPoxSVCAvailabilityChanged); /* Processed by g_pEventSourceClient. */

        g_pVPoxEventListener = vpoxListenerImpl;
        CHECK_ERROR_BREAK(g_pEventSource, RegisterListener(g_pVPoxEventListener, ComSafeArrayAsInParam(eventTypes), true /* Active listener */));
        CHECK_ERROR_BREAK(g_pEventSourceClient, RegisterListener(g_pVPoxEventListener, ComSafeArrayAsInParam(eventTypes), true /* Active listener */));
        ...
        if (g_pVPoxEventListener)
        {
            if (!g_pEventSource.isNull())
                CHECK_ERROR(g_pEventSource, UnregisterListener(g_pVPoxEventListener));
            g_pVPoxEventListener.setNull();
        }
...
{
    RTStrmPrintf(g_pStdErr, VPOX_PRODUCT " Watchdog " VPOX_VERSION_STRING "\n"
...
static void displayHelp(const char *pszImage)
{
    ...
    /** @todo Change VPOXBALLOONCTRL_RELEASE_LOG to WATCHDOG*. */
    RTStrmPrintf(g_pStdErr, "\nUse environment variable VPOXBALLOONCTRL_RELEASE_LOG for logging options.\n");
...
int main(int argc, char *argv[])
{
    ...
    rc = com::VPoxLogRelCreate("Watchdog", g_fDaemonize ? NULL : pszLogFile,
                               RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                               "all", "VPOXBALLOONCTRL_RELEASE_LOG",
    ...
    if (g_fDaemonize)
    {
        ...
        if (!pszLogFile || !*pszLogFile)
        {
            ...
            rc = RTPathAppend(szLogFile, sizeof(szLogFile), "vpoxballoonctrl.log");
            if (RT_FAILURE(rc))
        ...
        rc = com::VPoxLogRelCreate("Watchdog", pszLogFile,
                                   RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                                   "all", "VPOXBALLOONCTRL_RELEASE_LOG",
