/* $Id: VPoxServiceStats.cpp $ */
/** @file
 * VPoxStats - Guest statistics notification
 */
...
/** @page pg_vgsvc_vmstats VPoxService - VM Statistics
 *
...
#include <VPox/err.h>
#include <VPox/VMMDev.h> /* For VMMDevReportGuestStats and indirectly VbglR3StatReport. */
...
*********************************************************************************************************************************/
typedef struct _VPOXSTATSCONTEXT
{
...
*********************************************************************************************************************************/
/** Global data. */
static VPOXSTATSCONTEXT g_VMStat = {0};
...
static void vgsvcVMStatsReport(void)
{
    ...
    req.guestStats.u32StatCaps          = VPOX_GUEST_STAT_PHYS_MEM_TOTAL
                                        | VPOX_GUEST_STAT_PHYS_MEM_AVAIL
                                        | VPOX_GUEST_STAT_PAGE_FILE_SIZE
                                        | VPOX_GUEST_STAT_MEMORY_LOAD;
# ifdef VPOX_WITH_MEMBALLOON
    req.guestStats.u32PhysMemBalloon    = VGSvcBalloonQueryPages(_4K);
    req.guestStats.u32StatCaps         |= VPOX_GUEST_STAT_PHYS_MEM_BALLOON;
# else
...
# endif
    ...
    if (g_VMStat.pfnGetPerformanceInfo)
    {
        ...
        if (g_VMStat.pfnGetPerformanceInfo(&perfInfo, sizeof(perfInfo)))
        {
            ...
            req.guestStats.u32StatCaps |= VPOX_GUEST_STAT_PROCESSES | VPOX_GUEST_STAT_THREADS | VPOX_GUEST_STAT_HANDLES
                                       |  VPOX_GUEST_STAT_MEM_COMMIT_TOTAL | VPOX_GUEST_STAT_MEM_KERNEL_TOTAL
                                       |  VPOX_GUEST_STAT_MEM_KERNEL_PAGED | VPOX_GUEST_STAT_MEM_KERNEL_NONPAGED
                                       |  VPOX_GUEST_STAT_MEM_SYSTEM_CACHE;
        }
        else
        ...
        &&  cbReturned == cbStruct)
    {
        for (uint32_t i = 0; i < systemInfo.dwNumberOfProcessors; i++)
        {
            ...
            req.guestStats.u32StatCaps      |= VPOX_GUEST_STAT_CPU_LOAD_IDLE
                                            |  VPOX_GUEST_STAT_CPU_LOAD_KERNEL
                                            |  VPOX_GUEST_STAT_CPU_LOAD_USER;
...
DECLCALLBACK(int) vgsvcVMStatsWorker(bool volatile *pfShutdown)
{
    ...
    VGSvcVerbose(3, "VPoxStatsThread: finished statistics change request thread\n");
    return 0;
}
