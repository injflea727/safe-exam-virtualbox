/* $Id: VPoxNetBaseService.cpp $ */
/** @file
 * VPoxNetBaseService - common services for VPoxNetDHCP and VPoxNetNAT.
 */
...
*********************************************************************************************************************************/
struct VPoxNetBaseService::Data
{
...
DECLCALLBACK(int) VPoxNetBaseService::Data::recvLoop(RTTHREAD, void *pvUser)
{
    VPoxNetBaseService *pThis = static_cast<VPoxNetBaseService *>(pvUser);
...
VPoxNetBaseService::VPoxNetBaseService(const std::string& aName, const std::string& aNetworkName):m(NULL)
{
    m = new VPoxNetBaseService::Data(aName, aNetworkName);
...
VPoxNetBaseService::~VPoxNetBaseService()
{
    /*
...
int VPoxNetBaseService::init()
{
    if (isMainNeeded())
    {
        ...
        hrc = virtualpoxClient.createInprocObject(CLSID_VirtualPoxClient);
        AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);

        hrc = virtualpoxClient->COMGETTER(VirtualPox)(virtualpox.asOutParam());
        AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
    }
...
bool VPoxNetBaseService::isMainNeeded() const
{
    return m->m_fNeedMain;
}


int VPoxNetBaseService::run()
{
    /**
 ...
 */
int VPoxNetBaseService::parseArgs(int argc, char **argv)
{
        ...
        switch (rc)
        {
            ...
            case 'h': // --help (missed)
                RTPrintf("%s Version %sr%u\n"
                         "(C) 2009-" VPOX_C_YEAR " " VPOX_VENDOR "\n"
                         "All rights reserved.\n"
                         "\n"
                         "Usage: %s <options>\n"
                         "\n"
...
int VPoxNetBaseService::tryGoOnline(void)
{
    /*
    ...
    {
        m->m_pSession = NIL_RTR0PTR;
        LogRel(("VPoxNetBaseService: SUPR3Init -> %Rrc\n", rc));
        return rc;
    }
    ...
    if (RT_FAILURE(rc))
    {
        LogRel(("VPoxNetBaseService: RTPathExecDir -> %Rrc\n", rc));
        return rc;
    }
    ...
    if (RT_FAILURE(rc))
    {
        LogRel(("VPoxNetBaseService: SUPR3LoadVMM(\"%s\") -> %Rrc\n", szPath, rc));
        return rc;
    }
    ...
    if (RT_FAILURE(rc))
    {
        Log2(("VPoxNetBaseService: SUPR3CallVMMR0Ex(,VMMR0_DO_INTNET_OPEN,) failed, rc=%Rrc\n", rc));
        return rc;
    }
    m->m_hIf = OpenReq.hIf;
    ...
    if (RT_FAILURE(rc))
    {
        Log2(("VPoxNetBaseService: SUPR3CallVMMR0Ex(,VMMR0_DO_INTNET_IF_GET_BUFFER_PTRS,) failed, rc=%Rrc\n", rc));
        return rc;
    }
    ...
    /* bail out */
    Log2(("VPoxNetBaseService: SUPR3CallVMMR0Ex(,VMMR0_DO_INTNET_IF_SET_PROMISCUOUS_MODE,) failed, rc=%Rrc\n", rc));
...
void VPoxNetBaseService::shutdown(void)
{
    syncEnter();
    if (!m->fShutdown)
    {
...
int VPoxNetBaseService::syncEnter()
{
    return RTCritSectEnter(&m->m_csThis);
}


int VPoxNetBaseService::syncLeave()
{
    return RTCritSectLeave(&m->m_csThis);
}


int VPoxNetBaseService::waitForIntNetEvent(int cMillis)
{
...
int VPoxNetBaseService::abortWait()
{
...
/* S/G API */
int VPoxNetBaseService::sendBufferOnWire(PCINTNETSEG paSegs, size_t cSegs, size_t cbFrame)
{
    /* Allocate frame */
 ...
 */
void VPoxNetBaseService::flushWire()
{
...
int VPoxNetBaseService::hlpUDPBroadcast(unsigned uSrcPort, unsigned uDstPort,
                                         void const *pvData, size_t cbData) const
{
    return VPoxNetUDPBroadcast(m->m_pSession, m->m_hIf, m->m_pIfBuf,
...
const std::string VPoxNetBaseService::getServiceName() const
{
    return m->m_ServiceName;
}


void VPoxNetBaseService::setServiceName(const std::string& aName)
{
    m->m_ServiceName = aName;
}


const std::string VPoxNetBaseService::getNetworkName() const
{
    return m->m_NetworkName;
}


void VPoxNetBaseService::setNetworkName(const std::string& aName)
{
    m->m_NetworkName = aName;
}


const RTMAC VPoxNetBaseService::getMacAddress() const
{
    return m->m_MacAddress;
}


void VPoxNetBaseService::setMacAddress(const RTMAC& aMac)
{
    m->m_MacAddress = aMac;
}


const RTNETADDRIPV4 VPoxNetBaseService::getIpv4Address() const
{
    return m->m_Ipv4Address;
}


void VPoxNetBaseService::setIpv4Address(const RTNETADDRIPV4& aAddress)
{
    m->m_Ipv4Address = aAddress;
}


const RTNETADDRIPV4 VPoxNetBaseService::getIpv4Netmask() const
{
    return m->m_Ipv4Netmask;
}


void VPoxNetBaseService::setIpv4Netmask(const RTNETADDRIPV4& aNetmask)
{
    m->m_Ipv4Netmask = aNetmask;
}


uint32_t VPoxNetBaseService::getSendBufSize() const
{
    return m->m_cbSendBuf;
}


void VPoxNetBaseService::setSendBufSize(uint32_t cbBuf)
{
    m->m_cbSendBuf = cbBuf;
}


uint32_t VPoxNetBaseService::getRecvBufSize() const
{
    return m->m_cbRecvBuf;
}


void VPoxNetBaseService::setRecvBufSize(uint32_t cbBuf)
{
    m->m_cbRecvBuf = cbBuf;
}


int32_t VPoxNetBaseService::getVerbosityLevel() const
{
    return m->m_cVerbosity;
}


void VPoxNetBaseService::setVerbosityLevel(int32_t aVerbosity)
{
    m->m_cVerbosity = aVerbosity;
}


void VPoxNetBaseService::addCommandLineOption(const PRTGETOPTDEF optDef)
{
    m->m_vecOptionDefs.push_back(optDef);
}


void VPoxNetBaseService::doReceiveLoop()
{
    int rc;
    /* Well we're ready */
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
        {
            ...
            LogRel(("VPoxNetBaseService: waitForIntNetEvent returned %Rrc\n", rc));
            AssertRCReturnVoid(rc);
        }
            ...
            switch (u8Type)
            {
                case INTNETHDR_TYPE_FRAME:
                {
                    ...
                    if (RT_FAILURE(rc) && rc == VERR_IGNORED)
                    {
                        ...
                        VPOXNETUDPHDRS Hdrs;
                        size_t  cb;
                        void   *pv = VPoxNetUDPMatch(m->m_pIfBuf, RTNETIPV4_PORT_BOOTPS, &m->m_MacAddress,
                                                       VPOXNETUDP_MATCH_UNICAST
                                                     | VPOXNETUDP_MATCH_BROADCAST
                                                     | VPOXNETUDP_MATCH_CHECKSUM
                                                     | (m->m_cVerbosity > 2 ? VPOXNETUDP_MATCH_PRINT_STDERR : 0),
                                                     &Hdrs, &cb);
                        if (pv && cb)
                            processUDP(pv, cb);
                        else
                            VPoxNetArpHandleIt(m->m_pSession, m->m_hIf, m->m_pIfBuf, &m->m_MacAddress, m->m_Ipv4Address);
                    }
                    break;
                }
...
int VPoxNetBaseService::startReceiveThreadAndEnterEventLoop()
{
    ...
    int rc = RTThreadCreate(&m->m_hThrRecv, /* thread handle*/
                            &VPoxNetBaseService::Data::recvLoop,  /* routine */
                            this, /* user data */
                            128 * _1K, /* stack size */
...
void VPoxNetBaseService::debugPrint(int32_t iMinLevel, bool fMsg, const char *pszFmt, ...) const
{
 ...
 */
void VPoxNetBaseService::debugPrintV(int iMinLevel, bool fMsg, const char *pszFmt, va_list va) const
{
    RT_NOREF(fMsg);
    if (iMinLevel <= m->m_cVerbosity)
    {
...
PRTGETOPTDEF VPoxNetBaseService::getOptionsPtr()
{
