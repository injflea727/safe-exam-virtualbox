...
class BacktraceResolverOsLinux(BacktraceResolverOs):
    ...
    def prepareEnv(self):
        ...
        fRc = False;
        try:
            sDbgArchive = os.path.join(self.sBuildRoot, 'bin', 'VirtualPox-dbg.tar.bz2');
    ...
    def getBinaryListWithLoadAddrFromReport(self, asReport):
        ...
        for sLine in asReport[iLine:]:
            ...
            if     len(asCandidate) == 5 \
               ...
               and 'VirtualPox' in asCandidate[4]:
...
class BacktraceResolverOsDarwin(BacktraceResolverOs):
    ...
    def _getListOfBinariesFromReportPreSierra(self, asReport):
        ...
        if iLine < len(asReport):
            ...
            #     0x100042000 -        0x100095fff +VPoxDDU.dylib (4.3.15) <EB19C44D-F882-0803-DBDD-9995723111B7> /Application...
            ...
            #     /Applications/VirtualPox.app/Contents/MacOS
            oRegExpPath = re.compile(r'/VirtualPox.app/Contents/MacOS');
            oRegExpAddr = re.compile(r'0x\w+');
            oRegExpBinPath = re.compile(r'VirtualPox.app/Contents/MacOS/\S*');
            while iLine < len(asReport):
    ...
    def _getListOfBinariesFromReportSierra(self, asReport):
        ...
        # A line for a loaded binary looks like the following:
        #     4   VPoxXPCOMIPCC.dylib                 0x00000001139f17ea 0x1139e4000 + 55274
        # We need the start address and the library name.
        # To distinguish between our own libraries and ones from Apple we check whether the library
        # name contains VPox or VirtualPox
        iLine = 0;
        while iLine < len(asReport):
            ...
            if     len(asStackTrace) == 6 and asStackTrace[0].isdigit() \
               and (asStackTrace[1].find('VPox') != -1 or asStackTrace[1].find('VirtualPox') != -1) \
...
class BacktraceResolverOsSolaris(BacktraceResolverOs):
    ...
    def prepareEnv(self):
        ...
        fRc = False;
        try:
            sDbgArchive = os.path.join(self.sBuildRoot, 'bin', 'VirtualPoxDebug.tar.bz2');
    ...
    def getBinaryListWithLoadAddrFromReport(self, asReport):
        ...
        for sLine in asReport:
            asItems = sLine.split();
            if     len(asItems) == 4 \
               and asItems[3].startswith('/opt/VirtualPox') \
               and (   asItems[2] == 'r-x--' \
                    or asItems[2] == 'r-x----'):
                fFound = False;
