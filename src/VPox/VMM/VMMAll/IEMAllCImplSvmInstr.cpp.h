 ...
 */
DECLINLINE(VPOXSTRICTRC) iemSvmWorldSwitch(PVMCPUCC pVCpu)
{
    /*
...
IEM_STATIC VPOXSTRICTRC iemSvmVmexit(PVMCPUCC pVCpu, uint64_t uExitCode, uint64_t uExitInfo1, uint64_t uExitInfo2)
{
        ...
        || uExitCode == SVM_EXIT_INVALID)
    {
        ...
        {
            AssertMsgFailed(("iemSvmVmexit: Mapping VMCB at %#RGp failed. rc=%Rrc\n", pVCpu->cpum.GstCtx.hwvirt.svm.GCPhysVmcb, VPOXSTRICTRC_VAL(rcStrict)));
 ...
 */
IEM_STATIC VPOXSTRICTRC iemSvmVmrun(PVMCPUCC pVCpu, uint8_t cbInstr, RTGCPHYS GCPhysVmcb)
{
    LogFlow(("iemSvmVmrun\n"));
...
# endif
         ...
         * Update PGM, IEM and others of a world-switch.
         */
        VPOXSTRICTRC rcStrict = iemSvmWorldSwitch(pVCpu);
        if (rcStrict == VINF_SUCCESS)
        { /* likely */ }
        else if (RT_SUCCESS(rcStrict))
        {
        ...
        if (pEventInject->n.u1Valid)
        {
             ...
             * clearing it at any time, most likely before/after injecting the event. Since VirtualPox
 ...
 */
IEM_STATIC VPOXSTRICTRC iemSvmHandleIOIntercept(PVMCPUCC pVCpu, uint16_t u16Port, SVMIOIOTYPE enmIoType, uint8_t cbReg,
                                                uint8_t cAddrSizeBits, uint8_t iEffSeg, bool fRep, bool fStrIo, uint8_t cbInstr)
{
    ...
    /** @todo remove later (for debugging as VirtualPox always traps all IO
     *        intercepts). */
 ...
 */
IEM_STATIC VPOXSTRICTRC iemSvmHandleMsrIntercept(PVMCPUCC pVCpu, uint32_t idMsr, bool fWrite)
{
    /*
...
# else
    ...
    VPOXSTRICTRC rcStrict = iemSvmVmrun(pVCpu, cbInstr, GCPhysVmcb);
...
# else
    ...
    VPOXSTRICTRC rcStrict = PGMPhysSimpleReadGCPhys(pVCpu->CTX_SUFF(pVM), &VmcbNstGst, GCPhysVmcb + RT_UOFFSETOF(SVMVMCB, guest),
...
IEM_CIMPL_DEF_1(iemCImpl_Hypercall, uint16_t, uDisOpcode)
{
    ...
    {
        NOREF(uDisOpcode);
        VPOXSTRICTRC rcStrict = GIMHypercallEx(pVCpu, IEM_GET_CTX(pVCpu), uDisOpcode, cbInstr);
