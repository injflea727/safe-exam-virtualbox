/* $Id: VPoxPci-linux.c $ */
/** @file
 * VPoxPci - PCI Driver (Host), Linux Specific Code.
 */
...
# endif
#endif /* VPOX_WITH_IOMMU */


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int  __init VPoxPciLinuxInit(void);
static void __exit VPoxPciLinuxUnload(void);


/*********************************************************************************************************************************
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static VPOXRAWPCIGLOBALS g_VPoxPciGlobals;

module_init(VPoxPciLinuxInit);
module_exit(VPoxPciLinuxUnload);

MODULE_AUTHOR(VPOX_VENDOR);
MODULE_DESCRIPTION(VPOX_PRODUCT " PCI access Driver");
MODULE_LICENSE("GPL");
#ifdef MODULE_VERSION
MODULE_VERSION(VPOX_VERSION_STRING " r" RT_XSTR(VPOX_SVN_REV));
#endif
...
static int __init VPoxPciLinuxInit(void)
{
    ...
    LogRel(("VPoxPciLinuxInit\n"));

    RT_ZERO(g_VPoxPciGlobals);

    rc = vpoxPciInit(&g_VPoxPciGlobals);
    if (RT_FAILURE(rc))
    {
        LogRel(("cannot do VPoxPciInit: %Rc\n", rc));
        goto error;
    }

#if defined(CONFIG_PCI_STUB)
    /* nothing to do, pci_stub module part of the kernel */
    g_VPoxPciGlobals.fPciStubModuleAvail = true;
...
# if RTLNX_VER_MIN(2,6,30)
        ...
        g_VPoxPciGlobals.pciStubModule = find_module(PCI_STUB_MODULE_NAME);
        mutex_unlock(&module_mutex);
        if (g_VPoxPciGlobals.pciStubModule)
        {
            if (try_module_get(g_VPoxPciGlobals.pciStubModule))
                g_VPoxPciGlobals.fPciStubModuleAvail = true;
        }
        else
            printk(KERN_INFO "vpoxpci: find_module %s failed\n", PCI_STUB_MODULE);
# endif
    }
    else
        printk(KERN_INFO "vpoxpci: cannot load %s\n", PCI_STUB_MODULE);

#else
    printk(KERN_INFO "vpoxpci: %s module not available, cannot detach PCI devices\n",
                      PCI_STUB_MODULE);
#endif

#ifdef VPOX_WITH_IOMMU
    if (IOMMU_PRESENT())
        printk(KERN_INFO "vpoxpci: IOMMU found\n");
    else
        printk(KERN_INFO "vpoxpci: IOMMU not found (not registered)\n");
#else
    printk(KERN_INFO "vpoxpci: IOMMU not found (not compiled)\n");
#endif
...
static void __exit VPoxPciLinuxUnload(void)
{
    LogRel(("VPoxPciLinuxLinuxUnload\n"));

    /*
     * Undo the work done during start (in reverse order).
     */
    vpoxPciShutdown(&g_VPoxPciGlobals);

    RTR0Term();

    if (g_VPoxPciGlobals.pciStubModule)
    {
        module_put(g_VPoxPciGlobals.pciStubModule);
        g_VPoxPciGlobals.pciStubModule = NULL;
    }

    Log(("VPoxPciLinuxUnload - done\n"));
}

static int vpoxPciLinuxDevRegisterWithIommu(PVPOXRAWPCIINS pIns)
{
#ifdef VPOX_WITH_IOMMU
    int rc = VINF_SUCCESS;
    struct pci_dev *pPciDev = pIns->pPciDev;
    PVPOXRAWPCIDRVVM pData = VPOX_DRV_VMDATA(pIns);
...
static int vpoxPciLinuxDevUnregisterWithIommu(PVPOXRAWPCIINS pIns)
{
#ifdef VPOX_WITH_IOMMU
    int rc = VINF_SUCCESS;
    struct pci_dev *pPciDev = pIns->pPciDev;
...
static int vpoxPciLinuxDevReset(PVPOXRAWPCIINS pIns)
{
    int rc = VINF_SUCCESS;
    IPRT_LINUX_SAVE_EFL_AC();
...
static struct file* vpoxPciFileOpen(const char* path, int flags)
{
    ...
    {
        err = PTR_ERR(filp);
        printk(KERN_DEBUG "vpoxPciFileOpen: error %d\n", err);
        return NULL;
    }
...
static void  vpoxPciFileClose(struct file* file)
{
    filp_close(file, NULL);
}

static int vpoxPciFileWrite(struct file* file, unsigned long long offset, unsigned char* data, unsigned int size)
{
    int          ret;
    ...
    set_fs(fs_save);
    if (ret < 0)
        printk(KERN_DEBUG "vpoxPciFileWrite: error %d\n", ret);

    return ret;
}

static int vpoxPciLinuxDevDetachHostDriver(PVPOXRAWPCIINS pIns)
{
    ...
    if (!g_VPoxPciGlobals.fPciStubModuleAvail)
    {
        printk(KERN_INFO "vpoxpci: stub module %s not detected: cannot detach\n",
               PCI_STUB_MODULE);
    ...
    if (!pPciDev)
    {
        printk(KERN_INFO "vpoxpci: device at %02x:%02x.%d not found\n",
    ...
    printk(KERN_DEBUG "vpoxpci: detected device: %04x:%04x at %02x:%02x.%d, driver %s\n",
    ...
    if (fDetach && currentDriver)
    {
        ...
        {
            printk(KERN_DEBUG "vpoxpci: ERROR: %s contains invalid symbols\n", currentDriver);
            return VERR_ACCESS_DENIED;
        }
...
# endif
        ...
        RTStrPrintf(szFileBuf, cMaxBuf,
                              ...
                              PCI_STUB_MODULE);
        pFile = vpoxPciFileOpen(szFileBuf, O_WRONLY);
        if (pFile)
        {
            ...
            vpoxPciFileWrite(pFile, 0, szCmdBuf, iCmdLen);
            vpoxPciFileClose(pFile);
        }
        else
            printk(KERN_DEBUG "vpoxpci: cannot open %s\n", szFileBuf);
...
static int vpoxPciLinuxDevReattachHostDriver(PVPOXRAWPCIINS pIns)
{
    struct pci_dev *pPciDev = pIns->pPciDev;
...
DECLHIDDEN(int) vpoxPciOsDevInit(PVPOXRAWPCIINS pIns, uint32_t fFlags)
{
    struct pci_dev *pPciDev = NULL;
    int rc = VINF_SUCCESS;
    IPRT_LINUX_SAVE_EFL_AC();
    ...
    {
        rc = vpoxPciLinuxDevDetachHostDriver(pIns);
        if (RT_FAILURE(rc))
        {
...
DECLHIDDEN(int) vpoxPciOsDevDeinit(PVPOXRAWPCIINS pIns, uint32_t fFlags)
{
    int rc = VINF_SUCCESS;
    struct pci_dev *pPciDev = pIns->pPciDev;
    IPRT_LINUX_SAVE_EFL_AC();
    ...
    if (RT_LIKELY(pPciDev))
    {
        ...
        vpoxPciLinuxDevUnregisterWithIommu(pIns);
        ...
        vpoxPciLinuxDevReattachHostDriver(pIns);
...
DECLHIDDEN(int) vpoxPciOsDevDestroy(PVPOXRAWPCIINS pIns)
{
    return VINF_SUCCESS;
}

DECLHIDDEN(int) vpoxPciOsDevGetRegionInfo(PVPOXRAWPCIINS pIns,
                                          int32_t        iRegion,
...
DECLHIDDEN(int) vpoxPciOsDevMapRegion(PVPOXRAWPCIINS pIns,
                                      int32_t        iRegion,
                                      ...
                                      RTR0PTR        *pRegionBase)
{
    ...
    if (!pIns->aRegionR0Mapping[iRegion])
    {
        ...
        *pRegionBase = pIns->aRegionR0Mapping[iRegion];
        ...
        rcLnx = pci_request_region(pPciDev, iRegion, "vpoxpci");
        if (!rcLnx)
        {
...
DECLHIDDEN(int) vpoxPciOsDevUnmapRegion(PVPOXRAWPCIINS pIns,
                                        int32_t        iRegion,
                                        RTHCPHYS       RegionStart,
                                        uint64_t       u64RegionSize,
                                        RTR0PTR        RegionBase)
...
DECLHIDDEN(int) vpoxPciOsDevPciCfgWrite(PVPOXRAWPCIINS pIns, uint32_t Register, PCIRAWMEMLOC *pValue)
{
    struct pci_dev *pPciDev = pIns->pPciDev;
    int rc = VINF_SUCCESS;
    IPRT_LINUX_SAVE_EFL_AC();
...
DECLHIDDEN(int) vpoxPciOsDevPciCfgRead(PVPOXRAWPCIINS pIns, uint32_t Register, PCIRAWMEMLOC *pValue)
{
    struct pci_dev *pPciDev = pIns->pPciDev;
    int rc = VINF_SUCCESS;
 ...
 * @param   pvDevId         The device ID, a pointer to PVPOXRAWPCIINS.
 * @param   pRegs           Register set. Removed in 2.6.19.
 */
#if RTLNX_VER_MIN(2,6,19) && !defined(DOXYGEN_RUNNING)
static irqreturn_t vpoxPciOsIrqHandler(int iIrq, void *pvDevId)
#else
static irqreturn_t vpoxPciOsIrqHandler(int iIrq, void *pvDevId, struct pt_regs *pRegs)
#endif
{
    PVPOXRAWPCIINS pIns = (PVPOXRAWPCIINS)pvDevId;
    bool fTaken = true;

    if (pIns && pIns->IrqHandler.pfnIrqHandler)
        fTaken = pIns->IrqHandler.pfnIrqHandler(pIns->IrqHandler.pIrqContext, iIrq);
#ifndef VPOX_WITH_SHARED_PCI_INTERRUPTS
...
DECLHIDDEN(int) vpoxPciOsDevRegisterIrqHandler(PVPOXRAWPCIINS pIns, PFNRAWPCIISR pfnHandler, void* pIrqContext, int32_t *piHostIrq)
{
    int rc;
...
DECLHIDDEN(int) vpoxPciOsDevUnregisterIrqHandler(PVPOXRAWPCIINS pIns, int32_t iHostIrq)
{
    IPRT_LINUX_SAVE_EFL_AC();
...
DECLHIDDEN(int) vpoxPciOsDevPowerStateChange(PVPOXRAWPCIINS pIns, PCIRAWPOWERSTATE  aState)
{
    int rc;
...
static DECLCALLBACK(int) vpoxPciOsContigMemInfo(PRAWPCIPERVM pVmCtx, RTHCPHYS HostStart, RTGCPHYS GuestStart,
                                                uint64_t cMemSize, PCIRAWMEMINFOACTION Action)
{
    struct iommu_domain* domain = ((PVPOXRAWPCIDRVVM)(pVmCtx->pDriverData))->pIommuDomain;
    int rc = VINF_SUCCESS;
    IPRT_LINUX_SAVE_EFL_AC();
    ...
    switch (Action)
    {
        case PCIRAW_MEMINFO_MAP:
        {
            ...
            if (r)
            {
                printk(KERN_ERR "vpoxPciOsContigMemInfo:"
...
DECLHIDDEN(int) vpoxPciOsInitVm(PVPOXRAWPCIDRVVM pThis, PVM pVM, PRAWPCIPERVM pVmData)
{
    int rc = VINF_SUCCESS;
    ...
    if (IOMMU_PRESENT())
    {
        ...
        {
            pVmData->pfnContigMemInfo = vpoxPciOsContigMemInfo;
...
DECLHIDDEN(void) vpoxPciOsDeinitVm(PVPOXRAWPCIDRVVM pThis, PVM pVM)
{
#ifdef VPOX_WITH_IOMMU
    IPRT_LINUX_SAVE_EFL_AC();
