/* $Id: VPoxNetFltInstall.cpp $ */
/** @file
 * NetFltInstall - VPoxNetFlt installer command line tool
 */
...
#define VPOX_NETCFG_APP_NAME L"NetFltInstall"
            ...
            if (hr == S_OK)
            {
                ...
                if (dwSize > 0)
                {
                    ...
                    if (dwSize > 0)
                    {
                        ...
                        {
                            wprintf(L"error installing VPoxNetFlt (0x%x)\n", hr);
                        }
                    }
                    else
                    {
                        hr = HRESULT_FROM_WIN32(GetLastError());
                        wprintf(L"error getting full inf path for VPoxNetFltM.inf (0x%x)\n", hr);
                    }
                }
                else
                {
                    hr = HRESULT_FROM_WIN32(GetLastError());
                    wprintf(L"error getting full inf path for VPoxNetFlt.inf (0x%x)\n", hr);
                }
