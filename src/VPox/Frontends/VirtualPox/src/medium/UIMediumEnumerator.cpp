...
/** @file
 * VPox Qt GUI - UIMediumEnumerator class implementation.
 */
    ...
    , m_fMediumEnumerationInProgress(false)
{
    ...
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigStorageControllerChange,
            this, &UIMediumEnumerator::sltHandleStorageControllerChange);
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigStorageDeviceChange,
            this, &UIMediumEnumerator::sltHandleStorageDeviceChange);
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigMediumChange,
            this, &UIMediumEnumerator::sltHandleMediumChange);
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigMediumConfigChange,
            this, &UIMediumEnumerator::sltHandleMediumConfigChange);
    connect(gVPoxEvents, &UIVirtualPoxEventHandler::sigMediumRegistered,
...
void UIMediumEnumerator::enumerateMedia(const CMediumVector &comMedia /* = CMediumVector() */)
{
    ...
    if (comMedia.isEmpty())
    {
        ...
        addMediaToMap(uiCommon().virtualPox().GetHardDisks(), guiMedia);
        addMediaToMap(uiCommon().host().GetDVDDrives(), guiMedia);
        addMediaToMap(uiCommon().virtualPox().GetDVDImages(), guiMedia);
        addMediaToMap(uiCommon().host().GetFloppyDrives(), guiMedia);
        addMediaToMap(uiCommon().virtualPox().GetFloppyImages(), guiMedia);
    }
    else
    {
...
void UIMediumEnumerator::sltHandleMediumRegistered(const QUuid &uMediumId, KDeviceType enmMediumType, bool fRegistered)
{
    ...
    if (fRegistered)
    {
        ...
        {
            /* Get VPox for temporary usage, it will cache the error info: */
            ...
            CMedium comMedium = comVPox.OpenMedium(uMediumId.toString(), enmMediumType, KAccessMode_ReadWrite, false);
            if (!comVPox.isOk())
                LogRel(("GUI: UIMediumEnumerator:  Unable to open registered medium! %s\n",
                        UIErrorString::simplifiedErrorInfo(comVPox).toUtf8().constData()));
            else
            {
                /* Create new UIMedium: */
