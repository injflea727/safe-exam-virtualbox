...
/** @file
 * VPox Qt GUI - UIWizardNewVD class implementation.
 */
...
bool UIWizardNewVD::createVirtualDisk()
{
    ...
    CMedium virtualDisk = vpox.CreateMedium(mediumFormat.GetName(), strMediumPath, KAccessMode_ReadWrite, KDeviceType_HardDisk);
    if (!vpox.isOk())
    {
        msgCenter().cannotCreateHardDiskStorage(vpox, strMediumPath, this);
        return false;
    }
