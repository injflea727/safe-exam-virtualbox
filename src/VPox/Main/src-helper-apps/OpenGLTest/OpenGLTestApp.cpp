...
/** @file
 * VPox host opengl support test application.
 */
...
#define VPOXGLTEST_WITH_LOGGING

#ifdef VPOXGLTEST_WITH_LOGGING
 ...
 */
DECLINLINE(PFNRT) vpoxTestOglGetProc(const char *pszSymbol)
{
    int rc;
...
static int vpoxCheck3DAccelerationSupported()
{
    LogRel(("Testing 3D Support:\n"));

    for (uint32_t i = 0; i < RT_ELEMENTS(g_apszOglMethods); i++)
    {
        PFNRT pfn = vpoxTestOglGetProc(g_apszOglMethods[i]);
        if (!pfn)
        {
...
#ifdef VPOX_WITH_VIDEOHWACCEL
static int vpoxCheck2DVideoAccelerationSupported()
{
...
#ifdef VPOXGLTEST_WITH_LOGGING
static int vpoxInitLogging(const char *pszFilename, bool bGenNameSuffix)
{
    ...
    int vrc = RTLogCreateEx(&loggerRelease, fFlags, "all",
                            "VPOX_RELEASE_LOG", RT_ELEMENTS(s_apszGroups), s_apszGroups, UINT32_MAX, enmLogDest,
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        RTLogRelLogger(loggerRelease, 0, ~0U,
                       "VPoxTestGL %s r%u %s (%s %s) release log\n"
#ifdef VPOX_BLEEDING_EDGE
                       "EXPERIMENTAL build " VPOX_BLEEDING_EDGE "\n"
#endif
                       "Log opened %s\n",
                       VPOX_VERSION_STRING, RTBldCfgRevision(), VPOX_BUILD_TARGET,
...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
        ...
        rc = vpoxCheck3DAccelerationSupported();
    }
    else
    {
...
#endif
                case 'h':
                    RTPrintf(VPOX_PRODUCT " Helper for testing 2D/3D OpenGL capabilities %u.%u.%u\n"
                             ...
                             "\n"
                             "Logging can alternatively be enabled by specifying the VPOXGLTEST_LOG=<log_file_name> env variable\n"
        ...
        if(!rc)
        {
            ...
            if(!bLog)
            {
                /* check the VPOXGLTEST_LOG env var */
                pLog = RTEnvGet("VPOXGLTEST_LOG");
                if(pLog)
                    bLog = true;
                bLogSuffix = true;
            }
            if(bLog)
                rc = vpoxInitLogging(pLog, bLogSuffix);
            else
#endif
                rc = vpoxInitQuietMode();
...
#ifdef VPOX_WITH_VIDEOHWACCEL
            if(!rc && bTest2D)
                rc = vpoxCheck2DVideoAccelerationSupported();
#endif
