...
sudo "${MY_DIR}/vpoxdrv.sh" stop

if [ -z "${OPT_UNLOAD_ONLY}" ]; then
    # Build and load.
    MAKE_JOBS=`grep vendor_id /proc/cpuinfo | wc -l`
    if [ "${MAKE_JOBS}" -le "0" ]; then MAKE_JOBS=1; fi
    make "-j${MAKE_JOBS}" -C "${MY_DIR}/src/vpoxdrv" "$@"

    echo "Installing SUPDrv (aka VPoxDrv/vpoxdrv)"
    sudo /sbin/insmod "${MY_DIR}/src/vpoxdrv/vpoxdrv.ko"
fi
