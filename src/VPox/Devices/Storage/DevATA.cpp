/* $Id: DevATA.cpp $ */
/** @file
 * VPox storage devices: ATA/ATAPI controller device (disk and cdrom).
 */
...
#undef VPOX_INSTRUMENT_DMA_WRITES
...
/** The saved state version used by VirtualPox 3.0.
 * This lacks the config part and has the type at the and.  */
#define ATA_SAVED_STATE_VERSION_VPOX_30                 19
...
#ifdef VPOX_INSTRUMENT_DMA_WRITES
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (s->uTxDir == PDMMEDIATXDIR_FROM_DEVICE)
        {
                ...
                && s->fOverwriteInquiry)
            {
                ...
                scsiPadStr(&s->abIOBuffer[8], "VPOX", 8);
...
# if 1/*ndef VPOX*/  /** @todo implement MESN + AENC. (async notification on removal and stuff.) */
    pbBuf[2] = 0x00; /* ISO */
...
static VPOXSTRICTRC ataIOPortWriteU8(PPDMDEVINS pDevIns, PATACONTROLLER pCtl, uint32_t addr, uint32_t val, uintptr_t iCtl)
{
    RT_NOREF(iCtl);
...
static VPOXSTRICTRC ataIOPortReadU8(PPDMDEVINS pDevIns, PATACONTROLLER pCtl, uint32_t addr, uint32_t *pu32)
{
    ...
    switch (addr & 7)
    {
        ...
        case 7: /* primary status */
        {
             ...
             * this is no guarantee for any other VPox thread getting
...
ataIOPortWrite1Data(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t u32, unsigned cb)
{
    ...
    VPOXSTRICTRC rc = PDMDevHlpCritSectEnter(pDevIns, &pCtl->lock, VINF_IOM_R3_IOPORT_WRITE);
    if (rc == VINF_SUCCESS)
    {
        PATADEVSTATE s = &pCtl->aIfs[pCtl->iSelectedIf & ATA_SELECTED_IF_MASK];
        ...
        Log3(("%s: addr=%#x val=%.*Rhxs rc=%d\n", __FUNCTION__, offPort + pCtl->IOPortBase1, cb, &u32, VPOXSTRICTRC_VAL(rc)));
        PDMDevHlpCritSectLeave(pDevIns, &pCtl->lock);
    }
    else
        Log3(("%s: addr=%#x -> %d\n", __FUNCTION__, offPort + pCtl->IOPortBase1, VPOXSTRICTRC_VAL(rc)));
    return rc;
}
...
ataIOPortRead1Data(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t *pu32, unsigned cb)
{
    ...
    *pu32 = 0;
    ...
    VPOXSTRICTRC rc = PDMDevHlpCritSectEnter(pDevIns, &pCtl->lock, VINF_IOM_R3_IOPORT_READ);
    if (rc == VINF_SUCCESS)
    {
        PATADEVSTATE s = &pCtl->aIfs[pCtl->iSelectedIf & ATA_SELECTED_IF_MASK];
        ...
        Log3(("%s: addr=%#x val=%.*Rhxs rc=%d\n", __FUNCTION__, offPort, cb, pu32, VPOXSTRICTRC_VAL(rc)));

        PDMDevHlpCritSectLeave(pDevIns, &pCtl->lock);
    }
    else
        Log3(("%s: addr=%#x -> %d\n", __FUNCTION__, offPort, VPOXSTRICTRC_VAL(rc)));
...
static DECLCALLBACK(int) ataR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (   uVersion != ATA_SAVED_STATE_VERSION
        && uVersion != ATA_SAVED_STATE_VERSION_VPOX_30
     ...
     */
    if (uVersion > ATA_SAVED_STATE_VERSION_VPOX_30)
    {
        uint8_t u8Type;
...
#ifdef VPOX_WITH_STATISTICS /** @todo release too. */
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    rc = pHlp->pfnCFGMQueryStringDef(pCfgNode, "ModelNumber", pIf->szModelNumber, sizeof(pIf->szModelNumber),
                                                     pIf->fATAPI ? "VPOX CD-ROM" : "VPOX HARDDISK");
                    if (RT_FAILURE(rc))
                    {
                        if (rc == VERR_CFGM_NOT_ENOUGH_SPACE)
                            return PDMDEV_SET_ERROR(pDevIns, VERR_INVALID_PARAMETER,
                    ...
                    if (pIf->fATAPI)
                    {
                        rc = pHlp->pfnCFGMQueryStringDef(pCfgNode, "ATAPIVendorId", pIf->szInquiryVendorId,
                                                         sizeof(pIf->szInquiryVendorId), "VPOX");
                        if (RT_FAILURE(rc))
                        {
                            if (rc == VERR_CFGM_NOT_ENOUGH_SPACE)
                                return PDMDEV_SET_ERROR(pDevIns, VERR_INVALID_PARAMETER,
