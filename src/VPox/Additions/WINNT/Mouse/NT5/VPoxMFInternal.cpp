/* $Id: VPoxMFInternal.cpp $ */
/** @file
 * VPox Mouse Filter Driver - Internal functions.
 *
 * @todo r=bird: Would be better to merge this file into VPoxMFDriver.cpp...
 */
...
*********************************************************************************************************************************/
typedef struct _VPoxGlobalContext
{
    ...
    KSPIN_LOCK SyncLock;
    volatile PVPOXMOUSE_DEVEXT pCurrentDevExt;
...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static VPoxGlobalContext g_ctx = {};
...
/**
 * @callback_method_impl{FNVPOXGUESTMOUSENOTIFY}
 */
static DECLCALLBACK(void) vpoxNewProtMouseEventCb(void *pvUser)
{
    RT_NOREF(pvUser);
    PVPOXMOUSE_DEVEXT pDevExt = (PVPOXMOUSE_DEVEXT)ASMAtomicUoReadPtr((void * volatile *)&g_ctx.pCurrentDevExt);
    if (pDevExt)
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            VPoxDrvNotifyServiceCB(pDevExt, &g_ctx.LastReportedData, &g_ctx.LastReportedData + 1, &InputDataConsumed);
 ...
 * Lazy init callback.
 *
 * We don't have control over when VPoxGuest.sys is loaded and therefore cannot
 ...
 */
static bool vpoxNewProtLazyRegister(void)
{
    if (g_ctx.fIsNewProtEnabled)
        return true;
    int rc = VbglR0SetMouseNotifyCallback(vpoxNewProtMouseEventCb, NULL);
    if (RT_SUCCESS(rc))
    {
        g_ctx.fIsNewProtEnabled = true;
        LOG(("Successfully register mouse event callback with VPoxGuest."));
        return true;
    }
 ...
 * @returns NT status.
 */
static void vpoxNewProtTerm(void)
{
    Assert(IsListEmpty(&g_ctx.DevExtList));
    if (g_ctx.fIsNewProtEnabled)
    {
...
/**
 * Worker for VPoxDeviceAdded that enables callback processing of pDevExt.
 *
 * @param   pDevExt             The device instance that was added.
 */
static void vpoxNewProtDeviceAdded(PVPOXMOUSE_DEVEXT pDevExt)
{
    /*
     ...
     */
    vpoxNewProtLazyRegister();
}

/**
 * Worker for VPoxDeviceRemoved that disables callback processing of pDevExt.
 *
 * @param   pDevExt             The device instance that is being removed.
 */
static void vpoxNewProtDeviceRemoved(PVPOXMOUSE_DEVEXT pDevExt)
{
    /*
...
VOID VPoxDrvNotifyServiceCB(PVPOXMOUSE_DEVEXT pDevExt, PMOUSE_INPUT_DATA InputDataStart, PMOUSE_INPUT_DATA InputDataEnd,
...
void VPoxDeviceAdded(PVPOXMOUSE_DEVEXT pDevExt)
{
    ...
    if (cCalls == 1)
    {
        ...
        if (!vpoxIsVBGLInited() && !vpoxIsVBGLInitFailed())
        {
    ...
    vpoxNewProtDeviceAdded(pDevExt);
...
void VPoxInformHost(PVPOXMOUSE_DEVEXT pDevExt)
{
    ...
    if (vpoxIsVBGLInited())
    {
        ...
        if (pDevExt->bHostMouse && !vpoxIsHostInformed())
        {
        ...
        /* Preallocate request to be used in VPoxServiceCB*/
...
VOID VPoxDeviceRemoved(PVPOXMOUSE_DEVEXT pDevExt)
{
     ...
     */
    vpoxNewProtDeviceRemoved(pDevExt);

    /*
     * Free the preallocated request.
     * Note! This could benefit from merging with vpoxNewProtDeviceRemoved to
     ...
     * Note! This sequence could potentially be racing VPoxDeviceAdded, depending
