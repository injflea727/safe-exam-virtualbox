...
AssertCompileSize(MsixTableRecord, VPOX_MSIX_ENTRY_SIZE);
...
{
    return PCIDevGetWord(pDev, pDev->Int.s.u8MsixCapOffset + VPOX_MSIX_CAP_MESSAGE_CONTROL);
}

DECLINLINE(bool)      msixIsEnabled(PPDMPCIDEV pDev)
{
    return (msixGetMessageControl(pDev) & VPOX_PCI_MSIX_FLAGS_ENABLE) != 0;
}

DECLINLINE(bool)      msixIsMasked(PPDMPCIDEV pDev)
{
    return (msixGetMessageControl(pDev) & VPOX_PCI_MSIX_FLAGS_FUNCMASK) != 0;
}
...
{
    return (MsixTableRecord *)msixGetPageOffset(pDev, iVector * VPOX_MSIX_ENTRY_SIZE);
}
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) msixR3MMIORead(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void *pv, unsigned cb)
{
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) msixR3MMIOWrite(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void const *pv, unsigned cb)
{
    PPDMPCIDEV pPciDev = (PPDMPCIDEV)pvUser;
    LogFlowFunc(("off=%RGp cb=%d %#010RX32\n", off, cb, *(uint32_t *)pv));
    ...
    *(uint32_t *)&pPciDev->abMsixState[off] = *(uint32_t *)pv;
    ...
    msixR3CheckPendingVector(pDevIns, (PCPDMPCIHLP)pPciDev->Int.s.pvPciBusPtrR3, pPciDev, off / VPOX_MSIX_ENTRY_SIZE);
    return VINF_SUCCESS;
}
...
int MsixR3Init(PCPDMPCIHLP pPciHlp, PPDMPCIDEV pDev, PPDMMSIREG pMsiReg)
{
    ...
    AssertMsgReturn(cVectors <= VPOX_MSIX_MAX_ENTRIES, ("Too many MSI-X vectors: %d\n", cVectors), VERR_TOO_MUCH_DATA);
    ...
    pDev->Int.s.u8MsixCapSize   = VPOX_MSIX_CAP_SIZE;
    ...
    PCIDevSetByte(pDev,  iCapOffset + 0, VPOX_PCI_CAP_ID_MSIX);
    PCIDevSetByte(pDev,  iCapOffset + 1, iNextOffset); /* next */
    PCIDevSetWord(pDev,  iCapOffset + VPOX_MSIX_CAP_MESSAGE_CONTROL, cVectors - 1);

    PCIDevSetDWord(pDev,  iCapOffset + VPOX_MSIX_TABLE_BIROFFSET, offTable | iBar);
    PCIDevSetDWord(pDev,  iCapOffset + VPOX_MSIX_PBA_BIROFFSET,   offPBA   | iBar);
    ...
    for (uint32_t i = 0; i < len; i++)
    {
        ...
        switch (reg)
        {
            ...
            case 1: /* Next pointer,  ro */
                break;
            case VPOX_MSIX_CAP_MESSAGE_CONTROL:
                /* don't change read-only bits: 0-7 */
                break;
            case VPOX_MSIX_CAP_MESSAGE_CONTROL + 1:
            {
                ...
                fJustEnabled |= msixR3BitJustCleared(pDev->abConfig[uAddr], u8NewVal, VPOX_PCI_MSIX_FLAGS_ENABLE >> 8);
                fJustEnabled |= msixR3BitJustCleared(pDev->abConfig[uAddr], u8NewVal, VPOX_PCI_MSIX_FLAGS_FUNCMASK >> 8);
