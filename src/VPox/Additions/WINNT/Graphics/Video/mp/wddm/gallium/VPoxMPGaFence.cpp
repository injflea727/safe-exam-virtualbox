...
void GaFenceObjectsDestroy(VPOXWDDM_EXT_GA *pGaDevExt,
    ...
    RTListForEachSafe(&pGaDevExt->fenceObjects.list, pIter, pNext, GAFENCEOBJECT, node)
    {
            ...
            || pIter->pDevice == pDevice)
        {
            ...
            GaIdFree(pGaDevExt->fenceObjects.au32HandleBits, sizeof(pGaDevExt->fenceObjects.au32HandleBits),
                     VPOXWDDM_GA_MAX_FENCE_OBJECTS, pIter->u32FenceHandle);
...
static void gaFenceDelete(VPOXWDDM_EXT_GA *pGaDevExt, GAFENCEOBJECT *pFO)
{
    ...
    GaIdFree(pGaDevExt->fenceObjects.au32HandleBits, sizeof(pGaDevExt->fenceObjects.au32HandleBits),
             VPOXWDDM_GA_MAX_FENCE_OBJECTS, pFO->u32FenceHandle);
...
static void gaFenceDeleteLocked(VPOXWDDM_EXT_GA *pGaDevExt, GAFENCEOBJECT *pFO)
{
    GALOG(("u32FenceHandle = %d, pFO %p\n", pFO->u32FenceHandle, pFO));
...
DECLINLINE(void) gaFenceUnref(VPOXWDDM_EXT_GA *pGaDevExt, GAFENCEOBJECT *pFO)
{
...
void GaFenceUnrefLocked(VPOXWDDM_EXT_GA *pGaDevExt, GAFENCEOBJECT *pFO)
{
    uint32_t c = ASMAtomicDecU32(&pFO->cRefs);
    Assert(c < UINT32_MAX / 2);
    if (c == 0)
...
GAFENCEOBJECT *GaFenceLookup(VPOXWDDM_EXT_GA *pGaDevExt,
...
NTSTATUS GaFenceCreate(PVPOXWDDM_EXT_GA pGaDevExt,
                       PVPOXWDDM_DEVICE pDevice,
...
NTSTATUS GaFenceQuery(PVPOXWDDM_EXT_GA pGaDevExt,
                      uint32_t u32FenceHandle,
...
NTSTATUS GaFenceWait(PVPOXWDDM_EXT_GA pGaDevExt,
                     uint32_t u32FenceHandle,
...
NTSTATUS GaFenceDelete(PVPOXWDDM_EXT_GA pGaDevExt,
                       uint32_t u32FenceHandle)
{
    gaFenceObjectsLock(pGaDevExt);
