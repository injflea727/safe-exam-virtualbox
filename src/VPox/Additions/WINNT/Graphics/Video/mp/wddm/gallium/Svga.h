/* $Id: Svga.h $ */
/** @file
 * VirtualPox Windows Guest Mesa3D - Gallium driver VMSVGA.
 */
...
#include "VPoxMPGaUtils.h"

#include <iprt/assert.h>
#include <iprt/avl.h>
#include <iprt/list.h>

#include <VPoxGaTypes.h>
...
typedef struct VPOXWDDM_EXT_VMSVGA
{
...
typedef struct VPOXWDDM_EXT_VMSVGA *PVPOXWDDM_EXT_VMSVGA;
...
DECLINLINE(void) SvgaHostObjectsLock(VPOXWDDM_EXT_VMSVGA *pSvga, KIRQL *pOldIrql)
{
    KeAcquireSpinLock(&pSvga->HostObjectsSpinLock, pOldIrql);
}

DECLINLINE(void) SvgaHostObjectsUnlock(VPOXWDDM_EXT_VMSVGA *pSvga, KIRQL OldIrql)
{
    KeReleaseSpinLock(&pSvga->HostObjectsSpinLock, OldIrql);
}

NTSTATUS SvgaHostObjectsCleanup(VPOXWDDM_EXT_VMSVGA *pSvga);
...
NTSTATUS SvgaSurfaceCreate(VPOXWDDM_EXT_VMSVGA *pSvga,
                           GASURFCREATE *pCreateParms,
                           GASURFSIZE *paSizes,
                           uint32_t cSizes,
                           uint32_t *pu32Sid);
NTSTATUS SvgaSurfaceUnref(VPOXWDDM_EXT_VMSVGA *pSvga,
                          uint32_t u32Sid);
SURFACEOBJECT *SvgaSurfaceObjectQuery(VPOXWDDM_EXT_VMSVGA *pSvga,
                                      uint32_t u32Sid);
...
NTSTATUS SvgaSharedSidRemove(VPOXWDDM_EXT_VMSVGA *pSvga,
                             uint32_t u32Sid);
uint32_t SvgaSharedSidGet(VPOXWDDM_EXT_VMSVGA *pSvga,
                          uint32_t u32Sid);
