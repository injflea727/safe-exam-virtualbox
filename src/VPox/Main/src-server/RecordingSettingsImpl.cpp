...
/** @file
 *
 * VirtualPox COM class implementation - Machine capture settings.
 */
...
HRESULT RecordingSettings::i_copyFrom(RecordingSettings *aThat)
{
    ...
    /* sanity */
    AutoCaller autoCaller(this);
    AssertComRCReturn(autoCaller.rc(), VPOX_E_INVALID_OBJECT_STATE);

    /* sanity too */
    AutoCaller thatCaller(aThat);
    AssertComRCReturn(thatCaller.rc(), VPOX_E_INVALID_OBJECT_STATE);
...
int RecordingSettings::i_getDefaultFilename(Utf8Str &strFile, uint32_t idScreen, bool fWithFileExtension)
{
    ...
    strFile = m->pMachine->i_getSettingsFileFull(); // path/to/machinesfolder/vmname/vmname.vpox
    strFile.stripSuffix();
