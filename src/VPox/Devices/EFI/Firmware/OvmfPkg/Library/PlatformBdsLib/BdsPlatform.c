...
--*/
{
  ...
  BdsLibConnectAll ();
#ifdef VPOX
    {
        ...
        DEBUG ((EFI_D_INFO, "------------------ VPox Platform Specific Initialization Start -----------------------\n"));
        ...
        if (!BootOption0080)
        {
            rc = gBS->LocateHandleBuffer (ByProtocol,
                                          ...
                                          &cFileSystem,
                                          &phFileSystem);
            VPoxLogFlowFuncMarkRC(rc);
            VPoxLogFlowFuncMarkVar(cFileSystem, "%d");
            if (   rc == EFI_SUCCESS
                && cFileSystem > 0)
            {
                ...
                for (iFileSystem = 0; iFileSystem < cFileSystem; ++iFileSystem)
                {
                    ...
                    rc = gBS->HandleProtocol (phFileSystem[iFileSystem],
                                              &gEfiSimpleFileSystemProtocolGuid,
                                              (VOID *) &pFSVolume);
                    VPoxLogFlowFuncMarkVar(iFileSystem, "%d");
                    VPoxLogFlowFuncMarkRC(rc);
                    if (EFI_ERROR(rc))
                        continue;
                    ...
                    VPoxLogFlowFuncMarkVar(pDevicePath,"%p");
                    if (!pDevicePath)
                        continue;
        ...
        {
            VPoxLogFlowFuncMarkVar(BootOption0080->LoadOptionsSize, "%d");
            if (BootOption0080->LoadOptionsSize)
                VPoxLogFlowFuncMarkVar(BootOption0080->LoadOptions, "%s");
#if 0
        ...
        DEBUG ((EFI_D_INFO, "------------------ VPox Platform Specific Initialization End -----------------------\n"));
    }
#endif
