...
static DECLCALLBACK(int32_t) usbProxyDarwinInitOnce(void *pvUser1)
{
    ...
    if (krc == KERN_SUCCESS)
    {
       g_pRunLoopMode = CFStringCreateWithCString(kCFAllocatorDefault, "VPoxUsbProxyMode", kCFStringEncodingUTF8);
       if (g_pRunLoopMode)
           return VINF_SUCCESS;
...
static DECLCALLBACK(int) usbProxyDarwinOpen(PUSBPROXYDEV pProxyDev, const char *pszAddress, void *pvBackend)
{
    ...
    /*
     * Call the USBLib init to make sure we're a valid VPoxUSB client.
