/* $Id: VirtualPoxImpl.cpp $ */
/** @file
 * Implementation of IVirtualPox in VPoxSVC.
 */
...
#define LOG_GROUP LOG_GROUP_MAIN_VIRTUALPOX
#include <iprt/asm.h>
...
#include "VPox/com/MultiResult.h"
...
# include <VPox/GuestHost/SharedClipboard-transfers.h>
#endif
...
#endif /* VPOX_WITH_RESOURCE_USAGE_API */
#include "EventImpl.h"
...
#define VPOX_GLOBAL_SETTINGS_FILE "VirtualPox.xml"
...
// static
com::Utf8Str VirtualPox::sVersion;

// static
com::Utf8Str VirtualPox::sVersionNormalized;

// static
ULONG VirtualPox::sRevision;

// static
com::Utf8Str VirtualPox::sPackageType;

// static
com::Utf8Str VirtualPox::sAPIVersion;

// static
std::map<com::Utf8Str, int> VirtualPox::sNatNetworkNameToRefCount;

// static leaked (todo: find better place to free it.)
RWLockHandle *VirtualPox::spMtxNatNetworkNameToRefCountLock;
...
/**
 *  Abstract callback event class to asynchronously call VirtualPox callbacks
 ...
 *
 *  @note The VirtualPox instance passed to the constructor is strongly
 *  referenced, so that the VirtualPox singleton won't be released until the
 *  event gets handled by the event thread.
 */
class VirtualPox::CallbackEvent : public Event
{
public:

    CallbackEvent(VirtualPox *aVirtualPox, VPoxEventType_T aWhat)
        : mVirtualPox(aVirtualPox), mWhat(aWhat)
    {
        Assert(aVirtualPox);
    }
    ...
    virtual HRESULT prepareEventDesc(IEventSource* aSource, VPoxEventDesc& aEvDesc) = 0;

private:

    /**
     *  Note that this is a weak ref -- the CallbackEvent handler thread
     *  is bound to the lifetime of the VirtualPox instance, so it's safe.
     */
    VirtualPox         *mVirtualPox;
protected:
    VPoxEventType_T     mWhat;
};

////////////////////////////////////////////////////////////////////////////////
//
// VirtualPox private member data definition
//
////////////////////////////////////////////////////////////////////////////////

#if defined(RT_OS_WINDOWS) && defined(VPOXSVC_WITH_CLIENT_WATCHER)
/**
...
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
/**
 * Structure for keeping Shared Clipboard area data within the VirtualPox object.
 */
...
/**
 * Structure for keeping global Shared Clipboard data within the VirtualPox object.
 */
...
struct VirtualPox::Data
{
    ...
    // longer need the main VirtualPox object lock, but only the respective list
    ...
    // see VirtualPox::rememberMachineNameChangeForMedia())
...
DEFINE_EMPTY_CTOR_DTOR(VirtualPox)

HRESULT VirtualPox::FinalConstruct()
{
    LogRelFlowThisFuncEnter();
    LogRel(("VirtualPox: object creation starts\n"));
    ...
    LogRel(("VirtualPox: object created\n"));

    return rc;
}

void VirtualPox::FinalRelease()
{
    LogRelFlowThisFuncEnter();
    LogRel(("VirtualPox: object deletion starts\n"));

    uninit();

    BaseFinalRelease();

    LogRel(("VirtualPox: object deleted\n"));
    LogRelFlowThisFuncLeave();
}
...
/**
 *  Initializes the VirtualPox object.
 *
 *  @return COM result code
 */
HRESULT VirtualPox::init()
{
    LogRelFlowThisFuncEnter();
    ...
    if (sVersionNormalized.isEmpty())
    {
        ...
        if (tmp.endsWith(VPOX_BUILD_PUBLISHER))
            tmp = tmp.substr(0, tmp.length() - strlen(VPOX_BUILD_PUBLISHER));
        sVersionNormalized = tmp;
    }
    sRevision = RTBldCfgRevision();
    if (sPackageType.isEmpty())
        sPackageType = VPOX_PACKAGE_STRING;
    if (sAPIVersion.isEmpty())
        sAPIVersion = VPOX_API_VERSION_STRING;
    if (!spMtxNatNetworkNameToRefCountLock)
        spMtxNatNetworkNameToRefCountLock = new RWLockHandle(LOCKCLASS_VIRTUALPOXOBJECT);
    ...
    try
    {
        /* Get the VirtualPox home directory. */
        {
            char szHomeDir[RTPATH_MAX];
            int vrc = com::GetVPoxUserHomeDirectory(szHomeDir, sizeof(szHomeDir));
            if (RT_FAILURE(vrc))
                throw setErrorBoth(E_FAIL, vrc,
                                   tr("Could not create the VirtualPox home directory '%s' (%Rrc)"),
                                   szHomeDir, vrc);
        ...
        /* compose the VirtualPox.xml file name */
        unconst(m->strSettingsFilePath) = Utf8StrFmt("%s%c%s",
                                                     m->strHomeDir.c_str(),
                                                     RTPATH_DELIMITER,
                                                     VPOX_GLOBAL_SETTINGS_FILE);
        // load and parse VirtualPox.xml; this will throw on XML or logic errors
        try
        {
        ...
        if (SUCCEEDED(rc))
            rc = m->ptrExtPackManager->initExtPackManager(this, VPOXEXTPACKCTX_PER_USER_DAEMON);
        if (FAILED(rc))
            throw rc;
#endif
    ...
    {
        lock.release();
        m->ptrExtPackManager->i_callAllVirtualPoxReadyHooks();
    }
#endif
...
HRESULT VirtualPox::initMachines()
{
 ...
 *
 *  --  VirtualPox::init(), to load the global media registry from VirtualPox.xml;
 *
 *  --  Machine::loadMachineDataFromSettings(), to load the per-machine registry
 *      from machine XML, for machines created with VirtualPox 4.0 or later.
 *
 ...
 *       transient UUID created at VirtualPox startup for the global registry or
 *       a machine ID.
 * @param mediaRegistry The XML settings structure to load, either from VirtualPox.xml
 *       or a machine XML.
 * @param strMachineFolder The folder of the machine.
 * @return
 */
HRESULT VirtualPox::initMedia(const Guid &uuidRegistry,
                              const settings::MediaRegistry &mediaRegistry,
                              const Utf8Str &strMachineFolder)
{
    LogFlow(("VirtualPox::initMedia ENTERING, uuidRegistry=%s, strMachineFolder=%s\n",
    ...
    LogFlow(("VirtualPox::initMedia LEAVING\n"));

    return S_OK;
}

void VirtualPox::uninit()
{
    ...
    if (m->pHost)
    {
        /* It is necessary to hold the VirtualPox and Host locks here because
...
// Wrapped IVirtualPox properties
/////////////////////////////////////////////////////////////////////////////
HRESULT VirtualPox::getVersion(com::Utf8Str &aVersion)
{
    aVersion = sVersion;
    return S_OK;
}

HRESULT VirtualPox::getVersionNormalized(com::Utf8Str &aVersionNormalized)
{
    aVersionNormalized = sVersionNormalized;
    return S_OK;
}

HRESULT VirtualPox::getRevision(ULONG *aRevision)
{
    *aRevision = sRevision;
    return S_OK;
}

HRESULT VirtualPox::getPackageType(com::Utf8Str &aPackageType)
{
    aPackageType = sPackageType;
    return S_OK;
}

HRESULT VirtualPox::getAPIVersion(com::Utf8Str &aAPIVersion)
{
    aAPIVersion = sAPIVersion;
    return S_OK;
}

HRESULT VirtualPox::getAPIRevision(LONG64 *aAPIRevision)
{
    AssertCompile(VPOX_VERSION_MAJOR < 128 && VPOX_VERSION_MAJOR > 0);
    AssertCompile((uint64_t)VPOX_VERSION_MINOR < 256);
    uint64_t uRevision = ((uint64_t)VPOX_VERSION_MAJOR << 56)
                       | ((uint64_t)VPOX_VERSION_MINOR << 48)
                       | ((uint64_t)VPOX_VERSION_BUILD << 40);
...
HRESULT VirtualPox::getHomeFolder(com::Utf8Str &aHomeFolder)
{
...
HRESULT VirtualPox::getSettingsFilePath(com::Utf8Str &aSettingsFilePath)
{
...
HRESULT VirtualPox::getHost(ComPtr<IHost> &aHost)
{
...
HRESULT VirtualPox::getSystemProperties(ComPtr<ISystemProperties> &aSystemProperties)
{
...
HRESULT VirtualPox::getMachines(std::vector<ComPtr<IMachine> > &aMachines)
{
...
HRESULT VirtualPox::getMachineGroups(std::vector<com::Utf8Str> &aMachineGroups)
{
...
HRESULT VirtualPox::getHardDisks(std::vector<ComPtr<IMedium> > &aHardDisks)
{
...
HRESULT VirtualPox::getDVDImages(std::vector<ComPtr<IMedium> > &aDVDImages)
{
...
HRESULT VirtualPox::getFloppyImages(std::vector<ComPtr<IMedium> > &aFloppyImages)
{
...
HRESULT VirtualPox::getProgressOperations(std::vector<ComPtr<IProgress> > &aProgressOperations)
{
...
HRESULT VirtualPox::getGuestOSTypes(std::vector<ComPtr<IGuestOSType> > &aGuestOSTypes)
{
...
HRESULT VirtualPox::getSharedFolders(std::vector<ComPtr<ISharedFolder> > &aSharedFolders)
{
...
HRESULT VirtualPox::getPerformanceCollector(ComPtr<IPerformanceCollector> &aPerformanceCollector)
{
#ifdef VPOX_WITH_RESOURCE_USAGE_API
    ...
    return S_OK;
#else /* !VPOX_WITH_RESOURCE_USAGE_API */
    NOREF(aPerformanceCollector);
    ReturnComNotImplemented();
#endif /* !VPOX_WITH_RESOURCE_USAGE_API */
}

HRESULT VirtualPox::getDHCPServers(std::vector<ComPtr<IDHCPServer> > &aDHCPServers)
{
...
HRESULT VirtualPox::getNATNetworks(std::vector<ComPtr<INATNetwork> > &aNATNetworks)
{
#ifdef VPOX_WITH_NAT_SERVICE
...
HRESULT VirtualPox::getEventSource(ComPtr<IEventSource> &aEventSource)
{
...
HRESULT VirtualPox::getExtensionPackManager(ComPtr<IExtPackManager> &aExtensionPackManager)
{
    HRESULT hrc = S_OK;
#ifdef VPOX_WITH_EXTPACK
...
HRESULT VirtualPox::getInternalNetworks(std::vector<com::Utf8Str> &aInternalNetworks)
{
...
HRESULT VirtualPox::getGenericNetworkDrivers(std::vector<com::Utf8Str> &aGenericNetworkDrivers)
{
 ...
 */
#ifdef VPOX_WITH_CLOUD_NET
HRESULT VirtualPox::i_findCloudNetworkByName(const com::Utf8Str &aNetworkName,
                                             ComObjPtr<CloudNetwork> *aNetwork)
{
    HRESULT rc = VPOX_E_OBJECT_NOT_FOUND;
...
HRESULT VirtualPox::createCloudNetwork(const com::Utf8Str &aNetworkName,
...
HRESULT VirtualPox::findCloudNetworkByName(const com::Utf8Str &aNetworkName,
                                           ComPtr<ICloudNetwork> &aNetwork)
{
#ifdef VPOX_WITH_CLOUD_NET
...
HRESULT VirtualPox::removeCloudNetwork(const ComPtr<ICloudNetwork> &aNetwork)
{
#ifdef VPOX_WITH_CLOUD_NET
    Bstr name;
    ...
    {
        AutoWriteLock vpoxLock(this COMMA_LOCKVAL_SRC_POS);
        rc = i_saveSettings();
        vpoxLock.release();
...
HRESULT VirtualPox::getCloudNetworks(std::vector<ComPtr<ICloudNetwork> > &aCloudNetworks)
{
#ifdef VPOX_WITH_CLOUD_NET
...
#ifdef VPOX_WITH_CLOUD_NET
HRESULT VirtualPox::i_getEventSource(ComPtr<IEventSource>& aSource)
{
...
HRESULT VirtualPox::getCloudProviderManager(ComPtr<ICloudProviderManager> &aCloudProviderManager)
{
    HRESULT hrc = m->pCloudProviderManager.queryInterfaceTo(aCloudProviderManager.asOutParam());
    return hrc;
}

HRESULT VirtualPox::checkFirmwarePresent(FirmwareType_T aFirmwareType,
                                         ...
                                         BOOL *aResult)
{
    ...
    firmwareDesc[] =
    {
        ...
        {   FirmwareType_EFI32,   true,  "VPoxEFI32.fd",   NULL },
        {   FirmwareType_EFI64,   true,  "VPoxEFI64.fd",   NULL },
        {   FirmwareType_EFIDUAL, true,  "VPoxEFIDual.fd", NULL },
#else
        {   FirmwareType_EFI32,   false, "VPoxEFI32.fd",   "http://virtualpox.org/firmware/VPoxEFI32.fd" },
        {   FirmwareType_EFI64,   false, "VPoxEFI64.fd",   "http://virtualpox.org/firmware/VPoxEFI64.fd" },
        {   FirmwareType_EFIDUAL, false, "VPoxEFIDual.fd", "http://virtualpox.org/firmware/VPoxEFIDual.fd" },
#endif
    };
    ...
    for (size_t i = 0; i < sizeof(firmwareDesc) / sizeof(firmwareDesc[0]); i++)
    {
        ...
        AssertRCReturn(rc, VPOX_E_IPRT_ERROR);
        ...
        char szVPoxPath[RTPATH_MAX];
        rc = RTPathExecDir(szVPoxPath, RTPATH_MAX);
        AssertRCReturn(rc, VPOX_E_IPRT_ERROR);
        rc = RTPathAppend(szVPoxPath, sizeof(szVPoxPath), firmwareDesc[i].pszFileName);
        if (RTFileExists(szVPoxPath))
        {
            *aResult = TRUE;
            aFile = szVPoxPath;
            break;
...
// Wrapped IVirtualPox methods
/////////////////////////////////////////////////////////////////////////////

/* Helper for VirtualPox::ComposeMachineFilename */
static void sanitiseMachineFilename(Utf8Str &aName);

HRESULT VirtualPox::composeMachineFilename(const com::Utf8Str &aName,
                                           ...
                                           com::Utf8Str       &aFile)
{
    ...
    if (strGroup == "/")
        strGroup.setNull();
    aFile = com::Utf8StrFmt("%s%s%c%s%c%s.vpox",
                            strBase.c_str(),
                            strGroup.c_str(),
                            RTPATH_DELIMITER,
                            strDirName.c_str(),
...
HRESULT VirtualPox::createMachine(const com::Utf8Str &aSettingsFile,
...
HRESULT VirtualPox::openMachine(const com::Utf8Str &aSettingsFile,
                                ComPtr<IMachine> &aMachine)
{
    HRESULT rc = E_FAIL;
...
/** @note Locks objects! */
HRESULT VirtualPox::registerMachine(const ComPtr<IMachine> &aMachine)
{
    HRESULT rc;
...
HRESULT VirtualPox::findMachine(const com::Utf8Str &aSettingsFile,
                                ComPtr<IMachine> &aMachine)
{
    LogFlowThisFuncEnter();
    ...
    if (id.isValid() && !id.isZero())
        ...
        rc = i_findMachine(id,
                           ...
                           true /* setError */,
                           &pMachineFound);
        // returns VPOX_E_OBJECT_NOT_FOUND if not found and sets error
    else
    {
...
HRESULT VirtualPox::getMachinesByGroups(const std::vector<com::Utf8Str> &aGroups,
...
HRESULT VirtualPox::getMachineStates(const std::vector<ComPtr<IMachine> > &aMachines,
...
HRESULT VirtualPox::createUnattendedInstaller(ComPtr<IUnattended> &aUnattended)
{
#ifdef VPOX_WITH_UNATTENDED
...
HRESULT VirtualPox::createMedium(const com::Utf8Str &aFormat,
...
HRESULT VirtualPox::openMedium(const com::Utf8Str &aLocation,
                               DeviceType_T aDeviceType,
                               AccessMode_T aAccessMode,
                               BOOL aForceNewUuid,
                               ComPtr<IMedium> &aMedium)
{
    ...
    if (pMedium.isNull())
    {
        ...
        {
            if (rc != VPOX_E_INVALID_OBJECT_STATE)
                rc = VPOX_E_OBJECT_NOT_FOUND;
        }
    }
...
HRESULT VirtualPox::getGuestOSType(const com::Utf8Str &aId,
...
HRESULT VirtualPox::createSharedFolder(const com::Utf8Str &aName,
...
HRESULT VirtualPox::removeSharedFolder(const com::Utf8Str &aName)
{
    NOREF(aName);
    return setError(E_NOTIMPL, "Not yet implemented");
}
 ...
 *  @note Locks this object for reading.
 */
HRESULT VirtualPox::getExtraDataKeys(std::vector<com::Utf8Str> &aKeys)
{
 ...
 *  @note Locks this object for reading.
 */
HRESULT VirtualPox::getExtraData(const com::Utf8Str &aKey,
                                 com::Utf8Str &aValue)
{
 ...
 */
HRESULT VirtualPox::setExtraData(const com::Utf8Str &aKey,
                                 const com::Utf8Str &aValue)
{
    ...
    if ((fChanged = (strOldValue != strValue)))
    {
        ...
        // onExtraDataCanChange() only briefly requests the VirtualPox
 ...
 *
 */
HRESULT VirtualPox::setSettingsSecret(const com::Utf8Str &aPassword)
{
...
int VirtualPox::i_decryptMediumSettings(Medium *pMedium)
{
    Bstr bstrCipher;
 ...
 */
int VirtualPox::i_decryptSettings()
{
    bool fFailure = false;
    AutoReadLock al(m->allHardDisks.getLockHandle() COMMA_LOCKVAL_SRC_POS);
 ...
 */
int VirtualPox::i_encryptSetting(const Utf8Str &aPlaintext, Utf8Str *aCiphertext)
{
 ...
 */
int VirtualPox::i_decryptSetting(Utf8Str *aPlaintext, const Utf8Str &aCiphertext)
{
 ...
 */
int VirtualPox::i_encryptSettingBytes(const uint8_t *aPlaintext, uint8_t *aCiphertext,
 ...
 */
int VirtualPox::i_decryptSettingBytes(uint8_t *aPlaintext,
 ...
 */
void VirtualPox::i_storeSettingsKey(const Utf8Str &aKey)
{
...
#ifdef DEBUG
void VirtualPox::i_dumpAllBackRefs()
{
    {
        AutoReadLock al(m->allHardDisks.getLockHandle() COMMA_LOCKVAL_SRC_POS);
        for (MediaList::const_iterator mt = m->allHardDisks.begin();
 ...
 */
HRESULT VirtualPox::i_postEvent(Event *event)
{
    ...
    if (SUCCEEDED((rc = autoCaller.rc())))
    {
        if (getObjectState().getState() != ObjectState::Ready)
            Log1WarningFunc(("VirtualPox has been uninitialized (state=%d), the event is discarded!\n",
 ...
 */
HRESULT VirtualPox::i_addProgress(IProgress *aProgress)
{
 ...
 * @note Doesn't lock objects.
 */
HRESULT VirtualPox::i_removeProgress(IN_GUID aId)
{
    AutoCaller autoCaller(this);
    if (FAILED(autoCaller.rc())) return autoCaller.rc();
...
public:
    ...
    void handler()
    {
        VirtualPox::i_SVCHelperClientThreadTask(this);
    }

    const ComPtr<Progress>& GetProgressObject() const {return progress;}

    bool init(VirtualPox* aVpox,
              Progress* aProgress,
              bool aPrivileged,
              VirtualPox::SVCHelperClientFunc aFunc,
              void *aUser)
    {
        LogFlowFuncEnter();
        that = aVpox;
    ...
    bool initialized;
    ComObjPtr<VirtualPox> that;
    ComObjPtr<Progress> progress;
    bool privileged;
    VirtualPox::SVCHelperClientFunc func;
    void *user;
 ...
 */
HRESULT VirtualPox::i_startSVCHelperClient(bool aPrivileged,
 ...
 */
/* static */
void VirtualPox::i_SVCHelperClientThreadTask(StartSVCHelperClientData *pTask)
{
    LogFlowFuncEnter();
    HRESULT rc = S_OK;
    ...
    do
    {
        ...
        /* protect VirtualPox from uninitialization */
        ...
        id.create();
        SVCHlpClient client;
        vrc = client.create(Utf8StrFmt("VirtualPox\\SVCHelper\\{%RTuuid}",
                                       id.raw()).c_str());
        if (RT_FAILURE(vrc))
        {
 ...
 *  @note Doesn't lock anything.
 */
void VirtualPox::i_updateClientWatcher()
{
    AutoCaller autoCaller(this);
 ...
 *  @note Doesn't lock anything.
 */
void VirtualPox::i_addProcessToReap(RTPROCESS pid)
{
    AutoCaller autoCaller(this);
    AssertComRCReturnVoid(autoCaller.rc());
...
struct MachineEvent : public VirtualPox::CallbackEvent
{
    MachineEvent(VirtualPox *aVB, VPoxEventType_T aWhat, const Guid &aId, BOOL aBool)
        : CallbackEvent(aVB, aWhat), id(aId.toUtf16())
        , mBool(aBool)
        { }

    MachineEvent(VirtualPox *aVB, VPoxEventType_T aWhat, const Guid &aId, MachineState_T aState)
        : CallbackEvent(aVB, aWhat), id(aId.toUtf16())
        , mState(aState)
        {}

    virtual HRESULT prepareEventDesc(IEventSource* aSource, VPoxEventDesc& aEvDesc)
    {
        switch (mWhat)
        {
            case VPoxEventType_OnMachineDataChanged:
                aEvDesc.init(aSource, mWhat, id.raw(), mBool);
                break;

            case VPoxEventType_OnMachineStateChanged:
                aEvDesc.init(aSource, mWhat, id.raw(), mState);
                break;

            case VPoxEventType_OnMachineRegistered:
                aEvDesc.init(aSource, mWhat, id.raw(), mBool);
                break;
 ...
 * VD plugin load
 */
int VirtualPox::i_loadVDPlugin(const char *pszPluginLibrary)
{
 ...
 * VD plugin unload
 */
int VirtualPox::i_unloadVDPlugin(const char *pszPluginLibrary)
{
...
struct MediumRegisteredEventStruct : public VirtualPox::CallbackEvent
{
    MediumRegisteredEventStruct(VirtualPox *aVB, const Guid &aMediumId,
                          const DeviceType_T aDevType, const BOOL aRegistered)
        : CallbackEvent(aVB, VPoxEventType_OnMediumRegistered)
        , mMediumId(aMediumId.toUtf16()), mDevType(aDevType), mRegistered(aRegistered)
    {}

    virtual HRESULT prepareEventDesc(IEventSource *aSource, VPoxEventDesc &aEvDesc)
    {
        return aEvDesc.init(aSource, VPoxEventType_OnMediumRegistered, mMediumId.raw(), mDevType, mRegistered);
    }
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onMediumRegistered(const Guid &aMediumId, const DeviceType_T aDevType, const BOOL aRegistered)
{
    i_postEvent(new MediumRegisteredEventStruct(this, aMediumId, aDevType, aRegistered));
}

/** Event for onMediumConfigChanged() */
struct MediumConfigChangedEventStruct : public VirtualPox::CallbackEvent
{
    MediumConfigChangedEventStruct(VirtualPox *aVB, IMedium *aMedium)
        : CallbackEvent(aVB, VPoxEventType_OnMediumConfigChanged)
        , mMedium(aMedium)
    {}

    virtual HRESULT prepareEventDesc(IEventSource *aSource, VPoxEventDesc &aEvDesc)
    {
        return aEvDesc.init(aSource, VPoxEventType_OnMediumConfigChanged, mMedium);
    }

    IMedium* mMedium;
};

void VirtualPox::i_onMediumConfigChanged(IMedium *aMedium)
{
    i_postEvent(new MediumConfigChangedEventStruct(this, aMedium));
}

/** Event for onMediumChanged() */
struct MediumChangedEventStruct : public VirtualPox::CallbackEvent
{
    MediumChangedEventStruct(VirtualPox *aVB, IMediumAttachment *aMediumAttachment)
        : CallbackEvent(aVB, VPoxEventType_OnMediumChanged)
        , mMediumAttachment(aMediumAttachment)
    {}

    virtual HRESULT prepareEventDesc(IEventSource *aSource, VPoxEventDesc &aEvDesc)
    {
        return aEvDesc.init(aSource, VPoxEventType_OnMediumChanged, mMediumAttachment);
    }

    IMediumAttachment* mMediumAttachment;
};

void VirtualPox::i_onMediumChanged(IMediumAttachment *aMediumAttachment)
{
    i_postEvent(new MediumChangedEventStruct(this, aMediumAttachment));
}

/** Event for onStorageControllerChanged() */
struct StorageControllerChangedEventStruct : public VirtualPox::CallbackEvent
{
    StorageControllerChangedEventStruct(VirtualPox *aVB, const Guid &aMachineId,
                                        const com::Utf8Str &aControllerName)
        : CallbackEvent(aVB, VPoxEventType_OnStorageControllerChanged)
        , mMachineId(aMachineId.toUtf16()), mControllerName(aControllerName)
    {}

    virtual HRESULT prepareEventDesc(IEventSource *aSource, VPoxEventDesc &aEvDesc)
    {
        return aEvDesc.init(aSource, VPoxEventType_OnStorageControllerChanged, mMachineId.raw(), mControllerName.raw());
    }
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onStorageControllerChanged(const Guid &aMachineId, const com::Utf8Str &aControllerName)
{
    i_postEvent(new StorageControllerChangedEventStruct(this, aMachineId, aControllerName));
}

/** Event for onStorageDeviceChanged() */
struct StorageDeviceChangedEventStruct : public VirtualPox::CallbackEvent
{
    StorageDeviceChangedEventStruct(VirtualPox *aVB, IMediumAttachment *aStorageDevice, BOOL fRemoved, BOOL fSilent)
        : CallbackEvent(aVB, VPoxEventType_OnStorageDeviceChanged)
    ...
    virtual HRESULT prepareEventDesc(IEventSource *aSource, VPoxEventDesc &aEvDesc)
    {
        return aEvDesc.init(aSource, VPoxEventType_OnStorageDeviceChanged, mStorageDevice, mRemoved, mSilent);
    }
...
void VirtualPox::i_onStorageDeviceChanged(IMediumAttachment *aStorageDevice, const BOOL fRemoved, const BOOL fSilent)
{
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onMachineStateChange(const Guid &aId, MachineState_T aState)
{
    i_postEvent(new MachineEvent(this, VPoxEventType_OnMachineStateChanged, aId, aState));
}

/**
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onMachineDataChange(const Guid &aId, BOOL aTemporary)
{
    i_postEvent(new MachineEvent(this, VPoxEventType_OnMachineDataChanged, aId, aTemporary));
}

/**
 *  @note Locks this object for reading.
 */
BOOL VirtualPox::i_onExtraDataCanChange(const Guid &aId, IN_BSTR aKey, IN_BSTR aValue,
                                        Bstr &aError)
{
    ...
    VPoxEventDesc evDesc;
    evDesc.init(m->pEventSource, VPoxEventType_OnExtraDataCanChange, id.raw(), aKey, aValue);
...
struct ExtraDataEvent : public VirtualPox::CallbackEvent
{
    ExtraDataEvent(VirtualPox *aVB, const Guid &aMachineId,
                   IN_BSTR aKey, IN_BSTR aVal)
        : CallbackEvent(aVB, VPoxEventType_OnExtraDataChanged)
        , machineId(aMachineId.toUtf16()), key(aKey), val(aVal)
    {}

    virtual HRESULT prepareEventDesc(IEventSource* aSource, VPoxEventDesc& aEvDesc)
    {
        return aEvDesc.init(aSource, VPoxEventType_OnExtraDataChanged, machineId.raw(), key.raw(), val.raw());
    }
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onExtraDataChange(const Guid &aId, IN_BSTR aKey, IN_BSTR aValue)
{
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onMachineRegistered(const Guid &aId, BOOL aRegistered)
{
    i_postEvent(new MachineEvent(this, VPoxEventType_OnMachineRegistered, aId, aRegistered));
}

/** Event for onSessionStateChange() */
struct SessionEvent : public VirtualPox::CallbackEvent
{
    SessionEvent(VirtualPox *aVB, const Guid &aMachineId, SessionState_T aState)
        : CallbackEvent(aVB, VPoxEventType_OnSessionStateChanged)
        , machineId(aMachineId.toUtf16()), sessionState(aState)
    {}

    virtual HRESULT prepareEventDesc(IEventSource* aSource, VPoxEventDesc& aEvDesc)
    {
        return aEvDesc.init(aSource, VPoxEventType_OnSessionStateChanged, machineId.raw(), sessionState);
    }
    Bstr machineId;
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onSessionStateChange(const Guid &aId, SessionState_T aState)
{
...
struct SnapshotEvent : public VirtualPox::CallbackEvent
{
    SnapshotEvent(VirtualPox *aVB, const Guid &aMachineId, const Guid &aSnapshotId,
                  VPoxEventType_T aWhat)
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onSnapshotTaken(const Guid &aMachineId, const Guid &aSnapshotId)
{
    i_postEvent(new SnapshotEvent(this, aMachineId, aSnapshotId,
                                  VPoxEventType_OnSnapshotTaken));
}

/**
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onSnapshotDeleted(const Guid &aMachineId, const Guid &aSnapshotId)
{
    i_postEvent(new SnapshotEvent(this, aMachineId, aSnapshotId,
                                  VPoxEventType_OnSnapshotDeleted));
}

/**
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onSnapshotRestored(const Guid &aMachineId, const Guid &aSnapshotId)
{
    i_postEvent(new SnapshotEvent(this, aMachineId, aSnapshotId,
                                  VPoxEventType_OnSnapshotRestored));
}

/**
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onSnapshotChange(const Guid &aMachineId, const Guid &aSnapshotId)
{
    i_postEvent(new SnapshotEvent(this, aMachineId, aSnapshotId,
                                  VPoxEventType_OnSnapshotChanged));
}

/** Event for onGuestPropertyChange() */
struct GuestPropertyEvent : public VirtualPox::CallbackEvent
{
    GuestPropertyEvent(VirtualPox *aVPox, const Guid &aMachineId,
                       IN_BSTR aName, IN_BSTR aValue, IN_BSTR aFlags)
        : CallbackEvent(aVPox, VPoxEventType_OnGuestPropertyChanged),
    ...
    virtual HRESULT prepareEventDesc(IEventSource* aSource, VPoxEventDesc& aEvDesc)
    {
        return aEvDesc.init(aSource, VPoxEventType_OnGuestPropertyChanged,
 ...
 */
int VirtualPox::i_clipboardAreaCreate(ULONG uAreaID, uint32_t fFlags, SharedClipboardAreaData **ppAreaData)
{
    RT_NOREF(fFlags);
 ...
 */
int VirtualPox::i_clipboardAreaDestroy(SharedClipboardAreaData *pAreaData)
{
    if (!pAreaData)
        return VINF_SUCCESS;
 ...
 */
HRESULT VirtualPox::i_onClipboardAreaRegister(const std::vector<com::Utf8Str> &aParms, ULONG *aID)
{
    RT_NOREF(aParms);
 ...
 */
HRESULT VirtualPox::i_onClipboardAreaUnregister(ULONG aID)
{
    HRESULT rc = S_OK;
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (itArea != m->SharedClipboard.mapClipboardAreas.end())
        {
            ...
            rc = setError(VPOX_E_OBJECT_NOT_FOUND, /** @todo Find a better rc. */
 ...
 */
HRESULT VirtualPox::i_onClipboardAreaAttach(ULONG aID)
{
    HRESULT rc = S_OK;
 ...
 */
HRESULT VirtualPox::i_onClipboardAreaDetach(ULONG aID)
{
    HRESULT rc = S_OK;
 ...
 */
ULONG VirtualPox::i_onClipboardAreaGetMostRecent(void)
{
    ULONG aID = 0;
 ...
 */
ULONG VirtualPox::i_onClipboardAreaGetRefCount(ULONG aID)
{
    ULONG cRefCount = 0;
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onGuestPropertyChange(const Guid &aMachineId, IN_BSTR aName,
 ...
 *  @note Doesn't lock any object.
 */
void VirtualPox::i_onNatRedirectChange(const Guid &aMachineId, ULONG ulSlot, bool fRemove, IN_BSTR aName,
...
void VirtualPox::i_onNATNetworkChange(IN_BSTR aName)
{
    fireNATNetworkChangedEvent(m->pEventSource, aName);
}

void VirtualPox::i_onNATNetworkStartStop(IN_BSTR aName, BOOL fStart)
{
    fireNATNetworkStartStopEvent(m->pEventSource, aName, fStart);
}

void VirtualPox::i_onNATNetworkSetting(IN_BSTR aNetworkName, BOOL aEnabled,
...
void VirtualPox::i_onNATNetworkPortForward(IN_BSTR aNetworkName, BOOL create, BOOL fIpv6,
...
void VirtualPox::i_onHostNameResolutionConfigurationChange()
{
    if (m->pEventSource)
...
int VirtualPox::i_natNetworkRefInc(const Utf8Str &aNetworkName)
{
...
int VirtualPox::i_natNetworkRefDec(const Utf8Str &aNetworkName)
{
    AutoWriteLock safeLock(*spMtxNatNetworkNameToRefCountLock COMMA_LOCKVAL_SRC_POS);
 ...
 */
ComObjPtr<GuestOSType> VirtualPox::i_getUnknownOSType()
{
 ...
 */
void VirtualPox::i_getOpenedMachines(SessionMachinesList &aMachines,
 ...
 *
 * @note Caller must hold the VirtualPox object lock at least for reading.
 */
VirtualPox::MachinesOList &VirtualPox::i_getMachinesList(void)
{
 ...
 * @return
 */
HRESULT VirtualPox::i_findMachine(const Guid &aId,
 ...
 * @return
 */
HRESULT VirtualPox::i_findMachineByName(const Utf8Str &aName,
                                        bool  aSetError,
                                        ComObjPtr<Machine> *aMachine /* = NULL */)
{
    HRESULT rc = VPOX_E_OBJECT_NOT_FOUND;
...
static HRESULT i_validateMachineGroupHelper(const Utf8Str &aGroup, bool fPrimary, VirtualPox *pVirtualPox)
{
    ...
    while (pStr)
    {
        ...
        if (pSlash)
        {
            ...
            if (fPrimary)
            {
                HRESULT rc = pVirtualPox->i_findMachineByName(tmp,
                                                              false /* aSetError */);
                if (SUCCEEDED(rc))
 ...
 */
HRESULT VirtualPox::i_validateMachineGroup(const Utf8Str &aGroup, bool fPrimary)
{
    HRESULT rc = i_validateMachineGroupHelper(aGroup, fPrimary, this);
    if (FAILED(rc))
    {
        if (rc == VPOX_E_VM_ERROR)
 ...
 * @return S_OK or E_INVALIDARG
 */
HRESULT VirtualPox::i_convertMachineGroups(const std::vector<com::Utf8Str> aMachineGroups, StringsList *pllMachineGroups)
{
 ...
 *
 * @return S_OK, E_INVALIDARG or VPOX_E_OBJECT_NOT_FOUND when not found.
 *
 * @note Locks the media tree for reading.
 */
HRESULT VirtualPox::i_findHardDiskById(const Guid &aId,
                                       bool aSetError,
                                       ComObjPtr<Medium> *aHardDisk /*= NULL*/)
{
    ...
    if (aSetError)
        return setError(VPOX_E_OBJECT_NOT_FOUND,
                        tr("Could not find an open hard disk with UUID {%RTuuid}"),
                        aId.raw());

    return VPOX_E_OBJECT_NOT_FOUND;
}
 ...
 * @note Locks the media tree for reading.
 */
HRESULT VirtualPox::i_findHardDiskByLocation(const Utf8Str &strLocation,
                                             bool aSetError,
                                             ComObjPtr<Medium> *aHardDisk /*= NULL*/)
{
 ...
 *
 * @return S_OK when found or E_INVALIDARG or VPOX_E_OBJECT_NOT_FOUND when not found.
 *
 * @note Locks the media tree for reading.
 */
HRESULT VirtualPox::i_findDVDOrFloppyImage(DeviceType_T mediumType,
                                           const Guid *aId,
                                           ...
                                           ComObjPtr<Medium> *aImage /* = NULL */)
{
    ...
    if (!aLocation.isEmpty())
    {
        ...
        if (RT_FAILURE(vrc))
            return setError(VPOX_E_FILE_ERROR,
    ...
    HRESULT rc = found ? S_OK : VPOX_E_OBJECT_NOT_FOUND;
 ...
 *
 * If none of the above apply, this returns VPOX_E_OBJECT_NOT_FOUND.
 *
 ...
 * @return
 */
HRESULT VirtualPox::i_findRemoveableMedium(DeviceType_T mediumType,
                                           const Guid &uuid,
                                           bool fRefresh,
                                           bool aSetError,
                                           ComObjPtr<Medium> &pMedium)
...
HRESULT VirtualPox::i_findGuestOSType(const Utf8Str &strOSType,
 ...
 *
 * Starting with VirtualPox 4.0 each medium remembers in its instance data
 ...
 *
 * This UUID is only used to identify the VirtualPox object while VirtualPox
 * is running. It is a compile-time constant and not saved anywhere.
 *
 * @return
 */
const Guid& VirtualPox::i_getGlobalRegistryId() const
{
    return m->uuidMediaRegistry;
}

const ComObjPtr<Host>& VirtualPox::i_host() const
{
    return m->pHost;
}

SystemProperties* VirtualPox::i_getSystemProperties() const
{
    return m->pSystemProperties;
}

CloudProviderManager *VirtualPox::i_getCloudProviderManager() const
{
 ...
 * pack manager.
 */
ExtPackManager* VirtualPox::i_getExtPackManager() const
{
 ...
 */
AutostartDb* VirtualPox::i_getAutostartDb() const
{
    return m->pAutostartDb;
}

#ifdef VPOX_WITH_RESOURCE_USAGE_API
const ComObjPtr<PerformanceCollector>& VirtualPox::i_performanceCollector() const
{
 ...
 * @return
 */
void VirtualPox::i_getDefaultMachineFolder(Utf8Str &str) const
{
 ...
 * @return
 */
void VirtualPox::i_getDefaultHardDiskFormat(Utf8Str &str) const
{
    AutoReadLock propsLock(m->pSystemProperties COMMA_LOCKVAL_SRC_POS);
    str = m->pSystemProperties->m->strDefaultHardDiskFormat;
}

const Utf8Str& VirtualPox::i_homeDir() const
{
    return m->strHomeDir;
}

/**
 * Calculates the absolute path of the given path taking the VirtualPox home
 ...
 * @note Doesn't lock any object.
 */
int VirtualPox::i_calculateFullPath(const Utf8Str &strPath, Utf8Str &aResult)
{
    AutoCaller autoCaller(this);
...
/**
 * Copies strSource to strTarget, making it relative to the VirtualPox config folder
 ...
 */
void VirtualPox::i_copyPathRelativeToConfig(const Utf8Str &strSource,
                                            Utf8Str &strTarget)
{
    AutoCaller autoCaller(this);
    AssertComRCReturnVoid(autoCaller.rc());
 ...
 */
HRESULT VirtualPox::i_checkMediaForConflicts(const Guid &aId,
                                             const Utf8Str &aLocation,
                                             Utf8Str &aConflict,
 ...
 *                        conflicts.
 */
bool VirtualPox::i_isMediaUuidInUse(const Guid &aId, DeviceType_T deviceType)
{
 ...
 * VirtualPox::saveSettings() that follows later.
*
 ...
 * used to call VirtualPox::saveSettings implicitly, which was both
 * unintuitive and caused locking order problems. Now, we remember
 * such pending name changes with this method so that
 * VirtualPox::saveSettings() can process them properly.
 */
void VirtualPox::i_rememberMachineNameChangeForMedia(const Utf8Str &strOldConfigDir,
...
private:
    ...
    MediaList llMedia;
    ComObjPtr<VirtualPox> pVirtualPox;

    friend DECLCALLBACK(int) fntSaveMediaRegistries(void *pvUser);
    friend void VirtualPox::i_saveMediaRegistry(settings::MediaRegistry &mediaRegistry,
...
DECLCALLBACK(int) fntSaveMediaRegistries(void *pvUser)
{
    ...
    pDesc->pVirtualPox->i_saveModifiedRegistries();

    pDesc->llMedia.clear();
    pDesc->pVirtualPox.setNull();
 ...
 *
 *  -- VirtualPox::saveSettings() with the UUID of the global registry
 *     (VirtualPox::Data.uuidRegistry); this will save those media
 ...
 *     has been attached to a machine created with VirtualPox 4.0 or later.
 *
 ...
 */
void VirtualPox::i_saveMediaRegistry(settings::MediaRegistry &mediaRegistry,
                                     const Guid &uuidRegistry,
                                     const Utf8Str &strMachineFolder)
{
...
/**
 *  Helper function which actually writes out VirtualPox.xml, the main configuration file.
 *  Gets called from the public VirtualPox::SaveSettings() as well as from various other
 *  places internally when settings need saving.
 *
 *  @note Caller must have locked the VirtualPox object for writing and must not hold any
 *    other locks since this locks all kinds of member objects and trees temporarily,
 *    which could cause conflicts.
 */
HRESULT VirtualPox::i_saveSettings()
{
    AutoCaller autoCaller(this);
    AssertComRCReturnRC(autoCaller.rc());
 ...
 *
 *  When called during VirtualPox startup, adds the given machine to the
 ...
 *  object if calls this method not on VirtualPox startup.
 *
 *  @param aMachine     machine to register
 *
 *  @note Locks objects!
 */
HRESULT VirtualPox::i_registerMachine(Machine *aMachine)
{
...

    {
        ...
        ComAssertRet(rc == VPOX_E_OBJECT_NOT_FOUND, rc);
        rc = S_OK;
    }
 ...
 * @return
 */
HRESULT VirtualPox::i_registerMedium(const ComObjPtr<Medium> &pMedium,
                                     ComObjPtr<Medium> *ppMedium,
 ...
 */
HRESULT VirtualPox::i_unregisterMedium(Medium *pMedium)
{
    AssertReturn(pMedium != NULL, E_INVALIDARG);
 ...
 * @param pMedium
 */
void VirtualPox::i_pushMediumToListWithChildren(MediaList &llMedia, Medium *pMedium)
{
 ...
 * @return
 */
HRESULT VirtualPox::i_unregisterMachineMedia(const Guid &uuidMachine)
{
 ...
 * @return
 */
HRESULT VirtualPox::i_unregisterMachine(Machine *pMachine,
                                        const Guid &id)
{
 ...
 * @param uuid
 */
void VirtualPox::i_markRegistryModified(const Guid &uuid)
{
 ...
 * @param uuid
 */
void VirtualPox::i_unmarkRegistryModified(const Guid &uuid)
{
    uint64_t uOld;
    if (uuid == i_getGlobalRegistryId())
    {
 ...
 * objects and in the VirtualPox object.
 *
 * This locks machines and the VirtualPox object as necessary, so better not
 * hold any locks before calling this.
 *
 * @return
 */
void VirtualPox::i_saveModifiedRegistries()
{
    HRESULT rc = S_OK;
...
/* static */
const com::Utf8Str &VirtualPox::i_getVersionNormalized()
{
 ...
 */
/* static */
HRESULT VirtualPox::i_ensureFilePathExists(const Utf8Str &strFileName, bool fCreate)
{
    ...
    if (!RTDirExists(strDir.c_str()))
    {
        if (fCreate)
        {
            ...
            if (RT_FAILURE(vrc))
                return i_setErrorStaticBoth(VPOX_E_IPRT_ERROR, vrc,
                                            Utf8StrFmt(tr("Could not create the directory '%s' (%Rrc)"),
                                                       strDir.c_str(),
                                                       vrc));
        }
        else
            return i_setErrorStaticBoth(VPOX_E_IPRT_ERROR, VERR_FILE_NOT_FOUND,
...
const Utf8Str& VirtualPox::i_settingsFilePath()
{
 ...
 * VirtualPox lock, but by this more specialized lock. Mind the locking
 * order: always request this lock after the VirtualPox object lock but
 * before the locks of any machine object. See AutoLock.h.
 */
RWLockHandle& VirtualPox::i_getMachinesListLockHandle()
{
 ...
 * are no longer protected by the VirtualPox lock, but by this more
 * specialized lock. Mind the locking order: always request this lock
 * after the VirtualPox object lock but before the locks of the media
 * objects contained in these lists. See AutoLock.h.
 */
RWLockHandle& VirtualPox::i_getMediaTreeLockHandle()
{
    return m->lockMedia;
}
 ...
 */
// static
DECLCALLBACK(int) VirtualPox::AsyncEventHandler(RTTHREAD thread, void *pvUser)
{
    LogFlowFuncEnter();
 ...
 *
 *  @note Locks the managed VirtualPox object for reading but leaves the lock
 *        before iterating over callbacks and calling their methods.
 */
void *VirtualPox::CallbackEvent::handler()
{
    if (!mVirtualPox)
        return NULL;

    AutoCaller autoCaller(mVirtualPox);
    if (!autoCaller.isOk())
    {
        Log1WarningFunc(("VirtualPox has been uninitialized (state=%d), the callback event is discarded!\n",
                         mVirtualPox->getObjectState().getState()));
        /* We don't need mVirtualPox any more, so release it */
        mVirtualPox = NULL;
        return NULL;
    }
    ...
    {
        VPoxEventDesc evDesc;
        prepareEventDesc(mVirtualPox->m->pEventSource, evDesc);

        evDesc.fire(/* don't wait for delivery */0);
    }

    mVirtualPox = NULL; /* Not needed any longer. Still make sense to do this? */
    return NULL;
}

//STDMETHODIMP VirtualPox::CreateDHCPServerForInterface(/*IHostNetworkInterface * aIinterface,*/ IDHCPServer ** aServer)
//{
//    return E_NOTIMPL;
//}

HRESULT VirtualPox::createDHCPServer(const com::Utf8Str &aName,
...
HRESULT VirtualPox::findDHCPServerByNetworkName(const com::Utf8Str &aName,
                                                ComPtr<IDHCPServer> &aServer)
{
    HRESULT rc = S_OK;
...
HRESULT VirtualPox::removeDHCPServer(const ComPtr<IDHCPServer> &aServer)
{
 ...
 */
HRESULT VirtualPox::i_registerDHCPServer(DHCPServer *aDHCPServer,
                                         bool aSaveSettings /*= true*/)
{
    ...
    // Acquire a lock on the VirtualPox object early to avoid lock order issues
 ...
 * @note Locks this object for writing.
 */
HRESULT VirtualPox::i_unregisterDHCPServer(DHCPServer *aDHCPServer)
{
    AssertReturn(aDHCPServer != NULL, E_INVALIDARG);
 ...
 * NAT Network
 */
HRESULT VirtualPox::createNATNetwork(const com::Utf8Str &aNetworkName,
...
HRESULT VirtualPox::findNATNetworkByName(const com::Utf8Str &aNetworkName,
                                         ComPtr<INATNetwork> &aNetwork)
{
#ifdef VPOX_WITH_NAT_SERVICE
...
HRESULT VirtualPox::removeNATNetwork(const ComPtr<INATNetwork> &aNetwork)
{
#ifdef VPOX_WITH_NAT_SERVICE
    Bstr name;
    HRESULT rc = aNetwork->COMGETTER(NetworkName)(name.asOutParam());
 ...
 */
HRESULT VirtualPox::i_registerNATNetwork(NATNetwork *aNATNetwork,
                                         bool aSaveSettings /*= true*/)
{
#ifdef VPOX_WITH_NAT_SERVICE
 ...
 * @note Locks this object for writing.
 */
HRESULT VirtualPox::i_unregisterNATNetwork(NATNetwork *aNATNetwork,
                                           bool aSaveSettings /*= true*/)
{
#ifdef VPOX_WITH_NAT_SERVICE
    AssertReturn(aNATNetwork != NULL, E_INVALIDARG);
 ...
 */
void VirtualPox::i_reportDriverVersions()
{
    ...
    do
    {
        ...
        for (int i = 0; i < cDrivers; i++)
        {
            ...
            {
                if (_tcsnicmp(TEXT("vpox"), szDriver, 4))
                    continue;
            }
            else
                continue;
...
#else /* !RT_OS_WINDOWS */
void VirtualPox::i_reportDriverVersions(void)
{
}
#endif /* !RT_OS_WINDOWS */
 ...
 */
void VirtualPox::i_callHook(const char *a_pszFunction)
{
 ...
 */
bool VirtualPox::i_watchClientProcess(RTPROCESS a_pidClient, const char *a_pszFunction)
{
    ...
    if (pWatched)
    {
        ...
        if (It == m->WatchedProcesses.end())
        {
            ...
            else
            {
                RTCritSectRwLeaveExcl(&m->WatcherCritSect);
                delete pWatched;
                LogRel(("VirtualPox::i_watchClientProcess: out of memory inserting into client map!\n"));
            }
        }
        else
        {
    ...
    {
        LogRel(("VirtualPox::i_watchClientProcess: out of memory!\n"));
        CloseHandle(hClient);
...
/*static*/ void VirtualPox::i_logCaller(const char *a_pszFormat, ...)
{
    if (RTSystemGetNtVersion() >= RTSYSTEM_MAKE_NT_VERSION(6, 0, 0))
    {
        char szTmp[80];
...
#endif /* RT_OS_WINDOWS && VPOXSVC_WITH_CLIENT_WATCHER */
