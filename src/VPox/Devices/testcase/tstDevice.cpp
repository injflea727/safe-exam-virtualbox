...
static int tstDevPdmLoadR3Mod(PTSTDEVPDMMOD pMod)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = RTLdrGetSymbol(pMod->hLdrMod, "VPoxDevicesRegister", (void**)&pfnVPoxDevicesRegister);
        if (RT_FAILURE(rc) || !pfnVPoxDevicesRegister)
        {
            LogFunc(("error resolving the entry point %s in plugin %s, rc=%Rrc, pfnPluginLoad=%#p\n",
                     "VPoxDevicesRegister", pMod->pszFilename, rc, pfnVPoxDevicesRegister));
            if (RT_SUCCESS(rc))
                rc = VERR_SYMBOL_NOT_FOUND;
        }
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            RegCB.pMod             = pMod;
            rc = pfnVPoxDevicesRegister(&RegCB.Core, VPOX_VERSION);
        }
        else
            LogFunc(("ignored plugin '%s': rc=%Rrc\n", pMod->pszFilename, rc));
