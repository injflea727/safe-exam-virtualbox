...
VMMR3_INT_DECL(VPOXSTRICTRC) EMR3HmSingleInstruction(PVM pVM, PVMCPU pVCpu, uint32_t fFlags)
{
    ...
    for (;;)
    {
        ...
        {
            VPOXSTRICTRC rcStrict = emR3HmForcedActions(pVM, pVCpu);
        ...
        VPOXSTRICTRC rcStrict = VMMR3HmRunGC(pVM, pVCpu);
        ...
        {
            rcStrict = emR3HmHandleRC(pVM, pVCpu, VPOXSTRICTRC_TODO(rcStrict));
...
#endif
{
    ...
    if (idxContinueExitRec >= RT_ELEMENTS(pVCpu->em.s.aExitRecords))
    {
        ...
        rcStrict = VPOXSTRICTRC_TODO(IEMExecOne(pVCpu));
    }
    else
    {
