/* $Id: vpox-greeter.cpp $ */
/** @file
 * vpox-greeter - an own LightDM greeter module supporting auto-logons
...
#define VPOX_MODULE_NAME "vpox-lightdm-greeter"

/* UI elements used in this greeter. */
#define VPOX_GREETER_UI_WND_GREETER     "wnd_greeter"

#define VPOX_GREETER_UI_EDT_USER        "edt_username"
#define VPOX_GREETER_UI_EDT_PASSWORD    "edt_password"
#define VPOX_GREETER_UI_BTN_LOGIN       "btn_login"
#define VPOX_GREETER_UI_LBL_INFO        "lbl_info"

/* UI display options. */
/** Show the restart menu entry / button. */
#define VPOX_GREETER_UI_SHOW_RESTART    RT_BIT(0)
/** Show the shutdown menu entry / button. */
#define VPOX_GREETER_UI_SHOW_SHUTDOWN   RT_BIT(1)
/** Show the (customized) top banner.  */
#define VPOX_GREETER_UI_SHOW_BANNER     RT_BIT(2)
/** Enable custom colors */
#define VPOX_GREETER_UI_USE_THEMING     RT_BIT(3)

/** Extracts the 8-bit red component from an uint32_t. */
#define VPOX_RGB_COLOR_RED(uColor)      uColor & 0xFF
/** Extracts the 8-bit green component from an uint32_t. */
#define VPOX_RGB_COLOR_GREEN(uColor)    (uColor >> 8)  & 0xFF
/** Extracts the 8-bit blue component from an uint32_t. */
#define VPOX_RGB_COLOR_BLUE(uColor)     (uColor >> 16) & 0xFF
...
typedef struct VPOXGREETERCTX
{
    ...
    char           *pszPassword;
} VPOXGREETERCTX, *PVPOXGREETERCTX;

static void vpoxGreeterError(const char *pszFormat, ...)
{
    va_list va;
    char *buf;
    ...
    {
        RTLogRelPrintf("%s: error: %s", VPOX_MODULE_NAME, buf);
        RTStrFree(buf);
    }
    va_end(va);
}

static void vpoxGreeterLog(const char *pszFormat, ...)
{
    if (g_iVerbosity)
    {
        va_list va;
        ...
        if (RTStrAPrintfV(&buf, pszFormat, va))
        {
             ...
             * sensitive data! */
            RTLogRelPrintf("%s: %s", VPOX_MODULE_NAME, buf);
            RTStrFree(buf);
        }
        va_end(va);
    }
...
/** @tood Move the following two functions to VbglR3 (also see pam_vpox). */
#ifdef VPOX_WITH_GUEST_PROPS
 ...
 *                                  retrieved. Optional.
 */
static int vpox_read_prop(uint32_t uClientID,
...
#ifdef DEBUG
    vpoxGreeterLog("Read guest property \"%s\"=\"%s\" (Flags: %s, TS: %RU64): %Rrc\n",
 ...
 */
static int vpox_wait_prop(uint32_t uClientID,
 ...
 */
static int vpoxGreeterCheckCreds(PVPOXGREETERCTX pCtx)
{
    ...
    if (RT_FAILURE(rc))
    {
        if (rc != VERR_NOT_FOUND)
            vpoxGreeterError("vpoxGreeterCheckCreds: could not query for credentials! rc=%Rrc. Aborting\n", rc);
        else if (!s_fCredsNotFoundMsgShown)
        {
            vpoxGreeterLog("vpoxGreeterCheckCreds: no credentials available\n");
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
        {
            vpoxGreeterError("vpoxGreeterCheckCreds: could not retrieve credentials! rc=%Rrc. Aborting\n", rc);
        }
        else
        {
            vpoxGreeterLog("vpoxGreeterCheckCreds: credentials retrieved: user=%s, password=%s, domain=%s\n",
                           pszUsername,
#ifdef DEBUG
                           pCtx->pszPassword,
#else
...
#ifdef DEBUG
    vpoxGreeterLog("vpoxGreeterCheckCreds: returned with rc=%Rrc\n", rc);
#endif
    return rc;
}
...
static void cb_lightdm_show_prompt(LightDMGreeter *pGreeter,
                                   ...
                                   gpointer pvData)
{
    vpoxGreeterLog("cb_lightdm_show_prompt: text=%s, type=%d\n", pszText, enmType);

    PVPOXGREETERCTX pCtx = (PVPOXGREETERCTX)pvData;
    AssertPtr(pCtx);
...
{
    RT_NOREF(pGreeter);
    vpoxGreeterLog("cb_lightdm_show_message: text=%s, type=%d\n", pszText, enmType);
...
static void cb_lightdm_auth_complete(LightDMGreeter *pGreeter)
{
    ...
    if (lightdm_greeter_get_is_authenticated(pGreeter))
    {
        ...
        if (pszSession)
        {
            ...
            vpoxGreeterError("unable to get default session\n");
    }
    else
        vpoxGreeterLog("user not authenticated successfully (yet)\n");
}
...
#endif
{
    ...
    const char *pszPwd = pCtx->pEdtPassword->value();
#else
    GtkEntry *pEdtUser = GTK_ENTRY(gtk_builder_get_object(pCtx->pBuilder, VPOX_GREETER_UI_EDT_USER));
    AssertPtr(pEdtUser);
    const gchar *pszUser = gtk_entry_get_text(pEdtUser);

    GtkEntry *pEdtPwd = GTK_ENTRY(gtk_builder_get_object(pCtx->pBuilder, VPOX_GREETER_UI_EDT_PASSWORD));
    AssertPtr(pEdtPwd);
    const gchar *pszPwd = gtk_entry_get_text(pEdtPwd);
#endif

    /** @todo Add domain handling? */
    vpoxGreeterLog("login button pressed: greeter=%p, user=%s, password=%s\n",
                   pCtx->pGreeter,
...
#endif
{
    ...
    vpoxGreeterLog("menu button pressed\n");
}
...
{
    RT_NOREF(pWidget, pvData);
    vpoxGreeterLog("restart button pressed\n");
    ...
    if (fRestart)
    {
        vpoxGreeterLog("restart requested\n");
#ifndef DEBUG
...
{
    RT_NOREF(pWidget, pvData);
    vpoxGreeterLog("shutdown button pressed\n");
    ...
    if (fShutdown)
    {
        vpoxGreeterLog("shutdown requested\n");
#ifndef DEBUG
...
{
    RT_NOREF(pWidget);
    vpoxGreeterLog("cb_edt_username called\n");
...
{
    RT_NOREF(pWidget, pvData);
    vpoxGreeterLog("cb_edt_password called\n");
...
#endif
{
...
#ifdef DEBUG
    vpoxGreeterLog("cb_check_creds called, clientId=%RU32, timeoutMS=%RU32\n",
    ...
    {
        uint64_t tsAbort;
        rc = vpox_read_prop(pCtx->uClientId,
                            "/VirtualPox/GuestAdd/PAM/CredsWaitAbort",
        ...
        switch (rc)
        {
            case VINF_SUCCESS:
# ifdef DEBUG
                vpoxGreeterLog("cb_check_creds: tsAbort %RU64 <-> %RU64\n",
...
# endif
            ...
            case VERR_TOO_MUCH_DATA:
                vpoxGreeterError("cb_check_creds: temporarily unable to get abort notification\n");
                break;
            ...
            default:
                vpoxGreeterError("cb_check_creds: the abort notification request failed with rc=%Rrc\n", rc);
    ...
    if (fAbort)
    {
        ...
        szVal[0] = '\0';
        int rc2 = vpox_read_prop(pCtx->uClientId,
                                 "/VirtualPox/GuestAdd/PAM/CredsMsgWaitAbort",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
        if (   RT_FAILURE(rc2)
            && rc2 != VERR_NOT_FOUND)
            vpoxGreeterError("cb_check_creds: getting wait abort message failed with rc=%Rrc\n", rc2);
# ifdef VPOX_WITH_FLTK
        AssertPtr(pCtx->pLblInfo);
        pCtx->pLblInfo->copy_label(szVal);
# else /* !VPOX_WITH_FLTK */
        GtkLabel *pLblInfo = GTK_LABEL(gtk_builder_get_object(pCtx->pBuilder, VPOX_GREETER_UI_LBL_INFO));
        AssertPtr(pLblInfo);
        gtk_label_set_text(pLblInfo, szVal);
# endif /* !VPOX_WITH_FLTK */
        vpoxGreeterLog("cb_check_creds: got notification from host to abort waiting\n");
    }
    else
    {
#endif /* VPOX_WITH_GUEST_PROPS */
        rc = vpoxGreeterCheckCreds(pCtx);
        if (RT_SUCCESS(rc))
        {
    ...
    if (rc == VERR_NOT_FOUND) /* No credential found this round. */
    {
        ...
        if (pCtx->uTimeoutMS < u64Elapsed)
        {
            ...
            szVal[0] = '\0';
            int rc2 = vpox_read_prop(pCtx->uClientId,
                                     "/VirtualPox/GuestAdd/PAM/CredsMsgWaitTimeout",
                                     true /* Read-only on guest */,
                                     szVal, sizeof(szVal), NULL /* Timestamp. */);
            if (   RT_FAILURE(rc2)
                && rc2 != VERR_NOT_FOUND)
                vpoxGreeterError("cb_check_creds: getting wait timeout message failed with rc=%Rrc\n", rc2);
# ifdef VPOX_WITH_FLTK
            AssertPtr(pCtx->pLblInfo);
            pCtx->pLblInfo->copy_label(szVal);
# else
...
# endif
#endif /* VPOX_WITH_GUEST_PROPS */
            vpoxGreeterLog("cb_check_creds: no credentials retrieved within time (%RU32ms), giving up\n",
                           pCtx->uTimeoutMS);
            rc = VERR_TIMEOUT;
        }
    }
...
#ifdef DEBUG
    vpoxGreeterLog("cb_check_creds returned with rc=%Rrc\n", rc);
#endif
 ...
 * @param   pfnLog
 */
static DECLCALLBACK(void) vpoxGreeterLogHeaderFooter(PRTLOGGER pLoggerRelease, RTLOGPHASE enmPhase, PFNRTLOGPHASEMSG pfnLog)
{
    ...
    switch (enmPhase)
    {
        case RTLOGPHASE_BEGIN:
        {
            pfnLog(pLoggerRelease,
                   "vpox-greeter %s r%s (verbosity: %d) %s (%s %s) release log\n"
                   "Log opened %s\n",
                   RTBldCfgVersion(), RTBldCfgRevisionStr(), g_iVerbosity, VPOX_BUILD_TARGET,
 ...
 */
static int vpoxGreeterLogCreate(const char *pszLogFile)
{
...
#endif
    int rc = RTLogCreateEx(&g_pLoggerRelease, fFlags, "all", "VPOXGREETER_RELEASE_LOG",
                           RT_ELEMENTS(s_apszGroups), s_apszGroups, UINT32_MAX /*cMaxEntriesPerGroup*/, RTLOGDEST_STDOUT,
                           vpoxGreeterLogHeaderFooter, g_cHistory, g_uHistoryFileSize, g_uHistoryFileTime,
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        /* Explicitly flush the log in case of VPOXGREETER_RELEASE_LOG_FLAGS=buffered. */
...
static void vpoxGreeterLogDestroy(void)
{
    RTLogDestroy(RTLogRelSetDefaultInstance(NULL));
}

static int vpoxGreeterUsage(void)
{
    RTPrintf("Usage:\n"
    ...
    RTPrintf("\n"
             " Copyright (C) 2012-" VPOX_C_YEAR " " VPOX_VENDOR "\n");
...
int main(int argc, char **argv)
{
    ...
    rc = vpoxGreeterLogCreate(strlen(szLogFile) ? szLogFile : NULL);
    if (RT_FAILURE(rc))
    ...
    uint32_t uOptsUI = VPOX_GREETER_UI_SHOW_RESTART
                     | VPOX_GREETER_UI_SHOW_SHUTDOWN;
#ifdef VPOX_WITH_GUEST_PROPS
    uint32_t uClientId = 0;
    rc = VbglR3GuestPropConnect(&uClientId);
    if (RT_SUCCESS(rc))
    {
        vpoxGreeterLog("clientId=%RU32\n", uClientId);

        ctx.uClientId = uClientId;

        char szVal[256];
        int rc2 = vpox_read_prop(uClientId,
                                 "/VirtualPox/GuestAdd/Greeter/HideRestart",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
        if (   RT_SUCCESS(rc2)
            && !RTStrICmp(szVal, "1"))
        {
            uOptsUI &= ~VPOX_GREETER_UI_SHOW_RESTART;
        }

        rc2 = vpox_read_prop(uClientId,
                             "/VirtualPox/GuestAdd/Greeter/HideShutdown",
                             true /* Read-only on guest */,
                             szVal, sizeof(szVal), NULL /* Timestamp. */);
        if (   RT_SUCCESS(rc2)
            && !RTStrICmp(szVal, "1"))
        {
            uOptsUI &= ~VPOX_GREETER_UI_SHOW_SHUTDOWN;
        }

# ifdef VPOX_GREETER_WITH_PNG_SUPPORT
        /* Load the banner. */
        rc2 = vpox_read_prop(uClientId,
                             "/VirtualPox/GuestAdd/Greeter/BannerPath",
                             true /* Read-only on guest */,
        ...
        if (RT_SUCCESS(rc2))
        {
            ...
            {
                vpoxGreeterLog("showing banner from '%s'\n", szBannerPath);
                uOptsUI |= VPOX_GREETER_UI_SHOW_BANNER;
            }
            else
                vpoxGreeterLog("warning: unable to find banner at '%s', skipping\n", szBannerPath);
        }
# endif /* VPOX_GREETER_WITH_PNG_SUPPORT */

        /* Use theming?. */
        rc2 = vpox_read_prop(uClientId,
                             "/VirtualPox/GuestAdd/Greeter/UseTheming",
                             true /* Read-only on guest */,
                             szVal, sizeof(szVal), NULL /* Timestamp. */);
        if (   RT_SUCCESS(rc2)
            && !RTStrICmp(szVal, "1"))
        {
            vpoxGreeterLog("custom theming enabled\n");
                           uOptsUI |= VPOX_GREETER_UI_USE_THEMING;
        }

        if (uOptsUI & VPOX_GREETER_UI_USE_THEMING)
        {
            /* Get background color. */
            rc2 = vpox_read_prop(uClientId,
                                 "/VirtualPox/GuestAdd/Greeter/Theme/BackgroundColor",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
            if (RT_SUCCESS(rc2))
            {
            ...
            /* Get header color. */
            rc2 = vpox_read_prop(uClientId,
                                 "/VirtualPox/GuestAdd/Greeter/Theme/LogonDialog/HeaderColor",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
            if (RT_SUCCESS(rc2))
            {
            ...
            /* Get dialog color. */
            rc2 = vpox_read_prop(uClientId,
                                 "/VirtualPox/GuestAdd/Greeter/Theme/LogonDialog/BackgroundColor",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
            if (RT_SUCCESS(rc2))
            {
            ...
            /* Get button color. */
            rc2 = vpox_read_prop(uClientId,
                                 "/VirtualPox/GuestAdd/Greeter/Theme/LogonDialog/ButtonColor",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
            if (RT_SUCCESS(rc2))
            {
        ...
        vpoxGreeterError("unable to connect to guest property service, rc=%Rrc\n", rc);
#endif
    vpoxGreeterLog("UI options are: %RU32\n", uOptsUI);

#ifdef VPOX_WITH_FLTK
    int rc2 = Fl::scheme("plastic");
    if (!rc2)
        vpoxGreeterLog("warning: unable to set visual scheme\n");

    Fl::visual(FL_DOUBLE | FL_INDEX);
    Fl_Double_Window *pWndMain = new Fl_Double_Window(Fl::w(), Fl::h(), "VirtualPox Guest Additions");
    AssertPtr(pWndMain);
    if (uOptsUI & VPOX_GREETER_UI_USE_THEMING)
        pWndMain->color(fl_rgb_color(VPOX_RGB_COLOR_RED(uBgColor),
                                     VPOX_RGB_COLOR_GREEN(uBgColor),
                                     VPOX_RGB_COLOR_BLUE(uBgColor)));
    else /* Default colors. */
    ...
    if (uOptsUI & VPOX_GREETER_UI_USE_THEMING)
        pWndGreeter->color(fl_rgb_color(VPOX_RGB_COLOR_RED(uLogonDlgBgColor),
                                        VPOX_RGB_COLOR_GREEN(uLogonDlgBgColor),
                                        VPOX_RGB_COLOR_BLUE(uLogonDlgBgColor)));
    else /* Default colors. */
...
# ifdef VPOX_GREETER_WITH_PNG_SUPPORT
    ...
    if (uOptsUI & VPOX_GREETER_UI_SHOW_BANNER)
    {
...
# endif
    ...
    if (uOptsUI & VPOX_GREETER_UI_USE_THEMING)
        pLblHeader->labelcolor(fl_rgb_color(VPOX_RGB_COLOR_RED(uLogonDlgHdrColor),
                                            VPOX_RGB_COLOR_GREEN(uLogonDlgHdrColor),
                                            VPOX_RGB_COLOR_BLUE(uLogonDlgHdrColor)));
    else /* Default color. */
    ...
    if (uOptsUI & VPOX_GREETER_UI_USE_THEMING)
        pBtnLogin->color(fl_rgb_color(VPOX_RGB_COLOR_RED(uLogonDlgBtnColor),
                                      VPOX_RGB_COLOR_GREEN(uLogonDlgBtnColor),
                                      VPOX_RGB_COLOR_BLUE(uLogonDlgBtnColor)));
    else /* Default color. */
    ...
    if (uOptsUI & VPOX_GREETER_UI_USE_THEMING)
        pBtnMenu->color(fl_rgb_color(VPOX_RGB_COLOR_RED(uLogonDlgBtnColor),
                                     VPOX_RGB_COLOR_GREEN(uLogonDlgBtnColor),
                                     VPOX_RGB_COLOR_BLUE(uLogonDlgBtnColor)));
    else /* Default color. */
        pBtnMenu->color(fl_rgb_color(255, 255, 255));

    if (uOptsUI & VPOX_GREETER_UI_SHOW_RESTART)
        pBtnMenu->add("Restart", "" /* Shortcut */, cb_btn_restart, &ctx, 0 /* Flags */);
    if (uOptsUI & VPOX_GREETER_UI_SHOW_SHUTDOWN)
        pBtnMenu->add("Shutdown", "" /* Shortcut */, cb_btn_shutdown, &ctx, 0 /* Flags */);

    char szLabel[255];
    RTStrPrintf(szLabel, sizeof(szLabel), "Oracle VM VirtualPox Guest Additions %sr%s",
                RTBldCfgVersion(), RTBldCfgRevisionStr());
    ...
    AssertPtr(pBuilder);
    if (!gtk_builder_add_from_file(pBuilder, "/usr/share/xgreeters/vpox-greeter.ui", &pError))
    {
        AssertPtr(pError);
        vpoxGreeterError("unable to load UI: %s", pError->message);
        return RTEXITCODE_FAILURE;
    }

    GtkWindow *pWndGreeter = GTK_WINDOW(gtk_builder_get_object(pBuilder, VPOX_GREETER_UI_WND_GREETER));
    AssertPtr(pWndGreeter);
    GtkButton *pBtnLogin = GTK_BUTTON(gtk_builder_get_object(pBuilder, VPOX_GREETER_UI_BTN_LOGIN));
    AssertPtr(pBtnLogin);
    GtkLabel *pLblInfo = GTK_LABEL(gtk_builder_get_object(pBuilder, VPOX_GREETER_UI_LBL_INFO));
    AssertPtr(pLblInfo);
    ...
    {
        vpoxGreeterError("unable to connect to LightDM server, aborting\n");
        return RTEXITCODE_FAILURE;
    }

    vpoxGreeterLog("connected to LightDM server\n");
    ...
    if (uClientId) /* Connected to guest property service? */
    {
        char szVal[256];
        rc2 = vpox_read_prop(uClientId,
                             "/VirtualPox/GuestAdd/PAM/CredsWait",
                             true /* Read-only on guest */,
                            szVal, sizeof(szVal), NULL /* Timestamp. */);
        if (RT_SUCCESS(rc2))
        {
            ...
            rc2 = vpox_read_prop(uClientId,
                                 "/VirtualPox/GuestAdd/PAM/CredsWaitTimeout",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
            if (RT_SUCCESS(rc2))
            {
                ...
                if (!uTimeoutMS)
                {
                    vpoxGreeterError("pam_vpox_authenticate: invalid waiting timeout value specified, defaulting to infinite timeout\n");
            ...
            rc2 = vpox_read_prop(uClientId,
                                 "/VirtualPox/GuestAdd/PAM/CredsMsgWaiting",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
            if (RT_SUCCESS(rc2))
            {
...
# endif
        ...
        if (fCheckCreds)
        {
            vpoxGreeterLog("No credentials available on startup, starting to check periodically ...\n");
# ifdef VPOX_WITH_FLTK
...
# endif /* VPOX_GREETER_WITH_PNG_SUPPORT */
    ...
    vpoxGreeterLog("terminated with exit code %ld (rc=%Rrc)\n",
                   rcExit, rc);

    vpoxGreeterLogDestroy();
