...
int hlpReportStatus(VPoxGuestFacilityStatus statusCurrent)
{
    int rc = VbglR3ReportAdditionsStatus(VPoxGuestFacilityType_VPoxTrayClient,
                                         statusCurrent,
                                         0 /* Flags */);
    if (RT_FAILURE(rc))
        Log(("VPoxTray: Could not report VPoxTray status \"%ld\", rc=%Rrc\n", statusCurrent, rc));
    return rc;
}
