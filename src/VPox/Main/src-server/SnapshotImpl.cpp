/* $Id: SnapshotImpl.cpp $ */
/** @file
 * COM class implementation for Snapshot and SnapshotMachine in VPoxSVC.
 */
 ...
 */
HRESULT Snapshot::init(VirtualPox *aVirtualPox,
                       const Guid &aId,
                       const Utf8Str &aName,
                                          ...
                                          const Utf8Str &aStateFilePath)
{
    ...
    /* Don't need to lock aMachine when VirtualPox is starting up */
...
void SessionMachine::i_takeSnapshotHandler(TakeSnapshotTask &task)
{
    ...
    try
    {
        ...
        rc = i_saveSettings(NULL, alock);
            // no need to check for whether VirtualPox.xml needs changing since
        ...
        {
            Bstr value;
            rc = GetExtraData(Bstr("VPoxInternal2/ForceTakeSnapshotWithoutState").raw(),
                              value.asOutParam());
        ...
        if (task.m_strStateFilePath.isNotEmpty())
        {
            ...
            rc = VirtualPox::i_ensureFilePathExists(task.m_strStateFilePath, true /* fCreate */);
            if (FAILED(rc))
                throw rc;
        }
        ...
        if (strNVRAM.isNotEmpty() && strNVRAMSnap.isNotEmpty() && RTFileExists(strNVRAM.c_str()))
        {
            ...
            rc = VirtualPox::i_ensureFilePathExists(strNVRAMSnapAbs, true /* fCreate */);
            if (FAILED(rc))
                throw rc;
 ...
 *
 * @note Locks VirtualPox and this object for writing.
 *
 * @param   task
 * @param   alock
                                         ...
                                         ComPtr<IProgress> &aProgress)
{
    ...
    if (pSnapshot == mData->mCurrentSnapshot)
    {
        if (mData->flModifications)
        {
            ...
            rc = i_saveSettings(NULL, alock);
            snapshotLock.acquire();
                // no need to change for whether VirtualPox.xml needs saving since
                // we can't have a machine XML rename pending at this point
            if (FAILED(rc)) return rc;
        }
    }
...
HRESULT SessionMachine::finishOnlineMergeMedium()
{
    ...
    else
    {
        ...
        if (pDeleteRec->mpChildrenToReparent && !pDeleteRec->mpChildrenToReparent->IsEmpty())
        {
            ...
            // need manual fixing via VPoxManage to adjust the parent UUID.
            treeLock.release();
