/* $Id: VPoxModAPIMonitor.cpp $ */
/** @file
 * VPoxModAPIMonitor - API monitor module for detecting host isolation.
 */
...
#define VPOX_MOD_APIMON_NAME "apimon"
...
/* Copied from VPoxManageInfo.cpp. */
...
static int apimonMachineControl(const Bstr &strUuid, PVPOXWATCHDOG_MACHINE pMachine,
                                APIMON_RESPONSE enmResp, uint32_t cMsTimeout)
{
    ...
    CHECK_ERROR_RET(g_pVirtualPox, FindMachine(strUuid.raw(),
...
static bool apimonHandleVM(const PVPOXWATCHDOG_MACHINE pMachine)
{
...
/* Callbacks. */
static DECLCALLBACK(int) VPoxModAPIMonitorPreInit(void)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) VPoxModAPIMonitorOption(int argc, char *argv[], int *piConsumed)
{
...
static DECLCALLBACK(int) VPoxModAPIMonitorInit(void)
{
    HRESULT rc = S_OK;
    ...
    do
    {
        ...
        {
            CHECK_ERROR_BREAK(g_pVirtualPox, GetExtraData(Bstr("VPoxInternal2/Watchdog/APIMonitor/Groups").raw(),
                                                          strValue.asOutParam()));
            if (!strValue.isEmpty())
            {
        ...
        if (!g_cMsAPIMonIslnTimeout)
            cfgGetValueU32(g_pVirtualPox, NULL /* Machine */,
                           "VPoxInternal2/Watchdog/APIMonitor/IsolationTimeoutMS", NULL /* Per-machine */,
        ...
        {
            Utf8Str strResp;
            int rc2 = cfgGetValueStr(g_pVirtualPox, NULL /* Machine */,
                                     "VPoxInternal2/Watchdog/APIMonitor/IsolationResponse", NULL /* Per-machine */,
        ...
        if (!g_cMsAPIMonResponseTimeout)
            cfgGetValueU32(g_pVirtualPox, NULL /* Machine */,
                           "VPoxInternal2/Watchdog/APIMonitor/ResponseTimeoutMS", NULL /* Per-machine */,
...
static DECLCALLBACK(int) VPoxModAPIMonitorMain(void)
{
    ...
    {
        Bstr strHeartbeat;
        CHECK_ERROR_BREAK(g_pVirtualPox, GetExtraData(Bstr("Watchdog/APIMonitor/Heartbeat").raw(),
...
static DECLCALLBACK(int) VPoxModAPIMonitorStop(void)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(void) VPoxModAPIMonitorTerm(void)
{
}

static DECLCALLBACK(int) VPoxModAPIMonitorOnMachineRegistered(const Bstr &strUuid)
{
    RT_NOREF(strUuid);
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) VPoxModAPIMonitorOnMachineUnregistered(const Bstr &strUuid)
{
    RT_NOREF(strUuid);
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) VPoxModAPIMonitorOnMachineStateChanged(const Bstr &strUuid, MachineState_T enmState)
{
    RT_NOREF(strUuid, enmState);
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) VPoxModAPIMonitorOnServiceStateChanged(bool fAvailable)
{
    if (!fAvailable)
    {
        serviceLog(("apimon: VPoxSVC became unavailable, triggering action\n"));
 ...
 */
VPOXMODULE g_ModAPIMonitor =
{
    /* pszName. */
    VPOX_MOD_APIMON_NAME,
    /* pszDescription. */
    ...
    /* methods. */
    VPoxModAPIMonitorPreInit,
    VPoxModAPIMonitorOption,
    VPoxModAPIMonitorInit,
    VPoxModAPIMonitorMain,
    VPoxModAPIMonitorStop,
    VPoxModAPIMonitorTerm,
    /* callbacks. */
    VPoxModAPIMonitorOnMachineRegistered,
    VPoxModAPIMonitorOnMachineUnregistered,
    VPoxModAPIMonitorOnMachineStateChanged,
    VPoxModAPIMonitorOnServiceStateChanged
};
