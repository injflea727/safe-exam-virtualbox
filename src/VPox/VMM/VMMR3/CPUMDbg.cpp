...
{
    PVMCPU          pVCpu = (PVMCPU)pvUser;
    VPOXGDTR const *pGdtr = (VPOXGDTR const *)((uint8_t const *)&pVCpu->cpum + pDesc->offRegister);
...
{
    PVMCPU          pVCpu = (PVMCPU)pvUser;
    VPOXIDTR const *pIdtr = (VPOXIDTR const *)((uint8_t const *)&pVCpu->cpum + pDesc->offRegister);
...
static DECLCALLBACK(int) cpumR3RegGstGet_msr(void *pvUser, PCDBGFREGDESC pDesc, PDBGFREGVAL pValue)
{
    ...
    uint64_t u64Value;
    VPOXSTRICTRC rcStrict = CPUMQueryGuestMsr(pVCpu, pDesc->offRegister, &u64Value);
...
static DECLCALLBACK(int) cpumR3RegGstSet_msr(void *pvUser, PCDBGFREGDESC pDesc, PCDBGFREGVAL pValue, PCDBGFREGVAL pfMask)
{
    ...
    {
        uint64_t u64FullValue;
        VPOXSTRICTRC rcStrict = CPUMQueryGuestMsr(pVCpu, pDesc->offRegister, &u64FullValue);
        if (rcStrict != VINF_SUCCESS)
        {
     ...
     * Perform the assignment.
     */
    VPOXSTRICTRC rcStrict = CPUMSetGuestMsr(pVCpu, pDesc->offRegister, u64Value);
    if (rcStrict == VINF_SUCCESS)
        return VINF_SUCCESS;
    AssertRC(RT_FAILURE_NP(rcStrict));
    return VPOXSTRICTRC_VAL(rcStrict);
