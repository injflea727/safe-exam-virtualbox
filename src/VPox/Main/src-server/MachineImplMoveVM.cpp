...
HRESULT MachineMoveVM::init()
{
    ...
    /* adding a trailing slash if it's needed */
    {
        ...
        if (len >= RTPATH_MAX)
            return m_pMachine->setError(VPOX_E_IPRT_ERROR, m_pMachine->tr("The destination path exceeds the maximum value."));
    ...
    if (FAILED(vrc))
        return m_pMachine->setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
    ...
    m_vmFolders.insert(std::make_pair(VPox_SettingFolder, strSettingsFilePath));
    ...
    if (   m_type.equals("basic")
        ...
        m_vmFolders.insert(std::make_pair(VPox_LogFolder, strLogFolder));
    ...
    if (machineState == MachineState_Saved)
    {
        ...
        if (   m_type.equals("basic")
            ...
            m_vmFolders.insert(std::make_pair(VPox_StateFolder, strStateFilePath));
    }
    ...
    if (   m_type.equals("basic")
        ...
        m_vmFolders.insert(std::make_pair(VPox_SnapshotFolder, strSnapshotFolder));
    ...
    if (m_pMachine->i_isSnapshotMachine())
    {
        ...
        hrc = m_pMachine->i_getVirtualPox()->FindMachine(bstrSrcMachineId.raw(), newSrcMachine.asOutParam());
        if (FAILED(hrc))
            return hrc;
    }
    ...
    {
        Utf8Str strFolder = m_vmFolders[VPox_LogFolder];
    ...
    if (cbFree - neededFreeSpace <= _1M)
    {
        ...
        return m_pMachine->setError(VPOX_E_IPRT_ERROR,
                                    m_pMachine->tr("Insufficient disk space available (%RTfoff needed, %RTfoff free)"),
                                    neededFreeSpace, cbFree);
    }

    /* Add step for .vpox machine setting file */
    ++uCount;
    uTotalWeight += 1;
    ...
    {
        hrc = m_pProgress->init(m_pMachine->i_getVirtualPox(),
...
//  if (FAILED(autoCaller.rc())) return;//Should we return something here?
    ...
    try
    {
        ...
        Utf8Str strTrgSnapshotFolder;
        {
                ...
                && !RTDirExists(strTrgSnapshotFolder.c_str()))
            {
                ...
                if (RT_FAILURE(vrc))
                    throw machine->setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
        ...
        taskMoveVM->updatePathsToStateFiles(taskMoveVM->m_vmFolders[VPox_SettingFolder],
                                            strTargetFolder);
        ...
        taskMoveVM->updatePathsToNVRAMFiles(taskMoveVM->m_vmFolders[VPox_SettingFolder],
                                            strTargetFolder);
         ...
         */
        {
            ...
            strTargetSettingsFilePath.append(".vpox");
        ...
        /* Moving Machine log files */
        {
            ...
            if (taskMoveVM->m_vmFolders[VPox_LogFolder].isNotEmpty())
            {
                /* Check an original log folder existence */
                if (RTDirExists(taskMoveVM->m_vmFolders[VPox_LogFolder].c_str()))
                {
                    ...
                    fileList_t filesList;
                    taskMoveVM->getFilesList(taskMoveVM->m_vmFolders[VPox_LogFolder], filesList);
                    cit_t it = filesList.m_list.begin();
                    while (it != filesList.m_list.end())
                    {
        ...
        if (FAILED(hrc))
            throw hrc;
        strTargetSettingsFilePath.append(RTPATH_DELIMITER).append(Utf8Str(bstrMachineName)).append(".vpox");
        ...
        /* for saving the global settings we should hold only the VirtualPox lock */
        AutoWriteLock vpoxLock(machine->mParent COMMA_LOCKVAL_SRC_POS);

        /* Save global settings in the VirtualPox.xml */
    ...
    catch (...)
    {
        ...
        hrc = VirtualPoxBase::handleUnexpectedExceptions(machine, RT_SRC_POS);
        taskMoveVM->m_result = hrc;
    }
    ...
    if (FAILED(hrc))
    {
        ...
        try
        {
            ...
            taskMoveVM->updatePathsToStateFiles(strTargetFolder,
                                                taskMoveVM->m_vmFolders[VPox_SettingFolder]);
            ...
            /* Restore an original path to XML setting file */
            {
                ...
                Utf8Str strOriginalSettingsFilePath = taskMoveVM->m_vmFolders[VPox_SettingFolder];
                strOriginalSettingsFilePath.append(RTPATH_DELIMITER).append(Utf8Str(bstrMachineName)).append(".vpox");
    ...
    else /*Operation was successful and now we can delete the original files like the state files, XML setting, log files */
    {
        ...
        if (   taskMoveVM->m_vmFolders[VPox_SnapshotFolder].isNotEmpty()
            && RTDirExists(taskMoveVM->m_vmFolders[VPox_SnapshotFolder].c_str()))
            RTDirRemove(taskMoveVM->m_vmFolders[VPox_SnapshotFolder].c_str());

        if (   taskMoveVM->m_vmFolders[VPox_LogFolder].isNotEmpty()
            && RTDirExists(taskMoveVM->m_vmFolders[VPox_LogFolder].c_str()))
            RTDirRemove(taskMoveVM->m_vmFolders[VPox_LogFolder].c_str());

        if (   taskMoveVM->m_vmFolders[VPox_SettingFolder].isNotEmpty()
            && RTDirExists(taskMoveVM->m_vmFolders[VPox_SettingFolder].c_str()))
            RTDirRemove(taskMoveVM->m_vmFolders[VPox_SettingFolder].c_str());
    }
                                    ...
                                    const Utf8Str &strTargetFolder)
{
    ...
    try
    {
        ...
        while (itMedium != listOfDisks.end())
        {
            ...
            /* Drop lock early because IMedium::MoveTo needs to get the VirtualPox one. */
            machineLock.release();
    ...
    {
        Log2(("Exception during moving the disk %s\n", strLocation.c_str()));
        rc = VirtualPoxBase::handleUnexpectedExceptions(m_pMachine, RT_SRC_POS);
        machineLock.release();
    }
...
HRESULT MachineMoveVM::getFilesList(const Utf8Str &strRootFolder, fileList_t &filesList)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        hrc = m_pMachine->setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
...
HRESULT MachineMoveVM::queryMediasForAllStates()
{
    ...
    for (size_t i = 0; i < machineList.size(); ++i)
    {
        ...
        for (size_t a = 0; a < sfaAttachments.size(); ++a)
        {
            ...
            /* Check for "read-only" medium in terms that VPox can't create this one */
            ...
            while (!pMedium.isNull())
            {
                ...
                Utf8Str const &strFolder = m_vmFolders[VPox_SnapshotFolder];
...
HRESULT MachineMoveVM::isMediumTypeSupportedForMoving(const ComPtr<IMedium> &pMedium)
{
    ...
    /* Check whether VPox is able to create this medium format or not, i.e. medium can be "read-only" */
    Bstr bstrFormatName;
