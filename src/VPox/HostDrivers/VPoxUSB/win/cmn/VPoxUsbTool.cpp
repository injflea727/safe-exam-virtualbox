...
#define VPOXUSBTOOL_MEMTAG 'TUBV'

static PVOID vpoxUsbToolMemAlloc(SIZE_T cbBytes)
{
    PVOID pvMem = ExAllocatePoolWithTag(NonPagedPool, cbBytes, VPOXUSBTOOL_MEMTAG);
    Assert(pvMem);
    return pvMem;
}

static PVOID vpoxUsbToolMemAllocZ(SIZE_T cbBytes)
{
    PVOID pvMem = vpoxUsbToolMemAlloc(cbBytes);
    if (pvMem)
    {
...
static VOID vpoxUsbToolMemFree(PVOID pvMem)
{
    ExFreePoolWithTag(pvMem, VPOXUSBTOOL_MEMTAG);
}

VPOXUSBTOOL_DECL(PURB) VPoxUsbToolUrbAlloc(USHORT u16Function, USHORT cbSize)
{
    PURB pUrb = (PURB)vpoxUsbToolMemAlloc(cbSize);
    Assert(pUrb);
    if (!pUrb)
        return NULL;
...
VPOXUSBTOOL_DECL(PURB) VPoxUsbToolUrbAllocZ(USHORT u16Function, USHORT cbSize)
{
    PURB pUrb = (PURB)vpoxUsbToolMemAllocZ(cbSize);
    Assert(pUrb);
    if (!pUrb)
        return NULL;
...
VPOXUSBTOOL_DECL(PURB) VPoxUsbToolUrbReinit(PURB pUrb, USHORT cbSize, USHORT u16Function)
{
...
VPOXUSBTOOL_DECL(VOID) VPoxUsbToolUrbFree(PURB pUrb)
{
    vpoxUsbToolMemFree(pUrb);
}

VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolUrbPost(PDEVICE_OBJECT pDevObj, PURB pUrb, ULONG dwTimeoutMs)
{
    if (dwTimeoutMs == RT_INDEFINITE_WAIT)
        return VPoxUsbToolIoInternalCtlSendSync(pDevObj, IOCTL_INTERNAL_USB_SUBMIT_URB, pUrb, NULL);
    return VPoxUsbToolIoInternalCtlSendSyncWithTimeout(pDevObj, IOCTL_INTERNAL_USB_SUBMIT_URB, pUrb, NULL, dwTimeoutMs);
}

VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolGetDescriptor(PDEVICE_OBJECT pDevObj, void *pvBuffer, int cbBuffer, int Type, int iIndex, int LangId, ULONG dwTimeoutMs)
{
    NTSTATUS Status;
    USHORT cbUrb = sizeof (struct _URB_CONTROL_DESCRIPTOR_REQUEST);
    PURB pUrb = VPoxUsbToolUrbAllocZ(URB_FUNCTION_GET_DESCRIPTOR_FROM_DEVICE, cbUrb);
    if (!pUrb)
    {
    ...
    Status = VPoxUsbToolUrbPost(pDevObj, pUrb, dwTimeoutMs);
    ASSERT_WARN(Status == STATUS_SUCCESS, ("VPoxUsbToolUrbPost failed Status (0x%x)", Status));
...
VPOXUSBTOOL_DECL(VOID) VPoxUsbToolStringDescriptorToUnicodeString(PUSB_STRING_DESCRIPTOR pDr, PUNICODE_STRING pUnicode)
{
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolGetStringDescriptor(PDEVICE_OBJECT pDevObj, char *pszResult, ULONG cbResult,
                                                          int iIndex, int LangId, ULONG dwTimeoutMs)
{
    ...
    *pszResult = 0;
    ...
    NTSTATUS Status = VPoxUsbToolGetDescriptor(pDevObj, pDr, cbBuf, USB_STRING_DESCRIPTOR_TYPE, iIndex, LangId, dwTimeoutMs);
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolGetLangID(PDEVICE_OBJECT pDevObj, int *pLangId, ULONG dwTimeoutMs)
{
    char aBuf[MAXIMUM_USB_STRING_LENGTH];
    AssertCompile(sizeof (aBuf) <= UINT8_MAX);
    UCHAR cbBuf = (UCHAR)sizeof (aBuf);
    ...
    *pLangId = 0;
    ...
    NTSTATUS Status = VPoxUsbToolGetDescriptor(pDevObj, pDr, cbBuf, USB_STRING_DESCRIPTOR_TYPE, 0, 0, dwTimeoutMs);
    if (NT_SUCCESS(Status))
    {
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolGetDeviceSpeed(PDEVICE_OBJECT pDevObj, BOOLEAN *pbIsHigh)
{
    Assert(pbIsHigh);
    *pbIsHigh = FALSE;
    ...
    NTSTATUS Status = VPoxDrvToolIoPostSync(pDevObj, pIrp);
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolPipeClear(PDEVICE_OBJECT pDevObj, HANDLE hPipe, bool fReset)
{
    if (!hPipe)
    {
    ...
    PURB pUrb = VPoxUsbToolUrbAlloc(u16Function, sizeof (struct _URB_PIPE_REQUEST));
    if (!pUrb)
    {
    ...
    NTSTATUS Status = VPoxUsbToolUrbPost(pDevObj, pUrb, RT_INDEFINITE_WAIT);
    if (!NT_SUCCESS(Status) || !USBD_SUCCESS(pUrb->UrbHeader.Status))
    {
        AssertMsgFailed((__FUNCTION__": vpoxUsbToolRequest failed with %x (%x)\n", Status, pUrb->UrbHeader.Status));
    }
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolCurrentFrame(PDEVICE_OBJECT pDevObj, PIRP pIrp, PULONG piFrame)
{
    ...
    *piFrame = (ULONG)-1;
    ...
    NTSTATUS Status = VPoxUsbToolUrbPost(pDevObj, (PURB)&Urb, RT_INDEFINITE_WAIT);
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolDevUnconfigure(PDEVICE_OBJECT pDevObj)
{
    USHORT cbUrb = sizeof (struct _URB_SELECT_CONFIGURATION);
    PURB pUrb = VPoxUsbToolUrbAlloc(URB_FUNCTION_SELECT_CONFIGURATION, cbUrb);
    Assert(pUrb);
    if (!pUrb)
        return STATUS_INSUFFICIENT_RESOURCES;
...
VPOXUSBTOOL_DECL(PIRP) VPoxUsbToolIoBuildAsyncInternalCtl(PDEVICE_OBJECT pDevObj, ULONG uCtl, void *pvArg1, void *pvArg2)
{
    PIRP pIrp = IoAllocateIrp(pDevObj->StackSize, FALSE);
    Assert(pIrp);
    if (!pIrp)
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolIoInternalCtlSendSyncWithTimeout(PDEVICE_OBJECT pDevObj, ULONG uCtl,
                                                                       void *pvArg1, void *pvArg2, ULONG dwTimeoutMs)
{
     ...
     * */
    PIRP pIrp = VPoxUsbToolIoBuildAsyncInternalCtl(pDevObj, uCtl, pvArg1, pvArg2);
    if (!pIrp)
    {
        WARN(("VPoxUsbToolIoBuildAsyncInternalCtl failed"));
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    NTSTATUS Status = VPoxDrvToolIoPostSyncWithTimeout(pDevObj, pIrp, dwTimeoutMs);
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolIoInternalCtlSendAsync(PDEVICE_OBJECT pDevObj, ULONG uCtl, void *pvArg1, void *pvArg2,
...
VPOXUSBTOOL_DECL(NTSTATUS) VPoxUsbToolIoInternalCtlSendSync(PDEVICE_OBJECT pDevObj, ULONG uCtl, void *pvArg1, void *pvArg2)
{
    ...
    Status = VPoxUsbToolIoInternalCtlSendAsync(pDevObj, uCtl, pvArg1, pvArg2, &Event, &IoStatus);

    if (Status == STATUS_PENDING)
    {
        LOG(("VPoxUsbToolIoInternalCtlSendAsync returned pending for pDevObj(0x%p)", pDevObj));
        KeWaitForSingleObject(&Event, Executive, KernelMode, FALSE, NULL);
        Status = IoStatus.Status;
        LOG(("Pending VPoxUsbToolIoInternalCtlSendAsync completed with Status (0x%x) for pDevObj(0x%p)", Status, pDevObj));
    }
    else
    {
        LOG(("VPoxUsbToolIoInternalCtlSendAsync completed with Status (0x%x) for pDevObj(0x%p)", Status, pDevObj));
    }
