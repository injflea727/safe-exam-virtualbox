/* $Id: SessionImpl.cpp $ */
/** @file
 * VPox Client Session COM Class implementation in VPoxC.
 */
...
HRESULT Session::setName(const com::Utf8Str &aName)
{
    ...
    if (mState != SessionState_Unlocked)
        return setError(VPOX_E_INVALID_OBJECT_STATE, tr("Trying to set name for a session which is not in state \"unlocked\""));
...
HRESULT Session::getMachine(ComPtr<IMachine> &aMachine)
{
    ...
    HRESULT rc;
#ifndef VPOX_COM_INPROC_API_CLIENT
    if (mConsole)
...
#else  /* VPOX_COM_INPROC_API_CLIENT */
    RT_NOREF(aConsole);
    AssertFailed();
    return VPOX_E_INVALID_OBJECT_STATE;
#endif /* VPOX_COM_INPROC_API_CLIENT */
}

HRESULT Session::getNominalState(MachineState_T *aNominalState)
{
    AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
    AssertReturn(mState == SessionState_Locked, VPOX_E_INVALID_VM_STATE);
    AssertReturn(mType == SessionType_WriteLock, VPOX_E_INVALID_OBJECT_STATE);
#ifndef VPOX_COM_INPROC_API_CLIENT
    AssertReturn(mConsole, VPOX_E_INVALID_OBJECT_STATE);
...
#endif /* !VPOX_WITH_GENERIC_SESSION_WATCHER */
{
    ...
    AssertReturn(mState == SessionState_Unlocked, VPOX_E_INVALID_VM_STATE);
    ...
    if (!aMachine)
    {
        ...
        AssertReturn(mType == SessionType_Null, VPOX_E_INVALID_OBJECT_STATE);
    ...
    /*
     *  Reference the VirtualPox object to ensure the server is up
     *  until the session is closed
     */
    if (SUCCEEDED(rc))
       rc = aMachine->COMGETTER(Parent)(mVirtualPox.asOutParam());
...

{
    ...
    AssertReturn(mState == SessionState_Unlocked ||
                 mState == SessionState_Spawning, VPOX_E_INVALID_VM_STATE);
...
HRESULT Session::onUSBDeviceAttach(const ComPtr<IUSBDevice> &aDevice,
                                   const ComPtr<IVirtualPoxErrorInfo> &aError,
...
# ifndef VPOX_COM_INPROC_API_CLIENT
     ...
     * handles this correctly, by doing the operation in VPoxSVC. */
    if (!mConsole)
...
# else  /* VPOX_COM_INPROC_API_CLIENT */
     ...
     *        method call, VPoxSVC should be clever enough to see that the
...
{
#if defined(VPOX_WITH_GUEST_PROPS) && !defined(VPOX_COM_INPROC_API_CLIENT)
    if (mState != SessionState_Locked)
        return setError(VPOX_E_INVALID_VM_STATE,
                        tr("Machine is not locked by session (session state: %s)."),
                        Global::stringifySessionState(mState));
...
#else /* VPOX_WITH_GUEST_PROPS not defined */
    RT_NOREF(aPatterns, aKeys, aValues, aTimestamps, aFlags);
    ReturnComNotImplemented();
#endif /* VPOX_WITH_GUEST_PROPS not defined */
}
...
HRESULT Session::i_unlockMachine(bool aFinalRelease, bool aFromServer, AutoWriteLock &aLockW)
{
    ...
    if (mState != SessionState_Locked)
    {
         ...
         * a remote session to using IVirtualPox:: openRemoteSession(). It is OK
         ...
         * got from IVirtualPox:: openRemoteSession() to complete, so assert. */
        Assert(aFromServer);
...
#if !defined(DEBUG_bird) && !defined(DEBUG_andy) /* I don't want clients crashing on me just because VPoxSVC went belly up. */
        AssertComRC(rc);
#endif
    }
    ...
    /* release the VirtualPox instance as the very last step */
    mVirtualPox.setNull();
