...
/** @file
 * VPox Qt GUI - UICocoaApplication class implementation.
 */
...
@public
    ...
    uint32_t           fMask;
    /** Holds the callback. */
    PFNVPOXCACALLBACK  pfnCallback;
    /** Holds the user argument. */
    void              *pvUser;
}
- (id)initWithMask :(uint32)mask callback :(PFNVPOXCACALLBACK)callback user :(void *)user;
...
- (void)setCallback :(uint32_t)fMask :(PFNVPOXCACALLBACK)pfnCallback :(void *)pvUser;
- (void)unsetCallback :(uint32_t)fMask :(PFNVPOXCACALLBACK)pfnCallback :(void *)pvUser;
...
void UICocoaApplication::registerForNativeEvents(uint32_t fMask, PFNVPOXCACALLBACK pfnCallback, void *pvUser)
{
    [m_pNative setCallback:fMask :pfnCallback :pvUser];
}

void UICocoaApplication::unregisterForNativeEvents(uint32_t fMask, PFNVPOXCACALLBACK pfnCallback, void *pvUser)
{
