...
"""
VirtualPox Validation Kit - VM Move Test #1
"""
...
class SubTstDrvMoveVm1(base.SubTestDriverBase):
    ...
    def createTestMachine(self):
        ...
        aoDskFmts = self.oTstDrv.oVPoxMgr.getArray(self.oTstDrv.oVPox.systemProperties, 'mediumFormats')

        for oDskFmt in aoDskFmts:
            aoDskFmtCaps = self.oTstDrv.oVPoxMgr.getArray(oDskFmt, 'capabilities')
            if vpoxcon.MediumFormatCapabilities_File not in aoDskFmtCaps \
                or vpoxcon.MediumFormatCapabilities_CreateDynamic not in aoDskFmtCaps:
                continue
    ...
    def moveVMToLocation(self, sLocation, oVM):
        ...
        try:
            ...
            sType = 'basic'
            oProgress = vpoxwrappers.ProgressWrapper(oVM.moveTo(sLocation, sType), self.oTstDrv.oVPoxMgr, self.oTstDrv,
    ...
    def __testScenario_2(self, oSession, oMachine, sNewLoc, sOldLoc):
        ...
        sSettingFile = os.path.join(sNewLoc, os.path.join(oMachine.name, oMachine.name + '.vpox'))
    ...
    def __testScenario_4(self, oMachine, sNewLoc):
        ...
        if oMachine.state != self.oTstDrv.oVPoxMgr.constants.MachineState_Running:
    ...
    def testVMMove(self):
        ...
        Test machine moving.
        """
        if not self.oTstDrv.importVPoxApi():
            return False
        ...
        #    There are no any snapshots and logs.
        #    In this case only VM setting file should be moved (.vpox file)
        #
