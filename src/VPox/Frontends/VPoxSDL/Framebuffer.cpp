/* $Id: Framebuffer.cpp $ */
/** @file
 * VPoxSDL - Implementation of VPoxSDLFB (SDL framebuffer) class
 */
...
NS_IMPL_THREADSAFE_ISUPPORTS1_CI(VPoxSDLFB, IFramebuffer)
NS_DECL_CLASSINFO(VPoxSDLFB)
NS_IMPL_THREADSAFE_ISUPPORTS2_CI(VPoxSDLFBOverlay, IFramebufferOverlay, IFramebuffer)
NS_DECL_CLASSINFO(VPoxSDLFBOverlay)
#endif

#ifdef VPOX_SECURELABEL
/* function pointers */
extern "C"
{
...
HRESULT VPoxSDLFB::FinalConstruct()
{
    return 0;
}

void VPoxSDLFB::FinalRelease()
{
    return;
}
 ...
 */
HRESULT VPoxSDLFB::init(uint32_t uScreenId,
...
/* static */
bool VPoxSDLFB::init(bool fShowSDLConfig)
{
    LogFlow(("VPoxSDLFB::init\n"));
 ...
 */
STDMETHODIMP VPoxSDLFB::COMGETTER(Width)(ULONG *width)
{
    LogFlow(("VPoxSDLFB::GetWidth\n"));
    if (!width)
        return E_INVALIDARG;
    *width = mGuestXRes;
    return S_OK;
 ...
 */
STDMETHODIMP VPoxSDLFB::COMGETTER(Height)(ULONG *height)
{
    LogFlow(("VPoxSDLFB::GetHeight\n"));
    if (!height)
        return E_INVALIDARG;
    *height = mGuestYRes;
    return S_OK;
 ...
 */
STDMETHODIMP VPoxSDLFB::COMGETTER(BitsPerPixel)(ULONG *bitsPerPixel)
{
    LogFlow(("VPoxSDLFB::GetBitsPerPixel\n"));
    if (!bitsPerPixel)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP VPoxSDLFB::COMGETTER(BytesPerLine)(ULONG *bytesPerLine)
{
    LogFlow(("VPoxSDLFB::GetBytesPerLine\n"));
    if (!bytesPerLine)
        return E_INVALIDARG;
...
STDMETHODIMP VPoxSDLFB::COMGETTER(PixelFormat) (BitmapFormat_T *pixelFormat)
{
    if (!pixelFormat)
        return E_POINTER;
 ...
 */
STDMETHODIMP VPoxSDLFB::COMGETTER(HeightReduction)(ULONG *heightReduction)
{
    if (!heightReduction)
        return E_POINTER;
#ifdef VPOX_SECURELABEL
 ...
 */
STDMETHODIMP VPoxSDLFB::COMGETTER(Overlay)(IFramebufferOverlay **aOverlay)
{
    if (!aOverlay)
        return E_POINTER;
    /* Not yet implemented */
 ...
 */
STDMETHODIMP VPoxSDLFB::COMGETTER(WinId)(LONG64 *winId)
{
    if (!winId)
        return E_POINTER;
#ifdef RT_OS_DARWIN
    if (mWinId == NULL) /* (In case it failed the first time.) */
        mWinId = (intptr_t)VPoxSDLGetDarwinWindowId();
#endif
    *winId = mWinId;
    return S_OK;
}

STDMETHODIMP VPoxSDLFB::COMGETTER(Capabilities)(ComSafeArrayOut(FramebufferCapabilities_T, aCapabilities))
{
 ...
 *                   has to return immediately) or VPox should wait
 *                   for a call to the update complete API before
 *                   continuing with display updates.
 */
STDMETHODIMP VPoxSDLFB::NotifyUpdate(ULONG x, ULONG y,
                                     ULONG w, ULONG h)
{
    /*
     * The input values are in guest screen coordinates.
     */
    LogFlow(("VPoxSDLFB::NotifyUpdate: x = %d, y = %d, w = %d, h = %d\n",
             x, y, w, h));
...
#else /* !VPOXSDL_WITH_X11 */
    update(x, y, w, h, true /* fGuestRelative */);
#endif /* !VPOXSDL_WITH_X11 */

    return S_OK;
}

STDMETHODIMP VPoxSDLFB::NotifyUpdateImage(ULONG aX,
                                          ULONG aY,
                                          ULONG aWidth,
                                          ULONG aHeight,
...
STDMETHODIMP VPoxSDLFB::NotifyChange(ULONG aScreenId,
                                     ULONG aXOrigin,
                                     ULONG aYOrigin,
                                     ULONG aWidth,
                                     ULONG aHeight)
 ...
 */
STDMETHODIMP VPoxSDLFB::VideoModeSupported(ULONG width, ULONG height, ULONG bpp, BOOL *supported)
{
    RT_NOREF(bpp);
...
#ifdef DEBUG
        RTPrintf("VPoxSDL::VideoModeSupported: we refused mode %dx%dx%d\n", width, height, bpp);
#endif
        *supported = false;
    }
    else
...
STDMETHODIMP VPoxSDLFB::GetVisibleRegion(BYTE *aRectangles, ULONG aCount,
                                         ULONG *aCountCopied)
{
...
STDMETHODIMP VPoxSDLFB::SetVisibleRegion(BYTE *aRectangles, ULONG aCount)
{
    PRTRECT rects = (PRTRECT)aRectangles;
...
STDMETHODIMP VPoxSDLFB::ProcessVHWACommand(BYTE *pCommand, LONG enmCmd, BOOL fGuestCmd)
{
    RT_NOREF(pCommand, enmCmd, fGuestCmd);
    return E_NOTIMPL;
}

STDMETHODIMP VPoxSDLFB::Notify3DEvent(ULONG uType, ComSafeArrayIn(BYTE, aData))
{
...
void VPoxSDLFB::notifyChange(ULONG aScreenId)
{
    /* Disable screen updates. */
    RTCritSectEnter(&mUpdateLock);
...
void VPoxSDLFB::resizeGuest()
{
    ...
    if (mPtrVRAM)
    {
        ...
        LogFlow(("VPoxSDL:: using the source bitmap\n"));
    }
    else
    {
        mSurfVRAM = SDL_CreateRGBSurface(SDL_SWSURFACE, mGuestXRes, mGuestYRes, mBitsPerPixel,
                                         Rmask, Gmask, Bmask, Amask);
        LogFlow(("VPoxSDL:: using SDL_SWSURFACE\n"));
    }
    LogFlow(("VPoxSDL:: created VRAM surface %p\n", mSurfVRAM));

    if (mfSameSizeRequested)
    {
        mfSameSizeRequested = false;
        LogFlow(("VPoxSDL:: the same resolution requested, skipping the resize.\n"));
    }
    else
    {
 ...
 * @remarks Must be called from the SDL thread!
 */
void VPoxSDLFB::resizeSDL(void)
{
    LogFlow(("VPoxSDL:resizeSDL\n"));
 ...
 */
void VPoxSDLFB::update(int x, int y, int w, int h, bool fGuestRelative)
{
#ifdef VPOXSDL_WITH_X11
    AssertMsg(gSdlNativeThread == RTThreadNativeSelf(), ("Wrong thread! SDL is not threadsafe!\n"));
#endif
 ...
 * @remarks Must be called from the SDL thread!
 */
void VPoxSDLFB::repaint()
{
    AssertMsg(gSdlNativeThread == RTThreadNativeSelf(), ("Wrong thread! SDL is not threadsafe!\n"));
    LogFlow(("VPoxSDLFB::repaint\n"));
 ...
 * @remarks Must be called from the SDL thread!
 */
void VPoxSDLFB::setFullscreen(bool fFullscreen)
{
    AssertMsg(gSdlNativeThread == RTThreadNativeSelf(), ("Wrong thread! SDL is not threadsafe!\n"));
    LogFlow(("VPoxSDLFB::SetFullscreen: fullscreen: %d\n", fFullscreen));
    mfFullscreen = fFullscreen;
 ...
 */
void VPoxSDLFB::getFullscreenGeometry(uint32_t *width, uint32_t *height)
{
#ifndef VPOX_WITH_SDL2
    SDL_Rect **modes;
...
#ifdef VPOX_WITH_SDL2
int VPoxSDLFB::setWindowTitle(const char *pcszTitle)
{
 ...
 */
int VPoxSDLFB::initSecureLabel(uint32_t height, char *font, uint32_t pointsize, uint32_t labeloffs)
{
    LogFlow(("VPoxSDLFB:initSecureLabel: new offset: %d pixels, new font: %s, new pointsize: %d\n",
 ...
 * @remarks must be called from the SDL thread!
 */
void VPoxSDLFB::setSecureLabelText(const char *text)
{
 ...
 * @remarks must be called from the SDL thread!
 */
void VPoxSDLFB::setSecureLabelColor(uint32_t colorFG, uint32_t colorBG)
{
 ...
 * @remarks must be called from the SDL thread!
 */
void VPoxSDLFB::paintSecureLabel(int x, int y, int w, int h, bool fForce)
{
    RT_NOREF(x, w, h);
# ifdef VPOXSDL_WITH_X11
    AssertMsg(gSdlNativeThread == RTThreadNativeSelf(), ("Wrong thread! SDL is not threadsafe!\n"));
...
/**
 * Constructor for the VPoxSDLFBOverlay class (IFramebufferOverlay implementation)
 *
 ...
 */
VPoxSDLFBOverlay::VPoxSDLFBOverlay(ULONG x, ULONG y, ULONG width, ULONG height,
                                   BOOL visible, VPoxSDLFB *aParent) :
...
/**
 * Destructor for the VPoxSDLFBOverlay class.
 */
VPoxSDLFBOverlay::~VPoxSDLFBOverlay()
{
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(X)(ULONG *x)
{
    LogFlow(("VPoxSDLFBOverlay::GetX\n"));
    if (!x)
        return E_INVALIDARG;
    *x = mOverlayX;
    return S_OK;
 ...
 * @param   height Address of result buffer.
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(Y)(ULONG *y)
{
    LogFlow(("VPoxSDLFBOverlay::GetY\n"));
    if (!y)
        return E_INVALIDARG;
    *y = mOverlayY;
    return S_OK;
 ...
 * @param   width Address of result buffer.
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(Width)(ULONG *width)
{
    LogFlow(("VPoxSDLFBOverlay::GetWidth\n"));
    if (!width)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(BytesPerLine)(ULONG *bytesPerLine)
{
    LogFlow(("VPoxSDLFBOverlay::GetBytesPerLine\n"));
    if (!bytesPerLine)
        return E_INVALIDARG;
 ...
 * @param   height Address of result buffer.
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(Height)(ULONG *height)
{
    LogFlow(("VPoxSDLFBOverlay::GetHeight\n"));
    if (!height)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(Visible)(BOOL *visible)
{
    LogFlow(("VPoxSDLFBOverlay::GetVisible\n"));
    if (!visible)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMSETTER(Visible)(BOOL visible)
{
    LogFlow(("VPoxSDLFBOverlay::SetVisible\n"));
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(Alpha)(ULONG *alpha)
{
    RT_NOREF(alpha);
    LogFlow(("VPoxSDLFBOverlay::GetAlpha\n"));
    return E_NOTIMPL;
}
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMSETTER(Alpha)(ULONG alpha)
{
    RT_NOREF(alpha);
    LogFlow(("VPoxSDLFBOverlay::SetAlpha\n"));
    return E_NOTIMPL;
}
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(BitsPerPixel)(ULONG *bitsPerPixel)
{
    LogFlow(("VPoxSDLFBOverlay::GetBitsPerPixel\n"));
    if (!bitsPerPixel)
        return E_INVALIDARG;
    *bitsPerPixel = 32;
    return S_OK;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(PixelFormat)(ULONG *pixelFormat)
{
    LogFlow(("VPoxSDLFBOverlay::GetPixelFormat\n"));
    if (!pixelFormat)
        return E_INVALIDARG;
    *pixelFormat = BitmapFormat_BGR;
    return S_OK;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(UsesGuestVRAM)(BOOL *usesGuestVRAM)
{
    LogFlow(("VPoxSDLFBOverlay::GetUsesGuestVRAM\n"));
    if (!usesGuestVRAM)
        return E_INVALIDARG;
    *usesGuestVRAM = FALSE;
    return S_OK;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(HeightReduction)(ULONG *heightReduction)
{
    LogFlow(("VPoxSDLFBOverlay::GetHeightReduction\n"));
    if (!heightReduction)
        return E_INVALIDARG;
    *heightReduction = 0;
    return S_OK;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(Overlay)(IFramebufferOverlay **aOverlay)
{
    LogFlow(("VPoxSDLFBOverlay::GetOverlay\n"));
    if (!aOverlay)
        return E_INVALIDARG;
    *aOverlay = 0;
    return S_OK;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::COMGETTER(WinId)(LONG64 *winId)
{
    LogFlow(("VPoxSDLFBOverlay::GetWinId\n"));
    if (!winId)
        return E_INVALIDARG;
    *winId = 0;
    return S_OK;
 ...
 * @returns COM status code
 */
STDMETHODIMP VPoxSDLFBOverlay::Lock()
{
    LogFlow(("VPoxSDLFBOverlay::Lock\n"));
 ...
 * @returns COM status code
 */
STDMETHODIMP VPoxSDLFBOverlay::Unlock()
{
    LogFlow(("VPoxSDLFBOverlay::Unlock\n"));
    AssertMsgFailed(("You should not attempt to lock an IFramebufferOverlay object -\n"
                     "lock the parent IFramebuffer object instead.\n"));
    return E_NOTIMPL;
}
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::Move(ULONG x, ULONG y)
{
    mOverlayX = x;
    mOverlayY = y;
    return S_OK;
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::NotifyUpdate(ULONG x, ULONG y,
                            ULONG w, ULONG h)
{
 ...
 * @retval  finished    Set if the operation has completed.
 */
STDMETHODIMP VPoxSDLFBOverlay::RequestResize(ULONG aScreenId, ULONG pixelFormat, ULONG vram,
 ...
 */
STDMETHODIMP VPoxSDLFBOverlay::VideoModeSupported(ULONG width, ULONG height, ULONG bpp, BOOL *supported)
{
