...
{
#if   !defined(IN_RING3) && !defined(VPOX_STRICT)
...
#elif !defined(IN_RING3) && defined(VPOX_STRICT)
 ...
 * after a CR3 reload.
 *
 * @returns VPox status code suitable for scheduling.
...
#if 0 /* def VPOX_STRICT; triggers too often now */
    /*
 ...
 *
 * @returns Strict VPox status code, special care required.
...
{
    NOREF(pVM);
    VPOXSTRICTRC rc = EMInterpretInstruction(pVCpu, pRegFrame, pvFault);
    if (rc == VERR_EM_INTERPRETER)
        rc = VINF_EM_RAW_EMULATE_INSTR;
    if (rc != VINF_SUCCESS)
        Log(("PGMInterpretInstruction: returns %Rrc (pvFault=%RGv)\n", VPOXSTRICTRC_VAL(rc), pvFault));
    return rc;
}
 ...
 *
 * @returns VPox status code. May return VINF_PGM_SYNC_CR3 in RC/R0.
 ...
 *
 * @returns VPox status code, with the following informational code for
 *          VM scheduling.
 ...
 *
 * @returns VPox status code. May suspend or power off the VM on error, but this
        ...
        case PGMMODE_PAE:
            if (!pVM->cpum.ro.GuestFeatures.fPae)
                return VMSetRuntimeError(pVM, VMSETRTERR_FLAGS_FATAL, "PAEmode",
                                         N_("The guest is trying to switch to the PAE mode which is currently disabled by default in VirtualPox. PAE support can be enabled using the VM settings (System/Processor)"));
 ...
 */
#if (defined(VPOX_STRICT) && defined(IN_RING3)) || defined(DOXYGEN_RUNNING)
