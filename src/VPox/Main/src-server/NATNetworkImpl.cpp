...
HRESULT NATNetwork::init(VirtualPox *aVirtualPox, com::Utf8Str aName)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);

    m = new Data();
    /* share VirtualPox weakly */
...
HRESULT NATNetwork::i_saveSettings(settings::NATNetwork &data)
{
    ...
    m->pVirtualPox->i_onNATNetworkSetting(Bstr(m->s.strNetworkName).raw(),
                                          m->s.fEnabled,
...
HRESULT NATNetwork::setNetworkName(const com::Utf8Str &aNetworkName)
{
    ...
    HRESULT rc = m->pVirtualPox->i_saveSettings();
...
HRESULT NATNetwork::setNetwork(const com::Utf8Str &aIPv4NetworkCidr)
{
    ...
    AutoWriteLock vpoxLock(m->pVirtualPox COMMA_LOCKVAL_SRC_POS);
    HRESULT hrc = m->pVirtualPox->i_saveSettings();
...
HRESULT NATNetwork::setIPv6Prefix(const com::Utf8Str &aIPv6Prefix)
{
    ...
    AutoWriteLock vpoxLock(m->pVirtualPox COMMA_LOCKVAL_SRC_POS);
    hrc = m->pVirtualPox->i_saveSettings();
    ComAssertComRCRetRC(hrc);
                                       ...
                                       USHORT aGuestPort)
{
    ...
    m->pVirtualPox->i_onNATNetworkPortForward(Bstr(m->s.strNetworkName).raw(), TRUE, aIsIpv6,
...
HRESULT NATNetwork::removePortForwardRule(BOOL aIsIpv6, const com::Utf8Str &aPortForwardRuleName)
{
    ...
    m->pVirtualPox->i_onNATNetworkPortForward(Bstr(m->s.strNetworkName).raw(), FALSE, aIsIpv6,
...
void NATNetwork::i_updateDomainNameServerOption(ComPtr<IHost> &host)
{
    ...
    if (cAddresses)
    {
        ...
        /* The following code was copied (and adapted a bit) from VPoxNetDhcp::hostDnsServers */
        /*
...
{
    ComPtr<IHost> host;
    if (SUCCEEDED(m->pVirtualPox->COMGETTER(Host)(host.asOutParam())))
    {
...
HRESULT  NATNetwork::start()
{
    ...
    if (m->s.fNeedDhcpServer)
    {
         ...
         * 6. start dhcp server.
         */
        HRESULT hrc = m->pVirtualPox->FindDHCPServerByNetworkName(Bstr(m->s.strNetworkName).raw(),
                                                                  m->dhcpServer.asOutParam());
        switch (hrc)
        {
            case E_INVALIDARG:
                /* server haven't beeen found let create it then */
                hrc = m->pVirtualPox->CreateDHCPServer(Bstr(m->s.strNetworkName).raw(),
                                                       m->dhcpServer.asOutParam());
                if (FAILED(hrc))
                  return E_FAIL;
                /* breakthrough */
        ...
        i_updateDnsOptions();
#endif /* VPOX_WITH_DHCPD */
        /* XXX: AddGlobalOption(DhcpOpt_Router,) - enables attachement of DhcpServer to Main (no longer true with VPoxNetDhcpd). */
        ComPtr<IDHCPGlobalConfig> pDHCPConfig;
        hrc = m->dhcpServer->COMGETTER(GlobalConfig)(pDHCPConfig.asOutParam());
        if (FAILED(hrc))
        {
    ...
    {
        m->pVirtualPox->i_onNATNetworkStartStop(Bstr(m->s.strNetworkName).raw(), TRUE);
        return S_OK;
    }
...
{
#ifdef VPOX_WITH_NAT_SERVICE
    m->pVirtualPox->i_onNATNetworkStartStop(Bstr(m->s.strNetworkName).raw(), FALSE);
