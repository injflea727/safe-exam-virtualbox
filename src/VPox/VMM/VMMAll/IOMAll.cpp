 ...
 *
 * @returns Strict VPox status code. Informational status codes other than the one documented
 ...
 *        handle is buggy and doesn't handle all cases. */
    ...
    if (pRegEntry)
    {
         ...
         * Call the device.
         */
        VPOXSTRICTRC rcStrict = PDMCritSectEnter(pDevIns->CTX_SUFF(pCritSectRo), VINF_IOM_R3_IOPORT_READ);
 ...
 */
VMM_INT_DECL(VPOXSTRICTRC) IOMIOPortReadString(PVMCC pVM, PVMCPU pVCpu, RTIOPORT uPort,
...
# endif
            ...
            Log3(("IOMIOPortReadStr: uPort=%RTiop pvDst=%p pcTransfer=%p:{%#x->%#x} cb=%d rc=%Rrc\n",
                  uPort, pvDst, pcTransfers, cRequestedTransfers, *pcTransfers, cb, VPOXSTRICTRC_VAL(rcStrict)));
        }
#ifndef IN_RING3
        else
            STAM_COUNTER_INC(&pStats->InRZToR3);
 ...
 */
static VPOXSTRICTRC iomIOPortRing3WritePending(PVMCPU pVCpu, RTIOPORT Port, uint32_t u32Value, size_t cbValue)
{
...
VMMDECL(VPOXSTRICTRC) IOMIOPortWrite(PVMCC pVM, PVMCPU pVCpu, RTIOPORT Port, uint32_t u32Value, size_t cbValue)
{
    ...
    if (pRegEntry)
    {
         ...
         * Call the device.
         */
        VPOXSTRICTRC rcStrict = PDMCritSectEnter(pDevIns->CTX_SUFF(pCritSectRo), VINF_IOM_R3_IOPORT_WRITE);
        if (rcStrict == VINF_SUCCESS)
        {
...
# endif
            ...
            Log3(("IOMIOPortWriteStr: uPort=%RTiop pvSrc=%p pcTransfer=%p:{%#x->%#x} cb=%d rcStrict=%Rrc\n",
                  uPort, pvSrc, pcTransfers, cRequestedTransfers, *pcTransfers, cb, VPOXSTRICTRC_VAL(rcStrict)));
        }
#ifndef IN_RING3
        else
            STAM_COUNTER_INC(&pStats->OutRZToR3);
