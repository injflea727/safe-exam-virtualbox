...
/* Context activation is done by the caller. */
BOOL IWineD3DImpl_FillGLCaps(struct wined3d_adapter *adapter, struct VPOXVMSVGASHADERIF *pVPoxShaderIf)
{
#ifndef VPOX_WITH_VMSVGA
    ...
    VPOX_CHECK_GL_CALL(gl_renderer_str = (const char *)glGetString(GL_RENDERER));
    ...
    VPOX_CHECK_GL_CALL(gl_vendor_str = (const char *)glGetString(GL_VENDOR));
    ...
    VPOX_CHECK_GL_CALL(gl_version_str = (const char *)glGetString(GL_VERSION));
    ...
    VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_CLIP_PLANES, &gl_max));
    ...
    {
        pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, true /*fOtherProfile*/);
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_LIGHTS, &gl_max));
        pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, false /*fOtherProfile*/);
    }
#else
    VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_LIGHTS, &gl_max));
#endif
    ...
    VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_SIZE, &gl_max));
    ...
    {
        pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, true /*fOtherProfile*/);
        VPOX_CHECK_GL_CALL(glGetFloatv(GL_ALIASED_POINT_SIZE_RANGE, gl_floatv));
        if (glGetError() != GL_NO_ERROR)
    ...
    gl_info->supported[VPOX_SHARED_CONTEXTS] = TRUE;

#ifdef VPOX_WITH_VMSVGA
    {
        void *pvEnumCtx = NULL;
        char  szCurExt[256];
        while (pVPoxShaderIf->pfnGetNextExtension(pVPoxShaderIf, &pvEnumCtx, szCurExt, sizeof(szCurExt), false /*fOtherProfile*/))
            check_gl_extension(gl_info, szCurExt);

        /* The cheap way out. */
        pvEnumCtx = NULL;
        while (pVPoxShaderIf->pfnGetNextExtension(pVPoxShaderIf, &pvEnumCtx, szCurExt, sizeof(szCurExt), true /*fOtherProfile*/))
            check_gl_extension(gl_info, szCurExt);
    }
#else /* !VPOX_WITH_VMSVGA */
    while (*GL_Extensions)
    {
        const char *start;
...
#undef USE_GL_FUNC
    ...
    if (gl_info->supported[NV_REGISTER_COMBINERS])
    {
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_GENERAL_COMBINERS_NV, &gl_max));
        gl_info->limits.general_combiners = gl_max;
        TRACE_(d3d_caps)("Max general combiners: %d.\n", gl_max);
    }
    if (gl_info->supported[ARB_DRAW_BUFFERS])
    {
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_DRAW_BUFFERS_ARB, &gl_max));
    ...
    if (gl_info->supported[ARB_MULTITEXTURE])
    {
        ...
        if (glGetError() != GL_NO_ERROR)
            VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &gl_max));
#else
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_UNITS_ARB, &gl_max));
#endif
        ...
        {
            GLint tmp;
            VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS_ARB, &tmp));
        ...
        {
            GLint tmp;
            VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS_ARB, &tmp));
            gl_info->limits.vertex_samplers = tmp;
            VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS_ARB, &tmp));
    ...
    if (gl_info->supported[ARB_VERTEX_BLEND])
    {
        ...
        {
            pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, true /*fOtherProfile*/);
            VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VERTEX_UNITS_ARB, &gl_max));
            pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, false /*fOtherProfile*/);
        }
#else
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VERTEX_UNITS_ARB, &gl_max));
    ...
    {
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_3D_TEXTURE_SIZE_EXT, &gl_max));
        gl_info->limits.texture3d_size = gl_max;
        TRACE_(d3d_caps)("Max texture3D size: %d.\n", gl_info->limits.texture3d_size);
    }
    if (gl_info->supported[EXT_TEXTURE_FILTER_ANISOTROPIC])
    {
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &gl_max));
    ...
    if (gl_info->supported[ARB_FRAGMENT_PROGRAM])
    {
        ...
        if (glGetError() != GL_NO_ERROR)
            pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, true /*fOtherProfile*/);
#endif
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_ENV_PARAMETERS_ARB, &gl_max)));
        gl_info->limits.arb_ps_float_constants = gl_max;
        TRACE_(d3d_caps)("Max ARB_FRAGMENT_PROGRAM float constants: %d.\n", gl_info->limits.arb_ps_float_constants);
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_PARAMETERS_ARB, &gl_max)));
        gl_info->limits.arb_ps_native_constants = gl_max;
        TRACE_(d3d_caps)("Max ARB_FRAGMENT_PROGRAM native float constants: %d.\n",
                gl_info->limits.arb_ps_native_constants);
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_TEMPORARIES_ARB, &gl_max)));
        gl_info->limits.arb_ps_temps = gl_max;
        TRACE_(d3d_caps)("Max ARB_FRAGMENT_PROGRAM native temporaries: %d.\n", gl_info->limits.arb_ps_temps);
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_INSTRUCTIONS_ARB, &gl_max)));
        gl_info->limits.arb_ps_instructions = gl_max;
        TRACE_(d3d_caps)("Max ARB_FRAGMENT_PROGRAM native instructions: %d.\n", gl_info->limits.arb_ps_instructions);
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_LOCAL_PARAMETERS_ARB, &gl_max)));
    ...
    if (gl_info->supported[ARB_VERTEX_PROGRAM])
    {
        ...
        if (glGetError() != GL_NO_ERROR)
            pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, true /*fOtherProfile*/);
#endif
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_ENV_PARAMETERS_ARB, &gl_max)));
        gl_info->limits.arb_vs_float_constants = gl_max;
        TRACE_(d3d_caps)("Max ARB_VERTEX_PROGRAM float constants: %d.\n", gl_info->limits.arb_vs_float_constants);
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_PARAMETERS_ARB, &gl_max)));
        gl_info->limits.arb_vs_native_constants = gl_max;
        TRACE_(d3d_caps)("Max ARB_VERTEX_PROGRAM native float constants: %d.\n",
                gl_info->limits.arb_vs_native_constants);
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_TEMPORARIES_ARB, &gl_max)));
        gl_info->limits.arb_vs_temps = gl_max;
        TRACE_(d3d_caps)("Max ARB_VERTEX_PROGRAM native temporaries: %d.\n", gl_info->limits.arb_vs_temps);
        VPOX_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_INSTRUCTIONS_ARB, &gl_max)));
    ...
    if (gl_info->supported[ARB_VERTEX_SHADER])
    {
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VERTEX_UNIFORM_COMPONENTS_ARB, &gl_max));
    ...
    {
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_FRAGMENT_UNIFORM_COMPONENTS_ARB, &gl_max));
        ...
        {
            pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, true /*fOtherProfile*/);
            VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VARYING_FLOATS_ARB, &gl_max));
            pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, false /*fOtherProfile*/);
        }
#else
        VPOX_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VARYING_FLOATS_ARB, &gl_max));
    ...
    if (gl_info->supported[NV_LIGHT_MAX_EXPONENT])
    {
        ...
        {
            pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, true /*fOtherProfile*/);
            VPOX_CHECK_GL_CALL(glGetFloatv(GL_MAX_SHININESS_NV, &gl_info->limits.shininess));
            pVPoxShaderIf->pfnSwitchInitProfile(pVPoxShaderIf, false /*fOtherProfile*/);
        }
#else
        VPOX_CHECK_GL_CALL(glGetFloatv(GL_MAX_SHININESS_NV, &gl_info->limits.shininess));
    ...
    if (hdc) {
        /* Not all GL drivers might offer WGL extensions e.g. VirtualPox */
