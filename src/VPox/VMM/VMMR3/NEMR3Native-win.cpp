...
VPOXSTRICTRC nemR3NativeRunGC(PVM pVM, PVMCPU pVCpu)
{
#ifdef NEM_WIN_WITH_RING0_RUNLOOP
    if (pVM->nem.s.fUseRing0Runloop)
    {
        for (;;)
        {
            VPOXSTRICTRC rcStrict = VMMR3CallR0EmtFast(pVM, pVCpu, VMMR0_DO_NEM_RUN);
            if (RT_SUCCESS(rcStrict))
            {
                 ...
                 */
                /* Status codes: */
                VPOXSTRICTRC rcPending = pVCpu->nem.s.rcPending;
 ...
 * @endverbatim
 *
 * Same everything but native VT-x and VPox (stripped output a little):
 * @verbatim
bs3-memalloc-1: From 0x100000000 to 0xc20000000
bs3-memalloc-1:   Pages                                                     :       11 665 408 pages
bs3-memalloc-1:   MiBs                                                      :           45 568 MB
 ...
 * @endverbatim
 *
 * VPox with large pages disabled:
 * @verbatim
bs3-memalloc-1: From 0x100000000 to 0xc20000000
bs3-memalloc-1:   Pages                                                     :       11 665 408 pages
bs3-memalloc-1:   MiBs                                                      :           45 568 MB
 ...
 * of magnitude.  When disabling large pages in VPox the allocation numbers are
 * closer, and the is clear from the 2nd and 3rd access tests that VPox doesn't
 ...
 * (internal r123172) running a the release build of VirtualPox from the same
 ...
 * managing execution.  The 3rd column is regular VirtualPox using AMD-V directly,
 * hyper-V is disabled, main execution loop in ring-0.
 *
 * @verbatim
TESTING...                                                           WinHv API           Hypercalls + VID    VirtualPox AMD-V
 ...
 *    handle directly in ring-0 in the VPox AMD-V code.
 *
 *  - The WinHv API approach is 2 to 3 times slower for exits we have to
 *    go to ring-3 to handle with the VPox AMD-V code.
 *
 ...
 * VPox.log timestamps, starting with the state change to 'RUNNING' and stopping
 * at 'POWERING_OFF'.
 *
 * The host OS and VirtualPox build is the same as for the bootsector2-test1
 * scenario.
 *
 * Results:
 *
 ...
 *          3 min 23.18 seconds
 *
 *  - Regular VirtualPox using AMD-V directly, hyper-V is disabled, main
 ...
 *
 * Against host system:
 *   - 152 258 ns/roundtrip - 100% - regular VirtualPox SVM
 ...
 *   - 243 969 ns/roundtrip - 100% - regular VirtualPox SVM
