/* $Id: VPoxSharedClipboardSvc-win.cpp $ */
/** @file
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int vpoxClipboardSvcWinSyncInternal(PSHCLCONTEXT pCtx);
...
static void vpoxClipboardSvcWinGetData(uint32_t u32Format, const void *pvSrc, uint32_t cbSrc,
                                       void *pvDst, uint32_t cbDst, uint32_t *pcbActualDst)
{
    LogFlowFunc(("cbSrc = %d, cbDst = %d\n", cbSrc, cbDst));

    if (   u32Format == VPOX_SHCL_FMT_HTML
...
static int vpoxClipboardSvcWinDataSet(PSHCLCONTEXT pCtx, UINT cfFormat, void *pvData, uint32_t cbData)
{
...
static int vpoxClipboardSvcWinDataRead(PSHCLCONTEXT pCtx, UINT uFormat, void **ppvData, uint32_t *pcbData)
{
    SHCLFORMAT fFormat = SharedClipboardWinClipboardFormatToVPox(uFormat);
...
static LRESULT CALLBACK vpoxClipboardSvcWinWndProcMain(PSHCLCONTEXT pCtx,
                                                       HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        case WM_CLIPBOARDUPDATE:
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (pWinCtx->hWndClipboardOwnerUs != hWndClipboardOwner)
                {
                    ...
                    rc = vpoxClipboardSvcWinSyncInternal(pCtx);
                }
                else
                {
                    int rc2 = RTCritSectLeave(&pWinCtx->CritSect);
        ...
        case WM_RENDERFORMAT:
        {
            ...
            const SHCLFORMAT fFormat = SharedClipboardWinClipboardFormatToVPox(uFormat);
            ...
            else
            {
                void    *pvData = NULL;
                uint32_t cbData = 0;
                int rc = vpoxClipboardSvcWinDataRead(pCtx, uFormat, &pvData, &cbData);
                if (   RT_SUCCESS(rc)
                    && pvData
                    && cbData)
                {
                    /* Wrap HTML clipboard content info CF_HTML format if needed. */
                    if (fFormat == VPOX_SHCL_FMT_HTML
                    ...
                    rc = vpoxClipboardSvcWinDataSet(pCtx, uFormat, pvData, cbData);
 ...
 */
static LRESULT CALLBACK vpoxClipboardSvcWinWndProcInstance(HWND hWnd, UINT uMsg,
                                                           WPARAM wParam, LPARAM lParam)
{
    ...
    if (pCtx)
        return vpoxClipboardSvcWinWndProcMain(pCtx, hWnd, uMsg, wParam, lParam);
 ...
 */
static LRESULT CALLBACK vpoxClipboardSvcWinWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (uMsg == WM_NCCREATE)
    {
        ...
        SetWindowLongPtr(hWnd, GWLP_WNDPROC, (LONG_PTR)vpoxClipboardSvcWinWndProcInstance);

        return vpoxClipboardSvcWinWndProcInstance(hWnd, uMsg, wParam, lParam);
    }
...
DECLCALLBACK(int) vpoxClipboardSvcWinThread(RTTHREAD hThreadSelf, void *pvUser)
{
    LogFlowFuncEnter();
    ...
    wc.style         = CS_NOCLOSE;
    wc.lpfnWndProc   = vpoxClipboardSvcWinWndProc;
 ...
 *
 * @returns VPox status code, VINF_NO_CHANGE if no synchronization was required.
...
static int vpoxClipboardSvcWinSyncInternal(PSHCLCONTEXT pCtx)
{
    ...
    if (pCtx->pClient)
    {
        ...
        if (   RT_SUCCESS(rc)
            && fFormats != VPOX_SHCL_FMT_NONE /** @todo r=bird: BUGBUG: revisit this. */
...
int ShClSvcImplInit(VPOXHGCMSVCFNTABLE *pTable)
{
    RT_NOREF(pTable);
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
    HRESULT hr = OleInitialize(NULL);
...
int ShClSvcImplConnect(PSHCLCLIENT pClient, bool fHeadless)
{
    ...
    if (pCtx)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            rc = RTThreadCreate(&pCtx->hThread, vpoxClipboardSvcWinThread, pCtx /* pvUser */, _64K /* Stack size */,
...
int ShClSvcImplSync(PSHCLCLIENT pClient)
{
    ...
    return vpoxClipboardSvcWinSyncInternal(pClient->State.pCtx);
}
                        ...
                        SHCLFORMAT uFormat, void *pvData, uint32_t cbData, uint32_t *pcbActual)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (uFormat & VPOX_SHCL_FMT_BITMAP)
        {
            ...
            if (hClip != NULL)
            {
                ...
                if (lp != NULL)
                {
                    ...
                    vpoxClipboardSvcWinGetData(VPOX_SHCL_FMT_BITMAP, lp, GlobalSize(hClip),
        ...
        else if (uFormat & VPOX_SHCL_FMT_UNICODETEXT)
        {
            ...
            if (hClip != NULL)
            {
                ...
                if (uniString != NULL)
                {
                    ...
                    vpoxClipboardSvcWinGetData(VPOX_SHCL_FMT_UNICODETEXT, uniString, (lstrlenW(uniString) + 1) * 2,
                                               pvData, cbData, pcbActual);
            ...
            if (format != 0)
            {
                ...
                if (hClip != NULL)
                {
                    ...
                    if (lp != NULL)
                    {
                        ...
                        vpoxClipboardSvcWinGetData(VPOX_SHCL_FMT_HTML, lp, GlobalSize(hClip),
                                                   pvData, cbData, pcbActual);
#ifdef VPOX_STRICT
...
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
        else if (uFormat & VPOX_SHCL_FMT_URI_LIST)
        {
            AssertFailed(); /** @todo */
        }
    ...
    if (hClip == NULL)
    {
        ...
        vpoxClipboardSvcWinGetData(0, NULL, 0, pvData, cbData, pcbActual);
    }
