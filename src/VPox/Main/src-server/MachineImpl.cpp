/* $Id: MachineImpl.cpp $ */
/** @file
 * Implementation of IMachine in VPoxSVC.
 */
 ...
 */
HRESULT Machine::init(VirtualPox *aParent,
 ...
 *
 *      -- from VirtualPox::initMachines() during VirtualPox startup; in that case, the
 *         UUID is specified and we mark the machine as "registered";
 *
 *      -- from the public VirtualPox::OpenMachine() API, in which case the UUID is NULL
 ...
 *  @return  Success indicator. if not S_OK, the machine object is invalid
 */
HRESULT Machine::initFromSettings(VirtualPox *aParent,
                                  const Utf8Str &strConfigFile,
                                  const Guid *aId)
{
    LogFlowThisFuncEnter();
    ...
    {
        // opening an unregistered VM (VirtualPox::OpenMachine()):
        rc = initDataAndChildObjects();
 ...
 *  @param strSettingsFilename File name of .vpox file.
 ...
 * @param   aParent         The VirtualPox object.
 * @param   strConfigFile   Settings file.
 * @return
 */
HRESULT Machine::initImpl(VirtualPox *aParent,
                          const Utf8Str &strConfigFile)
{
    LogFlowThisFuncEnter();
    ...
    if (RT_FAILURE(vrc1))
        return setErrorBoth(VPOX_E_FILE_ERROR, vrc1,
...
HRESULT Machine::i_tryCreateMachineConfigFile(bool fForceOverwrite)
{
       ...
       )
    {
        ...
        else
        {
            ...
            if (RT_FAILURE(vrc2))
                rc = setErrorBoth(VPOX_E_FILE_ERROR, vrc2,
 ...
 *  retry the operation after VirtualPox startup instead of refusing to
 *  startup the whole VirtualPox server in case if the settings file of some
...
void Machine::uninit()
{
    ...
    {
        /* Theoretically, this can only happen if the VirtualPox server has been
         ...
         * VirtualPox::uninit(), we may be sure that SessionMachine::uninit()
         * won't happen on the client watcher thread (because it has a
         * VirtualPox caller for the duration of the
         * SessionMachine::i_checkForDeath() call, so that VirtualPox::uninit()
         * cannot happen until the VirtualPox caller is released). This is
...
/////////////////////////////////////////////////////////////////////////////
HRESULT Machine::getParent(ComPtr<IVirtualPox> &aParent)
{
    /* mParent is constant during life time, no need to lock */
    ComObjPtr<VirtualPox> pVirtualPox(mParent);
    aParent = pVirtualPox;
...
HRESULT Machine::getAccessError(ComPtr<IVirtualPoxErrorInfo> &aAccessError)
{
    ...
    ComObjPtr<VirtualPoxErrorInfo> errorInfo;
...
HRESULT Machine::setHardwareVersion(const com::Utf8Str &aHardwareVersion)
{
    ...
    if (    hwVersion.compare("1") != 0
        &&  hwVersion.compare("2") != 0)    // VPox 2.1.x and later (VMMDev heap)
                             ...
                             LockType_T aLockType)
{
    ...
    if (state != SessionState_Unlocked)
        return setError(VPOX_E_INVALID_OBJECT_STATE,
       ...
       )
    {
        ...
        // 1) VPoxSVC (where this code runs);
        ...
        if (mData->mSession.mState != SessionState_Locked)
        {
            ...
            return setError(VPOX_E_INVALID_SESSION_STATE,
    ...
    else
    {
         ...
         * (through the ComObjPtr's destructor) which requests the VirtualPox write
        ...
        /* Release the lock since SessionMachine::uninit() locks VirtualPox which
                              ...
                              const ComPtr<IMedium> &aMedium)
{
    ...
    strReconfig = i_getExtraData(Utf8Str("VPoxInternal2/SilentReconfigureWhilePaused"));
                                        ...
                                        aDevice)))
    {
        ...
        if (pMedium)
        {
            ...
            return setError(VPOX_E_OBJECT_IN_USE,
    ...
    if (!medium.isNull())
    {
        ...
        // by older VirtualPox version, so needs no global config version bump either.
        // For hard disks this type is not accepted.
        if (mtype == MediumType_MultiAttach)
        {
            // This type is new with VirtualPox 4.0 and therefore requires settings
            ...
            if (    medium->i_isInRegistry(uuidGlobalRegistry)
                ...
                return setError(VPOX_E_INVALID_OBJECT_STATE,
                                tr("Cannot attach medium '%s': the media type 'MultiAttach' can only be attached "
                                   "to machines that were created with VirtualPox 4.0 or later"),
                                medium->i_getLocationFull().c_str());
        }
    }
                              ...
                              LONG aDevice)
{
    ...
    if (fHotplug && !pAttach->i_getHotPluggable())
        return setError(VPOX_E_NOT_SUPPORTED,
...
HRESULT Machine::getNetworkAdapter(ULONG aSlot, ComPtr<INetworkAdapter> &aAdapter)
{
    /* Do not assert if slot is out of range, just return the advertised
       status.  testdriver/vpox.py triggers this in logVmInfo. */
...
HRESULT Machine::setExtraData(const com::Utf8Str &aKey, const com::Utf8Str &aValue)
{
    ...
    if ((fChanged = (strOldValue != aValue)))
    {
        ...
        // i_onExtraDataCanChange() only briefly requests the VirtualPox
        ...
        {
            // save the global settings; for that we should hold only the VirtualPox lock
            alock.release();
...
HRESULT Machine::discardSettings()
{
     ...
     * 11:06:01.934284 00:00:05.805182 ALIEN-1  My class:    Prior: #00: 2-VIRTUALPOXOBJECT, manually    , 4 lookups
     ...
     * 11:06:01.934284 00:00:05.805182 ALIEN-1  Other class: Prior: #00: 2-VIRTUALPOXOBJECT, manually    , 2 lookups
     ...
     * 00 00000000`0287bc90 00007ffc`8c0bc8dc VPoxRT!rtLockValComplainPanic+0x23 [e:\vpox\svn\trunk\src\vpox\runtime\common\misc\lockvalidator.cpp @ 807]
     * 01 00000000`0287bcc0 00007ffc`8c0bc083 VPoxRT!rtLockValidatorStackWrongOrder+0xac [e:\vpox\svn\trunk\src\vpox\runtime\common\misc\lockvalidator.cpp @ 2149]
     * 02 00000000`0287bd10 00007ffc`8c0bbfc3 VPoxRT!rtLockValidatorStackCheckLockingOrder2+0x93 [e:\vpox\svn\trunk\src\vpox\runtime\common\misc\lockvalidator.cpp @ 2227]
     * 03 00000000`0287bdd0 00007ffc`8c0bf3c0 VPoxRT!rtLockValidatorStackCheckLockingOrder+0x523 [e:\vpox\svn\trunk\src\vpox\runtime\common\misc\lockvalidator.cpp @ 2406]
     * 04 00000000`0287be40 00007ffc`8c180de4 VPoxRT!RTLockValidatorRecSharedCheckOrder+0x210 [e:\vpox\svn\trunk\src\vpox\runtime\common\misc\lockvalidator.cpp @ 3607]
     * 05 00000000`0287be90 00007ffc`8c1819b8 VPoxRT!rtCritSectRwEnterShared+0x1a4 [e:\vpox\svn\trunk\src\vpox\runtime\generic\critsectrw-generic.cpp @ 222]
     * 06 00000000`0287bf60 00007ff6`853f5e78 VPoxRT!RTCritSectRwEnterSharedDebug+0x58 [e:\vpox\svn\trunk\src\vpox\runtime\generic\critsectrw-generic.cpp @ 428]
     * 07 00000000`0287bfb0 00007ff6`853f6c34 VPoxSVC!util::RWLockHandle::lockRead+0x58 [e:\vpox\svn\trunk\src\vpox\main\glue\autolock.cpp @ 245]
     * 08 00000000`0287c000 00007ff6`853f68a1 VPoxSVC!util::AutoReadLock::callLockImpl+0x64 [e:\vpox\svn\trunk\src\vpox\main\glue\autolock.cpp @ 552]
     * 09 00000000`0287c040 00007ff6`853f6a59 VPoxSVC!util::AutoLockBase::callLockOnAllHandles+0xa1 [e:\vpox\svn\trunk\src\vpox\main\glue\autolock.cpp @ 455]
     * 0a 00000000`0287c0a0 00007ff6`85038fdb VPoxSVC!util::AutoLockBase::acquire+0x89 [e:\vpox\svn\trunk\src\vpox\main\glue\autolock.cpp @ 500]
     * 0b 00000000`0287c0d0 00007ff6`85216dcf VPoxSVC!util::AutoReadLock::AutoReadLock+0x7b [e:\vpox\svn\trunk\include\vpox\com\autolock.h @ 370]
     * 0c 00000000`0287c120 00007ff6`8521cf08 VPoxSVC!VirtualPox::i_findMachine+0x14f [e:\vpox\svn\trunk\src\vpox\main\src-server\virtualpoximpl.cpp @ 3216]
     * 0d 00000000`0287c260 00007ff6`8517a4b0 VPoxSVC!VirtualPox::i_markRegistryModified+0xa8 [e:\vpox\svn\trunk\src\vpox\main\src-server\virtualpoximpl.cpp @ 4697]
     * 0e 00000000`0287c2f0 00007ff6`8517fac0 VPoxSVC!Medium::i_markRegistriesModified+0x170 [e:\vpox\svn\trunk\src\vpox\main\src-server\mediumimpl.cpp @ 4056]
     * 0f 00000000`0287c500 00007ff6`8511ca9d VPoxSVC!Medium::i_deleteStorage+0xb90 [e:\vpox\svn\trunk\src\vpox\main\src-server\mediumimpl.cpp @ 5114]
     * 10 00000000`0287cad0 00007ff6`8511ef0e VPoxSVC!Machine::i_deleteImplicitDiffs+0x11ed [e:\vpox\svn\trunk\src\vpox\main\src-server\machineimpl.cpp @ 11117]
     * 11 00000000`0287d2e0 00007ff6`8511f896 VPoxSVC!Machine::i_rollbackMedia+0x42e [e:\vpox\svn\trunk\src\vpox\main\src-server\machineimpl.cpp @ 11657]
     * 12 00000000`0287d3c0 00007ff6`850fd17a VPoxSVC!Machine::i_rollback+0x6a6 [e:\vpox\svn\trunk\src\vpox\main\src-server\machineimpl.cpp @ 11786]
     * 13 00000000`0287d710 00007ff6`85342dbe VPoxSVC!Machine::discardSettings+0x9a [e:\vpox\svn\trunk\src\vpox\main\src-server\machineimpl.cpp @ 5096]
     * 14 00000000`0287d790 00007ffc`c06813ff VPoxSVC!MachineWrap::DiscardSettings+0x16e [e:\vpox\svn\trunk\out\win.amd64\debug\obj\vpoxapiwrap\machinewrap.cpp @ 9171]
     *
     */
                            ...
                            std::vector<ComPtr<IMedium> > &aMedia)
{
    ...
    if (!mData->mAccessible)
    {
        ...
        mParent->i_unregisterMachine(this, id);
            // calls VirtualPox::i_saveSettings()
    ...
    mParent->i_unregisterMachine(this, id);
            // calls VirtualPox::i_saveSettings() and VirtualPox::saveModifiedRegistries()
...
void Machine::i_deleteConfigHandler(DeleteConfigTask &task)
{
    ...
    try
    {
             ...
             ++it)
        {
            ...
            if (RT_FAILURE(vrc))
                throw setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
        ...
        if (mData->pMachineConfigFile->fileExists())
        {
            ...
            {
                /* Delete all VPox.log[.N] files from the Logs folder
                 * (this must be in sync with the rotation logic in
                 * Console::powerUpThread()). Also, delete the VPox.png[.N]
                 * files that may have been created by the GUI. */
                Utf8Str log = Utf8StrFmt("%s%cVPox.log",
                                         logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
                log = Utf8StrFmt("%s%cVPox.png",
                                 logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
                for (int i = uLogHistoryCount; i > 0; i--)
                {
                    log = Utf8StrFmt("%s%cVPox.log.%d",
                                     logFolder.c_str(), RTPATH_DELIMITER, i);
                    RTFileDelete(log.c_str());
                    log = Utf8StrFmt("%s%cVPox.png.%d",
                                     logFolder.c_str(), RTPATH_DELIMITER, i);
                    RTFileDelete(log.c_str());
                }
                log.printf("%s%cVPoxUI.log", logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
#if defined(RT_OS_WINDOWS)
                log = Utf8StrFmt("%s%cVPoxStartup.log", logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
                log = Utf8StrFmt("%s%cVPoxHardening.log", logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
#endif
...
HRESULT Machine::deleteConfig(const std::vector<ComPtr<IMedium> > &aMedia, ComPtr<IProgress> &aProgress)
{
    ...
    rc = pProgress->init(i_getVirtualPox(),
...
#ifdef VPOX_WITH_GUEST_PROPS
/**
 * Look up a guest property in VPoxSVC's internal structures.
 */
...
#ifdef VPOX_WITH_GUEST_PROPS
/**
 * Set a guest property in VPoxSVC's internal structures.
 */
...
#ifdef VPOX_WITH_GUEST_PROPS
/**
 * Enumerate the guest properties in VPoxSVC's internal structures.
 */
                                           ...
                                           BOOL  *aEnabled)
{
    ...
    if (RT_FAILURE(vrc))
    {
        ...
        *aEnabled = TRUE;
#endif
        return setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
    ...
    if (cbData > 0)
    {
        ...
        else if (aBitmapFormat == BitmapFormat_PNG)
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
...
HRESULT Machine::hotPlugCPU(ULONG aCpu)
{
    ...
    if (mHWData->mCPUAttached[aCpu])
        return setError(VPOX_E_OBJECT_IN_USE, tr("CPU %lu is already attached"), aCpu);
...
HRESULT Machine::hotUnplugCPU(ULONG aCpu)
{
    ...
    if (!mHWData->mCPUAttached[aCpu])
        return setError(VPOX_E_OBJECT_NOT_FOUND, tr("CPU %lu is not attached"), aCpu);
...
HRESULT Machine::readLog(ULONG aIdx, LONG64 aOffset, LONG64 aSize, std::vector<BYTE> &aData)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (RT_SUCCESS(vrc))
            ...
            rc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
...
HRESULT Machine::detachHostPCIDevice(LONG aHostAddress)
{
    ...
    return fRemoved ? S_OK : setError(VPOX_E_OBJECT_NOT_FOUND,
...
HRESULT Machine::setAutostartEnabled(BOOL aAutostartEnabled)
{
        ...
        && mHWData->mAutostart.fAutostartEnabled != !!aAutostartEnabled)
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            hrc = setError(VPOX_E_NOT_SUPPORTED,
...
void Machine::i_getLogFolder(Utf8Str &aLogFolder)
{
    ...
    int vrc = RTEnvGetEx(RTENV_DEFAULT, "VPOX_USER_VMLOGDIR", szTmp, sizeof(szTmp), NULL);
    if (RT_SUCCESS(vrc))
    {
    ...
    if (RT_FAILURE(vrc))
    {
        // fallback if VPOX_USER_LOGHOME is not set or invalid
        aLogFolder = mData->m_strConfigFileFull;    // path/to/machinesfolder/vmname/vmname.vpox
...
Utf8Str Machine::i_getLogFilename(ULONG idx)
{
    ...
    Utf8Str log;
    if (idx == 0)
        log = Utf8StrFmt("%s%cVPox.log", logFolder.c_str(), RTPATH_DELIMITER);
#if defined(RT_OS_WINDOWS) && defined(VPOX_WITH_HARDENING)
    else if (idx == 1)
        log = Utf8StrFmt("%s%cVPoxHardening.log", logFolder.c_str(), RTPATH_DELIMITER);
    else
        log = Utf8StrFmt("%s%cVPox.log.%u", logFolder.c_str(), RTPATH_DELIMITER, idx - 1);
#else
    else
        log = Utf8StrFmt("%s%cVPox.log.%u", logFolder.c_str(), RTPATH_DELIMITER, idx);
#endif
    return log;
}
...
Utf8Str Machine::i_getHardeningLogFilename(void)
{
    ...
    strFilename.append(RTPATH_SLASH_STR "VPoxHardening.log");
    return strFilename;
}
                                   ...
                                   ProgressProxy *aProgress)
{
    ...
    Utf8Str strSupHardeningLogArg("--sup-hardening-log=");
    {
        ...
        strOldStartupLogFile.append(RTPATH_SLASH_STR "VPoxStartup.log");
    ...
    strAppOverride = i_getExtraData(Utf8Str("VPoxInternal2/VirtualPoxVMAppOverride"));
#endif

    bool fUseVPoxSDS = false;
...
#if !defined(VPOX_WITH_SDS) || !defined(RT_OS_WINDOWS)
    RT_NOREF(fUseVPoxSDS);
#else
    DWORD idCallerSession = ~(DWORD)0;
    if (fUseVPoxSDS)
    {
        /*
         * The VPoxSDS should be used for process launching the VM with
         * GUI only if the caller and the VPoxSDS are in different Windows
    ...
    {
        /* connect to VPoxSDS */
        ComPtr<IVirtualPoxSDS> pVPoxSDS;
        HRESULT rc = pVPoxSDS.createLocalObject(CLSID_VirtualPoxSDS);
        if (FAILED(rc))
            return setError(rc, tr("Failed to start the machine '%s'. A connection to VPoxSDS cannot be established"),
        ...
        /* By default the RPC_C_IMP_LEVEL_IDENTIFY is used for impersonation the client. It allows
           ...
           elevates the impersonation level up to RPC_C_IMP_LEVEL_IMPERSONATE allowing the VPoxSDS
           service to access the files. */
        rc = CoSetProxyBlanket(pVPoxSDS,
                               RPC_C_AUTHN_DEFAULT,
                               RPC_C_AUTHZ_DEFAULT,
        ...
        ULONG uPid = 0;
        rc = pVPoxSDS->LaunchVMProcess(Bstr(idStr).raw(), Bstr(strMachineName).raw(), Bstr(strFrontend).raw(),
...
#endif /* VPOX_WITH_VPOXSDS && RT_OS_WINDOWS */
    {
...
HRESULT Machine::i_prepareRegister()
{
       ...
       )
    {
        rc = i_saveSettings(NULL, alock);
                // no need to check whether VirtualPox.xml needs saving too since
...
#ifdef VPOX_WITH_DTRACE_R3_MAIN
        VPOXAPI_MACHINE_STATE_CHANGED(this, aMachineState, enmOldState, mData->mUuid.toStringCurly().c_str());
#endif
 ...
 *                          not found
 *  @return
 *      S_OK when found or VPOX_E_OBJECT_NOT_FOUND when not found
 *
 *  @note
 ...
 *    VirtualPox startup. In this case, puuidRegistry is NULL because the media
                                ...
                                Snapshot *aParentSnapshot)
{
    ...
    rc = pSnapshot->init(mParent, // VirtualPox object
                         data.uuid,
                         data.strName,
                         data.strDescription,
                         data.timestamp,
...
HRESULT Machine::i_prepareSaveSettings(bool *pfNeedsGlobalSaveSettings)
{
       ...
       )
    {
        ...
        do
        {
            ...
            newConfigFile = Utf8StrFmt("%s%c%s.vpox",
            ...
            // store the old and new so that VirtualPox::i_saveSettings() can update
            // the media registry
            if (    mData->mRegistered
 ...
 *          the caller must invoke VirtualPox::i_saveSettings() because the global
 ...
 *      exported to OVF and we write the VirtualPox proprietary XML
 *      into a <vpox:Machine> tag.
 *
...
void Machine::i_copyMachineDataToSettings(settings::MachineConfigFile &config)
{
    ...
    // save machine's media registry if this is VirtualPox 4.0 or later
    if (config.canHaveOwnMediaRegistry())
    {
        // determine machine folder
        Utf8Str strMachineFolder = i_getSettingsFileFull();
 ...
 * the global VirtualPox media registry is used.
 *
...
void Machine::i_addMediumToRegistry(ComObjPtr<Medium> &pMedium)
{
    ...
    if (fCanHaveOwnMediaRegistry)
        // machine XML is VirtualPox 4.0 or higher:
        uuid = i_getId();     // machine UUID
    else
        uuid = mParent->i_getGlobalRegistryId(); // VirtualPox global registry UUID
         ...
         ++it)
    {
        ...
        if (pAttach->i_getType() == DeviceType_HardDisk && pMedium)
        {
            ...
            if (aOnline)
            {
                ...
                MediumLockList *pMediumLockList;
                rc = mData->mSession.mLockedMedia.Get(pAttach, pMediumLockList);
                if (RT_UNLIKELY(rc == VPOX_E_INVALID_OBJECT_STATE))
                {
...
{
    Utf8Str strMachineDirName(mData->m_strConfigFileFull);  // path/to/machinesfolder/vmname/vmname.vpox
    ...
    Utf8Str strConfigFileOnly(mData->m_strConfigFileFull);  // path/to/machinesfolder/vmname/vmname.vpox
    strConfigFileOnly.stripPath()                           // vmname.vpox
                     .stripSuffix();                        // vmname
    /** @todo hack, make somehow use of ComposeMachineFilename */
    if (mUserData->s.fDirectoryIncludesUUID)
...
void SessionMachine::uninit(Uninit::Reason aReason)
{
     ...
     *  the Session implementation has released the VirtualPox reference
     ...
     *  semaphore correctly (i.e. before it releases the VirtualPox reference),
     *  but the VirtualPox release event comes first to the server process.
...
/**
 *  Overrides VirtualPoxBase::lockHandle() in order to share the lock handle
...
void SessionMachine::i_saveStateHandler(SaveStateTask &task)
{
    ...
    try
    {
        ...
        if (directControl.isNull())
            throw setError(VPOX_E_INVALID_VM_STATE,
        ...
        if (SUCCEEDED(rc))
        {
            ...
            rc = i_saveSettings(NULL, alock);
                    // no need to check whether VirtualPox.xml needs saving also since
...
HRESULT SessionMachine::adoptSavedState(const com::Utf8Str &aSavedStateFile)
{
    ...
    if (RT_FAILURE(vrc))
        return setErrorBoth(VPOX_E_FILE_ERROR, vrc,
                            tr("Invalid saved state file path '%s' (%Rrc)"),
...
HRESULT SessionMachine::beginPowerUp(const ComPtr<IProgress> &aProgress)
{
    ...
    if (mData->mSession.mState != SessionState_Locked)
        return VPOX_E_INVALID_OBJECT_STATE;
...
HRESULT SessionMachine::beginPoweringDown(ComPtr<IProgress> &aProgress)
{
    ...
    ComObjPtr<Progress> pProgress;
    pProgress.createObject();
    pProgress->init(i_getVirtualPox(),
                    static_cast<IMachine *>(this) /* aInitiator */,
                                     ...
                                     ComPtr<IProgress> &aProgress)
{
    ...
    /* Creating a Progress object requires the VirtualPox lock, and
                                          ...
                                          const com::Utf8Str &aFlags)
{
    ...
    {
        return VirtualPoxBase::handleUnexpectedExceptions(this, RT_SRC_POS);
    }
    return S_OK;
#else
    ReturnComNotImplemented();
                                                  ...
                                                  IN_BSTR aGuestIp, LONG aGuestPort)
{
    ...
    if (!directControl)
        return S_OK;
    /*
     * instead acting like callback we ask IVirtualPox deliver corresponding event
     */
