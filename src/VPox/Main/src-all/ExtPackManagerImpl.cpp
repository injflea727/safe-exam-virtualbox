...
# define VPOX_EXTPACK_HELPER_NAME       "VPoxExtPackHelperApp.exe"
#else
# define VPOX_EXTPACK_HELPER_NAME       "VPoxExtPackHelperApp"
#endif
...
public:
    ...
    /** Pointer to the VirtualPox object so we can create a progress object. */
    VirtualPox         *pVirtualPox;
...
public:
    ...
    PCVPOXEXTPACKREG    pReg;
#else
    /** The extension pack main VM registration structure. */
    PCVPOXEXTPACKVMREG  pReg;
#endif
    /** The current context. */
    VPOXEXTPACKCTX      enmContext;
    /** Set if we've made the pfnVirtualPoxReady or pfnConsoleReady call. */
    bool                fMadeReadyCall;
#ifndef VPOX_COM_INPROC
    /** Pointer to the VirtualPox object so we can create a progress object. */
    VirtualPox         *pVirtualPox;
...
#ifndef VPOX_COM_INPROC
    /** Pointer to the VirtualPox object, our parent. */
    VirtualPox         *pVirtualPox;
#endif
    /** The current context. */
    VPOXEXTPACKCTX      enmContext;
...
public:
                 ...
                 const Utf8Str &strDispInfo, const ComPtr<ExtPackManager> &a_ptrExtPackMgr)
    {
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            hrc = ptrProgress->init(ptrExtPackFile->m->pVirtualPox,
...
public:
                 ...
                 bool a_fForcedRemoval, const Utf8Str &a_strDisplayInfo)
    {
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            hrc = ptrProgress->init(ptrExtPackMgr->m->pVirtualPox,
                                  ...
                                  VirtualPox *a_pVirtualPox)
{
    ...
    m->pVirtualPox                  = a_pVirtualPox;

    RTCString *pstrTarName = VPoxExtPackExtractNameFromTarballPath(a_pszFile);
    if (pstrTarName)
    {
    ...
    char        szError[8192];
    RTVFSFILE   hXmlFile;
    vrc = VPoxExtPackValidateTarball(m->hExtPackFile, NULL /*pszExtPackName*/, a_pszFile, a_pszDigest,
    ...
    RTCString *pStrLoadErr = VPoxExtPackLoadDescFromVfsFile(hXmlFile, &m->Desc, &m->ObjInfoDesc);
                                  ...
                                  const com::Utf8Str &aFormat, com::Utf8Str &aLicenseText)
{
     ...
     */
    char szName[sizeof(VPOX_EXTPACK_LICENSE_NAME_PREFIX "-de_DE.html") + 2];
    if (aPreferredLocale.isNotEmpty() && aPreferredLanguage.isNotEmpty())
        RTStrPrintf(szName, sizeof(szName), VPOX_EXTPACK_LICENSE_NAME_PREFIX "-%s_%s.%s",
                    aPreferredLocale.c_str(), aPreferredLanguage.c_str(), aFormat.c_str());
    else if (aPreferredLocale.isNotEmpty())
        RTStrPrintf(szName, sizeof(szName), VPOX_EXTPACK_LICENSE_NAME_PREFIX "-%s.%s",
                    aPreferredLocale.c_str(), aFormat.c_str());
    else if (aPreferredLanguage.isNotEmpty())
        RTStrPrintf(szName, sizeof(szName), VPOX_EXTPACK_LICENSE_NAME_PREFIX "-_%s.%s",
                    aPreferredLocale.c_str(), aFormat.c_str());
    else
        RTStrPrintf(szName, sizeof(szName), VPOX_EXTPACK_LICENSE_NAME_PREFIX ".%s",
                    aFormat.c_str());
    /*
    ...
    else
    {
        ...
        if (RTManifestEntryExists(m->hOurManifest, szName))
        {
            ...
            char            szError[8192];
            int vrc = VPoxExtPackOpenTarFss(m->hExtPackFile, szError, sizeof(szError), &hTarFss, NULL);
            if (RT_SUCCESS(vrc))
            {
                for (;;)
                {
                    ...
                    {
                        if (vrc != VERR_EOF)
                            hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("RTVfsFsStrmNext failed: %Rrc"), vrc);
                        else
                        ...
                        if (RT_SUCCESS(vrc))
                        {
                            ...
                            if (pvFile)
                            {
                                ...
                                if (RT_SUCCESS(vrc))
                                {
                                    ...
                                    hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Failed to read '%s': %Rrc"), szName, vrc);
                                RTMemFree(pvFile);
                            }
                            else
                            ...
                            hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("RTVfsIoStrmQueryInfo on '%s': %Rrc"), szName, vrc);
                ...
                hrc = setError(VPOX_E_OBJECT_NOT_FOUND, tr("%s"), szError);
        }
        else
            hrc = setError(VPOX_E_OBJECT_NOT_FOUND, tr("The license file '%s' was not found in '%s'"),
        ...
        try
        {
            ...
            if (SUCCEEDED(hrc))
            {
                ...
                if (SUCCEEDED(hrc))
                    ...
                    hrc = setError(VPOX_E_IPRT_ERROR,
 ...
 *
 * @returns COM status code.
 * @param   a_pVirtualPox   The VirtualPox object.
 ...
 */
HRESULT ExtPack::initWithDir(VirtualPox *a_pVirtualPox, VPOXEXTPACKCTX a_enmContext, const char *a_pszName, const char *a_pszDir)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);

    static const VPOXEXTPACKHLP s_HlpTmpl =
    {
        /* u32Version           = */ VPOXEXTPACKHLP_VERSION,
        /* uVPoxFullVersion     = */ VPOX_FULL_VERSION,
        /* uVPoxVersionRevision = */ 0,
        /* u32Padding           = */ 0,
        /* pszVPoxVersion       = */ "",
        ...
        /* uReserved7           = */ 0,
        /* u32EndMarker         = */ VPOXEXTPACKHLP_VERSION
    };
    ...
    m->Hlp                          = s_HlpTmpl;
    m->Hlp.pszVPoxVersion           = RTBldCfgVersion();
    m->Hlp.uVPoxInternalRevision    = RTBldCfgRevision();
    m->pThis                        = this;
    m->pReg                         = NULL;
 ...
 * @param   a_pVirtualPox       The VirtualPox interface.
 * @param   a_pLock             The write lock held by the caller.
 * @param   pErrInfo            Where to return error information.
 */
bool    ExtPack::i_callInstalledHook(IVirtualPox *a_pVirtualPox, AutoWriteLock *a_pLock, PRTERRINFO pErrInfo)
{
    if (   m != NULL
        && m->hMainMod != NIL_RTLDRMOD)
    {
        if (m->pReg->pfnInstalled)
        {
            ...
            a_pLock->release();
            pErrInfo->rc = m->pReg->pfnInstalled(m->pReg, a_pVirtualPox, pErrInfo);
            a_pLock->acquire();
            return true;
        }
    }
 ...
 */
HRESULT ExtPack::i_callUninstallHookAndClose(IVirtualPox *a_pVirtualPox, bool a_fForcedRemoval)
{
    HRESULT hrc = S_OK;
        ...
        && m->hMainMod != NIL_RTLDRMOD)
    {
        ...
        {
            int vrc = m->pReg->pfnUninstall(m->pReg, a_pVirtualPox);
            if (RT_FAILURE(vrc))
            {
...
/**
 * Calls the pfnVirtualPoxReady hook.
 *
 * @returns true if we left the lock, false if we didn't.
 * @param   a_pVirtualPox       The VirtualPox interface.
 * @param   a_pLock             The write lock held by the caller.
 */
bool ExtPack::i_callVirtualPoxReadyHook(IVirtualPox *a_pVirtualPox, AutoWriteLock *a_pLock)
{
    if (    m != NULL
        &&  m->fUsable
        &&  m->hMainMod != NIL_RTLDRMOD
        && !m->fMadeReadyCall)
    {
        ...
        if (m->pReg->pfnVirtualPoxReady)
        {
            ComPtr<ExtPack> ptrSelfRef = this;
            a_pLock->release();
            m->pReg->pfnVirtualPoxReady(m->pReg, a_pVirtualPox);
 ...
 * @param   a_pLock             The write lock held by the caller.
 */
bool ExtPack::i_callVmCreatedHook(IVirtualPox *a_pVirtualPox, IMachine *a_pMachine, AutoWriteLock *a_pLock)
{
    if (   m != NULL
        && m->hMainMod != NIL_RTLDRMOD
        && m->fUsable)
    {
        if (m->pReg->pfnVMCreated)
        {
            ComPtr<ExtPack> ptrSelfRef = this;
            a_pLock->release();
            m->pReg->pfnVMCreated(m->pReg, a_pVirtualPox, a_pMachine);
            a_pLock->acquire();
            return true;
        }
    }
...
HRESULT ExtPack::i_getVrdpLibraryName(Utf8Str *a_pstrVrdeLibrary)
{
    ...
    if (SUCCEEDED(hrc))
    {
        if (i_findModule(m->Desc.strVrdeModule.c_str(), NULL, VPOXEXTPACKMODKIND_R3,
...
{
    HRESULT hrc;
    if (i_findModule(a_pszModuleName, NULL, VPOXEXTPACKMODKIND_R3,
 ...
 * @remarks Only called in VPoxSVC.
 */
HRESULT ExtPack::i_refresh(bool *a_pfCanDelete)
{
    ...
    vrc = RTPathJoin(szDescFilePath, sizeof(szDescFilePath), m->strExtPackPath.c_str(), VPOX_EXTPACK_DESCRIPTION_NAME);
    if (RT_SUCCESS(vrc))
 ...
 *
 * This needs going through the cloud provider singleton in VirtualPox,
...
{
    Assert(m->pVirtualPox != NULL); /* Only called from VPoxSVC. */

    ComObjPtr<CloudProviderManager> cpm(m->pVirtualPox->i_getCloudProviderManager());
...
void ExtPack::i_probeAndLoad(void)
{
     ...
     */
    RTCString strSavedName(m->Desc.strName);
    RTCString *pStrLoadErr = VPoxExtPackLoadDesc(m->strExtPackPath.c_str(), &m->Desc, &m->ObjInfoDesc);
    if (pStrLoadErr != NULL)
    {
        m->strWhyUnusable.printf(tr("Failed to load '%s/%s': %s"),
                                 m->strExtPackPath.c_str(), VPOX_EXTPACK_DESCRIPTION_NAME, pStrLoadErr->c_str());
        m->Desc.strName = strSavedName;
        delete pStrLoadErr;
        return;
    }
...
#endif
    bool fIsNative;
    if (!i_findModule(pszMainModule, NULL /* default extension */, VPOXEXTPACKMODKIND_R3,
     ...
     */
#ifndef VPOX_COM_INPROC
    const char *pszMainEntryPoint = VPOX_EXTPACK_MAIN_MOD_ENTRY_POINT;
    PFNVPOXEXTPACKREGISTER pfnRegistration;
    uint32_t uVersion = VPOXEXTPACKREG_VERSION;
#else
    const char *pszMainEntryPoint = VPOX_EXTPACK_MAIN_VM_MOD_ENTRY_POINT;
    PFNVPOXEXTPACKVMREGISTER pfnRegistration;
    uint32_t uVersion = VPOXEXTPACKVMREG_VERSION;
#endif
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (   RT_SUCCESS(vrc)
            ...
            && VALID_PTR(m->pReg))
        {
            if (   VPOXEXTPACK_IS_MAJOR_VER_EQUAL(m->pReg->u32Version, uVersion)
                && m->pReg->u32EndMarker == m->pReg->u32Version)
            {
                ...
                if (   (!m->pReg->pfnInstalled       || RT_VALID_PTR(m->pReg->pfnInstalled))
                    ...
                    && (!m->pReg->pfnVirtualPoxReady || RT_VALID_PTR(m->pReg->pfnVirtualPoxReady))
 ...
 *                              the module. Optional.
 */
bool ExtPack::i_findModule(const char *a_pszName, const char *a_pszExt, VPOXEXTPACKMODKIND a_enmKind,
                           Utf8Str *a_pStrFound, bool *a_pfNative, PRTFSOBJINFO a_pObjInfo) const
{
    ...
    if (!a_pszExt)
    {
        ...
        switch (a_enmKind)
        {
            case VPOXEXTPACKMODKIND_RC: pszDefExt = ".rc"; break;
            case VPOXEXTPACKMODKIND_R0: pszDefExt = ".r0"; break;
            case VPOXEXTPACKMODKIND_R3: pszDefExt = RTLdrGetSuff(); break;
            default:
...
/**
 * @interface_method_impl{VPOXEXTPACKHLP,pfnFindModule}
 */
/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpFindModule(PCVPOXEXTPACKHLP pHlp, const char *pszName, const char *pszExt, VPOXEXTPACKMODKIND enmKind,
                         char *pszFound, size_t cbFound, bool *pfNative)
{
    ...
    AssertReturn(enmKind > VPOXEXTPACKMODKIND_INVALID && enmKind < VPOXEXTPACKMODKIND_END, VERR_INVALID_PARAMETER);

    AssertPtrReturn(pHlp, VERR_INVALID_POINTER);
    AssertReturn(pHlp->u32Version == VPOXEXTPACKHLP_VERSION, VERR_INVALID_POINTER);
...
/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpGetFilePath(PCVPOXEXTPACKHLP pHlp, const char *pszFilename, char *pszPath, size_t cbPath)
{
    /*
     * Validate the input and get our bearings.
     */
...
/*static*/ DECLCALLBACK(VPOXEXTPACKCTX)
ExtPack::i_hlpGetContext(PCVPOXEXTPACKHLP pHlp)
{
    /*
     * Validate the input and get our bearings.
     */
    AssertPtrReturn(pHlp, VPOXEXTPACKCTX_INVALID);
    AssertReturn(pHlp->u32Version == VPOXEXTPACKHLP_VERSION, VPOXEXTPACKCTX_INVALID);
    ExtPack::Data *m = RT_FROM_CPP_MEMBER(pHlp, Data, Hlp);
    AssertPtrReturn(m, VPOXEXTPACKCTX_INVALID);
    ExtPack *pThis = m->pThis;
    AssertPtrReturn(pThis, VPOXEXTPACKCTX_INVALID);

    return pThis->m->enmContext;
}

/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpLoadHGCMService(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IConsole) *pConsole,
...
/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpLoadVDPlugin(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IVirtualPox) *pVirtualPox, const char *pszPluginLibrary)
{
#ifndef VPOX_COM_INPROC
    /*
     * Validate the input and get our bearings.
    ...
    ExtPack *pThis = m->pThis;
    AssertPtrReturn(pThis, VERR_INVALID_POINTER);
    AssertPtrReturn(pVirtualPox, VERR_INVALID_POINTER);

    VirtualPox *pVPox = (VirtualPox *)pVirtualPox;
    return pVPox->i_loadVDPlugin(pszPluginLibrary);
#else
    NOREF(pHlp); NOREF(pVirtualPox); NOREF(pszPluginLibrary);
    return VERR_INVALID_STATE;
#endif
}

/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpUnloadVDPlugin(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IVirtualPox) *pVirtualPox, const char *pszPluginLibrary)
{
#ifndef VPOX_COM_INPROC
    /*
     * Validate the input and get our bearings.
    ...
    VirtualPox *pVPox = (VirtualPox *)pVirtualPox;
    return pVPox->i_unloadVDPlugin(pszPluginLibrary);
#else
    NOREF(pHlp); NOREF(pVirtualPox); NOREF(pszPluginLibrary);
    return VERR_INVALID_STATE;
#endif
...
ExtPack::i_hlpCreateProgress(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IUnknown) *pInitiator,
                             ...
                             uint32_t uFirstOperationWeight, VPOXEXTPACK_IF_CS(IProgress) **ppProgressOut)
{
    ...
    AssertReturn(pHlp->u32Version == VPOXEXTPACKHLP_VERSION, (uint32_t)E_INVALIDARG);
#ifndef VPOX_COM_INPROC
    ExtPack::Data *m = RT_FROM_CPP_MEMBER(pHlp, Data, Hlp);
#endif
...
/*static*/ DECLCALLBACK(uint32_t)
ExtPack::i_hlpGetCanceledProgress(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IProgress) *pProgress,
                                  bool *pfCanceled)
{
    /*
     * Validate the input and get our bearings.
...
/*static*/ DECLCALLBACK(uint32_t)
ExtPack::i_hlpUpdateProgress(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IProgress) *pProgress,
                             uint32_t uPercent)
{
    /*
     * Validate the input and get our bearings.
...
/*static*/ DECLCALLBACK(uint32_t)
ExtPack::i_hlpNextOperationProgress(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IProgress) *pProgress,
...
/*static*/ DECLCALLBACK(uint32_t)
ExtPack::i_hlpWaitOtherProgress(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IProgress) *pProgress,
                                VPOXEXTPACK_IF_CS(IProgress) *pProgressOther, uint32_t cTimeoutMS)
{
    /*
     * Validate the input and get our bearings.
     */
...
/*static*/ DECLCALLBACK(uint32_t)
ExtPack::i_hlpCompleteProgress(PCVPOXEXTPACKHLP pHlp, VPOXEXTPACK_IF_CS(IProgress) *pProgress,
                               uint32_t uResultCode)
{
    ...
    ComPtr<IVirtualPoxErrorInfo> errorInfo;
    if (FAILED((HRESULT)uResultCode))
    {
        ErrorInfoKeeper eik;
        eik.getVirtualPoxErrorInfo(errorInfo);
    }
    return pProgressControl->NotifyComplete(uResultCode, errorInfo);
}

/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpReservedN(PCVPOXEXTPACKHLP pHlp)
{
    /*
     * Validate the input and get our bearings.
     */
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (bstrLicense.isNotEmpty())
                {
                    ...
                    hrc = setError(VPOX_E_IPRT_ERROR, tr("The license file '%s' is empty or contains invalid UTF-8 encoding"),
                                   szPath);
                RTFileReadAllFree(pvFile, cbFile);
            }
            else if (vrc == VERR_FILE_NOT_FOUND || vrc == VERR_PATH_NOT_FOUND)
                hrc = setErrorBoth(VPOX_E_OBJECT_NOT_FOUND, vrc, tr("The license file '%s' was not found in extension pack '%s'"),
                                   szName, m->Desc.strName.c_str());
            else
                hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Failed to open the license file '%s': %Rrc"), szPath, vrc);
        }
        else
            hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("RTPathJoin failed: %Rrc"), vrc);
    }
    return hrc;
}
 ...
 * @param   a_enmContext            The context we're in.
 */
HRESULT ExtPackManager::initExtPackManager(VirtualPox *a_pVirtualPox, VPOXEXTPACKCTX a_enmContext)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);
    ...
    rc = RTPathAppend(szBaseDir, sizeof(szBaseDir), VPOX_EXTPACK_INSTALL_DIR);
    AssertLogRelRCReturn(rc, E_FAIL);

    char szCertificatDir[RTPATH_MAX];
    rc = RTPathAppPrivateNoArch(szCertificatDir, sizeof(szCertificatDir));
    AssertLogRelRCReturn(rc, E_FAIL);
    rc = RTPathAppend(szCertificatDir, sizeof(szCertificatDir), VPOX_EXTPACK_CERT_DIR);
    AssertLogRelRCReturn(rc, E_FAIL);
    ...
    if (RT_SUCCESS(vrc))
    {
        for (;;)
        {
            ...
            if (   RTFS_IS_DIRECTORY(Entry.Info.Attr.fMode)
                ...
                && strcmp(Entry.szName, "..") != 0
                && VPoxExtPackIsValidMangledName(Entry.szName) )
            {
                /*
                ...
                if (RT_SUCCESS(vrc))
                {
                    RTCString *pstrName = VPoxExtPackUnmangleName(Entry.szName, RTSTR_MAX);
                    ...
                    if (pstrName)
                    {
                        ...
                        if (SUCCEEDED(hrc2))
                            hrc2 = NewExtPack->initWithDir(a_pVirtualPox, a_enmContext, pstrName->c_str(), szExtPackDir);
                        delete pstrName;
                        if (SUCCEEDED(hrc2))
                        {
...
{
    Assert(m->enmContext == VPOXEXTPACKCTX_PER_USER_DAEMON);
...
{
    AssertReturn(m->enmContext == VPOXEXTPACKCTX_PER_USER_DAEMON, E_UNEXPECTED);
    ...
    if (SUCCEEDED(hrc))
        hrc = NewExtPackFile->initWithFile(strTarball.c_str(), strDigest.c_str(), this, m->pVirtualPox);
    if (SUCCEEDED(hrc))
...
HRESULT ExtPackManager::i_runSetUidToRootHelper(Utf8Str const *a_pstrDisplayInfo, const char *a_pszCommand, ...)
{
    ...
    vrc = RTPathAppend(szExecName, sizeof(szExecName), VPOX_EXTPACK_HELPER_NAME);
    AssertLogRelRCReturn(vrc, E_UNEXPECTED);
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Failed to launch the helper application '%s' (%Rrc)"), szExecName, vrc);
...
HRESULT ExtPackManager::i_refreshExtPack(const char *a_pszName, bool a_fUnusableIsError, ExtPack **a_ppExtPack)
{
    ...
    {
        /*
         * Do this check here, otherwise VPoxExtPackCalcDir() will fail with a strange
         * error.
         */
        bool fValid = VPoxExtPackIsValidName(a_pszName);
        if (!fValid)
        ...
        int vrc = VPoxExtPackCalcDir(szDir, sizeof(szDir), m->strBaseDir.c_str(), a_pszName);
        ...
        if (fExists)
        {
            ...
            if (SUCCEEDED(hrc))
                hrc = ptrNewExtPack->initWithDir(m->pVirtualPox, m->enmContext, a_pszName, szDir);
            if (SUCCEEDED(hrc))
            {
...
bool ExtPackManager::i_areThereAnyRunningVMs(void) const
{
    ...
    HRESULT hrc = m->pVirtualPox->COMGETTER(Machines)(ComSafeArrayAsOutParam(SaMachines));
    if (SUCCEEDED(hrc))
    {
        com::SafeArray<MachineState_T> SaStates;
        hrc = m->pVirtualPox->GetMachineStates(ComSafeArrayAsInParam(SaMachines), ComSafeArrayAsOutParam(SaStates));
        if (SUCCEEDED(hrc))
        {
            /*
...
HRESULT ExtPackManager::i_doInstall(ExtPackFile *a_pExtPackFile, bool a_fReplace, Utf8Str const *a_pstrDisplayInfo)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (SUCCEEDED(hrc))
        {
            if (pExtPack && a_fReplace)
            {
                ...
                else if (fVetoingCP)
                {
                    ...
                    hrc = pExtPack->i_callUninstallHookAndClose(m->pVirtualPox, false /*a_ForcedRemoval*/);
            }
            else if (pExtPack)
                hrc = setError(E_FAIL,
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            if (SUCCEEDED(hrc))
            {
                ...
                if (SUCCEEDED(hrc) && pExtPack)
                {
                    RTERRINFOSTATIC ErrInfo;
                    RTErrInfoInitStatic(&ErrInfo);
                    pExtPack->i_callInstalledHook(m->pVirtualPox, &autoLock, &ErrInfo.Core);
        ...
        /*
         * Do VirtualPoxReady callbacks now for any freshly installed
         * extension pack (old ones will not be called).
         */
        if (m->enmContext == VPOXEXTPACKCTX_PER_USER_DAEMON)
        {
            autoLock.release();
            i_callAllVirtualPoxReadyHooks();
        }
    }
            ...
            if (a_fForcedRemoval || (!fRunningVMs && !fVetoingCP))
            {
                ...
                if (SUCCEEDED(hrc))
                {
                    ...
                    else
                    {
                         ...
                         */
                        hrc = pExtPack->i_callUninstallHookAndClose(m->pVirtualPox, a_fForcedRemoval);
                        if (SUCCEEDED(hrc))
                        {
                            /*
...
/**
 * Calls the pfnVirtualPoxReady hook for all working extension packs.
 *
 * @remarks The caller must not hold any locks.
 */
void ExtPackManager::i_callAllVirtualPoxReadyHooks(void)
{
    AutoCaller autoCaller(this);
    HRESULT hrc = autoCaller.rc();
    if (FAILED(hrc))
    ...
    for (ExtPackList::iterator it = m->llInstalledExtPacks.begin();
         ...
         /* advancing below */)
    {
        if ((*it)->i_callVirtualPoxReadyHook(m->pVirtualPox, &autoLock))
...
void ExtPackManager::i_callAllVmCreatedHooks(IMachine *a_pMachine)
{
    ...
    for (ExtPackList::iterator it = llExtPacks.begin(); it != llExtPacks.end(); ++it)
        (*it)->i_callVmCreatedHook(m->pVirtualPox, a_pMachine, &autoLock);
}
#endif
 ...
 *
 * @returns VPox status code.  Stops on the first failure, expecting the caller
...
int ExtPackManager::i_callAllVmConfigureVmmHooks(IConsole *a_pConsole, PVM a_pVM)
{
    ...
    HRESULT hrc = autoCaller.rc();
    if (FAILED(hrc))
        return Global::vpoxStatusCodeFromCOM(hrc);
    AutoWriteLock           autoLock(this COMMA_LOCKVAL_SRC_POS);
    ComPtr<ExtPackManager>  ptrSelfRef = this; /* paranoia */
    ExtPackList             llExtPacks = m->llInstalledExtPacks;
...
HRESULT ExtPackManager::i_checkVrdeExtPack(Utf8Str const *a_pstrExtPack)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (pExtPack)
            ...
            hrc = setError(VPOX_E_OBJECT_NOT_FOUND, tr("No extension pack by the name '%s' was found"), a_pstrExtPack->c_str());
    }
...
int ExtPackManager::i_getVrdeLibraryPathForExtPack(Utf8Str const *a_pstrExtPack, Utf8Str *a_pstrVrdeLibrary)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (pExtPack)
            ...
            hrc = setError(VPOX_E_OBJECT_NOT_FOUND, tr("No extension pack by the name '%s' was found"),
                           a_pstrExtPack->c_str());
    }
...
HRESULT ExtPackManager::i_getLibraryPathForExtPack(const char *a_pszModuleName, const char *a_pszExtPack, Utf8Str *a_pstrLibrary)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (pExtPack)
            ...
            hrc = setError(VPOX_E_OBJECT_NOT_FOUND, tr("No extension pack by the name '%s' was found"), a_pszExtPack);
    }
