...
{
    struct VPOXWDDMDLLPROC *pIter = paProcs;
    while (pIter->pszName)
    {
...
static VPOXWDDMDLLPROC g_D3DKMTLoadTable[] =
{
...
DECLCALLBACK(int) D3DKMTLoad(void)
{
    ...
    if (hmod)
    {
        VPoxWddmLoadAdresses(hmod, g_D3DKMTLoadTable);
    }
    return hmod != NULL;
}
...
/*
 * Getting VirtualPox Graphics Adapter handle.
 */

static NTSTATUS vpoxDispKmtOpenAdapterFromHdc(D3DKMT_HANDLE *phAdapter, LUID *pLuid)
{
    *phAdapter = 0;
...
static NTSTATUS vpoxDispKmtOpenAdapterFromLuid(D3DKMT_HANDLE *phAdapter, LUID *pLuid)
{
    *phAdapter = 0;
...
{
    NTSTATUS Status = vpoxDispKmtOpenAdapterFromLuid(phAdapter, pLuid);
    if (Status != STATUS_SUCCESS)
    {
        /* Fallback for pre-Windows8 */
        Status = vpoxDispKmtOpenAdapterFromHdc(phAdapter, pLuid);
    }
...
{
    return vpoxDispKmtOpenAdapter2(phAdapter, NULL);
}
