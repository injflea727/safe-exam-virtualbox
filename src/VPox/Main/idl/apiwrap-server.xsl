...
<xsl:template name="fileheader">
    ...
    <xsl:value-of select="concat(' * VirtualPox API class wrapper ', $type, ' for I', $class, '.')"/>
    <xsl:text>
 *
 * DO NOT EDIT! This is a generated file.
 * Generated from: src/VPox/Main/idl/VirtualPox.xidl
 * Generator: src/VPox/Main/idl/apiwrap-server.xsl
 */
...
#include "Wrapper.h"
    ...
    <xsl:value-of select="concat('    , VPOX_SCRIPTABLE_IMPL(', @name, ')')"/>
    <xsl:value-of select="$G_sNewLine"/>
    <xsl:for-each select="exsl:node-set($addinterfaces)/token">
        <xsl:value-of select="concat('    , VPOX_SCRIPTABLE_IMPL(', text(), ')')"/>
        <xsl:value-of select="$G_sNewLine"/>
    </xsl:for-each>
    <xsl:text>{
    Q_OBJECT
...
public:
    ...
    <xsl:value-of select="concat('    VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT(', substring(@name, 2), 'Wrap, ', @name, ')', $G_sNewLine)"/>
    ...
    <xsl:value-of select="concat('        VPOX_TWEAK_INTERFACE_ENTRY(', @name, ')', $G_sNewLine)"/>
...
<xsl:template match="interface" mode="codefooter">
    <xsl:param name="addinterfaces"/>
    <xsl:text>#ifdef VPOX_WITH_XPCOM
</xsl:text>
    ...
    <xsl:text>#endif // VPOX_WITH_XPCOM
</xsl:text>
</xsl:template>
...
<xsl:text>
    ...
    <xsl:if test="$attrbasename != 'MidlDoesNotLikeEmptyInterfaces'">
        ...
        <xsl:value-of select="translate(concat('VPOXAPI_', $dtracetopclass, '_GET_', $dtraceattrname, '_ENTER('), $G_lowerCase, $G_upperCase)"/>
        <xsl:text>this);
#endif</xsl:text>
    </xsl:if>
    <xsl:text>
    ...
    <xsl:if test="$attrbasename != 'MidlDoesNotLikeEmptyInterfaces'">
        ...
        <xsl:value-of select="translate(concat('VPOXAPI_', $dtracetopclass, '_GET_', $dtraceattrname, '_RETURN('), $G_lowerCase, $G_upperCase)"/>
    ...
    {
        hrc = VirtualPoxBase::handleUnexpectedExceptions(this, RT_SRC_POS);</xsl:text>
    <xsl:if test="$attrbasename != 'MidlDoesNotLikeEmptyInterfaces'">
        <xsl:text>
#ifdef VPOX_WITH_DTRACE_R3_MAIN
        </xsl:text>
...
<xsl:text>
    ...
    try
    {
        ...
        <xsl:value-of select="translate(concat('VPOXAPI_', $topclass, '_SET_', $dtraceattrname, '_ENTER('), $G_lowerCase, $G_upperCase)"/>
        ...
        <xsl:value-of select="translate(concat('VPOXAPI_', $dtracetopclass, '_SET_', $dtraceattrname, '_RETURN('), $G_lowerCase, $G_upperCase)"/>
        <xsl:text>this, hrc, 0 /*normal*/,</xsl:text>
        <xsl:call-template name="emitDTraceParamValue">
            <xsl:with-param name="dir">in</xsl:with-param>
        </xsl:call-template>
    ...
    {
        hrc = VirtualPoxBase::handleUnexpectedExceptions(this, RT_SRC_POS);
#ifdef VPOX_WITH_DTRACE_R3_MAIN
        </xsl:text>
    <xsl:value-of select="translate(concat('VPOXAPI_', $dtracetopclass, '_SET_', $dtraceattrname, '_RETURN('), $G_lowerCase, $G_upperCase)"/>
    <xsl:text>this, hrc, 9 /*unhandled exception*/,</xsl:text>
...
<xsl:template name="emitTargetBegin">
    ...
    <xsl:choose>
        ...
        <xsl:when test="$target = 'midl'">
            <xsl:text>#ifndef VPOX_WITH_XPCOM
</xsl:text>
        </xsl:when>
...
<xsl:text>
    ...
    <xsl:value-of select="translate(concat('VPOXAPI_', $dtracetopclass, '_', $dtracemethodname, substring($dtracenamehack, 2), '_ENTER('), $G_lowerCase, $G_upperCase)"/>
    <xsl:text>this</xsl:text>
    ...
    <xsl:value-of select="translate(concat('VPOXAPI_', $dtracetopclass, '_', $dtracemethodname, substring($dtracenamehack, 2), '_RETURN('), $G_lowerCase, $G_upperCase)"/>
    <xsl:text>this, hrc, 0 /*normal*/</xsl:text>
    <xsl:for-each select="param">
        <xsl:text>, </xsl:text>
        <xsl:call-template name="emitDTraceParamValue">
...
<xsl:template match="interface">
    <xsl:if test="not(@internal='yes') and not(@autogen='VPoxEvent') and not(@supportsErrorInfo='no')">
