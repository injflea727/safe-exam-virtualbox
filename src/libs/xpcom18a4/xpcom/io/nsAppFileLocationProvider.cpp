...
#if (defined(XP_MAC) || defined(XP_MACOSX)) && !defined(VPOX_MACOSX_FOLLOWS_UNIX_IO)
#include <Folders.h>
#include <Script.h>
#include <Processes.h>
#include <Gestalt.h>
...
nsAppFileLocationProvider::GetFile(const char *prop, PRBool *persistant, nsIFile **_retval)
{
    ...
    *persistant = PR_TRUE;
    ...
    else if (nsCRT::strcmp(prop, NS_ENV_PLUGINS_DIR) == 0)
    {
        ...
        const char *pathVar = PR_GetEnv("VPOX_XPCOM_PLUGIN_PATH");
        if (pathVar)
...
NS_METHOD nsAppFileLocationProvider::GetProductDirectory(nsILocalFile **aLocalFile)
{
    ...
    if (NS_FAILED(rv)) return rv;
#elif defined(XP_MACOSX) && !defined(VPOX_MACOSX_FOLLOWS_UNIX_IO)
    FSRef fsRef;
    ...
    *_retval = nsnull;
    ...
    if (!nsCRT::strcmp(prop, NS_APP_PLUGINS_DIR_LIST))
    {
        ...
        *_retval = new nsAppDirectoryEnumerator(this, keys);
        ...
        if (!keys[0] && !(keys[0] = PR_GetEnv("VPOX_XPCOM_PLUGIN_PATH"))) {
