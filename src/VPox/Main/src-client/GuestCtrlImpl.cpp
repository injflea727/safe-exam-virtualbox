...
/** @file
 * VirtualPox COM class implementation: Guest
 */
                                                ...
                                                uint32_t cbData)
{
     ...
     */
    AssertMsgReturn(cbData == sizeof(VPOXGUESTCTRLHOSTCALLBACK),
                    ("Guest control host callback data has wrong size (expected %zu, got %zu) - buggy host service!\n",
                     sizeof(VPOXGUESTCTRLHOSTCALLBACK), cbData), VERR_INVALID_PARAMETER);
    PVPOXGUESTCTRLHOSTCALLBACK pSvcCb = (PVPOXGUESTCTRLHOSTCALLBACK)pvData;
    ...
    if (idMessage == GUEST_MSG_REPORT_FEATURES)
    {
        ...
        Assert(pSvcCb->mpaParms[0].type == VPOX_HGCM_SVC_PARM_64BIT);
        Assert(pSvcCb->mpaParms[1].type == VPOX_HGCM_SVC_PARM_64BIT);
        Assert(pSvcCb->mpaParms[1].u.uint64 & VPOX_GUESTCTRL_GF_1_MUST_BE_ONE);
    ...
    ASSERT_GUEST_MSG_RETURN(pSvcCb->mpaParms[0].type == VPOX_HGCM_SVC_PARM_32BIT,
                            ("type=%d\n", pSvcCb->mpaParms[0].type), VERR_WRONG_PARAMETER_TYPE);
    uint32_t const idContext = pSvcCb->mpaParms[0].u.uint32;

    VPOXGUESTCTRLHOSTCBCTX CtxCb = { idMessage, idContext };
    int rc = pGuest->i_dispatchToSession(&CtxCb, pSvcCb);

    Log2Func(("CID=%#x, idSession=%RU32, uObject=%RU32, uCount=%RU32, rc=%Rrc\n",
              idContext, VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(idContext), VPOX_GUESTCTRL_CONTEXTID_GET_OBJECT(idContext),
              VPOX_GUESTCTRL_CONTEXTID_GET_COUNT(idContext), rc));
    return rc;
}
 ...
 */
int Guest::i_dispatchToSession(PVPOXGUESTCTRLHOSTCBCTX pCtxCb, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb)
{
    ...
    const uint32_t uSessionID = VPOX_GUESTCTRL_CONTEXTID_GET_SESSION(pCtxCb->uContextID);
 ...
 * This will invoke VPoxService running on the guest creating a new (dedicated) guest session
                           ...
                           const GuestCredentials &guestCreds, ComObjPtr<GuestSession> &pGuestSession)
{
    ...
    if (mData.mGuestSessions.size() >= VPOX_GUESTCTRL_MAX_SESSIONS)
        return rc;

    try
    {
        /* Create a new session ID and assign it. */
        uint32_t uNewSessionID = VPOX_GUESTCTRL_SESSION_ID_BASE;
        uint32_t uTries = 0;
        ...
        for (;;)
        {
            ...
            uNewSessionID++;
            if (uNewSessionID >= VPOX_GUESTCTRL_MAX_SESSIONS)
                uNewSessionID = VPOX_GUESTCTRL_SESSION_ID_BASE;

            if (++uTries == VPOX_GUESTCTRL_MAX_SESSIONS)
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            case VERR_MAX_PROCS_REACHED:
                hr = setErrorBoth(VPOX_E_MAXIMUM_REACHED, vrc, tr("Maximum number of concurrent guest sessions (%d) reached"),
                                  VPOX_GUESTCTRL_MAX_SESSIONS);
                break;

            /** @todo Add more errors here. */

            default:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Could not create guest session: %Rrc"), vrc);
                break;
        }
    }
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        else
        {
            if (vrc == VERR_GSTCTL_GUEST_ERROR)
                vrc = rcGuest;
            hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Could not open guest session: %Rrc"), vrc);
        }
    }
    else
    {
        ...
        {
            case VERR_MAX_PROCS_REACHED:
                hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Maximum number of concurrent guest sessions (%d) reached"),
                                  VPOX_GUESTCTRL_MAX_SESSIONS);
                break;
           ...
           default:
                hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Could not create guest session: %Rrc"), vrc);
                break;
        }
    }
