/* $Id: SUPLib-win.cpp $ */
/** @file
 * VirtualPox Support Library - Windows NT specific parts.
 */
...
int suplibOsHardenedVerifyInit(void)
{
    ...
    {
#if defined(VPOX_WITH_HARDENING) && !defined(IN_SUP_HARDENED_R3) && !defined(IN_SUP_R3_STATIC)
...
int suplibOsInit(PSUPLIBDATA pThis, bool fPreInited, bool fUnrestricted, SUPINITOP *penmWhat, PRTERRINFO pErrInfo)
{
    ...
    if (fPreInited)
    {
#if defined(VPOX_WITH_HARDENING) && !defined(IN_SUP_HARDENED_R3)
...
# endif
    ...
    for (;;)
    {
        ...
        static const WCHAR  s_wszName[] = L"\\Device\\VPoxDrvU";
...
#ifdef IN_SUP_HARDENED_R3
        /*
         * Get more details from VPoxDrvErrorInfo if present.
         */
...
static int suplibOsCreateService(void)
{
    ...
    if (hSMgrCreate != NULL)
    {
        ...
        rc = RTPathExecDir(szDriver, sizeof(szDriver) - sizeof("\\VPoxSup.sys"));
        if (RT_SUCCESS(rc))
        {
            strcat(szDriver, "\\VPoxSup.sys");
...
int suplibOsDeleteService(void)
{
    ...
    if (hSMgr)
    {
         ...
         * Old service name.
         */
        SC_HANDLE hService = OpenService(hSMgr, "VPoxDrv", DELETE);
        if (hService)
        {
            if (!DeleteService(hService))
            {
                dwErr = GetLastError();
                AssertMsgFailed(("DeleteService failed for VPoxDrv dwErr=%Rwa\n", dwErr));
        ...
        else
        {
            ...
            {
                AssertMsgFailed(("OpenService failed for VPoxDrv dwErr=%Rwa\n", dwErr));
                rcRet = RTErrConvertFromWin32(dwErr);
            }
        }
...
static int suplibOsUpdateService(void)
{
    ...
    if (hSMgr)
    {
        ...
        {
            char szDriver[RTPATH_MAX];
            int rc = RTPathExecDir(szDriver, sizeof(szDriver) - sizeof("\\VPoxSup.sys"));
            if (RT_SUCCESS(rc))
            {
                strcat(szDriver, "\\VPoxSup.sys");
...
int suplibOsPageAlloc(PSUPLIBDATA pThis, size_t cPages, void **ppvPages)
{
    ...
    if (!(cPages & 511))
    {
        ...
        { /* likely */ }
        else if (RTEnvExistsUtf8("VPOX_DO_NOT_USE_LARGE_PAGES")) /** @todo add flag for this instead? */
...
# else
/**
 * Reverse of VPoxDrvNtErr2NtStatus
 * returns VPox status code.
...
static int suplibConvertNtStatus(NTSTATUS rcNt)
{
    ...
    /* See VPoxDrvNtErr2NtStatus. */
    if (SUP_NT_STATUS_IS_VPOX(rcNt))
        return SUP_NT_STATUS_TO_VPOX(rcNt);
