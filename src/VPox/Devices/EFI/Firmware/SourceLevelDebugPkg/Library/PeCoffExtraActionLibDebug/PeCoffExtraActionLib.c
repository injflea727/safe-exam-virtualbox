...
static void vpoxImageEvtU64(uint32_t uCmd, uint64_t uValue)
{
...
static void vpoxImageEvtString(uint32_t uCmd, const char *pszName)
{
    unsigned char uch;
    while ((uch = *pszName++) != '\0')
...
static void vpoxImageEvtEmitOne(PE_COFF_LOADER_IMAGE_CONTEXT const *pImageCtx, uint32_t uEvt)
{
    ASMOutU32(EFI_PORT_IMAGE_EVENT, uEvt);
    if (pImageCtx->DestinationAddress)
        vpoxImageEvtU64(EFI_IMAGE_EVT_CMD_ADDR0, pImageCtx->DestinationAddress);
    else
        vpoxImageEvtU64(EFI_IMAGE_EVT_CMD_ADDR0, pImageCtx->ImageAddress);
    vpoxImageEvtU64(EFI_IMAGE_EVT_CMD_SIZE0, pImageCtx->ImageSize);
    if (pImageCtx->PdbPointer)
        vpoxImageEvtString(EFI_IMAGE_EVT_CMD_NAME, pImageCtx->PdbPointer);
    ASMOutU32(EFI_PORT_IMAGE_EVENT, EFI_IMAGE_EVT_CMD_COMPLETE);
}
#endif


#ifdef VPOX_SOURCE_DEBUG_ENABLE
/**
...
#ifdef VPOX
# if ARCH_BITS == 32
  vpoxImageEvtEmitOne(ImageContext, EFI_IMAGE_EVT_CMD_START_LOAD32);
# else
  vpoxImageEvtEmitOne(ImageContext, EFI_IMAGE_EVT_CMD_START_LOAD64);
# endif
#endif
}
...
#ifdef VPOX
# if ARCH_BITS == 32
  vpoxImageEvtEmitOne(ImageContext, EFI_IMAGE_EVT_CMD_START_UNLOAD32);
# else
  vpoxImageEvtEmitOne(ImageContext, EFI_IMAGE_EVT_CMD_START_UNLOAD64);
# endif
#endif
}
...
VOID
EFIAPI
VPoxPeCoffLoaderMoveImageExtraAction(
...
#endif
    vpoxImageEvtU64(EFI_IMAGE_EVT_CMD_ADDR0, NewBase);
    vpoxImageEvtU64(EFI_IMAGE_EVT_CMD_ADDR1, OldBase);
    ASMOutU32(EFI_PORT_IMAGE_EVENT, EFI_IMAGE_EVT_CMD_COMPLETE);
}
#endif
