/* $Id: Svga.cpp $ */
/** @file
 * VirtualPox Windows Guest Mesa3D - VMSVGA.
 */
...
static NTSTATUS svgaHwInit(VPOXWDDM_EXT_VMSVGA *pSvga)
{
...
void SvgaAdapterStop(PVPOXWDDM_EXT_VMSVGA pSvga,
...
NTSTATUS SvgaAdapterStart(PVPOXWDDM_EXT_VMSVGA *ppSvga,
                          DXGKRNL_INTERFACE *pDxgkInterface,
...
// ASMBreakpoint();
    ...
    VPOXWDDM_EXT_VMSVGA *pSvga = (VPOXWDDM_EXT_VMSVGA *)GaMemAllocZero(sizeof(VPOXWDDM_EXT_VMSVGA));
    if (!pSvga)
...
NTSTATUS SvgaQueryInfo(PVPOXWDDM_EXT_VMSVGA pSvga,
                       VPOXGAHWINFOSVGA *pSvgaInfo)
{
    pSvgaInfo->cbInfoSVGA = sizeof(VPOXGAHWINFOSVGA);
...
NTSTATUS SvgaScreenDefine(PVPOXWDDM_EXT_VMSVGA pSvga,
                          uint32_t u32Offset,
...
NTSTATUS SvgaScreenDestroy(PVPOXWDDM_EXT_VMSVGA pSvga,
                           uint32_t u32ScreenId)
{
    NTSTATUS Status = STATUS_SUCCESS;
...
NTSTATUS SvgaIdAlloc(PVPOXWDDM_EXT_VMSVGA pSvga,
                     uint32_t *pu32Bits,
                     uint32_t cbBits,
                     uint32_t u32Limit,
                     uint32_t *pu32Id)
...
NTSTATUS SvgaIdFree(PVPOXWDDM_EXT_VMSVGA pSvga,
                    uint32_t *pu32Bits,
                    uint32_t cbBits,
                    uint32_t u32Limit,
                    uint32_t u32Id)
...
NTSTATUS SvgaContextIdAlloc(PVPOXWDDM_EXT_VMSVGA pSvga,
                            uint32_t *pu32Cid)
{
...
NTSTATUS SvgaContextIdFree(PVPOXWDDM_EXT_VMSVGA pSvga,
                           uint32_t u32Cid)
{
...
NTSTATUS SvgaSurfaceIdAlloc(PVPOXWDDM_EXT_VMSVGA pSvga,
                            uint32_t *pu32Sid)
{
...
NTSTATUS SvgaSurfaceIdFree(PVPOXWDDM_EXT_VMSVGA pSvga,
                           uint32_t u32Sid)
{
...
NTSTATUS SvgaGMRIdAlloc(PVPOXWDDM_EXT_VMSVGA pSvga,
                        uint32_t *pu32GMRId)
{
...
NTSTATUS SvgaGMRIdFree(PVPOXWDDM_EXT_VMSVGA pSvga,
                       uint32_t u32GMRId)
{
...
NTSTATUS SvgaContextCreate(PVPOXWDDM_EXT_VMSVGA pSvga,
                           uint32_t u32Cid)
{
    NTSTATUS Status = STATUS_SUCCESS;
...
NTSTATUS SvgaContextDestroy(PVPOXWDDM_EXT_VMSVGA pSvga,
                            uint32_t u32Cid)
{
    NTSTATUS Status = STATUS_SUCCESS;
...
NTSTATUS SvgaFence(PVPOXWDDM_EXT_VMSVGA pSvga,
                   uint32_t u32Fence)
{
    NTSTATUS Status = STATUS_SUCCESS;
...
NTSTATUS SvgaSurfaceDefine(PVPOXWDDM_EXT_VMSVGA pSvga,
...
NTSTATUS SvgaSurfaceDestroy(PVPOXWDDM_EXT_VMSVGA pSvga,
                            uint32_t u32Sid)
{
    NTSTATUS Status = STATUS_SUCCESS;
...
NTSTATUS SvgaSharedSidInsert(VPOXWDDM_EXT_VMSVGA *pSvga,
                             uint32_t u32Sid,
...
NTSTATUS SvgaSharedSidRemove(VPOXWDDM_EXT_VMSVGA *pSvga,
                             uint32_t u32Sid)
{
...
NTSTATUS SvgaProcessSurface(VPOXWDDM_EXT_VMSVGA *pSvga,
                            uint32_t *pu32Sid,
...
NTSTATUS SvgaRenderComplete(PVPOXWDDM_EXT_VMSVGA pSvga,
...
static NTSTATUS svgaUpdateCommand(VPOXWDDM_EXT_VMSVGA *pSvga,
                                  uint32_t u32CmdId,
                                  uint8_t *pu8Cmd,
                                  uint32_t cbCmd,
                                  SVGAHOSTOBJECTARRAY *pHOA)
{
    ...
    switch (u32CmdId)
    {
         ...
         * The VPox VMSVGA device does not implement them and most of them are not used by SVGA driver.
         */
 ...
 */
NTSTATUS SvgaRenderCommands(PVPOXWDDM_EXT_VMSVGA pSvga,
                            void *pvTarget,
                            uint32_t cbTarget,
                            const void *pvSource,
                            uint32_t cbSource,
...
NTSTATUS SvgaPresent(PVPOXWDDM_EXT_VMSVGA pSvga,
                     uint32_t u32Sid,
                     uint32_t u32Width,
                     uint32_t u32Height)
{
...
NTSTATUS SvgaGenPresentVRAM(PVPOXWDDM_EXT_VMSVGA pSvga,
                            uint32_t u32Sid,
                            uint32_t u32Width,
                            uint32_t u32Height,
...
NTSTATUS SvgaPresentVRAM(PVPOXWDDM_EXT_VMSVGA pSvga,
                         uint32_t u32Sid,
                         uint32_t u32Width,
                         uint32_t u32Height,
                         uint32_t u32VRAMOffset)
...
NTSTATUS SvgaGenSurfaceDMA(PVPOXWDDM_EXT_VMSVGA pSvga,
...
NTSTATUS SvgaGenBlitGMRFBToScreen(PVPOXWDDM_EXT_VMSVGA pSvga,
...
NTSTATUS SvgaBlitGMRFBToScreen(PVPOXWDDM_EXT_VMSVGA pSvga,
                               uint32_t idDstScreen,
                               int32_t xSrc,
                               int32_t ySrc,
                               RECT const *pDstRect)
...
NTSTATUS SvgaGenBlitScreenToGMRFB(PVPOXWDDM_EXT_VMSVGA pSvga,
...
NTSTATUS SvgaGenBlitSurfaceToScreen(PVPOXWDDM_EXT_VMSVGA pSvga,
                                    uint32_t sid,
                                    RECT const *pSrcRect,
                                    uint32 idDstScreen,
                                    RECT const *pDstRect,
...
NTSTATUS SvgaUpdate(PVPOXWDDM_EXT_VMSVGA pSvga,
                    uint32_t u32X,
                    uint32_t u32Y,
                    uint32_t u32Width,
                    uint32_t u32Height)
...
NTSTATUS SvgaGenDefineCursor(PVPOXWDDM_EXT_VMSVGA pSvga,
...
NTSTATUS SvgaDefineCursor(PVPOXWDDM_EXT_VMSVGA pSvga,
                          uint32_t u32HotspotX, uint32_t u32HotspotY, uint32_t u32Width, uint32_t u32Height,
                          uint32_t u32AndMaskDepth, uint32_t u32XorMaskDepth,
                          void const *pvAndMask, uint32_t cbAndMask, void const *pvXorMask, uint32_t cbXorMask)
{
...
NTSTATUS SvgaGenDefineAlphaCursor(PVPOXWDDM_EXT_VMSVGA pSvga,
                                  uint32_t u32HotspotX, uint32_t u32HotspotY, uint32_t u32Width, uint32_t u32Height,
...
NTSTATUS SvgaDefineAlphaCursor(PVPOXWDDM_EXT_VMSVGA pSvga,
                               uint32_t u32HotspotX, uint32_t u32HotspotY, uint32_t u32Width, uint32_t u32Height,
                               void const *pvImage, uint32_t cbImage)
{
    NTSTATUS Status = STATUS_SUCCESS;
...
NTSTATUS SvgaGenDefineGMRFB(PVPOXWDDM_EXT_VMSVGA pSvga,
                            uint32_t u32Offset,
...
NTSTATUS SvgaDefineGMRFB(PVPOXWDDM_EXT_VMSVGA pSvga,
                         uint32_t u32Offset,
                         uint32_t u32BytesPerLine,
                         bool fForce)
{
...
NTSTATUS SvgaGenGMRReport(PVPOXWDDM_EXT_VMSVGA pSvga,
                          uint32_t u32GmrId,
...
NTSTATUS SvgaGMRReport(PVPOXWDDM_EXT_VMSVGA pSvga,
                       uint32_t u32GmrId,
                       SVGARemapGMR2Flags fRemapGMR2Flags,
                       uint32_t u32NumPages,
                       RTHCPHYS *paPhysAddresses)
...
static void gaRegionFree(VPOXWDDM_EXT_VMSVGA *pSvga, GAWDDMREGION *pRegion)
{
    Assert(pRegion);
    gmrFree(pRegion);
...
void SvgaRegionsDestroy(VPOXWDDM_EXT_VMSVGA *pSvga,
                        void *pvOwner)
{
...
NTSTATUS SvgaRegionDestroy(VPOXWDDM_EXT_VMSVGA *pSvga,
                           uint32_t u32GmrId)
{
...
NTSTATUS SvgaRegionCreate(VPOXWDDM_EXT_VMSVGA *pSvga,
                          void *pvOwner,
                          uint32_t u32NumPages,
                          uint32_t *pu32GmrId,
