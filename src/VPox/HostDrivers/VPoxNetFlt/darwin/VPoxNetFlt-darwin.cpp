/* $Id: VPoxNetFlt-darwin.cpp $ */
/** @file
 * VPoxNetFlt - Network Filter Driver (Host), Darwin Specific Code.
 */
...
#define VPOXNETFLT_DARWIN_MAX_SEGS      32

#if 0
/** For testing extremely segmented frames. */
#define VPOXNETFLT_DARWIN_TEST_SEG_SIZE 14
#endif

/* XXX: hidden undef #ifdef __APPLE__ */
#define VPOX_IN_LOOPBACK(addr)  (((addr) & IN_CLASSA_NET) == 0x7f000000)
#define VPOX_IN_LINKLOCAL(addr) (((addr) & IN_CLASSB_NET) == 0xa9fe0000)
...
*********************************************************************************************************************************/
RT_C_DECLS_BEGIN
static kern_return_t    VPoxNetFltDarwinStart(struct kmod_info *pKModInfo, void *pvData);
static kern_return_t    VPoxNetFltDarwinStop(struct kmod_info *pKModInfo, void *pvData);

static void vpoxNetFltDarwinSysSockUpcall(socket_t pSysSock, void *pvData, int fWait);
RT_C_DECLS_END
 ...
 */
typedef struct VPOXNETFLTTAG
{
    /** The ethernet header of the outgoing frame. */
    RTNETETHERHDR EthHdr;
} VPOXNETFLTTAG;
/** Pointer to a VPoxNetFlt mbuf tag. */
typedef VPOXNETFLTTAG *PVPOXNETFLTTAG;
/** Pointer to a const VPoxNetFlt mbuf tag. */
typedef VPOXNETFLTTAG const *PCVPOXNETFLTTAG;
...
KMOD_EXPLICIT_DECL(VPoxNetFlt, VPOX_VERSION_STRING, _start, _stop)
DECLHIDDEN(kmod_start_func_t *) _realmain = VPoxNetFltDarwinStart;
DECLHIDDEN(kmod_stop_func_t  *) _antimain = VPoxNetFltDarwinStop;
...
#define VPOX_GET_PCOUNT(pIfNet) ( *(int *)((uintptr_t)pIfNet + g_offIfNetPCount) )
/** The size of area of ifnet structure we try to locate if_pcount in. */
#define VPOXNETFLT_DARWIN_IFNET_SIZE 256
 ...
 */
static unsigned vpoxNetFltDarwinSetAndDiff(ifnet_t pIfNet, int iPromisc)
{
    int aiSavedState[VPOXNETFLT_DARWIN_IFNET_SIZE / sizeof(int)];
 ...
 * @param   pIfNet      The interface we're attaching to.
 */
static void vpoxNetFltDarwinDetectPCountOffset(ifnet_t pIfNet)
{
    ...
    {
        offTry1 = vpoxNetFltDarwinSetAndDiff(pIfNet, 1);
        offTry2 = vpoxNetFltDarwinSetAndDiff(pIfNet, 1);
        offTry3 = vpoxNetFltDarwinSetAndDiff(pIfNet, 0);
        offTry4 = vpoxNetFltDarwinSetAndDiff(pIfNet, 0);
        if (offTry1 == offTry2 && offTry2 == offTry3 && offTry3 == offTry4)
        {
            if (g_offIfNetPCount != offTry1)
            {
                Log(("VPoxNetFltDarwinDetectPCountOffset: Adjusted if_pcount offset to %x from %x.\n", offTry1, g_offIfNetPCount));
    ...
    if (g_offIfNetPCount != offTry1)
        LogRel(("VPoxNetFlt: Failed to detect promiscuous count, all traffic may reach wire (%x != %x).\n", g_offIfNetPCount, offTry1));
}
...
static kern_return_t    VPoxNetFltDarwinStart(struct kmod_info *pKModInfo, void *pvData)
{
    ...
    if (RT_SUCCESS(rc))
    {
        Log(("VPoxNetFltDarwinStart\n"));
        errno_t err = mbuf_tag_id_find("org.VirtualPox.kext.VPoxFltDrv", &g_idTag);
        if (!err)
        {
            /*
            ...
            if (RT_SUCCESS(rc))
            {
                LogRel(("VPoxFltDrv: version " VPOX_VERSION_STRING " r%d\n", VPOX_SVN_REV));
                return KMOD_RETURN_SUCCESS;
            }

            LogRel(("VPoxFltDrv: failed to initialize device extension (rc=%d)\n", rc));
        }
        else
            LogRel(("VPoxFltDrv: mbuf_tag_id_find failed, err=%d\n", err));
        RTR0Term();
    }
    else
        printf("VPoxFltDrv: failed to initialize IPRT (rc=%d)\n", rc);
...
static kern_return_t VPoxNetFltDarwinStop(struct kmod_info *pKModInfo, void *pvData)
{
     ...
     * tracking for us!
     */
    int rc = vpoxNetFltTryDeleteIdcAndGlobals(&g_VPoxNetFltGlobals);
    if (RT_FAILURE(rc))
    {
        Log(("VPoxNetFltDarwinStop - failed, busy.\n"));
        return KMOD_RETURN_FAILURE;
    }
 ...
 * @param   pThis
 */
DECLINLINE(ifnet_t) vpoxNetFltDarwinRetainIfNet(PVPOXNETFLTINS pThis)
{
    ifnet_t pIfNet = NULL;
 ...
 * by vpoxNetFltDarwinRetainIfNet.
 *
 * @param   pThis           The instance.
 * @param   pIfNet          The vpoxNetFltDarwinRetainIfNet return value, NULL is fine.
 */
DECLINLINE(void) vpoxNetFltDarwinReleaseIfNet(PVPOXNETFLTINS pThis, ifnet_t pIfNet)
{
    NOREF(pThis);
    if (pIfNet)
...
/**
 * Checks whether this is an mbuf created by vpoxNetFltDarwinMBufFromSG,
 ...
 */
DECLINLINE(bool) vpoxNetFltDarwinMBufIsOur(PVPOXNETFLTINS pThis, mbuf_t pMBuf, void *pvFrame)
{
    NOREF(pThis);

    /*
     * Lookup the tag set by vpoxNetFltDarwinMBufFromSG.
     */
    PCVPOXNETFLTTAG pTagData;
    size_t cbTagData;
 ...
 */
static mbuf_t vpoxNetFltDarwinMBufFromSG(PVPOXNETFLTINS pThis, PINTNETSG pSG)
{
    ...
    if (!err)
    {
        ...
        if (!err)
        {
             ...
             */
            PVPOXNETFLTTAG pTagData;
            err = mbuf_tag_allocate(pPkt, g_idTag, 0 /* type */, sizeof(VPOXNETFLTTAG) /* tag len */, How, (void **)&pTagData);
            if (!err)
            {
 ...
 * @param   pvFrame             The frame pointer, optional.
 */
DECLINLINE(unsigned) vpoxNetFltDarwinMBufCalcSGSegs(PVPOXNETFLTINS pThis, mbuf_t pMBuf, void *pvFrame)
{
    NOREF(pThis);
...
#ifdef VPOXNETFLT_DARWIN_TEST_SEG_SIZE
    /* maximize the number of segments. */
    cSegs = RT_MAX(VPOXNETFLT_DARWIN_MAX_SEGS - 1, cSegs);
#endif
 ...
 */
DECLINLINE(void) vpoxNetFltDarwinMBufToSG(PVPOXNETFLTINS pThis, mbuf_t pMBuf, void *pvFrame, PINTNETSG pSG, unsigned cSegs,
                                          uint32_t fSrc)
{
    RT_NOREF(pThis, fSrc);
    ...
    if (pSG->cSegsUsed < pSG->cSegsAlloc)
    {
               ...
               &&   iDst < pSG->cSegsAlloc)
        {
            ...
            pSG->aSegs[iDst].cb = RT_MIN(pSG->aSegs[iSrc].cb, VPOXNETFLT_DARWIN_TEST_SEG_SIZE);
 ...
 * promiscuous.
 */
static bool vpoxNetFltDarwinIsPromiscuous(PVPOXNETFLTINS pThis)
{
    bool fRc = false;
    ifnet_t pIfNet = vpoxNetFltDarwinRetainIfNet(pThis);
    if (pIfNet)
    {
        /* gather the data */
        uint16_t fIf = ifnet_flags(pIfNet);
        unsigned cPromisc = VPOX_GET_PCOUNT(pIfNet);
        bool fSetPromiscuous = ASMAtomicUoReadBool(&pThis->u.s.fSetPromiscuous);
        vpoxNetFltDarwinReleaseIfNet(pThis, pIfNet);
 ...
 */
static void vpoxNetFltDarwinIffDetached(void *pvThis, ifnet_t pIfNet)
{
    PVPOXNETFLTINS pThis = (PVPOXNETFLTINS)pvThis;
    uint64_t NanoTS = RTTimeSystemNanoTS();
    LogFlow(("vpoxNetFltDarwinIffDetached: pThis=%p NanoTS=%RU64 (%d)\n",
             pThis, NanoTS, VALID_PTR(pIfNet) ? VPOX_GET_PCOUNT(pIfNet) :  -1));
    ...
    pIfNet = ASMAtomicUoReadPtrT(&pThis->u.s.pIfNet, ifnet_t);
    int cPromisc = VALID_PTR(pIfNet) ? VPOX_GET_PCOUNT(pIfNet) : - 1;
    ...
    if (pIfNet)
        ifnet_release(pIfNet);
    LogRel(("VPoxNetFlt: was detached from '%s' (%d)\n", pThis->szName, cPromisc));
}
 ...
 */
static errno_t vpoxNetFltDarwinIffIoCtl(void *pvThis, ifnet_t pIfNet, protocol_family_t eProtocol, u_long uCmd, void *pvArg)
{
    RT_NOREF(pIfNet);
    PVPOXNETFLTINS pThis = (PVPOXNETFLTINS)pvThis;
    LogFlow(("vpoxNetFltDarwinIffIoCtl: pThis=%p uCmd=%lx\n", pThis, uCmd));
 ...
 */
static void vpoxNetFltDarwinIffEvent(void *pvThis, ifnet_t pIfNet, protocol_family_t eProtocol, const struct kev_msg *pEvMsg)
{
    PVPOXNETFLTINS pThis = (PVPOXNETFLTINS)pvThis;
    LogFlow(("vpoxNetFltDarwinIffEvent: pThis=%p\n", pThis));
            ...
            if (pEvMsg->event_code == KEV_DL_LINK_ON)
            {
                if (ASMAtomicUoReadBool(&pThis->u.s.fNeedSetPromiscuous))
                {
                    ...
                    if (!err)
                    {
                        ...
                        ASMAtomicWriteBool(&pThis->u.s.fNeedSetPromiscuous, false);
                        Log(("vpoxNetFltDarwinIffEvent: enabled promiscuous mode on %s (%d)\n", pThis->szName, VPOX_GET_PCOUNT(pIfNet)));
                    }
                    else
                        Log(("vpoxNetFltDarwinIffEvent: ifnet_set_promiscuous failed on %s, err=%d (%d)\n", pThis->szName, err, VPOX_GET_PCOUNT(pIfNet)));
                }
                         ...
                         && !(ifnet_flags(pIfNet) & IFF_PROMISC))
                {
                    ...
                    if (!err && (ifnet_flags(pIfNet) & IFF_PROMISC))
                        Log(("vpoxNetFltDarwinIffEvent: fixed IFF_PROMISC on %s (%d)\n", pThis->szName, VPOX_GET_PCOUNT(pIfNet)));
                    else
                        Log(("vpoxNetFltDarwinIffEvent: failed to fix IFF_PROMISC on %s, err=%d flags=%#x (%d)\n",
                             pThis->szName, err, ifnet_flags(pIfNet), VPOX_GET_PCOUNT(pIfNet)));
                }
                else
                    Log(("vpoxNetFltDarwinIffEvent: online, '%s'. flags=%#x (%d)\n", pThis->szName, ifnet_flags(pIfNet), VPOX_GET_PCOUNT(pIfNet)));
            }
            else if (pEvMsg->event_code == KEV_DL_LINK_OFF)
                Log(("vpoxNetFltDarwinIffEvent: %s goes down (%d)\n", pThis->szName, VPOX_GET_PCOUNT(pIfNet)));
/** @todo KEV_DL_LINK_ADDRESS_CHANGED  -> pfnReportMacAddress */
/** @todo KEV_DL_SIFFLAGS              -> pfnReportPromiscuousMode */
        }
        else
            Log(("vpoxNetFltDarwinIffEvent: pThis->u.s.pIfNet=%p pIfNet=%p (%d)\n", pThis->u.s.pIfNet, pIfNet, VALID_PTR(pIfNet) ? VPOX_GET_PCOUNT(pIfNet) : -1));
    }
    else if (VALID_PTR(pEvMsg))
        Log(("vpoxNetFltDarwinIffEvent: vendor_code=%#x kev_class=%#x kev_subclass=%#x event_code=%#x\n",
             pEvMsg->vendor_code, pEvMsg->kev_class, pEvMsg->kev_subclass, pEvMsg->event_code));
}


/**
 * Internal worker for  vpoxNetFltDarwinIffInput and vpoxNetFltDarwinIffOutput,
 *
 ...
 */
static errno_t vpoxNetFltDarwinIffInputOutputWorker(PVPOXNETFLTINS pThis, mbuf_t pMBuf, void *pvFrame,
                                                    uint32_t fSrc, protocol_family_t eProtocol)
{
    /*
     * Drop it immediately?
     */
    Log2(("vpoxNetFltDarwinIffInputOutputWorker: pThis=%p pMBuf=%p pvFrame=%p fSrc=%#x cbPkt=%x\n",
...
#endif
    if (vpoxNetFltDarwinMBufIsOur(pThis, pMBuf, pvFrame))
        return 0;
     ...
     */
    bool fDropIt = false;
    unsigned cSegs = vpoxNetFltDarwinMBufCalcSGSegs(pThis, pMBuf, pvFrame);
    if (cSegs < VPOXNETFLT_DARWIN_MAX_SEGS)
    {
        PINTNETSG pSG = (PINTNETSG)alloca(RT_UOFFSETOF_DYN(INTNETSG, aSegs[cSegs]));
        vpoxNetFltDarwinMBufToSG(pThis, pMBuf, pvFrame, pSG, cSegs, fSrc);
        ...
        if (fDropIt)
        {
            ...
            if (   fSrc == INTNETTRUNKDIR_WIRE
                && vpoxNetFltDarwinIsPromiscuous(pThis))
            {
                fDropIt = false;
            }
 ...
 */
static errno_t vpoxNetFltDarwinIffOutput(void *pvThis, ifnet_t pIfNet, protocol_family_t eProtocol, mbuf_t *ppMBuf)
{
    /** @todo there was some note about the ethernet header here or something like that... */

    NOREF(eProtocol);
    NOREF(pIfNet);
    return vpoxNetFltDarwinIffInputOutputWorker((PVPOXNETFLTINS)pvThis, *ppMBuf, NULL, INTNETTRUNKDIR_HOST, eProtocol);
}
 ...
 */
static errno_t vpoxNetFltDarwinIffInput(void *pvThis, ifnet_t pIfNet, protocol_family_t eProtocol, mbuf_t *ppMBuf, char **ppchFrame)
{
    RT_NOREF(eProtocol, pIfNet);
    return vpoxNetFltDarwinIffInputOutputWorker((PVPOXNETFLTINS)pvThis, *ppMBuf, *ppchFrame, INTNETTRUNKDIR_WIRE, eProtocol);
}


/** A worker thread for vpoxNetFltSendDummy(). */
static DECLCALLBACK(int) vpoxNetFltSendDummyWorker(RTTHREAD hThreadSelf, void *pvUser)
{
    RT_NOREF(hThreadSelf);
    Assert(pvUser);
    ifnet_t pIfNet = (ifnet_t)pvUser;
    return VPoxNetSendDummy(pIfNet);
}


/**
 * Prevent GUI icon freeze issue when VirtualPoxVM process terminates.
 *
 ...
 * the kernel thread instead of VirtualPoxVM process.
 *
 * @param pIfNet    Interface to be used to send data.
 */
static void vpoxNetFltSendDummy(ifnet_t pIfNet)
{
    RTTHREAD hThread;
    int rc = RTThreadCreate(&hThread, vpoxNetFltSendDummyWorker, (void *)pIfNet, 0,
                            RTTHREADTYPE_DEFAULT, RTTHREADFLAGS_WAITABLE, "DummyThread");
    if (RT_SUCCESS(rc))
    {
        RTThreadWait(hThread, RT_INDEFINITE_WAIT, NULL);
        LogFlow(("vpoxNetFltSendDummy: a dummy packet has been successfully sent in order to prevent stuck-in-dock issue\n"));
    }
    else
        LogFlow(("vpoxNetFltSendDummy: unable to send dummy packet in order to prevent stuck-in-dock issue\n"));
}


/**
 * Internal worker for vpoxNetFltOsInitInstance and vpoxNetFltOsMaybeRediscovered.
 *
 ...
 */
static int vpoxNetFltDarwinAttachToInterface(PVPOXNETFLTINS pThis, bool fRediscovery)
{
    LogFlow(("vpoxNetFltDarwinAttachToInterface: pThis=%p (%s)\n", pThis, pThis->szName));
    ...
    if (err)
    {
        Assert(err == ENXIO);
        if (!fRediscovery)
            LogRel(("VPoxFltDrv: failed to find ifnet '%s' (err=%d)\n", pThis->szName, err));
        else
            Log(("VPoxFltDrv: failed to find ifnet '%s' (err=%d)\n", pThis->szName, err));
        IPRT_DARWIN_RESTORE_EFL_AC();
        return VERR_INTNET_FLT_IF_NOT_FOUND;
    }

    AssertCompileMemberAlignment(VPOXNETFLTINS, u.s.pIfNet, ARCH_BITS / 8);
    ...
    if (!err)
    {
        ...
        RegRec.iff_name     = "VPoxNetFlt";
        RegRec.iff_protocol = 0;
        RegRec.iff_input    = vpoxNetFltDarwinIffInput;
        RegRec.iff_output   = vpoxNetFltDarwinIffOutput;
        RegRec.iff_event    = vpoxNetFltDarwinIffEvent;
        RegRec.iff_ioctl    = vpoxNetFltDarwinIffIoCtl;
        RegRec.iff_detached = vpoxNetFltDarwinIffDetached;
            ...
            && vpoxNetFltTryRetainBusyNotDisconnected(pThis))
        {
             ...
             * traffic to the host.
             */
            pThis->pSwitchPort->pfnReportPromiscuousMode(pThis->pSwitchPort, vpoxNetFltDarwinIsPromiscuous(pThis));
#endif
    ...
    if (RT_SUCCESS(rc))
        LogRel(("VPoxFltDrv: attached to '%s' / %RTmac\n", pThis->szName, &pThis->u.s.MacAddr));
    else
        LogRel(("VPoxFltDrv: failed to attach to ifnet '%s' (err=%d)\n", pThis->szName, err));
    IPRT_DARWIN_RESTORE_EFL_AC();
    return rc;
}
...
{
    vpoxNetFltDarwinAttachToInterface(pThis, true /* fRediscovery */);
...
int  vpoxNetFltPortOsXmit(PVPOXNETFLTINS pThis, void *pvIfData, PINTNETSG pSG, uint32_t fDst)
{
    ...
    if (pIfNet)
    {
        ...
        {
            mbuf_t pMBuf = vpoxNetFltDarwinMBufFromSG(pThis, pSG);
            if (pMBuf)
            {
...
void vpoxNetFltPortOsSetActive(PVPOXNETFLTINS pThis, bool fActive)
{
    ...
    if (pIfNet)
    {
        ...
        else
        {
             ...
             */
            unsigned const cPromiscBefore = VPOX_GET_PCOUNT(pIfNet);
            u_int16_t fIf;
            if (fActive)
            {
                    ...
                    if (!err)
                    {
                        ...
                        if (!(ifnet_flags(pIfNet) & IFF_PROMISC))
                        {
                            ...
                            if (!err)
                                err = ifnet_ioctl(pIfNet, 0, SIOCSIFFLAGS, NULL);
                            if (!err)
                                Log(("vpoxNetFlt: fixed IFF_PROMISC on %s (%d->%d)\n", pThis->szName, cPromiscBefore, VPOX_GET_PCOUNT(pIfNet)));
                            else
                                Log(("VPoxNetFlt: failed to fix IFF_PROMISC on %s, err=%d (%d->%d)\n",
                                     pThis->szName, err, cPromiscBefore, VPOX_GET_PCOUNT(pIfNet)));
                        }
                    }
                    else
                        Log(("VPoxNetFlt: ifnet_set_promiscuous -> err=%d grr! (%d->%d)\n", err, cPromiscBefore, VPOX_GET_PCOUNT(pIfNet)));
                }
                else if (!err)
                    Log(("VPoxNetFlt: Waiting for the link to come up... (%d->%d)\n", cPromiscBefore, VPOX_GET_PCOUNT(pIfNet)));
                if (err)
                    LogRel(("VPoxNetFlt: Failed to put '%s' into promiscuous mode, err=%d (%d->%d)\n", pThis->szName, err, cPromiscBefore, VPOX_GET_PCOUNT(pIfNet)));
            }
            else
            {
                ASMAtomicWriteBool(&pThis->u.s.fNeedSetPromiscuous, false);
                ...
                fIf = ifnet_flags(pIfNet);
                Log(("VPoxNetFlt: fIf=%#x; %d->%d\n", fIf, cPromiscBefore, VPOX_GET_PCOUNT(pIfNet)));
            }
        }
...
int  vpoxNetFltOsInitInstance(PVPOXNETFLTINS pThis, void *pvContext)
{
    ...
    int rc = vpoxNetFltDarwinAttachToInterface(pThis, false /* fRediscovery */);
    if (RT_FAILURE(rc))
        return rc;
    ...
    error = sock_socket(PF_SYSTEM, SOCK_RAW, SYSPROTO_EVENT,
                        vpoxNetFltDarwinSysSockUpcall, pThis,
                        &pThis->u.s.pSysSock);
    if (error != 0)
    {
    ...
    for (ifaddr_t *pIfAddr = pIfAddrList; *pIfAddr != NULL; ++pIfAddr)
    {
        ...
        if (family == AF_INET)
        {
            ...
            if (VPOX_IN_LOOPBACK(u32Addr))
                continue;

            if (ifaddr_ifnet(ifa) != pIfNet && VPOX_IN_LINKLOCAL(u32Addr))
                continue;
     ...
     */
    vpoxNetFltDarwinSysSockUpcall(pThis->u.s.pSysSock, pThis, MBUF_DONTWAIT);
...
static void vpoxNetFltDarwinSysSockUpcall(socket_t pSysSock, void *pvData, int fWait)
{
    ...
    {
        Log(("vpoxNetFltDarwinSysSockUpcall: %p != %p?\n",
    ...
    for (;;)
    {
        ...
        if (error != 0)
        {
            ...
            {
                Log(("vpoxNetFltDarwinSysSockUpcall: EWOULDBLOCK - we are done\n"));
                error = 0;
            }
            else
        ...
        {
            Log(("vpoxNetFltDarwinSysSockUpcall: %u bytes is too short\n", (unsigned int)len));
            mbuf_freem(m);
            return;
        }
        ...
        if (msg->kev_subclass == KEV_INET_SUBCLASS)
        {
            ...
            {
                Log(("vpoxNetFltDarwinSysSockUpcall: %u bytes is too short for KEV_INET_SUBCLASS\n",
                     (unsigned int)len));
                mbuf_freem(m);
                return;
            }
        ...
        else if (msg->kev_subclass == KEV_INET6_SUBCLASS)
        {
            ...
            {
                Log(("vpoxNetFltDarwinSysSockUpcall: %u bytes is too short for KEV_INET6_SUBCLASS\n",
                        (unsigned int)len));
                mbuf_freem(m);
                return;
            }
            ...
            Log(("vpoxNetFltDarwinSysSockUpcall: subclass %u ignored\n", (unsigned)msg->kev_subclass));
