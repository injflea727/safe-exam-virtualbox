...
"""
VirtualPox Validation Kit - Storage benchmark.
"""
...
class IoPerfTest(object):
    ...
    def __init__(self, oExecutor, dCfg = None):
        ...
        self.asGstIoPerfPaths   = [
            '${CDROM}/vpoxvalidationkit/${OS/ARCH}/IoPerf${EXESUFF}',
...
class tdStorageBenchmark(vpox.TestDriver):                                      # pylint: disable=too-many-instance-attributes
    """
    Storage benchmark.
    """
    ...
    kdStorageCfgs = {
        ...
        'daedalus':                   (True, storagecfg.DiskCfg('darwin',  storagecfg.g_ksDiskCfgStatic, \
                                                               '/Volumes/VirtualPox/Testsuite/StorageScratch')),
    ...
    def actionConfig(self):
        ...
        if 'tst-storage' in self.asTestVMs:
            oVM = self.createTestVM('tst-storage', 1, '5.0/storage/tst-storage.vdi', sKind = 'ArchLinux_64', fIoApic = True, \
                                    ...
                                    sDvdImage = self.sVPoxValidationKitIso);
            if oVM is None:
                return False;
    ...
    def getGuestDisk(self, oSession, oTxsSession, eStorageController):
        ...
        if eStorageController == vpoxcon.StorageControllerType_NVMe:
    ...
    def getDiskFormatVariantsForTesting(self, sDiskFmt, asVariants):
        ...
        """
        lstDskFmts = self.oVPoxMgr.getArray(self.oVPox.systemProperties, 'mediumFormats');
        for oDskFmt in lstDskFmts:
            if oDskFmt.id == sDiskFmt:
                lstDskVariants = [];
                lstCaps = self.oVPoxMgr.getArray(oDskFmt, 'capabilities');

                if     vpoxcon.MediumFormatCapabilities_CreateDynamic in lstCaps \
                   and 'Dynamic' in asVariants:
                    lstDskVariants.append('Dynamic');

                if     vpoxcon.MediumFormatCapabilities_CreateFixed in lstCaps \
                   and 'Fixed' in asVariants:
                    lstDskVariants.append('Fixed');

                if     vpoxcon.MediumFormatCapabilities_CreateSplit2G in lstCaps \
                   and vpoxcon.MediumFormatCapabilities_CreateDynamic in lstCaps \
                   and 'DynamicSplit2G' in asVariants:
                    lstDskVariants.append('DynamicSplit2G');

                if     vpoxcon.MediumFormatCapabilities_CreateSplit2G in lstCaps \
                   and vpoxcon.MediumFormatCapabilities_CreateFixed in lstCaps \
                   and 'FixedSplit2G' in asVariants:
                    lstDskVariants.append('FixedSplit2G');

                if     vpoxcon.MediumFormatCapabilities_TcpNetworking in lstCaps \
    ...
    def convDiskToMediumVariant(self, sDiskVariant):
        ...
        if sDiskVariant == 'Dynamic':
            ...
            tMediumVariant = (vpoxcon.MediumVariant_Standard, vpoxcon.MediumVariant_VmdkSplit2G);
        elif sDiskVariant == 'FixedSplit2G':
            tMediumVariant = (vpoxcon.MediumVariant_Fixed, vpoxcon.MediumVariant_VmdkSplit2G);
    ...
    def getStorageCtrlFromName(self, sStorageCtrl):
        ...
        if sStorageCtrl == 'AHCI':
            ...
            eStorageCtrl = vpoxcon.StorageControllerType_NVMe;
        elif sStorageCtrl == 'VirtIoScsi':
            eStorageCtrl = vpoxcon.StorageControllerType_VirtioSCSI;
    ...
    def getStorageDriverFromEnum(self, eStorageCtrl, fHardDisk):
        ...
        """
        if eStorageCtrl == vpoxcon.StorageControllerType_IntelAhci:
            if fHardDisk:
                return ('ahci', False);
            return ('ahci', True);
        if eStorageCtrl == vpoxcon.StorageControllerType_PIIX4:
            return ('piix3ide', False);
        if eStorageCtrl == vpoxcon.StorageControllerType_LsiLogicSas:
            return ('lsilogicsas', True);
        if eStorageCtrl == vpoxcon.StorageControllerType_LsiLogic:
            return ('lsilogicscsi', True);
        if eStorageCtrl == vpoxcon.StorageControllerType_BusLogic:
            return ('buslogic', True);
        if eStorageCtrl == vpoxcon.StorageControllerType_NVMe:
            return ('nvme', False);
        if eStorageCtrl == vpoxcon.StorageControllerType_VirtioSCSI:
    ...
    def createHd(self, oSession, sDiskFormat, sDiskVariant, iDiffLvl, oHdParent, \
        ...
        if sDiskFormat == "iSCSI" and iDiffLvl == 0:
            ...
            if self.fpApiVer >= 5.0:
                oHd = oSession.oVPox.createMedium(sDiskFormat, sDiskPath, vpoxcon.AccessMode_ReadWrite, \
                                                  vpoxcon.DeviceType_HardDisk);
            else:
                oHd = oSession.oVPox.createHardDisk(sDiskFormat, sDiskPath);
            ...
            if oHd is not None and iDiffLvl == 0 and self.fEncryptDisk:
                try:
                    ...
                    oProgress = vpoxwrappers.ProgressWrapper(oIProgress, self.oVPoxMgr, self, 'Encrypting "%s"' % (sDiskPath,));
    ...
    def testOneCfg(self, sVmName, eStorageController, sHostIoCache, sDiskFormat, # pylint: disable=too-many-arguments,too-many-locals,too-many-statements
        ...
        for iDiffLvl in range(self.cDiffLvls + 1):
            ...
            if oSession is not None:
                ...
                #
                fRc = oSession.setupAudio(vpoxcon.AudioControllerType_AC97, False);
                # Attach HD
                ...
                iDevice = 0;
                if eStorageController in (vpoxcon.StorageControllerType_PIIX3, vpoxcon.StorageControllerType_PIIX4,):
                    ...
                    try:
                        if oSession.fpApiVer >= 4.0:
                            oSession.o.machine.attachDevice(self.controllerTypeToName(eStorageController),
                                                            0, iDevice, vpoxcon.DeviceType_HardDisk, oHd);
                        else:
                            oSession.o.machine.attachDevice(self.controllerTypeToName(eStorageController),
                                                            0, iDevice, vpoxcon.DeviceType_HardDisk, oHd.id);
                    except:
                ...
                if fRc and self.fIoLog:
                    try:
                        oSession.o.machine.setExtraData('VPoxInternal2/EnableDiskIntegrityDriver', '1');
                        ...
                        if fDrvScsi:
                            sCfgmPath = 'VPoxInternal/Devices/%s/0/LUN#%u/AttachedDriver/Config' % (sDrv, iLun);
                        else:
                            sCfgmPath = 'VPoxInternal/Devices/%s/0/LUN#%u/Config' % (sDrv, iLun);
            ...
            if fRc is True:
                ...
                if oSession is not None:
                    try:
                        ...
                        if eStorageController not in (vpoxcon.StorageControllerType_PIIX3, vpoxcon.StorageControllerType_PIIX4,):
