...
/** @file
 * VirtualPox IDisplay implementation, helpers for legacy GAs.
 *
                          ...
                          unsigned cFBInfos)
{
    ...
    for (uScreenId = 0; uScreenId < cFBInfos; uScreenId++)
    {
        ...
        {
            paFBInfos[uScreenId].pHostEvents->fu32Events |= VPOX_VIDEO_INFO_HOST_EVENTS_F_VRDP_RESET;
        }
    }
}
...
void Display::processAdapterData(void *pvVRAM, uint32_t u32VRAMSize)
{
    ...
    else
    {
         ...
         pu8 += u32VRAMSize - VPOX_VIDEO_ADAPTER_INFORMATION_SIZE;

         /// @todo
         uint8_t *pu8End = pu8 + VPOX_VIDEO_ADAPTER_INFORMATION_SIZE;

         VPOXVIDEOINFOHDR *pHdr;

         for (;;)
         {
             pHdr = (VPOXVIDEOINFOHDR *)pu8;
             pu8 += sizeof(VPOXVIDEOINFOHDR);

             if (pu8 >= pu8End)
             {
                 LogRel(("VPoxVideo: Guest adapter information overflow!!!\n"));
                 break;
             }

             if (pHdr->u8Type == VPOX_VIDEO_INFO_TYPE_DISPLAY)
             {
                 if (pHdr->u16Length != sizeof(VPOXVIDEOINFODISPLAY))
                 {
                     LogRel(("VPoxVideo: Guest adapter information %s invalid length %d!!!\n", "DISPLAY", pHdr->u16Length));
                     break;
                 }

                 VPOXVIDEOINFODISPLAY *pDisplay = (VPOXVIDEOINFODISPLAY *)pu8;

                 if (pDisplay->u32Index >= mcMonitors)
                 {
                     LogRel(("VPoxVideo: Guest adapter information invalid display index %d!!!\n", pDisplay->u32Index));
                     break;
                 }
                 ...
                 LogRelFlow(("VPOX_VIDEO_INFO_TYPE_DISPLAY: %d: at 0x%08X, size 0x%08X, info 0x%08X\n", pDisplay->u32Index,
                             pDisplay->u32Offset, pDisplay->u32FramebufferSize, pDisplay->u32InformationSize));
             }
             else if (pHdr->u8Type == VPOX_VIDEO_INFO_TYPE_QUERY_CONF32)
             {
                 if (pHdr->u16Length != sizeof(VPOXVIDEOINFOQUERYCONF32))
                 {
                     LogRel(("VPoxVideo: Guest adapter information %s invalid length %d!!!\n", "CONF32", pHdr->u16Length));
                     break;
                 }

                 VPOXVIDEOINFOQUERYCONF32 *pConf32 = (VPOXVIDEOINFOQUERYCONF32 *)pu8;

                 switch (pConf32->u32Index)
                 {
                     case VPOX_VIDEO_QCI32_MONITOR_COUNT:
                     {
                         pConf32->u32Value = mcMonitors;
                     } break;

                     case VPOX_VIDEO_QCI32_OFFSCREEN_HEAP_SIZE:
                     {
                     ...
                     default:
                         LogRel(("VPoxVideo: CONF32 %d not supported!!! Skipping.\n", pConf32->u32Index));
                 }
             }
             else if (pHdr->u8Type == VPOX_VIDEO_INFO_TYPE_END)
             {
                 if (pHdr->u16Length != 0)
                 {
                     LogRel(("VPoxVideo: Guest adapter information %s invalid length %d!!!\n", "END", pHdr->u16Length));
             ...
             else if (pHdr->u8Type != VPOX_VIDEO_INFO_TYPE_NV_HEAP)
             {
                 /** @todo why is Additions/WINNT/Graphics/Miniport/VPoxVideo. cpp pushing this to us? */
...
void Display::processDisplayData(void *pvVRAM, unsigned uScreenId)
{
    ...
    {
        LogRel(("VPoxVideo: Guest display information invalid display index %d!!!\n", uScreenId));
        return;
    }
    ...
    for (;;)
    {
        ...
        if (pu8 >= pu8End)
        {
            LogRel(("VPoxVideo: Guest display information overflow!!!\n"));
            break;
        }

        if (pHdr->u8Type == VPOX_VIDEO_INFO_TYPE_SCREEN)
        {
            if (pHdr->u16Length != sizeof(VPOXVIDEOINFOSCREEN))
            {
                LogRel(("VPoxVideo: Guest display information %s invalid length %d!!!\n", "SCREEN", pHdr->u16Length));
                break;
            }

            VPOXVIDEOINFOSCREEN *pScreen = (VPOXVIDEOINFOSCREEN *)pu8;
            ...
            LogRelFlow(("VPOX_VIDEO_INFO_TYPE_SCREEN: (%p) %d: at %d,%d, linesize 0x%X, size %dx%d, bpp %d, flags 0x%02X\n",
            ...
            if (uScreenId != VPOX_VIDEO_PRIMARY_SCREEN)
            {
        ...
        else if (pHdr->u8Type == VPOX_VIDEO_INFO_TYPE_HOST_EVENTS)
        {
            if (pHdr->u16Length != sizeof(VPOXVIDEOINFOHOSTEVENTS))
            {
                LogRel(("VPoxVideo: Guest display information %s invalid length %d!!!\n", "HOST_EVENTS", pHdr->u16Length));
                break;
            }

            VPOXVIDEOINFOHOSTEVENTS *pHostEvents = (VPOXVIDEOINFOHOSTEVENTS *)pu8;

            pFBInfo->pHostEvents = pHostEvents;

            LogFlow(("VPOX_VIDEO_INFO_TYPE_HOSTEVENTS: (%p)\n",
                     pHostEvents));
        }
        else if (pHdr->u8Type == VPOX_VIDEO_INFO_TYPE_LINK)
        {
            if (pHdr->u16Length != sizeof(VPOXVIDEOINFOLINK))
            {
                LogRel(("VPoxVideo: Guest adapter information %s invalid length %d!!!\n", "LINK", pHdr->u16Length));
                break;
            }

            VPOXVIDEOINFOLINK *pLink = (VPOXVIDEOINFOLINK *)pu8;
            pu8 += pLink->i32Offset;
        }
        else
        {
