/** $Id: VPoxClient.cpp $ */
/** @file
 * VPoxClient - User specific services, Darwin.
 */
...
static VPOXCLIENTSERVICE    g_aServices[] =
{
...
static int vbclInitLogger(char *pszLogFileName)
{
    ...
    int rc = RTLogCreateEx(&g_pLogger, RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG, "all", "VPOXCLIENT_RELEASE_LOG",
    ...
    /* Explicitly flush the log in case of VPOXCLIENT_RELEASE_LOG=buffered. */
 ...
 */
void VPoxClientVerbose(int iLevel, const char *pszFormat, ...)
{
...
static int vbclStartServices(void)
{
    ...
    if (RT_SUCCESS(rc))
        VPoxClientVerbose(1, "Services start completed.\n");
...
static void vbclStopServices(void)
{
    ...
    VPoxClientVerbose(1, "Stopping services...\n");
    for (iServiceId = 0; iServiceId < RT_ELEMENTS(g_aServices); iServiceId++)
    {
        VPoxClientVerbose(1, "Stopping service: %s\n", g_aServices[iServiceId].pszName);
    ...
    VPoxClientVerbose(1, "Services stop completed\n");
}
