/* $Id: VPoxNetAdp-solaris.c $ */
/** @file
 * VPoxNetAdapter - Network Adapter Driver (Host), Solaris Specific Code.
 */
...
*********************************************************************************************************************************/
#define DEVICE_NAME              "vpoxnet"
/** The module descriptions as seen in 'modinfo'. */
#define DEVICE_DESC_DRV          "VirtualPox NetAdp"
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int VPoxNetAdpSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd);
static int VPoxNetAdpSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd);
static int VPoxNetAdpSolarisQuiesceNotNeeded(dev_info_t *pDip);

/**
 * Streams: module info.
 */
static struct module_info g_VPoxNetAdpSolarisModInfo =
{
    0x0dd,                            /* module id */
    DEVICE_NAME,
    0,                                /* min. packet size */
 ...
 */
static struct qinit g_VPoxNetAdpSolarisReadQ =
{
    NULL,                             /* read */
    gld_rsrv,
    gld_open,
 ...
 */
static struct qinit g_VPoxNetAdpSolarisWriteQ =
{
    gld_wput,
    gld_wsrv,
    NULL,                             /* open */
 ...
 */
static struct streamtab g_VPoxNetAdpSolarisStreamTab =
{
    &g_VPoxNetAdpSolarisReadQ,
    &g_VPoxNetAdpSolarisWriteQ,
    NULL,                           /* muxread init */
    NULL                            /* muxwrite init */
};
 ...
 */
static struct cb_ops g_VPoxNetAdpSolarisCbOps =
{
    nulldev,                        /* cb open */
    nulldev,                        /* cb close */
    nodev,                          /* b strategy */
 ...
 */
static struct dev_ops g_VPoxNetAdpSolarisDevOps =
{
    ...
    nulldev,                        /* identify */
    nulldev,                        /* probe */
    VPoxNetAdpSolarisAttach,
    VPoxNetAdpSolarisDetach,
    nodev,                          /* reset */
    &g_VPoxNetAdpSolarisCbOps,
    (struct bus_ops *)0,
    nodev,                          /* power */
    VPoxNetAdpSolarisQuiesceNotNeeded
};

/**
 * modldrv: export driver specifics to kernel
 */
static struct modldrv g_VPoxNetAdpSolarisDriver =
{
    &mod_driverops,                 /* extern from kernel */
    DEVICE_DESC_DRV " " VPOX_VERSION_STRING "r"  RT_XSTR(VPOX_SVN_REV),
    &g_VPoxNetAdpSolarisDevOps
};

/**
 * modlinkage: export install/remove/info to the kernel
 */
static struct modlinkage g_VPoxNetAdpSolarisModLinkage =
{
    MODREV_1,                       /* loadable module system revision */
    {
        &g_VPoxNetAdpSolarisDriver, /* adapter streams driver framework */
...
/**
 * vpoxnetadp_state_t: per-instance data
 */
typedef struct vpoxnetadp_state_t
{
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int vpoxNetAdpSolarisGenerateMac(PRTMAC pMac);
static int vpoxNetAdpSolarisSetMacAddress(gld_mac_info_t *pMacInfo, unsigned char *pszMacAddr);
static int vpoxNetAdpSolarisSend(gld_mac_info_t *pMacInfo, mblk_t *pMsg);
static int vpoxNetAdpSolarisStub(gld_mac_info_t *pMacInfo);
static int vpoxNetAdpSolarisSetPromisc(gld_mac_info_t *pMacInfo, int fPromisc);
static int vpoxNetAdpSolarisSetMulticast(gld_mac_info_t *pMacInfo, unsigned char *pMulticastAddr, int fMulticast);
static int vpoxNetAdpSolarisGetStats(gld_mac_info_t *pMacInfo, struct gld_stats *pStats);
...
int _init(void)
{
     ...
     */
    modctl_t *pModCtl = mod_getctl(&g_VPoxNetAdpSolarisModLinkage);
    if (pModCtl)
    ...
    if (RT_SUCCESS(rc))
    {
        rc = mod_install(&g_VPoxNetAdpSolarisModLinkage);
        if (!rc)
            return rc;
...
int _fini(void)
{
     ...
     */
    int rc = mod_remove(&g_VPoxNetAdpSolarisModLinkage);
    if (!rc)
        RTR0Term();
...
int _info(struct modinfo *pModInfo)
{
    ...
    int rc = mod_info(&g_VPoxNetAdpSolarisModLinkage, pModInfo);
...
static int VPoxNetAdpSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ":VPoxNetAdpSolarisAttach pDip=%p enmCmd=%d\n", pDip, enmCmd));
    ...
    switch (enmCmd)
    {
        case DDI_ATTACH:
        {
            ...
            if (pMacInfo)
            {
                vpoxnetadp_state_t *pState = RTMemAllocZ(sizeof(vpoxnetadp_state_t));
                if (pState)
                {
                    pState->pDip = pDip;
                     ...
                     */
                    pMacInfo->gldm_reset = vpoxNetAdpSolarisStub;
                    pMacInfo->gldm_start = vpoxNetAdpSolarisStub;
                    pMacInfo->gldm_stop = vpoxNetAdpSolarisStub;
                    pMacInfo->gldm_set_mac_addr = vpoxNetAdpSolarisSetMacAddress;
                    pMacInfo->gldm_set_multicast = vpoxNetAdpSolarisSetMulticast;
                    pMacInfo->gldm_set_promiscuous = vpoxNetAdpSolarisSetPromisc;
                    pMacInfo->gldm_send = vpoxNetAdpSolarisSend;
                    pMacInfo->gldm_intr = NULL;
                    pMacInfo->gldm_get_stats = vpoxNetAdpSolarisGetStats;
                     ...
                     */
                    rc = vpoxNetAdpSolarisGenerateMac(&pState->FactoryMac);
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        if (rc == DDI_SUCCESS)
                        {
                            ...
                            LogRel((DEVICE_NAME ":VPoxNetAdpSolarisAttach failed to register GLD. rc=%d\n", rc));
                    }
                    else
                        LogRel((DEVICE_NAME ":VPoxNetAdpSolarisAttach failed to generate mac address.rc=%d\n"));

                    RTMemFree(pState);
                }
                else
                    LogRel((DEVICE_NAME ":VPoxNetAdpSolarisAttach failed to alloc state.\n"));

                gld_mac_free(pMacInfo);
            }
            else
                LogRel((DEVICE_NAME ":VPoxNetAdpSolarisAttach failed to alloc mac structure.\n"));
            return DDI_FAILURE;
        }
...
static int VPoxNetAdpSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ":VPoxNetAdpSolarisDetach pDip=%p enmCmd=%d\n", pDip, enmCmd));
    ...
    switch (enmCmd)
    {
        case DDI_DETACH:
        {
            ...
            if (pMacInfo)
            {
                vpoxnetadp_state_t *pState = (vpoxnetadp_state_t *)pMacInfo->gldm_private;
                if (pState)
                {
                    ...
                    if (rc == DDI_SUCCESS)
                    {
                        ...
                        LogRel((DEVICE_NAME ":VPoxNetAdpSolarisDetach failed to unregister GLD from MAC layer.rc=%d\n", rc));
                }
                else
                    LogRel((DEVICE_NAME ":VPoxNetAdpSolarisDetach failed to get internal state.\n"));
            }
            else
                LogRel((DEVICE_NAME ":VPoxNetAdpSolarisDetach failed to get driver private GLD data.\n"));
...
static int vpoxNetAdpSolarisGenerateMac(PRTMAC pMac)
{
    ...
    Log((DEVICE_NAME ":VPoxNetAdpSolarisGenerateMac Generated %.*Rhxs\n", sizeof(RTMAC), &pMac));
    return VINF_SUCCESS;
}
...
static int vpoxNetAdpSolarisSetMacAddress(gld_mac_info_t *pMacInfo, unsigned char *pszMacAddr)
{
    ...
    if (pState)
    {
        ...
        Log((DEVICE_NAME ":vpoxNetAdpSolarisSetMacAddress updated MAC %.*Rhxs\n", sizeof(RTMAC), &pState->CurrentMac));
        return GLD_SUCCESS;
    }
    else
        LogRel((DEVICE_NAME ":vpoxNetAdpSolarisSetMacAddress failed to get internal state.\n"));
    return GLD_FAILURE;
}
...
static int vpoxNetAdpSolarisGetStats(gld_mac_info_t *pMacInfo, struct gld_stats *pStats)
{
    ...
    if (pState)
    {
        ...
        LogRel((DEVICE_NAME ":vpoxNetAdpSolarisGetStats failed to get internal state.\n"));
    return GLD_FAILURE;
}
