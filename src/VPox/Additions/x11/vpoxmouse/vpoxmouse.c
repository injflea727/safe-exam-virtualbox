/* $Id: vpoxmouse.c $ */
/** @file
 * VirtualPox X11 Guest Additions, mouse driver for X.Org server 1.5
 */
...
#include <VPox/VMMDev.h> /* for VMMDEV_MOUSE_XXX */
...
static void
VPoxReadInput(InputInfoPtr pInfo)
{
...
static void
VPoxPtrCtrlProc(DeviceIntPtr device, PtrCtrl *ctrl)
{
...
#endif
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 2
                                 miPointerGetMotionEvents, VPoxPtrCtrlProc,
                                 miPointerGetMotionBufferSize()
#elif GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 3
                                 GetMotionHistory, VPoxPtrCtrlProc,
                                 GetMotionHistorySize(), 2 /* Number of axes */

#elif GET_ABI_MAJOR(ABI_XINPUT_VERSION) >= 3
                                 VPoxPtrCtrlProc, GetMotionHistorySize(),
                                 2 /* Number of axes */
#else
...
static int
VPoxProc(DeviceIntPtr device, int what)
{
    InputInfoPtr pInfo;
    int rc, xrc;
...
static int
VPoxProbe(InputInfoPtr pInfo)
{
    int rc = VbglR3Init();
    if (!RT_SUCCESS(rc)) {
        xf86Msg(X_ERROR, "%s: Failed to open the VirtualPox device (error %d)\n",
                pInfo->name, rc);
        return BadMatch;
    }
...
static Bool
VPoxConvert(InputInfoPtr pInfo, int first, int num, int v0, int v1, int v2,
...
static int
VPoxPreInitInfo(InputDriverPtr drv, InputInfoPtr pInfo, int flags)
{
    const char *device;
    int rc;
    RT_NOREF(drv, flags);
    ...
    pInfo->device_control = VPoxProc;
    pInfo->read_input = VPoxReadInput;
    ...
    if (pInfo->fd < 0) {
        xf86Msg(X_ERROR, "Unable to open VirtualPox device \"%s\".\n", device);
        return BadMatch;
    }
...
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 12
static InputInfoPtr
VPoxPreInit(InputDriverPtr drv, IDevPtr dev, int flags)
{
    ...
    pInfo->conf_idev = dev;
    pInfo->conversion_proc = VPoxConvert;
    pInfo->flags = XI86_POINTER_CAPABLE | XI86_SEND_DRAG_EVENTS;

    xf86CollectInputOptions(pInfo, NULL, NULL);
    xf86ProcessCommonOptions(pInfo, pInfo->options);

    if (VPoxPreInitInfo(drv, pInfo, flags) != Success) {
...
_X_EXPORT InputDriverRec VPOXMOUSE = {
    1,
    "vpoxmouse",
    NULL,
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 12
...
static pointer
VPoxPlug(pointer module, pointer options, int *errmaj, int *errmin)
{
    RT_NOREF(options, errmaj, errmin);
    xf86AddInputDriver(&VPOXMOUSE, module, 0);
    xf86Msg(X_CONFIG, "Load address of symbol \"VPOXMOUSE\" is %p\n",
            (void *)&VPOXMOUSE);
    return module;
}
...
_X_EXPORT XF86ModuleData vpoxmouseModuleData =
{
    &VPoxVersionRec,
    VPoxPlug,
    NULL
