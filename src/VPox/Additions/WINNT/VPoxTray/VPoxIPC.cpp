...
} VPOXIPCCONTEXT, *PVPOXIPCCONTEXT;
...
} VPOXIPCSESSION, *PVPOXIPCSESSION;


/*********************************************************************************************************************************
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static VPOXIPCCONTEXT       g_Ctx = { NULL, NIL_RTLOCALIPCSERVER };
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int vpoxIPCSessionStop(PVPOXIPCSESSION pSession);



static int vpoxIPCHandleVPoxTrayRestart(PVPOXIPCSESSION pSession, PVPOXTRAYIPCHEADER pHdr)
{
...
static int vpoxIPCHandleShowBalloonMsg(PVPOXIPCSESSION pSession, PVPOXTRAYIPCHEADER pHdr)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHdr, VERR_INVALID_POINTER);
    AssertReturn(pHdr->uMsgLen > 0, VERR_INVALID_PARAMETER);

    VPOXTRAYIPCMSG_SHOWBALLOONMSG ipcMsg;
...
static int vpoxIPCHandleUserLastInput(PVPOXIPCSESSION pSession, PVPOXTRAYIPCHEADER pHdr)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHdr, VERR_INVALID_POINTER);
 ...
 */
DECLCALLBACK(int) VPoxIPCInit(const PVPOXSERVICEENV pEnv, void **ppInstance)
{
    ...
    PVPOXIPCCONTEXT pCtx = &g_Ctx; /* Only one instance at the moment. */
    AssertPtr(pCtx);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = RTProcQueryUsername(NIL_RTPROCESS,
                                 &szPipeName[sizeof(VPOXTRAY_IPC_PIPE_PREFIX) - 1],
                                 sizeof(szPipeName) - sizeof(VPOXTRAY_IPC_PIPE_PREFIX) + 1,
                                 NULL /*pcbUser*/);
        AssertRC(rc);
        if (RT_SUCCESS(rc))
        {
...
DECLCALLBACK(void) VPoxIPCStop(void *pInstance)
{
    /* Can be NULL if VPoxIPCInit failed. */
    if (!pInstance)
        return;
    ...
    PVPOXIPCCONTEXT pCtx = (PVPOXIPCCONTEXT)pInstance;
    AssertPtr(pCtx);
    ...
    if (RT_SUCCESS(rc))
    {
        PVPOXIPCSESSION pSession;
        RTListForEach(&pCtx->SessionList, pSession, VPOXIPCSESSION, Node)
        {
            int rc2 = vpoxIPCSessionStop(pSession);
            if (RT_FAILURE(rc2))
            {
...
DECLCALLBACK(void) VPoxIPCDestroy(void *pInstance)
{
    AssertPtrReturnVoid(pInstance);
 ...
 */
static DECLCALLBACK(int) vpoxIPCSessionThread(RTTHREAD hThreadSelf, void *pvSession)
{
    RT_NOREF(hThreadSelf);
    PVPOXIPCSESSION pThis = (PVPOXIPCSESSION)pvSession;
    ...
    while (   !ASMAtomicUoReadBool(&pThis->fTerminate)
           && RT_SUCCESS(rc))
    {
        /* The next call will be cancelled via VPoxIPCStop if needed. */
        ...
        else
        {
            ...
            if (RT_SUCCESS(rc))
                fRejected =    ipcHdr.uMagic != VPOXTRAY_IPC_HDR_MAGIC
                ...
                && RT_SUCCESS(rc))
            {
                ...
                {
                    case VPOXTRAYIPCMSGTYPE_RESTART:
                        rc = vpoxIPCHandleVPoxTrayRestart(pThis, &ipcHdr);
                        break;

                    case VPOXTRAYIPCMSGTYPE_SHOWBALLOONMSG:
                        rc = vpoxIPCHandleShowBalloonMsg(pThis, &ipcHdr);
                        break;

                    case VPOXTRAYIPCMSGTYPE_USERLASTINPUT:
                        rc = vpoxIPCHandleUserLastInput(pThis, &ipcHdr);
                        break;
     ...
     */
    PVPOXIPCCONTEXT pCtx = ASMAtomicReadPtrT(&pThis->pCtx, PVPOXIPCCONTEXT);
...
static int vpoxIPCSessionCreate(PVPOXIPCCONTEXT pCtx, RTLOCALIPCSESSION hSession)
{
    ...
    if (RT_SUCCESS(rc))
    {
        PVPOXIPCSESSION pSession = (PVPOXIPCSESSION)RTMemAllocZ(sizeof(VPOXIPCSESSION));
        if (pSession)
        {
            pSession->pCtx      = pCtx;
            ...
            rc = RTThreadCreate(&pSession->hThread, vpoxIPCSessionThread,
 ...
 * requests
 */
DECLCALLBACK(int) VPoxIPCWorker(void *pInstance, bool volatile *pfShutdown)
{
    AssertPtr(pInstance);
    ...
    for (;;)
    {
        ...
        if (fShutdown)
            break;
        rc = vpoxIPCSessionCreate(pCtx, hClientSession);
        if (RT_FAILURE(rc))
        {
