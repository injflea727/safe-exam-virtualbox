...
the_code:
    ...
    ;
    ; VPox/Bochs shutdown request - write "Shutdown" byte by byte to shutdown port.
    ;
    mov cx, 64
    mov dx, VPOX_BIOS_SHUTDOWN_PORT
    mov bx, VPOX_BIOS_OLD_SHUTDOWN_PORT
retry:
    mov al, 'S'
    out dx, al
    mov al, 'h'
    ...
    mov al, 'n'
    out dx, al
    xchg dx, bx                         ; alternate between the new (VPox) and old (Bochs) ports.
    loop retry
