...
static DECLCALLBACK(int) dbgfR3PlugInLoad(PUVM pUVM, const char *pszName, const char *pszMaybeModule, PRTERRINFO pErrInfo)
{
    ...
    else
    {
        ...
        if (RT_FAILURE_NP(rc))
        {
            ...
            if (RT_FAILURE_NP(rc))
            {
                /* 3. The VPOXDBG_PLUG_IN_PATH environment variable. */
                rc2 = RTEnvGetEx(RTENV_DEFAULT, "VPOXDBG_PLUG_IN_PATH", szPath, sizeof(szPath), NULL);
                if (RT_SUCCESS(rc2))
                    rc = RTPathTraverseList(szPath, ';', dbgfR3PlugInLoadCallback, pPlugIn, pErrInfo);
            }
        }
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         * Try initialize it.
         */
        rc = pPlugIn->pfnEntry(DBGFPLUGINOP_INIT, pUVM, VPOX_VERSION);
        if (RT_SUCCESS(rc))
        {
            /*
...
/**
 * Load all plug-ins from the architechture private directory of VPox.
 *
 * @param   pUVM    Pointer to the user-mode VM structure.
 */
