...
/** @file
 * VPox Qt GUI - Declarations of utility functions for handling Darwin Keyboard specific tasks.
 */
...
#define VPOX_WITH_KBD_LEDS_SYNC
//#define VPOX_WITHOUT_KBD_LEDS_SYNC_FILTERING
...
#define K_EX            VPOXKEY_EXTENDED
/** Short hand for a modifier key. */
#define K_MOD           VPOXKEY_MODIFIER
/** Short hand for a lock key. */
#define K_LOCK          VPOXKEY_LOCK
#ifdef USE_HID_FOR_MODIFIERS
...
#define VPOX_BOOL_TO_STR_STATE(x) (x) ? "ON" : "OFF"
/** HID LEDs synchronization data: LED states. */
typedef struct VPoxLedState_t
{
...
typedef struct VPoxKbdState_t
{
    /** Holds the reference to IOKit HID device. */
    IOHIDDeviceRef    pDevice;
    /** Holds the LED states. */
    VPoxLedState_t    LED;
    /** Holds the  pointer to a VPoxHidsState_t instance where VPoxKbdState_t instance is stored. */
...
typedef struct VPoxHidsState_t
{
    /** Holds the IOKit HID manager reference. */
    IOHIDManagerRef     hidManagerRef;
    /** Holds the array which consists of VPoxKbdState_t elements. */
...
{
#ifndef VPOX_WITHOUT_KBD_LEDS_SYNC_FILTERING
    ...
    return fSupported;
#else /* !VPOX_WITH_KBD_LEDS_SYNC_FILTERING */
    return true;
#endif
}
...
static void darwinHidInputCbKeyDown(VPoxKbdState_t *pKbd, uint32_t iKeyCode, VPoxHidsState_t *pHidState)
{
    VPoxKbdEvent_t *pEvent = (VPoxKbdEvent_t *)malloc(sizeof(VPoxKbdEvent_t));
...
static bool darwinKbdCapsEventMatches(VPoxKbdEvent_t *pEvent, bool fCapsLed)
{
...
/** IOKit key press callback helper: take care about key-up event.
  * This code should be executed within a critical section under pHidState->fifoEventQueueLock. */
static void darwinHidInputCbKeyUp(VPoxKbdState_t *pKbd, uint32_t iKeyCode, VPoxHidsState_t *pHidState)
{
    CFIndex         iQueue = 0;
    VPoxKbdEvent_t *pEvent = NULL;
    ...
    {
        VPoxKbdEvent_t *pCachedEvent = (VPoxKbdEvent_t *)CFArrayGetValueAtIndex(pHidState->pFifoEventQueue, i);
...
static void darwinHidInputCallback(void *pData, IOReturn unused, void *unused1, IOHIDValueRef pValueRef)
{
    ...
    if (IOHIDElementGetUsagePage(pElementRef) == kHIDPage_KeyboardOrKeypad)    /* Keyboard or keypad event */
        ...
        {
            VPoxKbdState_t *pKbd = (VPoxKbdState_t *)pData;

            if (pKbd && pKbd->pParentContainer)
            {
                bool             fKeyDown  = (IOHIDValueGetIntegerValue(pValueRef) == 1);
                VPoxHidsState_t *pHidState = (VPoxHidsState_t *)pKbd->pParentContainer;
...
static VPoxKbdEvent_t *darwinCarbonCbFindEvent(VPoxHidsState_t *pHidState)
{
    VPoxKbdEvent_t *pEvent = NULL;

    for (CFIndex i = 0; i < CFArrayGetCount(pHidState->pFifoEventQueue); i++)
    {
        pEvent = (VPoxKbdEvent_t *)CFArrayGetValueAtIndex(pHidState->pFifoEventQueue, i);
...
static CGEventRef darwinCarbonCallback(CGEventTapProxy unused, CGEventType unused1, CGEventRef pEventRef, void *pData)
{
    ...
    VPoxHidsState_t *pHidState = (VPoxHidsState_t *)pData;
        ...
        key == kHIDUsage_KeypadNumLock)
    {
        ...
        VPoxKbdEvent_t *pEvent = darwinCarbonCbFindEvent(pHidState);
        if (pEvent)
        {
            VPoxKbdState_t *pKbd = pEvent->pKbd;

            LogRel2(("CARBON: KBD %d: caps=%s, num=%s. tsKeyDown=%llu, tsKeyUp=%llu [tsDiff=%llu ms]. %d events in queue.\n",
                (int)pKbd->idxPosition, VPOX_BOOL_TO_STR_STATE(fCaps), VPOX_BOOL_TO_STR_STATE(fNum),
            ...
            LogRel2(("CARBON: No KBD to take care when modifier key has been pressed: caps=%s, num=%s (%d events in queue)\n",
                VPOX_BOOL_TO_STR_STATE(fCaps), VPOX_BOOL_TO_STR_STATE(fNum), CFArrayGetCount(pHidState->pFifoEventQueue)));
    }
...
static void darwinUsbHidResyncLeds(VPoxKbdState_t *pKbd)
{
...
static VPoxKbdState_t *darwinUsbHidQueryKbdByLocationId(uint32_t idLocation, VPoxHidsState_t *pHidState)
{
    AssertReturn(pHidState, NULL);

    for (CFIndex i = 0; i < CFArrayGetCount(pHidState->pDeviceCollection); i++)
    {
        VPoxKbdState_t *pKbd = (VPoxKbdState_t *)CFArrayGetValueAtIndex(pHidState->pDeviceCollection, i);
...
static void darwinUsbHidDeviceMatchCb(void *pData, io_iterator_t iter)
{
    ...
    while ((service = IOIteratorNext(iter)))
    {
        ...
        if (ppUsbDeviceInterface)
        {
            ...
            {
                VPoxKbdState_t *pKbd = darwinUsbHidQueryKbdByLocationId((uint32_t)idLocation, pHidState);
...
static int darwinUsbHidSubscribeInterestNotifications(VPoxHidsState_t *pHidState)
{
...
static void darwinUsbHidUnsubscribeInterestNotifications(VPoxHidsState_t *pHidState)
{
    AssertReturnVoid(pHidState);
...
static void darwinHidRemovalCallback(void *pData, IOReturn unused, void *unused1)
{
    ...
    VPoxKbdState_t  *pKbd      = (VPoxKbdState_t  *)pData;                  AssertReturnVoid(pKbd);
    VPoxHidsState_t *pHidState = (VPoxHidsState_t *)pKbd->pParentContainer; AssertReturnVoid(pHidState);
...
static bool darwinIsDeviceInCache(VPoxHidsState_t *pState, IOHIDDeviceRef pDevice)
{
    ...
    {
        VPoxKbdState_t *pKbd = (VPoxKbdState_t *)CFArrayGetValueAtIndex(pState->pDeviceCollection, i);
...
/** Add device to cache. */
static void darwinHidAddDevice(VPoxHidsState_t *pHidState, IOHIDDeviceRef pDevice, bool fApplyLedState)
{
    int rc;
    ...
    if (!darwinIsDeviceInCache(pHidState, pDevice))
    {
        ...
        {
            VPoxKbdState_t *pKbd = (VPoxKbdState_t *)malloc(sizeof(VPoxKbdState_t));
            if (pKbd)
            {
                ...
                if (elementMatchingDict)
                {
                    ...
                    LogRel2(("Saved LEDs for KBD %d (%p): fNumLockOn=%s, fCapsLockOn=%s, fScrollLockOn=%s\n",
                        (int)pKbd->idxPosition, pKbd, VPOX_BOOL_TO_STR_STATE(pKbd->LED.fNumLockOn), VPOX_BOOL_TO_STR_STATE(pKbd->LED.fCapsLockOn),
                        VPOX_BOOL_TO_STR_STATE(pKbd->LED.fScrollLockOn)));
...
static int darwinAddCarbonHandler(VPoxHidsState_t *pHidState)
{
    CFMachPortRef pTapRef;
...
static void darwinRemoveCarbonHandler(VPoxHidsState_t *pHidState)
{
    AssertReturnVoid(pHidState);
...
#ifdef VPOX_WITH_KBD_LEDS_SYNC
    IOReturn         rc;
    VPoxHidsState_t *pHidState;

    pHidState = (VPoxHidsState_t *)malloc(sizeof(VPoxHidsState_t));
    AssertReturn(pHidState, NULL);
...
{
#ifdef VPOX_WITH_KBD_LEDS_SYNC
    VPoxHidsState_t *pHidState = (VPoxHidsState_t *)pState;
    IOReturn         rc, rc2 = 0;
    ...
    if (elementMatchingDict)
    {
        ...
        for (CFIndex i = 0; i < CFArrayGetCount(pHidState->pDeviceCollection); i++)
        {
            ...
            VPoxKbdState_t *pKbd;
            pKbd = (VPoxKbdState_t *)CFArrayGetValueAtIndex(pHidState->pDeviceCollection, i);
            ...
            if (pKbd)
            {
                ...
                LogRel2(("Restored LEDs for KBD %d (%p): fNumLockOn=%s, fCapsLockOn=%s, fScrollLockOn=%s\n",
                     (int)i, pKbd, VPOX_BOOL_TO_STR_STATE(pKbd->LED.fNumLockOn), VPOX_BOOL_TO_STR_STATE(pKbd->LED.fCapsLockOn),
                     VPOX_BOOL_TO_STR_STATE(pKbd->LED.fScrollLockOn)));
...
void DarwinHidDevicesBroadcastLeds(void *pState, bool fNumLockOn, bool fCapsLockOn, bool fScrollLockOn)
{
    ...
    if (elementMatchingDict)
    {
        LogRel2(("Start LEDs broadcast: fNumLockOn=%s, fCapsLockOn=%s, fScrollLockOn=%s\n",
            VPOX_BOOL_TO_STR_STATE(fNumLockOn), VPOX_BOOL_TO_STR_STATE(fCapsLockOn), VPOX_BOOL_TO_STR_STATE(fScrollLockOn)));
