...
#define VPOX_UEFI_UUID_GLOBALS L"{8BE4DF61-93CA-11D2-AA0D-00E098032B8C}"
/** Defines an UEFI dummy UUID, see MSDN docs of the API. */
#define VPOX_UEFI_UUID_DUMMY   L"{00000000-0000-0000-0000-000000000000}"
...
RTDECL(int) RTSystemQueryFirmwareType(PRTSYSFWTYPE penmFirmwareType)
{
    ...
    *penmFirmwareType = RTSYSFWTYPE_INVALID;
    ...
    else if (g_pfnGetFirmwareEnvironmentVariableW)
    {
        ...
        DWORD cbRet = g_pfnGetFirmwareEnvironmentVariableW(L"", VPOX_UEFI_UUID_DUMMY, abWhatever, sizeof(abWhatever));
...
RTDECL(int) RTSystemQueryFirmwareBoolean(RTSYSFWBOOL enmBoolean, bool *pfValue)
{
    *pfValue = false;
    ...
    uint8_t bValue = 0;
    DWORD cbRet = g_pfnGetFirmwareEnvironmentVariableW(pwszName, VPOX_UEFI_UUID_GLOBALS, &bValue, sizeof(bValue));
