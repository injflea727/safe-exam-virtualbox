/* $Id: VPoxGuestR3LibSharedFolders.cpp $ */
/** @file
 * VPoxGuestR3Lib - Ring-3 Support Library for VirtualPox guest additions, shared folders.
 */
...
#include <VPox/log.h>
#include <VPox/shflsvc.h> /** @todo File should be moved to VPox/HostServices/SharedFolderSvc.h */
...
{
    return VbglR3HGCMConnect("VPoxSharedFolders", pidClient);
}
...
{
    VPoxSFWaitForMappingsChanges Msg;
...
VBGLR3DECL(int) VbglR3SharedFolderGetMountPrefix(char **ppszPrefix)
{
    ...
    if (RT_SUCCESS(rc))
    {
        rc = VbglR3GuestPropReadValueAlloc(idClientGuestProp, "/VirtualPox/GuestAdd/SharedFolders/MountPrefix", ppszPrefix);
...
VBGLR3DECL(int) VbglR3SharedFolderGetMountDir(char **ppszDir)
{
    ...
    if (RT_SUCCESS(rc))
    {
        rc = VbglR3GuestPropReadValueAlloc(idClientGuestProp, "/VirtualPox/GuestAdd/SharedFolders/MountDir", ppszDir);
        VbglR3GuestPropDisconnect(idClientGuestProp);
    }
#endif
    return rc;
