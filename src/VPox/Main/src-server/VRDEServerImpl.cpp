...
HRESULT VRDEServer::getVRDEProperties(std::vector<com::Utf8Str> &aProperties)
{
    ...
    {
#ifdef VPOX_WITH_EXTPACK
        VirtualPox *pVirtualPox = mParent->i_getVirtualPox();
...
HRESULT VRDEServer::getAuthLibrary(com::Utf8Str &aLibrary)
{
    ...
    if (aLibrary.isEmpty())
    {
        ...
        HRESULT hrc = mParent->i_getVirtualPox()->COMGETTER(SystemProperties)(systemProperties.asOutParam());
        if (SUCCEEDED(hrc))
        {
            Bstr strlib;
...
HRESULT VRDEServer::getVRDEExtPack(com::Utf8Str &aExtPack)
{
    ...
    if (strExtPack.isNotEmpty())
    {
        ...
        {
#ifdef VPOX_WITH_EXTPACK
            ExtPackManager *pExtPackMgr = mParent->i_getVirtualPox()->i_getExtPackManager();
    ...
    else
    {
        /* Get the global setting. */
        ComPtr<ISystemProperties> systemProperties;
        hrc = mParent->i_getVirtualPox()->COMGETTER(SystemProperties)(systemProperties.asOutParam());
        if (SUCCEEDED(hrc))
        {
            Bstr bstr;
