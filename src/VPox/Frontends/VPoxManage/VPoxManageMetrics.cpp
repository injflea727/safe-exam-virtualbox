/* $Id: VPoxManageMetrics.cpp $ */
/** @file
 * VPoxManage - The 'metrics' command.
 */
...
static HRESULT parseFilterParameters(int argc, char *argv[],
                                     ComPtr<IVirtualPox> aVirtualPox,
                                     ...
                                     ComSafeArrayOut(IUnknown *, outObjects))
{
    ...
    if (argc > 0 && strcmp(argv[0], "*"))
    {
        ...
        {
            ComPtr<IHost> host;
            CHECK_ERROR(aVirtualPox, COMGETTER(Host)(host.asOutParam()));
            retObjects.reset(1);
        ...
        else
        {
            ...
            rc = aVirtualPox->FindMachine(Bstr(argv[0]).raw(),
                                          machine.asOutParam());
            if (SUCCEEDED(rc))
            {
                retObjects.reset(1);
                             ...
                             ComPtr<IPerformanceCollector> performanceCollector)
{
    ...
    rc = parseFilterParameters(argc - 1, &argv[1], aVirtualPox,
                                     ...
                                     ComPtr<IPerformanceCollector> performanceCollector)
{
    ...
    rc = parseFilterParameters(argc - i, &argv[i], aVirtualPox,
                               ComSafeArrayAsOutParam(metrics),
                               ComSafeArrayAsOutParam(objects));
    if (FAILED(rc))
        return RTEXITCODE_FAILURE;
...
RTEXITCODE handleMetrics(HandlerArg *a)
{
    ...
    ComPtr<IPerformanceCollector> performanceCollector;
    CHECK_ERROR2I_RET(a->virtualPox, COMGETTER(PerformanceCollector)(performanceCollector.asOutParam()), RTEXITCODE_FAILURE);

    RTEXITCODE rcExit;
    if (!strcmp(a->argv[0], "list"))
        rcExit = handleMetricsList(a->argc, a->argv, a->virtualPox, performanceCollector);
    else if (!strcmp(a->argv[0], "setup"))
        rcExit = handleMetricsSetup(a->argc, a->argv, a->virtualPox, performanceCollector);
    else if (!strcmp(a->argv[0], "query"))
        rcExit = handleMetricsQuery(a->argc, a->argv, a->virtualPox, performanceCollector);
    else if (!strcmp(a->argv[0], "collect"))
        rcExit = handleMetricsCollect(a->argc, a->argv, a->virtualPox, performanceCollector);
    else if (!strcmp(a->argv[0], "enable"))
        rcExit = handleMetricsEnable(a->argc, a->argv, a->virtualPox, performanceCollector);
    else if (!strcmp(a->argv[0], "disable"))
        rcExit = handleMetricsDisable(a->argc, a->argv, a->virtualPox, performanceCollector);
    else
