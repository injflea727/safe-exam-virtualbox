/* $Id: VPoxGuestR3LibHostChannel.cpp $ */
/** @file
 * VPoxGuestR3Lib - Ring-3 Support Library for VirtualPox guest additions, Host Channel.
 */
...
{
    return VbglR3HGCMConnect("VPoxHostChannel", pidClient);
}
                                        ...
                                        uint32_t u32Flags)
{
    ...
    VPoxHostChannelAttach parms;
    VBGL_HGCM_HDR_INIT(&parms.hdr, u32HGCMClientId, VPOX_HOST_CHANNEL_FN_ATTACH, 3);
...
VBGLR3DECL(void) VbglR3HostChannelDetach(uint32_t u32ChannelHandle,
                                         uint32_t u32HGCMClientId)
{
    VPoxHostChannelDetach parms;
    VBGL_HGCM_HDR_INIT(&parms.hdr, u32HGCMClientId, VPOX_HOST_CHANNEL_FN_DETACH, 1);
...
VBGLR3DECL(int) VbglR3HostChannelSend(uint32_t u32ChannelHandle,
                                      ...
                                      uint32_t cbData)
{
    VPoxHostChannelSend parms;
    VBGL_HGCM_HDR_INIT(&parms.hdr, u32HGCMClientId, VPOX_HOST_CHANNEL_FN_SEND, 2);
    VbglHGCMParmUInt32Set(&parms.handle, u32ChannelHandle);
...
{
    VPoxHostChannelRecv parms;
    VBGL_HGCM_HDR_INIT(&parms.hdr, u32HGCMClientId, VPOX_HOST_CHANNEL_FN_RECV, 4);
    VbglHGCMParmUInt32Set(&parms.handle, u32ChannelHandle);
    VbglHGCMParmPtrSet(&parms.data, pvData, cbData);
...
{
    VPoxHostChannelControl parms;
    VBGL_HGCM_HDR_INIT(&parms.hdr, u32HGCMClientId, VPOX_HOST_CHANNEL_FN_CONTROL, 5);
    VbglHGCMParmUInt32Set(&parms.handle, u32ChannelHandle);
...
{
    VPoxHostChannelEventWait parms;
    VBGL_HGCM_HDR_INIT(&parms.hdr, u32HGCMClientId, VPOX_HOST_CHANNEL_FN_EVENT_WAIT, 4);
    VbglHGCMParmUInt32Set(&parms.handle, 0);
                                             ...
                                             uint32_t u32HGCMClientId)
{
    ...
    VPoxHostChannelEventCancel parms;
    VBGL_HGCM_HDR_INIT(&parms.hdr, u32HGCMClientId, VPOX_HOST_CHANNEL_FN_EVENT_CANCEL, 0);
                                       ...
                                       uint32_t *pu32SizeDataReturned)
{
    ...
    VPoxHostChannelQuery parms;
    VBGL_HGCM_HDR_INIT(&parms.hdr, u32HGCMClientId, VPOX_HOST_CHANNEL_FN_QUERY, 5);
    VbglHGCMParmPtrSet(&parms.name, pszCopy, (uint32_t)cbName);
    VbglHGCMParmUInt32Set(&parms.code, u32Code);
    VbglHGCMParmPtrSet(&parms.parm, pvParm, cbParm);
    VbglHGCMParmPtrSet(&parms.data, pvData, cbData);
