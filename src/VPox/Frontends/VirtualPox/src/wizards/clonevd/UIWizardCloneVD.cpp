...
/** @file
 * VPox Qt GUI - UIWizardCloneVD class implementation.
 */
...
bool UIWizardCloneVD::copyVirtualDisk()
{
    ...
    CMedium comVirtualDisk = comVPox.CreateMedium(comMediumFormat.GetName(), strMediumPath, KAccessMode_ReadWrite, m_enmSourceVirtualDiskDeviceType);
    if (!comVPox.isOk())
    {
        msgCenter().cannotCreateMediumStorage(comVPox, strMediumPath, this);
        return false;
    }
    ...
    /* Make sure we register the medium to VPox: */
