/* $Id: file.cpp $ */
/** @file
 * VirtualPox Windows Guest Shared Folders - File System Driver file routines.
 */
 ...
 *
 * See VPoxEditCoffLib and the Makefile.kmk for the rest of the puzzle.
 *
...
static NTSTATUS vbsfNtReadWorker(PRX_CONTEXT RxContext)
{
    RxCaptureFcb;
    RxCaptureFobx;
    PMRX_VPOX_NETROOT_EXTENSION pNetRootX  = VPoxMRxGetNetRootExtension(capFcb->pNetRoot);
    ...
    if (pReq)
    {
        ...
        for (;;)
        {
             ...
             */
            int vrc = VbglR0SfHostReqReadPgLst(pNetRootX->map.root, pReq, pVPoxFobX->hFile, offFile, cbChunk, cPagesInChunk);
            if (RT_SUCCESS(vrc))
            {
                /*
                 ...
                 */
                pVPoxFobX->fTimestampsImplicitlyUpdated |= VPOX_FOBX_F_INFO_LASTACCESS_TIME;
                ...
                if (!cbLeft || cbActual < cbChunk)
                {
                    ...
                    if (RxContext->LowIoContext.Resource != capFcb->Header.PagingIoResource)
                    {
                        ...
                        if (   offFile < cbFileRdbss
                            ...
                            vbsfNtUpdateFcbSize(RxContext->pFobx->AssociatedFileObject, capFcb, pVPoxFobX, offFile, cbFileRdbss, -1);
                        else if (offFile > cbFileRdbss)
                            vbsfNtQueryAndUpdateFcbSize(pNetRootX, RxContext->pFobx->AssociatedFileObject,
                                                        pVPoxFobX, capFcb, pVPoxFcbX);
                    }
                    break;
                }
...
static VOID vbsfNtReadThreadWorker(VOID *pv)
{
    ...
    Log(("VPOXSF: vbsfNtReadThreadWorker: calling the worker\n"));

    RxContext->IoStatusBlock.Status = vbsfNtReadWorker(RxContext);

    Log(("VPOXSF: vbsfNtReadThreadWorker: Status 0x%08X\n",
 ...
 */
NTSTATUS VPoxMRxRead(IN PRX_CONTEXT RxContext)
{
    NTSTATUS Status;
    ...
    if (!(RxContext->Flags & RX_CONTEXT_FLAG_ASYNC_OPERATION))
    {
        ...
        Log(("VPOXSF: VPoxMRxRead: vbsfNtReadWorker: Status %#08X\n", Status));
    }
    else
    {
        Status = RxDispatchToWorkerThread(VPoxMRxDeviceObject, DelayedWorkQueue, vbsfNtReadThreadWorker, RxContext);

        Log(("VPOXSF: VPoxMRxRead: RxDispatchToWorkerThread: Status 0x%08X\n", Status));
...
static NTSTATUS vbsfNtWriteWorker(PRX_CONTEXT RxContext)
{
    ...
    uint32_t             cPagesLeft = ADDRESS_AND_SIZE_TO_SPAN_PAGES(MmGetMdlVirtualAddress(pBufferMdl), cbLeft);
    uint32_t             cMaxPages  = RT_MIN(cPagesLeft, VBSF_MAX_IO_PAGES);
    VPOXSFWRITEPGLSTREQ *pReq = (VPOXSFWRITEPGLSTREQ *)VbglR0PhysHeapAlloc(RT_UOFFSETOF_DYN(VPOXSFWRITEPGLSTREQ,
                                                                                            PgLst.aPages[cMaxPages]));
    while (!pReq && cMaxPages > 4)
    {
        cMaxPages /= 2;
    ...
    if (pReq)
    {
        ...
        for (;;)
        {
             ...
             * Issue the request and unlock the pages.
             */
            int vrc = VbglR0SfHostReqWritePgLst(pNetRootX->map.root, pReq, pVPoxFobX->hFile, offFile, cbChunk, cPagesInChunk);
            if (RT_SUCCESS(vrc))
            {
                /*
                 * Success, advance position and buffer.
                ...
                if (!cbLeft || cbActual < cbChunk)
                {
                    ...
                    if (cbRet > 0)
                    {
                        if (pVPoxFobX->Info.cbObject < offFile)
                            pVPoxFobX->Info.cbObject = offFile;

                        if (pVPoxFobX->Info.cbAllocated < offFile)
                        {
                            pVPoxFobX->Info.cbAllocated = offFile;
                            pVPoxFobX->nsUpToDate       = 0;
                        }
                    }
                    break;
                }
...
static VOID vbsfNtWriteThreadWorker(VOID *pv)
{
    ...
    Log(("VPOXSF: vbsfNtWriteThreadWorker: calling the worker\n"));

    RxContext->IoStatusBlock.Status = vbsfNtWriteWorker(RxContext);

    Log(("VPOXSF: vbsfNtWriteThreadWorker: Status 0x%08X\n",
         RxContext->IoStatusBlock.Status));

    RxLowIoCompletion(RxContext);
}

NTSTATUS VPoxMRxWrite(IN PRX_CONTEXT RxContext)
{
    NTSTATUS Status;
    ...
    if (!(RxContext->Flags & RX_CONTEXT_FLAG_ASYNC_OPERATION))
    {
        ...
        Log(("VPOXSF: VPoxMRxWrite: vbsfNtWriteWorker: Status %#08X\n", Status));
    }
    else
    {
        Status = RxDispatchToWorkerThread(VPoxMRxDeviceObject, DelayedWorkQueue, vbsfNtWriteThreadWorker, RxContext);

        Log(("VPOXSF: VPoxMRxWrite: RxDispatchToWorkerThread: Status 0x%08X\n", Status));
...
NTSTATUS VPoxMRxLocks(IN PRX_CONTEXT RxContext)
{
    ...
    vrc = VbglR0SfLock(&g_SfClient, &pNetRootExtension->map, pVPoxFobx->hFile,
                       LowIoContext->ParamsFor.Locks.ByteOffset, LowIoContext->ParamsFor.Locks.Length, fu32Lock);

    Status = vbsfNtVPoxStatusToNt(vrc);

    Log(("VPOXSF: MRxLocks: Returned 0x%08X\n", Status));
    return Status;
}

NTSTATUS VPoxMRxCompleteBufferingStateChangeRequest(IN OUT PRX_CONTEXT RxContext, IN OUT PMRX_SRV_OPEN SrvOpen,
                                                    IN PVOID pvContext)
{
    RT_NOREF(RxContext, SrvOpen, pvContext);
    Log(("VPOXSF: MRxCompleteBufferingStateChangeRequest: not implemented\n"));
    return STATUS_NOT_IMPLEMENTED;
}

NTSTATUS VPoxMRxFlush (IN PRX_CONTEXT RxContext)
{
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    vrc = VbglR0SfFlush(&g_SfClient, &pNetRootExtension->map, pVPoxFobx->hFile);

    Status = vbsfNtVPoxStatusToNt(vrc);

    Log(("VPOXSF: MRxFlush: Returned 0x%08X\n", Status));
    return Status;
}
 ...
 * stuck.
 */
ULONG NTAPI VPoxMRxExtendStub(IN OUT struct _RX_CONTEXT * RxContext, IN OUT PLARGE_INTEGER pNewFileSize,
                              OUT PLARGE_INTEGER pNewAllocationSize)
{
     ...
     */
    Log(("VPOXSF: MRxExtendStub: new size = %RX64\n",
