...
static SHFLROOT initWithWritableMapping(RTTEST hTest,
                                        VPOXHGCMSVCFNTABLE *psvcTable,
                                        VPOXHGCMSVCHELPERS *psvcHelpers,
                                        const char *pcszFolderName,
                                        const char *pcszMapping,
                                        bool fCaseSensitive = true)
{
    VPOXHGCMSVCPARM aParms[RT_MAX(SHFL_CPARMS_ADD_MAPPING,
    ...
    AssertReleaseRC(VPoxHGCMSvcLoad(psvcTable));
...
static void unmapAndRemoveMapping(RTTEST hTest, VPOXHGCMSVCFNTABLE *psvcTable,
                                  SHFLROOT root, const char *pcszFolderName)
{
    RT_NOREF1(hTest);
    VPOXHGCMSVCPARM aParms[RT_MAX(SHFL_CPARMS_UNMAP_FOLDER,
...
static int createFile(VPOXHGCMSVCFNTABLE *psvcTable, SHFLROOT Root,
                      const char *pcszFilename, uint32_t fCreateFlags,
                      SHFLHANDLE *pHandle, SHFLCREATERESULT *pResult)
{
    VPOXHGCMSVCPARM aParms[SHFL_CPARMS_CREATE];
...
static int readFile(VPOXHGCMSVCFNTABLE *psvcTable, SHFLROOT Root,
                    SHFLHANDLE hFile, uint64_t offSeek, uint32_t cbRead,
                    uint32_t *pcbRead, void *pvBuf, uint32_t cbBuf)
{
    VPOXHGCMSVCPARM aParms[SHFL_CPARMS_READ];
    VPOXHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int writeFile(VPOXHGCMSVCFNTABLE *psvcTable, SHFLROOT Root,
                     SHFLHANDLE hFile, uint64_t offSeek, uint32_t cbWrite,
                     uint32_t *pcbWritten, const void *pvBuf, uint32_t cbBuf)
{
    VPOXHGCMSVCPARM aParms[SHFL_CPARMS_WRITE];
    VPOXHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int flushFile(VPOXHGCMSVCFNTABLE *psvcTable, SHFLROOT root,
                     SHFLHANDLE handle)
{
    VPOXHGCMSVCPARM aParms[SHFL_CPARMS_FLUSH];
    VPOXHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int sfInformation(VPOXHGCMSVCFNTABLE *psvcTable, SHFLROOT root,
                         SHFLHANDLE handle, uint32_t fFlags, uint32_t cb,
                         SHFLFSOBJINFO *pInfo)
{
    VPOXHGCMSVCPARM aParms[SHFL_CPARMS_INFORMATION];
    VPOXHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int lockFile(VPOXHGCMSVCFNTABLE *psvcTable, SHFLROOT root,
                    SHFLHANDLE handle, int64_t offLock, uint64_t cbLock,
                    uint32_t fFlags)
{
    VPOXHGCMSVCPARM aParms[SHFL_CPARMS_LOCK];
    VPOXHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
