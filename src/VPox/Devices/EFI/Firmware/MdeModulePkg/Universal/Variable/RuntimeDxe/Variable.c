...
static UINT32 VPoxReadNVRAM(UINT8 *pu8Buffer, UINT32 cbBuffer)
{
    UINT32 idxBuffer = 0;
...
DECLINLINE(void) VPoxWriteNVRAMU32Param(UINT32 u32CodeParam, UINT32 u32Param)
{
    ASMOutU32(EFI_PORT_VARIABLE_OP, u32CodeParam);
    ASMOutU32(EFI_PORT_VARIABLE_PARAM, u32Param);
}

static UINT32 VPoxWriteNVRAMByteArrayParam(const UINT8 *pbParam, UINT32 cbParam)
{
    UINT32 idxParam = 0;
...
static void VPoxWriteNVRAMNameParam(const CHAR16 *pwszName)
{
    UINTN i;
...
DECLINLINE(UINT32) VPoxWriteNVRAMGuidParam(const EFI_GUID *pGuid)
{
    ASMOutU32(EFI_PORT_VARIABLE_OP, EFI_VM_VARIABLE_OP_GUID);
    return VPoxWriteNVRAMByteArrayParam((UINT8 *)pGuid, sizeof(EFI_GUID));
}

static UINT32 VPoxWriteNVRAMDoOp(UINT32 u32Operation)
{
    UINT32 u32Rc;
    VPoxLogFlowFuncEnter();
    VPoxLogFlowFuncMarkVar(u32Operation, "%x");
    VPoxWriteNVRAMU32Param(EFI_VM_VARIABLE_OP_START, u32Operation);
    ...
    VPoxLogFlowFuncMarkVar(u32Rc, "%x");
    VPoxLogFlowFuncLeave();
    return u32Rc;
}
#endif
...
Done:
  ...
  return Status;
     ...
     */
    VPoxWriteNVRAMGuidParam(VendorGuid);
    VPoxWriteNVRAMNameParam(VariableName);
    u32Rc = VPoxWriteNVRAMDoOp(EFI_VARIABLE_OP_QUERY);
    if (u32Rc == EFI_VARIABLE_OP_STATUS_OK)
    {
        ...
        VPoxLogFlowFuncMarkVar(*DataSize, "%d");
        VPoxLogFlowFuncMarkVar(VarLen, "%d");
        if (   VarLen <= *DataSize
            && Data)
        {
            /*
  ...
  )
{
  ...
  return Status;
    ...
    {
        VPoxLogFlowFuncLeaveRC(EFI_INVALID_PARAMETER);
        return EFI_INVALID_PARAMETER;
    }
     ...
     */
    if (!VariableName[0])
        u32Rc = VPoxWriteNVRAMDoOp(EFI_VARIABLE_OP_QUERY_REWIND);
    else
    {
        VPoxWriteNVRAMGuidParam(VendorGuid);
        VPoxWriteNVRAMNameParam(VariableName);
    ...
    if (u32Rc == EFI_VARIABLE_OP_STATUS_OK)
        u32Rc = VPoxWriteNVRAMDoOp(EFI_VARIABLE_OP_QUERY_NEXT);
    ...
    if (u32Rc == EFI_VARIABLE_OP_STATUS_OK)
    {
        ...
        if ((cwcName + 1) * 2 <= *VariableNameSize) /* ASSUMES byte size is specified */
        {
             ...
             */
            ASMOutU32(EFI_PORT_VARIABLE_OP, EFI_VM_VARIABLE_OP_GUID);
            VPoxReadNVRAM((UINT8 *)VendorGuid, sizeof(EFI_GUID));
...
Done:
  ...
  return Status;
    ...
    UINT32 u32Rc;
    VPoxLogFlowFuncEnter();
    VPoxLogFlowFuncMarkVar(VendorGuid, "%g");
    VPoxLogFlowFuncMarkVar(VariableName, "%s");
    VPoxLogFlowFuncMarkVar(DataSize, "%d");
    /* set guid */
    VPoxWriteNVRAMGuidParam(VendorGuid);
    /* set name */
    VPoxWriteNVRAMNameParam(VariableName);
    /* set attribute */
    VPoxWriteNVRAMU32Param(EFI_VM_VARIABLE_OP_ATTRIBUTE, Attributes);
    /* set value length */
    VPoxWriteNVRAMU32Param(EFI_VM_VARIABLE_OP_VALUE_LENGTH, (UINT32)DataSize);
    /* fill value bytes */
    ASMOutU32(EFI_PORT_VARIABLE_OP, EFI_VM_VARIABLE_OP_VALUE);
    VPoxWriteNVRAMByteArrayParam(Data, (UINT32)DataSize);
    /* start fetch operation */
    u32Rc = VPoxWriteNVRAMDoOp(EFI_VARIABLE_OP_ADD);
    /* process errors */
    VPoxLogFlowFuncLeave();
    switch (u32Rc)
    {
