/* $Id: VPoxGL.c $ */
/** @file
 * VirtualPox Windows Guest Mesa3D - OpenGL driver.
 */
...
{
    struct VPOXWDDMDLLPROC aDrvProcs[] =
    {
    ...
    HMODULE hmod = VPoxWddmLoadSystemDll(g_pszSvgaDll);
    if (hmod)
    {
        VPoxWddmLoadAdresses(hmod, aDrvProcs);
    }
    return hmod;
}
...
static NTSTATUS vpoxKmtPresent(D3DKMT_HANDLE hContext, HWND hwnd, D3DKMT_HANDLE hSource, LONG lWidth, LONG lHeight)
{
    RECT r;
    r.left  = 0;
    r.top    = 0;
...
NTSTATUS vpoxKmtOpenSharedSurface(D3DKMT_HANDLE hDevice, D3DKMT_HANDLE hSharedSurface, struct stw_shared_surface *pSurf)
{
    D3DKMTFUNCTIONS const *d3dkmt = D3DKMTFunctions();
    ...
    if (Status == STATUS_SUCCESS)
    {
        ...
        if (Status == STATUS_SUCCESS)
        {
            ...
            if (Status == STATUS_SUCCESS)
            {
                Assert(OpenAllocationInfoData.PrivateDriverDataSize == sizeof(VPOXWDDM_ALLOCINFO));
                VPOXWDDM_ALLOCINFO *pVPoxAllocInfo = (VPOXWDDM_ALLOCINFO *)OpenAllocationInfoData.pPrivateDriverData;
                pSurf->hResource = OpenResourceData.hResource;
                pSurf->hSurface = OpenAllocationInfoData.hAllocation;
                pSurf->u32Sid = pVPoxAllocInfo->hostID;
            }
        }
...
NTSTATUS vpoxKmtCloseSharedSurface(D3DKMT_HANDLE hDevice, struct stw_shared_surface *pSurf)
{
    D3DKMTFUNCTIONS const *d3dkmt = D3DKMTFunctions();
             ...
             HDC hDC)
{
    ...
    if (pEnv)
    {
        ...
        vpoxKmtPresent(hContext, hwnd, hSource, res->width0, res->height0);
    }
}
                         ...
                         HANDLE hSharedSurface)
{
    ...
    if (pEnv)
    {
        ...
        if (surface)
        {
            ...
            NTSTATUS Status = vpoxKmtOpenSharedSurface(hDevice, (D3DKMT_HANDLE)hSharedSurface, surface);
            if (Status != STATUS_SUCCESS)
            {
                free(surface);
                surface = NULL;
                         ...
                         struct stw_shared_surface *surface)
{
    ...
    {
        D3DKMT_HANDLE hDevice = GaDrvEnvKmtDeviceHandle(pEnv);
        vpoxKmtCloseSharedSurface(hDevice, surface);
    }
    free(surface);
}
