/* $Id: COMDefs.h $ */
/** @file
 * VPox Qt GUI - Various COM definitions and COM wrapper class declarations.
 *
 ...
 * COM/XPCOM Abstraction Layer library (the VPoxCOM glue library defined in
 * include/VPox/com and implemented in src/VPox/Main/glue).
 *
 * ...
 *
...
/* Both VPox/com/assert.h and qglobal.h contain a definition of ASSERT.
...
public:
    ...
    COMErrorInfo(const CVirtualPoxErrorInfo &info)
    {
        init(info);
    }
...
private:
    void init(const CVirtualPoxErrorInfo &info);
...
#ifdef VPOX_WITH_LESS_VIRTUALPOX_INCLUDING
