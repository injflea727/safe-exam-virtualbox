    ...
    for (;;)
    {
        ...
        else
        {
            ...
            else if (VM_FF_TEST_AND_CLEAR(pVM, VM_FF_RESET))
            {
                 ...
                 */
                rc = VPOXSTRICTRC_VAL(VMR3ResetFF(pVM));
 ...
 *
 * @return VPox status code. Failure on invalid CFGM data.
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3SetHaltMethodCallback(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    PUVM      pUVM = pVM->pUVM;
    int       rc   = VINF_SUCCESS;
 ...
 *
 * @returns VPox error status (never informational statuses).
 * @param   pVM                 The cross context VM structure.
