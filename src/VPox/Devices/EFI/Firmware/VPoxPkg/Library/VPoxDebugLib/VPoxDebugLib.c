/* $Id: VPoxDebugLib.c $ */
/** @file
 * VPoxDebugLib.c - Debug logging and assertions support routines using DevEFI.
 */
...
DebugPrint(IN UINTN ErrorLevel, IN CONST CHAR8 *Format, ...)
{
    ...
    VPoxPrintString("dbg/");
    VPoxPrintHex(ErrorLevel, sizeof(ErrorLevel));
    VPoxPrintChar(' ');
 ...
 */
VOID EFIAPI
VPoxLogWorker(const char *pszFormat, ...)
{
    CHAR8       szBuf[384];
    VA_LIST     va;
    BOOLEAN     InterruptState;
 ...
 */
static void
VPoxPanicMsgString(const char *pszString)
{
    char ch;
 ...
 */
static void
VPoxPanicMsgDecimalU32(uint32_t uValue)
{
    char     szTmp[32];
...
DebugAssert(IN CONST CHAR8 *FileName, IN UINTN LineNumber, IN CONST CHAR8 *Description)
{
    ...
    VPoxPanicMsgString("EFI Assertion failed!"
                       "\nFile:  ");
    VPoxPanicMsgString(FileName ? FileName : "<NULL>");
    VPoxPanicMsgString("\nLine:  ");
    VPoxPanicMsgDecimalU32((uint32_t)LineNumber);
    VPoxPanicMsgString("\nDescription: ");
    VPoxPanicMsgString(Description ? Description : "<NULL>");
