...
/** @file
 * VirtualPox Main - Guest session handling.
 */
...
#include "GuestImpl.h"
#ifndef VPOX_WITH_GUEST_CONTROL
# error "VPOX_WITH_GUEST_CONTROL must defined in this file"
#endif
...
public:
    ...
    STDMETHOD(HandleEvent)(VPoxEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case VPoxEventType_OnGuestSessionStateChanged:
            {
...
VPOX_LISTENER_DECLARE(GuestSessionListenerImpl)
                       ...
                       const GuestCredentials &guestCreds)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (SUCCEEDED(hr))
        {
            try
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    eventTypes.push_back(VPoxEventType_OnGuestSessionStateChanged);
        ...
        rc = Global::vpoxStatusCodeFromCOM(hr);
    }
...
HRESULT GuestSession::getEnvironmentChanges(std::vector<com::Utf8Str> &aEnvironmentChanges)
{
    ...
    return Global::vpoxStatusCodeToCOM(vrc);
}
...
HRESULT GuestSession::setEnvironmentChanges(const std::vector<com::Utf8Str> &aEnvironmentChanges)
{
    ...
    return setErrorBoth(Global::vpoxStatusCodeToCOM(vrc), vrc, tr("Failed to apply '%s', index %zu (%Rrc)"),
...
HRESULT GuestSession::getEnvironmentBase(std::vector<com::Utf8Str> &aEnvironmentBase)
{
    ...
    if (mData.mpBaseEnvironment)
    {
        ...
        hrc = Global::vpoxStatusCodeToCOM(vrc);
    }
    else if (mData.mProtocolVersion < 99999)
        hrc = setError(VPOX_E_NOT_SUPPORTED, tr("The base environment feature is not supported by the guest additions"));
    else
        hrc = setError(VPOX_E_INVALID_OBJECT_STATE, tr("The base environment has not yet been reported by the guest"));
...
HRESULT GuestSession::getUserHome(com::Utf8Str &aUserHome)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            case VERR_GSTCTL_GUEST_ERROR:
            {
                switch (rcGuest)
                {
                    case VERR_NOT_SUPPORTED:
                        hr = setErrorBoth(VPOX_E_IPRT_ERROR, rcGuest,
            ...
            default:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Getting the user's home path failed: %Rrc"), vrc);
                break;
        }
    }
...
HRESULT GuestSession::getUserDocuments(com::Utf8Str &aUserDocuments)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hr = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Getting the user's documents path failed: %Rrc"), vrc);
                break;
        }
    }
...
int GuestSession::i_closeSession(uint32_t uFlags, uint32_t uTimeoutMS, int *prcGuest)
{
    ...
    vrc = i_sendMessage(HOST_MSG_SESSION_CLOSE, i, paParms, VPOX_GUESTCTRL_DST_BOTH);
    if (RT_SUCCESS(vrc))
                                    ...
                                    uint32_t uFlags, int *prcGuest)
{
    ...
    procInfo.mExecutable = Utf8Str(VPOXSERVICE_TOOL_MKDIR);
 ...
 *
 * @returns VPox status code, or VERR_NOT_A_DIRECTORY if the file system object exists but is not a directory.
 ...
 *
 * @return VPox status code. VERR_NOT_FOUND if the directory is not registered (anymore).
                                 ...
                                 int *prcGuest)
{
    ...
    try
    {
        procInfo.mExecutable = Utf8Str(VPOXSERVICE_TOOL_MKTEMP);
        procInfo.mArguments.push_back(procInfo.mExecutable); /* Set argv0. */
                                  ...
                                  ComObjPtr<GuestDirectory> &pDirectory, int *prcGuest)
{
    ...
    if (FAILED(hr))
        return Global::vpoxStatusCodeFromCOM(hr);
    ...
    else
    {
        ...
        try
        {
            ...
            /** @todo Fire off a VPoxEventType_OnGuestDirectoryRegistered event? */
        }
        catch (std::bad_alloc &)
        {
            vrc = VERR_NO_MEMORY;
 ...
 *
 * @return VPox status code. VERR_NOT_FOUND if no corresponding object was found.
 * @param  pCtxCb               Host callback context.
 * @param  pSvcCb               Service callback data.
 */
int GuestSession::i_dispatchToObject(PVPOXGUESTCTRLHOSTCBCTX pCtxCb, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb)
{
    ...
    const uint32_t idObject = VPOX_GUESTCTRL_CONTEXTID_GET_OBJECT(pCtxCb->uContextID);
...
int GuestSession::i_dispatchToThis(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCb)
{
 ...
 *
 * @return VPox status code. VERR_NOT_FOUND if the file is not registered (anymore).
...
int GuestSession::i_fileRemove(const Utf8Str &strPath, int *prcGuest)
{
    ...
    procInfo.mFlags      = ProcessCreateFlag_WaitForStdOut;
    procInfo.mExecutable = Utf8Str(VPOXSERVICE_TOOL_RM);
...
int GuestSession::i_fsQueryInfo(const Utf8Str &strPath, bool fFollowSymlinks, GuestFsObjData &objData, int *prcGuest)
{
    ...
    try
    {
        procInfo.mExecutable = Utf8Str(VPOXSERVICE_TOOL_STAT);
        procInfo.mArguments.push_back(procInfo.mExecutable); /* Set argv0. */
        procInfo.mArguments.push_back(Utf8Str("--machinereadable"));
        if (fFollowSymlinks)
...
/** No locking! */
int GuestSession::i_onSessionStatusChange(PVPOXGUESTCTRLHOSTCBCTX pCbCtx, PVPOXGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
...
PathStyle_T GuestSession::i_getPathStyle(void)
{
    ...
    VPOXOSTYPE enmOsType = mParent->i_getGuestOSType();
    if (enmOsType < VPOXOSTYPE_DOS)
    {
        LogFlowFunc(("returns PathStyle_Unknown\n"));
        enmPathStyle = PathStyle_Unknown;
    }
    else if (enmOsType < VPOXOSTYPE_Linux)
    {
...
int GuestSession::i_startSession(int *prcGuest)
{
    ...
    vrc = i_sendMessage(HOST_MSG_SESSION_CREATE, i, paParms, VPOX_GUESTCTRL_DST_ROOT_SVC);
    if (RT_SUCCESS(vrc))
    {
...
int GuestSession::i_objectRegister(GuestObject *pObject, SESSIONOBJECTTYPE enmType, uint32_t *pidObject)
{
     ...
     */
    uint32_t idObject = RTRandU32Ex(1, VPOX_GUESTCTRL_MAX_OBJECTS - 2);
    AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
    if (!ASMBitTestAndSet(&mData.bmObjectIds[0], idObject))
    { /* likely */ }
    else if (mData.mObjects.size() < VPOX_GUESTCTRL_MAX_OBJECTS - 2 /* First and last are not used */)
    {
        /* Forward search. */
        int iHit = ASMBitNextClear(&mData.bmObjectIds[0], VPOX_GUESTCTRL_MAX_OBJECTS, idObject);
        if (iHit < 0)
            iHit = ASMBitFirstClear(&mData.bmObjectIds[0], VPOX_GUESTCTRL_MAX_OBJECTS);
 ...
 *
 * @return VPox status code. VERR_NOT_FOUND if the process is not registered (anymore).
...
int GuestSession::i_sendMessage(uint32_t uMessage, uint32_t uParms, PVPOXHGCMSVCPARM paParms,
                                uint64_t fDst /*= VPOX_GUESTCTRL_DST_SESSION*/)
{
    LogFlowThisFuncEnter();

#ifndef VPOX_GUESTCTRL_TEST_CASE
    ...
    Assert(fDst == VPOX_GUESTCTRL_DST_SESSION || fDst == VPOX_GUESTCTRL_DST_ROOT_SVC || fDst == VPOX_GUESTCTRL_DST_BOTH);
    Assert(paParms[0].type == VPOX_HGCM_SVC_PARM_32BIT);
    paParms[0].type = VPOX_HGCM_SVC_PARM_64BIT;
...
int GuestSession::i_setSessionStatus(GuestSessionStatus_T sessionStatus, int sessionRc)
{
    ...
    if (mData.mStatus != sessionStatus)
    {
        ...
        ComAssertComRC(hr);
        int rc2 = errorInfo->initEx(VPOX_E_IPRT_ERROR, sessionRc,
...
int GuestSession::i_determineProtocolVersion(void)
{
     ...
     * ASSUMING that VPoxService and VPoxDrv are at the same version.
     */
    ...
    if (uGaVersion >= VPOX_FULL_VERSION_MAKE(4,3,0))
        mData.mProtocolVersion = 2; /* Guest control 2.0. */

    LogFlowThisFunc(("uGaVersion=%u.%u.%u => mProtocolVersion=%u\n",
                     VPOX_FULL_VERSION_GET_MAJOR(uGaVersion), VPOX_FULL_VERSION_GET_MINOR(uGaVersion),
                     VPOX_FULL_VERSION_GET_BUILD(uGaVersion), mData.mProtocolVersion));
...
HRESULT GuestSession::close()
{
    ...
    if (RT_FAILURE(vrc))
    {
        ...
        return setError(VPOX_E_IPRT_ERROR, tr("Closing guest session \"%s\" failed with %Rrc"),
        ...
        else
        {
            switch (vrc)
            {
                case VERR_INVALID_PARAMETER:
                    hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Guest directory creation failed: Invalid parameters given"));
                    break;

                case VERR_BROKEN_PIPE:
                    hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Guest directory creation failed: Unexpectedly aborted"));
                    break;

                default:
                    hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Guest directory creation failed: %Rrc"), vrc);
                    break;
            }
        }
    }
                                          ...
                                          BOOL aSecure, com::Utf8Str &aDirectory)
{
    ...
    if (!RT_SUCCESS(vrc))
    {
        switch (vrc)
        {
            ...
            default:
               hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Temporary guest directory creation \"%s\" with template \"%s\" failed: %Rrc"),
...
HRESULT GuestSession::directoryExists(const com::Utf8Str &aPath, BOOL aFollowSymlinks, BOOL *aExists)
{
    ...
    else
    {
        switch (vrc)
        {
            ...
            default:
               hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Querying directory existence \"%s\" failed: %Rrc"),
                                  aPath.c_str(), vrc);
               break;
        }
    }
                                    ...
                                    const std::vector<DirectoryOpenFlag_T> &aFlags, ComPtr<IGuestDirectory> &aDirectory)
{
    ...
    else
    {
        ...
        {
            case VERR_INVALID_PARAMETER:
               hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Opening guest directory \"%s\" failed; invalid parameters given"),
                                  aPath.c_str());
               break;
            ...
            default:
               hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Opening guest directory \"%s\" failed: %Rrc"), aPath.c_str(), vrc);
               break;
        }
    }
...
HRESULT GuestSession::directoryRemove(const com::Utf8Str &aPath)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Removing guest directory \"%s\" failed: %Rrc"), aPath.c_str(), vrc);
                break;
        }
    }
                                               ...
                                               ComPtr<IProgress> &aProgress)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Recursively removing guest directory \"%s\" failed: %Rrc"),
                                   aPath.c_str(), vrc);
                break;
        }
    }
...
HRESULT GuestSession::fileExists(const com::Utf8Str &aPath, BOOL aFollowSymlinks, BOOL *aExists)
{
    ...
    *aExists = FALSE;
    ...
    switch (vrc)
    {
        ...
        default:
            hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Querying guest file information for \"%s\" failed: %Rrc"),
                               aPath.c_str(), vrc);
            break;
    }
                                 ...
                                 const std::vector<FileOpenExFlag_T> &aFlags, ComPtr<IGuestFile> &aFile)
{
    ...
    else
    {
        switch (vrc)
        {
            ...
            default:
                hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Opening guest file \"%s\" failed: %Rrc"), aPath.c_str(), vrc);
                break;
        }
    }
...
HRESULT GuestSession::fileQuerySize(const com::Utf8Str &aPath, BOOL aFollowSymlinks, LONG64 *aSize)
{
    ...
    else
    {
        if (GuestProcess::i_isGuestError(vrc))
            ...
            hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Querying guest file size of \"%s\" failed: %Rrc"),
                               vrc, aPath.c_str());
    }
...
HRESULT GuestSession::fsObjRemove(const com::Utf8Str &aPath)
{
    ...
    if (RT_FAILURE(vrc))
    {
        if (GuestProcess::i_isGuestError(vrc))
            ...
            hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Removing guest file \"%s\" failed: %Rrc"), aPath.c_str(), vrc);
    }
                                  ...
                                  const std::vector<FsObjRenameFlag_T> &aFlags)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Renaming guest path \"%s\" failed: %Rrc"),
                                   aSource.c_str(), vrc);
                break;
        }
    }
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                hr = setErrorVrc(vrc, tr("Maximum number of concurrent guest processes per session (%u) reached"),
                                 VPOX_GUESTCTRL_MAX_OBJECTS);
            else
                hr = setErrorVrc(vrc, tr("Failed to create guest process object: %Rrc"), vrc);
        }
        else
            hr = setErrorBoth(vrc == VERR_ENV_INVALID_VAR_NAME ? E_INVALIDARG : Global::vpoxStatusCodeToCOM(vrc), vrc,
