 ...
 * VPoxManage setextradata vmname VPoxInternal/PDM/Devices/playground/Path .../obj/VPoxPlaygroundDevice/VPoxPlaygroundDevice
 * VPoxManage setextradata vmname VPoxInternal/Devices/playground/0/Config/Whatever1 0
 */
 ...
 */
typedef struct VPOXPLAYGROUNDDEVICEFUNCTION
{
    /** The function number. */
    uint8_t         iFun;
...
} VPOXPLAYGROUNDDEVICEFUNCTION;
/** Pointer to a PCI function of the playground device. */
typedef VPOXPLAYGROUNDDEVICEFUNCTION *PVPOXPLAYGROUNDDEVICEFUNCTION;

/**
 * Playground device instance data.
 */
typedef struct VPOXPLAYGROUNDDEVICE
{
    /** PCI device functions. */
    VPOXPLAYGROUNDDEVICEFUNCTION aPciFuns[8];
} VPOXPLAYGROUNDDEVICE;
/** Pointer to the instance data of a playground device instance. */
typedef VPOXPLAYGROUNDDEVICE *PVPOXPLAYGROUNDDEVICE;
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) devPlaygroundMMIORead(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void *pv, unsigned cb)
{
    PVPOXPLAYGROUNDDEVICEFUNCTION pFun = (PVPOXPLAYGROUNDDEVICEFUNCTION)pvUser;
    NOREF(pDevIns);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) devPlaygroundMMIOWrite(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void const *pv, unsigned cb)
{
    PVPOXPLAYGROUNDDEVICEFUNCTION pFun = (PVPOXPLAYGROUNDDEVICEFUNCTION)pvUser;
    NOREF(pDevIns);
...
{
    PVPOXPLAYGROUNDDEVICE pThis = PDMDEVINS_2_DATA(pDevIns, PVPOXPLAYGROUNDDEVICE);
...
static DECLCALLBACK(int) devPlaygroundConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    for (uint32_t iPciFun = 0; iPciFun < uNumFunctions; iPciFun++)
    {
        ...
        PVPOXPLAYGROUNDDEVICEFUNCTION pFun    = &pThis->aPciFuns[iPciFun];
...
static const PDMDEVREG g_DevicePlayground =
{
    ...
    /* .cMaxInstances = */          1,
    /* .uSharedVersion = */         42,
    /* .cbInstanceShared = */       sizeof(VPOXPLAYGROUNDDEVICE),
    /* .cbInstanceCC = */           0,
    /* .cbInstanceRC = */           0,
    /* .cMaxPciDevices = */         8,
    /* .cMaxMsixVectors = */        0,
    /* .pszDescription = */         "VPox Playground Device.",
#if defined(IN_RING3)
    /* .pszRCMod = */               "",
    /* .pszR0Mod = */               "",
...
{
    LogFlow(("VPoxPlaygroundDevice::VPoxDevicesRegister: u32Version=%#x pCallbacks->u32Version=%#x\n", u32Version, pCallbacks->u32Version));
