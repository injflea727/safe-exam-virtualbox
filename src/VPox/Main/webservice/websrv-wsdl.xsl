...
<!--
    ...
    websrv-wsdl.xsl:
        XSLT stylesheet that generates vpoxweb.wsdl from
        VirtualPox.xidl. This WSDL file represents our
        web service API..
...
<xsl:template name="emitConvertedType">
  ...
  <xsl:choose>
    <xsl:when test="$type=$G_typeIsGlobalRequestElementMarker"><xsl:value-of select="concat('vpox:', $ifname, $G_classSeparator, $methodname, $G_requestMessageElementSuffix)" /></xsl:when>
    <xsl:when test="$type=$G_typeIsGlobalResponseElementMarker"><xsl:value-of select="concat('vpox:', $ifname, $G_classSeparator, $methodname, $G_responseMessageElementSuffix)" /></xsl:when>
    ...
    <xsl:when test="count(key('G_keyEnumsByName', $type)) > 0"><xsl:value-of select="concat('vpox:', $type)" /></xsl:when>
    ...
    <xsl:when test="count(key('G_keyInterfacesByName', $type)) > 0">
      ...
      <xsl:choose>
        <xsl:when test="$wsmap='struct'"><xsl:value-of select="concat('vpox:', $type)" /></xsl:when>
  ...
  <wsdl:operation>
    ...
    <wsdl:input>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:attribute name="message">vpox:<xsl:copy-of select="$_ifname" /><xsl:value-of select="$G_classSeparator" /><xsl:value-of select="$_methodname" /><xsl:copy-of select="$G_methodRequest" /></xsl:attribute>
        </xsl:otherwise>
      </xsl:choose>
    </wsdl:input>
    <xsl:choose>
      <xsl:when test="$_resulttype">
        <wsdl:output>
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:attribute name="message">vpox:<xsl:copy-of select="$_ifname" /><xsl:value-of select="$G_classSeparator" /><xsl:value-of select="$_methodname" /><xsl:copy-of select="$G_methodResponse" /></xsl:attribute>
            </xsl:otherwise>
          </xsl:choose>
        </wsdl:output>
      </xsl:when>
    ...
    <xsl:choose>
      <xsl:when test="not($_fSoap)">
        <wsdl:fault name="InvalidObjectFault" message="vpox:InvalidObjectFaultMsg" />
        <wsdl:fault name="RuntimeFault" message="vpox:RuntimeFaultMsg" />
      </xsl:when>
      <xsl:otherwise>
...
<xsl:template match="/idl">
  ...
  Generator: src/VPox/Main/webservice/websrv-wsdl.xsl
</xsl:comment>
...
<xsl:template match="library">
  ...
  ******************************************************
    ...
    <wsdl:message name="InvalidObjectFaultMsg">
      <wsdl:part name="fault" element="vpox:InvalidObjectFault" />
    </wsdl:message>
    <wsdl:message name="RuntimeFaultMsg">
      <wsdl:part name="fault" element="vpox:RuntimeFault" />
    </wsdl:message>
  ...
  ******************************************************
    ...
    <wsdl:portType>
      <xsl:attribute name="name"><xsl:copy-of select="'vpox'" /><xsl:value-of select="$G_portTypeSuffix" /></xsl:attribute>
    ...
    <wsdl:binding>
      <xsl:attribute name="name"><xsl:value-of select="concat('vpox', $G_bindingSuffix)" /></xsl:attribute>
      <xsl:attribute name="type"><xsl:value-of select="concat('vpox:vpox', $G_portTypeSuffix)" /></xsl:attribute>
