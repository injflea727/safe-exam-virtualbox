/* $Id: VPoxSF-Utils.cpp $ */
/** @file
 * VPoxSF - Darwin Shared Folders, Utility Functions.
 */
 ...
 * @param pParent   Parent vnode object (NULL for VPoxVFS root vnode)
 * @param fIsRoot   Flag that indicates if created vnode object is
 *                  VPoxVFS root vnode (TRUE for VPoxVFS root vnode, FALSE
 ...
 */
int
vpoxvfs_create_vnode_internal(struct mount *mp, enum vtype type, vnode_t pParent, int fIsRoot, PSHFLSTRING Path, vnode_t *ret)
{
    int     rc;
    vnode_t vnode;
    ...
    pMount = (vpoxvfs_mount_t *)vfs_fsprivate(mp);
    AssertReturn(pMount, EINVAL);
    AssertReturn(pMount->pLockGroup, EINVAL);

    AssertReturn(Path, EINVAL);

    pVnodeData = (vpoxvfs_vnode_t *)RTMemAllocZ(sizeof(vpoxvfs_vnode_t));
    ...
    if (pVnodeData->pLockAttr)
    {
        ...
        if (pVnodeData->pLock)
        {
            ...
            vnode_params.vnfs_vops       = g_papfnVPoxVFSVnodeDirOpsVector;
 ...
 */
int
vpoxvfs_guest_path_to_char_path_internal(mount_t mp, char *pszGuestPath, int cbGuestPath, char **pszHostPath, int *cbHostPath)
{
    vpoxvfs_mount_t *pMount;
    ...
    pMount = (vpoxvfs_mount_t *)vfs_fsprivate(mp); AssertReturn(pMount, EINVAL); AssertReturn(pMount->pRootVnode, EINVAL);
 ...
 */
int
vpoxvfs_guest_path_to_shflstring_path_internal(mount_t mp, char *pszGuestPath, int cbGuestPath, PSHFLSTRING *ppResult)
{
    vpoxvfs_mount_t *pMount;
    ...
    rc = vpoxvfs_guest_path_to_char_path_internal(mp, pszGuestPath, cbGuestPath, &pszHostPath, &cbHostPath);
    if (rc == 0)
    {
        ...
        if (pSFPath)
        {
            ...
            vpoxvfs_put_path_internal((void **)&pszHostPath);
...
/**
 * Wrapper function for vpoxvfs_guest_path_to_char_path_internal() which
 ...
 */
int
vpoxvfs_guest_vnode_to_char_path_internal(vnode_t vnode, char **ppHostPath, int *pcbHostPath)
{
    mount_t     mp;
    int         rc;
    ...
    if (pszPath)
    {
        ...
        if (rc == 0)
        {
            return vpoxvfs_guest_path_to_char_path_internal(mp, pszPath, cbPath, ppHostPath, pcbHostPath);
        }
    }
    else
    {
...
/**
 * Wrapper function for vpoxvfs_guest_path_to_shflstring_path_internal() which
 * converts guest path to host path using vnode object information.
 *
 * @param vnode     Guest's VFS object
 ...
 */
int
vpoxvfs_guest_vnode_to_shflstring_path_internal(vnode_t vnode, PSHFLSTRING *ppResult)
{
    mount_t     mp;
    int         rc;
    ...
    if (pszPath)
    {
        ...
        if (rc == 0)
        {
            return vpoxvfs_guest_path_to_shflstring_path_internal(mp, pszPath, cbPath, ppResult);
        }
    }
    else
    {
...
/**
 * Free resources allocated by vpoxvfs_path_internal() and vpoxvfs_guest_vnode_to_shflstring_path_internal().
 *
 * @param ppHandle  Reference to object to be freed.
 */
void
vpoxvfs_put_path_internal(void **ppHandle)
{
...
static void
vpoxvfs_g2h_mode_dump_inernal(uint32_t fHostMode)
{
...
/**
 * Open existing VPoxVFS object and return its handle.
 *
 ...
 */
int
vpoxvfs_open_internal(vpoxvfs_mount_t *pMount, PSHFLSTRING pPath, uint32_t fFlags, SHFLHANDLE *pHandle)
{
    ...
    vpoxvfs_g2h_mode_dump_inernal(fFlags);
    ...
    {
        PDEBUG("vpoxvfs_open_internal() failed: %d", rc);
    }
...
/**
 * Release VPoxVFS object handle openned by vpoxvfs_open_internal().
 *
 * @param pMount   Mount session data.
 ...
 */
int
vpoxvfs_close_internal(vpoxvfs_mount_t *pMount, SHFLHANDLE pHandle)
{
    AssertReturn(pMount, EINVAL);
 ...
 */
int
vpoxvfs_get_info_internal(mount_t mp, PSHFLSTRING pSHFLDPath, PSHFLFSOBJINFO Info)
{
    vpoxvfs_mount_t        *pMount;
    SHFLCREATEPARMS         parms;
 ...
 */
int
vpoxvfs_exist_internal(vnode_t vnode)
{
    int rc;
    ...
    rc = vpoxvfs_guest_vnode_to_shflstring_path_internal(vnode, &pSFPath); AssertReturn(rc == 0, rc);
    if (rc == 0)
    {
        rc = vpoxvfs_open_internal(pMount, pSFPath, fFlags, &handle);
        if (rc == 0)
        {
            rc = vpoxvfs_close_internal(pMount, handle);
            if (rc != 0)
            {
                PDEBUG("Unable to close() VPoxVFS object handle while checking if object exist on host: %d", rc);
            }
        }
    }

    vpoxvfs_put_path_internal((void **)&pSFPath);
 ...
 */
mode_t
vpoxvfs_h2g_mode_inernal(RTFMODE fHostMode)
{
    mode_t fGuestMode = 0;
 ...
 */
uint32_t
vpoxvfs_g2h_mode_inernal(mode_t fGuestMode)
{
...
/**
 * Mount helper: Contruct SHFLSTRING which contains VPox share name or path.
 *
 ...
 */
SHFLSTRING *
vpoxvfs_construct_shflstring(const char *pachName, size_t cchName)
{
