...
HRESULT MachineCloneVMPrivate::addSaveState(const ComObjPtr<Machine> &machine, bool fAttachCurrent, ULONG &uCount, ULONG &uTotalWeight)
{
    ...
    if (!bstrSrcSaveStatePath.isEmpty())
    {
        ...
        if (RT_FAILURE(vrc))
            return p->setErrorBoth(VPOX_E_IPRT_ERROR, vrc, p->tr("Could not query file size of '%s' (%Rrc)"),
                                                        ...
                                                        ComObjPtr<Medium> *ppDiff) const
{
    ...
    try
    {
        ...
        diff.createObject();
        rc = diff->init(p->i_getVirtualPox(),
    ...
    {
        rc = VirtualPoxBase::handleUnexpectedExceptions(pMachine, RT_SRC_POS);
    }
...
HRESULT MachineCloneVM::start(IProgress **pProgress)
{
    ...
    try
    {
            ...
            && d->pSrcMachine->i_isSnapshotMachine())
        {
            ...
            rc = d->pSrcMachine->i_getVirtualPox()->FindMachine(bstrSrcMachineId.raw(), newSrcMachine.asOutParam());
            if (FAILED(rc)) throw rc;
        ...
        if (d->mode == CloneMode_MachineAndChildStates)
        {
            if (d->pSrcMachine->i_isSnapshotMachine())
            {
                ...
                rc = d->pSrcMachine->i_getVirtualPox()->FindMachine(bstrSrcMachineId.raw(), pCurr.asOutParam());
                if (FAILED(rc)) throw rc;
                if (pCurr.isNull())
                    throw p->setError(VPOX_E_OBJECT_NOT_FOUND);
        ...
        if (FAILED(rc)) throw rc;
        rc = d->pProgress->init(p->i_getVirtualPox(),
        ...
        if (RT_FAILURE(vrc))
            p->setErrorBoth(VPOX_E_IPRT_ERROR, vrc, "Could not create machine clone thread (%Rrc)", vrc);
    }
    catch (HRESULT rc2)
    {
        rc = rc2;
...
HRESULT MachineCloneVM::run()
{
    ...
    try
    {
        ...
        for (size_t i = newMedia.size(); i > 0; --i)
        {
            ...
            pMedium->i_removeRegistry(p->i_getVirtualPox()->i_getGlobalRegistryId());
            ...
            && !RTDirExists(strTrgSnapshotFolder.c_str()))
        {
            ...
            if (RT_FAILURE(vrc))
                throw p->setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
        ...
        for (size_t i = 0; i < d->llNVRAMFiles.size(); ++i)
        {
            ...
            {
                rc = p->i_getVirtualPox()->i_ensureFilePathExists(strTrgNVRAM.c_str(), true);
                if (FAILED(rc)) throw rc;
    ...
    {
        rc = VirtualPoxBase::handleUnexpectedExceptions(p, RT_SRC_POS);
    }
    ...
    if (FAILED(rc))
    {
        ...
        for (size_t i = 0; i < newFiles.size(); ++i)
        {
            ...
            if (RT_FAILURE(vrc))
                mrc = p->setErrorBoth(VPOX_E_IPRT_ERROR, vrc,
