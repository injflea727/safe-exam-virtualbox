...
#  include <vpx/vpx_encoder.h>
# endif
#endif /* VPOX_WITH_LIBVPX */

/** No flags specified. */
#define VPOX_WEBM_BLOCK_FLAG_NONE           0
/** Invisible block which can be skipped. */
#define VPOX_WEBM_BLOCK_FLAG_INVISIBLE      0x08
/** The block marks a key frame. */
#define VPOX_WEBM_BLOCK_FLAG_KEY_FRAME      0x80
...
#define VPOX_WEBM_TIMECODESCALE_FACTOR_MS   1000000

/** Maximum time (in ms) a cluster can store. */
#define VPOX_WEBM_CLUSTER_MAX_LEN_MS        INT16_MAX
...
#define VPOX_WEBM_BLOCK_MAX_LEN_MS          UINT16_MAX
...
public:
    ...
    struct WebMTimecodeBlocks
    {
        ...
        void Enqueue(WebMSimpleBlock *a_pBlock)
        {
            ...
            if (a_pBlock->Data.fFlags & VPOX_WEBM_BLOCK_FLAG_KEY_FRAME)
                fClusterNeeded = true;
        }
    };
    ...
    struct WebMSegment
    {
        WebMSegment(void)
            ...
            , cClusters(0)
        {
            uTimecodeScaleFactor = VPOX_WEBM_TIMECODESCALE_FACTOR_MS;
