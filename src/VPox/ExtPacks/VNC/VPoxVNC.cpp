...
DECLCALLBACK(int) VNCServerImpl::VRDEEnableConnections(HVRDESERVER hServer, bool fEnable)
{
    ...
    vncServer->desktopName = "VPoxVNC";

#ifndef VPOX_USE_IPV6
...
DECLCALLBACK(void) VNCServerImpl::VRDEResize(HVRDESERVER hServer)
{
    ...
    {
        // Convert RGB (windows/vpox) to BGR(vnc)
        uint32_t i, j;
 ...
 * @param pvUpdate  Pointer to VPoxGuest.h::VRDEORDERHDR structure with extra data.
