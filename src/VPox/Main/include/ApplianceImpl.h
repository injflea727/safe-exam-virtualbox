...
public:
    ...
    HRESULT init(VirtualPox *aVirtualPox);
    void uninit();
...
private:
    ...
    void i_importMachineGeneric(const ovf::VirtualSystem &vsysThis,
                                ...
                                ImportStack &stack);
    void i_importVPoxMachine(ComObjPtr<VirtualSystemDescription> &vsdescThis,
                             ComPtr<IMachine> &pNewMachine,
                             ImportStack &stack);
...
struct VirtualSystemDescriptionEntry
{
    ...
    Utf8Str strVPoxSuggested;               ///< configuration value (type-dependent); original value suggested by interpret()
    Utf8Str strVPoxCurrent;                 ///< configuration value (type-dependent); current value, either from interpret() or setFinalValue()
...
public:
    ...
    void i_importVPoxMachineXML(const xml::ElementNode &elmMachine);
