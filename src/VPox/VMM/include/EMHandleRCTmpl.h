 ...
 * @returns VPox status code to return to the EM main loop.
 *
...
#endif
{
    switch (rc)
    {
        ...
        case VINF_EM_PENDING_R3_IOPORT_WRITE:
            rc = VPOXSTRICTRC_TODO(emR3ExecutePendingIoPortWrite(pVM, pVCpu));
            break;
        case VINF_EM_PENDING_R3_IOPORT_READ:
            rc = VPOXSTRICTRC_TODO(emR3ExecutePendingIoPortRead(pVM, pVCpu));
            break;
        ...
        case VINF_EM_RAW_INJECT_TRPM_EVENT:
            ...
            rc = VPOXSTRICTRC_VAL(IEMInjectTrpmEvent(pVCpu));
        ...
        case VINF_EM_EMULATE_SPLIT_LOCK:
            rc = VPOXSTRICTRC_TODO(emR3ExecuteSplitLockInstruction(pVM, pVCpu));
            break;
        ...
        case VINF_IOM_R3_MMIO_COMMIT_WRITE:
            AssertFailed();
            rc = VPOXSTRICTRC_TODO(IOMR3ProcessForceFlag(pVM, pVCpu, rc));
            break;
