...
struct PLEventQueue {
    ...
    CFStringRef         mRunLoopModeStr;                                                                /* vpox */
...
_pl_CleanupNativeNotifier(PLEventQueue* self)
{
    ...
    CFRunLoopRemoveSource(self->mMainRunLoop, self->mRunLoopSource, self->mRunLoopModeStr);             /* vpox */
    CFRelease(self->mRunLoopSource);
    CFRelease(self->mMainRunLoop);
    CFRelease(self->mRunLoopModeStr);                                                                   /* vpox */
...
_pl_AcknowledgeNativeNotify(PLEventQueue* self)
{
    ...
    */
    ...
    if ((count == -1) && ((errno == EAGAIN) || (errno == EWOULDBLOCK)))
        return PR_SUCCESS;
    return PR_FAILURE;
#elif defined(MAC_USE_CFRUNLOOPSOURCE)                                                                  /* vpox */
                                                                                                        /* vpox */
    CFRunLoopRunInMode(self->mRunLoopModeStr, 0.0, 1);                                                  /* vpox */
    return PR_SUCCESS;                                                                                  /* vpox */
#else
...
static void _md_CreateEventQueue( PLEventQueue *eventQueue )
{
    ...
    {                                                                                                   /* vpox */
        char szModeName[80];                                                                            /* vpox */
        snprintf(szModeName, sizeof(szModeName), "VPoxXPCOMQueueMode-%p", eventQueue);                  /* vpox */
        eventQueue->mRunLoopModeStr = CFStringCreateWithCString(kCFAllocatorDefault,                    /* vpox */
                                                                szModeName, kCFStringEncodingASCII);    /* vpox */
        CFRunLoopAddSource(eventQueue->mMainRunLoop,                                                    /* vpox */
                           eventQueue->mRunLoopSource, eventQueue->mRunLoopModeStr);                    /* vpox */
    }                                                                                                   /* vpox */
