/* $Id: HGSMIHost.cpp $ */
/** @file
 * VPox Host Guest Shared Memory Interface (HGSMI), host part.
 *
 * Host part:
...
#ifdef DEBUG_misha
//# define VPOXHGSMI_STATE_DEBUG
#endif

#ifdef VPOXHGSMI_STATE_DEBUG
# define VPOXHGSMI_STATE_START_MAGIC     UINT32_C(0x12345678)
# define VPOXHGSMI_STATE_STOP_MAGIC      UINT32_C(0x87654321)
# define VPOXHGSMI_STATE_FIFOSTART_MAGIC UINT32_C(0x9abcdef1)
# define VPOXHGSMI_STATE_FIFOSTOP_MAGIC  UINT32_C(0x1fedcba9)

# define VPOXHGSMI_SAVE_START(_pSSM)     do{ int rc2 = pHlp->pfnSSMPutU32(_pSSM, VPOXHGSMI_STATE_START_MAGIC);     AssertRC(rc2); }while(0)
# define VPOXHGSMI_SAVE_STOP(_pSSM)      do{ int rc2 = pHlp->pfnSSMPutU32(_pSSM, VPOXHGSMI_STATE_STOP_MAGIC);      AssertRC(rc2); }while(0)
# define VPOXHGSMI_SAVE_FIFOSTART(_pSSM) do{ int rc2 = pHlp->pfnSSMPutU32(_pSSM, VPOXHGSMI_STATE_FIFOSTART_MAGIC); AssertRC(rc2); }while(0)
# define VPOXHGSMI_SAVE_FIFOSTOP(_pSSM)  do{ int rc2 = pHlp->pfnSSMPutU32(_pSSM, VPOXHGSMI_STATE_FIFOSTOP_MAGIC);  AssertRC(rc2); }while(0)

# define VPOXHGSMI_LOAD_CHECK(_pSSM, _v) \
    do { \
        uint32_t u32; \
...
# define VPOXHGSMI_LOAD_START(_pSSM) VPOXHGSMI_LOAD_CHECK(_pSSM, VPOXHGSMI_STATE_START_MAGIC)
# define VPOXHGSMI_LOAD_FIFOSTART(_pSSM) VPOXHGSMI_LOAD_CHECK(_pSSM, VPOXHGSMI_STATE_FIFOSTART_MAGIC)
# define VPOXHGSMI_LOAD_FIFOSTOP(_pSSM) VPOXHGSMI_LOAD_CHECK(_pSSM, VPOXHGSMI_STATE_FIFOSTOP_MAGIC)
# define VPOXHGSMI_LOAD_STOP(_pSSM) VPOXHGSMI_LOAD_CHECK(_pSSM, VPOXHGSMI_STATE_STOP_MAGIC)
#else   /* !VPOXHGSMI_STATE_DEBUG */
# define VPOXHGSMI_SAVE_START(a_pSSM)       do { } while(0)
# define VPOXHGSMI_SAVE_STOP(a_pSSM)        do { } while(0)
# define VPOXHGSMI_SAVE_FIFOSTART(a_pSSM)   do { } while(0)
# define VPOXHGSMI_SAVE_FIFOSTOP(a_pSSM)    do { } while(0)

# define VPOXHGSMI_LOAD_START(a_pSSM)       do { } while(0)
# define VPOXHGSMI_LOAD_FIFOSTART(a_pSSM)   do { } while(0)
# define VPOXHGSMI_LOAD_FIFOSTOP(a_pSSM)    do { } while(0)
# define VPOXHGSMI_LOAD_STOP(a_pSSM)        do { } while(0)
#endif
 ...
 *
 * @return VPox status code. Pointer to the payload data in *ppvData.
