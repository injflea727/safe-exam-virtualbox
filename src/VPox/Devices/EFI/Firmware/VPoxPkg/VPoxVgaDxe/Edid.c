...
/**
  ...
  @param  Private             Pointer to VPOX_VGA_PRIVATE_DATA.
...
static EFI_STATUS VPoxVgaVideoModeInitExtra(void)
{
  UINT16 w, cur_info_ofs, vmode, xres, yres;
  UINTN  Index;
  VPOX_VGA_VIDEO_MODES *VideoMode;
  ...
  Index = VPoxVgaVideoModeCount - 16;
  VideoMode = &VPoxVgaVideoModes[Index];
  ...
  while (vmode != VBE_VESA_MODE_END_OF_LIST)
  {
    ...
    if (vmode >= VBE_VPOX_MODE_CUSTOM1 && vmode <= VBE_VPOX_MODE_CUSTOM16 && xres && yres && Index < VPoxVgaVideoModeCount) {
...
/**
  Construct the valid video modes for VPoxVga.
  ...
  )
{
  ...
  BOOLEAN                                TimingMatch;
  const VPOX_VGA_VIDEO_MODES             *VideoMode;
  EDID_TIMING                            TempTiming;
  ...
  if (EdidFound && 0) {
    ...
    //
    ValidModeCount = 0;
    Private->ModeData = AllocatePool(sizeof(VPOX_VGA_MODE_DATA) * VPoxVgaVideoModeCount);
    ModeData = &Private->ModeData[0];
    VideoMode = &VPoxVgaVideoModes[0];
    for (Index = 0; Index < VPoxVgaVideoModeCount; Index++) {

      TimingMatch = TRUE;

      //
      // Check whether match with VPoxVga video mode
      //
  ...
  for (Index = 0; Index < ValidModeCount - 1; Index ++) {
    UINT32 Index2;
    VPOX_VGA_MODE_DATA *ModeData2 = ModeData + 1;
