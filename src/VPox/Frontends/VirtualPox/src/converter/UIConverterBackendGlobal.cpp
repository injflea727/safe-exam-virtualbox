...
/** @file
 * VPox Qt GUI - UIConverterBackendGlobal implementation.
 */
...
template<> QString toString(const StorageSlot &storageSlot)
{
    ...
    switch (storageSlot.bus)
    {
        ...
        {
            int iMaxPort = uiCommon().virtualPox().GetSystemProperties().GetMaxPortCountForStorageBus(storageSlot.bus);
            int iMaxDevice = uiCommon().virtualPox().GetSystemProperties().GetMaxDevicesPerPortForStorageBus(storageSlot.bus);
    ...
    switch (iIndex)
    {
        ...
        case 3:
        {
            if (result.bus == KStorageBus_Null)
                break;
            const int iMaxPort = uiCommon().virtualPox().GetSystemProperties().GetMaxPortCountForStorageBus(result.bus);
            const int iMaxDevice = uiCommon().virtualPox().GetSystemProperties().GetMaxDevicesPerPortForStorageBus(result.bus);
...
template<> UIExtraDataMetaDefs::DetailsElementOptionTypeNetwork fromInternalString<UIExtraDataMetaDefs::DetailsElementOptionTypeNetwork>(const QString &strDetailsElementOptionTypeNetwork)
{
    ...
    keys << "BridgedAdapter";  values << UIExtraDataMetaDefs::DetailsElementOptionTypeNetwork_BridgedAdapter;
...
template<> QString toString(const UIMediumFormat &enmUIMediumFormat)
{
    ...
    {
        case UIMediumFormat_VDI:       strResult = QApplication::translate("UICommon", "VDI (VirtualPox Disk Image)", "UIMediumFormat"); break;
