/* $Id: HostPower.cpp $ */
/** @file
 * VirtualPox interface to host's power notification service
 */
...
HostPowerService::HostPowerService(VirtualPox *aVirtualPox)
{
    AssertPtr(aVirtualPox);
    mVirtualPox = aVirtualPox;
}
...
void HostPowerService::notify(Reason_T aReason)
{
    ...
    VirtualPox::InternalControlList controls;
    ...
    switch (aReason)
    {
        case Reason_HostSuspend:
        {
            ...
            PerformanceCollector *perfcollector = mVirtualPox->i_performanceCollector();

            if (perfcollector)
                perfcollector->suspendSampling();
#endif
            mVirtualPox->i_getOpenedMachines(machines, &controls);

            /* pause running VMs */
            for (VirtualPox::InternalControlList::const_iterator it = controls.begin();
                 it != controls.end();
                 ++it)
            {
        ...
        case Reason_HostBatteryLow:
        {
            ...
            Bstr value;
            rc = mVirtualPox->GetExtraData(Bstr("VPoxInternal2/SavestateOnBatteryLow").raw(),
                                           value.asOutParam());
            int fGlobal = 0;
                 ...
                 ++it)
            {
                ...
                rc = pMachine->GetExtraData(Bstr("VPoxInternal2/SavestateOnBatteryLow").raw(),
                                            value.asOutParam());
                int fPerVM = 0;
                if (SUCCEEDED(rc) && !value.isEmpty())
                {
