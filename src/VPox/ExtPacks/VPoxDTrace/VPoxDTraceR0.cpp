...
/** Caller indicator. */
typedef enum VPOXDTCALLER
{
    kVPoxDtCaller_Invalid = 0,
    kVPoxDtCaller_Generic,
    kVPoxDtCaller_ProbeFireUser,
    kVPoxDtCaller_ProbeFireKernel
} VPOXDTCALLER;
 ...
 */
typedef struct VPoxDtStackData
{
    ...
    /** Credentials allocated by VPoxDtGetCurrentCreds. */
    struct VPoxDtCred      *pCred;
    /** Thread structure currently being held by this thread. */
    struct VPoxDtThread    *pThread;
    /** Pointer to this structure.
     * This is the final bit of integrity checking. */
    struct VPoxDtStackData *pSelf;
} VBDTSTACKDATA;
...
#define VBDT_CLEAR_STACK_DATA() \
    ...
    { \
        ...
        if (pStackData->pThread) \
            VPoxDtReleaseThread(pStackData->pThread); \
    } while (0)
...
*********************************************************************************************************************************/
/** Per CPU information */
cpucore_t                       g_aVPoxDtCpuCores[RTCPUSET_MAX_CPUS];
/** Dummy mutex. */
struct VPoxDtMutex              g_DummyMtx;
/** Pointer to the tracer helpers provided by VPoxDrv. */
static PCSUPDRVTRACERHLP        g_pVPoxDTraceHlp;
 ...
 */
static PVBDTSTACKDATA vpoxDtGetStackData(void)
{
 ...
 */
static DECLCALLBACK(void) vpoxDtSyncCallback(RTCPUID idCpu, void *pvUser1, void *pvUser2)
{
...
{
    int rc = RTMpOnAll(vpoxDtSyncCallback, NULL, NULL);
    AssertRC(rc);
}
...
int  VPoxDtCopyIn(void const *pvUser, void *pvDst, size_t cb)
{
...
int  VPoxDtCopyOut(void const *pvSrc, void *pvUser, size_t cb)
{
...
{
    PVBDTSTACKDATA pData = vpoxDtGetStackData();
    if (pData->enmCaller == kVPoxDtCaller_ProbeFireKernel)
 ...
 */
void VPoxDtPanic(const char *pszFormat, ...)
{
    va_list va;
 ...
 * @param   ...                 Arguments to the panic message.
 */
void VPoxDtCmnErr(int iLevel, const char *pszFormat, ...)
{
    va_list va;
    va_start(va, pszFormat);
...
void VPoxDtUPrintf(const char *pszFormat, ...)
{
    va_list va;
    va_start(va, pszFormat);
    VPoxDtUPrintfV(pszFormat, va);
    va_end(va);
}


/** vuprintf implementation */
void VPoxDtUPrintfV(const char *pszFormat, va_list va)
{
    SUPR0Printf("%N", pszFormat, va);
}
...
/* CRED implementation. */
cred_t *VPoxDtGetCurrentCreds(void)
{
    PVBDTSTACKDATA pData = vpoxDtGetStackData();
    if (!pData->pCred)
    {
...
void VPoxDtCredHold(struct VPoxDtCred *pCred)
{
...
void VPoxDtCredFree(struct VPoxDtCred *pCred)
{
...
static struct VPoxDtThread *g_apThreadsHash[16384];
...
static struct VPoxDtThread  g_aThreads[8192];


static int vpoxDtInitThreadDb(void)
{
    int rc = RTSpinlockCreate(&g_hThreadSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "VPoxDtThreadDb");
    if (RT_FAILURE(rc))
        return rc;
...
static void vpoxDtTermThreadDb(void)
{
...
struct VPoxDtThread *VPoxDtGetCurrentThread(void)
{
    ...
    struct VPoxDtThread *pThread = g_apThreadsHash[iHash];
    while (pThread)
    {
        if (pThread->hNative == hNativeSelf)
        {
            ...
            /* Hold the thread in the on-stack data, making sure it does not
               get reused till the thread leaves VPoxDTrace. */
     ...
     */
    pThread = RTListGetLast(&g_ThreadAgeList, struct VPoxDtThread, AgeEntry);
    AssertFatal(pThread);
    RTListNodeRemove(&pThread->AgeEntry);
    if (pThread->hNative != NIL_RTNATIVETHREAD)
    {
        ...
        {
            for (struct VPoxDtThread *pPrev = g_apThreadsHash[iHash2]; ; pPrev = pPrev->pNext)
            {
                AssertPtr(pPrev);
 ...
 *
 */
static void VPoxDtReleaseThread(struct VPoxDtThread *pThread)
{
    RTSpinlockAcquire(g_hThreadSpinlock);
...
#define VPOXDTVMEMCHUNK_BITS    ( ((_64K - 32 - sizeof(uint32_t) * 2) / sizeof(uint32_t)) * 32)

/**
 * Resource allocator chunk.
 */
typedef struct  VPoxDtVMemChunk
{
    ...
    uint32_t            bm[VPOXDTVMEMCHUNK_BITS / 32];
} VPOXDTVMEMCHUNK;
/** Pointer to a resource allocator chunk. */
typedef VPOXDTVMEMCHUNK *PVPOXDTVMEMCHUNK;
...
typedef struct VPoxDtVMem
{
    ...
    PVPOXDTVMEMCHUNK    apChunks[1];
} VPOXDTVMEM;
/** Pointer to a resource allocator instance. */
typedef VPOXDTVMEM *PVPOXDTVMEM;

/** Magic value for the VPOXDTVMEM structure. */
#define VPOXDTVMEM_MAGIC        RT_MAKE_U32_FROM_U8('V', 'M',  'e',  'm')


/* vmem_create implementation */
struct VPoxDtVMem *VPoxDtVMemCreate(const char *pszName, void *pvBase, size_t cb, size_t cbUnit,
                                    PFNRT pfnAlloc, PFNRT pfnFree, struct VPoxDtVMem *pSrc,
                                    size_t cbQCacheMax, uint32_t fFlags)
{
     ...
     */
    uint32_t cChunks = (uint32_t)cb / VPOXDTVMEMCHUNK_BITS;
    if (cb % VPOXDTVMEMCHUNK_BITS)
        cChunks++;
    PVPOXDTVMEM pThis = (PVPOXDTVMEM)RTMemAllocZ(RT_UOFFSETOF_DYN(VPOXDTVMEM, apChunks[cChunks]));
    if (!pThis)
        return NULL;
    int rc = RTSpinlockCreate(&pThis->hSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "VPoxDtVMem");
    if (RT_FAILURE(rc))
    {
        RTMemFree(pThis);
        return NULL;
    }
    pThis->u32Magic     = VPOXDTVMEM_MAGIC;
    pThis->cCurFree     = 0;
    pThis->cCurChunks   = 0;
...
void  VPoxDtVMemDestroy(struct VPoxDtVMem *pThis)
{
    if (!pThis)
        return;
    AssertPtrReturnVoid(pThis);
    AssertReturnVoid(pThis->u32Magic == VPOXDTVMEM_MAGIC);
...
void *VPoxDtVMemAlloc(struct VPoxDtVMem *pThis, size_t cbMem, uint32_t fFlags)
{
    /*
     * Validate input.
     */
    ...
    AssertReturn(pThis->u32Magic == VPOXDTVMEM_MAGIC, NULL);
    ...
    for (;;)
    {
        ...
        if (RT_LIKELY(pThis->cCurFree > 0))
        {
            for (uint32_t iChunk = 0; iChunk < cChunks; iChunk++)
            {
                ...
                {
                    int iBit = ASMBitFirstClear(pChunk->bm, VPOXDTVMEMCHUNK_BITS);
                    AssertMsgReturnStmt(iBit >= 0 && (unsigned)iBit < VPOXDTVMEMCHUNK_BITS, ("%d\n", iBit),
                                        RTSpinlockRelease(pThis->hSpinlock),
                                        NULL);
         ...
         */
        uint32_t const  iFirstBit = cChunks > 0 ? pThis->apChunks[cChunks - 1]->iFirst + VPOXDTVMEMCHUNK_BITS : 0;
        uint32_t const  cFreeBits = cChunks + 1 == pThis->cMaxChunks
                                  ? pThis->cMaxItems - (iFirstBit - pThis->uBase)
                                  : VPOXDTVMEMCHUNK_BITS;
        Assert(cFreeBits <= VPOXDTVMEMCHUNK_BITS);

        RTSpinlockRelease(pThis->hSpinlock);

        pChunk = (PVPOXDTVMEMCHUNK)RTMemAllocZ(sizeof(*pChunk));
        if (!pChunk)
            return NULL;

        pChunk->iFirst   = iFirstBit;
        pChunk->cCurFree = cFreeBits;
        if (cFreeBits != VPOXDTVMEMCHUNK_BITS)
        {
            /* lazy bird. */
            uint32_t iBit = cFreeBits;
            while (iBit < VPOXDTVMEMCHUNK_BITS)
            {
                ASMBitSet(pChunk->bm, iBit);
                iBit++;
            }
...
void VPoxDtVMemFree(struct VPoxDtVMem *pThis, void *pvMem, size_t cbMem)
{
    /*
     * Validate input.
     */
     ...
     */
    RTSpinlockAcquire(pThis->hSpinlock);
    uint32_t const iChunk = uMem / VPOXDTVMEMCHUNK_BITS;
    if (iChunk < pThis->cCurChunks)
    {
        PVPOXDTVMEMCHUNK pChunk = pThis->apChunks[iChunk];
        uint32_t iBit = uMem - pChunk->iFirst;
        AssertReturnVoidStmt(iBit < VPOXDTVMEMCHUNK_BITS, RTSpinlockRelease(pThis->hSpinlock));
...
void *VPoxDtKMemAlloc(size_t cbMem, uint32_t fFlags)
{
    void    *pvMem;
#ifdef HAVE_RTMEMALLOCEX_FEATURES
...
void *VPoxDtKMemAllocZ(size_t cbMem, uint32_t fFlags)
{
    void    *pvMem;
#ifdef HAVE_RTMEMALLOCEX_FEATURES
...
void  VPoxDtKMemFree(void *pvMem, size_t cbMem)
{
...
struct VPoxDtMemCache *VPoxDtKMemCacheCreate(const char *pszName, size_t cbBuf, size_t cbAlign,
                                             ...
                                             void *pvUser, void *pvVM, uint32_t fFlags)
{
     ...
     */
    struct VPoxDtMemCache *pThis = (struct VPoxDtMemCache *)RTMemAlloc(sizeof(*pThis));
    if (!pThis)
        return NULL;
...
void  VPoxDtKMemCacheDestroy(struct VPoxDtMemCache *pThis)
{
    RTMemFree(pThis);
}
...
void *VPoxDtKMemCacheAlloc(struct VPoxDtMemCache *pThis, uint32_t fFlags)
{
    void    *pvMem;
#ifdef HAVE_RTMEMALLOCEX_FEATURES
    uint32_t fMemAllocFlags = (fFlags & KM_NOSLEEP ? RTMEMALLOCEX_FLAGS_ANY_CTX : 0) | RTMEMALLOCEX_FLAGS_ZEROED;
...
void  VPoxDtKMemCacheFree(struct VPoxDtMemCache *pThis, void *pvMem)
{
...
/** Initializes a mutex. */
int VPoxDtMutexInit(struct VPoxDtMutex *pMtx)
{
...
/** Deletes a mutex. */
void VPoxDtMutexDelete(struct VPoxDtMutex *pMtx)
{
...
void VPoxDtMutexEnter(struct VPoxDtMutex *pMtx)
{
    AssertPtr(pMtx);
    if (pMtx == &g_DummyMtx)
        return;
...
void VPoxDtMutexExit(struct VPoxDtMutex *pMtx)
{
    AssertPtr(pMtx);
    if (pMtx == &g_DummyMtx)
        return;
...
bool VPoxDtMutexIsOwner(struct VPoxDtMutex *pMtx)
{
 ...
 */
static void vpoxDtVtgConvAttr(dtrace_attribute_t *pDtAttr, PCVTGDESCATTR pVtgAttr)
{
 ...
 */
static const char *vpoxDtVtgGetString(PVTGOBJHDR pVtgHdr, uint32_t offStrTab)
{
 ...
 */
static void     vpoxDtPOps_Provide(void *pvProv, const dtrace_probedesc_t *pDtProbeDesc)
{
    ...
    for (uint32_t idxProbeLoc = 0; idxProbeLoc < cProbeLocs; idxProbeLoc++)
    {
        ...
        if (*pidProbe != 0)
         ...
         const char *pszPrbName = vpoxDtVtgGetString(pProv->pHdr, pProbeDesc->offName);
         ...
         if (dtrace_probe_lookup(idProvider, pProv->pszModName, pszFnNmBuf, pszPrbName) != DTRACE_IDNONE)
         {
             ...
             if (dtrace_probe_lookup(idProvider, pProv->pszModName, pszFnNmBuf, pszPrbName) != DTRACE_IDNONE)
             {
                 ...
                 if (iOrd >= 128)
                 {
                     LogRel(("VPoxDrv: More than 128 duplicate probe location instances at line %u in function %s [%s], probe %s\n",
 ...
 */
static int      vpoxDtPOps_Enable(void *pvProv, dtrace_id_t idProbe, void *pvProbe)
{
    PSUPDRVVDTPROVIDERCORE  pProv   = (PSUPDRVVDTPROVIDERCORE)pvProv;
 ...
 */
static void     vpoxDtPOps_Disable(void *pvProv, dtrace_id_t idProbe, void *pvProbe)
{
    PSUPDRVVDTPROVIDERCORE  pProv  = (PSUPDRVVDTPROVIDERCORE)pvProv;
    AssertPtrReturnVoid(pProv);
    LOG_DTRACE(("%s: %p / %p - %#x / %p\n", __FUNCTION__, pProv, pProv->TracerData.DTrace.idProvider, idProbe, pvProbe));
 ...
 */
static void     vpoxDtPOps_GetArgDesc(void *pvProv, dtrace_id_t idProbe, void *pvProbe,
                                      dtrace_argdesc_t *pArgDesc)
{
    ...
    if (!pProv->TracerData.DTrace.fZombie)
    {
        ...
        {
            const char *pszType = vpoxDtVtgGetString(pProv->pHdr, pArgList->aArgs[uArg].offType);
 ...
 */
static uint64_t vpoxDtPOps_GetArgVal(void *pvProv, dtrace_id_t idProbe, void *pvProbe,
                                     int iArg, int cFrames)
{
    ...
    if (iArg >= 20)
        u64Ret = UINT64_MAX;
    else if (pData->enmCaller == kVPoxDtCaller_ProbeFireKernel)
    {
#if ARCH_BITS == 64
    ...
    else if (pData->enmCaller == kVPoxDtCaller_ProbeFireUser)
    {
 ...
 */
static void    vpoxDtPOps_Destroy(void *pvProv, dtrace_id_t idProbe, void *pvProbe)
{
    PSUPDRVVDTPROVIDERCORE  pProv  = (PSUPDRVVDTPROVIDERCORE)pvProv;
    AssertPtrReturnVoid(pProv);
    LOG_DTRACE(("%s: %p / %p - %#x / %p\n", __FUNCTION__, pProv, pProv->TracerData.DTrace.idProvider, idProbe, pvProbe));
 ...
 */
static const dtrace_pops_t g_vpoxDtVtgProvOps =
{
    /* .dtps_provide         = */ vpoxDtPOps_Provide,
    /* .dtps_provide_module  = */ NULL,
    /* .dtps_enable          = */ vpoxDtPOps_Enable,
    /* .dtps_disable         = */ vpoxDtPOps_Disable,
    /* .dtps_suspend         = */ NULL,
    /* .dtps_resume          = */ NULL,
    /* .dtps_getargdesc      = */ vpoxDtPOps_GetArgDesc,
    /* .dtps_getargval       = */ vpoxDtPOps_GetArgVal,
    /* .dtps_usermode        = */ NULL,
    /* .dtps_destroy         = */ vpoxDtPOps_Destroy
};
 ...
 */
static DECLCALLBACK(void) vpoxDtTOps_ProbeFireKernel(struct VTGPROBELOC *pVtgProbeLoc, uintptr_t uArg0, uintptr_t uArg1, uintptr_t uArg2,
                                                     uintptr_t uArg3, uintptr_t uArg4)
{
    ...
    VBDT_SETUP_STACK_DATA(kVPoxDtCaller_ProbeFireKernel);
 ...
 */
static DECLCALLBACK(void) vpoxDtTOps_ProbeFireUser(PCSUPDRVTRACERREG pThis, PSUPDRVSESSION pSession, PCSUPDRVTRACERUSRCTX pCtx,
                                                   PCVTGOBJHDR pVtgHdr, PCVTGPROBELOC pProbeLocRO)
{
    ...
    VBDT_SETUP_STACK_DATA(kVPoxDtCaller_ProbeFireUser);
 ...
 */
static DECLCALLBACK(int) vpoxDtTOps_TracerOpen(PCSUPDRVTRACERREG pThis, PSUPDRVSESSION pSession, uint32_t uCookie,
                                               uintptr_t uArg, uintptr_t *puSessionData)
{
    ...
    VBDT_SETUP_STACK_DATA(kVPoxDtCaller_Generic);

    int rc = dtrace_open((dtrace_state_t **)puSessionData, VPoxDtGetCurrentCreds());
 ...
 */
static DECLCALLBACK(int) vpoxDtTOps_TracerIoCtl(PCSUPDRVTRACERREG pThis, PSUPDRVSESSION pSession, uintptr_t uSessionData,
 ...
 * interface_method_impl{SUPDRVTRACERREG,pfnTracerClose}
 */
static DECLCALLBACK(void) vpoxDtTOps_TracerClose(PCSUPDRVTRACERREG pThis, PSUPDRVSESSION pSession, uintptr_t uSessionData)
{
 ...
 */
static DECLCALLBACK(int) vpoxDtTOps_ProviderRegister(PCSUPDRVTRACERREG pThis, PSUPDRVVDTPROVIDERCORE pCore)
{
    ...
    vpoxDtVtgConvAttr(&DtAttrs.dtpa_provider, &pDesc->AttrSelf);
    vpoxDtVtgConvAttr(&DtAttrs.dtpa_mod,      &pDesc->AttrModules);
    vpoxDtVtgConvAttr(&DtAttrs.dtpa_func,     &pDesc->AttrFunctions);
    vpoxDtVtgConvAttr(&DtAttrs.dtpa_name,     &pDesc->AttrNames);
    vpoxDtVtgConvAttr(&DtAttrs.dtpa_args,     &pDesc->AttrArguments);
 ...
 */
static DECLCALLBACK(int) vpoxDtTOps_ProviderDeregister(PCSUPDRVTRACERREG pThis, PSUPDRVVDTPROVIDERCORE pCore)
{
 ...
 */
static DECLCALLBACK(int) vpoxDtTOps_ProviderDeregisterZombie(PCSUPDRVTRACERREG pThis, PSUPDRVVDTPROVIDERCORE pCore)
{
    uintptr_t idProvider = pCore->TracerData.DTrace.idProvider;
    LOG_DTRACE(("%s: %p / %p\n", __FUNCTION__, pThis, idProvider));
    AssertPtrReturn(idProvider, VERR_INTERNAL_ERROR_3);
...
/**
 * The tracer registration record of the VPox DTrace implementation
 */
static SUPDRVTRACERREG g_VPoxDTraceReg =
{
    SUPDRVTRACERREG_MAGIC,
    SUPDRVTRACERREG_VERSION,
    vpoxDtTOps_ProbeFireKernel,
    vpoxDtTOps_ProbeFireUser,
    vpoxDtTOps_TracerOpen,
    vpoxDtTOps_TracerIoCtl,
    vpoxDtTOps_TracerClose,
    vpoxDtTOps_ProviderRegister,
    vpoxDtTOps_ProviderDeregister,
    vpoxDtTOps_ProviderDeregisterZombie,
    SUPDRVTRACERREG_MAGIC
};
...
{
    int rc = vpoxDtInitThreadDb();
    if (RT_SUCCESS(rc))
    {
        rc = dtrace_attach();
        if (rc == DDI_SUCCESS)
        {
            rc = SUPR0TracerRegisterImpl(hMod, NULL, &g_VPoxDTraceReg, &g_pVPoxDTraceHlp);
            if (RT_SUCCESS(rc))
                return rc;
        ...
        SUPR0Printf("vpoxDtInitThreadDb -> %d\n", rc);
