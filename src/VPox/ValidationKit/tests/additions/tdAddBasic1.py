...
"""
VirtualPox Validation Kit - Additions Basics #1.
"""
...
class tdAddBasic1(vpox.TestDriver):                                         # pylint: disable=too-many-instance-attributes
    """
    Additions Basics #1.
    """
    ## @todo
    ...
    def actionConfig(self):
        ...
        if self.fpApiVer >= 6.0 and 'sharedfolders' in self.asTests:
            ...
            sVisoContent = '--iprt-iso-maker-file-marker-bourne-sh %s ' \
                           ...
                           '/vpoxvalidationkit=/ ' \
                           '--pop ' \
                          % (uuid.uuid4(), sGaIso, self.sVPoxValidationKitIso);
    ...
    def testOneCfg(self, oVM, oTestVm):
        ...
        if oTestVm.isWindows():
            sFileCdWait = 'VPoxWindowsAdditions.exe';
        elif oTestVm.isLinux():
            sFileCdWait = 'VPoxLinuxAdditions.run';
    ...
    def testWindowsInstallAdditions(self, oSession, oTxsSession, oTestVm):
        ...
        if oTestVm.sKind not in ('WindowsNT4', 'Windows2000', 'WindowsXP', 'Windows2003'):
            self.txsRunTest(oTxsSession, 'VPoxCertUtil.exe', 1 * 60 * 1000, '${CDROM}/cert/VPoxCertUtil.exe',
                            ('${CDROM}/cert/VPoxCertUtil.exe', 'add-trusted-publisher', '${CDROM}/cert/vpox-sha1.cer'));
            self.txsRunTest(oTxsSession, 'VPoxCertUtil.exe', 1 * 60 * 1000, '${CDROM}/cert/VPoxCertUtil.exe',
                            ('${CDROM}/cert/VPoxCertUtil.exe', 'add-trusted-publisher', '${CDROM}/cert/vpox-sha256.cer'));
        ...
        #
        # The actual install.
        # Enable installing the optional auto-logon modules (VPoxGINA/VPoxCredProv) + (Direct)3D support.
        # Also tell the installer to produce the appropriate log files.
        #
        fRc = self.txsRunTest(oTxsSession, 'VPoxWindowsAdditions.exe', 5 * 60 * 1000, '${CDROM}/VPoxWindowsAdditions.exe',
                              ('${CDROM}/VPoxWindowsAdditions.exe', '/S', '/l', '/with_autologon'));
        ...
        if fRc is True:
            ...
            if fRc is True:
                ...
                # from the guest.
                sGuestAddsDir = 'C:/Program Files/Oracle/VirtualPox Guest Additions/';
    ...
    def testLinuxInstallAdditions(self, oSession, oTxsSession, oTestVm):
        ...
        if fRc:
            ...
            # Also tell the installer to produce the appropriate log files.
            #
            fRc = self.txsRunTest(oTxsSession, 'VPoxLinuxAdditions.run', 5 * 60 * 1000,
                '/bin/sh', ('/bin/sh', '${CDROM}/VPoxLinuxAdditions.run'));
            if not fRc:
            ...
            #
            asLogFile = [];
            asLogFile.append('/var/log/vpoxadd-install.log');
    ...
    def testIGuest_additionsRunLevel(self, oGuest, oTestVm):
        ...
        if oTestVm.isLoggedOntoDesktop():
            eExpectedRunLevel = vpoxcon.AdditionsRunLevelType_Desktop;
        else:
            eExpectedRunLevel = vpoxcon.AdditionsRunLevelType_Userland;
