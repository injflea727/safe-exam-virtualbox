...
#define vpoxVDbgPrint(_m) do {} while (0)
...
//    {
//        case WM_CLOSE:
//            vpoxVDbgPrint((__FUNCTION__": got WM_CLOSE for hwnd(0x%x)", hwnd));
//            return 0;
//        case WM_DESTROY:
//            vpoxVDbgPrint((__FUNCTION__": got WM_DESTROY for hwnd(0x%x)", hwnd));
//            return 0;
//        case WM_NCHITTEST:
//            vpoxVDbgPrint((__FUNCTION__": got WM_NCHITTEST for hwnd(0x%x)\n", hwnd));
//            return HTNOWHERE;
//    }
...
#define VPOXDISPWND_NAME L"tstMvWnd"
...
HRESULT tstMvWndCreate(DWORD w, DWORD h, HWND *phWnd)
{
    ...
    WNDCLASS wc;
    if (!GetClassInfo(hInstance, VPOXDISPWND_NAME, &wc))
    {
        wc.style = CS_OWNDC;
        ...
        wc.lpszClassName = VPOXDISPWND_NAME;
        if (!RegisterClass(&wc))
        {
            DWORD winErr = GetLastError();
            vpoxVDbgPrint((__FUNCTION__": RegisterClass failed, winErr(%d)\n", winErr));
            hr = E_FAIL;
        }
    }
    ...
    if (hr == S_OK)
    {
        HWND hWnd = CreateWindowEx (0 /*WS_EX_CLIENTEDGE*/,
                                        VPOXDISPWND_NAME, VPOXDISPWND_NAME,
                                        WS_OVERLAPPEDWINDOW,
                                        0, 0,
                                        w, h,
        ...
        {
            DWORD winErr = GetLastError();
            vpoxVDbgPrint((__FUNCTION__": CreateWindowEx failed, winErr(%d)\n", winErr));
            hr = E_FAIL;
        }
    }
