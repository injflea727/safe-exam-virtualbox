/* $Id: VPoxVgaMiniPortDxe.c $ */
/** @file
 * VPoxVgaMiniPortDxe.c - VgaMiniPort Protocol Implementation.
 */
 ...
 */
typedef struct VPOXVGAMINIPORT
{
    /** The VGA Mini Port Protocol. */
    EFI_VGA_MINI_PORT_PROTOCOL  VgaMiniPort;
    /** Magic value, VPOX_VGA_MINI_PORT_MAGIC. */
    UINT32                      u32Magic;
...
/** Pointer to a VPOXVGAMINIPORT structure. */
typedef VPOXVGAMINIPORT *PVPOXVGAMINIPORT;

/** VPOXVGAMINIPORT::u32Magic value (Isaac Asimov). */
#define VPOX_VGA_MINI_PORT_MAGIC        0x19200102
/** VPOXVGAMINIPORT::u32Magic dead value. */
#define VPOX_VGA_MINI_PORT_MAGIC_DEAD   0x19920406
...
VPoxVgaMiniPortDB_Supported(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
                            IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath OPTIONAL);
static EFI_STATUS EFIAPI
VPoxVgaMiniPortDB_Start(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
                        IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath OPTIONAL);
static EFI_STATUS EFIAPI
VPoxVgaMiniPortDB_Stop(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
                       IN UINTN NumberOfChildren, IN EFI_HANDLE *ChildHandleBuffer OPTIONAL);


static EFI_STATUS EFIAPI
VPoxVgaMiniPortVMP_SetMode(IN EFI_VGA_MINI_PORT_PROTOCOL *This, IN UINTN ModeNumber);


static EFI_STATUS EFIAPI
VPoxVgaMiniPortCN_GetDriverName(IN EFI_COMPONENT_NAME_PROTOCOL *This,
                                IN CHAR8 *Language, OUT CHAR16 **DriverName);
static EFI_STATUS EFIAPI
VPoxVgaMiniPortCN_GetControllerName(IN EFI_COMPONENT_NAME_PROTOCOL *This,
...
static EFI_STATUS EFIAPI
VPoxVgaMiniPortCN2_GetDriverName(IN EFI_COMPONENT_NAME2_PROTOCOL *This,
                                 IN CHAR8 *Language, OUT CHAR16 **DriverName);
static EFI_STATUS EFIAPI
VPoxVgaMiniPortCN2_GetControllerName(IN EFI_COMPONENT_NAME2_PROTOCOL *This,
                                     IN EFI_HANDLE ControllerHandle,
                                     IN EFI_HANDLE ChildHandle OPTIONAL,
                                     IN CHAR8 *Language,  OUT CHAR16 **ControllerName);
...
static EFI_DRIVER_BINDING_PROTOCOL          g_VPoxVgaMiniPortDB =
{
    VPoxVgaMiniPortDB_Supported,
    VPoxVgaMiniPortDB_Start,
    VPoxVgaMiniPortDB_Stop,
...
static const EFI_COMPONENT_NAME_PROTOCOL    g_VPoxVgaMiniPortCN =
{
    VPoxVgaMiniPortCN_GetDriverName,
    VPoxVgaMiniPortCN_GetControllerName,
    "eng"
};

/** EFI Component Name 2 Protocol. */
static const EFI_COMPONENT_NAME2_PROTOCOL   g_VPoxVgaMiniPortCN2 =
{
    VPoxVgaMiniPortCN2_GetDriverName,
    VPoxVgaMiniPortCN2_GetControllerName,
    "en"
};

/** Driver name translation table. */
static CONST EFI_UNICODE_STRING_TABLE       g_aVPoxMiniPortDriverLangAndNames[] =
{
 ...
 */
EFI_STATUS EFIAPI
DxeInitializeVPoxVgaMiniPort(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
    EFI_STATUS  rc;
    DEBUG((DEBUG_INFO, "DxeInitializeVPoxVgaMiniPort\n"));

    rc = EfiLibInstallDriverBindingComponentName2(ImageHandle, SystemTable,
                                                  &g_VPoxVgaMiniPortDB, ImageHandle,
                                                  &g_VPoxVgaMiniPortCN, &g_VPoxVgaMiniPortCN2);
    ASSERT_EFI_ERROR(rc);
    return rc;
}
                        ...
                        IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath OPTIONAL)
{
    ...
    if (!EFI_ERROR(rc))
    {
         ...
         */
        PVPOXVGAMINIPORT pThisDev;
        rc = gBS->AllocatePool(EfiBootServicesData, sizeof(*pThisDev), (VOID **)&pThisDev);
        if (!EFI_ERROR(rc))
        {
            pThisDev->VgaMiniPort.SetMode                   = VPoxVgaMiniPortVMP_SetMode;
            ...
            pThisDev->u32Magic                              = VPOX_VGA_MINI_PORT_MAGIC;
                       ...
                       IN UINTN NumberOfChildren, IN EFI_HANDLE *ChildHandleBuffer OPTIONAL)
{
    ...
    if (!EFI_ERROR(rc))
    {
        ASSERT(pThisDev->u32Magic == VPOX_VGA_MINI_PORT_MAGIC);
        ASSERT(pThisDev->hController == ControllerHandle);
        if (    pThisDev->u32Magic == VPOX_VGA_MINI_PORT_MAGIC
            &&  pThisDev->hController == ControllerHandle)
        {
            /*
            ...
            if (!EFI_ERROR(rc))
            {
                 ...
                 */
                pThisDev->u32Magic = VPOX_VGA_MINI_PORT_MAGIC_DEAD;
                gBS->FreePool(pThisDev);
...
VPoxVgaMiniPortVMP_SetMode(IN EFI_VGA_MINI_PORT_PROTOCOL *This, IN UINTN ModeNumber)
{
    PVPOXVGAMINIPORT    pThisDev = (PVPOXVGAMINIPORT)This;
    UINT8               r[64];
    int                 i;
    ...
    if (pThisDev->u32Magic != VPOX_VGA_MINI_PORT_MAGIC)
    {
        DEBUG((DEBUG_INFO, "%a: u32Magic=%x/%x\n", __FUNCTION__, pThisDev->u32Magic, VPOX_VGA_MINI_PORT_MAGIC));
...
#define BOUTB(count, aport, dport)                                  \
    ...
    /* set all VPox extended registers */
                                ...
                                IN CHAR8 *Language, OUT CHAR16 **DriverName)
{
    return LookupUnicodeString2(Language,
                                ...
                                &g_aVPoxMiniPortDriverLangAndNames[0],
                                DriverName,
                                TRUE);
}
