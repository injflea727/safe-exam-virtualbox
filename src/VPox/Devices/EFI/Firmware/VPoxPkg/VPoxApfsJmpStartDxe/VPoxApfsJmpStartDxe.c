/* $Id: VPoxApfsJmpStartDxe.c $ */
/** @file
 * VPoxApfsJmpStartDxe.c - VirtualPox APFS jumpstart driver.
 */
...
static CONST EFI_UNICODE_STRING_TABLE       g_aVPoxApfsJmpStartDriverLangAndNames[] =
{
    {   "eng;en",   L"VPox APFS Jumpstart Wrapper Driver" },
    {   NULL,       NULL }
};
 ...
 */
static EFI_STATUS vpoxApfsJmpStartRead(IN PAPFSJMPSTARTCTX pCtx, IN APFSPADDR offRead, IN void *pvBuf, IN size_t cbRead)
{
 ...
 */
static BOOLEAN vpoxApfsObjPhysIsChksumValid(PCAPFSOBJPHYS pObjHdr, void *pvStruct, size_t cbStruct)
{
    if (cbStruct % sizeof(uint32_t) == 0)
    {
        ...
        if (!CompareMem(&u64ChksumFletcher64, &pObjHdr->abChkSum[0], sizeof(pObjHdr->abChkSum)))
            ...
            DEBUG((DEBUG_INFO, "vpoxApfsObjPhysIsChksumValid: Checksum mismatch, expected 0x%llx got 0x%llx", u64ChksumFletcher64, *(uint64_t *)&pObjHdr->abChkSum[0]));
    }
    else
        DEBUG((DEBUG_INFO, "vpoxApfsObjPhysIsChksumValid: Structure not a multiple of 32bit\n"));
 ...
 */
static EFI_STATUS vpoxApfsJmpStartLoadAndExecEfiDriver(IN PAPFSJMPSTARTCTX pCtx, IN PCAPFSEFIJMPSTART pJmpStart)
{
    ...
    if (pvApfsDrv)
    {
        ...
        for (i = 0; i < RT_LE2H_U32(pJmpStart->cExtents) && !EFI_ERROR(rc) && cbReadLeft; i++)
        {
            ...
            rc = vpoxApfsJmpStartRead(pCtx, RT_LE2H_U64(paExtents[i].PAddrStart), pbBuf, cbRead);
            pbBuf      += cbRead;
            cbReadLeft -= cbRead;
        }
                    ...
                    if (!EFI_ERROR(rc))
                    {
                        ...
                        if (pApfsDrvLoadedInfo)
                        {
                            ...
                            else
                            {
                                ...
                                DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Failed to install APFS driver loaded info protocol with %r\n", rc));
                            }
                        }
                        else
                        {
                            DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Failed to allocate %u bytes for the driver loaded structure\n", sizeof(APFS_DRV_LOADED_INFO)));
                            rc = EFI_OUT_OF_RESOURCES;
                        }
                    }
                    else
                        DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Starting APFS driver failed with %r\n", rc));

                    gBS->UnloadImage(hImage);
                }
                else
                    DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Loading read image failed with %r\n", rc));
            }
            else
                DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Querying device path protocol failed with %r\n", rc));
        }
        else
            DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Reading the jump start extents failed with %r\n", rc));

        FreePool(pvApfsDrv);
    }
    else
    {
        DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Failed to allocate %u bytes for the APFS driver image\n", cbReadLeft));
        rc = EFI_OUT_OF_RESOURCES;
    }
...
VPoxApfsJmpStart_Supported(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
 ...
 */
static EFI_STATUS EFIAPI
VPoxApfsJmpStart_Start(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
                       IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath OPTIONAL)
{
    APFSJMPSTARTCTX Ctx;
    ...
    if (!EFI_ERROR(rc))
    {
        ...
        if (!EFI_ERROR(rc))
        {
            ...
            rc = vpoxApfsJmpStartRead(&Ctx, 0, &Sb, sizeof(Sb));
            if (   !EFI_ERROR(rc)
                && RT_LE2H_U32(Sb.u32Magic) == APFS_NX_SUPERBLOCK_MAGIC)
            {
                ...
                if (pbBlock)
                {
                    ...
                    rc = vpoxApfsJmpStartRead(&Ctx, 0, pbBlock, Ctx.cbBlock);
                    if (   !EFI_ERROR(rc)
                        && RT_LE2H_U64(Sb.PAddrEfiJmpStart) > 0
                        && vpoxApfsObjPhysIsChksumValid(&pSb->ObjHdr, pbBlock, Ctx.cbBlock))
                    {
                        PCAPFSEFIJMPSTART pJmpStart = (PCAPFSEFIJMPSTART)pbBlock;

                        DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Found APFS superblock, reading jumpstart structure from %llx\n", RT_LE2H_U64(Sb.PAddrEfiJmpStart)));

                        CopyMem(&Ctx.Uuid, &pSb->Uuid, sizeof(Ctx.Uuid));

                        rc = vpoxApfsJmpStartRead(&Ctx, RT_LE2H_U64(Sb.PAddrEfiJmpStart), pbBlock, Ctx.cbBlock);
                        if (   !EFI_ERROR(rc)
                            && RT_H2LE_U32(pJmpStart->u32Magic) == APFS_EFIJMPSTART_MAGIC
                            && RT_H2LE_U32(pJmpStart->u32Version) == APFS_EFIJMPSTART_VERSION
                            && vpoxApfsObjPhysIsChksumValid(&pJmpStart->ObjHdr, pbBlock, Ctx.cbBlock)
                            && RT_H2LE_U32(pJmpStart->cExtents) <= (Ctx.cbBlock - sizeof(*pJmpStart)) / sizeof(APFSPRANGE))
                            rc = vpoxApfsJmpStartLoadAndExecEfiDriver(&Ctx, pJmpStart);
                        else
                        {
                            rc = EFI_UNSUPPORTED;
                            DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: The APFS EFI jumpstart structure is invalid\n"));
                        }
                    }
                    else
                    {
                        DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Invalid APFS superblock -> no APFS filesystem (%r %x %llx)\n", rc, Sb.u32Magic, Sb.PAddrEfiJmpStart));
                        rc = EFI_UNSUPPORTED;
                    }
                    ...
                    DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Failed to allocate memory for APFS block data (%u bytes)\n", RT_LE2H_U32(Sb.cbBlock)));
            }
            else
                DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Invalid APFS superblock -> no APFS filesystem (%r %x)\n", rc, Sb.u32Magic));
            ...
            DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Opening the Disk I/O protocol failed with %r\n", rc));
        ...
        DEBUG((DEBUG_INFO, "VPoxApfsJmpStart: Opening the Block I/O protocol failed with %r\n", rc));
 ...
 */
static EFI_STATUS EFIAPI
VPoxApfsJmpStart_Stop(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
...
static EFI_STATUS EFIAPI
VPoxApfsJmpStartCN_GetDriverName(IN EFI_COMPONENT_NAME_PROTOCOL *This,
                                 IN CHAR8 *Language, OUT CHAR16 **DriverName)
{
    return LookupUnicodeString2(Language,
                                This->SupportedLanguages,
                                &g_aVPoxApfsJmpStartDriverLangAndNames[0],
                                DriverName,
                                TRUE);
}
...
static EFI_STATUS EFIAPI
VPoxApfsJmpStartCN_GetControllerName(IN EFI_COMPONENT_NAME_PROTOCOL *This,
...
static EFI_STATUS EFIAPI
VPoxApfsJmpStartCN2_GetDriverName(IN EFI_COMPONENT_NAME2_PROTOCOL *This,
                        IN CHAR8 *Language, OUT CHAR16 **DriverName)
{
    return LookupUnicodeString2(Language,
                                This->SupportedLanguages,
...
static EFI_STATUS EFIAPI
VPoxApfsJmpStartCN2_GetControllerName(IN EFI_COMPONENT_NAME2_PROTOCOL *This,
                                      IN EFI_HANDLE ControllerHandle,
                                      IN EFI_HANDLE ChildHandle OPTIONAL,
                                      IN CHAR8 *Language, OUT CHAR16 **ControllerName)
{
...
static EFI_DRIVER_BINDING_PROTOCOL          g_VPoxApfsJmpStartDB =
{
    VPoxApfsJmpStart_Supported,
    VPoxApfsJmpStart_Start,
    VPoxApfsJmpStart_Stop,
    /* .Version             = */    1,
    /* .ImageHandle         = */ NULL,
...
static const EFI_COMPONENT_NAME_PROTOCOL    g_VPoxApfsJmpStartCN =
{
    VPoxApfsJmpStartCN_GetDriverName,
    VPoxApfsJmpStartCN_GetControllerName,
    "eng"
};

/** EFI Component Name 2 Protocol. */
static const EFI_COMPONENT_NAME2_PROTOCOL   g_VPoxApfsJmpStartCN2 =
{
    VPoxApfsJmpStartCN2_GetDriverName,
    VPoxApfsJmpStartCN2_GetControllerName,
    "en"
};


/**
 * VPoxApfsJmpStart entry point.
 *
 * @returns EFI status code.
 *
 ...
 */
EFI_STATUS EFIAPI
VPoxApfsjmpStartEntryDxe(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
    EFI_STATUS  rc;
    DEBUG((DEBUG_INFO, "VPoxApfsjmpStartEntryDxe\n"));

    rc = EfiLibInstallDriverBindingComponentName2(ImageHandle, SystemTable,
                                                  &g_VPoxApfsJmpStartDB, ImageHandle,
                                                  &g_VPoxApfsJmpStartCN, &g_VPoxApfsJmpStartCN2);
    ASSERT_EFI_ERROR(rc);
    return rc;
}

EFI_STATUS EFIAPI
VPoxApfsjmpStartUnloadDxe(IN EFI_HANDLE         ImageHandle)
{
    return EFI_SUCCESS;
}
