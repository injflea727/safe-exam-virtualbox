/* $Id: VPoxServiceControl.cpp $ */
/** @file
 * VPoxServiceControl - Host-driven Guest Control.
 */
...
/** @page pg_vgsvc_gstctrl VPoxService - Guest Control
 *
 ...
 * operates directly as the VPoxService user (root / system service account).
 *
 ...
 * directly from VPoxManage by the user by prepending the 'vpox_' prefix to the
 * unix command.
 *
 */
...
#endif
/** List of guest control session threads (VPOXSERVICECTRLSESSIONTHREAD).
 ...
 *  under behalf of the VPoxService main process. On newer protocol versions
 *  each session is a forked version of VPoxService using the appropriate
 *  user credentials for opening a guest session. These forked sessions then
 *  are kept in VPOXSERVICECTRLSESSIONTHREAD structures. */
VPOXSERVICECTRLSESSION      g_Session;
...
static DECLCALLBACK(int) vgsvcGstCtrlOption(const char **ppszShort, int argc, char **argv, int *pi)
{
    ...
    {
        g_Session.fFlags |= VPOXSERVICECTRLSESSION_FLAG_DUMPSTDOUT;
        rc = 0; /* Flag this command as parsed. */
    }
    else if (!strcmp(argv[*pi], "--control-dump-stderr"))
    {
        g_Session.fFlags |= VPOXSERVICECTRLSESSION_FLAG_DUMPSTDERR;
        rc = 0; /* Flag this command as parsed. */
    }
#endif
    return rc;
...
static DECLCALLBACK(int) vgsvcGstCtrlInit(void)
{
    ...
    VbglR3GetSessionId(&g_idControlSession); /* The status code is ignored as this information is not available with VPox < 3.2.10. */
...
static int vgsvcGstCtrlInvalidate(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        const uint64_t fGuestFeatures = VPOX_GUESTCTRL_GF_0_SET_SIZE
                                      | VPOX_GUESTCTRL_GF_0_PROCESS_ARGV0
                                      | VPOX_GUESTCTRL_GF_0_PROCESS_DYNAMIC_SIZES;
...
static int vgsvcGstCtrlHandleSessionClose(PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        PVPOXSERVICECTRLSESSIONTHREAD pThread;
        RTListForEach(&g_lstControlSessionThreads, pThread, VPOXSERVICECTRLSESSIONTHREAD, Node)
        {
