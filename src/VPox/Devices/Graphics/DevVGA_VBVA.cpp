/* $Id: DevVGA_VBVA.cpp $ */
/** @file
 * VirtualPox Video Acceleration (VBVA).
 */
...
{
    uint32_t cViews;
    VBVAVIEW aViews[VPOX_VIDEO_MAX_SCREENS];
    VBVAMOUSESHAPEINFO mouseShapeInfo;
    bool fPaused;
    VBVAMODEHINT aModeHints[VPOX_VIDEO_MAX_SCREENS];
} VBVACONTEXT;
                                 ...
                                 const VBVAMOUSEPOINTERSHAPE RT_UNTRUSTED_VOLATILE_GUEST *pShape, HGSMISIZE cbShape)
{
    ...
    const bool fVisible = RT_BOOL(SafeShape.fu32Flags & VPOX_MOUSE_POINTER_VISIBLE);
    const bool fAlpha =   RT_BOOL(SafeShape.fu32Flags & VPOX_MOUSE_POINTER_ALPHA);
    const bool fShape =   RT_BOOL(SafeShape.fu32Flags & VPOX_MOUSE_POINTER_SHAPE);
...
#define VPOXVBVASAVEDSTATE_VHWAAVAILABLE_MAGIC   0x12345678
#define VPOXVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC 0x9abcdef0

#ifdef VPOX_WITH_VIDEOHWACCEL

static void vbvaVHWAHHCommandReinit(VPOXVHWACMD* pHdr, VPOXVHWACMD_TYPE enmCmd, int32_t iDisplay)
{
    memset(pHdr, 0, VPOXVHWACMD_HEADSIZE());
    pHdr->cRefs = 1;
    pHdr->iDisplay = iDisplay;
    pHdr->rc = VERR_NOT_IMPLEMENTED;
    pHdr->enmCmd = enmCmd;
    pHdr->Flags = VPOXVHWACMD_FLAG_HH_CMD;
}

static VPOXVHWACMD *vbvaVHWAHHCommandCreate(VPOXVHWACMD_TYPE enmCmd, int32_t iDisplay, VPOXVHWACMD_LENGTH cbCmd)
{
    VPOXVHWACMD *pHdr = (VPOXVHWACMD *)RTMemAllocZ(cbCmd + VPOXVHWACMD_HEADSIZE());
    Assert(pHdr);
    if (pHdr)
...
DECLINLINE(void) vbvaVHWAHHCommandRelease(VPOXVHWACMD *pCmd)
{
...
DECLINLINE(void) vbvaVHWAHHCommandRetain(VPOXVHWACMD *pCmd)
{
    ASMAtomicIncU32(&pCmd->cRefs);
}

static void vbvaVHWACommandComplete(PVGASTATECC pThisCC, VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCommand, bool fAsyncCommand)
{
    if (fAsyncCommand)
    {
        Assert(pCommand->Flags & VPOXVHWACMD_FLAG_HG_ASYNCH);
    ...
    else
    {
        ...
        pCommand->Flags &= ~VPOXVHWACMD_FLAG_HG_ASYNCH;
    }
...
static void vbvaVHWACommandCompleteAllPending(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC, int rc)
{
    ...
    VPOX_VHWA_PENDINGCMD *pIter, *pNext;
    RTListForEachSafe(&pThis->pendingVhwaCommands.PendingList, pIter, pNext, VPOX_VHWA_PENDINGCMD, Node)
    {
...
static void vbvaVHWACommandPend(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC,
                                VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCommand)
{
    int rc = VERR_BUFFER_OVERFLOW;

    if (ASMAtomicUoReadU32(&pThis->pendingVhwaCommands.cPending) < VPOX_VHWA_MAX_PENDING_COMMANDS)
    {
        VPOX_VHWA_PENDINGCMD *pPend = (VPOX_VHWA_PENDINGCMD *)RTMemAlloc(sizeof(*pPend));
        if (pPend)
        {
            pCommand->Flags |= VPOXVHWACMD_FLAG_HG_ASYNCH;
...
static bool vbvaVHWACommandCanPend(VPOXVHWACMD_TYPE enmCmd)
{
    switch (enmCmd)
    {
...
static int vbvaVHWACommandSavePending(PCPDMDEVHLPR3 pHlp, PVGASTATE pThis, PVGASTATECC pThisCC, PSSMHANDLE pSSM)
{
    ...
    VPOX_VHWA_PENDINGCMD *pIter;
    RTListForEach(&pThis->pendingVhwaCommands.PendingList, pIter, VPOX_VHWA_PENDINGCMD, Node)
    {
                                      ...
                                      PSSMHANDLE pSSM, uint32_t u32Version)
{
    ...
    for (uint32_t i = 0; i < u32; ++i)
    {
        ...
        AssertRCReturn(rc, rc);
        VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCommand
            = (VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *)((uint8_t volatile *)pThisCC->pbVRam + off32);
...
static bool vbvaVHWACommandSubmitInner(PVGASTATE pThis, PVGASTATECC pThisCC,
                                       VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCommand, bool *pfPending)
{
    *pfPending = false;
     ...
     */
    VPOXVHWACMD_TYPE enmCmd = pCommand->enmCmd;
    RT_UNTRUSTED_NONVOLATILE_COPY_FENCE();

    bool fGuestCmd = (uintptr_t)pCommand - (uintptr_t)pThisCC->pbVRam < pThis->vram_size;
    ASSERT_GUEST_LOGREL_MSG_STMT_RETURN(   !fGuestCmd
                                        || (   enmCmd != VPOXVHWACMD_TYPE_HH_CONSTRUCT
                                            && enmCmd != VPOXVHWACMD_TYPE_HH_RESET
                                            && enmCmd != VPOXVHWACMD_TYPE_HH_DISABLE
                                            && enmCmd != VPOXVHWACMD_TYPE_HH_ENABLE
                                            && enmCmd != VPOXVHWACMD_TYPE_HH_SAVESTATE_SAVEBEGIN
                                            && enmCmd != VPOXVHWACMD_TYPE_HH_SAVESTATE_SAVEEND
                                            && enmCmd != VPOXVHWACMD_TYPE_HH_SAVESTATE_SAVEPERFORM
                                            && enmCmd != VPOXVHWACMD_TYPE_HH_SAVESTATE_LOADPERFORM),
                                        ("enmCmd=%d\n", enmCmd),
...
static bool vbvaVHWACommandSubmit(PVGASTATE pThis, PVGASTATECC pThisCC,
                                  VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCommand, bool fAsyncCommand)
{
    bool fPending = false;
...
static void vbvaVHWAHandleCommand(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC,
                                  VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCmd)
{
...
static int vbvaVHWAHHCommandPost(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC, VPOXVHWACMD *pCmd)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        VPOXVHWA_HH_CALLBACK_SET(pCmd, vbvaVHWAHHCommandSetEventCallback, (void *)hComplEvent);
        vbvaVHWAHandleCommand(pDevIns, pThis, pThisCC, pCmd);

        if ((ASMAtomicReadU32((volatile uint32_t *)&pCmd->Flags) & VPOXVHWACMD_FLAG_HG_ASYNCH) != 0)
...
int vbvaVHWAConstruct(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC)
{
    ...
    VPOXVHWACMD *pCmd = vbvaVHWAHHCommandCreate(VPOXVHWACMD_TYPE_HH_CONSTRUCT, 0, sizeof(VPOXVHWACMD_HH_CONSTRUCT));
    Assert(pCmd);
    if(pCmd)
    {
        uint32_t iDisplay = 0;
        ...
        for (;;)
        {
            memset(pBody, 0, sizeof(VPOXVHWACMD_HH_CONSTRUCT));
            ...
            if (iDisplay >= pThis->cMonitors)
                break;
            vbvaVHWAHHCommandReinit(pCmd, VPOXVHWACMD_TYPE_HH_CONSTRUCT, (int32_t)iDisplay);
        }
...
static int vbvaVHWAReset(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC)
{
    ...
    VPOXVHWACMD *pCmd = vbvaVHWAHHCommandCreate(VPOXVHWACMD_TYPE_HH_RESET, 0, 0);
    Assert(pCmd);
    if (pCmd)
    {
        int rc = VINF_SUCCESS;
        ...
        do
        {
            ...
            if (iDisplay >= pThis->cMonitors)
                break;
            vbvaVHWAHHCommandReinit(pCmd, VPOXVHWACMD_TYPE_HH_RESET, (int32_t)iDisplay);
...
typedef DECLCALLBACK(bool) FNVPOXVHWAHHCMDPRECB(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC, VPOXVHWACMD *pCmd,
                                                uint32_t iDisplay, void *pvContext);
typedef FNVPOXVHWAHHCMDPRECB *PFNVPOXVHWAHHCMDPRECB;

typedef DECLCALLBACK(bool) FNVPOXVHWAHHCMDPOSTCB(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC, VPOXVHWACMD *pCmd,
                                                 uint32_t iDisplay, int rc, void *pvContext);
typedef FNVPOXVHWAHHCMDPOSTCB *PFNVPOXVHWAHHCMDPOSTCB;

static int vbvaVHWAHHPost(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC, VPOXVHWACMD *pCmd,
                          PFNVPOXVHWAHHCMDPRECB pfnPre, PFNVPOXVHWAHHCMDPOSTCB pfnPost, void *pvContext)
{
    const VPOXVHWACMD_TYPE enmType = pCmd->enmCmd;
    int rc = VINF_SUCCESS;
    uint32_t iDisplay = 0;
...
{
    const VPOXVHWACMD_TYPE enmType = bEnable ? VPOXVHWACMD_TYPE_HH_ENABLE : VPOXVHWACMD_TYPE_HH_DISABLE;
    VPOXVHWACMD *pCmd = vbvaVHWAHHCommandCreate(enmType, 0, 0);
    Assert(pCmd);
    if(pCmd)
    {
...
int vpoxVBVASaveStatePrep(PPDMDEVINS pDevIns)
{
...
int vpoxVBVASaveStateDone(PPDMDEVINS pDevIns)
{
    /* ensure we have no pending commands */
                                                 ...
                                                 VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *pCmd)
{
    ...
    if ((uintptr_t)pCmd - (uintptr_t)pThisCC->pbVRam < pThis->vram_size)
    {
        ...
        Assert(!(pCmd->Flags & VPOXVHWACMD_FLAG_HH_CMD));
        Assert(pCmd->Flags & VPOXVHWACMD_FLAG_HG_ASYNCH);
#ifdef VPOX_WITH_WDDM
        if (pThis->fGuestCaps & VBVACAPS_COMPLETEGCMD_BY_IOREAD)
        {
            rc = HGSMICompleteGuestCommand(pIns, pCmd, !!(pCmd->Flags & VPOXVHWACMD_FLAG_GH_ASYNCH_IRQ));
            AssertRC(rc);
        }
        else
#endif
        {
            ...
            if (pCmd->Flags & VPOXVHWACMD_FLAG_GH_ASYNCH_EVENT)
            {
            ...
            if (RT_SUCCESS(rc))
            {
                rc = HGSMIHostCommandSubmitAndFreeAsynch(pIns, pHostCmd, RT_BOOL(pCmd->Flags & VPOXVHWACMD_FLAG_GH_ASYNCH_IRQ));
                AssertRC(rc);
                if (RT_SUCCESS(rc))
                    return rc;
    ...
    {
        Assert(pCmd->Flags & VPOXVHWACMD_FLAG_HH_CMD);
        PFNVPOXVHWA_HH_CALLBACK pfn = VPOXVHWA_HH_CALLBACK_GET(pCmd);
        if (pfn)
            pfn(VPOXVHWA_HH_CALLBACK_GET_ARG(pCmd));
        rc = VINF_SUCCESS;
    }
    return rc;
}

typedef struct VPOXVBVASAVEDSTATECBDATA
{
    PSSMHANDLE pSSM;
    int rc;
    bool ab2DOn[VPOX_VIDEO_MAX_SCREENS];
} VPOXVBVASAVEDSTATECBDATA, *PVPOXVBVASAVEDSTATECBDATA;

/**
 * @callback_method_impl{FNVPOXVHWAHHCMDPOSTCB}
 */
static DECLCALLBACK(bool) vpoxVBVASaveStateBeginPostCb(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC,
                                                       VPOXVHWACMD *pCmd, uint32_t iDisplay, int rc, void *pvContext)
{
    RT_NOREF(pDevIns, pThis, pThisCC, pCmd);
    PVPOXVBVASAVEDSTATECBDATA pData = (PVPOXVBVASAVEDSTATECBDATA)pvContext;
...
/**
 * @callback_method_impl{FNVPOXVHWAHHCMDPRECB}
 */
static DECLCALLBACK(bool) vpoxVBVASaveStatePerformPreCb(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC,
                                                        VPOXVHWACMD *pCmd, uint32_t iDisplay, void *pvContext)
{
    ...
    {
        rc = pHlp->pfnSSMPutU32(pData->pSSM, VPOXVBVASAVEDSTATE_VHWAAVAILABLE_MAGIC); AssertRC(rc);
        if (RT_FAILURE(rc))
        {
            pData->rc = rc;
            return false;
    ...
    rc = pHlp->pfnSSMPutU32(pData->pSSM, VPOXVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC); AssertRC(rc);
    if (RT_FAILURE(rc))
    {
        pData->rc = rc;
        return false;
 ...
 * @callback_method_impl{FNVPOXVHWAHHCMDPOSTCB}
 */
static DECLCALLBACK(bool) vpoxVBVASaveStateEndPreCb(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC,
                                                    VPOXVHWACMD *pCmd, uint32_t iDisplay, void *pvContext)
{
    RT_NOREF(pDevIns, pThis, pThisCC, pCmd);
    PVPOXVBVASAVEDSTATECBDATA pData = (PVPOXVBVASAVEDSTATECBDATA)pvContext;
 ...
 * @callback_method_impl{FNVPOXVHWAHHCMDPOSTCB}
 */
static DECLCALLBACK(bool) vpoxVBVALoadStatePerformPostCb(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC,
                                                         VPOXVHWACMD *pCmd, uint32_t iDisplay, int rc, void *pvContext)
{
    RT_NOREF(pThis, pThisCC, pCmd);
    PVPOXVBVASAVEDSTATECBDATA pData = (PVPOXVBVASAVEDSTATECBDATA)pvContext;
 ...
 * @callback_method_impl{FNVPOXVHWAHHCMDPOSTCB}
 */
static DECLCALLBACK(bool) vpoxVBVALoadStatePerformPreCb(PPDMDEVINS pDevIns, PVGASTATE pThis, PVGASTATECC pThisCC,
                                                        VPOXVHWACMD *pCmd, uint32_t iDisplay, void *pvContext)
{
    RT_NOREF(pThis, pThisCC, pCmd);
    PVPOXVBVASAVEDSTATECBDATA pData = (PVPOXVBVASAVEDSTATECBDATA)pvContext;
    ...
    switch (u32)
    {
        case VPOXVBVASAVEDSTATE_VHWAAVAILABLE_MAGIC:
            pData->ab2DOn[iDisplay] = true;
            return true;
        case VPOXVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC:
...
static int vpoxVBVASaveDevStateExec(PCPDMDEVHLPR3 pHlp, PVGASTATE pThis, PVGASTATECC pThisCC, PSSMHANDLE pSSM)
{
    PHGSMIINSTANCE pIns = pThisCC->pHGSMI;
...
int vpoxVBVASaveStateExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM)
{
    PVGASTATE       pThis   = PDMDEVINS_2_DATA(pDevIns, PVGASTATE);
    ...
    int rc;
#ifdef VPOX_WITH_VIDEOHWACCEL
    VPOXVBVASAVEDSTATECBDATA VhwaData = {0};
    VhwaData.pSSM = pSSM;
    uint32_t cbCmd = sizeof (VPOXVHWACMD_HH_SAVESTATE_SAVEPERFORM); /* maximum cmd size */
    VPOXVHWACMD *pCmd = vbvaVHWAHHCommandCreate(VPOXVHWACMD_TYPE_HH_SAVESTATE_SAVEBEGIN, 0, cbCmd);
    Assert(pCmd);
    if (pCmd)
    {
        vbvaVHWAHHPost(pDevIns, pThis, pThisCC, pCmd, NULL, vpoxVBVASaveStateBeginPostCb, &VhwaData);
        rc = VhwaData.rc;
        AssertRC(rc);
        if (RT_SUCCESS(rc))
        {
#endif
            rc = vpoxVBVASaveDevStateExec(pHlp, pThis, pThisCC, pSSM);
            AssertRC(rc);
#ifdef VPOX_WITH_VIDEOHWACCEL
            if (RT_SUCCESS(rc))
            {
                vbvaVHWAHHCommandReinit(pCmd, VPOXVHWACMD_TYPE_HH_SAVESTATE_SAVEPERFORM, 0);
                VPOXVHWACMD_HH_SAVESTATE_SAVEPERFORM *pSave = VPOXVHWACMD_BODY_HOST_HEAP(pCmd, VPOXVHWACMD_HH_SAVESTATE_SAVEPERFORM);
                pSave->pSSM = pSSM;
                vbvaVHWAHHPost(pDevIns, pThis, pThisCC, pCmd, vpoxVBVASaveStatePerformPreCb, NULL, &VhwaData);
                rc = VhwaData.rc;
                AssertRC(rc);
                if (RT_SUCCESS(rc))
                {
                    ...
                    vbvaVHWAHHCommandReinit(pCmd, VPOXVHWACMD_TYPE_HH_SAVESTATE_SAVEEND, 0);
                    vbvaVHWAHHPost(pDevIns, pThis, pThisCC, pCmd, vpoxVBVASaveStateEndPreCb, NULL, &VhwaData);
                    rc = VhwaData.rc;
                    AssertRC(rc);
                }
            }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            rc = pHlp->pfnSSMPutU32(pSSM, VPOXVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC);
            AssertRCReturn(rc, rc);
        }
    }
...
int vpoxVBVALoadStateExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion)
{
        ...
        else
        {
            ...
            if (uVersion > VGA_SAVEDSTATE_VERSION_WDDM)
            {
...
#ifdef VPOX_WITH_VIDEOHWACCEL
                uint32_t cbCmd = sizeof (VPOXVHWACMD_HH_SAVESTATE_LOADPERFORM); /* maximum cmd size */
                VPOXVHWACMD *pCmd = vbvaVHWAHHCommandCreate(VPOXVHWACMD_TYPE_HH_SAVESTATE_LOADPERFORM, 0, cbCmd);
                Assert(pCmd);
                if(pCmd)
                {
                    VPOXVBVASAVEDSTATECBDATA VhwaData = {0};
                    ...
                    pLoad->pSSM = pSSM;
                    vbvaVHWAHHPost(pDevIns, pThis, pThisCC, pCmd, vpoxVBVALoadStatePerformPreCb,
                                   vpoxVBVALoadStatePerformPostCb, &VhwaData);
                    rc = VhwaData.rc;
                    vbvaVHWAHHCommandRelease(pCmd);
                    AssertRCReturn(rc, rc);
                ...
                for (uint32_t i = 0; i < pThis->cMonitors; ++i)
                {
                    ...
                    if (u32 != VPOXVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC)
                    {
...
int vpoxVBVALoadStateDone(PPDMDEVINS pDevIns)
{
    PVGASTATE   pThis   = PDMDEVINS_2_DATA(pDevIns, PVGASTATE);
    PVGASTATECC pThisCC = PDMDEVINS_2_DATA_CC(pDevIns, PVGASTATECC);
...
static int vbvaHandleQueryConf32(PVGASTATECC pThisCC, VBVACONF32 RT_UNTRUSTED_VOLATILE_GUEST *pConf32)
{
    ...
    VBVACONTEXT *pCtx = (VBVACONTEXT *)HGSMIContext(pThisCC->pHGSMI);
    uint32_t     uValue;
    if (idxQuery == VPOX_VBVA_CONF32_MONITOR_COUNT)
        uValue = pCtx->cViews;
    else if (idxQuery == VPOX_VBVA_CONF32_HOST_HEAP_SIZE)
        uValue = _64K; /** @todo a value calculated from the vram size */
    else if (   idxQuery == VPOX_VBVA_CONF32_MODE_HINT_REPORTING
             || idxQuery == VPOX_VBVA_CONF32_GUEST_CURSOR_REPORTING)
        uValue = VINF_SUCCESS;
    else if (idxQuery == VPOX_VBVA_CONF32_CURSOR_CAPABILITIES)
        uValue = VPOX_VBVA_CURSOR_CAPABILITY_HARDWARE;
    else if (idxQuery == VPOX_VBVA_CONF32_SCREEN_FLAGS)
        uValue = VBVA_SCREEN_F_ACTIVE
               | VBVA_SCREEN_F_DISABLED
               | VBVA_SCREEN_F_BLANK
               | VBVA_SCREEN_F_BLANK2;
    else if (idxQuery == VPOX_VBVA_CONF32_MAX_RECORD_SIZE)
...
static int vbvaHandleQueryModeHints(PVGASTATECC pThisCC, VBVAQUERYMODEHINTS volatile *pQueryModeHints, HGSMISIZE cbBuffer)
{
    ...
    for (unsigned iHint = 0; iHint < cHintsQueried && iHint < VPOX_VIDEO_MAX_SCREENS; ++iHint)
    {
...
/*
 *
 * New VBVA uses a new interface id: #define VBE_DISPI_ID_VPOX_VIDEO         0xBE01
 *
...
{
#if defined(VPOX_WITH_HGSMI) && (defined(VPOX_WITH_VIDEOHWACCEL) || defined(VPOX_WITH_VDMA) || defined(VPOX_WITH_WDDM))
                                            ...
                                            void RT_UNTRUSTED_VOLATILE_GUEST *pvBuffer, HGSMISIZE cbBuffer)
{
    ...
    switch (u16ChannelInfo)
    {
        ...
        case VBVA_VDMA_CMD:
            if (cbBuffer >= VPoxSHGSMIBufferHeaderSize() + sizeof(VPOXVDMACBUF_DR))
            {
                VPOXVDMACBUF_DR RT_UNTRUSTED_VOLATILE_GUEST *pCmd
                    = (VPOXVDMACBUF_DR RT_UNTRUSTED_VOLATILE_GUEST *)VPoxSHGSMIBufferData((VPOXSHGSMIHEADER RT_UNTRUSTED_VOLATILE_GUEST *)pvBuffer);
                vpoxVDMACommand(pThisCC->pVdma, pCmd, cbBuffer - VPoxSHGSMIBufferHeaderSize());
                rc = VINF_SUCCESS;
            }
            else
                rc = VERR_INVALID_PARAMETER;
        ...
        case VBVA_VDMA_CTL:
            if (cbBuffer >= VPoxSHGSMIBufferHeaderSize() + sizeof(VPOXVDMA_CTL))
            {
                VPOXVDMA_CTL RT_UNTRUSTED_VOLATILE_GUEST *pCmd
                    = (VPOXVDMA_CTL RT_UNTRUSTED_VOLATILE_GUEST *)VPoxSHGSMIBufferData((VPOXSHGSMIHEADER RT_UNTRUSTED_VOLATILE_GUEST *)pvBuffer);
                vpoxVDMAControl(pThisCC->pVdma, pCmd, cbBuffer - VPoxSHGSMIBufferHeaderSize());
            }
            else
                rc = VERR_INVALID_PARAMETER;
            break;
#endif /* VPOX_WITH_VDMA */
...
#ifdef VPOX_WITH_VIDEOHWACCEL
        case VBVA_VHWA_CMD:
            if (cbBuffer >= VPOXVHWACMD_HEADSIZE())
            {
                vbvaVHWAHandleCommand(pDevIns, pThis, pThisCC, (VPOXVHWACMD RT_UNTRUSTED_VOLATILE_GUEST *)pvBuffer);
                rc = VINF_SUCCESS;
            }
            else
                rc = VERR_INVALID_PARAMETER;
