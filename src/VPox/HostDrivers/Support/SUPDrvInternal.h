/* $Id: SUPDrvInternal.h $ */
/** @file
 * VirtualPox Support Driver - Internal header.
 */
...
#ifndef VPOX_INCLUDED_SRC_Support_SUPDrvInternal_h
#define VPOX_INCLUDED_SRC_Support_SUPDrvInternal_h
...
typedef struct SUPDRVLDRIMAGE
{
    ...
    RTR0MEMOBJ                      hMemLock;
#endif
#if defined(RT_OS_SOLARIS) && defined(VPOX_WITH_NATIVE_SOLARIS_LOADING)
...
#endif
#if defined(RT_OS_DARWIN) && defined(VPOX_WITH_DARWIN_R0_DARWIN_IMAGE_VERIFICATION)
    /** Load module handle. */
    RTLDRMOD                        hLdrMod;
    /** Allocate object. */
...
# if defined(RT_OS_DARWIN)
    /** Pointer to the associated org_virtualpox_SupDrvClient object. */
    void                           *pvSupDrvClient;
...
# elif defined(RT_OS_DARWIN) && defined(VPOX_WITH_DARWIN_R0_DARWIN_IMAGE_VERIFICATION)
...
int  VPOXCALL   supdrvOSEnableVTx(bool fEnabled);
 ...
 */
int  VPOXCALL   supdrvOSLdrValidatePointer(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage,
 ...
 *
 * @returns One of the listed VPox status codes.
...
*******************************************************************************/
/* SUPDrv.c */
int  VPOXCALL   supdrvIOCtl(uintptr_t uIOCtl, PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPREQHDR pReqHdr, size_t cbReq);
int  VPOXCALL   supdrvIOCtlFast(uintptr_t uOperation, VMCPUID idCpu, PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession);
int  VPOXCALL   supdrvIDC(uintptr_t uIOCtl, PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPDRVIDCREQHDR pReqHdr);
int  VPOXCALL   supdrvInitDevExt(PSUPDRVDEVEXT pDevExt, size_t cbSession);
void VPOXCALL   supdrvDeleteDevExt(PSUPDRVDEVEXT pDevExt);
int  VPOXCALL   supdrvCreateSession(PSUPDRVDEVEXT pDevExt, bool fUser, bool fUnrestricted,  PSUPDRVSESSION *ppSession);
int  VPOXCALL   supdrvSessionHashTabInsert(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPDRVSESSION *ppOsSessionPtr, void *pvUser);
int  VPOXCALL   supdrvSessionHashTabRemove(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, void *pvUser);
PSUPDRVSESSION VPOXCALL supdrvSessionHashTabLookup(PSUPDRVDEVEXT pDevExt, RTPROCESS Process, RTR0PROCESS R0Process,
                                                   PSUPDRVSESSION *ppOsSessionPtr);
uint32_t VPOXCALL supdrvSessionRetain(PSUPDRVSESSION pSession);
uint32_t VPOXCALL supdrvSessionRelease(PSUPDRVSESSION pSession);
void VPOXCALL   supdrvBadContext(PSUPDRVDEVEXT pDevExt, const char *pszFile, uint32_t uLine, const char *pszExtra);
int VPOXCALL    supdrvQueryVTCapsInternal(uint32_t *pfCaps);
int VPOXCALL    supdrvLdrLoadError(int rc, PSUPLDRLOAD pReq, const char *pszFormat, ...);
int VPOXCALL    supdrvLdrGetExportedSymbol(const char *pszSymbol, uintptr_t *puValue);

/* SUPDrvGip.cpp */
int  VPOXCALL   supdrvGipCreate(PSUPDRVDEVEXT pDevExt);
void VPOXCALL   supdrvGipDestroy(PSUPDRVDEVEXT pDevExt);
int  VPOXCALL   supdrvIOCtl_TscDeltaMeasure(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPTSCDELTAMEASURE pReq);
int  VPOXCALL   supdrvIOCtl_TscRead(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPTSCREAD pReq);
int  VPOXCALL   supdrvIOCtl_GipSetFlags(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, uint32_t fOrMask, uint32_t fAndMask);


/* SUPDrvTracer.cpp */
int  VPOXCALL   supdrvTracerInit(PSUPDRVDEVEXT pDevExt);
void VPOXCALL   supdrvTracerTerm(PSUPDRVDEVEXT pDevExt);
void VPOXCALL   supdrvTracerModuleUnloading(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage);
void VPOXCALL   supdrvTracerCleanupSession(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession);
int  VPOXCALL   supdrvIOCtl_TracerUmodRegister(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession,
                                               RTR3PTR R3PtrVtgHdr, RTUINTPTR uVtgHdrAddr,
                                               RTR3PTR R3PtrStrTab, uint32_t cbStrTab,
                                               const char *pszModName, uint32_t fFlags);
int  VPOXCALL   supdrvIOCtl_TracerUmodDeregister(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, RTR3PTR R3PtrVtgHdr);
void  VPOXCALL  supdrvIOCtl_TracerUmodProbeFire(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPDRVTRACERUSRCTX pCtx);
int  VPOXCALL   supdrvIOCtl_TracerOpen(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, uint32_t uCookie, uintptr_t uArg);
int  VPOXCALL   supdrvIOCtl_TracerClose(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession);
int  VPOXCALL   supdrvIOCtl_TracerIOCtl(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, uintptr_t uCmd, uintptr_t uArg, int32_t *piRetVal);
extern PFNRT    g_pfnSupdrvProbeFireKernel;
DECLASM(void)   supdrvTracerProbeFireStub(void);

#ifdef VPOX_WITH_NATIVE_DTRACE
const SUPDRVTRACERREG * VPOXCALL supdrvDTraceInit(void);
#endif

RT_C_DECLS_END

#endif /* !VPOX_INCLUDED_SRC_Support_SUPDrvInternal_h */
