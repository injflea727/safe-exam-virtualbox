...
/** @page pg_vm     VM API
 *
 * This is the encapsulating bit.  It provides the APIs that Main and VPoxBFE
 ...
 * mostly) and for reporting error back to the VMM user (Main/VPoxBFE).
 *
 *
 ...
 * @returns VPox error code on failure.
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (ASMAtomicCmpXchgBool(&s_fRegisteredProbes, true, false))
                SUPR3TracerRegisterModule(~(uintptr_t)0, "VPoxVMM", &g_VTGObjHeader, (uintptr_t)&g_VTGObjHeader,
             ...
             */
            PVMREQ pReq;
            rc = VMR3ReqCallU(pUVM, VMCPUID_ANY, &pReq, RT_INDEFINITE_WAIT, VMREQFLAGS_VPOX_STATUS,
            ...
            switch (rc)
            {
...
#ifdef RT_OS_LINUX
                    pszError = N_("VirtualPox can't operate in VMX root mode. "
                                  "Please disable the KVM kernel extension, recompile your kernel and reboot");
#else
                    pszError = N_("VirtualPox can't operate in VMX root mode. Please close all other virtualization programs.");
#endif
                    break;
...
#ifdef RT_OS_LINUX
                    pszError = N_("VirtualPox can't enable the AMD-V extension. "
                                  "Please disable the KVM kernel extension, recompile your kernel and reboot");
#else
                    pszError = N_("VirtualPox can't enable the AMD-V extension. Please close all other virtualization programs.");
#endif
                    break;
                ...
                case VERR_SUPDRV_COMPONENT_NOT_FOUND:
                    pszError = N_("One of the kernel modules was not successfully loaded. Make sure "
                                  "that VirtualPox is correctly installed, and if you are using EFI "
                                  ...
                                  "'/sbin/vpoxconfig' as root");
                    break;
#endif

                case VERR_RAW_MODE_INVALID_SMP:
                    pszError = N_("VT-x/AMD-V is either not available on your host or disabled. "
                                  "VirtualPox requires this hardware extension to emulate more than one "
                                  "guest CPU");
                    break;

                case VERR_SUPDRV_KERNEL_TOO_OLD_FOR_VTX:
#ifdef RT_OS_LINUX
                    pszError = N_("Because the host kernel is too old, VirtualPox cannot enable the VT-x "
                ...
                case VERR_PDM_DEVICE_NOT_FOUND:
                    pszError = N_("A virtual device is configured in the VM settings but the device "
                                  ...
                                  "that as of VirtualPox 4.0, certain features (for example USB 2.0 "
...
#ifdef RT_OS_LINUX
                    pszError = N_("VirtualPox kernel driver not loaded. The vpoxdrv kernel module "
                                  "was either not loaded, /dev/vpoxdrv is not set up properly, "
                                  ...
                                  "'/sbin/vpoxconfig' as root");
#else
                    pszError = N_("VirtualPox kernel driver not loaded");
#endif
                    break;
                case VERR_VM_DRIVER_OPEN_ERROR:
                    pszError = N_("VirtualPox kernel driver cannot be opened");
                    break;
                case VERR_VM_DRIVER_NOT_ACCESSIBLE:
#ifdef VPOX_WITH_HARDENING
                    /* This should only happen if the executable wasn't hardened - bad code/build. */
                    pszError = N_("VirtualPox kernel driver not accessible, permission problem. "
                                  "Re-install VirtualPox. If you are building it yourself, you "
                                  "should make sure it installed correctly and that the setuid "
                                  "bit is set on the executables calling VMR3Create.");
#else
                    /* This should only happen when mixing builds or with the usual /dev/vpoxdrv access issues. */
# if defined(RT_OS_DARWIN)
                    pszError = N_("VirtualPox KEXT is not accessible, permission problem. "
                                  "If you have built VirtualPox yourself, make sure that you do not "
                                  "have the vpoxdrv KEXT from a different build or installation loaded.");
# elif defined(RT_OS_LINUX)
                    pszError = N_("VirtualPox kernel driver is not accessible, permission problem. "
                                  "If you have built VirtualPox yourself, make sure that you do "
                                  "not have the vpoxdrv kernel module from a different build or "
                                  "installation loaded. Also, make sure the vpoxdrv udev rule gives "
                                  "you the permission you need to access the device.");
# elif defined(RT_OS_WINDOWS)
                    pszError = N_("VirtualPox kernel driver is not accessible, permission problem.");
# else /* solaris, freebsd, ++. */
                    pszError = N_("VirtualPox kernel module is not accessible, permission problem. "
                                  "If you have built VirtualPox yourself, make sure that you do "
                                  "not have the vpoxdrv kernel module from a different install loaded.");
# endif
#endif
                    break;
...
#ifdef RT_OS_LINUX
                    pszError = N_("VirtualPox kernel driver not Installed. The vpoxdrv kernel module "
                                  "was either not loaded, /dev/vpoxdrv is not set up properly, "
                                  "or you are using EFI Secure Boot and the module is not signed "
                                  "in the right way for your system.  If necessary, try setting up "
                                  "the kernel module again by executing "
                                  "'/sbin/vpoxconfig' as root");
#else
                    pszError = N_("VirtualPox kernel driver not installed");
#endif
                    break;
                case VERR_NO_MEMORY:
                    pszError = N_("VirtualPox support library out of memory");
                    break;
                case VERR_VERSION_MISMATCH:
                case VERR_VM_DRIVER_VERSION_MISMATCH:
                    pszError = N_("The VirtualPox support driver which is running is from a different "
                                  "version of VirtualPox.  You can correct this by stopping all "
                                  "running instances of VirtualPox and reinstalling the software.");
                    break;
                default:
...
static int vmR3CreateU(PUVM pUVM, uint32_t cCpus, PFNCFGMCONSTRUCTOR pfnCFGMConstructor, void *pvUserCFGM)
{
#if (defined(RT_ARCH_AMD64) || defined(RT_ARCH_X86)) && !defined(VPOX_WITH_OLD_CPU_SUPPORT)
    /*
    ...
    {
        LogRel(("vpoxdrv: Requires SSE2 (cpuid(0).EDX=%#x)\n", ASMCpuId_EDX(1)));
...
static int vmR3InitRing0(PVM pVM)
{
     ...
     */
    int rc = VINF_SUCCESS;
    const char *psz = RTEnvGet("VPOX_SUPLIB_FAKE");
    if (!psz || strcmp(psz, "fake"))
    {
        ...
        Log(("vmR3InitRing0: skipping because of VPOX_SUPLIB_FAKE=fake\n"));
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3PowerOn(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 * @param   pvUser          Ignored.
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3Suspend(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 *
 * @returns VPox status code. When called on EMT, this will be a strict status
 ...
 * @param   pvUser          Reason.
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3Resume(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3LiveDoSuspend(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 * @param   pvUser          The pfSuspended argument of vmR3SaveTeleport.
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3LiveDoStep1Cleanup(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
        ...
        if (cErrorsPriorToSave == VMR3GetErrorCount(pUVM))
            rc = VMSetError(pVM, rc, RT_SRC_POS,
                            N_("Unable to restore the virtual machine's saved state from '%s'. "
                               "It may be damaged or from an older version of VirtualPox.  "
 ...
 * @param   pvUser          Ignored.
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3PowerOff(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
...
DECLCALLBACK(int) vmR3Destroy(PVM pVM)
{
    ...
    if (pVCpu->idCpu == 0)
    {
         ...
         */
#if defined(VPOX_WITH_STATISTICS) || defined(LOG_ENABLED)
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3SoftReset(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 * @param   pvUser          Ignored.
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3HardReset(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    Assert(!pvUser); NOREF(pvUser);
 ...
 */
static VPOXSTRICTRC vmR3ResetCommon(PVM pVM, bool fHardReset, uint32_t fResetFlags)
{
...
VMMR3DECL(int) VMR3Reset(PUVM pUVM)
{
    ...
    return VPOXSTRICTRC_VAL(vmR3ResetCommon(pVM, true, 0));
}
 ...
 */
VMMR3_INT_DECL(VPOXSTRICTRC) VMR3ResetFF(PVM pVM)
{
    LogFlow(("VMR3ResetFF:\n"));
 ...
 * @thread  EMT
 */
VMMR3_INT_DECL(VPOXSTRICTRC) VMR3ResetTripleFault(PVM pVM)
{
 ...
 * @param   pvUser          Ignored.
 */
static DECLCALLBACK(VPOXSTRICTRC) vmR3SetRuntimeErrorChangeState(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    NOREF(pVCpu);
    Assert(!pvUser); NOREF(pvUser);
 ...
 *
 * @returns VPox status code with modifications, see VMSetRuntimeErrorV.
 *
 * @param   pVM             The cross context VM structure.
 ...
 * The message is found in VMINT.
 *
 * @returns VPox status code, see VMSetRuntimeError.
 * @param   pVM             The cross context VM structure.
 * @thread  EMT.
 */
