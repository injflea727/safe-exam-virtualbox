...
#
import platform
VPoxBinDir = os.environ.get("VPOX_PROGRAM_PATH", None)
VPoxSdkDir = os.environ.get("VPOX_SDK_PATH", None)

if VPoxBinDir is None:
    if platform.system() == 'Darwin':
        VPoxBinDir = '/Applications/VirtualPox.app/Contents/MacOS'
    else: # Will be set by the installer
        VPoxBinDir = "%VPOX_INSTALL_PATH%"
else:
    VPoxBinDir = os.path.abspath(VPoxBinDir)

if VPoxSdkDir is None:
    if platform.system() == 'Darwin':
        VPoxSdkDir = '/Applications/VirtualPox.app/Contents/MacOS/sdk'
    else: # Will be set by the installer
        VPoxSdkDir = "%VPOX_SDK_PATH%"
else:
    VPoxSdkDir = os.path.abspath(VPoxSdkDir)

os.environ["VPOX_PROGRAM_PATH"] = VPoxBinDir
os.environ["VPOX_SDK_PATH"] = VPoxSdkDir
sys.path.append(VPoxBinDir)


#
# Import the generated VirtualPox constants.
#
from .VirtualPox_constants import VirtualPoxReflectionInfo
...
class PerfCollector(object):
    ...
    def __init__(self, mgr, vpox):
        """ Initializes the instance.

        """
        self.mgr = mgr
        self.isMscom = (mgr.type == 'MSCOM')
        self.collector = vpox.performanceCollector
...
class PlatformBase(object):
    ...
    def getVirtualPox(self):
        """
        Gets a the IVirtualPox singleton.
        """
        return None
    ...
    def getSessionObject(self):
        ...
        The oIVPox parameter is an getVirtualPox() return value, i.e. an
        IVirtualPox reference.
    ...
    def waitForEvents(self, cMsTimeout):
        ...
        that initialized VirtualPoxManager) or if the time isn't an integer.
        """
        _ = cMsTimeout
        return 2
    ...
    def xcptGetStatus(self, oXcpt):
        """
        Returns the COM status code from the VPox API given exception.
        """
        return None
...
class PlatformMSCOM(PlatformBase):
    ...
    ## @name VirtualPox COM Typelib definitions (should be generate)
    #
    # @remarks Must be updated when the corresponding VirtualPox.xidl bits
    #          are changed.  Fortunately this isn't very often.
    # @{
    VPOX_TLB_GUID = '{D7569351-1750-46F0-936E-BD127D5BC264}'
    VPOX_TLB_LCID = 0
    VPOX_TLB_MAJOR = 1
    VPOX_TLB_MINOR = 3
    ## @}
    ...
    def __init__(self, dParams):
        ...
        win32com.client.gencache.EnsureDispatch('VirtualPox.Session')
        win32com.client.gencache.EnsureDispatch('VirtualPox.VirtualPox')
        win32com.client.gencache.EnsureDispatch('VirtualPox.VirtualPoxClient')

        self.oClient = None     ##< instance of client used to support lifetime of VPoxSDS
    ...
    def flushGenPyCache(self, oGenCache):
        """
        Flushes VPox related files in the win32com gen_py cache.
        ...
        #
        sName = oGenCache.GetGeneratedFileName(self.VPOX_TLB_GUID, self.VPOX_TLB_LCID,
                                               self.VPOX_TLB_MAJOR, self.VPOX_TLB_MINOR)
        ...
        #
        return oGenCache.EnsureModule(self.VPOX_TLB_GUID, self.VPOX_TLB_LCID, self.VPOX_TLB_MAJOR, self.VPOX_TLB_MINOR)

    def getSessionObject(self):
        import win32com
        from win32com.client import Dispatch
        return win32com.client.Dispatch("VirtualPox.Session")

    def getVirtualPox(self):
        # Caching self.oClient is the trick for SDS. It allows to keep the
        # VPoxSDS in the memory  until the end of PlatformMSCOM lifetme.
        if self.oClient is None:
            import win32com
            from win32com.client import Dispatch
            self.oClient = win32com.client.Dispatch("VirtualPox.VirtualPoxClient")
        return self.oClient.virtualPox
    ...
    def createListener(self, oImplClass, dArgs):
        ...
        d['dArgs'] = dArgs
        d['tlb_guid'] = PlatformMSCOM.VPOX_TLB_GUID
        d['tlb_major'] = PlatformMSCOM.VPOX_TLB_MAJOR
        d['tlb_minor'] = PlatformMSCOM.VPOX_TLB_MINOR
        str_ = ""
...
class PlatformXPCOM(PlatformBase):
    ...
    def __init__(self, dParams):
        PlatformBase.__init__(self, dParams)
        sys.path.append(VPoxSdkDir + '/bindings/xpcom/python/')
        import xpcom.vpoxxpcom
        import xpcom
    ...
    def getSessionObject(self):
        import xpcom.components
        return xpcom.components.classes["@virtualpox.org/Session;1"].createInstance()

    def getVirtualPox(self):
        import xpcom.components
        client = xpcom.components.classes["@virtualpox.org/VirtualPoxClient;1"].createInstance()
        return client.virtualPox
    ...
    def xcptSetupConstants(self, oDst):
        ...
        oDst.E_ACCESSDENIED = -2147024891  # see VPox/com/defs.h
...
class PlatformWEBSERVICE(PlatformBase):
    """
    VirtualPox Web Services API specific code.
    """

    def __init__(self, dParams):
        PlatformBase.__init__(self, dParams)
        # Import web services stuff.  Fix the sys.path the first time.
        sWebServLib = os.path.join(VPoxSdkDir, 'bindings', 'webservice', 'python', 'lib')
        if sWebServLib not in sys.path:
            sys.path.append(sWebServLib)
        import VirtualPox_wrappers
        from VirtualPox_wrappers import IWebsessionManager2
    ...
    def getSessionObject(self):
        return self.wsmgr.getSessionObject(self.vpox)
    ...
    def isRemote(self):
        """ Returns True if remote VPox host, False if local. """
        return True
    ...
    def queryInterface(self, oIUnknown, sClassName):
        ...
        str = ""
        str += "from VirtualPox_wrappers import " + sClassName + "\n"
    ...
    def connect(self, url, user, passwd):
        ...
        self.vpox = self.wsmgr.logon(self.user, self.password)
        if not self.vpox.handle:
            raise Exception("cannot connect to '" + self.url + "' as '" + self.user + "'")
        return self.vpox

    def disconnect(self):
        if self.vpox is not None and self.wsmgr is not None:
            self.wsmgr.logoff(self.vpox)
            self.vpox = None
            self.wsmgr = None


## The current (last) exception class.
# This is reinitalized whenever VirtualPoxManager is called, so it will hold
# the reference to the error exception class for the last platform/style that
# was used.  Most clients does talk to multiple VPox instance on different
# platforms at the same time, so this should be sufficent for most uses and
# be way simpler to use than VirtualPoxManager::oXcptClass.
CurXcptClass = None


class VirtualPoxManager(object):
    """
    VirtualPox API manager class.

    The API users will have to instantiate this.  If no parameters are given,
    it will default to interface with the VirtualPox running on the local
    ...
    def __init__(self, sStyle=None, dPlatformParams=None):
        ...
        ## VirtualPox API constants (for webservices, enums are symbolic).
        self.constants = VirtualPoxReflectionInfo(sStyle == "WEBSERVICE")

        ## Status constants.
        self.statuses = self.platform.xcptSetupConstants(VirtualPoxManager.Statuses())
        ## @todo Add VPOX_E_XXX to statuses? They're already in constants...
        ...
        # Get the virtualpox singleton.
        try:
            vpox = self.platform.getVirtualPox()
        except NameError:
    ...
    #
    def getVirtualPox(self):
        """ See PlatformBase::getVirtualPox(). """
        return self.platform.getVirtualPox()

    def getSessionObject(self, oIVPox = None):
    ...
    def getSdkDir(self):
        """
        Returns the VirtualPox SDK directory.
        """
        global VPoxSdkDir
        return VPoxSdkDir
    ...
    def xcptIsOurXcptKind(self, oXcpt=None):
        """
        Checks if the exception is one that could come from the VPox API. If
