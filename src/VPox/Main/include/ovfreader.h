/* $Id: ovfreader.h $ */
/** @file
 * VirtualPox Main - OVF reader declarations.
 *
...
struct EnvelopeData
{
    OVFVersion_T version;//OVF standard version, it is used internally only by VirtualPox
...
struct DiskImage
{
    ...
    RTCString uuidVPox;      // optional; if the file was exported by VirtualPox >= 3.2,
...
struct HardDiskController
{
    ...
    RTCString        strControllerType;
            ...
            // note that we treat LsiLogicSAS as a SCSI controller (system == SCSI) even though VirtualPox
...
struct VirtualSystem
{
    ...
    RTCString    strTypeVPox;            // optional type from @vpox:ostype attribute (VirtualPox 4.0 or higher)
    ...
    const xml::ElementNode *pelmVPoxMachine; // pointer to <vpox:Machine> element under <VirtualSystem> element or NULL if not present
