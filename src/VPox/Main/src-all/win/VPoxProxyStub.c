/* $Id: VPoxProxyStub.c $ */
/** @file
 * VPoxProxyStub - Proxy Stub and Typelib, COM DLL exports and DLL init/term.
 *
...
#include <VPox/cdefs.h>                 /* for VPOX_STRICT */
#include <VPox/log.h>
 ...
 */
#if ARCH_BITS == 64 || defined(VPOX_IN_32_ON_64_MAIN_API)
# define VBPS_PROXY_STUB_FILE(a_fIs32On64) ( (a_fIs32On64) ? "x86\\VPoxProxyStub-x86.dll" : VBPS_PROXY_STUB_FILE_SUB() )
#else
...
#define VBPS_PROXY_STUB_FILE_SUB() \
    ( RT_MAKE_U64(((PKUSER_SHARED_DATA)MM_SHARED_USER_DATA_VA)->NtMinorVersion, \
      ...
      ? "VPoxProxyStub.dll" : "VPoxProxyStubLegacy.dll" )
...
/** Reference to VirtualPox_p.c structure. */
EXTERN_PROXY_FILE(VirtualPox)                                                                        /* see generated dlldata.c */
/** For NdrXxx and for returning. */
static const ProxyFileInfo *g_apProxyFiles[] =
{
    REFERENCE_PROXY_FILE(VirtualPox),
    NULL /* terminator */
};
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
    switch (dwReason)
    {
        case DLL_PROCESS_ATTACH:
            ...
            Log12(("VPoxProxyStub[%u]/DllMain: DLL_PROCESS_ATTACH\n", GetCurrentProcessId()));
        ...
        case DLL_PROCESS_DETACH:
            Log12(("VPoxProxyStub[%u]/DllMain: DLL_PROCESS_DETACH\n", GetCurrentProcessId()));
            break;
    }
...
void RPC_ENTRY GetProxyDllInfo(const ProxyFileInfo ***ppapInfo, const CLSID **ppClsid)
{
    ...
    *ppClsid  = &g_ProxyClsId;
    Log12(("VPoxProxyStub[%u]/GetProxyDllInfo:\n", GetCurrentProcessId()));
}
...
HRESULT STDAPICALLTYPE DllGetClassObject(REFCLSID rclsid, REFIID riid, void **ppv)
{
    ...
    Log12(("VPoxProxyStub[%u]/DllGetClassObject(%RTuuid, %RTuuid, %p): %#x + *ppv=%p\n",
...
HRESULT STDAPICALLTYPE DllCanUnloadNow(void)
{
    ...
    Log12(("VPoxProxyStub[%u]/DllCanUnloadNow: %Rhrc\n", GetCurrentProcessId(), hrc));
    return hrc;
}
...
/**
 * Release call that could be referenced by VirtualPox_p.c via
...
ULONG STDMETHODCALLTYPE CStdStubBuffer_Release(IRpcStubBuffer *pThis)                /* see CSTDSTUBBUFFERRELEASE in RpcProxy.h */
{
    ...
    Log12(("VPoxProxyStub[%u]/CStdStubBuffer_Release: %p -> %#x\n", GetCurrentProcessId(), pThis, cRefs));
    return cRefs;
}


/**
 * Release call referenced by VirtualPox_p.c via
...
ULONG WINAPI CStdStubBuffer2_Release(IRpcStubBuffer *pThis)                         /* see CSTDSTUBBUFFER2RELEASE in RpcProxy.h */
{
    ...
    Log12(("VPoxProxyStub[%u]/CStdStubBuffer2_Release: %p -> %#x\n", GetCurrentProcessId(), pThis, cRefs));
    return cRefs;
}


/**
 * Pure virtual method implementation referenced by VirtualPox_p.c
 */
 ...
 *                                      LocalServer32).
 * @param   pwszVPoxDir                 The VirtualPox install directory
 *                                      (unicode), trailing slash.
 * @param   pszServerSubPath            What to append to @a pwszVPoxDir to
...
LSTATUS VbpsRegisterClassId(VBPSREGSTATE *pState, const CLSID *pClsId, const char *pszDescription, const char *pszAppId,
                            ...
                            const char *pszServerType, PCRTUTF16 pwszVPoxDir, const char *pszServerSubPath,
                            const char *pszThreadingModel)
{
    ...
    Assert((pwszVPoxDir == NULL && !pState->fUpdate) || pwszVPoxDir[RTUtf16Len(pwszVPoxDir) - 1] == '\\');
    ...
    if (pState->fUpdate)
    {
        ...
        if (rc == ERROR_SUCCESS)
        {
            ...
            if (rc == ERROR_SUCCESS)
            {
                ...
                rc = RTUtf16Copy(pwszCur, MAX_PATH, pwszVPoxDir); AssertRC(rc);
...
/**
 * Register modules and classes from the VirtualPox.xidl file.
 *
 * @returns COM status code.
 * @param   pState
 * @param   pwszVPoxDir         The VirtualPox application directory.
 * @param   fIs32On64           Set if this is the 32-bit on 64-bit component.
 *
 * @todo convert to XSLT.
 */
void RegisterXidlModulesAndClassesGenerated(VBPSREGSTATE *pState, PCRTUTF16 pwszVPoxDir, bool fIs32On64)
{
    const char *pszAppId            = "{819B4D85-9CEE-493C-B6FC-64FFE759B3C9}";
    const char *pszInprocDll        = !fIs32On64 ? "VPoxC.dll" : "x86\\VPoxClient-x86.dll";
    const char *pszLocalServer      = "VPoxSVC.exe";
#ifdef VPOX_WITH_SDS
    const char *pszSdsAppId         = "{EC0E78E8-FA43-43E8-AC0A-02C784C4A4FA}";
    const char *pszSdsExe           = "VPoxSDS.exe";
    const char *pszSdsServiceName   = "VPoxSDS";
#endif

    /* VPoxSVC */
    VbpsRegisterAppId(pState, pszLocalServer, pszAppId, "VirtualPox Application", NULL);
    VbpsRegisterClassName(pState, "VirtualPox.VirtualPox.1", "VirtualPox Class", &CLSID_VirtualPox, NULL);
    VbpsRegisterClassName(pState, "VirtualPox.VirtualPox",   "VirtualPox Class", &CLSID_VirtualPox, ".1");
    VbpsRegisterClassId(pState, &CLSID_VirtualPox, "VirtualPox Class", pszAppId, "VirtualPox.VirtualPox", ".1",
                        &LIBID_VirtualPox, "LocalServer32", pwszVPoxDir, pszLocalServer, NULL /*N/A*/);
    /* VPoxC */
    VbpsRegisterClassName(pState, "VirtualPox.Session.1", "Session Class", &CLSID_Session, NULL);
    VbpsRegisterClassName(pState, "VirtualPox.Session", "Session Class", &CLSID_Session, ".1");
    VbpsRegisterClassId(pState, &CLSID_Session, "Session Class", pszAppId, "VirtualPox.Session", ".1",
                        &LIBID_VirtualPox, "InprocServer32", pwszVPoxDir, pszInprocDll, "Free");

    VbpsRegisterClassName(pState, "VirtualPox.VirtualPoxClient.1", "VirtualPoxClient Class", &CLSID_VirtualPoxClient, NULL);
    VbpsRegisterClassName(pState, "VirtualPox.VirtualPoxClient", "VirtualPoxClient Class", &CLSID_VirtualPoxClient, ".1");
    VbpsRegisterClassId(pState, &CLSID_VirtualPoxClient, "VirtualPoxClient Class", pszAppId,
                        "VirtualPox.VirtualPoxClient", ".1",
                        &LIBID_VirtualPox, "InprocServer32", pwszVPoxDir, pszInprocDll, "Free");

#ifdef VPOX_WITH_SDS
    /* VPoxSDS */
    VbpsRegisterAppId(pState, pszSdsExe, pszSdsAppId, "VirtualPox System Service", pszSdsServiceName);
    VbpsRegisterClassName(pState, "VirtualPox.VirtualPoxSDS.1", "VirtualPoxSDS Class", &CLSID_VirtualPoxSDS, NULL);
    VbpsRegisterClassName(pState, "VirtualPox.VirtualPoxSDS", "VirtualPoxSDS Class", &CLSID_VirtualPoxSDS, ".1");
    VbpsRegisterClassId(pState, &CLSID_VirtualPoxSDS, "VirtualPoxSDS Class", pszSdsAppId, "VirtualPox.VirtualPoxSDS", ".1",
                        &LIBID_VirtualPox, "LocalServer32", pwszVPoxDir, pszSdsExe, NULL /*N/A*/);
#endif
}


/**
 * Updates the VPox type lib registration.
 *
 ...
 *
 * @param   pState              The registry modifier state.
 * @param   pwszVPoxDir         The VirtualPox install directory (unicode),
 *                              trailing slash.
 * @param   fIs32On64           Set if we're registering the 32-bit proxy stub
 *                              on a 64-bit system.
 */
static void vbpsUpdateTypeLibRegistration(VBPSREGSTATE *pState, PCRTUTF16 pwszVPoxDir, bool fIs32On64)
{
    const char * const pszTypeLibDll  = VBPS_PROXY_STUB_FILE(fIs32On64);
#if ARCH_BITS == 32 && !defined(VPOX_IN_32_ON_64_MAIN_API)
    const char * const pszWinXx       = "win32";
#else
    const char * const pszWinXx       = !fIs32On64 ? "win64" : "win32";
#endif
    const char * const pszDescription = "VirtualPox Type Library";
    ...
    /* Create TypeLib/{UUID}. */
    rc = vbpsCreateRegKeyA(pState, hkeyTypeLibs, vbpsFormatUuidInCurly(szTypeLibId, &LIBID_VirtualPox), &hkeyTypeLibId, __LINE__);
    if (rc == ERROR_SUCCESS)
    {
        ...
        if (rc == ERROR_SUCCESS)
        {
            ...
            if (rc == ERROR_SUCCESS)
            {
                /* {UUID}/Major.Minor/0/winXX/Default = VPoxProxyStub. */
                rc = RTUtf16Copy(wszBuf, MAX_PATH, pwszVPoxDir); AssertRC(rc);
...
/**
 * Update the VPox proxy stub registration.
 *
 * This is only used when updating COM registrations during com::Initialize.
 ...
 *                              on a 64-bit system.
 */
static void vbpsUpdateProxyStubRegistration(VBPSREGSTATE *pState, PCRTUTF16 pwszVPoxDir, bool fIs32On64)
{
    /*
     * Register the proxy stub factory class ID.
     * It's simple compared to the VPox classes, thus all the NULL parameters.
     */
    ...
    VbpsRegisterClassId(pState, &g_ProxyClsId, "PSFactoryBuffer", NULL /*pszAppId*/,
                        ...
                        "InprocServer32", pwszVPoxDir, pszPsDll, "Both");
}


/**
 * Updates the VPox interface registrations.
 *
 * This is only used when updating COM registrations during com::Initialize.
 * For normal registration and unregistrations we use the NdrDllRegisterProxy
 * and NdrDllUnregisterProxy.
...
static void vbpsUpdateInterfaceRegistrations(VBPSREGSTATE *pState)
{
    ...
    vbpsFormatUuidInCurly(szTypeLibId, &LIBID_VirtualPox);
 ...
 * path to the VirtualPox application directory.
 *
 * @param   pwszDllPath     The path to strip, returns will end with a slash.
 */
static void vbpsDllPathToVPoxDir(PRTUTF16 pwszDllPath)
{
    RTUTF16 wc;
...
#ifdef VPOX_IN_32_ON_64_MAIN_API
    /*
 ...
 */
HRESULT RegisterXidlModulesAndClasses(PRTUTF16 pwszVPoxDir, bool fDelete, bool fUpdate)
{
#ifdef VPOX_IN_32_ON_64_MAIN_API
    ...
    if (rc == ERROR_SUCCESS)
    {
        ...
        RegisterXidlModulesAndClassesGenerated(&State, pwszVPoxDir, fIs32On64);
        rc = State.rc;
    }
    vbpsRegTerm(&State);
        ...
        if (rc == ERROR_SUCCESS)
        {
            ...
            for (idxKey = 0;; idxKey++)
            {
                ...
                if (rc == ERROR_SUCCESS)
                {
                    ...
                    {
                        /*
                         * Ok, it's an orphaned VirtualPox interface. Delete it.
                         */
                        wszCurNm[cwcCurNm] = '\0';
                ...
                if (rc == ERROR_SUCCESS)
                {
                    ...
                    if (rc == ERROR_SUCCESS)
                    {
                        ...
                        if (fDeleteMe)
                        {
                            ...
                            if (rc == ERROR_SUCCESS)
                            {
                                static RTUTF16 const s_wszProgIdPrefix[] = L"VirtualPox.";
                ...
                if (rc == ERROR_SUCCESS)
                {
                    ...
                    while (iVer-- > 0)
                    {
                        ...
                        if (rc == ERROR_SUCCESS)
                        {
                            ...
                            {
                                szValue[cbValue] = '\0';
                                if (!strcmp(szValue, "VirtualPox Type Library"))
                                {
                                    /*
 ...
 * minimize work the VPox type library, classes and servers are also registered.
 *
...
HRESULT STDAPICALLTYPE DllRegisterServer(void)
{
    ...
    /*
     * Register the VPox modules and classes.
     */
    vbpsDllPathToVPoxDir(wszDllName);
...
HRESULT STDAPICALLTYPE DllUnregisterServer(void)
{
     ...
     */
    hrc2 = UnRegisterTypeLib(&LIBID_VirtualPox, kTypeLibraryMajorVersion, kTypeLibraryMinorVersion,
 ...
 */
static void vbpsUpdateWindowsService(VBPSREGSTATE *pState, const WCHAR *pwszVPoxDir, const WCHAR *pwszModule,
                                     const WCHAR *pwszServiceName, const WCHAR *pwszDisplayName, const WCHAR *pwszDescription)
{
    ...
    /*
     * Make double quoted executable file path. ASSUMES pwszVPoxDir ends with a slash!
     */
    WCHAR wszFilePath[MAX_PATH + 2];
    int rc = RTUtf16CopyAscii(wszFilePath, RT_ELEMENTS(wszFilePath), "\"");
    if (RT_SUCCESS(rc))
        rc = RTUtf16Cat(wszFilePath, RT_ELEMENTS(wszFilePath), pwszVPoxDir);
    if (RT_SUCCESS(rc))
...
/**
 * Gently update the COM registrations for VirtualPox.
 *
 * API that com::Initialize (VPoxCOM/initterm.cpp) calls the first time COM is
...
DECLEXPORT(uint32_t) VbpsUpdateRegistrations(void)
{
    ...
    /** @todo Should probably skip this when VPoxSVC is already running...  Use
     *        some mutex or something for checking. */

    /*
     * Find the VirtualPox application directory first.
     */
    WCHAR wszVPoxDir[MAX_PATH];
    DWORD cwcRet = GetModuleFileNameW(g_hDllSelf, wszVPoxDir, RT_ELEMENTS(wszVPoxDir));
    AssertReturn(cwcRet > 0 && cwcRet < RT_ELEMENTS(wszVPoxDir), ERROR_BUFFER_OVERFLOW);
    vbpsDllPathToVPoxDir(wszVPoxDir);
...
#ifdef VPOX_WITH_SDS
        vbpsUpdateWindowsService(&State, wszVPoxDir, L"VPoxSDS.exe", L"VPoxSDS",
                                 L"VirtualPox system service", L"Used as a COM server for VirtualPox API.");
#endif
        vbpsUpdateTypeLibRegistration(&State, wszVPoxDir, fIs32On64);
        vbpsUpdateProxyStubRegistration(&State, wszVPoxDir, fIs32On64);
        vbpsUpdateInterfaceRegistrations(&State);
        RegisterXidlModulesAndClassesGenerated(&State, wszVPoxDir, fIs32On64);
...
#if (ARCH_BITS == 64 && defined(VPOX_WITH_32_ON_64_MAIN_API)) /*|| defined(VPOX_IN_32_ON_64_MAIN_API) ??*/
    /*
