/* $Id: VPoxDrvInst.cpp $ */
/** @file
 * VPoxDrvInst - Driver and service installation helper for Windows guests.
 */
...
#define VPOX_REG_STRINGLIST_NONE              0x00000000        /* No flags set. */
#define VPOX_REG_STRINGLIST_ALLOW_DUPLICATES  0x00000001        /* Allows duplicates in list when adding a value. */

#ifdef DEBUG
# define VPOX_DRVINST_LOGFILE                 "C:\\Temp\\VPoxDrvInstDIFx.log"
#endif
 ...
 *                              Optional.
 */
int VPoxInstallDriver(const BOOL fInstall, const _TCHAR *pszDriverPath, BOOL fSilent,
                      const _TCHAR *pszLogFile)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        INSTALLERINFO instInfo =
        {
            ...
            TEXT("VirtualPox Guest Additions Install Helper"),
            TEXT("VirtualPox Guest Additions"), /** @todo Add version! */
...
static UINT WINAPI vpoxDrvInstExecuteInfFileCallback(PVOID Context,
                                                     UINT Notification,
                                                     UINT_PTR Param1,
                                                     UINT_PTR Param2)
{
...
int ExecuteInfFile(const _TCHAR *pszSection, int iMode, const _TCHAR *pszInf)
{
    ...
    if (hINF != INVALID_HANDLE_VALUE)
    {
        ...
        BOOL fSuccess = SetupInstallFromInfSection(NULL,
                                                    ...
                                                    vpoxDrvInstExecuteInfFileCallback,
                                                    pvQueue,
                                                    NULL,
                                                    NULL
                                                    );
    ...
    if (lRet == ERROR_SUCCESS)
    {
        ...
        while (pszToken != NULL)
        {
            ...
            if (   !wcsicmp(pszToken, pszValueToAdd)
                && (dwFlags & VPOX_REG_STRINGLIST_ALLOW_DUPLICATES))
                fAddToList = TRUE;
...
int AddNetworkProvider(const TCHAR *pszProvider, unsigned int uiOrder)
{
    ...
    int rc = RegistryAddStringToList(_T("System\\CurrentControlSet\\Control\\NetworkProvider\\Order"),
                                     _T("ProviderOrder"),
                                     pszProvider, uiOrder, VPOX_REG_STRINGLIST_NONE /* No flags set */);
    if (rc == EXIT_OK)
...
void PrintHelp(void)
{
    _tprintf(_T("VirtualPox Guest Additions Installation Helper for Windows\n"));
    _tprintf(_T("Version: %d.%d.%d.%d\n\n"), VPOX_VERSION_MAJOR, VPOX_VERSION_MINOR, VPOX_VERSION_BUILD, VPOX_SVN_REV);
    _tprintf(_T("Syntax:\n"));
    _tprintf(_T("\n"));
    _tprintf(_T("Drivers:\n"));
    _tprintf(_T("\tVPoxDrvInst         driver install <inf-file> [log file]\n"));
    _tprintf(_T("\tVPoxDrvInst         driver uninstall <inf-file> [log file]\n"));
    _tprintf(_T("\tVPoxDrvInst         driver executeinf <inf-file>\n"));
    _tprintf(_T("\n"));
    _tprintf(_T("Network Provider:\n"));
    _tprintf(_T("\tVPoxDrvInst         netprovider add <name> [order]\n"));
    _tprintf(_T("\tVPoxDrvInst         netprovider remove <name>\n"));
    _tprintf(_T("\n"));
    _tprintf(_T("Registry:\n"));
    _tprintf(_T("\tVPoxDrvInst         registry write <root> <sub key>\n")
             _T("\t                    <key name> <key type> <value>\n")
             _T("\t                    [type] [size]\n"));
    _tprintf(_T("\tVPoxDrvInst         registry addmultisz <root> <sub key>\n")
             _T("\t                    <value> [order]\n"));
    _tprintf(_T("\tVPoxDrvInst         registry delmultisz <root> <sub key>\n")
            ...
            && argc >= 3)
        {
                ...
                && argc >= 4)
            {
                ...
                else
                {
                    ...
                    rc = VPoxInstallDriver(!_tcsicmp(argv[2], _T("install")) ? TRUE : FALSE, szINF,
