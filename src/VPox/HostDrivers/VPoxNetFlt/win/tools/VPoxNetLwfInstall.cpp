/* $Id: VPoxNetLwfInstall.cpp $ */
/** @file
 * NetLwfInstall - VPoxNetLwf installer command line tool
 */
...
#define VPOX_NETCFG_APP_NAME L"NetLwfInstall"
#define VPOX_NETLWF_INF L".\\VPoxNetLwf.inf"
#define VPOX_NETLWF_RETRIES 10
...
static int VPoxNetLwfInstall()
{
    ...
    if (hr == S_OK)
    {
        ...
        do
        {
            ...
            {
                DWORD dwSize;
                dwSize = MyGetfullPathNameW(VPOX_NETLWF_INF, sizeof(Inf)/sizeof(Inf[0]), Inf);
                if (dwSize > 0)
                {
                    /** @todo add size check for (sizeof(Inf)/sizeof(Inf[0])) == dwSize (string length in sizeof(Inf[0])) */
                    hr = VPoxNetCfgWinNetLwfInstall(pnc, Inf);
                    if (hr == S_OK)
                    {
                    ...
                    {
                        wprintf(L"error installing VPoxNetLwf (0x%x)\n", hr);
                    }
                }
                else
                {
                    hr = HRESULT_FROM_WIN32(GetLastError());
                    wprintf(L"error getting full inf path for VPoxNetLwf.inf (0x%x)\n", hr);
                }
            ...
            {
                if (i < VPOX_NETLWF_RETRIES && !wcscmp(lpszLockedBy, L"6to4svc.dll"))
                {
                    wprintf(L"6to4svc.dll is holding the lock, retrying %d out of %d\n", ++i, VPOX_NETLWF_RETRIES);
