/* $Id: vpoxfs_prov.c $ */
/** @file
 * VirtualPox File System for Solaris Guests, provider implementation.
...
#include "vpoxfs_prov.h"
#include <iprt/err.h>

#define	SFPROV_VERSION	1

static VBGLSFCLIENT vpox_client;

static int sfprov_vpox2errno(int rc)
{
...
sfprov_connect(int version)
{
	...
	if (RT_SUCCESS(rc))
	{
		rc = VbglR0SfConnect(&vpox_client);
		if (RT_SUCCESS(rc))
		{
			rc = VbglR0SfSetUtf8(&vpox_client);
			if (RT_SUCCESS(rc))
			{
				return ((sfp_connection_t *)&vpox_client);
			}
			else
				cmn_err(CE_WARN, "sfprov_connect: VbglR0SfSetUtf8() failed\n");

			VbglR0SfDisconnect(&vpox_client);
		}
		else
...
{
	if (conn != (sfp_connection_t *)&vpox_client)
...
sfprov_mount(sfp_connection_t *conn, char *path, sfp_mount_t **mnt)
{
	...
	rc = VbglR0SfMapFolder(&vpox_client, str, &m->map);
	if (RT_FAILURE(rc)) {
...
sfprov_unmount(sfp_mount_t *mnt)
{
	...
	rc = VbglR0SfUnmapFolder(&vpox_client, &mnt->map);
	if (RT_FAILURE(rc)) {
...
sfprov_get_fsinfo(sfp_mount_t *mnt, sffs_fsinfo_t *fsinfo)
{
	...
	rc = VbglR0SfFsInfo(&vpox_client, &mnt->map, 0, SHFL_INFO_GET | SHFL_INFO_VOLUME,
	...
	sffs_stat_t *stat)
{
	...
	rc = VbglR0SfCreate(&vpox_client, &mnt->map, str, &parms);
	kmem_free(str, size);
...
sfprov_diropen(sfp_mount_t *mnt, char *path, sfp_file_t **fp)
{
	...
	/*
	 * Our VPoxFS interface here isn't very clear regarding failure and informational status.
...
sfprov_close(sfp_file_t *fp)
{
	...
	rc = VbglR0SfClose(&vpox_client, &fp->map, fp->handle);
...
sfprov_read(sfp_file_t *fp, char *buffer, uint64_t offset, uint32_t *numbytes)
{
	...
	rc = VbglR0SfRead(&vpox_client, &fp->map, fp->handle, offset,
...
sfprov_write(sfp_file_t *fp, char *buffer, uint64_t offset, uint32_t *numbytes)
{
	...
	rc = VbglR0SfWrite(&vpox_client, &fp->map, fp->handle, offset,
			   numbytes, (uint8_t *)buffer, 0 /*fLocked*/);
	if (RT_FAILURE(rc))
		return (EINVAL);
	return (0);
...
sfprov_fsync(sfp_file_t *fp)
{
	...
	rc = VbglR0SfFlush(&vpox_client, &fp->map, fp->handle);
	if (RT_FAILURE(rc))
		return (EIO);
	return (0);
}
	...
	timestruc_t ctime)
{
	...
	bytes = sizeof(info);
	rc = VbglR0SfFsInfo(&vpox_client, &mnt->map, parms.Handle, SHFL_INFO_SET | SHFL_INFO_FILE,
			    &bytes, (SHFLDIRINFO *)&info);
	if (RT_FAILURE(rc)) {
		if (rc != VERR_ACCESS_DENIED && rc != VERR_WRITE_PROTECT)
		{
...
fail1:
	rc = VbglR0SfClose(&vpox_client, &mnt->map, parms.Handle);
	if (RT_FAILURE(rc)) {
...
sfprov_set_size(sfp_mount_t *mnt, char *path, uint64_t size)
{
	...
	info.cbObject = size;
	bytes = sizeof(info);
	rc = VbglR0SfFsInfo(&vpox_client, &mnt->map, parms.Handle, SHFL_INFO_SET | SHFL_INFO_SIZE,
			    &bytes, (SHFLDIRINFO *)&info);
	if (RT_FAILURE(rc)) {
...
sfprov_set_show_symlinks(void)
{
	...
	rc = VbglR0SfSetSymlinks(&vpox_client);
	if (RT_FAILURE(rc))
		return (sfprov_vpox2errno(rc));
...
sfprov_remove(sfp_mount_t *mnt, char *path, uint_t is_link)
{
	...
	str = sfprov_string(path, &size);
	rc = VbglR0SfRemove(&vpox_client, &mnt->map, str,
	...
	size_t tgt_size)
{
	...
	rc = VbglR0SfReadLink(&vpox_client, &mnt->map, str, (uint32_t) tgt_size,
	    target);
	if (RT_FAILURE(rc))
		rc = sfprov_vpox2errno(rc);
	...
	sffs_stat_t *stat)
{
	...
	rc = VbglR0SfSymlink(&vpox_client, &mnt->map, lnk, tgt, &info);
	if (RT_FAILURE(rc)) {
		rc = sfprov_vpox2errno(rc);
		goto done;
	}
...
sfprov_rmdir(sfp_mount_t *mnt, char *path)
{
	...
	str = sfprov_string(path, &size);
	rc = VbglR0SfRemove(&vpox_client, &mnt->map, str, SHFL_REMOVE_DIR);
	kmem_free(str, size);
	if (RT_FAILURE(rc))
		return (sfprov_vpox2errno(rc));
	return (0);
...
sfprov_rename(sfp_mount_t *mnt, char *from, char *to, uint_t is_dir)
{
	...
	rc = VbglR0SfRename(&vpox_client, &mnt->map, old, new,
	...
	int flag)
{
	...
	*dirents = kmem_alloc(SFFS_DIRENTS_SIZE, KM_SLEEP);
	...
	/*
	 * Create mask that VPox expects. This needs to be the directory path,
	...
	for (;;) {
		...
		error = VbglR0SfDirInfo(&vpox_client, &fp->map, fp->handle,
