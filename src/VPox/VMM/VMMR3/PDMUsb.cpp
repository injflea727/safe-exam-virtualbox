...
static int pdmR3UsbLoad(PVM pVM, PCPDMUSBREGCBINT pRegCB, const char *pszFilename, const char *pszName)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        FNPDMVPOXUSBREGISTER *pfnVPoxUsbRegister;
        rc = PDMR3LdrGetSymbolR3(pVM, pszName, "VPoxUsbRegister", (void **)&pfnVPoxUsbRegister);
        if (RT_SUCCESS(rc))
        {
            Log(("PDM: Calling VPoxUsbRegister (%p) of %s (%s)\n", pfnVPoxUsbRegister, pszName, pszFilename));
            rc = pfnVPoxUsbRegister(&pRegCB->Core, VPOX_VERSION);
            if (RT_SUCCESS(rc))
        ...
        {
            AssertMsgFailed(("Failed to locate 'VPoxUsbRegister' in %s (%s) rc=%Rrc\n", pszName, pszFilename, rc));
            if (rc == VERR_SYMBOL_NOT_FOUND)
                rc = VERR_PDM_NO_REGISTRATION_EXPORT;
        }
    }
    else
        AssertMsgFailed(("Failed to load VPoxDD!\n"));
    return rc;
}
...
int pdmR3UsbLoadModules(PVM pVM)
{
    ...
    if (fLoadBuiltin)
    {
        ...
        rc = pdmR3UsbLoad(pVM, &RegCB, pszFilename, "VPoxDD");
