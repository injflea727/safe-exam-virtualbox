                        ...
                        ComPtr<IProgress> &aProgress)
{
    ...
    if (   m->locInfo.storageType == VFSType_File
        ...
        return setError(VPOX_E_FILE_ERROR, tr("Appliance file must have .ovf or .ova extension"));
...
HRESULT Appliance::interpret()
{
    ...
    try
    {
             ...
             ++it)
        {
            ...
            // if the virtual system in OVF had a <vpox:Machine> element, have the
            // VirtualPox settings code parse that XML now
            if (vsysThis.pelmVPoxMachine)
                pNewDesc->i_importVPoxMachineXML(*vsysThis.pelmVPoxMachine);

            // Guest OS type
            // This is taken from one of three places, in this order:
            Utf8Str strOsTypeVPox;
            Utf8StrFmt strCIMOSType("%RU32", (uint32_t)vsysThis.cimos);
            // 1) If there is a <vpox:Machine>, then use the type from there.
            if (   vsysThis.pelmVPoxMachine
                && pNewDesc->m->pConfig->machineUserData.strOsType.isNotEmpty()
               )
                strOsTypeVPox = pNewDesc->m->pConfig->machineUserData.strOsType;
            // 2) Otherwise, if there is OperatingSystemSection/vpox:OSType, use that one.
            else if (vsysThis.strTypeVPox.isNotEmpty())      // OVFReader has found vpox:OSType
                strOsTypeVPox = vsysThis.strTypeVPox;
            // 3) Otherwise, make a best guess what the vpox type is from the OVF (CIM) OS type.
            else
                convertCIMOSType2VPoxOSType(strOsTypeVPox, vsysThis.cimos, vsysThis.strCimosDesc);
            ...
            /* VM name */
            Utf8Str nameVPox;
            /* If there is a <vpox:Machine>, we always prefer the setting from there. */
            if (   vsysThis.pelmVPoxMachine
                && pNewDesc->m->pConfig->machineUserData.strName.isNotEmpty())
                nameVPox = pNewDesc->m->pConfig->machineUserData.strName;
            else
                nameVPox = vsysThis.strName;
            /* If there isn't any name specified create a default one out
             * of the OS type */
            if (nameVPox.isEmpty())
                nameVPox = strOsTypeVPox;
            i_searchUniqueVMName(nameVPox);
            ...
            rc = mVirtualPox->ComposeMachineFilename(Bstr(nameVPox).raw(),
            ...
            mVirtualPox->i_getDefaultMachineFolder(strBaseFolder);
            ...
            mVirtualPox->GetGuestOSType(Bstr(strOsTypeVPox).raw(), pGuestOSType.asOutParam());
            ...
            if (   vsysThis.pelmVPoxMachine
                ...
                cpuCountVPox = pNewDesc->m->pConfig->hardwareMachine.cCPUs;
            else
                cpuCountVPox = vsysThis.cCPUs;
            /* Check for the constraints */
            if (cpuCountVPox > SchemaDefs::MaxCPUCount)
            {
                i_addWarning(tr("The virtual system \"%s\" claims support for %u CPU's, but VirtualPox has support for "
                                "max %u CPU's only."),
                                vsysThis.strName.c_str(), cpuCountVPox, SchemaDefs::MaxCPUCount);
                cpuCountVPox = SchemaDefs::MaxCPUCount;
            }
            if (vsysThis.cCPUs == 0)
                cpuCountVPox = 1;
            pNewDesc->i_addEntry(VirtualSystemDescriptionType_CPU,
                                ...
                                Utf8StrFmt("%RU32", (uint32_t)cpuCountVPox));

            /* RAM */
            uint64_t ullMemSizeVPox;
            /* If there is a <vpox:Machine>, we always prefer the setting from there. */
            if (   vsysThis.pelmVPoxMachine
                && pNewDesc->m->pConfig->hardwareMachine.ulMemorySizeMB)
                ullMemSizeVPox = pNewDesc->m->pConfig->hardwareMachine.ulMemorySizeMB;
            else
                ullMemSizeVPox = vsysThis.ullMemorySize / _1M;
            /* Check for the constraints */
            if (    ullMemSizeVPox != 0
                 && (    ullMemSizeVPox < MM_RAM_MIN_IN_MB
                      || ullMemSizeVPox > MM_RAM_MAX_IN_MB
                    )
               )
            {
                i_addWarning(tr("The virtual system \"%s\" claims support for %llu MB RAM size, but VirtualPox has "
                                "support for min %u & max %u MB RAM size only."),
                                vsysThis.strName.c_str(), ullMemSizeVPox, MM_RAM_MIN_IN_MB, MM_RAM_MAX_IN_MB);
                ullMemSizeVPox = RT_MIN(RT_MAX(ullMemSizeVPox, MM_RAM_MIN_IN_MB), MM_RAM_MAX_IN_MB);
            }
            if (vsysThis.ullMemorySize == 0)
            {
                ...
                {
                    rc = pGuestOSType->COMGETTER(RecommendedRAM)(&memSizeVPox2);
                    if (FAILED(rc)) throw rc;
                }
                else
                    memSizeVPox2 = 1024;
                /* VPox stores that in MByte */
                ullMemSizeVPox = (uint64_t)memSizeVPox2;
            }
            pNewDesc->i_addEntry(VirtualSystemDescriptionType_Memory,
                                 "",
                                 Utf8StrFmt("%RU64", (uint64_t)vsysThis.ullMemorySize),
                                 Utf8StrFmt("%RU64", (uint64_t)ullMemSizeVPox));
            ...
            if (vsysThis.pelmVPoxMachine)
            {
                ...
                if (llNetworkAdapters.size() > maxNetworkAdapters)
                    i_addWarning(tr("The virtual system \"%s\" claims support for %zu network adapters, but VirtualPox "
            ...
            else if (vsysThis.llEthernetAdapters.size() >  0)
            {
                ...
                NetworkAdapterType_T defaultAdapterVPox = NetworkAdapterType_Am79C970A;
                if (!pGuestOSType.isNull())
                {
                    rc = pGuestOSType->COMGETTER(AdapterType)(&defaultAdapterVPox);
                    if (FAILED(rc)) throw rc;
                }
                else
                {
#ifdef VPOX_WITH_E1000
                    defaultAdapterVPox = NetworkAdapterType_I82540EM;
#else
                    defaultAdapterVPox = NetworkAdapterType_Am79C973A;
#endif
                }

                ovf::EthernetAdaptersList::const_iterator itEA;
                /* Iterate through all abstract networks. Ignore network cards
                 * which exceed the limit of VirtualPox. */
                size_t a = 0;
                     ...
                     ++itEA, ++a)
                {
                    ...
                    NetworkAdapterType_T nwAdapterVPox = defaultAdapterVPox;
                    if (!ea.strAdapterType.compare("PCNet32", Utf8Str::CaseInsensitive))
                    {
                        ...
                        if (!(defaultAdapterVPox == NetworkAdapterType_Am79C970A ||
                              defaultAdapterVPox == NetworkAdapterType_Am79C973))
                            nwAdapterVPox = NetworkAdapterType_Am79C970A;
                    }
#ifdef VPOX_WITH_E1000
                    /* VMWare accidentally write this with VirtualCenter 3.5,
                       so make sure in this case always to use the VMWare one */
                        ...
                        nwAdapterVPox = NetworkAdapterType_I82545EM;
                    else if (!ea.strAdapterType.compare("E1000", Utf8Str::CaseInsensitive))
                    {
                        /* Check if this OVF was written by VirtualPox */
                        if (Utf8Str(vsysThis.strVirtualSystemType).contains("virtualpox", Utf8Str::CaseInsensitive))
                        {
                            /* If the default adapter is already one of the three
                             * E1000 adapters use the default one. If not use the
                             * I82545EM as fallback. */
                            if (!(defaultAdapterVPox == NetworkAdapterType_I82540EM ||
                                  defaultAdapterVPox == NetworkAdapterType_I82543GC ||
                                  defaultAdapterVPox == NetworkAdapterType_I82545EM))
                            nwAdapterVPox = NetworkAdapterType_I82540EM;
                        }
                        else
                            /* Always use this one since it's what VMware uses */
                            nwAdapterVPox = NetworkAdapterType_I82545EM;
                    }
#endif /* VPOX_WITH_E1000 */

                    pNewDesc->i_addEntry(VirtualSystemDescriptionType_NetworkAdapter,
                                         "",      // ref
                                         ea.strNetworkName,      // orig
                                         Utf8StrFmt("%RU32", (uint32_t)nwAdapterVPox),   // conf
                                         0,
                ...
                switch (hdc.system)
                {
                        ...
                        if (cIDEused < 4)
                        {
                            ...
                            if (cIDEused == 2)
                                i_addWarning(tr("The virtual \"%s\" system requests support for more than two "
                                              "IDE controller channels, but VirtualPox supports only two."),
                        ...
                        else
                        {
                            ...
                            if (cSATAused == 1)
                                i_addWarning(tr("The virtual system \"%s\" requests support for more than one "
                                                "SATA controller, but VirtualPox has support for only one"),
                                                vsysThis.strName.c_str());
                    ...
                    case ovf::HardDiskController::SCSI:
                        ...
                        if (cSCSIused < 1)
                        {
                            ...
                            {
                                // OVF considers SAS a variant of SCSI but VirtualPox considers it a class of its own
                            ...
                            i_addWarning(tr("The virtual system \"%s\" requests support for an additional "
                                            "SCSI controller of type \"%s\" with ID %s, but VirtualPox presently "
                    ...
                    case ovf::HardDiskController::VIRTIOSCSI:
                        ...
                        else
                        {
                            ...
                            if (cVIRTIOSCSIused == 1)
                                i_addWarning(tr("The virtual system \"%s\" requests support for more than one "
                                                "VirtioSCSI controller, but VirtualPox has support for only one"),
                                                vsysThis.strName.c_str());
            ...
            if (vsysThis.mapVirtualDisks.size() > 0)
            {
                     ...
                     ++itVD)
                {
                    ...
                    else if (vdf.compare("RAW", Utf8Str::CaseInsensitive) == 0)
                    {
                        ...
                        throw setError(VPOX_E_FILE_ERROR,
...
HRESULT Appliance::i_preCheckImageAvailability(ImportStack &stack)
{
    if (i_importEnsureOvaLookAhead(stack))
        return S_OK;
    throw setError(VPOX_E_FILE_ERROR, tr("Unexpected end of OVA package"));
...
RTVFSIOSTREAM Appliance::i_importOpenSourceFile(ImportStack &stack, Utf8Str const &rstrSrcPath, const char *pszManifestEntry)
{
    ...
    if (stack.hVfsFssOva != NIL_RTVFSFSSTREAM)
    {
        ...
        {
            if (!i_importEnsureOvaLookAhead(stack))
                throw setErrorBoth(VPOX_E_FILE_ERROR, VERR_EOF,
                                                         ...
                                                         Utf8Str const &rstrSrcLogNm)
{
     ...
     */
    HRESULT hrc = VirtualPox::i_ensureFilePathExists(rstrDstPath, true /*fCreate*/);
    if (SUCCEEDED(hrc))
    {
...
HRESULT Appliance::i_readImpl(const LocationInfo &aLocInfo, ComObjPtr<Progress> &aProgress)
{
    ...
    try
    {
        ...
        {
            /* 1 operation only */
            hrc = aProgress->init(mVirtualPox, static_cast<IAppliance*>(this),
        ...
        else
        {
            ...
            if (aLocInfo.storageType == VFSType_File)
                /* 1 operation only */
                hrc = aProgress->init(mVirtualPox, static_cast<IAppliance*>(this), strDesc, TRUE /* aCancelable */);
            else
...
HRESULT Appliance::i_gettingCloudData(TaskCloud *pTask)
{
    ...
    try
    {
        ...
        hrc = mVirtualPox->COMGETTER(CloudProviderManager)(cpm.asOutParam());
        if (FAILED(hrc))
        ...
        if (profileName.isEmpty())
            return setErrorVrc(VPOX_E_OBJECT_NOT_FOUND, tr("%s: Cloud user profile name wasn't found"), __FUNCTION__);
...
#define GET_VSD_DESCRIPTION_BY_TYPE(aParamType) \
    ...
    ComPtr<IGuestOSType> pGuestOSType;
    {
        VPOXOSTYPE guestOsType = VPOXOSTYPE_Unknown;
        GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_OS)//aVPoxValues is set in this #define
        if (aVPoxValues.size() != 0)
        {
            strOsType = aVPoxValues[0];
            /* Check the OS type */
            uint32_t const idxOSType = Global::getOSTypeIndexFromId(strOsType.c_str());
            guestOsType = idxOSType < Global::cOSTypes ? Global::sOSTypes[idxOSType].osType : VPOXOSTYPE_Unknown;

            /* Case when some invalid OS type or garbage was passed. Set to VPOXOSTYPE_Unknown. */
        ...
        /* Case when no OS type was passed. Set to VPOXOSTYPE_Unknown. */
        else
        {
            strOsType = Global::OSTypeId(guestOsType);
            vsd->AddDescription(VirtualSystemDescriptionType_OS,
        ...
        hrc = mVirtualPox->GetGuestOSType(Bstr(strOsType).raw(), pGuestOSType.asOutParam());
        if (FAILED(hrc))
            return hrc;
    }
    ...
    if (m->virtualSystemDescriptions.size() == 1)
    {
        do
        {
            ComPtr<IVirtualPox> VPox(mVirtualPox);

            {
                GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_Name)//aVPoxValues is set in this #define
                if (aVPoxValues.size() != 0)//paranoia but anyway...
                    strVMName = aVPoxValues[0];
...
//          i_searchUniqueVMName(strVMName);//internally calls setError() in the case of absent the registered VM with such name
            ...
            hrc = mVirtualPox->FindMachine(Bstr(strVMName.c_str()).raw(), machine.asOutParam());
            if (SUCCEEDED(hrc))
            {
            ...
            /* Check the target path. If the path exists and folder isn't empty return an error */
            {
                Bstr bstrSettingsFilename;
                /* Based on the VM name, create a target machine path. */
                hrc = mVirtualPox->ComposeMachineFilename(Bstr(strVMName).raw(),
                                                          Bstr("/").raw(),
                                                          NULL /* aCreateFlags */,
                                                          NULL /* aBaseFolder */,
                                                          bstrSettingsFilename.asOutParam());
            ...
            Utf8Str strInsId;
            GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_CloudInstanceId)//aVPoxValues is set in this #define
            if (aVPoxValues.size() == 0)
                return setErrorVrc(VERR_NOT_FOUND, "%s: Cloud Instance Id wasn't found", __FUNCTION__);

            strInsId = aVPoxValues[0];
...
*/
    ...
    if (FAILED(hrc))
    {
...

        {
            if (!fKeepDownloadedObject)
            {
                ...
                GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_HardDiskImage)//aVPoxValues is set in this #define
                if (aVPoxValues.size() == 0)
    ...
    else
    {
        ...
        try
        {
            ...
            pSysProps = mVirtualPox->i_getSystemProperties();
            if (pSysProps == NULL)
                throw VPOX_E_OBJECT_NOT_FOUND;
            ...
            /* Try to re-use some OVF stuff here */
            {
                ...
                uint32_t cpus = 1;
                {
                    GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_CPU)//aVPoxValues is set in this #define
                    if (aVPoxValues.size() != 0)
                    {
                        vsdData = aVPoxValues[0];
                ...
                {
                    GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_Memory)//aVPoxValues is set in this #define
                    if (aVPoxValues.size() != 0)
                    {
                        vsdData = aVPoxValues[0];
                ...
                {
                    GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_Description)//aVPoxValues is set in this #define
                    if (aVPoxValues.size() != 0)
                    {
                        vsdData = aVPoxValues[0];
...

                {
                    ...
                    if (aVPoxValues.size() != 0)
                        strOsType = aVPoxValues[0];
                    vsys.strTypeVPox = strOsType;
                    LogRel(("%s: OS type is %s\n", __FUNCTION__, strOsType.c_str()));
                }

                ovf::EthernetAdapter ea;
                {
                    GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_NetworkAdapter)//aVPoxValues is set in this #define
                    if (aVPoxValues.size() != 0)
                    {
                        ea.strAdapterType = (Utf8Str)(aVPoxValues[0]);
                ...
                ovf::HardDiskController hdc;
                {
                    ...
                    GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_HardDiskControllerSATA)//aVPoxValues is set in this #define
                    if (aVPoxValues.size() != 0)
                        hdc.strControllerType = (Utf8Str)(aVPoxValues[0]);
                    else
                ...
                {
                    GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_SoundCard)//aVPoxValues is set in this #define
                    if (aVPoxValues.size() != 0)
                        vsys.strSoundCardType  = (Utf8Str)(aVPoxValues[0]);
                    else
                    {
            ...
            do
            {
                    ...
                    || enmType  == RTVFSOBJTYPE_FILE)
                {
                    ...
                    else
                    {
                        ...
                        /* Image format is supported by VPox so extract the file and try to convert
                        ...
                        hrc = mVirtualPox->i_findHardDiskByLocation(strAbsDstPath, false, NULL);
                        if (SUCCEEDED(hrc))
                        ...
                        hrc = pTargetMedium->init(mVirtualPox,
                                                 strTargetFormat,
                                                 strAbsDstPath,
                        ...
                        hrc = pProgressImportTmp->init(mVirtualPox,
         ...
         * But it was the first record added into the list, so aVPoxValues[0] should be correct here.
         */
        GET_VSD_DESCRIPTION_BY_TYPE(VirtualSystemDescriptionType_HardDiskImage)//aVPoxValues is set in this #define
        if (!fKeepDownloadedObject)
        {
        ...
        if (FAILED(hrc))
        {
             ...
             *  - check the list of imported images, detach them and next delete if they have still registered in the VPox.
            ...
            /* m->llGuidsMachinesCreated is filled in the i_importMachineGeneric()/i_importVPoxMachine()
            ...
            hrc = mVirtualPox->FindMachine(Bstr(machineNameOrId).raw(), pMachine.asOutParam());
            ...
            else
            {
                 ...
                 * The rest items in the array aVPoxValues are the images which might
                 * have still been registered in the VPox.
                ...
                AutoWriteLock treeLock(mVirtualPox->i_getMediaTreeLockHandle() COMMA_LOCKVAL_SRC_POS);

                for (size_t i = 1; i < aVPoxValues.size(); ++i)
                {
                    vsdData = aVPoxValues[i];
                    ComObjPtr<Medium> poHardDisk;
                    hrc = mVirtualPox->i_findHardDiskByLocation(vsdData, false, &poHardDisk);
                    if (SUCCEEDED(hrc))
                    {
                        hrc = mVirtualPox->i_unregisterMedium((Medium*)(poHardDisk));
                        if (SUCCEEDED(hrc))
                        {
                            ComPtr<IProgress> pProgress1;
                    ...
                    else if (hrc == VPOX_E_OBJECT_NOT_FOUND)
                    {
            ...
            for (size_t i = 0; i < extraCreatedFiles.size(); ++i)
            {
                ...
                if (RT_FAILURE(vrc))
                    hrc = setErrorBoth(VPOX_E_IPRT_ERROR, vrc);
                else
...
HRESULT Appliance::i_readFSOVA(TaskOVF *pTask)
{
    ...
    if (SUCCEEDED(hrc) && !pszOvfNameBase)
        hrc = setError(VPOX_E_FILE_ERROR, tr("OVA '%s' does not contain an .ovf-file"), pTask->locInfo.strPath.c_str());
    if (SUCCEEDED(hrc))
    {
        /*
         * Do tail processing (check the signature).
...
HRESULT Appliance::i_readOVFFile(TaskOVF *pTask, RTVFSIOSTREAM hVfsIosOvf, const char *pszManifestEntry)
{
    ...
    {
        hrc = setError(VPOX_E_FILE_ERROR, rXcpt.what());
    }
    catch (HRESULT aRC)
    {
        hrc = aRC;
...
HRESULT Appliance::i_readManifestFile(TaskOVF *pTask, RTVFSIOSTREAM hVfsIosMf, const char *pszSubFileNm)
{
    ...
    AssertStmt(RT_SUCCESS(vrc), Global::vpoxStatusCodeToCOM(vrc));
    ...
    AssertRCReturn(vrc, Global::vpoxStatusCodeToCOM(vrc));
    m->fDeterminedDigestTypes = true;
...
HRESULT Appliance::i_readTailProcessing(TaskOVF *pTask)
{
    ...
    if (m->pbSignedDigest)
    {
        ...
        if (m->hMemFileTheirManifest == NIL_RTVFSFILE)
            return setError(VPOX_E_FILE_ERROR, tr("Found .cert-file but no .mf-file for '%s'"), pTask->locInfo.strPath.c_str());
...
/**
 * Implementation for importing OVF data into VirtualPox. This starts a new thread which will call
                                ...
                                ComObjPtr<Progress> &progress)
{
    ...
    if (locInfo.storageType != VFSType_Cloud)
    {
        ...
        if (FAILED(rc))
            return setError(rc, tr("Failed to create task for importing appliance into VirtualPox"));
        try
        {
    ...
    else
    {
        if (locInfo.strProvider.equals("OCI"))
        {
            ...
            try
            {
                ...
                if (SUCCEEDED(rc))
                    rc = progress->init(mVirtualPox, static_cast<IAppliance *>(this),
    ...
    if (SUCCEEDED(rc))
        return rc;
    return setError(rc, tr("Failed to start thread for importing appliance into VirtualPox"));
}

/**
 * Actual worker code for importing OVF data into VirtualPox.
 *
...
HRESULT Appliance::i_importFS(TaskOVF *pTask)
{
    ...
    if (FAILED(rc))
    {
             ...
             ++itID)
        {
            ...
            HRESULT rc2 = mVirtualPox->FindMachine(bstrGuid.raw(), failedMachine.asOutParam());
            if (SUCCEEDED(rc2))
            {
                SafeIfaceArray<IMedium> aMedia;
...
HRESULT Appliance::i_importFSOVA(TaskOVF *pTask, AutoWriteLockBase &rWriteLock)
{
     ...
     *       i_importMachineGeneric, i_importVPoxMachine and i_importOpenSourceFile
...
HRESULT Appliance::i_importDoIt(TaskOVF *pTask, AutoWriteLockBase &rWriteLock, RTVFSFSSTREAM hVfsFssOva /*= NIL_RTVFSFSSTREAM*/)
{
    ...
    try
    {
        ...
        if (FAILED(hrc))
        {
             ...
             * During import VPox creates new UUIDs for imported images and
...
HRESULT Appliance::i_verifyManifestFile(ImportStack &stack)
{
    ...
    else
    {
            ...
            && RTManifestEntryExists(m->hOurManifest, m->strOvfManifestEntry.c_str()) )
        {
            ...
            if (RT_FAILURE(vrc))
                return setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("Error fudging missing OVF digest in manifest: %Rrc"), vrc);
        }
...
/**
 * Helper that converts VirtualSystem attachment values into VirtualPox attachment values.
    ...
    switch (hdc.system)
    {
        case ovf::HardDiskController::IDE:
            ...
            switch (ulAddressOnParent)
            {
                ...
                default:
                    throw setError(VPOX_E_NOT_SUPPORTED,
 ...
 *  --  i_importVPoxMachine(); in that case, the information comes from the <vpox:Machine>
 *      tag.
 *
 ...
 * spec, even though this cannot really happen in the vpox:Machine case since such data
                                     ...
                                     ImportStack &stack)
{
    ...
    AssertRCStmt(vrc, throw Global::vpoxStatusCodeToCOM(vrc));
    ...
    if (vrc == VINF_SUCCESS)
    {
        rc = mVirtualPox->i_findHardDiskById(Guid(uuid), true, &pTargetMedium);
        if (FAILED(rc)) throw rc;
    }
    else
    {
        ...
        try
        {
            ...
            if (pszSuff != NULL)
            {
                 ...
                 * or via VPoxManage import command (option --importtovdi)
            ...
            if (strTrgFormat.compare("RAW", Utf8Str::CaseInsensitive) == 0)
            {
                try
                {
                    ...
                    ComPtr<IMedium> pTmp;
                    rc = mVirtualPox->OpenMedium(Bstr(strAbsDstPath).raw(),
                                                 DeviceType_DVD,
            ...
            else/* HDD case*/
            {
                ...
                rc = pTargetMedium->init(mVirtualPox,
                                         strTrgFormat,
                                         strAbsDstPath,
                                         Guid::Empty /* media registry: none yet */,
                                         DeviceType_HardDisk);
                ...
                else
                {
                    ...
                    if (FAILED(rc)) throw rc;
                    rc = pProgressImportTmp->init(mVirtualPox,
                                                  static_cast<IAppliance*>(this),
                                                  Utf8StrFmt(tr("Importing medium '%s'"),
                                                             strAbsDstPath.c_str()),
                                                  TRUE);
                ...
                pTargetMedium->i_removeRegistry(mVirtualPox->i_getGlobalRegistryId());
            }
        }
        catch (...)
        {
 ...
 * into VirtualPox by creating an IMachine instance, which is returned.
 *
                                       ...
                                       ImportStack &stack)
{
    ...
    rc = mVirtualPox->GetGuestOSType(Bstr(stack.strOsTypeVPox).raw(), osType.asOutParam());
    if (FAILED(rc)) throw rc;
    ...
    rc = mVirtualPox->CreateMachine(Bstr(stack.strSettingsFilename).raw(),
                                    Bstr(stack.strNameVPox).raw(),
                                    ComSafeArrayAsInParam(groups),
                                    Bstr(stack.strOsTypeVPox).raw(),
                                    NULL, /* aCreateFlags */
    ...
    ULONG vramVPox;
    rc = osType->COMGETTER(RecommendedVRAM)(&vramVPox);
    if (FAILED(rc)) throw rc;
    ...
    if (FAILED(rc)) throw rc;
    rc = pGraphicsAdapter->COMSETTER(VRAMSize)(vramVPox);
    if (FAILED(rc)) throw rc;
    ...
    if (vsdeNW.empty())
    {
        ...
        ComPtr<INetworkAdapter> nwVPox;
        rc = pNewMachine->GetNetworkAdapter(0, nwVPox.asOutParam());
        if (FAILED(rc)) throw rc;
        rc = nwVPox->COMSETTER(Enabled)(false);
        if (FAILED(rc)) throw rc;
    }
    else if (vsdeNW.size() > maxNetworkAdapters)
        throw setError(VPOX_E_FILE_ERROR,
                       tr("Too many network adapters: OVF requests %d network adapters, "
                          "but VirtualPox only supports %d"),
    ...
    else
    {
             ...
             ++nwIt, ++a)
        {
            ...
            const Utf8Str &nwTypeVPox = pvsys->strVPoxCurrent;
            uint32_t tt1 = RTStrToUInt32(nwTypeVPox.c_str());
            ...
            if (pvsys->strExtraConfigCurrent.endsWith("type=Bridged", Utf8Str::CaseInsensitive))
            {
                ...
                if (FAILED(rc)) throw rc;
                ComPtr<IHost> host;
                rc = mVirtualPox->COMGETTER(Host)(host.asOutParam());
                if (FAILED(rc)) throw rc;
            ...
            else if (pvsys->strExtraConfigCurrent.endsWith("type=NATNetwork", Utf8Str::CaseInsensitive))
            {
                ...
                rc = mVirtualPox->COMGETTER(NATNetworks)(ComSafeArrayAsOutParam(nwNATNetworks));
                if (FAILED(rc)) throw rc;
     ...
     * so VirtualPox's single IDE controller with two channels and two ports each counts as
    ...
    if (!vsdeHDCIDE.empty())
    {
        // one or two IDE controllers present in OVF: add one VirtualPox controller
        ComPtr<IStorageController> pController;
        rc = pNewMachine->AddStorageController(Bstr("IDE").raw(), StorageBus_IDE, pController.asOutParam());
        if (FAILED(rc)) throw rc;

        const char *pcszIDEType = vsdeHDCIDE.front()->strVPoxCurrent.c_str();
    ...
    {
        ComPtr<IStorageController> pController;
        const Utf8Str &hdcVPox = vsdeHDCSATA.front()->strVPoxCurrent;
        if (hdcVPox == "AHCI")
        {
    ...
    if (!vsdeHDCSCSI.empty())
    {
        ...
        const Utf8Str &hdcVPox = vsdeHDCSCSI.front()->strVPoxCurrent;
        if (hdcVPox == "LsiLogic")
            controllerType = StorageControllerType_LsiLogic;
        else if (hdcVPox == "LsiLogicSas")
        {
            // OVF treats LsiLogicSas as a SCSI controller but VPox considers it a class of its own
            strName = "SAS";
            busType = StorageBus_SAS;
            controllerType = StorageControllerType_LsiLogicSas;
        }
        else if (hdcVPox == "BusLogic")
    ...
    if (!vsdeHDCVirtioSCSI.empty())
    {
        ...
        const Utf8Str &hdcVPox = vsdeHDCVirtioSCSI.front()->strVPoxCurrent;
        if (hdcVPox == "VirtioSCSI")
        {
    ...
    rc = mVirtualPox->RegisterMachine(pNewMachine);
    if (FAILED(rc)) throw rc;
...
l_skipped:
                    ...
                    if (SUCCEEDED(rc))
                    {
                        ...
                        if (RTStrICmp(diCurrent.strHref.c_str(), stack.pszOvaLookAheadName) != 0)
                        {
                             ...
                             * May be VPox imports OVA package with wrong file sequence inside the archive.
                             */
                            if (!vsdeTargetHD)
                                throw setError(E_FAIL,
                                               tr("Internal inconsistency looking up disk image '%s'"),
...
////// end of duplicated code.
                ...
                {
                    /* We have already all disks prepared (converted and registered in the VPox)
                    ...
                    rc = mVirtualPox->i_findHardDiskById(id, false, &pTargetMedium);
                    if (FAILED(rc))
                        throw rc;
                }
                else
                   ...
                   && vsdeTargetHD->strExtraConfigSuggested != vsdeTargetHD->strExtraConfigCurrent)
               {
                ...
                Log(("Attaching disk %s to port %d on device %d\n",
                     vsdeTargetHD->strVPoxCurrent.c_str(), mhda.lControllerPort, mhda.lDevice));
...
/**
 * Imports one OVF virtual system (described by a vpox:Machine tag represented by the given config
 * structure) into VirtualPox by creating an IMachine instance, which is returned.
 *
 * This throws HRESULT error codes for anything that goes wrong, in which case the caller must clean
 * up any leftovers from this function. For this, the given ImportStack instance has received information
 * about what needs cleaning up (to support rollback).
 ...
 * the disk attachments used by the machine when it was exported. We also add vpox:uuid attributes
 ...
 *      and import the disk into VirtualPox, which creates a new UUID for it. In the machine config,
 ...
 *
 *  4)  Create the VirtualPox machine with the modfified machine config.
 *
 * @param   vsdescThis
 * @param   pReturnNewMachine
 * @param   stack
 */
void Appliance::i_importVPoxMachine(ComObjPtr<VirtualSystemDescription> &vsdescThis,
                                    ...
                                    ImportStack &stack)
{
    ...
    /* OS Type */
    config.machineUserData.strOsType = stack.strOsTypeVPox;
    /* Groups */
    if (stack.strPrimaryGroup.isEmpty() || stack.strPrimaryGroup == "/")
    {
...
*/
                ...
                && vsdeNW->strExtraConfigCurrent.length() > 6)
            {
                     ...
                     ++it1)
                {
                    ...
                    if (it1->ulSlot == iSlot)
                    {
                        ...
                        it1->type = (NetworkAdapterType_T)vsdeNW->strVPoxCurrent.toUInt32();
                        break;
                    }
                }
            }
     ...
     * attachment. Old VirtualPox versions (prior to 3.2.10) had all disk
...
////// end of duplicated code.
            ...
            && vsdeTargetHD->strExtraConfigSuggested != vsdeTargetHD->strExtraConfigCurrent)
        {
            ...
            if (itDiskImageMap == stack.mapDisks.end())
                throw setError(E_FAIL,
                               ...
                               vsdeTargetHD->strVPoxCurrent.c_str());
            const ovf::DiskImage &targetDiskImage = itDiskImageMap->second;
            Utf8Str strTargetDiskUuid = targetDiskImage.uuidVPox;;
             ...
             *    'AHCI' is used in 'strOvf' and 'strVPoxSuggested'.
             *  + The VirtualSystemDescriptionEntry 'strVPoxSuggested' field is populated by
             *    Appliance::interpret()->VirtualSystemDescription::i_addEntry() and is thus
             *    under VPox's control and has a fixed format and predictable content.
             */
                 ...
                 ++itSCL)
            {
                ...
                if (!vsdeOrigController->strVPoxSuggested.compare(pcszSCType, Utf8Str::CaseInsensitive))
                {
            ...
            if (fDiskRemoved)
            {
                     ...
                     ++itSCL)
                {
                    ...
                    /* There can only be one storage controller of each type in the OVF data. */
                    if (!vsdeTargetController->strVPoxSuggested.compare(pcszSCType, Utf8Str::CaseInsensitive))
                    {
                        settings::AttachedDevicesList::iterator itAD;
                        for (itAD = SC.llAttachedDevices.begin();
                             itAD != SC.llAttachedDevices.end();
                ...
                if (!fDiskAdded)
                    throw setError(E_FAIL,
                                   ...
                                   vsdeTargetHD->strVPoxCurrent.c_str(),
                                   originalAttachedDevice.uuid.raw(),
                                   vsdeTargetController->strVPoxSuggested.c_str());
            }
             ...
             */
            AutoWriteLock vpoxLock(mVirtualPox COMMA_LOCKVAL_SRC_POS);
            mVirtualPox->i_saveSettings();
            vpoxLock.release();
        }
             ...
             ++sit)
        {
                 ...
                 ++dit)
            {
                ...
                // When repairing a broken VirtualPox xml config section (written
                // by VirtualPox versions earlier than 3.2.10) assume the disks
        ...
        if (!fFound)
            throw setError(E_FAIL,
                           tr("<vpox:Machine> element in OVF contains a medium attachment for the disk image %s "
    ...
    // instance that we created from the vpox:Machine
    rc = pNewMachine->init(mVirtualPox,
                           stack.strNameVPox,// name from OVF preparations; can be suffixed to avoid duplicates
...
void Appliance::i_importMachines(ImportStack &stack)
{
         ...
         ++it, ++it1, ++i)
    {
        ...
        // there are two ways in which we can create a vpox machine from OVF:
        // -- either this OVF was written by vpox 3.2 or later, in which case there is a <vpox:Machine> element
        ...
        // -- or this is an OVF from an older vpox or an external source, and then we need to translate the
        //    VirtualSystemDescriptionEntry and do import work

        // Even for the vpox:Machine case, there are a number of configuration items that will be taken from
        ...
        if (vsdeName.size() < 1)
            throw setError(VPOX_E_FILE_ERROR,
                           tr("Missing VM name"));
        stack.strNameVPox = vsdeName.front()->strVPoxCurrent;
        ...
        {
            stack.strPrimaryGroup = vsdePrimaryGroup.front()->strVPoxCurrent;
        ...
        if (vsdeSettingsFile.size() >= 1)
        {
            ...
            if (vsdeSF1->strVPoxCurrent != vsdeSF1->strVPoxSuggested)
                stack.strSettingsFilename = vsdeSF1->strVPoxCurrent;
        }
        if (stack.strSettingsFilename.isEmpty())
        {
            ...
            if (vsdeBaseFolder.size() >= 1)
                strBaseFolder = vsdeBaseFolder.front()->strVPoxCurrent;
            Bstr bstrSettingsFilename;
            rc = mVirtualPox->ComposeMachineFilename(Bstr(stack.strNameVPox).raw(),
        ...
        LogFunc(("i=%zu strName=%s strSettingsFilename=%s\n", i, stack.strNameVPox.c_str(), stack.strSettingsFilename.c_str()));
        ...
        stack.strOsTypeVPox = vsdeOS.front()->strVPoxCurrent;
        ...
        if (firmware.size() != 1)
            ...
            stack.strFirmwareType = firmware.front()->strVPoxCurrent;

        // CPU count
        std::list<VirtualSystemDescriptionEntry*> vsdeCPU = vsdescThis->i_findByType(VirtualSystemDescriptionType_CPU);
        if (vsdeCPU.size() != 1)
            throw setError(VPOX_E_FILE_ERROR, tr("CPU count missing"));

        stack.cCPUs = vsdeCPU.front()->strVPoxCurrent.toUInt32();
        ...
        if (vsdeRAM.size() != 1)
            throw setError(VPOX_E_FILE_ERROR, tr("RAM size missing"));
        stack.ulMemorySizeMB = (ULONG)vsdeRAM.front()->strVPoxCurrent.toUInt64();
        ...
        if (!vsdeAudioAdapter.empty())
            stack.strAudioAdapter = vsdeAudioAdapter.front()->strVPoxCurrent;
        ...
        if (!vsdeDescription.empty())
            stack.strDescription = vsdeDescription.front()->strVPoxCurrent;

        // import vpox:machine or OVF now
        if (vsdescThis->m->pConfig)
            // vpox:Machine config
            i_importVPoxMachine(vsdescThis, pNewMachine, stack);
        else
            // generic OVF config
