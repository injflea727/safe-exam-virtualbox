...
/** @file
 * VPox Qt GUI - UIWizardExportAppPageBasic1 class implementation.
 */
...
void UIWizardExportAppPage1::populateVMSelectorItems(const QStringList &selectedVMNames)
{
    ...
    foreach (const CMachine &machine, uiCommon().virtualPox().GetMachines())
    {
        QPixmap pixIcon;
        QString strName;
        QUuid uUuid;
        ...
        else
        {
            ...
            strName = UICommon::hasAllowedExtension(fi.completeSuffix(), VPoxFileExts) ? fi.completeBaseName() : fi.fileName();
