...
static const char   *g_pszDefDmiBoardProduct    = "VirtualPox";
...
AssertCompile(sizeof(SMBIOSHDR) + sizeof(DMIMAINHDR) <= VPOX_DMI_HDR_SIZE);
...
typedef struct DMIOEMSTRINGS
{
    DMIHDR          header;
    uint8_t         u8Count;
    uint8_t         u8VPoxVersion;
    uint8_t         u8VPoxRevision;
} *PDMIOEMSTRINGS;
...
# endif
...
#ifdef VPOX_WITH_DMI_OEMSTRINGS
        ...
        char szTmp[64];
        RTStrPrintf(szTmp, sizeof(szTmp), "vpoxVer_%u.%u.%u",
                    RTBldCfgVersionMajor(), RTBldCfgVersionMinor(), RTBldCfgVersionBuild());
        DMI_READ_CFG_STR_DEF(pOEMStrings->u8VPoxVersion, "DmiOEMVPoxVer", szTmp);
        RTStrPrintf(szTmp, sizeof(szTmp), "vpoxRev_%u", RTBldCfgRevision());
        DMI_READ_CFG_STR_DEF(pOEMStrings->u8VPoxRevision, "DmiOEMVPoxRev", szTmp);
        DMI_TERM_STRUCT;
...
void FwCommonPlantSmbiosAndDmiHdrs(PPDMDEVINS pDevIns, uint8_t *pHdr, uint16_t cbDmiTables, uint16_t cNumDmiTables)
{
    ...
    aBiosHeaders =
    {
        // The SMBIOS header
        {
            ...
            VPOX_SMBIOS_MAJOR_VER,             // SMBIOS major version
            VPOX_SMBIOS_MINOR_VER,             // SMBIOS minor version
            VPOX_SMBIOS_MAXSS,                 // Maximum structure size
        ...
        // The DMI header
        {
            ...
            0x00,                              // checksum
            0,                                 // DMI tables length
            VPOX_DMI_TABLE_BASE,               // DMI tables base
            0,                                 // DMI tables entries
            VPOX_DMI_TABLE_VER,                // DMI version
        }
    };
...
void FwCommonPlantMpsTable(PPDMDEVINS pDevIns, uint8_t *pTable, unsigned cbMax, uint16_t cCpus)
{
    ...
    memcpy(pCfgTab->au8OemId, "VPOXCPU ", 8);
    memcpy(pCfgTab->au8ProductId, "VirtualPox  ", 12);
    ...
    AssertMsg(pCfgTab->u16Length < cbMax,
              ("VPOX_MPS_TABLE_SIZE=%d, maximum allowed size is %d",
