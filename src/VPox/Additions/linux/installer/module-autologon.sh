...
## @file
# VirtualPox Linux Guest Additions installer - autologon module
#
...
mod_autologon_install_ex()
{
    ...
    add_symlink "$INSTALLATION_DIR/other/vpox-greeter.desktop" "$greeter_dir/vpox-greeter.desktop"

    # Backup and activate greeter config.
    if ! test -f "$lightdm_config.vpox-backup"; then
        info "Backing up LightDM configuration file ..."
        cp "$lightdm_config" "$lightdm_config.vpox-backup" || return 1
        chmod 644 "$lightdm_config.vpox-backup" || return 1
    fi
    sed -i -e 's/^\s*greeter-session\s*=.*/greeter-session=vpox-greeter/g' "$lightdm_config" || return 1
...
mod_autologon_uninstall()
{
    ...
    if test -f "$MOD_AUTOLOGON_LIGHTDM_CONFIG.vpox-backup"; then
        mv "$MOD_AUTOLOGON_LIGHTDM_CONFIG.vpox-backup" "$MOD_AUTOLOGON_LIGHTDM_CONFIG"
        if test "$?" -ne "0"; then
