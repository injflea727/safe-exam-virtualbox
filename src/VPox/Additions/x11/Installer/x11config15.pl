...
# the graphics card (not of vpoxmouse as far as I can see).  Although this
...
# for use on a VirtualPox guest) and replace them with vpoxvideo.
...
foreach $cfg (@cfg_files) {
    ...
    if (open(CFG, $cfg)) {
        ...
        while (defined ($line = <CFG>)) {
            ...
            if ($in_section) {
                if ($line =~ /^\s*driver\s+\"(fbdev|vga|vesa|vpoxvideo|ChangeMe)\"/i) {
                    $line =~ s/(fbdev|vga|vesa|vpoxvideo|ChangeMe)/vpoxvideo/i;
                }
            }
            print TMP $line;
        }
        ...
        # We do not overwrite existing $cfg.".vpox" files because that will
