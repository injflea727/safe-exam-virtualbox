...
/** @file
 * VPox Qt GUI - UIMachineSettingsUSB class implementation.
 */
...
class VPoxUSBMenu : public QMenu
{
    Q_OBJECT;
...
public:
    ...
    VPoxUSBMenu(QWidget *)
    {
        connect(this, &VPoxUSBMenu::aboutToShow, this, &VPoxUSBMenu::processAboutToShow);
    }
...
bool UIMachineSettingsUSB::validate(QList<UIValidationMessage> &messages)
{
    ...
    const CExtPack extPack = uiCommon().virtualPox().GetExtensionPackManager().Find(GUI_ExtPackName);
    if (   mGbUSB->isChecked()
        ...
        && (extPack.isNull() || !extPack.GetUsable()))
    {
        ...
        message.second << tr("USB 2.0/3.0 is currently enabled for this virtual machine. "
                             ...
                             "Please install the Extension Pack from the VirtualPox download site "
...
void UIMachineSettingsUSB::prepareFiltersToolbar()
{
    ...
    AssertPtrReturnVoid(m_pFiltersToolBar);
    {
        ...
        m_pMenuUSBDevices = new VPoxUSBMenu(this);
...
void UIMachineSettingsUSB::prepareConnections()
{
    ...
    connect(m_pMenuUSBDevices, &VPoxUSBMenu::triggered,
