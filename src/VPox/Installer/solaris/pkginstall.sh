...
#
# If you just installed VirtualPox using IPS/pkg(5), you should run this
# script once to avoid rebooting the system before using VirtualPox.
#
...
${PKG_INSTALL_ROOT:=/}/opt/VirtualPox/vpoxconfig.sh --preremove --fatal ${ISIPS} ${DEBUGOPT}

if test "$?" -eq 0; then
    echo "Installing new ones..."
    $PKG_INSTALL_ROOT/opt/VirtualPox/vpoxconfig.sh --postinstall ${DEBUGOPT}
    rc=$?
