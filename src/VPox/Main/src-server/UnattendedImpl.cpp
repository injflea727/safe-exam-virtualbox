 ...
 */
HRESULT Unattended::initUnattended(VirtualPox *aParent)
{
...
{
    DETECTBUFFER uBuf;
    VPOXOSTYPE enmOsType = VPOXOSTYPE_Unknown;
    HRESULT hrc = i_innerDetectIsoOSWindows(hVfsIso, &uBuf, &enmOsType);
    if (hrc == S_FALSE && enmOsType == VPOXOSTYPE_Unknown)
        hrc = i_innerDetectIsoOSLinux(hVfsIso, &uBuf, &enmOsType);
    if (enmOsType != VPOXOSTYPE_Unknown)
    {
 ...
 *                      VPOXOSTYPE_Unknown.
 */
HRESULT Unattended::i_innerDetectIsoOSWindows(RTVFS hVfsIso, DETECTBUFFER *pBuf, VPOXOSTYPE *penmOsType)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        *penmOsType = VPOXOSTYPE_WinNT_x64;
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (   RTStrNICmp(pBuf->sz, RT_STR_TUPLE("vista")) == 0
                    ...
                    *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_WinVista);
                ...
                else if (RTStrNICmp(pBuf->sz, RT_STR_TUPLE("longhorn_rtm")) == 0)
                {
                    ...
                    *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_Win7);
                    ...
                    *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_Win81);
                else if (   RTStrNICmp(pBuf->sz, RT_STR_TUPLE("win8")) == 0
                         || RTStrNICmp(pBuf->sz, RT_STR_TUPLE("winmain_win8")) == 0 )
                    *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_Win8);
                else if (RTStrNICmp(pBuf->sz, RT_STR_TUPLE("th1")) == 0)
                {
                    pszVersion = "1507";    // aka. GA, retroactively 1507
                    *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_Win10);
                }
                ...
                {
                    pszVersion = "21H2";    // ??
                    *penmOsType = VPOXOSTYPE_Win11_x64;
                }
                else
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (RT_SUCCESS(vrc))
                {
                    ...
                    {
                        fClarifyProd = true;
                        *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_Win2k3);
                    ...
                    {
                        *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_WinXP);
                    ...
                    {
                        *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_Win2k);
 ...
 */
static bool detectLinuxArch(const char *pszArch, VPOXOSTYPE *penmOsType, VPOXOSTYPE enmBaseOsType)
{
    ...
    {
        *penmOsType = (VPOXOSTYPE)(enmBaseOsType | VPOXOSTYPE_x64);
        return true;
    }
...
static bool detectLinuxDistroName(const char *pszOsAndVersion, VPOXOSTYPE *penmOsType, const char **ppszNext)
{
    bool fRet = true;
...

    {
        ...
        if (   RTStrNICmp(pszOsAndVersion, RT_STR_TUPLE("Hat")) == 0
            && !RT_C_IS_ALNUM(pszOsAndVersion[3]))
        {
            *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_RedHat);
            pszOsAndVersion = RTStrStripL(pszOsAndVersion + 3);
        }
        else
            fRet = false;
    ...
    {
        *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_Oracle);
             ...
             && !RT_C_IS_ALNUM(pszOsAndVersion[6]))
    {
        *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_RedHat);
        pszOsAndVersion = RTStrStripL(pszOsAndVersion + 6);
             ...
             && !RT_C_IS_ALNUM(pszOsAndVersion[6]))
    {
        *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_FedoraCore);
        pszOsAndVersion = RTStrStripL(pszOsAndVersion + 6);
    }
    else if (   RTStrNICmp(pszOsAndVersion, RT_STR_TUPLE("Ubuntu")) == 0
             && !RT_C_IS_ALNUM(pszOsAndVersion[6]))
    {
        *penmOsType = (VPOXOSTYPE)((*penmOsType & VPOXOSTYPE_x64) | VPOXOSTYPE_Ubuntu);
        pszOsAndVersion = RTStrStripL(pszOsAndVersion + 6);
    }
 ...
 *                      VPOXOSTYPE_Unknown.
 */
HRESULT Unattended::i_innerDetectIsoOSLinux(RTVFS hVfsIso, DETECTBUFFER *pBuf, VPOXOSTYPE *penmOsType)
{
    /*
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (!detectLinuxArch(pBuf->sz, penmOsType, VPOXOSTYPE_RedHat))
        ...
        if (*penmOsType != VPOXOSTYPE_Unknown)
            return S_FALSE;
    }
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (!detectLinuxArch(apszLines[2], penmOsType, VPOXOSTYPE_RedHat))
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        {
            if (!detectLinuxArch(pszArch, penmOsType, VPOXOSTYPE_Ubuntu))
...
HRESULT Unattended::prepare()
{
     ...
     * VirtualPox because their locks have higher priorities than ours.
     */
    ...
    try
    {
        ...
        if (mStrHostname.isEmpty())
        {
            ...
            mStrHostname.append(".myguest.virtualpox.org");
        }
    ...
    meGuestOsType     = idxOSType < Global::cOSTypes ? Global::sOSTypes[idxOSType].osType : VPOXOSTYPE_Unknown;
...
HRESULT Unattended::reconfigureVM()
{
    ...
    /*
     * Interrogate VirtualPox/IGuestOSType before we lock stuff and create ordering issues.
     */
                                         ...
                                         ComPtr<IMachine> const &rPtrSessionMachine)
{
     ...
     *             the VirtualPox lock (via i_saveHardware/NetworkAdaptger::i_hasDefaults/VirtualPox::i_findGuestOSType).
     */
    if (SUCCEEDED(hrc))
    {
        rAutoLock.release();
...
VPOXOSTYPE Unattended::i_getGuestOsType() const
{
    Assert(isReadLockedOnCurrentThread());
                                  ...
                                  AutoMultiWriteLock2 &rLock)
{
    ...
    LogRelFlowFunc(("VirtualPox::openMedium -> %Rhrc\n", rc));
    if (SUCCEEDED(rc))
    {
