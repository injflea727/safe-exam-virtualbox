 ...
 *
 * @returns VPox status code, or VERR_SYMBOL_NOT_FOUND if the new API is not available.
...
/**
 * Converts a (registered or standard) Windows clipboard format to a VPox clipboard format.
 *
 * @returns Converted VPox clipboard format, or VPOX_SHCL_FMT_NONE if not found.
 * @param   uFormat             Windows clipboard format to convert.
 */
SHCLFORMAT SharedClipboardWinClipboardFormatToVPox(UINT uFormat)
{
    /* Insert the requested clipboard format data into the clipboard. */
    SHCLFORMAT vpoxFormat = VPOX_SHCL_FMT_NONE;

    switch (uFormat)
    {
        case CF_UNICODETEXT:
            vpoxFormat = VPOX_SHCL_FMT_UNICODETEXT;
            break;

        case CF_DIB:
            vpoxFormat = VPOX_SHCL_FMT_BITMAP;
            break;
        ...
        case CF_HDROP:
            vpoxFormat = VPOX_SHCL_FMT_URI_LIST;
            break;
#endif
    ...
    LogFlowFunc(("uFormat=%u -> vpoxFormat=0x%x\n", uFormat, vpoxFormat));
    return vpoxFormat;
}
...
int SharedClipboardWinGetFormats(PSHCLWINCTX pCtx, PSHCLFORMATS pfFormats)
{
    ...
    SHCLFORMATS fFormats = VPOX_SHCL_FMT_NONE;
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        while ((uCurFormat = EnumClipboardFormats(uCurFormat)) != 0)
            fFormats |= SharedClipboardWinClipboardFormatToVPox(uCurFormat);
 ...
 *
 * @returns VPox status code. VERR_NOT_SUPPORTED if the format is not supported / handled.
 * @param   pWinCtx             Windows context to use.
...
static int sharedClipboardWinAnnounceFormats(PSHCLWINCTX pWinCtx, SHCLFORMATS fFormats)
{
    ...
    static struct
    {
        uint32_t        fVPoxFormat;
        UINT            uWinFormat;
        const char     *pszWinFormat;
        const char     *pszLog;
    } s_aFormats[] =
    {
        { VPOX_SHCL_FMT_UNICODETEXT,    CF_UNICODETEXT, NULL,                 "CF_UNICODETEXT" },
        { VPOX_SHCL_FMT_BITMAP,         CF_DIB,         NULL,                 "CF_DIB" },
        { VPOX_SHCL_FMT_HTML,           0,              SHCL_WIN_REGFMT_HTML, "SHCL_WIN_REGFMT_HTML" },
    };
    ...
    {
        if (fFormatsLeft & s_aFormats[i].fVPoxFormat)
        {
            LogFunc(("%s\n", s_aFormats[i].pszLog));
            fFormatsLeft &= ~s_aFormats[i].fVPoxFormat;
