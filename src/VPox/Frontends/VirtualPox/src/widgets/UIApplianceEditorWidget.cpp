...
/** @file
 * VPox Qt GUI - UIApplianceEditorWidget class implementation.
 */
    ...
    if (idx.column() == ApplianceViewSection_ConfigValue)
    {
        switch (m_enmVSDType)
        {
            ...
            case KVirtualSystemDescriptionType_PrimaryGroup:
            {
                ...
                QVector<QString> groupsVector = uiCommon().virtualPox().GetMachineGroups();
...
void UIApplianceEditorWidget::initSystemSettings()
{
    if (m_minGuestRAM == -1)
    {
        /* We need some global defaults from the current VirtualPox
           installation */
        CSystemProperties sp = uiCommon().virtualPox().GetSystemProperties();
