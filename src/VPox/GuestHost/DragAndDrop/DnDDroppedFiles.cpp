                          ...
                          const char *pszPath, DNDURIDROPPEDFILEFLAGS fFlags /* = DNDURIDROPPEDFILE_FLAGS_NONE */)
{
    ...
    do
    {
        ...
        rc = RTPathAppend(szDropDir, sizeof(szDropDir), "VirtualPox Dropped Files"); /** @todo Make this tag configurable? */
        if (RT_FAILURE(rc))
            break;
 ...
 *
 * @returns VPox status code, or VERR_NOT_FOUND if the dropped files directory has not been opened before.
