/* $Id: VPoxModBallooning.cpp $ */
/** @file
 * VPoxModBallooning - Module for handling the automatic ballooning of VMs.
 */
...
#define VPOX_MOD_BALLOONING_NAME "balloon"
...
typedef struct VPOXWATCHDOG_BALLOONCTRL_PAYLOAD
{
...
} VPOXWATCHDOG_BALLOONCTRL_PAYLOAD, *PVPOXWATCHDOG_BALLOONCTRL_PAYLOAD;
...
*********************************************************************************************************************************/
static int balloonSetSize(PVPOXWATCHDOG_MACHINE pMachine, uint32_t cMbBalloonCur);
 ...
 */
static int32_t balloonGetDelta(PVPOXWATCHDOG_MACHINE pMachine, uint32_t cMbGuestMemFree,
 ...
 */
static uint32_t balloonGetMaxSize(PVPOXWATCHDOG_MACHINE pMachine)
{
    /*
     * Is a maximum ballooning size set? Make sure we're within bounds.
     *
     * The maximum balloning size can be set
     * - via global extra-data ("VPoxInternal/Guest/BalloonSizeMax")
    ...
    Bstr strValue;
    HRESULT hr = g_pVirtualPox->GetExtraData(Bstr("VPoxInternal/Guest/BalloonSizeMax").raw(),
                                             strValue.asOutParam());
    if (   SUCCEEDED(hr)
        && strValue.isNotEmpty())
    {
 ...
 */
static int balloonGetCurrentSize(PVPOXWATCHDOG_MACHINE pMachine, uint32_t *pcMbBalloonCur)
{
    LONG cKbBalloonCur;
 ...
 * @param   pMachine                Machine to determine maximum ballooning size for.
 */
static uint32_t balloonGetRequestedSize(PVPOXWATCHDOG_MACHINE pMachine)
{
    const ComPtr<IMachine> &rptrMachine = pMachine->machine;

    /*
     * The maximum balloning size can be set
     * - via per-VM extra-data ("VPoxInternal2/Watchdog/BalloonCtrl/BalloonSizeMax")
     * - via per-VM extra-data (legacy) ("VPoxInternal/Guest/BalloonSizeMax")
     *
     * Precedence from top to bottom.
     */
    ...
    Bstr strValue;
    HRESULT hr = rptrMachine->GetExtraData(Bstr("VPoxInternal2/Watchdog/BalloonCtrl/BalloonSizeMax").raw(),
                                           strValue.asOutParam());
    if (   SUCCEEDED(hr)
        && strValue.isNotEmpty())
    {
    ...
    {
        hr = rptrMachine->GetExtraData(Bstr("VPoxInternal/Guest/BalloonSizeMax").raw(),
                                       strValue.asOutParam());
        if (   SUCCEEDED(hr)
            && strValue.isNotEmpty())
        {
 ...
 */
static bool balloonIsEnabled(PVPOXWATCHDOG_MACHINE pMachine)
{
    const ComPtr<IMachine> &rptrMachine = pMachine->machine;
    ...
    Bstr strValue;
    HRESULT hr = g_pVirtualPox->GetExtraData(Bstr("VPoxInternal/Guest/BalloonEnabled").raw(),
                                             strValue.asOutParam());
    if (   SUCCEEDED(hr)
        && strValue.isNotEmpty())
    {
    ...
    {
        hr = rptrMachine->GetExtraData(Bstr("VPoxInternal2/Watchdog/BalloonCtrl/BalloonEnabled").raw(),
                                       strValue.asOutParam());
        if (SUCCEEDED(hr))
        {
            if (g_fVerbose)
...
int balloonMachineSetup(const Bstr& strUuid)
{
    ...
    do
    {
        PVPOXWATCHDOG_MACHINE pMachine = getMachine(strUuid);
        ...
        CHECK_ERROR_BREAK(g_pVirtualPox, COMGETTER(PerformanceCollector)(coll.asOutParam()));
 ...
 * @param   pMachine                Pointer to the machine's internal structure.
 */
static int balloonMachineUpdate(PVPOXWATCHDOG_MACHINE pMachine)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        PVPOXWATCHDOG_BALLOONCTRL_PAYLOAD pData;
        pData = (PVPOXWATCHDOG_BALLOONCTRL_PAYLOAD)payloadFrom(pMachine, VPOX_MOD_BALLOONING_NAME);
        AssertPtr(pData);
...
/* Callbacks. */
static DECLCALLBACK(int) VPoxModBallooningPreInit(void)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) VPoxModBallooningOption(int argc, char *argv[], int *piConsumed)
{
...
static DECLCALLBACK(int) VPoxModBallooningInit(void)
{
    if (!g_cMsMemoryBalloonTimeout)
        cfgGetValueU32(g_pVirtualPox, NULL /* Machine */,
                       "VPoxInternal2/Watchdog/BalloonCtrl/TimeoutMS", NULL /* Per-machine */,
                       &g_cMsMemoryBalloonTimeout, 30 * 1000 /* Default is 30 seconds timeout. */);

    if (!g_cMbMemoryBalloonIncrement)
        cfgGetValueU32(g_pVirtualPox, NULL /* Machine */,
                       "VPoxInternal2/Watchdog/BalloonCtrl/BalloonIncrementMB", NULL /* Per-machine */,
                       &g_cMbMemoryBalloonIncrement, 256);

    if (!g_cMbMemoryBalloonDecrement)
        cfgGetValueU32(g_pVirtualPox, NULL /* Machine */,
                       "VPoxInternal2/Watchdog/BalloonCtrl/BalloonDecrementMB", NULL /* Per-machine */,
                       &g_cMbMemoryBalloonDecrement, 128);

    if (!g_cMbMemoryBalloonLowerLimit)
        cfgGetValueU32(g_pVirtualPox, NULL /* Machine */,
                       "VPoxInternal2/Watchdog/BalloonCtrl/BalloonLowerLimitMB", NULL /* Per-machine */,
                       &g_cMbMemoryBalloonLowerLimit, 128);

    return VINF_SUCCESS;
}

static DECLCALLBACK(int) VPoxModBallooningMain(void)
{
...
static DECLCALLBACK(int) VPoxModBallooningStop(void)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(void) VPoxModBallooningTerm(void)
{
}

static DECLCALLBACK(int) VPoxModBallooningOnMachineRegistered(const Bstr &strUuid)
{
    PVPOXWATCHDOG_MACHINE pMachine = getMachine(strUuid);
    AssertPtrReturn(pMachine, VERR_INVALID_PARAMETER);

    PVPOXWATCHDOG_BALLOONCTRL_PAYLOAD pData;
    int rc = payloadAlloc(pMachine, VPOX_MOD_BALLOONING_NAME,
                          sizeof(VPOXWATCHDOG_BALLOONCTRL_PAYLOAD), (void**)&pData);
    if (RT_SUCCESS(rc))
...
static DECLCALLBACK(int) VPoxModBallooningOnMachineUnregistered(const Bstr &strUuid)
{
    PVPOXWATCHDOG_MACHINE pMachine = getMachine(strUuid);
    AssertPtrReturn(pMachine, VERR_INVALID_PARAMETER);

    payloadFree(pMachine, VPOX_MOD_BALLOONING_NAME);

    return VINF_SUCCESS;
}

static DECLCALLBACK(int) VPoxModBallooningOnMachineStateChanged(const Bstr &strUuid,
...
static DECLCALLBACK(int) VPoxModBallooningOnServiceStateChanged(bool fAvailable)
{
    RT_NOREF(fAvailable);
    return VINF_SUCCESS;
}
 ...
 */
VPOXMODULE g_ModBallooning =
{
    /* pszName. */
    VPOX_MOD_BALLOONING_NAME,
    /* pszDescription. */
...
#if 1
    /* (Legacy) note. */
    "Set \"VPoxInternal/Guest/BalloonSizeMax\" for a per-VM maximum ballooning size.\n"
#endif
    "--balloon-safety-margin Free memory when deflating a balloon in MB (1024 MB).\n"
    ,
    /* methods. */
    VPoxModBallooningPreInit,
    VPoxModBallooningOption,
    VPoxModBallooningInit,
    VPoxModBallooningMain,
    VPoxModBallooningStop,
    VPoxModBallooningTerm,
    /* callbacks. */
    VPoxModBallooningOnMachineRegistered,
    VPoxModBallooningOnMachineUnregistered,
    VPoxModBallooningOnMachineStateChanged,
    VPoxModBallooningOnServiceStateChanged
};
