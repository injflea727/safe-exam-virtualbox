/* $Id: VPoxNetCfg.cpp $ */
/** @file
 * VPoxNetCfg.cpp - Network Configuration API.
 */
/*
...
#define VPOX_NETCFG_LOCK_TIME_OUT     5000  /** @todo r=bird: What does this do? */

#define VPOXNETCFGWIN_NETADP_ID L"sun_VPoxNetAdp"
...
/*
* Forward declaration for using vpoxNetCfgWinSetupMetric()
*/
HRESULT vpoxNetCfgWinSetupMetric(IN NET_LUID* pLuid);
HRESULT vpoxNetCfgWinGetInterfaceLUID(IN HKEY hKey, OUT NET_LUID* pLUID);
...
static HRESULT vpoxNetCfgWinINetCfgLock(IN INetCfg *pNetCfg,
...
static HRESULT vpoxNetCfgWinINetCfgUnlock(IN INetCfg *pNetCfg)
{
    INetCfgLock *pLock;
    HRESULT hr = pNetCfg->QueryInterface(IID_INetCfgLock, (PVOID*)&pLock);
    if (FAILED(hr))
                                                      ...
                                                      OUT LPWSTR *ppszwClientDescription)
{
    ...
    if (fGetWriteLock)
    {
        hr = vpoxNetCfgWinINetCfgLock(pNetCfg, pszwClientDescription, cmsTimeout, ppszwClientDescription);
        if (hr == S_FALSE)
        {
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinReleaseINetCfg(IN INetCfg *pNetCfg, IN BOOL fHasWriteLock)
{
    ...
    if (fHasWriteLock)
    {
        HRESULT hr2 = vpoxNetCfgWinINetCfgUnlock(pNetCfg);
        if (FAILED(hr2))
            NonStandardLogFlow(("vpoxNetCfgWinINetCfgUnlock failed, hr (0x%x)\n", hr2));
        if (SUCCEEDED(hr))
            hr = hr2;
    }
...
static HRESULT vpoxNetCfgWinGetComponentByGuidEnum(IEnumNetCfgComponent *pEnumNcc,
                                                   IN const GUID *pGuid,
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinGetComponentByGuid(IN INetCfg *pNc,
                                                            ...
                                                            OUT INetCfgComponent **ppncc)
{
    ...
    if (SUCCEEDED(hr))
    {
        hr = vpoxNetCfgWinGetComponentByGuidEnum(pEnumNcc, pComponentGuid, ppncc);
        if (hr == S_FALSE)
        {
        ...
        {
            NonStandardLogFlow(("vpoxNetCfgWinGetComponentByGuidEnum failed, hr (0x%x)\n", hr));
        }
        pEnumNcc->Release();
    }
    else
...
static HRESULT vpoxNetCfgWinQueryInstaller(IN INetCfg *pNetCfg, IN const GUID *pguidClass, INetCfgClassSetup **ppSetup)
{
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinInstallComponent(IN INetCfg *pNetCfg, IN LPCWSTR pszwComponentId, IN const GUID *pguidClass,
                                                          OUT INetCfgComponent **ppComponent)
{
    INetCfgClassSetup *pSetup;
    HRESULT hr = vpoxNetCfgWinQueryInstaller(pNetCfg, pguidClass, &pSetup);
    if (FAILED(hr))
    {
        NonStandardLogFlow(("vpoxNetCfgWinQueryInstaller failed, hr (0x%x)\n", hr));
        return hr;
    }
        ...
        if (pTempComponent != NULL)
        {
            ...
            */
            ...
            if (   SUCCEEDED(res)
                ...
                && wcsnicmp(pszwComponentId, VPOXNETCFGWIN_NETADP_ID, 256) == 0)
            {
                NET_LUID luid;
                res = vpoxNetCfgWinGetInterfaceLUID(hkey, &luid);
                ...
                if (FAILED(res))
                {
                     ...
                     */
                    NonStandardLogFlow(("VPoxNetCfgWinInstallComponent Warning! "
                                        "vpoxNetCfgWinGetInterfaceLUID failed, default metric "
                                        "for new interface will not be set, hr (0x%x)\n", res));
                }
                else
                {
                    res = vpoxNetCfgWinSetupMetric(&luid);
                    if (FAILED(res))
                    {
                        /*
                         *   The setting of Metric is not very important functionality,
                         ...
                         */
                        NonStandardLogFlow(("VPoxNetCfgWinInstallComponent Warning! "
                                            "vpoxNetCfgWinSetupMetric failed, default metric "
                                            "for new interface will not be set, hr (0x%x)\n", res));
                    }
                }
            }
...
static HRESULT vpoxNetCfgWinInstallInfAndComponent(IN INetCfg *pNetCfg, IN LPCWSTR pszwComponentId, IN const GUID *pguidClass,
                                                   ...
                                                   OUT INetCfgComponent **ppComponent)
{
    ...
    for (; cFilesProcessed < cInfPaths; cFilesProcessed++)
    {
        ...
        hr = VPoxDrvCfgInfInstall(apInfPaths[cFilesProcessed]);
        if (FAILED(hr))
        {
            NonStandardLogFlow(("VPoxNetCfgWinInfInstall failed, hr (0x%x)\n", hr));
            break;
        }
    }
    ...
    if (SUCCEEDED(hr))
    {
        hr = VPoxNetCfgWinInstallComponent(pNetCfg, pszwComponentId, pguidClass, ppComponent);
        if (FAILED(hr))
            NonStandardLogFlow(("VPoxNetCfgWinInstallComponent failed, hr (0x%x)\n", hr));
    }
    ...
    if (FAILED(hr))
    {
        ...
        do
        {
            HRESULT hr2 = VPoxDrvCfgInfUninstall(apInfPaths[cFilesProcessed], 0);
            if (FAILED(hr2))
                NonStandardLogFlow(("VPoxDrvCfgInfUninstall failed, hr (0x%x)\n", hr2));
                /* Keep going. */
            if (!cFilesProcessed)
                break;
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinUninstallComponent(IN INetCfg *pNetCfg, IN INetCfgComponent *pComponent)
{
    GUID GuidClass;
    ...
    hr = vpoxNetCfgWinQueryInstaller(pNetCfg, &GuidClass, &pSetup);
    if (FAILED(hr))
    {
        NonStandardLogFlow(("vpoxNetCfgWinQueryInstaller failed, hr (0x%x)\n", hr));
        return hr;
...
typedef BOOL (*VPOXNETCFGWIN_NETCFGENUM_CALLBACK) (IN INetCfg *pNetCfg, IN INetCfgComponent *pNetCfgComponent, PVOID pContext);

static HRESULT vpoxNetCfgWinEnumNetCfgComponents(IN INetCfg *pNetCfg,
                                                 IN const GUID *pguidClass,
                                                 VPOXNETCFGWIN_NETCFGENUM_CALLBACK callback,
                                                 PVOID pContext)
{
...
/*
 * Forward declarations of functions used in vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback.
 */
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinGenHostonlyConnectionName(PCWSTR DevName, WCHAR *pBuf, PULONG pcbBuf);
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinRenameConnection(LPWSTR pGuid, PCWSTR NewName);

static BOOL vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDev, PVOID pvContext)
{
    RT_NOREF1(pvContext);
                                     ...
                                     &rmdParams.ClassInstallHeader, sizeof(rmdParams)))
    {
        if (SetupDiSetSelectedDevice(hDevInfo, pDev))
        {
            ...
            if (hKey == INVALID_HANDLE_VALUE)
            {
                NonStandardLogFlow(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiOpenDevRegKey failed with error %ld\n",
                                    GetLastError()));
            }
            else
            {
                ...
                {
                    NonStandardLogFlow(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: Processing device ID \"%S\"\n",
                                        wszCfgGuidString));
                                                          ...
                                                          NULL                /* OUT PDWORD RequiredSize OPTIONAL*/))
                    {
                         ...
                         */
                        HRESULT hr = VPoxNetCfgWinGenHostonlyConnectionName(wszDevName, wszTempName, &cbName);
                        wcscat_s(wszTempName, sizeof(wszTempName), L" removed");
                        if (SUCCEEDED(hr))
                            hr = VPoxNetCfgWinRenameConnection(wszCfgGuidString, wszTempName);
                        //NonStandardLogFlow(("VPoxNetCfgWinRenameConnection(%S,%S) => 0x%x\n", pWCfgGuidString, TempName, hr_tmp));
                    }
                    else
                    {
                        NonStandardLogFlow(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: Failed to get friendly name for device \"%S\"\n",
                                            wszCfgGuidString));
                    }
                }
                else
                {
                    NonStandardLogFlow(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: Querying instance ID failed with %d\n",
                                        ret));
                }
            ...
            if (SetupDiCallClassInstaller(DIF_REMOVE, hDevInfo, pDev))
            {
                ...
                if (SetupDiGetDeviceInstallParams(hDevInfo, pDev, &devParams))
                {
                    ...
                    {
                        NonStandardLog(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: A reboot is required\n"));
                    }
                }
                else
                    NonStandardLogFlow(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiGetDeviceInstallParams failed with %ld\n",
                                        GetLastError()));
            }
            else
                NonStandardLogFlow(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiCallClassInstaller failed with %ld\n",
                                    GetLastError()));
        }
        else
            NonStandardLogFlow(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiSetSelectedDevice failed with %ld\n",
                                GetLastError()));
    }
    else
        NonStandardLogFlow(("vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiSetClassInstallParams failed with %ld\n",
                            GetLastError()));
...
typedef struct VPOXNECTFGWINPROPCHANGE
{
    VPOXNECTFGWINPROPCHANGE_TYPE enmPcType;
    HRESULT hr;
} VPOXNECTFGWINPROPCHANGE ,*PVPOXNECTFGWINPROPCHANGE;

static BOOL vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDev, PVOID pContext)
{
    PVPOXNECTFGWINPROPCHANGE pPc = (PVPOXNECTFGWINPROPCHANGE)pContext;
    ...
    {
        case VPOXNECTFGWINPROPCHANGE_TYPE_DISABLE:
            PcParams.StateChange = DICS_DISABLE;
            NonStandardLogFlow(("vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback: Change type (DICS_DISABLE): %d\n", pPc->enmPcType));
            break;
        case VPOXNECTFGWINPROPCHANGE_TYPE_ENABLE:
            PcParams.StateChange = DICS_ENABLE;
            NonStandardLogFlow(("vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback: Change type (DICS_ENABLE): %d\n", pPc->enmPcType));
            break;
        default:
            NonStandardLogFlow(("vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback: Unexpected prop change type: %d\n", pPc->enmPcType));
            pPc->hr = E_INVALIDARG;
            return FALSE;
    }
            ...
            if (SetupDiCallClassInstaller(DIF_PROPERTYCHANGE, hDevInfo, pDev))
            {
                ...
                if (SetupDiGetDeviceInstallParams(hDevInfo,pDev,&devParams))
                {
                    if (   (devParams.Flags & DI_NEEDRESTART)
                        || (devParams.Flags & DI_NEEDREBOOT))
                    {
                        NonStandardLog(("vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback: A reboot is required\n"));
                    }
                }
                else
                    NonStandardLogFlow(("vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback: SetupDiGetDeviceInstallParams failed with %ld\n",
                                        GetLastError()));
            }
            else
                NonStandardLogFlow(("vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback: SetupDiCallClassInstaller failed with %ld\n",
                                    GetLastError()));
        }
        else
...
typedef BOOL (*PFNVPOXNETCFGWINNETENUMCALLBACK)(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDev, PVOID pContext);
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinEnumNetDevices(LPCWSTR pwszPnPId,
                                                        PFNVPOXNETCFGWINNETENUMCALLBACK pfnCallback, PVOID pvContext)
{
    NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: Searching for: %S\n", pwszPnPId));
    ...
    if (hDevInfo != INVALID_HANDLE_VALUE)
    {
        ...
        for (;;)
        {
            ...
            NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: Enumerating device %ld ... \n", dwDevId));
            dwDevId++;
                                                   ...
                                                   &cbRequired       /* OUT PDWORD RequiredSize OPTIONAL */))
            {
                ...
                {
                    NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: SetupDiGetDeviceRegistryPropertyW (1) failed with %ld\n", winEr));
                    break;
                }
                ...
                if (!pBuffer)
                {
                    NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: Out of memory allocating %ld bytes\n",
                                        cbRequired));
                ...
                {
                    winEr = GetLastError();
                    NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: SetupDiGetDeviceRegistryPropertyW (2) failed with %ld\n",
                                        winEr));
                    break;
                }
            }
            ...
            NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: Device %ld: %S\n", dwDevId, pCurId));
        ...
        NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: Found %ld devices total\n", dwDevId));
    ...
    else
    {
        ...
        NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: SetupDiGetClassDevsExW failed with %ld\n", winEr));
        hr = HRESULT_FROM_WIN32(winEr);
    }

    NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices: Ended with hr (0x%x)\n", hr));
    return hr;
}

VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinRemoveAllNetDevicesOfId(IN LPCWSTR lpszPnPId)
{
    return VPoxNetCfgWinEnumNetDevices(lpszPnPId, vpoxNetCfgWinRemoveAllNetDevicesOfIdCallback, NULL);
}
...
{
    VPOXNECTFGWINPROPCHANGE Pc;
    Pc.enmPcType = enmPcType;
    Pc.hr = S_OK;
    NonStandardLogFlow(("Calling VPoxNetCfgWinEnumNetDevices with lpszPnPId =(%S) and vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback\n", lpszPnPId));

    HRESULT hr = VPoxNetCfgWinEnumNetDevices(lpszPnPId, vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback, &Pc);
    if (!SUCCEEDED(hr))
    {
        NonStandardLogFlow(("VPoxNetCfgWinEnumNetDevices failed 0x%x\n", hr));
        return hr;
    }

    if (!SUCCEEDED(Pc.hr))
    {
        NonStandardLogFlow(("vpoxNetCfgWinPropChangeAllNetDevicesOfIdCallback failed 0x%x\n", Pc.hr));
        return Pc.hr;
    }
...
static HRESULT netIfWinIsHostOnly(IWbemClassObject * pAdapterConfig, BOOL * pbIsHostOnly)
{
    ...
    if (SUCCEEDED(hr))
    {
        *pbIsHostOnly = bstr_t(vtServiceName.bstrVal) == bstr_t("VPoxNetAdp");
...
static HRESULT vpoxNetCfgWinIsDhcpEnabled(IWbemClassObject * pAdapterConfig, BOOL *pEnabled)
{
    VARIANT vtEnabled;
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinGetAdapterSettings(IN const GUID * pGuid, OUT PADAPTER_SETTINGS pSettings)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = vpoxNetCfgWinIsDhcpEnabled(pAdapterConfig, &pSettings->bDhcp);
            if (SUCCEEDED(hr))
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinIsDhcpEnabled(const GUID * pGuid, BOOL *pEnabled)
{
    HRESULT hr;
    ComPtr <IWbemServices> pSvc;
    hr = netIfWinCreateIWbemServices(pSvc.asOutParam());
...
{
    NonStandardLogFlow(("VPoxNetCfgWinEnableStaticIpConfig: ip=0x%x mask=0x%x\n", ip, mask));
    ComPtr<IWbemServices> pSvc;
    ...
    NonStandardLogFlow(("VPoxNetCfgWinEnableStaticIpConfig: returns 0x%x\n", hr));
    return hr;
}
...
static const char *vpoxNetCfgWinAddrToStr(char *pszBuf, LPSOCKADDR pAddr)
{
...
typedef bool (*PFNVPOXNETCFG_IPSETTINGS_CALLBACK) (ULONG ip, ULONG mask, PVOID pContext);

static void vpoxNetCfgWinEnumIpConfig(PIP_ADAPTER_ADDRESSES pAddresses, PFNVPOXNETCFG_IPSETTINGS_CALLBACK pfnCallback, PVOID pContext)
{
    ...
    for (pAdapter = pAddresses; pAdapter; pAdapter = pAdapter->Next)
    {
        ...
        {
            const char *pcszAddress = vpoxNetCfgWinAddrToStr(szBuf, pPrefix->Address.lpSockaddr);
...
static bool vpoxNetCfgWinIpProbeCallback (ULONG ip, ULONG mask, PVOID pContext)
{
    ...
    if (dwRc == NO_ERROR)
    {
        ...
        *pNetMask = 0;
        ...
        for (int i = 0; i < 255; i++)
        {
            ...
            vpoxNetCfgWinEnumIpConfig(pAddresses, vpoxNetCfgWinIpProbeCallback, &Context);
 ...
 */
#define VPOXNETCFGWIN_NETFLT_ID    L"sun_VPoxNetFlt"
#define VPOXNETCFGWIN_NETFLT_MP_ID L"sun_VPoxNetFltmp"

static HRESULT vpoxNetCfgWinNetFltUninstall(IN INetCfg *pNc, DWORD InfRmFlags)
{
    INetCfgComponent *pNcc = NULL;
    HRESULT hr = pNc->FindComponent(VPOXNETCFGWIN_NETFLT_ID, &pNcc);
    if (hr == S_OK)
    {
        NonStandardLog("NetFlt is installed currently, uninstalling ...\n");

        hr = VPoxNetCfgWinUninstallComponent(pNc, pNcc);
    ...
    VPoxDrvCfgInfUninstallAllF(L"NetService", VPOXNETCFGWIN_NETFLT_ID, InfRmFlags);
    VPoxDrvCfgInfUninstallAllF(L"Net", VPOXNETCFGWIN_NETFLT_MP_ID, InfRmFlags);
...
{
    return vpoxNetCfgWinNetFltUninstall(pNc, 0);
}

VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinNetFltInstall(IN INetCfg *pNc,
                                                       IN LPCWSTR const *apInfFullPaths, IN UINT cInfFullPaths)
{
    HRESULT hr = vpoxNetCfgWinNetFltUninstall(pNc, SUOI_FORCEDELETE);
    if (SUCCEEDED(hr))
    {
        NonStandardLog("NetFlt will be installed ...\n");
        hr = vpoxNetCfgWinInstallInfAndComponent(pNc, VPOXNETCFGWIN_NETFLT_ID,
...
static HRESULT vpoxNetCfgWinNetAdpUninstall(IN INetCfg *pNc, LPCWSTR pwszId, DWORD InfRmFlags)
{
    NOREF(pNc);
    NonStandardLog("Finding NetAdp driver package and trying to uninstall it ...\n");

    VPoxDrvCfgInfUninstallAllF(L"Net", pwszId, InfRmFlags);
...
{
    return vpoxNetCfgWinNetAdpUninstall(pNc, pwszId, SUOI_FORCEDELETE);
}

VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinNetAdpInstall(IN INetCfg *pNc,
                                                       IN LPCWSTR const pInfFullPath)
{
    NonStandardLog("NetAdp will be installed ...\n");
    HRESULT hr = vpoxNetCfgWinInstallInfAndComponent(pNc, VPOXNETCFGWIN_NETADP_ID,
                                             &GUID_DEVCLASS_NET,
                                             &pInfFullPath,
                                             1,
                                             NULL);
...
#define VPOXNETCFGWIN_NETLWF_ID    L"oracle_VPoxNetLwf"

static HRESULT vpoxNetCfgWinNetLwfUninstall(IN INetCfg *pNc, DWORD InfRmFlags)
{
    INetCfgComponent * pNcc = NULL;
    HRESULT hr = pNc->FindComponent(VPOXNETCFGWIN_NETLWF_ID, &pNcc);
    if (hr == S_OK)
    {
    ...
    VPoxDrvCfgInfUninstallAllF(L"NetService", VPOXNETCFGWIN_NETLWF_ID, InfRmFlags);
...
{
    return vpoxNetCfgWinNetLwfUninstall(pNc, 0);
}

static void VPoxNetCfgWinFilterLimitWorkaround(void)
{
    /*
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinNetLwfInstall(IN INetCfg *pNc,
                                                       IN LPCWSTR const pInfFullPath)
{
    HRESULT hr = vpoxNetCfgWinNetLwfUninstall(pNc, SUOI_FORCEDELETE);
    if (SUCCEEDED(hr))
    {
        VPoxNetCfgWinFilterLimitWorkaround();
        NonStandardLog("NetLwf will be installed ...\n");
        hr = vpoxNetCfgWinInstallInfAndComponent(pNc, VPOXNETCFGWIN_NETLWF_ID,
                                                 &GUID_DEVCLASS_NETSERVICE,
                                                 &pInfFullPath,
                                                 1,
                                                 NULL);
...
#define VPOX_CONNECTION_NAME L"VirtualPox Host-Only Network"
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinGenHostonlyConnectionName(PCWSTR DevName, WCHAR *pBuf, PULONG pcbBuf)
{
    const WCHAR * pSuffix = wcsrchr( DevName, L'#' );
    ULONG cbSize = sizeof(VPOX_CONNECTION_NAME);
    ...
    wcscpy(pBuf, VPOX_CONNECTION_NAME);
    if (pSuffix)
    {
        wcscat(pBuf, L" ");
        wcscat(pBuf, pSuffix);
...
static BOOL vpoxNetCfgWinAdjustHostOnlyNetworkInterfacePriority(IN INetCfg *pNc, IN INetCfgComponent *pNcc, PVOID pContext)
{
    RT_NOREF1(pNc);
                                ...
                                if (hr == S_OK)
                                {
                                    ...
                                    if (hr == S_OK)
                                    {
                                            ...
                                            && guid == *pGuid)
                                        {
                                            ...
                                            else if (_wcsnicmp(pwszHwId, VPOXNETCFGWIN_NETLWF_ID,
                                                               sizeof(VPOXNETCFGWIN_NETLWF_ID)/2))
...
static UINT WINAPI vpoxNetCfgWinPspFileCallback(
        PVOID Context,
        UINT Notification,
        UINT_PTR Param1,
        UINT_PTR Param2
...
/* The original source of the VPoxNetAdp adapter creation/destruction code has the following copyright */
/*
...
static bool vpoxNetCfgWinDetectStaleConnection(PCWSTR pName)
{
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinRenameConnection (LPWSTR pGuid, PCWSTR NewName)
{
     ...
     */
    if (vpoxNetCfgWinDetectStaleConnection(NewName))
        return S_OK;
...
#define DRIVERHWID _T("sun_VPoxNetAdp")
...
{
    return VPoxDrvCfgDrvUpdate(pcsxwId, pcsxwInf, pbRebootRequired);
}

static const char *vpoxNetCfgWinGetStateText(DWORD dwState)
{
    switch (dwState)
    {
...
static DWORD vpoxNetCfgWinGetNetSetupState(SC_HANDLE hService)
{
    SERVICE_STATUS status;
    status.dwCurrentState = SERVICE_RUNNING;
    if (hService) {
        if (QueryServiceStatus(hService, &status))
            NonStandardLogFlow(("NetSetupSvc %s\n", vpoxNetCfgWinGetStateText(status.dwCurrentState)));
        else
...
DECLINLINE(bool) vpoxNetCfgWinIsNetSetupRunning(SC_HANDLE hService)
{
    return vpoxNetCfgWinGetNetSetupState(hService) == SERVICE_RUNNING;
}

DECLINLINE(bool) vpoxNetCfgWinIsNetSetupStopped(SC_HANDLE hService)
{
    return vpoxNetCfgWinGetNetSetupState(hService) == SERVICE_STOPPED;
}
...
static BOOL vpoxNetCfgWinRenameHostOnlyNetworkInterface(IN INetCfg *pNc, IN INetCfgComponent *pNcc, PVOID pContext)
{
    RT_NOREF1(pNc);
...
/* We assume the following name matches the device description in vpoxnetadp6.inf */
#define HOSTONLY_ADAPTER_NAME "VirtualPox Host-Only Ethernet Adapter"
 ...
 */
static HRESULT vpoxNetCfgWinNextAvailableDevName(bstr_t& bstrName)
{
    SP_DEVINFO_DATA DeviceInfoData;
    /* initialize the structure size */
    DeviceInfoData.cbSize = sizeof (SP_DEVINFO_DATA);
...
static HRESULT vpoxNetCfgWinCreateHostOnlyNetworkInterface(IN LPCWSTR pInfPath, IN bool bIsInfPathFile, IN BSTR bstrDesiredName,
                                                           OUT GUID *pGuid, OUT BSTR *lppszName, OUT BSTR *pErrMsg)
{
    ...
    {
        hrc = vpoxNetCfgWinNextAvailableDevName(bstrNewInterfaceName);
        if (FAILED(hrc))
            NonStandardLogFlow(("vpoxNetCfgWinNextAvailableDevName failed with 0x%x\n", hrc));
    }
    ...
    do
    {
        ...
        if (fResult)
        {
            ...
            if (pQueueCallbackContext)
            {
                ...
                DeviceInstallParams.InstallMsgHandler = (PSP_FILE_CALLBACK)vpoxNetCfgWinPspFileCallback;
                fResult = SetupDiSetDeviceInstallParams (hDeviceInfo, &DeviceInfoData,
                                                    &DeviceInstallParams);
                if (!fResult)
                {
        ...
        NET_LUID luid;
        HRESULT hSMRes = vpoxNetCfgWinGetInterfaceLUID(hkey, &luid);
        ...
        if (FAILED(hSMRes))
        {
            ...
            *   So we will not break installation process due to this error.
            */
            NonStandardLogFlow(("vpoxNetCfgWinCreateHostOnlyNetworkInterface Warning! "
                                "vpoxNetCfgWinGetInterfaceLUID failed, default metric "
                                "for new interface will not be set, hr (0x%x)\n", hSMRes));
        }
        else
        {
             ...
             *   See @bugref{6379} for details.
             */
            hSMRes = vpoxNetCfgWinSetupMetric(&luid);
            if (FAILED(hSMRes))
            {
                /*
                 *   The setting of Metric is not very important functionality,
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        hr = VPoxNetCfgWinQueryINetCfg(&pNetCfg, TRUE, L"VirtualPox Host-Only Creation",
        ...
        if (hr == S_OK)
        {
            if (!!bstrNewInterfaceName)
            {
                ...
                context.pGuid = pGuid;
                hr = vpoxNetCfgWinEnumNetCfgComponents(pNetCfg,
                                                       &GUID_DEVCLASS_NET,
                                                       vpoxNetCfgWinRenameHostOnlyNetworkInterface,
                                                       &context);
            }
            if (SUCCEEDED(hr))
                hr = vpoxNetCfgWinEnumNetCfgComponents(pNetCfg,
                                                       &GUID_DEVCLASS_NETSERVICE,
                                                       vpoxNetCfgWinAdjustHostOnlyNetworkInterfacePriority,
                                                       pGuid);
            if (SUCCEEDED(hr))
                hr = vpoxNetCfgWinEnumNetCfgComponents(pNetCfg,
        ...
        else if (hr == NETCFG_E_NO_WRITE_LOCK && lpszApp)
        {
            ...
            NonStandardLogFlow(("VPoxNetCfgWinQueryINetCfg failed, hr 0x%x\n", hr));
        ...
        hr = VPoxNetCfgWinGenHostonlyConnectionName(DevName, ConnectionName, &cbName);
        if (SUCCEEDED(hr))
            hr = VPoxNetCfgWinRenameConnection(pWCfgGuidString, ConnectionName);
#endif
        if (lppszName)
        {
...
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinCreateHostOnlyNetworkInterface(IN LPCWSTR pInfPath, IN bool bIsInfPathFile, IN BSTR pwsDesiredName,
                                                                        OUT GUID *pGuid, OUT BSTR *lppszName, OUT BSTR *pErrMsg)
{
    HRESULT hrc = vpoxNetCfgWinCreateHostOnlyNetworkInterface(pInfPath, bIsInfPathFile, pwsDesiredName, pGuid, lppszName, pErrMsg);
    if (hrc == E_ABORT)
    {
         ...
         */
        hrc = vpoxNetCfgWinCreateHostOnlyNetworkInterface(pInfPath, bIsInfPathFile, pwsDesiredName, pGuid, lppszName, pErrMsg);
        if (hrc == E_ABORT)
        {
            ...
            if (hSCM)
            {
                ...
                if (hService)
                {
                    for (int retries = 0; retries < 60 && !vpoxNetCfgWinIsNetSetupStopped(hService); ++retries)
                        Sleep(1000);
...
HRESULT vpoxLoadIpHelpFunctions(HINSTANCE& pIpHlpInstance)
{
...
HRESULT vpoxNetCfgWinGetLoopbackMetric(OUT int* Metric)
{
    HRESULT rc = S_OK;
...
HRESULT vpoxNetCfgWinSetInterfaceMetric(
...
HRESULT vpoxNetCfgWinSetupMetric(IN NET_LUID* pLuid)
{
    ...
    HRESULT rc = vpoxLoadIpHelpFunctions(hModule);
    if (SUCCEEDED(rc))
    {
        int loopbackMetric;
        rc = vpoxNetCfgWinGetLoopbackMetric(&loopbackMetric);
        if (SUCCEEDED(rc))
            rc = vpoxNetCfgWinSetInterfaceMetric(pLuid, loopbackMetric - 1);
    }
...
#ifdef VPOXNETCFG_DELAYEDRENAME
VPOXNETCFGWIN_DECL(HRESULT) VPoxNetCfgWinRenameHostOnlyConnection(IN const GUID *pGuid, IN LPCWSTR pwszId, OUT BSTR *pDevName)
{
    HRESULT hr = S_OK;
    ...
    if (hDevInfo != INVALID_HANDLE_VALUE)
    {
        ...
        if (SetupDiOpenDeviceInfo(hDevInfo, pwszId, NULL, 0, &DevInfoData))
        {
            ...
            {
                hr = VPoxNetCfgWinGenHostonlyConnectionName(wszDevName, wszConnectionNewName, &cbName);
                if (SUCCEEDED(hr))
                {
                    WCHAR wszGuid[50];
                    ...
                    if (cbWGuid)
                    {
                        hr = VPoxNetCfgWinRenameConnection(wszGuid, wszConnectionNewName);
                        if (FAILED(hr))
                            NonStandardLogFlow(("VPoxNetCfgWinRenameHostOnlyConnection: VPoxNetCfgWinRenameConnection failed (0x%x)\n", hr));
                    }
                    else
                    {
                        err = GetLastError();
                    ...
                    NonStandardLogFlow(("VPoxNetCfgWinRenameHostOnlyConnection: VPoxNetCfgWinGenHostonlyConnectionName failed (0x%x)\n", hr));
            ...
            {
                hr = HRESULT_FROM_WIN32(err);
                NonStandardLogFlow(("VPoxNetCfgWinRenameHostOnlyConnection: SetupDiGetDeviceRegistryPropertyW failed (0x%x)\n", err));
            }
        }
        else
        {
            DWORD err = GetLastError();
            hr = HRESULT_FROM_WIN32(err);
            NonStandardLogFlow(("VPoxNetCfgWinRenameHostOnlyConnection: SetupDiOpenDeviceInfo failed (0x%x)\n", err));
        }
        SetupDiDestroyDeviceInfoList(hDevInfo);
    }
