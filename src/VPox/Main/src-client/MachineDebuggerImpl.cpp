...
/** @file
 * VPox IMachineDebugger COM class implementation (VPoxC).
 */
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        else
        {
            ...
            if (SUCCEEDED(hrc))
            {
                ...
                if (RT_FAILURE(vrc))
                    hrc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("EMR3SetExecutionPolicy failed with %Rrc"), vrc);
            }
        }
    }
    return hrc;
...
HRESULT MachineDebugger::setPATMEnabled(BOOL aPATMEnabled)
{
    ...
    if (aPATMEnabled)
        return setErrorBoth(VPOX_E_VM_ERROR, VERR_RAW_MODE_NOT_SUPPORTED, tr("PATM not present"), VERR_NOT_SUPPORTED);
    return S_OK;
}
...
HRESULT MachineDebugger::setCSAMEnabled(BOOL aCSAMEnabled)
{
    ...
    if (aCSAMEnabled)
        return setErrorBoth(VPOX_E_VM_ERROR, VERR_RAW_MODE_NOT_SUPPORTED, tr("CASM not present"));
    return S_OK;
}
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            *pstrSettings = "";
            AssertReturn(vrc == VERR_BUFFER_OVERFLOW,
                         setErrorBoth(VPOX_E_IPRT_ERROR, vrc, tr("%s returned %Rrc"), pszLogGetStr, vrc));
        }
        else
...
HRESULT MachineDebugger::getOSName(com::Utf8Str &aOSName)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            hrc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("DBGFR3OSQueryNameAndVersion failed with %Rrc"), vrc);
    }
    return hrc;
}
...
HRESULT MachineDebugger::setVirtualTimeRate(ULONG aVirtualTimeRate)
{
    ...
    else
    {
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            if (RT_FAILURE(vrc))
                hrc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("TMR3SetWarpDrive(, %u) failed with rc=%Rrc"), aVirtualTimeRate, vrc);
        }
    }
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                hrc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("DBGFR3Info failed with %Rrc"), vrc);
...
HRESULT MachineDebugger::detectOS(com::Utf8Str &aOs)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (RT_SUCCESS(vrc) && vrc != VINF_DBGF_OS_NOT_DETCTED)
        {
            ...
            hrc = setErrorBoth(VPOX_E_VM_ERROR, vrc, tr("DBGFR3OSDetect failed with %Rrc"), vrc);
    }
    return hrc;
}
...
HRESULT MachineDebugger::getRegister(ULONG aCpuId, const com::Utf8Str &aName, com::Utf8Str &aValue)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            hrc = setErrorBoth(VPOX_E_VM_ERROR, vrc,
...
HRESULT MachineDebugger::resetStats(const com::Utf8Str &aPattern)
{
    ...
    if (!ptrVM.isOk())
        return setError(VPOX_E_INVALID_VM_STATE, "Machine is not running");
...
HRESULT MachineDebugger::getCPULoad(ULONG aCpuId, ULONG *aPctExecuting, ULONG *aPctHalted, ULONG *aPctOther, LONG64 *aMsInterval)
{
    ...
    if (ptrVM.isOk())
    {
        ...
        hrc = setError(VPOX_E_INVALID_VM_STATE, "Machine is not running");
    return hrc;
}
