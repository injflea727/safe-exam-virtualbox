/* $Id: PGMAllBth.h $ */
/** @file
 * VPox - Page Manager, Shadow+Guest Paging Template - All context code.
 *
 ...
 */
PGM_BTH_DECL(VPOXSTRICTRC, Trap0eHandlerGuestFault)(PVMCPUCC pVCpu, PGSTPTWALK pGstWalk, RTGCUINT uErr)
{
 ...
 *                          when we're called.
 */
static VPOXSTRICTRC PGM_BTH_NAME(Trap0eHandlerDoAccessHandlers)(PVMCPUCC pVCpu, RTGCUINT uErr, PCPUMCTXCORE pRegFrame,
 ...
 *
 * @returns VPox status code (appropriate for trap handling and GC return).
 *
 * @param   pVCpu       The cross context virtual CPU structure.
...
#  if PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE)
    ...
    if (RT_FAILURE_NP(rc))
        return VPOXSTRICTRC_TODO(PGM_BTH_NAME(Trap0eHandlerGuestFault)(pVCpu, &GstWalk, uErr));
...
#   if PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE)
        ...
        if (RT_SUCCESS(rc) && PGM_PAGE_HAS_ACTIVE_ALL_HANDLERS(pPage))
            return VPOXSTRICTRC_TODO(PGM_BTH_NAME(Trap0eHandlerDoAccessHandlers)(pVCpu, uErr, pRegFrame, pvFault, pPage,
                                                                                 pfLockTaken, &GstWalk));
...
# if PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE)
        return VPOXSTRICTRC_TODO(PGM_BTH_NAME(Trap0eHandlerDoAccessHandlers)(pVCpu, uErr, pRegFrame, pvFault, pPage, pfLockTaken,
                                                                             &GstWalk));
# else
        return VPOXSTRICTRC_TODO(PGM_BTH_NAME(Trap0eHandlerDoAccessHandlers)(pVCpu, uErr, pRegFrame, pvFault, pPage, pfLockTaken));
# endif
    ...
    if (!(uErr & X86_TRAP_PF_P))
    {
        ...
        if (PGM_PAGE_IS_BALLOONED(pPage))
        {
            ...
            rc = VPOXSTRICTRC_TODO(PGMInterpretInstruction(pVM, pVCpu, pRegFrame, pvFault));
...
#   if PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE) && defined(VPOX_STRICT)
        /*
...
01:01:15.623511 00:08:43.266064 Location  : e:\vpox\svn\trunk\srcPage flags mismatch! pvFault=fffff801b0d7b000 uErr=11 GCPhys=0000000019b52000 fPageShw=0 fPageGst=77b0000000000121 rc=0
...
e:\vpox\svn\trunk\srcPage flags mismatch!
...
#   endif /* PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE) && VPOX_STRICT */
    }
...
# ifndef VPOX_WITH_NEW_LAZY_PAGE_ALLOC
...
#  ifdef VPOX_WITH_REAL_WRITE_MONITORED_PAGES
 ...
 *
 * @returns VPox status code (informational included).
 * @param   pVCpu       The cross context virtual CPU structure.
 ...
 *
 * @returns VPox status code, R0/RC may return VINF_PGM_SYNC_CR3, no other
...
#   ifndef VPOX_WITH_2X_4GB_ADDR_SPACE
 ...
 *
 * @returns VPox status, no specials.
 * @param   pVCpu       The cross context virtual CPU structure.
 */
PGM_BTH_DECL(int, UnmapCR3)(PVMCPUCC pVCpu)
{
