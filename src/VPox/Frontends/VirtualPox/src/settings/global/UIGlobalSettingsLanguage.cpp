...
/** @file
 * VPox Qt GUI - UIGlobalSettingsLanguage class implementation.
 */
...
void UIGlobalSettingsLanguage::reloadLanguageTree(const QString &strLanguageId)
{
    ...
    AssertRC(rc);
    QString strNlsPath = QString(szNlsPath) + UICommon::vpoxLanguageSubDirectory();
    QDir nlsDir(strNlsPath);
    QStringList files = nlsDir.entryList(QStringList(QString("%1*%2").arg(UICommon::vpoxLanguageFileBase(),
                                                                          UICommon::vpoxLanguageFileExtension())),
                                         QDir::Files);
    ...
    new UILanguageItem(m_pLanguageTree, translator, UICommon::vpoxBuiltInLanguageName(), true /* built-in */);
    /* Add all existing languages */
    for (QStringList::Iterator it = files.begin(); it != files.end(); ++it)
    {
        QString strFileName = *it;
        QRegExp regExp(UICommon::vpoxLanguageFileBase() + UICommon::vpoxLanguageIdRegExp());
