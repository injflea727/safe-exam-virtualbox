...
# define VPOX_WITH_IPCCLIENT_RW_CS
#endif
...
public:
  ...
  ~ipcClientState()
  {
#ifndef VPOX_WITH_IPCCLIENT_RW_CS
    if (monitor)
      nsAutoMonitor::DestroyMonitor(monitor);
#else
  ...
  //
  PRMonitor    *monitor;
#else  /* VPOX_WITH_IPCCLIENT_RW_CS */
  RTCRITSECTRW  critSect;
#endif /* VPOX_WITH_IPCCLIENT_RW_CS */
...
private:
  ...
  ipcClientState()
    ...
    , selfID(0)
  {
#ifdef VPOX_WITH_IPCCLIENT_RW_CS
      ...
      case IPCM_MSG_PSH_CLIENT_STATE:
      {
                 ...
                 status->ClientState() == IPCM_CLIENT_STATE_UP)
        {
          ...
          if (data->senderID == IPC_SENDER_ANY)
          {
            ...
            /* VPoxSVC/VPoxXPCOMIPCD auto-start can cause that a client up
