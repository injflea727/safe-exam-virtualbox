...
/** @file
 * VirtualPox API client session crash watcher
 */
...
VirtualPox::ClientWatcher::ClientWatcher() :
...
#elif defined(RT_OS_OS2) || defined(VPOX_WITH_SYS_V_IPC_SESSION_WATCHER) || defined(VPOX_WITH_GENERIC_SESSION_WATCHER)
...
VirtualPox::ClientWatcher::ClientWatcher(const ComObjPtr<VirtualPox> &pVirtualPox) :
...
bool VirtualPox::ClientWatcher::isReady()
{
 ...
 */
void VirtualPox::ClientWatcher::update()
{
 ...
 */
void VirtualPox::ClientWatcher::addProcess(RTPROCESS pid)
{
    AssertReturnVoid(mThread != NIL_RTTHREAD);
 ...
 */
uint32_t VirtualPox::ClientWatcher::reapProcesses(void)
{
    uint32_t cReaped = 0;
    ...
    if (mProcesses.size())
    {
        ...
        VirtualPox::ClientWatcher::ProcessList::iterator it = mProcesses.begin();
 ...
 */
void VirtualPox::ClientWatcher::winResetHandleArray(uint32_t cProcHandles)
{
 ...
 */
void VirtualPox::ClientWatcher::subworkerWait(VirtualPox::ClientWatcher::PerSubworker *pSubworker, uint32_t cMsWait)
{
    /*
 ...
 */
/*static*/
DECLCALLBACK(int) VirtualPox::ClientWatcher::subworkerThread(RTTHREAD hThreadSelf, void *pvUser)
{
    VirtualPox::ClientWatcher::PerSubworker *pSubworker = (VirtualPox::ClientWatcher::PerSubworker *)pvUser;
    VirtualPox::ClientWatcher               *pThis = pSubworker->pSelf;
    int                                      vrc;
 ...
 */
/*static*/
DECLCALLBACK(int) VirtualPox::ClientWatcher::worker(RTTHREAD hThreadSelf, void *pvUser)
{
    LogFlowFuncEnter();
    NOREF(hThreadSelf);

    VirtualPox::ClientWatcher *that = (VirtualPox::ClientWatcher *)pvUser;
    Assert(that);
    ...
    do
    {
        /* VirtualPox has been early uninitialized, terminate. */
        AutoCaller autoCaller(that->mVirtualPox);
        if (!autoCaller.isOk())
            break;
        ...
        for (;;)
        {
            ...
            for (uint32_t iSubworker = 1; iSubworker < cSubworkers; iSubworker++)
            {
                ...
                else
                {
                    vrc = RTThreadCreateF(&that->maSubworkers[iSubworker].hThread,
                                          VirtualPox::ClientWatcher::subworkerThread, &that->maSubworkers[iSubworker],
            ...
            if (update)
            {
                ...
                // get reference to the machines list in VirtualPox
                VirtualPox::MachinesOList &allMachines = that->mVirtualPox->i_getMachinesList();
    ...
    do
    {
        ...
        for (;;)
        {
            ...
            /* Restore the caller before using VirtualPox. If it fails, this
             * means VirtualPox is being uninitialized and we must terminate. */
            autoCaller.add();
            if (!autoCaller.isOk())
                break;
