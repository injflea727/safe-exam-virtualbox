/* $Id: VPoxNetUDP.cpp $ */
/** @file
 * VPoxNetUDP - IntNet UDP Client Routines.
 */
...
void *VPoxNetUDPMatch(PINTNETBUF pBuf, unsigned uDstPort, PCRTMAC pDstMac, uint32_t fFlags, PVPOXNETUDPHDRS pHdrs, size_t *pcb)
{
    ...
    *pcb = 0;
    ...
    if (fFlags & VPOXNETUDP_MATCH_PRINT_STDERR)
    {
    ...
    if (    (   !(fFlags & VPOXNETUDP_MATCH_UNICAST)
             || memcmp(&pEthHdr->DstMac, pDstMac, sizeof(pEthHdr->DstMac)))
        &&  (   !(fFlags & VPOXNETUDP_MATCH_BROADCAST)
    ...
    if (!pGso)
    {
        ...
        if (fFlags & (VPOXNETUDP_MATCH_CHECKSUM | VPOXNETUDP_MATCH_REQUIRE_CHECKSUM))
        {
            if (!RTNetIPv4IsUDPValid(pIpHdr, pUdpHdr, pUdpHdr + 1, cbFrame - offUdpHdr, true /*fChecksum*/))
                return NULL;
            if (    (fFlags & VPOXNETUDP_MATCH_REQUIRE_CHECKSUM)
                &&  !pUdpHdr->uh_sum)
                return NULL;
        }
        else
...
/** Internal worker for VPoxNetUDPUnicast and VPoxNetUDPBroadcast. */
static int vpoxnetudpSend(PSUPDRVSESSION pSession, INTNETIFHANDLE hIf, PINTNETBUF pBuf,
                          ...
                          void const *pvData, size_t cbData)
{
    ...
    /* send it */
    return VPoxNetIntIfSend(pSession, hIf, pBuf, RT_ELEMENTS(aSegs), &aSegs[0], true /* fFlush */);
}
...
int     VPoxNetUDPUnicast(PSUPDRVSESSION pSession, INTNETIFHANDLE hIf, PINTNETBUF pBuf,
                          ...
                          void const *pvData, size_t cbData)
{
    return vpoxnetudpSend(pSession, hIf, pBuf,
