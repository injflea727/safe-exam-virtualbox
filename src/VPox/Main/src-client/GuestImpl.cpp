/* $Id: GuestImpl.cpp $ */
/** @file
 * VirtualPox COM class implementation: Guest features.
 */
...
void Guest::i_updateStats(uint64_t iTick)
{
    ...
    /*
     * Calling SessionMachine may take time as the object resides in VPoxSVC
...
HRESULT Guest::getAdditionsVersion(com::Utf8Str &aAdditionsVersion)
{
    ...
    else
    {
        ...
        Bstr bstr;
        hrc = ptrMachine->GetGuestPropertyValue(Bstr("/VirtualPox/GuestAdd/Version").raw(), bstr.asOutParam());
        if (   SUCCEEDED(hrc)
            && !bstr.isEmpty())
        {
            Utf8Str str(bstr);
            if (str.count('.') == 0)
                hrc = ptrMachine->GetGuestPropertyValue(Bstr("/VirtualPox/GuestAdd/Revision").raw(), bstr.asOutParam());
            str = bstr;
            if (str.count('.') != 2)
                hrc = E_FAIL;
        }
...
{
#ifndef VPOX_WITH_DRAG_AND_DROP
    RT_NOREF(aDnDSource);
...
HRESULT Guest::getAdditionsStatus(AdditionsRunLevelType_T aLevel, BOOL *aActive)
{
    ...
    switch (aLevel)
    {
        ...
        default:
            rc = setError(VPOX_E_NOT_SUPPORTED,
                              ...
                              const com::Utf8Str &aDomain, BOOL aAllowInteractiveLogon)
{
    ...
    return setError(VPOX_E_VM_ERROR, tr("VMM device is not available (is the VM running?)"));
}
 ...
 * @param aOsType
 */
void Guest::i_setAdditionsInfo(const com::Utf8Str &aInterfaceVersion, VPOXOSTYPE aOsType)
{
    ...
    if (mData.mAdditionsVersionNew.isEmpty())
    {
        ...
        else
        {
             ...
             * interface version.
             */
            i_facilityUpdate(VPoxGuestFacilityType_Graphics, VPoxGuestFacilityStatus_Active, 0 /*fFlags*/, &TimeSpecTS);
        }
    }
     ...
     */
    i_facilityUpdate(VPoxGuestFacilityType_Graphics,
                     i_facilityIsActive(VPoxGuestFacilityType_VPoxGuestDriver)
                     ? VPoxGuestFacilityStatus_Active : VPoxGuestFacilityStatus_Inactive,
 ...
 * state).
 *
 * @param   a_uFullVersion          VPoxGuestInfo2::additionsMajor,
 *                                  VPoxGuestInfo2::additionsMinor and
 *                                  VPoxGuestInfo2::additionsBuild combined into
 *                                  one value by VPOX_FULL_VERSION_MAKE.
 *
 ...
 * @param   a_uRevision             See VPoxGuestInfo2::additionsRevision.
 * @param   a_fFeatures             See VPoxGuestInfo2::additionsFeatures.
 */
void Guest::i_setAdditionsInfo2(uint32_t a_uFullVersion, const char *a_pszName, uint32_t a_uRevision, uint32_t a_fFeatures)
{
    ...
    if (a_uFullVersion)
    {
        mData.mAdditionsVersionNew  = Utf8StrFmt(*a_pszName ? "%u.%u.%u_%s" : "%u.%u.%u",
                                                 VPOX_FULL_VERSION_GET_MAJOR(a_uFullVersion),
                                                 VPOX_FULL_VERSION_GET_MINOR(a_uFullVersion),
                                                 VPOX_FULL_VERSION_GET_BUILD(a_uFullVersion),
                                                 a_pszName);
...
bool Guest::i_facilityIsActive(VPoxGuestFacilityType enmFacility)
{
...
bool Guest::i_facilityUpdate(VPoxGuestFacilityType a_enmFacility, VPoxGuestFacilityStatus a_enmStatus,
                             uint32_t a_fFlags, PCRTTIMESPEC a_pTimeSpecTS)
{
    AssertReturn(   a_enmFacility < VPoxGuestFacilityType_All
                 && a_enmFacility > VPoxGuestFacilityType_Unknown, false);
 ...
 */
void Guest::i_onUserStateChange(Bstr aUser, Bstr aDomain, VPoxGuestUserState enmState,
 ...
 */
void Guest::i_setAdditionsStatus(VPoxGuestFacilityType a_enmFacility, VPoxGuestFacilityStatus a_enmStatus,
                                 uint32_t a_fFlags, PCRTTIMESPEC a_pTimeSpecTS)
{
    Assert(   a_enmFacility > VPoxGuestFacilityType_Unknown
           && a_enmFacility <= VPoxGuestFacilityType_All); /* Paranoia, VMMDev checks for this. */
    ...
    if (a_enmFacility == VPoxGuestFacilityType_All)
        for (FacilityMapIter it = mData.mFacilityMap.begin(); it != mData.mFacilityMap.end(); ++it)
            fFireEvent |= i_facilityUpdate((VPoxGuestFacilityType)it->first, a_enmStatus, a_fFlags, a_pTimeSpecTS);
    ...
    if (i_facilityIsActive(VPoxGuestFacilityType_VPoxTrayClient))
        mData.mAdditionsRunLevel = AdditionsRunLevelType_Desktop;
    else if (i_facilityIsActive(VPoxGuestFacilityType_VPoxService))
        mData.mAdditionsRunLevel = AdditionsRunLevelType_Userland;
    else if (i_facilityIsActive(VPoxGuestFacilityType_VPoxGuestDriver))
        mData.mAdditionsRunLevel = AdditionsRunLevelType_System;
    else
        mData.mAdditionsRunLevel = AdditionsRunLevelType_None;
...
void Guest::i_setSupportedFeatures(uint32_t aCaps)
{
    ...
    bool fFireEvent = i_facilityUpdate(VPoxGuestFacilityType_Seamless,
