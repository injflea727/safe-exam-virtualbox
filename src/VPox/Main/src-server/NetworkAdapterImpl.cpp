...
/** @file
 * Implementation of INetworkAdapter in VPoxSVC.
 */
...
HRESULT NetworkAdapter::setBridgedInterface(const com::Utf8Str &aBridgedInterface)
{
    ...
    ComPtr<IHost> host;
    HRESULT rc = mParent->i_getVirtualPox()->COMGETTER(Host)(host.asOutParam());
    if (SUCCEEDED(rc))
    {
...
bool NetworkAdapter::i_hasDefaults()
{
    ...
    HRESULT rc = mParent->i_getVirtualPox()->i_findGuestOSType(mParent->i_getOSTypeId(),
                                                               pGuestOSType);
    if (FAILED(rc))
        return false;
...
HRESULT NetworkAdapter::i_switchFromNatNetworking(const com::Utf8Str &networkName)
{
        ...
        || state == MachineState_Paused)
    {
        ...
        int natCount = mParent->i_getVirtualPox()->i_natNetworkRefDec(Bstr(networkName).raw());
        if (natCount == -1)
...
HRESULT NetworkAdapter::i_switchToNatNetworking(const com::Utf8Str &aNatNetworkName)
{
        ...
        || state == MachineState_Paused)
    {
        ...
        int natCount = mParent->i_getVirtualPox()->i_natNetworkRefInc(Bstr(aNatNetworkName).raw());
        if (natCount == -1)
