...
#endif
#if 0 /*defined(VPOX_STRICT) && defined(PGMPOOL_WITH_OPTIMIZED_DIRTY_PT)*/
                                         ...
                                         PCPUMCTXCORE pRegFrame, RTGCPHYS GCPhysFault, RTGCPTR pvFault)
{
     ...
     */
    int rc = VINF_SUCCESS;
    VPOXSTRICTRC rc2 = EMInterpretInstructionDisasState(pVCpu, pDis, pRegFrame, pvFault, EMCODETYPE_ALL);
    if (rc2 == VINF_SUCCESS)
    { /* do nothing */ }
    else if (rc2 == VINF_EM_RESCHEDULE)
    {
        rc = VPOXSTRICTRC_VAL(rc2);
# ifndef IN_RING3
...
# endif
    ...
    else if (rc2 == VERR_EM_INTERPRETER)
    {
        ...
        AssertMsgFailed(("%Rrc\n", VPOXSTRICTRC_VAL(rc2))); /* ASSUMES no complicated stuff here. */
...
# endif
     ...
     */
    VPOXSTRICTRC rc = EMInterpretInstructionDisasState(pVCpu, pDis, pRegFrame, pvFault, EMCODETYPE_ALL);
    if (RT_SUCCESS(rc))
        AssertMsg(rc == VINF_SUCCESS, ("%Rrc\n", VPOXSTRICTRC_VAL(rc))); /* ASSUMES no complicated stuff here. */
 ...
 */
DECLEXPORT(VPOXSTRICTRC) pgmRZPoolAccessPfHandler(PVMCC pVM, PVMCPUCC pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
...
# if 0 /* test code defined(VPOX_STRICT) && defined(PGMPOOL_WITH_OPTIMIZED_DIRTY_PT) */
...
int pgmPoolTrackUpdateGCPhys(PVMCC pVM, RTGCPHYS GCPhysPage, PPGMPAGE pPhysPage, bool fFlushPTEs, bool *pfFlushTLBs)
{
    ...
    if (u16)
    {
         ...
         * have to flush the whole shebang. Unless VPOX_WITH_NEW_LAZY_PAGE_ALLOC
        ...
        {
# ifdef VPOX_WITH_2X_4GB_ADDR_SPACE_IN_R0 /** @todo we can drop this now. */
