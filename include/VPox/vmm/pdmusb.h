...
#ifndef VPOX_INCLUDED_vmm_pdmusb_h
#define VPOX_INCLUDED_vmm_pdmusb_h
 ...
 *
 * This structure is used when registering a device from VPoxUsbRegister() in HC Ring-3.
...
/**
 * VPOX_STRICT wrapper for pHlpR3->pfnDBGFStopV.
 *
 * @returns VPox status code which must be passed up to the VMM.
...
/** Pointer to callbacks provided to the VPoxUsbRegister() call. */
typedef const struct PDMUSBREGCB *PCPDMUSBREGCB;

/**
 * Callbacks for VPoxUSBDeviceRegister().
 */
...
/**
 * The VPoxUsbRegister callback function.
 *
 ...
 */
typedef DECLCALLBACK(int) FNPDMVPOXUSBREGISTER(PCPDMUSBREGCB pCallbacks, uint32_t u32Version);
...
#endif /* !VPOX_INCLUDED_vmm_pdmusb_h */
