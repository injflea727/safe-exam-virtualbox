...
class tdGuestHostTimings(vpox.TestDriver):                                         # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self):
        ...
        oTestVm = vpoxtestvms.TestVm('mw7x64', oSet = self.oTestVmSet, sHd = 'mw7x64.vdi',
    ...
    def actionExecute(self):
        ...
        self.sTempPathHost  = os.path.normpath(os.environ.get("TEMP") + "/VPoxAudioValKit")
    ...
    def doTest(self, oSession):
        ...
        guestSessionWaitResult = oGuestSession.waitFor(self.oVPoxMgr.constants.GuestSessionWaitResult_Start, 2000)
        ...
        for duration in range(3, 6):
            ...
            processWaitResult = oProcess.waitFor(self.oVPoxMgr.constants.ProcessWaitForFlag_Start, 1000)
            reporter.log("Started: pid %d, waitResult %d" % (oProcess.PID, processWaitResult))

            processWaitResult = oProcess.waitFor(self.oVPoxMgr.constants.ProcessWaitForFlag_Terminate, 2 * duration * 1000)
    ...
    def actionExecuteOnRunnigVM(self):
        ...
        oVirtualPox = self.oVPoxMgr.getVirtualPox()
        oMachine = oVirtualPox.findMachine(self.sVMname)

        if oMachine == None:
            reporter.log("Machine '%s' is unknown" % (oMachine.name))
            return False

        if oMachine.state != self.oVPoxMgr.constants.MachineState_Running:
            reporter.log("Machine '%s' is not Running" % (oMachine.name))
            return False

        oSession = self.oVPoxMgr.mgr.getSessionObject(oVirtualPox)
        oMachine.lockMachine(oSession, self.oVPoxMgr.constants.LockType_Shared)
    ...
    def seekLatestAudioFileName(self, guestSession, duration):
        ...
        # Assuming that .wav files are named like 2016-11-15T12_08_27.669573100Z.wav by VPOX audio backend
