...
/* The data in the simulated VPox clipboard. */
static int g_tst_rcDataVPox = VINF_SUCCESS;
static void *g_tst_pvDataVPox = NULL;
static uint32_t g_tst_cbDataVPox = 0;

/* Set empty data in the simulated VPox clipboard. */
static void tstClipEmptyVPox(PSHCLX11CTX pCtx, int retval)
{
    g_tst_rcDataVPox = retval;
    RTMemFree(g_tst_pvDataVPox);
    g_tst_pvDataVPox = NULL;
    g_tst_cbDataVPox = 0;
    ShClX11ReportFormatsToX11(pCtx, 0);
}

/* Set the data in the simulated VPox clipboard. */
static int tstClipSetVPoxUtf16(PSHCLX11CTX pCtx, int retval,
                               const char *pcszData, size_t cb)
{
    ...
    g_tst_pvDataVPox = pv;
    g_tst_cbDataVPox = cb;
    ShClX11ReportFormatsToX11(pCtx, VPOX_SHCL_FMT_UNICODETEXT);
    return VINF_SUCCESS;
}

/* Return the data in the simulated VPox clipboard. */
DECLCALLBACK(int) ShClX11RequestDataForX11Callback(PSHCLCONTEXT pCtx, uint32_t Format, void **ppv, uint32_t *pcb)
{
    ...
    *pcb = g_tst_cbDataVPox;
    if (g_tst_pvDataVPox != NULL)
    {
        void *pv = RTMemDup(g_tst_pvDataVPox, g_tst_cbDataVPox);
        *ppv = pv;
        return pv != NULL ? g_tst_rcDataVPox : VERR_NO_MEMORY;
    }
    *ppv = NULL;
    return g_tst_rcDataVPox;
}
                             ...
                             const char *pcszExp, int rcExp)
{
    ...
    if (tstClipQueryFormats() != VPOX_SHCL_FMT_UNICODETEXT)
    {
    ...
    else
    {
        ...
        ShClX11ReadDataFromX11(pCtx, VPOX_SHCL_FMT_UNICODETEXT, pReq);
        int rc = VINF_SUCCESS;
        uint32_t cbActual = 0;
...
static void tstStringFromVPox(RTTEST hTest, PSHCLX11CTX pCtx, const char *pcszTarget, Atom typeExp,  const char *valueExp)
{
    RT_NOREF(pCtx);
    bool retval = false;
    Atom type;
...
static void tstNoX11(PSHCLX11CTX pCtx, const char *pcszTestCtx)
{
    ...
    int rc = ShClX11ReadDataFromX11(pCtx, VPOX_SHCL_FMT_UNICODETEXT, pReq);
    RTTESTI_CHECK_MSG(rc == VERR_NO_DATA, ("context: %s\n", pcszTestCtx));
}

static void tstStringFromVPoxFailed(RTTEST hTest, PSHCLX11CTX pCtx, const char *pcszTarget)
{
    RT_NOREF(pCtx);
    Atom type;
    XtPointer value = NULL;
...
static void tstBadFormatRequestFromHost(RTTEST hTest, PSHCLX11CTX pCtx)
{
    ...
    else
    {
        char *pc;
        CLIPREADCBREQ *pReq = (CLIPREADCBREQ *)&pReq, *pReqRet = NULL;
        ShClX11ReadDataFromX11(pCtx, 0xF000 /* vpoxFormat */, pReq);  /* Bad format. */
        int rc = VINF_SUCCESS;
        uint32_t cbActual = 0;
        tstClipGetCompletedRequest(&rc, &pc, &cbActual, &pReqRet);
...
int main()
{
    ...
    tstClipSetSelectionValues("UTF8_STRING", XA_STRING, NULL,
                              0, 8);
    ShClX11ReadDataFromX11(&X11Ctx, VPOX_SHCL_FMT_UNICODETEXT, pReq);
    tstClipGetCompletedRequest(&rc, &pc, &cbActual, &pReqRet);
    ...
    /*
     * Ensure that VPox is notified when we return the CB to X11
     */
    ...
    /*
     * Request for an invalid VPox format from X11
     */
    RTTestSub(hTest, "a request for an invalid VPox format from X11");
    /* Testing for 0xffff will go into handling VPOX_SHCL_FMT_UNICODETEXT, where we don't have
     * have any data at the moment so far, so this will return VERR_NO_DATA. */
    ShClX11ReadDataFromX11(&X11Ctx, 0xffff /* vpoxFormat */, pReq);
    tstClipGetCompletedRequest(&rc, &pc, &cbActual, &pReqRet);
    RTTEST_CHECK_MSG(hTest, rc == VERR_NO_DATA,
                     (hTest, "Returned %Rrc instead of VERR_NO_DATA\n",
                      rc));
     ...
     * UTF-8 from VPox
     */
    RTTestSub(hTest, "reading UTF-8 from VPox");
    /* Simple test */
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "hello world",
                        sizeof("hello world") * 2);
    tstStringFromVPox(hTest, &X11Ctx, "UTF8_STRING",
                      clipGetAtom(&X11Ctx, "UTF8_STRING"), "hello world");
    /* With an embedded carriage return */
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "hello\r\nworld",
                        sizeof("hello\r\nworld") * 2);
    tstStringFromVPox(hTest, &X11Ctx, "text/plain;charset=UTF-8",
                      clipGetAtom(&X11Ctx, "text/plain;charset=UTF-8"),
                      "hello\nworld");
    /* With an embedded CRCRLF */
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "hello\r\r\nworld",
                        sizeof("hello\r\r\nworld") * 2);
    tstStringFromVPox(hTest, &X11Ctx, "text/plain;charset=UTF-8",
                      clipGetAtom(&X11Ctx, "text/plain;charset=UTF-8"),
                      "hello\r\nworld");
    ...
    tstStringFromVPox(hTest, &X11Ctx, "text/plain;charset=UTF-8",
                      ...
                      "hello\n\rworld");
    /* An empty string */
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "", 2);
    tstStringFromVPox(hTest, &X11Ctx, "text/plain;charset=utf-8",
                      clipGetAtom(&X11Ctx, "text/plain;charset=utf-8"), "");
    /* With an embedded UTF-8 character. */
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "100\xE2\x82\xAC" /* 100 Euro */,
                        10);
    tstStringFromVPox(hTest, &X11Ctx, "STRING",
                      clipGetAtom(&X11Ctx, "STRING"), "100\xE2\x82\xAC");
    /* A non-zero-terminated string */
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "hello world",
                        sizeof("hello world") * 2 - 2);
    tstStringFromVPox(hTest, &X11Ctx, "TEXT", clipGetAtom(&X11Ctx, "TEXT"),
                     "hello world");

    /*
     * Timeout from VPox
     */
    RTTestSub(hTest, "reading from VPox with timeout");
    tstClipEmptyVPox(&X11Ctx, VERR_TIMEOUT);
    tstStringFromVPoxFailed(hTest, &X11Ctx, "UTF8_STRING");

    /*
     * No data in VPox clipboard
     */
    RTTestSub(hTest, "an empty VPox clipboard");
    tstClipSetSelectionValues("TEXT", XA_STRING, "", sizeof(""), 8);
    tstClipEmptyVPox(&X11Ctx, VINF_SUCCESS);
    RTTEST_CHECK_MSG(hTest, g_tst_fOwnsSel,
                     (hTest, "VPox grabbed the clipboard with no data and we ignored it\n"));
    tstStringFromVPoxFailed(hTest, &X11Ctx, "UTF8_STRING");

    /*
     * An unknown VPox format
     */
    RTTestSub(hTest, "reading an unknown VPox format");
    tstClipSetSelectionValues("TEXT", XA_STRING, "", sizeof(""), 8);
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "", 2);
    ShClX11ReportFormatsToX11(&X11Ctx, 0xa0000);
    RTTEST_CHECK_MSG(hTest, g_tst_fOwnsSel,
                     (hTest, "VPox grabbed the clipboard with unknown data and we ignored it\n"));
    tstStringFromVPoxFailed(hTest, &X11Ctx, "UTF8_STRING");

    /*
     * VPox requests a bad format
     */
    ...
    /* Simple test */
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "",
                        sizeof("") * 2);
    tstClipSetSelectionValues("UTF8_STRING", XA_STRING, "hello world",
                              sizeof("hello world"), 8);
    ...
    /* Read from VPox */
    RTTestSub(hTest, "reading from VPox, headless clipboard");
    /* Simple test */
    tstClipEmptyVPox(&X11Ctx, VERR_WRONG_ORDER);
    tstClipSetSelectionValues("TEXT", XA_STRING, "", sizeof(""), 8);
    tstClipSetVPoxUtf16(&X11Ctx, VINF_SUCCESS, "hello world",
                        sizeof("hello world") * 2);
    tstNoSelectionOwnership(&X11Ctx, "reading from VPox, headless clipboard");
