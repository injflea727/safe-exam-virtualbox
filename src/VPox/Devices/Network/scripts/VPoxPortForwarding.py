...
# > python VPoxPortForwarding.py --vm winXP -a 1 -p TCP -l 8080 -g 80 -P www
# generates sequence of API calls, equivalent to:
# > VPoxManage setextradata "winXP"
#        "VPoxInternal/Devices/pcnet/0/LUN#0/Config/www/Protocol" TCP
# > VPoxManage setextradata "winXP"
#        "VPoxInternal/Devices/pcnet/0/LUN#0/Config/www/GuestPort" 80
# > VPoxManage setextradata "winXP"
#        "VPoxInternal/Devices/pcnet/0/LUN#0/Config/www/HostPort" 8080
################################################################################
...
def main(argv):
    ...
    man = VirtualPoxManager(None, None)
    vb = man.getVirtualPox()
    print "VirtualPox version: %s" % vb.version,
    ...
    if profile_name == None:
        profile_name = generate_profile_name(options.proto.upper(),
                                            options.host_port,
                                            options.guest_port)
    config = "VPoxInternal/Devices/" + name + "/"
