...
static int Error(const char *pszFormat, ...)
{
    ...
    va_end(va);
    fprintf(stderr, "VPoxPeSetVersion: %s: error: %s\n", g_pszFilename, szTmp);
...
static void Info(unsigned iLevel, const char *pszFormat, ...)
{
    if (iLevel <= g_cVerbosity)
    {
        ...
        _vsnprintf(szTmp, sizeof(szTmp), pszFormat, va);
        va_end(va);
        fprintf(stderr, "VPoxPeSetVersion: %s: info: %s\n", g_pszFilename, szTmp);
    }
}
...
{
    fprintf(pOutput,
            "Usage: VPoxPeSetVersion [options] <PE-image>\n"
            "Options:\n"
            "  -v, --verbose\n"
            "    Increases verbosity.\n"
            "  -q, --quiet\n"
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (fAcceptOptions && *psz == '-')
        {
            ...
            if (ch == '-')
            {
                ...
                else
                {
                    ...
                    {
                        fprintf(stderr, "VPoxPeSetVersion: syntax error: Unknown option: --%s\n", psz);
                        return RTEXITCODE_SYNTAX;
                    }
                    continue;
                }
            ...
            do
            {
                switch (ch)
                {
                    ...
                    case 'h':
                        ...
                        return RTEXITCODE_SUCCESS;
                    default:
                        fprintf(stderr, "VPoxPeSetVersion: syntax error: Unknown option: -%c\n", ch ? ch : ' ');
                        return RTEXITCODE_SYNTAX;
                }
            } while ((ch = *psz++) != '\0');
        ...
        {
            fprintf(stderr, "VPoxPeSetVersion: syntax error: More than one PE-image specified!\n");
            return RTEXITCODE_SYNTAX;
        }
    }
    ...
    if (!pszFilename)
    {
        fprintf(stderr, "VPoxPeSetVersion: syntax error: No PE-image specified!\n");
        return RTEXITCODE_SYNTAX;
    }
