/* $Id: server.cpp $ */
/** @file
 * XPCOM server process (VPoxSVC) start point.
 */
...
/////////////////////////////////////////////////////////////////////////////
// VirtualPox component instantiation
/////////////////////////////////////////////////////////////////////////////
...
// is defined in the automatically generated VirtualPoxWrap.cpp
extern nsIClassInfo *NS_CLASSINFO_NAME(VirtualPoxWrap);
NS_DECL_CI_INTERFACE_GETTER(VirtualPoxWrap)

////////////////////////////////////////////////////////////////////////////////

static bool gAutoShutdown = false;
/** Delay before shutting down the VirtualPox server after the last
 * VirtualPox instance is released, in ms */
...
/**
 *  VirtualPox class factory that destroys the created instance right after
 *  the last reference to it is released by the client, and recreates it again
 *  when necessary (so VirtualPox acts like a singleton object).
 */
class VirtualPoxClassFactory : public VirtualPox
{
public:

    virtual ~VirtualPoxClassFactory()
    {
        LogFlowFunc(("Deleting VirtualPox...\n"));

        FinalRelease();
        sInstance = NULL;

        LogFlowFunc(("VirtualPox object deleted.\n"));
        RTPrintf("Informational: VirtualPox object deleted.\n");
    }

    NS_IMETHOD_(nsrefcnt) Release()
    {
        /* we overload Release() to guarantee the VirtualPox destructor is
         * always called on the main thread */

        nsrefcnt count = VirtualPox::Release();
        ...
        if (count == 1)
        {
            ...
            if (sTimer != NULL)
            {
                LogFlowFunc(("Last VirtualPox instance was released.\n"));
            ...
            if (!timerStarted)
            {
                ...
                else
                {
                     ...
                     *    client was faster and requested VirtualPox again.
                     *
    ...
    private:
        ...
        void *handler()
        {
            ...
            if (sInstance != NULL)
            {
                 ...
                 * (see VirtualPoxClassFactory::Release) and will start the
    ...
    static NS_IMETHODIMP FactoryDestructor()
    {
        ...
        if (sInstance != NULL)
        {
             ...
             * VirtualPox instance (so NS_ShutdownXPCOM() is doing a cleanup).
             * Release the guard reference we added in GetInstance(). */
            sInstance->Release();
        }

        /* Destroy lock after releasing the VirtualPox instance, otherwise
    ...
    static nsresult GetInstance(VirtualPox **inst)
    {
        LogFlowFunc(("Getting VirtualPox object...\n"));
        ...
        if (sInstance == NULL)
        {
            LogFlowFunc(("Creating new VirtualPox object...\n"));
            sInstance = new VirtualPoxClassFactory();
            if (sInstance != NULL)
            {
                /* make an extra AddRef to take the full control
                 * on the VirtualPox destruction (see FinalRelease()) */
                sInstance->AddRef();

                sInstance->AddRef(); /* protect FinalConstruct() */
                rv = sInstance->FinalConstruct();
                RTPrintf("Informational: VirtualPox object created (rc=%Rhrc).\n", rv);
                if (NS_FAILED(rv))
                {
                    /* On failure diring VirtualPox initialization, delete it
                     ...
                     * info from a server-side IVirtualPoxErrorInfo object. */
                    sInstance->Release();
                    sInstance->Release();
                    Assert(sInstance == NULL);
                }
        ...
        {
            LogFlowFunc(("Using existing VirtualPox object...\n"));
            nsrefcnt count = sInstance->AddRef();
            Assert(count > 1);

            if (count >= 2)
            {
                LogFlowFunc(("Another client has requested a reference to VirtualPox, canceling destruction...\n"));
...
private:
    ...
    static VirtualPoxClassFactory *sInstance;
    static RTCRITSECT sLock;

    static RTTIMERLR sTimer;
};

VirtualPoxClassFactory *VirtualPoxClassFactory::sInstance = NULL;
RTCRITSECT VirtualPoxClassFactory::sLock;

RTTIMERLR VirtualPoxClassFactory::sTimer = NIL_RTTIMERLR;

NS_GENERIC_FACTORY_SINGLETON_CONSTRUCTOR_WITH_RC(VirtualPox, VirtualPoxClassFactory::GetInstance)
    ...
    if (q && gKeepRunning)
    {
        if (sig == SIGUSR1)
        {
            if (gAllowSigUsrQuit)
            {
                ...
                VirtualPoxClassFactory::MaybeQuitEvent *ev = new VirtualPoxClassFactory::MaybeQuitEvent(true /* fSignal */);
                if (!q->postEvent(ev))
                    delete ev;
            }
            /* else do nothing */
...
static nsresult vpoxsvcSpawnDaemonByReExec(const char *pszPath, bool fAutoShutdown, const char *pszPidFile)
{
...
int main(int argc, char **argv)
{
    ...
    if (fDaemonize)
    {
        vpoxsvcSpawnDaemonByReExec(argv[0], gAutoShutdown, g_pszPidFile);
        exit(126);
    }
    ...
    vrc = com::VPoxLogRelCreate("XPCOM Server", szLogFile,
    ...
    daemon_pipe_wr = PR_GetInheritedFD(VPOXSVC_STARTUP_PIPE_NAME);
    RTEnvUnset("NSPR_INHERIT_FDS");

    const nsModuleComponentInfo VirtualPoxInfo = {
        "VirtualPox component",
        NS_VIRTUALPOX_CID,
        NS_VIRTUALPOX_CONTRACTID,
        VirtualPoxConstructor, // constructor function
        NULL, // registration function
        NULL, // deregistration function
        VirtualPoxClassFactory::FactoryDestructor, // factory destructor function
        NS_CI_INTERFACE_GETTER_NAME(VirtualPoxWrap),
        NULL, // language helper
        &NS_CLASSINFO_NAME(VirtualPoxWrap),
        0 // flags
    };
    ...
    const nsModuleComponentInfoPlusFactoryConstructor components[] = {
        {
            &VirtualPoxInfo,
            VirtualPoxClassFactory::FactoryConstructor // factory constructor function
        }
    };
    ...
    do /* goto avoidance only */
    {
        ...
        rc = gIpcServ->AddName(VPOXSVC_IPC_NAME);
        if (NS_FAILED(rc))
        {
...
// XXX Temporary allow release assertions to terminate VPoxSVC
//            sigaction(SIGTRAP, &sa, NULL);
...

        {
            ...
            cSize = RTStrPrintf(szBuf, sizeof(szBuf),
                                VPOX_PRODUCT" XPCOM Server Version "
                                VPOX_VERSION_STRING);
            for (size_t i = cSize; i > 0; i--)
                putchar('*');
            RTPrintf("\n%s\n", szBuf);
            RTPrintf("(C) 2004-" VPOX_C_YEAR " " VPOX_VENDOR "\n"
                     "All rights reserved.\n");
#ifdef DEBUG
        ...
        gIpcServ->RemoveName(VPOXSVC_IPC_NAME);
    }
