/* $Id: VPoxBugReport.cpp $ */
/** @file
 * VPoxBugReport - VirtualPox command-line diagnostics tool, main file.
 */
...
static char *g_pszVPoxManage = NULL;
 ...
 * VirtualPox API. In case API is not functioning properly these paths
...
{
    /* Collect all log files from VPoxSVC */
    VBRDir HomeDir(PathJoin(pszHome, "VPoxSVC.log*"));
    ...
    report->addItem(new BugReportFile(PathJoin(pszHome, "VirtualPox.xml"), "VirtualPox.xml"));
    report->addItem(new BugReportCommand("HostUsbDevices", g_pszVPoxManage, "list", "usbhost", NULL));
    report->addItem(new BugReportCommand("HostUsbFilters", g_pszVPoxManage, "list", "usbfilters", NULL));
    for (MachineInfoList::iterator it = machines.begin(); it != machines.end(); ++it)
    {
        VBRDir VmDir(PathJoin((*it)->getLogPath(), "VPox.log*"));
        ...
        report->addItem(new BugReportCommand(PathJoin((*it)->getName(), "GuestProperties"),
                                            g_pszVPoxManage, "guestproperty", "enumerate",
                                            (*it)->getName(), NULL));
    }
...
{
    RTStrmPrintf(g_pStdErr, VPOX_PRODUCT " Bug Report Tool " VPOX_VERSION_STRING "\n"
...
int main(int argc, char *argv[])
{
    ...
    com::GetVPoxUserHomeDirectory(homeDir, sizeof(homeDir));

    try
    {
        /* Figure out the full path to VPoxManage */
        char szVPoxBin[RTPATH_MAX];
        if (!RTProcGetExecutablePath(szVPoxBin, sizeof(szVPoxBin)))
            throw RTCError("RTProcGetExecutablePath failed\n");
        RTPathStripFilename(szVPoxBin);
        g_pszVPoxManage = RTPathJoinA(szVPoxBin, VPOXMANAGE);
        if (!g_pszVPoxManage)
            throw RTCError("Out of memory\n");

        handleComError(com::Initialize(VPOX_COM_INIT_F_DEFAULT | VPOX_COM_INIT_F_NO_COM_PATCHING), "Failed to initialize COM");
        ...
        do
        {
            ...
            hr = virtualPoxClient.createLocalObject(CLSID_VirtualPoxClient);
            if (SUCCEEDED(hr))
                hr = virtualPoxClient->COMGETTER(VirtualPox)(virtualPox.asOutParam());
            else
                hr = virtualPox.createLocalObject(CLSID_VirtualPox);
            if (FAILED(hr))
                RTStrmPrintf(g_pStdErr, "WARNING: Failed to create the VirtualPox object (hr=0x%x)\n", hr);
            else
            {
            ...
            if (SUCCEEDED(hr))
            {
                if (fAllMachines)
                {
                    ...
                    hr = virtualPox->COMGETTER(Machines)(ComSafeArrayAsOutParam(machines));
                    if (SUCCEEDED(hr))
                    {
                ...
                else
                {
                    for ( std::list<const char *>::iterator it = nameList.begin(); it != nameList.end(); ++it)
                    {
                        ...
                        handleComError(virtualPox->FindMachine(com::Bstr(*it).raw(), machine.asOutParam()),
