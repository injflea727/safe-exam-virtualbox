...
# if !defined(DEBUG) || defined(DEBUG_sandervl) || defined(DEBUG_frank)
    int cWait = 10;
# else
    int cWait = RTEnvExist("VPOX_DBGF_NO_WAIT_FOR_ATTACH")
             || (   !VM_IS_RAW_MODE_ENABLED(pVM)
                 && (   enmEvent == DBGFEVENT_ASSERTION_HYPER
                     || enmEvent == DBGFEVENT_FATAL_ERROR)
                 && !RTEnvExist("VPOX_DBGF_WAIT_FOR_ATTACH"))
              ? 10
              : 150;
# endif
...
#ifdef DEBUG
                 "      Set VPOX_DBGF_NO_WAIT_FOR_ATTACH=1 for short wait or VPOX_DBGF_WAIT_FOR_ATTACH=1 longer.\n"
#endif
                 ,
...
VMMR3_INT_DECL(int) DBGFR3VMMForcedAction(PVM pVM, PVMCPU pVCpu)
{
    ...
    if (VMCPU_FF_TEST_AND_CLEAR(pVCpu, VMCPU_FF_DBGF))
    {
        ...
        {
            VPOXSTRICTRC rcStrict2 = DBGFR3EventHandlePending(pVM, pVCpu);
 ...
 *
 * @returns VPox status code. (clearify)
 * @param   pVM     The cross context VM structure.
 */
static int dbgfR3VMMWait(PVM pVM)
{
 ...
 *
 * @returns VPox status code. Will not return VPOX_INTERRUPTED.
 * @param   pUVM        The user mode VM handle.
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) dbgfR3EventConfigEx(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    if (pVCpu->idCpu == 0)
    {
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) dbgfR3InterruptConfigEx(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    if (pVCpu->idCpu == 0)
    {
