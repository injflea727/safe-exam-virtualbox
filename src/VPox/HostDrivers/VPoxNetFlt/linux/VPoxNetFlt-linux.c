/* $Id: VPoxNetFlt-linux.c $ */
/** @file
 * VPoxNetFlt - Network Filter Driver (Host), Linux Specific Code.
 */
...
#define VPOXNETFLT_LINUX_NO_XMIT_QUEUE
...
#include <VPox/vmm/pdmnetinline.h>
...
typedef struct VPOXNETFLTNOTIFIER {
    struct notifier_block Notifier;
    PVPOXNETFLTINS pThis;
} VPOXNETFLTNOTIFIER;
typedef struct VPOXNETFLTNOTIFIER *PVPOXNETFLTNOTIFIER;


/*********************************************************************************************************************************
*   Defined Constants And Macros                                                                                                 *
*********************************************************************************************************************************/
#define VPOX_FLT_NB_TO_INST(pNB)    RT_FROM_MEMBER(pNB, VPOXNETFLTINS, u.s.Notifier)
#define VPOX_FLT_PT_TO_INST(pPT)    RT_FROM_MEMBER(pPT, VPOXNETFLTINS, u.s.PacketType)
#ifndef VPOXNETFLT_LINUX_NO_XMIT_QUEUE
# define VPOX_FLT_XT_TO_INST(pXT)   RT_FROM_MEMBER(pXT, VPOXNETFLTINS, u.s.XmitTask)
#endif

#if RTLNX_VER_MIN(3,11,0)
# define VPOX_NETDEV_NOTIFIER_INFO_TO_DEV(ptr) netdev_notifier_info_to_dev(ptr)
#else
# define VPOX_NETDEV_NOTIFIER_INFO_TO_DEV(ptr) ((struct net_device *)ptr)
#endif

#if RTLNX_VER_MIN(3,5,0)
# define VPOX_SKB_KMAP_FRAG(frag) kmap_atomic(skb_frag_page(frag))
# define VPOX_SKB_KUNMAP_FRAG(vaddr) kunmap_atomic(vaddr)
#else
# if RTLNX_VER_MIN(3,2,0)
#  define VPOX_SKB_KMAP_FRAG(frag) kmap_atomic(skb_frag_page(frag), KM_SKB_DATA_SOFTIRQ)
#  define VPOX_SKB_KUNMAP_FRAG(vaddr) kunmap_atomic(vaddr, KM_SKB_DATA_SOFTIRQ)
# else
#  define VPOX_SKB_KMAP_FRAG(frag) kmap_atomic(frag->page, KM_SKB_DATA_SOFTIRQ)
#  define VPOX_SKB_KUNMAP_FRAG(vaddr) kunmap_atomic(vaddr, KM_SKB_DATA_SOFTIRQ)
# endif
#endif

#if RTLNX_VER_MIN(2,6,34)
# define VPOX_NETDEV_NAME(dev)              netdev_name(dev)
#else
# define VPOX_NETDEV_NAME(dev)              ((dev)->reg_state != NETREG_REGISTERED ? "(unregistered net_device)" : (dev)->name)
#endif

#if RTLNX_VER_MIN(2,6,25)
# define VPOX_IPV4_IS_LOOPBACK(addr)        ipv4_is_loopback(addr)
# define VPOX_IPV4_IS_LINKLOCAL_169(addr)   ipv4_is_linklocal_169(addr)
#else
# define VPOX_IPV4_IS_LOOPBACK(addr)        ((addr & htonl(IN_CLASSA_NET)) == htonl(0x7f000000))
# define VPOX_IPV4_IS_LINKLOCAL_169(addr)   ((addr & htonl(IN_CLASSB_NET)) == htonl(0xa9fe0000))
#endif

#if RTLNX_VER_MIN(2,6,22)
# define VPOX_SKB_RESET_NETWORK_HDR(skb)    skb_reset_network_header(skb)
# define VPOX_SKB_RESET_MAC_HDR(skb)        skb_reset_mac_header(skb)
# define VPOX_SKB_CSUM_OFFSET(skb)          skb->csum_offset
#else
# define VPOX_SKB_RESET_NETWORK_HDR(skb)    skb->nh.raw = skb->data
# define VPOX_SKB_RESET_MAC_HDR(skb)        skb->mac.raw = skb->data
# define VPOX_SKB_CSUM_OFFSET(skb)          skb->csum
#endif

#if RTLNX_VER_MIN(2,6,19)
# define VPOX_SKB_CHECKSUM_HELP(skb)        skb_checksum_help(skb)
#else
# define CHECKSUM_PARTIAL                   CHECKSUM_HW
# if RTLNX_VER_MIN(2,6,10)
#  define VPOX_SKB_CHECKSUM_HELP(skb)       skb_checksum_help(skb, 0)
# else
...
# define VPOX_HAVE_SKB_VLAN
#endif

#ifdef VPOX_HAVE_SKB_VLAN
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int      __init VPoxNetFltLinuxInit(void);
static void     __exit VPoxNetFltLinuxUnload(void);
static void     vpoxNetFltLinuxForwardToIntNet(PVPOXNETFLTINS pThis, struct sk_buff *pBuf);
...
module_init(VPoxNetFltLinuxInit);
module_exit(VPoxNetFltLinuxUnload);

MODULE_AUTHOR(VPOX_VENDOR);
MODULE_DESCRIPTION(VPOX_PRODUCT " Network Filter Driver");
MODULE_LICENSE("GPL");
...
static int __init VPoxNetFltLinuxInit(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = vpoxNetFltInitGlobalsAndIdc(&g_VPoxNetFltGlobals);
        if (RT_SUCCESS(rc))
        {
            LogRel(("VPoxNetFlt: Successfully started.\n"));
            return 0;
        }

        LogRel(("VPoxNetFlt: failed to initialize device extension (rc=%d)\n", rc));
        RTR0Term();
    }
    else
        LogRel(("VPoxNetFlt: failed to initialize IPRT (rc=%d)\n", rc));
...
{
    int rc;
    Log(("VPoxNetFltLinuxUnload\n"));
    Assert(vpoxNetFltCanUnload(&g_VPoxNetFltGlobals));

    /*
     * Undo the work done during start (in reverse order).
     */
    rc = vpoxNetFltTryDeleteIdcAndGlobals(&g_VPoxNetFltGlobals);
    ...
    Log(("VPoxNetFltLinuxUnload - done\n"));
}
 ...
 */
#define VPOXNETFLT_WITH_HOST2WIRE_FILTER

#ifdef VPOXNETFLT_WITH_HOST2WIRE_FILTER
 ...
 */
typedef struct VPoxNetDeviceOpsOverride
{
    /** Our overridden ops. */
    OVR_OPSTYPE                     Ops;
    /** Magic word. */
...
# endif /* RTLNX_VER_MAX(2,6,29) */
    ...
    PVPOXNETFLTINS                  pVPoxNetFlt;
    /** The number of filtered packages. */
    uint64_t                        cFiltered;
    /** The total number of packets */
    uint64_t                        cTotal;
} VPOXNETDEVICEOPSOVERRIDE, *PVPOXNETDEVICEOPSOVERRIDE;
/** VPOXNETDEVICEOPSOVERRIDE::u32Magic value. */
#define VPOXNETDEVICEOPSOVERRIDE_MAGIC  UINT32_C(0x00c0ffee)
 ...
 */
static int vpoxNetFltLinuxStartXmitFilter(struct sk_buff *pSkb, struct net_device *pDev)
{
    PVPOXNETDEVICEOPSOVERRIDE   pOverride = (PVPOXNETDEVICEOPSOVERRIDE)pDev->OVR_OPS;
     ...
     *
     * Note! We're racing vpoxNetFltLinuxUnhookDev here.  If this was supposed
     *       to be production quality code, we would have to be much more
     *       careful here and avoid the race.
     */
    if (   !VALID_PTR(pOverride)
        || pOverride->u32Magic != VPOXNETDEVICEOPSOVERRIDE_MAGIC
# if RTLNX_VER_MIN(2,6,29)
        || !VALID_PTR(pOverride->pOrgOps)
# endif /* RTLNX_VER_MIN(2,6,29) */
        )
    {
        printk("vpoxNetFltLinuxStartXmitFilter: bad override %p\n", pOverride);
        dev_kfree_skb(pSkb);
        return NETDEV_TX_OK;
    }
    pOverride->cTotal++;
    ...
    if (   pEtherHdr
        && VALID_PTR(pOverride->pVPoxNetFlt)
        && (pSwitchPort = pOverride->pVPoxNetFlt->pSwitchPort) != NULL
 ...
 * @param   pDev                The net device.
 */
static void vpoxNetFltLinuxHookDev(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
    PVPOXNETDEVICEOPSOVERRIDE   pOverride;
...
# else /* RTLNX_VER_MIN(2,6,29) */
    pOverride->Ops.ndo_start_xmit   = vpoxNetFltLinuxStartXmitFilter;
# endif /* RTLNX_VER_MIN(2,6,29) */
    pOverride->u32Magic             = VPOXNETDEVICEOPSOVERRIDE_MAGIC;
    pOverride->cTotal               = 0;
    pOverride->cFiltered            = 0;
    pOverride->pVPoxNetFlt          = pThis;

    RTSpinlockAcquire(pThis->hSpinlock); /* (this isn't necessary, but so what) */
    ASMAtomicWritePtr((void * volatile *)&pDev->OVR_OPS, pOverride);
# if RTLNX_VER_MAX(2,6,29)
    ASMAtomicXchgPtr((void * volatile *)&pDev->hard_start_xmit, vpoxNetFltLinuxStartXmitFilter);
# endif /* RTLNX_VER_MAX(2,6,29) */
    RTSpinlockRelease(pThis->hSpinlock);
}

/**
 * Undos what vpoxNetFltLinuxHookDev did.
 *
 * @param   pThis               The net filter instance.
 * @param   pDev                The net device.  Can be NULL, in which case
 *                              we'll try retrieve it from @a pThis.
 */
static void vpoxNetFltLinuxUnhookDev(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
    PVPOXNETDEVICEOPSOVERRIDE   pOverride;
    ...
    if (VALID_PTR(pDev))
    {
        pOverride = (PVPOXNETDEVICEOPSOVERRIDE)pDev->OVR_OPS;
        if (    VALID_PTR(pOverride)
            &&  pOverride->u32Magic == VPOXNETDEVICEOPSOVERRIDE_MAGIC
            &&  VALID_PTR(pOverride->pOrgOps)
           )
...
# endif /* RTLNX_VER_MAX(2,6,29) */
    ...
    if (pOverride)
    {
        printk("vpoxnetflt: %llu out of %llu packets were not sent (directed to host)\n", pOverride->cFiltered, pOverride->cTotal);
        RTMemFree(pOverride);
    }
}

#endif /* VPOXNETFLT_WITH_HOST2WIRE_FILTER */
 ...
 * @param   pThis
 */
DECLINLINE(struct net_device *) vpoxNetFltLinuxRetainNetDev(PVPOXNETFLTINS pThis)
{
#if 0
    struct net_device *pDev = NULL;

    Log(("vpoxNetFltLinuxRetainNetDev\n"));
    /*
    ...
    if (!ASMAtomicUoReadBool(&pThis->fDisconnectedFromHost))
    {
        ...
        {
            dev_hold(pDev);
            Log(("vpoxNetFltLinuxRetainNetDev: Device %p(%s) retained. ref=%d\n",
                 pDev, pDev->name,
    ...
    Log(("vpoxNetFltLinuxRetainNetDev - done\n"));
    return pDev;
#else
 ...
 * by vpoxNetFltLinuxRetainNetDev.
 *
 * @param   pThis           The instance.
 * @param   pDev            The vpoxNetFltLinuxRetainNetDev
 *                          return value, NULL is fine.
 */
DECLINLINE(void) vpoxNetFltLinuxReleaseNetDev(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
#if 0
    Log(("vpoxNetFltLinuxReleaseNetDev\n"));
    NOREF(pThis);
    if (pDev)
    {
        dev_put(pDev);
        Log(("vpoxNetFltLinuxReleaseNetDev: Device %p(%s) released. ref=%d\n",
             pDev, pDev->name,
#if RTLNX_VER_MIN(2,6,37)
             netdev_refcnt_read(pDev)
#else
    ...
    Log(("vpoxNetFltLinuxReleaseNetDev - done\n"));
#endif
}

#define VPOXNETFLT_CB_TAG(skb) (0xA1C90000 | (skb->dev->ifindex & 0xFFFF))
#define VPOXNETFLT_SKB_TAG(skb) (*(uint32_t*)&((skb)->cb[sizeof((skb)->cb)-sizeof(uint32_t)]))

/**
 * Checks whether this is an mbuf created by vpoxNetFltLinuxMBufFromSG,
 * i.e. a buffer which we're pushing and should be ignored by the filter callbacks.
 *
 * @returns true / false accordingly.
 * @param   pBuf            The sk_buff.
 */
DECLINLINE(bool) vpoxNetFltLinuxSkBufIsOur(struct sk_buff *pBuf)
{
    return VPOXNETFLT_SKB_TAG(pBuf) == VPOXNETFLT_CB_TAG(pBuf);
}
 ...
 */
DECLINLINE(bool) vpoxNetFltLinuxIsGso(PINTNETSG pSG)
{
#if defined(VPOXNETFLT_WITH_GSO_XMIT_WIRE) || defined(VPOXNETFLT_WITH_GSO_XMIT_HOST)
    return !((PDMNETWORKGSOTYPE)pSG->GsoCtx.u8Type == PDMNETWORKGSOTYPE_INVALID);
#else /* !VPOXNETFLT_WITH_GSO_XMIT_WIRE && !VPOXNETFLT_WITH_GSO_XMIT_HOST */
    return false;
#endif /* !VPOXNETFLT_WITH_GSO_XMIT_WIRE && !VPOXNETFLT_WITH_GSO_XMIT_HOST */
}
 ...
 * @param   pSG             The (scatter/)gather list.
 */
DECLINLINE(uint32_t) vpoxNetFltLinuxFrameSize(PINTNETSG pSG)
{
    uint16_t u16Type = 0;
    ...
    if (u16Type == RTNET_ETHERTYPE_VLAN)
        cbVlanTag = 4;
    return (vpoxNetFltLinuxIsGso(pSG) ? (uint32_t)pSG->GsoCtx.cbMaxSeg + pSG->GsoCtx.cbHdrsTotal : pSG->cbTotal) - cbVlanTag;
}
 ...
 */
static struct sk_buff *vpoxNetFltLinuxSkBufFromSG(PVPOXNETFLTINS pThis, PINTNETSG pSG, bool fDstWire)
{
    struct sk_buff *pPkt;
    ...
    if (pSG->cbTotal == 0)
    {
        LogRel(("VPoxNetFlt: Dropped empty packet coming from internal network.\n"));
        return NULL;
    }
    Log5(("VPoxNetFlt: Packet to %s of %d bytes (frame=%d).\n", fDstWire?"wire":"host", pSG->cbTotal, vpoxNetFltLinuxFrameSize(pSG)));
    if (fDstWire && (vpoxNetFltLinuxFrameSize(pSG) > ASMAtomicReadU32(&pThis->u.s.cbMtu) + 14))
    {
        static bool s_fOnce = true;
        if (s_fOnce)
        {
            s_fOnce = false;
            printk("VPoxNetFlt: Dropped over-sized packet (%d bytes) coming from internal network.\n", vpoxNetFltLinuxFrameSize(pSG));
        }
        return NULL;
    }
    ...
    if (RT_UNLIKELY(!pPkt))
    {
        Log(("vpoxNetFltLinuxSkBufFromSG: Failed to allocate sk_buff(%u).\n", pSG->cbTotal));
        pSG->pvUserData = NULL;
        return NULL;
    }
    pPkt->dev       = pDev;
...
# endif
    ...
    if (fDstWire)
    {
        VPOX_SKB_RESET_NETWORK_HDR(pPkt);

        /* Restore ethernet header back. */
        skb_push(pPkt, ETH_HLEN); /** @todo VLAN: +4 if VLAN? */
        VPOX_SKB_RESET_MAC_HDR(pPkt);
    }
    VPOXNETFLT_SKB_TAG(pPkt) = VPOXNETFLT_CB_TAG(pPkt);
 ...
 */
DECLINLINE(unsigned) vpoxNetFltLinuxGetChecksumStartOffset(struct sk_buff *pBuf)
{
 ...
 */
static void vpoxNetFltLinuxSkBufToSG(PVPOXNETFLTINS pThis, struct sk_buff *pBuf, PINTNETSG pSG,
                                     unsigned cbExtra, unsigned cSegs, uint32_t fSrc, PCPDMNETWORKGSO pGsoCtx)
{
    int i;
    NOREF(pThis);

#ifndef VPOXNETFLT_SG_SUPPORT
    Assert(!skb_shinfo(pBuf)->frag_list);
#else /* VPOXNETFLT_SG_SUPPORT */
...
# endif /* RTLNX_VER_MIN(2,6,27) */
    ...
    {
        unsigned uCsumStartOffset = vpoxNetFltLinuxGetChecksumStartOffset(pBuf);
        unsigned uCsumStoreOffset = uCsumStartOffset + VPOX_SKB_CSUM_OFFSET(pBuf) - cbConsumed;
...
# if RTLNX_VER_MIN(5,4,0) || RTLNX_SUSE_MAJ_PREREQ(15, 2)
        ...
        pSG->aSegs[iSeg].pv = VPOX_SKB_KMAP_FRAG(pFrag) + pFrag->bv_offset;
# else /* < KERNEL_VERSION(5, 4, 0) */
        pSG->aSegs[iSeg].cb = pFrag->size;
        pSG->aSegs[iSeg].pv = VPOX_SKB_KMAP_FRAG(pFrag) + pFrag->page_offset;
...
# endif /* LOG_ENABLED */
    ...
    if (cbProduced)
    {
        LogRel(("vpoxNetFltLinuxSkBufToSG: original packet dump:\n%.*Rhxd\n", pBuf->len-pBuf->data_len, skb_mac_header(pBuf)));
        LogRel(("vpoxNetFltLinuxSkBufToSG: cbConsumed=%u cbProduced=%u cbExtra=%u\n", cbConsumed, cbProduced, cbExtra));
        uint32_t offset = 0;
        for (i = 0; i < pSG->cSegsUsed; ++i)
        {
            LogRel(("vpoxNetFltLinuxSkBufToSG: seg#%d (%d bytes, starting at 0x%x):\n%.*Rhxd\n",
    ...
    Log6(("vpoxNetFltLinuxSkBufToSG: allocated=%d, segments=%d frags=%d next=%p frag_list=%p pkt_type=%x fSrc=%x\n",
          pSG->cSegsAlloc, pSG->cSegsUsed, skb_shinfo(pBuf)->nr_frags, pBuf->next, skb_shinfo(pBuf)->frag_list, pBuf->pkt_type, fSrc));
    for (i = 0; i < pSG->cSegsUsed; i++)
        Log6(("vpoxNetFltLinuxSkBufToSG:   #%d: cb=%d pv=%p\n",
...
static int vpoxNetFltLinuxPacketHandler(struct sk_buff *pBuf,
...
#endif
{
    PVPOXNETFLTINS pThis;
    struct net_device *pDev;
    LogFlow(("vpoxNetFltLinuxPacketHandler: pBuf=%p pSkbDev=%p pPacketType=%p\n",
             pBuf, pSkbDev, pPacketType));
#if RTLNX_VER_MIN(2,6,18)
    Log3(("vpoxNetFltLinuxPacketHandler: skb len=%u data_len=%u truesize=%u next=%p nr_frags=%u gso_size=%u gso_seqs=%u gso_type=%x frag_list=%p pkt_type=%x\n",
...
# if RTLNX_VER_MIN(2,6,22)
    Log6(("vpoxNetFltLinuxPacketHandler: packet dump follows:\n%.*Rhxd\n", pBuf->len-pBuf->data_len, skb_mac_header(pBuf)));
# endif
#else
    Log3(("vpoxNetFltLinuxPacketHandler: skb len=%u data_len=%u truesize=%u next=%p nr_frags=%u tso_size=%u tso_seqs=%u frag_list=%p pkt_type=%x\n",
    ...
    if (pBuf->pkt_type == PACKET_LOOPBACK)
    {
         ...
         */
        Log2(("vpoxNetFltLinuxPacketHandler: dropped loopback packet (cb=%u)\n", pBuf->len));
        dev_kfree_skb(pBuf); /* We must 'consume' all packets we get (@bugref{6539})! */
        return 0;
    }

    pThis = VPOX_FLT_PT_TO_INST(pPacketType);
    pDev = ASMAtomicUoReadPtrT(&pThis->u.s.pDev, struct net_device *);
    if (pDev != pSkbDev)
    {
        Log(("vpoxNetFltLinuxPacketHandler: Devices do not match, pThis may be wrong! pThis=%p\n", pThis));
        kfree_skb(pBuf); /* This is a failure, so we use kfree_skb instead of dev_kfree_skb. */
        return 0;
    }

    Log6(("vpoxNetFltLinuxPacketHandler: pBuf->cb dump:\n%.*Rhxd\n", sizeof(pBuf->cb), pBuf->cb));
    if (vpoxNetFltLinuxSkBufIsOur(pBuf))
    {
        Log2(("vpoxNetFltLinuxPacketHandler: got our own sk_buff, drop it.\n"));
        dev_kfree_skb(pBuf);
        return 0;
    }
...
#ifndef VPOXNETFLT_SG_SUPPORT
    {
        ...
        if (!pCopy)
        {
            LogRel(("VPoxNetFlt: Failed to allocate packet buffer, dropping the packet.\n"));
            return 0;
        }
        pBuf = pCopy;
...
# if RTLNX_VER_MIN(2,6,18)
        Log3(("vpoxNetFltLinuxPacketHandler: skb copy len=%u data_len=%u truesize=%u next=%p nr_frags=%u gso_size=%u gso_seqs=%u gso_type=%x frag_list=%p pkt_type=%x\n",
              pBuf->len, pBuf->data_len, pBuf->truesize, pBuf->next, skb_shinfo(pBuf)->nr_frags, skb_shinfo(pBuf)->gso_size, skb_shinfo(pBuf)->gso_segs, skb_shinfo(pBuf)->gso_type, skb_shinfo(pBuf)->frag_list, pBuf->pkt_type));
#  if RTLNX_VER_MIN(2,6,22)
        Log6(("vpoxNetFltLinuxPacketHandler: packet dump follows:\n%.*Rhxd\n", pBuf->len-pBuf->data_len, skb_mac_header(pBuf)));
#  endif /* RTLNX_VER_MIN(2,6,22) */
# else /* RTLNX_VER_MAX(2,6,18) */
        Log3(("vpoxNetFltLinuxPacketHandler: skb copy len=%u data_len=%u truesize=%u next=%p nr_frags=%u tso_size=%u tso_seqs=%u frag_list=%p pkt_type=%x\n",
              pBuf->len, pBuf->data_len, pBuf->truesize, pBuf->next, skb_shinfo(pBuf)->nr_frags, skb_shinfo(pBuf)->tso_size, skb_shinfo(pBuf)->tso_segs, skb_shinfo(pBuf)->frag_list, pBuf->pkt_type));
# endif /* RTLNX_VER_MAX(2,6,18) */
    }
#endif /* !VPOXNETFLT_SG_SUPPORT */

#ifdef VPOXNETFLT_LINUX_NO_XMIT_QUEUE
    /* Forward it to the internal network. */
    vpoxNetFltLinuxForwardToIntNet(pThis, pBuf);
#else /* !VPOXNETFLT_LINUX_NO_XMIT_QUEUE */
    ...
    Log6(("vpoxNetFltLinuxPacketHandler: scheduled work %p for sk_buff %p\n",
          &pThis->u.s.XmitTask, pBuf));
#endif /* !VPOXNETFLT_LINUX_NO_XMIT_QUEUE */
 ...
 *                              a temporary storage.
 */
DECLINLINE(unsigned) vpoxNetFltLinuxCalcSGSegments(struct sk_buff *pBuf, unsigned *pcbTemp)
{
    *pcbTemp = 0;
#ifdef VPOXNETFLT_SG_SUPPORT
    ...
    if (pBuf->ip_summed == CHECKSUM_PARTIAL && pBuf->pkt_type == PACKET_OUTGOING)
    {
        *pcbTemp = vpoxNetFltLinuxGetChecksumStartOffset(pBuf) + VPOX_SKB_CSUM_OFFSET(pBuf) + sizeof(uint16_t);
    }
# if RTLNX_VER_MIN(2,6,27)
    if (vlan_tx_tag_present(pBuf))
    {
...
# endif /* RTLNX_VER_MIN(2,6,27) */
    ...
    {
        Log6(("vpoxNetFltLinuxCalcSGSegments: frag=%p len=%d data_len=%d frags=%d frag_list=%p next=%p\n",
...
#endif
#ifdef PADD_RUNT_FRAMES_FROM_HOST
    /* vpoxNetFltLinuxSkBufToSG adds a padding segment if it's a runt. */
    if (pBuf->len < 60)
        cSegs++;
#endif
    return cSegs;
 ...
 * vpoxNetFltLinuxSkBufToSG.
 *
 * @param   pSG             The (scatter/)gather list.
 * @param   pBuf            The original socket buffer that was used to create
 *                          the scatter/gather list.
 */
static void vpoxNetFltLinuxDestroySG(PINTNETSG pSG, struct sk_buff *pBuf)
{
#ifdef VPOXNETFLT_SG_SUPPORT
...
# endif /* LOG_ENABLED */
    ...
    {
        Log6((" %p", pSG->aSegs[iSeg].pv));
        VPOX_SKB_KUNMAP_FRAG(pSG->aSegs[iSeg++].pv);
    }
    struct sk_buff *pFragBuf;
    for (pFragBuf = skb_shinfo(pBuf)->frag_list; pFragBuf; pFragBuf = pFragBuf->next)
    {
 ...
 * Logging helper.
 */
static void vpoxNetFltDumpPacket(PINTNETSG pSG, bool fEgress, const char *pszWhere, int iIncrement)
{
    int i, offSeg;
    uint8_t *pInt, *pExt;
    ...
    Log(("VPoxNetFlt: (int)%02x:%02x:%02x:%02x:%02x:%02x"
...
#else
# define vpoxNetFltDumpPacket(a, b, c, d) do {} while (0)
#endif

#ifdef VPOXNETFLT_WITH_GSO_RECV

/**
 * Worker for vpoxNetFltLinuxForwardToIntNet that checks if we can forwards a
 ...
 */
static bool vpoxNetFltLinuxCanForwardAsGso(PVPOXNETFLTINS pThis, struct sk_buff *pSkb, uint32_t fSrc,
                                           PPDMNETWORKGSO pGsoCtx)
{
    ...
    {
        Log5(("vpoxNetFltLinuxCanForwardAsGso: gso_type=%#x\n", skb_shinfo(pSkb)->gso_type));
        return false;
    }
    if (RT_UNLIKELY(   skb_shinfo(pSkb)->gso_size < 1
                    || pSkb->len > VPOX_MAX_GSO_SIZE ))
    {
        Log5(("vpoxNetFltLinuxCanForwardAsGso: gso_size=%#x skb_len=%#x (max=%#x)\n", skb_shinfo(pSkb)->gso_size, pSkb->len, VPOX_MAX_GSO_SIZE));
        return false;
    }
    ...
    switch (uEtherType)
    {
        case RT_H2N_U16_C(RTNET_ETHERTYPE_IPV4):
        {
            ...
            {
                Log5(("vpoxNetFltLinuxCanForwardAsGso: failed to access IPv4 hdr\n"));
                return false;
            }
            ...
            if (RT_UNLIKELY(   cbHdr < RTNETIPV4_MIN_LEN
                            || cbHdr > cbTransport ))
            {
                Log5(("vpoxNetFltLinuxCanForwardAsGso: invalid IPv4 lengths: ip_hl=%u ip_len=%u\n", pIPv4->ip_hl, RT_N2H_U16(pIPv4->ip_len)));
                return false;
            }
            cbTransport -= cbHdr;
        ...
        case RT_H2N_U16_C(RTNET_ETHERTYPE_IPV6):
        {
            ...
            {
                Log5(("vpoxNetFltLinuxCanForwardAsGso: failed to access IPv6 hdr\n"));
                return false;
            }
        ...
        default:
            Log5(("vpoxNetFltLinuxCanForwardAsGso: uEtherType=%#x\n", RT_H2N_U16(uEtherType)));
            return false;
    }

    if (enmGsoType == PDMNETWORKGSOTYPE_INVALID)
    {
        Log5(("vpoxNetFltLinuxCanForwardAsGso: Unsupported protocol %d\n", uProtocol));
        return false;
    }
    ...
    {
        Log5(("vpoxNetFltLinuxCanForwardAsGso: Bad transport length; off=%#x + cb=%#x => %#x; skb_len=%#x (%s)\n",
    ...
    if (uProtocol == RTNETIPV4_PROT_TCP)
    {
        ...
        if (RT_UNLIKELY(!pTcp))
        {
            Log5(("vpoxNetFltLinuxCanForwardAsGso: failed to access TCP hdr\n"));
            return false;
        }
        ...
        {
            Log5(("vpoxNetFltLinuxCanForwardAsGso: No space for TCP header; off=%#x cb=%#x skb_len=%#x\n", offTransport, cbTransportHdr, pSkb->len));
            return false;
        }
    ...
    else
    {
        ...
        if (RT_UNLIKELY(   offTransport + cbTransportHdr >= UINT8_MAX
                        || offTransport + cbTransportHdr >= pSkb->len ))
        {
            Log5(("vpoxNetFltLinuxCanForwardAsGso: No space for UDP header; off=%#x skb_len=%#x\n", offTransport, pSkb->len));
            return false;
        }
    }
 ...
 * @param   pGsoCtx             Where to return the GSO context on success.
 */
static int vpoxNetFltLinuxForwardAsGso(PVPOXNETFLTINS pThis, struct sk_buff *pSkb, uint32_t fSrc, PCPDMNETWORKGSO pGsoCtx)
{
    int         rc;
    unsigned    cbExtra;
    unsigned    cSegs = vpoxNetFltLinuxCalcSGSegments(pSkb, &cbExtra);
    PINTNETSG pSG = (PINTNETSG)alloca(RT_UOFFSETOF_DYN(INTNETSG, aSegs[cSegs]) + cbExtra);
    if (RT_LIKELY(pSG))
    {
        vpoxNetFltLinuxSkBufToSG(pThis, pSkb, pSG, cbExtra, cSegs, fSrc, pGsoCtx);

        vpoxNetFltDumpPacket(pSG, false, (fSrc & INTNETTRUNKDIR_HOST) ? "host" : "wire", 1);
        pThis->pSwitchPort->pfnRecv(pThis->pSwitchPort, NULL /* pvIf */, pSG, fSrc);

        vpoxNetFltLinuxDestroySG(pSG, pSkb);
        rc = VINF_SUCCESS;
    }
    else
    {
        Log(("VPoxNetFlt: Dropping the sk_buff (failure case).\n"));
        rc = VERR_NO_MEMORY;
    }
    return rc;
}

#endif /* VPOXNETFLT_WITH_GSO_RECV */

/**
 * Worker for vpoxNetFltLinuxForwardToIntNet.
 *
 ...
 * @param   fSrc                The source.
 */
static int vpoxNetFltLinuxForwardSegment(PVPOXNETFLTINS pThis, struct sk_buff *pBuf, uint32_t fSrc)
{
    int         rc;
    unsigned    cbExtra;
    unsigned    cSegs = vpoxNetFltLinuxCalcSGSegments(pBuf, &cbExtra);
    PINTNETSG pSG = (PINTNETSG)alloca(RT_UOFFSETOF_DYN(INTNETSG, aSegs[cSegs]) + cbExtra);
    if (RT_LIKELY(pSG))
    {
        vpoxNetFltLinuxSkBufToSG(pThis, pBuf, pSG, cbExtra, cSegs, fSrc, NULL /*pGsoCtx*/);

        vpoxNetFltDumpPacket(pSG, false, (fSrc & INTNETTRUNKDIR_HOST) ? "host" : "wire", 1);
        pThis->pSwitchPort->pfnRecv(pThis->pSwitchPort, NULL /* pvIf */, pSG, fSrc);

        vpoxNetFltLinuxDestroySG(pSG, pBuf);
        rc = VINF_SUCCESS;
    }
    else
    {
        Log(("VPoxNetFlt: Failed to allocate SG buffer.\n"));
        rc = VERR_NO_MEMORY;
    }
    return rc;
}
 ...
 */
static void vpoxNetFltLinuxForwardToIntNetInner(PVPOXNETFLTINS pThis, struct sk_buff *pBuf, uint32_t fSrc)
{
#ifdef VPOXNETFLT_WITH_GSO
    if (skb_is_gso(pBuf))
    {
        PDMNETWORKGSO GsoCtx;
        Log6(("vpoxNetFltLinuxForwardToIntNetInner: skb len=%u data_len=%u truesize=%u next=%p"
...
# ifdef VPOXNETFLT_WITH_GSO_RECV
        if (   (skb_shinfo(pBuf)->gso_type & (SKB_GSO_TCPV6 | SKB_GSO_TCPV4))
            && vpoxNetFltLinuxCanForwardAsGso(pThis, pBuf, fSrc, &GsoCtx) )
            vpoxNetFltLinuxForwardAsGso(pThis, pBuf, fSrc, &GsoCtx);
        else
# endif /* VPOXNETFLT_WITH_GSO_RECV */
        {
            ...
            if (IS_ERR(pSegment))
            {
                LogRel(("VPoxNetFlt: Failed to segment a packet (%d).\n", PTR_ERR(pSegment)));
                return;
            }

            for (; pSegment; pSegment = pNext)
            {
                Log6(("vpoxNetFltLinuxForwardToIntNetInner: segment len=%u data_len=%u truesize=%u next=%p"
                ...
                pNext = pSegment->next;
                pSegment->next = 0;
                vpoxNetFltLinuxForwardSegment(pThis, pSegment, fSrc);
                dev_kfree_skb(pSegment);
            }
        }
    }
    else
#endif /* VPOXNETFLT_WITH_GSO */
    {
        Log6(("vpoxNetFltLinuxForwardToIntNetInner: ptk_type=%d ip_summed=%d len=%d"
              " data_len=%d headroom=%d hdr_len=%d csum_offset=%d\n",
              pBuf->pkt_type, pBuf->ip_summed, pBuf->len, pBuf->data_len, skb_headroom(pBuf),
              skb_headlen(pBuf), vpoxNetFltLinuxGetChecksumStartOffset(pBuf)));
#ifndef VPOXNETFLT_SG_SUPPORT
        if (pBuf->ip_summed == CHECKSUM_PARTIAL && pBuf->pkt_type == PACKET_OUTGOING)
        {
# if RTLNX_VER_MIN(2,6,19)
            int rc = VPOX_SKB_CHECKSUM_HELP(pBuf);
# else
            /*
             * Try to work around the problem with CentOS 4.7 and 5.2 (2.6.9
             * and 2.6.18 kernels), they pass wrong 'h' pointer down. We take IP
...
# endif
            if (rc)
            {
                LogRel(("VPoxNetFlt: Failed to compute checksum, dropping the packet.\n"));
                return;
            }
        }
#endif /* !VPOXNETFLT_SG_SUPPORT */
        vpoxNetFltLinuxForwardSegment(pThis, pBuf, fSrc);
    }
}
...
static void vpoxNetFltLinuxForwardToIntNet(PVPOXNETFLTINS pThis, struct sk_buff *pBuf)
{
    ...
    if (RT_UNLIKELY(fSrc & INTNETTRUNKDIR_WIRE))
    {
        ...
        Log5(("vpoxNetFltLinuxForwardToIntNet: mac_len=%d data=%p mac_header=%p network_header=%p\n",
              pBuf->mac_len, pBuf->data, skb_mac_header(pBuf), skb_network_header(pBuf)));
    }

    vpoxNetFltLinuxForwardToIntNetInner(pThis, pBuf, fSrc);
 ...
 * vpoxNetFltLinuxPacketHandler to the internal network.
 *
 * @param   pWork               The work queue.
 */
# if RTLNX_VER_MIN(2,6,20)
static void vpoxNetFltLinuxXmitTask(struct work_struct *pWork)
# else
static void vpoxNetFltLinuxXmitTask(void *pWork)
# endif
{
    PVPOXNETFLTINS  pThis   = VPOX_FLT_XT_TO_INST(pWork);
    struct sk_buff *pBuf;

    Log6(("vpoxNetFltLinuxXmitTask: Got work %p.\n", pWork));

    /*
     * Active? Retain the instance and increment the busy counter.
     */
    if (vpoxNetFltTryRetainBusyActive(pThis))
    {
 ...
 *                              reference to this.
 */
static void vpoxNetFltLinuxReportNicGsoCapabilities(PVPOXNETFLTINS pThis)
{
#ifdef VPOXNETFLT_WITH_GSO_XMIT_WIRE
    if (vpoxNetFltTryRetainBusyNotDisconnected(pThis))
    {
        ...
        if (pThis->pSwitchPort)
        {
            ...
            Log3(("vpoxNetFltLinuxReportNicGsoCapabilities: reporting wire %s%s%s%s\n",
...
#endif /* VPOXNETFLT_WITH_GSO_XMIT_WIRE */
}
 ...
 */
static bool vpoxNetFltLinuxPromiscuous(PVPOXNETFLTINS pThis)
{
    bool                fRc  = false;
    struct net_device * pDev = vpoxNetFltLinuxRetainNetDev(pThis);
    if (pDev)
    {
        fRc = !!(pDev->promiscuity - (ASMAtomicUoReadBool(&pThis->u.s.fPromiscuousSet) & 1));
        LogFlow(("vpoxNetFltPortOsIsPromiscuous: returns %d, pDev->promiscuity=%d, fPromiscuousSet=%d\n",
                 fRc, pDev->promiscuity, pThis->u.s.fPromiscuousSet));
        vpoxNetFltLinuxReleaseNetDev(pThis, pDev);
    }
    return fRc;
}
 ...
 * Currently we need it for our own VPoxNetAdp and TAP.
 */
static bool vpoxNetFltNeedsLinkState(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
    if (pDev->ethtool_ops && pDev->ethtool_ops->get_drvinfo)
    {
        ...
        if (!strncmp(Info.driver, "vpoxnet", sizeof(Info.driver)))
            return true;
 ...
 */
static void vpoxNetFltSetLinkState(PVPOXNETFLTINS pThis, struct net_device *pDev, bool fLinkUp)
{
    if (vpoxNetFltNeedsLinkState(pThis, pDev))
    {
...
/**
 * Internal worker for vpoxNetFltLinuxNotifierCallback.
 *
 * @returns VPox status code.
 * @param   pThis           The instance.
 * @param   pDev            The device to attach to.
 */
static int vpoxNetFltLinuxAttachToInterface(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
    LogFlow(("vpoxNetFltLinuxAttachToInterface: pThis=%p (%s)\n", pThis, pThis->szName));
    ...
    Log(("vpoxNetFltLinuxAttachToInterface: Device %p(%s) retained. ref=%d\n",
          pDev, pDev->name,
#if RTLNX_VER_MIN(2,6,37)
          netdev_refcnt_read(pDev)
#else
    ...
    Log(("vpoxNetFltLinuxAttachToInterface: Got pDev=%p pThis=%p pThis->u.s.pDev=%p\n",
    ...
    pThis->u.s.PacketType.func = vpoxNetFltLinuxPacketHandler;
    dev_add_pack(&pThis->u.s.PacketType);
    ASMAtomicUoWriteBool(&pThis->u.s.fPacketHandler, true);
    Log(("vpoxNetFltLinuxAttachToInterface: this=%p: Packet handler installed.\n", pThis));

#ifdef VPOXNETFLT_WITH_HOST2WIRE_FILTER
    vpoxNetFltLinuxHookDev(pThis, pDev);
#endif

    /*
     * Are we the "carrier" for this device (e.g. vpoxnet or tap)?
     */
    vpoxNetFltSetLinkState(pThis, pDev, true);
    ...
    if (vpoxNetFltTryRetainBusyNotDisconnected(pThis))
    {
        vpoxNetFltLinuxReportNicGsoCapabilities(pThis);
        pThis->pSwitchPort->pfnReportMacAddress(pThis->pSwitchPort, &pThis->u.s.MacAddr);
        pThis->pSwitchPort->pfnReportPromiscuousMode(pThis->pSwitchPort, vpoxNetFltLinuxPromiscuous(pThis));
        pThis->pSwitchPort->pfnReportNoPreemptDsts(pThis->pSwitchPort, INTNETTRUNKDIR_WIRE | INTNETTRUNKDIR_HOST);
        vpoxNetFltRelease(pThis, true /*fBusy*/);
    }

    LogRel(("VPoxNetFlt: attached to '%s' / %RTmac\n", pThis->szName, &pThis->u.s.MacAddr));
    return VINF_SUCCESS;
}


static int vpoxNetFltLinuxUnregisterDevice(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
    bool fRegistered;
    Assert(!pThis->fDisconnectedFromHost);

#ifdef VPOXNETFLT_WITH_HOST2WIRE_FILTER
    vpoxNetFltLinuxUnhookDev(pThis, pDev);
#endif

    if (ASMAtomicCmpXchgBool(&pThis->u.s.fPacketHandler, false, true))
    {
        dev_remove_pack(&pThis->u.s.PacketType);
        Log(("vpoxNetFltLinuxUnregisterDevice: this=%p: packet handler removed.\n", pThis));
    }
...
#endif
        Log(("vpoxNetFltLinuxUnregisterDevice: this=%p: xmit queue purged.\n", pThis));
        Log(("vpoxNetFltLinuxUnregisterDevice: Device %p(%s) released. ref=%d\n",
             pDev, pDev->name,
#if RTLNX_VER_MIN(2,6,37)
             netdev_refcnt_read(pDev)
#else
...
static int vpoxNetFltLinuxDeviceIsUp(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
        ...
        && !ASMAtomicUoReadBool(&pThis->u.s.fPromiscuousSet))
    {
        ...
        Log(("vpoxNetFltLinuxDeviceIsUp: enabled promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
    }
    else
        Log(("vpoxNetFltLinuxDeviceIsUp: no need to enable promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
    return NOTIFY_OK;
}

static int vpoxNetFltLinuxDeviceGoingDown(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
    ...
    if (ASMAtomicUoReadBool(&pThis->u.s.fPromiscuousSet))
    {
        ...
        Log(("vpoxNetFltLinuxDeviceGoingDown: disabled promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
    }
    else
        Log(("vpoxNetFltLinuxDeviceGoingDown: no need to disable promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
    return NOTIFY_OK;
}
 ...
 */
static int vpoxNetFltLinuxDeviceMtuChange(PVPOXNETFLTINS pThis, struct net_device *pDev)
{
    ASMAtomicWriteU32(&pThis->u.s.cbMtu, pDev->mtu);
    Log(("vpoxNetFltLinuxDeviceMtuChange: set MTU for %s to %d\n", pThis->szName, pDev->mtu));
    return NOTIFY_OK;
}

#ifdef LOG_ENABLED
/** Stringify the NETDEV_XXX constants. */
static const char *vpoxNetFltLinuxGetNetDevEventName(unsigned long ulEventType)
{
 ...
 *                              relates to.
 */
static int vpoxNetFltLinuxNotifierCallback(struct notifier_block *self, unsigned long ulEventType, void *ptr)

{
    PVPOXNETFLTINS      pThis = VPOX_FLT_NB_TO_INST(self);
    struct net_device  *pMyDev = ASMAtomicUoReadPtrT(&pThis->u.s.pDev, struct net_device *);
    struct net_device  *pDev  = VPOX_NETDEV_NOTIFIER_INFO_TO_DEV(ptr);
    int                 rc    = NOTIFY_OK;

    Log(("VPoxNetFlt: got event %s(0x%lx) on %s, pDev=%p pThis=%p pThis->u.s.pDev=%p\n",
         vpoxNetFltLinuxGetNetDevEventName(ulEventType), ulEventType, pDev->name, pDev, pThis, pMyDev));

    if (ulEventType == NETDEV_REGISTER)
    {
#if RTLNX_VER_MIN(2,6,24) /* cgroups/namespaces introduced */
# if RTLNX_VER_MIN(2,6,26)
#  define VPOX_DEV_NET(dev)             dev_net(dev)
#  define VPOX_NET_EQ(n1, n2)           net_eq((n1), (n2))
# else
#  define VPOX_DEV_NET(dev)             ((dev)->nd_net)
#  define VPOX_NET_EQ(n1, n2)           ((n1) == (n2))
# endif
        struct net *pMyNet = current->nsproxy->net_ns;
        struct net *pDevNet = VPOX_DEV_NET(pDev);

        if (VPOX_NET_EQ(pDevNet, pMyNet))
#endif  /* namespaces */
        {
            if (strcmp(pDev->name, pThis->szName) == 0)
            {
                vpoxNetFltLinuxAttachToInterface(pThis, pDev);
            }
        }
    }
    else
        ...
        if (pDev == pMyDev)
        {
            switch (ulEventType)
            {
                case NETDEV_UNREGISTER:
                    rc = vpoxNetFltLinuxUnregisterDevice(pThis, pDev);
                    break;
                case NETDEV_UP:
                    rc = vpoxNetFltLinuxDeviceIsUp(pThis, pDev);
                    break;
                case NETDEV_GOING_DOWN:
                    rc = vpoxNetFltLinuxDeviceGoingDown(pThis, pDev);
                    break;
                case NETDEV_CHANGEMTU:
                    rc = vpoxNetFltLinuxDeviceMtuChange(pThis, pDev);
                    break;
 ...
 */
static int vpoxNetFltLinuxEnumeratorCallback(struct notifier_block *self, unsigned long ulEventType, void *ptr)
{
    PVPOXNETFLTINS pThis = ((PVPOXNETFLTNOTIFIER)self)->pThis;
    struct net_device *dev  = VPOX_NETDEV_NOTIFIER_INFO_TO_DEV(ptr);
    ...
    if (in_dev != NULL)
    {
        ...
        for (ifa = in_dev->ifa_list; ifa; ifa = ifa->ifa_next) {
            if (VPOX_IPV4_IS_LOOPBACK(ifa->ifa_address))
                return NOTIFY_OK;

            if (   dev != pThis->u.s.pDev
                && VPOX_IPV4_IS_LINKLOCAL_169(ifa->ifa_address))
                continue;

            Log(("%s: %s: IPv4 addr %RTnaipv4 mask %RTnaipv4\n",
                 __FUNCTION__, VPOX_NETDEV_NAME(dev),
...
static int vpoxNetFltLinuxNotifierIPv4Callback(struct notifier_block *self, unsigned long ulEventType, void *ptr)
{
    PVPOXNETFLTINS     pThis = RT_FROM_MEMBER(self, VPOXNETFLTINS, u.s.NotifierIPv4);
    struct net_device *pDev, *pEventDev;
    struct in_ifaddr  *ifa   = (struct in_ifaddr *)ptr;
    bool               fMyDev;
    int                rc    = NOTIFY_OK;

    pDev = vpoxNetFltLinuxRetainNetDev(pThis);
    pEventDev = ifa->ifa_dev->dev;
    fMyDev = (pDev == pEventDev);
    Log(("VPoxNetFlt: %s: IPv4 event %s(0x%lx) %s: addr %RTnaipv4 mask %RTnaipv4\n",
         pDev ? VPOX_NETDEV_NAME(pDev) : "<unknown>",
         vpoxNetFltLinuxGetNetDevEventName(ulEventType), ulEventType,
         pEventDev ? VPOX_NETDEV_NAME(pEventDev) : "<unknown>",
         ifa->ifa_address, ifa->ifa_mask));
...
static int vpoxNetFltLinuxNotifierIPv6Callback(struct notifier_block *self, unsigned long ulEventType, void *ptr)
{
    PVPOXNETFLTINS       pThis = RT_FROM_MEMBER(self, VPOXNETFLTINS, u.s.NotifierIPv6);
    struct net_device   *pDev, *pEventDev;
    ...
    fMyDev = (pDev == pEventDev);
    Log(("VPoxNetFlt: %s: IPv6 event %s(0x%lx) %s: %RTnaipv6\n",
         pDev ? VPOX_NETDEV_NAME(pDev) : "<unknown>",
         vpoxNetFltLinuxGetNetDevEventName(ulEventType), ulEventType,
         pEventDev ? VPOX_NETDEV_NAME(pEventDev) : "<unknown>",
         &ifa->addr));
...
int  vpoxNetFltPortOsXmit(PVPOXNETFLTINS pThis, void *pvIfData, PINTNETSG pSG, uint32_t fDst)
{
    ...
    LogFlow(("vpoxNetFltPortOsXmit: pThis=%p (%s)\n", pThis, pThis->szName));
    ...
    if (pDev)
    {
        ...
        {
            struct sk_buff *pBuf = vpoxNetFltLinuxSkBufFromSG(pThis, pSG, true);
            if (pBuf)
            {
                vpoxNetFltDumpPacket(pSG, true, "wire", 1);
                Log6(("vpoxNetFltPortOsXmit: pBuf->cb dump:\n%.*Rhxd\n", sizeof(pBuf->cb), pBuf->cb));
                Log6(("vpoxNetFltPortOsXmit: dev_queue_xmit(%p)\n", pBuf));
        ...
        {
            struct sk_buff *pBuf = vpoxNetFltLinuxSkBufFromSG(pThis, pSG, false);
            if (pBuf)
            {
                vpoxNetFltDumpPacket(pSG, true, "host", (fDst & INTNETTRUNKDIR_WIRE) ? 0 : 1);
                Log6(("vpoxNetFltPortOsXmit: pBuf->cb dump:\n%.*Rhxd\n", sizeof(pBuf->cb), pBuf->cb));
                Log6(("vpoxNetFltPortOsXmit: netif_rx_ni(%p)\n", pBuf));
...
void vpoxNetFltPortOsSetActive(PVPOXNETFLTINS pThis, bool fActive)
{
    ...
    LogFlow(("vpoxNetFltPortOsSetActive: pThis=%p (%s), fActive=%RTbool, fDisablePromiscuous=%RTbool\n",
    ...
    if (pDev)
    {
         ...
         * host the interface promiscuity for vpoxNetFltPortOsIsPromiscuous.
         */
#ifdef LOG_ENABLED
        u_int16_t fIf;
        ...
        if (fActive)
        {
            ...
            Log(("vpoxNetFltPortOsSetActive: enabled promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
        }
        else
        {
            if (pThis->u.s.fPromiscuousSet)
            {
                ...
                dev_set_promiscuity(pDev, -1);
                rtnl_unlock();
                Log(("vpoxNetFltPortOsSetActive: disabled promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
            }
            pThis->u.s.fPromiscuousSet = false;

#ifdef LOG_ENABLED
            fIf = dev_get_flags(pDev);
            Log(("VPoxNetFlt: fIf=%#x; %d->%d\n", fIf, cPromiscBefore, pDev->promiscuity));
#endif
        }
...
int vpoxNetFltOsDisconnectIt(PVPOXNETFLTINS pThis)
{
    ...
    if (ASMAtomicCmpXchgBool(&pThis->u.s.fPacketHandler, false, true))
    {
        IPRT_LINUX_SAVE_EFL_AC();
        dev_remove_pack(&pThis->u.s.PacketType);
        Log(("vpoxNetFltOsDisconnectIt: this=%p: Packet handler removed.\n", pThis));
        IPRT_LINUX_RESTORE_EFL_AC();
    }
    return VINF_SUCCESS;
}
...
#if defined(VPOXNETFLT_WITH_GSO_XMIT_HOST)
    Log3(("vpoxNetFltOsConnectIt: reporting host tso tso6\n"));
...
void vpoxNetFltOsDeleteInstance(PVPOXNETFLTINS pThis)
{
...
#ifdef VPOXNETFLT_WITH_HOST2WIRE_FILTER
    vpoxNetFltLinuxUnhookDev(pThis, NULL);
#endif

    /** @todo This code may race vpoxNetFltLinuxUnregisterDevice (very very
    ...
    if (fRegistered)
    {
        vpoxNetFltSetLinkState(pThis, pDev, false);

#ifndef VPOXNETFLT_LINUX_NO_XMIT_QUEUE
        skb_queue_purge(&pThis->u.s.XmitQueue);
#endif
        Log(("vpoxNetFltOsDeleteInstance: this=%p: xmit queue purged.\n", pThis));
        Log(("vpoxNetFltOsDeleteInstance: Device %p(%s) released. ref=%d\n",
             pDev, pDev->name,
#if RTLNX_VER_MIN(2,6,37)
             netdev_refcnt_read(pDev)
#else
    ...
    Log(("vpoxNetFltOsDeleteInstance: this=%p: Notifier removed.\n", pThis));
...
int  vpoxNetFltOsInitInstance(PVPOXNETFLTINS pThis, void *pvContext)
{
    ...
    pThis->u.s.Notifier.notifier_call = vpoxNetFltLinuxNotifierCallback;
    ...
    {
        unregister_netdevice_notifier(&pThis->u.s.Notifier);
        LogRel(("VPoxNetFlt: failed to find %s.\n", pThis->szName));
        IPRT_LINUX_RESTORE_EFL_AC();
        return VERR_INTNET_FLT_IF_NOT_FOUND;
    }

    Log(("vpoxNetFltOsInitInstance: this=%p: Notifier installed.\n", pThis));
    ...
    if (pThis->pSwitchPort->pfnNotifyHostAddress)
    {
         ...
         */
        RT_ZERO(Enumerator);
        Enumerator.Notifier.notifier_call = vpoxNetFltLinuxEnumeratorCallback;
        ...
        pThis->u.s.NotifierIPv4.notifier_call = vpoxNetFltLinuxNotifierIPv4Callback;
        err = register_inetaddr_notifier(&pThis->u.s.NotifierIPv4);
        if (err)
            LogRel(("%s: failed to register IPv4 notifier: error %d\n", __FUNCTION__, err));

        pThis->u.s.NotifierIPv6.notifier_call = vpoxNetFltLinuxNotifierIPv6Callback;
...
# if RTLNX_VER_MIN(2,6,20)
    INIT_WORK(&pThis->u.s.XmitTask, vpoxNetFltLinuxXmitTask);
# else
    INIT_WORK(&pThis->u.s.XmitTask, vpoxNetFltLinuxXmitTask, &pThis->u.s.XmitTask);
# endif
#endif
...
int vpoxNetFltPortOsConnectInterface(PVPOXNETFLTINS pThis, void *pvIf, void **pvIfData)
{
    /* Nothing to do */
