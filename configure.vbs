...
sub CfgInit
   ...
   if g_blnInternalMode = False then
      CfgPrint "VPOX_OSE := 1"
      CfgPrint "VPOX_VCC_WERR = $(NO_SUCH_VARIABLE)"
   end if
end sub
...
sub DisableCOM(strReason)
   if g_blnDisableCOM = False then
      ...
      CfgPrint "VPOX_WITH_MAIN="
      CfgPrint "VPOX_WITH_QTGUI="
      CfgPrint "VPOX_WITH_VPOXSDL="
      CfgPrint "VPOX_WITH_DEBUGGER_GUI="
   end if
end sub
...
sub CheckForVisualCPP(strOptVC, strOptVCCommon, blnOptVCExpressEdition)
   ...
   if InStr(1, g_strShellOutput, "Version 16.") > 0 then
      ...
      elseif   LogFileExists(g_strPathDDK, "inc/atl71/atlbase.h") _
         ...
         CfgPrint "VPOX_WITHOUT_COMPILER_REDIST=1"
...
sub CheckForPlatformSDK(strOptSDK)
   ...
   CfgPrint "VPOX_WINPSDK          := WINPSDK" & g_strVerPSDK
...
sub CheckForMidl()
   ...
   CfgPrint "VPOX_MAIN_IDL         := " & strMidl
...
sub CheckForXml2(strOptXml2)
   ...
   CfgPrint "SDK_VPOX_LIBXML2_DEFS  := _REENTRANT"
   CfgPrint "SDK_VPOX_LIBXML2_INCS  := " & strPathXml2 & "/include"
   CfgPrint "SDK_VPOX_LIBXML2_LIBS  := " & strPathXml2 & "/lib/libxml2.lib"
...
sub CheckForSsl(strOptSsl, bln32Bit)
   ...
   if bln32Bit = True then
      CfgPrint "SDK_VPOX_OPENSSL-x86_INCS := " & strPathSsl & "/include"
      CfgPrint "SDK_VPOX_OPENSSL-x86_LIBS := " & strPathSsl & "/lib/libcrypto.lib" & " " & strPathSsl & "/lib/libssl.lib"
      CfgPrint "SDK_VPOX_BLD_OPENSSL-x86_LIBS := " & strPathSsl & "/lib/libcrypto.lib" & " " & strPathSsl & "/lib/libssl.lib"
   else
      CfgPrint "SDK_VPOX_OPENSSL_INCS := " & strPathSsl & "/include"
      CfgPrint "SDK_VPOX_OPENSSL_LIBS := " & strPathSsl & "/lib/libcrypto.lib" & " " & strPathSsl & "/lib/libssl.lib"
      CfgPrint "SDK_VPOX_BLD_OPENSSL_LIBS := " & strPathSsl & "/lib/libcrypto.lib" & " " & strPathSsl & "/lib/libssl.lib"
   end if
...
sub CheckForCurl(strOptCurl, bln32Bit)
   ...
   if bln32Bit = True then
      CfgPrint "SDK_VPOX_LIBCURL-x86_INCS := " & strPathCurl & "/include"
      CfgPrint "SDK_VPOX_LIBCURL-x86_LIBS.x86 := " & strPathCurl & "/libcurl.lib"
   else
      CfgPrint "SDK_VPOX_LIBCURL_INCS := " & strPathCurl & "/include"
      CfgPrint "SDK_VPOX_LIBCURL_LIBS := " & strPathCurl & "/libcurl.lib"
   end if
...
sub CheckForQt(strOptQt5)
   ...
   if strPathQt5 <> "" then
      ...
      CfgPrint "VPOX_PATH_QT          := $(PATH_SDK_QT5)"
   end if
   if strPathQt5 = "" then
      CfgPrint "VPOX_WITH_QTGUI       :="
   end if
...
function CheckForQt5Sub(strPathQt5)
   ...
   if   LogFileExists(strPathQt5, "bin/moc.exe") _
    ...
    And (   LogFileExists(strPathQt5, "lib/Qt5Core.lib") _
         Or LogFileExists(strPathQt5, "lib/Qt5CoreVPox.lib")) _
    And (   LogFileExists(strPathQt5, "lib/Qt5Network.lib") _
         Or LogFileExists(strPathQt5, "lib/Qt5NetworkVPox.lib")) _
      then
         CheckForQt5Sub = True
   end if
...
function CheckForPython(strPathPython)
   ...
   if LogFileExists(strPathPython, "python.exe") then
      CfgPrint "VPOX_BLD_PYTHON       := " & strPathPython & "\python.exe"
      CheckForPython = True
   end if
...
Sub Main
   ...
   CfgPrint "VPOX_WITH_OPEN_WATCOM := " '' @todo look for openwatcom 1.9+
   CfgPrint "VPOX_WITH_LIBVPX := " '' @todo look for libvpx 1.1.0+
   CfgPrint "VPOX_WITH_LIBOPUS := " '' @todo look for libopus 1.2.1+
   ...
   Print ""
   Print "Execute env.bat once before you start to build VPox:"
   Print ""
   Print "  env.bat"
   Print "  kmk"
   Print ""
