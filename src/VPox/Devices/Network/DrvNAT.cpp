 ...
 * search string) at runtime (VPOX_NAT_ENFORCE_INTERNAL_DNS_UPDATE), we can do it safely on NAT thread,
...
DECLINLINE(void) drvNATUpdateDNS(PDRVNAT pThis, bool fFlapLink)
{
    ...
    switch (strategy)
    {
        ...
        case VPOX_NAT_DNS_HOSTRESOLVER:
        default:
            return;
    }
}
...
#endif /* !VPOX_WITH_DNSMAPPING_IN_HOSTRESOLVER */
...
# include "counters.h"
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            SCDynamicStoreRef hDynStor = SCDynamicStoreCreate(NULL, CFSTR("org.virtualpox.drvnat"), drvNatDnsChanged, &SCDynStorCtx);
            if (hDynStor)
            {
