 ...
 */
DECLHIDDEN(VPOXSTRICTRC) flashWrite(PFLASHCORE pThis, uint32_t off, const void *pv, size_t cb)
{
    ...
    if (cb == 1)
    {
        ...
        if (rcStrict == VINF_SUCCESS)
            ...
            LogFlow(("flashWrite: incomplete write at %08RX32 (LB %u): rc=%Rrc bCmd=%#x cBusCycle=%u\n",
                     off, cb, VPOXSTRICTRC_VAL(rcStrict), *pbSrc, pThis->cBusCycle));
    }
    else
    {
 ...
 */
DECLHIDDEN(VPOXSTRICTRC) flashRead(PFLASHCORE pThis, uint32_t off, void *pv, size_t cb)
{
...
#else
        VPOXSTRICTRC rcStrict = flashMemReadByte(pThis, off + offRead, &pbDst[offRead]);
        if (rcStrict != VINF_SUCCESS)
        {
            LogFlow(("flashRead: incomplete read at %08RX32+%#x (LB %u): rc=%Rrc bCmd=%#x\n",
                     off, offRead, cb, VPOXSTRICTRC_VAL(rcStrict), pThis->bCmd));
            return rcStrict;
        }
#endif
    }
