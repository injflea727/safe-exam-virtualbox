 ...
 *  - On Linux 4.6 and later guests, VPoxClient --vmsvga should be running as
 ...
 *    later, VPoxClient --vmsvga should never be running as root, and should run
 ...
 *  - On guests running X.Org Server 1.2 or older, VPoxClient --vmsvga should
 ...
 *  - When VMSVGA is not enabled, VPoxClient --vmsvga should never stay running.
...
static int startX11MonitorThread()
{
    ...
    if (mX11MonitorThread == NIL_RTTHREAD)
    {
        ...
        if (RT_FAILURE(rc))
            VBClLogFatalError("Warning: failed to start X11 monitor thread (VPoxClient) rc=%Rrc!\n", rc);
    }
    else
...
/**
 * An abbreviated copy of the VGSvcReadProp from VPoxServiceUtils.cpp
 */
...
/**
 * We start VPoxDRMClient from VPoxService in case  some guest property is set.
...
static bool checkDRMClient()
{
   ...
   if (RT_FAILURE(rc))
       return false;
   rc = readGuestProperty(uGuestPropSvcClientID, "/VirtualPox/GuestAdd/DRMResize" /*pszPropName*/);
   if (RT_FAILURE(rc))
       return false;
   return true;
}
...
# define VPOX_DRMCLIENT_LEGACY_EXECUTABLE    "/usr/bin/VPoxClient"
    const char *apszArgs[3] = { VPOX_DRMCLIENT_LEGACY_EXECUTABLE, "--display", NULL };

    int rc = RTProcCreate(VPOX_DRMCLIENT_LEGACY_EXECUTABLE, apszArgs, RTENV_DEFAULT,
...
static XRRModeInfo *createMode(int iXRes, int iYRes)
{
    ...
    char sModeName[126];
    sprintf(sModeName, "%dx%d_vpox", iXRes, iYRes);
#ifdef WITH_DISTRO_XRAND_XINERAMA
...
{
    return ".vpoxclient-display-svga-x11.pid";
}
