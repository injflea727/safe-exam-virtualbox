...
public:
    ...
    HRESULT initUnattended(VirtualPox *aParent);
    void uninit();
    ...
    VPOXOSTYPE     i_getGuestOsType() const;
    Utf8Str const &i_getDetectedOSVersion();

private:
    ComPtr<VirtualPox> const mParent;       /**< Strong reference to the parent object (VirtualPox/IMachine). */
    ...
    VPOXOSTYPE      meGuestOsType;          /**< The guest OS type (set by prepare). */
    ...
    HRESULT i_innerDetectIsoOSWindows(RTVFS hVfsIso, DETECTBUFFER *puBuf, VPOXOSTYPE *penmOsType);
    HRESULT i_innerDetectIsoOSLinux(RTVFS hVfsIso, DETECTBUFFER *puBuf, VPOXOSTYPE *penmOsType);
