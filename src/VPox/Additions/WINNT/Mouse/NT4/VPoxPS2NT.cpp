...

                {
                    ...
                    if (pReq)
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            Log(("VPoxMouseNT: ERROR querying mouse capabilities from VMMDev. rc = %Rrc\n", rc));
                    }
                }
                QueueInput(pDevObj);
                pDevExt->MouExt.InputState = MouseIdle;
...
#define NAME_MAX 256
    ...
    LogFlow(("VPoxMouseNT::DriverEntry: enter\n"));
    ...
    for (unsigned i = 0; i < KEYBOARD_PORTS_MAXIMUM; i++)
    {
        ...
        LogFlow(("VPoxMouseNT::DriverEntry: Creating device object named %S\n", KbdNameFull.Buffer));
    ...
    *pDevExt = pInit->DevExt;
    ...
    if (pDevExt->HardwarePresent & MOUSE_HARDWARE_PRESENT)
    {
        ...
        for (unsigned i = 0; i < POINTER_PORTS_MAXIMUM; i++)
        {
            ...
            LogFlow(("VPoxMouseNT::DriverEntry: pointer port name (symbolic link) = %S\n", MouNameFull.Buffer));
    ...
    int rcVPox = VbglR0InitClient();
    if (RT_FAILURE(rcVPox))
    {
        Log(("VPoxMouseNT::DriverEntry: could not initialize guest library, rc = %Rrc\n", rcVPox));
        /* Continue working in non-VPox mode. */
    }
    else
    {
        VMMDevReqMouseStatus *pReq = NULL;

        rcVPox = VbglR0GRAlloc((VMMDevRequestHeader**)&pReq, sizeof(VMMDevReqMouseStatus), VMMDevReq_SetMouseStatus);
        if (RT_SUCCESS(rcVPox))
        {
            /* Inform host that we support absolute */
            pReq->mouseFeatures = VMMDEV_MOUSE_GUEST_CAN_ABSOLUTE;
            pReq->pointerXPos = 0;
            pReq->pointerYPos = 0;
            rcVPox = VbglR0GRPerform(&pReq->header);
            if (RT_FAILURE(rcVPox))
                Log(("VPoxMouseNT::DriverEntry: ERROR communicating new mouse capabilities to VMMDev. rc = %Rrc\n", rcVPox));
            else
            {
        ...
        else
        {
            ...
            Log(("VPoxMouseNT::DriverEntry: could not allocate request buffer, rc = %Rrc\n", rcVPox));
            /* Continue working in non-VPox mode. */
        }
    }
...
fail:
    ...
    LogFlow(("VPoxMouseNT::DriverEntry: leave, status = %d\n", status));
    RTR0Term();
    return status;
}
