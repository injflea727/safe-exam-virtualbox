...
#include <VPox/vmm/pdmcritsectrw.h>
...
static VPOXSTRICTRC         vmmR3EmtRendezvousCommon(PVM pVM, PVMCPU pVCpu, bool fIsCaller,
...
#define RTLogCalcSizeForR0(cGroups, fFlags) (RT_UOFFSETOF_DYN(VMMR0LOGGER, Logger.afGroups[cGroups]) + PAGE_SIZE)
     ...
     */
#if defined(LOG_ENABLED) && defined(VPOX_WITH_R0_LOGGING)
...
#endif /* LOG_ENABLED && VPOX_WITH_R0_LOGGING */
...
static DECLCALLBACK(int) vmmR3Load(PVM pVM, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (uVersion <= VMM_SAVED_STATE_VERSION_3_0)
    {
        ...
        SSMR3GetRCPtr(pSSM, &RCPtrIgnored);
#ifdef RT_OS_DARWIN
        if (   SSMR3HandleVersion(pSSM)  >= VPOX_FULL_VERSION_MAKE(3,0,0)
            && SSMR3HandleVersion(pSSM)  <  VPOX_FULL_VERSION_MAKE(3,1,0)
...
static DECLCALLBACK(int) vmmR3SendStarupIpi(PVM pVM, VMCPUID idCpu, uint32_t uVector)
{
    ...
    if (CPUMIsGuestInVmxRootMode(pCtx))
    {
        ...
        if (CPUMIsGuestInVmxNonRootMode(pCtx))
            return VPOXSTRICTRC_TODO(IEMExecVmxVmexitStartupIpi(pVCpu, uVector));
...
static DECLCALLBACK(int) vmmR3SendInitIpi(PVM pVM, VMCPUID idCpu)
{
    ...
    if (CPUMIsGuestInVmxNonRootMode(pCtx))
        return VPOXSTRICTRC_TODO(IEMExecVmxVmexit(pVCpu, VMX_EXIT_INIT_SIGNAL, 0 /* uExitQual */));
#endif
 ...
 */
static VPOXSTRICTRC vmmR3EmtRendezvousCommonRecursion(PVM pVM, PVMCPU pVCpu, VPOXSTRICTRC rcStrict)
{
    int rc2;
     ...
     */
    VPOXSTRICTRC rcStrict2 = vmmR3EmtRendezvousCommon(pVM, pVCpu, false /* fIsCaller */, pVM->vmm.s.fRendezvousFlags,
     ...
     */
    AssertRC(VPOXSTRICTRC_VAL(rcStrict2));
 ...
 *
 * @returns VPox strict informational status code for EM scheduling. No failures
 ...
 *                              returned.
 */
DECL_FORCE_INLINE(VPOXSTRICTRC) vmmR3EmtRendezvousNonCallerReturn(PVM pVM, VPOXSTRICTRC rcStrict)
{
...
{
    int rc;
    VPOXSTRICTRC rcStrictRecursion = VINF_SUCCESS;
    ...
    {
        VPOXSTRICTRC rcStrict2 = pfnRendezvous(pVM, pVCpu, pvUser);
        if (rcStrict2 != VINF_SUCCESS)
        {
            ...
            do
            {
                ...
                if (    rcStrict2 == i32RendezvousStatus
                    ...
                    break;
            } while (!ASMAtomicCmpXchgS32(&pVM->vmm.s.i32RendezvousStatus, VPOXSTRICTRC_VAL(rcStrict2), i32RendezvousStatus));
        }
    }
 ...
 *
 * @returns VPox strict status code - EM scheduling.  No errors will be returned
...
VMMR3_INT_DECL(int) VMMR3EmtRendezvousFF(PVM pVM, PVMCPU pVCpu)
{
    ...
    VPOXSTRICTRC rcStrict = vmmR3EmtRendezvousCommon(pVM, pVCpu, false /* fIsCaller */, pVM->vmm.s.fRendezvousFlags,
                                                     pVM->vmm.s.pfnRendezvous, pVM->vmm.s.pvRendezvousUser);
 ...
 *
 * @returns VPox strict status code.  This will be the first error,
 ...
 * @thread  EMT(pVCpu)
 */
static VPOXSTRICTRC vmmR3EmtRendezvousRecursive(PVM pVM, PVMCPU pVCpu, uint32_t fFlags,
                                                PFNVMMEMTRENDEZVOUS pfnRendezvous, void *pvUser)
{
    ...
    VPOXSTRICTRC rcStrict = vmmR3EmtRendezvousCommon(pVM, pVCpu, true /*fIsCaller*/, fFlags, pfnRendezvous, pvUser);
     ...
     */
    VPOXSTRICTRC rcStrict2 = pVM->vmm.s.i32RendezvousStatus;
    if (    rcStrict2 != VINF_SUCCESS
        &&  (   rcStrict == VINF_SUCCESS
             || rcStrict > rcStrict2))
        rcStrict = rcStrict2;
    ...
    Log(("vmmR3EmtRendezvousRecursive: %#x EMT#%u depth=%d returns %Rrc\n",
         fFlags, pVCpu->idCpu, pVM->vmm.s.cRendezvousRecursions, VPOXSTRICTRC_VAL(rcStrict)));
    return rcStrict;
}
    ...
    else
    {
        ...
        if (RT_UNLIKELY(!ASMAtomicCmpXchgU32(&pVM->vmm.s.u32RendezvousLock, 0x77778888, 0)))
        {
            ...
            if (   pVCpu->vmm.s.fInRendezvous
                ...
                return VPOXSTRICTRC_TODO(vmmR3EmtRendezvousRecursive(pVM, pVCpu, fFlags, pfnRendezvous, pvUser));
         ...
         */
        VPOXSTRICTRC rcStrict2 = vmmR3EmtRendezvousCommon(pVM, pVCpu, true /* fIsCaller */, fFlags, pfnRendezvous, pvUser);
         ...
         */
        VPOXSTRICTRC rcStrict3 = pVM->vmm.s.i32RendezvousStatus;
        ...
        if (    rcStrict3 != VINF_SUCCESS
            &&  (   rcStrict == VINF_SUCCESS
                 || rcStrict > rcStrict3))
            rcStrict = rcStrict3;
        Log(("VMMR3EmtRendezvous: %#x EMT#%u returns %Rrc\n", fFlags, pVCpu->idCpu, VPOXSTRICTRC_VAL(rcStrict)));
    }
