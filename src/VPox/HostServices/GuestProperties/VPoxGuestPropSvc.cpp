...
struct GuestCall
{
    ...
    GuestCall(uint32_t aClientId, VPOXHGCMCALLHANDLE aHandle, uint32_t aFunction,
              uint32_t aParmsCnt, VPOXHGCMSVCPARM aParms[], int aRc)
...
private:
    ...
    {
        if (RTStrStartsWith(pszName, "/VirtualPox/GuestAdd/VPoxService/"))
            return true;
        if (RTStrStartsWith(pszName, "/VirtualPox/GuestAdd/PAM/"))
            return true;
        if (RTStrStartsWith(pszName, "/VirtualPox/GuestAdd/Greeter/"))
            return true;
        if (RTStrStartsWith(pszName, "/VirtualPox/GuestAdd/SharedFolders/"))
            return true;
        if (RTStrStartsWith(pszName, "/VirtualPox/HostInfo/"))
            return true;
        if (RTStrStartsWith(pszName, "/VirtualPox/VMInfo/"))
            return true;
        return false;
    }
...
public:
    explicit Service(PVPOXHGCMSVCHELPERS pHelpers)
        : mpHelpers(pHelpers)
...
private:
    ...
    int setPropertyBlock(uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int getProperty(uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int setProperty(uint32_t cParms, VPOXHGCMSVCPARM paParms[], bool isGuest);
    int setPropertyInternal(const char *pcszName, const char *pcszValue, uint32_t fFlags, uint64_t nsTimestamp,
                            bool fIsGuest = false);
    int delProperty(uint32_t cParms, VPOXHGCMSVCPARM paParms[], bool isGuest);
    int enumProps(uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int getNotification(uint32_t u32ClientId, VPOXHGCMCALLHANDLE callHandle, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int getOldNotificationInternal(const char *pszPattern, uint64_t nsTimestamp, Property *pProp);
    int getNotificationWriteOut(uint32_t cParms, VPOXHGCMSVCPARM paParms[], Property const &prop);
    ...
    void call(VPOXHGCMCALLHANDLE callHandle, uint32_t u32ClientID,
              void *pvClient, uint32_t eFunction, uint32_t cParms,
              VPOXHGCMSVCPARM paParms[]);
    int hostCall(uint32_t eFunction, uint32_t cParms, VPOXHGCMSVCPARM paParms[]);
    int uninit();
 ...
 * @thread  HGCM
 */
int Service::setPropertyBlock(uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
 ...
 * @thread  HGCM
 */
int Service::getProperty(uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    int         rc;
 ...
 * @thread  HGCM
 */
int Service::setProperty(uint32_t cParms, VPOXHGCMSVCPARM paParms[], bool isGuest)
{
    int rc = VINF_SUCCESS;
    const char *pcszName = NULL;        /* shut up gcc */
 ...
 * @thread  HGCM
 */
int Service::delProperty(uint32_t cParms, VPOXHGCMSVCPARM paParms[], bool isGuest)
{
    int         rc;
    const char *pcszName = NULL;        /* shut up gcc */
    uint32_t    cbName;
 ...
 * @thread  HGCM
 */
int Service::enumProps(uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    int rc = VINF_SUCCESS;
...
int Service::getNotificationWriteOut(uint32_t cParms, VPOXHGCMSVCPARM paParms[], Property const &rProp)
{
 ...
 * @throws  nothing
 */
int Service::getNotification(uint32_t u32ClientId, VPOXHGCMCALLHANDLE callHandle,
 ...
 * @thread  HGCM
 */
void Service::call (VPOXHGCMCALLHANDLE callHandle, uint32_t u32ClientID,
                    void * /* pvClient */, uint32_t eFunction, uint32_t cParms,
                    VPOXHGCMSVCPARM paParms[])
{
    int rc;
 ...
 * @thread  hgcm
 */
int Service::hostCall (uint32_t eFunction, uint32_t cParms, VPOXHGCMSVCPARM paParms[])
{
    int rc;
...
/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnDisconnect}
 */
...
/**
 * Sets the VPoxVer, VPoxVerExt and VPoxRev properties.
 */
int Service::setHostVersionProps()
{
    uint64_t nsTimestamp = getCurrentTimestamp();

    /* Set the raw VPox version string as a guest property. Used for host/guest
     * version comparison. */
    int rc = setPropertyInternal("/VirtualPox/HostInfo/VPoxVer", VPOX_VERSION_STRING_RAW,
                                 GUEST_PROP_F_TRANSIENT | GUEST_PROP_F_RDONLYGUEST, nsTimestamp);
    AssertRCReturn(rc, rc);

    /* Set the full VPox version string as a guest property. Can contain vendor-specific
     * information/branding and/or pre-release tags. */
    rc = setPropertyInternal("/VirtualPox/HostInfo/VPoxVerExt", VPOX_VERSION_STRING,
                             GUEST_PROP_F_TRANSIENT | GUEST_PROP_F_RDONLYGUEST, nsTimestamp + 1);
    AssertRCReturn(rc, rc);

    /* Set the VPox SVN revision as a guest property */
    rc = setPropertyInternal("/VirtualPox/HostInfo/VPoxRev", RTBldCfgRevisionStr(),
...
/**
 * @interface_method_impl{VPOXHGCMSVCFNTABLE,pfnNotify}
 */
/*static*/ DECLCALLBACK(void) Service::svcNotify(void *pvService, HGCMNOTIFYEVENT enmEvent)
{
    ...
    if (enmEvent == HGCMNOTIFYEVENT_RESUME)
        pThis->incrementCounterProp("/VirtualPox/VMInfo/ResumeCounter");

    if (enmEvent == HGCMNOTIFYEVENT_RESET)
        pThis->incrementCounterProp("/VirtualPox/VMInfo/ResetCounter");
}
...
int Service::initialize()
{
    ...
    /* Sysprep execution by VPoxService (host is allowed to change these). */
    uint64_t nsNow = getCurrentTimestamp();
    rc = setPropertyInternal("/VirtualPox/HostGuest/SysprepExec", "", GUEST_PROP_F_TRANSIENT | GUEST_PROP_F_RDONLYGUEST, nsNow);
    AssertRCReturn(rc, rc);
    rc = setPropertyInternal("/VirtualPox/HostGuest/SysprepArgs", "", GUEST_PROP_F_TRANSIENT | GUEST_PROP_F_RDONLYGUEST, nsNow);
    AssertRCReturn(rc, rc);

    /* Resume and reset counters. */
    rc = setPropertyInternal("/VirtualPox/VMInfo/ResumeCounter", "0", GUEST_PROP_F_TRANSIENT | GUEST_PROP_F_RDONLYGUEST, nsNow);
    AssertRCReturn(rc, rc);
    rc = setPropertyInternal("/VirtualPox/VMInfo/ResetCounter",  "0", GUEST_PROP_F_TRANSIENT | GUEST_PROP_F_RDONLYGUEST, nsNow);
    AssertRCReturn(rc, rc);
