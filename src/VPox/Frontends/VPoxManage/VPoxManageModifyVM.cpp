/* $Id: VPoxManageModifyVM.cpp $ */
/** @file
 * VPoxManage - Implementation of modifyvm command.
 */
 ...
 *        around VirtualPox - typedefs more than covers that demand! */
...
RTEXITCODE handleModifyVM(HandlerArg *a)
{
    ...
    CHECK_ERROR_RET(a->virtualPox, FindMachine(Bstr(a->argv[0]).raw(),
                                               machine.asOutParam()), RTEXITCODE_FAILURE);


    /* Get the number of network adapters */
    ULONG NetworkAdapterCount = getMaxNics(a->virtualPox, machine);
           ...
           && (c = RTGetOpt(&GetOptState, &ValueUnion)))
    {
        switch (c)
        {
            ...
            case MODIFYVM_GRAPHICSCONTROLLER:
            {
                ...
                else if (   !RTStrICmp(ValueUnion.psz, "vpoxvga")
                         || !RTStrICmp(ValueUnion.psz, "vpox")
                         || !RTStrICmp(ValueUnion.psz, "vga")
                         || !RTStrICmp(ValueUnion.psz, "vesa"))
                    CHECK_ERROR(pGraphicsAdapter, COMSETTER(GraphicsControllerType)(GraphicsControllerType_VPoxVGA));
                ...
                else if (   !RTStrICmp(ValueUnion.psz, "vpoxsvga")
                         || !RTStrICmp(ValueUnion.psz, "svga"))
                    CHECK_ERROR(pGraphicsAdapter, COMSETTER(GraphicsControllerType)(GraphicsControllerType_VPoxSVGA));
#endif
                else
                {
...
#endif /* VPOX_WITH_USB_CARDREADER */
