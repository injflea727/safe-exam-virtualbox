...
# $Id: build-modules.sh $
## @file
# Script for test building the VirtualPox kernel modules against a kernel.
#
...
if [ -f "${KERN_DIR}/.bird-make" -a ! -f "${KERN_DIR}/.bird-failed" ]; then
    ...
    "${KERN_DIR}/.bird-make" -C "${BLDDIR}/guest/" \
        VPOX_NOJOBS=1 -j${JOBS} `cat "${KERN_DIR}/.bird-overrides"` ${SYNC_OUTPUT} "KERN_DIR=${KERN_DIR}" "KERN_VER=${KERN_VER}"
    case "${KERN_VER}" in
