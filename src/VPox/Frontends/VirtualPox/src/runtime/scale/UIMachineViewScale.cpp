...
/** @file
 * VPox Qt GUI - UIMachineViewScale class implementation.
 */
    ...
    if (scaledSize.isValid())
    {
        ...
        if (machine().GetGraphicsAdapter().GetAccelerate3DEnabled() && uiCommon().is3DAvailable())
        {
            ...
            display().NotifyScaleFactorChange(m_uScreenId,
                                              (uint32_t)(xScaleFactor * VPOX_OGL_SCALE_FACTOR_MULTIPLIER),
                                              (uint32_t)(yScaleFactor * VPOX_OGL_SCALE_FACTOR_MULTIPLIER));
        }
    }
