...
<xsl:variable name="G_virtualPoxPackage" select="concat('org.virtualpox', $G_vpoxApiSuffix)" />
<xsl:variable name="G_virtualPoxPackageCom" select="concat('org.virtualpox', $G_vpoxApiSuffix, '.', $G_vpoxGlueStyle)" />
<xsl:variable name="G_virtualPoxWsdl" select="concat('&quot;vpoxwebService', $G_vpoxApiSuffix, '.wsdl&quot;')" />
 ...
 * Copyright (C) 2010-2020 Oracle Corporation
 *
 * This file is part of the VirtualPox SDK, as available from
 * http://www.virtualpox.org.  This library is free software; you can
 * redistribute it and/or modify it under the terms of the GNU Lesser General
 * Public License as published by the Free Software Foundation, in version 2.1
 * as it comes in the "COPYING.LIB" file of the VirtualPox SDK distribution.
 ...
 * Generator: src/VPox/Main/glue/glue-java.xsl
 */
...
<xsl:template name="startFile">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <xsl:value-of select="concat(' \', $G_sNewLine, '&#9;', $G_vpoxDirPrefix, $file)"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template match="link" mode="middle">
  ...
  <xsl:choose>
    ...
    <xsl:when test="contains($linktext, '::')">
      ...
      <xsl:value-of select="concat($G_virtualPoxPackage, '.', $context, '#')"/>
      <xsl:call-template name="emit_refsig">
        <xsl:with-param name="context" select="$context"/>
        <xsl:with-param name="identifier" select="$linkname"/>
      </xsl:call-template>
    ...
    <xsl:otherwise>
      <xsl:value-of select="concat($G_virtualPoxPackage, '.', $linktext)"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
<!--
...
<xsl:template name="genEnum">
  ...
  <xsl:call-template name="startFile">
    ...
    <xsl:with-param name="package" select="$G_virtualPoxPackage" />
  </xsl:call-template>
...
<xsl:template name="startExcWrapper">
  ...
  <xsl:if test="$G_vpoxGlueStyle='jaxws' and $preventObjRelease">
...
<xsl:template name="endExcWrapper">
  ...
  <xsl:choose>
    <xsl:when test="$G_vpoxGlueStyle='xpcom'">
      ...
      <xsl:text>        {&#10;</xsl:text>
      <xsl:text>            throw new VPoxException(e.getMessage(), e);&#10;</xsl:text>
      <xsl:text>        }&#10;</xsl:text>
    </xsl:when>
    ...
    <xsl:when test="$G_vpoxGlueStyle='jaxws'">
      ...
      <xsl:text>        {&#10;</xsl:text>
      <xsl:text>            throw new VPoxException(e.getMessage(), e, this.getObjMgr(), this.port);&#10;</xsl:text>
      <xsl:text>        }&#10;</xsl:text>
...
<xsl:template name="fullClassName">
  ...
  <xsl:choose>
    <xsl:when test="(count(key('G_keyEnumsByName', $name)) > 0) or (count(key('G_keyEnumsByName', $origname)) > 0)">
      <xsl:value-of select="concat($G_virtualPoxPackage, concat('.', $name))" />
    </xsl:when>
  ...
  <xsl:choose>
    ...
    <xsl:when test="($G_vpoxGlueStyle='jaxws')">
      ...
      <xsl:choose>
        ...
        <xsl:when test="key('G_keyInterfacesByName', $type)/@wsmap='struct'">
          <xsl:value-of select="concat($G_virtualPoxPackageCom, '.', $type)" />
        </xsl:when>
...
<xsl:template name="genStructWrapperJaxws">
  ...
  <xsl:value-of select="concat('    private ', $G_virtualPoxPackageCom, '.', $ifname, ' real;&#10;')"/>
  <xsl:text>    private VpoxPortType port;&#10;</xsl:text>
  ...
  <xsl:value-of select="concat('&#10;    public ', $ifname, '(', $G_virtualPoxPackageCom, '.', $ifname, ' real, ObjectRefManager objMgr, VpoxPortType port)&#10;')" />
  <xsl:text>    {&#10;</xsl:text>
...
<xsl:template name="genMethod">
  ...
  <xsl:choose>
    <xsl:when test="(param[@mod='ptr']) or (($G_vpoxGlueStyle='jaxws') and (param[@type=($G_setSuppressedInterfaces/@name)]))" >
      <xsl:value-of select="concat('    // Skipping method ', $methodname, ' for it has parameters with suppressed types&#10;')" />
    </xsl:when>
    <xsl:when test="($G_vpoxGlueStyle='jaxws') and (@wsmap = 'suppress')" >
    ...
    <xsl:otherwise>
      ...
      <xsl:variable name="retValValue">
        <xsl:choose>
          <xsl:when test="(param[@dir='out']) and ($G_vpoxGlueStyle='jaxws')">
...
<xsl:template name="genIfaceWrapper">
  ...
  <xsl:choose>
      <xsl:when test="($G_vpoxGlueStyle='jaxws')">
        <xsl:value-of select="concat('    public ', $ifname, '(String wrapped, ObjectRefManager objMgr, VpoxPortType port)&#10;')" />
        <xsl:text>    {&#10;</xsl:text>
        <xsl:text>        super(wrapped, objMgr, port);&#10;</xsl:text>
        <xsl:text>    }&#10;</xsl:text>
      </xsl:when>

      <xsl:when test="($G_vpoxGlueStyle='xpcom') or ($G_vpoxGlueStyle='mscom')">
  ...
  <xsl:for-each select="attribute[not(@mod='ptr')]">
    ...
    <xsl:choose>
      <xsl:when test="($G_vpoxGlueStyle='jaxws') and ($attrtype=($G_setSuppressedInterfaces/@name))">
...
<xsl:template name="genIface">
  ...
  <xsl:if test="$filelistonly=''">
    ...
    <xsl:choose>
      <xsl:when test="($wsmap='struct') and ($G_vpoxGlueStyle='jaxws')">
...
<xsl:template name="emitHandwrittenXpcom">
  ...
  <xsl:call-template name="startFile">
    ...
    <xsl:with-param name="package" select="$G_virtualPoxPackageCom" />
  </xsl:call-template>
...
public class Helper
{
  ...
  <xsl:call-template name="startFile">
    <xsl:with-param name="file" select="'VPoxException.java'" />
    <xsl:with-param name="package" select="$G_virtualPoxPackage" />
  </xsl:call-template>
...
public class VPoxException extends RuntimeException
{
    private int resultCode;
    private IVirtualPoxErrorInfo errorInfo;

    public VPoxException(String message)
    {
        super(message);
        resultCode = -1;
        errorInfo = null;
    ...
    public VPoxException(String message, Throwable cause)
    {
        super(message, cause);
        if (cause instanceof org.mozilla.xpcom.XPCOMException)
        {
            ...
            try
            {
                ...
                errorInfo = new IVirtualPoxErrorInfo((org.mozilla.interfaces.IVirtualPoxErrorInfo)ex.queryInterface(org.mozilla.interfaces.IVirtualPoxErrorInfo.IVIRTUALPOXERRORINFO_IID));
            }
    ...
    public IVirtualPoxErrorInfo getVirtualPoxErrorInfo()
    {
        return errorInfo;
    }
}
  ...
  <xsl:call-template name="startFile">
    <xsl:with-param name="file" select="'VirtualPoxManager.java'" />
    <xsl:with-param name="package" select="$G_virtualPoxPackage" />
  </xsl:call-template>
...
public class VirtualPoxManager
{
    private Mozilla mozilla;
    private IVirtualPox vpox;
    private nsIComponentManager componentManager;

    private VirtualPoxManager(Mozilla mozilla)
    {
        this.mozilla = mozilla;
        this.componentManager = mozilla.getComponentManager();
        this.vpox = new IVirtualPox((org.mozilla.interfaces.IVirtualPox) this.componentManager
                    .createInstanceByContractID("@virtualpox.org/VirtualPox;1",
                                                null,
                                                org.mozilla.interfaces.IVirtualPox.IVIRTUALPOX_IID));
    }

    public void connect(String url, String username, String passwd)
    {
        throw new VPoxException("Connect doesn't make sense for local bindings");
    }

    public void disconnect()
    {
        throw new VPoxException("Disconnect doesn't make sense for local bindings");
    }
    ...
    {
        return "]]></xsl:text>
    <xsl:value-of select="substring($G_vpoxApiSuffix, 2)" />
    <xsl:text><![CDATA[";
    }

    public IVirtualPox getVPox()
    {
        return this.vpox;
    }
    ...
    public ISession getSessionObject()
    {
        return new ISession((org.mozilla.interfaces.ISession) componentManager
                .createInstanceByContractID("@virtualpox.org/Session;1", null,
    ...
    public static synchronized VirtualPoxManager createInstance(String home)
    {
        if (hasInstance)
            throw new VPoxException("only one instance of VirtualPoxManager at a time allowed");
        if (home == null || home.equals(""))
            home = System.getProperty("vpox.home");

        if (home == null)
            throw new VPoxException("vpox.home Java property must be defined to use XPCOM bridge");
        ...
        return new VirtualPoxManager(mozilla);
    }
...
public class EventListenerImpl extends nsISupportsBase implements org.mozilla.interfaces.IEventListener
{
  ...
  <xsl:call-template name="startFile">
    <xsl:with-param name="file" select="'VPoxObjectBase.java'" />
    <xsl:with-param name="package" select="$G_virtualPoxPackage" />
  </xsl:call-template>
...
public class VPoxException extends RuntimeException
{
    ...
    public VPoxException(String message, Throwable cause)
    {
        ...
        if (cause instanceof com.jacob.com.ComException)
        {
            ...
            // and that means no way of getting to IVirtualPoxErrorInfo.
            errorInfo = null;
        }
        else
            resultCode = -1;
...
public class VirtualPoxManager
{
    ...
    private VirtualPoxManager()
    {
        initPerThread();
        vpox = new IVirtualPox(new ActiveXComponent("VirtualPox.VirtualPox"));
    }
    ...
    public ISession getSessionObject()
    {
        return new ISession(new ActiveXComponent("VirtualPox.Session"));
    }
...
public class IUnknown
{
    ...
    protected final VpoxPortType port;

    public IUnknown(String obj, ObjectRefManager objMgr, VpoxPortType port)
    {
        this.obj    = obj;
        this.objMgr = objMgr;
        this.port   = port;
    ...
    public final VpoxPortType getRemoteWSPort()
    {
        return this.port;
    }
...
public class Helper
{
    public static <T> List<T> wrap(Class<T> wrapperClass, ObjectRefManager objMgr, VpoxPortType pt, List<String> values)
    {
        try
        {
            if (values == null)
            ...
            Constructor<T> c = wrapperClass.getConstructor(String.class, ObjectRefManager.class, VpoxPortType.class);
    ...
    public static <T1, T2> List<T1> wrap2(Class<T1> wrapperClass1, Class<T2> wrapperClass2, ObjectRefManager objMgr, VpoxPortType pt, List<T2> values)
    {
        try
        {
            if (values == null)
            ...
            Constructor<T1> c = wrapperClass1.getConstructor(wrapperClass2, ObjectRefManager.class, VpoxPortType.class);
    ...
    public static byte[] decodeBase64(String str)
    {
        ...
        if ((validChars * 3 % 4) != 0)
            throw new VPoxException("invalid base64 encoded string " + str);
...
public class VPoxException extends RuntimeException
{
    ...
    public VPoxException(String message, Throwable cause, ObjectRefManager objMgr, VpoxPortType port)
    {
        super(message, cause);
        if (cause instanceof RuntimeFaultMsg)
        {
            ...
            errorInfo = (retVal.length() > 0) ? new IVirtualPoxErrorInfo(retVal, objMgr, port) : null;
        }
        else
            resultCode = -1;
    }
...
class PortPool
{
    ...
    <xsl:value-of select="$G_virtualPoxWsdl" />
    <xsl:text><![CDATA[;
    private Map<VpoxPortType, Integer> known;
    private boolean initStarted;
    private VpoxService svc;

    PortPool(boolean usePreinit)
    {
        known = new HashMap<VpoxPortType, Integer>();
    ...
    {
        VpoxPortType port = getPort();
        releasePort(port);
    }

    synchronized VpoxPortType getPort()
    {
        VpoxPortType port = null;
        int ttl = 0;

        for (VpoxPortType cur: known.keySet())
        {
        ...
        if (port == null)
        {
            if (svc == null)
            {
                ...
                svc = new VpoxService(wsdl,
                                      new QName("http://www.virtualpox.org/Service",
                                                "vpoxService"));
            }
            port = svc.getVpoxServicePort();
    ...
    synchronized void releasePort(VpoxPortType port)
    {
...
class ObjectRefManager
{
    ...
    static class ManagedObj
    {
        ...
        private final VpoxPortType                         port;
        private final ConcurrentLinkedQueue<ManagedObjRef> refQ;

        ManagedObj(String objId,  VpoxPortType port)
        {
          this.objId = objId;
          this.port  = port;
    ...
    static class ObjRefMgrCleanupThread extends Thread
    {
        ...
        private void init(ObjectRefManager objRefMgr, int cStubsReleaseThreshold)
        {
            ...
            setName("ObjectRefManager-VPoxWSObjRefGcThrd");
            /*
...
class VPoxTLSSocketFactory extends SSLSocketFactory
{
    ...
    {
        return new VPoxTLSSocketFactory();
    }
...
public class VirtualPoxManager
{
    ...
    public void connect(String url, String username, String passwd)
    {
        ...
        try
        {
            ...
            VPoxTLSSocketFactory sf = new VPoxTLSSocketFactory();
            ((BindingProvider)port).getRequestContext().
            ...
            try
            {
                this.vpox = new IVirtualPox(handle, this.objMgr, port);
            }
            finally
            {
        ...
        catch (Throwable t)
        {
            ...
            throw new VPoxException(t.getMessage(), t, this.objMgr, this.port);
        }
    }
    ...
    public void disconnect()
    {
        ...
        try
        {
            if (this.vpox != null && port != null)
                port.iWebsessionManagerLogoff(this.vpox.getWrapped());
        }
        catch (InvalidObjectFaultMsg e)
        {
            throw new VPoxException(e.getMessage(), e, this.objMgr, this.port);
        }
        catch (RuntimeFaultMsg e)
        {
            throw new VPoxException(e.getMessage(), e, this.objMgr, this.port);
    ...
    {
        if (this.vpox == null)
            throw new VPoxException("connect first");
        try
        {
            String handle = port.iWebsessionManagerGetSessionObject(this.vpox.getWrapped());
            this.objMgr.preventObjRelease();
            try
            {
    ...
    public IEventListener createListener(Object sink)
    {
        throw new VPoxException("no active listeners here");
    }
...
<xsl:template match="/">
  ...
  <xsl:if test="not($G_vpoxApiSuffix)">
    <xsl:call-template name="fatalError">
      <xsl:with-param name="msg" select="'G_vpoxApiSuffix must be given'" />
    </xsl:call-template>
  </xsl:if>
