/* $Id: VirtualPoxBase.h $ */
/** @file
 * VirtualPox COM base classes definition
 */
...
#ifndef MAIN_INCLUDED_VirtualPoxBase_h
#define MAIN_INCLUDED_VirtualPoxBase_h
...
// avoid including VPox/settings.h and VPox/xml.h; only declare the classes
namespace xml
{
class File;
}
...
/**
 *  Special version of the Assert macro to be used within VirtualPoxBase
 *  subclasses.
 *
...
/**
 *  Special version of the AssertFailed macro to be used within VirtualPoxBase
 *  subclasses.
 *
...
/**
 *  Special version of the AssertMsg macro to be used within VirtualPoxBase
 *  subclasses.
 *
...
/**
 *  Special version of the AssertMsgFailed macro to be used within VirtualPoxBase
 *  subclasses.
 *
 *  See ComAssert for more info.
 *
...
/**
 *  Special version of the AssertRC macro to be used within VirtualPoxBase
 *  subclasses.
 *
 *  See ComAssert for more info.
 *
 * @param   vrc     VPox status code.
 */
#define ComAssertRC(vrc)            ComAssertMsgRC(vrc, ("%Rra", vrc))

/**
 *  Special version of the AssertMsgRC macro to be used within VirtualPoxBase
 *  subclasses.
 *
 *  See ComAssert for more info.
 *
...
/**
 *  Special version of the AssertComRC macro to be used within VirtualPoxBase
 *  subclasses.
 *
 *  See ComAssert for more info.
 *
...
/**
 * Parent class of VirtualPoxBase which enables translation support (which
 * Main doesn't have yet, but this provides the tr() function which will one
 * day provide translations).
 *
 * This class sits in between Lockable and VirtualPoxBase only for the one
 * reason that the USBProxyService wants translation support but is not
 * implemented as a COM object, which VirtualPoxBase implies.
 */
class ATL_NO_VTABLE VirtualPoxTranslatable
    : public Lockable
{
public:
     ...
     * translation"). See VirtualPoxBase::translate() for more info.
     *
    ...
    {
        return VirtualPoxTranslatable::translate(NULL, // getComponentName(), eventually
                                                 pcszSourceText,
                                                 aComment);
    }
};
...
#define VIRTUALPOXBASE_ADD_VIRTUAL_COMPONENT_METHODS(cls, iface) \
...
/**
 * VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT:
 * This macro must be used once in the declaration of any class derived
 * from VirtualPoxBase. It implements the pure virtual getClassIID() and
 * getComponentName() methods. If this macro is not present, instances
 * of a class derived from VirtualPoxBase cannot be instantiated.
 *
 ...
 */
  ...
  #define VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT(cls, iface) \
    VIRTUALPOXBASE_ADD_VIRTUAL_COMPONENT_METHODS(cls, iface)
...
/**
 * VPOX_TWEAK_INTERFACE_ENTRY:
 * Macro for defining magic interface entries needed for all interfaces
 * implemented by any subclass of VirtualPoxBase.
 */
#ifdef VPOX_WITH_XPCOM
#define VPOX_TWEAK_INTERFACE_ENTRY(iface)
#else // !VPOX_WITH_XPCOM
#define VPOX_TWEAK_INTERFACE_ENTRY(iface)                                   \
...
public:
     ...
     *
     * This *must* be implemented by every subclass deriving from VirtualPoxBase;
     * use the VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT macro to do that most easily.
     */
     ...
     */
    virtual VPoxLockingClass getLockingClass() const
    {
    ...
    static HRESULT handleUnexpectedExceptions(VirtualPoxBase *const aThis, RT_SRC_POS_DECL);
...
private:
    DECLARE_CLS_COPY_CTOR_ASSIGN_NOOP(VirtualPoxBase); /* Shuts up MSC warning C4625. */
};
...
/* global variables (defined in VirtualPoxBase.cpp) */
 ...
 * VirtualPoxTranslatable, to make lupdate happy.
 */
#define Q_OBJECT
...
#endif /* !MAIN_INCLUDED_VirtualPoxBase_h */
