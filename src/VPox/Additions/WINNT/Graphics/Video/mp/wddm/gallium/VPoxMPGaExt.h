...
#ifndef GA_INCLUDED_SRC_WINNT_Graphics_Video_mp_wddm_gallium_VPoxMPGaExt_h
#define GA_INCLUDED_SRC_WINNT_Graphics_Video_mp_wddm_gallium_VPoxMPGaExt_h
...
#define VPOXWDDM_GA_MAX_FENCE_OBJECTS 4096

/* Gallium related device extension. */
typedef struct VPOXWDDM_EXT_GA
{
    union
    {
    ...
    struct
    {
        ...
        uint32_t au32HandleBits[(VPOXWDDM_GA_MAX_FENCE_OBJECTS + 31) / 32];
    } fenceObjects;
} VPOXWDDM_EXT_GA;
...
typedef struct GAFENCEOBJECT
{
    ...
    PVPOXWDDM_DEVICE pDevice;   /* Device the fence is associated with. */
...
DECLINLINE(void) gaFenceObjectsLock(VPOXWDDM_EXT_GA *pGaDevExt)
{
    KeAcquireSpinLock(&pGaDevExt->fenceObjects.SpinLock, &pGaDevExt->fenceObjects.OldIrql);
}

DECLINLINE(void) gaFenceObjectsUnlock(VPOXWDDM_EXT_GA *pGaDevExt)
{
...
GAFENCEOBJECT *GaFenceLookup(VPOXWDDM_EXT_GA *pGaDevExt,
                             uint32_t u32FenceHandle);
void GaFenceUnrefLocked(VPOXWDDM_EXT_GA *pGaDevExt,
                        GAFENCEOBJECT *pFO);

#endif /* !GA_INCLUDED_SRC_WINNT_Graphics_Video_mp_wddm_gallium_VPoxMPGaExt_h */
