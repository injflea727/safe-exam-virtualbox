/* $Id: VPoxClipboard.cpp $ */
/** @file
 * VPoxClipboard - Shared clipboard, Windows Guest Implementation.
 */
...
#include <VPox/HostServices/VPoxClipboardSvc.h> /* Temp, remove. */
...
struct SHCLCONTEXT
{
    /** Pointer to the VPoxClient service environment. */
...
*********************************************************************************************************************************/
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
static DECLCALLBACK(int)  vpoxClipboardOnTransferInitCallback(PSHCLTRANSFERCALLBACKDATA pData);
static DECLCALLBACK(int)  vpoxClipboardOnTransferStartCallback(PSHCLTRANSFERCALLBACKDATA pData);
static DECLCALLBACK(void) vpoxClipboardOnTransferCompleteCallback(PSHCLTRANSFERCALLBACKDATA pData, int rc);
static DECLCALLBACK(void) vpoxClipboardOnTransferErrorCallback(PSHCLTRANSFERCALLBACKDATA pData, int rc);
#endif
 ...
 */
static void vpoxClipboardTransferCallbackCleanup(PSHCLTRANSFERCALLBACKDATA pData)
{
    LogFlowFuncEnter();
...
static DECLCALLBACK(void) vpoxClipboardOnTransferCompleteCallback(PSHCLTRANSFERCALLBACKDATA pData, int rc)
{
    ...
    vpoxClipboardTransferCallbackCleanup(pData);
}
...
static LRESULT vpoxClipboardWinProcessMsg(PSHCLCONTEXT pCtx, HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    AssertPtr(pCtx);
    ...
    switch (msg)
    {
        ...
        case WM_RENDERFORMAT:
        {
            ...
            const SHCLFORMAT fFormat = SharedClipboardWinClipboardFormatToVPox(cfFormat);

            LogFunc(("WM_RENDERFORMAT: cfFormat=%u -> fFormat=0x%x\n", cfFormat, fFormat));

            if (fFormat == VPOX_SHCL_FMT_NONE)
            {
                    ...
                    if (pMem)
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            if (hMem)
                            {
                                 ...
                                 */
                                if (fFormat == VPOX_SHCL_FMT_UNICODETEXT)
                                {
                                    size_t cbActual = 0;
                                ...
                                else if (fFormat == VPOX_SHCL_FMT_HTML)
                                {
        ...
        case SHCL_WIN_WM_REPORT_FORMATS:
        {
            ...
            if (fFormats != VPOX_SHCL_FMT_NONE) /* Could arrive with some older GA versions. */
            {
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
                if (fFormats & VPOX_SHCL_FMT_URI_LIST)
                {
                    LogFunc(("VPOX_SHCL_FMT_URI_LIST\n"));

                    /*
                     * Creating and starting the actual transfer will be done in vbglR3ClipboardTransferStart() as soon
                     * as the host announces the start of the transfer via a VPOX_SHCL_HOST_MSG_TRANSFER_STATUS message.
        ...
        case SHCL_WIN_WM_READ_DATA:
        {
            ...
            if (RT_SUCCESS(rc))
            {
                if (fFormat & VPOX_SHCL_FMT_BITMAP)
                {
                ...
                else if (fFormat & VPOX_SHCL_FMT_UNICODETEXT)
                {
            ...
            if (hClip == NULL)
                VbglR3ClipboardWriteDataEx(&pEvent->cmdCtx, VPOX_SHCL_FMT_NONE, NULL, 0);
            break;
        }
        ...
        case WM_DESTROY:
        {
             ...
             * the VPoxTray already finished a message loop.
             */
...
static LRESULT CALLBACK vpoxClipboardWinWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);

static int vpoxClipboardCreateWindow(PSHCLCONTEXT pCtx)
{
    ...
    {
        wc.style         = CS_NOCLOSE;
        wc.lpfnWndProc   = vpoxClipboardWinWndProc;
...
static int vpoxClipboardWindowThread(RTTHREAD hThread, void *pvUser)
{
    ...
    int rc = vpoxClipboardCreateWindow(pCtx);
    if (RT_FAILURE(rc))
    {
...
static void vpoxClipboardDestroy(PSHCLCONTEXT pCtx)
{
...
static LRESULT CALLBACK vpoxClipboardWinWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    PSHCLCONTEXT pCtx = &g_Ctx; /** @todo r=andy Make pCtx available through SetWindowLongPtr() / GWL_USERDATA. */
    AssertPtr(pCtx);

    /* Forward with proper context. */
    return vpoxClipboardWinProcessMsg(pCtx, hWnd, uMsg, wParam, lParam);
}

DECLCALLBACK(int) VPoxShClInit(const PVPOXSERVICEENV pEnv, void **ppInstance)
{
    LogFlowFuncEnter();
    ...
    pCtx->CmdCtx.Transfers.Callbacks.pfnTransferInitialize = vpoxClipboardOnTransferInitCallback;
    pCtx->CmdCtx.Transfers.Callbacks.pfnTransferStart      = vpoxClipboardOnTransferStartCallback;
    pCtx->CmdCtx.Transfers.Callbacks.pfnTransferComplete   = vpoxClipboardOnTransferCompleteCallback;
    pCtx->CmdCtx.Transfers.Callbacks.pfnTransferError      = vpoxClipboardOnTransferErrorCallback;
#endif
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
            rc = VbglR3ClipboardConnectEx(&pCtx->CmdCtx, VPOX_SHCL_GF_0_CONTEXT_ID);
        if (RT_SUCCESS(rc))
        {
#ifdef VPOX_WITH_SHARED_CLIPBOARD_TRANSFERS
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                rc = RTThreadCreate(&pCtx->hThread, vpoxClipboardWindowThread, pCtx /* pvUser */,
...
DECLCALLBACK(int) VPoxShClWorker(void *pInstance, bool volatile *pfShutdown)
{
    AssertPtr(pInstance);
...
DECLCALLBACK(int) VPoxShClStop(void *pInstance)
{
     ...
     * This will also send a VPOX_SHCL_HOST_MSG_QUIT from the host so that we can break out from our message worker. */
...
DECLCALLBACK(void) VPoxShClDestroy(void *pInstance)
{
 ...
 */
VPOXSERVICEDESC g_SvcDescClipboard =
{
    /* pszName. */
    "clipboard",
    /* pszDescription. */
