/* $Id: vpoximgMedia.cpp $ */
/** @file
 * vpoximgMedia.cpp - Disk Image Flattening FUSE Program.
 */
...
extern VPOXIMGOPTS g_vpoximgOpts;
...
{
    char *pszSzScaled = vpoximgScaledSize(pInfo->cbSize);
    int cPad = nestLevel * 2;
    if (g_vpoximgOpts.fWide && !g_vpoximgOpts.fVerbose)
    {
...
static int vpoximgListBranch(IMachine *pMachine, IMedium *pMedium, uint8_t nestLevel, bool fLast)
{
    int rc;
    ...
    for (size_t i = 0; i < pChildren.size(); i++)
        vpoximgListBranch(pMachine, pChildren[i], nestLevel + 1, fLast);
    delete pMediumInfo;
    return VINF_SUCCESS;
}

static int
listMedia(IVirtualPox *pVirtualPox, IMachine *pMachine, char *vmName, char *vmUuid)
{
    ...
    for (size_t i = 0; i < pMediumAttachments.size(); i++)
    {
        ...
        if (g_vpoximgOpts.fWide && !g_vpoximgOpts.fVerbose)
            ...
            RTPrintf("\n");
        rc = vpoximgListBranch(pMachine, pBase, 0, fLast);
        if (FAILED(rc))
        {
            RTPrintf("vpoximgListBranch failed %d\n", rc);
            return rc;
        }
 ...
 */
int
vpoximgListVMs(IVirtualPox *pVirtualPox)
{
    HRESULT rc = 0;
    com::SafeIfaceArray<IMachine> pMachines;
    CHECK_ERROR(pVirtualPox, COMGETTER(Machines)(ComSafeArrayAsOutParam(pMachines)));
    if (g_vpoximgOpts.fWide)
    {
        RTPrintf("\n");
    ...
    for (size_t i = 0; i < pMachines.size(); ++i)
    {
        ...
        if (pMachine)
        {
            ...
            if (fAccessible)
            {
                ...
                if (   g_vpoximgOpts.pszVm == NULL
                    || RTStrNCmp(CSTR(machineUuid), g_vpoximgOpts.pszVm, MAX_UUID_LEN) == 0
                    || RTStrNCmp((const char *)machineName.raw(), g_vpoximgOpts.pszVm, MAX_UUID_LEN) == 0)
                {
                    if (g_vpoximgOpts.fVerbose)
                    {
                        RTPrintf("-----------------------------------------------------------------\n");
                    ...
                    rc = listMedia(pVirtualPox, pMachine,
