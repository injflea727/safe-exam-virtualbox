 ...
 *
 * The VirtualPox VMCore Format:
 * [ ELF 64 Header]  -- Only 1
 *
 * [ PT_NOTE ]       -- Only 1
 *    - Offset into CoreDescriptor followed by list of Notes (Note Hdr + data) of VPox CPUs.
 ...
 *    - VPox 1 Note Hdr + Data.
 *    - VPox 2 Note Hdr + Data.
 *    ...
 * Memory dump
 *
 */
...
#include <VPox/vmm/dbgfcorefmt.h>
...
static const char *g_pcszCoreVPoxCore = "VBCORE";
static const char *g_pcszCoreVPoxCpu  = "VBCPU";
...
static int dbgfR3CoreWriteWorker(PVM pVM, RTFILE hFile)
{
    ...
    CoreDescriptor.u32VPoxVersion     = VPOX_FULL_VERSION;
    CoreDescriptor.u32VPoxRevision    = VMMGetSvnRev();
    CoreDescriptor.cCpus              = pVM->cCpus;

    Log((DBGFLOG_NAME ": CoreDescriptor Version=%u Revision=%u\n", CoreDescriptor.u32VPoxVersion, CoreDescriptor.u32VPoxRevision));
    ...
    uint64_t const cbCoreDescriptor   = Elf64NoteSectionSize(g_pcszCoreVPoxCore, sizeof(CoreDescriptor));
    uint64_t const offCpuDumps        = offCoreDescriptor + cbCoreDescriptor;
    uint64_t const cbCpuDumps         = pVM->cCpus * Elf64NoteSectionSize(g_pcszCoreVPoxCpu, sizeof(DBGFCORECPU));
    ...
    rc = Elf64WriteNoteHdr(hFile, NT_VPOXCORE, g_pcszCoreVPoxCore, &CoreDescriptor, sizeof(CoreDescriptor));
    if (RT_FAILURE(rc))
    {
        LogRel((DBGFLOG_NAME ": Elf64WriteNoteHdr failed for Note '%s' rc=%Rrc\n", g_pcszCoreVPoxCore, rc));
        return rc;
    }
    ...
    for (VMCPUID idCpu = 0; idCpu < pVM->cCpus; idCpu++)
    {
        ...
        rc = Elf64WriteNoteHdr(hFile, NT_VPOXCPU, g_pcszCoreVPoxCpu, pDbgfCoreCpu, sizeof(*pDbgfCoreCpu));
        if (RT_FAILURE(rc))
        {
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) dbgfR3CoreWriteRendezvous(PVM pVM, PVMCPU pVCpu, void *pvData)
{
    /*
     * Validate input.
     */
