...
#ifdef VPOX
  //
  // VPOX already has tables prepared in memory - just reuse them.
  //
  FillSysTablesInfo(VPoxTables, sizeof(VPoxTables)/sizeof(VPoxTables[0]));
#else
  //
  //
...
#ifdef VPOX
    CurrentTable = (EFI_ACPI_COMMON_HEADER *)VPoxTables[Instance];
...
#ifndef VPOX /* In case we're reading ACPI tables from memory we haven't
