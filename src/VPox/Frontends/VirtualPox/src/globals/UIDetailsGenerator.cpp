...
/** @file
 * VPox Qt GUI - UIDetailsGenerator implementation.
 */
                                                                 ...
                                                                 const UIExtraDataMetaDefs::DetailsElementOptionTypeSystem &fOptions)
{
    ...
    if (fOptions & UIExtraDataMetaDefs::DetailsElementOptionTypeSystem_Acceleration)
    {
        ...
        if (uiCommon().virtualPox().GetHost().GetProcessorFeature(KProcessorFeature_HWVirtEx))
        {
            /* VT-x/AMD-V: */
                                                                  ...
                                                                  const UIExtraDataMetaDefs::DetailsElementOptionTypeNetwork &fOptions)
{
    ...
    const ulong uCount = uiCommon().virtualPox().GetSystemProperties().GetMaxNetworkAdapters(comMachine.GetChipsetType());
                                                                 ...
                                                                 const UIExtraDataMetaDefs::DetailsElementOptionTypeSerial &fOptions)
{
    ...
    const ulong uCount = uiCommon().virtualPox().GetSystemProperties().GetSerialPortCount();
    for (ulong uSlot = 0; uSlot < uCount; ++uSlot)
    {
