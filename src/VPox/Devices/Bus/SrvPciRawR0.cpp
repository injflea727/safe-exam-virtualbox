...
static DECLCALLBACK(bool) pcirawr0Isr(void* pContext, int32_t iHostIrq)
{
    ...
    Loc.cb = 2;
    rc = pThis->pPort->pfnPciCfgRead(pThis->pPort, VPOX_PCI_STATUS, &Loc);
...
PCIRAWR0DECL(int)  PciRawR0InitVM(PGVM pGVM)
{
    ...
    int rc = SUPR0ComponentQueryFactory(pGVM->pSession, "VPoxRawPci", RAWPCIFACTORY_UUID_STR, (void **)&pFactory);
    if (RT_SUCCESS(rc))
    {
        if (pFactory)
        {
...
static DECLCALLBACK(int) dummyPciDevInit(PRAWPCIDEVPORT pPort, uint32_t fFlags)
{
    ...
    dummyPciSetWord(pThis, VPOX_PCI_VENDOR_ID, 0xccdd);
    dummyPciSetWord(pThis, VPOX_PCI_DEVICE_ID, 0xeeff);
    dummyPciSetWord(pThis, VPOX_PCI_COMMAND,   PCI_COMMAND_IOACCESS | PCI_COMMAND_MEMACCESS | PCI_COMMAND_BUSMASTER);
    dummyPciSetByte(pThis, VPOX_PCI_INTERRUPT_PIN, 1);
                              ...
                              uint32_t        *pfDevFlags)
{
    ...
    PRAWPCIFACTORY pFactory = NULL;
    rc = SUPR0ComponentQueryFactory(pSession, "VPoxRawPci", RAWPCIFACTORY_UUID_STR, (void **)&pFactory);
