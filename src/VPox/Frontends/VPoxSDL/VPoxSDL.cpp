...
#if defined(VPOXSDL_WITH_X11)
# include <VPox/VPoxKeyboard.h>

# include <X11/Xlib.h>
# include <X11/cursorfont.h>      /* for XC_left_ptr */
# if !defined(VPOX_WITHOUT_XCURSOR)
...
static void    SendMouseEvent(VPoxSDLFB *fb, int dz, int button, int down);
...
static VPoxSDLFB *getFbFromWinId(Uint32 id);
#endif
...
static ComObjPtr<VPoxSDLFB> gpFramebuffer[64];
...
#if defined(VPOXSDL_WITH_X11) && !defined(VPOX_WITH_SDL2)
...
/**
 * Event handler for VirtualPoxClient events
 */
class VPoxSDLClientEventListener
{
public:
    VPoxSDLClientEventListener()
    {
    }

    virtual ~VPoxSDLClientEventListener()
    {
    }
...
/**
 * Event handler for VirtualPox (server) events
 */
class VPoxSDLEventListener
{
public:
    VPoxSDLEventListener()
    {
    }

    virtual ~VPoxSDLEventListener()
    {
    }
            ...
            case VPoxEventType_OnExtraDataChanged:
            {
                ...
                if (gpMachine)
                {
                    ...
                    if (bstrOurId == bstrMachineId)
                    {
                        ...
                        if (bstrKey == VPOXSDL_SECURELABEL_EXTRADATA)
                        {
                            /*
 ...
 */
class VPoxSDLConsoleEventListener
{
public:
    VPoxSDLConsoleEventListener() : m_fIgnorePowerOffEvents(false)
    {
    }

    virtual ~VPoxSDLConsoleEventListener()
    {
    }
...
# ifdef VPOX_WITH_SDL2
                ...
                VPoxSDLFB *fb = getFbFromWinId(winId);
...
typedef ListenerImpl<VPoxSDLClientEventListener>  VPoxSDLClientEventListenerImpl;
typedef ListenerImpl<VPoxSDLEventListener>        VPoxSDLEventListenerImpl;
typedef ListenerImpl<VPoxSDLConsoleEventListener> VPoxSDLConsoleEventListenerImpl;
...
#endif
#ifdef VPOXSDL_ADVANCED_OPTIONS
...
#if defined(DEBUG) || defined(VPOX_WITH_STATISTICS)
             "\n"
...
static void PrintError(const char *pszName, CBSTR pwszDescr, CBSTR pwszComponent=NULL)
{
    ...
    if (   (pszFile = strstr(pszBuffer, "At '"))
        ...
        && (pszStat = strstr(pszBuffer, "VPox status code: ")))
        RTPrintf("  %.*s  %.*s\n  In%.*s  %s",
...
DECLEXPORT(int) TrustedMain(int argc, char **argv, char **envp)
{
...
#ifndef VPOX_WITH_SDL2 /** @todo Is this correct? */
    ...
    uint32_t vramSize = 0;
    ComPtr<IEventListener> pVPoxClientListener;
    ComPtr<IEventListener> pVPoxListener;
    ComObjPtr<VPoxSDLConsoleEventListenerImpl> pConsoleListener;
    ...
    RTPrintf(VPOX_PRODUCT " SDL GUI version %s\n"
             "(C) 2005-" VPOX_C_YEAR " " VPOX_VENDOR "\n"
             "All rights reserved.\n\n",
             VPOX_VERSION_STRING);
...
#endif /* VPOXSDL_ADVANCED_OPTIONS */
#ifdef VPOX_WIN32_UI
    ...
    if (FAILED(rc))
    {
        ...
        if (info.isFullAvailable())
            PrintError("Failed to create VirtualPoxClient object",
                       info.getText().raw(), info.getComponent().raw());
        else
            RTPrintf("Failed to create VirtualPoxClient object! No error information available (rc=%Rhrc).\n", rc);
        goto leave;
    }

    rc = pVirtualPoxClient->COMGETTER(VirtualPox)(pVirtualPox.asOutParam());
    if (FAILED(rc))
    {
        RTPrintf("Failed to get VirtualPox object (rc=%Rhrc)!\n", rc);
        goto leave;
    }
    rc = pVirtualPoxClient->COMGETTER(Session)(pSession.asOutParam());
    if (FAILED(rc))
    {
    ...
    if (pcszSettingsPw)
    {
        CHECK_ERROR(pVirtualPox, SetSettingsSecret(Bstr(pcszSettingsPw).raw()));
        if (FAILED(rc))
            goto leave;
    }
    else if (pcszSettingsPwFile)
    {
        int rcExit = settingsPasswordFile(pVirtualPox, pcszSettingsPwFile);
    ...
    if (uuidVM.isValid())
    {
        rc = pVirtualPox->FindMachine(uuidVM.toUtf16().raw(), pMachine.asOutParam());
    ...
    else if (vmName)
    {
         ...
         */
        rc = pVirtualPox->FindMachine(Bstr(vmName).raw(), pMachine.asOutParam());
        if ((rc == S_OK) && pMachine)
        {
            Bstr bstrId;
    ...
    rc = pVirtualPoxClient->CheckMachineError(pMachine);
    if (FAILED(rc))
    {
        com::ErrorInfo info;
        if (info.isFullAvailable())
    ...
    if (FAILED(rc))
    {
        com::ErrorInfo info;
        if (info.isFullAvailable())
            PrintError("Could not open VirtualPox session",
                       info.getText().raw(), info.getComponent().raw());
        goto leave;
    }
    if (!pSession)
    {
        RTPrintf("Could not open VirtualPox session!\n");
        goto leave;
    }
    sessionOpened = true;
    ...
    if (pcszHdaFile)
    {
        ...
        pVirtualPox->OpenMedium(bstrHdaFile.raw(), DeviceType_HardDisk,
    ...
    do
    {
        ...
        else
        {
            ...
            ComPtr<IHost> pHost;
            CHECK_ERROR_BREAK(pVirtualPox, COMGETTER(Host)(pHost.asOutParam()));
            rc = pHost->FindHostFloppyDrive(bstrFdaFile.raw(),
                                            pMedium.asOutParam());
            if (FAILED(rc))
            {
                /* try to find an existing one */
                rc = pVirtualPox->OpenMedium(bstrFdaFile.raw(),
                                             DeviceType_Floppy,
    ...
    do
    {
        ...
        else
        {
            ...
            {
                /* try to find an existing one */
                rc = pVirtualPox->OpenMedium(bstrCdromFile.raw(),
                                            DeviceType_DVD,
                                            AccessMode_ReadWrite,
                                            FALSE /* fForceNewUuid */,
                                            pMedium.asOutParam());
    ...
    if (!VPoxSDLFB::init(fShowSDLConfig))
        goto leave;
    ...
    if (fSecureLabel)
    {
        ...
        Bstr bstrLabel;
        gpMachine->GetExtraData(Bstr(VPOXSDL_SECURELABEL_EXTRADATA).raw(), bstrLabel.asOutParam());
    ...
    {
        // register listener for VirtualPoxClient events
        ComPtr<IEventSource> pES;
        CHECK_ERROR(pVirtualPoxClient, COMGETTER(EventSource)(pES.asOutParam()));
        ComObjPtr<VPoxSDLClientEventListenerImpl> listener;
        listener.createObject();
        listener->init(new VPoxSDLClientEventListener());
        pVPoxClientListener = listener;
        com::SafeArray<VPoxEventType_T> eventTypes;
        eventTypes.push_back(VPoxEventType_OnVPoxSVCAvailabilityChanged);
        CHECK_ERROR(pES, RegisterListener(pVPoxClientListener, ComSafeArrayAsInParam(eventTypes), true));
    }

    {
        // register listener for VirtualPox (server) events
        ComPtr<IEventSource> pES;
        CHECK_ERROR(pVirtualPox, COMGETTER(EventSource)(pES.asOutParam()));
        ComObjPtr<VPoxSDLEventListenerImpl> listener;
        listener.createObject();
        listener->init(new VPoxSDLEventListener());
        pVPoxListener = listener;
        com::SafeArray<VPoxEventType_T> eventTypes;
        eventTypes.push_back(VPoxEventType_OnExtraDataChanged);
        CHECK_ERROR(pES, RegisterListener(pVPoxListener, ComSafeArrayAsInParam(eventTypes), true));
    }

    {
        ...
        pConsoleListener->init(new VPoxSDLConsoleEventListener());
        com::SafeArray<VPoxEventType_T> eventTypes;
        eventTypes.push_back(VPoxEventType_OnMousePointerShapeChanged);
        eventTypes.push_back(VPoxEventType_OnMouseCapabilityChanged);
        eventTypes.push_back(VPoxEventType_OnKeyboardLedsChanged);
        eventTypes.push_back(VPoxEventType_OnStateChanged);
        eventTypes.push_back(VPoxEventType_OnRuntimeError);
...
# endif /* VPOXSDL_WITH_X11 */
    ...
    LogRel(("VPoxSDL: NUM lock initially %s, CAPS lock initially %s\n",
            ...
            )
        {
            ...
            if (WaitSDLEvent(&event))
            {
                switch (event.type)
                {
                    ...
                    {
                        if (event.user.code != VPOXSDL_TERM_NORMAL)
                        {
                    ...
                    default:
                    {
                        Log8(("VPoxSDL: Unknown SDL event %d (pre)\n", event.type));
                        break;
                    }
                }
...
#endif
    LogFlow(("VPoxSDL: Entering big event loop\n"));
        ...
        switch (event.type)
        {
            ...
            case SDL_WINDOWEVENT:
            {
                switch (event.window.event)
                {
                    ...
                    {
                        VPoxSDLFB *fb = getFbFromWinId(event.window.windowID);
                        if (fb)
                            fb->repaint();
                        break;
                    }
...
# endif /* VPOXSDL_WITH_X11 */
        SDL_SetCursor(gpDefaultCursor);
# if defined(VPOXSDL_WITH_X11) && !defined(VPOX_WITHOUT_XCURSOR)
        if (gfXCursorEnabled)
            XFreeCursor(gSdlInfo.info.x11.display, pDefaultTempX11Cursor);
# endif /* VPOXSDL_WITH_X11 && !VPOX_WITHOUT_XCURSOR */
    }
...
# elif defined(VPOXSDL_WITH_X11) && !defined(VPOX_WITHOUT_XCURSOR)
            if (gfXCursorEnabled)
...
# endif /* VPOXSDL_WITH_X11 && !VPOX_WITHOUT_XCURSOR */
    ...
    /* VirtualPox (server) listener unregistration. */
    if (pVPoxListener)
    {
        ComPtr<IEventSource> pES;
        CHECK_ERROR(pVirtualPox, COMGETTER(EventSource)(pES.asOutParam()));
        if (!pES.isNull())
            CHECK_ERROR(pES, UnregisterListener(pVPoxListener));
        pVPoxListener.setNull();
    }

    /* VirtualPoxClient listener unregistration. */
    if (pVPoxClientListener)
    {
        ComPtr<IEventSource> pES;
        CHECK_ERROR(pVirtualPoxClient, COMGETTER(EventSource)(pES.asOutParam()));
        if (!pES.isNull())
            CHECK_ERROR(pES, UnregisterListener(pVPoxClientListener));
        pVPoxClientListener.setNull();
    }

    LogFlow(("Releasing machine, session...\n"));
    gpMachine = NULL;
    pSession = NULL;
    LogFlow(("Releasing VirtualPox object...\n"));
    pVirtualPox = NULL;
    LogFlow(("Releasing VirtualPoxClient object...\n"));
    pVirtualPoxClient = NULL;
...
static void ProcessKey(SDL_KeyboardEvent *ev)
{
#if (defined(DEBUG) || defined(VPOX_WITH_STATISTICS)) && !defined(VPOX_WITH_SDL2)
...
#endif // DEBUG || VPOX_WITH_STATISTICS
 ...
 */
static void SendMouseEvent(VPoxSDLFB *fb, int dz, int down, int button)
{
    ...
    if (abs)
    {
         ...
         * following problem: Some guests switch off the VPox hardware mouse
...
static void UpdateTitlebar(TitlebarMode mode, uint32_t u32User)
{
    ...
    RTStrPrintf(szTitle, sizeof(szTitle), "%s - " VPOX_PRODUCT,
...
# else
    SDL_WM_SetCaption(szTitle, VPOX_PRODUCT);
# endif
#endif
}

#if 0
static void vpox_show_shape(unsigned short w, unsigned short h,
...
static void SetPointerShape(const PointerShapeChangeData *data)
{
    ...
    if (data->shape.size() > 0)
    {
        ...
        // vpox_show_shape(data->width, data->height, 0, data->shape);
...
#ifndef VPOX_WITH_SDL2 /** @BUGBUG Implement alpha cursor support handling. */
            ICONINFO ii;
            ii.fIcon = FALSE;
