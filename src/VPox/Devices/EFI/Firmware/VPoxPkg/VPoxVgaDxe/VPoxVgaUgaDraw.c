  ...
  )
{
  ...
  Private = VPOX_VGA_PRIVATE_DATA_FROM_UGA_DRAW_THIS (This);
  ...
  )
{
  ...
  for (Index = 0; Index < Private->MaxMode; Index++) {
    ...
    InitializeGraphicsMode (Private, &VPoxVgaVideoModes[Private->ModeData[Index].ModeNumber]);
  ...
  )
{
  ...
  UgaDraw->GetMode  = VPoxVgaUgaDrawGetMode;
  UgaDraw->SetMode  = VPoxVgaUgaDrawSetMode;
  UgaDraw->Blt      = VPoxVgaUgaDrawBlt;
