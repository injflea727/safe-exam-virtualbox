/* $Id: DevIchAc97.cpp $ */
/** @file
 * DevIchAc97 - VPox ICH AC97 Audio Controller.
 */
...
static int ichac97R3MixerSetVolume(PAC97STATE pThis, PAC97STATER3 pThisCC, int index, PDMAUDIOMIXERCTL enmMixerCtl, uint32_t uVal)
{
    ...
    {
# ifndef VPOX_WITH_AC97_GAIN_SUPPORT
...
static DECLCALLBACK(int) ichac97R3Construct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    PCIDevSetStatus(pPciDev,                VPOX_PCI_STATUS_DEVSEL_MEDIUM |  VPOX_PCI_STATUS_FAST_BACK); /* 06 rwc?,ro? - pcists. */  Assert(pPciDev->abConfig[0x06] == 0x80); Assert(pPciDev->abConfig[0x07] == 0x02);
