...
static int dbgGuiCreate(ISession *pSession, PUVM pUVM, PDBGGUI *ppGui, PCDBGGUIVT *ppGuiVT)
{
    ...
    pGui->pVPoxDbgGui = new VPoxDbgGui();

    int rc;
    if (pSession)
        rc = pGui->pVPoxDbgGui->init(pSession);
    else
        rc = pGui->pVPoxDbgGui->init(pUVM);
    if (RT_SUCCESS(rc))
    {
        /*
 ...
 *
 * @returns VPox status code.
 * @param   pSession    The VirtualPox session.
...
DBGDECL(void) DBGGuiAdjustRelativePos(PDBGGUI pGui, int x, int y, unsigned cx, unsigned cy)
{
    ...
    pGui->pVPoxDbgGui->adjustRelativePos(x, y, cx, cy);
}
...
DBGDECL(int) DBGGuiShowStatistics(PDBGGUI pGui)
{
    ...
    AssertMsgReturn(pGui->u32Magic == DBGGUI_MAGIC, ("u32Magic=%#x\n", pGui->u32Magic), VERR_INVALID_PARAMETER);
    return pGui->pVPoxDbgGui->showStatistics();
}
...
DBGDECL(int) DBGGuiShowCommandLine(PDBGGUI pGui)
{
    AssertReturn(pGui, VERR_INVALID_PARAMETER);
    AssertMsgReturn(pGui->u32Magic == DBGGUI_MAGIC, ("u32Magic=%#x\n", pGui->u32Magic), VERR_INVALID_PARAMETER);
    return pGui->pVPoxDbgGui->showConsole();
}
...
{
    return pGui->pVPoxDbgGui->setParent((QWidget *)pvParent);
}
...
{
    return pGui->pVPoxDbgGui->setMenu((QMenu *)pvMenu);
}
