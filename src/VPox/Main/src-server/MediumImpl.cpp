...
struct Medium::Data
{
    ...
    /** weak VirtualPox parent */
    VirtualPox * const pVirtualPox;

    // pParent and llChildren are protected by VirtualPox::i_getMediaTreeLockHandle()
...
public:
          ...
          mNotifyAboutChanges(fNotifyAboutChanges)
    {
        ...
        /* Get strong VirtualPox reference, see below. */
        VirtualPox *pVirtualPox = aMedium->m->pVirtualPox;
        mVirtualPox = pVirtualPox;
        mVirtualPoxCaller.attach(pVirtualPox);
        mRC = mVirtualPoxCaller.rc();
        if (FAILED(mRC))
            return;
...
private:
    ...
    /* Must have a strong VirtualPox reference during a task otherwise the
    ...
    ComObjPtr<VirtualPox> mVirtualPox;
    AutoCaller mVirtualPoxCaller;
 ...
 * storage unit.
 *
 * This gets called by VirtualPox::CreateMedium() in which case uuidMachineRegistry
 * is empty since starting with VirtualPox 4.0, we no longer add opened media to a
 * registry automatically (this is deferred until the medium is attached to a machine).
 *
 * This also gets called when VirtualPox creates diff images; in this case uuidMachineRegistry
 ...
 * with the means of VirtualPox) the associated storage unit is assumed to be
 * ready for use so the state of the hard disk object will be set to Created.
 *
 * @param aVirtualPox   VirtualPox object.
 * @param aFormat
                     ...
                     const DeviceType_T aDeviceType)
{
    ...
    unconst(m->pVirtualPox) = aVirtualPox;
       ...
       )
    {
         ...
         * created by VirtualPox nor deleted, so we place the medium to
        ...
        AutoWriteLock treeLock(m->pVirtualPox->i_getMediaTreeLockHandle() COMMA_LOCKVAL_SRC_POS);
        ...
        for (unsigned i = 0; i < 5; i++)
        {
            ...
            fInUse = m->pVirtualPox->i_isMediaUuidInUse(m->id, aDeviceType);
            if (fInUse)
            {
                // create new UUID
                unconst(m->id).create();
        ...
        rc = m->pVirtualPox->i_registerMedium(this, &pMedium, treeLock);
 ...
 *
 * This gets called by VirtualPox::OpenMedium() and also by
 * Machine::AttachDevice() and createImplicitDiffs() when new diff
 * images are created.
 *
 * There is no registry for this case since starting with VirtualPox 4.0, we
 ...
 * If the detected or set parent is not known to VirtualPox, then this method
 * will fail.
 *
 * @param aVirtualPox   VirtualPox object.
 * @param aLocation     Storage unit location.
...
{
    AssertReturn(aVirtualPox, E_INVALIDARG);
 ...
 *                         if empty, then we use the VirtualPox home directory
 *
                        ...
                        const Utf8Str &strMachineFolder)
{
    ...
    /* register with VirtualPox/parent early, since uninit() will
        ...
        && !itCph->second.isEmpty())
    {
        ...
        int vrc = m->pVirtualPox->i_decryptSetting(&strPlaintext, itCph->second);
        if (RT_SUCCESS(vrc))
    ...
    if (m->formatObj->i_getCapabilities() & MediumFormatCapabilities_File)
    {
        ...
        {
            // Otherwise use the old VirtualPox "make absolute path" logic:
            rc = m->pVirtualPox->i_calculateFullPath(data.strLocation, strFull);
            if (FAILED(rc)) return rc;
        }
    }
    else
 ...
 *                      paths; if empty, then we use the VirtualPox home directory
                     ...
                     AutoWriteLock &mediaTreeLock)
{
    ...
    Assert(aVirtualPox->i_getMediaTreeLockHandle().isWriteLockOnCurrentThread());
    AssertReturn(aVirtualPox, E_INVALIDARG);
     ...
     * thread (i.e. the VirtualPox server startup thread) from an unexpected
         ...
         ++it)
    {
        ...
        rc = m->pVirtualPox->i_registerMedium(pMedium, &pMedium, mediaTreeLock);
        if (FAILED(rc)) break;
    }
 ...
 *
 * @note Locks VirtualPox lock for writing.
 */
HRESULT Medium::init(VirtualPox *aVirtualPox,
                     DeviceType_T aDeviceType,
                     const Utf8Str &aLocation,
                     const Utf8Str &aDescription /* = Utf8Str::Empty */)
{
    ...
    // We do not store host drives in VirtualPox.xml or anywhere else, so if we want
    // host drives to be identifiable by UUID and not give the drive a different UUID
    // every time VirtualPox starts, we need to fake a reproducible UUID here:
    RTUUID uuid;
    RTUuidClear(&uuid);
    if (aDeviceType == DeviceType_DVD)
...
void Medium::uninit()
{
     ...
     * the pVirtualPox reference, and in this case we don't need to continue.
    ...
    VirtualPox *pVirtualPox = m->pVirtualPox;
    if (!pVirtualPox)
        return;

    /* Caller must not hold the object or media tree lock over uninit(). */
    Assert(!isWriteLockOnCurrentThread());
    Assert(!pVirtualPox->i_getMediaTreeLockHandle().isWriteLockOnCurrentThread());

    AutoWriteLock treeLock(pVirtualPox->i_getMediaTreeLockHandle() COMMA_LOCKVAL_SRC_POS);
...
HRESULT Medium::setDescription(AutoCaller &autoCaller, const com::Utf8Str &aDescription)
{
    ...
    try
    {
        ...
        m->pVirtualPox->i_saveModifiedRegistries();
        m->pVirtualPox->i_onMediumConfigChanged(this);
    }
...
HRESULT Medium::setType(AutoCaller &autoCaller, MediumType_T aType)
{
    ...
    /* It is possible that some previous/concurrent uninit has already cleared
     * the pVirtualPox reference, see #uninit(). */
    ComObjPtr<VirtualPox> pVirtualPox(m->pVirtualPox);

    // we access m->pParent
    AutoReadLock treeLock(!pVirtualPox.isNull() ? &pVirtualPox->i_getMediaTreeLockHandle() : NULL COMMA_LOCKVAL_SRC_POS);
    ...
    while (m->queryInfoRunning)
    {
        ...
        Assert(!m->pVirtualPox->i_getMediaTreeLockHandle().isWriteLockOnCurrentThread());
    ...
    if (aType == MediumType_MultiAttach)
    {
        ...
        // global registry:
        const Guid &uuidGlobalRegistry = m->pVirtualPox->i_getGlobalRegistryId();
        if (i_isInRegistry(uuidGlobalRegistry))
            return setError(VPOX_E_INVALID_OBJECT_STATE,
                            tr("Cannot change type for medium '%s': the media type 'MultiAttach' can only be used "
                               "on media registered with a machine that was created with VirtualPox 4.0 or later"),
                            m->strLocationFull.c_str());
    }
                       ...
                       const com::Guid &aParentId)
{
    ...
    if (SUCCEEDED(rc))
    {
        ...
        {
            if (pPrevParent)
                m->pVirtualPox->i_onMediumConfigChanged(pPrevParent);
            if (pCurrParent)
                m->pVirtualPox->i_onMediumConfigChanged(pCurrParent);
        }
    }
...
HRESULT Medium::i_unlockRead(MediumState_T *aState)
{
    ...
    switch (m->state)
    {
        ...
        {
            LogFlowThisFunc(("Failing - state=%d\n", m->state));
            rc = setError(VPOX_E_INVALID_OBJECT_STATE,
...
{
    // make a copy of VirtualPox pointer which gets nulled by uninit()
    ComObjPtr<VirtualPox> pVirtualPox(m->pVirtualPox);
    ...
    pVirtualPox->i_saveModifiedRegistries();

    if (SUCCEEDED(mrc) && uId.isValid() && !uId.isZero())
        pVirtualPox->i_onMediumRegistered(uId, devType, FALSE);
                                  ...
                                  ComPtr<IProgress> &aProgress)
{
    ...
    try
    {
        ...
        rc = pProgress->init(m->pVirtualPox,
                        ...
                        ComPtr<IProgress> &aProgress)
{
    ...
    try
    {
        ...
        // and we need to write-lock the media involved
        uint32_t    cHandles    = 3;
        LockHandle* pHandles[4] = { &m->pVirtualPox->i_getMediaTreeLockHandle(),
                                    this->lockHandle(),
                                    pTarget->lockHandle() };
...
HRESULT Medium::moveTo(AutoCaller &autoCaller, const com::Utf8Str &aLocation, ComPtr<IProgress> &aProgress)
{
    ...
    /// this medium), this will also require to add the mRegistered flag to data
        ...
        /* play with locations */
        {
            ...
            rc = VirtualPox::i_ensureFilePathExists(destPath, true);
            if (FAILED(rc))
                throw rc;
        ...
        while (currMachineID != lastMachineID)
        {
            ...
            autoCaller.release();
            treeLock.release();
            rc = m->pVirtualPox->i_findMachine(id, false, true, &aMachine);
            treeLock.acquire();
            autoCaller.add();
            AssertComRCThrowRC(autoCaller.rc());
            alock.acquire();
...
HRESULT Medium::reset(AutoCaller &autoCaller, ComPtr<IProgress> &aProgress)
{
    ...
    try
    {
         ...
         * cleared the pVirtualPox reference, see #uninit(). */
        ComObjPtr<VirtualPox> pVirtualPox(m->pVirtualPox);

        /* canClose() needs the tree lock */
        AutoMultiWriteLock2 multilock(!pVirtualPox.isNull() ? &pVirtualPox->i_getMediaTreeLockHandle() : NULL,
                                      this->lockHandle()
                                      COMMA_LOCKVAL_SRC_POS);
...
HRESULT Medium::getEncryptionSettings(AutoCaller &autoCaller, com::Utf8Str &aCipher, com::Utf8Str &aPasswordId)
{
    ...
    try
    {
        ...
        if (it == pBase->m->mapProperties.end())
            throw VPOX_E_NOT_SUPPORTED;

# ifdef VPOX_WITH_EXTPACK
        ExtPackManager *pExtPackManager = m->pVirtualPox->i_getExtPackManager();
        if (pExtPackManager->i_isExtPackUsable(ORACLE_PUEL_EXTPACK_NAME))
        {
            ...
            if (SUCCEEDED(rc))
            {
                ...
                if (RT_FAILURE(vrc))
                    throw setErrorBoth(VPOX_E_NOT_SUPPORTED, vrc,
        ...
        if (RT_FAILURE(vrc))
            throw setErrorBoth(VPOX_E_INVALID_OBJECT_STATE, vrc,
                               tr("Failed to load the encryption filter: %s"),
                               i_vdError(vrc).c_str());

        it = pBase->m->mapProperties.find("CRYPT/KeyId");
        if (it == pBase->m->mapProperties.end())
            throw setError(VPOX_E_INVALID_OBJECT_STATE,
...
# ifdef VPOX_WITH_EXTPACK
        ...
        if (vrc == VERR_VD_PASSWORD_INCORRECT)
            throw setError(VPOX_E_PASSWORD_INCORRECT,
...
HRESULT Medium::openForIO(BOOL aWritable, com::Utf8Str const &aPassword, ComPtr<IMediumIO> &aMediumIO)
{
    ...
    if (SUCCEEDED(hrc))
    {
        hrc = ptrIO->initForMedium(this, m->pVirtualPox, aWritable != FALSE, strKeyId, aPassword);
        if (SUCCEEDED(hrc))
 ...
 * VirtualPox::getGlobalRegistryId().
 *
 ...
 * medium should be registered. The UUID can either be a machine UUID,
 * to add a machine registry, or the global registry UUID as returned by
 * VirtualPox::getGlobalRegistryId(). This recurses over all children.
 *
 * Note that for hard disks, this method does nothing if the medium is
 * already in another registry to avoid having hard disks in more than
 * one registry, which causes trouble with keeping diff images in sync.
 ...
 * machines have their own media registries, or it is the pseudo-UUID of the VirtualPox
 * object if the machine is old and still needs the global registry in VirtualPox.xml.
 *
...
void Medium::i_markRegistriesModified()
{
    ...
    // Get local copy, as keeping the lock over VirtualPox::markRegistryModified
    ...
    for (GuidList::const_iterator it = llRegistryIDs.begin();
         ...
         ++it)
    {
        m->pVirtualPox->i_markRegistryModified(*it);
    }
}
 ...
 * media registry is about to be deleted in VirtualPox::unregisterMachine().
 *
...
#ifdef DEBUG
/**
 * Debugging helper that gets called after VirtualPox initialization that writes all
...
ComObjPtr<Medium> Medium::i_getBase(uint32_t *aLevel /*= NULL*/)
{
    ...
    /* we access m->pParent */
    AutoReadLock treeLock(pVirtualPox->i_getMediaTreeLockHandle() COMMA_LOCKVAL_SRC_POS);
...
bool Medium::i_isReadOnly()
{
    ...
    /* we access children */
    AutoReadLock treeLock(m->pVirtualPox->i_getMediaTreeLockHandle() COMMA_LOCKVAL_SRC_POS);
...
void Medium::i_saveSettingsOne(settings::Medium &data, const Utf8Str &strHardDiskFolder)
{
        ...
        && !itPln->second.isEmpty())
    {
        ...
        int rc = m->pVirtualPox->i_encryptSetting(itPln->second, &strCiphertext);
        if (RT_SUCCESS(rc))
...
Utf8Str Medium::i_getPreferredDiffFormat()
{
    ...
    if (!(m->formatObj->i_getCapabilities() & MediumFormatCapabilities_Differencing))
    {
        ...
        Utf8Str tmp;
        m->pVirtualPox->i_getDefaultHardDiskFormat(tmp);
        return tmp;
    }
 ...
 * VirtualPox::saveRegistries(), in case someone wants to call this for several
 * media.
 *
...
HRESULT Medium::i_close(AutoCaller &autoCaller)
{
    ...
    AutoMultiWriteLock2 multilock(&m->pVirtualPox->i_getMediaTreeLockHandle(),
                                  this->lockHandle()
                                  COMMA_LOCKVAL_SRC_POS);
 ...
 *                      during execution of the function.
 *
 * @note Locks mVirtualPox and this object for writing. Locks medium tree for
 *       writing.
 */
                                ...
                                bool aWait, bool aNotify)
{
    ...
    try
    {
         ...
         * registered media are initially Inaccessible upon VPoxSVC startup
        ...
        if (m->backRefs.size() != 0)
        {
            ...
            i_dumpBackRefs();
#endif
            throw setError(VPOX_E_OBJECT_IN_USE,
         ...
         * which would have been broken if unregisterWithVirtualPox() failed
 ...
 * @retval  VPOX_E_INVALID_OBJECT_STATE if both media are not related to each other
 *          An error is set.
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (RT_SUCCESS(vrc))
                {
                    ...
                    if (RT_FAILURE(vrc))
                        hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Failed to add '%s' to output (%Rrc)"), aFilename, vrc);
                }
                else
                    hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc,
                                       tr("RTVfsCreateProgressForFile failed when processing '%s' (%Rrc)"), aFilename, vrc);
            }
            else
                hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("VDCreateVfsFileFromDisk failed for '%s' (%Rrc)"), aFilename, vrc);
            VDDestroy(pHdd);
        }
    }
    return hrc;
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (SUCCEEDED(hrc))
                {
                    ...
                    if (fDstCapabilities & MediumFormatCapabilities_File)
                    {
                        ...
                        hrc = VirtualPox::i_ensureFilePathExists(strDstLocation.c_str(),
                    ...
                    if (SUCCEEDED(hrc))
                    {
                        ...
                        if (RT_SUCCESS(vrc))
                        {
                            ...
                            if (RT_SUCCESS(vrc))
                                ...
                                hrc = setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Could not create the exported medium '%s'%s"),
 ...
 *       here and run in the task synchronously?  VPoxSVC has enough threads as
 *       it is...
 */
                             ...
                             bool aNotify)
{
    ...
    try
    {
        ...
        // and we need to write-lock the media involved
        uint32_t    cHandles    = 2;
        LockHandle* pHandles[3] = { &m->pVirtualPox->i_getMediaTreeLockHandle(),
                                    this->lockHandle() };
        /* Only add parent to the lock if it is not null */
        if (!aParent.isNull())
 ...
 *                  then it can be relative to the VirtualPox home directory.
 *
...
HRESULT Medium::i_queryInfo(bool fSetImageId, bool fSetParentId, AutoCaller &autoCaller)
{
     ...
     * time in VirtualPox (such as VMDK for which VDOpen() needs to
        ...
        try
        {
            ...
            if (formatObj->i_getCapabilities() & MediumFormatCapabilities_Uuid)
            {
                ...
                else
                {
                    ...
                    if (mediumId != uuid)
                    {
                        /** @todo r=klaus this always refers to VirtualPox.xml as the medium registry, even for new VMs */
                        lastAccessError = Utf8StrFmt(
                                tr("UUID {%RTuuid} of the medium '%s' does not match the value {%RTuuid} stored in the media registry ('%s')"),
                                &uuid,
                                location.c_str(),
                                mediumId.raw(),
                                pVirtualPox->i_settingsFilePath().c_str());
                        throw S_OK;
                    }
                }
            }
            ...
            if (uImageFlags & VD_IMAGE_FLAGS_DIFF)
            {
                    ...
                    && !(uImageFlags & VD_VMDK_IMAGE_FLAGS_STREAM_OPTIMIZED))
                {
                     ...
                     * call locking methods of mVirtualPox and parent, as all
                    ...
                    if (RTUuidIsNull(&parentId))
                        ...
                        rc = pVirtualPox->i_findHardDiskById(Guid(parentId), false /* aSetError */, &pParent);
                    if (FAILED(rc))
                    {
                ...
                else
                {
                    ...
                    if (m->pParent.isNull())
                    {
                        /* Due to a bug in VDCopy() in VirtualPox 3.0.0-3.0.14
                        ...
                        lastAccessError = Utf8StrFmt(
                                ...
                                location.c_str(),
                                pVirtualPox->settingsFilePath().c_str());
                        treeLock.release();
                        throw S_OK;
#endif /* 0 */
                    }
...
/**
 * Unregisters this medium with mVirtualPox. Called by close() under the medium tree lock.
 *
 * @note Caller must have locked the media tree lock for writing!
 */
HRESULT Medium::i_unregisterWithVirtualPox()
{
    /* Note that we need to de-associate ourselves from the parent to let
     * VirtualPox::i_unregisterMedium() properly save the registry */
    ...
    HRESULT rc = m->pVirtualPox->i_unregisterMedium(this);
    if (FAILED(rc))
    {
        if (pParentBackup)
        {
...
HRESULT Medium::i_setFormat(const Utf8Str &aFormat)
{
    ...
    {
        SystemProperties *pSysProps = m->pVirtualPox->i_getSystemProperties();
 ...
 *
 * @param aVRC  VPox error code to use when no error message is provided.
 */
 ...
 *          acquire the medium lock for writing and others (VirtualPox).
 *
 * @returns COM status code.
                            ...
                            MediumCryptoFilterSettings *pCryptoSettings)
{
    ...
    try
    {
        ...
        if (itKeyStore != pBase->m->mapProperties.end())
        {
            ...
            if (vrc == VERR_VD_PASSWORD_INCORRECT)
                throw setErrorBoth(VPOX_E_PASSWORD_INCORRECT, vrc, tr("The password to decrypt the image is incorrect"));
            if (RT_FAILURE(vrc))
                throw setErrorBoth(VPOX_E_INVALID_OBJECT_STATE, vrc, tr("Failed to load the decryption filter: %s"),
                                   i_vdError(vrc).c_str());
#else
        ...
        for (MediumLockList::Base::const_iterator it = sourceListBegin; it != sourceListEnd; ++it)
        {
            ...
            vrc = VDOpen(pHdd,
                         ...
                         pMedium->m->vdImageIfaces);
            if (RT_FAILURE(vrc))
                throw setErrorBoth(VPOX_E_FILE_ERROR, vrc,
 ...
 * asynchronously. As a result, we always save the VirtualPox.xml file when
 * we're done here.
 *
 * @param task
 * @return
...
HRESULT Medium::i_taskCreateBaseHandler(Medium::CreateBaseTask &task)
{
    ...
    try
    {
        ...
        * the moveTo() argument). Otherwise we have to generate it */
        ...
        {
            id.create();
            /* VirtualPox::i_registerMedium() will need UUID */
            unconst(m->id) = id;
        }
        ...
        try
        {
            ...
            {
                rc = VirtualPox::i_ensureFilePathExists(location, !(task.mVariant & MediumVariant_NoCreateDir) /* fCreate */);
                if (FAILED(rc))
                    throw rc;
            }
            ...
            if (task.mVariant & MediumVariant_Formatted)
            {
                ...
                if (RT_FAILURE(vrc))
                    throw setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Opening medium storage unit '%s' failed%s"),
                                       location.c_str(), i_vdError(vrc).c_str());
                ...
                if (RT_FAILURE(vrc) && RTErrInfoIsSet(&ErrInfo.Core))
                    throw setErrorBoth(VPOX_E_FILE_ERROR, vrc, tr("Formatting medium storage unit '%s' failed: %s"),
    ...
    if (SUCCEEDED(rc))
    {
        /* register with mVirtualPox as the last step and move to
    ...
    {
        m->pVirtualPox->i_onMediumConfigChanged(this);
        m->pVirtualPox->i_onMediumRegistered(m->id, m->devType, TRUE);
    }
...
HRESULT Medium::i_taskCreateDiffHandler(Medium::CreateDiffTask &task)
{
    ...
    try
    {
        ...
        try
        {
            ...
            if (capabilities & MediumFormatCapabilities_File)
            {
                HRESULT rc = VirtualPox::i_ensureFilePathExists(targetLocation,
    ...
    if (SUCCEEDED(mrc))
    {
         ...
         * better than breaking media registry consistency) */
        ComObjPtr<Medium> pMedium;
        mrc = m->pVirtualPox->i_registerMedium(pTarget, &pMedium, treeLock);
...
HRESULT Medium::i_taskMergeHandler(Medium::MergeTask &task)
{
    ...
    try
    {
        ...
        if (sourceSize > targetSize)
        {
            ...
            {
                AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
                HRESULT rc = setError(VPOX_E_NOT_SUPPORTED,
        ...
        {
            rcTmp = setErrorBoth(VPOX_E_FILE_ERROR, aVRC,
    ...
    if (SUCCEEDED(mrc))
    {
        ...
        if (task.mfMergeForward)
        {
            ...
            rc2 = m->pVirtualPox->i_unregisterMedium(pTarget);
            AssertComRC(rc2);
            ...
            /* then, register again */
            ComObjPtr<Medium> pMedium;
            rc2 = m->pVirtualPox->i_registerMedium(pTarget, &pMedium,
                                                   treeLock);
            AssertComRC(rc2);
        }
        else
             ...
             )
        {
            ...
            rc2 = pMedium->m->pVirtualPox->i_unregisterMedium(pMedium);
            AssertComRC(rc2);
    ...
    if (FAILED(mrc))
    {
         ...
         * and so on) or VirtualPox::saveRegistries() failed (where we
    ...
    else if (task.NotifyAboutChanges())
    {
        ...
        {
            if (it->isNotNull())
                m->pVirtualPox->i_onMediumConfigChanged(*it);
        }
        for (std::map<Guid, DeviceType_T>::const_iterator it = uIdsForNotify.begin();
             it != uIdsForNotify.end();
             ++it)
        {
            m->pVirtualPox->i_onMediumRegistered(it->first, it->second, FALSE);
        }
    }
 ...
 * As a result, we always save the VirtualPox.xml file when we're done here.
 *
 * @param task
 * @return
 */
HRESULT Medium::i_taskCloneHandler(Medium::CloneTask &task)
{
    ...
    try
    {
        ...
        {
            targetId.create();
            /* VirtualPox::registerMedium() will need UUID */
            unconst(pTarget->m->id) = targetId;
        }
    ...
    if (SUCCEEDED(mrc) && fCreatingTarget)
    {
        ...
        if (pParent)
        {
             ...
             * from VirtualPox. Depth check above. */
            ...
            eik.restore();
            ComObjPtr<Medium> pMedium;
            mrc = pParent->m->pVirtualPox->i_registerMedium(pTarget, &pMedium,
                                                            treeLock);
            Assert(   FAILED(mrc)
                   || pTarget == pMedium);
            eik.fetch();
        ...
        else
        {
            ...
            eik.restore();
            ComObjPtr<Medium> pMedium;
            mrc = m->pVirtualPox->i_registerMedium(pTarget, &pMedium,
                                                   treeLock);
            Assert(   FAILED(mrc)
                   || pTarget == pMedium);
            eik.fetch();
                ...
                if (task.NotifyAboutChanges())
                {
                    if (!fCreatingTarget)
                    {
                        if (!aFilterPropNames.empty())
                            m->pVirtualPox->i_onMediumConfigChanged(pTargetBase);
                        if (pParent)
                            m->pVirtualPox->i_onMediumConfigChanged(pParent);
                    }
                    else
                    {
                        m->pVirtualPox->i_onMediumRegistered(pTarget->i_getId(), pTarget->i_getDeviceType(), TRUE);
                    }
                }
            }
        }
...
HRESULT Medium::i_taskMoveHandler(Medium::MoveTask &task)
{
    ...
    {
        HRESULT rc = setError(VPOX_E_FILE_ERROR,
...
HRESULT Medium::i_taskDeleteHandler(Medium::DeleteTask &task)
{
    ...
    if (task.NotifyAboutChanges() && SUCCEEDED(rc))
    {
        if (m->pParent.isNotNull())
            m->pVirtualPox->i_onMediumConfigChanged(m->pParent);
        m->pVirtualPox->i_onMediumRegistered(uOldId, m->devType, FALSE);
    }
 ...
 * always save the VirtualPox.xml file when we're done here.
 *
 * @param task
 * @return
 */
HRESULT Medium::i_taskImportHandler(Medium::ImportTask &task)
{
    ...
    if (SUCCEEDED(mrc) && fCreatingTarget)
    {
        ...
        if (pParent)
        {
            ...
            eik.restore();
            ComObjPtr<Medium> pMedium;
            mrc = pParent->m->pVirtualPox->i_registerMedium(this, &pMedium,
                                                            treeLock);
            Assert(this == pMedium);
            eik.fetch();
        ...
        else
        {
            ...
            eik.restore();
            ComObjPtr<Medium> pMedium;
            mrc = m->pVirtualPox->i_registerMedium(this, &pMedium, treeLock);
            Assert(this == pMedium);
            eik.fetch();
        }
    }
        ...
        try
        {
            ...
            if (task.mstrCipher.isNotEmpty())
            {
                    ...
                    && task.mstrCurrentPassword.isNotEmpty())
                {
                    ...
                    throw setError(VPOX_E_OBJECT_NOT_FOUND,
