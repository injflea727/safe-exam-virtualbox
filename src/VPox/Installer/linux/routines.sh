...
# VirtualPox installer shell routines
#
...
## Abort if a copy of VirtualPox is already running
check_running()
{
    VPOXSVC_PID=`pidof VPoxSVC 2> /dev/null`
    if [ -n "$VPOXSVC_PID" ]; then
        if [ -f /etc/init.d/vpoxweb-service ]; then
            kill -USR1 $VPOXSVC_PID
        fi
        sleep 1
        if pidof VPoxSVC > /dev/null 2>&1; then
            echo 1>&2 "A copy of VirtualPox is currently running.  Please close it and try again."
            abort "Please note that it can take up to ten seconds for VirtualPox to finish running."
        fi
    fi
}
...
if sys.version_info >= (2, 6) and sys.version_info < (3, 0):
    ...
    $SHELL -c "cd $VPOX_INSTALL_PATH/sdk/installer && $PYTHON vpoxapisetup.py install \
    ...
    rm -rf $VPOX_INSTALL_PATH/sdk/installer/build
