...
#ifndef VPOX_INCLUDED_vmm_dbgf_h
#define VPOX_INCLUDED_vmm_dbgf_h
#ifndef RT_WITHOUT_PRAGMA_ONCE
# pragma once
#endif

#include <VPox/types.h>
#include <VPox/log.h>                   /* LOG_ENABLED */
#include <VPox/vmm/vmm.h>
#include <VPox/vmm/dbgfsel.h>
 ...
 *
 * @returns VPox status code which must be propagated up to EM if not VINF_SUCCESS.
...
VMMR3_INT_DECL(VPOXSTRICTRC)    DBGFR3EventHandlePending(PVM pVM, PVMCPU pVCpu);
...
VMM_INT_DECL(VPOXSTRICTRC)  DBGFBpCheckIo(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx, RTIOPORT uIoPort, uint8_t cbValue);
VMM_INT_DECL(VPOXSTRICTRC)  DBGFEventGenericWithArgs(PVM pVM, PVMCPU pVCpu, DBGFEVENTTYPE enmEvent, DBGFEVENTCTX enmCtx,
                                                     unsigned cArgs, ...);
...
typedef struct DBGFOSREG
{
     ...
     * This is only called after pfnInit().
     *
     * @returns VPox status code (allocation error or something of  similar fatality).
     * @param   pUVM            The user mode VM handle.
     * @param   pvData          Pointer to the instance data.
...
typedef enum DBGFPLUGINOP
{
     ...
     * uArg: The full VirtualPox version, see VPox/version.h. */
    DBGFPLUGINOP_INIT,
 ...
 *
 * @returns VPox status code. Any non VINF_SUCCESS status code will abort the dumping.
 *
 ...
 * @{ */
VMMR3DECL(VPOXSTRICTRC)      DBGFR3ReportBugCheck(PVM pVM, PVMCPU pVCpu, DBGFEVENTTYPE enmEvent, uint64_t uBugCheck,
...
#endif /* !VPOX_INCLUDED_vmm_dbgf_h */
