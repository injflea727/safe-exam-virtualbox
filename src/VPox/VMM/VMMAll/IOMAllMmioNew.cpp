...
#if defined(VPOX_WITH_STATISTICS) || defined(DOXYGEN_RUNNING)
 ...
 */
static VPOXSTRICTRC iomMmioRing3WritePending(PVMCPU pVCpu, RTGCPHYS GCPhys, void const *pvBuf, size_t cbBuf,
 ...
 *
 * @returns Strict VPox status code. Any EM scheduling status code,
 ...
 */
static VPOXSTRICTRC iomMmioDoComplicatedWrite(PVM pVM, PVMCPU pVCpu, CTX_SUFF(PIOMMMIOENTRY) pRegEntry,
                                              ...
                                              void const *pvValue, unsigned cbValue IOM_MMIO_STATS_COMMA_DECL)
{
     ...
     */
    VPOXSTRICTRC rc = VINF_SUCCESS; NOREF(pVM);
#ifdef VPOX_STRICT
...
# endif
    ...
    for (;;)
    {
         ...
         */
        VPOXSTRICTRC rc2 = pRegEntry->pfnWriteCallback(pRegEntry->pDevIns, pRegEntry->pvUser,
                                                       !(pRegEntry->fFlags & IOMMMIO_FLAGS_ABS)
                                                       ? offRegion & ~(RTGCPHYS)3 : GCPhys & ~(RTGCPHYS)3,
 ...
 */
DECLINLINE(VPOXSTRICTRC) iomMmioDoWrite(PVMCC pVM, PVMCPU pVCpu, CTX_SUFF(PIOMMMIOENTRY) pRegEntry,
                                        RTGCPHYS GCPhys, RTGCPHYS offRegion,
 ...
 */
VPOXSTRICTRC iomR3MmioCommitWorker(PVM pVM, PVMCPU pVCpu, PIOMMMIOENTRYR3 pRegEntry, RTGCPHYS offRegion)
{
...
# endif
    ...
    VPOXSTRICTRC rcStrict = iomMmioDoWrite(pVM, pVCpu, pRegEntry, pVCpu->iom.s.PendingMmioWrite.GCPhys, offRegion,
 ...
 * @param   pStats      Pointer to the statistics (never NULL).
 */
static VPOXSTRICTRC iomMMIODoComplicatedRead(PVM pVM, CTX_SUFF(PIOMMMIOENTRY) pRegEntry, RTGCPHYS GCPhys, RTGCPHYS offRegion,
...
# endif
    ...
    for (;;)
    {
         ...
         */
        uint32_t u32Value;
        VPOXSTRICTRC rcStrict2 = pRegEntry->pfnReadCallback(pRegEntry->pDevIns, pRegEntry->pvUser,
                                                            !(pRegEntry->fFlags & IOMMMIO_FLAGS_ABS)
                                                            ? offRegion & ~(RTGCPHYS)3 : GCPhys & ~(RTGCPHYS)3,
                                                            &u32Value, sizeof(u32Value));
        switch (VPOXSTRICTRC_VAL(rcStrict2))
        {
            case VINF_SUCCESS:
                break;
            case VINF_IOM_MMIO_UNUSED_FF:
            ...
            case VINF_IOM_R3_MMIO_WRITE:
                ...
                LogFlow(("iomMMIODoComplicatedRead: GCPhys=%RGp GCPhysStart=%RGp cbValue=%u rcStrict2=%Rrc\n",
                         GCPhys, GCPhysStart, cbValue, VPOXSTRICTRC_VAL(rcStrict2)));
                return rcStrict2;
            default:
 ...
 */
DECLINLINE(VPOXSTRICTRC) iomMmioDoRead(PVMCC pVM, CTX_SUFF(PIOMMMIOENTRY) pRegEntry, RTGCPHYS GCPhys, RTGCPHYS offRegion,
                                       void *pvValue, uint32_t cbValue IOM_MMIO_STATS_COMMA_DECL)
{
    ...
    {
        switch (VPOXSTRICTRC_VAL(rcStrict))
        {
 ...
 *
 * @returns VPox status code (appropriate for GC return).
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure of the calling EMT.
 ...
 * @param   pRegEntry   The MMIO entry for the current context.
 */
DECLINLINE(VPOXSTRICTRC) iomMmioCommonPfHandlerNew(PVMCC pVM, PVMCPUCC pVCpu, uint32_t uErrorCode,
 ...
 */
DECLEXPORT(VPOXSTRICTRC) iomMmioPfHandlerNew(PVMCC pVM, PVMCPUCC pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pCtxCore,
...
# endif
    ...
    VPOXSTRICTRC rcStrict = iomMmioCommonPfHandlerNew(pVM, pVCpu, (uint32_t)uErrorCode, GCPhysFault, pRegEntry);
 ...
 */
VMM_INT_DECL(VPOXSTRICTRC) IOMR0MmioPhysHandler(PVMCC pVM, PVMCPUCC pVCpu, uint32_t uErrorCode, RTGCPHYS GCPhysFault)
{
     ...
     */
    VPOXSTRICTRC rcStrict = IOM_LOCK_SHARED(pVM);
    if (RT_SUCCESS(rcStrict))
    {
        RTGCPHYS offRegion;
 ...
 * @remarks The @a pvUser argument is the MMIO handle.
 */
PGM_ALL_CB2_DECL(VPOXSTRICTRC) iomMmioHandlerNew(PVMCC pVM, PVMCPUCC pVCpu, RTGCPHYS GCPhysFault, void *pvPhys, void *pvBuf,
...
# endif
     ...
     */
    VPOXSTRICTRC rcStrict = PDMCritSectEnter(pDevIns->CTX_SUFF(pCritSectRo), rcToRing3);
    if (rcStrict == VINF_SUCCESS)
    {
        if (enmAccessType == PGMACCESSTYPE_READ)
        {
        ...
        AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc -  Access type %d - %RGp - %s\n",
                                             VPOXSTRICTRC_VAL(rcStrict), enmAccessType, GCPhysFault, pRegEntry->pszDesc));
#else
        AssertMsg(   rcStrict == VINF_SUCCESS
                  ...
                  , ("%Rrc - Access type %d - %RGp - %s #%u\n",
                     VPOXSTRICTRC_VAL(rcStrict), enmAccessType, GCPhysFault, pDevIns->pReg->szName, pDevIns->iInstance));
#endif
    }
    /*
 ...
 *
 * @returns VPox status code.  This API may return VINF_SUCCESS even if no
 *          remapping is made.
 *
 * @param   pVM             The cross context VM structure.
