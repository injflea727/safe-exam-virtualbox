/* $Id: SUPDrv-win.cpp $ */
/** @file
 * VPoxDrv - The VirtualPox Support Driver - Windows NT specifics.
 */
...
#define SERVICE_NAME    "VPoxDrv"
/** The Pool tag (VPox). */
#define SUPDRV_NT_POOL_TAG  'xoBV'

/** NT device name for user access. */
#define DEVICE_NAME_NT_USR          L"\\Device\\VPoxDrvU"
#ifdef VPOX_WITH_HARDENING
/** Macro for checking for deflecting calls to the stub device. */
# define VPOXDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_DEV(a_pDevObj, a_pIrp) \
 ...
 *  devices. */
# define VPOXDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_OR_ERROR_INFO_DEV(a_pDevObj, a_pIrp) \
    do { if ((a_pDevObj) == g_pDevObjStub || (a_pDevObj) == g_pDevObjErrorInfo) \
            return supdrvNtCompleteRequest(STATUS_ACCESS_DENIED, a_pIrp); \
    } while (0)
#else
# define VPOXDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_DEV(a_pDevObj, a_pIrp)                 do {} while (0)
# define VPOXDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_OR_ERROR_INFO_DEV(a_pDevObj, a_pIrp)   do {} while (0)
#endif

/** Enables the fast I/O control code path. */
#define VPOXDRV_WITH_FAST_IO
...
*********************************************************************************************************************************/
/**
 * Device extension used by VPoxDrvU.
 */
...
/** Pointer to the VPoxDrvU device extension. */
...
/**
 * Device extension used by VPoxDrvStub.
 */
...
/** Pointer to the VPoxDrvStub device extension. */
...
/**
 * Device extension used by VPoxDrvErrorInfo.
 */
...
/** Pointer to the VPoxDrvErrorInfo device extension. */
...
/**
 * Error info for a failed VPoxDrv or VPoxDrvStub open attempt.
 */
...
typedef struct SUPDRVNTPROTECT
{
    ...
    HANDLE              hParentPid;
    /** The TID of the thread opening VPoxDrv or VPoxDrvStub, NULL if not opened. */
    HANDLE              hOpenTid;
...
#endif /* VPOX_WITH_HARDENINIG */


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static void     _stdcall   VPoxDrvNtUnload(PDRIVER_OBJECT pDrvObj);
static NTSTATUS _stdcall   VPoxDrvNtCreate(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static NTSTATUS _stdcall   VPoxDrvNtCleanup(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static NTSTATUS _stdcall   VPoxDrvNtClose(PDEVICE_OBJECT pDevObj, PIRP pIrp);
#ifdef VPOXDRV_WITH_FAST_IO
static BOOLEAN  _stdcall   VPoxDrvNtFastIoDeviceControl(PFILE_OBJECT pFileObj, BOOLEAN fWait, PVOID pvInput, ULONG cbInput,
                                                        PVOID pvOutput, ULONG cbOutput, ULONG uCmd,
                                                        PIO_STATUS_BLOCK pIoStatus, PDEVICE_OBJECT pDevObj);
#endif
static NTSTATUS _stdcall   VPoxDrvNtDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static int                 VPoxDrvNtDeviceControlSlow(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PIRP pIrp, PIO_STACK_LOCATION pStack);
static NTSTATUS _stdcall   VPoxDrvNtInternalDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static VOID     _stdcall   VPoxPowerDispatchCallback(PVOID pCallbackContext, PVOID pArgument1, PVOID pArgument2);
static NTSTATUS _stdcall   VPoxDrvNtRead(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static NTSTATUS _stdcall   VPoxDrvNtNotSupportedStub(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static NTSTATUS            VPoxDrvNtErr2NtStatus(int rc);
#ifdef VPOX_WITH_HARDENING
...
static FAST_IO_DISPATCH const g_VPoxDrvFastIoDispatch =
{
    /* .SizeOfFastIoDispatch            = */ sizeof(g_VPoxDrvFastIoDispatch),
    ...
    /* .FastIoDeviceControl             = */ VPoxDrvNtFastIoDeviceControl,
 ...
 */
static NTSTATUS vpoxdrvNtCreateDevices(PDRIVER_OBJECT pDrvObj)
{
    /*
     * System device.
     */
...
/**
 * Destroys the devices and links created by vpoxdrvNtCreateDevices.
 */
static void vpoxdrvNtDestroyDevices(void)
{
    if (g_pDevObjUsr)
    {
        PSUPDRVDEVEXTUSR pDevExtUsr = (PSUPDRVDEVEXTUSR)g_pDevObjUsr->DeviceExtension;
...
NTSTATUS _stdcall DriverEntry(PDRIVER_OBJECT pDrvObj, PUNICODE_STRING pRegPath)
{
     ...
     */
#ifdef VPOXDRV_WITH_FAST_IO
    if (g_VPoxDrvFastIoDispatch.FastIoDeviceControl != VPoxDrvNtFastIoDeviceControl)
    {
        DbgPrint("VPoxDrv: FastIoDeviceControl=%p instead of %p\n",
                 g_VPoxDrvFastIoDispatch.FastIoDeviceControl, VPoxDrvNtFastIoDeviceControl);
     ...
     */
    NTSTATUS rcNt;
    PWSTR pwszCopy = (PWSTR)ExAllocatePoolWithTag(g_enmNonPagedPoolType, pRegPath->Length + sizeof(WCHAR), 'VPox');
    if (pwszCopy)
    {
        ...
        ExFreePoolWithTag(pwszCopy, 'VPox');
    ...
    if (RT_SUCCESS(vrc))
    {
...
#endif
        {
             ...
             */
            rcNt = vpoxdrvNtCreateDevices(pDrvObj);
            if (NT_SUCCESS(rcNt))
            {
                /*
                ...
                if (!vrc)
                {
                     ...
                     */
                    pDrvObj->DriverUnload                                   = VPoxDrvNtUnload;
                    pDrvObj->MajorFunction[IRP_MJ_CREATE]                   = VPoxDrvNtCreate;
                    pDrvObj->MajorFunction[IRP_MJ_CLEANUP]                  = VPoxDrvNtCleanup;
                    pDrvObj->MajorFunction[IRP_MJ_CLOSE]                    = VPoxDrvNtClose;
                    pDrvObj->MajorFunction[IRP_MJ_DEVICE_CONTROL]           = VPoxDrvNtDeviceControl;
                    pDrvObj->MajorFunction[IRP_MJ_INTERNAL_DEVICE_CONTROL]  = VPoxDrvNtInternalDeviceControl;
                    pDrvObj->MajorFunction[IRP_MJ_READ]                     = VPoxDrvNtRead;
                    pDrvObj->MajorFunction[IRP_MJ_WRITE]                    = VPoxDrvNtNotSupportedStub;

#ifdef VPOXDRV_WITH_FAST_IO
                    /* Fast I/O to speed up guest execution roundtrips. */
                    pDrvObj->FastIoDispatch = (PFAST_IO_DISPATCH)&g_VPoxDrvFastIoDispatch;
#endif
                    ...
                    if (rcNt == STATUS_SUCCESS)
                        pDevExt->hPowerCallback = ExRegisterCallback(pDevExt->pObjPowerCallback,
                                                                     VPoxPowerDispatchCallback,
                                                                     g_pDevObjSys);

                    /*
                     * Done! Returning success!
                     */
                    Log(("VPoxDrv::DriverEntry returning STATUS_SUCCESS\n"));
                    return STATUS_SUCCESS;
                }

                Log(("supdrvInitDevExit failed with vrc=%d!\n", vrc));
                rcNt = VPoxDrvNtErr2NtStatus(vrc);

                vpoxdrvNtDestroyDevices();
            }
#ifdef VPOX_WITH_HARDENING
            supdrvNtProtectTerm();
#endif
 ...
 */
void _stdcall VPoxDrvNtUnload(PDRIVER_OBJECT pDrvObj)
{
    PSUPDRVDEVEXT pDevExt = (PSUPDRVDEVEXT)g_pDevObjSys->DeviceExtension;

    Log(("VPoxDrvNtUnload at irql %d\n", KeGetCurrentIrql()));
 ...
 */
NTSTATUS _stdcall VPoxDrvNtCreate(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    Log(("VPoxDrvNtCreate: RequestorMode=%d\n", pIrp->RequestorMode));
     ...
     * VPoxDrvNtInternalDeviceControl.  The first request will be one to
     * create a session.
     */
    NTSTATUS rcNt;
    ...
    {
#if defined(VPOX_WITH_HARDENING) && !defined(VPOX_WITHOUT_DEBUGGER_CHECKS)
        /*
        ...
        {
            LogRel(("vpoxdrv: Process %p is being debugged, access to vpoxdrv / vpoxdrvu declined.\n",
...
#endif
        {
            ...
            if (pDevObj == g_pDevObjStub)
            {
                ...
                LogRel(("vpoxdrv: Declined %p access to VPoxDrvStub: rc=%d\n", PsGetProcessId(PsGetCurrentProcess()), rc));
            }
            /*
            ...
            else if (pDevObj != g_pDevObjUsr)
            {
                ...
                if (pNtProtect)
                {
                    if (pNtProtect->enmProcessKind == kSupDrvNtProtectKind_VmProcessUnconfirmed)
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            LogRel(("vpoxdrv: supdrvCreateSession failed for process %p: rc=%d.\n",
                                    PsGetProcessId(PsGetCurrentProcess()), rc));
                        }
                        else
                            LogRel(("vpoxdrv: Process %p failed process verification: rc=%d.\n",
                                    PsGetProcessId(PsGetCurrentProcess()), rc));
                    }
                    else
                    {
                        LogRel(("vpoxdrv: %p is not a budding VM process (enmProcessKind=%d).\n",
                ...
                {
                    LogRel(("vpoxdrv: %p is not a budding VM process.\n", PsGetProcessId(PsGetCurrentProcess())));
 ...
 * @param   pIrp        Request packet.
 */
NTSTATUS _stdcall VPoxDrvNtCleanup(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    PSUPDRVDEVEXT       pDevExt  = SUPDRVNT_GET_DEVEXT(pDevObj);
    PIO_STACK_LOCATION  pStack   = IoGetCurrentIrpStackLocation(pIrp);
    PFILE_OBJECT        pFileObj = pStack->FileObject;
    ...
    if (pDevObj == g_pDevObjStub)
    {
        ...
        Log(("VPoxDrvNtCleanup: pDevExt=%p pFileObj=%p pNtProtect=%p\n", pDevExt, pFileObj, pNtProtect));
        if (pNtProtect)
        {
            supdrvNtProtectRelease(pNtProtect);
            pFileObj->FsContext = NULL;
...
#endif
    {
        ...
        Log(("VPoxDrvNtCleanup: pDevExt=%p pFileObj=%p pSession=%p\n", pDevExt, pFileObj, pSession));
        if (pSession)
        {
 ...
 * @param   pIrp        Request packet.
 */
NTSTATUS _stdcall VPoxDrvNtClose(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    PSUPDRVDEVEXT       pDevExt  = SUPDRVNT_GET_DEVEXT(pDevObj);
    PIO_STACK_LOCATION  pStack   = IoGetCurrentIrpStackLocation(pIrp);
    PFILE_OBJECT        pFileObj = pStack->FileObject;
    ...
    {
        PSUPDRVNTPROTECT pNtProtect = (PSUPDRVNTPROTECT)pFileObj->FsContext;
        Log(("VPoxDrvNtClose: pDevExt=%p pFileObj=%p pNtProtect=%p\n", pDevExt, pFileObj, pNtProtect));
        if (pNtProtect)
        {
            supdrvNtProtectRelease(pNtProtect);
            pFileObj->FsContext = NULL;
                                                     ...
                                                     PIO_STATUS_BLOCK pIoStatus, PDEVICE_OBJECT pDevObj)
{
    ...
    unsigned    cbOut = 0;
    int         rc = 0;
    Log2(("VPoxDrvNtFastIoDeviceControl(%p): ioctl=%#x pvIn=%p cbIn=%#x pvOut=%p cbOut=%#x pSession=%p\n",
...
# endif
    {
        ...
        if ((uCmd & 0x3) == METHOD_BUFFERED)
        {
            ...
            if (NT_SUCCESS(rcNt))
            {
                    ...
                    && cbBuf < _1M*16)
                {
                    ...
                    PSUPREQHDR pHdr = (PSUPREQHDR)ExAllocatePoolWithTag(g_enmNonPagedPoolType, cbBuf, 'VPox');
                    if (pHdr)
                    {
                        __try
                        {
                        ...
                        if (NT_SUCCESS(rcNt))
                        {
                            ...
                            if (RT_SUCCESS(rc))
                            {
                                ...
                                {
                                    cbOut = cbOutput;
                                    OSDBGPRINT(("VPoxDrvNtFastIoDeviceControl: too much output! %#x > %#x; uCmd=%#x!\n",
                            ...
                            Log2(("VPoxDrvNtFastIoDeviceControl: returns %#x cbOut=%d rc=%#x\n", rcNt, cbOut, rc));
                        }
                        else
                            Log(("VPoxDrvNtFastIoDeviceControl: Error reading %u bytes of user memory at %p (uCmd=%#x)\n",
                                 cbInput, pvInput, uCmd));
                        ExFreePoolWithTag(pHdr, 'VPox');
                    }
                    else
                ...
                {
                    Log(("VPoxDrvNtFastIoDeviceControl: Mismatching sizes (%#x) - Hdr=%#lx/%#lx Irp=%#lx/%#lx!\n",
        ...
        {
            Log(("VPoxDrvNtFastIoDeviceControl: not buffered request (%#x) - not supported\n", uCmd));
            rcNt = STATUS_NOT_SUPPORTED;
        }
    }
# ifdef RT_ARCH_AMD64
    ...
    {
        Log(("VPoxDrvNtFastIoDeviceControl: WOW64 req - not supported\n"));
        rcNt = STATUS_NOT_SUPPORTED;
    }
# endif
 ...
 * @param   pIrp        Request packet.
 */
NTSTATUS _stdcall VPoxDrvNtDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    VPOXDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_OR_ERROR_INFO_DEV(pDevObj, pIrp);
    ...
    return VPoxDrvNtDeviceControlSlow(pDevExt, pSession, pIrp, pStack);
}


/**
 * Worker for VPoxDrvNtDeviceControl that takes the slow IOCtl functions.
 *
 * @returns NT status code.
 *
...
static int VPoxDrvNtDeviceControlSlow(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PIRP pIrp, PIO_STACK_LOCATION pStack)
{
    ...
    uint32_t    cbOut = 0;
    int         rc = 0;
    Log2(("VPoxDrvNtDeviceControlSlow(%p,%p): ioctl=%#x pBuf=%p cbIn=%#x cbOut=%#x pSession=%p\n",
...
#endif
    {
                ...
                &&  pStack->Parameters.DeviceIoControl.OutputBufferLength ==  pHdr->cbOut)
            {
                ...
                if (!rc)
                {
                    ...
                    if (cbOut > pStack->Parameters.DeviceIoControl.OutputBufferLength)
                    {
                        ...
                        OSDBGPRINT(("VPoxDrvNtDeviceControlSlow: too much output! %#x > %#x; uCmd=%#x!\n",
                                    pHdr->cbOut, cbOut, pStack->Parameters.DeviceIoControl.IoControlCode));
                    }
                }
                else
                    rcNt = STATUS_INVALID_PARAMETER;
                Log2(("VPoxDrvNtDeviceControlSlow: returns %#x cbOut=%d rc=%#x\n", rcNt, cbOut, rc));
            }
            else
            {
                Log(("VPoxDrvNtDeviceControlSlow: Mismatching sizes (%#x) - Hdr=%#lx/%#lx Irp=%#lx/%#lx!\n",
        ...
        {
            Log(("VPoxDrvNtDeviceControlSlow: not buffered request (%#x) - not supported\n",
                 pStack->Parameters.DeviceIoControl.IoControlCode));
            rcNt = STATUS_NOT_SUPPORTED;
        }
    }
    ...
    {
        Log(("VPoxDrvNtDeviceControlSlow: WOW64 req - not supported\n"));
        rcNt = STATUS_NOT_SUPPORTED;
    }
#endif
 ...
 * @param   pIrp        Request packet.
 */
NTSTATUS _stdcall VPoxDrvNtInternalDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    VPOXDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_OR_ERROR_INFO_DEV(pDevObj, pIrp);
    ...
    unsigned            cbOut = 0;
    int                 rc = 0;
    Log2(("VPoxDrvNtInternalDeviceControl(%p,%p): ioctl=%#x pBuf=%p cbIn=%#x cbOut=%#x pSession=%p\n",
          pDevExt, pIrp, pStack->Parameters.DeviceIoControl.IoControlCode,
          pIrp->AssociatedIrp.SystemBuffer, pStack->Parameters.DeviceIoControl.InputBufferLength,
          pStack->Parameters.DeviceIoControl.OutputBufferLength, pSession));
    ...
    if ((pStack->Parameters.DeviceIoControl.IoControlCode & 0x3) == METHOD_BUFFERED)
    {
           ...
           )
        {
                ...
                &&  pStack->Parameters.DeviceIoControl.OutputBufferLength == pHdr->cb)
            {
                ...
                Log2(("VPoxDrvNtInternalDeviceControl: returns %#x/rc=%#x\n", rcNt, rc));
            }
            else
            {
                Log(("VPoxDrvNtInternalDeviceControl: Mismatching sizes (%#x) - Hdr=%#lx Irp=%#lx/%#lx!\n",
                     pStack->Parameters.DeviceIoControl.IoControlCode,
    ...
    {
        Log(("VPoxDrvNtInternalDeviceControl: not buffered request (%#x) - not supported\n",
             pStack->Parameters.DeviceIoControl.IoControlCode));
        rcNt = STATUS_NOT_SUPPORTED;
    }
...
/**
 * Implementation of the read major function for VPoxDrvErrorInfo.
 *
 ...
 */
NTSTATUS _stdcall VPoxDrvNtRead(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    Log(("VPoxDrvNtRead\n"));
    RT_NOREF1(pDevObj);
 ...
 * @param   pIrp        IRP.
 */
NTSTATUS _stdcall VPoxDrvNtNotSupportedStub(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    Log(("VPoxDrvNtNotSupportedStub\n"));
    NOREF(pDevObj);
 ...
 */
VOID _stdcall VPoxPowerDispatchCallback(PVOID pCallbackContext, PVOID pvArgument1, PVOID pvArgument2)
{
    /*PDEVICE_OBJECT pDevObj = (PDEVICE_OBJECT)pCallbackContext;*/ RT_NOREF1(pCallbackContext);
    Log(("VPoxPowerDispatchCallback: %x %x\n", pvArgument1, pvArgument2));

    /* Power change imminent? */
    if ((uintptr_t)pvArgument1 == PO_CB_SYSTEM_STATE_LOCK)
    {
        if (pvArgument2 == NULL)
            Log(("VPoxPowerDispatchCallback: about to go into suspend mode!\n"));
        else
            Log(("VPoxPowerDispatchCallback: resumed!\n"));
...
size_t VPOXCALL supdrvOSGipGetGroupTableSize(PSUPDRVDEVEXT pDevExt)
{
    NOREF(pDevExt);
...
int VPOXCALL supdrvOSInitGipGroupTable(PSUPDRVDEVEXT pDevExt, PSUPGLOBALINFOPAGE pGip, size_t cbGipCpuGroups)
{
...
void VPOXCALL supdrvOSGipInitGroupBitsForCpu(PSUPDRVDEVEXT pDevExt, PSUPGLOBALINFOPAGE pGip, PSUPGIPCPU pGipCpu)
{
    NOREF(pDevExt);
...
# ifndef RT_ARCH_X86
#  error "VPOX_WITHOUT_NATIVE_R0_LOADER is only safe on x86."
# endif
    ...
    if (iDiff)
    {
        ...
        for (size_t off = uRva; cbLeft > 0; off++, cbLeft--)
            if (pbNativeBits[off] != pbImageBits[off])
            {
                ...
                SUPR0Printf("VPoxDrv: %s", pReq->u.Out.szError);
                break;
            }
    }
    return iDiff;
...
void VPOXCALL   supdrvOSLdrUnload(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage)
{
    if (pImage->pvNtSectionObj)
    {
        ...
        if (rcNt != STATUS_SUCCESS)
            SUPR0Printf("VPoxDrv: failed to unload '%s', rcNt=%#x\n", pImage->szName, rcNt);
        pImage->pvNtSectionObj = NULL;
    }
    NOREF(pDevExt);
}
...
static bool supdrvNtProtectGetAlpcPortObjectType2(PCRTUTF16 pwszPortNm, POBJECT_TYPE *ppObjType)
{
    ...
    if (NT_SUCCESS(rcNt))
    {
        ...
        if (NT_SUCCESS(rcNt))
        {
            ...
            if (pObjType)
            {
                SUPR0Printf("vpoxdrv: ALPC Port Object Type %p (vs %p)\n", pObjType, *ppObjType);
                *ppObjType = pObjType;
                fDone = true;
            }
...
static POBJECT_TYPE supdrvNtProtectGetAlpcPortObjectType(uint32_t uSessionId, const char *pszSessionId)
{
        ...
        && g_pfnObGetObjectType)
    {
        ...
        rc |= RTUtf16CatAscii(wszPortNm, RT_ELEMENTS(wszPortNm), "\\VPoxDrv-");
        ...
        if (!fDone)
        {
             ...
             */
            if (uSessionId == 0)
                rc  = RTUtf16CopyAscii(wszPortNm, RT_ELEMENTS(wszPortNm), "\\BaseNamedObjects\\VPoxDrv-");
            else
            {
                rc  = RTUtf16CopyAscii(wszPortNm, RT_ELEMENTS(wszPortNm), "\\Sessions\\");
                rc |= RTUtf16CatAscii(wszPortNm, RT_ELEMENTS(wszPortNm), pszSessionId);
                rc |= RTUtf16CatAscii(wszPortNm, RT_ELEMENTS(wszPortNm), "\\BaseNamedObjects\\VPoxDrv-");
            }
            cchTmp = RTStrFormatU32(szTmp, sizeof(szTmp), (uint32_t)(uintptr_t)PsGetProcessId(PsGetCurrentProcess()), 16, 0, 0, 0);
            Assert(cchTmp > 0);
            rc |= RTUtf16CatAscii(wszPortNm, RT_ELEMENTS(wszPortNm), szTmp);
...
/**
 * Called in the context of VPoxDrvNtCreate to determin the CSRSS for the
 * current process.
 *
...
static int supdrvNtProtectFindAssociatedCsrss(PSUPDRVNTPROTECT pNtProtect)
{
    ...
    if (!NT_SUCCESS(rcNt))
    {
        SUPR0Printf("vpoxdrv: Error opening '%ls': %#x\n", wszApiPort, rcNt);
...
#endif /* VPOX_WITHOUT_DEBUGGER_CHECKS */
...
/**
 * Cleans up VPoxDrv or VPoxDrvStub error info not collected by the dead process.
 *
...
static void supdrvNtProtectVerifyNewChildProtection(PSUPDRVNTPROTECT pNtStub, PSUPDRVNTPROTECT pNtVm, bool fCallerChecks)
{
    ...
    LogRel(("vpoxdrv: Misdetected vm stub; hParentPid=%p hChildPid=%p\n", pNtStub->AvlCore.Key, pNtVm->AvlCore.Key));
...
supdrvNtProtectCallback_ProcessCreateNotifyEx(PEPROCESS pNewProcess, HANDLE hNewPid, PPS_CREATE_NOTIFY_INFO pInfo)
{
    ...
    if (pInfo)
    {
        ...
        Log(("vpoxdrv/NewProcessEx: ctx=%04zx/%p pid=%04zx ppid=%04zx ctor=%04zx/%04zx rcNt=%#x %.*ls\n",
    ...
    if (pNtProtect)
    {
        ...
        if (pOpInfo->Operation == OB_OPERATION_HANDLE_CREATE)
        {
            ...
            if ((PEPROCESS)pOpInfo->Object == PsGetCurrentProcess())
            {
                ...
                Log(("vpoxdrv/ProcessHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d] %s\n",
            ...
            else if (supdrvNtProtectIsWhitelistedDebugger(PsGetCurrentProcess()))
            {
                ...
                Log(("vpoxdrv/ProcessHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d] %s [debugger]\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->CreateHandleInformation.DesiredAccess,
                     pOpInfo->Object, pNtProtect->AvlCore.Key, pNtProtect->enmProcessKind,
                     PsGetProcessImageFileName(PsGetCurrentProcess()) ));
            ...
            else
            {
                    ...
                    && supdrvNtProtectIsAssociatedCsrss(pNtProtect, PsGetCurrentProcess()) )
                {
                    ...
                    if (fDesiredAccess == s_fCsrssStupidDesires)
                    {
                        ...
                        fAllowedRights = fAllowedRights
                                       | PROCESS_VM_OPERATION
                                       | PROCESS_VM_WRITE
                                       | PROCESS_DUP_HANDLE /* Needed for CreateProcess/VPoxTestOGL. */
                                       | 0;
                        pOpInfo->CallContext = NULL; /* don't assert this. */
                    }
                }
                ...
                Log(("vpoxdrv/ProcessHandlePre: %sctx=%04zx/%p wants %#x to %p/pid=%04zx [%d], allow %#x => %#x; %s [prev=%#x]\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
        ...
        else
        {
            ...
            {
                Log(("vpoxdrv/ProcessHandlePre: ctx=%04zx/%p[%p] dup from %04zx/%p with %#x to %p in pid=%04zx [%d] %s\n",
                     PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
            ...
            else
            {
                    ...
                    && supdrvNtProtectIsAssociatedCsrss(pNtProtect, PsGetCurrentProcess()))
                {
                    if (ASMAtomicDecS32(&pNtProtect->cCsrssFirstProcessDuplicateHandle) >= 0)
                    {
                        ...
                        fAllowedRights = fAllowedRights
                                       | PROCESS_VM_OPERATION
                                       | PROCESS_VM_WRITE
                                       | PROCESS_DUP_HANDLE /* Needed for launching VPoxTestOGL. */
                                       | 0;
                        pOpInfo->CallContext = NULL; /* don't assert this. */
                    }
                }
                ...
                Log(("vpoxdrv/ProcessHandlePre: %sctx=%04zx/%p[%p] dup from %04zx/%p with %#x to %p in pid=%04zx [%d] %s\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->DuplicateHandleInformation.TargetProcess,
                     PsGetProcessId((PEPROCESS)pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess),
                     pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess,
...
supdrvNtProtectCallback_ThreadHandlePre(PVOID pvUser, POB_PRE_OPERATION_INFORMATION pOpInfo)
{
    ...
    if (pNtProtect)
    {
        ...
        if (pOpInfo->Operation == OB_OPERATION_HANDLE_CREATE)
        {
            ...
            {
                Log(("vpoxdrv/ThreadHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d] self\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->CreateHandleInformation.DesiredAccess,
                     pOpInfo->Object, pNtProtect->AvlCore.Key, pNtProtect->enmProcessKind));
                pOpInfo->CallContext = NULL; /* don't assert */
            ...
            {
                Log(("vpoxdrv/ThreadHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d] %s [debugger]\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->CreateHandleInformation.DesiredAccess,
                     pOpInfo->Object, pNtProtect->AvlCore.Key, pNtProtect->enmProcessKind,
                     PsGetProcessImageFileName(PsGetCurrentProcess()) ));
            ...
            else
            {
                ...
                /* Special case 2 on 8.1, possibly also Vista, 7, 8:
                   When creating a process like VPoxTestOGL from the VM process,
                ...
                Log(("vpoxdrv/ThreadHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d], allow %#x => %#x; %s [prev=%#x]\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->CreateHandleInformation.DesiredAccess,
        ...
        else
        {
            ...
            {
                Log(("vpoxdrv/ThreadHandlePre: %sctx=%04zx/%p[%p] dup from %04zx/%p with %#x to %p in pid=%04zx [%d] self\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->DuplicateHandleInformation.TargetProcess,
                     PsGetProcessId((PEPROCESS)pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess),
                     pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess,
            ...
            else
            {
                ...
                Log(("vpoxdrv/ThreadHandlePre: %sctx=%04zx/%p[%p] dup from %04zx/%p with %#x to %p in pid=%04zx [%d], allow %#x => %#x; %s\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->DuplicateHandleInformation.TargetProcess,
                     PsGetProcessId((PEPROCESS)pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess),
                     pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess,
...
# endif
        ...
        {
            LogRel(("vpoxdrv: Found evil handle to budding VM process: pid=%p h=%p acc=%#x attr=%#x type=%s\n",
...
# ifndef VPOX_WITHOUT_DEBUGGER_CHECKS
...
# endif
    ...
    {
        LogRel(("vpoxdrv: Cannot locate ZwQueryVirtualMemory in ntoskrnl, nor were we able to cook up a replacement.\n"));
...
# ifdef VPOX_STRICT
    ...
    {
        LogRel(("vpoxdrv: g_pfnObGetObjectType=%p g_pfnZwAlpcCreatePort=%p.\n", g_pfnObGetObjectType, g_pfnZwAlpcCreatePort));
        return STATUS_PROCEDURE_NOT_FOUND;
    }
# endif
    ...
    if (RT_FAILURE(rc))
        return VPoxDrvNtErr2NtStatus(rc);
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (NT_SUCCESS(rcNt))
            {
                ...
                if (g_pfnObRegisterCallbacks && g_pfnObUnRegisterCallbacks)
                {
                    ...
                    LogRel(("vpoxdrv: ObRegisterCallbacks failed with rcNt=%#x\n", rcNt));
                    g_pvObCallbacksCookie = NULL;
                }
                else
                {
...
# endif
                    {
                        DbgPrint("vpoxdrv: ObRegisterCallbacks was not found. Please make sure you got the latest updates and service packs installed\n");
                        rcNt = STATUS_SXS_VERSION_CONFLICT;
                    }
                    else
                    {
                        Log(("vpoxdrv: ObRegisterCallbacks was not found; ignored pre-Vista\n"));
                ...
                LogRel(("vpoxdrv: PsSetCreateProcessNotifyRoutine%s failed with rcNt=%#x\n",
