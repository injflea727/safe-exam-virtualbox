/* $Id: VPoxGallium.cpp $ */
/** @file
 * VirtualPox Windows Guest Mesa3D - Gallium driver interface. Constructs Gallium stack.
 */
...
#include <VPoxGaNine.h>

#include "VPoxD3DAdapter9.h"
#include "VPoxPresent.h"
#include "VPoxGaD3DDevice9Ex.h"
 ...
 *   1) load the hardware driver VPoxVMSVGA or VPoxVirGL:
...
static HRESULT getProcAddresses(HMODULE hmod, struct VPOXGAPROC *paProcs)
{
    struct VPOXGAPROC *pIter = paProcs;
    while (pIter->pszName)
    {
...
static HRESULT loadDll(const char *pszName, HMODULE *phmod, struct VPOXGAPROC *paProcs)
{
 ...
 */
class VPoxGalliumStack: public IGalliumStack
{
    public:
        VPoxGalliumStack();
        virtual ~VPoxGalliumStack();
...
{
    public:
        GaDirect3D9Ex(VPoxGalliumStack *pStack);
...
/*
 * VPoxGalliumStack implementation.
 */

VPoxGalliumStack::VPoxGalliumStack()
    :
    mcRefs(0),
    mhmodStateTracker(0),
    mhmodDriver(0)
...
STDMETHODIMP_(ULONG) VPoxGalliumStack::AddRef()
{
    ULONG refs = InterlockedIncrement(&mcRefs);
    return refs;
}

STDMETHODIMP_(ULONG) VPoxGalliumStack::Release()
{
...
STDMETHODIMP VPoxGalliumStack::QueryInterface(REFIID riid,
                                              void **ppvObject)
{
    if (!ppvObject)
    {
...
HRESULT VPoxGalliumStack::Load()
{
    struct VPOXGAPROC aNineProcs[] =
    {
...
STDMETHODIMP VPoxGalliumStack::GaNineD3DAdapter9Create(struct pipe_screen *s,
                                                       ID3DAdapter9 **ppOut)
{
    return mNine.pfnGaNineD3DAdapter9Create(s, ppOut);
}

STDMETHODIMP_(struct pipe_resource *) VPoxGalliumStack::GaNinePipeResourceFromSurface(IUnknown *pSurface)
{
    return mNine.pfnGaNinePipeResourceFromSurface(pSurface);
}

STDMETHODIMP_(struct pipe_context *) VPoxGalliumStack::GaNinePipeContextFromDevice(IDirect3DDevice9 *pDevice)
{
    return mNine.pfnGaNinePipeContextFromDevice(pDevice);
}

STDMETHODIMP_(struct pipe_screen *) VPoxGalliumStack::GaDrvScreenCreate(const WDDMGalliumDriverEnv *pEnv)
{
    return mDrv.pfnGaDrvScreenCreate(pEnv);
}

STDMETHODIMP_(void) VPoxGalliumStack::GaDrvScreenDestroy(struct pipe_screen *s)
{
    mDrv.pfnGaDrvScreenDestroy(s);
}

STDMETHODIMP_(WDDMGalliumDriverEnv const *) VPoxGalliumStack::GaDrvGetWDDMEnv(struct pipe_screen *pScreen)
{
    return mDrv.pfnGaDrvGetWDDMEnv(pScreen);
}

STDMETHODIMP_(uint32_t) VPoxGalliumStack::GaDrvGetContextId(struct pipe_context *pPipeContext)
{
    return mDrv.pfnGaDrvGetContextId(pPipeContext);
}

STDMETHODIMP_(uint32_t) VPoxGalliumStack::GaDrvGetSurfaceId(struct pipe_screen *pScreen,
                                                            struct pipe_resource *pResource)
{
    return mDrv.pfnGaDrvGetSurfaceId(pScreen, pResource);
}

STDMETHODIMP_(void) VPoxGalliumStack::GaDrvContextFlush(struct pipe_context *pPipeContext)
{
    mDrv.pfnGaDrvContextFlush(pPipeContext);
}

STDMETHODIMP VPoxGalliumStack::CreateDirect3DEx(HANDLE hAdapter,
                                                HANDLE hDevice,
                                                const D3DDDI_DEVICECALLBACKS *pDeviceCallbacks,
                                                const VPOXGAHWINFO *pHWInfo,
                                                IDirect3D9Ex **ppOut)
...
STDMETHODIMP VPoxGalliumStack::GaCreateDeviceEx(D3DDEVTYPE DeviceType,
                                                HWND hFocusWindow,
                                                DWORD BehaviorFlags,
                                                D3DPRESENT_PARAMETERS* pPresentationParameters,
                                                D3DDISPLAYMODEEX* pFullscreenDisplayMode,
...
{
    VPoxGalliumStack *p = new VPoxGalliumStack();
    if (!p)
        return E_OUTOFMEMORY;
...
GaDirect3D9Ex::GaDirect3D9Ex(VPoxGalliumStack *pStack)
    :
    mcRefs(0),
    mpStack(pStack),
    mpPipeScreen(0),
