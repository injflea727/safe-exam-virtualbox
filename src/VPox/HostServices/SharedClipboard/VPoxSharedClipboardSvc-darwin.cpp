/* $Id: VPoxSharedClipboardSvc-darwin.cpp $ */
/** @file
 ...
 */
static int vpoxClipboardChanged(SHCLCONTEXT *pCtx)
{
    if (pCtx->pClient == NULL)
        return VINF_SUCCESS;

    /* Retrieve the formats currently in the clipboard and supported by vpox */
    uint32_t fFormats = 0;
    bool     fChanged = false;
 ...
 */
static DECLCALLBACK(int) vpoxClipboardThread(RTTHREAD ThreadSelf, void *pvUser)
{
...
int ShClSvcImplInit(VPOXHGCMSVCFNTABLE *pTable)
{
    ...
    rc = RTThreadCreate(&g_ctx.hThread, vpoxClipboardThread, &g_ctx, 0,
...
int ShClSvcImplSync(PSHCLCLIENT pClient)
{
    ...
    int rc = vpoxClipboardChanged(pClient->State.pCtx);
...
int ShClSvcImplFormatAnnounce(PSHCLCLIENT pClient, SHCLFORMATS fFormats)
{
    ...
    if (fFormats == VPOX_SHCL_FMT_NONE)
    {
    ...
    if (fFormats & VPOX_SHCL_FMT_URI_LIST) /* No transfer support yet. */
        return VINF_SUCCESS;
#endif
    ...
    RTStrPrintf(pCtx->szGuestOwnershipFlavor, sizeof(pCtx->szGuestOwnershipFlavor),
                "org.virtualpox.sharedclipboard.%RTproc.%RX64", RTProcSelf(), idFlavor);
