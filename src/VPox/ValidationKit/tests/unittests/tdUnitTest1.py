...
"""
VirtualPox Validation Kit - Unit Tests.
"""
...
class tdUnitTest1(vpox.TestDriver):
    """
    Unit Tests.
    """
    ...
    kdTestCasesBuggyPerOs = {
        ...
        'linux.amd64': {
            'testcase/tstLdr-4': '',        # failed: Failed to get bits for '/home/vpox/test/tmp/bin/testcase/tstLdrObjR0.r0'/0,
    ...
    kdTestCasesBlackList = {
        ...
        'testcase/tstSupLoadModule': '',                # Needs parameters and vpoxdrv access. Covered elsewhere.
        'testcase/tstSeamlessX11': '',                  # graphics test
        'testcase/tstTime-3': '',                       # parameters required
        'testcase/tstVPoxControl': '',                  # works only inside a guest
        ...
        'testcase/tstRTSemXRoads': '',                  # sporadically failed: Traffic - 8 threads per direction, 10 sec :
                                                        # FAILED (8 errors)
        'tstVPoxAPILinux': '',                          # creates VirtualPox directories for root user because of sudo
                                                        # (should be in vpox)
    ...
    kasHardened = [
        ...
        "testcase/tstR0ThreadPreemptionDriver", # VPox 4.3
        "testcase/tstRTR0ThreadPreemptionDriver",
        "testcase/tstRTR0MemUserKernelDriver",
        "testcase/tstRTR0SemMutexDriver",
    ...
    def _detectPaths(self):
        ...
        This sets sVPoxInstallRoot and sUnitTestsPathBase and returns True/False.
        """

        #
        # We need a VPox install (/ build) to test.
        #
        if False is True:
            if not self.importVPoxApi():
                reporter.error('Unabled to import the VPox Python API.')
                return False
        else:
            self._detectBuild();
            if self.oBuild is None:
                reporter.error('Unabled to detect the VPox build.');
                return False;
        ...
        if utils.getHostOs() == 'win':
            ...
            if sPath and sPath[-1] != ';':
                sPath += ';';
            os.environ[sPathName] = sPath + self.sVPoxInstallRoot + ';';
        ...
        if utils.getHostOs() == 'darwin':
            for i in range(1, len(asCandidates)):
                asCandidates[i] = os.path.join(asCandidates[i], 'VirtualPox.app', 'Contents', 'MacOS');
    ...
    def _figureVersion(self):
        """ Tries to figure which VPox version this is, setting self.aiVPoxVer. """
        try:
            sVer = utils.processOutputChecked(['VPoxManage', '--version'])
            ...
            self.aiVPoxVer = [int(sComp) for sComp in sVer.split('.')];

            reporter.log('VPox version: %s' % (self.aiVPoxVer,));
        except:
            reporter.logXcpt();
            return False;
        return True;
    ...
    def _compareVersion(self, aiVer):
        """
        Compares the give version string with the vpox version string,
        returning a result similar to C strcmp().  aiVer is on the right side.
        """
        cComponents = min(len(self.aiVPoxVer), len(aiVer));
        for i in range(cComponents):
            if self.aiVPoxVer[i] < aiVer[i]:
                return -1;
            if self.aiVPoxVer[i] > aiVer[i]:
                return 1;
        return len(self.aiVPoxVer) - len(aiVer);
            ...
            for sExpr in asExprs:
                ...
                for sSubExpr in sExpr.split('&&'):
                    ...
                    if sOp == '>=' and iCmp < 0:
                        ...
                        fResult = False;
                    reporter.log2('iCmp=%s; %s %s %s -> %s' % (iCmp, self.aiVPoxVer, sOp, aiValue, fResult));
    ...
    def _executeTestCase(self, sName, sFullPath, sTestCaseSubDir, oDevNull): # pylint: disable=too-many-locals
        ...
        # needs to be copied to and execute from the sVPoxInstallRoot
        ...
        if    sName in self.kasHardened \
          and self.sUnitTestsPathBase != self.sVPoxInstallRoot:

            sDstDir = os.path.join(self.sVPoxInstallRoot, sTestCaseSubDir);
