...
/** @file
 * VPox Qt GUI - UINetworkAttachmentEditor class implementation.
 */
...
{
    return QList<QString>::fromVector(uiCommon().virtualPox().GetInternalNetworks());
}
...
{
    return QList<QString>::fromVector(uiCommon().virtualPox().GetGenericNetworkDrivers());
}
...
QStringList UINetworkAttachmentEditor::natNetworks()
{
    ...
    foreach (const CNATNetwork &comNetwork, uiCommon().virtualPox().GetNATNetworks())
...
QStringList UINetworkAttachmentEditor::cloudNetworks()
{
    ...
    foreach (const CCloudNetwork &comNetwork, uiCommon().virtualPox().GetCloudNetworks())
