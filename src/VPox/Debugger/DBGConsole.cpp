 ...
 * debugging facilities for the VirtualPox VMM.  It was initially only
 * accessible thru a telnet session in debug builds.  Later it was hastily built
 * into the VPoxDbg module with a very simple Qt wrapper around it.
 *
 * The current state is that it's by default shipped with all standard
 * VirtualPox builds.  The GUI component is by default accessible in all
 ...
 * an optional feature that can be omitted when building VirtualPox.  Therefore,
 * all external code interfacing DBGC need to be enclosed in
 * \#ifdef VPOX_WITH_DEBUGGER blocks. This is mandatory for components that
 ...
 *
 * The DBGC code is linked into the VPoxVMM module.
 *
...
#include <VPox/vmm/vmapi.h> /* VMR3GetVM() */
#include <VPox/vmm/hm.h>    /* HMR3IsEnabled */
#include <VPox/vmm/nem.h>   /* NEMR3IsEnabled */
#include <VPox/err.h>
#include <VPox/log.h>
 ...
 * @returns VPox status on failure.
 ...
 *
 * @returns VPox status code. Any error indicates the termination of the console session.
...
int dbgcProcessInput(PDBGC pDbgc, bool fNoExecute)
{
    ...
    if (pDbgc->cInputLines)
    {
        ...
        if (    RT_SUCCESS(rc)
            ...
            &&  pDbgc->fReady)
            rc = pDbgc->CmdHlp.pfnPrintf(&pDbgc->CmdHlp, NULL, "VPoxDbg> ");
 ...
 *
 * @returns VPox status code. (output related)
 * @param   pDbgc   Debugger console instance data.
 */
static int dbgcProcessLog(PDBGC pDbgc)
{
...
static int dbgcReadConfig(PDBGC pDbgc, PUVM pUVM)
{
    ...
    /** @cfgm{/DBGC/HistoryFile, string, ${HOME}/.vpoxdbgc-history}
     * The command history file of the VPox debugger. */
    rc = RTPathAppend(szHomeDefault, sizeof(szHomeDefault), ".vpoxdbgc-history");
    AssertLogRelRCReturn(rc, rc);
    ...
    /** @cfgm{/DBGC/GlobalInitFile, string, ${HOME}/.vpoxdbgc-init}
     * The global init script of the VPox debugger. */
    szHomeDefault[cchHome] = '\0';
    rc = RTPathAppend(szHomeDefault, sizeof(szHomeDefault), ".vpoxdbgc-init");
    AssertLogRelRCReturn(rc, rc);
     ...
     * The VM local init script of the VPox debugger. */
 ...
 * @returns The VPox status code causing the console termination.
 *
 * @param   pUVM        The user mode VM handle.
...
DBGDECL(int) DBGCCreate(PUVM pUVM, PDBGCBACK pBack, unsigned fFlags)
{
     ...
     */
    rc = pDbgc->CmdHlp.pfnPrintf(&pDbgc->CmdHlp, NULL,
                                 "Welcome to the VirtualPox Debugger!\n");
    ...
    if (RT_SUCCESS(rc) && pUVM)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                rc = pDbgc->CmdHlp.pfnVPoxError(&pDbgc->CmdHlp, rc, "When trying to attach to VM %p\n", pDbgc->pVM);
        }
        else
            rc = pDbgc->CmdHlp.pfnVPoxError(&pDbgc->CmdHlp, rc, "Error reading configuration\n");
    }
