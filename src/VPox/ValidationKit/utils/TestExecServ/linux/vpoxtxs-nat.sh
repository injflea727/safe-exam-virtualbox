#!/bin/sh
## @file
# VirtualPox Test Execution Service Init Script for NATted setups.
#
...
SCRIPTNAME=vpoxtxs-nat.sh

CDROM_PATH=/media/cdrom
SCRATCH_PATH=/tmp/vpoxtxs-scratch

PIDFILE="/var/run/vpoxtxs"
...
start() {
    if ! test -f $PIDFILE; then
        begin_msg "Starting VirtualPox Test Execution Service" console
        fixAndTestBinary
        ...
        if test $RETVAL -eq 0; then
            succ_msg "VirtualPox Test Execution service started"
        else
            fail_msg "VirtualPox Test Execution service failed to start"
        fi
    fi
    return $RETVAL
}
...
stop() {
    if test -f $PIDFILE; then
        begin_msg "Stopping VirtualPox Test Execution Service" console
        killproc $binary
    fi
}
