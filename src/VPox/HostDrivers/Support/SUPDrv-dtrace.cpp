...
/** @file
 * VPoxDrv - The VirtualPox Support Driver - DTrace Provider.
 */
...
#ifdef RT_OS_DARWIN
# include VPOX_PATH_MACOSX_DTRACE_H
...
/* DTrace experiments with the Unbreakable Enterprise Kernel (UEK2)
   ...
   3. No tracepoints in vpoxdrv, vpoxnet* or vpoxpci yet.  This requires yasm
      and VPoxTpG and build time. */
#  include "dtrace.h"
# else
/* DTrace experiments with the Unbreakable Enterprise Kernel (UEKR3)
   ...
   1. To generate the missing entries for the dtrace module in Module.symvers
      ...
      >> Module.symvers
      Update: Althernative workaround (active), resolve symbols dynamically.
   2. No tracepoints in vpoxdrv, vpoxnet* or vpoxpci yet.  This requires yasm
      and VPoxTpG and build time. */
        ...
        if (*pidProbe != 0)
         ...
         if (dtrace_probe_lookup(idProvider, pProv->pszModName, pszFnNmBuf, pszPrbName) != DTRACE_IDNONE)
         {
             ...
             if (dtrace_probe_lookup(idProvider, pProv->pszModName, pszFnNmBuf, pszPrbName) != DTRACE_IDNONE)
             {
                 ...
                 if (iOrd >= 128)
                 {
                     LogRel(("VPoxDrv: More than 128 duplicate probe location instances %s at line %u in function %s [%s], probe %s\n",
...
static const dtrace_pops_t g_vpoxDtVtgProvOps =
{
    ...
    /* .dtps_enable          = */ (FNPOPS_ENABLE *)vpoxDtPOps_Enable,
...
# endif
    ...
    return &g_VPoxDTraceReg;
}

#ifndef VPOX_WITH_NATIVE_DTRACE
# error "VPOX_WITH_NATIVE_DTRACE is not defined as it should"
#endif
