/* $Id: VPoxNetDhcpd.cpp $ */
/** @file
 * VPoxNetDhcpd - DHCP server for host-only and NAT networks.
 */
...
class VPoxNetDhcpd
{
    DECLARE_CLS_COPY_CTOR_ASSIGN_NOOP(VPoxNetDhcpd);
 ...
 */
int VPoxNetDhcpd::logInitStderr()
{
...
int VPoxNetDhcpd::ifInit(const RTCString &strNetwork,
                         const RTCString &strTrunk,
                         INTNETTRUNKTYPE enmTrunkType)
{
    int rc;
...
int VPoxNetDhcpd::ifOpen(const RTCString &strNetwork,
                         const RTCString &strTrunk,
                         INTNETTRUNKTYPE enmTrunkType)
{
    AssertReturn(m_pSession != NIL_RTR0PTR, VERR_GENERAL_FAILURE);
...
int VPoxNetDhcpd::ifProcessInput()
{
    AssertReturn(m_pSession != NIL_RTR0PTR, VERR_GENERAL_FAILURE);
    AssertReturn(m_hIf != INTNET_HANDLE_INVALID, VERR_GENERAL_FAILURE);
    AssertReturn(m_pIfBuf != NULL, VERR_GENERAL_FAILURE);
 ...
 */
int VPoxNetDhcpd::ifInput(void *pvFrame, uint32_t cbFrame)
{
    if (pvFrame == NULL)
 ...
 */
err_t VPoxNetDhcpd::netifLinkOutput(pbuf *pPBuf)
{
...
/* static */ DECLCALLBACK(void) VPoxNetDhcpd::lwipInitCB(void *pvArg)
{
    AssertPtrReturnVoid(pvArg);

    VPoxNetDhcpd *self = static_cast<VPoxNetDhcpd *>(pvArg);
    self->lwipInit();
}


/* static */ err_t VPoxNetDhcpd::netifInitCB(netif *pNetif)
{
    AssertPtrReturn(pNetif, ERR_ARG);

    VPoxNetDhcpd *self = static_cast<VPoxNetDhcpd *>(pNetif->state);
    return self->netifInit(pNetif);
}


/* static */ err_t VPoxNetDhcpd::netifLinkOutputCB(netif *pNetif, pbuf *pPBuf)
{
    AssertPtrReturn(pNetif, ERR_ARG);
...
/* static */ void VPoxNetDhcpd::dhcp4RecvCB(void *arg, struct udp_pcb *pcb,
                                            struct pbuf *p,
                                            ip_addr_t *addr, u16_t port)
{
    AssertPtrReturnVoid(arg);

    VPoxNetDhcpd *self = static_cast<VPoxNetDhcpd *>(arg);
...
int VPoxNetDhcpd::main(int argc, char **argv)
{
    /*
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            /* setup lwip */
            rc = vpoxLwipCoreInitialize(lwipInitCB, this);
            if (RT_SUCCESS(rc))
            {
                /*
                ...
                DHCP_LOG_MSG_ERROR(("Terminating - vpoxLwipCoreInitialize failed: %Rrc\n", rc));
        }
        else
...
void VPoxNetDhcpd::lwipInit()
{
    ...
    netif *pNetif = netif_add(&m_LwipNetif,
                              ...
                              IP_ADDR_ANY,               /* gateway */
                              this,                      /* state */
                              VPoxNetDhcpd::netifInitCB, /* netif_init_fn */
...
err_t VPoxNetDhcpd::netifInit(netif *pNetif)
{
...
void VPoxNetDhcpd::dhcp4Recv(struct udp_pcb *pcb, struct pbuf *p,
                             ip_addr_t *addr, u16_t port)
{
    ...
    {
        LogRel(("VPoxNetDhcpd::dhcp4Recv: Caught std::bad_alloc!\n"));
    }
}
