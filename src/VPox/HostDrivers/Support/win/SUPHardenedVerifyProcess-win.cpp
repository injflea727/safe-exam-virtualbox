...
/** @file
 * VirtualPox Support Library/Driver - Hardened Process Verification, Windows.
 */
...
#endif
#ifdef VPOX_PERMIT_MORE
# define VPOX_PERMIT_MORE_FIRST_IDX 5
    "sfc.dll",
    "sfc_os.dll",
    "user32.dll",
    "acres.dll",
...
/**
 * VPox executables allowed to start VMs.
    ...
    for (uint32_t i = 0; i < cSections; i++)
    {
        ...
        if (!pImage->fApiSetSchemaOnlySection1 || i == 0)
        {
            ...
            /* The section bits. Child purification verifies all, normal
               ...
               concerned (due to opening vpoxdrv during early process init). */
...
#ifndef VPOX_PERMIT_VISUAL_STUDIO_PROFILING
...
# ifdef VPOX_PERMIT_MORE
            if (g_uNtVerCombined < SUP_NT_VER_W70 && i >= VPOX_PERMIT_MORE_FIRST_IDX)
                pImage->pszName = NULL; /* hard limit: user32.dll is unwanted prior to w7. */
# endif

#endif /* VPOX_PERMIT_VISUAL_STUDIO_PROFILING */
            break;
        }
    if (!pImage->pszName)
    {
    ...
    if (!pImage->pszName)
    {
                ...
                || supHardNtVpAreNamesEqual("sysfrethunk.dll", pwszFilename)) )
        {
            supHardNtVpSetInfo2(pThis, VERR_SUP_VP_SYSFER_DLL,
                                ...
                                "component (or disable it) to prevent ADC from injecting itself into the VirtualPox VM processes. "
    ...
    if (NT_SUCCESS(rcNt) && (pThis->fFlags & SUPHARDNTVP_F_EXEC_ALLOC_REPLACE_WITH_RW))
    {
        ...
        if (!NT_SUCCESS(rcNt))
        {
            supHardNtVpSetInfo2(pThis, VERR_SUP_VP_REPLACE_VIRTUAL_MEMORY_FAILED,
                                ...
                                "See VPoxStartup.log for more details",
                                pvAlloc, cbFree, rcNt);
            supR3HardenedLogFlush();
 ...
 * after respawning and the second is when we open the vpoxdrv device for
 * unrestricted access.
 */
