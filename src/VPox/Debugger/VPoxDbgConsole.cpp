...
/*static*/ const uint32_t VPoxDbgConsoleOutput::s_uMinFontSize = 6;


VPoxDbgConsoleOutput::VPoxDbgConsoleOutput(QWidget *pParent/* = NULL*/, IVirtualPox *a_pVirtualPox /* = NULL */,
                                           const char *pszName/* = NULL*/)
    : QTextEdit(pParent), m_uCurLine(0), m_uCurPos(0), m_hGUIThread(RTThreadNativeSelf()), m_pVirtualPox(a_pVirtualPox)
{
    setReadOnly(true);
    ...
    /* color scheme: */
    com::Bstr bstrColor;
    HRESULT hrc = m_pVirtualPox ? m_pVirtualPox->GetExtraData(com::Bstr("DbgConsole/ColorScheme").raw(), bstrColor.asOutParam()) : E_FAIL;
    if (  SUCCEEDED(hrc)
    ...
    /* font: */
    com::Bstr bstrFont;
    hrc = m_pVirtualPox ? m_pVirtualPox->GetExtraData(com::Bstr("DbgConsole/Font").raw(), bstrFont.asOutParam()) : E_FAIL;
    if (  SUCCEEDED(hrc)
    ...
    hrc = m_pVirtualPox ? m_pVirtualPox->GetExtraData(com::Bstr("DbgConsole/FontSize").raw(), bstrFontSize.asOutParam()) : E_FAIL;
    if (SUCCEEDED(hrc))
    {
...
VPoxDbgConsoleOutput::~VPoxDbgConsoleOutput()
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
    if (m_pVirtualPox)
    {
        m_pVirtualPox->Release();
        m_pVirtualPox = NULL;
    }
}
...
void
VPoxDbgConsoleOutput::contextMenuEvent(QContextMenuEvent *pEvent)
{
    /*
...
void
VPoxDbgConsoleOutput::setColorScheme(VPoxDbgConsoleColor enmScheme, bool fSaveIt)
{
    ...
    if (m_pVirtualPox && fSaveIt)
        m_pVirtualPox->SetExtraData(com::Bstr("DbgConsole/ColorScheme").raw(), com::Bstr(pszSetting).raw());
}


void
VPoxDbgConsoleOutput::setFontType(VPoxDbgConsoleFontType enmFontType, bool fSaveIt)
{
    QFont Font = font();
    QAction *pAction;
    const char *pszSetting;
    ...
    /* Make this setting persistent. */
    if (m_pVirtualPox && fSaveIt)
        m_pVirtualPox->SetExtraData(com::Bstr("DbgConsole/Font").raw(), com::Bstr(pszSetting).raw());
}


void
VPoxDbgConsoleOutput::setFontSize(uint32_t uFontSize, bool fSaveIt)
{
    ...
    if (idxAction < (uint32_t)RT_ELEMENTS(m_apFontSizeActions))
    {
        ...
        if (fSaveIt && m_pVirtualPox)
            m_pVirtualPox->SetExtraData(com::Bstr("DbgConsole/FontSize").raw(), com::BstrFmt("%u", uFontSize).raw());
    }
}


void
VPoxDbgConsoleOutput::sltSelectColorScheme()
{
    QAction *pAction = qobject_cast<QAction *>(sender());
    if (pAction)
        setColorScheme((VPoxDbgConsoleColor)pAction->data().toInt(), true /*fSaveIt*/);
}


void
VPoxDbgConsoleOutput::sltSelectFontType()
{
    QAction *pAction = qobject_cast<QAction *>(sender());
    if (pAction)
        setFontType((VPoxDbgConsoleFontType)pAction->data().toInt(), true /*fSaveIt*/);
}


void
VPoxDbgConsoleOutput::sltSelectFontSize()
{
    QAction *pAction = qobject_cast<QAction *>(sender());
    if (pAction)
...
void
VPoxDbgConsoleOutput::appendText(const QString &rStr, bool fClearSelection)
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
...
VPoxDbgConsoleInput::VPoxDbgConsoleInput(QWidget *pParent/* = NULL*/, const char *pszName/* = NULL*/)
...
VPoxDbgConsoleInput::~VPoxDbgConsoleInput()
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
}
...
void
VPoxDbgConsoleInput::setLineEdit(QLineEdit *pEdit)
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
...
void
VPoxDbgConsoleInput::returnPressed()
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
...
VPoxDbgConsole::VPoxDbgConsole(VPoxDbgGui *a_pDbgGui, QWidget *a_pParent/* = NULL*/, IVirtualPox *a_pVirtualPox/* = NULL */)
    : VPoxDbgBaseWindow(a_pDbgGui, a_pParent, "Console"), m_pOutput(NULL), m_pInput(NULL), m_fInputRestoreFocus(false),
    ...
    m_fTerminate(false), m_fThreadTerminated(false)
{
     ...
     */
    m_pOutput = new VPoxDbgConsoleOutput(this, a_pVirtualPox);
    ...
    m_pInput = new VPoxDbgConsoleInput(NULL);
...
# endif
    ...
    rc = RTThreadCreate(&m_Thread, backThread, this, 0, RTTHREADTYPE_DEBUGGER, RTTHREADFLAGS_WAITABLE, "VPoxDbgC");
    AssertRC(rc);
    if (RT_FAILURE(rc))
...
void
VPoxDbgConsole::commandSubmitted(const QString &rCommand)
{
    Assert(isGUIThread());
    ...
    Log(("VPoxDbgConsole::commandSubmitted: %s (input-enabled=%RTbool)\n", psz, m_pInput->isEnabled()));
    unlock();
}


void
VPoxDbgConsole::updateOutput()
{
    Assert(isGUIThread());
...
VPoxDbgConsole::backInput(PDBGCBACK pBack, uint32_t cMillies)
{
    VPoxDbgConsole *pThis = VPOXDBGCONSOLE_FROM_DBGCBACK(pBack);
    pThis->lock();
...
VPoxDbgConsole::backRead(PDBGCBACK pBack, void *pvBuf, size_t cbBuf, size_t *pcbRead)
{
    VPoxDbgConsole *pThis = VPOXDBGCONSOLE_FROM_DBGCBACK(pBack);
 ...
 */
/*static*/ DECLCALLBACK(int)
VPoxDbgConsole::backWrite(PDBGCBACK pBack, const void *pvBuf, size_t cbBuf, size_t *pcbWritten)
{
    VPoxDbgConsole *pThis = VPOXDBGCONSOLE_FROM_DBGCBACK(pBack);
    int rc = VINF_SUCCESS;
    ...
    if (!pThis->m_fUpdatePending)
        QApplication::postEvent(pThis, new VPoxDbgConsoleEvent(VPoxDbgConsoleEvent::kUpdate));
...
VPoxDbgConsole::backSetReady(PDBGCBACK pBack, bool fReady)
{
    VPoxDbgConsole *pThis = VPOXDBGCONSOLE_FROM_DBGCBACK(pBack);
    if (fReady)
        QApplication::postEvent(pThis, new VPoxDbgConsoleEvent(VPoxDbgConsoleEvent::kInputEnable));
}
 ...
 */
/*static*/ DECLCALLBACK(int)
VPoxDbgConsole::backThread(RTTHREAD Thread, void *pvUser)
{
    VPoxDbgConsole *pThis = (VPoxDbgConsole *)pvUser;
    ...
    if (!ASMAtomicUoReadBool(&pThis->m_fTerminate))
        QApplication::postEvent(pThis, new VPoxDbgConsoleEvent(rc == VINF_SUCCESS
                                                               ? VPoxDbgConsoleEvent::kTerminatedUser
                                                               : VPoxDbgConsoleEvent::kTerminatedOther));
...
bool
VPoxDbgConsole::event(QEvent *pGenEvent)
{
    Assert(isGUIThread());
    if (pGenEvent->type() == (QEvent::Type)VPoxDbgConsoleEvent::kEventNumber)
    {
        VPoxDbgConsoleEvent *pEvent = (VPoxDbgConsoleEvent *)pGenEvent;

        switch (pEvent->command())
        {
            /* make update pending. */
            case VPoxDbgConsoleEvent::kUpdate:
                lock();
                if (!m_fUpdatePending)
                {
            ...
            case VPoxDbgConsoleEvent::kInputEnable:
                Log(("VPoxDbgConsole: kInputEnable (input-enabled=%RTbool)\n", m_pInput->isEnabled()));
            ...
            case VPoxDbgConsoleEvent::kTerminatedUser:
                Log(("VPoxDbgConsole: kTerminatedUser (input-enabled=%RTbool)\n", m_pInput->isEnabled()));
                m_pInput->setEnabled(false);
                close();
                break;

            /* The thread terminated for some unknown reason., disable input */
            case VPoxDbgConsoleEvent::kTerminatedOther:
                Log(("VPoxDbgConsole: kTerminatedOther (input-enabled=%RTbool)\n", m_pInput->isEnabled()));
                m_pInput->setEnabled(false);
                break;
    ...
    return VPoxDbgBaseWindow::event(pGenEvent);
}


void
VPoxDbgConsole::keyReleaseEvent(QKeyEvent *pEvent)
{
    //RTAssertMsg2("VPoxDbgConsole::keyReleaseEvent: %d (%#x); mod=%#x\n", pEvent->key(), pEvent->key(), pEvent->modifiers());
    switch (pEvent->key())
    {
        case Qt::Key_F5:
...
void
VPoxDbgConsole::closeEvent(QCloseEvent *a_pCloseEvt)
{
...
void
VPoxDbgConsole::actFocusToInput()
{
...
void
VPoxDbgConsole::actFocusToOutput()
{
