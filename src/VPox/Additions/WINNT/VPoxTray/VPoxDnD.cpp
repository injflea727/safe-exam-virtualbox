/* $Id: VPoxDnD.cpp $ */
/** @file
 * VPoxDnD.cpp - Windows-specific bits of the drag and drop service.
 */
...
#define VPOX_DND_WND_CLASS            "VPoxTrayDnDWnd"

/** @todo Merge this with messages from VPoxTray.h. */
#define WM_VPOXTRAY_DND_MESSAGE       WM_APP + 401
...
static VPOXDNDCONTEXT           g_Ctx = { 0 };


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static LRESULT CALLBACK vpoxDnDWndProcInstance(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
static LRESULT CALLBACK vpoxDnDWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);




VPoxDnDWnd::VPoxDnDWnd(void)
    : m_hThread(NIL_RTTHREAD),
      m_EvtSem(NIL_RTSEMEVENT),
      m_hWnd(NULL),
      m_lstActionsAllowed(VPOX_DND_ACTION_IGNORE),
      ...
      m_enmState(Uninitialized)
{
    ...
    const RTCString arrEntries[] = { VPOX_DND_FORMATS_DEFAULT };
 ...
 */
int VPoxDnDWnd::Initialize(PVPOXDNDCONTEXT a_pCtx)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = RTThreadCreate(&m_hThread, VPoxDnDWnd::Thread, this,
...
void VPoxDnDWnd::Destroy(void)
{
    ...
    {
        UnregisterClass(VPOX_DND_WND_CLASS, m_pCtx->pEnv->hInstance);
        m_pCtx->wndClass = 0;
    }
 ...
 * @param   pvUser                  Pointer to VPoxDnDWnd instance which
 *                                  is using the thread.
 */
/*static*/ DECLCALLBACK(int) VPoxDnDWnd::Thread(RTTHREAD hThread, void *pvUser)
{
    AssertPtrReturn(pvUser, VERR_INVALID_POINTER);

    LogFlowFuncEnter();

    VPoxDnDWnd *pThis = (VPoxDnDWnd*)pvUser;
    AssertPtr(pThis);

    PVPOXDNDCONTEXT m_pCtx = pThis->m_pCtx;
    AssertPtr(m_pCtx);
    ...
    if (!GetClassInfoEx(hInstance, VPOX_DND_WND_CLASS, &wc))
    {
        wc.lpfnWndProc   = vpoxDnDWndProc;
        wc.lpszClassName = VPOX_DND_WND_CLASS;
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        pThis->m_hWnd = CreateWindowEx(dwExStyle,
                                       VPOX_DND_WND_CLASS, VPOX_DND_WND_CLASS,
                                       dwStyle,
 ...
 */
/* static */
BOOL CALLBACK VPoxDnDWnd::MonitorEnumProc(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM lParam)
{
 ...
 */
LRESULT CALLBACK VPoxDnDWnd::WndProc(HWND a_hWnd, UINT a_uMsg, WPARAM a_wParam, LPARAM a_lParam)
{
    switch (a_uMsg)
    {
        case WM_CREATE:
        ...
        case WM_MOUSEMOVE:
        {
            ...
            {
                /* Starting here VPoxDnDDropTarget should
        ...
        case WM_VPOXTRAY_DND_MESSAGE:
        {
            PVPOXDNDEVENT pEvent = (PVPOXDNDEVENT)a_lParam;
            if (!pEvent)
 ...
 * @return  IPRT status code.
 */
int VPoxDnDWnd::RegisterAsDropTarget(void)
{
    ...
    try
    {
        m_pDropTarget = new VPoxDnDDropTarget(this /* pParent */);
 ...
 * @return  IPRT status code.
 */
int VPoxDnDWnd::UnregisterAsDropTarget(void)
{
    LogFlowFuncEnter();
 ...
 */
void VPoxDnDWnd::OnDestroy(void)
{
 ...
 */
int VPoxDnDWnd::OnHgEnter(const RTCList<RTCString> &a_lstFormats, VPOXDNDACTIONLIST a_fDndLstActionsAllowed)
{
    ...
    try
    {
        if (g_cVerbosity)
        {
            ...
            hlpShowBalloonTip(g_hInstance, g_hwndToolWindow, ID_TRAYICON,
                              strMsg.c_str(), "VirtualPox Drag'n Drop",
        ...
        {
            if (a_fDndLstActionsAllowed & VPOX_DND_ACTION_COPY)
                m_startupInfo.dwOKEffects |= DROPEFFECT_COPY;
            if (a_fDndLstActionsAllowed & VPOX_DND_ACTION_MOVE)
                m_startupInfo.dwOKEffects |= DROPEFFECT_MOVE;
            if (a_fDndLstActionsAllowed & VPOX_DND_ACTION_LINK)
                m_startupInfo.dwOKEffects |= DROPEFFECT_LINK;
        }

        LogRel2(("DnD: Supported drop actions: 0x%x\n", m_startupInfo.dwOKEffects));

        m_startupInfo.pDropSource = new VPoxDnDDropSource(this);
        m_startupInfo.pDataObject = new VPoxDnDDataObject(pFormatEtc, pStgMeds, cFormatsActive);
 ...
 *                                  Currently ignored.
 */
int VPoxDnDWnd::OnHgMove(uint32_t u32xPos, uint32_t u32yPos, VPOXDNDACTION dndAction)
{
    RT_NOREF(dndAction);
    int rc;

    uint32_t uActionNotify = VPOX_DND_ACTION_IGNORE;
    if (m_enmMode == HG)
    {
...
int VPoxDnDWnd::OnHgLeave(void)
{
    ...
    if (g_cVerbosity)
        hlpShowBalloonTip(g_hInstance, g_hwndToolWindow, ID_TRAYICON,
                          "Leave: Host -> Guest", "VirtualPox Drag'n Drop",
                          15 * 1000 /* Time to display in msec */, NIIF_INFO);
...
int VPoxDnDWnd::OnHgDrop(void)
{
    ...
    if (m_enmState == Dragging)
    {
        if (g_cVerbosity)
            hlpShowBalloonTip(g_hInstance, g_hwndToolWindow, ID_TRAYICON,
                              "Drop: Host -> Guest", "VirtualPox Drag'n Drop",
                              15 * 1000 /* Time to display in msec */, NIIF_INFO);
        ...
        if (m_lstFmtActive.size() >= 1)
        {
            ...
            {
                if (m_startupInfo.pDataObject)
                    m_startupInfo.pDataObject->SetStatus(VPoxDnDDataObject::Status_Dropping);
                else
                    rc = VERR_NOT_FOUND;
 ...
 */
int VPoxDnDWnd::OnHgDataReceive(PVBGLR3GUESTDNDMETADATA pMeta)
{
 ...
 * @return  IPRT status code.
 */
int VPoxDnDWnd::OnHgCancel(void)
{
    return Abort();
}
 ...
 * @return  IPRT status code.
 */
int VPoxDnDWnd::OnGhIsDnDPending(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            dndActionDefault = VPOX_DND_ACTION_COPY;
        ...
        if (RT_FAILURE(rc2))
        {
            ...
            RTStrPrintf(szTitle, sizeof(szTitle), "VirtualPox Guest Additions Drag and Drop");
            RTStrPrintf(szMsg, sizeof(szMsg), "Drag and drop to the host either is not supported or disabled. "
                                              ...
                                              "or re-install the VirtualPox Guest Additions.");
            switch (rc2)
            {
 ...
 */
int VPoxDnDWnd::OnGhDrop(const RTCString &strFormat, uint32_t dndActionDefault)
{
...
void VPoxDnDWnd::PostMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
 ...
 */
int VPoxDnDWnd::ProcessEvent(PVPOXDNDEVENT pEvent)
{
    AssertPtrReturn(pEvent, VERR_INVALID_POINTER);

    BOOL fRc = ::PostMessage(m_hWnd, WM_VPOXTRAY_DND_MESSAGE,
 ...
 */
int VPoxDnDWnd::checkForSessionChange(void)
{
    uint64_t uSessionID;
 ...
 * @return  IPRT status code.
 */
int VPoxDnDWnd::makeFullscreen(void)
{
    int rc = VINF_SUCCESS;
    ...
    if (hDC)
    {
        fRc = g_pfnEnumDisplayMonitors
            ...
            ? g_pfnEnumDisplayMonitors(hDC, NULL, VPoxDnDWnd::MonitorEnumProc, (LPARAM)&r):
              FALSE;
 ...
 */
int VPoxDnDWnd::mouseMove(int x, int y, DWORD dwMouseInputFlags)
{
    int iScreenX = GetSystemMetrics(SM_CXSCREEN) - 1;
    int iScreenY = GetSystemMetrics(SM_CYSCREEN) - 1;
 ...
 * @return  IPRT status code.
 */
int VPoxDnDWnd::mouseRelease(void)
{
    LogFlowFuncEnter();
...
void VPoxDnDWnd::Reset(void)
{
    ...
    this->m_lstActionsAllowed = VPOX_DND_ACTION_IGNORE;
 ...
 */
int VPoxDnDWnd::setMode(Mode enmMode)
{
...
static LRESULT CALLBACK vpoxDnDWndProcInstance(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    VPoxDnDWnd *pWnd = reinterpret_cast<VPoxDnDWnd *>(pUserData);
    if (pWnd)
...
static LRESULT CALLBACK vpoxDnDWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (uMsg == WM_NCCREATE)
    {
        ...
        SetWindowLongPtr(hWnd, GWLP_WNDPROC, (LONG_PTR)vpoxDnDWndProcInstance);

        return vpoxDnDWndProcInstance(hWnd, uMsg, wParam, lParam);
    }
 ...
 */
DECLCALLBACK(int) VPoxDnDInit(const PVPOXSERVICEENV pEnv, void **ppInstance)
{
    ...
    PVPOXDNDCONTEXT pCtx = &g_Ctx; /* Only one instance at the moment. */
    AssertPtr(pCtx);
...
DECLCALLBACK(int) VPoxDnDStop(void *pInstance)
{
    AssertPtrReturn(pInstance, VERR_INVALID_POINTER);

    LogFunc(("Stopping pInstance=%p\n", pInstance));

    PVPOXDNDCONTEXT pCtx = (PVPOXDNDCONTEXT)pInstance;
    AssertPtr(pCtx);
...
DECLCALLBACK(void) VPoxDnDDestroy(void *pInstance)
{
    ...
    VPoxDnDWnd *pWnd = pCtx->lstWnd.first();
    if (pWnd)
    {
        delete pWnd;
        pWnd = NULL;
...
DECLCALLBACK(int) VPoxDnDWorker(void *pInstance, bool volatile *pfShutdown)
{
    AssertPtr(pInstance);
    ...
    PVPOXDNDEVENT pEvent = NULL;

    for (;;)
    {
        pEvent = (PVPOXDNDEVENT)RTMemAllocZ(sizeof(VPOXDNDEVENT));
        if (!pEvent)
        {
            rc = VERR_NO_MEMORY;
            break;
