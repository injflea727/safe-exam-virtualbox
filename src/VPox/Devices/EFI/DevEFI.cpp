/* $Id: DevEFI.cpp $ */
/** @file
 * DevEFI - EFI <-> VirtualPox Integration Framework.
 */
...
# define DEVEFI_WITH_VPOXDBG_SCRIPT
#endif
...
typedef struct DEVEFIR3
{
    ...
    /** EFI message buffer. */
    char                    szMsg[VPOX_EFI_DEBUG_BUFFER];
...
/** The saved state version from VPox 4.2. */
#define EFI_SSM_VERSION_4_2              1

/** Non-volatile EFI variable. */
#define VPOX_EFI_VARIABLE_NON_VOLATILE  UINT32_C(0x00000001)
/** Non-volatile EFI variable. */
#define VPOX_EFI_VARIABLE_READ_ONLY     UINT32_C(0x00000008)
...
static const char g_szEfiBuiltin32[] = "VPoxEFI32.fd";
/** Special file name value for indicating the 64-bit built-in EFI firmware. */
static const char g_szEfiBuiltin64[] = "VPoxEFI64.fd";
# endif
#endif /* IN_RING3 */
...
static int nvramStore(PDEVEFIR3 pThisCC)
{
    ...
    RTListForEach(&pThisCC->NVRAM.VarList, pEfiVar, EFIVAR, ListNode)
        if (pEfiVar->fAttributes & VPOX_EFI_VARIABLE_NON_VOLATILE)
            cNonVolatile++;
...
static uint8_t efiInfoNextByte(PDEVEFIR3 pThisCC)
{
    switch (pThisCC->iInfoSelector)
    {
        ...
        case EFI_INFO_INDEX_TEMPMEM_BASE:       return efiInfoNextByteU32(pThisCC, VPOX_EFI_TOP_OF_STACK); /* just after stack */
        ...
        case EFI_INFO_INDEX_STACK_BASE:         return efiInfoNextByteU32(pThisCC,  VPOX_EFI_TOP_OF_STACK - _128K); /* 2M - 128 K */
...
#ifdef IN_RING3
static void efiVPoxDbgScript(const char *pszFormat, ...)
{
# ifdef DEVEFI_WITH_VPOXDBG_SCRIPT
    PRTSTREAM pStrm;
    int rc2 = RTStrmOpen("./DevEFI.VPoxDbg", "a", &pStrm);
    if (RT_SUCCESS(rc2))
    {
        va_list va;
 ...
 *
 * @returns VPox status suitable for I/O port write handler.
 *
 * @param   pThisCC             The EFI state for the current context.
...
# ifdef IN_RING3
            ...
            if (s_cImageEvtLogged < 2048)
            {
                ...
                switch (pThisCC->ImageEvt.uEvt)
                {
                    ...
                    case EFI_IMAGE_EVT_CMD_START_LOAD32:
                        LogRel(("EFI: VPoxDbg> loadimage32 '%.*s.efi' %#llx LB %#llx\n",
                        ...
                        if (pThisCC->ImageEvt.offName > 4)
                            efiVPoxDbgScript("loadimage32 '%.*s.efi' %#llx\n",
                                             pThisCC->ImageEvt.offName - 4, pThisCC->ImageEvt.szName, pThisCC->ImageEvt.uAddr0);
                        break;
                    case EFI_IMAGE_EVT_CMD_START_LOAD64:
                        LogRel(("EFI: VPoxDbg> loadimage64 '%.*s.efi' %#llx LB %#llx\n",
                                pThisCC->ImageEvt.offName - 4, pThisCC->ImageEvt.szName, pThisCC->ImageEvt.uAddr0, pThisCC->ImageEvt.cb0));
                        if (pThisCC->ImageEvt.offName > 4)
                            efiVPoxDbgScript("loadimage64 '%.*s.efi' %#llx\n",
                                             pThisCC->ImageEvt.offName - 4, pThisCC->ImageEvt.szName, pThisCC->ImageEvt.uAddr0);
                        break;
                    case EFI_IMAGE_EVT_CMD_START_UNLOAD32:
                    case EFI_IMAGE_EVT_CMD_START_UNLOAD64:
                    {
                        LogRel(("EFI: VPoxDbg> unload '%.*s.efi' # %#llx LB %#llx\n",
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) efiR3IoPortRead(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t *pu32, unsigned cb)
{
    RT_NOREF(pvUser);
    PDEVEFIR3 pThisCC = PDMDEVINS_2_DATA_CC(pDevIns, PDEVEFIR3);
    Log4(("EFI in: %x %x\n", offPort, cb));
 ...
 * @note The @a offPort parameter is absolute!
 */
static DECLCALLBACK(VPOXSTRICTRC) efiR3IoPortWrite(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT offPort, uint32_t u32, unsigned cb)
{
    RT_NOREF(pvUser);
    PDEVEFIR3 pThisCC = PDMDEVINS_2_DATA_CC(pDevIns, PDEVEFIR3);
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) efiR3NvMmioWrite(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void const *pv, unsigned cb)
{
 ...
 */
static DECLCALLBACK(VPOXSTRICTRC) efiR3NvMmioRead(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS off, void *pv, unsigned cb)
{
    PDEVEFI pThis = PDMDEVINS_2_DATA(pDevIns, PDEVEFI);
    RT_NOREF(pvUser);
...
static DECLCALLBACK(int)  efiConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    if (rc == VERR_CFGM_VALUE_NOT_FOUND)
    {
        ...
        AssertRCReturn(rc, rc);
        rc = RTPathAppend(pThisCC->pszEfiRomFile, RTPATH_MAX, "VPoxEFI32.fd");
        AssertRCReturn(rc, rc);
    }
     ...
     */
    rc = FwCommonPlantDMITable(pDevIns, pThisCC->au8DMIPage, VPOX_DMI_TABLE_SIZE, &pThisCC->aUuid,
                               pDevIns->pCfg, pThisCC->cCpus, &pThisCC->cbDmiTables, &pThisCC->cNumDmiTables,
                               true /*fUefi*/);
    AssertRCReturn(rc, rc);

    /*
     * NB: VPox/Devices/EFI/Firmware/VPoxPkg/VPoxSysTables/VPoxSysTables.c scans memory for
     * the SMBIOS header. The header must be placed in a range that EFI will scan.
     */
    FwCommonPlantSmbiosAndDmiHdrs(pDevIns, pThisCC->au8DMIPage + VPOX_DMI_TABLE_SIZE,
                                  pThisCC->cbDmiTables, pThisCC->cNumDmiTables);

    if (pThisCC->u8IOAPIC)
    {
        FwCommonPlantMpsTable(pDevIns,
                              pThisCC->au8DMIPage /* aka VPOX_DMI_TABLE_BASE */ + VPOX_DMI_TABLE_SIZE + VPOX_DMI_HDR_SIZE,
                              _4K - VPOX_DMI_TABLE_SIZE - VPOX_DMI_HDR_SIZE, pThisCC->cCpus);
        FwCommonPlantMpsFloatPtr(pDevIns, VPOX_DMI_TABLE_BASE + VPOX_DMI_TABLE_SIZE + VPOX_DMI_HDR_SIZE);
    }

    rc = PDMDevHlpROMRegister(pDevIns, VPOX_DMI_TABLE_BASE, _4K, pThisCC->au8DMIPage, _4K,
