...
The VirtualPox Validation Kit is our new public tool for doing automated
testing of VirtualPox.  We are continually working on adding new features
...
./testdriver/
    ...
    the VPox centric specialization is in ./testdriver/vpox.py. Various VPox
    API wrappers that makes things easier to use and glosses over a lot of API
    version differences that live in ./testdriver/vpoxwrappers.py.

    Test VM collections are often managed thru ./testdriver/vpoxtestvms.py, but
    ...
    There is also a VPoxTXS client implementation in txsclient.py and a stacked
    test driver for installing VPox (vpoxinstaller.py).  Most test drivers will
    use the TXS client indirectly thru vpox.py methods.  The installer driver
...
./tests/
    ...
    vpox.py of course).  Most tests use one or more VMs from a standard set of
    preconfigured VMs defined by ./testdriver/vpoxtestvms.py (mentioned above),
...
./vms/
    ...
    ./testdrive/vpoxtestvms.py.  This will also contain description of how to
...
./utils/
    ...
    zip and on the VPoxValidationKit.iso respectively.

    The Test eXecution Service (VPoxTXS) found in ./utils/TestExecServ is one
    ...
    ./utils/TestExecServ/vpoxtxs-readme.txt for more details.
...
./testmanager/
    The VirtualPox Test Manager (server side code).  This is written in Python
    ...
    things in there that are of more use when testing VirtualPox than other
...
:Status: $Id: VPoxValidationKitReadMe.txt $
