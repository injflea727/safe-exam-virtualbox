...
/** @file
 * VPox Qt GUI - UIFDCreationDialog class implementation.
 */
...
QString UIFDCreationDialog::getDefaultFolder() const
{
    ...
    if (strInitialPath.isEmpty())
        strInitialPath = uiCommon().virtualPox().GetSystemProperties().GetDefaultMachineFolder();
...
void UIFDCreationDialog::accept()
{
    ...
    CMedium newMedium = vpox.CreateMedium(mediumFormats[0].GetName(), strMediumLocation,
                                          KAccessMode_ReadWrite, KDeviceType_Floppy);
    if (!vpox.isOk())
    {
        msgCenter().cannotCreateMediumStorage(vpox, strMediumLocation, this);
        return;
    }
