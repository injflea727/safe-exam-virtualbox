...
/** @file
 * VirtualPox Support Library - Hardened main().
 */
...
/** @page pg_hardening      %VirtualPox %VM Process Hardening
 *
 * The %VM process hardening is to prevent malicious software from using
 * %VirtualPox as a vehicle to obtain kernel level access.
 *
 * The %VirtualPox %VMM requires supervisor (kernel) level access to the CPU.
 ...
 * VPoxDrv or vpoxdrv depending on the OS, as it is ultimately the link and
 ...
 *      - /dev/vpoxdrv (win: \\Device\\VPoxDrv) for full unrestricted access.
 *        Offers a rich I/O control interface, which needs protecting.
 *      - /dev/vpoxdrvu (win: \\Device\\VPoxDrvU) for restricted access, which
 *        VPoxSVC uses to query VT-x and AMD-V capabilities.  This does not
 *        require protecting, though we limit it to the vpoxgroup on some
 *        systems.
 *      - \\Device\\VPoxDrvStub on Windows for protecting the second stub
 *        process and its child, the %VM process.  This is an open+close
 *        interface, only available to partially verified stub processes.
 *      - \\Device\\VPoxDrvErrorInfo on Windows for obtaining detailed error
 *        information on a previous attempt to open \\Device\\VPoxDrv or
 *        \\Device\\VPoxDrvStub.  Open, read and close only interface.
 *
 * The rest of VPox accesses the device interface thru the support library,
 * @ref grp_sup "SUPR3" / sup.h.
 *
 * The support driver also exposes a set of functions and data that other VPox
 ...
 * both translates to a sequence of I/O controls against /dev/vpoxdrv.  On
 ...
 * The device node corresponding to unrestricted access (/dev/vpoxdrv) is own by
 ...
 *      1. Validate the VirtualPox installation (#supR3HardenedVerifyAll):
 *          - Check that the executable file of the process is one of the known
 *            VirtualPox executables.
 ...
 *
 *      5. Load the VPoxRT dynamic link library and hand over the file
 ...
 *
 * The VirtualPox installation location is hardcoded, which means the any
 ...
 *
 * In addition to what the dynamic linker does for us, the VirtualPox code will
 ...
 * (\\Device\\VPoxDrvStub), that is a special support driver device node that
 ...
 *        DLL sections) in it.
 *      - Check that the process executable is a known VPox executable which may
 ...
 * has opened \\Device\\VPoxDrvSub, it is protected from malicious OpenProcess &
 ...
 * \\Device\\VPoxDrvStub from #supR3HardenedEarlyProcessInit, it opens the
 * support driver for full unrestricted access, i.e. \\Device\\VPoxDrv.
 *
 * The support driver will perform the same checks as it did when
 * \\Device\\VPoxDrvStub was opened, but in addition it will:
 ...
 * do unrevertable changes to the process before this point, VirtualPox won't
 * work.
 *
 ...
 * recheck the VirtualPox installation, keeping all known files open just like
...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
/** The pre-init data we pass on to SUPR3 (residing in VPoxRT). */
...
#ifdef RT_OS_WINDOWS
/** Pointer to VPoxRT's RTLogRelPrintf function so we can write errors to the
    ...
    switch (g_fSupHardenedMain & SUPSECMAIN_FLAGS_LOC_MASK)
    {
        ...
        case SUPSECMAIN_FLAGS_LOC_OSX_HLP_APP:
        {
            ...
            for (uintptr_t i = 0; i < RT_ELEMENTS(s_aComponentsToSkip); i++)
            {
                ...
                if (cchMatch > 0)
                {
                    if (   cchPath >= cchMatch + sizeof("VirtualPox.app/Contents")
                ...
                else if (   cchPath > g_cchSupLibHardenedExecName  + sizeof("VirtualPox.app/Contents/Resources/.app")
        ...
        if (strncmp(papszArgs[iArg], s_szLogOption, sizeof(s_szLogOption) - 1) == 0)
        {
            ...
            *pcArgs -= 1;
            ...
            if (g_hStartupLog == NULL)
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    SUP_DPRINTF(("Log file opened: " VPOX_VERSION_STRING "r%u g_hStartupLog=%p g_uNtVerCombined=%#x\n",
                                 VPOX_SVN_REV, g_hStartupLog, g_uNtVerCombined));
                                                        ...
                                                        const char *pszMsgFmt, va_list va)
{
    ...
    switch (enmWhat)
    {
        case kSupInitOp_Driver:
            suplibHardenedPrintChr('\n');
            suplibHardenedPrintPrefix();
            suplibHardenedPrintStr("Tip! Make sure the kernel module is loaded. It may also help to reinstall VirtualPox.\n");
            break;
        ...
        case kSupInitOp_RootCheck:
            suplibHardenedPrintChr('\n');
            suplibHardenedPrintPrefix();
            suplibHardenedPrintStr("Tip! It may help to reinstall VirtualPox.\n");
            break;
...
/**
 * Attempts to open /dev/vpoxdrv (or equvivalent).
 *
...
# ifdef RT_OS_LINUX
    ...
    if (getuid() != 0)
    {
         ...
         * CAP_NET_RAW.
         * Default: enabled.
         * Can be disabled with 'export VPOX_HARD_CAP_NET_RAW=0'.
         */
        const char *pszOpt = getenv("VPOX_HARD_CAP_NET_RAW");
        if (   !pszOpt
         ...
         * Default: disabled.
         * Can be enabled with 'export VPOX_HARD_CAP_NET_BIND_SERVICE=1'.
         */
        pszOpt = getenv("VPOX_HARD_CAP_NET_BIND_SERVICE");
        if (   pszOpt
            && memcmp(pszOpt, "0", sizeof("0")) != 0)
         ...
         * CAP_SYS_NICE.
         * Default: enabled.
         * Can be disabled with 'export VPOX_HARD_CAP_SYS_NICE=0'.
         */
        pszOpt = getenv("VPOX_HARD_CAP_SYS_NICE");
        if (   !pszOpt
            || memcmp(pszOpt, "0", sizeof("0")) != 0)
...
/**
 * Loads the VPoxRT DLL/SO/DYLIB, hands it the open driver,
 * and calls RTR3InitEx.
 *
 * @param   fFlags      The SUPR3HardenedMain fFlags argument, passed to supR3PreInit.
 *
 * @remarks VPoxRT contains both IPRT and SUPR3.
 * @remarks This function will not return on failure.
 */
static void supR3HardenedMainInitRuntime(uint32_t fFlags)
{
     ...
     */
    char szPath[RTPATH_MAX];
    supR3HardenedPathAppSharedLibs(szPath, sizeof(szPath) - sizeof("/VPoxRT" SUPLIB_DLL_SUFF));
    suplibHardenedStrCat(szPath, "/VPoxRT" SUPLIB_DLL_SUFF);
     ...
     *          process terminates (VPoxNetDHCP, Ctrl-C, etc).
     */
 ...
 * and when using the open-vpoxdrv-via-root-service setup on Windows.
 *
 * This function will perform the integrity checks of the VirtualPox
...
DECLHIDDEN(int) SUPR3HardenedMain(const char *pszProgName, uint32_t fFlags, int argc, char **argv, char **envp)
{
    ...
    if (!(fFlags & SUPSECMAIN_FLAGS_DONT_OPEN_DEV))
    {
         ...
         * with vpoxdrv to monitor access to the new process from its inception.
         */
        ...
        SUP_DPRINTF(("SUPR3HardenedMain: Final process, opening VPoxDrv...\n"));
        supR3HardenedWinFlushLoaderCache();

#else
        /*
         * Open the vpoxdrv device.
         */
        supR3HardenedMainOpenDevice();
#endif /* !RT_OS_WINDOWS */
    }
