/* $Id: VPoxAboutDlg.cpp $ */
/** @file
 * VPox Qt GUI - VPoxAboutDlg class implementation.
 */
...
/* Other VPox includes: */
#include <iprt/path.h>
#include <VPox/version.h> /* VPOX_VENDOR */


VPoxAboutDlg::VPoxAboutDlg(QWidget *pParent, const QString &strVersion)
#ifdef VPOX_WS_MAC
...
bool VPoxAboutDlg::event(QEvent *pEvent)
{
...
void VPoxAboutDlg::paintEvent(QPaintEvent *)
{
    /* Draw About-VirtualPox background image: */
    QPainter painter(this);
    painter.drawPixmap(0, 0, m_pixmap);
}

void VPoxAboutDlg::retranslateUi()
{
    setWindowTitle(tr("VirtualPox - About"));
    const QString strAboutText = tr("VirtualPox Graphical User Interface");
#ifdef VPOX_BLEEDING_EDGE
    const QString strVersionText = "EXPERIMENTAL build %1 - " + QString(VPOX_BLEEDING_EDGE);
#else
    const QString strVersionText = tr("Version %1");
#endif
#ifdef VPOX_OSE
    m_strAboutText = strAboutText + " " + strVersionText.arg(m_strVersion) + "\n"
                   + QString("%1 2004-" VPOX_C_YEAR " " VPOX_VENDOR).arg(QChar(0xa9));
#else
    ...
    m_strAboutText = m_strAboutText + "\n" + QString("Copyright %1 %2 %3 and/or its affiliates. All rights reserved.")
                                                     .arg(QChar(0xa9)).arg(VPOX_C_YEAR).arg(VPOX_VENDOR);
    AssertPtrReturnVoid(m_pLabel);
    m_pLabel->setText(m_strAboutText);
}

void VPoxAboutDlg::prepare()
{
    ...
    if (m_pPseudoParent)
        connect(m_pPseudoParent, &QObject::destroyed, this, &VPoxAboutDlg::close);
...
void VPoxAboutDlg::prepareMainLayout()
{
    /* Create main-layout: */
...
void VPoxAboutDlg::prepareLabel()
{
...
void VPoxAboutDlg::prepareCloseButton()
{
    /* Create button-box: */
    ...
    if (pButtonBox)
    {
        ...
        /* Prepare close-button: */
        connect(pButtonBox, &QDialogButtonBox::rejected, this, &VPoxAboutDlg::reject);
