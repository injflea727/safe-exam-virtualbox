/** $Id: VPoxClientClipboardGuestToHost.cpp $ */
/** @file
 * VPoxClient - Shared Clipboard Guest -> Host copying, Darwin.
 */
...
#define VPOXCL_ADD_FORMAT_IF_PRESENT(a_kDarwinFmt, a_fVPoxFmt) \
    if (PasteboardCopyItemFlavorData(pPasteboard, iItemID, a_kDarwinFmt, &flavorData) == noErr) \
    { \
        fFormats |= (uint32_t)a_fVPoxFmt; \
    ...
    for (iItem = 1; iItem <= cItems; iItem++)
    {
        ...
        if (rc == noErr)
        {
            VPOXCL_ADD_FORMAT_IF_PRESENT(kUTTypeUTF16PlainText, VPOX_SHCL_FMT_UNICODETEXT);
            VPOXCL_ADD_FORMAT_IF_PRESENT(kUTTypeUTF8PlainText,  VPOX_SHCL_FMT_UNICODETEXT);
            VPOXCL_ADD_FORMAT_IF_PRESENT(kUTTypeBMP,            VPOX_SHCL_FMT_BITMAP     );
            VPOXCL_ADD_FORMAT_IF_PRESENT(kUTTypeHTML,           VPOX_SHCL_FMT_HTML       );
            ...
            if (rc == noErr)
            {
                VPoxClientVerbose(3, "SCAN..\n");
                flavorCount = CFArrayGetCount(flavorTypeArray);
                VPoxClientVerbose(3, "SCAN (%d)..\n", (int)flavorCount);
...
#undef VPOXCL_ADD_FORMAT_IF_PRESENT
...
static int vbclClipboardHostPasteText(uint32_t u32ClientId, PRTUTF16 pwszData, uint32_t cbData)
{
    ...
    if (RT_SUCCESS(rc))
        rc = vbclClipboardHostPasteData(u32ClientId, VPOX_SHCL_FMT_UNICODETEXT,
...
static int vbclClipboardHostPasteBitmap(uint32_t u32ClientId, void *pvData, uint32_t cbData)
{
    ...
    rc = vbclClipboardHostPasteData(u32ClientId, VPOX_SHCL_FMT_BITMAP, pvDib, cbDib);
...
int vbclClipboardForwardToHost(uint32_t u32ClientId, PasteboardRef pPasteboard, uint32_t fFormats)
{
    ...
    VPoxClientVerbose(3, "vbclClipboardForwardToHost: %d\n", fFormats);

    /* Walk across all item(s) formats */
    uint32_t  fFormatsLeft = fFormats;
    while (fFormatsLeft)
    {
        if (fFormatsLeft & VPOX_SHCL_FMT_UNICODETEXT)
        {
            VPoxClientVerbose(3, "requested VPOX_SHCL_FMT_UNICODETEXT: %d\n", fFormats);
            ...
            else
            {
                ...
                rc = vbclClipboardHostPasteData(u32ClientId, VPOX_SHCL_FMT_UNICODETEXT, NULL, 0);
            }

            fFormatsLeft &= ~(uint32_t)VPOX_SHCL_FMT_UNICODETEXT;
        }

        else if (fFormatsLeft & VPOX_SHCL_FMT_BITMAP)
        {
            VPoxClientVerbose(3, "requested VPOX_SHCL_FMT_BITMAP: %d\n", fFormats);
            ...
            {
                /* No data found or error occurred: send empty buffer */
                rc = vbclClipboardHostPasteData(u32ClientId, VPOX_SHCL_FMT_BITMAP, NULL, 0);
            }

            fFormatsLeft &= ~(uint32_t)VPOX_SHCL_FMT_BITMAP;
        }

        else if (fFormatsLeft & VPOX_SHCL_FMT_HTML)
        {
            VPoxClientVerbose(3, "requested VPOX_SHCL_FMT_HTML: %d\n", fFormats);

            rc = vbclClipboardReadGuestData(pPasteboard, kUTTypeHTML, &pvData, &cbData, &cbAlloc);
            if (RT_SUCCESS(rc))
            {
                rc = vbclClipboardHostPasteData(u32ClientId, VPOX_SHCL_FMT_HTML, pvData, cbData);
                vbclClipboardReleaseGuestData(&pvData, cbAlloc);
            }
            else
            {
                /* No data found or error occurred: send empty buffer */
                rc = vbclClipboardHostPasteData(u32ClientId, VPOX_SHCL_FMT_HTML, NULL, 0);
            }

            fFormatsLeft &= ~(uint32_t)VPOX_SHCL_FMT_HTML;
        }

        else
        {
            VPoxClientVerbose(3, "requested data in unsupported format: %#x\n", fFormatsLeft);
            break;
        }
    }
