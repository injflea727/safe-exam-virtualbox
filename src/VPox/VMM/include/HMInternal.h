...
#define HM_VTX_TSS_SIZE                             (sizeof(VPOXTSS) + 2 * PAGE_SIZE + 1)
...
typedef struct HMPHYSCPU
{
    ...
    bool                fConfigured;
    /** Set if the VPOX_HWVIRTEX_IGNORE_SVM_IN_USE hack is active. */
...
typedef struct HM
{
    ...
    struct
    {
        ...
        R3PTRTYPE(PVPOXTSS)         pRealModeTSS;
 ...
 *
 * @returns VPox status code (no informational stuff).
