...
/** @todo Docs */
static PVPOXSERVICEVEPROPCACHEENTRY vgsvcPropCacheFindInternal(PVPOXSERVICEVEPROPCACHE pCache, const char *pszName,
                                                               uint32_t fFlags)
{
    RT_NOREF1(fFlags);
     ...
     *        moment. */
    PVPOXSERVICEVEPROPCACHEENTRY pNode = NULL;
    if (RT_SUCCESS(RTCritSectEnter(&pCache->CritSect)))
    {
        PVPOXSERVICEVEPROPCACHEENTRY pNodeIt;
        RTListForEach(&pCache->NodeHead, pNodeIt, VPOXSERVICEVEPROPCACHEENTRY, NodeSucc)
        {
...
/** @todo Docs */
static PVPOXSERVICEVEPROPCACHEENTRY vgsvcPropCacheInsertEntryInternal(PVPOXSERVICEVEPROPCACHE pCache, const char *pszName)
{
    AssertPtrReturn(pCache, NULL);
    AssertPtrReturn(pszName, NULL);

    PVPOXSERVICEVEPROPCACHEENTRY pNode = (PVPOXSERVICEVEPROPCACHEENTRY)RTMemAlloc(sizeof(VPOXSERVICEVEPROPCACHEENTRY));
    if (pNode)
    {
...
int VGSvcPropCacheUpdateEntry(PVPOXSERVICEVEPROPCACHE pCache, const char *pszName, uint32_t fFlags, const char *pszValueReset)
{
    AssertPtrReturn(pCache, VERR_INVALID_POINTER);
    AssertPtrReturn(pszName, VERR_INVALID_POINTER);
    PVPOXSERVICEVEPROPCACHEENTRY pNode = vgsvcPropCacheFindInternal(pCache, pszName, 0);
    if (pNode == NULL)
...
void VGSvcPropCacheDestroy(PVPOXSERVICEVEPROPCACHE pCache)
{
    ...
    if (RT_SUCCESS(rc))
    {
        PVPOXSERVICEVEPROPCACHEENTRY pNode = RTListGetFirst(&pCache->NodeHead, VPOXSERVICEVEPROPCACHEENTRY, NodeSucc);
        while (pNode)
        {
            PVPOXSERVICEVEPROPCACHEENTRY pNext = RTListNodeIsLast(&pCache->NodeHead, &pNode->NodeSucc)
                                                                  ? NULL :
                                                                    RTListNodeGetNext(&pNode->NodeSucc,
                                                                                      VPOXSERVICEVEPROPCACHEENTRY, NodeSucc);
