...
#define VPOX_VIRTIO_NO_DEVICE 0xffff

#define VPOX_VIRTIO_NIL_CFG             0xff
...
int virtio_scsi_read_sectors(bio_dsk_t __far *bios_dsk)
{
    ...
    device_id = VPOX_GET_SCSI_DEVICE(bios_dsk->drqp.dev_id);
                                ...
                                uint16_t skip_b, uint32_t length, uint8_t inout, char __far *buffer)
{
    ...
    device_id = VPOX_GET_SCSI_DEVICE(device_id);
...
void BIOSCALL virtio_scsi_init(void)
{
    ...
    if (busdevfn != VPOX_VIRTIO_NO_DEVICE)
    {
        uint8_t     u8Bus, u8DevFn;
        uint8_t     u8PciCapOff;
        uint8_t     u8PciCapOffVirtIo = VPOX_VIRTIO_NIL_CFG;
        ...
        while (u8PciCapOff != 0)
        {
                ...
                && cbPciCap >= sizeof(virtio_pci_cap_t))
            {
                ...
                if (u8PciCapOffVirtIo == VPOX_VIRTIO_NIL_CFG)
