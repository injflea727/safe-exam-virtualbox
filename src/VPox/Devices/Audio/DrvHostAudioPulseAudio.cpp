...
#include <VPox/vmm/pdmaudiohostenuminline.h>
...
#define VPOX_PULSEAUDIO_MAX_LOG_REL_ERRORS  99
...
typedef struct DRVHSTAUDPAENUMCBCTX
{
    ...
    uint32_t                fFlags;
    /** VPox status code for the operation.
...
/**
 * Converts a pulse audio error to a VPox status.
 *
 * @returns VPox status code.
 * @param   rcPa    The error code to convert.
 */
static int drvHstAudPaErrorToVPox(int rcPa)
{
    /** @todo Implement some PulseAudio -> VPox mapping here. */
    RT_NOREF(rcPa);
...
/**
 * Logs a pulse audio (from context) and converts it to VPox status.
 *
 * @returns VPox status code.
...
static int drvHstAudPaError(PDRVHSTAUDPA pThis, const char *pszFormat, ...)
{
    ...
    int const rcVPox = drvHstAudPaErrorToVPox(rcPa);

    if (   pThis->cLogErrors < VPOX_PULSEAUDIO_MAX_LOG_REL_ERRORS
        && LogRelIs2Enabled())
    {
        va_list va;
        va_start(va, pszFormat);
        LogRel(("PulseAudio: %N: %s (%d, %Rrc)\n", pszFormat, &va, pa_strerror(rcPa), rcPa, rcVPox));
        va_end(va);

        if (++pThis->cLogErrors == VPOX_PULSEAUDIO_MAX_LOG_REL_ERRORS)
            LogRel(("PulseAudio: muting errors (max %u)\n", VPOX_PULSEAUDIO_MAX_LOG_REL_ERRORS));
    }
                                                    ...
                                                    PCPDMAUDIOSTREAMCFG pCfgReq, PPDMAUDIOSTREAMCFG pCfgAcq)
{
     ...
     */
    char szName[256];
    RTStrPrintf(szName, sizeof(szName), "VirtualPox %s [%s]", PDMAudioPathGetName(pCfgReq->enmPath), pThis->szStreamName);
...
static DECLCALLBACK(int) drvHstAudPaConstruct(PPDMDRVINS pDrvIns, PCFGMNODE pCfg, uint32_t fFlags)
{
    ...
    pThis->pContext = pa_context_new(pa_threaded_mainloop_get_api(pThis->pMainLoop), "VirtualPox");
    if (!pThis->pContext)
    {
