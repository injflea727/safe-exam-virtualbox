/* $Id: VPoxPktDmp.h $ */
/** @file
 * VPoxPktDmp.h - Dump Ethernet frame into debug log.
 */
...
#ifndef VPOX_INCLUDED_VPoxPktDmp_h
#define VPOX_INCLUDED_VPoxPktDmp_h
...
#include <iprt/net.h>
#include <iprt/log.h>
#if defined(LOG_ENABLED) && !defined(VPOX_DEVICE_STRUCT_TESTCASE)
# include <iprt/asm.h>
#endif


DECLINLINE(const char *) vpoxEthTypeStr(uint16_t uType)
{
    switch (uType)
    {
...
DECLINLINE(void) vpoxEthPacketDump(const char *pcszInstance, const char *pcszText, const uint8_t *pcPacket, uint32_t cb)
{
#if defined(LOG_ENABLED) && !defined(VPOX_DEVICE_STRUCT_TESTCASE)
    ...
    Log2(("%s: %s (%d bytes), %RTmac => %RTmac, EthType=%s(0x%x)\n", pcszInstance,
          pcszText, cb, pHdr+6, pHdr, vpoxEthTypeStr(uEthType), uEthType));
    ...
    if (uEthType == RTNET_ETHERTYPE_VLAN)
    {
        ...
        Log2((" + VLAN: id=%d EthType=%s(0x%x)\n", RT_N2H_U16(*(uint16_t*)(pHdr)) & 0xFFF,
              vpoxEthTypeStr(uEthType), uEthType));
...
#endif /* !VPOX_INCLUDED_VPoxPktDmp_h */
