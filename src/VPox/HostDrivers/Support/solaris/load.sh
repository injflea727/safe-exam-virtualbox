...
DRVNAME="vpoxdrv"
DRIVERS_USING_IT="vpoxusb vpoxusbmon vpoxnet vpoxflt vpoxbow"
...
servicefound=`svcs -H "virtualpox/zoneaccess" 2>/dev/null | grep '^online'`
if test ! -z "$servicefound"; then
    $SUDO svcadm disable svc:/application/virtualpox/zoneaccess:default
fi
...
if $SUDO add_drv -v $DRVNAME; then
    ...
    if $SUDO modload "/platform/i86pc/kernel/drv/amd64/${DRVNAME}"; then
        ...
        MY_RC=0
        if test ! -h "/dev/vpoxdrv"; then
            $SUDO ln -sf "/devices/pseudo/vpoxdrv@0:vpoxdrv" /dev/vpoxdrv
            $SUDO chmod 0666 /dev/vpoxdrv
        fi
    else
        dmesg | tail
