/* $Id: EventImpl.cpp $ */
/** @file
 * VirtualPox COM Event class implementation
 */
 ...
 * to make any VirtualPox object an event source (by aggregating an EventSource instance).
...
struct VPoxEvent::Data
{
    Data()
        : mType(VPoxEventType_Invalid),
          mWaitEvent(NIL_RTSEMEVENT),
          mWaitable(FALSE),
          mProcessed(FALSE)
    {}

    VPoxEventType_T         mType;
...
DEFINE_EMPTY_CTOR_DTOR(VPoxEvent)

HRESULT VPoxEvent::FinalConstruct()
{
    m = new Data;
    return BaseFinalConstruct();
}

void VPoxEvent::FinalRelease()
{
    if (m)
    {
        uninit();
...
HRESULT VPoxEvent::init(IEventSource *aSource, VPoxEventType_T aType, BOOL aWaitable)
{
    HRESULT rc = S_OK;
...
HRESULT VPoxEvent::getType(VPoxEventType_T *aType)
{
...
HRESULT VPoxEvent::getSource(ComPtr<IEventSource> &aSource)
{
    m->mSource.queryInterfaceTo(aSource.asOutParam());
    return S_OK;
}

HRESULT VPoxEvent::getWaitable(BOOL *aWaitable)
{
    // never changes while event alive, no locking
...
HRESULT VPoxEvent::setProcessed()
{
...
HRESULT VPoxEvent::waitProcessed(LONG aTimeout, BOOL *aResult)
{
    ...
    else
    {
        *aResult = FALSE;
         ...
         * the VirtualPox GUI crashed) so we flag this to our caller so it can remove this
...
struct VPoxVetoEvent::Data
{
    Data() :
        mVetoed(FALSE)
    {}
    ComObjPtr<VPoxEvent>    mEvent;
    BOOL                    mVetoed;
    VetoList                mVetoList;
    ApprovalList            mApprovalList;
};

HRESULT VPoxVetoEvent::FinalConstruct()
{
    m = new Data;
...
void VPoxVetoEvent::FinalRelease()
{
    if (m)
    {
        uninit();
...
DEFINE_EMPTY_CTOR_DTOR(VPoxVetoEvent)

HRESULT VPoxVetoEvent::init(IEventSource *aSource, VPoxEventType_T aType)
{
    HRESULT rc = S_OK;
...
HRESULT VPoxVetoEvent::getType(VPoxEventType_T *aType)
{
    return m->mEvent->COMGETTER(Type)(aType);
}

HRESULT VPoxVetoEvent::getSource(ComPtr<IEventSource> &aSource)
{
    return m->mEvent->COMGETTER(Source)(aSource.asOutParam());
}

HRESULT VPoxVetoEvent::getWaitable(BOOL *aWaitable)
{
    return m->mEvent->COMGETTER(Waitable)(aWaitable);
}

HRESULT VPoxVetoEvent::setProcessed()
{
    return m->mEvent->SetProcessed();
}

HRESULT VPoxVetoEvent::waitProcessed(LONG aTimeout, BOOL *aResult)
{
    return m->mEvent->WaitProcessed(aTimeout, aResult);
}

HRESULT VPoxVetoEvent::addVeto(const com::Utf8Str &aReason)
{
    // AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
    if (aReason.length())
...
HRESULT VPoxVetoEvent::isVetoed(BOOL *aResult)
{
    // AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
    *aResult = m->mVetoed;
...
HRESULT VPoxVetoEvent::getVetos(std::vector<com::Utf8Str> &aResult)
{
    // AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
...
HRESULT VPoxVetoEvent::addApproval(const com::Utf8Str &aReason)
{
    // AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
...
HRESULT VPoxVetoEvent::isApproved(BOOL *aResult)
{
    // AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
...
HRESULT VPoxVetoEvent::getApprovals(std::vector<com::Utf8Str> &aResult)
{
    // AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
...
static const int FirstEvent = (int)VPoxEventType_LastWildcard + 1;
static const int LastEvent = (int)VPoxEventType_Last;
...
public:
    ListenerRecord(IEventListener *aListener,
                   com::SafeArray<VPoxEventType_T> &aInterested,
                   BOOL aActive,
                   EventSource *aOwner);
    ~ListenerRecord();
 ...
 */
static BOOL implies(VPoxEventType_T who, VPoxEventType_T what)
{
    switch (who)
    {
        case VPoxEventType_Any:
            return TRUE;
        case VPoxEventType_Vetoable:
            return    (what == VPoxEventType_OnExtraDataCanChange)
                   || (what == VPoxEventType_OnCanShowWindow);
        case VPoxEventType_MachineEvent:
            return    (what == VPoxEventType_OnMachineStateChanged)
                   || (what == VPoxEventType_OnMachineDataChanged)
                   || (what == VPoxEventType_OnMachineRegistered)
                   || (what == VPoxEventType_OnSessionStateChanged)
                   || (what == VPoxEventType_OnGuestPropertyChanged);
        case VPoxEventType_SnapshotEvent:
            return    (what == VPoxEventType_OnSnapshotTaken)
                   || (what == VPoxEventType_OnSnapshotDeleted)
                   || (what == VPoxEventType_OnSnapshotChanged) ;
        case VPoxEventType_InputEvent:
            return    (what == VPoxEventType_OnKeyboardLedsChanged)
                   || (what == VPoxEventType_OnMousePointerShapeChanged)
                   || (what == VPoxEventType_OnMouseCapabilityChanged);
        case VPoxEventType_Invalid:
            return FALSE;
        default:
            break;
    }
    ...
    mListener(aListener), mActive(aActive), mOwner(aOwner), mQEventBusyCnt(0), mRefCnt(0)
{
    ...
    {
        VPoxEventType_T interested = aInterested[i];
        for (int j = FirstEvent; j < LastEvent; j++)
        {
            VPoxEventType_T candidate = (VPoxEventType_T)j;
...
{
    {
        ...
        // indefinite) waits will block VPoxSVC termination (just one example)
                                      ...
                                      BOOL aActive)
{
    ...
    com::SafeArray<VPoxEventType_T> interested(aInteresting);
    ...
    evDesc.init(this, VPoxEventType_OnEventSourceChanged, (IEventListener *)aListener, TRUE);
    evDesc.fire(0);
...
HRESULT EventSource::unregisterListener(const ComPtr<IEventListener> &aListener)
{
    ...
    {
        rc = setError(VPOX_E_OBJECT_NOT_FOUND,
    ...
    {
        VPoxEventDesc evDesc;
        evDesc.init(this, VPoxEventType_OnEventSourceChanged, (IEventListener *)aListener, FALSE);
        evDesc.fire(0);
    }
                               ...
                               BOOL *aResult)
{
    ...
    if (fWaitable)
    {
         ...
         */
        if (!*aResult && hrc == E_ABORT && implies(VPoxEventType_Vetoable, evType))
        {
            AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
            ...
            /*
             * VPoxEventDesc::fire() requires TRUE to be returned so it can handle
             * vetoable events.
             */
            return S_OK;
        }
                                    ...
                                    const ComPtr<IEvent> &aEvent)
{
    ...
    if (it != m->mListeners.end())
    {
        ...
        if (fWaitable)
        {
            ...
            {
                AssertFailed();
                hrc = setError(VPOX_E_OBJECT_NOT_FOUND,
                               tr("Unknown event"));
            }
            else
...
class ATL_NO_VTABLE PassiveEventListener :
    public VirtualPoxBase,
    VPOX_SCRIPTABLE_IMPL(IEventListener)
{
public:

    VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT(PassiveEventListener, IEventListener)
    ...
    BEGIN_COM_MAP(PassiveEventListener)
        ...
        VPOX_TWEAK_INTERFACE_ENTRY(IEventListener)
    END_COM_MAP()
...
public:
    ...
    VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT(ProxyEventListener, IEventListener)
...
class ATL_NO_VTABLE EventSourceAggregator :
    public VirtualPoxBase,
    VPOX_SCRIPTABLE_IMPL(IEventSource)
{
...
public:
    ...
    VIRTUALPOXBASE_ADD_ERRORINFO_SUPPORT(EventSourceAggregator, IEventSource)
    ...
    BEGIN_COM_MAP(EventSourceAggregator)
        ...
        VPOX_TWEAK_INTERFACE_ENTRY(IEventSource)
    END_COM_MAP()
    ...
    STDMETHOD(RegisterListener)(IEventListener *aListener,
                                ComSafeArrayIn(VPoxEventType_T, aInterested),
                                BOOL aActive);
