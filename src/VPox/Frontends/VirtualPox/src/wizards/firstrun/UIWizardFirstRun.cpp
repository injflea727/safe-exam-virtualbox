...
/** @file
 * VPox Qt GUI - UIWizardFirstRun class implementation.
 */
...
bool UIWizardFirstRun::insertMedium()
{
    ...
    /* Get global VPox object: */
    CVirtualPox comVpox = uiCommon().virtualPox();
    /* Get machine OS type: */
    const CGuestOSType &comOsType = comVpox.GetGuestOSType(m_machine.GetOSTypeId());
...
bool UIWizardFirstRun::isBootHardDiskAttached(const CMachine &machine)
{
    ...
    /* Get 'vpox' global object: */
    CVirtualPox vpox = uiCommon().virtualPox();
    /* Determine machine 'OS type': */
    const CGuestOSType &osType = vpox.GetGuestOSType(machine.GetOSTypeId());
