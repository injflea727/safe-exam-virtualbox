 ...
 *
 * @returns VPox status code. Any failure will stop the enumeration.
...
static DECLCALLBACK(int) dbgcCmdListSource(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "Expression: (%Dv) + %x\n", &pDbgc->SourcePos, 1);
        if (iRangeLeft <= 0)
            break;
    }
                                          ...
                                          const char *pszPrefix)
{
    ...
    if (RT_FAILURE(rc))
    {
        if (rc == VERR_DBGF_REGISTER_NOT_FOUND)
            return DBGCCmdHlpVPoxError(pCmdHlp, VERR_INVALID_PARAMETER, "Unknown register: '%s%s'.\n",
                                       pszActualPrefix,  pszReg);
        return DBGCCmdHlpVPoxError(pCmdHlp, rc, "DBGFR3RegNmQuery failed querying '%s%s': %Rrc.\n",
    ...
    if (cArgs == 1)
    {
        ...
        if (RT_SUCCESS(rc))
            ...
            rc = DBGCCmdHlpVPoxError(pCmdHlp, rc, "DBGFR3RegFormatValue failed: %Rrc.\n", rc);
    }
    else
    {
        DBGCVAR   NewValueTmp;
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_FAILURE(rc))
                rc = DBGCCmdHlpVPoxError(pCmdHlp, rc, "DBGFR3RegNmSet failed settings '%s%s': %Rrc\n",
                                         pszActualPrefix, pszReg, rc);
            if (rc != VINF_SUCCESS)
...
static DECLCALLBACK(int) dbgcCmdRegGuest(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    if (cArgs == 0)
    {
        ...
        if (RT_FAILURE(rc))
            return DBGCCmdHlpVPoxError(pCmdHlp, rc, "DBGFR3RegPrintf failed");
...
static DECLCALLBACK(int) dbgcCmdStepTraceTo(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    if (RT_FAILURE(rc))
        return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "VarToDbgfAddr(,%Dv,)\n", &paArgs[0]);
    ...
    for (unsigned i = 0; i < cArgs; i++)
    {
        ...
        if (paArgs[i].u.u64Number < 256)
        {
                   ...
                   &&   iInt < 256)
            {
                ...
                if (RT_FAILURE(rc))
                    return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "Reading IDT entry %#04x.\n", (unsigned)iInt);
...
#define DBGC_DUMP_MEM_F_SIZE     UINT32_C(0x0000ffff)
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
        {
            if (u16Prev && u16Prev != '\n')
                DBGCCmdHlpPrintf(pCmdHlp, "\n");
            return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "Reading memory at %DV.\n", &pDbgc->DumpPos);
        }
...
static DECLCALLBACK(int) dbgcCmdDumpPageDir(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
            return DBGCCmdHlpVPoxError(pCmdHlp, rc, "%%(%Dv)", &paArgs[0]);
        ...
        if (fLME)
        {
            ...
            if (RT_FAILURE(rc))
                return DBGCCmdHlpVPoxError(pCmdHlp, rc, "Reading PML4E memory at %DV.\n", &VarCur);
        ...
        if (fPAE)
        {
            ...
            if (RT_FAILURE(rc))
                return DBGCCmdHlpVPoxError(pCmdHlp, rc, "Reading PDPE memory at %DV.\n", &VarCur);
            if (!Pdpe.n.u1Present)
    ...
    do
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "Reading PDE memory at %DV.\n", &VarPDEAddr);
...
static DECLCALLBACK(int) dbgcCmdDumpPageTable(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    else
    {
        ...
        if (fPAE)
        {
            ...
            if (RT_FAILURE(rc))
                return DBGCCmdHlpVPoxError(pCmdHlp, rc, "Reading PDE memory at %DV.\n", &VarCur);
            if (!Pde.n.u1Present)
    ...
    do
    {
        ...
        if (RT_FAILURE(rc))
            return DBGCCmdHlpVPoxError(pCmdHlp, rc, "Reading PTE memory at %DV.\n", &VarPTEAddr);
        ...
        switch (paVars[i].enmType)
        {
            ...
            case DBGCVAR_TYPE_SYMBOL:
            {
                ...
                if (cbUnit & RT_BIT_32(31))
                {
                    ...
                    {
                        pCmdHlp->pfnVPoxError(pCmdHlp, VERR_TOO_MUCH_DATA, "Max %d bytes.\n", uEnd.pu8 - (uint8_t *)pvBuf);
            ...
            default:
                *pcbBuf = u.pu8 - (uint8_t *)pvBuf;
                pCmdHlp->pfnVPoxError(pCmdHlp, VERR_INTERNAL_ERROR,
                                      "i=%d enmType=%d\n", i, paVars[i].enmType);
...
static DECLCALLBACK(int) dbgcCmdEditMem(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    for (unsigned iArg = 1;;)
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "Writing memory at %DV.\n", &Addr);
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
        {
            if (rc != VERR_DBGF_MEM_NOT_FOUND)
                return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "DBGFR3MemScan\n");
        ...
        if (RT_FAILURE(rc))
            return DBGCCmdHlpVPoxError(pCmdHlp, rc, "DBGCCmdHlpVarFromDbgfAddr\n");
        if (!pResult)
                                  ...
                                  PCDBGCVAR paPatArgs, unsigned cPatArgs, PDBGCVAR pResult)
{
    ...
    switch (chType)
    {
        ...
        default:
            return pCmdHlp->pfnVPoxError(pCmdHlp, VERR_INVALID_PARAMETER, "chType=%c\n", chType);
    }
 ...
 *
 * @returns VPox status code (fully bitched)
 * @param   pCmdHlp         The command helpers.
 * @param   pUVM            The user mode VM handle.
                                 ...
                                 PCDBGFEVENTCONFIG paEventCfgs, uint32_t cEventCfgs, const char *pszCmd, bool fChangeCmdOnly)
{
    ...
    if (!fChangeCmdOnly)
    {
        if (cIntCfgs)
        {
            ...
            if (RT_FAILURE(rc))
                return DBGCCmdHlpVPoxError(pCmdHlp, rc, "DBGFR3InterruptConfigEx: %Rrc\n", rc);
        }
        if (cEventCfgs)
        {
            rc = DBGFR3EventConfigEx(pUVM, paEventCfgs, cEventCfgs);
            if (RT_FAILURE(rc))
                return DBGCCmdHlpVPoxError(pCmdHlp, rc, "DBGFR3EventConfigEx: %Rrc\n", rc);
        }
    }
...
static DECLCALLBACK(int) dbgcCmdEventCtrl(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    switch (pCmd->pszCmd[2])
    {
        ...
        default:
            return DBGCCmdHlpVPoxError(pCmdHlp, VERR_INVALID_PARAMETER, "pszCmd=%s\n", pCmd->pszCmd);
    }
    ...
    if (fChangeCmdOnly && !pszCmd)
        return DBGCCmdHlpVPoxError(pCmdHlp, VERR_INVALID_PARAMETER, "The 'sx-' requires the '-c cmd' arguments.\n");
    ...
    for (; iArg < cArgs; iArg++)
    {
        ...
        if (!cHits)
            return DBGCCmdHlpVPoxError(pCmdHlp, VERR_INVALID_PARAMETER, "Unknown event: '%s'\n", paArgs[iArg].u.pszString);
    }
...
static int dbgcDoListNear(PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR pArg)
{
    ...
    if (pArg->enmType == DBGCVAR_TYPE_SYMBOL)
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "DBGFR3AsSymbolByName(,,%s,)\n", pArg->u.pszString);
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "%%(%DV)\n", pArg);
        ...
        rc = DBGFR3AsSymbolByAddr(pUVM, pDbgc->hDbgAs, DBGFR3AddrFromFlat(pDbgc->pUVM, &Addr, AddrVar.u.GCFlat),
                                  ...
                                  &offDisp, &Symbol, NULL);
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "DBGFR3AsSymbolByAddr(,,%RGv,,)\n", AddrVar.u.GCFlat);
...
static DECLCALLBACK(int) dbgcCmdListNear(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    if (!cArgs)
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnVPoxError(pCmdHlp, rc, "%s\n", pszFmtExpr + 1);
