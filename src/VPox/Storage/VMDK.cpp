...
typedef struct VMDKIMAGE
{
    ...
    /** Open flags passed by VPoxHD layer. */
    unsigned        uOpenFlags;
...
static int vmdkParseDescriptor(PVMDKIMAGE pImage, char *pDescData, size_t cbDescData)
{
    ...
    if (rc == VERR_VD_VMDK_VALUE_NOT_FOUND)
    {
         ...
         * by VirtualPox. Can only be added for images opened in read/write
...
#ifdef VPOX_WITH_VMDK_STRICT_SIZE_CHECK
...
#endif /* VPOX_WITH_VMDK_STRICT_SIZE_CHECK */
    if (   RT_SUCCESS(rc)
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_LIKELY(pExtent->pszFullname))
        {
                ...
                && pExtent->cDescriptorSectors)
            {
                 ...
                 * entries required to make it work with VPox but for stream optimized images
                                  ...
                                  unsigned uPercentStart, unsigned uPercentSpan)
{
    ...
    {
        /* VirtualPox doesn't care, but VMWare ESX freaks out if the wrong
                                ...
                                uint64_t cbWrite)
{
    ...
    /* Zero byte write optimization. Since we don't tell VPoxHDD that we need
