...
/** @file
 * VPox storage devices: Host block device management API - darwin specifics.
 */
...
/**
 * Converts a given DA return code to a VPox status code.
 *
 * @returns VPox status code.
 * @param   hReturn        The status code returned by a DA API call.
 */
static int hbdMgrDAReturn2VPoxStatus(DAReturn hReturn)
{
...
static DADissenterRef hbdMgrDAMountApprovalCallback(DADiskRef hDiskRef, void *pvContext)
{
    ...
    RTListForEach(&pThis->ListClaimed, pIt, HBDMGRDEV, ListNode)
    {
        ...
        {
            CFStringRef hStrReason = CFStringCreateWithCString(kCFAllocatorDefault, "The disk is currently in use by VirtualPox and cannot be mounted", kCFStringEncodingUTF8);
    ...
    if (!pDev)
    {
        ...
        if (hDiskRef)
        {
                ...
                && CalllbackArgs.rcDA == kDAReturnSuccess)
            {
                ...
                {
                    rc = hbdMgrDAReturn2VPoxStatus(CalllbackArgs.rcDA);
