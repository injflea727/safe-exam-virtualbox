                                  ...
                                  uint32_t fFlags, const char *pszExec, uint32_t idDesiredSession)
{
     ...
     * - If valid get the correct Explorer/VPoxTray instance corresponding to that
     *   user. This of course is only possible if that user is logged in (over
     *   physical console or terminal services).
     * - If we found the user's Explorer/VPoxTray app, use and modify the token to
     *   use it in order to allow the newly started process to access the user's
     *   desktop. If there's no Explorer/VPoxTray app we cannot display the started
        ...
        if (fFlags & RTPROC_FLAGS_SERVICE)
        {
            ...
            if (pSid)
            {
                if (IsValidSid(pSid))
                {
                    ...
                    {
#ifdef VPOX             /* The explorer entry is a fallback in case GA aren't installed. */
                        { "VPoxTray.exe" },
# ifndef IN_GUEST
                        { "VirtualPox.exe" },
# endif
        ...
        {
            /*
             * If we didn't find a matching VPoxTray, just use the token we got
