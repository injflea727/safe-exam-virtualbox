...
static struct
{
    ...
    DECLR0CALLBACKMEMBER(VPOXSTRICTRC, pfnRunGuestCode, (PVMCPUCC pVCpu));
...
static DECLCALLBACK(VPOXSTRICTRC) hmR0DummyRunGuestCode(PVMCPUCC pVCpu)
{
    RT_NOREF(pVCpu);
    return VINF_SUCCESS;
}
 ...
 *
 * @returns VPox status code (will only fail if out of memory).
 */
 ...
 *
 * @returns Strict VPox status code. (VPOXSTRICTRC isn't used because it's
...
VMMR0_INT_DECL(int) HMR0RunGuestCode(PVMCC pVM, PVMCPUCC pVCpu)
{
    ...
    VPOXSTRICTRC rcStrict = g_HmR0.pfnRunGuestCode(pVCpu);
