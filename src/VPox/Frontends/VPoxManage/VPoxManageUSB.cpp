...
RTEXITCODE handleUSBFilter(HandlerArg *a)
{
            ...
            for (int i = 2; i < a->argc; i++)
            {
                    ...
                    || !strcmp(a->argv[i], "-target"))
                {
                    ...
                    else
                    {
                        ...
                        CHECK_ERROR_RET(a->virtualPox, FindMachine(Bstr(a->argv[i]).raw(),
    ...
    if (cmd.mGlobal)
        CHECK_ERROR_RET(a->virtualPox, COMGETTER(Host)(host.asOutParam()), RTEXITCODE_FAILURE);
    else
    {
