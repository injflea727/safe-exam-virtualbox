/* $Id: VPoxAutostartStop.cpp $ */
/** @file
 * VPoxAutostart - VirtualPox Autostart service, stop machines during system shutdown.
 */
...
DECLHIDDEN(RTEXITCODE) autostartStopMain(PCFGAST pCfgAst)
{
    ...
    HRESULT rc = g_pVirtualPox->COMGETTER(Machines)(ComSafeArrayAsOutParam(machines));
    if (SUCCEEDED(rc))
    {
        /*
            ...
            && !listVM.empty())
        {
            ...
            for (it = listVM.begin(); it != listVM.end(); ++it)
            {
                ...
                CHECK_ERROR_BREAK(g_pVirtualPox, FindMachine((*it).strId.raw(),
                                                             machine.asOutParam()));
