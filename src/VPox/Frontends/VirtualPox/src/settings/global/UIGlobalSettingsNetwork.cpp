...
/** @file
 * VPox Qt GUI - UIGlobalSettingsNetwork class implementation.
 */
...
void UIGlobalSettingsNetwork::loadToCacheFrom(QVariant &data)
{
    ...
    foreach (const CNATNetwork &network, uiCommon().virtualPox().GetNATNetworks())
...
bool UIGlobalSettingsNetwork::removeNATNetwork(const UISettingsCacheGlobalNetworkNAT &cache)
{
    ...
    if (fSuccess)
    {
        ...
        CNATNetwork comNetwork = comVPox.FindNATNetworkByName(oldNatData.m_strName);
        fSuccess = comVPox.isOk() && comNetwork.isNotNull();

        /* Remove NAT network: */
        if (fSuccess)
        {
            comVPox.RemoveNATNetwork(comNetwork);
            fSuccess = comVPox.isOk();
        }

        /* Show error message if necessary: */
        if (!fSuccess)
            notifyOperationProgressError(UIErrorString::formatErrorInfo(comVPox));
    }
    /* Return result: */
    return fSuccess;
}
...
bool UIGlobalSettingsNetwork::createNATNetwork(const UISettingsCacheGlobalNetworkNAT &cache)
{
    ...
    if (fSuccess)
    {
        ...
        CNATNetwork comNetwork = comVPox.CreateNATNetwork(newNatData.m_strNewName);
        fSuccess = comVPox.isOk() && comNetwork.isNotNull();
