/* $Id: VPoxCredProvCredential.cpp $ */
/** @file
 * VPoxCredProvCredential - Class for keeping and handling the passed credentials.
 */
...
VPoxCredProvCredential::VPoxCredProvCredential(void) :
    m_enmUsageScenario(CPUS_INVALID),
    m_cRefs(1),
    m_pEvents(NULL),
    m_fHaveCreds(false)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential: Created\n");
    VPoxCredentialProviderAcquire();

    for (unsigned i = 0; i < VPOXCREDPROV_NUM_FIELDS; i++)
    {
        const VPOXCREDPROV_FIELD *pField = &s_VPoxCredProvDefaultFields[i];
...
VPoxCredProvCredential::~VPoxCredProvCredential(void)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential: Destroying\n");
...
ULONG VPoxCredProvCredential::AddRef(void)
{
    LONG cRefs = InterlockedIncrement(&m_cRefs);
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::AddRef: Returning refcount=%ld\n",
                        cRefs);
    return cRefs;
}


ULONG VPoxCredProvCredential::Release(void)
{
    LONG cRefs = InterlockedDecrement(&m_cRefs);
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::Release: Returning refcount=%ld\n",
                        cRefs);
    if (!cRefs)
    {
        VPoxCredProvVerbose(0, "VPoxCredProvCredential: Calling destructor\n");
        delete this;
    }
    return cRefs;
}
...
HRESULT VPoxCredProvCredential::QueryInterface(REFIID interfaceID, void **ppvInterface)
{
    HRESULT hr = S_OK;;
    if (ppvInterface)
    {
 ...
 */
HRESULT VPoxCredProvCredential::RTUTF16ToUnicode(PUNICODE_STRING pUnicodeDest, PRTUTF16 pwszSource, bool fCopy)
{
 ...
 */
HRESULT VPoxCredProvCredential::RTUTF16ToUnicodeA(PUNICODE_STRING pUnicodeDest, PRTUTF16 pwszSource)
{
    AssertPtrReturn(pUnicodeDest, E_POINTER);
    AssertPtrReturn(pwszSource,   E_POINTER);
 ...
 */
void VPoxCredProvCredential::UnicodeStringFree(PUNICODE_STRING pUnicode)
{
    if (!pUnicode)
        return;
 ...
 */
HRESULT VPoxCredProvCredential::kerberosLogonCreate(KERB_INTERACTIVE_LOGON *pLogon,
                                                    ...
                                                    PRTUTF16 pwszUser, PRTUTF16 pwszPassword, PRTUTF16 pwszDomain)
{
    ...
    else /* No domain (FQDN) given, try local computer name. */
    {
        ...
        if (GetComputerNameW(wszComputerName, &cch))
        {
            ...
            VPoxCredProvVerbose(0, "VPoxCredProvCredential::kerberosLogonInit: Local computer name=%ls\n",
                                wszComputerName);
        }
        else
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                switch (enmUsage)
                {
                    ...
                    default:
                        VPoxCredProvVerbose(0, "VPoxCredProvCredential::kerberosLogonInit: Unknown usage scenario=%ld\n",
                                            enmUsage);
                        hr = E_FAIL;
                        break;
                }
 ...
 */
void VPoxCredProvCredential::kerberosLogonDestroy(KERB_INTERACTIVE_LOGON *pLogon)
{
    if (!pLogon)
        return;
...
HRESULT VPoxCredProvCredential::kerberosLogonSerialize(const KERB_INTERACTIVE_LOGON *pLogonIn,
                                                       PBYTE *ppPackage, DWORD *pcbPackage)
{
    ...
    VPoxCredProvVerbose(1, "VPoxCredProvCredential::AllocateLogonPackage: Allocating %ld bytes (%zu bytes credentials)\n",
 ...
 */
PCRTUTF16 VPoxCredProvCredential::getField(DWORD dwFieldID)
{
    if (dwFieldID >= VPOXCREDPROV_NUM_FIELDS)
        return NULL;

    /* Paranoia: Don't ever reveal passwords. */
    if (dwFieldID == VPOXCREDPROV_FIELDID_PASSWORD)
        return NULL;
 ...
 */
HRESULT VPoxCredProvCredential::setField(DWORD dwFieldID, const PRTUTF16 pcwszString, bool fNotifyUI)
{
    if (dwFieldID >= VPOXCREDPROV_NUM_FIELDS)
        return E_INVALIDARG;
    ...
    pwszField = RTUtf16Dup(pcwszString ? pcwszString : s_VPoxCredProvDefaultFields[dwFieldID].desc.pszLabel);
    if (pwszField)
    {
    ...
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::setField: Setting field dwFieldID=%ld to '%ls', fNotifyUI=%RTbool, hr=0x%08x\n",
                        dwFieldID,
#ifdef DEBUG
                        pwszField,
#else
                        /* Don't show any passwords in release mode. */
                        dwFieldID == VPOXCREDPROV_FIELDID_PASSWORD ? L"XXX" : pwszField,
#endif
                        fNotifyUI, hr);
    return hr;
}
 ...
 * @return  HRESULT
 */
HRESULT VPoxCredProvCredential::Reset(void)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::Reset: Wiping credentials user=%ls, pw=%ls, domain=%ls\n",
                        m_apwszFields[VPOXCREDPROV_FIELDID_USERNAME] ? m_apwszFields[VPOXCREDPROV_FIELDID_USERNAME] : L"<NULL>",
#ifdef DEBUG
                        m_apwszFields[VPOXCREDPROV_FIELDID_PASSWORD] ? m_apwszFields[VPOXCREDPROV_FIELDID_PASSWORD] : L"<NULL>",
#else
                        L"XXX" /* Don't show any passwords in release mode. */,
#endif
                        m_apwszFields[VPOXCREDPROV_FIELDID_DOMAINNAME] ? m_apwszFields[VPOXCREDPROV_FIELDID_DOMAINNAME] : L"<NULL>");
    ...
    HRESULT hr = setField(VPOXCREDPROV_FIELDID_PASSWORD, NULL /* Use default value */, true /* fNotifyUI */);

    m_fIsSelected = false;

    VPoxCredProvVerbose(0, "VPoxCredProvCredential::Reset\n");
    return hr;
}
 ...
 */
int VPoxCredProvCredential::RetrieveCredentials(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        VPoxCredProvReportStatus(VPoxGuestFacilityStatus_Terminating);

        rc = VbglR3CredentialsRetrieveUtf16(&pwszUser, &pwszPassword, &pwszDomain);

        VPoxCredProvVerbose(0, "VPoxCredProvCredential::RetrieveCredentials: Retrieved credentials with rc=%Rrc\n", rc);
    }

    if (RT_SUCCESS(rc))
    {
        VPoxCredProvVerbose(0, "VPoxCredProvCredential::RetrieveCredentials: Received credentials for user '%ls'\n", pwszUser);
        ...
        if (   TranslateAccountName(pwszUser, &pwszExtractedName)
            && pwszExtractedName)
        {
            VPoxCredProvVerbose(0, "VPoxCredProvCredential::RetrieveCredentials: Translated account name '%ls' -> '%ls'\n",
        ...
        else
        {
            ...
            if (ExtractAccoutData(pwszUser, &pwszExtractedName, &pwszExtractedDomain))
            {
                ...
                VPoxCredProvVerbose(0, "VPoxCredProvCredential::RetrieveCredentials: Extracted account name '%ls' + domain '%ls'\n",
    ...
    if (m_fHaveCreds)
    {
        VPoxCredProvVerbose(0, "VPoxCredProvCredential::RetrieveCredentials: Setting fields\n");

        setField(VPOXCREDPROV_FIELDID_USERNAME,   pwszUser,     true /* fNotifyUI */);
        setField(VPOXCREDPROV_FIELDID_PASSWORD,   pwszPassword, true /* fNotifyUI */);
        setField(VPOXCREDPROV_FIELDID_DOMAINNAME, pwszDomain,   true /* fNotifyUI */);
    }

    VPoxCredProvVerbose(0, "VPoxCredProvCredential::RetrieveCredentials: Wiping ...\n");

    VbglR3CredentialsDestroyUtf16(pwszUser, pwszPassword, pwszDomain, 3 /* cPasses */);

    VPoxCredProvVerbose(0, "VPoxCredProvCredential::RetrieveCredentials: Returned rc=%Rrc\n", rc);
    return rc;
}
 ...
 * usage scenario.
 */
HRESULT VPoxCredProvCredential::Initialize(CREDENTIAL_PROVIDER_USAGE_SCENARIO enmUsageScenario)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::Initialize: enmUsageScenario=%ld\n", enmUsageScenario);
 ...
 */
HRESULT VPoxCredProvCredential::Advise(ICredentialProviderCredentialEvents *pEvents)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::Advise: pEvents=0x%p\n", pEvents);
 ...
 */
HRESULT VPoxCredProvCredential::UnAdvise(void)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::UnAdvise\n");
 ...
 * to FALSE (if set).
 */
HRESULT VPoxCredProvCredential::SetSelected(PBOOL pfAutoLogon)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::SetSelected\n");
 ...
 */
HRESULT VPoxCredProvCredential::SetDeselected(void)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::SetDeselected\n");
 ...
 */
HRESULT VPoxCredProvCredential::GetFieldState(DWORD dwFieldID, CREDENTIAL_PROVIDER_FIELD_STATE *pFieldState,
                                              CREDENTIAL_PROVIDER_FIELD_INTERACTIVE_STATE *pFieldstateInteractive)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::GetFieldState: dwFieldID=%ld\n", dwFieldID);
    ...
    {
        if (pFieldState)
            *pFieldState            = s_VPoxCredProvDefaultFields[dwFieldID].state;

        if (pFieldstateInteractive)
            *pFieldstateInteractive = s_VPoxCredProvDefaultFields[dwFieldID].stateInteractive;
    }
    else
        hr = E_INVALIDARG;
 ...
 */
BOOL VPoxCredProvCredential::TranslateAccountName(PWSTR pwszDisplayName, PWSTR *ppwszAccoutName)
{
    AssertPtrReturn(pwszDisplayName, FALSE);
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::TranslateAccountName: Getting account name for \"%ls\" ...\n",
                        pwszDisplayName);
                        ...
                        && StrCmpI(pwszDisplayName, pCurBuf->usri2_full_name) == 0)
                    {
                        ...
                        if (hr == S_OK)
                        {
                            *ppwszAccoutName = pwszTemp;
                            ...
                            VPoxCredProvVerbose(0, "VPoxCredProvCredential::TranslateAccountName: Error copying data, hr=%08x\n", hr);
                        break;
                    }
                    pCurBuf++;
                }
    ...
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::TranslateAccountName returned rcStatus=%ld, fFound=%RTbool\n",
                        rcStatus, fFound);
    return fFound;
    ...
    if (   TranslateNameW(pwszName, NameUnknown, NameUserPrincipal, NULL, &cbLen)
        && cbLen > 0)
    {
        VPoxCredProvVerbose(0, "VPoxCredProvCredential::GetAccountName: Translated ADS name has %u characters\n", cbLen));
        ...
        {
            VPoxCredProvVerbose(0, "VPoxCredProvCredential::GetAccountName: Real ADS account name of '%ls' is '%ls'\n",
 ...
 *                                  Needs to be free'd with CoTaskMemFree().
 */
BOOL VPoxCredProvCredential::ExtractAccoutData(PWSTR pwszAccountData, PWSTR *ppwszAccoutName, PWSTR *ppwszDomain)
{
    AssertPtrReturn(pwszAccountData, FALSE);
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::ExtractAccoutData: Getting account name for \"%ls\" ...\n",
                        pwszAccountData);
    HRESULT hr = E_FAIL;
        ...
        &&  pPos != pwszAccountData)
    {
                ...
                && *pPos != L'\0')
            {
                ...
                if (SUCCEEDED(hr))
                {
                    *ppwszDomain = pwszDomain;
                    ...
                    VPoxCredProvVerbose(0, "VPoxCredProvCredential::ExtractAccoutData: Error copying domain data, hr=%08x\n", hr);
            }
            else
            {
                hr = E_FAIL;
                VPoxCredProvVerbose(0, "VPoxCredProvCredential::ExtractAccoutData: No domain name found!\n");
            }
        }
        else
            VPoxCredProvVerbose(0, "VPoxCredProvCredential::ExtractAccoutData: Error copying account data, hr=%08x\n", hr);
        ...
        VPoxCredProvVerbose(0, "VPoxCredProvCredential::ExtractAccoutData: No valid principal account name found!\n");
 ...
 */
HRESULT VPoxCredProvCredential::GetStringValue(DWORD dwFieldID, PWSTR *ppwszString)
{
    HRESULT hr;
    ...
    if (dwFieldID < VPOXCREDPROV_NUM_FIELDS)
    {
        switch (dwFieldID)
        {
            case VPOXCREDPROV_FIELDID_SUBMIT_BUTTON:
            {
    ...
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::GetStringValue: m_fIsSelected=%RTbool, dwFieldID=%ld, pwszString=%ls, hr=%Rhrc\n",
                        m_fIsSelected, dwFieldID,
#ifdef DEBUG
                        pwszString ? pwszString : L"<NULL>",
#else
                        /* Don't show any passwords in release mode. */
                        dwFieldID == VPOXCREDPROV_FIELDID_PASSWORD ? L"XXX" : (pwszString ? pwszString : L"<NULL>"),
#endif
                        hr);
 ...
 */
HRESULT VPoxCredProvCredential::GetSubmitButtonValue(DWORD dwFieldID, DWORD *pdwAdjacentTo)
{
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::GetSubmitButtonValue: dwFieldID=%ld\n",
                        dwFieldID);

    HRESULT hr = S_OK;

    /* Validate parameters. */
    if (   dwFieldID == VPOXCREDPROV_FIELDID_SUBMIT_BUTTON
        && pdwAdjacentTo)
    {
        /* pdwAdjacentTo is a pointer to the fieldID you want the submit button to appear next to. */
        *pdwAdjacentTo = VPOXCREDPROV_FIELDID_PASSWORD;
        VPoxCredProvVerbose(0, "VPoxCredProvCredential::GetSubmitButtonValue: dwFieldID=%ld, *pdwAdjacentTo=%ld\n",
 ...
 */
HRESULT VPoxCredProvCredential::SetStringValue(DWORD dwFieldID, PCWSTR pwszValue)
{
    ...
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::SetStringValue: dwFieldID=%ld, pcwzString=%ls, hr=%Rhrc\n",
                        dwFieldID,
#ifdef DEBUG
...
HRESULT VPoxCredProvCredential::GetBitmapValue(DWORD dwFieldID, HBITMAP *phBitmap)
{
    NOREF(dwFieldID);
    NOREF(phBitmap);
...
HRESULT VPoxCredProvCredential::GetCheckboxValue(DWORD dwFieldID, BOOL *pfChecked, PWSTR *ppwszLabel)
{
    NOREF(dwFieldID);
    NOREF(pfChecked);
    NOREF(ppwszLabel);
...
HRESULT VPoxCredProvCredential::GetComboBoxValueCount(DWORD dwFieldID, DWORD *pcItems, DWORD *pdwSelectedItem)
{
    NOREF(dwFieldID);
    NOREF(pcItems);
...
HRESULT VPoxCredProvCredential::GetComboBoxValueAt(DWORD dwFieldID, DWORD dwItem, PWSTR *ppwszItem)
{
    NOREF(dwFieldID);
    NOREF(dwItem);
    NOREF(ppwszItem);
...
HRESULT VPoxCredProvCredential::SetCheckboxValue(DWORD dwFieldID, BOOL fChecked)
{
    NOREF(dwFieldID);
    NOREF(fChecked);
    return E_NOTIMPL;
...
HRESULT VPoxCredProvCredential::SetComboBoxSelectedValue(DWORD dwFieldId, DWORD dwSelectedItem)
{
    NOREF(dwFieldId);
...
HRESULT VPoxCredProvCredential::CommandLinkClicked(DWORD dwFieldID)
{
    NOREF(dwFieldID);
    return E_NOTIMPL;
}
 ...
 */
HRESULT VPoxCredProvCredential::GetSerialization(CREDENTIAL_PROVIDER_GET_SERIALIZATION_RESPONSE *pcpGetSerializationResponse,
                                                 ...
                                                 CREDENTIAL_PROVIDER_STATUS_ICON *pcpsiOptionalStatusIcon)
{
    ...
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::GetSerialization: Username=%ls, Password=%ls, Domain=%ls\n",
                        m_apwszFields[VPOXCREDPROV_FIELDID_USERNAME],
                        m_apwszFields[VPOXCREDPROV_FIELDID_PASSWORD],
                        m_apwszFields[VPOXCREDPROV_FIELDID_DOMAINNAME]);
#endif
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            pcpCredentialSerialization->clsidCredentialProvider = CLSID_VPoxCredProvider;

                            /* We're done -- let the logon UI know. */
                            *pcpGetSerializationResponse = CPGSR_RETURN_CREDENTIAL_FINISHED;

                            VPoxCredProvVerbose(1, "VPoxCredProvCredential::GetSerialization: Finished for user '%ls' (domain '%ls')\n",
                                                m_apwszFields[VPOXCREDPROV_FIELDID_USERNAME],
                                                m_apwszFields[VPOXCREDPROV_FIELDID_DOMAINNAME]);
                        }
                        else
                            VPoxCredProvVerbose(1, "VPoxCredProvCredential::GetSerialization: LsaLookupAuthenticationPackage failed with ntStatus=%ld\n", s);
                    }
                }
                ...
                VPoxCredProvVerbose(1, "VPoxCredProvCredential::GetSerialization: LsaConnectUntrusted failed with ntStatus=%ld\n", s);
        }
        else
            VPoxCredProvVerbose(1, "VPoxCredProvCredential::GetSerialization: kerberosLogonSerialize failed with hr=0x%08x\n", hr);

        kerberosLogonDestroy(pLogon);
        pLogon = NULL;
    }
    else
        VPoxCredProvVerbose(1, "VPoxCredProvCredential::GetSerialization: kerberosLogonCreate failed with hr=0x%08x\n", hr);

    VPoxCredProvVerbose(1, "VPoxCredProvCredential::GetSerialization returned hr=0x%08x\n", hr);
    return hr;
}
 ...
 */
HRESULT VPoxCredProvCredential::ReportResult(NTSTATUS ntStatus,
                                             NTSTATUS ntSubStatus,
                                             PWSTR *ppwszOptionalStatusText,
                                             CREDENTIAL_PROVIDER_STATUS_ICON *pcpsiOptionalStatusIcon)
{
    RT_NOREF(ntStatus, ntSubStatus, ppwszOptionalStatusText, pcpsiOptionalStatusIcon);
    VPoxCredProvVerbose(0, "VPoxCredProvCredential::ReportResult: ntStatus=%ld, ntSubStatus=%ld\n",
                        ntStatus, ntSubStatus);
    return E_NOTIMPL;
}
