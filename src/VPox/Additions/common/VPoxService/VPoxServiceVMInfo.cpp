/* $Id: VPoxServiceVMInfo.cpp $ */
/** @file
 * VPoxService - Virtual Machine Information for the Host.
 */
...
/** @page pg_vgsvc_vminfo VPoxService - VM Information
 *
 ...
 * service in cooperation with the Main API (VPoxSVC).  Properties have a name
 ...
 * via the Main API or VPoxManage on the host side and VPoxControl in the guest.
 *
 * The namespace "/VirtualPox/" is reserved for value provided by VirtualPox.
 * This service provides all the information under "/VirtualPox/GuestInfo/".
 *
 *
 ...
 *
 * For the networking part "/VirtualPox/GuestInfo/Net/Count" is the value to
 * watch out for.
 *
 * For the login part, it's possible that we intended to use
 * "/VirtualPox/GuestInfo/OS/LoggedInUsers" for this, however it is not defined
...
#include "VPoxServicePropCache.h"
...
typedef struct VPOXSERVICELACLIENTINFO
{
    uint32_t    uID;
    char       *pszName;
    char       *pszLocation;
...
} VPOXSERVICELACLIENTINFO, *PVPOXSERVICELACLIENTINFO;
...
static VPOXSERVICEVEPROPCACHE   g_VMInfoPropCache;
static const char              *g_pszPropCacheValLoggedInUsersList = "/VirtualPox/GuestInfo/OS/LoggedInUsersList";
static const char              *g_pszPropCacheValLoggedInUsers = "/VirtualPox/GuestInfo/OS/LoggedInUsers";
static const char              *g_pszPropCacheValNoLoggedInUsers = "/VirtualPox/GuestInfo/OS/NoLoggedInUsers";
static const char              *g_pszPropCacheValNetCount = "/VirtualPox/GuestInfo/Net/Count";
/** A guest user's guest property root key. */
static const char              *g_pszPropCacheValUser = "/VirtualPox/GuestInfo/User/";
...
static VPOXSERVICELACLIENTINFO  g_LAClientInfo;
...
*   Defines                                                                                                                      *
*********************************************************************************************************************************/
static const char *g_pszLAActiveClient = "/VirtualPox/HostInfo/VRDP/ActiveClient";
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnPreInit}
 */
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnOption}
 */
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnInit}
 */
static DECLCALLBACK(int) vbsvcVMInfoInit(void)
{
    ...
    /* The status code is ignored as this information is not available with VPox < 3.2.10. */
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        char *pszValue;
        rc2 = VGSvcReadHostProp(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestAdd/VPoxService/--vminfo-user-idle-threshold",
...
/** @todo r=bird: Another pointless RTStrPrintf test with wrong status code to boot. */
    if (RTStrPrintf(pszClientPath, sizeof(pszClientPath),
                    "/VirtualPox/HostInfo/VRDP/Client/%RU32/%s", uClientID, pszProperty))
    {
 ...
 * objects which need to be free'd with vpoxServiceFreeLAClientInfo.
 *
 * @return  IPRT status code.
 * @param   uClientID               Client ID to retrieve information for.
 * @param   pClient                 Pointer where to store the client information.
 */
static int vgsvcGetLAClientInfo(uint32_t uClientID, PVPOXSERVICELACLIENTINFO pClient)
{
    AssertReturn(uClientID, VERR_INVALID_PARAMETER);
 ...
 */
static void vgsvcFreeLAClientInfo(PVPOXSERVICELACLIENTINFO pClient)
{
    if (pClient)
    {
        if (pClient->pszName)
 ...
 *                                  the property.
 */
int VGSvcUserUpdateF(PVPOXSERVICEVEPROPCACHE pCache, const char *pszUser, const char *pszDomain,
...
static void vgsvcVMInfoWriteFixedProperties(void)
{
    ...
    VGSvcWritePropF(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestInfo/OS/Product",
                    "%s", RT_FAILURE(rc) ? "" : szInfo);

    rc = RTSystemQueryOSInfo(RTSYSOSINFO_RELEASE, szInfo, sizeof(szInfo));
    VGSvcWritePropF(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestInfo/OS/Release",
                    "%s", RT_FAILURE(rc) ? "" : szInfo);

    rc = RTSystemQueryOSInfo(RTSYSOSINFO_VERSION, szInfo, sizeof(szInfo));
    VGSvcWritePropF(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestInfo/OS/Version",
                    "%s", RT_FAILURE(rc) ? "" : szInfo);

    rc = RTSystemQueryOSInfo(RTSYSOSINFO_SERVICE_PACK, szInfo, sizeof(szInfo));
    VGSvcWritePropF(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestInfo/OS/ServicePack",
                    "%s", RT_FAILURE(rc) ? "" : szInfo);
    ...
    VGSvcWritePropF(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestAdd/Version",
                    "%s", RT_FAILURE(rc) ? "" : pszAddVer);
    VGSvcWritePropF(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestAdd/VersionExt",
                    "%s", RT_FAILURE(rc) ? "" : pszAddVerExt);
    VGSvcWritePropF(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestAdd/Revision",
    ...
    VGSvcWritePropF(g_uVMInfoGuestPropSvcClientID, "/VirtualPox/GuestAdd/InstallDir",
...
#if defined(VPOX_WITH_DBUS) && defined(RT_OS_LINUX) /* Not yet for Solaris/FreeBSB. */
/*
 * Simple wrapper to work around compiler-specific va_list madness.
 */
static dbus_bool_t vpoxService_dbus_message_get_args(DBusMessage *message, DBusError *error, int first_arg_type, ...)
{
    va_list va;
            ...
            && dbus_message_get_type(pMsgSessions) == DBUS_MESSAGE_TYPE_METHOD_CALL)
        {
                ...
                && !dbus_error_is_set(&dbErr))
            {
                ...
                if (   dbus_message_get_type(pMsgSessions) == DBUS_MESSAGE_TYPE_METHOD_CALL
                    && vpoxService_dbus_message_get_args(pReplySessions, &dbErr, DBUS_TYPE_ARRAY,
...
# endif
    ...
    for (int i = 0; i < cIfacesSystem; ++i)
    {
        ...
        RTStrPrintf(szPropPath, sizeof(szPropPath), "/VirtualPox/GuestInfo/Net/%RU32/V4/IP", cIfsReported);
        VGSvcPropCacheUpdate(&g_VMInfoPropCache, szPropPath, "%s", szIp);

        pAddress = (sockaddr_in *) & (aInterfaces[i].iiBroadcastAddress);
        RTStrPrintf(szPropPath, sizeof(szPropPath), "/VirtualPox/GuestInfo/Net/%RU32/V4/Broadcast", cIfsReported);
        ...
        RTStrPrintf(szPropPath, sizeof(szPropPath), "/VirtualPox/GuestInfo/Net/%RU32/V4/Netmask", cIfsReported);
        VGSvcPropCacheUpdate(&g_VMInfoPropCache, szPropPath, "%s", g_pfninet_ntoa(pAddress->sin_addr));

        RTStrPrintf(szPropPath, sizeof(szPropPath), "/VirtualPox/GuestInfo/Net/%RU32/Status", cIfsReported);
        ...
        if (pAdpInfo)
        {
            ...
            RTStrPrintf(szPropPath, sizeof(szPropPath), "/VirtualPox/GuestInfo/Net/%RU32/MAC", cIfsReported);
            if (pAdp)
            {
                char szMac[32];
...
# endif
        ...
        if (!fSkip)
        {
            size_t offSubProp = RTStrPrintf(szPropPath, sizeof(szPropPath), "/VirtualPox/GuestInfo/Net/%RU32", cIfsReported);
...
# endif
        ...
        && cIfsReportedOld > cIfsReported) /* Are some ifaces not around anymore? */
    {
        ...
        do
        {
            ...
            rc = VGSvcPropCacheUpdateByPath(&g_VMInfoPropCache, NULL /* Value, delete */, 0 /* Flags */, "/VirtualPox/GuestInfo/Net/%RU32", uIfaceDeleteIdx++);
        } while (RT_SUCCESS(rc));
    }
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnWorker}
 */
static DECLCALLBACK(int) vbsvcVMInfoWorker(bool volatile *pfShutdown)
{
    ...
    for (;;)
    {
         ...
         * works with VPox (latest) 4.1 and up. */
                        ...
                        || (g_LAClientAttachedTS != uLAClientAttachedTS)))
                {
                    ...
                    if (RT_SUCCESS(rc2))
                    {
                        VGSvcVerbose(1, "VRDP: Hotdesk client %s with ID=%RU32, Name=%s, Domain=%s\n",
                                      ...
                                      * hotdesk session when VPoxService started. */
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnStop}
 */
...
/**
 * @interface_method_impl{VPOXSERVICE,pfnTerm}
 */
static DECLCALLBACK(void) vbsvcVMInfoTerm(void)
{
    if (g_hVMInfoEvent != NIL_RTSEMEVENTMULTI)
    {
        ...
        const char *apszPat[1] = { "/VirtualPox/GuestInfo/Net/*" };
