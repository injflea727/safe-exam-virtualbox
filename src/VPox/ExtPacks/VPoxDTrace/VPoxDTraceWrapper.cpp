/* $Id: VPoxDTraceWrapper.cpp $ */
/** @file
 * VPoxDTrace - Wrapper that selects the right dtrace implemetation and adds
...
*********************************************************************************************************************************/
/** The VPoxDTrace extension pack name.   */
#define VPOX_EXTPACK_VPOXDTRACE_NAME            "Oracle VPoxDTrace Extension Pack"
/** The mangled version of VPOX_EXTPACK_VPOXDTRACE_NAME (also in Config.kmk). */
#define VPOX_EXTPACK_VPOXDTRACE_MANGLED_NAME    "Oracle_VPoxDTrace_Extension_Pack"


/*********************************************************************************************************************************
*   Structures and Typedefs                                                                                                      *
*********************************************************************************************************************************/
/** The main function of VPoxDTrace.so/dylib/dll. */
typedef int (RTCALL *PFNVPOXDTRACEMAIN)(int argc, char **argv);
...
int main(int argc, char **argv)
{
    ...
    if (!RTEnvExist("VPOX_DTRACE_NO_NATIVE"))
...
# ifdef RT_OS_LINUX
                /** @todo Warn if the dtrace modules haven't been loaded or vpoxdrv isn't
...
# endif
...
#endif
    {
        ...
        if (RT_SUCCESS(rc))
            rc = RTPathAppend(szDTraceCmd, sizeof(szDTraceCmd),
                              VPOX_EXTPACK_INSTALL_DIR RTPATH_SLASH_STR VPOX_EXTPACK_VPOXDTRACE_MANGLED_NAME);
        if (RT_SUCCESS(rc))
            rc = RTPathAppend(szDTraceCmd, sizeof(szDTraceCmd), RTBldCfgTargetDotArch());
        if (RT_SUCCESS(rc))
            rc = RTPathAppend(szDTraceCmd, sizeof(szDTraceCmd), "VPoxDTraceCmd");
        if (RT_SUCCESS(rc))
        ...
        if (!RTFileExists(szDTraceCmd))
            return RTMsgErrorExit(RTEXITCODE_FAILURE,
                                  "Unable to find a DTrace implementation. VPoxDTrace Extension Pack installed?");
    ...
    if (RT_FAILURE(rc))
        return RTMsgErrorExit(RTEXITCODE_FAILURE, "Error constructing dtrace library path for VPox: %Rrc", rc);
    ...
    /*
     * While the VPoxDTrace we load and call the main function of.
     */
    else
    {
        ...
        if (RT_SUCCESS(rc))
        {
            PFNVPOXDTRACEMAIN pfnVPoxDTraceMain;
            rc = RTLdrGetSymbol(hMod, "VPoxDTraceMain", (void **)&pfnVPoxDTraceMain);
            if (RT_SUCCESS(rc))
                rcExit = (RTEXITCODE)pfnVPoxDTraceMain(cArgs, papszArgs);
            else
                rcExit = RTMsgErrorExit(RTEXITCODE_FAILURE, "Error locating 'VPoxDTraceMain' in '%s': %Rrc", szDTraceCmd, rc);
        }
        else
