...
int VPoxMainDriveInfo::updateDVDs ()
{
    ...
    try
    {
        ...
        if (RT_SUCCESS(rc) && (!success || testing()))
            rc = getDriveInfoFromEnv ("VPOX_CDROM", &mDVDList, true /* isDVD */,
                                      &success);
        setNoProbe(false);
        if (RT_SUCCESS(rc) && (!success || testing()))
...
int VPoxMainDriveInfo::updateFloppies ()
{
    LogFlowThisFunc(("entered\n"));
    int rc = VINF_SUCCESS;
    bool success = false;  /* Have we succeeded in finding anything yet? */
    try
    {
        mFloppyList.clear ();
        if (RT_SUCCESS(rc) && (!success || testing()))
            rc = getDriveInfoFromEnv("VPOX_FLOPPY", &mFloppyList,
                                     false /* isDVD */, &success);
        setNoProbe(false);
        if (   RT_SUCCESS(rc) && (!success || testing()))
...
class hotplugNullImpl : public VPoxMainHotplugWaiterImpl
{
public:
...
class hotplugInotifyImpl : public VPoxMainHotplugWaiterImpl
{
...
# endif /* VPOX_USB_WITH_INOTIFY */
#endif  /* VPOX_USB_WTH_SYSFS */

VPoxMainHotplugWaiter::VPoxMainHotplugWaiter(const char *pcszDevicesRoot)
{
    try
    {
#ifdef VPOX_USB_WITH_SYSFS
