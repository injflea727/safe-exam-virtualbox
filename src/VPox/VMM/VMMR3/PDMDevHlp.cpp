...
static DECLCALLBACK(VPOXSTRICTRC) pdmR3DevHlp_TimerLockClock(PPDMDEVINS pDevIns, TMTIMERHANDLE hTimer, int rcBusy)
{
...
static DECLCALLBACK(VPOXSTRICTRC) pdmR3DevHlp_TimerLockClock2(PPDMDEVINS pDevIns, TMTIMERHANDLE hTimer,
                                                              PPDMCRITSECT pCritSect, int rcBusy)
{
    VPOXSTRICTRC rc = TMTimerLock(pdmR3DevHlp_TimerToPtr(pDevIns, hTimer), rcBusy);
    if (rc == VINF_SUCCESS)
    {
...
static DECLCALLBACK(int) pdmR3DevHlp_PhysRead(PPDMDEVINS pDevIns, RTGCPHYS GCPhys, void *pvBuf, size_t cbRead)
{
...
#if defined(VPOX_STRICT) && defined(PDM_DEVHLP_DEADLOCK_DETECTION)
    if (!VM_IS_EMT(pVM))
    {
        char szNames[128];
    ...
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", VPOXSTRICTRC_VAL(rcStrict))); /** @todo track down the users for this bugger. */

    Log(("pdmR3DevHlp_PhysRead: caller='%s'/%d: returns %Rrc\n", pDevIns->pReg->szName, pDevIns->iInstance, VPOXSTRICTRC_VAL(rcStrict) ));
                                                 ...
                                                 uint8_t uPciDevNo, uint8_t uPciFunNo, const char *pszName)
{
    ...
    if (pBus->pDevInsR3)
    {
        ...
        pPciDev->Int.s.fReassignableDevNo = uPciDevNoRaw >= VPOX_PCI_MAX_DEVICES;
        pPciDev->Int.s.fReassignableFunNo = uPciFunNo >= VPOX_PCI_MAX_FUNCTIONS;
                                                         ...
                                                         uint64_t hHandle, PFNPCIIOREGIONMAP pfnMapUnmap)
{
    ...
    if (iRegion >= VPOX_PCI_NUM_REGIONS)
    {
        Assert(iRegion < VPOX_PCI_NUM_REGIONS);
...
pdmR3DevHlp_PCIConfigWrite(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev, uint32_t uAddress, unsigned cb, uint32_t u32Value)
{
     ...
     * Do the job.
     */
    VPOXSTRICTRC rcStrict = pBus->pfnConfigWrite(pBus->pDevInsR3, pPciDev, uAddress, cb, u32Value);
...
pdmR3DevHlp_PCIConfigRead(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev, uint32_t uAddress, unsigned cb, uint32_t *pu32Value)
{
     ...
     * Do the job.
     */
    VPOXSTRICTRC rcStrict = pBus->pfnConfigRead(pBus->pDevInsR3, pPciDev, uAddress, cb, pu32Value);

    LogFlow(("pdmR3DevHlp_PCIConfigRead: caller='%s'/%d: returns %Rrc (*pu32Value=%#x)\n",
             pDevIns->pReg->szName, pDevIns->iInstance, VPOXSTRICTRC_VAL(rcStrict), *pu32Value));
    return rcStrict;
}
...
static DECLCALLBACK(void) pdmR3DevHlp_PCISetIrq(PPDMDEVINS pDevIns, PPDMPCIDEV pPciDev, int iIrq, int iLevel)
{
    ...
    if (iLevel & PDM_IRQ_LEVEL_HIGH)
    {
        ...
        if (iLevel == PDM_IRQ_LEVEL_HIGH)
            VPOXVMM_PDM_IRQ_HIGH(VMMGetCpu(pVM), RT_LOWORD(uTagSrc), RT_HIWORD(uTagSrc));
        else
            VPOXVMM_PDM_IRQ_HILO(VMMGetCpu(pVM), RT_LOWORD(uTagSrc), RT_HIWORD(uTagSrc));
    }
    else
    ...
    if (iLevel == PDM_IRQ_LEVEL_LOW)
        VPOXVMM_PDM_IRQ_LOW(VMMGetCpu(pVM), RT_LOWORD(uTagSrc), RT_HIWORD(uTagSrc));
    pdmUnlock(pVM);
 ...
 *
 * @returns VPox status code fit for scheduling.
