/* $Id: VPoxGuest.cpp $ */
/** @file
 * VPoxGuest - Guest Additions Driver, Common Code.
 */
...
/** @page pg_vbdrv VPoxGuest
 *
 * VPoxGuest is the device driver for VMMDev.
 *
 * The device driver is shipped as part of the guest additions.  It has roots in
 * the host VMM support driver (usually known as VPoxDrv), so fixes in platform
 * specific code may apply to both drivers.
 *
 * The common code lives in VPoxGuest.cpp and is compiled both as C++ and C.
 * The VPoxGuest.cpp source file shall not contain platform specific code,
 ...
 * restrict set of functions.  These are generally referred to as 'vpoxguest'
 * and 'vpoxuser' respectively.  Currently, this two device approach is only
...
#include <VPox/VMMDev.h> /* for VMMDEV_RAM_SIZE */
#include <VPox/err.h>
#include <VPox/log.h>
#include <VPox/HostServices/GuestPropertySvc.h>
...
*********************************************************************************************************************************/
#define VPOXGUEST_ACQUIRE_STYLE_EVENTS (VMMDEV_EVENT_DISPLAY_CHANGE_REQUEST | VMMDEV_EVENT_SEAMLESS_MODE_CHANGE_REQUEST)
...
#endif
static int      vgdrvIoCtl_CancelAllWaitEvents(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession);
static void     vgdrvBitUsageTrackerClear(PVPOXGUESTBITUSAGETRACER pTracker);
static uint32_t vgdrvGetAllowedEventMaskForSession(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession);
static int      vgdrvResetEventFilterOnHost(PVPOXGUESTDEVEXT pDevExt, uint32_t fFixedEvents);
static int      vgdrvResetMouseStatusOnHost(PVPOXGUESTDEVEXT pDevExt);
static int      vgdrvResetCapabilitiesOnHost(PVPOXGUESTDEVEXT pDevExt);
static int      vgdrvSetSessionEventFilter(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession,
                                           uint32_t fOrMask, uint32_t fNotMask, bool fSessionTermination);
static int      vgdrvSetSessionMouseStatus(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession,
                                           uint32_t fOrMask, uint32_t fNotMask, bool fSessionTermination);
static int      vgdrvSetSessionCapabilities(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession,
                                            uint32_t fOrMask, uint32_t fNoMask,
                                            uint32_t *pfSessionCaps, uint32_t *pfGlobalCaps, bool fSessionTermination);
static int      vgdrvAcquireSessionCapabilities(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession,
                                                uint32_t fOrMask, uint32_t fNotMask, uint32_t fFlags, bool fSessionTermination);
static int      vgdrvDispatchEventsLocked(PVPOXGUESTDEVEXT pDevExt, uint32_t fEvents);
 ...
 */
static int vgdrvInitFixateGuestMappings(PVPOXGUESTDEVEXT pDevExt)
{
    /*
    ...
    else
    {
        ...
        for (iTry = 0; iTry < RT_ELEMENTS(ahTries); iTry++)
        {
            ...
            if (rc == VERR_NOT_SUPPORTED)
            {
                ...
                {
                    rc = RTR0MemObjEnterPhys(&hObj, VPOXGUEST_HYPERVISOR_PHYSICAL_START, cbHypervisor + _4M, RTMEM_CACHE_POLICY_DONT_CARE);
                    if (RT_FAILURE(rc))
                        break;
                    hFictive = hObj;
                }
            ...
            if (RT_FAILURE(rc))
            {
                LogRel(("VPoxGuest: Failed to reserve memory for the hypervisor: rc=%Rrc (cbHypervisor=%#x uAlignment=%#x iTry=%u)\n",
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                Log(("VPoxGuest: %p LB %#x; uAlignment=%#x iTry=%u hGuestMappings=%p (%s)\n",
        ...
        if (RT_FAILURE(rc) && !fBitched)
            LogRel(("VPoxGuest: Warning: failed to reserve %#d of memory for guest mappings.\n", cbHypervisor));
    }
    VbglR0GRFree(&pReq->header);
 ...
 * @param   pDevExt     The device extension.
 */
static void vgdrvTermUnfixGuestMappings(PVPOXGUESTDEVEXT pDevExt)
{
 ...
 */
static int vgdrvReportGuestInfo(VPOXOSTYPE enmOSType)
{
    /*
    ...
    if (RT_SUCCESS(rc))
    {
        pReqInfo2->guestInfo.additionsMajor    = VPOX_VERSION_MAJOR;
        pReqInfo2->guestInfo.additionsMinor    = VPOX_VERSION_MINOR;
        pReqInfo2->guestInfo.additionsBuild    = VPOX_VERSION_BUILD;
        pReqInfo2->guestInfo.additionsRevision = VPOX_SVN_REV;
        pReqInfo2->guestInfo.additionsFeatures = VPOXGSTINFO2_F_REQUESTOR_INFO;
        RTStrCopy(pReqInfo2->guestInfo.szName, sizeof(pReqInfo2->guestInfo.szName), VPOX_VERSION_STRING);
...
{
    /*
     * Report guest status of the VPox driver to the host.
     */
    ...
    if (RT_SUCCESS(rc))
    {
        pReq2->guestStatus.facility = VPoxGuestFacilityType_VPoxGuestDriver;
        pReq2->guestStatus.status = fActive ?
                                    VPoxGuestFacilityStatus_Active
                                  : VPoxGuestFacilityStatus_Inactive;
 ...
 *                          (VINF_SUCCESS if set).
 */
static int vgdrvSetBalloonSizeKernel(PVPOXGUESTDEVEXT pDevExt, uint32_t cBalloonChunks, bool *pfHandleInR3)
{
    int rc = VINF_SUCCESS;
    ...
    if (pDevExt->MemBalloon.fUseKernelAPI)
    {
            ...
            for (i = pDevExt->MemBalloon.cChunks; i < cBalloonChunks; i++)
            {
                ...
                if (RT_FAILURE(rc))
                {
                    if (rc == VERR_NOT_SUPPORTED)
                    {
                        ...
                        Log(("VPoxGuestSetBalloonSizeKernel: PhysNC allocs not supported, falling back to R3 allocs.\n"));
                    }
                ...
                if (RT_FAILURE(rc))
                {
                    Log(("vpoxGuestSetBalloonSize(inflate): failed, rc=%Rrc!\n", rc));
        ...
        else
        {
            ...
            for (i = pDevExt->MemBalloon.cChunks; i-- > cBalloonChunks;)
            {
                ...
                if (RT_FAILURE(rc))
                {
                    Log(("vpoxGuestSetBalloonSize(deflate): failed, rc=%Rrc!\n", rc));
                    break;
                }
                pDevExt->MemBalloon.cChunks--;
            }
 ...
 */
static int vgdrvSetBalloonSizeFromUser(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, RTR3PTR pvChunk, bool fInflate)
{
    VMMDevChangeMemBalloon *pReq;
 ...
 */
static void vgdrvCloseMemBalloon(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession)
{
 ...
 * @returns VPox status code.
 */
static int vgdrvHeartbeatSend(PVPOXGUESTDEVEXT pDevExt)
{
    int rc;
...
{
    PVPOXGUESTDEVEXT pDevExt = (PVPOXGUESTDEVEXT)pvUser;
    int rc;
 ...
 */
static int vgdrvHeartbeatHostConfigure(PVPOXGUESTDEVEXT pDevExt, bool fEnabled)
{
 ...
 *
 * @returns VPox status (ignored).
 * @param   pDevExt             The device extension.
 */
static int vgdrvHeartbeatInit(PVPOXGUESTDEVEXT pDevExt)
{
    /*
...
{
#ifdef VPOX_GUESTDRV_WITH_RELEASE_LOGGER
    /*
     * Create the release log.
     */
    static const char * const s_apszGroups[] = VPOX_LOGGROUP_NAMES;
    PRTLOGGER pRelLogger;
    int rc = RTLogCreate(&pRelLogger, 0 /*fFlags*/, "all", "VPOXGUEST_RELEASE_LOG", RT_ELEMENTS(s_apszGroups), s_apszGroups,
...
{
    int rc;
    AssertMsg(   pDevExt->uInitState != VPOXGUESTDEVEXT_INIT_STATE_FUNDAMENT
              && pDevExt->uInitState != VPOXGUESTDEVEXT_INIT_STATE_RESOURCES, ("uInitState=%#x\n", pDevExt->uInitState));
     ...
     */
    rc = RTSpinlockCreate(&pDevExt->EventSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "VPoxGuestEvent");
    if (RT_SUCCESS(rc))
    {
        rc = RTSpinlockCreate(&pDevExt->SessionSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "VPoxGuestSession");
        if (RT_SUCCESS(rc))
        {
            rc = RTSemFastMutexCreate(&pDevExt->MemBalloon.hMtx);
            if (RT_SUCCESS(rc))
            {
                pDevExt->uInitState = VPOXGUESTDEVEXT_INIT_STATE_FUNDAMENT;
                return VINF_SUCCESS;
            }
...
{
    int rc2;
    AssertMsgReturnVoid(pDevExt->uInitState == VPOXGUESTDEVEXT_INIT_STATE_FUNDAMENT, ("uInitState=%#x\n", pDevExt->uInitState));
    pDevExt->uInitState = VPOXGUESTDEVEXT_INIT_STATE_DELETED;
...
/**
 * Initializes the VPoxGuest device extension resource parts.
 *
...
{
    int rc;
    AssertMsgReturn(pDevExt->uInitState == VPOXGUESTDEVEXT_INIT_STATE_FUNDAMENT, ("uInitState=%#x\n", pDevExt->uInitState),
                    VERR_INVALID_STATE);
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            rc = vgdrvReportDriverStatus(true /* Driver is active */);
                            if (RT_FAILURE(rc))
                                LogRel(("VGDrvCommonInitDevExtResources: VPoxReportGuestDriverStatus failed, rc=%Rrc\n", rc));

                            pDevExt->uInitState = VPOXGUESTDEVEXT_INIT_STATE_RESOURCES;
...
static void vgdrvDeleteWaitList(PRTLISTNODE pList)
{
    ...
    {
        int             rc2;
        PVPOXGUESTWAIT  pWait = RTListGetFirst(pList, VPOXGUESTWAIT, ListNode);
...
void VGDrvCommonDeleteDevExtResources(PVPOXGUESTDEVEXT pDevExt)
{
    ...
    AssertMsgReturnVoid(pDevExt->uInitState == VPOXGUESTDEVEXT_INIT_STATE_RESOURCES, ("uInitState=%#x\n", pDevExt->uInitState));
    pDevExt->uInitState = VPOXGUESTDEVEXT_INIT_STATE_FUNDAMENT;
    ...
    vgdrvCloseMemBalloon(pDevExt, (PVPOXGUESTSESSION)NULL);
...
/**
 * Initializes the VPoxGuest device extension when the device driver is loaded.
 *
 * The native code locates the VMMDev on the PCI bus and retrieve the MMIO and
 * I/O port ranges, this function will take care of mapping the MMIO memory (if
 * present). Upon successful return the native code should set up the interrupt
 ...
 *                          will ever be allowed to mask.
 */
int VGDrvCommonInitDevExt(PVPOXGUESTDEVEXT pDevExt, uint16_t IOPortBase,
                          void *pvMMIOBase, uint32_t cbMMIO, VPOXOSTYPE enmOSType, uint32_t fFixedEvents)
{
    int rc;
...
void VGDrvCommonProcessOption(PVPOXGUESTDEVEXT pDevExt, const char *pszName, const char *pszValue)
{
             ...
             || RTStrNICmpAscii(pszName, RT_STR_TUPLE("dbg_log")) == 0)
    {
            ...
            || RTStrICmpAscii(pszSubName, "_dest") == 0)
        {
            ...
            LogRel(("VPoxGuest: Ignoring unknown option '%s' (value '%s')\n", pszName, pszValue));
    }
    else if (!VGDrvNativeProcessOption(pDevExt, pszName, pszValue))
        LogRel(("VPoxGuest: Ignoring unknown option '%s' (value '%s')\n", pszName, pszValue));
}
...
void VGDrvCommonProcessOptionsFromHost(PVPOXGUESTDEVEXT pDevExt)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        RTStrCopy(uBuf.Connect.u.In.Loc.u.host.achName, sizeof(uBuf.Connect.u.In.Loc.u.host.achName),
                  "VPoxGuestPropSvc"); /** @todo Add a define to the header for the name. */
        rc = VGDrvCommonIoCtl(VBGL_IOCTL_HGCM_CONNECT, pDevExt, pSession, &uBuf.Connect.Hdr, sizeof(uBuf.Connect));
        if (RT_SUCCESS(rc))
        {
            static const char   g_szzPattern[] = "/VirtualPox/GuestAdd/VPoxGuest/*\0";
                        ...
                        && *apszFields[0] != '\0')
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            if (fFlags & GUEST_PROP_F_RDONLYGUEST)
                            {
                                ...
                                LogRel(("VPoxGuest: Ignoring '%s' as it does not have RDONLYGUEST set\n", apszFields[0]));
                        }
                        else
                            LogRel(("VPoxGuest: Invalid flags '%s' for '%s': %Rrc\n", apszFields[2], apszFields[0], rc));
                    }
                    else if (off < cbStrings)
                    {
                        LogRel(("VPoxGuest: Malformed guest properties enum result!\n"));
                        Log(("VPoxGuest: off=%#x cbStrings=%#x\n%.*Rhxd\n", off, cbStrings, cbStrings, pszzStrings));
                        break;
                    }
                    else if (!fValidFields)
                        LogRel(("VPoxGuest: Ignoring %.*Rhxs as it has invalid characters in one or more fields\n",
...
/**
 * Destroys the VPoxGuest device extension.
 *
...
void VGDrvCommonDeleteDevExt(PVPOXGUESTDEVEXT pDevExt)
{
    ...
    Log(("VPoxGuest: The additions driver is terminating.\n"));
...
/**
 * Creates a VPoxGuest user session.
 *
...
{
    PVPOXGUESTSESSION pSession = (PVPOXGUESTSESSION)RTMemAllocZ(sizeof(*pSession));
...
/**
 * Creates a VPoxGuest kernel session.
 *
 ...
 */
static PVPOXGUESTWAIT vgdrvWaitAlloc(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession)
{
    /*
     * Allocate it one way or the other.
     */
    PVPOXGUESTWAIT pWait = RTListGetFirst(&pDevExt->FreeList, VPOXGUESTWAIT, ListNode);
    if (pWait)
    {
        RTSpinlockAcquire(pDevExt->EventSpinlock);

        pWait = RTListGetFirst(&pDevExt->FreeList, VPOXGUESTWAIT, ListNode);
        if (pWait)
            RTListNodeRemove(&pWait->ListNode);
    ...
    if (!pWait)
    {
        ...
        pWait = (PVPOXGUESTWAIT)RTMemAlloc(sizeof(*pWait));
        if (!pWait)
        {
 ...
 */
static void vgdrvWaitFreeLocked(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTWAIT pWait)
{
    pWait->fReqEvents = 0;
    pWait->fResEvents = 0;
#ifdef VPOX_WITH_HGCM
 ...
 * @param   pWait           The wait-for-event entry to free.
 */
static void vgdrvWaitFreeUnlocked(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTWAIT pWait)
{
    RTSpinlockAcquire(pDevExt->EventSpinlock);
...
void VGDrvCommonWaitDoWakeUps(PVPOXGUESTDEVEXT pDevExt)
{
    if (!RTListIsEmpty(&pDevExt->WakeUpList))
    {
        ...
        {
            int            rc;
            PVPOXGUESTWAIT pWait = RTListGetFirst(&pDevExt->WakeUpList, VPOXGUESTWAIT, ListNode);
            if (!pWait)
                break;
...
#endif /* VPOXGUEST_USE_DEFERRED_WAKE_UP */
 ...
 */
static int vgdrvIoCtl_DriverVersionInfo(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCDRIVERVERSIONINFO pReq)
{
    int rc;
    ...
    pReq->u.Out.uDriverRevision = VPOX_SVN_REV;
 ...
 * @param   pReq            The request info.
 */
static int vgdrvIoCtl_IdcConnect(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCIDCCONNECT pReq)
{
    int rc;
 ...
 * @param   pReq            The request info.
 */
static int vgdrvIoCtl_IdcDisconnect(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCIDCDISCONNECT pReq)
{
 ...
 */
static int vgdrvIoCtl_GetVMMDevIoInfo(PVPOXGUESTDEVEXT pDevExt, PVBGLIOCGETVMMDEVIOINFO pInfo)
{
 ...
 */
static int vgdrvIoCtl_SetMouseNotifyCallback(PVPOXGUESTDEVEXT pDevExt, PVBGLIOCSETMOUSENOTIFYCALLBACK pNotify)
{
    LogFlow(("VPOXGUEST_IOCTL_SET_MOUSE_NOTIFY_CALLBACK: pfnNotify=%p pvUser=%p\n", pNotify->u.In.pfnNotify, pNotify->u.In.pvUser));
 ...
 */
DECLINLINE(int) vbdgCheckWaitEventCondition(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession,
                                            PVBGLIOCWAITFOREVENTS pInfo, int iEvent, const uint32_t fReqEvents)
{
    uint32_t fMatches = pDevExt->f32PendingEvents & fReqEvents;
    if (fMatches & VPOXGUEST_ACQUIRE_STYLE_EVENTS)
    ...
    if (fMatches || pSession->fPendingCancelWaitEvents)
    {
        ...
        if (fReqEvents & ~((uint32_t)1 << iEvent))
            LogFlow(("VPOXGUEST_IOCTL_WAITEVENT: returns %#x\n", pInfo->u.Out.fEvents));
        else
            LogFlow(("VPOXGUEST_IOCTL_WAITEVENT: returns %#x/%d\n", pInfo->u.Out.fEvents, iEvent));
...
static int vgdrvIoCtl_WaitForEvents(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession,
                                    PVBGLIOCWAITFOREVENTS pInfo, bool fInterruptible)
{
    ...
    {
        LogRel(("VPOXGUEST_IOCTL_WAITEVENT: Invalid input mask %#x!!\n", fReqEvents));
        return VERR_INVALID_PARAMETER;
    }
    ...
    if (!cMsTimeout)
    {
        LogFlow(("VPOXGUEST_IOCTL_WAITEVENT: returns VERR_TIMEOUT\n"));
        return VERR_TIMEOUT;
    }
    ...
    {
        rc = VERR_INTERRUPTED;
        LogFlow(("VPOXGUEST_IOCTL_WAITEVENT: returns VERR_INTERRUPTED\n"));
    }
    else if (rc == VERR_TIMEOUT)
        LogFlow(("VPOXGUEST_IOCTL_WAITEVENT: returns VERR_TIMEOUT (2)\n"));
    else
    {
        if (RT_SUCCESS(rc))
        {
            LogRelMax(32, ("VPOXGUEST_IOCTL_WAITEVENT: returns %Rrc but no events!\n", rc));
            rc = VERR_INTERNAL_ERROR;
        }
        LogFlow(("VPOXGUEST_IOCTL_WAITEVENT: returns %Rrc\n", rc));
    }
 ...
 *  VPOXGUEST_IOCTL_WAITEVENT are allowed in a session after it has been
 *  called.  Change the code to make calls to VPOXGUEST_IOCTL_WAITEVENT made
 *  after that to return VERR_INTERRUPTED or something appropriate. */
static int vgdrvIoCtl_CancelAllWaitEvents(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession)
{
    PVPOXGUESTWAIT          pWait;
    PVPOXGUESTWAIT          pSafe;
    int                     rc = 0;
    ...
    LogFlow(("VPOXGUEST_IOCTL_CANCEL_ALL_WAITEVENTS\n"));

    /*
     * Walk the event list and wake up anyone with a matching session.
     */
    RTSpinlockAcquire(pDevExt->EventSpinlock);
    RTListForEachSafe(&pDevExt->WaitList, pWait, pSafe, VPOXGUESTWAIT, ListNode)
    {
 ...
 */
static int vgdrvCheckIfVmmReqIsAllowed(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, VMMDevRequestType enmType,
                                       VMMDevRequestHeader const *pReqHdr)
{
    ...
    enum
    {
        kLevel_Invalid, kLevel_NoOne, kLevel_OnlyVPoxGuest, kLevel_OnlyKernel, kLevel_TrustedUsers, kLevel_AllUsers
    } enmRequired;
    RT_NOREF1(pDevExt);
...
# endif
        ...
        case VMMDevReq_ChangeMemBalloon:
            enmRequired = kLevel_OnlyVPoxGuest;
            break;
        ...
        case VMMDevReq_ReportGuestCapabilities:
            ...
            {
                case VPoxGuestFacilityType_All:
                case VPoxGuestFacilityType_VPoxGuestDriver:
                    enmRequired = kLevel_OnlyVPoxGuest;
                    break;
                case VPoxGuestFacilityType_VPoxService:
                    enmRequired = kLevel_TrustedUsers;
                    break;
                case VPoxGuestFacilityType_VPoxTrayClient:
                case VPoxGuestFacilityType_Seamless:
                case VPoxGuestFacilityType_Graphics:
                default:
                    enmRequired = kLevel_AllUsers;
                    break;
            }
...
static int vgdrvIoCtl_VMMDevRequest(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession,
                                    VMMDevRequestHeader *pReqHdr, size_t cbData)
{
    ...
    LogFlow(("VPOXGUEST_IOCTL_VMMREQUEST: type %d\n", pReqHdr->requestType));

    if (cbReq < cbMinSize)
    {
        LogRel(("VPOXGUEST_IOCTL_VMMREQUEST: invalid hdr size %#x, expected >= %#x; type=%#x!!\n",
                cbReq, cbMinSize, enmType));
        return VERR_INVALID_PARAMETER;
    }
    if (cbReq > cbData)
    {
        LogRel(("VPOXGUEST_IOCTL_VMMREQUEST: invalid size %#x, expected >= %#x (hdr); type=%#x!!\n",
    ...
    if (RT_FAILURE(rc))
    {
        Log(("VPOXGUEST_IOCTL_VMMREQUEST: invalid header: size %#x, expected >= %#x (hdr); type=%#x; rc=%Rrc!!\n",
    ...
    if (RT_FAILURE(rc))
    {
        Log(("VPOXGUEST_IOCTL_VMMREQUEST: Operation not allowed! type=%#x rc=%Rrc\n", enmType, rc));
        return rc;
    }

    /*
     * Make a copy of the request in the physical memory heap so
     * the VPoxGuestLibrary can more easily deal with the request.
    ...
    if (RT_FAILURE(rc))
    {
        Log(("VPOXGUEST_IOCTL_VMMREQUEST: failed to allocate %u (%#x) bytes to cache the request. rc=%Rrc!!\n",
             cbReq, cbReq, rc));
        return rc;
    }
        ...
        && RT_SUCCESS(pReqCopy->rc))
    {
        ...
        Log(("VPOXGUEST_IOCTL_VMMREQUEST: VbglR0GRPerform - rc=%Rrc!\n", rc));
    else
    {
        Log(("VPOXGUEST_IOCTL_VMMREQUEST: request execution failed; VMMDev rc=%Rrc!\n", pReqCopy->rc));
        rc = pReqCopy->rc;
    }
...
static int vgdrvHgcmAsyncWaitCallbackWorker(VMMDevHGCMRequestHeader volatile *pHdr, PVPOXGUESTDEVEXT pDevExt,
                                            bool fInterruptible, uint32_t cMillies)
{
    ...
    {
        RTSpinlockAcquire(pDevExt->EventSpinlock);
        if ((pHdr->fu32Flags & VPOX_HGCM_REQ_DONE) != 0)
        {
            RTSpinlockRelease(pDevExt->EventSpinlock);
            return VINF_SUCCESS;
        }
...
static int vgdrvIoCtl_HGCMConnect(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCHGCMCONNECT pInfo)
{
    int rc;
     ...
     */
    Log(("VPOXGUEST_IOCTL_HGCM_CONNECT: %.128s\n",
    ...
    rc = VbglR0HGCMInternalConnect(&pInfo->u.In.Loc, pSession->fRequestor, &idClient,
                                   vgdrvHgcmAsyncWaitCallback, pDevExt, RT_INDEFINITE_WAIT);
    Log(("VPOXGUEST_IOCTL_HGCM_CONNECT: idClient=%RX32 (rc=%Rrc)\n", idClient, rc));
    if (RT_SUCCESS(rc))
    {
        /*
        ...
        {
            LogRelMax(32, ("VPOXGUEST_IOCTL_HGCM_CONNECT: too many HGCMConnect calls for one session!\n"));
...
static int vgdrvIoCtl_HGCMDisconnect(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCHGCMDISCONNECT pInfo)
{
    /*
    ...
    if (i >= RT_ELEMENTS(pSession->aHGCMClientIds))
    {
        LogRelMax(32, ("VPOXGUEST_IOCTL_HGCM_DISCONNECT: idClient=%RX32\n", idClient));
...
static int vgdrvIoCtl_HGCMCallInner(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCHGCMCALL pInfo,
                                    ...
                                    size_t cbExtra, size_t cbData)
{
    ...
    {
        LogRel(("VPOXGUEST_IOCTL_HGCM_CALL: cParm=%RX32 is not sane\n", pInfo->cParms));
        return VERR_INVALID_PARAMETER;
    }
    ...
    {
        LogRel(("VPOXGUEST_IOCTL_HGCM_CALL: cbData=%#zx (%zu) required size is %#zx (%zu)\n",
    ...
    {
        LogRelMax(32, ("VPOXGUEST_IOCTL_HGCM_CALL: Invalid handle. u32Client=%RX32\n", u32ClientId));
        return VERR_INVALID_HANDLE;
    }
     ...
     */
    LogFlow(("VPOXGUEST_IOCTL_HGCM_CALL: u32Client=%RX32\n", pInfo->u32ClientID));
    ...
    {
        rc = pInfo->Hdr.rc;
        LogFlow(("VPOXGUEST_IOCTL_HGCM_CALL: result=%Rrc\n", rc));
    }
    else
    {
        if (   rc != VERR_INTERRUPTED
            && rc != VERR_TIMEOUT)
            LogRelMax(32, ("VPOXGUEST_IOCTL_HGCM_CALL: %s Failed. rc=%Rrc (Hdr.rc=%Rrc).\n", f32bit ? "32" : "64", rc, pInfo->Hdr.rc));
        else
            Log(("VPOXGUEST_IOCTL_HGCM_CALL: %s Failed. rc=%Rrc (Hdr.rc=%Rrc).\n", f32bit ? "32" : "64", rc, pInfo->Hdr.rc));
    }
    return rc;
}


static int vgdrvIoCtl_HGCMCallWrapper(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCHGCMCALL pInfo,
 ...
 *
 * @returns VPox status code of the request submission part.
 * @param   pDevExt     The device extension.
 * @param   pCallReq    The call request.
 */
static int vgdrvIoCtl_HGCMFastCall(PVPOXGUESTDEVEXT pDevExt, VBGLIOCIDCHGCMFASTCALL volatile *pCallReq)
{
    ...
    if (rc == VINF_HGCM_ASYNC_EXECUTE)
    {
        ...
        if (pHgcmCall->header.fu32Flags & VPOX_HGCM_REQ_DONE)
        {
            Assert(!(pHgcmCall->header.fu32Flags & VPOX_HGCM_REQ_CANCELLED));
            rc = VINF_SUCCESS;
        }
        else
        {
        ...
        Assert((pHgcmCall->header.fu32Flags & VPOX_HGCM_REQ_DONE) || RT_FAILURE_NP(rc));
 ...
 */
static int vgdrvIoCtl_CheckMemoryBalloon(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCCHECKBALLOON pInfo)
{
 ...
 */
static int vgdrvIoCtl_ChangeMemoryBalloon(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCCHANGEBALLOON pInfo)
{
    int rc;
 ...
 * @param   pInfo               The output buffer.
 */
static int vgdrvIoCtl_WriteCoreDump(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCWRITECOREDUMP pInfo)
{
    VMMDevReqWriteCoreDump *pReq = NULL;
    int rc;
    LogFlow(("VPOXGUEST_IOCTL_WRITE_CORE_DUMP\n"));
    RT_NOREF1(pDevExt);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = VbglR0GRPerform(&pReq->header);
        if (RT_FAILURE(rc))
            Log(("VPOXGUEST_IOCTL_WRITE_CORE_DUMP: VbglR0GRPerform failed, rc=%Rrc!\n", rc));

        VbglR0GRFree(&pReq->header);
    }
    else
        Log(("VPOXGUEST_IOCTL_WRITE_CORE_DUMP: failed to allocate %u (%#x) bytes to cache the request. rc=%Rrc!!\n",
 ...
 * @param   fUserSession        Copy of VPOXGUESTSESSION::fUserSession for the
 *                              call.  True normal user, false root user.
 */
static int vgdrvIoCtl_Log(PVPOXGUESTDEVEXT pDevExt, const char *pch, size_t cbData, bool fUserSession)
{
 ...
 */
static void vgdrvBitUsageTrackerCheckMask(PCVPOXGUESTBITUSAGETRACER pTracker, uint32_t cMax, const char *pszWhat)
{
    uint32_t fMask = 0;
    uint32_t iBit;
    AssertCompile(sizeof(pTracker->acPerBitUsage) == 32 * sizeof(uint32_t));
 ...
 * @param   pszWhat             Identifies the tracker in assertions.
 */
static bool vgdrvBitUsageTrackerChange(PVPOXGUESTBITUSAGETRACER pTracker, uint32_t fChanged, uint32_t fPrevious,
 ...
 */
static int vgdrvIoCtl_ChangeFilterMask(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCCHANGEFILTERMASK pInfo)
{
 ...
 *
 * This is called in response to VPOXGUEST_IOCTL_SET_MOUSE_STATUS as well as to
 * do session cleanup.
 *
 * @returns VPox status code.
 * @param   pDevExt             The device extension.
 ...
 */
static int vgdrvIoCtl_SetMouseStatus(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, uint32_t fFeatures)
{
 ...
 *          VPOXGUEST_ACQUIRE_STYLE_EVENTS mask.
 */
static uint32_t vgdrvGetAllowedEventMaskForSession(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession)
{
     ...
     *       WITHOUT holding VPOXGUESTDEVEXT::SessionSpinlock.
     */
 ...
 */
static int vgdrvUpdateCapabilitiesOnHostWithReqAndLock(PVPOXGUESTDEVEXT pDevExt, VMMDevReqGuestCapabilities2 *pReq)
{
    int rc;
 ...
 * the given session.
 *
 * This is called in response to VPOXGUEST_IOCTL_GUEST_CAPS_ACQUIRE as well as
 ...
 *                              VPOXGUESTCAPSACQUIRE_FLAGS_CONFIG_ACQUIRE_MODE.
 * @param   fFlags              Confusing operation modifier.
 *                              VPOXGUESTCAPSACQUIRE_FLAGS_NONE means to both
 ...
 */
static int vgdrvIoCtl_GuestCapsAcquire(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCACQUIREGUESTCAPS pAcquire)
{
    int rc;
...
#ifndef VPOXGUEST_DISREGARD_ACQUIRE_MODE_GUEST_CAPS
    /*
 ...
 * @param   pInfo               The request.
 */
static int vgdrvIoCtl_SetCapabilities(PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLIOCSETGUESTCAPS pInfo)
{
    int rc;
 ...
 */
int VGDrvCommonIoCtl(uintptr_t iFunction, PVPOXGUESTDEVEXT pDevExt, PVPOXGUESTSESSION pSession, PVBGLREQHDR pReqHdr, size_t cbReq)
{
 ...
 * @param   pDevExt             The VPoxGuest device extension.
    ...
    RTListForEachSafe(&pDevExt->WaitList, pWait, pSafe, VPOXGUESTWAIT, ListNode)
    {
            ...
            &&  !pWait->fResEvents)
        {
            /* Does this one wait on any of the events we're dispatching?  We do a quick
               check first, then deal with VPOXGUEST_ACQUIRE_STYLE_EVENTS as applicable. */
            if (fHandledEvents & VPOXGUEST_ACQUIRE_STYLE_EVENTS)
    ...
    if (fOurIrq)
    {
        ...
        if (RT_SUCCESS(rc))
        {
...
#if !defined(VPOXGUEST_MOUSE_NOTIFY_CAN_PREEMPT)
            ...
            if (fEvents & VMMDEV_EVENT_HGCM)
            {
                PVPOXGUESTWAIT pWait;
                PVPOXGUESTWAIT pSafe;
                RTListForEachSafe(&pDevExt->HGCMWaitList, pWait, pSafe, VPOXGUESTWAIT, ListNode)
                {
                    if (pWait->pHGCMReq->fu32Flags & VPOX_HGCM_REQ_DONE)
                    {
...
# endif
     ...
     */
#if defined(VPOXGUEST_MOUSE_NOTIFY_CAN_PREEMPT) && !defined(RT_OS_WINDOWS) /* (Windows does this in the Dpc callback) */
    if (   fMousePositionChanged
        && pDevExt->pfnMouseNotifyCallback)
        pDevExt->pfnMouseNotifyCallback(pDevExt->pvMouseNotifyCallbackArg);
#endif

#if defined(VPOXGUEST_USE_DEFERRED_WAKE_UP) && !defined(RT_OS_WINDOWS)
    /*
     * Do wake-ups.
