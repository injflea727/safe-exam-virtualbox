/* $Id: VPoxDispMouse.cpp $ */
/** @file
 * VPox XPDM Display driver, mouse pointer related functions
 */
...
static BOOL VPoxDispFillMonoShape(PVPOXDISPDEV pDev, SURFOBJ *psoMask)
{
...
static SURFOBJ *VPoxDispConvSurfTo32BPP(PVPOXDISPDEV pDev, SURFOBJ *psoScreen, SURFOBJ *psoSrc, XLATEOBJ *pxlo, HSURF *phDstSurf)
{
    *phDstSurf = NULL;
...
static BOOL VPoxDispFillColorShape(PVPOXDISPDEV pDev, SURFOBJ *psoScreen, SURFOBJ *psoMask, SURFOBJ *psoColor,
                                   XLATEOBJ *pxlo, FLONG fl)
{
    ...
    else
    {
        ...
        pso32bpp = VPoxDispConvSurfTo32BPP(pDev, psoScreen, psoColor, pxlo, &hSurf32bpp);
        if (!pso32bpp)
        {
...
int VPoxDispInitPointerCaps(PVPOXDISPDEV pDev, DEVINFO *pDevInfo)
{
    ...
    rc = VPoxDispMPGetPointerCaps(pDev->hDriver, &pDev->pointer.caps);
...
VOID APIENTRY VPoxDispDrvMovePointer(SURFOBJ *pso, LONG x, LONG y, RECTL *prcl)
{
    ...
    if (-1==x) /* hide pointer */
    {
        rc = VPoxDispMPDisablePointer(pDev->hDriver);
        VPOX_WARNRC(rc);
    }
    else
    {
        ...
        rc = VPoxDispMPSetPointerPosition(pDev->hDriver, &pos);
        VPOX_WARNRC(rc);
    }
...
ULONG APIENTRY
VPoxDispDrvSetPointerShape(SURFOBJ *pso, SURFOBJ *psoMask, SURFOBJ *psoColor, XLATEOBJ *pxlo,
                           LONG xHot, LONG yHot, LONG x, LONG y, RECTL *prcl, FLONG fl)
{
    ...
    if (!psoColor) /* Monochrome pointer */
    {
        if (!(pDev->pointer.caps.Flags & VIDEO_MODE_MONO_POINTER)
            || !VPoxDispFillMonoShape(pDev, psoMask))
        {
            rc = VPoxDispMPDisablePointer(pDev->hDriver);
            VPOX_WARNRC(rc);
            return SPS_DECLINE;
    ...
    {
        if (!(pDev->pointer.caps.Flags & VIDEO_MODE_COLOR_POINTER)
            || !VPoxDispFillColorShape(pDev, pso, psoMask, psoColor, pxlo, fl))
        {
            rc = VPoxDispMPDisablePointer(pDev->hDriver);
            VPOX_WARNRC(rc);
            return SPS_DECLINE;
    ...
    pDev->pointer.pAttrs->Enable = VPOX_MOUSE_POINTER_SHAPE;
    pDev->pointer.pAttrs->Enable |= (yHot & 0xFF) << 24;
    pDev->pointer.pAttrs->Enable |= (xHot & 0xFF) << 16;

    if (x!=-1)
    {
        pDev->pointer.pAttrs->Enable |= VPOX_MOUSE_POINTER_VISIBLE;
    }

    if (fl & SPS_ALPHA)
    {
        pDev->pointer.pAttrs->Enable |= VPOX_MOUSE_POINTER_ALPHA;
    }
    ...
    rc = VPoxDispMPSetPointerAttrs(pDev);
    if (RT_FAILURE(rc))
    {
        VPOX_WARNRC(rc);
        rc = VPoxDispMPDisablePointer(pDev->hDriver);
    ...
    if (x!=-1)
    {
        VPoxDispDrvMovePointer(pso, x, y, NULL);
    }
