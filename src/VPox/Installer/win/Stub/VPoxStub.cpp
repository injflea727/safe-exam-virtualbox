...
#include "VPoxStub.h"
#include "../StubBld/VPoxStubBld.h"
#include "resource.h"

#ifdef VPOX_WITH_CODE_SIGNING
# include "VPoxStubCertUtil.h"
# include "VPoxStubPublicCert.h"
#endif
...
#define VPOX_STUB_WITH_OWN_CONSOLE
...
static RTEXITCODE ShowError(const char *pszFmt, ...)
{
    ...
    if (RTStrAPrintfV(&pszMsg, pszFmt, va))
    {
        ...
        else
        {
            ...
            if (RT_SUCCESS(rc))
            {
                MessageBoxW(GetDesktopWindow(), pwszMsg, MY_UNICODE(VPOX_STUB_TITLE), MB_ICONERROR);
                RTUtf16Free(pwszMsg);
            }
            else
                MessageBoxA(GetDesktopWindow(), pszMsg, VPOX_STUB_TITLE, MB_ICONERROR);
        }
        RTStrFree(pszMsg);
    }
...
static void ShowInfo(const char *pszFmt, ...)
{
    ...
    if (rc >= 0)
    {
        ...
        else
        {
            ...
            if (RT_SUCCESS(rc))
            {
                MessageBoxW(GetDesktopWindow(), pwszMsg, MY_UNICODE(VPOX_STUB_TITLE), MB_ICONINFORMATION);
                RTUtf16Free(pwszMsg);
            }
            else
                MessageBoxA(GetDesktopWindow(), pszMsg, VPOX_STUB_TITLE, MB_ICONINFORMATION);
        }
    }
    else /* Should never happen! */
        AssertMsgFailed(("Failed to format error text of format string: %s!\n", pszFmt));
 ...
 */
static const VPOXSTUBPKG *FindPackageHeader(unsigned iPackage)
{
    char szHeaderName[32];
    RTStrPrintf(szHeaderName, sizeof(szHeaderName), "HDR_%02d", iPackage);

    VPOXSTUBPKG const *pPackage;
 ...
 * @returns iprt status code.
 *
 * @param   pPackage            Pointer to a VPOXSTUBPKG struct that contains the resource.
 * @param   pszTempFile         The full file path + name to extract the resource to.
 * @param   hFile               Handle to pszTempFile if RTFileCreateUnique was
 *                              used to generate the name, otherwise NIL_RTFILE.
 * @param   idxPackage          The package index for annotating the cleanup
 *                              record with (HACK ALERT).
 */
static int Extract(VPOXSTUBPKG const *pPackage, const char *pszTempFile, RTFILE hFile, unsigned idxPackage)
{
 ...
 * @param   pPackage            Pointer to a VPOXSTUBPKG struct that contains the resource.
 */
static bool PackageIsNeeded(VPOXSTUBPKG const *pPackage)
{
    if (pPackage->byArch == VPOXSTUBPKGARCH_ALL)
        return true;
    VPOXSTUBPKGARCH enmArch = IsWow64() ? VPOXSTUBPKGARCH_AMD64 : VPOXSTUBPKGARCH_X86;
...
static RTEXITCODE ProcessPackage(unsigned iPackage, const char *pszMsiArgs, const char *pszMsiLogFile)
{
     ...
     */
    VPOXSTUBPKG const * const pPackage = FindPackageHeader(iPackage);
    if (pPackage == NULL)
        return RTEXITCODE_FAILURE;
...
static RTEXITCODE InstallTimestampCA(bool fForce)
{
    ...
    if (!addCertToStore(CERT_SYSTEM_STORE_LOCAL_MACHINE, "Root", g_abVPoxLegacyWinCA, sizeof(g_abVPoxLegacyWinCA)))
        return ShowError("Failed add the legacy Windows timestamp CA to the root certificate store.");
    return RTEXITCODE_SUCCESS;
}
# endif  /* VPOX_WITH_VPOX_LEGACY_TS_CA*/
...
{
    for (uint32_t i = 0; i < RT_ELEMENTS(g_aVPoxStubTrustedCerts); i++)
    {
        if (!addCertToStore(CERT_SYSTEM_STORE_LOCAL_MACHINE,
                            "TrustedPublisher",
                            g_aVPoxStubTrustedCerts[i].pab,
                            g_aVPoxStubTrustedCerts[i].cb))
            return ShowError("Failed to add our certificate(s) to trusted publisher store.");
    }
    return RTEXITCODE_SUCCESS;
}

#endif /* VPOX_WITH_CODE_SIGNING */
...
static RTEXITCODE ExtractFiles(unsigned cPackages, const char *pszDstDir, bool fExtractOnly, PSTUBCLEANUPREC *ppExtractDirRec)
{
    ...
    {
        VPOXSTUBPKG const * const pPackage = FindPackageHeader(k);
        if (!pPackage)
        ...
        if (fExtractOnly || PackageIsNeeded(pPackage))
        {
            ...
            if (fExtractOnly || pPackage->byArch == VPOXSTUBPKGARCH_ALL)
                   ...
                   int        nCmdShow)
{
    ...
    enum KVPOXSTUBOPT
    {
        KVPOXSTUBOPT_MSI_LOG_FILE = 1000
    };
    ...
    static const RTGETOPTDEF s_aOptions[] =
    {
        ...
        { "-logging",           'l',                         RTGETOPT_REQ_NOTHING },
        { "--msi-log-file",     KVPOXSTUBOPT_MSI_LOG_FILE,   RTGETOPT_REQ_STRING  },
        { "-msilogfile",        KVPOXSTUBOPT_MSI_LOG_FILE,   RTGETOPT_REQ_STRING  },
        { "/logging",           'l',                         RTGETOPT_REQ_NOTHING },
    ...
    while ((ch = RTGetOpt(&GetState, &ValueUnion)) != 0)
    {
        switch (ch)
        {
            ...
            case KVPOXSTUBOPT_MSI_LOG_FILE:
                if (*ValueUnion.psz == '\0')
                    szMSILogFile[0] = '\0';
                else
                {
            ...
            case 'V':
                ShowInfo("Version: %u.%u.%ur%u", VPOX_VERSION_MAJOR, VPOX_VERSION_MINOR, VPOX_VERSION_BUILD, VPOX_SVN_REV);
                return RTEXITCODE_SUCCESS;
            ...
            case 'h':
                ShowInfo("-- %s v%u.%u.%ur%u --\n"
...
#ifdef VPOX_WITH_CODE_SIGNING
                         "--no-silent-cert\n"
                         "    Do not install VirtualPox Certificate automatically\n"
                         ...
                         "    type of Windows VirtualPox is being installed on.\n"
                         ...
                         "--reinstall\n"
                         "    Forces VirtualPox to get re-installed\n"
                         "--ignore-reboot\n"
                         ...
                         "\n"
                         "Examples:\n"
                         "  %s --msiparams \"INSTALLDIR=\"\"C:\\Program Files\\VirtualPox\"\"\"\n"
                         "  %s --extract -path C:\\VPox",
                         VPOX_STUB_TITLE, VPOX_VERSION_MAJOR, VPOX_VERSION_MINOR, VPOX_VERSION_BUILD, VPOX_SVN_REV,
                         argv[0], argv[0]);
                return RTEXITCODE_SUCCESS;
     ...
     */
    SetLastError(0);
    HANDLE hMutexAppRunning = CreateMutex(NULL, FALSE, "VPoxStubInstaller");
 ...
 *   */
     ...
     */
#ifdef VPOX_STUB_WITH_OWN_CONSOLE
    if (g_iVerbosity)
    {
        if (!AllocConsole())
...
#endif /* VPOX_STUB_WITH_OWN_CONSOLE */
        ...
        && szMSILogFile[0] == '\0') /* No log file explicitly specified? Use the extract path by default. */
    {
        ...
        if (RT_SUCCESS(vrc))
            vrc = RTPathAppend(szMSILogFile, sizeof(szMSILogFile), "VPoxInstallLog.txt");
        if (RT_FAILURE(vrc))
    ...
    if (   !fExtractOnly
        && !g_fSilent
        && !IsWow64())
        rcExit = ShowError("32-bit Windows hosts are not supported by this VirtualPox release.");
    else
    {
        /*
         * Read our manifest.
         */
        VPOXSTUBPKGHEADER const *pHeader = NULL;
