...
I.  Running a Test Manager instance with Docker:
  ...
  - There are two ways of doing development with this setup:
    ...
    a. The Test Manager source is stored inside a separate data volume called
       "docker_vpox-testmgr-web". The source will be checked out automatically on
    ...
    b. Edit the (hidden) .env file in this directory and change VPOX_TESTMGR_DATA
       to point to your checked out VPox root, e.g. VPOX_TESTMGR_DATA=/path/to/VPox/trunk
...
II. Steps for manually setting up a local Test Manager instance for development:
  ...
  - Configure apache using the ./apache-template-2.4.conf (see top of
    file for details), for example:
        ...
        Define TestManagerRootDir "/mnt/scratch/vpox/svn/trunk/src/VPox/ValidationKit/testmanager"
        Define VPoxBuildOutputDir "/tmp"
  ...
  - Configure apache:
      - sudo joe /opt/local/etc/apache2/httpd.conf:
        ...
        - At the end of the file add (edit paths):
            Define TestManagerRootDir "/Users/bird/coding/vpox/svn/trunk/src/VPox/ValidationKit/testmanager"
            Define VPoxBuildOutputDir "/tmp"
            Include "${TestManagerRootDir}/apache-template-2.4.conf"
      - Test the config:
      ...
      - Give apache access to read everything under TestManagerRootDir:
          chmod -R a:rX /Users/bird/coding/vpox/svn/trunk/src/VPox/ValidationKit/testmanager
          MYDIR=/Users/bird/coding/vpox/svn/trunk/src/VPox/ValidationKit; while [ '!' "$MYDIR" '<' "$HOME" ]; do \
