/* $Id: ConsoleImpl.h $ */
/** @file
 * VPox Console COM Class definition
 */
...
# include <VPox/HostServices/GuestPropertySvc.h>  /* For the property notification callback */
#endif

#if    defined(VPOX_WITH_GUEST_PROPS) || defined(VPOX_WITH_SHARED_CLIPBOARD) \
    || defined(VPOX_WITH_DRAG_AND_DROP)
...
# endif
    ...
    HRESULT i_onUSBDeviceAttach(IUSBDevice *aDevice, IVirtualPoxErrorInfo *aError, ULONG aMaskedIfs,
                                const Utf8Str &aCaptureFilename);
    HRESULT i_onUSBDeviceDetach(IN_BSTR aId, IVirtualPoxErrorInfo *aError);
...
/* Note: FreeBSD needs this whether netflt is used or not. */
    ...
    void i_configAudioDriver(IVirtualPox *pVirtualPox, IMachine *pMachine, PCFGMNODE pLUN, const char *pszDriverName,
                             bool fAudioEnabledIn, bool fAudioEnabledOut);
    int i_configConstructorInner(PUVM pUVM, PVM pVM, AutoWriteLock *pAlock);
    int i_configCfgmOverlay(PCFGMNODE pRoot, IVirtualPox *pVirtualPox, IMachine *pMachine);
    int i_configDumpAPISettingsTweaks(IVirtualPox *pVirtualPox, IMachine *pMachine);
    ...
    /** Shadow of the VPoxInternal2/TurnResetIntoPowerOff extra data setting.
